...
case class AdminServerArgs(ip: String = "localhost", port: Int = 7071)

case class DashboardArgs(ip: String = "0.0.0.0", port: Int = 9000)
...
object Console extends Logging {
    ...
    val parser =
      new scopt.OptionParser[ConsoleArgs]("pio") {
        ...
        opt[File]("sbt") action { (x, c) =>
          ...
          ("Path to sbt. Default: sbt")
        opt[Unit]("verbose") action { (x, c) =>
          c.copy(common = c.common.copy(verbose = true))
        }
        opt[Unit]("spark-kryo") abbr
          ("sk") action { (x, c) =>
        ...
        cmd("undeploy")
          ...
          (
            ...
            opt[Int]("port") action { (x, c) =>
              ...
              ("Port to unbind from. Default: 8000"))
        note("")
        cmd("dashboard")
            ...
            cmd("delete")
              ...
              (
                ...
                opt[Unit]("force") abbr
                  ...
                  ("Delete an app without prompting for confirmation")),
            note(""),
            cmd("data-delete")
              ...
              (
                arg[String]("<name>") action { (x, c) =>
                  ...
                  ("Name of the app whose data to be deleted."),
                opt[String]("channel") action { (x, c) =>
                  c.copy(app = c.app.copy(dataDeleteChannel = Some(x)))
                } text
                  ("Name of channel whose data to be deleted."),
                opt[Unit]("all") action { (x, c) =>
                  c.copy(app = c.app.copy(all = true))
                } text
                  ("Delete data of all channels including default"),
                opt[Unit]("force") abbr
                  ("f") action { (x, c) =>
                    c.copy(app = c.app.copy(force = true))
                  } text
                  ("Delete data of an app without prompting for confirmation")),
            note(""),
            cmd("channel-new")
          ...
          (
            cmd("new")
              ...
              (
                ...
                arg[String]("[<event1> <event2> ...]") unbounded
                  () optional
                  () action { (x, c) =>
                    c.copy(accessKey = c
                      .accessKey
        ...
        cmd("template").action { (_, c) =>
          ...
          (
            cmd("get").action { (_, c) =>
              ...
              (
                arg[String]("<template ID>") required
                  () action { (x, c) =>
        ...
        cmd("export").action { (_, c) =>
          ...
          (
            opt[Int]("appid") required
              () action { (x, c) =>
                c.copy(export = c.export.copy(appId = x))
              },
            opt[String]("output") required
              () action { (x, c) =>
    ...
    withRegisteredManifest(
      ...
      engineInstance map { r =>
        ...
        ca.engineInstanceId map { eid =>
          error(s"Invalid engine instance ID ${ca.engineInstanceId}. Aborting.")
        } getOrElse {
          error(
  ...
  def dashboard(ca: ConsoleArgs): Unit = {
    ...
    Dashboard.createDashboard(
      DashboardConfig(ip = ca.dashboard.ip, port = ca.dashboard.port))
  }
  ...
  def regenerateManifestJson(json: File): Unit = {
    ...
    if (json.exists) {
      ...
      if (em.description == Some(manifestAutogenTag) && ha != em.version) {
        warn(
          "This engine project directory contains an auto-generated " +
  ...
  def readManifestJson(json: File): EngineManifest = {
    ...
    try {
      ...
      case e: MappingException =>
        error(s"${json.getCanonicalPath} has invalid content: " + e.getMessage)
        sys.exit(1)
    }
  }
  ...
  def withRegisteredManifest(
      ...
      engineVersion: Option[String])(op: EngineManifest => Int): Int = {
  ...
  def jarFilesForScala: Array[File] = {
    ...
    val targetFiles = jarFilesForScalaFilter(
      jarFilesAt(
        new File("target" + File.separator + s"scala-${scalaVersionNoPatch}")))
