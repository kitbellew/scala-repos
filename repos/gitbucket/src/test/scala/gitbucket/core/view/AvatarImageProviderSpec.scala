...
class AvatarImageProviderSpec extends FunSpec with MockitoSugar {
  ...
  class AvatarImageProviderImpl(account: Option[Account])
    ...
    override def getAccountByMailAddress(mailAddress: String)(implicit
        context: Context): Option[Account] = account
    override def getAccountByUserName(userName: String)(implicit
        context: Context): Option[Account] = account
  }
