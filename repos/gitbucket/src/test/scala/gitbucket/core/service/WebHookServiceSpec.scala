...
class WebHookServiceSpec extends FunSuite with ServiceSpecBase {
  ...
  test("WebHookPullRequestService.getPullRequestsByRequestForWebhook") {
    withTestDB { implicit session =>
      ...
      val r = service.getPullRequestsByRequestForWebhook(
        "user2",
        "repo2",
        "master2").mapValues(_.map(_.url).toSet)
      ...
      val r2 = service.getPullRequestsByRequestForWebhook(
        "user2",
        "repo2",
        "master2").mapValues(_.map(_.url).toSet)
      assert(r2.size == 2)
      ...
      assert(
        service.getWebHook("user1", "repo1", "http://example.com") == Some(
          (
            ...
            Set(WebHook.PullRequest))))
      assert(
        service.getWebHooksByEvent(
          "user1",
          "repo1",
          WebHook.PullRequest) == List(
          (WebHook("user1", "repo1", "http://example.com", Some("key")))))
