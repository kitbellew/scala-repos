...
class AccessTokenAuthenticationFilter extends Filter with AccessTokenService {
  ...
  override def doFilter(
    ...
    implicit val session = req.getAttribute(Keys.Request.DBSession)
    ...
    Option(request.getHeader("Authorization")).map {
      case auth if auth.startsWith("token ") =>
        AccessTokenService.getAccountByAccessToken(auth.substring(6).trim)
          .toRight(Unit)
      // TODO Basic Authentication Support
      case _ => Left(Unit)
    }.orElse {
      Option(
        request.getSession.getAttribute(Keys.Session.LoginAccount)
