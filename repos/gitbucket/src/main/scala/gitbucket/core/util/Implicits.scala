...
object Implicits {
  ...
  implicit def request2Session(implicit
      request: HttpServletRequest): JdbcBackend#Session =
    Database.getSession(request)

  implicit def context2ApiJsonFormatContext(implicit
      context: Context): JsonFormat.Context =
  ...
  implicit class RichString(value: String) {
    ...
    def toIntOpt: Option[Int] =
      catching(classOf[NumberFormatException]) opt { Integer.parseInt(value) }
  }
  ...
  implicit class RichRequest(request: HttpServletRequest) {
    ...
    def baseUrl: String = {
      ...
      val len = url.length - (request.getRequestURI.length - request
