  ...
  def getFileList(
    ...
    using(new RevWalk(git.getRepository)) { revWalk =>
        ...
        if (restList.isEmpty) {
          ...
          if (thisTimeChecks.isEmpty) {
            ...
            useTreeWalk(newCommit) { walk =>
              while (walk.next) {
                rest
                  .remove(walk.getNameString -> walk.getObjectId(0))
                  .map {
                    case (tuple, _) =>
      ...
      val nextParentsMap = Option(lastCommit)
        .map(_.getParents.map(_ -> lastCommit).toMap)
        .getOrElse(Map())
      ...
      git
        ...
        .map { ref =>
          ...
          try {
            ...
            val mergeInfo =
              if (origin && branchName == defaultBranch) {
                None
              } else {
                walk.reset()
  ...
  def getBlame(git: Git, id: String, path: String): Iterable[BlameInfo] = {
    Option(git.getRepository.resolve(id))
      .map { commitId =>
        ...
        val commits = 0
          .to(blame.getResultContents().size() - 1)
          .map { i =>
