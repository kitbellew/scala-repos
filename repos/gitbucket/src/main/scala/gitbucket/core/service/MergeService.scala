...
object MergeService {
  object Util {
    ...
    def updateRefs(
        ...
        force: Boolean,
        committer: PersonIdent,
        refLogMessage: Option[String] = None): Unit = {
      // update refs
  ...
  case class MergeCacheInfo(git: Git, branch: String, issueId: Int) {
    ...
    private def parseCommit(id: ObjectId) =
      using(new RevWalk(repository))(_.parseCommit(id))
  }
}
