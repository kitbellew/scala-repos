...
trait ProtectedBranchComponent extends TemplateComponent { self: Profile =>
  ...
  class ProtectedBranches(tag: Tag)
    ...
    def * =
      (userName, repositoryName, branch, statusCheckAdmin) <> (ProtectedBranch.tupled, ProtectedBranch.unapply)
    ...
    def byPrimaryKey(
        ...
        branch: Column[String]) = byBranch(userName, repositoryName, branch)
  }
