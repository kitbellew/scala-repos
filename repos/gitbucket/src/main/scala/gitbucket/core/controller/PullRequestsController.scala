...
trait PullRequestsControllerBase extends ControllerBase {
  ...
  val mergeForm =
    mapping("message" -> trim(label("Message", text(required))))(
      MergeForm.apply)
  ...
  ajaxGet("/:owner/:repository/pull/:id/mergeguide")(
    referrersOnly { repository =>
      params("id")
        .toIntOpt
        .flatMap {
  ...
  get("/:owner/:repository/pull/:id/delete/*")(
            ...
            LockUtil.lock(s"${owner}/${name}") {
              val alias =
                if (pullreq.repositoryName == pullreq
                      .requestRepositoryName && pullreq
                      .userName == pullreq.requestUserName) {
                  pullreq.branch
                } else {
                ...
                case Some(oldId) =>
                  ...
                  using(Git.open(Directory.getRepositoryDir(owner, name))) {
                    git =>
                      ...
                      if (pullreq.requestBranch == repository
                            .repository
                            .defaultBranch) {
                        commits.map { commit =>
  ...
  post("/:owner/:repository/pull/:id/merge", mergeForm)(
  ...
  get("/:owner/:repository/compare")(
    referrersOnly { forkedRepository =>
      ...
      (
        ...
        case (Some(originUserName), Some(originRepositoryName)) => {
          getRepository(originUserName, originRepositoryName).map {
            originRepository =>
              using(
                Git
                  .open(getRepositoryDir(originUserName, originRepositoryName)),
                Git.open(
                  getRepositoryDir(
                ...
                (oldGit, newGit) =>
                  ...
                  val oldBranch = originRepository
                    .branchList
                    .find(_ == newBranch)
                    .getOrElse(
                      JGitUtil
                        .getDefaultBranch(oldGit, originRepository)
                        .get
                        ._2)
        ...
        case _ => {
          using(
            ...
            git =>
              JGitUtil
                .getDefaultBranch(git, forkedRepository)
                .map {
            ...
            using(
              ...
              case (oldGit, newGit) =>
                val (oldId, newId) =
                  if (originRepository
                        .branchList
                        .contains(originId) && forkedRepository
                        .branchList
                        .contains(forkedId)) {
                    // Branch name
                    ...
                    html.compare(
                      ...
                      (
                        getCollaborators(
                          originRepository.owner,
                          originRepository.name) ::: (
                          if (getAccountByUserName(originRepository.owner)
                                .get
                                .isGroupAccount)
                            Nil
                          else
                            List(originRepository.owner)
                        )
                      ).sorted,
                      getMilestones(
                        originRepository.owner,
  ...
  ajaxGet("/:owner/:repository/compare/*...*/mergecheck")(
    collaboratorsOnly { forkedRepository =>
      val Seq(origin, forked) = multiParams("splat")
  ...
  post("/:owner/:repository/pulls/new", pullRequestForm)(
    referrersOnly { (form, repository) =>
      ...
      (oldGit, newGit) =>
        ...
        val commits = newGit
          ...
          .splitWith { (commit1, commit2) =>
            helpers
              .date(commit1.commitTime) == view.helpers.date(commit2.commitTime)
          }
