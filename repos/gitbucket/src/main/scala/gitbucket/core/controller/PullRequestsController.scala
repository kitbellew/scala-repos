...
trait PullRequestsControllerBase extends ControllerBase {
  ...
  get("/:owner/:repository/pulls")(referrersOnly { repository =>
    ...
    if (Option(q).exists(_.contains("is:issue"))) {
      redirect(
        s"/${repository.owner}/${repository.name}/issues?q=" + StringUtil.urlEncode(
          q))
    } else {
                ...
                html.pullreq(
                  ...
                  (commits.flatten.map(commit =>
                    getCommitComments(
                      ...
                      name,
                      commit.id,
                      true)).flatten.toList ::: getComments(
                    owner,
                    name,
                    issueId))
                    .sortWith((a, b) =>
              ...
              case Some(oldId) =>
                ...
                using(Git.open(Directory.getRepositoryDir(owner, name))) {
                  git =>
                    ...
                    val commits = git.log.addRange(
                      oldId,
                      newCommitId).call.iterator.asScala.map(c =>
                      new JGitUtil.CommitInfo(c)).toList
          ...
          originRepository =>
            using(
              ...
              (oldGit, newGit) =>
                ...
                val oldBranch =
                  originRepository.branchList.find(_ == newBranch).getOrElse(
    ...
    (for (originRepositoryName <-
      ...
      yield {
        using(
          ...
          case (oldGit, newGit) =>
            val (oldId, newId) =
              if (originRepository.branchList.contains(
                    originId) && forkedRepository.branchList.contains(
                    forkedId)) {
                // Branch name
            ...
            (oldId, newId) match {
              case (Some(oldId), Some(newId)) => {
                ...
                html.compare(
                  ...
                  (getCollaborators(
                    originRepository.owner,
                    originRepository.name) ::: (if (getAccountByUserName(
    ...
    forkedRepository =>
      ...
      (for (originRepositoryName <-
        ...
        using(
          ...
          case (oldGit, newGit) =>
            val originBranch = JGitUtil.getDefaultBranch(
              oldGit,
              originRepository,
              tmpOriginBranch).get._2
            val forkedBranch = JGitUtil.getDefaultBranch(
              newGit,
              forkedRepository,
              tmpForkedBranch).get._2
      ...
      case (owner, repoName) =>
        ...
        val condition = session.putAndGet(
          ...
          if (request.hasQueryString) IssueSearchCondition(request)
            ...
            session.getAs[IssueSearchCondition](sessionKey).getOrElse(
