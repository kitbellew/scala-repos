...
class SimpleFunctionRegistry extends FunctionRegistry {
  ...
  override def registerFunction(
      ...
      info: ExpressionInfo,
      builder: FunctionBuilder): Unit =
    synchronized { functionBuilders.put(name, (info, builder)) }
  ...
  override def listFunction(): Seq[String] =
    synchronized { functionBuilders.iterator.map(_._1).toList.sorted }
...
object FunctionRegistry {
  ...
  /** See usage above. */
  def expression[T <: Expression](name: String)(implicit
      tag: ClassTag[T]): (String, (ExpressionInfo, FunctionBuilder)) = {
    ...
    val builder = (expressions: Seq[Expression]) => {
      if (varargCtor.isDefined) {
        ...
        Try(
          varargCtor.get.newInstance(expressions)
