...
abstract class TreeNode[BaseType <: TreeNode[BaseType]] extends Product {
  ...
  def mapChildren(f: BaseType => BaseType): BaseType = {
    ...
    val newArgs = productIterator.map {
      case arg: TreeNode[_] if containsChild(arg) =>
        ...
        if (newChild fastEquals arg) { arg }
        else {
          changed = true
          newChild
        }
    ...
    val newArgs = productIterator.map {
      ...
      case s: Seq[_] => s.map {
          case arg: TreeNode[_] if containsChild(arg) =>
            ...
            if (newChild fastEquals oldChild) { oldChild }
            else {
              changed = true
              newChild
            }
  ...
  def transformDown(rule: PartialFunction[BaseType, BaseType]): BaseType = {
    ...
    } else { afterRule.transformChildren(rule, (t, r) => t.transformDown(r)) }
  }
  ...
  def makeCopy(newArgs: Array[AnyRef]): BaseType =
    attachTree(this, "makeCopy") {
      ...
      if (ctors.isEmpty) { sys.error(s"No valid constructor for $nodeName") }
      ...
      try {
        CurrentOrigin.withOrigin(origin) {
          ...
          if (otherCopyArgs.isEmpty) {
            ...
            defaultCtor.newInstance((newArgs ++ otherCopyArgs).toArray: _*)
  ...
  def treeString: String =
    generateTreeString(0, Nil, new StringBuilder).toString
  ...
  protected def getNodeNumbered(number: MutableInt): BaseType = {
    if (number.i < 0) { null.asInstanceOf[BaseType] }
    else if (number.i == 0) { this }
    else {
      number.i -= 1
      children.map(_.getNodeNumbered(number)).find(_ != null)
  ...
  protected def jsonFields: List[JField] = {
    ...
    fieldNames.zip(fieldValues).map {
      ...
      case (name, value: Seq[BaseType])
        ...
        name -> JArray(
          value.map(v => JInt(children.indexOf(v.asInstanceOf[TreeNode[_]])))
            .toList)
...
object TreeNode {
  ...
  private def reconstruct(
    ...
    def parseNextNode(): TreeNode[_] = {
      ...
      if (cls == classOf[Literal]) { Literal.fromJSON(nextNode) }
      else if (cls.getName.endsWith("$")) {
        cls.getField("MODULE$").get(cls).asInstanceOf[TreeNode[_]]
      } else {
        val numChildren = (nextNode \ "num-children").asInstanceOf[JInt].num
          .toInt
  ...
  private def parseToProduct(
    ...
    val ctor = Utils.classForName(clsName).getConstructors
      .maxBy(_.getParameterTypes.size)
    ctor.newInstance(parameters: _*).asInstanceOf[AnyRef]
  }

  private def isScalaObject(jValue: JValue): Boolean =
    (jValue \ "object") match {
      case JString(str) if str.endsWith("$") => true
      case _                                 => false
    }

  private def isScalaProduct(jValue: JValue): Boolean =
    (jValue \ "product-class") match {
      case _: JString => true
      case _          => false
    }
}
