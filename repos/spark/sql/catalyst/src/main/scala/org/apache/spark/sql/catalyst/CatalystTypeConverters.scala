...
object CatalystTypeConverters {
    ...
    val converter =
      dataType match {
        ...
        case arrayType: ArrayType =>
          ArrayConverter(arrayType.elementType)
        case mapType: MapType =>
          MapConverter(mapType.keyType, mapType.valueType)
        case structType: StructType =>
          StructConverter(structType)
        case StringType =>
          StringConverter
        case DateType =>
          DateConverter
  ...
  private object TimestampConverter
      extends CatalystTypeConverter[Timestamp, Timestamp, Any] {
  ...
  private abstract class PrimitiveConverter[T]
      extends CatalystTypeConverter[T, Any, Any] {
  ...
  private object ByteConverter extends PrimitiveConverter[Byte] {
    override def toScalaImpl(row: InternalRow, column: Int): Byte =
      row.getByte(column)
  }

  private object ShortConverter extends PrimitiveConverter[Short] {
    override def toScalaImpl(row: InternalRow, column: Int): Short =
      row.getShort(column)
  }
  ...
  def convertToCatalyst(a: Any): Any =
    a match {
      case s: String =>
        StringConverter.toCatalyst(s)
      case d: Date =>
        DateConverter.toCatalyst(d)
      case t: Timestamp =>
        TimestampConverter.toCatalyst(t)
      case d: BigDecimal =>
      ...
      case r: Row =>
        InternalRow(r.toSeq.map(convertToCatalyst): _*)
      case arr: Array[Any] =>
        new GenericArrayData(arr.map(convertToCatalyst))
      case m: Map[_, _] =>
        val length = m.size
        val convertedKeys = new Array[Any](length)
        val convertedValues = new Array[Any](length)
