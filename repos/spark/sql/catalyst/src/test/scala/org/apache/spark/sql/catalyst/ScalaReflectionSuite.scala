...
class ScalaReflectionSuite extends SparkFunSuite {
  ...
  test("infer schema from case class with multiple constructors") {
    ...
    dataType match {
      case s: StructType =>
        ...
        assert(
          s.fields.map(_.dataType) === Seq(IntegerType, StringType, DoubleType))
    }
  }
  ...
  test("get parameter type from a function object") {
    ...
    assert(
      boxedTypes === Seq(classOf[java.lang.Integer], classOf[java.lang.Long]))
  ...
  Seq(
    ...
    (
      ...
      () => getConstructorParameterNames(classOf[ComplexData])),
