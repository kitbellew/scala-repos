...
class UnsafeRowConverterSuite extends SparkFunSuite with Matchers {
  ...
  test("basic conversion with primitive, string and binary types") {
    ...
    assert(
      unsafeRow.getSizeInBytes ===
        8 +
        (8 * 3) + roundedSize("Hello".getBytes(StandardCharsets.UTF_8).length) +
  ...
  test("basic conversion with primitive, string, date and timestamp types") {
    ...
    assert(
      unsafeRow.getSizeInBytes ===
        8 +
        (8 * 4) + roundedSize("Hello".getBytes(StandardCharsets.UTF_8).length))

    assert(unsafeRow.getLong(0) === 0)
    assert(unsafeRow.getString(1) === "Hello")
    // Date is represented as Int in unsafeRow
    assert(
      DateTimeUtils.toJavaDate(unsafeRow.getInt(2)) ===
  ...
  test("null handling") {
    ...
    assert(
      setToNullAfterCreation.getBoolean(1) ===
    ...
    assert(
      setToNullAfterCreation.getDouble(7) === rowWithNoNullColumns.getDouble(7))
    assert(
      setToNullAfterCreation.getString(8) === rowWithNoNullColumns.getString(8))
    assert(
      setToNullAfterCreation.getBinary(9) === rowWithNoNullColumns.getBinary(9))
    assert(
      setToNullAfterCreation.getDecimal(10, 10, 0) ===
        rowWithNoNullColumns.getDecimal(10, 10, 0))
    assert(
      setToNullAfterCreation.getDecimal(11, 38, 18) ===
  ...
  test("basic conversion with struct type") {
    ...
    assert(
      unsafeRow.getSizeInBytes ==
        8 + 2 * 8 + row1.getSizeInBytes + row2.getSizeInBytes)
  }
  ...
  private def testMapInt(
    ...
    assert(
      map.getSizeInBytes ==
        4 + map.keyArray.getSizeInBytes + map.valueArray.getSizeInBytes)
  }
  ...
  test("basic conversion with map type") {
    ...
    assert(
      unsafeMap2.getSizeInBytes ==
        4 + keyArray.getSizeInBytes + valueArray.getSizeInBytes)
  ...
  test("basic conversion with struct and array") {
    ...
    assert(
      field1.getSizeInBytes == 8 + 8 + roundedSize(innerArray.getSizeInBytes))
  ...
  test("basic conversion with array and map") {
    ...
    assert(
      unsafeRow.getSizeInBytes ==
        8 + 8 * 2 +
        roundedSize(field1.getSizeInBytes) + roundedSize(field2.getSizeInBytes))
  }
}
