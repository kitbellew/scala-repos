...
class SessionCatalogSuite extends SparkFunSuite {
  ...
  test("create table") {
    ...
    assert(
      externalCatalog.listTables("db2").toSet == Set("tbl1", "tbl2", "tbl3"))
  ...
  test("create table when database does not exist") {
    ...
    intercept[AnalysisException] {
      catalog
        .createTable(newTable("tbl1", "does_not_exist"), ignoreIfExists = false)
    }
    intercept[AnalysisException] {
      catalog
        .createTable(newTable("tbl1", "does_not_exist"), ignoreIfExists = true)
    }
  ...
  test("rename table when database/table does not exist") {
    ...
    intercept[AnalysisException] {
      catalog.renameTable(
        TableIdentifier("tbl1", Some("unknown_db")),
        TableIdentifier("tbl2", Some("unknown_db")))
    }
    intercept[AnalysisException] {
      catalog.renameTable(
        TableIdentifier("unknown_table", Some("db2")),
  ...
  test("get table") {
    ...
    val sessionCatalog = new SessionCatalog(externalCatalog)
    assert(
      sessionCatalog.getTable(TableIdentifier("tbl1", Some("db2"))) ==
        externalCatalog.getTable("db2", "tbl1"))
    // Get table without explicitly specifying database
    sessionCatalog.setCurrentDatabase("db2")
    assert(
      sessionCatalog.getTable(TableIdentifier("tbl1")) ==
        externalCatalog.getTable("db2", "tbl1"))
  }
  ...
  test("lookup table relation") {
    ...
    assert(
      sessionCatalog.lookupRelation(TableIdentifier("tbl1", Some("db2"))) ==
        SubqueryAlias("tbl1", CatalogRelation("db2", metastoreTable1)))
    // Otherwise, we'll first look up a temporary table with the same name
    assert(
      sessionCatalog.lookupRelation(TableIdentifier("tbl1")) ==
  ...
  test("lookup table relation with alias") {
    ...
    assert(
      catalog
        .lookupRelation(TableIdentifier("tbl1", Some("db2")), alias = None) ==
        relation)
    assert(
  ...
  test("list tables with pattern") {
    ...
    catalog.createTempTable("tbl4", tempTable, ignoreIfExists = false)
    assert(
      catalog.listTables("db1", "*").toSet == catalog.listTables("db1").toSet)
    assert(
      catalog.listTables("db2", "*").toSet == catalog.listTables("db2").toSet)
    assert(
      catalog.listTables("db2", "tbl*").toSet == Set(
        TableIdentifier("tbl1"),
        TableIdentifier("tbl4"),
        TableIdentifier("tbl1", Some("db2")),
        TableIdentifier("tbl2", Some("db2"))))
  ...
  test("basic create and list partitions") {
    ...
    assert(
      catalogPartitionsEqual(externalCatalog, "mydb", "tbl", Seq(part1, part2)))
  ...
  test("drop partitions") {
    ...
    val sessionCatalog = new SessionCatalog(externalCatalog)
    assert(
      catalogPartitionsEqual(externalCatalog, "db2", "tbl2", Seq(part1, part2)))
  ...
  test("get partition") {
    ...
    catalog.setCurrentDatabase("db2")
    assert(
      catalog.getPartition(TableIdentifier("tbl2"), part1.spec).spec ==
        part1.spec)
    assert(
      catalog.getPartition(TableIdentifier("tbl2"), part2.spec).spec ==
        part2.spec)
  ...
  test("rename partitions") {
    ...
    catalog.renamePartitions(
      ...
      newSpecs)
    assert(
      catalog.getPartition(TableIdentifier("tbl2", Some("db2")), newPart1.spec)
        .spec === newPart1.spec)
    assert(
      catalog.getPartition(TableIdentifier("tbl2", Some("db2")), newPart2.spec)
        .spec === newPart2.spec)
    intercept[AnalysisException] {
      catalog.getPartition(TableIdentifier("tbl2", Some("db2")), part1.spec)
    }
  ...
  test("alter partitions") {
    ...
    val newPart1 = catalog
      .getPartition(TableIdentifier("tbl2", Some("db2")), part1.spec)
    val newPart2 = catalog
      .getPartition(TableIdentifier("tbl2", Some("db2")), part2.spec)
  ...
  test("list partitions") {
    val catalog = new SessionCatalog(newBasicCatalog())
    assert(
      catalog.listPartitions(TableIdentifier("tbl2", Some("db2"))).toSet ==
        Set(part1, part2))
    // List partitions without explicitly specifying database
    catalog.setCurrentDatabase("db2")
    assert(
      catalog.listPartitions(TableIdentifier("tbl2")).toSet ==
        Set(part1, part2))
  }
  ...
  test("basic create and list functions") {
    ...
    assert(
      externalCatalog.listFunctions("mydb", "*").toSet ==
        Set("myfunc", "myfunc2"))
  }
  ...
  test("drop function when database/function does not exist") {
    ...
    intercept[AnalysisException] {
      catalog
        .dropFunction(FunctionIdentifier("something", Some("does_not_exist")))
    }
    intercept[AnalysisException] {
  ...
  test("get temp function") {
    ...
    assert(
      sessionCatalog.getFunction(FunctionIdentifier("func1", Some("db2"))) ==
        metastoreFunc)
  ...
  test("rename function") {
    ...
    assert(
      sessionCatalog.getFunction(FunctionIdentifier("func1", Some("db2"))) ==
        newFunc("func1", Some("db2")))
    assert(externalCatalog.listFunctions("db2", "*").toSet == Set("func1"))
    sessionCatalog.renameFunction(
      FunctionIdentifier("func1", Some("db2")),
      FunctionIdentifier(newName, Some("db2")))
    assert(
      sessionCatalog.getFunction(FunctionIdentifier(newName, Some("db2"))) ==
        newFunc(newName, Some("db2")))
    assert(externalCatalog.listFunctions("db2", "*").toSet == Set(newName))
    // Rename function without explicitly specifying database
    sessionCatalog.setCurrentDatabase("db2")
    sessionCatalog
      .renameFunction(FunctionIdentifier(newName), FunctionIdentifier("func1"))
    assert(
      sessionCatalog.getFunction(FunctionIdentifier("func1")) ==
        newFunc("func1", Some("db2")))
    assert(externalCatalog.listFunctions("db2", "*").toSet == Set("func1"))
    ...
    intercept[AnalysisException] {
      sessionCatalog.renameFunction(
        FunctionIdentifier("func1", Some("db2")),
        FunctionIdentifier("func2", Some("db1")))
    }
  }
  ...
  test("rename temp function") {
    ...
    sessionCatalog.renameFunction(
      FunctionIdentifier("func1", Some("db2")),
      FunctionIdentifier("func3", Some("db2")))
    ...
    sessionCatalog
      .renameFunction(FunctionIdentifier("func1"), FunctionIdentifier("func4"))
    assert(
      sessionCatalog.getTempFunction("func4") ==
        Some(tempFunc.copy(name = FunctionIdentifier("func4"))))
    assert(sessionCatalog.getTempFunction("func1") == None)
    assert(
      externalCatalog.listFunctions("db2", "*").toSet == Set("func1", "func3"))
    // Then, if no such temporary function exist, rename the function in the current database
    sessionCatalog
      .renameFunction(FunctionIdentifier("func1"), FunctionIdentifier("func5"))
    assert(sessionCatalog.getTempFunction("func5") == None)
    assert(
      externalCatalog.listFunctions("db2", "*").toSet == Set("func3", "func5"))
  }

  test("alter function") {
    val catalog = new SessionCatalog(newBasicCatalog())
    assert(
      catalog.getFunction(FunctionIdentifier("func1", Some("db2"))).className ==
        funcClass)
    catalog
      .alterFunction(newFunc("func1", Some("db2")).copy(className = "muhaha"))
    assert(
      catalog.getFunction(FunctionIdentifier("func1", Some("db2"))).className ==
        "muhaha")
    ...
    assert(
      catalog.getFunction(FunctionIdentifier("func1")).className == "derpy")
  }
  ...
  test("list functions") {
    ...
    catalog.createTempFunction(tempFunc2, ignoreIfExists = false)
    assert(
      catalog.listFunctions("db1", "*").toSet ==
