...
class FilterPushdownSuite extends PlanTest {
  ...
  test("simple push down") {
    val originalQuery = testRelation.select('a).where('a === 1)

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a === 1).select('a).analyze

    comparePlans(optimized, correctAnswer)
  }

  test("combine redundant filters") {
    val originalQuery = testRelation.where('a === 1 && 'b === 1)
      .where('a === 1 && 'c === 1)

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a === 1 && 'b === 1 && 'c === 1)
      .analyze
  ...
  test("can't push without rewrite") {
    val originalQuery = testRelation.select('a + 'b as 'e).where('e === 1)
      .analyze

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a + 'b === 1).select('a + 'b as 'e)
      .analyze
  ...
  test(
    "nondeterministic: can't push down filter with nondeterministic condition through project") {
    val originalQuery = testRelation.select(Rand(10).as('rand), 'a)
      .where('rand > 5 || 'a > 5).analyze
  ...
  test(
    "nondeterministic: can't push down filter through project with nondeterministic field") {
    val originalQuery = testRelation.select(Rand(10).as('rand), 'a)
      .where('a > 5).analyze
  ...
  test("filters: combines filters") {
    val originalQuery = testRelation.select('a).where('a === 1).where('a === 2)

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = testRelation.where('a === 1 && 'a === 2).select('a)
      .analyze
  ...
  test("joins: push to either side") {
    ...
    val originalQuery = {
      x.join(y).where("x.b".attr === 1).where("y.b".attr === 2)
    }

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where('b === 1)
    val right = testRelation.where('b === 2)
    val correctAnswer = left.join(right).analyze
  ...
  test("joins: push to one side") {
    ...
    val originalQuery = { x.join(y).where("x.b".attr === 1) }
  ...
  test("joins: push to one side after transformCondition") {
    ...
    val correctAnswer = left.join(
      right,
      condition = Some("d".attr === "b".attr || "d".attr === "c".attr)).analyze
  ...
  test("joins: push down left semi join") {
    ...
    val originalQuery = {
      x.join(
        y,
        LeftSemi,
        Option("x.a".attr === "y.d".attr && "x.b".attr >= 1 && "y.d".attr >= 2))
    }
    ...
    val correctAnswer = left
      .join(right, LeftSemi, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: push down left outer join #1") {
    ...
    val originalQuery = {
      x.join(y, LeftOuter).where("x.b".attr === 1 && "y.b".attr === 2)
    }

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where('b === 1)
    val correctAnswer = left.join(y, LeftOuter).where("y.b".attr === 2).analyze
  ...
  test("joins: push down right outer join #1") {
    ...
    val originalQuery = {
      x.join(y, RightOuter).where("x.b".attr === 1 && "y.b".attr === 2)
    }

    val optimized = Optimize.execute(originalQuery.analyze)
    val right = testRelation.where('b === 2).subquery('d)
    val correctAnswer = x.join(right, RightOuter).where("x.b".attr === 1)
      .analyze
  ...
  test("joins: push down left outer join #2") {
    ...
    val correctAnswer = left.join(y, LeftOuter, Some("d.b".attr === 1))
      .where("y.b".attr === 2).analyze
  ...
  test("joins: push down right outer join #2") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val right = testRelation.where('b === 2).subquery('d)
    val correctAnswer = x.join(right, RightOuter, Some("d.b".attr === 1))
      .where("x.b".attr === 2).analyze
  ...
  test("joins: push down left outer join #3") {
    ...
    val correctAnswer = left.join(right, LeftOuter).where("r.b".attr === 2)
      .analyze
  ...
  test("joins: push down right outer join #3") {
    ...
    val correctAnswer = x.join(right, RightOuter, Some("r.b".attr === 1))
      .where("x.b".attr === 2).analyze
  ...
  test("joins: push down left outer join #4") {
    ...
    val originalQuery = {
      x.join(y, LeftOuter, Some("y.b".attr === 1)).where(
    ...
    val left = testRelation.where('b === 2).subquery('l)
    val right = testRelation.where('b === 1).subquery('r)
    val correctAnswer = left.join(right, LeftOuter)
  ...
  test("joins: push down right outer join #4") {
    ...
    val originalQuery = {
      x.join(y, RightOuter, Some("y.b".attr === 1)).where(
        "x.b".attr === 2 && "y.b".attr === 2 && "x.c".attr === "y.c".attr)
    }
    ...
    val right = testRelation.where('b === 2).subquery('r)
    val correctAnswer = left.join(right, RightOuter, Some("r.b".attr === 1))
  ...
  test("joins: push down left outer join #5") {
    ...
    val originalQuery = {
      x.join(y, LeftOuter, Some("y.b".attr === 1 && "x.a".attr === 3)).where(
        "x.b".attr === 2 && "y.b".attr === 2 && "x.c".attr === "y.c".attr)
    }
    ...
    val left = testRelation.where('b === 2).subquery('l)
    val right = testRelation.where('b === 1).subquery('r)
    val correctAnswer = left.join(right, LeftOuter, Some("l.a".attr === 3))
      .where("r.b".attr === 2 && "l.c".attr === "r.c".attr).analyze
  ...
  test("joins: push down right outer join #5") {
    ...
    val originalQuery = {
      x.join(y, RightOuter, Some("y.b".attr === 1 && "x.a".attr === 3)).where(
        "x.b".attr === 2 && "y.b".attr === 2 && "x.c".attr === "y.c".attr)
    }
  ...
  test("joins: conjunctive predicates") {
    ...
    val originalQuery = {
      x.join(y).where(
        ("x.b".attr === "y.b".attr) && ("x.a".attr === 1) && ("y.a".attr === 1))
    }
    ...
    val correctAnswer = left
      .join(right, condition = Some("x.b".attr === "y.b".attr)).analyze
  ...
  test("joins: conjunctive predicates #2") {
    ...
    val originalQuery = {
      x.join(y).where(("x.b".attr === "y.b".attr) && ("x.a".attr === 1))
    }
  ...
  test("joins: conjunctive predicates #3") {
    ...
    val left = testRelation.where('a === 1).subquery('x)
    val right = testRelation.subquery('y)
    val correctAnswer = lleft.join(
      left.join(right, condition = Some("x.b".attr === "y.b".attr)),
      condition = Some("z.a".attr === "x.b".attr)).analyze
  ...
  test("generate: predicate referenced no generated column") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = {
      testRelationWithArrayType.where(('b >= 5) && ('a > 6))
  ...
  test("generate: non-deterministic predicate referenced no generated column") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer = {
      testRelationWithArrayType.where('b >= 5)
        .generate(Explode('c_arr), true, false, Some("arr"))
        .where('a + Rand(10).as("rnd") > 6).analyze
    }
  ...
  test("generate: part of conjuncts referenced generated column") {
    ...
    val originalQuery = {
      testRelationWithArrayType.generate(generator, true, false, Some("arr"))
        .where(('b >= 5) && ('c > 6))
    }
    val optimized = Optimize.execute(originalQuery.analyze)
    val referenceResult = {
      testRelationWithArrayType.where('b >= 5)
        .generate(generator, true, false, Some("arr")).where('c > 6).analyze
    }
  ...
  test("push project and filter down into sample") {
    val x = testRelation.subquery('x)
    val originalQuery = Sample(0.0, 0.6, false, 11L, x)().select('a)

    val originalQueryAnalyzed = EliminateSubqueryAliases(
      analysis.SimpleAnalyzer.execute(originalQuery))

    val optimized = Optimize.execute(originalQueryAnalyzed)

    val correctAnswer = Sample(0.0, 0.6, false, 11L, x.select('a))()

    comparePlans(optimized, correctAnswer.analyze)
  }

  test("aggregate: push down filter when filter on group by expression") {
    val originalQuery = testRelation.groupBy('a)('a, count('b) as 'c)
      .select('a, 'c).where('a === 2)

    val optimized = Optimize.execute(originalQuery.analyze)

    val correctAnswer = testRelation.where('a === 2)
      .groupBy('a)('a, count('b) as 'c).analyze
    comparePlans(optimized, correctAnswer)
  }

  test(
    "aggregate: don't push down filter when filter not on group by expression") {
    val originalQuery = testRelation.select('a, 'b)
      .groupBy('a)('a, count('b) as 'c).where('c === 2L)
  ...
  test(
    ...
    val originalQuery = testRelation.select('a, 'b)
      .groupBy('a)('a, count('b) as 'c).where('c === 2L && 'a === 3)

    val optimized = Optimize.execute(originalQuery.analyze)

    val correctAnswer = testRelation.select('a, 'b).where('a === 3)
      .groupBy('a)('a, count('b) as 'c).where('c === 2L).analyze
  ...
  test("aggregate: push down filters with alias") {
    ...
    val correctAnswer = testRelation.select('a, 'b).where('a + 1 < 3)
      .groupBy('a)(('a + 1) as 'aa, count('b) as 'c).where('c === 2L || 'aa > 4)
      .analyze
  ...
  test("aggregate: push down filters with literal") {
    ...
    val correctAnswer = testRelation.select('a, 'b).where("s" === "s")
      .groupBy('a)('a, count('b) as 'c, "s" as 'd).where('c === 2L).analyze
  ...
  test("aggregate: don't push down filters that are nondeterministic") {
    ...
    val correctAnswer = testRelation.select('a, 'b)
      .groupBy('a)('a + Rand(10) as 'aa, count('b) as 'c, Rand(11).as("rnd"))
      .where('c === 2L && 'aa + Rand(10).as("rnd") === 3 && 'rnd === 5).analyze
