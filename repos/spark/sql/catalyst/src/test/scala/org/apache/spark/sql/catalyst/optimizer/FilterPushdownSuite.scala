...
class FilterPushdownSuite extends PlanTest {
  ...
  test("combine redundant filters") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer =
      testRelation.where('a === 1 && 'b === 1 && 'c === 1).analyze
  ...
  test("can't push without rewrite") {
    val originalQuery =
      testRelation.select('a + 'b as 'e).where('e === 1).analyze

    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer =
      testRelation.where('a + 'b === 1).select('a + 'b as 'e).analyze
  ...
  test(
    ...
    val originalQuery =
      testRelation.select(Rand(10).as('rand), 'a).where('a > 5).analyze
  ...
  test("filters: combines filters") {
    ...
    val optimized = Optimize.execute(originalQuery.analyze)
    val correctAnswer =
      testRelation.where('a === 1 && 'a === 2).select('a).analyze
  ...
  test("joins: push down left outer join #2") {
    ...
    val correctAnswer =
      left
        .join(y, LeftOuter, Some("d.b".attr === 1))
        .where("y.b".attr === 2)
        .analyze
  ...
  test("joins: conjunctive predicates") {
    ...
    val originalQuery = {
      x.join(y)
        .where(
          ("x.b"
            .attr === "y.b".attr) && ("x.a".attr === 1) && ("y.a".attr === 1))
    }
