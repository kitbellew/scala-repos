...
class OuterJoinEliminationSuite extends PlanTest {
  ...
  test("joins: full outer to inner") {
    ...
    val originalQuery = x.join(y, FullOuter, Option("x.a".attr === "y.d".attr))
      .where("x.b".attr >= 1 && "y.d".attr >= 2)

    val optimized = Optimize.execute(originalQuery.analyze)
    val left = testRelation.where('b >= 1)
    val right = testRelation1.where('d >= 2)
    val correctAnswer = left.join(right, Inner, Option("a".attr === "d".attr))
      .analyze
  ...
  test("joins: full outer to right") {
    ...
    val correctAnswer = left
      .join(right, RightOuter, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: full outer to left") {
    ...
    val correctAnswer = left
      .join(right, LeftOuter, Option("a".attr === "d".attr)).analyze
  ...
  test("joins: right to inner") {
    ...
    val originalQuery = x.join(y, RightOuter, Option("x.a".attr === "y.d".attr))
      .where("x.b".attr > 2)
  ...
  test("joins: left to inner") {
    ...
    val originalQuery = x.join(y, LeftOuter, Option("x.a".attr === "y.d".attr))
  ...
  test("joins: left to inner with complicated filter predicates #4") {
    ...
    val correctAnswer = left.join(
      right,
      Inner,
      Option("b".attr + 3 === "e".attr && "a".attr === "d".attr)).analyze
