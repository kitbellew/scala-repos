...
class ConstantFoldingSuite extends PlanTest {
  ...
  test("Constant folding test: expressions only have literals") {
    ...
    val correctAnswer = testRelation.select(
      ...
      Literal(14) as Symbol("2*(3+4)")).where(Literal(true))
      .groupBy(Literal(3.0))(Literal(3.0) as Symbol("9/3")).analyze
  ...
  test(
    ...
    val correctAnswer = testRelation.select(
      ...
      'a + Literal(2) + Literal(3) as Symbol("c2"),
      Literal(2) * 'a + Literal(4) as Symbol("c3"),
      'a * Literal(7) as Symbol("c4")).analyze
  ...
  test(
    ...
    val correctAnswer = testRelation
      .where(('a > 1 || 'b > 1) && ('a < 10 && 'b < 10)).analyze
  ...
  test("Constant folding test: expressions have foldable functions") {
    ...
    val correctAnswer = testRelation
      .select(Literal(5) + 'a as Symbol("c1"), Literal(3) as Symbol("c2"))
      .analyze
  ...
  test("Constant folding test: expressions have nonfoldable functions") {
    val originalQuery = testRelation
      .select(Rand(5L) + Literal(1) as Symbol("c1"), sum('a) as Symbol("c2"))
  ...
  test("Constant folding test: Fold In(v, list) into true or false") {
    val originalQuery = testRelation.select('a)
