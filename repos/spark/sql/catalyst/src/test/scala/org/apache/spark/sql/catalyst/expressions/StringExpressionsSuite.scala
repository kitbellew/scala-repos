...
class StringExpressionsSuite extends SparkFunSuite with ExpressionEvalHelper {
  ...
  test("string substring_index function") {
    ...
    // scalastyle:off
    checkEvaluation(
      SubstringIndex(Literal("大千世界大千世界"), Literal("千"), Literal(2)),
      "大千世界大")
    // scalastyle:on
    checkEvaluation(
      SubstringIndex(Literal("www||apache||org"), Literal("||"), Literal(2)),
  ...
  test("LIKE Non-literal Regular Expression") {
    ...
    checkEvaluation(
      Literal.create(null, StringType) like regEx,
      null,
      create_row("bc%"))
  }
  ...
  test("base64/unbase64 for string") {
    ...
    checkEvaluation(
      Base64(UnBase64(Literal("AQIDBA=="))),
      "AQIDBA==",
      create_row("abdef"))
    checkEvaluation(Base64(UnBase64(Literal(""))), "", create_row("abdef"))
    checkEvaluation(
      Base64(UnBase64(Literal.create(null, StringType))),
      null,
      create_row("abdef"))
    ...
    checkEvaluation(
      Base64(Literal.create(null, BinaryType)),
      null,
      create_row("abdef"))

    checkEvaluation(UnBase64(a), null, create_row(null))
    checkEvaluation(
      UnBase64(Literal.create(null, StringType)),
      null,
      create_row("abdef"))
  }
  ...
  test("encode/decode for string") {
    ...
    // non ascii characters are not allowed in the code, so we disable the scalastyle here.
    checkEvaluation(
      Decode(Encode(Literal("大千世界"), Literal("UTF-16LE")), Literal("UTF-16LE")),
      "大千世界")
    checkEvaluation(
    ...
    checkEvaluation(
      Encode(Literal.create(null, StringType), Literal("utf-8")),
      null)
    checkEvaluation(
      Encode(a, Literal.create(null, StringType)),
      null,
      create_row(""))

    checkEvaluation(Decode(b, Literal("utf-8")), null, create_row(null))
    checkEvaluation(
      Decode(Literal.create(null, BinaryType), Literal("utf-8")),
      null)
    checkEvaluation(
      Decode(b, Literal.create(null, StringType)),
      null,
      create_row(null))
  }
  ...
  test("Levenshtein distance") {
    checkEvaluation(
      Levenshtein(Literal.create(null, StringType), Literal("")),
      null)
    checkEvaluation(
      Levenshtein(Literal(""), Literal.create(null, StringType)),
      null)
  ...
  test("translate") {
    ...
    checkEvaluation(
      StringTranslate(Literal("花花世界"), Literal("花界"), Literal("ab")),
      "aa世b")
    // scalastyle:on
  }
  ...
  test("FORMAT") {
    checkEvaluation(
      FormatString(Literal("aa%d%s"), Literal(123), Literal("a")),
      "aa123a")
    ...
    checkEvaluation(
      FormatString(Literal.create(null, StringType), 12, "cc"),
      null)
    checkEvaluation(
  ...
  test("LOCATE") {
    ...
    checkEvaluation(
      StringLocate(Literal("aa"), Literal("aaads"), Literal(1)),
      2,
      row1)
    checkEvaluation(
      StringLocate(Literal("aa"), Literal("aaads"), Literal(2)),
      0,
      row1)
    ...
    checkEvaluation(
      StringLocate(s3, s1, Literal.create(null, IntegerType)),
      0,
      row1)
    checkEvaluation(new StringLocate(s2, s1), null, row2)
    checkEvaluation(new StringLocate(s2, s1), null, row3)
    checkEvaluation(
      new StringLocate(s2, s1, Literal.create(null, IntegerType)),
      0,
      row4)
  }
  ...
  test("LPAD/RPAD") {
    ...
    checkEvaluation(
      ...
      row1)
    checkEvaluation(
      StringLPad(Literal("hi"), Literal(1), Literal("??")),
      "h",
      row1)
    ...
    checkEvaluation(
      ...
      row1)
    checkEvaluation(
      StringRPad(Literal("hi"), Literal(1), Literal("??")),
      "h",
      row1)
  ...
  test("SPLIT") {
    ...
    checkEvaluation(
      StringSplit(Literal("aa2bb3cc"), Literal("[1-9]+")),
      Seq("aa", "bb", "cc"),
      row1)
  ...
  test("length for string / binary") {
    ...
    checkEvaluation(
      Length(Literal.create(null, StringType)),
      null,
      create_row(string))
    checkEvaluation(
      Length(Literal.create(null, BinaryType)),
      null,
      create_row(bytes))
  }
  ...
  test("format_number / FormatNumber") {
    checkEvaluation(
      FormatNumber(Literal(4.asInstanceOf[Byte]), Literal(3)),
      "4.000")
    checkEvaluation(
      FormatNumber(Literal(4.asInstanceOf[Short]), Literal(3)),
      "4.000")
    checkEvaluation(FormatNumber(Literal(4.0f), Literal(3)), "4.000")
    checkEvaluation(FormatNumber(Literal(4), Literal(3)), "4.000")
    checkEvaluation(
      FormatNumber(Literal(12831273.23481d), Literal(3)),
      "12,831,273.235")
    checkEvaluation(
      FormatNumber(Literal(12831273.83421d), Literal(0)),
      "12,831,274")
    checkEvaluation(
      FormatNumber(Literal(123123324123L), Literal(3)),
      "123,123,324,123.000")
    ...
    checkEvaluation(
      FormatNumber(Literal.create(null, IntegerType), Literal(3)),
      null)
    checkEvaluation(
      FormatNumber(Literal.create(null, NullType), Literal(3)),
      null)
  }
  ...
  test("find in set") {
    checkEvaluation(
      ...
      null)
    checkEvaluation(
      FindInSet(Literal("ab"), Literal.create(null, StringType)),
      null)
    checkEvaluation(
