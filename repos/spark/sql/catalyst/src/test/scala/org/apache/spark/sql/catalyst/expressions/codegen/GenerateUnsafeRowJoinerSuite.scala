...
class GenerateUnsafeRowJoinerSuite extends SparkFunSuite {
  ...
  private def testConcatOnce(
    ...
    val extRow1 = RandomDataGenerator.forType(schema1, nullable = false).get
      .apply()
    val extRow2 = RandomDataGenerator.forType(schema2, nullable = false).get
      .apply()
    val row1 = converter1
        ...
        if (!output.isNullAt(i)) {
          assert(
            output.get(i, mergedSchema(i).dataType) ===
              row1.get(i, mergedSchema(i).dataType))
        }
      } else {
