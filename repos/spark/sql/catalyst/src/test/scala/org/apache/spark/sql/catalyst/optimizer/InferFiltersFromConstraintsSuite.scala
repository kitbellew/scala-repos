...
class InferFiltersFromConstraintsSuite extends PlanTest {
  ...
  test("single inner join: filter out nulls on either side on non equal keys") {
    ...
    val originalQuery = x.join(
      y,
      condition = Some(
        ("x.a".attr =!= "y.a".attr) && ("x.b".attr === 1) && ("y.c".attr > 5)))
      .analyze
  ...
  test(
    ...
    val x = testRelation.subquery('x)
    val y = testRelation.subquery('y)
    val originalQuery = x.where('b > 5).join(
      y.where('a === 10),
  ...
  test("single outer join: no null filters are generated") {
    ...
    val y = testRelation.subquery('y)
    val originalQuery = x
      .join(y, FullOuter, condition = Some("x.a".attr === "y.a".attr)).analyze
    val optimized = Optimize.execute(originalQuery)
  ...
  test(
    ...
    val correctAnswer = t1.where(IsNotNull('b) && 'b > 5).join(
      t2.where(IsNotNull('b) && 'b > 5),
      condition = Some("t1.b".attr === "t2.b".attr)).join(
      t3.where(IsNotNull('b) && 'b > 5),
      condition = Some("t2.b".attr === "t3.b".attr)).join(
      t4.where(IsNotNull('b) && 'b > 5),
      condition = Some("t3.b".attr === "t4.b".attr)).analyze
    val optimized = Optimize.execute(originalQuery)
    comparePlans(optimized, correctAnswer)
  }
  ...
  test(
    ...
    val originalQuery = x.join(y, Inner, Some("x.a".attr === "y.a".attr))
      .where("x.a".attr > 5).analyze
    val correctAnswer = x.where(IsNotNull('a) && 'a.attr > 5).join(
