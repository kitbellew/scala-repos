...
class InferFiltersFromConstraintsSuite extends PlanTest {
  ...
  test(
    ...
    val originalQuery = x.where('b > 5).join(
      y.where('a === 10),
      condition =
        Some("x.a".attr === "y.a".attr && "x.b".attr === "y.b".attr)).analyze
  ...
  test("single outer join: no null filters are generated") {
    ...
    val y = testRelation.subquery('y)
    val originalQuery =
      x.join(y, FullOuter, condition = Some("x.a".attr === "y.a".attr)).analyze
    val optimized = Optimize.execute(originalQuery)
  ...
  test(
    ...
    val originalQuery =
      x.join(y, Inner, Some("x.a".attr === "y.a".attr)).where(
        "x.a".attr > 5).analyze
