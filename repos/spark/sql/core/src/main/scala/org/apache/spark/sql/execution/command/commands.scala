...
/**
  ...
  */
private[sql] case class ExecutedCommand(cmd: RunnableCommand)
    extends SparkPlan {
...
/**
  ...
  * }}}
  */
case class ShowTablesCommand(databaseName: Option[String])
    extends RunnableCommand {
...
/**
  ...
  * }}}
  */
case class DescribeFunction(functionName: String, isExtended: Boolean)
    extends RunnableCommand {
      ...
      case Some(info) =>
        val result =
          Row(s"Function: ${info.getName}") ::
            ...
            Row(s"Usage: ${replaceFunctionName(info.getUsage(), info.getName)}") ::
            Nil
