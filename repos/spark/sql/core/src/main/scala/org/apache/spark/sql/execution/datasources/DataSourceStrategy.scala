...
private[sql] object DataSourceStrategy extends Strategy with Logging {
  def apply(plan: LogicalPlan): Seq[execution.SparkPlan] =
    plan match {
      ...
      case PhysicalOperation(
        ...
        val partitionFilters = filters
          .filter(_.references.subsetOf(partitionColumns))

        // Only pushes down predicates that do not reference partition keys.
        val pushedFilters = filters
        ...
        partitionAndNormalColumnFilters.reduceLeftOption(expressions.And)
          .map(cf =>
          ...
          case Some(spec) if t.sqlContext.conf.bucketingEnabled =>
            val scanBuilder
              ...
              (requiredColumns: Seq[Attribute], filters: Array[Filter]) =>
                {
                  val bucketed = t.location.allFiles()
                    .filterNot(_.getPath.getName startsWith "_").groupBy { f =>
                      BucketingUtils.getBucketId(f.getPath.getName)
                        .getOrElse(sys.error(
      ...
      (requiredColumns: Seq[Attribute], filters: Array[Filter]) =>
        {
          ...
          relation.bucketSpec match {
            case Some(spec) if relation.sqlContext.conf.bucketingEnabled =>
              val requiredDataColumns = requiredColumns
                .filterNot(c => partitionColumnNames.contains(c.name))

              // Builds RDD[Row]s for each selected partition.
              val perPartitionRows: Seq[(Int, RDD[InternalRow])] = partitions
                .flatMap {
              ...
              val bucketed = new UnionRDD(
                ...
                (0 until spec.numBuckets).map { bucketId =>
                  bucketedDataMap.get(bucketId)
                    .map(i => i.reduce(_ ++ _).coalesce(1)).getOrElse {
  ...
  private def projectedColumnBatch(
    ...
    val result = ColumnarBatch
      .allocate(StructType.fromAttributes(requiredColumns))
    var resultIdx = 0
    var inputIdx = 0
    ...
    while (resultIdx < requiredColumns.length) {
      ...
      if (inputIdx < dataColumns.length && requiredColumns(
        ...
        require(
          partitionColumnSchema.fields.filter(_.name.equals(attr.name))
            .length == 1)
        var partitionIdx = 0
    ...
    if (requiredColumns != dataColumns) {
      ...
      val mapPartitionsFunc =
        (_: TaskContext, _: Int, iterator: Iterator[Object]) => {
          ...
          val unsafePartitionValues = UnsafeProjection
            .create(partitionColumnSchema)(partitionValues)
  ...
  def getBucketId(bucketColumn: Attribute, numBuckets: Int, value: Any): Int = {
    ...
    val bucketIdGeneration = UnsafeProjection.create(
      HashPartitioning(bucketColumn :: Nil, numBuckets)
  ...
  private def getBuckets(
    ...
    filters.foreach {
      ...
      case expressions.In(a: Attribute, list)
          if list.forall(_.isInstanceOf[Literal]) && a
  ...
  protected def pruneFilterProjectRaw(
    ...
    val (unhandledPredicates, pushedFilters) = selectFilters(
      relation.relation,
      candidatePredicates)
    ...
    val handledSet = {
      val handledPredicates = filterPredicates
        .filterNot(unhandledPredicates.contains)
  ...
  protected[sql] def translateFilter(predicate: Expression): Option[Filter] = {
    predicate match {
      case expressions.EqualTo(a: Attribute, Literal(v, t)) => Some(
          sources.EqualTo(a.name, convertToScala(v, t)))
      case expressions.EqualTo(Literal(v, t), a: Attribute) => Some(
          sources.EqualTo(a.name, convertToScala(v, t)))

      case expressions.EqualNullSafe(a: Attribute, Literal(v, t)) => Some(
          sources.EqualNullSafe(a.name, convertToScala(v, t)))
      case expressions.EqualNullSafe(Literal(v, t), a: Attribute) => Some(
          sources.EqualNullSafe(a.name, convertToScala(v, t)))

      case expressions.GreaterThan(a: Attribute, Literal(v, t)) => Some(
          sources.GreaterThan(a.name, convertToScala(v, t)))
      case expressions.GreaterThan(Literal(v, t), a: Attribute) => Some(
          sources.LessThan(a.name, convertToScala(v, t)))

      case expressions.LessThan(a: Attribute, Literal(v, t)) => Some(
          sources.LessThan(a.name, convertToScala(v, t)))
      case expressions.LessThan(Literal(v, t), a: Attribute) => Some(
          sources.GreaterThan(a.name, convertToScala(v, t)))

      case expressions.GreaterThanOrEqual(a: Attribute, Literal(v, t)) => Some(
          sources.GreaterThanOrEqual(a.name, convertToScala(v, t)))
      case expressions.GreaterThanOrEqual(Literal(v, t), a: Attribute) => Some(
          sources.LessThanOrEqual(a.name, convertToScala(v, t)))

      case expressions.LessThanOrEqual(a: Attribute, Literal(v, t)) => Some(
          sources.LessThanOrEqual(a.name, convertToScala(v, t)))
      case expressions.LessThanOrEqual(Literal(v, t), a: Attribute) => Some(
          sources.GreaterThanOrEqual(a.name, convertToScala(v, t)))
      ...
      case expressions.IsNull(a: Attribute) => Some(sources.IsNull(a.name))
      case expressions.IsNotNull(a: Attribute) => Some(
          sources.IsNotNull(a.name))

      case expressions.And(left, right) =>
        (translateFilter(left) ++ translateFilter(right))
          .reduceOption(sources.And)

      case expressions.Or(left, right) => for {
          leftFilter <- translateFilter(left)
          rightFilter <- translateFilter(right)
        } yield sources.Or(leftFilter, rightFilter)

      case expressions.Not(child) => translateFilter(child).map(sources.Not)
      ...
      case expressions
            .EndsWith(a: Attribute, Literal(v: UTF8String, StringType)) => Some(
          sources.StringEndsWith(a.name, v.toString))

      case expressions
            .Contains(a: Attribute, Literal(v: UTF8String, StringType)) => Some(
          sources.StringContains(a.name, v.toString))
  ...
  protected[sql] def selectFilters(
    ...
    val unhandledFilters = relation
      .unhandledFilters(translatedMap.values.toArray).toSet

    val (unhandled, handled) = translated.partition {
      case (predicate, filter) => unhandledFilters.contains(filter)
    }
