  ...
  override def compute(
    ...
    val resultIter =
      for (x <- rdd1.iterator(partition.s1, context);
           y <- createIter())
        yield (x, y)
    CompletionIterator[(UnsafeRow, UnsafeRow), Iterator[(
        UnsafeRow,
        UnsafeRow)]](resultIter, sorter.cleanupResources)
  }
}
...
case class CartesianProduct(left: SparkPlan, right: SparkPlan)
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    ...
    val pair =
      new UnsafeCartesianRDD(leftResults, rightResults, right.output.size)
