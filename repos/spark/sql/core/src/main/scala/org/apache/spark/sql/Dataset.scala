...
class Dataset[T] private[sql] (
  ...
  /**
    ...
    */
  override private[sql] def showString(
      _numRows: Int,
  ...
  def toDF(): DataFrame =
    new Dataset[Row](sqlContext, queryExecution, RowEncoder(schema))
  ...
  def join(right: DataFrame): DataFrame =
    withPlan { Join(logicalPlan, right.logicalPlan, joinType = Inner, None) }
  ...
  def as(alias: String): Dataset[T] =
    withTypedPlan { SubqueryAlias(alias, logicalPlan) }
  ...
  def select(cols: Column*): DataFrame =
    withPlan { Project(cols.map(_.named), logicalPlan) }
  ...
  def filter(condition: Column): Dataset[T] =
    withTypedPlan { Filter(condition.expr, logicalPlan) }
  ...
  def limit(n: Int): Dataset[T] =
    withTypedPlan { Limit(Literal(n), logicalPlan) }
  ...
  def intersect(other: Dataset[T]): Dataset[T] =
    withTypedPlan { Intersect(logicalPlan, other.logicalPlan) }
  ...
  def except(other: Dataset[T]): Dataset[T] =
    withTypedPlan { Except(logicalPlan, other.logicalPlan) }
  ...
  def explode[A, B: TypeTag](inputColumn: String, outputColumn: String)(
    ...
    val elementTypes = attributes.map { attr =>
      (attr.dataType, attr.nullable, attr.name)
    }
  ...
  def foreachPartition(f: Iterator[T] => Unit): Unit =
    withNewExecutionId { rdd.foreachPartition(f) }
  ...
  def repartition(numPartitions: Int): Dataset[T] =
    withTypedPlan { Repartition(numPartitions, shuffle = true, logicalPlan) }
  ...
  def coalesce(numPartitions: Int): Dataset[T] =
    withTypedPlan { Repartition(numPartitions, shuffle = false, logicalPlan) }
