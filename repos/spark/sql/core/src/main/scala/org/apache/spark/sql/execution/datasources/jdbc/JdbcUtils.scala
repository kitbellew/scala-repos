...
object JdbcUtils extends Logging {
  ...
  def getCommonJDBCType(dt: DataType): Option[JdbcType] = {
    dt match {
      ...
      case DoubleType => Option(
          JdbcType("DOUBLE PRECISION", java.sql.Types.DOUBLE))
      ...
      case TimestampType => Option(
          JdbcType("TIMESTAMP", java.sql.Types.TIMESTAMP))
          ...
          while (i < numFields) {
            if (row.isNullAt(i)) { stmt.setNull(i + 1, nullTypes(i)) }
              ...
              rddSchema.fields(i).dataType match {
                ...
                case TimestampType =>
                  stmt.setTimestamp(i + 1, row.getAs[java.sql.Timestamp](i))
                case DateType =>
                  stmt.setDate(i + 1, row.getAs[java.sql.Date](i))
                case t: DecimalType =>
