...
private[sql] class SQLListener(conf: SparkConf)
  ...
  def executionIdToData: Map[Long, SQLExecutionUIData] =
    synchronized { _executionIdToData.toMap }

  def jobIdToExecutionId: Map[Long, Long] =
    synchronized { _jobIdToExecutionId.toMap }

  def stageIdToStageMetrics: Map[Long, SQLStageMetrics] =
    synchronized { _stageIdToStageMetrics.toMap }
  ...
  def getRunningExecutions: Seq[SQLExecutionUIData] =
    synchronized { activeExecutions.values.toSeq }

  def getFailedExecutions: Seq[SQLExecutionUIData] =
    synchronized { failedExecutions }

  def getCompletedExecutions: Seq[SQLExecutionUIData] =
  ...
  /**
    ...
    */
  def getExecutionMetrics(executionId: Long): Map[Long, String] =
    synchronized {
...
private[spark] class SQLHistoryListener(conf: SparkConf, sparkUI: SparkUI)
  ...
  override def onExecutorMetricsUpdate(
      u: SparkListenerExecutorMetricsUpdate): Unit = {
...
private[ui] class SQLExecutionUIData(
  ...
  def hasRunningJobs: Boolean =
    jobs.values.exists(_ == JobExecutionStatus.RUNNING)
