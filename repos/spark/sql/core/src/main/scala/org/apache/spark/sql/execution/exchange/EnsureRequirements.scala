...
case class EnsureRequirements(conf: SQLConf) extends Rule[SparkPlan] {
  ...
  private def withExchangeCoordinator(
    ...
    val supportsCoordinator =
      if (children.exists(_.isInstanceOf[ShuffleExchange])) {
        ...
        children.forall {
          ...
          case child => child.outputPartitioning match {
              ...
              case collection: PartitioningCollection =>
                collection.partitionings
                  .forall(_.isInstanceOf[HashPartitioning])
              case _ => false
            }
        }
      } else {
        ...
        val supportsDistribution = requiredChildDistributions
      ...
      if (adaptiveExecutionEnabled && supportsCoordinator) {
        ...
        children.zip(requiredChildDistributions).map {
          ...
          case (child, distribution) =>
            ...
            val targetPartitioning = createPartitioning(
              distribution,
  ...
  private def ensureDistributionAndOrdering(operator: SparkPlan): SparkPlan = {
    val requiredChildDistributions: Seq[Distribution] = operator
      .requiredChildDistribution
    val requiredChildOrderings: Seq[Seq[SortOrder]] = operator
    ...
    if (children.length > 1 &&
      ...
      val maxChildrenNumPartitions = children
        .map(_.outputPartitioning.numPartitions).max
      val useExistingPartitioning = children.zip(requiredChildDistributions)
        .forall {
          case (child, distribution) =>
      ...
      children =
        if (useExistingPartitioning) {
          ...
          val numPartitions = {
            ...
            val shufflesAllChildren = children.zip(requiredChildDistributions)
              .forall {
                case (child, distribution) =>
                  !child.outputPartitioning.guarantees(
                    createPartitioning(distribution, maxChildrenNumPartitions))
