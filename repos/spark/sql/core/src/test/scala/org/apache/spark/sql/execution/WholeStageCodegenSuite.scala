...
class WholeStageCodegenSuite extends SparkPlanTest with SharedSQLContext {
    ...
    assert(
      plan.find(p =>
        p.isInstanceOf[WholeStageCodegen] &&
          p.asInstanceOf[WholeStageCodegen].child
            .isInstanceOf[TungstenAggregate]).isDefined)
  ...
  test("BroadcastHashJoin should be included in WholeStageCodegen") {
    ...
    assert(
      df.queryExecution.executedPlan.find(p =>
        p.isInstanceOf[WholeStageCodegen] &&
          p.asInstanceOf[WholeStageCodegen].child
            .isInstanceOf[BroadcastHashJoin]).isDefined)
    assert(
