...
private[sql] trait SQLTestData {
  ...
  protected lazy val emptyTestData: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(Seq.empty[Int].map(i => TestData(i, i.toString))).toDF()
  ...
  protected lazy val testData2: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      TestData2(1, 1) :: TestData2(1, 2) :: TestData2(2, 1) ::
        TestData2(2, 2) :: TestData2(3, 1) :: TestData2(3, 2) :: Nil,
      2).toDF()
  ...
  protected lazy val testData3: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(TestData3(1, None) :: TestData3(2, Some(2)) :: Nil).toDF()
  ...
  protected lazy val largeAndSmallInts: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      LargeAndSmallInts(2147483644, 1) :: LargeAndSmallInts(1, 2) ::
        LargeAndSmallInts(2147483645, 1) :: LargeAndSmallInts(2, 2) ::
        LargeAndSmallInts(2147483646, 1) :: LargeAndSmallInts(3, 2) :: Nil)
      .toDF()
  ...
  protected lazy val decimalData: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      DecimalData(1, 1) :: DecimalData(1, 2) :: DecimalData(2, 1) ::
        DecimalData(2, 2) :: DecimalData(3, 1) :: DecimalData(3, 2) :: Nil)
      .toDF()
  ...
  protected lazy val upperCaseData: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      UpperCaseData(1, "A") :: UpperCaseData(2, "B") :: UpperCaseData(3, "C") ::
        UpperCaseData(4, "D") :: UpperCaseData(5, "E") ::
  ...
  protected lazy val lowerCaseData: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      LowerCaseData(1, "a") :: LowerCaseData(2, "b") :: LowerCaseData(3, "c") ::
  ...
  protected lazy val mapData: RDD[MapData] = {
    val rdd = sqlContext.sparkContext.parallelize(
      MapData(Map(1 -> "a1", 2 -> "b1", 3 -> "c1", 4 -> "d1", 5 -> "e1")) ::
        ...
        MapData(Map(1 -> "a4", 2 -> "b4")) :: MapData(Map(1 -> "a5")) :: Nil)
  ...
  protected lazy val repeatedData: RDD[StringData] = {
    val rdd = sqlContext.sparkContext
      .parallelize(List.fill(2)(StringData("test")))
  ...
  protected lazy val nullableRepeatedData: RDD[StringData] = {
    val rdd = sqlContext.sparkContext.parallelize(
      List.fill(2)(StringData(null)) ++ List.fill(2)(StringData("test")))
  ...
  protected lazy val nullInts: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      NullInts(1) :: NullInts(2) :: NullInts(3) :: NullInts(null) :: Nil).toDF()
  ...
  protected lazy val allNulls: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      NullInts(null) :: NullInts(null) :: NullInts(null) :: NullInts(null) ::
        Nil).toDF()
  ...
  protected lazy val nullStrings: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      NullStrings(1, "abc") :: NullStrings(2, "ABC") :: NullStrings(3, null) ::
        Nil).toDF()
  ...
  protected lazy val tableName: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(TableName("test") :: Nil)
      .toDF()
  ...
  protected lazy val unparsedStrings: RDD[String] = {
    sqlContext.sparkContext.parallelize(
      "1, A1, true, null" :: "2, B2, false, null" :: "3, C3, true, null" ::
  ...
  protected lazy val person: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(Person(0, "mike", 30) :: Person(1, "jim", 20) :: Nil).toDF()
  ...
  protected lazy val salary: DataFrame = {
    val df = sqlContext.sparkContext
      .parallelize(Salary(0, 2000.0) :: Salary(1, 1000.0) :: Nil).toDF()
  ...
  protected lazy val courseSales: DataFrame = {
    val df = sqlContext.sparkContext.parallelize(
      CourseSales("dotNET", 2012, 10000) :: CourseSales("Java", 2012, 20000) ::
