...
class DataFrameNaFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("drop") {
    ...
    checkAnswer(input.na.drop("age" :: "height" :: Nil), rows(0) :: Nil)

    checkAnswer(input.na.drop(), rows(0))
  ...
  test("fill") {
    ...
    checkAnswer(
      fillNumeric,
      Row("Bob", 16, 176.5) :: Row("Alice", 50, 164.3) :: Row("David", 60, 50.6) :: Row(
        "Nina",
        25,
    ...
    checkAnswer(
      ...
      Row("Bob") :: Row("Alice") :: Row("David") :: Row("Nina") :: Row("Amy") :: Row(
        "unknown") :: Nil)
    ...
    checkAnswer(
      ...
      Row("Bob", 16) :: Row("Alice", 50) :: Row("David", 60) :: Row("Nina", 25) :: Row(
        "Amy",
        50) :: Row(null, 50) :: Nil)
