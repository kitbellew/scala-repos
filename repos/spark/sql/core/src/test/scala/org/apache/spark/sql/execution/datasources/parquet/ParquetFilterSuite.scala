...
class ParquetFilterSuite
  ...
  test("filter pushdown - boolean") {
    withParquetDataFrame(
      (true :: false :: Nil).map(b => Tuple1.apply(Option(b)))) { implicit df =>
  ...
  test(
    ...
    withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      withTempPath { dir =>
        ...
        // "part" is not a valid column in the actual Parquet file
        checkAnswer(
          sqlContext.read.parquet(dir.getCanonicalPath).filter(
            "a > 0 and (part = 0 or a > 1)"),
  ...
  test(
    ...
    withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      withTempPath { dir =>
        ...
        (1 to 3).map(i => (i, i + 1, i + 2, i + 3)).toDF(
          "a",
          "b",
          "c",
          "d").write.partitionBy("a").parquet(path)
  ...
  test("SPARK-12231: test the new projection in partitioned DataSource scan") {
    ...
    withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      withTempPath { dir =>
        ...
        checkAnswer(
          df1.filter("a > 1 or b > 2").orderBy("a").selectExpr(
            "a",
            "b",
            "c",
            "d"),
  ...
  test(
    ...
    withSQLConf(
      ...
      withTempPath { dir =>
        ...
        val df = sqlContext.read.parquet(pathOne, pathTwo).filter(
          "c = 1").selectExpr("c", "b", "a")
        checkAnswer(df, Row(1, "1", null))

        // The fields "a" and "c" only exist in one Parquet file.
        assert(
          df.schema("a").metadata.getBoolean(
            StructType.metadataKeyForOptionalField))
        assert(
          df.schema("c").metadata.getBoolean(
            StructType.metadataKeyForOptionalField))
        ...
        // Parquet emits.
        val dfStruct3 =
          sqlContext.read.parquet(pathFour, pathFive).filter("s.c = 1")
            .selectExpr("s")
        ...
        (1 to 3).map(i => (i, i.toString)).toDF("a", "b").write.parquet(
          pathSeven)
        val pathEight = s"${dir.getCanonicalPath}/table8"
        (4 to 6).map(i => (i, i.toString)).toDF("a", "b").write.parquet(
          pathEight)

        val df2 = sqlContext.read.parquet(pathSeven, pathEight).filter(
          "a = 1").selectExpr("a", "b")
        checkAnswer(df2, Row(1, "1"))

        // The fields "a" and "b" exist in both two Parquet files. No metadata is set.
        assert(
          !df2.schema("a").metadata.contains(
            StructType.metadataKeyForOptionalField))
        assert(
          !df2.schema("b").metadata.contains(
            StructType.metadataKeyForOptionalField))
      }
    }
  }
  ...
  test("SPARK-12218: 'Not' is included in Parquet filter pushdown") {
    ...
    withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      withTempPath { dir =>
        ...
        (1 to 5).map(i => (i, (i % 2).toString)).toDF("a", "b").write.parquet(
          path)
  ...
  test("SPARK-11164: test the parquet filter in") {
    ...
    withSQLConf(SQLConf.PARQUET_FILTER_PUSHDOWN_ENABLED.key -> "true") {
      withSQLConf(SQLConf.PARQUET_VECTORIZED_READER_ENABLED.key -> "false") {
        withTempPath { dir =>
          val path = s"${dir.getCanonicalPath}/table1"
          (1 to 5).map(i => (i.toFloat, i % 3)).toDF("a", "b").write.parquet(
            path)
