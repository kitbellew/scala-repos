...
class SQLMetricsSuite extends SparkFunSuite with SharedSQLContext {
  ...
  test("Filter metrics") {
    ...
    testSparkPlanMetrics(
      df,
      1,
      Map(0L -> ("Filter", Map("number of output rows" -> 1L))))
  }
  ...
  test("SortMergeJoin metrics") {
    ...
    withTempTable("testDataForJoin") {
      ...
      val df =
        sqlContext.sql("SELECT * FROM testData2 JOIN testDataForJoin ON testData2.a = testDataForJoin.a")
      testSparkPlanMetrics(
        df,
        1,
        Map(
  ...
  test("SortMergeJoin(outer) metrics") {
    ...
    withTempTable("testDataForJoin") {
      ...
      // ... -> SortMergeJoin(nodeId = 1) -> TungstenProject(nodeId = 0)
      val df =
        sqlContext.sql("SELECT * FROM testData2 left JOIN testDataForJoin ON testData2.a = testDataForJoin.a")
      testSparkPlanMetrics(
        df,
        1,
        Map(
      ...
      val df2 =
        sqlContext.sql("SELECT * FROM testDataForJoin right JOIN testData2 ON testData2.a = testDataForJoin.a")
      testSparkPlanMetrics(
        df2,
        1,
        Map(
  ...
  test("BroadcastHashJoin metrics") {
    ...
    testSparkPlanMetrics(
      df,
      2,
      Map(1L -> ("BroadcastHashJoin", Map("number of output rows" -> 2L))))
  }
  ...
  test("BroadcastHashJoin(outer) metrics") {
    ...
    testSparkPlanMetrics(
      df,
      2,
      Map(0L -> ("BroadcastHashJoin", Map("number of output rows" -> 5L))))

    val df3 = df1.join(broadcast(df2), $"key" === $"key2", "right_outer")
    testSparkPlanMetrics(
      df3,
      2,
      Map(0L -> ("BroadcastHashJoin", Map("number of output rows" -> 6L))))
  }
  ...
  test("ShuffledHashJoin metrics") {
    withSQLConf(SQLConf.AUTO_BROADCASTJOIN_THRESHOLD.key -> "0") {
      ...
      testSparkPlanMetrics(
        df,
        1,
        Map(0L -> ("ShuffledHashJoin", Map("number of output rows" -> 2L))))
    }
  }
  ...
  test("CartesianProduct metrics") {
    ...
    withTempTable("testDataForJoin") {
      ...
      val df = sqlContext.sql("SELECT * FROM testData2 JOIN testDataForJoin")
      testSparkPlanMetrics(
        df,
        1,
