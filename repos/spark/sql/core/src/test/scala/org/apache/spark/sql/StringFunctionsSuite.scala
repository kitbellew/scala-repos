...
class StringFunctionsSuite extends QueryTest with SharedSQLContext {
  ...
  test("string locate function") {
    ...
    checkAnswer(df.select(locate("aa", $"a"), locate("aa", $"a", 1)), Row(1, 2))
  ...
  test("string / binary length function") {
    ...
    checkAnswer(df.selectExpr("length(a)", "length(b)"), Row(3, 4))
  ...
  test("initcap function") {
    ...
    checkAnswer(df.select(initcap($"l"), initcap($"r")), Row("Ab", "A B"))

    checkAnswer(df.selectExpr("InitCap(l)", "InitCap(r)"), Row("Ab", "A B"))
  }
