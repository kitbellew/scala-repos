...
// to the writer.
private[parquet] class TestGroupWriteSupport(schema: MessageType)
...
class ParquetIOSuite extends QueryTest with ParquetTest with SharedSQLContext {
  ...
  test("date type") {
    def makeDateRDD(): DataFrame =
      sparkContext.parallelize(0 to 1000)
        .map(i => Tuple1(DateTimeUtils.toJavaDate(i))).toDF().select($"_1")
  ...
  testStandardAndLegacyModes("struct") {
    ...
    withParquetDataFrame(data) { df =>
      ...
      checkAnswer(
        df,
        data.map {
          case Tuple1(struct) => Row(Row(struct.productIterator.toSeq: _*))
        })
    }
  }
  ...
  test(
    ...
    withTempPath { location =>
      ...
      val conf = sparkContext.hadoopConfiguration
      writeMetadata(parquetSchema, path, conf)
      val sparkTypes = sqlContext.read.parquet(path.toString).schema
        .map(_.dataType)
      assert(sparkTypes === expectedSparkTypes)
    }
  }
  ...
  test("compression codec") {
    def compressionCodecFor(path: String, codecName: String): String = {
      val codecs = for {
        footer <-
          readAllFootersWithoutSummaryFiles(new Path(path), hadoopConfiguration)
    ...
    checkCompressionCodec(
      CompressionCodecName.fromConf(sqlContext.conf.parquetCompressionCodec))
  ...
  test("write metadata") {
    withTempPath { file =>
      ...
      val actualSchema = readFooter(path, hadoopConfiguration).getFileMetaData
        .getSchema
  ...
  test("save - throw") {
    ...
    withParquetFile(data) { file =>
      val newData = (11 to 20).map(i => (i, i.toString))
      val errorMessage = intercept[Throwable] {
        newData.toDF().write.format("parquet").mode(SaveMode.ErrorIfExists)
          .save(file)
      }.getMessage
        ...
        assertResult(df.schema) {
          StructType(
            StructField("a", BooleanType, nullable = true) ::
              StructField("b", IntegerType, nullable = true) :: Nil)
        }
      }
    }
  }
    ...
    try {
      ...
      withTempPath { dir =>
        intercept[org.apache.spark.SparkException] {
          sqlContext.sql("select div0(1) as div0").write
  ...
  test("SPARK-11044 Parquet writer version fixed as version1 ") {
    ...
    withTempPath { dir =>
      ...
      try {
        ...
        sqlContext.range(1 << 16).selectExpr("(id % 4) AS i").coalesce(1).write
          .mode("overwrite").parquet(path)

        val blockMetadata = readFooter(new Path(path), hadoopConfiguration)
          .getBlocks.asScala.head
  ...
  test("null and non-null strings") {
    ...
    val data: Dataset[String] = sqlContext.range(200)
  ...
  test("VectorizedParquetRecordReader - direct path read") {
    ...
    withTempPath { dir =>
      sqlContext.createDataFrame(data).repartition(1).write
        .parquet(dir.getCanonicalPath)
