...
class UnsafeRowSuite extends SparkFunSuite {
  ...
  test("writeToStream") {
    ...
    val arrayBackedUnsafeRow: UnsafeRow = UnsafeProjection
      .create(Array[DataType](StringType, StringType, IntegerType)).apply(row)
    ...
    val (bytesFromOffheapRow, field0StringFromOffheapRow): (
        Array[Byte],
        String) = {
      val offheapRowPage = MemoryAllocator.UNSAFE
  ...
  test("calling getDouble() and getFloat() on null columns") {
    ...
    val unsafeRow = UnsafeProjection
      .create(Array[DataType](FloatType, DoubleType)).apply(row)
  ...
  test("calling get(ordinal, datatype) on null columns") {
    ...
    val unsafeRow = UnsafeProjection.create(Array[DataType](NullType))
      .apply(row)
  ...
  test("calling hashCode on unsafe array returned by getArray(ordinal)") {
    ...
    val unsafeRow = UnsafeProjection
      .create(Array[DataType](ArrayType(LongType))).apply(row)
