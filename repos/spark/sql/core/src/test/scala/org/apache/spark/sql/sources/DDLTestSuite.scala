  ...
  override def schema: StructType =
    StructType(Seq(
      StructField(
        ...
        IntegerType,
        nullable = false,
        new MetadataBuilder().putString("comment", s"test comment $table")
          .build()),
      ...
      StructField(
        "structType",
        StructType(
          StructField("f1", StringType) :: StructField("f2", IntegerType) ::
            Nil))
    ))
...
class DDLTestSuite extends DataSourceTest with SharedSQLContext {
  ...
  test(
    ...
    val attributes = sql("describe ddlPeople").queryExecution.executedPlan
      .output
