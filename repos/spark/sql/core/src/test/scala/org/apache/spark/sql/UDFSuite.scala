...
class UDFSuite extends QueryTest with SharedSQLContext {
  ...
  test("SPARK-8005 input_file_name") {
    withTempPath { dir =>
      ...
      sqlContext.read.parquet(dir.getCanonicalPath).registerTempTable(
        "test_table")
      val answer =
        sql("select input_file_name() from test_table").head().getString(0)
      assert(answer.contains(dir.getCanonicalPath))
      assert(sql(
        "select input_file_name() from test_table").distinct().collect().length >= 2)
  ...
  test("udf in different types") {
    ...
    checkAnswer(
      sql(
        "SELECT tmp.t.* FROM (SELECT testDataFunc(key, value) AS t from testData) tmp").toDF(),
      testData)
    checkAnswer(
      sql("""
           | SELECT tmp.t.* FROM
  ...
  test(
    ...
    checkAnswer(
      sql(
        "SELECT tmp.t.* FROM (SELECT testDataFunc(a, b) AS t from testData2) tmp").toDF(),
      testData2)
  }
}
