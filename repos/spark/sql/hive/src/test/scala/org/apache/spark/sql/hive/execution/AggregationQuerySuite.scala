...
class ScalaAggregateFunction(schema: StructType)
...
class ScalaAggregateFunctionWithoutInputSchema
  ...
  def update(buffer: MutableAggregationBuffer, input: Row): Unit =
    buffer.update(
      0,
      input.getAs[Seq[Row]](0).map(_.getAs[Int]("v")).sum +
        buffer.getLong(0))
...
abstract class AggregationQuerySuite
  ...
  test("only do grouping") {
    ...
    checkAnswer(
      sqlContext.sql("""
          ...
          |FROM agg3
        """.stripMargin),
      Row(Seq[Integer](1, 1)) :: Row(Seq[Integer](null)) :: Row(Seq[Integer](1)) :: Row(
        Seq[Integer](2)) :: Row(null) :: Row(Seq[Integer](2, 3)) :: Row(
        Seq[Integer](2, 3, 4)) :: Row(Seq[Integer](3)) :: Nil
    )
  ...
  test("interpreted and expression-based aggregation functions") {
    ...
    checkAnswer(
      sqlContext.sql("""
          ...
          |GROUP BY key
        """.stripMargin),
      Row(64.5, 19.0, 1, 55.5, 20.0) :: Row(5.0, -2.5, 2, -7.0, -0.5) :: Row(
        null,
        null,
        3,
        null,
  ...
  test("single distinct column set") {
    ...
    checkAnswer(
      ...
      Row(120.0, 70.0 / 3.0, -10.0 / 3.0, 1, 67.0 / 3.0 + 100.0, 12.0, 20.0) :: Row(
        ...
        -2.0 / 3.0 + 100.0,
        10.0,
        2.0) :: Row(null, null, 3.0, 3, null, null, null) :: Row(
        110.0,
        10.0,
        20.0,
        null,
    ...
    checkAnswer(
      sqlContext.sql("""
          ...
          |GROUP BY key
        """.stripMargin),
      Row(1, 120.0, -10.0, 40.0, 120.0, 70.0 / 3.0 + 100.0) :: Row(
        2,
        100.0,
        3.0,
        0.0,
        100.0,
        1.0 / 3.0 + 100.0) :: Row(3, null, 3.0, null, null, null) :: Row(
        null,
        110.0,
        60.0,
        30.0,
    ...
    checkAnswer(
      sqlContext.sql("""
          ...
          |GROUP BY key
        """.stripMargin),
      Row(3, 3, 3, 2, 1) :: Row(3, 4, 4, 2, 2) :: Row(0, 2, 2, 0, 3) :: Row(
        3,
        4,
        4,
        3,
  ...
  test("multiple distinct multiple columns sets") {
    checkAnswer(
      ...
      Row(null, 3, 30, 3, 60, 3, -4700, 3, 30, 3, 60, -4700, 4, 4) :: Row(1, 2,
        40, 3, -10, 3, -100, 3, 70, 3, -10, -100, 3, 3) :: Row(2, 2, 0, 1, 1, 1,
        1, 3, 1, 3, 3, 2, 4,
  ...
  test("test count") {
    checkAnswer(
      ...
      Row(1, 10, 1, 1, 1) :: Row(1, -60, 1, 1, null) :: Row(2, 30, 2, 2, 1) :: Row(
        ...
        2,
        2,
        2) :: Row(1, -10, 1, 1, null) :: Row(0, -1, 1, 1, 2) :: Row(
        1,
        null,
        1,
        1,
        2) :: Row(1, 100, 1, 1, null) :: Row(1, null, 2, 2, 3) :: Row(
        0,
        null,
        1,
        1,
  ...
  test("pearson correlation") {
    val df =
      Seq.tabulate(10)(i => (1.0 * i, 2.0 * i, i * -1.0)).toDF("a", "b", "c")
    val corr1 =
      df.repartition(2).groupBy().agg(corr("a", "b")).collect()(0).getDouble(0)
    ...
    val df2 =
      Seq.tabulate(20)(x => (1.0 * x, x * x - 2 * x + 3.5)).toDF("a", "b")
  ...
  test("covariance: covar_pop and covar_samp") {
    ...
    val df =
      Seq.tabulate(20)(x => (1.0 * x, x * x - 2 * x + 3.5)).toDF("a", "b")
    val cov_samp =
      df.groupBy().agg(covar_samp("a", "b")).collect()(0).getDouble(0)
    assert(math.abs(cov_samp - 595.0) < 1e-12)

    val cov_pop =
      df.groupBy().agg(covar_pop("a", "b")).collect()(0).getDouble(0)
    assert(math.abs(cov_pop - 565.25) < 1e-12)

    val df2 = Seq.tabulate(20)(x => (1 * x, x * x * x - 2)).toDF("a", "b")
    val cov_samp2 =
      df2.groupBy().agg(covar_samp("a", "b")).collect()(0).getDouble(0)
    assert(math.abs(cov_samp2 - 11564.0) < 1e-12)

    val cov_pop2 =
      df2.groupBy().agg(covar_pop("a", "b")).collect()(0).getDouble(0)
...
class TungstenAggregationQueryWithControlledFallbackSuite
    extends AggregationQuerySuite {
