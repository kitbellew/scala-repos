...
class SQLQuerySuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  test("UDTF") {
    sql(
      s"ADD JAR ${hiveContext.getHiveFile("TestUDTF.jar").getCanonicalPath()}")
  ...
  test("SPARK-6835: udtf in lateral view") {
    ...
    val query = sql(
      "SELECT c1, v FROM table1 LATERAL VIEW stack(3, 1, c1 + 1, c1 + 2) d AS v")
  ...
  test("show functions") {
    ...
    checkAnswer(
      sql("SHOW functions `sha.*`"),
      Row("sha") :: Row("sha1") :: Row("sha2") :: Nil)
  }
  ...
  test("specifying the column list for CTAS") {
    Seq((1, "111111"), (2, "222222")).toDF("key", "value")
  ...
  test("double nested data") {
    sparkContext.parallelize(Nested1(Nested2(Nested3(1))) :: Nil).toDF()
  ...
  test("SPARK-4825 save join to table") {
    val testData = sparkContext.parallelize(1 to 10)
    ...
    val schema = StructType(
      StructField(
        ...
        StructType(
          StructField(
            "innerStruct",
            StructType(StructField("s1", StringType, true) :: Nil)) ::
  ...
  test("SPARK-4296 Grouping field with Hive UDF as sub expression") {
    ...
    read.json(rdd).registerTempTable("data")
    checkAnswer(
      sql("SELECT year(c) + 1 FROM data GROUP BY year(c) + 1"),
      Row(1971))
  ...
  test("SPARK-5203 union with different decimal precision") {
    Seq.empty[(Decimal, Decimal)].toDF("d1", "d2")
      .select($"d1".cast(DecimalType(10, 5)).as("d")).registerTempTable("dn")

    sql("select d from dn union all select d * 2 from dn").queryExecution
      .analyzed
  }
  ...
  test("Sorting columns are not in Generate") {
    withTempTable("data") {
      sqlContext.range(1, 5).select(
        array($"id", $"id" + 1).as("a"),
        $"id".as("b"),
        (lit(10) - $"id").as("c")).registerTempTable("data")

      // case 1: missing sort columns are resolvable if join is true
      checkAnswer(
        sql(
          "SELECT explode(a) AS val, b FROM data WHERE b < 2 order by val, c"),
        Row(1, 1) :: Row(2, 1) :: Nil)
  ...
  test("window function: multiple window expressions in a single expression") {
    val nums = sparkContext.parallelize(1 to 10).map(x => (x, x % 2))
      .toDF("x", "y")
  ...
  test("SPARK-7269 Check analysis failed in case in-sensitive") {
    Seq(1, 2, 3).map { i => (i.toString, i.toString) }.toDF("key", "value")
  ...
  test("Cast STRING to BIGINT") {
    checkAnswer(
      sql("SELECT CAST('775983671874188101' as BIGINT)"),
  ...
  test("Convert hive interval term into Literal of CalendarIntervalType") {
    checkAnswer(
      sql("select interval '10-9' year to month"),
  ...
  test(
    ...
    sqlContext.range(5).selectExpr("id AS a", "id AS b")
  ...
  test("run sql directly on files") {
    ...
    withTempPath(f => {
      ...
      checkAnswer(
        sql(
          s"select id from `org.apache.spark.sql.parquet`.`${f.getCanonicalPath}`"),
        df)
      checkAnswer(
  ...
  test("correctly parse CREATE VIEW statement") {
    withSQLConf(SQLConf.NATIVE_VIEW.key -> "true") {
      withTable("jt") {
        ...
        checkAnswer(
          sql("SELECT c1, c2 FROM testView ORDER BY c1"),
  ...
  test("correctly handle CREATE VIEW IF NOT EXISTS") {
    withSQLConf(SQLConf.NATIVE_VIEW.key -> "true") {
      withTable("jt", "jt2") {
        ...
        checkAnswer(
          sql("SELECT * FROM testView ORDER BY id"),
          (1 to 9).map(i => Row(i)))
        sql("DROP VIEW testView")
      }
    }
    ...
    test(s"$prefix correctly handle CREATE OR REPLACE VIEW") {
      withSQLConf(
        ...
        withTable("jt", "jt2") {
          ...
          checkAnswer(
            sql("SELECT * FROM testView ORDER BY i"),
            (1 to 9).map(i => Row(i, i)))
  ...
  test("CTE within view") {
    withSQLConf(
      SQLConf.NATIVE_VIEW.key -> "true",
      SQLConf.CANONICAL_NATIVE_VIEW.key -> "true") {
      withView("cte_view") {
      ...
      withView("v") {
        ...
        withTempDatabase { db =>
          activateDatabase(db) {
            ...
            checkAnswer(
              sql("SELECT * FROM default.v"),
              sql("SELECT * FROM default.src"))
    ...
    withSQLConf(
      ...
      withTable("add_col") {
        ...
        withView("v") {
          ...
          sqlContext.range(10).select('id, 'id as 'a).write.mode("overwrite")
            .saveAsTable("add_col")
  ...
  test("create hive view for joined tables") {
    ...
    withSQLConf(SQLConf.NATIVE_VIEW.key -> "true") {
      withTable("jt1", "jt2") {
        sqlContext.range(1, 10).toDF("id1").write.format("json")
          .saveAsTable("jt1")
        sqlContext.range(1, 10).toDF("id2").write.format("json")
          .saveAsTable("jt2")
  ...
  test("SPARK-11453: append data to partitioned table") {
    withTable("tbl11453") {
      Seq("1" -> "10", "2" -> "20").toDF("i", "j").write.partitionBy("i")
        .saveAsTable("tbl11453")

      Seq("3" -> "30").toDF("i", "j").write.mode(SaveMode.Append)
      ...
      Seq("4" -> "40").toDF("i", "j").write.mode(SaveMode.Append)
        .partitionBy("I").saveAsTable("tbl11453")
      checkAnswer(
        sqlContext.read.table("tbl11453").select("i", "j").orderBy("i"),
  ...
  test("multi-insert with lateral view") {
    withTempTable("t1") {
      sqlContext.range(10).select(array($"id", $"id" + 1).as("arr"), $"id")
