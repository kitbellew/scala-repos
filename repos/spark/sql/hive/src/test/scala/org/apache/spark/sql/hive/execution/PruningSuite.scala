...
class PruningSuite extends HiveComparisonTest with BeforeAndAfter {
  ...
  createPruningTest(
    "Column pruning - with partitioned table",
    "SELECT key FROM srcpart WHERE ds = '2008-04-08' LIMIT 3",
    Seq("key"),
    Seq("key"),
    Seq(Seq("2008-04-08", "11"), Seq("2008-04-08", "12"))
  )

  createPruningTest(
    "Column pruning - with non-partitioned table",
  ...
  createPruningTest(
    "Column pruning - without filters",
  ...
  createPruningTest(
    "Partition pruning - non-partitioned, non-trivial project",
  ...
  createPruningTest(
    "Partition pruning - non-partitioned table",
  ...
  createPruningTest(
    "Partition pruning - with filter on string partition key",
  ...
  createPruningTest(
    "Partition pruning - with filter on int partition key",
  ...
  def createPruningTest(
    ...
    test(s"$testCaseName - pruning test") {
      ...
      val (actualScannedColumns, actualPartValues) =
        plan
          .collect {
      ...
      assert(
        ...
        "Partitions selected do not match")
    }
