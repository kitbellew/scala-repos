...
class StatisticsSuite extends QueryTest with TestHiveSingleton {
  ...
  test("analyze MetastoreRelations") {
    ...
    assert(
      queryTotalSize("analyzeTable_part") === hiveContext
        .conf
        .defaultSizeInBytes)
  ...
  test("auto converts to broadcast hash join, by size estimate of a relation") {
    def mkTest(
      ...
      assert(
        sizes
          .size === 2 && sizes(0) <= hiveContext.conf.autoBroadcastJoinThreshold
  ...
  test(
    ...
    val sizes = df
      ...
      .collect {
        case r
            if implicitly[ClassTag[MetastoreRelation]]
              .runtimeClass
    ...
    assert(
      sizes
        .size === 2 && sizes(1) <= hiveContext.conf.autoBroadcastJoinThreshold
