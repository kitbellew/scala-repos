...
class HiveQuerySuite extends HiveComparisonTest with BeforeAndAfter {
  ...
  test("SPARK-7270: consider dynamic partition when comparing table output") {
    ...
    assertResult(false, "Incorrect cast detected\n" + analyzedPlan) {
      ...
      analyzedPlan.collect {
        case p: Project => p.transformExpressionsUp {
  ...
  test("case sensitivity: registered table") {
    val testData = TestHive.sparkContext
  ...
  test("Query Hive native command execution result") {
    ...
    assert(
      isExplanation(sql(s"EXPLAIN SELECT key, COUNT(*) FROM src GROUP BY key")))
  ...
  test("use database") {
    ...
    assert(
      currentDatabase == sql("select current_database()").first().getString(0))
  }
