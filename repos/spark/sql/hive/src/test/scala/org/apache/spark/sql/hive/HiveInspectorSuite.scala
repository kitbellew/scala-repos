...
class HiveInspectorSuite extends SparkFunSuite with HiveInspectors {
  ...
  val data =
    Literal(true) :: Literal(0.asInstanceOf[Byte]) :: Literal(0.asInstanceOf[
      Short]) :: Literal(0) :: Literal(0.asInstanceOf[Long]) :: Literal(0.asInstanceOf[
      Float]) :: Literal(0.asInstanceOf[Double]) :: Literal("0") :: Literal(
      java.sql.Date.valueOf("2014-09-23")) :: Literal(Decimal(BigDecimal(
      123.123))) :: Literal(new java.sql.Timestamp(123123)) :: Literal(
      ...
      Row(1, 2.0d, 3.0f),
      StructType(
        StructField("c1", IntegerType) :: StructField("c2", DoubleType) :: StructField(
          "c3",
          FloatType) :: Nil)) :: Nil
  ...
  test("oi => datatype => oi") {
    ...
    checkDataType(
      dataTypes.map(toWritableInspector).map(inspectorToDataType),
      dataTypes)
  }
  ...
  test("wrap / unwrap Array Type") {
    ...
    checkValue(
      d,
      unwrap(
        wrap(d, toInspector(Literal.create(d, dt)), dt),
        toInspector(Literal.create(d, dt))))
    checkValue(
      d,
      unwrap(
        wrap(null, toInspector(Literal.create(d, dt)), dt),
        toInspector(Literal.create(d, dt))))
  }
