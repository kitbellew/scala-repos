...
class ParquetMetastoreSuite extends ParquetPartitioningTest {
  ...
  override def beforeAll(): Unit = {
    ...
    (1 to 10).map(i => Tuple1(Seq(new Integer(i), null))).toDF(
      "a").registerTempTable("jt_array")
...
class ParquetSourceSuite extends ParquetPartitioningTest {
  ...
  test("SPARK-6016 make sure to use the latest footers") {
    ...
    df1.write.mode(SaveMode.Overwrite).format("parquet").saveAsTable(
      "spark_6016_fix")
    checkAnswer(
    ...
    df2.write.mode(SaveMode.Overwrite).format("parquet").saveAsTable(
      "spark_6016_fix")
  ...
  test("SPARK-8811: compatibility with array of struct in Hive") {
    withTempPath { dir =>
      ...
      withTable("array_of_struct") {
        ...
        withSQLConf(conf: _*) {
          sql(s"""CREATE TABLE array_of_struct
  ...
  test(
    ...
    val df2 = df.as('x).join(df.as('y), $"x.str" === $"y.str").groupBy(
      "y.str").max("y.int")
