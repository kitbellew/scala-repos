...
private[client] class Shim_v0_12 extends Shim with Logging {
  ...
  private lazy val startMethod = findStaticMethod(
    ...
    "start",
    classOf[SessionState])
  private lazy val getDataLocationMethod = findMethod(
    classOf[Table],
    "getDataLocation")
  private lazy val setDataLocationMethod = findMethod(
    classOf[Table],
    "setDataLocation",
    classOf[URI])
  private lazy val getAllPartitionsMethod = findMethod(
    classOf[Hive],
  ...
  private lazy val getCommandProcessorMethod = findStaticMethod(
    ...
    classOf[String],
    classOf[HiveConf])
  private lazy val getDriverResultsMethod = findMethod(
    classOf[Driver],
    "getResults",
  ...
  override def getCommandProcessor(
    ...
    getCommandProcessorMethod.invoke(null, token, conf)
...
private[client] class Shim_v0_13 extends Shim_v0_12 {
  ...
  def convertFilters(table: Table, filters: Seq[Expression]): String = {
    ...
    val varcharKeys = table.getPartitionKeys.asScala.filter(col =>
    ...
    filters.collect {
      ...
      case op @ BinaryComparison(a: Attribute, Literal(v, _: StringType))
          if !varcharKeys.contains(a.name) => s"""${a.name} ${op.symbol} "$v""""
      case op @ BinaryComparison(Literal(v, _: StringType), a: Attribute)
          if !varcharKeys.contains(a.name) => s""""$v" ${op.symbol} ${a.name}"""
    }.mkString(" and ")
  }
  ...
  override def getCommandProcessor(
      token: String,
      conf: HiveConf): CommandProcessor =
    getCommandProcessorMethod.invoke(null, Array(token), conf)
      .asInstanceOf[CommandProcessor]
