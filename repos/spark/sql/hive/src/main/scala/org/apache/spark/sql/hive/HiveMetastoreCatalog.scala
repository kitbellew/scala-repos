...
private[hive] class HiveMetastoreCatalog(
  ...
  protected[hive] val cachedDataSourceTables
    ...
    val cacheLoader =
      new CacheLoader[QualifiedTableName, LogicalPlan]() {
        override def load(in: QualifiedTableName): LogicalPlan = {
          ...
          def schemaStringFromParts: Option[String] = {
            table
              .properties
              .get("spark.sql.sources.schema.numParts")
              .map { numParts =>
          ...
          val bucketSpec = table
            .properties
            .get("spark.sql.sources.schema.numBuckets")
            .map { n =>
              BucketSpec(
                n.toInt,
  ...
  private def convertToParquetRelation(
    ...
    val result =
      if (metastoreRelation.hiveQlTable.isPartitioned) {
        ...
        // are empty.
        val partitions = metastoreRelation
          .getHiveQlPartitions()
          .map { p =>
    ...
    if (partLocations.nonEmpty) {
      ...
      Array(
        table
          ...
          .getOrElse(
            sys
              .error(s"Could not get the location of ${table.qualifiedName}.")))
    }
  }
