...
private[hive] class HadoopTableReader(
  ...
  private val _broadcastedHiveConf = sc
    .sparkContext
  ...
  def makeRDDForPartitionedTable(
    ...
    val hivePartitionRDDs =
      verifyPartitionPath(partitionToDeserializer)
        .map {
          case (partition, partDeserializer) =>
            ...
            val (partitionKeyAttrs, nonPartitionKeyAttrs) = attributes
              .zipWithIndex
              .partition {
                case (attr, _) =>
            ...
            createHadoopRdd(tableDesc, inputPathStr, ifc).mapPartitions {
              iter =>
                val hconf = broadcastedHiveConf.value.value
