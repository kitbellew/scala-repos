...
class SQLBuilder(logicalPlan: LogicalPlan, sqlContext: SQLContext)
  ...
  def toSQL: String = {
    ...
    // present.
    val finalName =
      if (qualifiers.length == 1) { qualifiers.head }
  ...
  private def groupingSetToSQL(
    ...
    val groupByAttributes = agg.groupingExpressions.dropRight(1)
    ...
    val groupByExprs = project.projectList.drop(numOriginalOutput)
  ...
  object Canonicalizer extends RuleExecutor[LogicalPlan] {
    ...
    object RemoveSubqueriesAboveSQLTable extends Rule[LogicalPlan] {
      override def apply(plan: LogicalPlan): LogicalPlan =
        plan transformUp { case SubqueryAlias(_, t @ ExtractSQLTable(_)) => t }
    }
