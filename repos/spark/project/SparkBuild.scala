...
object BuildCommons {
  ...
  val sqlProjects @ Seq(catalyst, sql, hive, hiveThriftServer) = Seq(
    ...
    "sql",
    "hive",
    "hive-thriftserver").map(ProjectRef(buildLocation, _))

  val streamingProjects @ Seq(streaming, streamingKafka) = Seq(
    "streaming",
    "streaming-kafka").map(ProjectRef(buildLocation, _))
      ...
      Def
        .task {
          ...
          grammarFileNames
            ...
            .foreach {
              gFilePath =>
                ...
                log.info(
                  "ANTLR: Grammar file '%s' detected.".format(relGFilePath))
          ...
          if (errorState.errors > 0) {
            sys
              .error("ANTLR: Caught %d build errors.".format(errorState.errors))
...
object Hive {
  ...
  lazy val settings = Seq(
    ...
    javaOptions in Test :=
      (javaOptions in Test).value.filterNot(_ == "-ea"),
...
object Assembly {
  ...
  lazy val settings = assemblySettings ++
    Seq(
      test in assembly := {},
      hadoopVersion := {
        sys
      ...
      deployDatanucleusJars := {
        ...
        var libManagedJars =
          new File(BuildCommons.sparkHome, "lib_managed/jars")
...
object Unidoc {
  ...
  lazy val settings = scalaJavaUnidocSettings ++
    Seq(
      publish := {},
      ...
      javacOptions in doc :=
        Seq(
          ...
          "-group",
          "Core Java API",
          packageList("api.java", "api.java.function"),
          "-group",
          "Spark Streaming",
          packageList(
            "streaming.api.java",
...
object CopyDependencies {
  ...
  val copyDeps = TaskKey[Unit](
    "copyDeps",
    "Copies needed dependencies to the build directory.")
  ...
  lazy val settings = Seq(
    ...
    packageBin in Compile <<=
      (packageBin in Compile).dependsOn(copyDeps)
  )
