...
private[graphx] class EdgePartition[
    @specialized(Char, Int, Boolean, Byte, Long, Float, Double) ED: ClassTag,
    VD: ClassTag](
  ...
  @inline private def srcIds(pos: Int): VertexId = local2global(
    localSrcIds(pos))

  @inline private def dstIds(pos: Int): VertexId = local2global(
    localDstIds(pos))
