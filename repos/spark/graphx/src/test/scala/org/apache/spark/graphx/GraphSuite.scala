...
class GraphSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("triplets") {
    withSpark { sc =>
      ...
      assert(
        star.triplets.map(et =>
          (et.srcId, et.dstId, et.srcAttr, et.dstAttr)).collect().toSet
  ...
  test("partitionBy") {
    withSpark { sc =>
      ...
      val graph = Graph.fromEdgeTuples(
        sc.parallelize(
          verts.flatMap(x =>
            verts.withFilter(y => y % x == 0).map(y =>
              (x: VertexId, y: VertexId))),
          p),
        0)
      ...
      val partitionSets = partitionedGraph.edges.partitionsRDD.mapPartitions {
        iter =>
      ...
      if (!verts.forall(id => partitionSets.count(_.contains(id)) <= bound)) {
        val numFailures =
          verts.count(id => partitionSets.count(_.contains(id)) > bound)
      ...
      val partitionSetsUnpartitioned = graph.edges.partitionsRDD.mapPartitions {
        iter =>
          val part = iter.next()._2
          Iterator(
            (part.iterator.flatMap(e => Iterator(e.srcId, e.dstId))).toSet)
      ...
      assert(
        g.triplets.collect().map(_.toTuple).toSet ===
          Set(((0L, "a"), (1L, "b"), 1), ((0L, "a"), (2L, "c"), 1)))
      val gPart = g.partitionBy(EdgePartition2D)
      assert(
        gPart.triplets.collect().map(_.toTuple).toSet ===
          Set(((0L, "a"), (1L, "b"), 1), ((0L, "a"), (2L, "c"), 1)))
    }
  }
  ...
  test("mapVertices") {
    withSpark { sc =>
      ...
      assert(mappedVAttrs.vertices.collect().toSet === (0 to n).map(x =>
        (x: VertexId, "v2")).toSet)
      // mapVertices changing type
      val mappedVAttrs2 = star.mapVertices((vid, attr) => attr.length)
      assert(mappedVAttrs2.vertices.collect().toSet === (0 to n).map(x =>
        (x: VertexId, 1)).toSet)
    }
  }
  ...
  test("mapTriplets") {
    withSpark { sc =>
      ...
      assert(
        star.mapTriplets(et =>
          et.srcAttr + et.dstAttr).edges.collect().toSet ===
  ...
  test("reverse") {
    withSpark { sc =>
      val n = 5
      val star = starGraph(sc, n)
      assert(star.reverse.outDegrees.collect().toSet === (1 to n).map(x =>
        (x: VertexId, 1)).toSet)
    }
  }
  ...
  test("subgraph") {
    withSpark { sc =>
      ...
      assert(subgraph.vertices.collect().toSet === (0 to n by 2).map(x =>
        (x, "v")).toSet)
  ...
  test("mask") {
    withSpark { sc =>
      ...
      val e = projectedGraph.edges.map(e =>
  ...
  test("groupEdges") {
    withSpark { sc =>
      ...
      assert(
        star2.edges.collect().toArray.sorted(
          Edge.lexicographicOrdering[Int]) ===
  ...
  test("outerJoinVertices") {
    withSpark { sc =>
      ...
      val newReverseStar =
        reverseStar.outerJoinVertices(messages) { (vid, a, bOpt) =>
          a + bOpt.getOrElse("")
        }
      assert(
  ...
  test("more edge partitions than vertex partitions") {
    withSpark { sc =>
      ...
      val triplets =
        graph.triplets.map(et => (et.srcId, et.dstId, et.srcAttr, et.dstAttr))
          .collect().toSet
      assert(
        triplets ===
          Set(
  ...
  test("unpersist graph RDD") {
    withSpark { sc =>
      ...
      val edges =
        sc.parallelize(List(Edge[Long](1L, 2L), Edge[Long](1L, 3L)), 1)
