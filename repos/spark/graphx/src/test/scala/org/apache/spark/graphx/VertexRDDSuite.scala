...
class VertexRDDSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("mapValues") {
    withSpark { sc =>
      ...
      val negatives =
        verts.mapValues(x =>
          -x).cache() // Allow joining b with a derived RDD of b
  ...
  test("minus") {
    withSpark { sc =>
      val vertexA =
        VertexRDD(sc.parallelize(0 until 75, 2).map(i => (i.toLong, 0))).cache()
  ...
  test("minus with non-equal number of partitions") {
    withSpark { sc =>
      val vertexA =
        VertexRDD(sc.parallelize(0 until 75, 5).map(i => (i.toLong, 0)))
      val vertexB =
        VertexRDD(sc.parallelize(50 until 100, 2).map(i => (i.toLong, 1)))
  ...
  test("diff") {
    withSpark { sc =>
      ...
      assert(
        verts.diff(flipEvens).map(_._2).collect().toSet === (2 to n by 2).map(
          -_).toSet)
    ...
    withSpark { sc =>
      ...
      val flipEvens: RDD[(VertexId, Int)] =
        sc.parallelize(0L to 100L)
          .map(id =>
            if (id % 2 == 0) (id, -id.toInt) else (id, id.toInt)).cache()
      // diff should keep only the changed vertices
      assert(
        verts.diff(flipEvens).map(_._2).collect().toSet === (2 to n by 2).map(
          -_).toSet)
  ...
  test("diff vertices with non-equal number of partitions") {
    withSpark { sc =>
      val vertexA =
        VertexRDD(sc.parallelize(0 until 24, 3).map(i => (i.toLong, 0)))
      val vertexB =
        VertexRDD(sc.parallelize(8 until 16, 2).map(i => (i.toLong, 1)))
      assert(vertexA.partitions.size != vertexB.partitions.size)
  ...
  test("leftJoin") {
    withSpark { sc =>
      ...
      assert(
        verts.leftJoin(evens) { (id, a, bOpt) =>
          a - bOpt.getOrElse(0)
        }.collect().toSet ===
          (0 to n by 2).map(x => (x.toLong, 0)).toSet ++ (1 to n by 2).map(x =>
            (x.toLong, x)).toSet)
      // leftJoin with an RDD
      val evensRDD = evens.map(identity)
      assert(
        verts.leftJoin(evensRDD) { (id, a, bOpt) =>
          a - bOpt.getOrElse(0)
        }.collect().toSet ===
          (0 to n by 2).map(x => (x.toLong, 0)).toSet ++ (1 to n by 2).map(x =>
            (x.toLong, x)).toSet)
  ...
  test("leftJoin vertices with non-equal number of partitions") {
    withSpark { sc =>
      val vertexA =
        VertexRDD(sc.parallelize(0 until 100, 2).map(i => (i.toLong, 1)))
      val vertexB = VertexRDD(
      ...
      assert(
        vertexC.filter(v => v._2 != 0).map(
          _._1).collect().toSet == (1 to 99 by 2).toSet)
    }
  }
  ...
  test("innerJoin vertices with the non-equal number of partitions") {
    withSpark { sc =>
      ...
      assert(
        vertexC.filter(v => v._2 == 0).map(
          _._1).collect().toSet == (0 to 98 by 2).toSet)
    }
  }
