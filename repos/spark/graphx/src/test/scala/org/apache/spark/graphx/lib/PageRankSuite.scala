...
class PageRankSuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("Star PersonalPageRank") {
    withSpark { sc =>
      ...
      val staticRanks1 =
        starGraph.staticPersonalizedPageRank(0, numIter = 1, resetProb).vertices
      val staticRanks2 =
        starGraph.staticPersonalizedPageRank(0, numIter = 2, resetProb)
          .vertices.cache()
      ...
      val otherStaticRanks2 =
        starGraph.staticPersonalizedPageRank(1, numIter = 2, resetProb)
          .vertices.cache()
