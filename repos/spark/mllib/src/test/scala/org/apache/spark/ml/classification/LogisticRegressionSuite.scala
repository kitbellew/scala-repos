...
class LogisticRegressionSuite
  ...
  test("logistic regression: default params") {
    ...
    model.transform(dataset)
      .select("label", "probability", "prediction", "rawPrediction").collect()
  ...
  test("empty probabilityCol") {
    ...
    assert((dataset.schema.fieldNames.toSet).subsetOf(fieldNames.toSet))
  ...
  test("logistic regression with setters") {
    ...
    val lr = new LogisticRegression().setMaxIter(10).setRegParam(1.0)
      .setThreshold(0.6).setProbabilityCol("myProbability")
    val model = lr.fit(dataset)
    ...
    val predAllZero = model.transform(dataset)
      .select("prediction", "myProbability").collect().map {
    ...
    val predNotAllZero = model.transform(
      dataset,
      model.threshold -> 0.0,
      model.probabilityCol -> "myProb").select("prediction", "myProb").collect()
  ...
  test("binary logistic regression with weighted samples") {
    ...
    val trainer1b = (new LogisticRegression).setFitIntercept(true)
  ...
  test("logistic regression with all labels the same") {
    val sameLabels = dataset.withColumn("zeroLabel", lit(0.0))
      .withColumn("oneLabel", lit(1.0))

    // fitIntercept=true
    val lrIntercept = new LogisticRegression().setFitIntercept(true)
      .setMaxIter(3)

    val allZeroInterceptModel = lrIntercept.setLabelCol("zeroLabel")
      .fit(sameLabels)
    assert(
    ...
    val allOneInterceptModel = lrIntercept.setLabelCol("oneLabel")
      .fit(sameLabels)
    ...
    // fitIntercept=false
    val lrNoIntercept = new LogisticRegression().setFitIntercept(false)
      .setMaxIter(3)

    val allZeroNoInterceptModel = lrNoIntercept.setLabelCol("zeroLabel")
      .fit(sameLabels)
    assert(allZeroNoInterceptModel.intercept === 0.0)
    assert(allZeroNoInterceptModel.summary.totalIterations > 0)

    val allOneNoInterceptModel = lrNoIntercept.setLabelCol("oneLabel")
      .fit(sameLabels)
