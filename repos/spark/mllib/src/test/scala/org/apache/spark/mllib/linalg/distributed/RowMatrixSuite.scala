...
class RowMatrixSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("QR Decomposition") {
    for (mat <- Seq(denseMat, sparseMat)) {
      ...
      assert(
        closeToZero(abs(expected.r) - abs(calcR.toBreeze.asInstanceOf[BDM[Double]])))
      ...
      assert(
        closeToZero(
          abs(expected.r) - abs(rOnly.R.toBreeze.asInstanceOf[BDM[Double]])))
    }
  }
  ...
  test("compute covariance") {
    for (mat <- Seq(denseMat, sparseMat)) {
      ...
      assert(
        closeToZero(abs(expected) - abs(result.toBreeze.asInstanceOf[BDM[Double]])))
    }
  }
