...
class GradientDescentSuite
  ...
  test("Assert the loss is decreasing.") {
    ...
    val initialWeightsWithIntercept = Vectors
      .dense(initialWeights.toArray :+ 1.0)
  ...
  test("Test the loss and gradient of first iteration with regularization.") {
      ...
      loss1(0) ~= (
        loss0(0) + (
          math.pow(initialWeightsWithIntercept(0), 2) +
            math.pow(initialWeightsWithIntercept(1), 2)
        ) / 2
      ) absTol 1e-5,
    ...
    assert(
      (
        newWeights1(0) ~= (
          newWeights0(0) - initialWeightsWithIntercept(0)
        ) absTol 1e-5
      ) &&
        (
          newWeights1(1) ~= (
            newWeights0(1) - initialWeightsWithIntercept(1)
          ) absTol 1e-5
        ),
