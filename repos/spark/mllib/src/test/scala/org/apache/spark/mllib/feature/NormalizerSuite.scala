...
class NormalizerSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("Normalization using L1 distance") {
    ...
    assert(
      data1(2) ~== Vectors
        .dense(0.12765957, -0.23404255, -0.63829787) absTol 1e-5)
    assert(
      data1(3) ~== Vectors
  ...
  test("Normalization using L2 distance") {
    ...
    assert(
      data2(2) ~== Vectors
        .dense(0.184549876, -0.3383414, -0.922749378) absTol 1e-5)
    assert(
      data2(3) ~== Vectors
        .sparse(3, Seq((1, 0.27352993), (2, 0.96186349))) absTol 1e-5)
    assert(
      data2(4) ~== Vectors
        .dense(0.897906166, 0.113419726, 0.42532397) absTol 1e-5)
