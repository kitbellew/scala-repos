...
class VectorsSuite extends SparkFunSuite with Logging {
  ...
  test("sqdist") {
    ...
    for (m <- 1 until 1000 by 100) {
      ...
      assert(
        Vectors.sqdist(denseVector1, denseVector2) ~== squaredDist relTol 1e-8)
    }
  }
  ...
  test("vector p-norm") {
    ...
    assert(
      Vectors.norm(dv, 2.0) ~== math.sqrt(
        dv.toArray.foldLeft(0.0)((a, v) => a + v * v)) relTol 1e-8)
    assert(
      Vectors.norm(sv, 2.0) ~== math.sqrt(
        sv.toArray.foldLeft(0.0)((a, v) => a + v * v)) relTol 1e-8)
