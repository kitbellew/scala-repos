...
class StandardScalerSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("Standardization with dense input when means and stds are provided") {
    ...
    assert(
      (denseData, data1, data1RDD.collect())
        .zipped
        .forall {
    ...
    assert(
      (denseData, data2, data2RDD.collect())
        .zipped
        .forall {
          case (v1: DenseVector, v2: DenseVector, v3: DenseVector) =>
            true
    ...
    assert(
      (denseData, data3, data3RDD.collect())
        .zipped
        .forall {
          case (v1: DenseVector, v2: DenseVector, v3: DenseVector) =>
            true
    ...
    assert(
      data1(0) ~== Vectors.dense(-1.31527964, 1.023470449, 0.11637768424) absTol
        1e-5)
    assert(
      data1(3) ~== Vectors.dense(1.637735298, 0.156973995, 1.32247368462) absTol
        1e-5)
    assert(
      data2(4) ~== Vectors.dense(0.865538862, -0.22604255, 0.0) absTol 1e-5)
    assert(data2(5) ~== Vectors.dense(0.0, 0.71580142, 0.0) absTol 1e-5)
    assert(
      data3(1) ~== Vectors.dense(-0.58333333, -0.58333333, -2.8166666666) absTol
        1e-5)
    assert(
      data3(5) ~== Vectors.dense(-0.58333333, 2.316666666, 0.18333333333) absTol
        1e-5)
  }
  ...
  test("Standardization with sparse input when means and stds are provided") {
    ...
    assert(
      (sparseData, data2, data2RDD.collect())
        .zipped
        .forall {
          case (v1: DenseVector, v2: DenseVector, v3: DenseVector) =>
            true
    ...
    assert(
      data2(4) ~==
        Vectors.sparse(3, Seq((0, 0.865538862), (1, -0.22604255))) absTol 1e-5)
