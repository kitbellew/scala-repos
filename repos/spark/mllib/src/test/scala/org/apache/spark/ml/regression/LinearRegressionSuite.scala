...
class LinearRegressionSuite
  ...
  test("linear regression with intercept without regularization") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      ...
      model1.transform(datasetWithDenseFeature).select(
        "features",
        "prediction").collect().foreach {
        case Row(features: DenseVector, prediction1: Double) =>
          val prediction2 =
            features(0) * model1.coefficients(0) + features(
              1) * model1.coefficients(1) +
              model1.intercept
  ...
  test("linear regression without intercept without regularization") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      ...
      val trainer2 =
        (new LinearRegression).setFitIntercept(false).setStandardization(false)
          .setSolver(solver)
      val model1 = trainer1.fit(datasetWithDenseFeature)
  ...
  test("linear regression with intercept with L1 regularization") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      val trainer1 =
        (new LinearRegression).setElasticNetParam(1.0).setRegParam(0.57)
          .setSolver(solver)
      val trainer2 =
        (new LinearRegression).setElasticNetParam(1.0).setRegParam(0.57)
  ...
  test("linear regression with intercept with L2 regularization") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      val trainer1 =
        (new LinearRegression).setElasticNetParam(0.0).setRegParam(2.3)
          .setSolver(solver)
      val trainer2 =
        (new LinearRegression).setElasticNetParam(0.0).setRegParam(2.3)
  ...
  test("linear regression with intercept with ElasticNet regularization") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      val trainer1 =
        (new LinearRegression).setElasticNetParam(0.3).setRegParam(1.6)
          .setSolver(solver)
      val trainer2 =
        (new LinearRegression).setElasticNetParam(0.3).setRegParam(1.6)
          .setStandardization(false).setSolver(solver)
    ...
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      for (standardization <- Seq(false, true)) {
        val model = new LinearRegression().setFitIntercept(false)
          .setRegParam(0.1).setStandardization(standardization).setSolver(
            solver)
        intercept[IllegalArgumentException] {
  ...
  test("linear regression model training summary") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      ...
      assert(
        (datasetWithDenseFeature.schema.fieldNames.toSet + "prediction").subsetOf(
      ...
      assert(
        (datasetWithDenseFeature.schema.fieldNames.toSet).subsetOf(
        ...
        datasetWithDenseFeature.select("features", "label")
          ...
          .map {
            case Row(features: DenseVector, label: Double) =>
              val prediction =
                features(0) * model.coefficients(0) + features(
                  1) * model.coefficients(1) +
                  model.intercept
              label - prediction
          }
  ...
  test("linear regression with weighted samples") {
    Seq("auto", "l-bfgs", "normal").foreach { solver =>
      ...
      val trainer1a = (new LinearRegression).setFitIntercept(true)
        .setElasticNetParam(0.0).setRegParam(0.21).setStandardization(
          true).setSolver(solver)
      val trainer1b =
        (new LinearRegression).setFitIntercept(true).setWeightCol("weight")
          .setElasticNetParam(0.0).setRegParam(0.21).setStandardization(
            true).setSolver(solver)
      ...
      val trainer3a = (new LinearRegression).setFitIntercept(false)
        .setElasticNetParam(0.0).setRegParam(0.21).setStandardization(
          true).setSolver(solver)
      val trainer3b =
        (new LinearRegression).setFitIntercept(false).setWeightCol("weight")
          .setElasticNetParam(0.0).setRegParam(0.21).setStandardization(
            true).setSolver(solver)
