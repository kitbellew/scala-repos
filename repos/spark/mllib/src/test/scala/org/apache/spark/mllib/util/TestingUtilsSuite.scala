...
class TestingUtilsSuite extends SparkFunSuite {
  ...
  test("Comparing doubles using relative error.") {
    ...
    assert(
      -Double.MinPositiveValue ~== 1.38 * -Double.MinPositiveValue relTol 0.012)
  }
  ...
  test("Comparing vectors using relative error.") {
    ...
    assert(
      Vectors
        .dense(Array(3.1, 3.5)) ~== Vectors.dense(Array(3.130, 3.534)) relTol
        0.01)
    assert(
    ...
    assert(
      !(
        Vectors
          .dense(Array(3.1, 3.5)) ~= Vectors.dense(Array(3.135, 3.534)) relTol
          0.01
      ))
