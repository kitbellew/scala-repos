...
class BucketizerSuite
  ...
  test("params") { ParamsSuite.checkParams(new Bucketizer) }
  ...
  test("Bucket continuous features, without -inf,inf") {
    ...
    withClue(
      ...
      intercept[SparkException] { bucketizer.transform(badDF2).collect() }
    }
  }
  ...
  test("Binary search correctness on hand-picked examples") {
    ...
    checkBinarySearch(
      Array(Double.NegativeInfinity, 1.0, Double.PositiveInfinity))
  ...
  test(
    ...
    val splits: Array[Double] =
      Double.NegativeInfinity +: Array.fill(10)(Random.nextDouble()).sorted :+
        Double.PositiveInfinity
