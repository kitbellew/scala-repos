...
class LDASuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("running and DistributedLDAModel with default Optimizer (EM)") {
    ...
    model
      .topDocumentsPerTopic(3)
      .zip(topDocsByTopicDistributions(3))
      .foreach {
        case (t1, t2) =>
          assert(t1._1 === t2._1)
          assert(t1._2 === t2._2)
    ...
    model
      .topDocumentsPerTopic(q)
      .zip(topDocsByTopicDistributions(q))
      .foreach {
        case (t1, t2) =>
          assert(t1._1 === t2._1)
          assert(t1._2 === t2._2)
