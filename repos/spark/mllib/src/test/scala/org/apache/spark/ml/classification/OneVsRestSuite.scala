...
class OneVsRestSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("one-vs-rest: default params") {
    ...
    assert(
      expectedMetrics.confusionMatrix ~== ovaMetrics.confusionMatrix absTol 400)
  }
  ...
  test("one-vs-rest: pass label metadata correctly during train") {
    ...
    val labelMetadata = NominalAttribute.defaultAttr.withName("label")
  ...
  test(
    ...
    val indexedDataset = labelIndexer.fit(dataset).transform(dataset)
      .drop("label").withColumnRenamed("features", "f")

    val ova = new OneVsRest()
    ova.setClassifier(new LogisticRegression())
      .setLabelCol(labelIndexer.getOutputCol).setFeaturesCol("f")
      .setPredictionCol("p")
...
private class MockLogisticRegression(uid: String)
    extends LogisticRegression(uid) {
  ...
  override protected[spark] def train(
      dataset: DataFrame): LogisticRegressionModel = {
