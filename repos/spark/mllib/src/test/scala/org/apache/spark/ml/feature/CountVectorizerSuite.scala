...
class CountVectorizerSuite
  ...
  test("CountVectorizer common cases") {
    val df = sqlContext.createDataFrame(Seq(
      (
        ...
        split("a b c d e"),
        Vectors
          .sparse(5, Seq((0, 1.0), (1, 1.0), (2, 1.0), (3, 1.0), (4, 1.0)))),
  ...
  test("CountVectorizer vocabSize and minDF") {
    ...
    val cvModel = new CountVectorizer().setInputCol("words")
      .setOutputCol("features").setVocabSize(3) // limit vocab size to 3
      .fit(df)
    assert(cvModel.vocabulary === Array("a", "b", "c"))

    // minDF: ignore terms with count less than 3
    val cvModel2 = new CountVectorizer().setInputCol("words")
      .setOutputCol("features").setMinDF(3).fit(df)
    ...
    val cvModel3 = new CountVectorizer().setInputCol("words")
      .setOutputCol("features").setMinDF(3.0 / df.count()).fit(df)
  ...
  test("CountVectorizer throws exception when vocab is empty") {
    intercept[IllegalArgumentException] {
      val df = sqlContext
        .createDataFrame(Seq((0, split("a a b b c c")), (1, split("aa bb cc"))))
        .toDF("id", "words")
      val cvModel = new CountVectorizer().setInputCol("words")
        .setOutputCol("features").setVocabSize(3) // limit vocab size to 3
        .setMinDF(3).fit(df)
  ...
  test("CountVectorizerModel with minTF count") {
    ...
    // minTF: count
    val cv = new CountVectorizerModel(Array("a", "b", "c", "d"))
      .setInputCol("words").setOutputCol("features").setMinTF(3)
    cv.transform(df).select("features", "expected").collect().foreach {
      case Row(features: Vector, expected: Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
  ...
  test("CountVectorizerModel with minTF freq") {
    ...
    // minTF: set frequency
    val cv = new CountVectorizerModel(Array("a", "b", "c", "d"))
      .setInputCol("words").setOutputCol("features").setMinTF(0.3)
    cv.transform(df).select("features", "expected").collect().foreach {
      case Row(features: Vector, expected: Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
  ...
  test("CountVectorizerModel with binary") {
    ...
    val cv = new CountVectorizerModel(Array("a", "b", "c", "d"))
      .setInputCol("words").setOutputCol("features").setBinary(true)
    cv.transform(df).select("features", "expected").collect().foreach {
      case Row(features: Vector, expected: Vector) =>
        assert(features ~== expected absTol 1e-14)
    }
  ...
  test("CountVectorizer read/write") {
    val t = new CountVectorizer().setInputCol("myInputCol")
      .setOutputCol("myOutputCol").setMinDF(0.5).setMinTF(3.0).setVocabSize(10)
  ...
  test("CountVectorizerModel read/write") {
    val instance =
      new CountVectorizerModel("myCountVectorizerModel", Array("a", "b", "c"))
        .setInputCol("myInputCol").setOutputCol("myOutputCol").setMinTF(3.0)
