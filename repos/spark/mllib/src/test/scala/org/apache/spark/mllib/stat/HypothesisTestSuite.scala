...
class HypothesisTestSuite extends SparkFunSuite with MLlibTestSparkContext {
  ...
  test("chi squared pearson goodness of fit") {
    ...
    assert(
      pearson
        .nullHypothesis === ChiSqTest.NullHypothesis.goodnessOfFit.toString)
    ...
    intercept[IllegalArgumentException](
      Statistics.chiSqTest(observed3, expected3))
    ...
    assert(
      inf.nullHypothesis === ChiSqTest.NullHypothesis.goodnessOfFit.toString)
  ...
  test("chi squared pearson matrix independence") {
    ...
    intercept[IllegalArgumentException](
      Statistics.chiSqTest(Matrices.dense(2, 2, negCounts)))

    // Row sum = 0.0
    val rowZero = Array(0.0, 1.0, 0.0, 2.0)
    intercept[IllegalArgumentException](
      Statistics.chiSqTest(Matrices.dense(2, 2, rowZero)))
  ...
  test("chi squared pearson RDD[LabeledPoint]") {
    ...
    for (numParts <- List(2, 4, 6, 8)) {
      ...
      assert(
        feature1
          .nullHypothesis === ChiSqTest.NullHypothesis.independence.toString)
      val feature2 = chi(1)
  ...
  test(
    ...
    val referenceStat1 = ksTest
      .kolmogorovSmirnovStatistic(stdNormalDist, sampledNorm.collect())
    ...
    val referenceStat2 = ksTest
      .kolmogorovSmirnovStatistic(stdNormalDist, sampledExp.collect())
    ...
    val expCDF =
      (x: Double) => new ExponentialDistribution(0.2).cumulativeProbability(x)
  ...
  test("1 sample Kolmogorov-Smirnov test: R implementation equivalence") {
    ...
    val rData = sc.parallelize(
      Array(
        ...
        -1.33259371048501, -0.446566766553219, 0.569606122374976,
        -2.88971761441412, -0.869018343326555, -0.461702683149641,
