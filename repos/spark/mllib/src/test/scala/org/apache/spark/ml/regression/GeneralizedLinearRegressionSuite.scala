...
class GeneralizedLinearRegressionSuite
  ...
  override def beforeAll(): Unit = {
    ...
    datasetGaussianIdentity = sqlContext.createDataFrame(sc.parallelize(
    ...
    datasetGaussianLog = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 0.25,
    ...
    datasetGaussianInverse = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 2.5,
        coefficients = Array(2.2, 0.6),
        xMean = Array(2.9, 10.5),
    ...
    datasetPoissonLog = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 0.25,
        coefficients = Array(0.22, 0.06),
        xMean = Array(2.9, 10.5),
    ...
    datasetPoissonIdentity = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 2.5,
        coefficients = Array(2.2, 0.6),
        xMean = Array(2.9, 10.5),
    ...
    datasetPoissonSqrt = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 2.5,
        coefficients = Array(2.2, 0.6),
        xMean = Array(2.9, 10.5),
    ...
    datasetGammaInverse = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 2.5,
        coefficients = Array(2.2, 0.6),
        xMean = Array(2.9, 10.5),
    ...
    datasetGammaIdentity = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 2.5,
        coefficients = Array(2.2, 0.6),
        xMean = Array(2.9, 10.5),
    ...
    datasetGammaLog = sqlContext.createDataFrame(sc.parallelize(
      generateGeneralizedLinearRegressionInput(
        intercept = 0.25,
        coefficients = Array(0.22, 0.06),
        xMean = Array(2.9, 10.5),
  ...
  test("params") {
    ...
    val model =
      new GeneralizedLinearRegressionModel("genLinReg", Vectors.dense(0.0), 0.0)
  ...
  test("generalized linear regression: gaussian family against glm") {
    ...
    for ((link, dataset) <- Seq(
      ...
      for (fitIntercept <- Seq(false, true)) {
        ...
        model.transform(dataset).select("features", "prediction").collect()
          .foreach {
  ...
  test("generalized linear regression: binomial family against glm") {
    ...
    for ((link, dataset) <- Seq(
      ...
      for (fitIntercept <- Seq(false, true)) {
        val trainer = new GeneralizedLinearRegression().setFamily("binomial")
          .setLink(link).setFitIntercept(fitIntercept)
        val model = trainer.fit(dataset)
  ...
  test("glm summary: gaussian family with weight") {
     ...
     */
    val datasetWithWeight = sqlContext.createDataFrame(sc.parallelize(
      Seq(
    ...
    val devianceResiduals = summary.residuals().select(col("devianceResiduals"))
      .collect().map(_.getDouble(0))
    val pearsonResiduals = summary.residuals("pearson")
      .select(col("pearsonResiduals")).collect().map(_.getDouble(0))
    val workingResiduals = summary.residuals("working")
      .select(col("workingResiduals")).collect().map(_.getDouble(0))
    val responseResiduals = summary.residuals("response")
      .select(col("responseResiduals")).collect().map(_.getDouble(0))
  ...
  test("glm summary: binomial family with weight") {
     ...
     */
    val trainer = new GeneralizedLinearRegression().setFamily("binomial")
      .setWeightCol("weight").setFitIntercept(false)
  ...
  test("glm summary: poisson family with weight") {
     ...
     */
    val trainer = new GeneralizedLinearRegression().setFamily("poisson")
      .setWeightCol("weight").setFitIntercept(true)
...
object GeneralizedLinearRegressionSuite {
  ...
  def generateGeneralizedLinearRegressionInput(
    ...
    (0 until nPoints).map { _ =>
      val features = Vectors.dense(coefficients.indices.map { rndElement(_) }
        .toArray)
