...
private[mllib] object GridPartitioner {
  ...
  def apply(
      rows: Int,
      cols: Int,
      suggestedNumPartitions: Int): GridPartitioner = {
...
class BlockMatrix @Since("1.3.0") (
  ...
  def toLocalMatrix(): Matrix = {
    ...
    val mem = m * n / 125000
    if (mem > 500)
      logWarning(s"Storing this matrix will require $mem MB of memory!")
