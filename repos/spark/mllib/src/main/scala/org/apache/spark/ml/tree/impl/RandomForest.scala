...
private[ml] object RandomForest extends Logging {
  ...
  def run(
    ...
    val topNodes = Array
      .fill[LearningNode](numTrees)(LearningNode.emptyNode(nodeIndex = 1))
    Range(0, numTrees)
  ...
  private[tree] def findBestSplits(
    ...
    def getNodeToFeatures(
      ...
      if (!metadata.subsamplingFeatures) { None }
      else {
    ...
    nodesForGroup.foreach {
      case (treeIndex, nodesForTree) => nodesForTree.foreach { node =>
          ...
          val aggNodeIndex = nodeInfo.nodeIndexInGroup
          val (split: Split, stats: ImpurityStats) = nodeToBestSplits(
            aggNodeIndex)
          logDebug("best split = " + split)

          // Extract info for this node.  Create children if not leaf.
          val isLeaf =
            (stats.gain <= 0) || (LearningNode
  ...
  private def calculateImpurityStats(
    ...
    val parentImpurityCalculator: ImpurityCalculator =
      if (stats == null) {
        leftImpurityCalculator.copy.add(rightImpurityCalculator)
      } else { stats.impurityCalculator }

    val impurity: Double =
      if (stats == null) { parentImpurityCalculator.calculate() }
      else { stats.impurity }
    ...
    val leftImpurity = leftImpurityCalculator
      .calculate() // Note: This equals 0 if count = 0
  ...
  private[tree] def binsToBestSplit(
    ...
    var gainAndImpurityStats: ImpurityStats =
      if (level == 0) { null }
      else { node.stats }

    // For each (feature, split), calculate the gain, and select the best (feature, split).
    val (bestSplit, bestSplitStats) = Range(
      0,
      binAggregates.metadata.numFeaturesPerNode).map { featureIndexIdx =>
      val featureIndex =
        if (featuresForNode.nonEmpty) {
          featuresForNode.get.apply(featureIndexIdx)
        } else { featureIndexIdx }
      ...
      if (binAggregates.metadata.isContinuous(featureIndex)) {
        ...
        // Find best split.
        val (bestFeatureSplitIndex, bestFeatureGainStats) = Range(0, numSplits)
          .map {
            case splitIdx =>
        ...
        val categoriesSortedByCentroid = centroidForCategories.toList
          .sortBy(_._2)
        ...
        val categoriesForSplit = categoriesSortedByCentroid.map(_._1.toDouble)
  ...
  protected[tree] def findSplits(
    ...
    val sampledInput =
      ...
      } else { input.sparkContext.emptyRDD[LabeledPoint] }
    ...
    val continuousSplits: scala.collection.Map[Int, Array[Split]] = {
      ...
      input.flatMap(point =>
        ...
        .groupByKey(numPartitions).map {
          case (idx, samples) =>
            val thresholds = findSplitsForContinuousFeature(
              samples,
              metadata,
              idx)
    ...
    val splits: Array[Array[Split]] = Array.tabulate(numFeatures) {
      ...
      case i if metadata.isCategorical(i) && metadata.isUnordered(i) =>
        ...
        Array.tabulate[Split](metadata.numSplits(i)) { splitIndex =>
          val categories = extractMultiClassCategories(
            splitIndex + 1,
            featureArity)
  ...
  private[tree] def findSplitsForContinuousFeature(
    ...
    val splits = {
      ...
      val (valueCountMap, numSamples) = featureSamples
        .foldLeft((Map.empty[Double, Int], 0)) {
      ...
      if (possibleSplits <= numSplits) { valueCounts.map(_._1) }
      else {
  ...
  private[tree] def selectNodesToSplit(
    ...
    val mutableNodesForGroup =
      new mutable.HashMap[Int, mutable.ArrayBuffer[LearningNode]]()
    ...
    val nodesForGroup: Map[Int, Array[LearningNode]] = mutableNodesForGroup
      .mapValues(_.toArray).toMap
    val treeToNodeToIndexInfo = mutableTreeToNodeToIndexInfo.mapValues(_.toMap)
      .toMap
  ...
  private def aggregateSizeForNode(
    ...
    val totalBins =
      if (featureSubset.nonEmpty) {
        featureSubset.get
          .map(featureIndex => metadata.numBins(featureIndex).toLong).sum
