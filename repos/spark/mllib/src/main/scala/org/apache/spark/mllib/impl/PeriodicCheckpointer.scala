...
private[mllib] abstract class PeriodicCheckpointer[T](
  ...
  def update(newData: T): Unit = {
    ...
    if ((updateCount % checkpointInterval) == 0 && sc.getCheckpointDir
          .nonEmpty) {
      ...
      while (checkpointQueue.size > 1 && canDelete) {
        ...
        if (isCheckpointed(checkpointQueue.head)) { removeCheckpointFile() }
  ...
  def deleteAllCheckpoints(): Unit = {
    while (checkpointQueue.nonEmpty) { removeCheckpointFile() }
  }
