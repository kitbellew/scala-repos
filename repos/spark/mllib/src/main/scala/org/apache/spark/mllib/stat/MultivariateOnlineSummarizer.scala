...
class MultivariateOnlineSummarizer
  ...
  private[spark] def add(instance: Vector, weight: Double): this.type = {
    ...
    instance.foreachActive { (index, value) =>
      if (value != 0.0) {
        if (localCurrMax(index) < value) { localCurrMax(index) = value }
        if (localCurrMin(index) > value) { localCurrMin(index) = value }
    ...
    if (this.weightSum != 0.0 && other.weightSum != 0.0) {
      ...
      while (i < n) {
        ...
        if (totalNnz != 0.0) {
          ...
          // merge m2n together
          currM2n(i) += other.currM2n(
            i) + deltaMean * deltaMean * thisNnz * otherNnz / totalNnz
          // merge m2 together
