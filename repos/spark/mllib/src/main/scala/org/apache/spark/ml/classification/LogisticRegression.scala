...
class LogisticRegression @Since("1.2.0") (
  ...
  /** @group setParam */
  private[spark] def setInitialModel(
      model: LogisticRegressionModel): this.type = {
...
class LogisticRegressionModel private[spark] (
  ...
  private val margin: Vector => Double =
    (features) => {
      BLAS.dot(features, coefficients) + intercept
    }

  /** Score (probability) for class label 1.  For binary classification only. */
  private val score: Vector => Double =
    (features) => {
...
object LogisticRegressionModel extends MLReadable[LogisticRegressionModel] {
  ...
  @Since("1.6.0")
  override def read: MLReader[LogisticRegressionModel] =
    new LogisticRegressionModelReader
...
class BinaryLogisticRegressionSummary private[classification] (
  ...
  @Since("1.5.0")
  @transient
  lazy val roc: DataFrame = binaryMetrics.roc().toDF("FPR", "TPR")
  ...
  @Since("1.5.0")
  @transient
  lazy val pr: DataFrame = binaryMetrics.pr().toDF("recall", "precision")
