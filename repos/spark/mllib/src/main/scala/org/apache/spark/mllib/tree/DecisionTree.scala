...
object DecisionTree extends Serializable with Logging {
  ...
  def trainClassifier(
    ...
    trainClassifier(
      input.rdd,
      numClasses,
      categoricalFeaturesInfo.asInstanceOf[java.util.Map[Int, Int]].asScala
        .toMap,
      impurity,
      maxDepth,
      maxBins)
  ...
  private def predictNodeIndex(
    ...
    if (node.isLeaf || node.split.isEmpty) {
      ...
      if (node.leftNode.isEmpty || node.rightNode.isEmpty) {
        ...
        if (splitLeft) { Node.leftChildIndex(node.id) }
        else { Node.rightChildIndex(node.id) }
      } else {
        if (splitLeft) {
          predictNodeIndex(
            node.leftNode.get,
    ...
    while (featureIndexIdx < numFeaturesPerNode) {
      ...
      if (unorderedFeatures.contains(featureIndex)) {
        ...
        while (splitIndex < numSplits) {
          if (splits(featureIndex)(splitIndex).categories
                .contains(featureValue)) {
            agg.featureUpdate(
  ...
  private def calculatePredict(
      impurityCalculator: ImpurityCalculator): Predict = {
  ...
  private[tree] def binsToBestSplit(
    ...
    var predictWithImpurity: Option[(Predict, Double)] =
      if (level == 0) { None }
      else { Some((node.predict, node.impurity)) }
    ...
    val (bestSplit, bestSplitStats) = Range(
      ...
      if (binAggregates.metadata.isContinuous(featureIndex)) {
         ...
         */
        val centroidForCategories = Range(0, numBins).map {
          case featureValue =>
  ...
  private def findSplitsBinsBySorting(
    ...
    def findSplits(
      ...
      val splits = {
        ...
        featureSplits
          .map(threshold => new Split(featureIndex, threshold, Continuous, Nil))
      }
    ...
    val continuousSplits = {
      ...
      input.flatMap(point =>
        ...
        .groupByKey(numPartitions).map { case (k, v) => findSplits(k, v) }
        .collectAsMap()
    }
