...
private[shared] object SharedParamsCodeGen {
  ...
  def main(args: Array[String]): Unit = {
    val params = Seq(
      ...
      ParamDesc[String](
        "solver",
        "the solver algorithm for optimization. If this is not set or " +
          "empty, default value is 'auto'",
        Some("\"auto\""))
    )
  ...
  private def genHasParamTrait(param: ParamDesc[_]): String = {
    ...
    val defaultValueDoc = defaultValue.map { v => s" (default: $v)" }
      .getOrElse("")
