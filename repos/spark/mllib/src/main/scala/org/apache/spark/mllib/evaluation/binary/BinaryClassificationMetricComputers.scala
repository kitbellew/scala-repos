...
private[evaluation] object Precision
  ...
  override def apply(c: BinaryConfusionMatrix): Double = {
    ...
    if (totalPositives == 0) { 1.0 }
    else { c.numTruePositives.toDouble / totalPositives }
  }
}
...
private[evaluation] object FalsePositiveRate
    extends BinaryClassificationMetricComputer {
  override def apply(c: BinaryConfusionMatrix): Double = {
    if (c.numNegatives == 0) { 0.0 }
    else { c.numFalsePositives.toDouble / c.numNegatives }
  }
}
...
private[evaluation] object Recall extends BinaryClassificationMetricComputer {
  override def apply(c: BinaryConfusionMatrix): Double = {
    if (c.numPositives == 0) { 0.0 }
    else { c.numTruePositives.toDouble / c.numPositives }
  }
}
