...
class MultilabelMetrics @Since("1.2.0") (
  ...
  lazy val accuracy: Double = predictionAndLabels.map {
    case (predictions, labels) =>
      labels.intersect(predictions).length.toDouble /
        (labels.length + predictions.length - labels.intersect(
          predictions).length)
  }.sum / numDocs
  ...
  lazy val hammingLoss: Double = predictionAndLabels.map {
    case (predictions, labels) =>
      labels.length + predictions.length - 2 * labels.intersect(
        predictions).length
  ...
  lazy val f1Measure: Double = predictionAndLabels.map {
    case (predictions, labels) =>
      2.0 * predictions.intersect(
        labels).length / (predictions.length + labels.length)
  }.sum / numDocs
