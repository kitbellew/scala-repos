...
abstract class ProbabilisticClassifier[
  ...
  def setProbabilityCol(value: String): E =
    set(probabilityCol, value).asInstanceOf[E]

  /** @group setParam */
  def setThresholds(value: Array[Double]): E =
    set(thresholds, value).asInstanceOf[E]
}
...
abstract class ProbabilisticClassificationModel[
  ...
  def setProbabilityCol(value: String): M =
    set(probabilityCol, value).asInstanceOf[M]
  ...
  protected def probability2prediction(probability: Vector): Double = {
    if (!isDefined(thresholds)) { probability.argmax }
      ...
      val scaledProbability: Array[Double] = probability.toArray.zip(thresholds)
        .map { case (p, t) => if (t == 0.0) Double.PositiveInfinity else p / t }
