...
private[ml] class WeightedLeastSquares(
  ...
  def fit(instances: RDD[Instance]): WeightedLeastSquaresModel = {
    ...
    val aa =
      if (fitIntercept) { Array.concat(aaBar.values, aBar.values, Array(1.0)) }
      else { aaBar.values }
    val ab =
      if (fitIntercept) { Array.concat(abBar.values, Array(bBar)) }
      else { abBar.values }
    ...
    val (coefficients, intercept) =
      if (fitIntercept) { (new DenseVector(x.slice(0, x.length - 1)), x.last) }
