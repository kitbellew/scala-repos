...
private[spark] class DecisionTreeMetadata(
  ...
  def isMulticlassWithCategoricalFeatures: Boolean =
    isMulticlass && (featureArity.size > 0)

  def isCategorical(featureIndex: Int): Boolean =
...
private[spark] object DecisionTreeMetadata extends Logging {
  ...
  def buildMetadata(
    ...
    if (strategy.categoricalFeaturesInfo.nonEmpty) {
      ...
      val maxCategory = strategy.categoricalFeaturesInfo
        .find(_._2 == maxCategoriesPerFeature).get._1
      require(
