    ...
    splits.zipWithIndex.foreach {
      case ((training, validation), splitIndex) =>
        ...
        while (i < numModels) {
          ...
          val metric = eval
            .evaluate(models(i).transform(validationDataset, epm(i)))
...
object CrossValidator extends MLReadable[CrossValidator] {
  ...
  private class CrossValidatorReader extends MLReader[CrossValidator] {
    ...
    override def load(path: String): CrossValidator = {
      ...
      new CrossValidator(metadata.uid).setEstimator(estimator)
        .setEvaluator(evaluator).setEstimatorParamMaps(estimatorParamMaps)
  ...
  private object CrossValidatorReader {
    ...
    def getUidMapImpl(instance: Params): List[(String, Params)] = {
      ...
      val subStageMaps = subStages.map(getUidMapImpl)
  ...
  private[tuning] object SharedReadWrite {
    ...
    def validateParams(instance: ValidatorParams): Unit = {
      ...
      val uidToInstance: Map[String, Params] = CrossValidatorReader
        .getUidMap(instance)
    ...
    private[tuning] def saveImpl(
      ...
      DefaultParamsWriter
        .saveMetadata(instance, path, sc, extraMetadata, Some(jsonParams))
...
object CrossValidatorModel extends MLReadable[CrossValidatorModel] {
  ...
  @Since("1.6.0")
  override def read: MLReader[CrossValidatorModel] =
    new CrossValidatorModelReader
  ...
  private class CrossValidatorModelReader
      extends MLReader[CrossValidatorModel] {
    ...
    override def load(path: String): CrossValidatorModel = {
      ...
      val avgMetrics = (metadata.metadata \ "avgMetrics").extract[Seq[Double]]
        .toArray
      val cv = new CrossValidatorModel(metadata.uid, bestModel, avgMetrics)
      cv.set(cv.estimator, estimator).set(cv.evaluator, evaluator)
