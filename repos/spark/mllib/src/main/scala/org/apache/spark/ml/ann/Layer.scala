...
private[ann] object AffineLayerModel {
  ...
  def unroll(
    ...
    val b =
      new BDV[Double](weightsCopy, position + (numOut * numIn), 1, numOut)
    (a, b)
  }
...
/**
  ...
  */
private[ann] class FunctionalLayer(val activationFunction: ActivationFunction)
    extends Layer {
  override def getInstance(weights: Vector, position: Int): LayerModel =
    getInstance(0L)
...
/**
  ...
  * @param layers
  */
private[ann] class FeedForwardTopology private (val layers: Array[Layer])
    extends Topology {
...
private[ml] object FeedForwardTopology {
  ...
  def multiLayerPerceptron(
      ...
      softmax: Boolean = true): FeedForwardTopology = {
