...
package object config {
  ...
  private[spark] val APPLICATION_TAGS = ConfigBuilder("spark.yarn.tags").doc(
    ...
    .stringConf.toSequence.optional

  private[spark] val ATTEMPT_FAILURE_VALIDITY_INTERVAL_MS =
    ConfigBuilder("spark.yarn.am.attemptFailuresValidityInterval").doc(
      "Interval after which AM failures will be considered independent and " +
        "not accumulate towards the attempt count.")
      .timeConf(TimeUnit.MILLISECONDS).optional

  private[spark] val MAX_APP_ATTEMPTS =
    ConfigBuilder("spark.yarn.maxAppAttempts")
      .doc("Maximum number of AM attempts before failing the app.").intConf
      .optional
  ...
  private[spark] val SPARK_JARS = ConfigBuilder("spark.yarn.jars")
    .doc("Location of jars containing Spark classes.").stringConf.toSequence
    .optional
  ...
  private[spark] val AM_JAVA_OPTIONS =
    ConfigBuilder("spark.yarn.am.extraJavaOptions")
      .doc("Extra Java options for the client-mode AM.").stringConf.optional

  private[spark] val AM_LIBRARY_PATH =
    ConfigBuilder("spark.yarn.am.extraLibraryPath")
      .doc("Extra native library path for the client-mode AM.").stringConf
      .optional

  private[spark] val AM_MEMORY_OVERHEAD =
    ConfigBuilder("spark.yarn.am.memoryOverhead").bytesConf(ByteUnit.MiB)
      .optional

  private[spark] val AM_MEMORY = ConfigBuilder("spark.yarn.am.memory")
    .bytesConf(ByteUnit.MiB).withDefaultString("512m")

  /* Driver configuration. */

  private[spark] val DRIVER_CORES = ConfigBuilder("spark.driver.cores").intConf
    .optional

  private[spark] val DRIVER_MEMORY_OVERHEAD =
    ConfigBuilder("spark.yarn.driver.memoryOverhead").bytesConf(ByteUnit.MiB)
      .optional

  /* Executor configuration. */

  private[spark] val EXECUTOR_MEMORY_OVERHEAD =
    ConfigBuilder("spark.yarn.executor.memoryOverhead").bytesConf(ByteUnit.MiB)
      .optional

  private[spark] val EXECUTOR_NODE_LABEL_EXPRESSION =
    ConfigBuilder("spark.yarn.executor.nodeLabelExpression")
      .doc("Node label expression for executors.").stringConf.optional
  ...
  private[spark] val NAMENODES_TO_ACCESS = ConfigBuilder(
    "spark.yarn.access.namenodes").doc(
    "Extra NameNode URLs for which to request delegation tokens. The NameNode that hosts " +
      "fs.defaultFS does not need to be listed here.").stringConf.toSequence
    .withDefault(Nil)
  ...
  private[spark] val APP_JAR = ConfigBuilder("spark.yarn.user.jar").internal
    .stringConf.optional

  // Internal config to propagate the locations of any extra jars to add to the classpath
  // of the executors
  private[spark] val SECONDARY_JARS = ConfigBuilder("spark.yarn.secondary.jars")
    .internal.stringConf.toSequence.optional
