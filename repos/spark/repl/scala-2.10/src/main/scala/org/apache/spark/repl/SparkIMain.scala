...
class SparkIMain(
  ...
  private[repl] lazy val reporter: ConsoleReporter =
    new SparkIMain.ReplReporter(this)
  ...
  private class TranslatingClassLoader(parent: ClassLoader)
      extends AbstractFileClassLoader(virtualDirectory, parent) {
  ...
  private def directBind[T: ru.TypeTag: ClassTag](
      name: String,
      value: T): IR.Result = directBind((name, value))
  ...
  private def quietImport(ids: String*): IR.Result = beQuietDuring(
    addImports(ids: _*))
  ...
  private def bind[T: ru.TypeTag: ClassTag](name: String, value: T): IR.Result =
    bind((name, value))
  ...
  private def bindValue(name: String, x: Any): IR.Result =
  ...
  class Request(val line: String, val trees: List[Tree]) {
    ...
    def applyToResultMember[T](name: Name, f: Symbol => T) = afterTyper(
  ...
  @DeveloperApi
  def definedSymbols =
    prevRequestList.flatMap(_.definedSymbols.values).toSet[Symbol]
  ...
  private def types[T: ClassTag]: Symbol = types(
    classTag[T].runtimeClass.getName)
  private def terms[T: ClassTag]: Symbol = terms(
    classTag[T].runtimeClass.getName)
  private def apply[T: ClassTag]: Symbol = apply(
    classTag[T].runtimeClass.getName)
  ...
  private def allDefHandlers = allHandlers collect { case x: MemberDefHandler =>
    x
  }
...
object SparkIMain {
  ...
  def stripString(s: String) = removeSparkVals(
    removeIWPackages(removeLineWrapper(s)))
  ...
  class ReplStrippingWriter(intp: SparkIMain)
