...
""".format(SPARK_VERSION))
  ...
  private val initCompilerCondition = initLock
    .newCondition() // signal the compiler is initialized
  private val initLoopCondition = initLock
    .newCondition() // signal the whole repl is initialized
  ...
  protected def postInitThunks =
    List[Option[() => Unit]](
      ...
      if (isReplPower) Some(() => enablePowerMode(true)) else None).flatten
  // ++ (
  //   warningsThunks
  // )
  ...
  protected def addThunk(body: => Unit) =
    synchronized { pendingThunks :+= (() => body) }
