...
/**
  ...
  * receivers.
  */
private[streaming] case object StopAllReceivers
    extends ReceiverTrackerLocalMessage
...
/**
  ...
  */
private[streaming] case object AllReceiverIds
    extends ReceiverTrackerLocalMessage

private[streaming] case class UpdateReceiverRateLimit(
    streamUID: Int,
    newRate: Long)
    extends ReceiverTrackerLocalMessage
...
private[streaming] class ReceiverTracker(
  ...
  // be called.
  private val receiverJobExitLatch =
    new CountDownLatch(receiverInputStreams.length)
  ...
  private def deregisterReceiver(
      streamId: Int,
      message: String,
      error: String) {
    val lastErrorTime =
    ...
    val newReceiverTrackingInfo =
      receiverTrackingInfos.get(streamId) match {
        case Some(oldInfo) =>
          oldInfo
  ...
  private def updateReceiverScheduledExecutors(
    ...
    val newReceiverTrackingInfo =
      receiverTrackingInfos.get(receiverId) match {
        case Some(oldInfo) =>
          oldInfo.copy(
  ...
  private class ReceiverTrackerEndpoint(override val rpcEnv: RpcEnv)
    ...
    private def startReceiver(
      ...
      val future = ssc
        .sparkContext
        .submitJob[Receiver[_], Unit, Unit](
          receiverRDD,
          startReceiverFunc,
          Seq(0),
          (_, _) => Unit,
    ...
    private def stopReceivers() {
      ...
      logInfo(
        "Sent stop signal to all " + receiverTrackingInfos.size + " receivers")
    }
  }
