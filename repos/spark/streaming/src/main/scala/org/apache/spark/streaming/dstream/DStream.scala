...
abstract class DStream[T: ClassTag](
    ...
    baseScope.map { bsJson =>
      ...
      val scopeName =
        if (baseName.length > 10) {
          ...
          s"$baseName\n@ $formattedBatchTime"
        } else { s"$baseName @ $formattedBatchTime" }
  ...
  private def writeObject(oos: ObjectOutputStream): Unit =
    Utils.tryOrIOException {
      ...
      if (graph != null) {
        graph.synchronized {
          if (graph.checkpointInProgress) { oos.defaultWriteObject() }
          else {
            val msg =
  ...
  /**
    ...
    * an array.
    */
  def glom(): DStream[Array[T]] = ssc.withScope { new GlommedDStream(this) }
  ...
  def repartition(numPartitions: Int): DStream[T] =
    ssc.withScope { this.transform(_.repartition(numPartitions)) }
  ...
  def transform[U: ClassTag](
      transformFunc: (RDD[T], Time) => RDD[U]): DStream[U] =
    ssc.withScope {
      // because the DStream is reachable from the outer object here, and because
      // DStreams can't be serialized with closures, we can't proactively check
      // it for serializability and so we pass the optional false to SparkContext.clean
  ...
  def window(windowDuration: Duration, slideDuration: Duration): DStream[T] =
    ssc.withScope { new WindowedDStream(this, windowDuration, slideDuration) }
  ...
  def union(that: DStream[T]): DStream[T] =
    ssc.withScope { new UnionDStream[T](Array(this, that)) }

  /**
    * Return all the RDDs defined by the Interval object (both end times included)
    */
  def slice(interval: Interval): Seq[RDD[T]] =
    ssc.withScope { slice(interval.beginTime, interval.endTime) }
