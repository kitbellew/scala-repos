...
/**
  ...
  * up DStream metadata.
  */
private[streaming] class JobGenerator(jobScheduler: JobScheduler)
    extends Logging {
  ...
  def stop(processReceivedData: Boolean): Unit =
    synchronized {
      ...
      if (processReceivedData) {
        ...
        while (!hasTimedOut && jobScheduler.receiverTracker
                 .hasUnallocatedBlocks) { Thread.sleep(pollTime) }
        logInfo(
