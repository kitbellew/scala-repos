...
class ReceivedBlockTrackerSuite
  ...
  before {
    conf = new SparkConf().setMaster("local[2]")
  ...
  test("block addition, and block to batch allocation") {
    ...
    receivedBlockTracker.isWriteAheadLogEnabled should
      be(false) // should be disable by default
    ...
    receivedBlockTracker.getBlocksOfBatchAndStream(1, streamId) shouldEqual
      blockInfos
    receivedBlockTracker.getBlocksOfBatch(1) shouldEqual
      Map(streamId -> blockInfos)
    ...
    receivedBlockTracker.getBlocksOfBatch(2) shouldEqual
      Map(streamId -> Seq.empty)
  ...
  test("recovery and cleanup with write ahead logs") {
    ...
    tracker2.getBlocksOfBatchAndStream(batchTime1, streamId) shouldEqual
      blockInfos1
    tracker2.getBlocksOfBatch(batchTime1) shouldEqual
      Map(streamId -> blockInfos1)
    ...
    tracker4.getBlocksOfBatchAndStream(batchTime1, streamId) shouldBe
      empty // should be cleaned
  ...
  test(
    ...
    def compareTrackers(
      ...
      subject.getBlocksOfBatchAndStream(t(3), streamId) should
        be(base.getBlocksOfBatchAndStream(t(3), streamId))
      subject.getBlocksOfBatchAndStream(t(1), streamId) should
        be(base.getBlocksOfBatchAndStream(t(1), streamId))
    ...
    logFiles.flatMap { file =>
      ...
      val validBuffer =
        if (WriteAheadLogUtils.isBatchingEnabled(conf, isDriver = true)) {
          Utils.deserialize[Array[Array[Byte]]](byteBuffer.array())
            .map(ByteBuffer.wrap)
        } else { Array(byteBuffer) }
      validBuffer
        .map(b => Utils.deserialize[ReceivedBlockTrackerLogEvent](b.array()))
    }.toList
  }
