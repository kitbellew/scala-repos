...
class StreamingContextSuite
  ...
  test("stop slow receiver gracefully") {
    ...
    val input = ssc
      .receiverStream(new SlowTestReceiver(totalNumRecords, recordsPerSecond))
    input.count().foreachRDD { rdd =>
      val count = rdd.first()
      runningCount += count.toInt
      logInfo("Count = " + count + ", Running count = " + runningCount)
  ...
  test("registering and de-registering of streamingSource") {
    ...
    ssc.stop()
    val sourcesAfterStop = StreamingContextSuite
      .getSources(ssc.env.metricsSystem)
  ...
  test("awaitTermination") {
    ...
    assert(
      exception.isInstanceOf[TestFailedDueToTimeoutException],
      "Did not wait for stop")
  ...
  test("awaitTermination with error in task") {
    ...
    assert(
      exception.getMessage.contains("map task"),
  ...
  test("awaitTermination with error in job generation") {
    ...
    assert(
      exception.getMessage.contains("transform"),
      "Expected exception not thrown")
  }
  ...
  test("getOrCreate") {
    ...
    intercept[Exception] {
      ssc = StreamingContext
        .getOrCreate(corruptedCheckpointPath, creatingFunction _)
    }
    ...
    testGetOrCreate {
      ...
      assert(
        ssc.conf.get("someKey") === "someValue",
        "checkpointed config not recovered")
    }
    ...
    testGetOrCreate {
      ...
      assert(!newContextCreated, "old context not recovered")
      assert(
        !ssc.conf.contains("someKey"),
        "checkpointed config unexpectedly recovered")
    }
  }
  ...
  test("getActive and getActiveOrCreate") {
    ...
    testGetActiveOrCreate {
      ...
      ssc.start()
      assert(
        StreamingContext.getActive() === Some(ssc),
        "active context not returned")
      assert(
        StreamingContext.getActiveOrCreate(creatingFunc _) === ssc,
        "active context not returned")
      ssc.stop()
  ...
  test("getActiveOrCreate with checkpoint") {
    ...
    testGetActiveOrCreate {
      ...
      addInputStream(ssc).register()
      ssc.start()
      val returnedSsc = StreamingContext
        .getActiveOrCreate(checkpointPath, creatingFunction _)
      assert(!newContextCreated, "new context created instead of returning")
      assert(
        returnedSsc.eq(ssc),
        "returned context is not the activated context")
    }
  ...
  test("queueStream doesn't support checkpointing") {
    ...
    val e = intercept[SparkException] {
      ssc = StreamingContext
        .getOrCreate(checkpointDirectory, creatingFunction _)
    }
  ...
  def createCorruptedCheckpoint(): String = {
    val checkpointDirectory = Utils.createTempDir().getAbsolutePath()
    val fakeCheckpointFile = Checkpoint
      .checkpointFile(checkpointDirectory, Time(1000))
  ...
  def test() {
    ...
    try {
      ...
      inputStream.foreachRDD { rdd =>
        ...
        foreachCallSiteCorrect = rdd.sparkContext.getCallSite().shortForm
      ...
      eventually(timeout(10000 millis), interval(10 millis)) {
        assert(
          rddGenerated && rddCreationSiteCorrect,
          "RDD creation site was not correct")
        assert(
