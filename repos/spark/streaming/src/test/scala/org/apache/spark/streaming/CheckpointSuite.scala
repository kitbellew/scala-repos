...
trait DStreamCheckpointTester {
  ...
  private def assertOutput[V: ClassTag](
    ...
    val setComparison = output
      .zip(expectedPartialOutput)
      .forall {
        case (o, e) =>
          o.toSet === e.toSet
      }
...
class CheckpointSuite
  ...
  test("basic rdd checkpoints + dstream graph checkpoint recovery") {
    ...
    val checkpointFiles = stateStream
      .checkpointData
      .currentCheckpointFiles
      .map(x => new File(x._2))
  ...
  test("recovery with file input stream") {
    ...
    def recordedFiles(ssc: StreamingContext): Seq[Int] = {
      val fileInputDStream = ssc
        .graph
        .getInputStreams()
        .head
        .asInstanceOf[FileInputDStream[_, _, _]]
      val filenames = fileInputDStream
    ...
    try {
      ...
      withStreamingContext(new StreamingContext(checkpointDir)) { ssc =>
        ...
        logInfo(
          "Output after restart = " + outputStream
            .output
            .asScala
            .mkString("[", ", ", "]"))
  ...
  test("recovery from checkpoint contains array object") {
    ...
    val ois =
      new ObjectInputStreamWithLoader(
