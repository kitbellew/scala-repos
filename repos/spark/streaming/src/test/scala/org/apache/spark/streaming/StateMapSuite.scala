...
class StateMapSuite extends SparkFunSuite {
  ...
  test("OpenHashMapBasedStateMap - serializing and deserializing") {
    ...
    testSerialization(
      map2,
      "error deserializing and serialized map with 1 delta + no new data")
    ...
    testSerialization(
      map3,
      "error deserializing and serialized map with 2 delta + no new data")
    map3.put(3, 600, 3)
    map3.remove(2)
    testSerialization(
      map3,
  ...
  private def assertMap(
    ...
    withClue(msg) {
      ...
      for (keyId <- refMapToTestWith.keys) {
        assert(
          mapToTest.get(keyId) === refMapToTestWith
            .get(keyId)
            .map {
              _._1
            })
      ...
      // Assert that every time threshold returns the correct data
      for (t <- 0L to (time + 1)) {
        val expectedRecords = refMapToTestWith
          .iterator
          .filter {
            _._2._2 < t
          }
