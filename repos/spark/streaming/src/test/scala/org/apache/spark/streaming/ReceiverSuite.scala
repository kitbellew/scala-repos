...
class ReceiverSuite extends TestSuiteBase with Timeouts with Serializable {
  ...
  ignore("block generator throttling") {
    ...
    val minExpectedMessagesPerBlock =
  ...
  test("write ahead log - generating and cleaning") {
    ...
    val logDirectory1 =
      new File(checkpointDirToLogDir(tempDirectory.getAbsolutePath, 0))
    val logDirectory2 =
      new File(checkpointDirToLogDir(tempDirectory.getAbsolutePath, 1))
  ...
  /**
    ...
    */
  class FakeBlockGeneratorListener(pushDelay: Long = 0)
