...
class DStreamClosureSuite extends SparkFunSuite with BeforeAndAfterAll {
  ...
  private def testMap(ds: DStream[Int]): Unit =
    expectCorrectException { ds.map { _ => return; 1 } }
  private def testFlatMap(ds: DStream[Int]): Unit =
    expectCorrectException { ds.flatMap { _ => return; Seq.empty } }
  private def testFilter(ds: DStream[Int]): Unit =
    expectCorrectException { ds.filter { _ => return; true } }
  ...
  private def testReduce(ds: DStream[Int]): Unit =
    expectCorrectException { ds.reduce { case (_, _) => return; 1 } }
  ...
  private def testReduceByWindow(ds: DStream[Int]): Unit = {
    ...
    expectCorrectException {
      ds.reduceByWindow(reduceF, reduceF, Seconds(1), Seconds(2))
    }
  }
  ...
  private def testMapValues(ds: DStream[(Int, Int)]): Unit =
    expectCorrectException { ds.mapValues { _ => return; 1 } }
  private def testFlatMapValues(ds: DStream[(Int, Int)]): Unit =
    expectCorrectException { ds.flatMapValues { _ => return; Seq.empty } }
