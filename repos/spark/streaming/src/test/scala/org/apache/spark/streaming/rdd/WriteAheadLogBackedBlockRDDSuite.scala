...
class WriteAheadLogBackedBlockRDDSuite
  ...
  private def testRDD(
    ...
    require(
      numPartitionsInWAL <= numPartitions,
    ...
    val rdd =
      new WriteAheadLogBackedBlockRDD[String](
        sparkContext,
        blockIds.toArray,
  ...
  private def generateWALRecordHandles(
      ...
      blockIds: Seq[BlockId]): Seq[FileBasedWriteAheadLogSegment] = {
    require(blockData.size === blockIds.size)
    val writer =
      new FileBasedWriteAheadLogWriter(
        new File(dir, "logFile").toString,
        hadoopConf)
    val segments = blockData
      .zip(blockIds)
      .map {
        case (data, id) =>
          writer
            .write(blockManager.dataSerialize(id, data.iterator).toByteBuffer)
      }
    writer.close()
    segments
  }
