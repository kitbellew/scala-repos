...
class BasicOperationsSuite extends TestSuiteBase {
  ...
  test("rdd cleanup - map and window") {
    ...
    assert(
      mappedStream.rememberDuration === rememberDuration +
  ...
  test("rdd cleanup - input blocks and persisted RDDs") {
    ...
    withTestServer(new TestServer()) { testServer =>
      withStreamingContext(new StreamingContext(conf, batchDuration)) { ssc =>
        ...
        def collectRddInfo() {
          // get all RDD info required for verification
        ...
        val latestPersistedRddId =
          persistentRddIds(persistentRddIds.keySet.max)
