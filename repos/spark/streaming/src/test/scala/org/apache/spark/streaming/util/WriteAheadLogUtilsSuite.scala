...
class WriteAheadLogUtilsSuite extends SparkFunSuite {
  ...
  test("log selection and creation") {
    ...
    val driverWALConf = new SparkConf().set(
    ...
    val receiverWALConf = new SparkConf().set(
    ...
    val receiverWALConf2 = new SparkConf().set(
      "spark.streaming.receiver.writeAheadLog.class",
    ...
    intercept[SparkException] {
      val receiverWALConf3 = new SparkConf().set(
        "spark.streaming.receiver.writeAheadLog.class",
  ...
  test("wrap WriteAheadLog in BatchedWriteAheadLog when batching is enabled") {
    ...
    // Verify setting driver WAL class
    val driverWALConf = getBatchedSparkConf.set(
      "spark.streaming.driver.writeAheadLog.class",
      classOf[MockWriteAheadLog0].getName())
    assertDriverLogClass[MockWriteAheadLog0](driverWALConf, isBatched = true)
    assertReceiverLogClass[FileBasedWriteAheadLog](driverWALConf)
    ...
    val receiverWALConf = getBatchedSparkConf.set(
      "spark.streaming.receiver.writeAheadLog.class",
      classOf[MockWriteAheadLog0].getName())
    assertDriverLogClass[FileBasedWriteAheadLog](
      receiverWALConf,
...
object WriteAheadLogUtilsSuite {
  ...
  class MockWriteAheadLog0() extends WriteAheadLog {
    ...
    override def readAll(): util.Iterator[ByteBuffer] = {
      null
    }
  ...
  class MockWriteAheadLog2(val conf: SparkConf, x: Int)
