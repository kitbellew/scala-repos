...
private[spark] object SerializationDebugger extends Logging {
  ...
  private class SerializationDebugger {
    ...
    def visit(o: Any, stack: List[String]): List[String] = {
      if (o == null) { List.empty }
      else if (visited.contains(o)) { List.empty }
      else {
        visited += o
        o match {
          ...
          case _
              if o.getClass.isArray &&
                o.getClass.getComponentType.isPrimitive => List.empty
    ...
    private def visitSerializableWithWriteObjectMethod(
      ...
      } else { visited ++= innerObjectsCatcher.outputArray }
      return List.empty
    }
  }
  ...
  private def findObjectAndDescriptor(
    ...
    if (!desc.hasWriteReplaceMethod) { (o, desc) }
    else {
      // write place
