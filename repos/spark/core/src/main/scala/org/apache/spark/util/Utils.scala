...
private[spark] object Utils extends Logging {
  ...
  private def filesEqualRecursive(file1: File, file2: File): Boolean = {
    if (file1.isDirectory && file2.isDirectory) {
      ...
      subfiles1
        .sortBy(_.getName)
        .zip(subfiles2.sortBy(_.getName))
        .forall {
          case (f1, f2) =>
  ...
  private var customHostname: Option[String] = sys
    .env
  ...
  def getThreadDump(): Array[ThreadStackTrace] = {
    ...
    val threadInfos = ManagementFactory
      .getThreadMXBean
      .dumpAllThreads(true, true)
      .filter(_ != null)
    threadInfos
      .sortBy(_.getThreadId)
      .map {
        case threadInfo =>
          val stackTrace = threadInfo
            .getStackTrace
            .map(_.toString)
            .mkString("\n")
          ThreadStackTrace(
