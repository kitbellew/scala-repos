...
private[spark] trait Spillable[C] extends Logging {
  ...
  // For testing only
  private[this] val initialMemoryThreshold: Long = SparkEnv
    .get
    .conf
  ...
  // For testing only
  private[this] val numElementsForceSpillThreshold: Long = SparkEnv
    .get
    .conf
    .getLong(
  ...
  protected def maybeSpill(collection: C, currentMemory: Long): Boolean = {
    ...
    if (elementsRead % 32 == 0 && currentMemory >= myMemoryThreshold) {
      ...
      val granted = taskMemoryManager
        .acquireExecutionMemory(amountToRequest, MemoryMode.ON_HEAP, null)
  ...
  /**
    ...
    */
  @inline
  private def logSpillage(size: Long) {
    val threadId = Thread.currentThread().getId
    logInfo(
      "Thread %d spilling in-memory map of %s to disk (%d time%s so far)"
        .format(
          threadId,
