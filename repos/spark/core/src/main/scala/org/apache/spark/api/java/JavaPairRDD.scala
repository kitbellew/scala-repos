...
class JavaPairRDD[K, V](val rdd: RDD[(K, V)])(
  ...
  def sampleByKey(
      ...
      seed: Long): JavaPairRDD[K, V] =
    new JavaPairRDD[K, V](
      rdd.sampleByKey(withReplacement, fractions.asScala, seed))
  ...
  def sampleByKeyExact(
      ...
      seed: Long): JavaPairRDD[K, V] =
    new JavaPairRDD[K, V](
      rdd.sampleByKeyExact(withReplacement, fractions.asScala, seed))
  ...
  def countByKeyApprox(
      timeout: Long): PartialResult[java.util.Map[K, BoundedDouble]] =
  ...
  /**
    ...
    */
  def flatMapValues[U](
      f: JFunction[V, java.lang.Iterable[U]]): JavaPairRDD[K, U] = {
  ...
  /**
    ...
    */
  def cogroup[W](
      other: JavaPairRDD[K, W]): JavaPairRDD[K, (JIterable[V], JIterable[W])] =
...
object JavaPairRDD {
  ...
  private[spark] implicit def toScalaFunction[T, R](
      fun: JFunction[T, R]): T => R = x => fun.call(x)

  private[spark] implicit def pairFunToScalaFun[A, B, C](
      x: PairFunction[A, B, C]): A => (B, C) = y => x.call(y)
