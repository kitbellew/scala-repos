...
private[spark] object TaskState extends Enumeration {
  ...
  def toMesos(state: TaskState): MesosTaskState =
    state match {
      case LAUNCHING =>
  ...
  def fromMesos(mesosState: MesosTaskState): TaskState =
    mesosState match {
      case MesosTaskState.TASK_STAGING =>
        LAUNCHING
      case MesosTaskState.TASK_STARTING =>
        LAUNCHING
      case MesosTaskState.TASK_RUNNING =>
        RUNNING
      case MesosTaskState.TASK_FINISHED =>
        FINISHED
      ...
      case MesosTaskState.TASK_LOST =>
        LOST
      case MesosTaskState.TASK_ERROR =>
        LOST
    }
}
