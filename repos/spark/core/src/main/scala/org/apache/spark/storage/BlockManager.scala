...
private[spark] class BlockManager(
  ...
  private val externalShuffleServicePort = {
    val tmpPort = Utils
      .getSparkOrYarnConfig(conf, "spark.shuffle.service.port", "7337").toInt
    if (tmpPort == 0) {
   ...
   * loaded yet. */
  private lazy val compressionCodec: CompressionCodec = CompressionCodec
    .createCodec(conf)
  ...
  private def registerWithExternalShuffleServer() {
    ...
    for (i <- 1 to MAX_ATTEMPTS) {
      try {
        ...
        shuffleClient.asInstanceOf[ExternalShuffleClient]
    ...
    asyncReregisterLock.synchronized {
      if (asyncReregisterTask == null) {
        asyncReregisterTask = Future[Unit] {
          ...
          // thread pool
          reregister()
          asyncReregisterLock.synchronized { asyncReregisterTask = null }
  ...
  def waitForAsyncReregister(): Unit = {
    ...
    if (task != null) { Await.ready(task, Duration.Inf) }
  }
  ...
  private def reportBlockStatus(
    ...
    val needReregister =
      !tryToReportBlockStatus(blockId, info, status, droppedMemorySize)
    if (needReregister) {
      ...
      case Some(info) =>
        ...
        if (level.useMemory && memoryStore.contains(blockId)) {
          val iter: Iterator[Any] =
            if (level.deserialized) { memoryStore.getValues(blockId).get }
            else { dataDeserialize(blockId, memoryStore.getBytes(blockId).get) }
  ...
  private def doGetLocalBytes(
      blockId: BlockId,
      info: BlockInfo): ChunkedByteBuffer = {
    val level = info.level
    logDebug(s"Level for block $blockId is $level")
  ...
  def getRemoteBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    ...
    while (locationIterator.hasNext) {
      ...
      if (data != null) { return Some(new ChunkedByteBuffer(data)) }
  ...
  /**
    ...
    */
  def releaseLock(blockId: BlockId): Unit = { blockInfoManager.unlock(blockId) }
    ...
    doPut(
      ...
      if (level.useMemory) {
        ...
        val putSucceeded =
          ...
          } else { memoryStore.putBytes(blockId, size, () => bytes) }
        if (!putSucceeded && level.useDisk) {
          logWarning(s"Persisting block $blockId to disk instead.")
          diskStore.putBytes(blockId, bytes)
        }
      } else if (level.useDisk) { diskStore.putBytes(blockId, bytes) }
      ...
      if (blockWasSuccessfullyStored) { None }
      else { Some(bytes) }
    }.isEmpty
  }
  ...
  private def doPut[T](
    ...
    val putBlockInfo = {
      ...
      if (blockInfoManager.lockNewBlockForWriting(blockId, newInfo)) { newInfo }
      else {
        logWarning(
    ...
    val result: Option[T] =
      try {
        ...
        if (blockWasSuccessfullyStored) {
          if (keepReadLock) { blockInfoManager.downgradeLock(blockId) }
          else { blockInfoManager.unlock(blockId) }
        } else {
    ...
    if (level.replication > 1) {
      logDebug("Putting block %s with replication took %s".format(
        blockId,
        Utils.getUsedTimeMs(startTimeMs)))
    } else {
      logDebug("Putting block %s without replication took %s".format(
        blockId,
        Utils.getUsedTimeMs(startTimeMs)))
    }
    result
    ...
    doPut(
      ...
      if (level.useMemory) {
        ...
        memoryStore.putIterator(blockId, iterator(), level) match {
          ...
          case Left(iter) =>
            ...
            } else { iteratorFromFailedMemoryStorePut = Some(iter) }
        }
      ...
      if (blockWasSuccessfullyStored) {
        ...
        if (level.replication > 1) {
          ...
          try { replicate(blockId, bytesToReplicate, level) }
  ...
  private def getPeers(forceFetch: Boolean): Seq[BlockManagerId] = {
    peerFetchLock.synchronized {
      ...
      if (cachedPeers == null || forceFetch || timeout) {
        ...
        logDebug(
          "Fetched peers from master: " + cachedPeers.mkString("[", ",", "]"))
      }
      cachedPeers
    }
  }
  ...
  def dropFromMemory(
    ...
    if (blockIsRemoved) { blockIsUpdated = true }
    else {
      logWarning(
  ...
  def removeRdd(rddId: Int): Int = {
    ...
    val blocksToRemove = blockInfoManager.entries.flatMap(_._1.asRDDId)
  ...
  def dataSerialize(
      blockId: BlockId,
      values: Iterator[Any]): ChunkedByteBuffer = {
  ...
  def dataDeserializeStream(
      blockId: BlockId,
      inputStream: InputStream): Iterator[Any] = {
    val stream = new BufferedInputStream(inputStream)
    defaultSerializer.newInstance()
      .deserializeStream(wrapForCompression(blockId, stream)).asIterator
  }
...
private[spark] object BlockManager {
  ...
  def blockIdsToHosts(
    ...
    val blockLocations: Seq[Seq[BlockManagerId]] =
      ...
      } else { blockManagerMaster.getLocations(blockIds) }
