...
private[spark] object JsonProtocol {
  ...
  def stageInfoToJson(stageInfo: StageInfo): JValue = {
    ...
    val submissionTime = stageInfo
      .submissionTime
      .map(JInt(_))
      .getOrElse(JNothing)
    val completionTime = stageInfo
      .completionTime
      .map(JInt(_))
      .getOrElse(JNothing)
    val failureReason = stageInfo
      .failureReason
      .map(JString(_))
      .getOrElse(JNothing)
  ...
  def taskMetricsToJson(taskMetrics: TaskMetrics): JValue = {
    val shuffleReadMetrics: JValue = taskMetrics
      .shuffleReadMetrics
      .map { rm =>
      ...
      .getOrElse(JNothing)
    val shuffleWriteMetrics: JValue = taskMetrics
      .shuffleWriteMetrics
      .map { wm =>
      ...
      .getOrElse(JNothing)
    val inputMetrics: JValue = taskMetrics
      .inputMetrics
      .map { im =>
      ...
      .getOrElse(JNothing)
    val outputMetrics: JValue = taskMetrics
      .outputMetrics
      .map { om =>
  ...
  def taskStartFromJson(json: JValue): SparkListenerTaskStart = {
    ...
    val stageAttemptId = (json \ "Stage Attempt ID")
      .extractOpt[Int]
      .getOrElse(0)
  ...
  def jobStartFromJson(json: JValue): SparkListenerJobStart = {
    ...
    val submissionTime = Utils
      .jsonOption(json \ "Submission Time")
      .map(_.extract[Long])
      .getOrElse(-1L)
    val stageIds = (json \ "Stage IDs")
      .extract[List[JValue]]
      .map(_.extract[Int])
    val properties = propertiesFromJson(json \ "Properties")
    // The "Stage Infos" field was added in Spark 1.2.0
    val stageInfos = Utils
  ...
  def jobEndFromJson(json: JValue): SparkListenerJobEnd = {
    val jobId = (json \ "Job ID").extract[Int]
    val completionTime = Utils
      .jsonOption(json \ "Completion Time")
      .map(_.extract[Long])
      .getOrElse(-1L)
  ...
  def applicationStartFromJson(json: JValue): SparkListenerApplicationStart = {
    ...
    val appAttemptId = Utils
      .jsonOption(json \ "App Attempt ID")
  ...
  def executorMetricsUpdateFromJson(
    ...
    val accumUpdates = (json \ "Metrics Updated")
      .extract[List[JValue]]
      .map { json =>
  ...
  def taskInfoFromJson(json: JValue): TaskInfo = {
    ...
    val speculative = (json \ "Speculative")
      .extractOpt[Boolean]
      .getOrElse(false)
  ...
  def accumulableInfoFromJson(json: JValue): AccumulableInfo = {
    ...
    val update = Utils
      .jsonOption(json \ "Update")
      .map { v =>
        accumValueFromJson(name, v)
      }
    val value = Utils
      .jsonOption(json \ "Value")
      .map { v =>
        accumValueFromJson(name, v)
      }
    val internal = (json \ "Internal").extractOpt[Boolean].getOrElse(false)
    val countFailedValues = (json \ "Count Failed Values")
      .extractOpt[Boolean]
      .getOrElse(false)
  ...
  def taskMetricsFromJson(json: JValue): TaskMetrics = {
    ...
    Utils
      .jsonOption(json \ "Shuffle Read Metrics")
      .foreach { readJson =>
        ...
        readMetrics.incLocalBlocksFetched(
          (readJson \ "Local Blocks Fetched").extract[Int])
        readMetrics
        ...
        readMetrics
          .incFetchWaitTime((readJson \ "Fetch Wait Time").extract[Long])
    ...
    Utils
      .jsonOption(json \ "Shuffle Write Metrics")
      .foreach { writeJson =>
    ...
    // Output metrics
    Utils
      .jsonOption(json \ "Output Metrics")
      .foreach { outJson =>
    ...
    // Updated blocks
    Utils
      .jsonOption(json \ "Updated Blocks")
      .foreach { blocksJson =>
  ...
  def taskEndReasonFromJson(json: JValue): TaskEndReason = {
    ...
    (json \ "Reason").extract[String] match {
      ...
      case `taskCommitDenied` =>
        ...
        val jobId = Utils
          .jsonOption(json \ "Job ID")
          .map(_.extract[Int])
          .getOrElse(-1)
        val partitionId = Utils
          .jsonOption(json \ "Partition ID")
          .map(_.extract[Int])
          .getOrElse(-1)
        val attemptNo = Utils
          .jsonOption(json \ "Attempt Number")
          .map(_.extract[Int])
          .getOrElse(-1)
        TaskCommitDenied(jobId, partitionId, attemptNo)
      case `executorLostFailure` =>
        val exitCausedByApp = Utils
          .jsonOption(json \ "Exit Caused By App")
          .map(_.extract[Boolean])
        val executorId = Utils
          .jsonOption(json \ "Executor ID")
          .map(_.extract[String])
        val reason = Utils
          .jsonOption(json \ "Loss Reason")
          .map(_.extract[String])
        ExecutorLostFailure(
  ...
  def rddInfoFromJson(json: JValue): RDDInfo = {
    ...
    val scope = Utils
      .jsonOption(json \ "Scope")
      .map(_.extract[String])
      .map(RDDOperationScope.fromJson)
    val callsite = Utils
      .jsonOption(json \ "Callsite")
      .map(_.extract[String])
      .getOrElse("")
    val parentIds = Utils
      .jsonOption(json \ "Parent IDs")
