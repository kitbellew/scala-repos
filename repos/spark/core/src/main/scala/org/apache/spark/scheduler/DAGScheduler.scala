...
private[spark] class DAGScheduler(
  ...
  private val disallowStageRetryForTest = sc.getConf
    .getBoolean("spark.test.noStageRetry", false)

  private val messageScheduler = ThreadUtils
    .newDaemonSingleThreadScheduledExecutor("dag-scheduler-message")

  private[scheduler] val eventProcessLoop = new DAGSchedulerEventProcessLoop(
    this)
  ...
  def taskEnded(
    ...
    eventProcessLoop
      .post(CompletionEvent(task, reason, result, accumUpdates, taskInfo))
  }
      ...
      if (!cacheLocs.contains(rdd.id)) {
        ...
        val locs: IndexedSeq[Seq[TaskLocation]] =
          if (rdd.getStorageLevel == StorageLevel.NONE) {
            ...
            val blockIds = rdd.partitions.indices
              .map(index => RDDBlockId(rdd.id, index)).toArray[BlockId]
  ...
  private def clearCacheLocs(): Unit =
    cacheLocs.synchronized { cacheLocs.clear() }
  ...
  /**
    ...
    */
  private def getParentStagesAndId(
      rdd: RDD[_],
  ...
  private def newShuffleMapStage(
    ...
    val (parentStages: List[Stage], id: Int) = getParentStagesAndId(
      rdd,
      firstJobId)
    val stage: ShuffleMapStage = new ShuffleMapStage(
      id,
      rdd,
      numTasks,
      parentStages,
  ...
  private def newResultStage(
    ...
    val stage =
      new ResultStage(id, rdd, func, partitions, parentStages, jobId, callSite)
    stageIdToStage(id) = stage
  ...
  private def newOrUsedShuffleStage(
    ...
    if (mapOutputTracker.containsShuffle(shuffleDep.shuffleId)) {
      val serLocs = mapOutputTracker
        .getSerializedMapOutputStatuses(shuffleDep.shuffleId)
  ...
  private def cleanupStateForJobAndIndependentStages(job: ActiveJob): Unit = {
    ...
    if (registeredStages.isEmpty || registeredStages.get.isEmpty) {
      ...
      stageIdToStage
        .filterKeys(stageId => registeredStages.get.contains(stageId)).foreach {
          case (stageId, stage) =>
  ...
  private[scheduler] def doCancelAllJobs() {
    ...
    runningStages.map(_.firstJobId).foreach(
  ...
  private[scheduler] def handleBeginEvent(task: Task[_], taskInfo: TaskInfo) {
    ...
    val stageAttemptId = stageIdToStage.get(task.stageId)
      .map(_.latestInfo.attemptId).getOrElse(-1)
    listenerBus
      .post(SparkListenerTaskStart(task.stageId, stageAttemptId, taskInfo))
    submitWaitingStages()
  }
    ...
    val taskIdToLocations: Map[Int, Seq[TaskLocation]] =
      try {
        ...
        case NonFatal(e) =>
          ...
          listenerBus
            .post(SparkListenerStageSubmitted(stage.latestInfo, properties))
          abortStage(
            stage,
  ...
  private def updateAccumulators(event: CompletionEvent): Unit = {
    ...
    try {
      event.accumUpdates.foreach { ainfo =>
        assert(
          ainfo.update.isDefined,
          "accumulator from task should have a partial value")
        val id = ainfo.id
      ...
      case NonFatal(e) =>
        logError(
          s"Failed to update accumulators for task ${task.partitionId}",
          e)
    }
  }
                ...
                if (!job.finished(rt.outputId)) {
                  ...
                  try { job.listener.taskSucceeded(rt.outputId, event.result) }
                    ...
                    case e: Exception =>
                      ...
                      job.listener
                        .jobFailed(new SparkDriverExecutionException(e))
                  }
                }
              case None =>
                logInfo(
  ...
  private[scheduler] def abortStage(
    ...
    val dependentJobs: Seq[ActiveJob] = activeJobs
  ...
  private def failJobAndIndependentStages(
    ...
    stages.foreach { stageId =>
      val jobsForStage: Option[HashSet[Int]] = stageIdToStage.get(stageId)
        .map(_.jobIds)
