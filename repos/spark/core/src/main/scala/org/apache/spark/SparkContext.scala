...
class SparkContext(config: SparkConf)
  ...
  def deployMode: String =
    _conf.getOption("spark.submit.deployMode").getOrElse("client")
  def appName: String = _conf.get("spark.app.name")

  private[spark] def isEventLogEnabled: Boolean =
    _conf.getBoolean("spark.eventLog.enabled", false)
  ...
  private[spark] def getExecutorThreadDump(
    ...
    try if (executorId == SparkContext.DRIVER_IDENTIFIER)
  ...
  def binaryRecords(
    ...
    assertNotStopped()
    conf
      .setInt(FixedLengthBinaryInputFormat.RECORD_LENGTH_PROPERTY, recordLength)
    ...
    if (!Array("http", "https", "ftp").contains(scheme)) {
      ...
      if (!fs.exists(hadoopPath))
        throw new FileNotFoundException(
          s"Added file $hadoopPath does not exist.")
      val isDir = fs.getFileStatus(hadoopPath).isDirectory
      if (!isLocal && scheme == "file" && isDir)
        throw new SparkException(
          s"addFile does not support local directories when not running " +
            "local mode.")
      if (!recursive && isDir)
        throw new SparkException(
          s"Added file $hadoopPath is a directory and recursive is not " +
            "turned on.")
    }
  ...
  private def setupAndStartListenerBus(): Unit = {
    ...
    try {
      ...
      case e: Exception =>
        try stop()
        finally throw new SparkException(
