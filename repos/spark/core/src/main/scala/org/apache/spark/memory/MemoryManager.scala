...
private[spark] abstract class MemoryManager(
  ...
  final def setMemoryStore(store: MemoryStore): Unit =
    synchronized { storageMemoryPool.setMemoryStore(store) }
  ...
  def releaseStorageMemory(numBytes: Long): Unit =
    synchronized { storageMemoryPool.releaseMemory(numBytes) }

  /**
    * Release all storage memory acquired.
    */
  final def releaseAllStorageMemory(): Unit =
    synchronized { storageMemoryPool.releaseAllMemory() }

  /**
    * Release N bytes of unroll memory.
    */
  final def releaseUnrollMemory(numBytes: Long): Unit =
    synchronized { releaseStorageMemory(numBytes) }
  ...
  final def storageMemoryUsed: Long =
    synchronized { storageMemoryPool.memoryUsed }
