...
private[mesos] trait MesosSchedulerUtils extends Logging {
  ...
  protected def createSchedulerDriver(
    ...
    conf
      .getOption("spark.mesos.principal")
      .foreach { principal =>
        fwInfoBuilder.setPrincipal(principal)
        credBuilder.setPrincipal(principal)
      }
    conf
      .getOption("spark.mesos.secret")
      .foreach { secret =>
  ...
  def partitionResources(
    ...
    val remainingResources = resources
      .asScala
      .map {
        case r => {
  ...
  /**
    ...
    */
  def parseConstraintString(
      constraintsVal: String): Map[String, Set[String]] = {
    /*
      Based on mesos docs:
