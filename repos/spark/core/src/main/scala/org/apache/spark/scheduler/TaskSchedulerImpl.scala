...
private[spark] class TaskSchedulerImpl(
  ...
  val SPECULATION_INTERVAL_MS =
    conf.getTimeAsMs("spark.speculation.interval", "100ms")
  ...
  private[scheduler] val taskIdToTaskSetManager =
    new HashMap[Long, TaskSetManager]
  ...
  def resourceOffers(offers: Seq[WorkerOffer]): Seq[Seq[TaskDescription]] =
    synchronized {
      ...
      val tasks =
        shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores))
  ...
  def handleSuccessfulTask(
    ...
    synchronized { taskSetManager.handleSuccessfulTask(tid, taskResult) }
  ...
  def hasExecutorsAliveOnHost(host: String): Boolean =
    synchronized { executorsByHost.contains(host) }

  def hasHostAliveOnRack(rack: String): Boolean =
    synchronized { hostsByRack.contains(rack) }

  def isExecutorAlive(execId: String): Boolean =
    synchronized { executorIdToTaskCount.contains(execId) }

  def isExecutorBusy(execId: String): Boolean =
    synchronized { executorIdToTaskCount.getOrElse(execId, -1) > 0 }
