...
class PairRDDFunctions[K, V](self: RDD[(K, V)])(implicit
    ...
    self.withScope {
      ...
      if (self.partitioner == Some(partitioner)) {
        ...
        new ShuffledRDD[K, V, C](self, partitioner).setSerializer(serializer)
          .setAggregator(aggregator).setMapSideCombine(mapSideCombine)
      }
    }
  ...
  def aggregateByKey[U: ClassTag](zeroValue: U, partitioner: Partitioner)(
    ...
    self.withScope {
      ...
      val zeroBuffer = SparkEnv.get.serializer.newInstance()
        .serialize(zeroValue)
  ...
  def foldByKey(zeroValue: V)(func: (V, V) => V): RDD[(K, V)] =
    self.withScope { foldByKey(zeroValue, defaultPartitioner(self))(func) }
  ...
  def reduceByKey(func: (V, V) => V): RDD[(K, V)] =
    self.withScope { reduceByKey(defaultPartitioner(self), func) }
  ...
  def groupByKey(numPartitions: Int): RDD[(K, Iterable[V])] =
    self.withScope { groupByKey(new HashPartitioner(numPartitions)) }
  ...
  def groupByKey(): RDD[(K, Iterable[V])] =
    self.withScope { groupByKey(defaultPartitioner(self)) }
  ...
  def join[W](other: RDD[(K, W)]): RDD[(K, (V, W))] =
    self.withScope { join(other, defaultPartitioner(self, other)) }
  ...
  def leftOuterJoin[W](
    ...
    self.withScope { leftOuterJoin(other, new HashPartitioner(numPartitions)) }
  ...
  def rightOuterJoin[W](
    ...
    self.withScope { rightOuterJoin(other, new HashPartitioner(numPartitions)) }
  ...
  def fullOuterJoin[W](
    ...
    self.withScope { fullOuterJoin(other, new HashPartitioner(numPartitions)) }
  ...
  def subtractByKey[W: ClassTag](
    ...
    self.withScope { subtractByKey(other, new HashPartitioner(numPartitions)) }

  /** Return an RDD with the pairs from `this` whose keys are not in `other`. */
  def subtractByKey[W: ClassTag](
      other: RDD[(K, W)],
      p: Partitioner): RDD[(K, V)] =
    self.withScope { new SubtractedRDD[K, V, W](self, other, p) }
  ...
  def saveAsNewAPIHadoopDataset(conf: Configuration): Unit =
    self.withScope {
      ...
      val jobTaskContext =
        new TaskAttemptContextImpl(wrappedConf.value, jobAttemptId)
...
private[spark] object PairRDDFunctions {
  ...
  /**
    ...
    */
  val disableOutputSpecValidation: DynamicVariable[Boolean] =
