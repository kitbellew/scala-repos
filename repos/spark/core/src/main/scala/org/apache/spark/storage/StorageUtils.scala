...
class StorageStatus(val blockManagerId: BlockManagerId, val maxMem: Long) {
  ...
  /**
    ...
    */
  private val _rddBlocks =
    new mutable.HashMap[Int, mutable.Map[BlockId, BlockStatus]]
  ...
  private val _rddStorageInfo =
    new mutable.HashMap[Int, (Long, Long, StorageLevel)]
  ...
  def rddBlocksById(rddId: Int): Map[BlockId, BlockStatus] =
    _rddBlocks.getOrElse(rddId, Map.empty)
  ...
  def containsBlock(blockId: BlockId): Boolean = {
    blockId match {
      case RDDBlockId(rddId, _) => _rddBlocks.get(rddId)
          .exists(_.contains(blockId))
  ...
  def diskUsedByRdd(rddId: Int): Long =
    _rddStorageInfo.get(rddId).map(_._2).getOrElse(0L)

  /** Return the storage level, if any, used by the given RDD in this block manager. */
  def rddStorageLevel(rddId: Int): Option[StorageLevel] =
    _rddStorageInfo.get(rddId).map(_._3)
  ...
  private def updateStorageInfo(
    ...
    val (oldMem, oldDisk) = blockId match {
      case RDDBlockId(rddId, _) => _rddStorageInfo.get(rddId).map {
...
private[spark] object StorageUtils extends Logging {
  ...
  def updateRddInfo(
    ...
    rddInfos.foreach { rddInfo =>
      ...
      val storageLevel = statuses.flatMap(_.rddStorageLevel(rddId)).headOption
