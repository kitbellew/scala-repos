...
class ExecutorsListener(
  ...
  override def onTaskEnd(taskEnd: SparkListenerTaskEnd): Unit = synchronized {
    ...
    if (info != null) {
      ...
      executorToDuration(eid) = executorToDuration
        .getOrElse(eid, 0L) + info.duration
        ...
        metrics.outputMetrics.foreach { outputMetrics =>
          executorToOutputBytes(eid) =
            executorToOutputBytes
              .getOrElse(eid, 0L) + outputMetrics.bytesWritten
        ...
        metrics.shuffleReadMetrics.foreach { shuffleRead =>
          executorToShuffleRead(eid) =
            executorToShuffleRead
              .getOrElse(eid, 0L) + shuffleRead.remoteBytesRead
        }
