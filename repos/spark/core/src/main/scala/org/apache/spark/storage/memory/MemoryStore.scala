...
private[spark] class MemoryStore(
  ...
  private val unrollMemoryThreshold: Long = conf
    .getLong("spark.storage.unrollMemoryThreshold", 1024 * 1024)
  ...
  private def blocksMemoryUsed: Long =
    memoryManager.synchronized { memoryUsed - currentUnrollMemory }
        ...
        while (freedMemory < space && iterator.hasNext) {
          ...
          if (blockIsEvictable(blockId)) {
            ...
            if (blockManager.blockInfoManager
                  .lockForWriting(blockId, blocking = false).isDefined) {
      ...
      if (freedMemory >= space) {
        ...
        for (blockId <- selectedBlocks) {
          ...
          if (entry != null) {
            ...
            val newEffectiveStorageLevel = blockManager
  ...
  def currentUnrollMemory: Long =
    memoryManager.synchronized { unrollMemoryMap.values.sum }
...
private[storage] class PartiallyUnrolledIterator(
  ...
  private[this] var iter: Iterator[Any] = {
    val completionIterator = CompletionIterator[Any, Iterator[Any]](
      unrolled, {
