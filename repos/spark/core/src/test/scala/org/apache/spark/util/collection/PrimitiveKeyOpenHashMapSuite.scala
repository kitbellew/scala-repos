...
class PrimitiveKeyOpenHashMapSuite extends SparkFunSuite with Matchers {
  ...
  test("basic operations") {
    ...
    for (i <- 1 to 1000) { assert(map(i + longBase) === i) }
  ...
  test("null values") {
    ...
    for (i <- 1 to 100) { map(i.toLong) = null }
    assert(map.size === 100)
  ...
  test("changeValue") {
    val map = new PrimitiveKeyOpenHashMap[Long, String]()
    for (i <- 1 to 100) { map(i.toLong) = i.toString }
    assert(map.size === 100)
    for (i <- 1 to 100) {
    ...
    for (i <- 101 to 400) {
      val res = map
        .changeValue(i.toLong, { i + "!" }, v => { assert(false); v })
      assert(res === i + "!")
    }
    assert(map.size === 400)
  }
  ...
  test("inserting in capacity-1 map") {
    ...
    for (i <- 1 to 100) { map(i.toLong) = i.toString }
    assert(map.size === 100)
    for (i <- 1 to 100) { assert(map(i.toLong) === i.toString) }
  }
