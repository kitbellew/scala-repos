...
abstract class RpcEnvSuite extends SparkFunSuite with BeforeAndAfterAll {
  ...
  test("send a message locally") {
    ...
    val rpcEndpointRef = env.setupEndpoint(
      ...
      new RpcEndpoint {
        ...
        override def receive = { case msg: String => message = msg }
      })
  ...
  test("send a message remotely") {
    ...
    val rpcEndpointRef = anotherEnv
      .setupEndpointRef(env.address, "send-remotely")
    try {
      rpcEndpointRef.send("hello")
      eventually(timeout(5 seconds), interval(10 millis)) {
        assert("hello" === message)
  ...
  test("ask a message remotely") {
    ...
    // Use anotherEnv to find out the RpcEndpointRef
    val rpcEndpointRef = anotherEnv
      .setupEndpointRef(env.address, "ask-remotely")
    try {
      val reply = rpcEndpointRef.askWithRetry[String]("hello")
      assert("hello" === reply)
    } finally {
  ...
  test("onError: error in onStop") {
    ...
    val endpointRef = env.setupEndpoint(
      ...
      new RpcEndpoint {
        ...
        override def onStop(): Unit = { throw new RuntimeException("Oops!") }
      }
    )
  ...
  test("call receive in sequence") {
    ...
    for (i <- 0 until 100) {
      ...
      (0 until 10) foreach { _ =>
        new Thread {
          override def run() {
            (0 until 100) foreach { _ => endpointRef.send("Hello") }
          }
        }.start()
      }
  ...
  test("sendWithReply: remotely") {
    ...
    // Use anotherEnv to find out the RpcEndpointRef
    val rpcEndpointRef = anotherEnv
      .setupEndpointRef(env.address, "sendWithReply-remotely")
    try {
  ...
  test("sendWithReply: error") {
    ...
    val f = endpointRef.ask[String]("Hi")
    val e = intercept[SparkException] { Await.result(f, 5 seconds) }
  ...
  test("sendWithReply: remotely error") {
    ...
    // Use anotherEnv to find out the RpcEndpointRef
    val rpcEndpointRef = anotherEnv
      .setupEndpointRef(env.address, "sendWithReply-remotely-error")
    try {
      val f = rpcEndpointRef.ask[String]("hello")
      val e = intercept[SparkException] { Await.result(f, 5 seconds) }
      assert("Oops" === e.getMessage)
  ...
  test("network events in sever RpcEnv when another RpcEnv is in server mode") {
    val serverEnv1 = createRpcEnv(
      new SparkConf(),
      "server1",
      0,
      clientMode = false)
    val serverEnv2 = createRpcEnv(
      new SparkConf(),
      "server2",
      0,
      clientMode = false)
    ...
    try {
      val serverRefInServer2 = serverEnv1
        .setupEndpointRef(serverRef2.address, serverRef2.name)
  ...
  test("network events in sever RpcEnv when another RpcEnv is in client mode") {
    val serverEnv = createRpcEnv(
      new SparkConf(),
      "server",
      0,
      clientMode = false)
    val (serverRef, events) = setupNetworkEndpoint(serverEnv, "network-events")
    val clientEnv = createRpcEnv(
      new SparkConf(),
      "client",
      0,
      clientMode = true)
    try {
      val serverRefInClient = clientEnv
        .setupEndpointRef(serverRef.address, serverRef.name)
      // Send a message to set up the connection
  ...
  test("sendWithReply: unserializable error") {
    ...
    try {
      val f = rpcEndpointRef.ask[String]("hello")
      val e = intercept[Exception] { Await.result(f, 1 seconds) }
  ...
  test("send with authentication") {
    ...
    try {
      ...
      val rpcEndpointRef = remoteEnv
        .setupEndpointRef(localEnv.address, "send-authentication")
      rpcEndpointRef.send("hello")
      eventually(timeout(5 seconds), interval(10 millis)) {
        assert("hello" === message)
      }
  ...
  test("ask with authentication") {
    ...
    try {
      ...
      val rpcEndpointRef = remoteEnv
        .setupEndpointRef(localEnv.address, "ask-authentication")
      val reply = rpcEndpointRef.askWithRetry[String]("hello")
      assert("hello" === reply)
    } finally {
      localEnv.shutdown()
