...
class StoragePageSuite extends SparkFunSuite {
  ...
  test("rddTable") {
    ...
    val rdd2 = new RDDInfo(2, "rdd2", 10, StorageLevel.DISK_ONLY, Seq.empty)
    rdd2.diskSize = 200
    rdd2.numCachedPartitions = 5

    val rdd3 =
      new RDDInfo(3, "rdd3", 10, StorageLevel.MEMORY_AND_DISK_SER, Seq.empty)
    rdd3.memSize = 400
    rdd3.diskSize = 500
  ...
  test("receiverBlockTables") {
    ...
    assert(
      ((blockTable \\ "tr")(0) \\ "td")(0).attribute("rowspan").map(
        _.text) === Some("2"))
    assert(
      ((blockTable \\ "tr")(0) \\ "td")(1).attribute("rowspan").map(
        _.text) === Some("2"))
    ...
    // Check "rowspan=2" for the first 2 columns
    assert(
      ((blockTable \\ "tr")(2) \\ "td")(0).attribute("rowspan").map(
        _.text) === Some("2"))
    assert(
      ((blockTable \\ "tr")(2) \\ "td")(1).attribute("rowspan").map(
        _.text) === Some("2"))
