...
class MesosSchedulerUtilsSuite
  ...
  test("parse a non-empty constraint string correctly") {
    ...
    utils
      .parseConstraintString("tachyon:true;zone:us-east-1a,us-east-1b") should
      be(expectedMap)
  }
  ...
  test("empty values for attributes' constraints matches all values") {
    ...
    val zoneSet = Value.Set.newBuilder().addItem("us-east-1a")
      .addItem("us-east-1b").build()
    ...
    utils
      .matchesAttributeRequirements(parsedConstraints, noTachyonOffer) shouldBe
      false
    utils
  ...
  test("subset match is performed for set attributes") {
    val supersetConstraint = Map(
      "tachyon" -> Value.Text.newBuilder().setValue("true").build(),
      "zone" -> Value.Set.newBuilder().addItem("us-east-1a")
        .addItem("us-east-1b").addItem("us-east-1c").build()
    )
