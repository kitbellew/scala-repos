...
class DiskStoreSuite extends SparkFunSuite {
  ...
  test("reads of memory-mapped and non memory-mapped files are equivalent") {
    ...
    val diskBlockManager =
      new DiskBlockManager(new SparkConf(), deleteFilesOnStop = true)

    val diskStoreMapped =
      new DiskStore(new SparkConf().set(confKey, "0"), diskBlockManager)
    ...
    val diskStoreNotMapped =
      new DiskStore(new SparkConf().set(confKey, "1m"), diskBlockManager)
    diskStoreNotMapped.putBytes(blockId, byteBuffer)
    val notMapped = diskStoreNotMapped.getBytes(blockId)

    // Not possible to do isInstanceOf due to visibility of HeapByteBuffer
    assert(
      notMapped.getChunks().forall(
        _.getClass.getName.endsWith("HeapByteBuffer")),
