...
class StorageSuite extends SparkFunSuite {
  ...
  test("storage status containsBlock") {
    ...
    assert(
      status.blocks.contains(TestBlockId("dan")) ===
        status.containsBlock(TestBlockId("dan")))
    assert(
      status.blocks.contains(TestBlockId("man")) ===
        status.containsBlock(TestBlockId("man")))
    assert(
      status.blocks.contains(RDDBlockId(0, 0)) ===
        status.containsBlock(RDDBlockId(0, 0)))
    assert(
      status.blocks.contains(RDDBlockId(1, 1)) ===
        status.containsBlock(RDDBlockId(1, 1)))
    assert(
      status.blocks.contains(RDDBlockId(2, 2)) ===
        status.containsBlock(RDDBlockId(2, 2)))
    assert(
      status.blocks.contains(RDDBlockId(2, 3)) ===
        status.containsBlock(RDDBlockId(2, 3)))
    assert(
      status.blocks.contains(RDDBlockId(2, 4)) ===
        status.containsBlock(RDDBlockId(2, 4)))
    // blocks that don't exist
    assert(
      status.blocks.contains(TestBlockId("fan")) ===
        status.containsBlock(TestBlockId("fan")))
    assert(
      status.blocks.contains(RDDBlockId(100, 0)) ===
        status.containsBlock(RDDBlockId(100, 0)))
  }
  ...
  test("storage status getBlock") {
    ...
    // blocks that actually exist
    assert(
      status.blocks.get(TestBlockId("dan")) ===
        status.getBlock(TestBlockId("dan")))
    assert(
      status.blocks.get(TestBlockId("man")) ===
    ...
    // blocks that don't exist
    assert(
      status.blocks.get(TestBlockId("fan")) ===
        status.getBlock(TestBlockId("fan")))
    assert(
      status.blocks.get(RDDBlockId(100, 0)) ===
        status.getBlock(RDDBlockId(100, 0)))
  }
