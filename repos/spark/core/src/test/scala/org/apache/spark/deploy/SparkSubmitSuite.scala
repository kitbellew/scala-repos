...
class SparkSubmitSuite
  ...
  test("handles arguments to user program with name collision") {
    val clArgs = Seq(
      ...
      "args")
    val appArgs = new SparkSubmitArguments(clArgs)
    appArgs.childArgs should
      be(Seq("--master", "local", "some", "--weird", "args"))
  }
  ...
  test("resolves command line argument paths correctly") {
    ...
    sysProps3("spark.submit.pyFiles") should
      be(PythonRunner.formatPaths(Utils.resolveURIs(pyFiles)).mkString(","))
  }
  ...
  test("SPARK_CONF_DIR overrides spark-defaults.conf") {
    forConfDir(Map("spark.executor.memory" -> "2.3g")) { path =>
      ...
      val args = Seq(
        ...
        unusedJar.toString)
      val appArgs =
        new SparkSubmitArguments(args, Map("SPARK_CONF_DIR" -> path))
