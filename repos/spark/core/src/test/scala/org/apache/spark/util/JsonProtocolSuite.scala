...
class JsonProtocolSuite extends SparkFunSuite {
  ...
  test("SparkListenerEvent") {
    val stageSubmitted = SparkListenerStageSubmitted(
      ...
      properties)
    val stageCompleted = SparkListenerStageCompleted(
      makeStageInfo(101, 201, 301, 401L, 501L))
    val taskStart = SparkListenerTaskStart(
      111,
      0,
      makeTaskInfo(222L, 333, 1, 444L, false))
    val taskGettingResult = SparkListenerTaskGettingResult(
      makeTaskInfo(1000L, 2000, 5, 3000L, true))
  ...
  test("BlockManager events backward compatibility") {
    ...
    val oldBmRemoved = JsonProtocol
      .blockManagerRemovedToJson(blockManagerRemoved).removeField({
        _._1 == "Timestamp"
      })
  ...
  test("FetchFailed backwards compatibility") {
    ...
    val fetchFailed = FetchFailed(
      ...
      19,
      "ignored")
    val oldEvent = JsonProtocol.taskEndReasonToJson(fetchFailed).removeField({
      _._1 == "Message"
    })
  ...
  test("SparkListenerApplicationStart backwards compatibility") {
    ...
    val oldEvent = JsonProtocol.applicationStartToJson(applicationStart)
      .removeField({ _._1 == "App ID" }).removeField({
        _._1 == "App Attempt ID"
  ...
  test("SparkListenerJobStart backward compatibility") {
    ...
    val expectedJobStart = SparkListenerJobStart(
      10,
      jobSubmissionTime,
      dummyStageInfos,
      properties)
  ...
  test("SparkListenerJobStart and SparkListenerJobEnd backward compatibility") {
    ...
    val jobStart = SparkListenerJobStart(
      ...
      stageInfos,
      properties)
    val oldStartEvent = JsonProtocol.jobStartToJson(jobStart).removeField({
    ...
    val oldEndEvent = JsonProtocol.jobEndToJson(jobEnd).removeField({
  ...
  test("RDDInfo backward compatibility (scope, parent IDs, callsite)") {
    ...
    val oldRddInfoJson = JsonProtocol.rddInfoToJson(rddInfo).removeField({
      _._1 == "Parent IDs"
    }).removeField({ _._1 == "Scope" }).removeField({ _._1 == "Callsite" })
    val expectedRddInfo =
  ...
  test("StageInfo backward compatibility (parent IDs)") {
    ...
    val stageInfo =
      new StageInfo(1, 1, "me-stage", 1, Seq.empty, Seq(1, 2, 3), "details")
  ...
  test("TaskCommitDenied backward compatibility") {
    ...
    val oldDenied = JsonProtocol.taskEndReasonToJson(denied).removeField({
      _._1 == "Job ID"
    }).removeField({ _._1 == "Partition ID" }).removeField({
      _._1 == "Attempt Number"
    })
  ...
  test("AccumulableInfo backward compatibility") {
    ...
    val accumulableInfo = makeAccumulableInfo(
      1,
      internal = true,
...
private[spark] object JsonProtocolSuite extends Assertions {
  ...
  private def makeTaskMetrics(
    ...
    if (hasHadoopInput) {
      ...
      inputMetrics
        .incRecordsReadInternal(if (hasRecords) (d + e + f) / 100 else -1)
    } else {
  ...
  private val executorAddedJsonString = s"""
      |{
  ...
  private val executorRemovedJsonString = s"""
      |{
  ...
  private val executorMetricsUpdateJsonString = s"""
      |{
