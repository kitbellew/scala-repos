...
class PoolSuite extends SparkFunSuite with LocalSparkContext {
  ...
  def scheduleTaskAndVerifyId(
    ...
    val nextTaskSetToSchedule = taskSetQueue
      .find(t => (t.runningTasks + t.tasksSuccessful) < t.numTasks)
  ...
  test("Fair Scheduler Test") {
    val xmlPath = getClass.getClassLoader.getResource("fairscheduler.xml")
      .getFile()
