...
class MockSampler extends RandomSampler[Long, Long] {
  ...
  override def setSeed(seed: Long) { s = seed }

  override def sample(items: Iterator[Long]): Iterator[Long] = { Iterator(s) }
...
class PartitionwiseSampledRDDSuite
  ...
  test("concurrency") {
    ...
    for (sampler <-
           Seq(new BernoulliSampler[Int](0.5), new PoissonSampler[Int](0.5))) {
