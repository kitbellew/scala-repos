...
class SparkSubmitUtilsSuite extends SparkFunSuite with BeforeAndAfterAll {
  ...
  test("create repo resolvers") {
    ...
    assert(
      res1.getResolvers.get(0).asInstanceOf[IBiblioResolver].getName ===
        "local-m2-cache")
    assert(
      res1.getResolvers.get(1).asInstanceOf[FileSystemResolver].getName ===
        "local-ivy-cache")
    assert(
      res1.getResolvers.get(2).asInstanceOf[IBiblioResolver].getName ===
        "central")
    assert(
      res1.getResolvers.get(3).asInstanceOf[IBiblioResolver].getName ===
        "spark-packages")

    val repos = "a/1,b/2,c/3"
    val resolver2 = SparkSubmitUtils
      .createRepoResolvers(Option(repos), settings)
    assert(resolver2.getResolvers.size() === 7)
    val expected = repos.split(",").map(r => s"$r/")
    resolver2.getResolvers.toArray.zipWithIndex.foreach {
      case (resolver: AbstractResolver, i) =>
        if (i < 3) {
  ...
  test("add dependencies works correctly") {
    ...
    val artifacts = SparkSubmitUtils.extractMavenCoordinates(
  ...
  test("excludes works correctly") {
    ...
    excludes.foreach { e =>
      md.addExcludeRule(
        SparkSubmitUtils.createExclusion(e + ":*", new IvySettings, "default"))
    }
  ...
  test("ivy path works correctly") {
    val md = SparkSubmitUtils.getModuleDescriptor
    val artifacts =
      for (i <- 0 until 3) yield new MDArtifact(md, s"jar-$i", "jar", "jar")
    var jPaths = SparkSubmitUtils
      .resolveDependencyPaths(artifacts.toArray, new File(tempIvyPath))
    for (i <- 0 until 3) {
  ...
  test("search for artifact at local repositories") {
    ...
    IvyTestUtils
      .withRepository(main, Some(dep), Some(SparkSubmitUtils.m2Path)) { repo =>
        val jarPath = SparkSubmitUtils
          .resolveMavenCoordinates(main.toString, None, None, isTest = true)
    ...
    val settings = new IvySettings
    val ivyLocal =
      new File(settings.getDefaultIvyUserDir, "local" + File.separator)
    IvyTestUtils
      .withRepository(main, Some(dep), Some(ivyLocal), useIvyLayout = true) {
        repo =>
          val jarPath = SparkSubmitUtils
            .resolveMavenCoordinates(main.toString, None, None, isTest = true)
          assert(jarPath.indexOf("mylib") >= 0, "should find artifact")
  ...
  test("dependency not found throws RuntimeException") {
    intercept[RuntimeException] {
      SparkSubmitUtils
        .resolveMavenCoordinates("a:b:c", None, None, isTest = true)
    }
  }
  ...
  test("neglects Spark and Spark's dependencies") {
    ...
    val coordinates =
      components.map(comp => s"org.apache.spark:spark-${comp}2.10:1.2.0")
