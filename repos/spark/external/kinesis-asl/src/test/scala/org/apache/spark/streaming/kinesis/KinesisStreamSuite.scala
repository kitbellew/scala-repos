...
abstract class KinesisStreamTests(aggregateTestData: Boolean)
  ...
  before { ssc = new StreamingContext(sc, batchDuration) }
  ...
  after {
    ...
    if (testUtils != null) { testUtils.deleteDynamoDBTable(appName) }
  }
  ...
  test("KinesisUtils API") {
    ...
    val kinesisStream2 = KinesisUtils.createStream(
      ssc,
      "myAppName",
      "mySparkStream",
      dummyEndpointUrl,
  ...
  test("RDD generation") {
    val inputStream = KinesisUtils.createStream(
      ssc,
      appName,
      "dummyStream",
      dummyEndpointUrl,
