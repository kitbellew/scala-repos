...
class PostgresIntegrationSuite extends DockerJDBCIntegrationSuite {
  ...
  override def dataPreparation(conn: Connection): Unit = {
    ...
    conn.prepareStatement(
      "CREATE TYPE enum_type AS ENUM ('d1', 'd2')").executeUpdate()
    conn.prepareStatement(
  ...
  test("Type mapping for various types") {
    ...
    assert(
      rows(0).getSeq(13) == Seq("0.11", "0.22").map(BigDecimal(_).bigDecimal))
  ...
  test("Basic write test") {
    ...
    assert(
      sqlContext.read.jdbc(jdbcUrl, "public.barcopy", new Properties).schema(
        13).dataType ==
