...
class MySQLIntegrationSuite extends DockerJDBCIntegrationSuite {
  ...
  override def dataPreparation(conn: Connection): Unit = {
    ...
    conn.prepareStatement("CREATE TABLE tbl (x INTEGER, y TEXT(8))")
      .executeUpdate()
  ...
  test("Date types") {
    ...
    assert(rows(0).getAs[Timestamp](1).equals(Timestamp.valueOf(
      "1970-01-01 13:31:24")))
    assert(rows(0).getAs[Timestamp](2).equals(Timestamp.valueOf(
      "1996-01-01 01:23:45")))
    assert(rows(0).getAs[Timestamp](3).equals(Timestamp.valueOf(
      "2009-02-13 23:31:30")))
