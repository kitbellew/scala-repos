...
class MySQLIntegrationSuite extends DockerJDBCIntegrationSuite {
  ...
  override def dataPreparation(conn: Connection): Unit = {
    ...
    conn.prepareStatement(
      "CREATE TABLE tbl (x INTEGER, y TEXT(8))").executeUpdate()
    ...
    conn.prepareStatement(
      "CREATE TABLE strings (a CHAR(10), b VARCHAR(10), c TINYTEXT, "
        + "d TEXT, e MEDIUMTEXT, f LONGTEXT, g BINARY(4), h VARBINARY(10), i BLOB)").executeUpdate()
    conn.prepareStatement(
  ...
  test("Date types") {
    ...
    assert(
      rows(0).getAs[Timestamp](1).equals(
        Timestamp.valueOf("1970-01-01 13:31:24")))
    assert(
      rows(0).getAs[Timestamp](2).equals(
        Timestamp.valueOf("1996-01-01 01:23:45")))
    assert(
      rows(0).getAs[Timestamp](3).equals(
  ...
  test("String types") {
    ...
    assert(
      java.util.Arrays.equals(
        rows(0).getAs[Array[Byte]](7),
        Array[Byte](108, 97, 122, 121)))
    assert(
      java.util.Arrays.equals(
        rows(0).getAs[Array[Byte]](8),
