...
final case class Zipper[+A](lefts: Stream[A], focus: A, rights: Stream[A]) {
  ...
  /**
    ...
    */
  def tryPrevious: Zipper[A] =
    previousOr(sys.error("cannot move to previous element"))
  ...
  def insertLeft[AA >: A](y: AA): Zipper[AA] =
    zipper(lefts, y, focus #:: rights)

  /**
    * Inserts an element to the right of focus and focuses on the new element.
    */
  def insertRight[AA >: A](y: AA): Zipper[AA] =
    zipper(focus #:: lefts, y, rights)
  ...
  def findPrevious(p: A => Boolean): Option[Zipper[A]] =
    findBy((z: Zipper[A]) => z.previous)(p)
  ...
  def traverse[G[_]: Applicative, B](f: A => G[B]): G[Zipper[B]] = {
    ...
    G.apF(
      G.apF(
        G.map(Traverse[Stream].traverse[G, A, B](lefts.reverse)(f))(s =>
          z(s.reverse)))(f(focus)))(
  ...
  implicit val zipperInstance
    ...
    new Traverse1[Zipper] with Applicative[Zipper] with Comonad[Zipper] {
      ...
      override def foldLeft[A, B](fa: Zipper[A], z: B)(f: (B, A) => B): B =
        fa.foldLeft(z)(f)
      override def foldMap[A, B](fa: Zipper[A])(f: A => B)(implicit
          F: Monoid[B]) = fa.foldLeft(F.zero)((b, a) => F.append(b, f(a)))
      ...
      override def traverse1Impl[G[_], A, B](fa: Zipper[A])(f: A => G[B])(
        ...
        fa.lefts.reverse match {
          case h1 #:: t1 =>
            val x =
              G.map(F.traverse1(OneAnd(h1, t1))(f)) { s =>
