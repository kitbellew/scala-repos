...
final case class LazyEitherT[F[_], A, B](run: F[LazyEither[A, B]]) {
  ...
  def orElse(x: => LazyEitherT[F, A, B])(implicit
      m: Bind[F]): LazyEitherT[F, A, B] = {
    val g = run
    LazyEitherT(m.bind(g)(_.fold(_ => x.run, _ => g)))
  }

  /** Return this if it is a right, otherwise, return the given value. Alias for `orElse` */
  def |||(x: => LazyEitherT[F, A, B])(implicit
      F: Bind[F]): LazyEitherT[F, A, B] = orElse(x)
  ...
  def flatMap[C](f: (=> B) => LazyEitherT[F, A, C])(implicit
      M: Monad[F]): LazyEitherT[F, A, C] =
    lazyEitherT(
      M.bind(run)(_.fold(a => M.point(lazyLeft[C](a)), b => f(b).run)))

  def bimap[C, D](f: (=> A) => C, g: (=> B) => D)(implicit
      F: Functor[F]): LazyEitherT[F, C, D] = map(g).left.map(f)
  ...
  def bitraverse[G[_], C, D](f: A => G[C], g: B => G[D])(implicit
    ...
    Applicative[G].map(
      F.traverse(run)(Bitraverse[LazyEither].bitraverseF(f, g)))(LazyEitherT(
      _: F[LazyEither[C, D]]))
  }
  ...
  /** Apply a function in the environment of the right of this
    ...
    */
  def app[C](f: => LazyEitherT[F, A, B => C])(implicit
      F: Apply[F]): LazyEitherT[F, A, C] = {
    // TODO check laziness
    LazyEitherT[F, A, C](
...
object LazyEitherT extends LazyEitherTInstances {
  ...
  def lazyLeftT[F[_], A, B](a: => A)(implicit
      p: Applicative[F]): LazyEitherT[F, A, B] =
    lazyEitherT(p.point(lazyLeft(a)))

  def lazyRightT[F[_], A, B](b: => B)(implicit
      p: Applicative[F]): LazyEitherT[F, A, B] =
    lazyEitherT(p.point(lazyRight(b)))

  import Isomorphism.{IsoFunctorTemplate, IsoBifunctorTemplate, <~>, <~~>}

  def lazyEitherTLeftProjectionEIso2[F[_], E]
      : LazyEitherT.LeftProjectionT[F, E, ?] <~> LazyEitherT[F, E, ?] =
    new IsoFunctorTemplate[
      LazyEitherT.LeftProjectionT[F, E, ?],
      LazyEitherT[F, E, ?]] {
      def to[A](
          fa: LazyEitherT.LeftProjectionT[F, E, A]): LazyEitherT[F, E, A] =
        fa.lazyEitherT
      def from[A](
          ga: LazyEitherT[F, E, A]): LazyEitherT.LeftProjectionT[F, E, A] =
        ga.left
    }

  def lazyEitherTLeftProjectionIso2[F[_]]
      : LazyEitherT.LeftProjectionT[F, ?, ?] <~~> LazyEitherT[F, ?, ?] =
    new IsoBifunctorTemplate[
      LazyEitherT.LeftProjectionT[F, ?, ?],
      LazyEitherT[F, ?, ?]] {
      def to[A, B](
          fa: LazyEitherT.LeftProjectionT[F, A, B]): LazyEitherT[F, A, B] =
        fa.lazyEitherT
      def from[A, B](
          ga: LazyEitherT[F, A, B]): LazyEitherT.LeftProjectionT[F, A, B] =
        ga.left
    }

  final class LeftProjectionT[F[_], A, B](
    ...
    def flatMap[C](f: (=> A) => LazyEitherT[F, C, B])(implicit
        M: Monad[F]): LazyEitherT[F, C, B] =
      LazyEitherT(M.bind(lazyEitherT.run)(
        _.fold(a => f(a).run, b => M.point(LazyEither.lazyRight[C](b)))))
  }

  implicit def lazyEitherTMonadPlus[F[_], L](implicit
      F0: Monad[F],
      L: Monoid[L]): MonadPlus[LazyEitherT[F, L, ?]] =
...
sealed abstract class LazyEitherTInstances1 {
  implicit def lazyEitherTFunctor[F[_], L](implicit
      F0: Functor[F]): Functor[LazyEitherT[F, L, ?]] =
    new LazyEitherTFunctor[F, L] {
      implicit def F = F0
    }

  implicit def lazyEitherTLeftProjectionFunctor[F[_], L](implicit
      F0: Functor[F]): Functor[LazyEitherT.LeftProjectionT[F, L, ?]] =
    new IsomorphismFunctor[
  ...
  implicit def lazyEitherTMonadError[F[_], L](implicit
      F0: Monad[F]): MonadError[LazyEitherT[F, L, ?], L] =
    new LazyEitherTMonadError[F, L] {
      implicit def F = F0
    }

  implicit def lazyEitherTPlus[F[_], L](implicit
      F0: Monad[F],
      L: Semigroup[L]): Plus[LazyEitherT[F, L, ?]] =
    new LazyEitherTPlus[F, L] {
      override def F = F0
      override def E = L
    }

  implicit def lazyEitherTBindRec[F[_], L](implicit
      F0: Monad[F],
      B0: BindRec[F]): BindRec[LazyEitherT[F, L, ?]] =
...
sealed abstract class LazyEitherTInstances0 extends LazyEitherTInstances1 {
  implicit def lazyEitherTBifunctor[F[_]](implicit
      F0: Functor[F]): Bifunctor[LazyEitherT[F, ?, ?]] =
    new LazyEitherTBifunctor[F] {
      implicit def F = F0
    }

  implicit def lazyEitherTBifoldable[F[_]](implicit
      F0: Foldable[F]): Bifoldable[LazyEitherT[F, ?, ?]] =
  ...
  implicit def lazyEitherTMonad[F[_], L](implicit
      F0: Monad[F]): Monad[LazyEitherT[F, L, ?]] =
    new LazyEitherTMonad[F, L] {
      implicit def F = F0
    }

  implicit def lazyEitherTLeftProjectionMonad[F[_], L](implicit
      F0: Monad[F]): Monad[LazyEitherT.LeftProjectionT[F, L, ?]] =
    new IsomorphismMonad[
      LazyEitherT.LeftProjectionT[F, L, ?],
      LazyEitherT[F, L, ?]] {
  ...
  implicit def lazyEitherTFoldable[F[_], L](implicit
      F0: Foldable[F]): Foldable[LazyEitherT[F, L, ?]] =
...
sealed abstract class LazyEitherTInstances extends LazyEitherTInstances0 {
  implicit def lazyEitherTBitraverse[F[_]](implicit
      F0: Traverse[F]): Bitraverse[LazyEitherT[F, ?, ?]] =
  ...
  implicit def lazyEitherTTraverse[F[_], L](implicit
      F0: Traverse[F]): Traverse[LazyEitherT[F, L, ?]] =
  ...
  implicit def lazyEitherTHoist[A]
      : Hoist[λ[(a[_], b) => LazyEitherT[a, A, b]]] =
...
private trait LazyEitherTFunctor[F[_], E]
    extends Functor[LazyEitherT[F, E, ?]] {
...
private trait LazyEitherTMonad[F[_], E]
    extends Monad[LazyEitherT[F, E, ?]]
    with LazyEitherTFunctor[F, E] {
...
private trait LazyEitherTMonadPlus[F[_], E]
    extends MonadPlus[LazyEitherT[F, E, ?]]
    with LazyEitherTMonad[F, E]
    with LazyEitherTPlus[F, E] {
  override def E: Monoid[E]

  override def empty[A] = LazyEitherT.lazyLeftT(E.zero)
}

private trait LazyEitherTFoldable[F[_], E]
    extends Foldable.FromFoldr[LazyEitherT[F, E, ?]] {
  implicit def F: Foldable[F]

  override final def foldRight[A, B](fa: LazyEitherT[F, E, A], z: => B)(
      f: (A, => B) => B): B = fa.foldRight(z)(f)
}

private trait LazyEitherTTraverse[F[_], E]
    extends Traverse[LazyEitherT[F, E, ?]]
    with LazyEitherTFoldable[F, E]
    with LazyEitherTFunctor[F, E] {
...
private trait LazyEitherTBifunctor[F[_]]
    extends Bifunctor[LazyEitherT[F, ?, ?]] {
  implicit def F: Functor[F]

  override final def bimap[A, B, C, D](
      fab: LazyEitherT[F, A, B])(f: A => C, g: B => D) =
    fab.map(x => g(x)).left.map(x => f(x))
}

private trait LazyEitherTBifoldable[F[_]]
    extends Bifoldable.FromBifoldMap[LazyEitherT[F, ?, ?]] {
  implicit def F: Foldable[F]

  override final def bifoldMap[A, B, M: Monoid](fab: LazyEitherT[F, A, B])(
      f: A => M)(g: B => M) =
    F.foldMap(fab.run)(Bifoldable[LazyEither].bifoldMap(_)(f)(g))
}

private trait LazyEitherTBitraverse[F[_]]
    extends Bitraverse[LazyEitherT[F, ?, ?]]
    with LazyEitherTBifunctor[F]
    with LazyEitherTBifoldable[F] {
  implicit def F: Traverse[F]

  def bitraverseImpl[G[_]: Applicative, A, B, C, D](fab: LazyEitherT[F, A, B])(
      f: A => G[C],
      g: B => G[D]): G[LazyEitherT[F, C, D]] =
    Applicative[G].map(F.traverse(fab.run)(
      Bitraverse[LazyEither].bitraverseF(f, g)))(LazyEitherT.lazyEitherT(_))
}

private trait LazyEitherTBindRec[F[_], E]
    extends BindRec[LazyEitherT[F, E, ?]]
    with LazyEitherTMonad[F, E] {
  implicit def B: BindRec[F]

  final def tailrecM[A, B](f: A => LazyEitherT[F, E, A \/ B])(
...
private trait LazyEitherTMonadError[F[_], E]
    extends MonadError[LazyEitherT[F, E, ?], E]
    with LazyEitherTMonad[F, E] {
  def raiseError[A](e: E): LazyEitherT[F, E, A] = LazyEitherT.lazyLeftT(e)
  def handleError[A](fa: LazyEitherT[F, E, A])(
      f: E => LazyEitherT[F, E, A]): LazyEitherT[F, E, A] =
