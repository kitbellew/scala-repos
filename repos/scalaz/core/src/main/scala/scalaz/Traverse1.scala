...
trait Traverse1[F[_]] extends Traverse[F] with Foldable1[F] { self =>
  ...
  def product0[G[_]](
      implicit G0: Traverse[G]): Traverse1[λ[α => (F[α], G[α])]] =
  ...
  final def sequence1U[GA](fga: F[GA])(
      implicit G: Unapply[Apply, GA]): G.M[F[G.A]] =
  ...
  trait Traverse1Law extends TraverseLaw {
    ...
    /** Traversal through the [[scalaz.Id]] effect is equivalent to
      * `Functor#map`.
      */
    def identityTraverse1[A, B](fa: F[A], f: A => B)(
