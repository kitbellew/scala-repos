...
trait Traverse1[F[_]] extends Traverse[F] with Foldable1[F] {
  ...
  override def foldMap1[A, B](fa: F[A])(f: A => B)(implicit
      F: Semigroup[B]): B =
    foldLeft1(traverse1Impl[Id, A, B](fa)(f))(F.append(_, _))

  def traverse1[G[_], A, B](fa: F[A])(f: A => G[B])(implicit
      a: Apply[G]): G[F[B]] = traverse1Impl(fa)(f)

  final def traverse1U[A, GB](fa: F[A])(f: A => GB)(implicit
  ...
  final def sequence1U[GA](fga: F[GA])(implicit
      G: Unapply[Apply, GA]): G.M[F[G.A]] =
  ...
  trait Traverse1Law extends TraverseLaw {
    ...
    /** Traversal through the [[scalaz.Id]] effect is equivalent to
      * `Functor#map`.
      */
    def identityTraverse1[A, B](fa: F[A], f: A => B)(implicit
        FB: Equal[F[B]]) = { FB.equal(traverse1[Id, A, B](fa)(f), map(fa)(f)) }
    ...
    def sequentialFusion1[N[_], M[_], A, B, C](
      ...
      type MN[A] = M[N[A]]
      val t1: MN[F[C]] = M
        .map(traverse1[M, A, B](fa)(amb))(fb => traverse1[N, B, C](fb)(bnc))
