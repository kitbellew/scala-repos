...
trait IdInstances {
  ...
  val id: Traverse1[Id]
    ...
    new Traverse1[Id]
      ...
      override def traverse[A, G[_], B](value: G[A])(f: A => Id[B])(implicit
          G: Traverse[G]): Id[G[B]] = G.map(value)(f)

      override def sequence[A, G[_]: Traverse](as: G[Id[A]]): Id[G[A]] = as

      override def ap[A, B](fa: => Id[A])(f: => Id[A => B]): Id[B] = f(fa)

      override def compose[G[_]](implicit G0: Applicative[G]): Applicative[G] =
        G0
