...
sealed abstract class Dequeue[A] {
  ...
  def size: Int =
    this match {
      case EmptyDequeue() =>
        0
      case SingletonDequeue(_) =>
        1
      case FullDequeue(_, fs, _, bs) =>
        fs + bs
    }
...
private[scalaz] final case class FullDequeue[A](
    front: NonEmptyIList[A],
    fsize: Int,
    back: NonEmptyIList[A],
    backSize: Int)
    extends Dequeue[A] {
  override def isEmpty = false
...
sealed abstract class DequeueInstances {
  ...
  implicit def dequeueMonoid[A]: Monoid[Dequeue[A]] =
    new Monoid[Dequeue[A]] {
  ...
  implicit val dequeueInstances
    ...
    new Foldable[Dequeue] with IsEmpty[Dequeue] with Functor[Dequeue] {
      ...
      override def foldLeft[A, B](fa: Dequeue[A], b: B)(f: (B, A) => B): B =
        fa.foldLeft(b)(f)
      override def foldMap[A, B](fa: Dequeue[A])(f: A => B)(implicit
          F: Monoid[B]): B = fa.foldLeft(F.zero)((b, a) => F.append(b, f(a)))
