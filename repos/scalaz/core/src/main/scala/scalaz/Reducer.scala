...
sealed abstract class Reducer[C, M] {
  ...
  def compose[N](r: Reducer[C, N]): Reducer[C, (M, N)] = {
    ...
    new Reducer[C, (M, N)] {
      ...
      override def snoc(p: (M, N), x: C) =
        (Reducer.this.snoc(p._1, x), r.snoc(p._2, x))
...
sealed abstract class ReducerInstances {
  ...
  /** The "and" monoid. */
  implicit val AllReducer: Reducer[Boolean, Boolean @@ Conjunction] =
    unitReducer(b => Tag[Boolean, Conjunction](b))
  ...
  implicit val IntProductReducer: Reducer[Int, Int @@ Multiplication] =
    unitReducer(i => Tag[Int, Multiplication](i))

  implicit val CharProductReducer: Reducer[Char, Char @@ Multiplication] =
    unitReducer(c => Tag[Char, Multiplication](c))

  implicit val ByteProductReducer: Reducer[Byte, Byte @@ Multiplication] =
    unitReducer(b => Tag[Byte, Multiplication](b))

  implicit val LongProductReducer: Reducer[Long, Long @@ Multiplication] =
    unitReducer(l => Tag[Long, Multiplication](l))
  ...
  implicit def FirstReducer[A]: Reducer[A, Option[A] @@ First] =
    unitReducer(a => Tag[Option[A], First](Some(a)))

  implicit def FirstOptionReducer[A]: Reducer[Option[A], Option[A] @@ First] =
    unitReducer(o => Tag[Option[A], First](o))

  implicit def LastReducer[A]: Reducer[A, Option[A] @@ Last] =
    unitReducer(a => Tag[Option[A], Last](Some(a)))

  implicit def LastOptionReducer[A]: Reducer[Option[A], Option[A] @@ Last] =
