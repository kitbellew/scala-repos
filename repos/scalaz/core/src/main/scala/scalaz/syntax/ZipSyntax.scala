...
final class ZipOps[F[_], A] private[syntax] (val self: F[A])(implicit
    val F: Zip[F])
    extends Ops[F[A]] {
  ////
  final def fzip[B](b: => F[B]): F[(A, B)] = F.zip(self, b)
  final def fzipWith[B, C](b: => F[B])(f: (A, B) => C)(implicit
      T: Functor[F]): F[C] = F.zipWith(self, b)(f)
...
trait ZipSyntax[F[_]] {
  implicit def ToZipOps[A](v: F[A]): ZipOps[F, A] =
    new ZipOps[F, A](v)(ZipSyntax.this.F)
