...
final class MonadTellOps[F[_], S, A] private[syntax] (self: F[A])(implicit
    val F: MonadTell[F, S]) {
  ////
...
trait ToMonadTellOps extends ToMonadOps {
  implicit def ToMonadTellOps[F[_], S, A](v: F[A])(implicit
      F0: MonadTell[F, S]) = new MonadTellOps[F, S, A](v)
