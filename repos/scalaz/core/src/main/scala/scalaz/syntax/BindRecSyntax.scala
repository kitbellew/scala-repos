...
final class BindRecOps[F[_], A] private[syntax] (val self: F[A])(implicit
    val F: BindRec[F])
    extends Ops[F[A]] {
  ////
...
sealed trait ToBindRecOps0 {
  implicit def ToBindRecOpsUnapply[FA](v: FA)(implicit
      F0: Unapply[BindRec, FA]) = new BindRecOps[F0.M, F0.A](F0(v))(F0.TC)
...
trait BindRecSyntax[F[_]] extends BindSyntax[F] {
  implicit def ToBindRecOps[A](v: F[A]): BindRecOps[F, A] =
    new BindRecOps[F, A](v)(BindRecSyntax.this.F)
