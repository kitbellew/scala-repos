...
sealed trait ToApplicativePlusOps0 {
  implicit def ToApplicativePlusOpsUnapply[FA](v: FA)(implicit
      F0: Unapply[ApplicativePlus, FA]) =
...
trait ToApplicativePlusOps
    ...
    with ToPlusEmptyOps {
  implicit def ToApplicativePlusOps[F[_], A](v: F[A])(implicit
      F0: ApplicativePlus[F]) = new ApplicativePlusOps[F, A](v)
