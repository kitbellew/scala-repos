...
final class AssociativeOps[F[_, _], A, B] private[syntax] (val self: F[A, B])(
  ...
  final def reassociateLeft[TT, C](implicit
      ev: B === F[TT, C]): F[F[A, TT], C] =
    F.reassociateLeft(ev.subst[F[A, ?]](self))

  final def reassociateRight[TT, C](implicit
      ev: A === F[TT, C]): F[TT, F[C, B]] =
...
sealed trait ToAssociativeOps0 {
  implicit def ToAssociativeOpsUnapply[FA](v: FA)(implicit
      F0: Unapply2[Associative, FA]) =
...
trait ToAssociativeOps extends ToAssociativeOps0 {
  ...
  implicit def ToAssociativeOps[F[_, _], A, B](v: F[A, B])(implicit
      F0: Associative[F]) = new AssociativeOps[F, A, B](v)
