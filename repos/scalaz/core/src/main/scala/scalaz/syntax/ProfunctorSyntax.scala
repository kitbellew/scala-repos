...
sealed trait ToProfunctorOps0 {
  implicit def ToProfunctorOpsUnapply[FA](v: FA)(implicit
      F0: Unapply2[Profunctor, FA]) =
...
trait ToProfunctorOps extends ToProfunctorOps0 {
  ...
  implicit def ToProfunctorOps[F[_, _], A, B](v: F[A, B])(implicit
      F0: Profunctor[F]) = new ProfunctorOps[F, A, B](v)
