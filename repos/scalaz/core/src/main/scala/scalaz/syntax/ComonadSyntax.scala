...
final class ComonadOps[F[_], A] private[syntax] (val self: F[A])(implicit
    val F: Comonad[F])
    extends Ops[F[A]] {
  ////
...
sealed trait ToComonadOps0 {
  implicit def ToComonadOpsUnapply[FA](v: FA)(implicit
      F0: Unapply[Comonad, FA]) = new ComonadOps[F0.M, F0.A](F0(v))(F0.TC)
...
trait ComonadSyntax[F[_]] extends CobindSyntax[F] {
  implicit def ToComonadOps[A](v: F[A]): ComonadOps[F, A] =
    new ComonadOps[F, A](v)(ComonadSyntax.this.F)
