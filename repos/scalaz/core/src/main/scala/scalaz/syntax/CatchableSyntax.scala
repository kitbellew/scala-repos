...
sealed trait ToCatchableOps0 {
  implicit def ToCatchableOpsUnapply[FA](v: FA)(implicit
      F0: Unapply[Catchable, FA]) = new CatchableOps[F0.M, F0.A](F0(v))(F0.TC)
...
trait CatchableSyntax[F[_]] {
  implicit def ToCatchableOps[A](v: F[A]): CatchableOps[F, A] =
    new CatchableOps[F, A](v)(CatchableSyntax.this.F)
