...
final class BifunctorOps[F[_, _], A, B] private[syntax] (val self: F[A, B])(
  ...
  final def <:>[C](f: A => C)(implicit z: B <~< C): F[C, C] =
    F.bimap(self)(f, z)
  final def umap[C](f: A => C)(implicit ev: F[A, B] =:= F[A, A]): F[C, C] =
    F.umap(ev(self))(f)
...
sealed trait ToBifunctorOps0 {
  implicit def ToBifunctorOpsUnapply[FA](v: FA)(implicit
      F0: Unapply2[Bifunctor, FA]) =
...
trait ToBifunctorOps extends ToBifunctorOps0 {
  ...
  implicit def ToBifunctorOps[F[_, _], A, B](v: F[A, B])(implicit
      F0: Bifunctor[F]) = new BifunctorOps[F, A, B](v)
