...
final class FunctorOps[F[_], A] private[syntax] (val self: F[A])(implicit
  ...
  final def distribute[G[_], B](f: A => G[B])(implicit
      D: Distributive[G]): G[F[B]] = D.distribute(self)(f)
...
sealed trait ToFunctorOps0 {
  implicit def ToFunctorOpsUnapply[FA](v: FA)(implicit
      F0: Unapply[Functor, FA]) = new FunctorOps[F0.M, F0.A](F0(v))(F0.TC)
...
trait ToFunctorOps extends ToFunctorOps0 with ToInvariantFunctorOps {
  ...
  implicit def ToFunctorIdV[A](v: A) =
    new FunctorIdV[A] {
      def self = v
    }
