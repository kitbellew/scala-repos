...
final class PlusOps[F[_], A] private[syntax] (val self: F[A])(
    implicit val F: Plus[F])
    extends Ops[F[A]] {
  ////
...
trait PlusSyntax[F[_]] {
  implicit def ToPlusOps[A](v: F[A]): PlusOps[F, A] =
    new PlusOps[F, A](v)(PlusSyntax.this.F)
