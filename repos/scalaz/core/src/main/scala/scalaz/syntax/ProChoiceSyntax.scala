...
sealed trait ToProChoiceOps0 {
  implicit def ToProChoiceOpsUnapply[FA](v: FA)(implicit
      F0: Unapply2[ProChoice, FA]) =
...
trait ToProChoiceOps extends ToProChoiceOps0 with ToProfunctorOps {
  ...
  implicit def ToProChoiceOps[F[_, _], A, B](v: F[A, B])(implicit
      F0: ProChoice[F]) = new ProChoiceOps[F, A, B](v)
