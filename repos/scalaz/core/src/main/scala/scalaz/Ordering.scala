...
sealed abstract class OrderingInstances {
  ...
  implicit val orderingInstance
      : Enum[Ordering] with Show[Ordering] with Monoid[Ordering] =
    new Enum[Ordering] with Show[Ordering] with Monoid[Ordering] {
      def order(a1: Ordering, a2: Ordering): Ordering =
        (a1, a2) match {
          case (LT, LT)      => EQ
          case (LT, EQ | GT) => LT
          case (EQ, LT)      => GT
      ...
      def append(f1: Ordering, f2: => Ordering): Ordering =
        f1 match {
          case Ordering.EQ => f2
          case o           => o
        }
