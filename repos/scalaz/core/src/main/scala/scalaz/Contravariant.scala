...
trait Contravariant[F[_]] extends InvariantFunctor[F] {
  ...
  def product[G[_]](implicit
      G0: Contravariant[G]): Contravariant[λ[α => (F[α], G[α])]] =
