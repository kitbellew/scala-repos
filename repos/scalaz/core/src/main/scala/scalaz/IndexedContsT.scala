...
final class IndexedContsT[W[_], M[_], R, O, A] private (
  ...
  def flatMap[E, B](f: A => IndexedContsT[W, M, O, E, B])(implicit
      W: Cobind[W]): IndexedContsT[W, M, R, E, B] =
    IndexedContsT { wbme =>
...
trait IndexedContsTFunctions {
  def point[W[_], M[_], R, A](a: => A)(implicit
      W: Comonad[W]): ContsT[W, M, R, A] = ContsT { k => W.copoint(k)(a) }
...
sealed abstract class IndexedContsTInstances1 {
  implicit def IndexedContsTFunctorRight[W[_], M[_], R, O](implicit
      W0: Functor[W]): Functor[IndexedContsT[W, M, R, O, ?]] =
...
abstract class IndexedContsTInstances extends IndexedContsTInstances0 {
  implicit def IndexedContsTFunctorLeft[W[_], M[_], O, A](implicit
      M0: Functor[M]): Functor[IndexedContsT[W, M, ?, O, A]] =
  ...
  implicit def ContsTMonad[W[_], M[_], R](implicit
      W0: Comonad[W]): Monad[ContsT[W, M, R, ?]] =
    new ContsTMonad[W, M, R] {
      implicit val W: Comonad[W] = W0
    }
}

private sealed trait IndexedContsTFunctorLeft[W[_], M[_], O, A0]
    extends Functor[IndexedContsT[W, M, ?, O, A0]] {
  implicit val M: Functor[M]

  def map[A, B](fa: IndexedContsT[W, M, A, O, A0])(
      f: A => B): IndexedContsT[W, M, B, O, A0] = fa.imap(f)
}

private sealed trait IndexedContsTFunctorRight[W[_], M[_], R, O]
    extends Functor[IndexedContsT[W, M, R, O, ?]] {
  implicit val W: Functor[W]

  override def map[A, B](fa: IndexedContsT[W, M, R, O, A])(
      f: A => B): IndexedContsT[W, M, R, O, B] = fa.map(f)
}

private sealed trait IndexedContsTContravariant[W[_], M[_], R, A0]
    extends Contravariant[IndexedContsT[W, M, R, ?, A0]] {
  implicit val W: Functor[W]
  implicit val M: Functor[M]

  def contramap[A, B](fa: IndexedContsT[W, M, R, A, A0])(
      f: B => A): IndexedContsT[W, M, R, B, A0] = fa.contramap(f)
}

private sealed trait IndexedContsTBifunctor[W[_], M[_], O]
    extends Bifunctor[IndexedContsT[W, M, ?, O, ?]] {
  implicit val W: Functor[W]
  implicit val M: Functor[M]
  ...
  override def leftMap[A, B, C](fa: IndexedContsT[W, M, A, O, B])(
      f: A => C): IndexedContsT[W, M, C, O, B] = fa.imap(f)

  override def rightFunctor[X]: Functor[IndexedContsT[W, M, X, O, ?]] =
    IndexedContsT.IndexedContsTFunctorRight

  override def rightMap[A, B, D](fa: IndexedContsT[W, M, A, O, B])(
      f: B => D): IndexedContsT[W, M, A, O, D] = fa.map(f)
}

private sealed trait ContsTBind[W[_], M[_], R]
    extends Bind[ContsT[W, M, R, ?]]
    with IndexedContsTFunctorRight[W, M, R, R] {
  implicit val W: Cobind[W]

  override def bind[A, B](fa: ContsT[W, M, R, A])(f: A => ContsT[W, M, R, B]) =
    fa.flatMap(f)
