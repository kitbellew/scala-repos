...
case class One[V, A](v: V, a1: A)(implicit r: Reducer[A, V])
    extends Finger[V, A] {
...
case class Two[V, A](v: V, a1: A, a2: A)(implicit r: Reducer[A, V])
    extends Finger[V, A] {
...
case class Four[V, A](v: V, a1: A, a2: A, a3: A, a4: A)(implicit
    r: Reducer[A, V])
    extends Finger[V, A] {
  def foldMap[B](f: A => B)(implicit m: Semigroup[B]) =
...
sealed abstract class FingerTree[V, A](implicit measurer: Reducer[A, V]) {
  ...
  def +:(a: => A): FingerTree[V, A] = {
    ...
    fold(
      ...
      (v, b) =>
        deep(measurer.cons(az, v), one(az), empty[V, Node[V, A]], one(b)),
      (v, pr, m, sf) => {
        val mz = m
        pr match {
  ...
  def :+(a: => A): FingerTree[V, A] = {
    ...
    fold(
      ...
      (v, b) =>
        deep(measurer.snoc(v, az), one(b), empty[V, Node[V, A]], one(az)),
      (v, pr, m, sf) => {
        val mz = m
        sf match {
          case Four(vf, b, c, d, e) =>
    ...
    fold(
      ...
      (v1, pr1, m1, sf1) =>
        rightz.fold(
          ...
          (v, x) => this :+ x,
          (v2, pr2, m2, sf2) =>
            deep(measurer.append(v1, v2), pr1, addDigits0(m1, sf1, pr2, m2), sf2)
        )
    )
  }
    ...
    fold(
      ...
      (v1, pr1, m1, sf1) =>
        rightz.fold(
          ...
          (v2, pr2, m2, sf2) =>
            deep(
              measurer.append(measurer.snoc(measurer.snoc(v1, n1), n2), v2),
              pr1,
      ...
      (v1, pr1, m1, sf1) =>
        rightz.fold(
          ...
          (v2, pr2, m2, sf2) =>
            deep(
              measurer.append(
                measurer.snoc(measurer.snoc(measurer.snoc(v1, n1), n2), n3),
                v2),
              pr1,
  ...
  def add4(n1t: => A, n2t: => A, n3t: => A, n4t: => A, right: => ATree): ATree = {
    ...
    lazy val n4 = n4t
    fold(
      v => n1 +: n2 +: n3 +: n4 +: rightz,
      (v, x) => x +: n1 +: n2 +: n3 +: n4 +: rightz,
      (v1, pr1, m1, sf1) =>
        rightz.fold(
          v => this :+ n1 :+ n2 :+ n3 :+ n4,
    ...
    d1 match {
      ...
      case Four(_, a, b, c, d) =>
        d2 match {
          ...
          case Three(_, e, f, g) =>
            m1.add3(node3(a, b, c), node3(d, x, e), node2(f, g), m2)
          case Four(_, e, f, g, h) =>
            m1.add3(node3(a, b, c), node3(d, x, e), node3(f, g, h), m2)
        }
    }
  }
    ...
    d1 match {
      ...
      case Three(_, a, b, c) =>
        d2 match {
          ...
          case Three(_, d, e, f) =>
            m1.add3(node3(a, b, c), node3(x, y, d), node2(e, f), m2)
          case Four(_, d, e, f, g) =>
      ...
      case Four(_, a, b, c, d) =>
        d2 match {
          ...
          case Two(_, e, f) =>
            m1.add3(node3(a, b, c), node3(d, x, y), node2(e, f), m2)
          case Three(_, e, f, g) =>
            m1.add3(node3(a, b, c), node3(d, x, y), node3(e, f, g), m2)
          case Four(_, e, f, g, h) =>
            m1.add4(node3(a, b, c), node3(d, x, y), node2(e, f), node2(g, h), m2)
        }
    }
  }
    ...
    d1 match {
      ...
      case Two(_, a, b) =>
        d2 match {
          ...
          case Three(_, c, d, e) =>
            m1.add3(node3(a, b, x), node3(y, z, c), node2(d, e), m2)
          case Four(_, c, d, e, f) =>
      ...
      case Three(_, a, b, c) =>
        d2 match {
          ...
          case Two(_, d, e) =>
            m1.add3(node3(a, b, c), node3(x, y, z), node2(d, e), m2)
          case Three(_, d, e, f) =>
            m1.add3(node3(a, b, c), node3(x, y, z), node3(d, e, f), m2)
          case Four(_, d, e, f, g) =>
            m1.add4(node3(a, b, c), node3(x, y, z), node2(d, e), node2(f, g), m2)
        }
      case Four(_, a, b, c, d) =>
        d2 match {
          case One(_, e) =>
          ...
          case Two(_, e, f) =>
            m1.add3(node3(a, b, c), node3(d, x, y), node3(z, e, f), m2)
          case Three(_, e, f, g) =>
            m1.add4(node3(a, b, c), node3(d, x, y), node2(z, e), node2(f, g), m2)
          case Four(_, e, f, g, h) =>
            m1.add4(
              node3(a, b, c),
              node3(d, x, y),
    ...
    d1 match {
      case One(_, a) =>
        d2 match {
          ...
          case Three(_, b, c, d) =>
            m1.add3(node3(a, x, y), node3(z, w, b), node2(c, d), m2)
          case Four(_, b, c, d, e) =>
      ...
      case Two(_, a, b) =>
        d2 match {
          ...
          case Two(_, c, d) =>
            m1.add3(node3(a, b, x), node3(y, z, w), node2(c, d), m2)
          case Three(_, c, d, e) =>
            m1.add3(node3(a, b, x), node3(y, z, w), node3(c, d, e), m2)
          case Four(_, c, d, e, f) =>
            m1.add4(node3(a, b, x), node3(y, z, w), node2(c, d), node2(e, f), m2)
        }
      case Three(_, a, b, c) =>
        d2 match {
          case One(_, d) =>
          ...
          case Two(_, d, e) =>
            m1.add3(node3(a, b, c), node3(x, y, z), node3(w, d, e), m2)
          case Three(_, d, e, f) =>
            m1.add4(node3(a, b, c), node3(x, y, z), node2(w, d), node2(e, f), m2)
          case Four(_, d, e, f, g) =>
            m1.add4(
              node3(a, b, c),
              node3(x, y, z),
      ...
      case Four(_, a, b, c, d) =>
        d2 match {
          ...
          case Two(_, e, f) =>
            m1.add4(node3(a, b, c), node3(d, x, y), node2(z, w), node2(e, f), m2)
          case Three(_, e, f, g) =>
            m1.add4(
              node3(a, b, c),
              node3(d, x, y),
  ...
  def split1(pred: V => Boolean): (FingerTree[V, A], A, FingerTree[V, A]) =
    split1(pred, measurer.monoid.zero)
  ...
  def traverseTree[F[_], V2, B](f: A => F[B])(implicit
    ...
    fold(
      _ => F.pure(FingerTree.empty[V2, B]),
...
sealed abstract class FingerTreeInstances {
  ...
  implicit def viewLFunctor[S[_]](implicit
  ...
  implicit def fingerMeasure[A, V](implicit
      m: Reducer[A, V]): Reducer[Finger[V, A], V] = {
  ...
  implicit def fingerTreeMeasure[A, V](implicit
      m: Reducer[A, V]): Reducer[FingerTree[V, A], V] = {
    implicit val vm = m.monoid
  ...
  implicit def fingerTreeMonoid[V, A](implicit
      m: Reducer[A, V]): Monoid[FingerTree[V, A]] =
...
object FingerTree extends FingerTreeInstances {
  ...
  def Node3[V, A](v: V, a1: => A, a2: => A, a3: => A)(implicit
      r: Reducer[A, V]) =
    new Node[V, A] {
      def fold[B](
          two: (V, => A, => A) => B,
  ...
  def four[V, A](a1: A, a2: A, a3: A, a4: A)(implicit measure: Reducer[A, V]) =
    Four(
      measure.snoc(measure.snoc(measure.snoc(measure.unit(a1), a2), a3), a4),
      a1,
      a2,
      a3,
      a4)
  ...
  def mappendVal[V, A](v: V, t: FingerTree[V, A])(implicit
      measure: Reducer[A, V]) =
    t.fold(
      x => v,
      (x, y) => fingerTreeMeasure[A, V].snoc(v, t),
      (x, p, m, s) => fingerTreeMeasure[A, V].snoc(v, t))
  ...
  def empty[V, A](implicit ms: Reducer[A, V]) =
    new FingerTree[V, A] {
      def fold[B](
          b: V => B,
          s: (V, A) => B,
          d: (V, Finger[V, A], => FingerTree[V, Node[V, A]], Finger[V, A]) => B): B =
        b(ms.monoid.zero)
    }
  ...
  def single[V, A](v: V, a: => A)(implicit
...
sealed abstract class IndSeqInstances {
  ...
  implicit val indSeqInstance
    ...
    new MonadPlus[IndSeq]
      ...
      def traverseImpl[G[_], A, B](fa: IndSeq[A])(f: A => G[B])(implicit
          G: Applicative[G]) = {
        import std.anyVal._
...
object OrdSeq {
  private def ordSeq[A: Order](t: FingerTree[LastOption[A], A]): OrdSeq[A] =
    new OrdSeq[A] {
      val self = t
      val ord = Order[A]
    }
