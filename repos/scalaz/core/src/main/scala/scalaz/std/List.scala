...
trait ListInstances extends ListInstances0 {
  implicit val listInstance: Traverse[List]
    with MonadPlus[List]
    with BindRec[List]
    with Zip[List]
    with Unzip[List]
    ...
    new Traverse[List]
      ...
      override def filter[A](fa: List[A])(p: A => Boolean): List[A] =
        fa filter p
...
trait ListFunctions {
  ...
  final def toNel[A](as: List[A]): Option[NonEmptyList[A]] =
    as match {
      case Nil =>
        None
      case h :: t =>
        Some(NonEmptyList.nel(h, IList.fromList(t)))
    }
  ...
  final def <^>[A, B: Monoid](as: List[A])(f: NonEmptyList[A] => B): B =
    as match {
      case Nil =>
        Monoid[B].zero
      case h :: t =>
        f(NonEmptyList.nel(h, IList.fromList(t)))
    }
  ...
  /** Run `p(a)`s and collect `as` while `p` yields false.  Don't run
    ...
    */
  final def takeUntilM[A, M[_]: Monad](as: List[A])(
      p: A => M[Boolean]): M[List[A]] =
