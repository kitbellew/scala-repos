...
trait OptionInstances extends OptionInstances0 {
  ...
  implicit def optionFirst[A] = new Monoid[FirstOption[A]] {
    ...
    def append(f1: FirstOption[A], f2: => FirstOption[A]) = Tag(
      Tag.unwrap(f1).orElse(Tag.unwrap(f2)))
  }
  ...
  implicit def optionLast[A] = new Monoid[LastOption[A]] {
    ...
    def append(f1: LastOption[A], f2: => LastOption[A]) = Tag(
      Tag.unwrap(f2).orElse(Tag.unwrap(f1)))
  }
  ...
  implicit def optionMin[A](implicit o: Order[A]) = new Monoid[MinOption[A]] {
    ...
    def append(f1: MinOption[A], f2: => MinOption[A]) = Tag(
  ...
  implicit def optionMax[A](implicit o: Order[A]) = new Monoid[MaxOption[A]] {
    ...
    def append(f1: MaxOption[A], f2: => MaxOption[A]) = Tag(
      (Tag unwrap f1, Tag unwrap f2) match {
