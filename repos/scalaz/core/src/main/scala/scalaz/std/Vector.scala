...
trait VectorInstances extends VectorInstances0 {
  implicit val vectorInstance: Traverse[Vector]
    ...
    new Traverse[Vector]
      ...
      override def foldRight[A, B](fa: Vector[A], z: => B)(
          f: (A, => B) => B) = {
        var i = fa.length
        var r = z
        while (i > 0) {
...
trait VectorFunctions {
  ...
  final def intersperse[A](as: Vector[A], a: A): Vector[A] =
    if (as.isEmpty)
      ...
      as.init.foldRight(as.last +: empty)(_ +: a +: _)

  final def toNel[A](as: Vector[A]): Option[NonEmptyList[A]] =
    if (as.isEmpty)
      None
    else
      Some(NonEmptyList.nel(as.head, IList.fromFoldable(as.tail)))
  ...
  final def <^>[A, B: Monoid](as: Vector[A])(f: NonEmptyList[A] => B): B =
    if (as.isEmpty)
      ...
      f(NonEmptyList.nel(as.head, IList.fromFoldable(as.tail)))
  ...
  final def takeWhileM[A, M[_]: Monad](as: Vector[A])(
    ...
    lazyFoldRight(as, Monad[M].point(empty[A]))((a, as) =>
      Monad[M].bind(p(a))(b =>
        if (b)
          Monad[M].map(as)((tt: Vector[A]) => a +: tt)
        else
          Monad[M].point(empty)))
