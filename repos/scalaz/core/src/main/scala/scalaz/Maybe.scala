...
object Maybe extends MaybeInstances {
  ...
  val optionMaybeIso: Option <~> Maybe = new IsoFunctorTemplate[Option, Maybe] {
  ...
  def fromTryCatchThrowable[T, E <: Throwable](
    ...
    try { just(a) }
    catch { case e if ex.runtimeClass.isInstance(e) => empty }

  def fromTryCatchNonFatal[T](a: => T): Maybe[T] =
    try { just(a) }
    catch { case NonFatal(t) => empty }
}
...
sealed abstract class MaybeInstances {
  ...
  implicit def maybeLastMonoid[A]: Monoid[LastMaybe[A]] =
    new Monoid[LastMaybe[A]] {
      ...
      def append(fa1: LastMaybe[A], fa2: => LastMaybe[A]): LastMaybe[A] =
  ...
  implicit val maybeInstance: Traverse[Maybe]
    ...
    with IsEmpty[Maybe]
    with Cobind[Maybe]
    with Optional[Maybe] = new Traverse[Maybe]
    with MonadPlus[Maybe]
    with BindRec[Maybe]
    with Cozip[Maybe]
    with Zip[Maybe]
