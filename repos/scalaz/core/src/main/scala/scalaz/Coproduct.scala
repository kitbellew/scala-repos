...
final case class Coproduct[F[_], G[_], A](run: F[A] \/ G[A]) {
  ...
  def map[B](
      f: A => B)(implicit F: Functor[F], G: Functor[G]): Coproduct[F, G, B] =
  ...
  def contramap[B](f: B => A)(implicit
      F: Contravariant[F],
      G: Contravariant[G]): Coproduct[F, G, B] =
    Coproduct(run.bimap(F.contramap(_)(f), G.contramap(_)(f)))

  def foldRight[Z](z: => Z)(
      f: (A, => Z) => Z)(implicit F: Foldable[F], G: Foldable[G]): Z =
    run.fold(a => F.foldRight(a, z)(f), a => G.foldRight(a, z)(f))

  def foldMap[B](
      f: A => B)(implicit F: Foldable[F], G: Foldable[G], M: Monoid[B]): B =
    run.fold(F.foldMap(_)(f), G.foldMap(_)(f))

  def foldMap1[B](f: A => B)(implicit
      F: Foldable1[F],
      G: Foldable1[G],
      M: Semigroup[B]): B = run.fold(F.foldMap1(_)(f), G.foldMap1(_)(f))
  ...
  def traverse[X[_], B](g: A => X[B])(implicit
      F: Traverse[F],
      G: Traverse[G],
  ...
  def unary_~ : Coproduct[G, F, A] = Coproduct(~run)

  def validation: Validation[F[A], G[A]] = run.validation

}

object Coproduct extends CoproductInstances {
  implicit def coproductTraverse1[F[_], G[_]](implicit
      F0: Traverse1[F],
      G0: Traverse1[G]): Traverse1[Coproduct[F, G, ?]] =
...
sealed abstract class CoproductInstances3 {
  ...
  implicit def coproductFunctor[F[_], G[_]](implicit
      F0: Functor[F],
      G0: Functor[G]): Functor[Coproduct[F, G, ?]] =
    new CoproductFunctor[F, G] {
      implicit def F: Functor[F] = F0
      implicit def G: Functor[G] = G0
    }

  implicit def coproductFoldable[F[_], G[_]](implicit
      F0: Foldable[F],
      G0: Foldable[G]): Foldable[Coproduct[F, G, ?]] =
...
sealed abstract class CoproductInstances2 extends CoproductInstances3 {
  implicit def coproductContravariant[F[_], G[_]](implicit
      F0: Contravariant[F],
      G0: Contravariant[G]): Contravariant[Coproduct[F, G, ?]] =
    new CoproductContravariant[F, G] {
      implicit def F: Contravariant[F] = F0
      implicit def G: Contravariant[G] = G0
    }

  implicit def coproductFoldable1[F[_], G[_]](implicit
      F0: Foldable1[F],
      G0: Foldable1[G]): Foldable1[Coproduct[F, G, ?]] =
...
sealed abstract class CoproductInstances1 extends CoproductInstances2 {
  implicit def coproductCobind[F[_], G[_]](implicit
      F0: Cobind[F],
      G0: Cobind[G]): Cobind[Coproduct[F, G, ?]] =
...
sealed abstract class CoproductInstances0 extends CoproductInstances1 {
  implicit def coproductTraverse[F[_], G[_]](implicit
      F0: Traverse[F],
      G0: Traverse[G]): Traverse[Coproduct[F, G, ?]] =
...
sealed abstract class CoproductInstances extends CoproductInstances0 {
  implicit def coproductComonad[F[_], G[_]](implicit
      F0: Comonad[F],
      G0: Comonad[G]): Comonad[Coproduct[F, G, ?]] =
...
private trait CoproductFunctor[F[_], G[_]] extends Functor[Coproduct[F, G, ?]] {
  ...
  override def map[A, B](a: Coproduct[F, G, A])(f: A => B) = a map f
}

private trait CoproductContravariant[F[_], G[_]]
    extends Contravariant[Coproduct[F, G, ?]] {
  implicit def F: Contravariant[F]
  implicit def G: Contravariant[G]

  override def contramap[A, B](a: Coproduct[F, G, A])(f: B => A) = a contramap f
}
...
private trait CoproductFoldable1[F[_], G[_]]
    extends Foldable1[Coproduct[F, G, ?]] {
  implicit def F: Foldable1[F]
  implicit def G: Foldable1[G]

  override final def foldMap1[A, B: Semigroup](fa: Coproduct[F, G, A])(
      f: A => B): B = fa.foldMap1(f)

  override final def foldMapRight1[A, B](fa: Coproduct[F, G, A])(z: A => B)(
      f: (A, => B) => B): B = fa.foldMapRight1(z)(f)
}
...
private trait CoproductTraverse1[F[_], G[_]]
  ...
  override final def traverse1Impl[X[_]: Apply, A, B](fa: Coproduct[F, G, A])(
      f: A => X[B]): X[Coproduct[F, G, B]] = fa traverse1 f

  override final def map[A, B](a: Coproduct[F, G, A])(f: A => B) = a map f
}
...
private trait CoproductComonad[F[_], G[_]] extends Comonad[Coproduct[F, G, ?]] {
  ...
  override def copoint[A](p: Coproduct[F, G, A]) = p.copoint

  override def cobind[A, B](a: Coproduct[F, G, A])(f: Coproduct[F, G, A] => B) =
    a cobind f

  override def cojoin[A](a: Coproduct[F, G, A]) = a.duplicate
}
