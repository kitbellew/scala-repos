...
private sealed trait OneAndApply[F[_]]
    extends Apply[OneAnd[F, ?]]
    with OneAndFunctor[F] {
  def F: Applicative[F]
  def G: Plus[F]
  ...
  override def ap[A, B](fa: => OneAnd[F, A])(
    ...
    val OneAnd(hf, tf) = f
    val OneAnd(ha, ta) = fa
    OneAnd(hf(ha), G.plus(F.map(ta)(hf), F.ap(G.plus(F.point(ha), ta))(tf)))
  }
}

private sealed trait OneAndAlign[F[_]]
    extends Align[OneAnd[F, ?]]
    with OneAndFunctor[F] {
  def F: Align[F]
...
private sealed trait OneAndMonad[F[_]]
    extends Monad[OneAnd[F, ?]]
    with OneAndBind[F]
    with OneAndApplicative[F] {
  def F: MonadPlus[F]
  def G = F
}
...
private sealed trait OneAndFoldable[F[_]] extends Foldable1[OneAnd[F, ?]] {
  ...
  override def foldMapRight1[A, B](fa: OneAnd[F, A])(z: A => B)(
    ...
    (F.foldRight(fa.tail, none[B])((a, ob) =>
      ob map (f(a, _)) orElse some(z(a))) map (f(fa.head, _)) getOrElse
      z(fa.head))

  override def foldMapLeft1[A, B](fa: OneAnd[F, A])(z: A => B)(f: (B, A) => B) =
    F.foldLeft(fa.tail, z(fa.head))(f)

  override def foldMap[A, B](fa: OneAnd[F, A])(f: A => B)(implicit
      M: Monoid[B]) = M.append(f(fa.head), F.foldMap(fa.tail)(f))
...
private sealed trait OneAndFoldable1[F[_]] extends OneAndFoldable[F] {
  ...
  override def foldMap1[A, B](fa: OneAnd[F, A])(f: A => B)(implicit
      S: Semigroup[B]) = S.append(f(fa.head), F.foldMap1(fa.tail)(f))
...
private sealed trait OneAndTraverse[F[_]]
  ...
  def traverse1Impl[G[_], A, B](fa: OneAnd[F, A])(f: A => G[B])(implicit
      G: Apply[G]) =
    G.applyApplicative.traverse(fa.tail)(f andThen \/.left)(F).fold(
      ftl => G.apply2(f(fa.head), ftl)(OneAnd.apply),
      tl => G.map(f(fa.head))(OneAnd(_, tl)))

  override def traverseImpl[G[_], A, B](fa: OneAnd[F, A])(f: A => G[B])(implicit
      G: Applicative[G]) =
...
private sealed trait OneAndTraverse1[F[_]]
    extends OneAndTraverse[F]
    with OneAndFoldable1[F] {
  def F: Traverse1[F]
