...
trait Traverse[F[_]] extends Functor[F] with Foldable[F] { self =>
  ...
  def foldMap[A, B](fa: F[A])(f: A => B)(implicit F: Monoid[B]): B =
    foldLShape(fa, F.zero)((b, a) => F.append(b, f(a)))._1
  ...
  def indexed[A](fa: F[A]): F[(Int, A)] =
    mapAccumL(fa, 0) { case (s, a) => (s + 1, (s, a)) }._2
  ...
  trait TraverseLaw extends FunctorLaw {
    ...
    def purity[G[_], A](
        fa: F[A])(implicit G: Applicative[G], GFA: Equal[G[F[A]]]): Boolean =
    ...
    /**
      ...
      */
    def naturality[N[_], M[_], A](nat: (M ~> N))(fma: F[M[A]])(implicit
        N: Applicative[N],
        M: Applicative[M],
    ...
    def parallelFusion[N[_], M[_], A, B](
      ...
      val t2: MN[F[B]] = traverse[MN, A, B](fa)(a => (amb(a), anb(a)))(
        M product N)
      MN.equal(t1, t2)
    }
  }
