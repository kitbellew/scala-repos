...
final class NonEmptyList[A] private[scalaz] (val head: A, val tail: IList[A]) {
  ...
  def traverse1[F[_], B](f: A => F[B])(implicit
      F: Apply[F]): F[NonEmptyList[B]] = {
    tail match {
      case INil() =>
        F.map(f(head))(nel(_, INil()))
      case ICons(b, bs) =>
        F.apply2(
          f(head),
  ...
  def zipperEnd: Zipper[A] = {
    ...
    tail.reverse match {
      case INil() =>
        zipper(empty, head, empty)
      case ICons(t, ts) =>
        zipper(ts.toStream :+ head, t, empty)
    }
  }
  ...
  def tails: NonEmptyList[NonEmptyList[A]] = {
    ...
    def tails0(
      ...
      as.tail match {
        case INil() =>
          nel(as, accum).reverse
        case ICons(h, t) =>
          tails0(nel(h, t), as :: accum)
      }
  ...
  def unzip[X, Y](implicit
      ev: A <~< (X, Y)): (NonEmptyList[X], NonEmptyList[Y]) = {
    val (a, b) = ev(head)
...
sealed abstract class NonEmptyListInstances extends NonEmptyListInstances0 {
  implicit val nonEmptyList: Traverse1[NonEmptyList]
    ...
    new Traverse1[NonEmptyList]
      ...
      override def foldMap1[A, B](fa: NonEmptyList[A])(f: A => B)(implicit
          F: Semigroup[B]): B = {
      ...
      override def foldLeft[A, B](fa: NonEmptyList[A], z: B)(
          f: (B, A) => B): B = fa.tail.foldLeft(f(z, fa.head))(f)
