...
sealed class StreamT[M[_], A](val step: M[StreamT.Step[A, StreamT[M, A]]]) {
  ...
  def trans[N[_]](
    ...
    StreamT(t(M.map(this.step)(_(
      yieldd = (a, as) => Yield(a, as trans t),
  ...
  def dropWhile(p: A => Boolean)(implicit m: Functor[M]): StreamT[M, A] =
    stepMap {
      _(
        yieldd = (a, as) => if (p(a)) Skip(as dropWhile p) else Yield(a, as),
  ...
  def takeWhile(p: A => Boolean)(implicit m: Functor[M]): StreamT[M, A] =
    stepMap {
      _(
        yieldd = (a, as) => if (p(a)) Yield(a, as takeWhile p) else Done,
  ...
  def flatMap[B](f: A => StreamT[M, B])(implicit m: Functor[M]): StreamT[M, B] =
    stepMap {
      _(
        yieldd = (a, s) => Skip(f(a) ++ (s flatMap f)),
  ...
  def foldRight[B](z: => B)(f: (=> A, => B) => B)(implicit M: Monad[M]): M[B] =
    M.map(rev) { _.foldLeft(z)((a, b) => f(b, a)) }

  def foldMap[B](f: A => B)(implicit M: Foldable[M], B: Monoid[B]): B =
    M.foldMap(step) {
      _(
        yieldd = (a, s) => B.append(f(a), s.foldMap(f)),
        skip = s => s.foldMap(f),
        done = B.zero)
    }
...
object StreamT extends StreamTInstances {
  ...
  object Yield {
    def apply[A, S](a: A, s: => S): Step[A, S] =
      new Step[A, S] {
        def apply[Z](yieldd: (A, => S) => Z, skip: => S => Z, done: => Z) =
          yieldd(a, s)
      }
  ...
  object Done extends Step[Nothing, Nothing] {
    def apply[Z](
        yieldd: (Nothing, => Nothing) => Z,
        skip: => Nothing => Z,
        done: => Z) = done
    def unapply[A, S](s: Step[A, S]): Boolean =
      s((_, _) => false, _ => false, true)
  }
}
...
private trait StreamTInstance1[F[_]]
  ...
  override final def map[A, B](fa: StreamT[F, A])(f: A => B) = fa map f

  override final def bind[A, B](fa: StreamT[F, A])(f: A => StreamT[F, B]) =
    fa flatMap f

  override final def plus[A](a: StreamT[F, A], b: => StreamT[F, A]) = a ++ b
}
