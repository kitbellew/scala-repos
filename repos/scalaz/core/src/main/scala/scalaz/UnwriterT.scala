...
final case class UnwriterT[F[_], U, A](run: F[(U, A)]) {
  ...
  def bimap[C, D](f: U => C, g: A => D)(implicit F: Functor[F]) =
    unwriterT[F, C, D](F.map(run)({ case (a, b) => (f(a), g(b)) }))
  ...
  def wpoint[G[_]](implicit
    ...
    unwriterT(F.map(self.run) { case (u, a) => (P.point(u), a) })
...
private trait UnwriterTFoldable[F[_], W]
    extends Foldable.FromFoldr[UnwriterT[F, W, ?]] {
  implicit def F: Foldable[F]

  override def foldRight[A, B](fa: UnwriterT[F, W, A], z: => B)(
