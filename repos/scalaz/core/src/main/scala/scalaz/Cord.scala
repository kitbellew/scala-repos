...
final case class Cord(self: FingerTree[Int, String]) {
  ...
  private def rangeError(i: Int) =
    sys.error("Index out of range: " + i + " >= " + self.measure)
  ...
  def flatMap(f: Char => Cord): Cord =
    toVector.foldLeft(Cord())((as, a) => as ++ f(a))
...
object Cord {
  ...
  implicit val sizer: Reducer[String, Int] = UnitReducer((a: String) =>
    a.length)
  ...
  implicit lazy val CordShow: Show[Cord] =
    new Show[Cord] {
      override def show(x: Cord) = x
      override def shows(x: Cord) = x.toString
    }
  implicit lazy val CordMonoid: Monoid[Cord] =
    new Monoid[Cord] {
      def zero = empty
