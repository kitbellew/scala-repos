...
abstract class Adjunction[F[_], G[_]](implicit
  ...
  implicit val comonad: Comonad[λ[α => F[G[α]]]] =
    new Comonad[λ[α => F[G[α]]]] {
      ...
      def cobind[A, B](a: F[G[A]])(f: F[G[A]] => B): F[G[B]] =
        F.map(a)(leftAdjunct(_)(f))
  ...
  def compose[P[_], Q[_]](implicit
    ...
    new (λ[α => P[F[α]]] -| λ[α => G[Q[α]]]) {
      ...
      override def counit[A](a: P[F[G[Q[A]]]]): A =
        A.counit(P.map(a)(self.counit))
    }
  }
}
...
object Adjunction extends AdjunctionInstances {
  ...
  def apply[F[_], G[_]](implicit A: F -| G, F: Functor[F], G: Functor[F]): F -|
    G = A
}
...
sealed abstract class AdjunctionInstances {
  ...
  implicit def curryUncurryAdjunction[S]: (S, ?) -| (S => ?) =
    new Adjunction[(S, ?), (S => ?)] {
      override def leftAdjunct[A, B](a: => A)(f: ((S, A)) => B): S => B =
        s => f(s, a)
      override def rightAdjunct[A, B](a: (S, A))(f: A => S => B): B =
        f(a._2)(a._1)
    }
  ...
  implicit val f0Adjunction: Function0 -| Function0 =
    new Adjunction[Function0, Function0] {
      override def leftAdjunct[A, B](a: => A)(f: (() => A) => B): () => B =
        () => f(() => a)
      override def rightAdjunct[A, B](a: () => A)(f: A => () => B): B = f(a())()
    }
  ...
  implicit def writerReaderAdjunction[E]
