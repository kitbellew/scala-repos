...
private trait CompositionApply[F[_], G[_]]
    extends Apply[λ[α => F[G[α]]]]
    with CompositionFunctor[F, G] {
...
private trait CompositionApplicative[F[_], G[_]]
    extends Applicative[λ[α => F[G[α]]]]
    with CompositionApply[F, G] {
...
private trait CompositionPlusEmpty[F[_], G[_]]
    extends PlusEmpty[λ[α => F[G[α]]]]
    with CompositionPlus[F, G] {
  implicit def F: PlusEmpty[F]

  def empty[A]: F[G[A]] = F.empty[G[A]]
}

private trait CompositionApplicativePlus[F[_], G[_]]
    extends ApplicativePlus[λ[α => F[G[α]]]]
    with CompositionApplicative[F, G]
    with CompositionPlusEmpty[F, G] {
  implicit def F: ApplicativePlus[F]

  implicit def G: Applicative[G]
}

private trait CompositionFoldable[F[_], G[_]]
    extends Foldable[λ[α => F[G[α]]]] {
...
private trait CompositionTraverse[F[_], G[_]]
    extends Traverse[λ[α => F[G[α]]]]
    with CompositionFunctor[F, G]
    with CompositionFoldable[F, G] {
...
private trait CompositionFoldable1[F[_], G[_]]
    extends Foldable1[λ[α => F[G[α]]]]
    with CompositionFoldable[F, G] {
  ...
  override final def foldMapRight1[A, B](fa: F[G[A]])(z: A => B)(
      f: (A, => B) => B) =
...
private trait CompositionTraverse1[F[_], G[_]]
    extends Traverse1[λ[α => F[G[α]]]]
    with CompositionFoldable1[F, G] {
...
private trait CompositionDistributive[F[_], G[_]]
    extends Distributive[λ[α => F[G[α]]]]
    with CompositionFunctor[F, G] {
...
private trait CompositionBifunctor[F[_, _], G[_, _]]
    extends Bifunctor[λ[(α, β) => F[G[α, β], G[α, β]]]] {
...
private trait CompositionBifoldable[F[_, _], G[_, _]]
    extends Bifoldable[λ[(α, β) => F[G[α, β], G[α, β]]]] {
...
private trait CompositionBitraverse[F[_, _], G[_, _]]
    ...
    with CompositionBifunctor[F, G]
    with CompositionBifoldable[F, G] {
...
private trait CompositionFunctorBifunctor[F[_], G[_, _]]
    extends Bifunctor[λ[(α, β) => F[G[α, β]]]] {
  def F: Functor[F]
...
private trait CompositionBifunctorFunctors[F[_, _], G[_], H[_]]
    extends Bifunctor[λ[(α, β) => F[G[α], H[β]]]] {
  def F: Bifunctor[F]
...
private trait CompositionFoldableBifoldable[F[_], G[_, _]]
    extends Bifoldable[λ[(α, β) => F[G[α, β]]]] {
  def F: Foldable[F]
...
private trait CompositionBifoldableFoldables[F[_, _], G[_], H[_]]
    extends Bifoldable[λ[(α, β) => F[G[α], H[β]]]] {
  def F: Bifoldable[F]
  ...
  override def bifoldMap[A, B, M: Monoid](fgahb: F[G[A], H[B]])(f: A => M)(
      g: B => M) =
