...
sealed abstract class Free[S[_], A] {
  ...
  final def zap[G[_], B](
      fs: Cofree[G, A => B])(implicit S: Functor[S], d: Zap[S, G]): B =
    zapWith(fs)((a, f) => f(a))
  ...
  final def bounce(f: S[Free[S, A]] => Free[S, A])(implicit
      S: Functor[S]): Free[S, A] = resume match {
    case -\/(s) => f(s)
    case \/-(r) => Return(r)
  }
  ...
  def extendF[T[_]](f: Free[S, ?] ~> T): Free[T, A] = mapSuspension(
    new (S ~> T) {
      def apply[X](x: S[X]) = f(liftF(x))
    })

  /** Extraction from `Free` as a comonad in the endofunctor category. */
  def extractF(implicit S: Monad[S]): S[A] = foldMap(
    NaturalTransformation.refl[S])
...
sealed trait TrampolineInstances {
  implicit val trampolineInstance
      : Monad[Trampoline] with Comonad[Trampoline] with BindRec[Trampoline] =
    new Monad[Trampoline] with Comonad[Trampoline] with BindRec[Trampoline] {
      ...
      def cobind[A, B](fa: Trampoline[A])(f: Trampoline[A] => B) = return_(
        f(fa))
...
private sealed trait FreeTraverse[F[_]]
  ...
  override final def traverseImpl[G[_], A, B](fa: Free[F, A])(f: A => G[B])(
      implicit G: Applicative[G]): G[Free[F, B]] =
    fa.resume match {
      case -\/(s) =>
