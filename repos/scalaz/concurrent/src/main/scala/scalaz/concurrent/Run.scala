...
object Run {
  def apply[A](c: A => Unit)(implicit s: Strategy): Run[A] =
    new Run[A] {
      def run(a: A) = c(a)
      val strategy = s
    }
  ...
  implicit val runContravariant =
    new Contravariant[Run] {
      def contramap[A, B](r: Run[A])(f: B => A): Run[B] =
        new Run[B] {
