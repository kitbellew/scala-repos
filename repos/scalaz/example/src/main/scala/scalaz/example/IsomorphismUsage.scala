...
object IsomorphismUsage extends App {
  ...
  implicit val isoTraverse: IsomorphismTraverse[Seq, List] =
    new IsomorphismTraverse[Seq, List] {
      def G = Traverse[List]
      def iso = isoFunctor
    }
  ...
  assert(
    List(Seq(1, 2), Seq(3, 4)).sequence.toList ===
      List(List(1, 2), List(3, 4)).sequence)
}
