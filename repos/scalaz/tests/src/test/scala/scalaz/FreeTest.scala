...
object FreeList extends FreeListInstances {
  ...
  implicit def freeListArb[A](implicit
    ...
    Arbitrary(
      FreeTest.freeGen[List, A](
        Gen.choose(0, 2).flatMap(
          Gen.listOfN(_, freeListArb[A].arbitrary.map(_.f)))
      ).map(FreeList.apply))

  implicit def freeListEq[A](implicit A: Equal[A]): Equal[FreeList[A]] =
    new Equal[FreeList[A]] {
      def equal(a: FreeList[A], b: FreeList[A]) =
...
object FreeOption {
  implicit def freeOptionBindRec: BindRec[FreeOption] =
    new BindRec[FreeOption] {
  ...
  implicit def freeOptionArb[A](implicit
    ...
    Arbitrary(
      FreeTest.freeGen[Option, A](
        Gen.choose(0, 1).flatMap(
  ...
  implicit def freeOptionEq[A](implicit A: Equal[A]): Equal[FreeOption[A]] =
    new Equal[FreeOption[A]] {
      def equal(a: FreeOption[A], b: FreeOption[A]) =
