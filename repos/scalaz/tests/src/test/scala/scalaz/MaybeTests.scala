...
object MaybeTest extends SpecLite {
  ...
  "Empty is ignored in Maybe[A]@@Min" !
    forAll { x: Maybe[Int] =>
      import syntax.monoid._
      (Min(x) |+| Min(empty)) must_=== Min(x)
    }

  "Empty is ignored in Maybe[A]@@Max" !
    forAll { x: Maybe[Int] =>
      import syntax.monoid._
  ...
  "empty toLeft is right" !
    forAll { s: String =>
      empty.toLeft(s).isRight
    }

  "just toRight is right" !
    forAll { (x: Int, s: String) =>
      just(x).toRight(s).isRight
    }

  "empty toRight is left" !
    forAll { s: String =>
  ...
  "just orElse is just" !
    forAll { (x: Int, m: Maybe[Int]) =>
      just(x).orElse(m).isJust
    }

  "fromNullable(null) is Empty" !
    check {
      val s: String = null
      Maybe.fromNullable(s).isEmpty
    }

  "fromNullable(notNull) is just" !
    forAll { (s: String) =>
