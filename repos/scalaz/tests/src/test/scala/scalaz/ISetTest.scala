...
object ISetTest extends SpecLite {
  ...
  "findLeft" !
    forAll { a: ISet[Int] =>
      val f = (_: Int) % 3 == 0
      Foldable[ISet].findLeft(a)(f) must_===
        Foldable[List].findLeft(a.toList)(f)
    }
  ...
  "filter" !
    forAll { (a: ISet[Int], p: Int => Boolean) =>
  ...
  "map" !
    forAll { (a: ISet[Int], f: Int => Int) =>
      a.map(f).toList must_=== a.toList.map(f).distinct.sorted
    }

  "filter" !
    forAll { (s: ISet[Int], p: Int => Boolean) =>
