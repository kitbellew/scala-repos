...
object ZipperTest extends SpecLite {
  ...
  "Next Affects Lengths" ! forAll { (xs: Stream[Int]) =>
    (for (z <- xs.toZipper; zn <- z.next) yield {
  ...
  "nextC moves focus or loops" ! forAll { z: Zipper[Int] =>
    ...
    if (z.atEnd) zn.atStart must_== (true) else zn.index must_=== (z.index + 1)
  }
  ...
  "previousC moves focus or loops" ! forAll { z: Zipper[Int] =>
    ...
    if (z.atStart) zp.atEnd must_== (true) else zp.index must_=== (z.index - 1)
  }
  ...
  "Move" ! forAll { (xs: Stream[Int], ys: Stream[Int], f: Int, n: Short) =>
    zipper(xs, f, ys).move(n) map { (z: Zipper[Int]) =>
      ...
      if (n > 0) ys(n - 1) must_=== (z.focus)
      else if (n < 0) xs(-(n + 1)) must_=== (z.focus)
      else f must_=== (z.focus)
    } getOrElse {
  ...
  "moveOr should return some of move or an alternative" ! forAll {
    (l: Stream[Int], f: Int, r: Stream[Int], n: Short, alt: Zipper[Int]) =>
      ...
      if (l.length < (-n) || r.length < n) z must_=== (alt)
        ...
        if (n > 0) r(n - 1) must_=== (z.focus)
        else if (n < 0) l(-(n + 1)) must_=== (z.focus)
        else f must_=== (z.focus)
      }
  }

  "Length should return the size of the zipper" ! forAll {
    (l: Stream[Int], f: Int, r: Stream[Int]) =>
  ...
  "The zipper should be atEnd when the right stream is empty" ! forAll {
    (l: Stream[Int], f: Int, r: Stream[Int]) =>
      if (zipper(l, f, r).atEnd) r.isEmpty else !r.isEmpty
  }

  "Find" ! forAll {
    (xs: Stream[Int], ys: Stream[Int], f: Int, n: Int, m: Int) =>
      val p = (i: Int) => i < n && i > m
      zipper(xs, f, ys).findZ(p) map { z => p(z.focus) } getOrElse
        !(xs.find(p).isDefined || ys.find(p).isDefined || p(f))
  }
  ...
  "findBy if given a function that returns None should not return anything" !
    forAll { z: Zipper[Int] => z.findBy(z => None)(x => x == z.focus).isEmpty }
  ...
  "Update Modifies Zipper Correctly" ! forAll {
    (xs: Stream[Int], ys: Stream[Int], f: Int, u: Int) =>
      zipper(xs, f, ys).update(u) must_=== (zipper(xs, u, ys))
  }

  "Modify Modifies Zipper Correctly" ! forAll {
    (xs: Stream[Int], ys: Stream[Int], f: Int, u: Int) =>
