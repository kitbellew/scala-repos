...
abstract class XMapTest[Map[K, V] <: SMap[K, V] with MapLike[
    ...
    forAll {
      ...
      (kvs: List[(String, Int)], kvs2: List[(String, Int)]) => {
        ...
        ((m1.size == kvs.size) && (m2.size == kvs2.size)) ==> {
          ...
          val r: Boolean = (if (m1.size < m2.size)
                              true
                            else if (m1.size > m2.size)
                              false
                            else
                              O2.lt(kvs.sortBy(_._1), kvs2.sortBy(_._1)))
          l == r
        }
      }
    }
  ...
  "align" !
    forAll { (a: Map[Int, String], b: Map[Int, Long]) =>
      import std.set._, \&/._
      val F = Align[Map[Int, ?]]
      val x = F.align(a, b)
      val keysA = a.keySet
  ...
  "getOrAdd" !
    forAll { (m0: Map[Int, Long], k: Int, vOld: Long, vNew: Long) =>
