...
object VectorTest extends SpecLite {
  ...
  "filter consistent with fiterM[Id]" ! forAll { (xs: Vector[Int], p: Int => Boolean) =>
  ...
  "mapAccumLeft" ! forAll { (xs: Vector[Int]) =>
    mapAccumLeft(xs)(Vector[Int](), (c: Vector[Int], a) => (c :+ a, a)) must_=== (xs, xs)
  }

  "mapAccumRight" ! forAll { (xs: Vector[Int]) =>
    mapAccumRight(xs)(Vector[Int](), (c: Vector[Int], a) => (c :+ a, a)) must_=== (xs.reverse, xs)
  }
