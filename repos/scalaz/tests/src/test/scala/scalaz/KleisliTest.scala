...
object KleisliTest extends SpecLite {
  ...
  implicit def Function1IntOptInt[A](implicit
      A: Arbitrary[Option[Int]]): Arbitrary[Int => Option[Int]] =
    Arbitrary(
  ...
  implicit def KleisliEqual[M[_]](implicit
      M: Equal[M[Int]]): Equal[Kleisli[M, Int, Int]] =
  ...
  "mapK" ! forAll {
    (f: Int => Option[Int], a: Int) =>
      Kleisli(f).mapK(_.toList.map(_.toString)).run(a) must_=== (f(
        a).toList.map(_.toString))
  }
  ...
  object instances {
    ...
    def semigroup[F[_], A, B](implicit FB: Monoid[F[B]]) =
      Semigroup[Kleisli[F, A, B]]
  ...
  "Catchable[Kleisli]" should {
    ...
    "catch ambient exceptions (1/2)" in {
      C.attempt(Kleisli(_ => IO[Int](throw err))).run(
        1).unsafePerformIO must_== -\/(err)
    }
