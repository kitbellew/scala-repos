...
object EphemeralStreamTest extends SpecLite {
  ...
  "foldLeft" !
    forAll { xs: List[List[Int]] =>
      Foldable[EphemeralStream]
        .foldLeft(EphemeralStream(xs: _*), List[Int]())(_ ::: _) must_===
        (xs.foldLeft(List[Int]())(_ ::: _))
    }

  "unzip zip" !
    forAll { xs: EphemeralStream[(Int, Int)] =>
      val (firsts, seconds) = xs.unzip
      (firsts zip seconds) must_=== (xs)
    }

  "zip has right length" !
    forAll { (xs: EphemeralStream[Int], ys: EphemeralStream[Int]) =>
      (xs zip ys).length must_=== (xs.length min ys.length)
    }

  "interleave has right length" !
    forAll { (xs: EphemeralStream[Int], ys: EphemeralStream[Int]) =>
      (xs interleave ys).length must_=== (xs.length + ys.length)
    }

  "take" !
    forAll { (xs: Stream[Int], n: Int) =>
  ...
  "index" !
    forAll { (xs: EphemeralStream[Int], i: Int) =>
  ...
  "foldMap evaluates lazily" in {
    ...
    Foldable[EphemeralStream]
      .foldMap(infiniteStream)(identity)(booleanInstance.conjunction) must_===
      (false)
  }
  ...
  "zipWithIndex from infinite stream" in {
    ...
    EphemeralStream.iterate(0)(_ + 1).zipWithIndex.take(n) must_===
      (EphemeralStream
        .fromStream(Stream.iterate(0)(_ + 1).zipWithIndex.take(n)))
  }
}
