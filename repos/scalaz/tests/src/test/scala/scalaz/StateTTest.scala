...
object StateTTest extends SpecLite {
  ...
  implicit def stateTListEqual =
    Equal[List[(Int, Int)]].contramap((_: StateTListInt[Int]).runZero[Int])
  ...
  "monadPlus.plus (List)" in {
    ...
    instances.monadPlus[Int, List].plus(a, b).run(0) must_=== (List(
      (0, false),
      (0, true)))
  }
  ...
  "StateT can be trampolined without stack overflow" in {
    ...
    val result = (0 to 4000).toList
      .map(i => StateT[Trampoline, Int, Int]((ii: Int) => Trampoline.done((i, i))))
