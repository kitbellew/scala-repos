...
object MapTest extends SpecLite {
  ...
  "==>> fromList" ! {
    ...
    fromList(List(5 -> "a", 3 -> "b")) must_=== (fromList(
      List(3 -> "b", 5 -> "a")))
    fromList(List(5 -> "a", 3 -> "b", 5 -> "c")) must_=== (fromList(
      List(5 -> "c", 3 -> "b")))
    fromList(List(5 -> "c", 3 -> "b", 5 -> "a")) must_=== (fromList(
      List(5 -> "a", 3 -> "b")))
  }
  ...
  "==>> conversions" should {
    "toAscList" in {
      ...
      fromList(List(5 -> "a", 3 -> "b")).toAscList must_=== (fromList(
        List(3 -> "b", 5 -> "a")).toAscList)
      fromList(List(5 -> "a", 7 -> "c", 3 -> "b")) must_=== (fromList(
        List(3 -> "b", 5 -> "a", 7 -> "c")))
    }
  }
  ...
  "==>> lookups" should {
    ...
    "value lookup" in {
      ...
      fromList(List(("John", "Sales"), ("Bob", "IT")))
        .lookup("Sarah") must_=== (None)
    }
  ...
  "updateAt" should {
    "succeed" in {
      ...
      d.updateAt(0, (_, _) => "x".some) must_=== (fromList(
        List(3 -> "x", 5 -> "a")))
      d.updateAt(1, (_, _) => "x".some) must_=== (fromList(
        List(3 -> "b", 5 -> "x")))
  ...
  "delete" should {
    ...
    "not remove an element" in {
      fromList(List((5, "a"), (3, "b"))).delete(7) must_=== (fromList(
        List((5, "a"), (3, "b"))))
    }
  ...
  "==>> union operations" should {
    ...
    "unions" in {
      unions(List(
        ...
        fromList(List((5, "A3"), (3, "B3"))))) must_== fromList(
        List((3, "b"), (5, "a"), (7, "C")))
  ...
  "==>> intersection operations" should {
    "intersection" in {
      val r = fromList(List(5 -> "a", 3 -> "b")) intersection fromList(
        List(5 -> "A", 7 -> "C"))
  ...
  "==>> update" should {
    "adjust" in {
      ...
      fromList(List(5 -> "a", 3 -> "b")).adjust(5, f) must_=== (fromList(
        List(3 -> "b", 5 -> "new a")))
      fromList(List(5 -> "a", 3 -> "b")).adjust(7, f) must_=== (fromList(
        List(3 -> "b", 5 -> "a")))
    ...
    "adjustWithKey" in {
      ...
      fromList(List(5 -> "a", 3 -> "b")).adjustWithKey(5, f) must_=== (fromList(
        List(3 -> "b", 5 -> "5:new a")))
      fromList(List(5 -> "a", 3 -> "b")).adjustWithKey(7, f) must_=== (fromList(
        List(3 -> "b", 5 -> "a")))
    ...
    "update" in {
      ...
      fromList(List(5 -> "a", 3 -> "b")).update(5, f) must_=== (fromList(
        List(3 -> "b", 5 -> "new a")))
      fromList(List(5 -> "a", 3 -> "b")).update(7, f) must_=== (fromList(
        List(3 -> "b", 5 -> "a")))
      fromList(List(5 -> "a", 3 -> "b")).update(3, f) must_=== (singleton(
        5,
        "a"))
    }
    ...
    "updateWithKey" in {
      ...
      fromList(List(5 -> "a", 3 -> "b")).updateWithKey(5, f) must_=== (fromList(
        List(3 -> "b", 5 -> "5:new a")))
      fromList(List(5 -> "a", 3 -> "b")).updateWithKey(7, f) must_=== (fromList(
        List(3 -> "b", 5 -> "a")))
    ...
    "alter" in {
      ...
      fromList(List(5 -> "a", 3 -> "b")).alter(7, f1) must_=== (fromList(
        List(3 -> "b", 5 -> "a")))
      fromList(List(5 -> "a", 3 -> "b")).alter(5, f1) must_=== (singleton(
        3,
        "b"))

      val f2 = (_: Option[String]) => "c".some
      fromList(List(5 -> "a", 3 -> "b")).alter(7, f2) must_=== (fromList(
        List(3 -> "b", 5 -> "a", 7 -> "c")))
      fromList(List(5 -> "a", 3 -> "b")).alter(5, f2) must_=== (fromList(
        List(3 -> "b", 5 -> "c")))
    }
  }
  ...
  "==>> map" should {
    ...
    "map" in {
      fromList(List(5 -> "a", 3 -> "b")).map(_ + "x") must_=== (fromList(
        List(3 -> "bx", 5 -> "ax")))
    }

    "mapWithKey" in {
      val f = (k: Int, x: String) => k.toString + ":" + x
      fromList(List(5 -> "a", 3 -> "b")).mapWithKey(f) must_=== (fromList(
        List(3 -> "3:b", 5 -> "5:a")))
    }
    ...
    "mapOption" in {
      val f = (x: String) => if (x == "a") Some("new a") else None
      fromList(List(5 -> "a", 3 -> "b")).mapOption(f) must_=== (singleton(
        5,
        "new a"))
    }
  ...
  "==>> list operations" should {
    ...
    "keySet" in {
      fromList(List(5 -> "a", 3 -> "b")).keySet must_=== (ISet
        .fromList(List(3, 5)))
    ...
    "toList" in {
      import std.tuple._
      fromList(List(5 -> "a", 3 -> "b")).toList must_=== (List(
        3 -> "b",
        5 -> "a"))
  ...
  "align" ! forAll { (a: Int ==>> String, b: Int ==>> Long) =>
    ...
    x.filter(_.isThis) must_=== a.filterWithKey((k, _) => !keysB.member(k))
      .map(This(_))
    x.filter(_.isThat) must_=== b.filterWithKey((k, _) => !keysA.member(k))
      .map(That(_))
    x.filter(_.isBoth) must_=== a.filterWithKey((k, _) => keysB.member(k))
