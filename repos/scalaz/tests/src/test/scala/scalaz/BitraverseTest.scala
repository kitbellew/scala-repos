...
object BitraverseTest extends SpecLite {
  ...
  "left/right bias" in {
    ...
    Bitraverse[\/].rightTraverse.traverse(42.left[Int])(x =>
      Vector(x + 3)) must_=== (Vector(-\/(42)))
    Bitraverse[\/].leftTraverse.traverse(42.left[Int])(x =>
      Vector(x + 3)) must_=== (Vector(-\/(45)))
  ...
  "both sides, left and right embedding" in {
    implicit val L_E_LO_RL: Traverse[λ[α => Option[α] \/ List[Int]]] =
