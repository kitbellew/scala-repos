...
object OrderTest extends SpecLite {
  "duals" ! forAll { (xs: List[Int]) =>
    ...
    (F maximum xs: Option[Int]) must_=== (Tag unsubst (F minimum xsdual: Option[
      Int @@ Tags.Dual]))
    (F minimum xs: Option[Int]) must_=== (Tag unsubst (F maximum xsdual: Option[
      Int @@ Tags.Dual]))
  }
  ...
  "semigroups max" ! forAll { (xs: NonEmptyList[Int]) =>
    val F = Foldable1[NonEmptyList]
    import Tags._
    import syntax.foldable1._
