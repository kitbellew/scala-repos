...
object DievTest extends SpecLite {
  ...
  "fromValuesSeq / toSet" ! forAll { (set: Set[Int]) =>
    Diev.fromValuesSeq(set.toSeq).toSet must_=== (set)
  }
  ...
  "++ associativity" ! forAll { (first: Diev[Int], second: Diev[Int]) =>
    first ++ second must_=== (second ++ first)
  }

  "intervals / fromIntervalsSeq" ! forAll { (original: Diev[Int]) =>
    Diev.fromIntervalsSeq(original.intervals) must_=== (original)
  }

  "-- / ++" ! forAll { (first: Diev[Int], second: Diev[Int]) =>
    first -- second ++ second must_=== (first ++ second)
  }
