...
object ValidationTest extends SpecLite {
  ...
  "ap2" should {
    "accumulate failures in order" in {
      ...
      val f = (_: Int) + (_: Int)
      Apply[ValidationNel[String, ?]].ap2(fail1, fail2)(
        Success(f)).shows must_=== ("""Failure(["1","2"])""")
    }
  }
  ...
  "apply2" should {
    "accumulate failures in order" in {
      ...
      val fail2 = Failure("2").toValidationNel
      val f = (_: Int) + (_: Int)
      Apply[ValidationNel[String, ?]].apply2(fail1, fail2)(
        f).shows must_=== ("""Failure(["1","2"])""")
    }
  }
  ...
  "ensure" in {
    import syntax.std.string._
    import syntax.validation._
    List("1", "2") map (_.parseInt.leftMap(_.toString).ensure("Fail")(
      _ >= 0)) must_=== (List(1.success[String], 2.success[String]))
    List("1", "-2") map (_.parseInt.leftMap(_.toString).ensure("Fail")(
      _ >= 0)) must_=== (List(1.success[String], "Fail".failure[Int]))
  }
