...
object FoldableTest extends SpecLite {
  ...
  "distinct" ! forAll { (xs: List[Int]) =>
    ...
    if (xs.length > 0)
      F.distinct(xs)(Order.order((_, _) => Ordering.EQ)).length must_== 1
  }

  "distinctE" ! forAll { (xs: List[Int]) =>
    xs.distinctE.toList must_== xs.distinct
    if (xs.length > 0)
      xs.distinctE(Equal.equal((_, _) => true)).length must_== 1
  }
  ...
  "non-empty folding" should {
    ...
    "foldMapM" ! forAll { (xs: List[String]) =>
      xs.foldMapM(x => Some(x): Option[String]) must_== Some(xs.mkString)
    }
    ...
    "findMapM: finding the first element performs transform and only runs only necessary effects" !
      forAll { (x: Int, xs: List[Int]) =>
        (x :: xs).findMapM[StateInt, Int](found).run(0) must_==
          (1 -> Some(x * 2))
      }

    "findMapM: finding the last element performs transform and runs all effects (once only)" !
      forAll { (x: Int, xs: List[Int]) =>
        !xs.contains(x) ==> {
          (xs ++ List(x))
    ...
    "findMapM: runs all effects but doesn't return a value for not found" !
      forAll { (xs: List[Int]) =>
    ...
    "findLeft" ! forAll { (x: Int, xs: List[Int]) =>
      (x :: xs).findLeft(_ == x) must_== Some(x)
    }

    "findRight" ! forAll { (x: Int, xs: List[Int]) =>
      (xs ++ List(x)).findRight(_ == x) must_== Some(x)
    }
  }

  private val L = Foldable[List]

  "product foldRight equivalence" ! forAll { (l: List[Int], l2: List[Int]) =>
