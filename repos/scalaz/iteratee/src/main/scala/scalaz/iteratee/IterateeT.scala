...
sealed abstract class IterateeT[E, F[_], A] {
  ...
  def zip[B](other: IterateeT[E, F, B])(implicit
    ...
    def step[Z](i: IterateeT[E, F, Z], in: Input[E]) =
      IterateeT.IterateeTMonadTrans[E]
...
trait IterateeTFunctions {
  ...
  /**
    ...
    */
  def dropWhile[E, F[_]: Applicative](
...
private trait IterateeTMonad[E, F[_]] extends Monad[IterateeT[E, F, ?]] {
  ...
  override def map[A, B](fa: IterateeT[E, F, A])(
      f: A => B): IterateeT[E, F, B] = fa map f
  def bind[A, B](fa: IterateeT[E, F, A])(
      f: A => IterateeT[E, F, B]): IterateeT[E, F, B] = fa flatMap f
}

private trait IterateeTHoist[E]
    extends Hoist[λ[(β[_], α) => IterateeT[E, β, α]]] {
  trait IterateeTF[F[_]] {
    type λ[α] = IterateeT[E, F, α]
  }

  def hoist[F[_]: Monad, G[_]](f: F ~> G) =
    new (IterateeTF[F]#λ ~> IterateeTF[G]#λ) {
  ...
  implicit def apply[G[_]: Monad]: Monad[IterateeTF[G]#λ] =
    IterateeT.IterateeTMonad[E, G]
}
