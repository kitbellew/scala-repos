...
trait EnumerateeTFunctions {
  ...
  def collect[O, I, F[_]: Monad](
      pf: PartialFunction[O, I]): EnumerateeT[O, I, F] =
    new EnumerateeT[O, I, F] {
      def apply[A] = {
        def loop = step andThen cont[O, F, StepT[I, F, A]]
        def step: (Input[I] => IterateeT[I, F, A]) => (
  ...
  def zipWithIndex[E, F[_]: Monad]: EnumerateeT[E, (E, Long), F] =
    new EnumerateeT[E, (E, Long), F] {
      def apply[A] = {
        ...
        def loop(i: Long) =
          (step(_: StepEl, i)) andThen (cont[E, F, StepT[(E, Long), F, A]])
        def step(
            k: StepEl,
  ...
  def group[E, F[_], G[_]](n: Int)(implicit
      F: Applicative[F],
      FE: Monoid[F[E]],
  ...
  def splitOn[E, F[_], G[_]](p: E => Boolean)(implicit
      F: Applicative[F],
      FE: Monoid[F[E]],
      G: Monad[G]): EnumerateeT[E, F[E], G] =
    new EnumerateeT[E, F[E], G] {
