...
abstract class EnumeratorP[E, F[_]] { self =>
  ...
  def join(other: EnumeratorP[E, F])(implicit
      order: Order[E],
...
trait EnumeratorPFunctions {
  ...
  def enumPStream[E, F[_]: Monad](xs: Stream[E]): EnumeratorP[E, F] =
    new EnumeratorP[E, F] {
      def apply[G[_]](implicit
          MO: MonadPartialOrder[G, F]): EnumeratorT[E, G] = {
        import MO._
        enumStream[E, G](xs)
      }
    }
  ...
  def liftE2[J, K, I, F[_]](e2t: ForallM[λ[β[_] => Enumeratee2T[J, K, I, β]]])
    ...
    (e1: EnumeratorP[J, F], e2: EnumeratorP[K, F]) =>
      new EnumeratorP[I, F] {
        def apply[G[_]](implicit
            MO: MonadPartialOrder[G, F]): EnumeratorT[I, G] =
          new EnumeratorT[I, G] {
            import MO._
  ...
  def cogroupE[J, K, F[_]](implicit M: Monad[F], ord: (J, K) => Ordering) =
  ...
  def joinE[J, K, F[_]](implicit M: Monad[F], ord: (J, K) => Ordering) =
    liftE2[J, K, (J, K), F] {
...
sealed abstract class EnumeratorPInstances {
  implicit def enumeratorPMonoid[E, F[_]]: Monoid[EnumeratorP[E, F]] =
    new Monoid[EnumeratorP[E, F]] {
