...
/**
  ...
  * 11/19/13
  */
class IdeaIncrementalBuilder(category: BuilderCategory)
    extends ModuleLevelBuilder(category) {
  ...
  private def collectSources(
    ...
    for {
      ...
      tempRoot <-
        project.getBuildRootIndex.getTempTargetRoots(target, context).asScala
    } {
