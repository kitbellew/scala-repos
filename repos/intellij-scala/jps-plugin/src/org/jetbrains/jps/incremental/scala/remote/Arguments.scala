...
case class Arguments(
  ...
  def asStrings: Seq[String] = {
    ...
    val compilerJarPaths = compilerData.compilerJars
    ...
    val sbtIncOptions = compilationData.sbtIncOptions.map(_.asString)
      .getOrElse("")
...
object Arguments {
  ...
  def from(strings: Seq[String]): Arguments =
    strings match {
      case Seq(
        ...
        val javaHome = javaHomePath.map { case PathToFile(file) => file }
  ...
  private def filesToPaths(files: Iterable[File]): String =
    sequenceToString(files.map(fileToPath))
