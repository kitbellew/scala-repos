...
class PackageObjectsData extends Serializable {
  ...
  def add(baseSource: File, packageObject: File): Unit =
    synchronized {
      baseSourceToPackageObjects.update(
        baseSource,
        baseSourceToPackageObjects
          .getOrElse(baseSource, HashSet.empty) + packageObject)
      packageObjectToBaseSources.update(
        packageObject,
        packageObjectToBaseSources
          .getOrElse(packageObject, HashSet.empty) + baseSource)
    }
  ...
  def save(context: CompileContext): Unit = {
    ...
    PackageObjectsData.synchronized {
      using(new ObjectOutputStream(new BufferedOutputStream(
        new FileOutputStream(file)))) { stream =>
...
object PackageObjectsData {
  ...
  def getFor(context: CompileContext): PackageObjectsData = {
    ...
    def tryToReadData(file: File) = {
      synchronized {
        try {
          using(new ObjectInputStream(new BufferedInputStream(
