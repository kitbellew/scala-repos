...
trait MockSbt {
  def addSbtAsModuleDependency(module: Module): Unit = {
    val sbtLibraries = Seq(
      ...
      "sbt").map(n =>
      new File(
        TestUtils.getIvyCachePath + s"/org.scala-sbt/$n/jars/$n-0.13.5.jar"))
    ...
    val scalaLibraryJars = Seq(
      ...
      TestUtils.getScalaReflectPath(scala210)).map(new File(_))
    val classesPath = (sbtLibraries ++ scalaLibraryJars)
