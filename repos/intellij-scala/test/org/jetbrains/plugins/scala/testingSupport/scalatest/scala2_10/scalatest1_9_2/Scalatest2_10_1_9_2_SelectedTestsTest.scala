...
class Scalatest2_10_1_9_2_SelectedTestsTest extends {
  ...
  override val featureSpecTestPath = List(
    "[root]",
    "Feature 1 Scenario: Scenario A")
  override val flatSpecTestPath = List(
    "[root]",
    "A FlatSpecTest should be able to run single test")
  override val freeSpecPathTestPath = List(
    "[root]",
    "A FreeSpecTest should be able to run single test")
  override val freeSpecTestPath = List(
    "[root]",
    "A FreeSpecTest should be able to run single tests")
  override val funSpecTestPath = List(
    "[root]",
