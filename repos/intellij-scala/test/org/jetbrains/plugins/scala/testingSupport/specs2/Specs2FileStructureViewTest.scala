...
abstract class Specs2FileStructureViewTest extends Specs2TestCase {
  ...
  def testExclamationView(): Unit =
    prepareAndRunTestInner(normalStatusId, "\"child1\"")

  def testGreaterView(): Unit =
    prepareAndRunTestInner(normalStatusId, "\"child2\"")

  def testInView(): Unit = prepareAndRunTestInner(normalStatusId, "\"child3\"")

  def testCanView(): Unit =
    prepareAndRunTestInner(normalStatusId, "\"parent2\"")
