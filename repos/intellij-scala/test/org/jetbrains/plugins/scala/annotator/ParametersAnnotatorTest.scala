...
class ParametersAnnotatorTest extends SimpleTestCase {
  ...
  def testMissingTypeAnnotation(): Unit = {
    assertMatches(messages(
      "def test(p1: String, p2 = \"default\") = p1 concat p2")) { //SCL-3799
      case Error(
            "p2 = \"default\"",
  ...
  def messages(@Language(value = "Scala", prefix = Header) code: String)
    ...
    val owner = (Header + code).parse.depthFirst.filterByType(
      classOf[ScParameterOwner]).collectFirst {
      case named: ScNamedElement if !Set("A", "B", "C").contains(named.name) =>
        named
    }.get
