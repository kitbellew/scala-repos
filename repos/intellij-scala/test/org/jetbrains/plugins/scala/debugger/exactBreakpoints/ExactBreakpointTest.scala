...
abstract class ExactBreakpointTestBase extends ScalaDebuggerTestCase {
  ...
  private def addBreakpoint(b: Breakpoint): Unit =
    addBreakpoint(b.line, mainFileName, b.ordinal)

  protected def checkVariants(lineNumber: Int, variants: String*) = {
    val xSourcePosition = XDebuggerUtil.getInstance().createPosition(
      getVirtualFile(getFileInSrc(mainFileName)),
      lineNumber)
    val foundVariants = scalaLineBreakpointType.computeVariants(
      getProject,
      xSourcePosition).asScala.map(_.getText)
    Assert.assertEquals(
        ...
        managed {
          ...
          inReadAction {
            val sourcePosition = ScalaPositionManager.instance(
              getDebugProcess).get.getSourcePosition(location)
  ...
  def testSeveralLines(): Unit = {
    ...
    checkStopResumeSeveralTimes(Breakpoint(2, null), Breakpoint(3, null))(
      "Option(\"aaa\")...",
      "_.headOption",
      ".find(...",
      "c => c.isDigit",
  ...
  def testEarlyDefAndTemplateBody(): Unit = {
    ...
    checkStopResumeSeveralTimes(Breakpoint(7, null), Breakpoint(9, null))(
      "val x = ...",
      "Seq(1)",
      "_ > 0",
      "Seq(1).map...",
  ...
  addSourceFile(
    "NewTemplateDefinitionAsLambda.scala",
