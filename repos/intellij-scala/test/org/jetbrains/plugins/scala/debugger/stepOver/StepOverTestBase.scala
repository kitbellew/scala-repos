...
abstract class StepOverTestBase extends ScalaDebuggerTestCase {
  def doStepOver(): Unit = {
    val stepOverCommand = getDebugProcess
      .createStepOverCommand(suspendContext, false)
  ...
  def testStepThrough(expectedLineNumbers: Seq[Int]): Unit = {
    ...
    Assert.assertTrue(
      "Method main should be defined on a second line",
      lines(1).trim.startsWith("def main") && lines(2).trim.nonEmpty)
    ...
    runDebugger(mainClassName) {
      while (!processTerminatedNoBreakpoints()) {
        if (expectedNumbers.hasNext)
          checkLine(expectedNumbers.next())
        else {
