...
class ReachingDefsTest extends LightCodeInsightFixtureTestCase {
  ...
  def doTest() {
    val input: java.util.List[String] = TestUtils
      .readInput(getBasePath + getTestName(true) + ".test")
    ...
    val start: PsiElement = file
      .findElementAt(if (model.hasSelection) model.getSelectionStart else 0)
