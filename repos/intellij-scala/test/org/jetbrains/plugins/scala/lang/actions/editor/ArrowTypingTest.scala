...
class ArrowTypingTest extends ScalaLightCodeInsightFixtureTestAdapter {
  ...
  private def convertLoadedString(str: String) =
    str.replace("$CARET_MARKER", CARET_MARKER).replace(
  ...
  def testReplaceLambdaArrow() {
    ...
    val before1 = convertLoadedString(FileUtil.loadFile(new File(
      getTestDataPath + s"/${getTestName(true)}Before.test")))
    val after1 = convertLoadedString(FileUtil.loadFile(new File(
      getTestDataPath + s"/${getTestName(true)}After.test")))
