...
class HoconHighlightUsagesTest extends LightPlatformCodeInsightFixtureTestCase {
  ...
  private def testUsages(expectedHighlights: (Int, Int, Int)*): Unit = {
    ...
    val actualHighlights = myFixture.testHighlightUsages(testName + ".conf")
      .toSeq.map { rh =>
        val logicalStart = myFixture.getEditor
  ...
  def testNested() = testUsages((4, 2, 3), (7, 4, 3), (10, 16, 3))

  def testInArrayElement() = testUsages((6, 3, 3), (6, 13, 3))

  def testDifferentTexts() =
    testUsages((0, 0, 3), (1, 0, 5), (2, 0, 5), (3, 0, 9), (4, 10, 7))
