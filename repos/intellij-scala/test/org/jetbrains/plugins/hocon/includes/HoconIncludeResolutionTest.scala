...
trait HoconIncludeResolutionTest {
  ...
  protected def findHoconFile(path: String): PsiFile =
    PsiManager.getInstance(project).findFile(findFile(path)).asOptionOf[
      HoconPsiFile]
  ...
  protected def checkFile(path: String): Unit = {
    ...
    psiFile.depthFirst.foreach {
      case it: HIncludeTarget =>
        val prevComments = it.parent.map(
          _.parent.map(_.nonWhitespaceChildren).getOrElse(
            Iterator.empty)).getOrElse(Iterator.empty)
          .takeWhile(e =>
        ...
        if (prevComments.nonEmpty) {
          ...
          val expectedFiles =
            prevComments.map(_.getText.stripPrefix("#")).mkString(",")
              .split(',').iterator.map(_.trim).filter(_.nonEmpty).map(
                findFile).toSet
