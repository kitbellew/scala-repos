...
object DecompilerUtil {
  protected val LOG: Logger = Logger
    .getInstance("#org.jetbrains.plugins.scala.decompiler.DecompilerUtil")
  ...
  private def openedNotDisposedProjects: Array[Project] = {
    ...
    if (ApplicationManager.getApplication.isUnitTestMode) {
      val testProject = manager.asInstanceOf[ProjectManagerEx].getOpenProjects
        .find(!_.isDisposed).orNull
  ...
  def decompile(
    ...
    if (res == null || res.timeStamp != timeStamp) {
      ...
      def updateAttributeAndData() {
        ...
        if (attributesSupported) {
          val writeAttribute = SCALA_DECOMPILER_FILE_ATTRIBUTE
            .writeAttribute(file)
          try {
