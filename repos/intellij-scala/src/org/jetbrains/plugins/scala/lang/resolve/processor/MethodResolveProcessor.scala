...
class MethodResolveProcessor(
  ...
  override def execute(element: PsiElement, state: ResolveState): Boolean = {
    ...
    def implicitConversionClass: Option[PsiClass] =
      state.get(ScImplicitlyConvertible.IMPLICIT_RESOLUTION_KEY).toOption
    def implFunction: Option[PsiNamedElement] =
      state.get(CachesUtil.IMPLICIT_FUNCTION).toOption
    def implType: Option[ScType] = state.get(CachesUtil.IMPLICIT_TYPE).toOption
    def isNamedParameter: Boolean =
      state.get(CachesUtil.NAMED_PARAM_KEY).toOption.exists(_.booleanValue)
    def fromType: Option[ScType] =
      state.get(BaseProcessor.FROM_TYPE_KEY).toOption
    def unresolvedTypeParameters: Option[Seq[TypeParameter]] =
      state.get(BaseProcessor.UNRESOLVED_TYPE_PARAMETERS_KEY).toOption
  ...
  private def collectCandidates(
    ...
    if (!isShapeResolve && enableTupling && argumentClauses.nonEmpty) {
      ...
      if (!isDynamic && (cand1.isEmpty || cand1.forall(_.tuplingUsed))) {
        ...
        val tpl = ScalaPsiUtil.tuplizy(
          argumentClauses.head,
          ref.getResolveScope,
          ref.getManager,
          ScalaPsiUtil.firstLeaf(ref))
        if (tpl.isEmpty) {
...
object MethodResolveProcessor {
  private def problemsFor(
    ...
    val elementForUndefining = element match {
      case m: PsiMethod if m.isConstructor && !selfConstructorResolve =>
        realResolveResult.getActualElement
    ...
    def addExpectedTypeProblems(
      ...
      for (expected <- eOption) {
        val retType: ScType = element match {
          ...
          case m: PsiMethod => Option(m.getReturnType).map { rt =>
              substitutor
                .subst(ScType.create(rt, ref.getProject, getResolveScope))
            }.getOrElse(Nothing)
          case _ => Nothing
        }
    ...
    def checkFunction(fun: PsiNamedElement): ConformanceExtResult = {
      ...
      def processFunctionType(
          retType: ScType,
          params: Seq[ScType]): ConformanceExtResult = {
        ...
        val result = Compatibility.compatible(
          ...
          scope = ref.getResolveScope,
          isShapesResolve = isShapeResolve)
      ...
      expectedOption().map(_.removeAbstracts) match {
        case Some(ScFunctionType(retType, params)) =>
          processFunctionType(retType, params)
    ...
    def constructorCompatibility(
      ...
      val classTypeParameters: Seq[ScTypeParam] = constr.getClassTypeParameters
        .map(_.typeParameters).getOrElse(Seq())
      if (typeArgElements.isEmpty || typeArgElements
            .length == classTypeParameters.length) {
        val result = Compatibility.compatible(
          constr,
          substitutor,
          argumentClauses,
    ...
    def javaConstructorCompatibility(
      ...
      if (typeArgElements.isEmpty || typeArgElements
            .length == classTypeParmeters.length) {
        val result = Compatibility.compatible(
          constr,
          substitutor,
          argumentClauses,
    ...
    val result = element match {
      ...
      case c: ScPrimaryConstructor with PsiNamedElement =>
        constructorCompatibility(c)
      case method: PsiMethod if method.isConstructor =>
        javaConstructorCompatibility(method)
      case fun: ScFunction
          if (typeArgElements.isEmpty ||
            typeArgElements.length == fun.typeParameters.length) && fun
            .paramClauses.clauses.length == 1 &&
      ...
      case fun: ScTypeParametersOwner
          if (typeArgElements.isEmpty ||
            typeArgElements.length == fun.typeParameters
              .length) && argumentClauses.isEmpty &&
      ...
      case fun: PsiTypeParameterListOwner
          if (typeArgElements.isEmpty ||
            typeArgElements.length == fun.getTypeParameters
              .length) && argumentClauses.isEmpty &&
            fun.isInstanceOf[PsiNamedElement] =>
        checkFunction(fun.asInstanceOf[PsiNamedElement])
      ...
      case tp: ScTypeParametersOwner with PsiNamedElement =>
        ...
        if (typeArgCount > 0 && typeArgCount != typeParamCount) {
          if (typeParamCount == 0) { problems += DoesNotTakeTypeParameters }
            ...
            problems ++= typeArgElements.drop(typeParamCount)
              .map(ExcessTypeArgument)
          } else {
            problems ++= tp.typeParameters.drop(typeArgCount)
      ...
      case tp: PsiTypeParameterListOwner with PsiNamedElement =>
        ...
        if (typeArgCount > 0 && typeArgCount != typeParamCount) {
          if (typeParamCount == 0) { problems += DoesNotTakeTypeParameters }
            ...
            problems ++= tp.getTypeParameters.drop(typeArgCount)
              .map(ptp => MissedTypeParameter(new TypeParameter(ptp)))
          }
          addExpectedTypeProblems()
          new ConformanceExtResult(problems)
  ...
  def undefinedSubstitutor(
    ...
    (constructorTypeParameters, element) match {
      case (Some(typeParameterClause), _) =>
        ...
        s.followed(
          if (typeArgElements.nonEmpty && typeParameters
                .length == typeArgElements.length) {
      ...
      case (None, t: ScTypeParametersOwner) =>
        s.followed(
          if (typeArgElements.nonEmpty && t.typeParameters
                .length == typeArgElements.length) {
            ScalaPsiUtil.genericCallSubstitutor(
              t.typeParameters
                .map(p => (p.name, ScalaPsiUtil.getPsiElementId(p))),
      ...
      case (None, p: PsiTypeParameterListOwner) =>
        s.followed(
          if (typeArgElements.nonEmpty && p.getTypeParameters
                .length == typeArgElements.length) {
            ScalaPsiUtil.genericCallSubstitutor(
              p.getTypeParameters
                .map(p => (p.name, ScalaPsiUtil.getPsiElementId(p))),
  ...
  def candidates(
      ...
      _input: Set[ScalaResolveResult]): Set[ScalaResolveResult] = {
    import proc._
                  ...
                  _input.forall {
                    case r2: ScalaResolveResult => r2.element match {
                        ...
                        case b2: ScTypedDefinition => b2.nameContext match {
                            case m2: ScMember =>
                              val clazz2: ScTemplateDefinition = m2
                                .containingClass
                              if (clazz2 == null) true
                              else {
    ...
    def expand(r: ScalaResolveResult): Set[ScalaResolveResult] = {
      ...
      r.element match {
        ...
        case b: ScTypedDefinition if argumentClauses.nonEmpty =>
          val tp = r.substitutor.subst(
            b.getType(TypingContext.empty).getOrElse(return HashSet.empty))
          applyMethodsFor(tp)
    ...
    var filtered = mapped
      .filter(_.isApplicableInternal(withExpectedType = true))
    if (filtered.isEmpty)
    ...
    if (filtered.isEmpty && !noImplicitsForArgs) {
      ...
      if (filtered.isEmpty)
        filtered = mapped
          .filter(_.isApplicableInternal(withExpectedType = false))
    }
