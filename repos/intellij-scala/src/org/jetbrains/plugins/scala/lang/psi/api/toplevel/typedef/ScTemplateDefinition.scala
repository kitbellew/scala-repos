...
trait ScTemplateDefinition extends ScNamedElement with PsiClass {
  ...
  def extendsBlock: ScExtendsBlock = {
    this match {
      case st: ScalaStubBasedElementImpl[_] =>
        val stub = st.getStub
        if (stub != null) {
          return stub.findChildStubByType(ScalaElementTypes.EXTENDS_BLOCK)
            .getPsi
        }
      case _ =>
    }
  ...
  override def findMethodsAndTheirSubstitutorsByName(
    ...
    PsiClassImplUtil
      .findMethodsAndTheirSubstitutorsByName(this, name, checkBases).toList
      .sortBy(myPair =>
        myPair.first match {
      ...
      case Some(selfType) =>
        ...
        Bounds.glb(selfType, clazzType) match {
          case c: ScCompoundType =>
            TypeDefinitionMembers.getTypes(c, Some(clazzType), this)
              .allFirstSeq()
    ...
    selfType match {
      case Some(selfType) =>
        ...
        Bounds.glb(selfType, clazzType) match {
          case c: ScCompoundType =>
            TypeDefinitionMembers.getSignatures(c, Some(clazzType), this)
              .allFirstSeq().flatMap(_.map { case (_, n) => n.info })
  ...
  def addMember(member: ScMember, anchor: Option[PsiElement]): ScMember = {
    extendsBlock.templateBody match {
      ...
      case None =>
        ...
        eBlockNode.addChild(
          ScalaPsiElementFactory.createBodyFromMember(member, member.getManager)
            .getNode)
        return members(0)
    }
    member
  ...
  def functionsByName(name: String): Seq[PsiMethod] = {
    (for ((p: PhysicalSignature, _) <- TypeDefinitionMembers.getSignatures(this)
            .forName(name)._1) yield p.method)
