...
/**
  ...
  *         Date: 08.07.13
  */
class ScalaRearranger
    extends Rearranger[ScalaArrangementEntry]
    with ArrangementStandardSettingsAware {
  ...
  private def setupJavaGettersAndSetters(info: ScalaArrangementParseInfo) =
    setupGettersAndSetters(info.javaProperties)

  private def setupScalaGettersAndSetters(info: ScalaArrangementParseInfo) =
...
object ScalaRearranger {
  ...
  private def addCondition(
    ...
    if (conditions.length == 1) {
      ArrangementSectionRule
        .create(new StdArrangementMatchRule(new StdArrangementEntryMatcher(
