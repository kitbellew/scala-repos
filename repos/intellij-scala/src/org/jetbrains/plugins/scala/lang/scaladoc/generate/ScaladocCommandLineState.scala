...
class ScaladocCommandLineState(env: ExecutionEnvironment, project: Project)
  ...
  private def processAdditionalParams(params: String) = {
    ...
    paramTokens.foldLeft(false) {
      ...
      case (_, param: String)
        ...
        true
      case (_, param: String) =>
        if (!ScaladocCommandLineState.generatedParamsWithoutArgs
              .contains(param)) result += param
        false
    }
  ...
  def createJavaParameters() = {
    ...
    val sourcePath = OrderEnumerator.orderEntries(project).withoutLibraries()
      .withoutSdk().getAllSourceRoots
