  ...
  def collect(fullInfo: Boolean = false): Seq[ScalaResolveResult] = {
    def calc(): Seq[ScalaResolveResult] = {
      ...
      val candidates: Seq[ScalaResolveResult] =
        if (fullInfo)
          Seq.empty //will be collected together with second part
        else
          processor.candidatesS(fullInfo).toSeq
                            ...
                            if (isExtensionConversion) {
                              tp match {
                                case ScTypePolymorphicType(
                                      internalType,
                                      typeParams) =>
                                  val filteredTypeParams = typeParams
                                    .filter(tp =>
                                      !tp
                                        .lowerType()
