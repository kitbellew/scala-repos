...
object ImplicitCollector {
  val cache = ContainerUtil.newConcurrentMap[
    (PsiElement, ScType),
...
class ImplicitCollector(
  ...
  def collect(fullInfo: Boolean = false): Seq[ScalaResolveResult] = {
    def calc(): Seq[ScalaResolveResult] = {
      ScType.extractClass(tp, Some(place.getProject)) match {
        case Some(clazz)
            if InferUtil.skipQualSet.contains(clazz.qualifiedName) =>
          return Seq.empty
        case _ =>
      }
      ...
      var stop = false
      while (!stop) {
        if (placeForTreeWalkUp == null || !placeForTreeWalkUp.processDeclarations(
              processor,
        ...
        if (!stop) {
          if (!placeCalculated) {
            ...
            if (predicate.isEmpty)
              result = ImplicitCollector.cache.get((place, tp))
            if (result != null && !fullInfo) return result
          }
      ...
      if (candidates.nonEmpty && !candidates.forall(!_.isApplicable()))
        return candidates
  ...
  class ImplicitParametersProcessor(withoutPrecedence: Boolean)
    ...
    def execute(element: PsiElement, state: ResolveState): Boolean = {
      ...
      named match {
        ...
        case param: ScParameter if param.isImplicitParameter =>
          ...
          param match {
            case c: ScClassParameter =>
              if (!isPredefPriority && !ResolveUtils.isAccessible(c, getPlace))
                return true
            case _ =>
          }
          addResult(
        ...
        case function: ScFunction if function.hasModifierProperty("implicit") =>
          placeCalculated = true
          if (isPredefPriority || (ScImplicitlyConvertible.checkFucntionIsEligible(
                function,
                place) &&
    ...
    def candidatesS(fullInfo: Boolean): collection.Set[ScalaResolveResult] = {
                  ...
                  def checkType(ret: ScType)
                    ...
                    def compute()
                      ...
                      if (typeParameters.isEmpty && lastImplicit.isEmpty)
                        ...
                        val methodType = lastImplicit.map(li =>
                          subst.subst(
                            ScMethodType(
                              ret,
                              ...
                              isImplicit = true)(
                              place.getProject,
                              place.getResolveScope))).getOrElse(ret)
                              ...
                              case ScTypePolymorphicType(
                                ...
                                val filteredTypeParams =
                                  typeParams.filter(tp =>
                                    !tp.lowerType().equiv(
                                      types.Nothing) || !tp.upperType().equiv(
                                      types.Any))
                        ...
                        (tp: Object, searches: Seq[Object]) => {
                          !searches.exists {
                            case t: ScType if tp.isInstanceOf[ScType] =>
                              if (Equivalence.equivInner(
                                    t,
                                    tp.asInstanceOf[ScType],
                                    new ScUndefinedSubstitutor(),
                                    falseUndef = false)._1) true
      ...
      def mapCandidates(withLocalTypeInference: Boolean)
        ...
        while (candidatesSeq.nonEmpty) {
          val (next, rest) =
            mostSpecific.nextLayerSpecificForImplicitParameters(
              lastResult,
              candidatesSeq)
          next match {
            case Some(c) =>
        ...
        return (candidates.toSeq.map(c =>
          ...
          candidates.toSeq.map(c =>
            forMap(
              ...
              checkFast = false))).flatMap(_.toSeq).map(_._1).toSet

      var applicable = mapCandidates(withLocalTypeInference = false)
      if (applicable.isEmpty)
        applicable = mapCandidates(withLocalTypeInference = true)
  ...
  private def dominates(t: ScType, u: ScType): Boolean = {
    complexity(t) > complexity(u) && topLevelTypeConstructors(t).intersect(
      topLevelTypeConstructors(u)).nonEmpty
  }
