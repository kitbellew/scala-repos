...
import org.jetbrains.plugins.scala.lang.resolve.processor.{
  CompletionProcessor,
  ExpandedExtractorResolveProcessor
}
...
trait ScPattern extends ScalaPsiElement {
  ...
  def getType(ctx: TypingContext): TypeResult[ScType] =
    Failure("Cannot type pattern", Some(this))
      ...
      case Some(ScalaResolveResult(fun: ScFunction, substitutor: ScSubstitutor))
        ...
        val subst =
          if (fun.typeParameters.isEmpty)
            ...
            expected match {
              case Some(tp) =>
                calculateSubstitutor(tp, funType, substitutor)
              case _ =>
                substitutor
            }
          }
          ...
          case Success(rt, _) =>
            def updateRes(tp: ScType): ScType = {
              ...
              tp.recursiveVarianceUpdate {
                case (tp: ScTypeParameterType, variance)
                    if parameters.contains(tp.param) =>
                  (
                    true,
                    if (variance == -1)
                      substitutor.subst(tp.lower.v)
                    else
                      substitutor.subst(tp.upper.v))
                case (typez, _) =>
                  (false, typez)
              }
            }
            val subbedRetTp: ScType = subst.subst(rt)
            if (subbedRetTp.equiv(lang.psi.types.Boolean))
              None
            else {
              ...
              if (totalNumberOfPatterns == 1 && args.length > 1)
                ...
                Some(updateRes(subst.subst(args(argIndex)).unpackedType))
              else
                None
            }
          case _ =>
  ...
  def expectedType: Option[ScType] =
    getContext match {
      case list: ScPatternList =>
        list.getContext match {
          case _var: ScVariable =>
            _var.getType(TypingContext.empty).toOption
          case _val: ScValue =>
            _val.getType(TypingContext.empty).toOption
        }
      ...
      case composite: ScCompositePattern =>
        composite.expectedType
      case infix: ScInfixPattern =>
        val i =
          if (infix.leftPattern == this)
            0
          ...
          case _: ScXmlPattern =>
            ...
            nodeClass.flatMap { nodeClass =>
              this match {
                ...
                case _ =>
                  Some(ScDesignatorType(nodeClass))
              }
            }
          case _ =>
            None
        ...
        clause.getContext /*clauses*/ .getContext match {
          case matchStat: ScMatchStmt =>
            matchStat.expr match {
              case Some(e) =>
                Some(e.getType(TypingContext.empty).getOrAny)
              case _ =>
                None
            }
            ...
            b.expectedType(fromUnderscore = false) match {
              case Some(et) =>
                et.removeAbstracts match {
                  case ScFunctionType(_, Seq()) =>
                    Some(types.Unit)
                  case ScFunctionType(_, Seq(p0)) =>
                    Some(p0)
                  case ScFunctionType(_, params) =>
                    val tt = ScTupleType(params)(getProject, getResolveScope)
                    Some(tt)
                  case ScPartialFunctionType(_, param) =>
                    Some(param)
                  case _ =>
                    None
                }
...
object ScPattern {
  ...
  private def extractPossibleProductParts(
      receiverType: ScType,
      place: PsiElement,
      isOneArgCaseClass: Boolean): Seq[ScType] = {
