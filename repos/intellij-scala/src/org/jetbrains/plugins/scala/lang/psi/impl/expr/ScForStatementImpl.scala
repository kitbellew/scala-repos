  ...
  def getDesugarizedExprText(forDisplay: Boolean): Option[String] = {
    ...
    if (guards.isEmpty && enums.isEmpty && gens.length == 1) {
      ...
      exprText.append("(").append(gen.rvalue.getText).append(")").append(".")
        .append(if (isYield) "map" else "foreach").append(" { case ")
      ...
      next match {
        ...
        case guard: ScGuard =>
          ...
          exprText.append(gen.pattern.getText).append(" <- ((")
            .append(gen.rvalue.getText).append(s").$filterText { case ").append(
        ...
        case gen2: ScGenerator =>
          exprText.append("(").append(gen.rvalue.getText).append(")")
            .append(".").append(if (isYield) "flatMap " else "foreach ")
            .append("{ case ")
          gen.pattern.desugarizedPatternIndex = exprText.length
          exprText.append(gen.pattern.getText).append(s" $arrow ")
            .append("for {")
        ...
        case enum: ScEnumerator =>
          ...
          exprText.append(") <- (for (").append(freshName1).append("@(")
            .append(gen.pattern.getText).append(") <- ")
            .append(gen.rvalue.getText).append(") yield {val ")
            .append(freshName2).append("@(").append(enum.pattern.getText)
            .append(") = ").append(enum.rvalue.getText).append("; (")
            .append(freshName2).append(", ").append(freshName1).append(")})")
          next = nextEnumerator(next)
          if (next != null) exprText.append(" ; ")
          while (next != null) {
            next match {
