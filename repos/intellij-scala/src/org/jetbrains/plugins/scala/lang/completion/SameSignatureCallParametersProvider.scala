...
class SameSignatureCallParametersProvider extends ScalaCompletionContributor {
  val constructorFilter = PlatformPatterns.psiElement()
  ...
  val superCallFilter = PlatformPatterns.psiElement()
    .withParent(classOf[ScReferenceExpression])
    .withSuperParent(2, classOf[ScArgumentExprList])
    ...
    PsiTreeUtil
      ...
      case c: ScClass =>
        ...
        typeElement.getType(TypingContext.empty) match {
          case Success(tp, _) =>
            val signatures = ScType
              .extractClassType(tp, Some(position.getProject)) match {
  ...
  private def checkSignatures(
      signatures: Seq[Seq[(String, ScType)]],
    ...
    for (signature <- signatures if signature.forall(_._1 != null)) {
      ...
      if (!names.contains("")) {
        ...
        val element = LookupElementBuilder.create(res).withIcon(icon)
