...
class ScalaPsiBuilderImpl(builder: PsiBuilder)
  ...
  private def countNewlineBeforeCurrentToken(): Int = {
    ...
    var i = 1
    while (i < getCurrentOffset && TokenSets.WHITESPACE_OR_COMMENT_SET
             .contains(rawLookup(-i))) i += 1
    val textBefore = getOriginalText
      .subSequence(rawTokenTypeStart(-i + 1), rawTokenTypeStart(0)).toString
  ...
  def disableNewlines { newlinesEnabled.push(false) }

  def enableNewlines { newlinesEnabled.push(true) }
