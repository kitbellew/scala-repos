...
case class ScExistentialType(
  ...
  private def boundNamesInner: List[String] =
    wildcards.map {
      _.name
    }
  ...
  private def updateRecursive(
    ...
    tp match {
      ...
      case ScParameterizedType(designator, typeArgs) =>
        ...
        ScParameterizedType(
          updateRecursive(designator, rejected, variance),
          newTypeArgs)
      case ex @ ScExistentialType(_quantified, _wildcards) =>
        var newSet =
          if (ex ne this)
  ...
  def simplify(): ScType = {
    ...
    if (updated) {
      res match {
        case ex: ScExistentialType if ex != this =>
          ex.simplify()
        case _ =>
          res
      }
...
case class ScExistentialArgument(
    name: String,
    args: List[ScTypeParameterType],
    lowerBound: ScType,
    upperBound: ScType) {
