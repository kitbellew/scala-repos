...
object NameSuggester {
  ...
  private def namesByType(
      ...
      shortVersion: Boolean = true)(implicit
      validator: NameValidator): ArrayBuffer[String] = {
      ...
      def inner(classOfBaseType: PsiClass) {
        ...
        classOfBaseType match {
          case c if c.qualifiedName == arrayClassName && args.nonEmpty =>
            addPlurals(args(0))
          case c
              if needPrefix.keySet.contains(c.qualifiedName) && args.nonEmpty =>
            for {
