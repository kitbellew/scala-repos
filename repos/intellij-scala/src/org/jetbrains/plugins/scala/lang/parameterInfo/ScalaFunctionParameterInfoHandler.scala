        ...
        def applyToParameters(
          ...
          if (parameters.nonEmpty) {
            ...
            while (k < parameters.length) {
              ...
              if (k == index || (k == parameters
                    .length - 1 && index >= parameters.length &&
        ...
        p match {
            ...
            sign.method match {
              case method: ScFunction =>
                ...
                if (clauses.length <= i || (i == -1 && clauses.isEmpty))
                  ...
                  val clause: ScParameterClause =
                    if (i >= 0) clauses(i) else clauses.head
                  val length = clause.effectiveParameters.length
                  val parameters: Seq[ScParameter] =
                    if (i != -1) clause.effectiveParameters
                    else clause.effectiveParameters.take(length - 1)
                  applyToParameters(
                    parameters.map(param =>
                      (new Parameter(param), paramText(param, subst))),
                    subst,
                    canBeNaming = true,
                    isImplicit = clause.isImplicit)
                }
              case method: FakePsiMethod =>
                if (method.params.length == 0)
                  ...
                  buffer.append(
                    method.params.map((param: Parameter) => {
                      val buffer: StringBuilder = new StringBuilder("")
                      ...
                      val isBold =
                        if (method.params.indexOf(param) == index || (param
                              .isRepeated && method.params
              ...
              case method: PsiMethod =>
                ...
                if (p.getParameters.isEmpty)
                  ...
                  buffer.append(
                    p.getParameters.map((param: PsiParameter) => {
                      val buffer: StringBuilder = new StringBuilder("")
                      ...
                      val isBold =
                        if (p.getParameters.indexOf(param) == index || (param
                              .isVarArgs && p.getParameters
                              .indexOf(param) <= index)) true
                        else {
                          //todo: check type
                          false
          ...
          case (constructor: ScPrimaryConstructor, subst: ScSubstitutor, i: Int)
            ...
            if (clauses.length <= i)
              ...
              applyToParameters(
                clause.effectiveParameters.map(param =>
                  (new Parameter(param), paramText(param, subst))),
                subst,
                canBeNaming = true,
                isImplicit = clause.isImplicit)
                    ...
                    ref.bind() match {
                      case Some(
                            ScalaResolveResult(
                              function: ScFunction,
                              subst: ScSubstitutor))
                          if function.effectiveParameterClauses
                            .length >= count =>
                        res += (
                          (
                            new PhysicalSignature(
                      ...
                      case Some(constr: ScPrimaryConstructor)
                        ...
                        typeElement match {
                          case gen: ScParameterizedTypeElement =>
                            val tp = clazz.typeParameters.map(p =>
                              (p.name, ScalaPsiUtil.getPsiElementId(p)))
                            val typeArgs: Seq[ScTypeElement] = gen.typeArgList
                              .typeArgs
                    ...
                    val resulting: (AnnotationParameters, Int) = (
                      AnnotationParameters(
                        clazz.getMethods.toSeq
                          .filter(_.isInstanceOf[PsiAnnotationMethod]).map(
                            meth =>
                              (
                                meth.name,
                                ScType.create(
                                ...
                                meth.asInstanceOf[PsiAnnotationMethod]
                                  .getDefaultValue))),
                      i)
                    res += resulting
                  case clazz: PsiClass
                    ...
                    for (constructor <- clazz.getConstructors) {
                      typeElement match {
                        case gen: ScParameterizedTypeElement =>
                          val tp = clazz.getTypeParameters
                            .map(p => (p.name, ScalaPsiUtil.getPsiElementId(p)))
                          val typeArgs: Seq[ScTypeElement] = gen.typeArgList
                            .typeArgs
                          val map = new collection.mutable.HashMap[
              ...
              case clazz: ScClass =>
                ...
                for {
                  ...
                  if (!PsiTreeUtil.isAncestor(constr, self, true) &&
                      constr.getTextRange.getStartOffset < self.getTextRange
                        .getStartOffset) {
                    res += (
                      (
...
object ScalaFunctionParameterInfoHandler {
  case class AnnotationParameters(
      seq: Seq[(String, ScType, PsiAnnotationMemberValue)])
}
