                    ...
                    method.params.map((param: Parameter) => {
                      ...
                      val isBold =
                        if (method.params.indexOf(param) == index ||
                            (param.isRepeated &&
                            method.params.indexOf(param) <= index)) true
                        else {
                          //todo: check type
                          false
                        }
                    ...
                    p.getParameters.map((param: PsiParameter) => {
                      ...
                      val isBold =
                        if (p.getParameters.indexOf(param) == index ||
                            (param.isVarArgs &&
                            p.getParameters.indexOf(param) <= index)) true
                        else {
                          //todo: check type
                          false
                        }
              ...
              val canBeUpdate = call.getParent match {
                ...
                case notExpr
                    if !notExpr.isInstanceOf[ScExpression] ||
                      notExpr.isInstanceOf[ScBlockExpr] => true
                case _                                  => false
              }
            ...
            clazz match {
              case clazz: ScClass =>
                ...
                for {
                  ...
                  if !constr.isInstanceOf[ScPrimaryConstructor] &&
                    constr.isConstructor && constr.clauses.map(_.clauses.length)
                      .getOrElse(1) > i
                } {
