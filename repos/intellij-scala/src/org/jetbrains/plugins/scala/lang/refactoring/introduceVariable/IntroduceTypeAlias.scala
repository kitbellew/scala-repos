        ...
        def handleScope(scopeItem: SimpleScopeItem, needReplacement: Boolean) {
          ...
          CommandProcessor
            ...
            .executeCommand(
              ...
              new Runnable {
                def run() {
                  val computable = ApplicationManager
                    .getApplication
                  ...
                  if (mtypeElement != null && mtypeElement.isValid) {
                    editor
                      .getCaretModel
                      .moveToOffset(mtypeElement.getTextOffset)
...
//          editor.getUserData(IntroduceTypeAlias.REVERT_TYPE_ALIAS_INFO).clearData()
  ...
  private def runRefactoringForTypeInside(
    ...
    val typeElementIdx = occurrences
      .getUsualOccurrences
  ...
  protected def getDialogForTypes(
    ...
    val dialog =
      new ScalaIntroduceTypeAliasDialog(
        project,
        typeElement,
        possibleScopes,
        mainScope,
...
object IntroduceTypeAlias {
  val REVERT_TYPE_ALIAS_INFO: Key[IntroduceTypeAliasData] =
    new Key("RevertTypeAliasInfo")
}
