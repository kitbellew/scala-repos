...
final class DfaEngine[E](
  ...
  def performDFA: collection.mutable.Map[Instruction, E] = {
    ...
    val workList: java.util.Set[Instruction] =
      new java.util.HashSet[Instruction](java.util.Arrays.asList(cfg.toArray: _*))
    while (!workList.isEmpty) {
      ...
      if (!l.eq(newAfter, after(v))) {
        ...
        workList addAll java.util.Arrays.asList(
          (if (forward) v.succ().toArray else v.pred().toArray): _*)
      }
    }
    after
  }
