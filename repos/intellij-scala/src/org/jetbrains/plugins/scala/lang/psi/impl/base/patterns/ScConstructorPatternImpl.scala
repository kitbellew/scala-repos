    ...
    ref.bind() match {
      case Some(ScalaResolveResult(clazz: ScClass, _)) if clazz.isCase =>
        ScType.extractClassType(t.get, Some(clazz.getProject)) match {
          case Some((clazz2: ScClass, substitutor: ScSubstitutor))
              if clazz2 == clazz =>
              ...
              case Some(constr: ScPrimaryConstructor) =>
                ...
                if (clauses.isEmpty) subpatterns.isEmpty
                  ...
                  while (i < subpatterns.length) {
                    val tp = {
                      substitutor
                        .subst(params(i).getType(TypingContext.empty).getOrElse(
                          return false))
                    }
          ...
          case fun: ScFunction /*It's unapply method*/
            ...
            val subst =
              if (fun.typeParameters.isEmpty) substitutor
                ...
                val undefSubst: ScSubstitutor = fun.typeParameters
            ...
            fun.paramClauses.clauses.apply(0).parameters.apply(0)
              .getType(TypingContext.empty).map(subst.subst)
