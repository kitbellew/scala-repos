...
object ScalaSpacingProcessor extends ScalaTokenTypes {
  ...
  private def spacesToPreventNewIds(
    ...
    val concatString =
      if (textRange.contains(rightNode.getTextRange) && textRange
        ...
        leftNode.getTextRange.substring(fileText) + rightNode
          .getTextRange
          .substring(fileText)
      } else
        return 0
    (
      ...
      case (ScalaElementTypes.INTERPOLATED_STRING_LITERAL, _) =>
        0
      case (_, ScalaElementTypes.INTERPOLATED_STRING_LITERAL) =>
        0
      case _ =>
        if (ScalaNamesUtil.isIdentifier(concatString) || ScalaNamesUtil
              .isKeyword(concatString))
          1
        else
          0
    }
  ...
  def getSpacing(left: ScalaBlock, right: ScalaBlock): Spacing = {
    ...
    val scalaSettings: ScalaCodeStyleSettings = left
      .getSettings
    ...
    def getDependentLFSpacing(x: Int, y: Int, range: TextRange) = {
      if (keepLineBreaks)
        ...
        Spacing.createDependentLFSpacing(y, y, range, false, 0)
    }
    ...
    if ((
          leftPsi.isInstanceOf[PsiComment] || leftPsi
    ...
    def docCommentOf(node: ASTNode) =
      node
        .getPsi
        .parentsInFile
        .findByType(classOf[ScDocComment])
        .getOrElse {
    ...
    (
      ...
      case (ScalaDocTokenType.DOC_COMMENT_START, _, _, _) =>
        return if (docCommentOf(leftNode).version == 1) {
          ...
          if (getText(rightNode, fileText)(0) != ' ')
            WITH_SPACING
          else
            WITHOUT_SPACING
        }
      ...
      case (ScalaDocTokenType.DOC_COMMENT_LEADING_ASTERISKS, _, _, _) =>
        return if (getText(rightNode, fileText).apply(0) == ' ')
          WITHOUT_SPACING
        else
          WITH_SPACING
      ...
      case (x, _, _, _) if ScalaDocTokenType.ALL_SCALADOC_TOKENS.contains(x) =>
        return Spacing.getReadOnlySpacing
      case (ScalaTokenTypes.tLINE_COMMENT, _, _, _) =>
        return ON_NEW_LINE
      case _ =>
    }
      ...
      case (el1, el2, _, _)
          if scalaSettings.KEEP_XML_FORMATTING &&
            (
              ScalaXmlTokenTypes
                .XML_ELEMENTS
                .contains(el1) || ScalaXmlTokenTypes.XML_ELEMENTS.contains(el2)
            ) =>
        return Spacing.getReadOnlySpacing
    ...
    if (leftElementType == tLPARENTHESIS &&
      ...
      if (settings.PARENTHESES_EXPRESSION_LPAREN_WRAP) {
        if (settings.SPACE_WITHIN_PARENTHESES)
          ...
          return WITHOUT_SPACING_DEPENDENT(leftPsi.getParent.getTextRange)
      } else if (settings.SPACE_WITHIN_PARENTHESES)
        return WITH_SPACING
      else
        return WITHOUT_SPACING
    }
    ...
    if (Set(tINTERPOLATED_STRING, tINTERPOLATED_MULTILINE_STRING)
      ...
      return if (leftString == MultilineStringUtil
                   .getMarginChar(leftPsi)
                   .toString)
        Spacing.getReadOnlySpacing
      else
        WITHOUT_SPACING
    ...
    if (rightElementType == tRPARENTHESIS &&
      ...
      if (settings.PARENTHESES_EXPRESSION_RPAREN_WRAP) {
        if (settings.SPACE_WITHIN_PARENTHESES)
          ...
          return WITHOUT_SPACING_DEPENDENT(rightPsi.getParent.getTextRange)
      } else if (settings.SPACE_WITHIN_PARENTHESES)
        return WITH_SPACING
      else
        return WITHOUT_SPACING
    ...
    if (leftElementType == tLPARENTHESIS && leftPsi
          .getParent
      ...
      if (settings.METHOD_PARAMETERS_LPAREN_ON_NEXT_LINE) {
        if (settings.SPACE_WITHIN_METHOD_PARENTHESES)
          ...
          return WITHOUT_SPACING_DEPENDENT(leftPsi.getParent.getTextRange)
      } else if (settings.SPACE_WITHIN_METHOD_PARENTHESES)
        return WITH_SPACING
      else
        return WITHOUT_SPACING
    }
    if (rightElementType == tRPARENTHESIS && rightPsi
          .getParent
          .isInstanceOf[ScParameterClause]) {
        ...
        settings.CLASS_BRACE_STYLE match {
          ...
          case CommonCodeStyleSettings.END_OF_LINE =>
            ...
            if (settings.SPACE_BEFORE_CLASS_LBRACE)
              return if (leftLineComment)
                WITH_SPACING
              else
                WITH_SPACING_NO_KEEP
            else
              return if (leftLineComment)
                WITHOUT_SPACING
              else
          ...
          case CommonCodeStyleSettings.NEXT_LINE_IF_WRAPPED =>
            ...
            if (settings.SPACE_BEFORE_CLASS_LBRACE)
              ...
              return WITHOUT_SPACING_DEPENDENT(range)
        }
      } else {
    ...
    if (leftPsi.isInstanceOf[ScImportStmt] && !rightPsi
      ...
      } else if (settings.SPACE_BEFORE_SEMICOLON)
        return WITH_SPACING
      else
        return WITHOUT_SPACING
    }
    ...
    if (rightNode.getElementType == ScalaTokenTypes.tRBRACE) {
      rightNode.getTreeParent.getPsi match {
        case block @ (
              _: ScEarlyDefinitions | _: ScTemplateBody | _: ScPackaging |
              _: ScBlockExpr | _: ScMatchStmt | _: ScTryBlock | _: ScCatchBlock
            ) =>
          val oneLineNonEmpty =
        ...
        case _: ScImportSelectors =>
          return if (scalaSettings.SPACES_IN_IMPORTS)
            WITH_SPACING
          else
            WITHOUT_SPACING
        case _ =>
      ...
      if (!scalaSettings.PLACE_CLOSURE_PARAMETERS_ON_NEW_LINE) {
        ...
        rightNode.getElementType match {
          case ScalaElementTypes.FUNCTION_EXPR =>
            return spacing
          case ScalaElementTypes.CASE_CLAUSES =>
            if (b.getParent.isInstanceOf[ScArgumentExprList] || b
                  .getParent
      ...
      leftNode.getTreeParent.getPsi match {
        case b: ScTemplateBody if rightPsi.isInstanceOf[ScSelfTypeElement] =>
          if (scalaSettings.PLACE_SELF_TYPE_ON_NEW_LINE) {
            ...
            return WITHOUT_SPACING_NO_KEEP //todo: spacing setting
        case b @ (_: ScEarlyDefinitions | _: ScTemplateBody) =>
          if (settings
                .KEEP_SIMPLE_BLOCKS_IN_ONE_LINE && !getText(b.getNode, fileText)
                .contains('\n')) {
            return Spacing.createDependentLFSpacing(
              0,
              0,
          ...
          val setting =
            if (c.isInstanceOf[ScTypeDefinition])
              ...
              settings.BLANK_LINES_AFTER_ANONYMOUS_CLASS_HEADER
          return Spacing.createSpacing(
            0,
            0,
            setting + 1,
        ...
        case b: ScBlockExpr if b.getParent.isInstanceOf[ScFunction] =>
          if (settings.KEEP_SIMPLE_METHODS_IN_ONE_LINE && !getText(
                ...
                fileText).contains('\n')) {
            val spaces =
              if (scalaSettings.SPACES_IN_ONE_LINE_BLOCKS)
                1
              else
                0
            return Spacing.createDependentLFSpacing(
        ...
        case b: ScBlockExpr
            if scalaSettings
              .SPACE_INSIDE_CLOSURE_BRACES && !getText(b.getNode, fileText)
              .contains('\n') &&
              scalaSettings.KEEP_ONE_LINE_LAMBDAS_IN_ARG_LIST &&
              (
                b.getParent.isInstanceOf[ScArgumentExprList] || b
                  .getParent
                  .isInstanceOf[ScInfixExpr]
              ) =>
          return WITH_SPACING
          ...
          case b @ (_: ScEarlyDefinitions | _: ScTemplateBody) =>
            ...
            val setting =
              leftPsi match {
                ...
                case _: ScFunction =>
                  settings.BLANK_LINES_AROUND_METHOD
                case _ =>
                  rightPsi match {
                    case _: ScFunction if p.isInstanceOf[ScTrait] =>
                      settings.BLANK_LINES_AROUND_METHOD_IN_INTERFACE
    ...
    if (rightPsi.isInstanceOf[PsiComment] || rightPsi
      ...
      if (pseudoRightPsi
            .isInstanceOf[ScFunction] || pseudoRightPsi.isInstanceOf[ScValue] ||
    ...
    if (leftElementType == ScalaElementTypes.MODIFIERS) {
      if (rightPsi.isInstanceOf[ScParameters]) {
        if (scalaSettings.SPACE_AFTER_MODIFIERS_CONSTRUCTOR)
          return WITH_SPACING
        else
          return WITHOUT_SPACING
      }
    ...
    if (rightPsi.isInstanceOf[ScFinallyBlock]) {
      if (settings.FINALLY_ON_NEW_LINE)
        return ON_NEW_LINE
      else
        return WITH_SPACING
    }
    ...
    if (rightElementType == tAT && rightNode.getTreeParent != null &&
        rightNode.getTreeParent.getElementType == ScalaElementTypes
          .NAMING_PATTERN) {
      return if (scalaSettings.SPACES_AROUND_AT_IN_PATTERNS)
        WITH_SPACING
      else
        WITHOUT_SPACING
    }
    ...
    //comments processing
    if (leftNode.getPsi.isInstanceOf[ScDocComment])
      return ON_NEW_LINE
    if (rightNode.getPsi.isInstanceOf[ScDocComment] && leftNode
          .getElementType == ScalaTokenTypes.tLBRACE)
      return ON_NEW_LINE
    if (rightNode.getPsi.isInstanceOf[ScDocComment])
      return DOUBLE_LINE
    if (rightNode.getPsi.isInstanceOf[PsiComment] || leftNode
          .getPsi
    ...
    if (rightString.length > 0 && rightString(0) == '.') {
      if (rightNode.getElementType != ScalaTokenTypes
            .tFLOAT && !rightNode.getPsi.isInstanceOf[ScLiteral])
        return WITHOUT_SPACING
    }
    if (rightString.length > 0 && rightString(0) == ',') {
    ...
    if (rightNode.getElementType == ScalaTokenTypes.tCOLON) {
      ...
      if (tp ne null) {
        return if (tp.nameId.getNode eq left)
          WITHOUT_SPACING
        else
          WITH_SPACING
      }
    ...
    if (rightString.length > 0 && rightString(0) == ';') {
      if (settings.SPACE_BEFORE_SEMICOLON && !rightNode
            .getTreeParent
            .getPsi
    ...
    if (leftString.length > 0 && leftString(leftString.length - 1) == '.') {
      return if (leftElementType == ScalaElementTypes.LITERAL)
        WITH_SPACING
      else
        WITHOUT_SPACING
    }
    ...
    if (leftNode.getElementType == ScalaTokenTypes.tCOLON) {
      if (scalaSettings.SPACE_AFTER_TYPE_COLON)
        return WITH_SPACING
      else
        return WITHOUT_SPACING
    }
    if (leftString.length > 0 && leftString(leftString.length - 1) == ';') {
      if (settings.SPACE_AFTER_SEMICOLON && !rightNode
            .getTreeParent
            .getPsi
            .isInstanceOf[ScalaFile] &&
          rightNode.getPsi.getParent.getParent.isInstanceOf[ScForStatement])
    ...
    if (rightNode.getElementType == ScalaTokenTypes.tLPARENTHESIS) {
      leftNode.getElementType match {
        case ScalaTokenTypes.kIF =>
          if (settings.SPACE_BEFORE_IF_PARENTHESES)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case ScalaTokenTypes.kWHILE =>
          if (settings.SPACE_BEFORE_WHILE_PARENTHESES)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case ScalaTokenTypes.kFOR =>
          if (settings.SPACE_BEFORE_FOR_PARENTHESES)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _ =>
    ...
    if (rightPsi.isInstanceOf[ScArguments] &&
        ...
        rightPsi.isInstanceOf[ScArguments] && rightNode
          .getTreeParent
          .getPsi
      ...
      if (settings.SPACE_BEFORE_METHOD_CALL_PARENTHESES && !rightString
            .startsWith("{") &&
          (
            leftNode.getLastChildNode == null || !leftNode
              .getLastChildNode
              .getPsi
        ...
        return WITH_SPACING
      else if (scalaSettings
                 .SPACE_BEFORE_BRACE_METHOD_CALL && rightString.startsWith("{"))
        return WITH_SPACING
      else
        return WITHOUT_SPACING
    }
    ...
    // SCL-2601
    if ((
          rightNode.getPsi.isInstanceOf[ScUnitExpr] || rightNode
            .getPsi
    ...
    if (rightPsi.isInstanceOf[ScArguments] && rightNode
          .getTreeParent
          .getPsi
          .isInstanceOf[ScSelfInvocation]) {}
    ...
    if (leftNode.getElementType == ScalaTokenTypes.tLPARENTHESIS) {
      ...
      leftNode.getTreeParent.getPsi match {
        case _: ScForStatement if left.isLeaf =>
          if (settings.SPACE_WITHIN_FOR_PARENTHESES)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScIfStmt =>
          if (settings.SPACE_WITHIN_IF_PARENTHESES)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScWhileStmt | _: ScDoStmt =>
          if (settings.SPACE_WITHIN_WHILE_PARENTHESES)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
    ...
    if (leftNode.getElementType == ScalaTokenTypes.tLSQBRACKET) {
      if (rightNode.getElementType == ScalaTokenTypes.tRSQBRACKET) {
        ...
        if (settings.SPACE_WITHIN_BRACKETS)
          return WITH_SPACING
        else
          return WITHOUT_SPACING
      }
      ...
      parentPsi match {
        ...
        case _: ScIfStmt =>
          if (settings.SPACE_BEFORE_IF_LBRACE && !(
            ...
            return WITHOUT_SPACING
        case _: ScWhileStmt =>
          if (settings.SPACE_BEFORE_WHILE_LBRACE)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScForStatement =>
          if (settings.SPACE_BEFORE_FOR_LBRACE && leftNode
                .getElementType != ScalaTokenTypes.kFOR)
            return WITH_SPACING
            ...
            return WITHOUT_SPACING
        case _: ScDoStmt =>
          if (settings.SPACE_BEFORE_DO_LBRACE)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScMatchStmt =>
          if (scalaSettings.SPACE_BEFORE_MATCH_LBRACE)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScTryBlock =>
          if (settings.SPACE_BEFORE_TRY_LBRACE)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScCatchBlock =>
          if (settings.SPACE_BEFORE_CATCH_LBRACE)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
        case _: ScFinallyBlock =>
          if (settings.SPACE_BEFORE_FINALLY_LBRACE)
            return WITH_SPACING
          else
            return WITHOUT_SPACING
    ...
    (
      ...
      case (ScalaElementTypes.REFERENCE, ret, _, _)
          if ret != ScalaElementTypes.PACKAGING &&
            leftNode.getTreePrev != null && leftNode
            .getTreePrev
            .getTreePrev != null &&
      ...
      case (
            ...
            _) =>
        NO_SPACING
      case (_, ScalaTokenTypes.tAT, _, _) =>
        NO_SPACING_WITH_NEWLINE
      case (ScalaElementTypes.ANNOTATION, _, _, _) =>
        COMMON_SPACING
      //Prefix Identifier
      case (
            (
      ...
      case (ScalaTokenTypes.tLBRACE, _, _, _) =>
        NO_SPACING_WITH_NEWLINE
      case (
            _,
            ...
            _) =>
        IMPORT_BETWEEN_SPACING
      case (_, ScalaTokenTypes.tRBRACE, _, _) =>
        NO_SPACING_WITH_NEWLINE
      //Semicolon
      ...
      case (ScalaElementTypes.IMPORT_STMT, _, ScalaElementTypes.PACKAGING, _) =>
        DOUBLE_LINE
      case (ScalaElementTypes.IMPORT_STMT, _, _, _) =>
        IMPORT_BETWEEN_SPACING
      //Dot
      case (ScalaTokenTypes.tDOT, _, _, _) =>
        NO_SPACING_WITH_NEWLINE
      case (_, ScalaTokenTypes.tDOT, _, _) =>
        NO_SPACING
      //Comma
      case (ScalaTokenTypes.tCOMMA, _, _, _) =>
        COMMON_SPACING
      case (_, ScalaTokenTypes.tCOMMA, _, _) =>
        NO_SPACING
      ...
      case (
            ...
            _) =>
        NO_SPACING_WITH_NEWLINE
      case (_, ScalaTokenTypes.tLSQBRACKET, _, _) =>
        NO_SPACING
      case (
            _,
      ...
      case (
        ...
        NO_SPACING_WITH_NEWLINE
      //Case clauses
      case (ScalaElementTypes.CASE_CLAUSE, _, _, _) =>
        IMPORT_BETWEEN_SPACING
      case (_, ScalaElementTypes.CASE_CLAUSE, _, _) =>
        IMPORT_BETWEEN_SPACING
      //#
      case (ScalaTokenTypes.tINNER_CLASS, _, _, _) =>
        NO_SPACING
      ...
      case (_, ScalaTokenTypes.tINNER_CLASS, _, _) =>
        NO_SPACING
      case (ScalaElementTypes.ANNOTATIONS, ScalaTokenTypes.kDEF, _, _)
          if scalaSettings.NEWLINE_AFTER_ANNOTATIONS =>
        ON_NEW_LINE
      //Other cases
      case _ =>
        COMMON_SPACING
