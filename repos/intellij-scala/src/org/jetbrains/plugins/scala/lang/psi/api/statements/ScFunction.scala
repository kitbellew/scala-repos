...
import org.jetbrains.plugins.scala.lang.psi.fake.{
  FakePsiReferenceList,
  FakePsiTypeParameterList
}
import org.jetbrains.plugins.scala.lang.psi.impl.ScalaPsiElementFactory
import org.jetbrains.plugins.scala.lang.psi.impl.toplevel.synthetic.{
  JavaIdentifier,
  ScSyntheticFunction,
  ScSyntheticTypeParameter,
  SyntheticClasses
}
import org.jetbrains.plugins.scala.lang.psi.impl.toplevel.typedef.TypeDefinitionMembers
import org.jetbrains.plugins.scala.lang.psi.light.ScFunctionWrapper
import org.jetbrains.plugins.scala.lang.psi.light.scala.{
  ScLightFunctionDeclaration,
  ScLightFunctionDefinition
}
...
trait ScFunction
  ...
  def isSyntheticUnapply: Boolean =
    synthNavElement.nonEmpty && name == "unapply"
  def isSyntheticUnapplySeq: Boolean =
    synthNavElement.nonEmpty && name == "unapplySeq"
  ...
  private val probablyRecursive: ThreadLocal[Boolean] =
  ...
  def removeAllClauses() {
    paramClauses.clauses.headOption.zip(paramClauses.clauses.lastOption)
      ...
      case None =>
        val superReturnType = superMethodAndSubstitutor match {
          ...
          case Some((fun: PsiMethod, subst)) =>
            ...
            Some(typeParamSubst.followed(subst).subst(
  ...
  def effectiveParameterClauses: Seq[ScParameterClause] =
    paramClauses.clauses ++ syntheticParamClause
  ...
  def getParamByName(
    ...
    clausePosition match {
      ...
      case _ =>
        effectiveParameterClauses.apply(clausePosition).effectiveParameters
          .find {
            case param =>
              ScalaPsiUtil.memberNamesEquals(param.name, name) ||
                param.deprecatedName
  ...
  def superMethods: Seq[PsiMethod] = {
    ...
    if (clazz != null)
      TypeDefinitionMembers.getSignatures(clazz)
        .forName(ScalaPsiUtil.convertMemberName(name))._1
        .get(new PhysicalSignature(this, ScSubstitutor.empty))
        .getOrElse(return Seq.empty).supers
        .filter(_.info.isInstanceOf[PhysicalSignature]).map {
  ...
  def superSignatures: Seq[Signature] = {
    ...
    val t = TypeDefinitionMembers.getSignatures(clazz)
      .forName(ScalaPsiUtil.convertMemberName(name))._1
      ...
      override def getReferencedTypes: Array[PsiClassType] = {
        hasAnnotation("scala.throws") match {
          case Some(annotation) =>
            annotation.constructor.args.map(_.exprs).getOrElse(Seq.empty)
              .flatMap { expr =>
        ...
        while (i >= 0) {
          ...
          res match {
            case Success(t: ScType, _) =>
              res = collectFailures(paramTypes, Nothing)(
                ScFunctionType(t, _)(getProject, getResolveScope))
            case _ =>
          }
          i = i - 1
        }
  ...
  def collectReverseParamTypesNoImplicits: Option[Seq[Seq[ScType]]] = {
    ...
    while (i >= 0) {
      val cl = paramClauses.clauses.apply(i)
      if (!cl.isImplicit) {
        val paramTypes: Seq[TypeResult[ScType]] = cl.parameters
...
object ScFunction {
  ...
  def getCompoundCopy(
    ...
    fun match {
      ...
      case light: ScLightFunctionDefinition =>
        getCompoundCopy(pTypes, tParams, rt, light.fun)
      case decl: ScFunctionDeclaration =>
        new ScLightFunctionDeclaration(pTypes, tParams, rt, decl)
