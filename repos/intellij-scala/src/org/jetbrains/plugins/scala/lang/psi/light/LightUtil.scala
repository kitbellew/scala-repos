...
object LightUtil {
  ...
  def getThrowsSection(holder: ScAnnotationsHolder): String = {
    val throwAnnotations = holder.allMatchingAnnotations(
      "scala.throws").foldLeft[ArrayBuffer[String]](ArrayBuffer()) {
      case (accumulator, annotation) =>
        val classes = annotation.constructor.args.map(_.exprs).getOrElse(
        ...
        if (classes.isEmpty) {
          annotation.constructor.typeArgList match {
            case Some(args) =>
              val classes =
                args.typeArgs.map(_.getType(TypingContext.empty)).filter(
                  _.isDefined).map(_.get).flatMap { arg =>
                  ScType.toPsi(
                    arg,
                    holder.getProject,
                    holder.getResolveScope) match {
