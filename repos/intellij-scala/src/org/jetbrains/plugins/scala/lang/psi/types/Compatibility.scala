...
object Compatibility {
    ...
    private def eval(typez: ScType, expectedOption: Option[ScType])
      ...
      expectedOption match {
        case Some(expected) if typez.conforms(expected) =>
          (Success(typez, None), Set.empty)
        case Some(expected) =>
          val defaultResult: (TypeResult[ScType], Set[ImportUsed]) =
            (Success(typez, None), Set.empty)
  ...
  object Expression {
    ...
    implicit def scExpression2Expression(expr: ScExpression): Expression =
      Expression(expr)
  ...
  def checkConformanceExt(
    ...
    if (clashedAssignments.nonEmpty) {
      val problems = clashedAssignments
        .map(new ParameterSpecifiedMultipleTimes(_))
    ...
    def doNoNamed(expr: Expression): List[ApplicabilityProblem] = {
      if (namedMode) { List(new PositionalAfterNamedArgument(expr.expr)) }
        ...
        val typeResult = expr.getTypeAfterImplicitConversion(
          checkWithImplicits,
          isShapesResolve,
      ...
      exprs(k) match {
        ...
        case Expression(assign @ NamedAssignStmt(name)) =>
          ...
          if (index == -1 || used(index)) {
            ...
            if (!checkNames)
              return ConformanceExtResult(
                Seq(new ApplicabilityProblem("9")),
                undefSubst,
                defaultParameterUsed,
                matched,
                matchedTypes)
            ...
            assign.getRExpression match {
              ...
              case _ =>
                return ConformanceExtResult(
                  Seq(new ApplicabilityProblem("11")),
                  undefSubst,
                  defaultParameterUsed,
                  matched,
                  matchedTypes)
    ...
    if (exprs.length == parameters.length)
      ...
      if (namedMode)
        return ConformanceExtResult(
          Seq(new ApplicabilityProblem("12")),
          undefSubst,
          defaultParameterUsed,
          matched,
          matchedTypes)
      if (!parameters.last.isRepeated)
        return ConformanceExtResult(
          Seq(new ApplicabilityProblem("13")),
          undefSubst,
          defaultParameterUsed,
          matched,
          matchedTypes)
      ...
      if (exprs.length == parameters.length - 1 && !namedMode && parameters.last
            .isRepeated)
        return ConformanceExtResult(
          Seq.empty,
          undefSubst,
      ...
      if (missed.nonEmpty)
        ...
        for ((param, use) <- pack if param.isDefault && !use) {
          ...
          param.defaultType match {
            ...
            case Some(defaultTp) =>
              return ConformanceExtResult(
                Seq(new DefaultTypeParameterMismatch(defaultTp, paramType)),
                undefSubst,
  ...
  def toParameter(p: ScParameter, substitutor: ScSubstitutor): Parameter = {
    ...
    val default = p.getDefaultExpression
      .flatMap(_.getType().toOption.map(substitutor.subst))
    new Parameter(
      p.name,
      p.deprecatedName,
      t,
  ...
  def compatible(
      ...
      isShapesResolve: Boolean): ConformanceExtResult = {
    val exprs: Seq[Expression] = argClauses.headOption match {
      case Some(seq) => seq
      case _         => Seq.empty
    }
    named match {
      case synthetic: ScSyntheticFunction =>
        ...
        checkConformanceExt(
          checkNames = false,
          parameters = synthetic.paramClauses.head.map { p =>
      ...
      case fun: ScFunction =>
        ...
        val parameters: Seq[ScParameter] = fun.effectiveParameterClauses
          .headOption.toList.flatMap(_.effectiveParameters)
        ...
        val obligatory = parameters
          .filter(p => !p.isDefaultParam && !p.isRepeatedParameter)
      ...
      case constructor: ScPrimaryConstructor =>
        val parameters: Seq[ScParameter] = constructor
      ...
      case method: PsiMethod =>
        val parameters: Seq[PsiParameter] = method.getParameterList
          .getParameters.toSeq
