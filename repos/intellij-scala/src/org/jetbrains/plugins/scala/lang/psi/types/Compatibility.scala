...
object Compatibility {
      ...
      expectedOption match {
        ...
        case Some(expected) =>
          val defaultResult: (TypeResult[ScType], Set[ImportUsed]) =
            (Success(typez, None), Set.empty)
          val functionType =
            ScFunctionType(expected, Seq(typez))(
              place.getProject,
  ...
  def checkConformanceExt(
    ...
    if (exprs.length == parameters.length)
      ...
      defaultParameterUsed = parameters
        .zip(used)
        .exists {
          case (param, bool) =>
  ...
  def compatible(
    ...
    named match {
      ...
      case fun: ScFunction =>
        ...
        val parameters: Seq[ScParameter] = fun
      ...
      case method: PsiMethod =>
        val parameters: Seq[PsiParameter] =
          method.getParameterList.getParameters.toSeq
