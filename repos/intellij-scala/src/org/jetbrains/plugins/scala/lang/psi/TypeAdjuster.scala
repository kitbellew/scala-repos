...
object TypeAdjuster extends ApplicationAdapter {
  ...
  def adjustFor(
    ...
    val typeElements =
      elements.toVector.flatMap(collectAdjustableTypeElements).distinct
          ...
          .flatMap { resolved =>
            ...
            resolved match {
              ...
              case named: PsiNamedElement
                  if ScalaPsiUtil.hasStablePath(named) =>
                named match {
                  case clazz: PsiClass =>
                    availableTypeAliasFor(
                      clazz,
                      position,
                      useTypeAliases) match {
