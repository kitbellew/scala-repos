...
private[changeSignature] trait ScalaChangeSignatureUsageHandler {
  ...
  protected def handleChangedName(
      change: ChangeInfo,
    ...
    val nameId =
      usage match {
        case ScalaNamedElementUsageInfo(scUsage) =>
          scUsage.namedElement.nameId
        case MethodCallUsageInfo(ref, _) =>
          ref.nameId
        case RefExpressionUsage(r) =>
          r.nameId
        case InfixExprUsageInfo(i) =>
          i.operation.nameId
        case PostfixExprUsageInfo(p) =>
          p.operation.nameId
        case AnonFunUsageInfo(_, ref) =>
          ref.nameId
        case ImportUsageInfo(ref) =>
          ref.nameId
        case _ =>
          null
      }
  ...
  protected def handleVisibility(
      change: ChangeInfo,
      usage: ScalaNamedElementUsageInfo): Unit = {
    val visibility =
      change match {
        case j: JavaChangeInfo =>
          j.getNewVisibility
        case _ =>
          return
      }
  ...
  protected def handleReturnTypeChange(
      change: ChangeInfo,
      usage: ScalaNamedElementUsageInfo): Unit = {
    ...
    val substType: ScType =
      UsageUtil.returnType(change, usage) match {
        case Some(result) =>
          result
        case None =>
          return
      }
    val newTypeElem = ScalaPsiElementFactory
      .createTypeElementFromText(substType.canonicalText, element.getManager)

    val oldTypeElem =
      element match {
        case fun: ScFunction =>
          fun.returnTypeElement
        ...
        val (context, anchor) =
          ScalaPsiUtil.nameContext(element) match {
            ...
            case cp: ScClassParameter =>
              (cp.getParent, cp)
            case ctx =>
              (ctx, ctx.getLastChild)
          }
        AddOnlyStrategy
          .withoutEditor
          .addTypeAnnotation(substType, context, anchor)
    }
  }
  ...
  def handleAnonFunUsage(change: ChangeInfo, usage: AnonFunUsageInfo): Unit = {
    if (!change.isParameterSetOrOrderChanged)
      return
    val jChange =
      change match {
        case j: JavaChangeInfo =>
    ...
    val paramTypes =
      usage.expr.getType() match {
        case Success(ScFunctionType(_, pTypes), _) =>
          pTypes
        case _ =>
          Seq.empty
      }
          ...
          for {
            ...
            val paramName =
              if (!param.name.isEmpty)
                ...
                param.nameInCode match {
                  case Some(n) =>
                    n
                  case None =>
                    NameSuggester.suggestNamesByType(param.paramType)(0)
                }
            paramsBuf = paramsBuf :+ paramName
            val text = ScalaPsiElementFactory
              .createExpressionFromText(paramName, arg.getManager)
    ...
    val funExpr = ScalaPsiElementFactory
      .createExpressionFromText(newFunExprText, usage.expr.getManager)
    val replaced = usage
      .expr
  ...
  protected def handleChangedParameters(
    ...
    val result =
      usage.paramClauses match {
        case Some(p) =>
          p.replace(newClauses)
        case None =>
          nameId.getParent.addAfter(newClauses, nameId)
      }
  ...
  protected def handleUsageArguments(
    ...
    usage match {
      ...
      case m: MethodCallUsageInfo =>
        handleMethodCallUsagesArguments(change, m)
      case r: RefExpressionUsage =>
        handleRefUsageArguments(change, r)
      case i: InfixExprUsageInfo =>
        handleInfixUsage(change, i)
      case p: PostfixExprUsageInfo =>
        handlePostfixUsage(change, p)
      case _ =>
    }
  }
  ...
  protected def handleInfixUsage(
    ...
    if (newParams.length != 1) {
      ...
      val argText =
        arguments(change, usage).headOption match {
      ...
      val expr = ScalaPsiElementFactory
        .createExpressionWithContextFromText(argText, infix, infix.getArgExpr)
      infix.getArgExpr.replaceExpression(expr, removeParenthesis = true)
    }
  }

  def handleConstructorUsageArguments(
      change: ChangeInfo,
      usage: ConstructorUsageInfo): Unit = {
    val constr = usage.constr
    val typeElem = constr.typeElement
    val text = typeElem.getText + argsText(change, usage)
    val newConstr = ScalaPsiElementFactory
      .createConstructorFromText(text, constr.getContext, constr)

    constr.replace(newConstr)
  }

  protected def handleRefUsageArguments(
      change: ChangeInfo,
      usage: RefExpressionUsage): Unit = {
    if (change.getNewParameters.isEmpty)
      return

    val ref = usage.refExpr
    val text = ref.getText + argsText(change, usage)
    val call = ScalaPsiElementFactory
      .createExpressionWithContextFromText(text, ref.getContext, ref)
    ref.replaceExpression(call, removeParenthesis = true)
  }

  protected def handlePostfixUsage(
      change: ChangeInfo,
      usage: PostfixExprUsageInfo): Unit = {
    if (change.getNewParameters.isEmpty)
      return
    ...
    val call = ScalaPsiElementFactory
      .createExpressionWithContextFromText(text, postfix.getContext, postfix)
    postfix.replaceExpression(call, removeParenthesis = true)
  }

  protected def handleMethodCallUsagesArguments(
      change: ChangeInfo,
      usage: MethodCallUsageInfo): Unit = {
    val call = usage.call
    val newText = usage.ref.getText + argsText(change, usage)
    val newCall = ScalaPsiElementFactory
      .createExpressionWithContextFromText(newText, call.getContext, call)
    call.replace(newCall)
  }

  private def arguments(
      change: ChangeInfo,
      methodUsage: MethodUsageInfo): Seq[Seq[String]] = {
    if (change.getNewParameters.isEmpty)
      return Seq.empty
    val isAddDefault =
      change match {
        case c: ScalaChangeInfo =>
          c.isAddDefaultArgs
        case c: JavaChangeInfo =>
          c.isGenerateDelegate
        case _ =>
          true
      }
    ...
    def nonVarargArgs(clause: Seq[ParameterInfo]) = {
      ...
      for {
        ...
        newArgumentExpression(
          ...
          case Some(text) =>
            buffer += text
            if (text.contains("=") && idx > buffer.size - 1)
              needNamed = true
          case None =>
            needNamed = true
        }
          ...
          if (oldIndex < 0) {
            ...
            val (argExprs, wasNamed) =
              oldArgsInfo.byOldParameterIndex.get(oldIndex) match {
                case Some(Seq(ScAssignStmt(_, Some(expr)))) =>
                  (Seq(expr), true)
                case Some(seq) =>
                  (seq, false)
                case _ =>
            ...
            if (jChangeInfo.isArrayToVarargs) {
              argExprs match {
                ...
                case Seq(expr) =>
                  val typedText = ScalaExtractMethodUtils.typedName(
                    expr.getText,
                    "_*",
                    expr.getProject,
                    byName = false)
    ...
    def toArgs(clause: Seq[ParameterInfo]) =
      nonVarargArgs(clause) ++: varargsExprs(clause)

    val clauses =
      change match {
        case sc: ScalaChangeInfo =>
          sc.newParams.filter(_.nonEmpty)
        case _ =>
          Seq(change.getNewParameters.toSeq)
      }
  ...
  private def newArgumentExpression(
      ...
      if (oldIdx < 0) {
        ...
        argsInfo.byOldParameterIndex.get(oldIdx) match {
          ...
          case Some(seq) if seq.size > 1 =>
            return None
          case Some(Seq(assignStmt: ScAssignStmt)) =>
    ...
    val argText =
      if (named)
        s"${newParam.getName} = $withoutName"
      else
        withoutName
    Some(argText)
  }
      ...
      case jRef: PsiReferenceExpression =>
        jRef.getReferenceNameElement match {
          case nameId: PsiIdentifier =>
            ...
            val newNameIdentifier: PsiIdentifier = factory
  ...
  private def parameterListText(
      change: ChangeInfo,
      usage: ScalaNamedElementUsageInfo): String = {
    ...
    def newParamName(p: ParameterInfo) = {
      ...
      change match {
        case jc: JavaChangeInfo if oldIdx >= 0 =>
          ...
          if (oldNameOfChanged != oldNameOfCurrent)
            oldNameOfCurrent
          else
            p.getName
        case _ =>
    ...
    def paramText(p: ParameterInfo) = {
      val typedName = ScalaExtractMethodUtils
        .typedName(newParamName(p), paramType(p), project, byName = false)
      val default = scalaDefaultValue(p).fold("")(" = " + _)
      val keywordsAndAnnots =
        p match {
          case spi: ScalaParameterInfo =>
            spi.keywordsAndAnnotations
          case _ =>
            ""
        }
  ...
  private def isRepeated(p: ParameterInfo) =
    p match {
      case p: ScalaParameterInfo =>
        p.isRepeatedParameter
      case p: JavaParameterInfo =>
        p.isVarargType
      case _ =>
        false
    }
