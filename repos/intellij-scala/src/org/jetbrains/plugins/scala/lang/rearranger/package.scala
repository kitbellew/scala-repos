...
package object rearranger {
  ...
  //access modifiers
  val scalaAccessModifiersByName = immutable
    .ListMap("public" -> PUBLIC, "protected" -> PROTECTED, "private" -> PRIVATE)
  val scalaAccessModifiersById = immutable.HashMap(
    PUBLIC.getId -> PUBLIC,
    PROTECTED.getId -> PROTECTED,
    PRIVATE.getId -> PRIVATE)
  ...
  val scalaTypesValues = immutable
    .HashSet(TYPE, FUNCTION, CLASS, VAL, VAR, TRAIT, MACRO, CONSTRUCTOR, OBJECT)
  ...
  val scalaAccessModifiersValues = scalaAccessModifiersByName
    .toSet
