...
class ScalaAotCompletionContributor extends ScalaCompletionContributor {
    ...
    new CompletionProvider[CompletionParameters] {
      ...
      def addCompletions(
        ...
        if (!scope.isInstanceOf[ScVariableDeclaration] &&
            !scope.isInstanceOf[ScValueDeclaration]) return
  ...
  private def isSuitableIdentifier(s: String) =
    ScalaNamesUtil.isIdentifier(s) && s.forall(_.isLetterOrDigit)
}
...
private class MyInsertHandler(name: String, typed: Boolean)
  ...
  def handleInsert(context: InsertionContext, item: LookupElement) = {
    ...
    if (typed) {
      ...
      val range =
        TextRange.from(begin, text.length).shiftRight(name.length + 2)
