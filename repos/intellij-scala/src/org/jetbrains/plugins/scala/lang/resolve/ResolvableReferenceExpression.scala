        ...
        r match {
          ...
          case ScalaResolveResult(fun: ScFunction, subst: ScSubstitutor) =>
            if (!processor.isInstanceOf[CompletionProcessor]) {
              fun.getParamByName(
                ref.refName,
                invocationCount - 1) match { //todo: why -1?
                case Some(param) =>
  ...
  private def processConstructorReference(
    ...
    def processConstructor(
      ...
      ScType.extractClassType(tp) match {
        ...
        case Some((clazz, subst)) =>
          val processor: MethodResolveProcessor =
    ...
    args.getContext match {
      ...
      case constr: ScConstructor =>
        val tp: ScType = constr
          .typeElement
          .getType(TypingContext.empty)
          .getOrElse(
            return
          )
        val typeArgs: Seq[ScTypeElement] = constr
          .typeArgList
          .map(_.typeArgs)
          .getOrElse(Seq())
        val arguments = constr.arguments
        val secondaryConstructors =
          (clazz: ScClass) => clazz.secondaryConstructors
        processConstructor(
          constr,
          tp,
          typeArgs,
  ...
  private def collectImplicits(
    ...
    processor match {
      case _: CompletionProcessor =>
        val convertible: ScImplicitlyConvertible =
          new ScImplicitlyConvertible(e)
