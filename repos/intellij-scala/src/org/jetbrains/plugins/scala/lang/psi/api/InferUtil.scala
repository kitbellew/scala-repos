...
object InferUtil {
  ...
  def findImplicits(
    ...
    while (iterator.hasNext) {
      ...
      val paramType =
        abstractSubstitutor.subst(param.paramType) //we should do all of this with information known before
    ...
    def applyImplicitViewToResult(
      ...
      expectedType match {
        case Some(expectedType @ ScFunctionType(expectedRet, expectedParams))
          ...
          val updatedResultType = dummyExpr.getTypeAfterImplicitConversion(
            expectedOption = Some(expectedRet))
