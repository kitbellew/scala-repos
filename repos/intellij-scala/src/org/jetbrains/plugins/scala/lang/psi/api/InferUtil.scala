...
object InferUtil {
  ...
  private val LOG = Logger
    .getInstance("#org.jetbrains.plugins.scala.lang.psi.api.InferUtil$")
  ...
  def logInfo(searchLevel: Int, message: => String) {
    ...
    if (isDebugImplicitParameters) { LOG.debug(indent + message) }
  }
  ...
  def updateTypeWithImplicitParameters(
      ...
      fullInfo: Boolean): (ScType, Option[Seq[ScalaResolveResult]]) = {
    var resInner = res
      ...
      case t @ ScTypePolymorphicType(
        ...
        updatedType match {
          case tpt: ScTypePolymorphicType =>
            resInner = t.copy(
              internalType = mt
                .copy(returnType = tpt.internalType)(mt.project, mt.scope),
              typeParameters = tpt.typeParameters)
          case _ => //shouldn't be there
            resInner = t.copy(internalType = mt
              .copy(returnType = updatedType)(mt.project, mt.scope))
        }
      case t @ ScTypePolymorphicType(
            mt @ ScMethodType(retType, params, impl),
            typeParams) if impl =>
        val fullAbstractSubstitutor = t.abstractOrLowerTypeSubstitutor
        val coreTypes = params
          .map(p => fullAbstractSubstitutor.subst(p.paramType))
        ...
        coreTypes.foreach {
          case coreType => resInner match {
              case t @ ScTypePolymorphicType(
                ...
                val abstractSubstitutor: ScSubstitutor = t
                  .abstractOrLowerTypeSubstitutor
  ...
  def findImplicits(
    ...
    while (iterator.hasNext) {
      ...
      val paramType = abstractSubstitutor
        .subst(
          param.paramType
  ...
  def updateAccordingToExpectedType(
      _nonValueType: TypeResult[ScType],
    ...
    nonValueType match {
      ...
      case Success(ScTypePolymorphicType(internal, typeParams), _)
    ...
    def applyImplicitViewToResult(
      ...
      expectedType match {
        case Some(expectedType @ ScFunctionType(expectedRet, expectedParams))
          ...
          dummyExpr.asInstanceOf[ScLiteral]
          ...
          expr.asInstanceOf[ScExpression]
  ...
  def extractImplicitParameterType(r: ScalaResolveResult): ScType = {
    r match {
      ...
      case ScalaResolveResult(f: ScFieldId, subst) =>
        subst.subst(f.getType(TypingContext.empty).get)
  ...
  def localTypeInferenceWithApplicability(
      retType: ScType,
      params: Seq[Parameter],
      exprs: Seq[Expression],
      typeParams: Seq[TypeParameter],
  ...
  def localTypeInferenceWithApplicabilityExt(
    ...
    val abstractSubst = ScTypePolymorphicType(retType, typeParams)
                    ...
                    case tp =>
                      ...
                      if (removeMe && safeCheck) {
                        ...
                        def checkTypeParam(
                            ...
                            tp: => ScType): Boolean = {
                          val typeParams: Seq[ScTypeParam] = typeParam
                            .typeParameters
