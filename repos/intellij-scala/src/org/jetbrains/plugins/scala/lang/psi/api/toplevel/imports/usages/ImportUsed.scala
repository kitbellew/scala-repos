...
object ImportUsed {
  val key: Key[_root_.scala.collection.Set[ImportUsed]] = Key
    .create("scala.used.imports.key")
...
case class ImportExprUsed(expr: ScImportExpr) extends ImportUsed(expr) {
  override def qualName: Option[String] = {
    if (expr.qualifier == null)
      None
    else if (expr.singleWildcard)
      Some(expr.qualifier.qualName + "._")
    else
      expr.reference.map(ref => expr.qualifier.qualName + "." + ref.refName)
  }
...
case class ImportSelectorUsed(sel: ScImportSelector) extends ImportUsed(sel) {
  override def qualName: Option[String] = {
    val expr: ScImportExpr = PsiTreeUtil
      .getParentOfType(sel, classOf[ScImportExpr])
