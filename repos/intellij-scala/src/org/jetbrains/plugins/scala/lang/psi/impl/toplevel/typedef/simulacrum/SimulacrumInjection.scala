...
class SimulacrumInjection extends SyntheticMembersInjector {
  override def needsCompanionObject(source: ScTypeDefinition): Boolean = {
    source.findAnnotationNoAliases(
      "simulacrum.typeclass") != null && source.typeParameters.length == 1
  }
        ...
        ScalaPsiUtil.getCompanionModule(obj) match {
          case Some(clazz)
            ...
            val ops = clazz.functions.flatMap {
              case f: ScFunction =>
                f.parameters.headOption.flatMap(
                  _.getType(TypingContext.empty).toOption).flatMap(tp =>
                  isProperTpt(tp)) match {
                  case Some(funTypeParamToLift) =>
                    ...
                    names.map {
                      case name =>
                        ...
                        def paramText(p: ScParameter): String = {
                          substOpt match {
                            case Some(subst) =>
                              p.name + " : " + subst.subst(
                                p.getType(
                                  TypingContext.empty).getOrAny).canonicalText
                            case _ => p.getText
                          }
                        }
                        def clauseText(p: ScParameterClause): String = {
                          p.parameters.map(paramText).mkString(
                        ...
                        val headParams =
                          f.paramClauses.clauses.head.parameters.tail.map(
                            paramText)
                        val restHeadClause =
                          if (headParams.isEmpty) ""
                          else headParams.mkString("(", ", ", ")")
                        val restClauses = f.paramClauses.clauses.tail.map(
                          clauseText).mkString("")
                    ...
                    te.getType(TypingContext.empty) match {
                      case Success(ScParameterizedType(classType, Seq(tp)), _)
                        ...
                        (for {
                          ...
                          s" with ${ref.getText}.AllOps[$tpName$additionalWithComma]")).getOrElse(
                          fromType)
                      case _ => Seq.empty
                    }
                }).mkString
