    ...
    source match {
      case obj: ScObject => obj.fakeCompanionClassOrCompanionClass match {
          case clazz: ScTypeDefinition
            ...
            Seq(s"def apply[$tpText](implicit instance: ${clazz
    ...
    source match {
      case obj: ScObject => ScalaPsiUtil.getCompanionModule(obj) match {
          case Some(clazz)
            ...
            val additionalWithBracket = tpAdditional.map("[" + _ + "]")
              .getOrElse("")
                    ...
                    names.map {
                      case name =>
                        ...
                        val typeParamClasue = f.typeParametersClause
                          .map(_.getText).getOrElse("")
                        val headParams = f.paramClauses.clauses.head.parameters
                          .tail.map(paramText)
                        val restHeadClause =
                          if (headParams.isEmpty) ""
                          else headParams.mkString("(", ", ", ")")
                        val restClauses = f.paramClauses.clauses.tail
                          .map(clauseText).mkString("")
