...
class ScPrimaryConstructorWrapper(
    val constr: ScPrimaryConstructor,
    isJavaVarargs: Boolean = false,
    forDefault: Option[Int] = None)
    extends {
  val elementFactory =
    JavaPsiFacade.getInstance(constr.getProject).getElementFactory
  val containingClass = {
...
with LightScalaMethod {
  ...
  override def navigate(requestFocus: Boolean): Unit =
    function.navigate(requestFocus)
    ...
    if (returnType == null && !isConstructor) {
      ...
      forDefault match {
        case Some(i) =>
          ...
          val scalaType = generifySubst subst ScFunctionWrapper.getSubstitutor(
            cClass,
            function).subst(param.getType(TypingContext.empty).getOrAny)
        ...
        case None =>
          val scalaType = generifySubst subst ScFunctionWrapper.getSubstitutor(
            cClass,
            function).subst(function.returnType.getOrAny)
          returnType = ScType.toPsi(
            scalaType,
            function.getProject,
            function.getResolveScope)
...
object ScFunctionWrapper {
  ...
  def methodText(
              ...
              tp.upperBound.map(subst.subst) match {
                ...
                case Success(_: StdType, _) =>
                  JavaPsiFacade.getInstance(
                    function.getProject).getElementFactory.createTypeByFQClassName(
                    "java.lang.Object",
                    function.getResolveScope)
    ...
    }.flatMap(_.effectiveParameters).map {
      case param =>
        val builder = new StringBuilder
      ...
      case (Some(clazz), function: ScFunction) =>
        clazz match {
          case td: ScTypeDefinition =>
            td.signaturesByName(function.name).find(
              _.method == function) match {
