...
class ScSimpleTypeElementImpl(node: ASTNode)
  ...
  override def getTypeNoConstructor(ctx: TypingContext): TypeResult[ScType] =
    innerNonValueType(ctx, inferValueType = true, noConstructor = true)

  @CachedWithRecursionGuard[ScSimpleTypeElement](
    this,
  ...
  private def innerNonValueType(
      ...
      if (lastImplicit) {
        ...
        val newTp = tp match {
          case ScTypePolymorphicType(i, p) =>
            ScTypePolymorphicType(
              ScMethodType(i, params.last, isImplicit = true)(
                getProject,
                getResolveScope),
              p)
          case _ =>
            ScMethodType(tp, params.last, isImplicit = true)(
              getProject,
              getResolveScope)
        }
        val res = InferUtil.updateTypeWithImplicitParameters(
          newTp,
          this,
          None,
          withExpected,
    ...
    def typeForConstructor(
        ...
        parentElement: PsiNamedElement): ScType = {
      val clazz = constr.containingClass
      val (constrTypParameters: Seq[ScTypeParam], constrSubst: ScSubstitutor) =
        parentElement match {
          ...
          case s: ScTypeParametersOwner if s.typeParameters.nonEmpty =>
            constr match {
              case method: ScMethodLike =>
                val params = method.getConstructorTypeParameters.map(
                  _.typeParameters).getOrElse(Seq.empty)
                val subst = new ScSubstitutor(
                  s.typeParameters.zip(params).map {
        ...
        case _ =>
          parametrise(
            ScSimpleTypeElementImpl.calculateReferenceType(
              ref,
              shapesOnly = false).getOrElse(return types.Nothing),
            clazz,
            subst)
      }
      val res = subst.subst(tp)

      val (params: Seq[Seq[Parameter]], lastImplicit: Boolean) =
        getConstructorParams(constr, subst)
        ...
        case Some(c) =>
          ...
          while (i < params.length - 1 && i < c.arguments.length - 1) {
            nonValueType = InferUtil.localTypeInference(
              ...
              params(i),
              c.arguments(i).exprs.map(new Expression(_)),
                  ...
                  nonValueType = InferUtil.localTypeInference(
                    ...
                    Seq(
                      new Parameter(
                        ...
                        false,
                        0)),
                    Seq(new Expression(InferUtil.undefineSubstitutor(
                      nonValueType.typeParameters).subst(res.inferValueType))),
                    nonValueType.typeParameters,
    ...
    reference match {
      case Some(ref) =>
        def updateForParameterized(
            subst: ScSubstitutor,
            elem: PsiNamedElement,
            p: ScParameterizedTypeElement): (ScType, ScSubstitutor) = {
          val tp = elem match {
            case ta: ScTypeAliasDefinition =>
              ta.aliasedType.getOrElse(
                return (types.Nothing, ScSubstitutor.empty))
            case clazz: PsiClass =>
              parametrise(
                ScSimpleTypeElementImpl.calculateReferenceType(
                  ref,
                  shapesOnly = false).getOrElse(
                  return (types.Nothing, ScSubstitutor.empty)),
                clazz,
                subst)
          }
        ...
        ref.resolveNoConstructor match {
          ...
          case Array(
              ...
              if constrRef && to.isInstanceOf[PsiNamedElement] &&
                (to.typeParameters.isEmpty || getContext.isInstanceOf[
          ...
          case Array(
                ScalaResolveResult(
                  ...
                  subst: ScSubstitutor))
              if constrRef && to.isInstanceOf[PsiNamedElement] &&
                (to.getTypeParameters.isEmpty || getContext.isInstanceOf[
                  ScParameterizedTypeElement]) =>
                                         ...
                                         if (ref.isConstructorReference) {
                                           ref.resolveNoConstructor match {
                                             case Array(
                                                 ...
                                                 if to.isInstanceOf[
                                                   PsiNamedElement] &&
                                                   (to.typeParameters.isEmpty || ref.getContext.isInstanceOf[
                                                     ScParameterizedTypeElement]) =>
                                               Some(r)
                                             case Array(r @ ScalaResolveResult(
                                                 ...
                                                 if to.isInstanceOf[
                                                   PsiNamedElement] &&
                                                   (to.getTypeParameters.isEmpty || ref.getContext.isInstanceOf[
                                                     ScParameterizedTypeElement]) =>
                                               Some(r)
                                             case _ => ref.bind()
                                           }
        ...
        qual.resolve() match {
          ...
          case _ =>
            calculateReferenceType(qual, shapesOnly) match {
              ...
              case Success(tp, _) =>
                Success(
                  ScProjectionType(tp, resolvedElement, superReference = false),
                  Some(ref))
            }
        }
      case None =>
