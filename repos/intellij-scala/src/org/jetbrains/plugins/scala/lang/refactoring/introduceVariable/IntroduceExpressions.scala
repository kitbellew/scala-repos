...
trait IntroduceExpressions {
  ...
  val INTRODUCE_VARIABLE_REFACTORING_NAME = ScalaBundle
      ...
      def runInplace() {
        ...
        val callback = new Pass[OccurrencesChooser.ReplaceChoice] {
          def pass(replaceChoice: OccurrencesChooser.ReplaceChoice) {
            val replaceAll = OccurrencesChooser.ReplaceChoice
              .NO != replaceChoice
            ...
            val asVar = ScalaApplicationSettings.getInstance()
              .INTRODUCE_VARIABLE_IS_VAR
            ...
            CommandProcessor.getInstance.executeCommand(
              ...
              new Runnable {
                def run() {
                  val newDeclaration: PsiElement = ApplicationManager
                    .getApplication.runWriteAction(introduceRunnable).getElement
                  ...
                  if (namedElement != null && namedElement.isValid) {
                    editor.getCaretModel
                      .moveToOffset(namedElement.getTextOffset)
                    ...
                    if (ScalaRefactoringUtil.isInplaceAvailable(editor)) {
                      ...
                      val variableIntroducer =
                        new ScalaInplaceVariableIntroducer(
                          ...
                          forceInferType)
                      variableIntroducer
                        .performInplaceRefactoring(suggestedNamesSet)
                    }
                  }
                }
              },
  ...
  private def runRefactoringInside(
      ...
      file: PsiFile,
      editor: Editor,
      expression_ : ScExpression,
      occurrences_ : Array[TextRange],
      varName: String,
      varType: ScType,
      replaceAllOccurrences: Boolean,
      isVariable: Boolean): SmartPsiElementPointer[PsiElement] = {

    def isIntroduceEnumerator(
        parExpr: PsiElement,
        prev: PsiElement,
    ...
    def replaceRangeByDeclaration(
      ...
      val decl = PsiTreeUtil.findElementOfClassAtOffset(
        file,
        start,
        classOf[ScMember], /*strictStart =*/ false)
      lazy val enum = PsiTreeUtil.findElementOfClassAtOffset(
        file,
        start,
        classOf[ScEnumerator], /*strictStart =*/ false)
    ...
    val replacedOccurences = ScalaRefactoringUtil
      .replaceOccurences(occurrences, varName, file)
    ...
    val parentExprs =
      if (occCount == 1)
        ...
        replacedOccurences.toSeq
          .map(ScalaRefactoringUtil.findParentExpr(file, _))
    val commonParent: PsiElement = PsiTreeUtil.findCommonParent(parentExprs: _*)

    val nextParent: PsiElement = ScalaRefactoringUtil
      .nextParent(commonParent, file)
    ...
    def createEnumeratorIn(forStmt: ScForStatement): ScEnumerator = {
      ...
      val created = ScalaPsiElementFactory.createEnumerator(
        ...
        file.getManager,
        typeName)
      val elem = parent.getChildren.filter(_.getTextRange.contains(firstRange))
        .head
      ...
      if (elem != null) {
        ...
        if (inParentheses) {
          ...
          val semicolon = parent.addBefore(
            ...
            elem)
          result = parent.addBefore(created, semicolon)
          ...
          if (needSemicolon) {
            parent.addBefore(
              ScalaPsiElementFactory.createNewLineNode(parent.getManager)
                .getPsi,
              result)
          }
        }
    ...
    def createVariableDefinition(): PsiElement = {
      val created = ScalaPsiElementFactory.createDeclaration(
        varName,
        typeName,
        isVariable,
        ScalaRefactoringUtil.unparExpr(expression),
      ...
      if (fastDefinition) {
        ...
        if (anchor != null) {
          result = ScalaPsiUtil.addStatementBefore(
            created.asInstanceOf[ScBlockStatement],
            parent,
            Some(anchor))
    ...
    val createdDeclaration: PsiElement = isIntroduceEnumerator(
      commonParent,
      nextParent,
    ...
    SmartPointerManager.getInstance(file.getProject)
      .createSmartPsiElementPointer(createdDeclaration)
  }
  ...
  protected def introduceVariable(
      startOffset: Int,
      endOffset: Int,
      file: PsiFile,
      editor: Editor,
