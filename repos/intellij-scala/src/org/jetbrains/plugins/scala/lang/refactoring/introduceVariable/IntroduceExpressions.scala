...
trait IntroduceExpressions {
  ...
  def invokeExpression(
    ...
    try {
      ...
      def runInplace() {
        ...
        val chooser =
          new OccurrencesChooser[TextRange](editor) {
  ...
  private def runRefactoringInside(
    ...
    def createEnumeratorIn(forStmt: ScForStatement): ScEnumerator = {
      ...
      val created = ScalaPsiElementFactory.createEnumerator(
        ...
        typeName)
      val elem =
        parent.getChildren.filter(_.getTextRange.contains(firstRange)).head
    ...
    val createdDeclaration: PsiElement =
      isIntroduceEnumerator(
        commonParent,
        nextParent,
  ...
  protected def getDialog(
    ...
    val dialog =
      new ScalaIntroduceVariableDialog(
        project,
        typez,
        occurrences.length,
        validator,
