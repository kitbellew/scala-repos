...
object ScalaPsiElementFactory {
    ...
    val packageName = holder match {
      ...
      case _ =>
        ...
        while (element != null && !element.isInstanceOf[
                 ScalaFile] && !element.isInstanceOf[ScPackaging])
  ...
  def createBigImportStmt(
    ...
    if ((names("_") ||
        ScalaCodeStyleSettings.getInstance(
          manager.getProject).getClassCountToUseImportOnDemand <=
          names.size) &&
  ...
  def createStableReferenceElement(name: String, manager: PsiManager) = {
    ...
    (parents: @unchecked) match {
      case Some(p) =>
        ...
        (elements.head.asInstanceOf[
          ScSimpleTypeElement].reference: @unchecked) match {
          case Some(r) => r
        }
  ...
  def createValFromVarDefinition(
    ...
    val text = "class a {" + varText.substring(
      0,
      startOffset) + "val" + varText.substring(startOffset + 3) + " }"
    val dummyFile = createScalaFile(text, manager)
    val classDef = dummyFile.typeDefinitions(0)
  ...
  def createVarFromValDeclaration(
    ...
    val text = "class a {" + valText.substring(
      0,
      startOffset) + "var" + valText.substring(startOffset + 3) + " }"
    val dummyFile = createScalaFile(text, manager)
    val classDef = dummyFile.typeDefinitions(0)
      ...
      case method: ScFunction =>
        ...
        val retAndBody = (needsInferType, retType) match {
          case (true, Some(scType)) =>
            ...
            val needWhitespace =
              method.paramClauses.clauses.isEmpty && method.typeParameters.isEmpty && ScalaNamesUtil.isIdentifier(
                method.name + ":")
  ...
  def createElementWithContext(
      ...
      child: PsiElement,
      parse: ScalaPsiBuilder => Unit): PsiElement = {
    val holder: FileElement = DummyHolderFactory.createHolder(
      context.getManager,
      context).getTreeElement
    val builder: ScalaPsiBuilderImpl =
      new ScalaPsiBuilderImpl(
        PsiBuilderFactory.getInstance.createBuilder(
          context.getProject,
  ...
  def createColon(manager: PsiManager): PsiElement = {
    ...
    dummyFile.getFirstChild.asInstanceOf[ScalaPsiElement].findChildrenByType(
      ScalaTokenTypes.tCOLON).head
  }
  ...
  def createExistentialClauseForName(
    ...
    dummyFile.getChildren.head.asInstanceOf[
      ScValueDeclaration].typeElement.get.asInstanceOf[
      ScExistentialTypeElement].clause
  }
  ...
  def createWildcardPattern(manager: PsiManager): ScWildcardPattern = {
    ...
    dummyFile.getChildren.head.getChildren.apply(
      2).getChildren.head.asInstanceOf[ScWildcardPattern]
  }
  ...
  def createIdsListFromText(
    ...
    val valDef = createDeclarationFromText(
      s"val $text : Int",
      context,
      child).asInstanceOf[ScValueDeclaration]
    if (valDef != null) {
  ...
  def createBlockCommentFromText(
    ...
    createScalaFile(
      "/*" + text + "*/" + " class a { }",
      manager).typeDefinitions(0).simpleComment.get
  }
  ...
  def createDocHeaderElement(length: Int, manager: PsiManager): PsiElement =
    createScalaFile(
      ...
      manager).typeDefinitions(0).docComment.get.getNode.getChildren(null)(
      1).getLastChildNode.getPsi

  def createDocWhiteSpace(manager: PsiManager): PsiElement =
    createScalaFile("/**\n *\n*/ class a {}", manager).typeDefinitions(
      0).docComment.get.getNode.getChildren(null)(1).getPsi

  def createLeadingAsterisk(manager: PsiManager): PsiElement =
    createScalaFile("/**\n *\n*/ class a {}", manager).typeDefinitions(
      0).docComment.get.getNode.getChildren(null)(2).getPsi

  def createDocSimpleData(text: String, manager: PsiManager): PsiElement =
    createScalaFile("/**" + text + "*/ class a {}", manager).typeDefinitions(
      0).docComment.get.getNode.getChildren(null)(1).getPsi
  ...
  def createDocTagValue(text: String, manager: PsiManager): PsiElement = {
    createScalaFile(
      "/**@param " + text + "\n*/ class a{}",
      manager).typeDefinitions(0).docComment.get.getNode.getChildren(null)(
      1).getChildren(null)(2).getPsi
  }

  def createDocTag(text: String, manager: PsiManager): PsiElement =
    createScalaFile(s"/**$text*/ class a", manager).typeDefinitions(
      0).docComment.get.getNode.getChildren(null)(1).getPsi

  def createDocTagName(name: String, manager: PsiManager): PsiElement = {
    createScalaFile("/**@" + name + " qwerty */", manager).typeDefinitions(
      0).docComment.get.getNode.getChildren(null)(1).getChildren(null)(0).getPsi
  }
  ...
  def createDocLinkValue(
    ...
    createScalaFile("/**[[" + text + "]]*/ class a{}", manager).typeDefinitions(
      0).docComment.get.getNode.getChildren(null)(1).getChildren(null)(
      1).getPsi.asInstanceOf[ScDocResolvableCodeReference]
  }
  ...
  def createDocInnerCode(
    ...
    createScalaFile(
      "/**{{{" + text + "}}}\n*/\n class a{}",
      manager).typeDefinitions(0).docComment.get.getNode.getChildren(null)(
      1).getPsi.asInstanceOf[ScDocInnerCodeElement]
  }

  def createXmlEndTag(tagName: String, manager: PsiManager): ScXmlEndTag = {
    createScalaFile(
      "val a = <" + tagName + "></" + tagName + ">",
      manager).getFirstChild.getLastChild.getFirstChild.getLastChild.asInstanceOf[
      ScXmlEndTag]
  }
  ...
  def createXmlStartTag(
    ...
    createScalaFile(
      ...
      manager).getFirstChild.getLastChild.getFirstChild.getFirstChild.asInstanceOf[
      ScXmlStartTag]
  }
  ...
  def createEquivMethodCall(infixExpr: ScInfixExpr): ScMethodCall = {
    ...
    referenceExpr.qualifier.get.replaceExpression(
      exprA,
