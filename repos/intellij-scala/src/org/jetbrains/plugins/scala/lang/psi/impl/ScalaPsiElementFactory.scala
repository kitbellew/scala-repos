...
import org.jetbrains.plugins.scala.lang.scaladoc.psi.api.{
  ScDocComment,
  ScDocInnerCodeElement,
  ScDocResolvableCodeReference,
  ScDocSyntaxElement
}

import scala.collection.mutable

class ScalaPsiElementFactoryImpl(manager: PsiManager)
    extends JVMElementFactory {
  ...
  def isValidParameterName(name: String): Boolean =
    ScalaNamesUtil.isIdentifier(name)

  def isValidFieldName(name: String): Boolean =
    ScalaNamesUtil.isIdentifier(name)

  def isValidLocalVariableName(name: String): Boolean =
    ScalaNamesUtil.isIdentifier(name)
  ...
  def createParameter(
      name: String,
      `type`: PsiType,
      context: PsiElement): PsiParameter = ???

  def createClass(name: String): PsiClass =
    throw new IncorrectOperationException

  def createInterface(name: String): PsiClass =
    throw new IncorrectOperationException

  def createEnum(name: String): PsiClass = throw new IncorrectOperationException

  def createField(name: String, `type`: PsiType): PsiField =
    throw new IncorrectOperationException

  def createMethod(name: String, returnType: PsiType): PsiMethod =
    throw new IncorrectOperationException

  def createMethod(
      name: String,
      returnType: PsiType,
      context: PsiElement): PsiMethod = throw new IncorrectOperationException
  ...
  def createParameter(name: String, `type`: PsiType): PsiParameter = {
    ...
    ScalaPsiElementFactory.createParameterFromText(
      s"$name : ${ScType.canonicalText(scType)}",
      manager)
  }

  def createParameterList(
      names: Array[String],
      types: Array[PsiType]): PsiParameterList =
    throw new IncorrectOperationException

  def createMethodFromText(text: String, context: PsiElement): PsiMethod =
    throw new IncorrectOperationException

  def createAnnotationFromText(
      annotationText: String,
      context: PsiElement): PsiAnnotation =
    throw new IncorrectOperationException
  ...
  def createTypeParameter(
      name: String,
      superTypes: Array[PsiClassType]): PsiTypeParameter = ???
  ...
  def createType(
      ...
      languageLevel: LanguageLevel): PsiClassType = ???

  def createType(
      resolve: PsiClass,
      substitutor: PsiSubstitutor,
      languageLevel: LanguageLevel,
      annotations: Array[PsiAnnotation]): PsiClassType = ???
  ...
  def createSubstitutor(
      map: util.Map[PsiTypeParameter, PsiType]): PsiSubstitutor = ???

  def createPrimitiveType(text: String): PsiPrimitiveType = ???

  def createTypeByFQClassName(qName: String): PsiClassType = ???

  def createTypeByFQClassName(
      qName: String,
      resolveScope: GlobalSearchScope): PsiClassType = ???
...
object ScalaPsiElementFactory {
  ...
  def createClauseFromText(
      clauseText: String,
      manager: PsiManager): ScParameterClause = {
  ...
  def createImplicitClauseFromTextWithContext(
      clauseText: String,
      manager: PsiManager,
  ...
  def createImplicitClassParamClauseFromTextWithContext(
      clauseText: String,
      manager: PsiManager,
      context: PsiElement): ScParameterClause = {
    createElementWithContext(
  ...
  def createClassParamClausesWithContext(
      text: String,
  ...
  def createParamClausesWithContext(
      text: String,
      context: PsiElement,
  ...
  def createParameterFromText(
      paramText: String,
      manager: PsiManager): ScParameter = {
  ...
  def createClassParameterFromText(
      paramText: String,
      manager: PsiManager): ScParameter = {
  ...
  def createCaseClauseFromText(
      clauseText: String,
      manager: PsiManager): ScCaseClause = {
  ...
  def createTypeParameterFromText(
      name: String,
      manager: PsiManager): ScTypeParam = {
  ...
  def createMatch(
      ...
      manager: PsiManager): ScMatchStmt = {
  ...
  def createScalaFile(manager: PsiManager): ScalaFile =
    ScalaPsiElementFactory.parseFile("", manager)
  ...
  def parseFile(text: String, manager: PsiManager): ScalaFile = {
    ...
    factory.createFileFromText(name, ScalaFileType.SCALA_FILE_TYPE, text)
  ...
  def createOptionExpressionFromText(
      text: String,
      manager: PsiManager): Option[ScExpression] = {
    val dummyFile: ScalaFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
        ScalaFileType.SCALA_FILE_TYPE,
        text.trim).asInstanceOf[ScalaFile]
    val child = dummyFile.getFirstChild
    child match {
      case expr: ScExpression =>
        if (expr.getNextSibling == null && !PsiTreeUtil
              .hasErrorElements(dummyFile)) Some(expr)
        else None
      case _ => None
    }
  }
    ...
    try {
      ...
      case t: Throwable =>
        throw new IllegalArgumentException(
          s"Cannot create identifier from text $name")
    }
  }
  ...
  def createImportExprFromText(
      name: String,
      manager: PsiManager): ScImportExpr = {
  ...
  def createReferenceFromText(
      name: String,
      manager: PsiManager): ScStableCodeReferenceElement = {
    val text = "import " + name
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
    ...
    try {
      ...
      case t: Throwable =>
        throw new IllegalArgumentException(
          s"Cannot create reference with text $name")
    }
  }

  def createImportStatementFromClass(
  ...
  def createBigImportStmt(
      expr: ScImportExpr,
      exprs: Array[ScImportExpr],
      manager: PsiManager): ScImportStmt = {
    ...
    if ((names("_") ||
        ScalaCodeStyleSettings.getInstance(manager.getProject)
  ...
  def createStableReferenceElement(name: String, manager: PsiManager) = {
    ...
    (parents: @unchecked) match {
      case Some(p) =>
        ...
        (elements.head.asInstanceOf[ScSimpleTypeElement]
          ...
          val paramText =
            if (parSeq.size == 1) {
              ...
              if (par.typeElement.isDefined && par
                    .getPrevSiblingNotWhitespace == null) s"(${par.getText})"
  ...
  def createValFromVarDefinition(
      varDef: ScVariableDefinition,
      manager: PsiManager): ScValue = {
  ...
  def createVarFromValDeclaration(
      valDef: ScValue,
      manager: PsiManager): ScVariable = {
  ...
  def createNewLine(manager: PsiManager): PsiElement =
    createNewLineNode(manager, "\n").getPsi

  def createNewLine(manager: PsiManager, text: String): PsiElement =
    createNewLineNode(manager, text).getPsi
  ...
  def createBodyFromMember(
      element: PsiElement,
      manager: PsiManager): ScTemplateBody = {
    val text = "class a {\n" + element.getText + "}"
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
  ...
  def createClassTemplateParents(
      superName: String,
      manager: PsiManager): (PsiElement, ScTemplateParents) = {
    ...
    val extendToken = extendsBlock
      .findFirstChildByType(ScalaTokenTypes.kEXTENDS)
  ...
  private def addModifiersFromSignature(
      function: ScFunction,
      sign: PhysicalSignature,
      addOverride: Boolean): ScFunction = {
    sign.method match {
      case fun: ScFunction =>
        ...
        if (!fun.hasModifierProperty("override") && addOverride)
          function.setModifierProperty("override", value = true)
      case m: PsiMethod =>
          ...
          for (modifier <- m.getModifierList.getNode.getChildren(null);
            ...
            modText match {
              case "override" =>
                ...
                function.setModifierProperty("override", value = true)
              case "protected" =>
                function.setModifierProperty("protected", value = true)
              case "final" =>
                function.setModifierProperty("final", value = true)
              case _ =>
            }
          }
  ...
  private def methodFromSignatureText(
      sign: PhysicalSignature,
      body: String,
    ...
    method match {
        ...
        if (method.typeParameters.length > 0) {
          def buildText(typeParam: ScTypeParam): String = {
            ...
            val clauseText = typeParam.typeParametersClause match {
              case None => ""
              case Some(x) => x.typeParameters.map(buildText)
                  .mkString("[", ",", "]")
            }
            ...
            val boundsText = (lowerBoundText.toSeq ++ upperBoundText
              .toSeq ++ viewBoundText ++ contextBoundText).mkString
        ...
        val retAndBody = (needsInferType, retType) match {
          case (true, Some(scType)) =>
            ...
            val needWhitespace = method.paramClauses.clauses.isEmpty && method
              .typeParameters.isEmpty && ScalaNamesUtil
      ...
      case _ =>
        ...
        val retType = substitutor
          .subst(ScType.create(method.getReturnType, method.getProject))
        val retAndBody =
          if (needsInferType) {
            val typeText =
  ...
  def getOverrideImplementTypeSign(
      alias: ScTypeAlias,
      substitutor: ScSubstitutor,
      body: String,
  ...
  def getOverrideImplementVariableSign(
      variable: ScTypedDefinition,
      substitutor: ScSubstitutor,
      body: String,
      ...
      isVal: Boolean,
      needsInferType: Boolean): String = {
    val modOwner: ScModifierListOwner = ScalaPsiUtil
      .nameContext(variable) match {
      case m: ScModifierListOwner => m
      case _                      => null
    }
    val overrideText =
      if (needsOverride && (modOwner == null || !modOwner
            .hasModifierProperty("override"))) "override "
      else ""
    val modifiersText =
  ...
  private def getShortName(
    ...
    var i = 0
    while (i < qArray.length - 1 && i < pArray.length && qArray(i) == pArray(i))
      i = i + 1
    var res = ""
  ...
  def createDefinitionWithContext(
      text: String,
      context: PsiElement,
  ...
  def createTypeDefinitionWithContext(
      text: String,
      context: PsiElement,
  ...
  def createReferenceFromText(
      text: String,
      context: PsiElement,
      child: PsiElement): ScStableCodeReferenceElement = {
    createElementWithContext(
      text,
      context,
      child,
  ...
  def createExpressionWithContextFromText(
      text: String,
      context: PsiElement,
  ...
  def createConstructorBodyWithContextFromText(
      text: String,
      context: PsiElement,
      child: PsiElement): ScExpression = {
    createElementWithContext(
  ...
  def createElement(
    ...
    val context = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        ...
        ScalaFileType.SCALA_FILE_TYPE,
        "").asInstanceOf[ScalaFile]
    val holder: FileElement = DummyHolderFactory.createHolder(manager, context)
      .getTreeElement
  ...
  def createImportFromTextWithContext(
      text: String,
      context: PsiElement,
  ...
  def createTypeElementFromText(
      text: String,
      manager: PsiManager): ScTypeElement = {
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
        ScalaFileType.SCALA_FILE_TYPE,
        "var f: " + text).asInstanceOf[ScalaFile]
    try {
      val child = dummyFile.getLastChild.getLastChild
      if (child == null)
        throw new IncorrectOperationException(
          "wrong type element to parse: " + text)
  ...
  def createColon(manager: PsiManager): PsiElement = {
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
        ScalaFileType.SCALA_FILE_TYPE,
        "var f: Int").asInstanceOf[ScalaFile]
    dummyFile.getFirstChild.asInstanceOf[ScalaPsiElement]
      .findChildrenByType(ScalaTokenTypes.tCOLON).head
  }
  ...
  def createAssign(manager: PsiManager): PsiElement = {
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
        ScalaFileType.SCALA_FILE_TYPE,
        "val x = 0").asInstanceOf[ScalaFile]
  ...
  def createConstructorTypeElementFromText(
      text: String,
      context: PsiElement,
      child: PsiElement): ScTypeElement = {
    createElementWithContext(text, context, child, Constructor.parse(_)) match {
  ...
  def createExistentialClauseForName(
      name: String,
      manager: PsiManager): ScExistentialClause = {
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
        ScalaFileType.SCALA_FILE_TYPE,
        "val x: T forSome {type " + name + "}").asInstanceOf[ScalaFile]
  ...
  def createWildcardPattern(manager: PsiManager): ScWildcardPattern = {
    val dummyFile = PsiFileFactory.getInstance(manager.getProject)
      .createFileFromText(
        DUMMY + ScalaFileType.SCALA_FILE_TYPE.getDefaultExtension,
        ScalaFileType.SCALA_FILE_TYPE,
        "val _ = x").asInstanceOf[ScalaFile]
    dummyFile.getChildren.head.getChildren.apply(2).getChildren.head
      .asInstanceOf[ScWildcardPattern]
  }

  def createPatterListFromText(
      text: String,
      context: PsiElement,
  ...
  def createIdsListFromText(
      text: String,
      context: PsiElement,
      child: PsiElement): ScIdList = {
    val valDef = createDeclarationFromText(s"val $text : Int", context, child)
  ...
  def createTypeAliasDefinitionFromText(
      text: String,
      context: PsiElement,
  ...
  def createBlockCommentFromText(
      text: String,
      manager: PsiManager): PsiComment = {
    createScalaFile("/*" + text + "*/" + " class a { }", manager)
      .typeDefinitions(0).simpleComment.get
  }

  def createDocCommentFromText(
      text: String,
      manager: PsiManager): ScDocComment = {
    createScalaFile("/**\n" + text + "\n*/" + " class a { }", manager)
      .typeDefinitions(0).docComment.get
  }
  ...
  def createDocHeaderElement(length: Int, manager: PsiManager): PsiElement =
    createScalaFile(
      ...
      manager).typeDefinitions(0).docComment.get.getNode.getChildren(null)(1)
      .getLastChildNode.getPsi

  def createDocWhiteSpace(manager: PsiManager): PsiElement =
    createScalaFile("/**\n *\n*/ class a {}", manager).typeDefinitions(0)
      .docComment.get.getNode.getChildren(null)(1).getPsi

  def createLeadingAsterisk(manager: PsiManager): PsiElement =
    createScalaFile("/**\n *\n*/ class a {}", manager).typeDefinitions(0)
      .docComment.get.getNode.getChildren(null)(2).getPsi

  def createDocSimpleData(text: String, manager: PsiManager): PsiElement =
    createScalaFile("/**" + text + "*/ class a {}", manager).typeDefinitions(0)
      .docComment.get.getNode.getChildren(null)(1).getPsi

  def createDocTagValue(text: String, manager: PsiManager): PsiElement = {
    createScalaFile("/**@param " + text + "\n*/ class a{}", manager)
      .typeDefinitions(0).docComment.get.getNode.getChildren(null)(1)
      .getChildren(null)(2).getPsi
  }

  def createDocTag(text: String, manager: PsiManager): PsiElement =
    createScalaFile(s"/**$text*/ class a", manager).typeDefinitions(0)
      .docComment.get.getNode.getChildren(null)(1).getPsi

  def createDocTagName(name: String, manager: PsiManager): PsiElement = {
    createScalaFile("/**@" + name + " qwerty */", manager).typeDefinitions(0)
      .docComment.get.getNode.getChildren(null)(1).getChildren(null)(0).getPsi
  }

  def createDocLinkValue(
      text: String,
      manager: PsiManager): ScDocResolvableCodeReference = {
    createScalaFile("/**[[" + text + "]]*/ class a{}", manager)
      .typeDefinitions(0).docComment.get.getNode.getChildren(null)(1)
      .getChildren(null)(1).getPsi.asInstanceOf[ScDocResolvableCodeReference]
  }

  def createDocInnerCode(
      text: String,
      manager: PsiManager): ScDocInnerCodeElement = {
    createScalaFile("/**{{{" + text + "}}}\n*/\n class a{}", manager)
      .typeDefinitions(0).docComment.get.getNode.getChildren(null)(1).getPsi
      .asInstanceOf[ScDocInnerCodeElement]
  }

  def createXmlEndTag(tagName: String, manager: PsiManager): ScXmlEndTag = {
    createScalaFile("val a = <" + tagName + "></" + tagName + ">", manager)
      .getFirstChild.getLastChild.getFirstChild.getLastChild
      .asInstanceOf[ScXmlEndTag]
  }

  def createXmlStartTag(
      tagName: String,
      manager: PsiManager,
      attributes: String = ""): ScXmlStartTag = {
    createScalaFile(
      "val a = <" + tagName + attributes + "></" + tagName + ">",
      manager).getFirstChild.getLastChild.getFirstChild.getFirstChild
      .asInstanceOf[ScXmlStartTag]
  }

  def createInterpolatedStringPrefix(
      prefix: String,
      manager: PsiManager): PsiElement = {
  ...
  def createEquivQualifiedReference(
      postfix: ScPostfixExpr): ScReferenceExpression = {
    val operand = postfix.operand
    val operandText = operand.getText
    val qualRefText = s"($operandText).${postfix.operation.getText}"
    val expr = createExpressionWithContextFromText(
      qualRefText,
      postfix.getContext,
    ...
    val qualWithoutPars = createExpressionWithContextFromText(
      ...
      operand)
    expr.qualifier
      .foreach(_.replaceExpression(qualWithoutPars, removeParenthesis = true))
    expr
  }
}
