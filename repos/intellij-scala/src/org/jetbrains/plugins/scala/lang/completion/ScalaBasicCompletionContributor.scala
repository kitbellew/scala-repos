...
abstract class ScalaCompletionContributor extends CompletionContributor {
  def positionFromParameters(parameters: CompletionParameters) =
    ScalaCompletionUtil.positionFromParameters(parameters)
}
...
class ScalaBasicCompletionContributor extends ScalaCompletionContributor {
  ...
  extend(
    ...
    new CompletionProvider[CompletionParameters] {
      def addCompletions(
        ...
        val (position, inString, inInterpolatedString) =
          dummyPosition.getNode.getElementType match {
            case ScalaTokenTypes.tIDENTIFIER | ScalaDocTokenType
                  .DOC_TAG_VALUE_TOKEN                                       => (dummyPosition, false, false)
            ...
            case ScalaTokenTypes.tINTERPOLATED_STRING | ScalaTokenTypes
              ...
              val dummyInterpolated = parameters.getPosition.getParent
              ...
              //it's ok to use parameters here as we want just to calculate offset
              val offsetInString = offset - dummyInterpolated.getTextRange
                .getStartOffset
              ...
              val newInterpolated = ScalaPsiElementFactory
                .createExpressionFromText(
                  stringText.substring(0, exprStartInString) + "{" +
        ...
        val (expectedTypesAfterNew, isAfterNew) = ScalaAfterNewCompletionUtil
          .expectedTypesAfterNew(position, context)

        //if prefix is capitalized, class name completion is enabled
        val classNameCompletion = shouldRunClassNameCompletion(
        ...
        if (!inString && !inInterpolatedString && !ScalaPsiUtil
              .fileContext(insertedElement).isInstanceOf[ScalaFile]) return
        val lookingForAnnotations: Boolean =
          Option(
            insertedElement.getContainingFile findElementAt (insertedElement
              .getTextOffset - 1)) exists {
        ...
        position.getContext match {
          case ref: ScReferenceElement =>
                  ...
                  elem match {
                    case clazz: PsiClass =>
                      ...
                      val isExcluded: Boolean = ApplicationManager
                        .getApplication.runReadAction(new Computable[Boolean] {
                          def compute: Boolean = {
            ...
            def postProcessMethod(resolveResult: ScalaResolveResult) {
              ...
              val lookupItems: Seq[ScalaLookupItem] = LookupElementManager
                .getLookupElement(
                  resolveResult,
            ...
            ref match {
              case refImpl: ScStableCodeReferenceElementImpl =>
                refImpl.doResolve(refImpl, completionProcessor(refImpl))
              case refImpl: ScReferenceExpressionImpl =>
                refImpl.doResolve(
                  refImpl,
                  completionProcessor(refImpl, collectImplicit = true))
            ...
            val runtimeQualifierType: ScType = getQualifierCastType(
              ref,
              parameters)
            if (runtimeQualifierType != null) {
              def addElementWithDecorator(
                  elem: LookupElement,
                  decorator: InsertHandlerDecorator[LookupElement]) {
                if (!addedElements.contains(elem.getLookupString)) {
                  val newElem = LookupElementDecorator
                    .withInsertHandler(elem, decorator)
              ...
              def postProcess(resolveResult: ScalaResolveResult): Unit = {
                ...
                val decorator = castDecorator(
                  runtimeQualifierType.canonicalText)
                lookupItems.foreach(item =>
                  applyVariant(item, addElementWithDecorator(_, decorator)))
              }

              val newRef = createReferenceWithQualifierType(
        ...
        if (position.getNode.getElementType == ScalaDocTokenType
              .DOC_TAG_VALUE_TOKEN) result.stopHere()
      }
    }
  )
  ...
  override def beforeCompletion(context: CompletionInitializationContext) {
    ...
    context
      .setDummyIdentifier(ScalaCompletionUtil.getDummyIdentifier(offset, file))
    ...
    ref match {
      case refExpr: ScReferenceExpression =>
        (for (qualifier <- refExpr.qualifier) yield {
          val evaluator = refExpr.getContainingFile
            .getCopyableUserData(ScalaRuntimeTypeEvaluator.KEY)
  ...
  private def createReferenceWithQualifierType(
      qualType: ScType,
      context: PsiElement,
      child: PsiElement): ScReferenceElement = {
    val text = s"""|{
  ...
  private def castDecorator(canonTypeName: String) =
    new InsertHandlerDecorator[LookupElement] {
      def handleInsert(
        ...
        val file = PsiDocumentManager.getInstance(context.getProject)
          .getPsiFile(document)
        val ref: ScReferenceElement = PsiTreeUtil.findElementOfClassAtOffset(
          file,
...
object ScalaBasicCompletionContributor {
  def getStartEndPointForInterpolatedString(
      interpolated: ScInterpolated,
      index: Int,
      offsetInString: Int): Option[(Int, Int)] = {
      ...
      if (stringText.charAt(pointPosition) == '.') {
        ...
        if (lexer.getTokenType == ScalaTokenTypes.tIDENTIFIER) {
          ...
          if (endPoint >= offsetInString) {
            val exprStartInString = expr.getTextRange
