        ...
        if (isNamedParameter) {
          ...
          val tailText1 = tailText + params + (
            if (shouldImport && isClassName && containingClass != null)
              ...
              " in " + containingClass
                .name + " " + containingClass.getPresentation.getLocationString
            else
              ""
          )
          presentation.setTailText(tailText1)
    ...
    if (getInsertHandler != null)
      ...
      element match {
        ...
        case TypeToImport(cl) =>
          if (isInSimpleString) {
            val literal =
              context.getFile.findElementAt(context.getStartOffset).getParent
