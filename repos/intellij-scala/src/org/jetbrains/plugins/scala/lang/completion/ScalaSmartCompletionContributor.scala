...
class ScalaSmartCompletionContributor extends ScalaCompletionContributor {
      ...
      variant match {
        case el: ScalaLookupItem if isAccessible(el) =>
          ...
          def checkType(
              ...
              etaExpanded: Boolean = false): Boolean = {
          ...
          if (!el.isNamedParameterOrAssignment)
            elem match {
              case fun: ScSyntheticFunction =>
                val second = checkForSecondCompletion && fun
                  .paramClauses
                  .flatten
                  .isEmpty
                checkType(fun.retType, ScSubstitutor.empty, second)
              case fun: ScFunction =>
                if (fun.containingClass != null && fun
                      .containingClass
              ...
              case method: PsiMethod =>
                val second = checkForSecondCompletion && method
                  .getParameterList
  ...
  private def argumentsForFunction(
                ...
                case realPresentation: RealLookupElementPresentation =>
                  if (!realPresentation.hasEnoughSpaceFor(text, false)) {
                    ...
                    while (prefixIndex > 0 && !end) {
                      val prefix = ScalaCompletionUtil
                        .generateAnonymousFunctionText(
                          braceArgs,
  ...
  extend(
    ...
    new CompletionProvider[CompletionParameters] {
      def addCompletions(
          ...
          result: CompletionResultSet) {
        val element = positionFromParameters(parameters)
        val (referenceExpression, args) = extractReference[ScArgumentExprList](
          element)
  ...
  extend(
    ...
    new CompletionProvider[CompletionParameters] {
      def addCompletions(
          ...
          result: CompletionResultSet) {
        val element = positionFromParameters(parameters)
        val referenceExpression = element
          .getContext
  ...
  extend(
    ...
    new CompletionProvider[CompletionParameters] {
      def addCompletions(
          ...
          result: CompletionResultSet) {
        val element = positionFromParameters(parameters)
        val (referenceExpression, _) = extractReference[ScArgumentExprList](
          element)
        acceptTypes(
  ...
  extend(
    ...
    new CompletionProvider[CompletionParameters] {
      def addCompletions(
        ...
        val types: Array[ScType] = newExpr
          .expectedTypes()
          .map {
...
private[completion] object ScalaSmartCompletionContributor {
  def extractReference[T <: PsiElement](
      element: PsiElement): (ScReferenceExpression, T) = {
