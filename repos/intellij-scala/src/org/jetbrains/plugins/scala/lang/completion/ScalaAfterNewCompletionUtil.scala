...
object ScalaAfterNewCompletionUtil {
    ...
    while (iterator.hasNext) {
      ...
      if (predefinedType.conforms(typez)) {
        ...
        undef.getSubstitutor match {
          case Some(subst) =>
            val lookupElement = getLookupElementFromTypeAndClass(
              subst.subst(noUndefType),
              clazz,
              ScSubstitutor.empty,
  ...
  class AfterNewLookupElementRenderer(
      tp: ScType,
      psiClass: PsiClass,
      subst: ScSubstitutor)
      extends LookupElementRenderer[LookupElement] {
    def renderElement(
        ignore: LookupElement,
  ...
  private def getLookupElementFromTypeAndClass(
      tp: ScType,
      psiClass: PsiClass,
      subst: ScSubstitutor,
      renderer: (ScType, PsiClass, ScSubstitutor) =>
  ...
  def convertTypeToLookupElement(
      tp: ScType,
      place: PsiElement,
