...
class ScalaInplaceVariableIntroducer(
  ...
  protected override def getComponent: JComponent = {
    ...
    if (!isEnumerator) {
      ...
      myVarCheckbox.addActionListener(
        new ActionListener {
          def actionPerformed(e: ActionEvent): Unit = {
            val writeAction =
              new WriteCommandAction[Unit](
                myProject,
                getCommandName,
                getCommandName) {
    ...
    if (types.nonEmpty && forceInferType.isEmpty) {
      ...
      mySpecifyTypeChb.addActionListener(
        new ActionListener {
          def actionPerformed(e: ActionEvent): Unit = {
  ...
  override def finish(success: Boolean): Unit = {
    ...
    try {
      ...
      if (named != null && templateState != null) {
        val occurrences =
          (
            for (i <- 0 to templateState.getSegmentsCount - 1)
              yield templateState.getSegmentRange(i)
          ).toArray
