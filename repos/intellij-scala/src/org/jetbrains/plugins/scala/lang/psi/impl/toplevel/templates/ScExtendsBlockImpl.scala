...
class ScExtendsBlockImpl private (
    stub: StubElement[ScExtendsBlock],
  ...
  def templateBody: Option[ScTemplateBody] = {
    ...
    def lastChildTemplateBody: Option[ScTemplateBody] = {
      getLastChild match {
        case childTemplateBody: ScTemplateBody =>
          Some(childTemplateBody)
        case _ =>
          None
      }
    }
    ...
    Option(getStub) match {
      case Some(stub) =>
        childStubTemplate(stub)
      case _ =>
        lastChildTemplateBody
    }
  }
  ...
  def superTypes: List[ScType] = {
    ...
    if (!isScalaObject) {
      val obj = scalaObject
      if (obj != null && !obj.element.asInstanceOf[PsiClass].isDeprecated)
        buffer += obj
    }
    ...
    def extract(scType: ScType): Boolean = {
      ScType.extractClass(scType, Some(getProject)) match {
        ...
        case Some(c: ScClass) =>
          true
        case Some(c: PsiClass) if !c.isInterface =>
          true
        case _ =>
          false
      }
  ...
  def supers: Seq[PsiClass] = {
    ...
    buffer.find {
      ...
      case c: ScClass =>
        true
      case c: PsiClass if !c.isInterface =>
        true
      case _ =>
        false
    } match {
