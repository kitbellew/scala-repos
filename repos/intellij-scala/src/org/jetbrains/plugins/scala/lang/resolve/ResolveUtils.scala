...
object ResolveUtils {
  def kindMatches(
    ...
    kinds == null ||
      (element match {
        ...
        case _: PsiParameter =>
          kinds contains
            VAL //to enable named Parameters resolve in Play 2.0 routing file for java methods
        case _ =>
          false
      })
    ...
    ScFunctionType(
      ...
      m.getParameterList
        ...
        .map({ p =>
          ...
          if (pt.equalsToText("java.lang.Object"))
            types.Any
          else
            s.subst(ScType.create(pt, m.getProject, scope))
        })
        .toSeq
    )(m.getProject, scope)
  ...
  def isAccessible(
    ...
    if (forCompletion && place != null) {
      ...
      if (originalFile == member.getContainingFile) {
        val newPlace = originalFile
          .findElementAt(place.getTextRange.getStartOffset)
        place = newPlace
      }
    }
    ...
    def checkProtected(td: PsiClass, withCompanion: Boolean): Boolean = {
      ...
      if (isConstr) {
        if (placeTd != null && !placeTd.isInstanceOf[ScTypeDefinition] &&
            placeTd.extendsBlock.templateBody == None) {
          placeTd = getPlaceTd(placeTd)
        } else if (placeTd != null) {
          if (td != null && isInheritorOrSelfOrSame(placeTd, td))
            return true
        }
        while (placeTd != null) {
          if (td == placeTd)
          ...
          if (companion != null && companion == td)
            return true
          placeTd = getPlaceTd(placeTd)
        }
        return false
      ...
      while (placeTd != null) {
        ...
        if (withCompanion && companion != null && td != null &&
            ScalaPsiUtil.cachedDeepIsInheritor(companion, td))
          return true
        placeTd = getPlaceTd(placeTd)
      }
      false
                  ...
                  case ref: ScReferenceElement =>
                    ref.qualifier match {
                      ...
                      case Some(ref: ScReferenceElement) =>
                        ...
                        if (enclosing.extendsBlock.selfTypeElement ==
                              Some(resolve))
                          return true
                        else
                          return false
  ...
  private def isInheritorOrSelfOrSame(
      placeTd: ScTemplateDefinition,
      td: PsiClass): Boolean = {
    if (ScalaPsiUtil.cachedDeepIsInheritor(placeTd, td))
      return true
            ...
            def isInheritorOrSame(tp: ScType): Boolean = {
              ScType.extractClass(tp) match {
                case Some(clazz) =>
                  if (clazz == td)
                    return true
                  if (ScalaPsiUtil.cachedDeepIsInheritor(clazz, td))
                    return true
                case _ =>
              }
              false
  ...
  def packageContains(packageName: String, potentialChild: String): Boolean = {
    potentialChild == packageName ||
    potentialChild.startsWith(packageName + ".")
  }
    ...
    processor match {
      case b: BaseProcessor if b.isImplicitProcessor =>
        ...
        while (objectsIterator.hasNext) {
          ...
          if (!processor.execute(obj, state))
            return false
        }
        true
          ...
          try {
            ...
            if (base.getClassKindInner) {
              ...
              def calcForName(name: String): Boolean = {
                ...
                var classes: Array[PsiClass] = manager
            ...
            if (base.kinds.contains(ResolveTargets.PACKAGE)) {
              ...
              val subPackage =
                ScalaPsiManager
                  ...
                  .orNull
              if (subPackage != null) {
                if (!processor.execute(subPackage, state))
                  return false
              }
              true
            } else
