...
trait ScType {
  ...
  /**
    ...
    * val x: t = (y: this)
    */
  final def conforms(t: ScType, checkWeak: Boolean = false): Boolean =
    Conformance.conforms(t, this, checkWeak)

  final def weakConforms(t: ScType): Boolean =
    Conformance.conforms(t, this, checkWeak = true)
  ...
  def baseTypeSeqDepth: Int =
    1 //todo: should be implemented according to Scala compiler sources. However concerns about performance stops me.

  def isGenericType(project: Option[Project] = None): Boolean =
    ScType.extractClass(this, project).isEmpty
}
...
object ScType extends ScTypePresentation with ScTypePsiTypeBridge {
  ...
  def extractClassType(
    ...
    t match {
      ...
      case ScThisType(clazz) =>
        Some(clazz, new ScSubstitutor(t))
      case ScDesignatorType(clazz: PsiClass) =>
        Some(clazz, ScSubstitutor.empty)
      ...
      case ScExistentialType(quantified, _) =>
        extractClassType(quantified, project, visitedAlias)
      case p @ ScParameterizedType(t1, _) =>
        extractClassType(t1, project, visitedAlias) match {
          case Some((c, s)) =>
            Some((c, s.followed(p.substitutor)))
          case None =>
            None
        }
      case std @ StdType(_, _) =>
        val asClass = std
          .asClass(project.getOrElse(DecompilerUtil.obtainProject))
        if (asClass.isEmpty)
          return None
  ...
  /**
    ...
    */
  def extractDesignated(
      t: ScType,
      withoutAliases: Boolean): Option[(PsiNamedElement, ScSubstitutor)] =
    t match {
      case n: NonValueType =>
        ...
        proj.actualElement match {
          case t: ScTypeAliasDefinition if withoutAliases =>
            ...
            extractDesignated(
              ...
              withoutAliases)
          case _ =>
            Some((proj.actualElement, proj.actualSubst))
        }
      case p @ ScParameterizedType(t1, _) =>
        extractDesignated(t1, withoutAliases) match {
          case Some((e, s)) =>
            Some((e, s.followed(p.substitutor)))
          case None =>
            None
        }
      case std @ StdType(_, _) =>
  ...
  def isStable(t: ScType): Boolean =
    t match {
      ...
      case ScDesignatorType(o: ScObject) =>
        true
      case ScDesignatorType(r: ScTypedDefinition) if r.isStable =>
        true
      case _ =>
        false
    }
  ...
  def expandAliases(
    ...
    tp match {
      ...
      case ScDesignatorType(t: ScType) =>
        expandAliases(t, visited + tp)
      case ScDesignatorType(ta: ScTypeAliasDefinition) =>
        expandAliases(
  ...
  def removeAliasDefinitions(
    ...
    val res = tp.recursiveUpdate { t =>
      t.isAliasType match {
        case Some(AliasType(ta: ScTypeAliasDefinition, _, upper))
            if !expandableOnly || ScTypePresentation.shouldExpand(ta) =>
          updated = true
          (true, upper.getOrAny)
        case _ =>
          (false, t)
  ...
  def designator(element: PsiNamedElement): ScType = {
    element match {
      ...
      case _ =>
        ...
        clazzOpt match {
          case Some(clazz) =>
            ScProjectionType(ScThisType(clazz), element, superReference = false)
          case _ =>
    ...
    val baseType =
      named match {
        case p: ScPrimaryConstructor =>
          None
        case e: ScFunction if e.isConstructor =>
          None
        case e: ScFunction =>
          e.returnType.toOption
        case e: ScBindingPattern =>
          e.getType(TypingContext.empty).toOption
        case e: ScFieldId =>
          e.getType(TypingContext.empty).toOption
        case e: ScParameter =>
          e.getRealParameterType(TypingContext.empty).toOption
        case e: PsiMethod if e.isConstructor =>
          None
        case e: PsiMethod =>
          create(e.getReturnType, named.getProject, named.getResolveScope)
            .toOption
        case e: PsiVariable =>
