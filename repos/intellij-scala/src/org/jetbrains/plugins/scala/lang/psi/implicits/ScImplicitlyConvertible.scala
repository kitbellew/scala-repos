...
class ScImplicitlyConvertible(
  ...
  private def buildImplicitMap(
    ...
    if (isFromCompanion) {
      ...
      val expandedType: ScType = exp match {
        case Some(expected) =>
          ScFunctionType(expected, Seq(typez) ++ args)(
            place.getProject,
  ...
  @CachedMappedWithRecursionGuard(
    ...
    ArrayBuffer.empty,
    ModCount.getBlockModificationCount)
  private def buildSimpleImplicitMap(
      fromUnder: Boolean,
  ...
  def forMap(r: ScalaResolveResult, typez: ScType): ImplicitMapResult = {
    ...
    if (!PsiTreeUtil.isContextAncestor(
      ...
      lazy val funType =
        Option(ScalaPsiManager.instance(place.getProject).getCachedClass(
          "scala.Function1",
          place.getResolveScope,
      ...
      val (tp: ScType, retTp: ScType) = r.element match {
        ...
        case f: ScFunction =>
          Conformance.undefinedSubst(
            funType.getOrElse(return default),
            subst.subst(f.returnType.getOrElse(return default)))
        ...
        case b: ScBindingPattern =>
          Conformance.undefinedSubst(
            funType.getOrElse(return default),
            subst
              .subst(b.getType(TypingContext.empty).getOrElse(return default)))
            .getSubstitutor match {
            case Some(innerSubst) =>
              (
                innerSubst.subst(firstArgType.getOrElse(return default)),
        ...
        case param: ScParameter =>
          ...
          Conformance.undefinedSubst(
            funType.getOrElse(return default),
            subst.subst(
              param.getType(TypingContext.empty).getOrElse(return default)))
            .getSubstitutor match {
            case Some(innerSubst) =>
              (
                innerSubst.subst(firstArgType.getOrElse(return default)),
        ...
        case obj: ScObject =>
          Conformance.undefinedSubst(
            funType.getOrElse(return default),
            subst.subst(
              obj.getType(TypingContext.empty).getOrElse(return default)))
            .getSubstitutor match {
            case Some(innerSubst) =>
              (
                innerSubst.subst(firstArgType.getOrElse(return default)),
                ...
                uSubst.getSubstitutor(notNonable = false) match {
                  case Some(unSubst) =>
                    ...
                    val dependentSubst = new ScSubstitutor(() => {
                      ...
                      if (level >= Scala_2_10) {
                        f.paramClauses.clauses.headOption.map(_.parameters)
                          .toSeq.flatten.map {
                            case (param: ScParameter) =>
                    ...
                    val implicitDependentSubst = new ScSubstitutor(() => {
                      ...
                      if (level >= Scala_2_10) {
                        if (probablyHasDepententMethodTypes) {
                          val params: Seq[Parameter] = f.paramClauses.clauses
                            .last.effectiveParameters
                    ...
                    ImplicitMapResult(
                      ...
                      r,
                      tp,
                      dependentSubst.subst(retTp),
                      newSubst,
                      uSubst,
                      implicitDependentSubst)
                  case _ =>
                    ScalaPsiUtil.debug(
                      s"Implicit $r has problems with type parameters bounds for type $typez",
                      LOG)
                    ImplicitMapResult(
                      condition = false,
                      r,
  ...
  class CollectImplicitsProcessor(withoutPrecedence: Boolean)
    ...
    private val funType: ScType = {
      val funClass: PsiClass = ScalaPsiManager.instance(place.getProject)
        .getCachedClass(place.getResolveScope, "scala.Function1").orNull
      funClass match {
        case cl: ScTrait =>
          ScParameterizedType(
    ...
    def execute(element: PsiElement, state: ResolveState): Boolean = {
      ...
      element match {
        case named: PsiNamedElement if kindMatches(element) =>
          named match {
...
object ScImplicitlyConvertible {
  ...
  case class Implicit(
      tp: ScType,
      fun: ScTypedDefinition,
      importsUsed: Set[ImportUsed])
  ...
  case class ImplicitMapResult(
      condition: Boolean,
      resolveResult: ScalaResolveResult,
      tp: ScType,
      rt: ScType,
      newSubst: ScSubstitutor,
      subst: ScUndefinedSubstitutor,
      implicitDependentSubst: ScSubstitutor)

  case class ImplicitResolveResult(
      tp: ScType,
      element: PsiNamedElement,
      importUsed: Set[ImportUsed],
      subst: ScSubstitutor,
      implicitDependentSubst: ScSubstitutor,
      isFromCompanion: Boolean,
