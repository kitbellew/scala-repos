...
abstract class MixinNodes {
  ...
  class Map extends mutable.HashMap[String, ArrayBuffer[(T, Node)]] {
    ...
    private def forAll()
        : (mutable.HashMap[String, AllNodes], mutable.HashMap[String, AllNodes]) = {
  ...
  def build(tp: ScType, compoundThisType: Option[ScType] = None): Map = {
    ...
    val (superTypes, subst, thisTypeSubst): (
        Seq[ScType],
        ScSubstitutor,
...
object MixinNodes {
  ...
  private def generalLinearization(
