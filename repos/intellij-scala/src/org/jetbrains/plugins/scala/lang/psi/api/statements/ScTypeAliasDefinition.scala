...
trait ScTypeAliasDefinition extends ScTypeAlias {
  def aliasedTypeElement: ScTypeElement = {
    val stub = this.asInstanceOf[ScalaStubBasedElementImpl[_ <: PsiElement]]
      .getStub
    if (stub != null) {
  ...
  def isAliasFor(cls: PsiClass): Boolean = {
    if (cls.getTypeParameters.length != typeParameters.length) false
      ...
      val typeParamsAreAppliedInOrderToCorrectClass = aliasedType
        .getOrAny match {
          ...
          (typeParameters corresponds sc.typeParameters) {
            case (tp1, tp2) =>
              tp1.variance == tp2.variance && tp1.upperBound == tp2
                ...
                .viewBound.isEmpty && tp2.viewBound.isEmpty
          }
        case _ => // Java class
          (typeParameters corresponds cls.getTypeParameters) {
            case (tp1, tp2) =>
              tp1.variance == ScTypeParam.Invariant && tp1.upperTypeElement
                .isEmpty && tp2.getExtendsListTypes.isEmpty &&
                tp1.lowerTypeElement.isEmpty && tp1.contextBound.isEmpty && tp1
                .viewBound.isEmpty
          }
      }
