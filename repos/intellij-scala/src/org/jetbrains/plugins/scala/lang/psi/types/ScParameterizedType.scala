...
case class JavaArrayType(arg: ScType) extends ValueType {
  ...
  def getParameterizedType(
    ...
    val arrayClasses = ScalaPsiManager.instance(project)
...
class ScParameterizedType private (
  ...
  private def substitutorInner: ScSubstitutor = {
    def forParams[T](
        ...
        initial: ScSubstitutor,
        map: T => ScTypeParameterType): ScSubstitutor = {
  ...
  override def recursiveVarianceUpdateModifiable[T](
    ...
    update(this, variance, data) match {
      ...
      case (_, _, newData) =>
        val des = ScType
          .extractDesignated(designator, withoutAliases = false) match {
...
case class ScTypeParameterType(
    ...
    lower: Suspension[ScType],
    upper: Suspension[ScType],
    ...
    this(
        ...
        case _ =>
          new Suspension[ScType]({ () =>
            s.subst(ScCompoundType(
              ptp.getExtendsListTypes.map(ScType.create(_, ptp.getProject))
                .toSeq ++
                ptp.getImplementsListTypes.map(ScType.create(_, ptp.getProject))
                  .toSeq,
              Map.empty,
              Map.empty
            ))
      ...
      ptp match {
        ...
        case _ =>
          new Suspension[ScType]({ () =>
            s.subst(
              ScalaPsiManager.instance(ptp.getProject)
