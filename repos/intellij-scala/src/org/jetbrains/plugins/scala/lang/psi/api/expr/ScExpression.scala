...
trait ScExpression
  ...
  def getTypeAfterImplicitConversion(
    ...
    if (isShape) {
      ...
      val expected: ScType = expectedOption
        .getOrElse(expectedType(fromUnderscore).orNull)
      if (expected == null) {
        ExpressionTypeResult(
        ...
        def defaultResult: ExpressionTypeResult =
          ExpressionTypeResult(tr, Set.empty, None)
        if (!checkImplicits)
          defaultResult //do not try implicit conversions for shape check
        else {
          tr match {
                      ...
                      case function1: ScTrait =>
                        ScParameterizedType(
                          ...
                          case funTp: ScParameterizedType =>
                            ...
                            Conformance.undefinedSubst(funTp, paramType)
    ...
    inner match {
      case Success(rtp, _) =>
        ...
        def tryUpdateRes(checkExpectedType: Boolean) {
          ...
          val checkImplicitParameters = ScalaPsiUtil
            .withEtaExpansion(ScExpression.this)
        ...
        def removeMethodType(
          ...
          def updateRes(exp: Option[ScType]) {
            ...
            exp match {
              case Some(expected) => expected.removeAbstracts match {
            ...
            case Some(expected) =>
              ...
              if (check.isDefined) check.get
                ...
                def checkWidening(
                  ...
                  (l, r) match {
                    case (Byte, Short | Int | Long | Float | Double) => Some(
                        Success(expected, Some(ScExpression.this)))
                    case (Short, Int | Long | Float | Double) => Some(
                        Success(expected, Some(ScExpression.this)))
                    case (Char, Byte | Short | Int | Long | Float | Double) =>
                      Some(Success(expected, Some(ScExpression.this)))
                    case (Int, Long | Float | Double) => Some(
                        Success(expected, Some(ScExpression.this)))
                    case (Long, Float | Double) => Some(
                        Success(expected, Some(ScExpression.this)))
                    case (Float, Double) => Some(
                        Success(expected, Some(ScExpression.this)))
                    case _ => None
                  }
                }
  ...
  def getShape(ignoreAssign: Boolean = false): (ScType, String) = {
    this match {
      case assign: ScAssignStmt
        ...
        (
          assign.getRExpression.map(_.getShape(ignoreAssign = true)._1)
            .getOrElse(Nothing),
      ...
      case expr: ScExpression =>
        ScalaPsiUtil.isAnonymousExpression(expr) match {
          ...
          case (i, expr: ScFunctionExpr) =>
            (
              ScFunctionType(
                expr.result.map(_.getShape(ignoreAssign = true)._1)
                  .getOrElse(Nothing),
  ...
  @volatile
  protected var implicitParametersFromUnder: Option[Seq[ScalaResolveResult]] =
    None
  ...
  def findImplicitParameters: Option[Seq[ScalaResolveResult]] = {
    ...
    if (ScUnderScoreSectionUtil.underscores(this).nonEmpty) {
      getTypeWithoutImplicits(fromUnderscore =
        true
    ...
    if (fromUnderscore) innerType(TypingContext.empty)
      ...
      if (unders.isEmpty) innerType(TypingContext.empty)
        ...
        val params = unders.zipWithIndex.map {
          case (u, index) =>
            val tpe = u.getNonValueType(TypingContext.empty, ignoreBaseType)
              .getOrAny.inferValueType.unpackedType
  ...
  def replaceExpression(
    ...
    if (removeParenthesis && oldParent.isInstanceOf[ScParenthesisedExpr]) {
      return oldParent.asInstanceOf[ScExpression]
  ...
  def setAdditionalExpression(
      additionalExpression: Option[(ScExpression, ScType)]) {
  ...
  final def calculateReturns(
      withBooleanInfix: Boolean = false): Seq[PsiElement] = {
    ...
    def calculateReturns0(el: PsiElement) {
      el match {
        ...
        case block: ScBlock => block.lastExpr match {
            case Some(expr) => calculateReturns0(expr)
            case _          => res += block
          }
        case pe: ScParenthesisedExpr => pe.expr.foreach(calculateReturns0)
        case m: ScMatchStmt          => m.getBranches.foreach(calculateReturns0)
        case i: ScIfStmt => i.elseBranch match {
            case Some(e) =>
              calculateReturns0(e)
              i.thenBranch match {
...
object ScExpression {
  case class ExpressionTypeResult(
      tr: TypeResult[ScType],
      importsUsed: scala.collection.Set[ImportUsed],
      implicitFunction: Option[PsiNamedElement])

  object Type {
    def unapply(exp: ScExpression): Option[ScType] =
      exp.getType(TypingContext.empty).toOption
  }

  implicit class ScExpressionExt(val expr: ScExpression) extends AnyVal {
    def getNonValueType(
        ctx: TypingContext = TypingContext.empty, //todo: remove?
