...
abstract class ScalaIntroduceFieldHandlerBase extends RefactoringActionHandler {
  ...
  protected def isSuitableClass(
      elem: PsiElement,
      clazz: ScTemplateDefinition): Boolean

  def afterClassChoosing[T <: PsiElement](
      elem: T,
      types: Array[ScType],
      project: Project,
      editor: Editor,
  ...
  protected def anchorForNewDeclaration(
    ...
    val anchor = ScalaRefactoringUtil.statementsAndMembersInClass(aClass)
...
object ScalaIntroduceFieldHandlerBase {
  ...
  def canBeInitializedInDeclaration(
    ...
    (Iterator(expr) ++ expr.parents).find(stmtsAndMmbrs.contains(_))
  ...
  def anchorForInitializer(
      ...
      file: PsiFile): Option[PsiElement] = {
