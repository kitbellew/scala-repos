...
import org.jetbrains.plugins.scala.lang.resolve.processor.{
  CompoundTypeCheckSignatureProcessor,
  CompoundTypeCheckTypeAliasProcessor
}
...
object Conformance {
        ...
        case _ => argsPair match {
            ...
            case (l, ScAbstractType(tpt, lower, upper)) =>
              ...
              if (!addAbstract(upper, lower, left, alternateLeft))
                return (false, undefinedSubst)
            case (aliasType, _)
                if aliasType.isAliasType != None && aliasType.isAliasType.get.ta
  ...
  private class LeftConformanceVisitor(
      l: ScType,
      r: ScType,
      visited: Set[PsiClass],
      subst: ScUndefinedSubstitutor,
      checkWeak: Boolean = false)
      extends ScalaTypeVisitor {
    /*
    ...
    trait ParameterizedSkolemizeVisitor extends ScalaTypeVisitor {
      override def visitParameterizedType(p: ScParameterizedType) {
        p.designator match {
          case s: ScSkolemizedType => s.upper match {
      ...
      override def visitStdType(x: StdType) {
        if (x eq types.Nothing) result = (true, undefinedSubst)
          ...
          ScType.extractDesignated(l, withoutAliases = false) match {
            case Some((el, _)) =>
              val notNullClass = ScalaPsiManager.instance(el.getProject)
                .getCachedClass(
                  "scala.NotNull",
                  el.getResolveScope,
        ...
        result = l.isAliasType match {
          case Some(
                AliasType(
                  ...
                  _)) => conformsInner(comp, c, HashSet.empty, undefinedSubst)
          ...
          case _ => l match {
              case proj1: ScProjectionType
                ...
                result = conformsInner(
                  ...
                  visited,
                  undefinedSubst)
              case _ => proj2.actualElement match {
                  ...
                  case v: ScBindingPattern =>
                    ...
                    if (res.isEmpty) result = (false, undefinedSubst)
                      ...
                      result = conformsInner(
                        l,
                        proj2.actualSubst.subst(res.get),
                        visited,
                        undefinedSubst)
                  case v: ScParameter =>
                    val res = v.getType(TypingContext.empty)
    ...
    override def visitStdType(x: StdType) {
      ...
      if (checkWeak && r.isInstanceOf[ValType]) {
        (r, x) match {
          case (
                types.Byte,
                types.Short | types.Int | types.Long | types.Float | types
                  .Double) =>
          ...
          case (
                types.Char,
                types.Byte | types.Short | types.Int | types.Long | types
                  .Float | types.Double) =>
            result = (true, undefinedSubst)
            return
      ...
      if (x eq types.AnyVal) {
        result = (
          r.isInstanceOf[ValType] || ValueClassType.isValueType(r),
          undefinedSubst)
        return
      }
    ...
    override def visitProjectionType(proj: ScProjectionType) {
      ...
      r match {
        case proj1: ScProjectionType
            if ScEquivalenceUtil
              .smartEquivalence(proj1.actualElement, proj.actualElement) =>
          ...
          argsPair match {
            ...
            case (lt, u: ScUndefinedType) =>
              ...
              undefinedSubst = undefinedSubst
                .addUpper((u.tpt.name, u.tpt.getId), lt, variance = 0)
            case (tp, _)
                if tp.isAliasType != None && tp.isAliasType.get.ta
                  .isExistentialTypeAlias =>
              val y = Conformance.conformsInner(
                argsPair._1,
                argsPair._2,
    ...
    override def visitParameterizedType(p: ScParameterizedType) {
      ...
      p.designator match {
        case s: ScSkolemizedType => s.lower match {
            case ScParameterizedType(lower, _) =>
              result = conformsInner(
                lower,
                r,
        ...
        case ScDesignatorType(a: ScTypeAlias) =>
          r match {
            case ScParameterizedType(
                  ...
                  args2)
                if a.isInstanceOf[ScTypeAliasDeclaration] && (p
                  .designator equiv des2) =>
              processEquivalentDesignators(args2)
              return
            case _ =>
          }
            ...
            argsPair match {
              ...
              case (lt, u: ScUndefinedType) =>
                ...
                undefinedSubst = undefinedSubst
                  .addUpper((u.tpt.name, u.tpt.getId), lt, variance = 0)
              case (tp, _)
                  if tp.isAliasType.isDefined && tp.isAliasType.get.ta
                    .isExistentialTypeAlias =>
                val y = Conformance.conformsInner(
                  argsPair._1,
                  argsPair._2,
          ...
          (des1, des2) match {
            case (owner1: ScTypeParameterType, _: ScTypeParameterType) =>
              if (des1 equiv des2) {
                ...
                result = checkParameterizedType(
                  owner1.args.map(_.param).iterator,
                  args1,
                  args2,
                  undefinedSubst,
                  visited,
            ...
            case (proj1: ScProjectionType, proj2: ScProjectionType)
                if ScEquivalenceUtil
                  .smartEquivalence(proj1.actualElement, proj2.actualElement) =>
    ...
    override def visitExistentialType(e: ScExistentialType) {
      ...
      def updateType(t: ScType): ScType = {
        t.recursiveUpdate {
          case t: ScTypeVariable => e.wildcards.find(_.name == t.name) match {
              case Some(wild) =>
                val tpt = tptsMap.getOrElseUpdate(
                  wild.name,
                  ScTypeParameterType(
                    wild.name,
                    wild.args,
        ...
        unSubst.getSubstitutor(notNonable = false) match {
          case Some(uSubst) =>
            for (tpt <- tptsMap.values if result == null) {
              ...
              var t = conformsInner(
                substedTpt,
                uSubst.subst(updateType(tpt.lower.v)),
                immutable.Set.empty,
                undefinedSubst)
              ...
              undefinedSubst = t._2
              t = conformsInner(
                uSubst.subst(updateType(tpt.upper.v)),
                substedTpt,
                immutable.Set.empty,
                undefinedSubst)
              if (substedTpt != tpt && !t._1) {
        ...
        case t2: ScTypePolymorphicType =>
          ...
          val subst = new ScSubstitutor(
            new collection.immutable.HashMap[
              (String, PsiElement),
              ScType] ++ typeParameters1.zip(typeParameters2).map({ tuple =>
              (
  ...
  val cache: ConcurrentWeakHashMap[
    ...
    (Boolean, ScUndefinedSubstitutor)] = new ConcurrentWeakHashMap[
    (ScType, ScType, Boolean),
  ...
  private def smartIsInheritor(
      ...
      rightClass: PsiClass): (Boolean, ScType) = {
    if (ScEquivalenceUtil.areClassesEquivalent(leftClass, rightClass))
      return (false, null)
    if (!ScalaPsiUtil.cachedDeepIsInheritor(leftClass, rightClass))
      return (false, null)
    smartIsInheritor(
      leftClass,
      substitutor,
      ScEquivalenceUtil.areClassesEquivalent(_, rightClass),
  ...
  private def smartIsInheritor(
    ...
    val later: ArrayBuffer[(PsiClass, ScSubstitutor)] =
      new ArrayBuffer[(PsiClass, ScSubstitutor)]()
    var res: ScType = null
    while (iterator.hasNext) {
      ...
      ScType.extractClassType(tp) match {
        ...
        case Some((clazz: PsiClass, subst)) => later += ((clazz, subst))
        case _                              =>
      }
    }
