...
private[expr] object ExpectedTypes {
  ...
  def expectedExprTypes(
    ...
    def mapResolves(
      ...
      resolves.zip(types).map {
        case (r: ScalaResolveResult, tp) =>
          val isNamedDynamic = r.isDynamic &&
            r.name == ResolvableReferenceExpression.APPLY_DYNAMIC_NAMED
          (tp, isNamedDynamic)
        case (_, tp) => (tp, false)
      }
    }
    ...
    val result: Array[(ScType, Option[ScTypeElement])] = expr.getContext match {
      ...
      //see SLS[6.16]
      case cond: ScIfStmt
          if cond.condition.getOrElse(null: ScExpression) ==
            expr.getSameElementInContext => Array((types.Boolean, None))
      ...
      case tb: ScTryBlock => tb.lastExpr match {
          case Some(e) if e == expr =>
            tb.getContext.asInstanceOf[ScTryStmt]
              .expectedTypesEx(fromUnderscore = true)
          case _ => Array.empty
        }
      case wh: ScWhileStmt
          if wh.condition.getOrElse(null: ScExpression) ==
            expr.getSameElementInContext => Array((types.Boolean, None))
      case wh: ScWhileStmt               => Array((types.Unit, None))
      case d: ScDoStmt
          if d.condition.getOrElse(null: ScExpression) ==
            expr.getSameElementInContext => Array((types.Boolean, None))
      ...
      case te: ScThrowStmt               =>
        ...
        val throwableClass = ScalaPsiManager.instance(te.getProject)
          .getCachedClass(te.getResolveScope, "java.lang.Throwable")
        val throwableType = throwableClass.map(new ScDesignatorType(_))
          .getOrElse(Any)
      ...
      case c: ScCaseClause => c.getContext.getContext match {
          ...
          case b: ScBlockExpr if b.isInCatchBlock =>
            b.getContext.getContext.asInstanceOf[ScTryStmt]
              .expectedTypesEx(fromUnderscore = true)
      ...
      //see SLS[6.23]
      case f: ScFunctionExpr => f.expectedTypesEx(fromUnderscore = true)
          .flatMap(tp => fromFunction(tp))
      ...
      //SLS[6.15]
      case a: ScAssignStmt
          if a.getRExpression.getOrElse(null: ScExpression) ==
            expr.getSameElementInContext =>
        a.getLExpression match {
          case ref: ScReferenceExpression
              if (!a.getContext.isInstanceOf[ScArgumentExprList] && !(a
                .getContext.isInstanceOf[ScInfixArgumentExpression] &&
                a.getContext.asInstanceOf[ScInfixArgumentExpression].isCall)) ||
                ref.qualifier.isDefined || ScUnderScoreSectionUtil.isUnderscore(
                  expr
              ...
              case Some(
                ...
                ScalaPsiUtil.nameContext(named) match {
                  case v: ScValue =>
                    Array((
                      subst.subst(
                        named.asInstanceOf[ScTypedDefinition]
                          .getType(TypingContext.empty).getOrAny),
                      v.typeElement))
                  case v: ScVariable =>
                    Array((
                      subst.subst(
      ...
      case tuple: ScTuple if tuple.isCall =>
        ...
        val callExpression = tuple.getContext.asInstanceOf[ScInfixExpr]
          .operation
      ...
      case ret: ScReturnStmt =>
        val fun: ScFunction = PsiTreeUtil
          .getContextOfType(ret, true, classOf[ScFunction])
          ...
          var tps: Array[(TypeResult[ScType], Boolean)] = callExpression match {
            ...
            case gen: ScGenericCall =>
              if (!withResolvedFunction) {
                ...
                gen.shapeMultiResolve.map(mapResolves(_, multiType))
                  .getOrElse(multiType.map((_, false)))
              } else {
                val multiType = gen.multiType
                gen.multiResolve.map(mapResolves(_, multiType))
                  .getOrElse(multiType.map((_, false)))
              }
            case _ => Array(
                (callExpression.getNonValueType(TypingContext.empty), false))
      ...
      case b: ScBlock
          if b.getContext.isInstanceOf[ScTryBlock] ||
            ...
            b.getContext.isInstanceOf[ScFunctionExpr] =>
        b.lastExpr match {
  ...
  private def processArgsExpected(
      res: ArrayBuffer[(ScType, Option[ScTypeElement])],
      expr: ScExpression,
      i: Int,
      tp: TypeResult[ScType],
    ...
    def applyForParams(params: Seq[Parameter]) {
      ...
      expr match {
                    ...
                    (
                      ...
                      te.map {
                        case t: ScTupleTypeElement
                            if t.components.length == 2 => t.components(1)
                        case t                          => t
                      }))
              case _ => res += p
            }
        ...
        case typedStmt: ScTypedStmt
            if typedStmt.isSequenceArg && params.nonEmpty =>
          val seqClass: Array[PsiClass] = ScalaPsiManager
    ...
    tp match {
      ...
      case Success(
        ...
        val newParams = params
          .map(p => p.copy(paramType = subst.subst(p.paramType)))
        if (newParams.length == 1 && !newParams.head.isRepeated &&
            exprs.length > 1) {
