...
object getDummyBlocks {
  ...
  private val alignmentsMap = scala
    .collection
    .mutable
  ...
  private def applyInner(
    ...
    node.getPsi match {
      ...
      case _: ScIfStmt =>
        val alignment =
          if (scalaSettings.ALIGN_IF_ELSE)
      ...
      case pack: ScPackaging if pack.isExplicit =>
        ...
        val hasValidTail = afterOpenBrace
      ...
      case _ if node.getElementType == ScalaDocElementTypes.DOC_TAG =>
        ...
        childBlocks match {
          case tagName :: space :: tagParameter :: tail
              if Option(docTag.getValueElement)
                .map(_.getNode)
            ...
            subBlocks.add(
              getSubBlock(
                ...
                if (tail.isEmpty)
                  ...
                  tail.last))
          case tagName :: tail
              if Option(docTag.getNameElement)
                .map(_.getNode)
                .exists(_ == tagName) =>
            subBlocks.add(getSubBlock(block, scalaSettings, tagName))
            if (tail.nonEmpty) {
      ...
      case interpolated: ScInterpolatedStringLiteral =>
        ...
        alignmentsMap.put(
          SmartPointerManager
            .getInstance(interpolated.getProject)
          ...
          case _: ScMethodCall | _: ScReferenceExpression =>
            if (child.getElementType == ScalaTokenTypes.tIDENTIFIER &&
                ...
                child
                  ...
                  .isEmpty)
              null
            else if (child.getPsi.isInstanceOf[ScExpression])
              null
            else
              alignment
                ...
                firstSibling.getText match {
                  case "@param" | "@tparam" =>
                    if (scalaSettings.SD_ALIGN_PARAMETERS_COMMENTS)
                      ...
                      null
                  case "@return" =>
                    if (scalaSettings.SD_ALIGN_RETURN_COMMENTS)
                      normalAlignment
                    else
                      null
                  case "@throws" =>
  ...
  private def getExtendsSubBlocks(
    ...
    if (extBlock.getFirstChild == null)
      return subBlocks
  ...
  private def getIfSubBlocks(
    ...
    while (child.getTreeNext != null && child
             .getTreeNext
  ...
  private def getMultilineStringBlocks(
    ...
    val validAlignment = Option(
      ScalaPsiUtil
        .getParentOfType(node.getPsi, classOf[ScInterpolatedStringLiteral]))
      ...
      if (trimmedLine.startsWith(marginChar)) {
        ...
        if (line.length > linePrefixLength + 2 && line
            ...
            line.length > linePrefixLength + 1 && line
              .charAt(linePrefixLength + 1) != ' ') {
          val suffixOffset =
            if (line.charAt(linePrefixLength + 1) == ' ')
              2
            else
              1
  ...
  def getMethodCallOrRefExprSubBlocks(
    ...
    children match {
      ...
      case caller :: args :: Nil
        ...
        subBlocks.addAll(
          getMethodCallOrRefExprSubBlocks(
            caller,
            block,
            parentAlignment,
