...
object getDummyBlocks {
  val fieldGroupAlignmentKey: Key[Alignment] = Key
    .create("field.group.alignment.key")
  private val alignmentsMap = scala.collection.mutable
    .Map[SmartPsiElementPointer[ScInterpolatedStringLiteral], Alignment]()
  ...
  private def applyInner(
    ...
    val scalaSettings = block.getSettings
    ...
    node.getPsi match {
      ...
      case _: ScLiteral
          if node.getFirstChildNode != null &&
            node.getFirstChildNode.getElementType == ScalaTokenTypes
              .tMULTILINE_STRING &&
            scalaSettings.MULTILINE_STRING_SUPORT != ScalaCodeStyleSettings
              .MULTILINE_STRING_NONE =>
        subBlocks.addAll(getMultilineStringBlocks(node, block))
        return subBlocks
      case _: ScTryBlock
          if children.headOption
            .exists(_.getElementType == ScalaTokenTypes.kTRY) =>
        //add try block
      ...
      case pack: ScPackaging if pack.isExplicit =>
        ...
        val (beforeOpenBrace, afterOpenBrace) = correctChildren
          .span(_.getElementType != ScalaTokenTypes.tLBRACE)
        val hasValidTail = afterOpenBrace.nonEmpty && afterOpenBrace.head
          ...
          val context =
            (child.getElementType match {
              case ScalaDocElementTypes.DOC_TAG =>
                val currentTag = Option(child.getFirstChildNode)
                  .filter(_.getElementType == ScalaDocTokenType.DOC_TAG_NAME)
                  .map(_.getText)
                if (scalaDocPrevChildTag
                      .isEmpty || scalaDocPrevChildTag != currentTag) {
      ...
      case _ if node.getElementType == ScalaDocElementTypes.DOC_TAG =>
        ...
        childBlocks match {
          case tagName :: space :: tagParameter :: tail
              if Option(docTag.getValueElement).map(_.getNode)
                .exists(_ == tagParameter) =>
            ...
            subBlocks.add(getSubBlock(
              ...
              scalaSettings,
              tagParameter,
              if (tail.isEmpty) null else tail.last))
          case tagName :: tail
              if Option(docTag.getNameElement).map(_.getNode)
                .exists(_ == tagName) =>
            subBlocks.add(getSubBlock(block, scalaSettings, tagName))
            if (tail.nonEmpty) {
              if (tail.head.getElementType != ScalaDocTokenType
                    .DOC_COMMENT_LEADING_ASTERISKS)
                subBlocks
                  .add(getSubBlock(block, scalaSettings, tail.head, tail.last))
              else
                for (child <- tail) {
      ...
      case interpolated: ScInterpolatedStringLiteral =>
        ...
        alignmentsMap.put(
          SmartPointerManager.getInstance(interpolated.getProject)
            .createSmartPsiElementPointer(interpolated),
    ...
    for (child <- children if isCorrectBlock(child)) {
      ...
      val childAlignment: Alignment = {
        node.getPsi match {
          case _: ScParameterClause => child.getElementType match {
              case ScalaTokenTypes.tRPARENTHESIS | ScalaTokenTypes
                    .tLPARENTHESIS => null
              case _               => alignment
            }
          ...
          case _: ScMethodCall | _: ScReferenceExpression =>
            if (child.getElementType == ScalaTokenTypes.tIDENTIFIER &&
                ...
                child.getPsi.getParent.asInstanceOf[ScReferenceExpression]
          ...
          case _: ScXmlElement => child.getElementType match {
              case ScalaElementTypes.XML_START_TAG | ScalaElementTypes
                    .XML_END_TAG => alignment
              case _             => null
            }
          case _ => alignment
        }
      ...
      val childWrap = arrangeSuggestedWrapForChild(
        ...
        scalaSettings,
        block.suggestedWrap)
      if (child.getFirstChildNode == null && child
    ...
    def flattenChildren(multilineNode: ASTNode, buffer: ArrayBuffer[ASTNode]) {
      for (nodeChild <- multilineNode.getChildren(null)) {
        if (nodeChild.getText.contains("\n") && nodeChild
              .getFirstChildNode != null) { flattenChildren(nodeChild, buffer) }
        else { buffer += nodeChild }
      }
    }

    if (ScalaDocTokenType.ALL_SCALADOC_TOKENS.contains(node.getElementType) ||
        (node.getTreeParent != null && node.getTreeParent
          .getElementType == ScalaDocElementTypes.DOC_TAG &&
      ...
      children.foreach { child =>
        ...
        if (isCorrectBlock(child)) {
          ...
          val (childAlignment, childWrap) =
            if (node.getTreeParent.getElementType == ScalaDocElementTypes
                  .DOC_TAG &&
                child.getElementType != ScalaDocTokenType.DOC_WHITESPACE &&
                child.getElementType != ScalaDocTokenType
                  .DOC_COMMENT_LEADING_ASTERISKS &&
                child != firstSibling &&
      ...
      do {
        ...
        if (isCorrectBlock(child) && !child.getPsi
          ...
          subBlocks.add(new ScalaBlock(
            block,
            child,
            block.getChildBlockLastNode(child),
            childAlignment,
            indent,
            childWrap,
            block.getSettings,
            block.subBlocksContext
              .flatMap(_.childrenAdditionalContexts.get(child))))
        } else if (isCorrectBlock(child)) {
          subBlocks.addAll(getTemplateParentsBlocks(child, block))
        }
      } while (
        child != lastNode && { child = child.getTreeNext; child != null }
      )
    }
    ...
    for (child <- children if isCorrectBlock(child)) {
      def getPrevGroupNode(node: ASTNode): ASTNode = {
        ...
        def isOk(psi: PsiElement): Boolean = {
          if (psi.isInstanceOf[PsiWhiteSpace] || ScalaPsiUtil
                .isLineTerminator(psi)) {
          ...
          psi match {
            ...
            case _: ScVariable | _: ScValue =>
              val hasMod1 = psi.isInstanceOf[ScModifierListOwner] &&
                psi.asInstanceOf[ScModifierListOwner].getModifierList
                  .getText == ""
              val hasMod2 = node.getPsi.isInstanceOf[ScModifierListOwner] &&
                node.getPsi.asInstanceOf[ScModifierListOwner].getModifierList
                  .getText == ""
              if (hasMod1 != hasMod2) {
                breaks += 2
                false
  ...
  private def getForSubBlocks(
      node: ASTNode,
      block: ScalaBlock,
      children: Array[ASTNode]): util.ArrayList[Block] = {
    var prevChild: ASTNode = null
    val subBlocks = new util.ArrayList[Block]()
    val scalaSettings = block.getSettings
      .getCustomSettings(classOf[ScalaCodeStyleSettings])
    ...
    def addTail(tail: List[ASTNode]): Unit = {
      for (child <- tail) {
        if (child.getElementType != ScalaTokenTypes.kYIELD) {
          if (prevChild != null && prevChild.getElementType == ScalaTokenTypes
                .kYIELD) { addSubBlock(prevChild, child) }
  ...
  private def getIfSubBlocks(
      node: ASTNode,
      block: ScalaBlock,
      alignment: Alignment): util.ArrayList[Block] = {
    val settings = block.getCommonSettings
    val scalaSettings = block.getSettings
      .getCustomSettings(classOf[ScalaCodeStyleSettings])
    val subBlocks = new util.ArrayList[Block]
    ...
    while (child.getTreeNext != null && child.getTreeNext
        ...
        child.getTreeNext.getPsi match {
          case _: ScIfStmt if settings.SPECIAL_ELSE_IF_TREATMENT =>
            ...
            subBlocks.add(new ScalaBlock(
              ...
              block.getSettings))
            subBlocks
              .addAll(getIfSubBlocks(child.getTreeNext, block, alignment))
          case _ =>
        }
        child = child.getTreeNext
      }
  ...
  private def getMultilineStringBlocks(
    ...
    val validAlignment = Option(ScalaPsiUtil.getParentOfType(
      ...
      .map(_.asInstanceOf[ScInterpolatedStringLiteral]).flatMap(literal =>
        alignmentsMap.find {
          case (pointer, _) => pointer.getElement == literal
    ...
    val prefixIndent = Indent.getSpaceIndent(
      marginIndent +
        (if (node.getElementType == ScalaTokenTypes
               .tINTERPOLATED_MULTILINE_STRING) 1
         else 0),
      true)
      ...
      val linePrefixLength =
        if (settings useTabCharacter ScalaFileType.SCALA_FILE_TYPE) {
          ...
          tabsCount /* *settings.getTabSize(ScalaFileType.SCALA_FILE_TYPE)*/ + line
            .substring(tabsCount).prefixLength(_ == ' ')
        } else { line.prefixLength(_ == ' ') }

      if (trimmedLine.startsWith(marginChar)) {
        subBlocks.add(new StringLineScalaBlock(
          new TextRange(
            node.getStartOffset + acc + linePrefixLength,
            node.getStartOffset + acc + linePrefixLength + 1),
          node,
          block,
          validAlignment,
          prefixIndent,
          null,
          settings))
        if (line.length > linePrefixLength + 2 && line
              .charAt(linePrefixLength + 1) == ' ' ||
            line.length > linePrefixLength + 1 && line
              .charAt(linePrefixLength + 1) != ' ') {
          val suffixOffset =
          ...
          subBlocks.add(new StringLineScalaBlock(
            new TextRange(
              node.getStartOffset + acc + linePrefixLength + suffixOffset,
        ...
        val (startOffset, endOffset, myIndent, myAlignment) =
          if (trimmedLine.startsWith("\"\"\"") && acc != 0)
            (
              node.getStartOffset + acc + linePrefixLength,
              node.getStartOffset + acc + line.length,
            ...
            if (Option(node.getTreePrev).exists(
                  _.getElementType == ScalaElementTypes
              ...
              subBlocks.add(new StringLineScalaBlock(
                new TextRange(node.getStartOffset, node.getStartOffset + 3),
                node,
                block,
                alignment,
                Indent.getNoneIndent,
              ...
              (
                node.getStartOffset + 3,
                node.getStartOffset + line.length,
                Indent.getNoneIndent,
                validAlignment)
            } else
              (
      ...
      def checkSamePriority: Boolean = {
        ...
        val childPriority = child.getPsi match {
          case inf: ScInfixExpr =>
            priority(inf.operation.getText, assignments = true)
  ...
  def getMethodCallOrRefExprSubBlocks(
      node: ASTNode,
      block: ScalaBlock,
    ...
    children match {
      ...
      case expr :: dot :: id :: Nil
        ...
        addSubBlock(
          dot,
          (id :: delegatedChildren).sortBy(_.getTextRange.getStartOffset)
            .lastOption.orNull,
          Some(SubBlocksContext(id, alignment, delegatedChildren)))
      case expr :: typeArgs :: Nil
          if typeArgs.getPsi.isInstanceOf[ScTypeArgs] =>
        addSubBlock(
          expr,
          (typeArgs :: delegatedChildren).sortBy(_.getTextRange.getStartOffset)
            .lastOption.orNull,
      ...
      case expr :: Nil =>
        addSubBlock(
          expr,
          delegatedChildren.sortBy(_.getTextRange.getStartOffset).lastOption
            .orNull,
  ...
  private val INFIX_ELEMENTS = TokenSet.create(
  ...
  private class StringLineScalaBlock(
    ...
    override def getSpacing(child1: Block, child2: Block) =
