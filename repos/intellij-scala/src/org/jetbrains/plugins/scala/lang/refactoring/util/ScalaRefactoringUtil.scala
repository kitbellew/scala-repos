...
object ScalaRefactoringUtil {
  ...
  def getExprOccurrences(
    ...
    val occurrences: ArrayBuffer[ScExpression] =
      new ArrayBuffer[ScExpression]()
    ...
    parent match {
      ...
      case lit: ScLiteral =>
        ...
        if (replaceAsInjection) {
          ...
          val needBraces =
            ScalaNamesUtil.isIdentifier(withNextChar) &&
              withNextChar.last != '$'
  ...
  def findParentExpr(elem: PsiElement): ScExpression = {
    ...
    val expr =
      interpolated getOrElse PsiTreeUtil.getParentOfType(
        elem,
        classOf[ScExpression],
        false)
