...
object ScalaRefactoringUtil {
  ...
  def getTypeParameterOwnerList(
    ...
    val ownersArray: ArrayBuffer[ScTypeParametersOwner] =
      new ArrayBuffer[ScTypeParametersOwner]()
    typeElement
      .breadthFirst
      .foreach {
  ...
  def getMinOwner(
      ownres: Array[ScTypeParametersOwner],
  ...
  def getExpression(
      ...
      file: PsiFile,
      startOffset: Int,
      endOffset: Int): Option[(ScExpression, Array[ScType])] = {
    ...
    def partOfStringLiteral(): Option[(ScExpression, Array[ScType])] = {
      ...
      if (lit == null || !lit.isString || lit != endLit)
        return None
      ...
      val expr = ScalaPsiElementFactory
        .createExpressionWithContextFromText(text, lit.getContext, lit)
  ...
  def getOccurrenceRanges(
      element: PsiElement,
      enclosingContainer: PsiElement): Array[TextRange] = {
    def collectOccurrencesInLiteral(
        literal: ScLiteral,
        text: String,
        result: ArrayBuffer[TextRange]) {
      if (text.isEmpty)
        return
    ...
    def getTextOccurrenceInLiterals(
        text: String,
    ...
    element match {
      case intrp: ScInterpolatedStringLiteral =>
        ...
        intrp
          ...
          .foreach {
            case ref: ScReferenceExpression =>
              refNameToResolved += ((ref.refName, ref.resolve()))
            case _ =>
          }
      ...
      case lit: ScLiteral if lit.isString =>
        ...
        getTextOccurrenceInLiterals(text, enclosingContainer, filter)
      case _ =>
        getExprOccurrences(element, enclosingContainer).map(_.getTextRange)
    }
  }

  def getExprOccurrences(
      element: PsiElement,
      enclosingContainer: PsiElement): Array[ScExpression] = {
  ...
  def getTypeElementOccurrences(
      element: ScTypeElement,
      enclosingContainer: PsiElement): Array[ScTypeElement] = {
    val occurrences: ArrayBuffer[ScTypeElement] =
      new ArrayBuffer[ScTypeElement]()
    if (enclosingContainer == element)
  ...
  def getOccurrencesInInheritors(
    ...
    val collectedOccurrences: mutable.MutableList[Array[ScTypeElement]] =
      mutable.MutableList()
    val collectedValidators: mutable.MutableList[ScalaTypeValidator] = mutable
      .MutableList()
  ...
  def getCompatibleTypeNames(
      myTypes: Array[ScType]): util.LinkedHashMap[String, ScType] = {
  ...
  def showChooser[T <: PsiElement](
      ...
      toHighlight: T => PsiElement = (t: T) => t.asInstanceOf[PsiElement]) {
    class Selection {
  ...
  def getShortText(expr: ScalaPsiElement): String = {
    ...
    expr match {
      ...
      case m: ScMethodCall =>
        ...
        if (m.argumentExpressions.isEmpty)
          builder.append("()")
        else
          builder.append("(...)")
      ...
      case t: ScTryStmt =>
        builder.append("try {...}")
        if (t.catchBlock.isDefined)
          builder.append(" catch {...}")
        if (t.finallyBlock.isDefined)
  ...
  private[refactoring] def getLineText(editor: Editor): String = {
    val lineNumber = Option(
      editor.getSelectionModel.getSelectionEndPosition.getLine)
      .getOrElse(editor.getCaretModel.getLogicalPosition.line)
    if (lineNumber >= editor.getDocument.getLineCount)
      return ""
    val lineStart = editor
      .visualToLogicalPosition(new VisualPosition(lineNumber, 0))
    val nextLineStart = editor
      .visualToLogicalPosition(new VisualPosition(lineNumber + 1, 0))
  ...
  def afterExpressionChoosing(
      project: Project,
      editor: Editor,
      file: PsiFile,
      dataContext: DataContext,
      refactoringName: String,
      exprFilter: (ScExpression) => Boolean = e => true)(invokesNext: => Unit) {
  ...
  def afterTypeElementChoosing(
      project: Project,
      editor: Editor,
      file: PsiFile,
      dataContext: DataContext,
      currentSelectedElement: ScTypeElement,
    ...
    if (!editor.getSelectionModel.hasSelection) {
      ...
      def chooseTypeElement(typeElement: ScTypeElement) {
        editor
          .getSelectionModel
          .setSelection(
            typeElement.getTextRange.getStartOffset,
  ...
  def fileEncloser(startOffset: Int, file: PsiFile): PsiElement = {
    if (file.asInstanceOf[ScalaFile].isScriptFile())
      file
    else {
  ...
  def isLiteralPattern(file: PsiFile, textRange: TextRange): Boolean = {
    ...
    val literalPattern = PsiTreeUtil
      .getParentOfType(parent, classOf[ScLiteralPattern])
  ...
  /**
    ...
    */
  def showErrorMessageWithException(
      text: String,
      project: Project,
      editor: Editor,
  ...
  def checkCanBeIntroduced(
      expr: ScExpression,
      action: (String) => Unit = s => {}): Boolean = {
    ...
    if (guard != null && guard.getParent.isInstanceOf[ScCaseClause])
      errorMessage = ScalaBundle
        .message("refactoring.is.not.supported.in.guard")
    ...
    if (errorMessage == null)
      errorMessage = expr.getParent match {
        case inf: ScInfixExpr if inf.operation == expr =>
          ScalaBundle.message("cannot.refactor.not.expression")
        case post: ScPostfixExpr if post.operation == expr =>
          ScalaBundle.message("cannot.refactor.not.expression")
        case _: ScGenericCall =>
          ScalaBundle.message("cannot.refactor.under.generic.call")
        case _ if expr.isInstanceOf[ScConstrExpr] =>
          ScalaBundle.message("cannot.refactor.constr.expression")
        case _: ScArgumentExprList if expr.isInstanceOf[ScAssignStmt] =>
  ...
  def replaceOccurence(
      textRange: TextRange,
      newString: String,
      file: PsiFile): RangeMarker = {
    ...
    parent match {
      case null =>
      case ChildOf(pars @ ScParenthesisedExpr(inner))
          if !ScalaPsiUtil.needParentheses(pars, inner) =>
        ...
        val afterWord = textRange.getStartOffset > 0 && {
          ...
          ScalaTokenTypes
            ...
            .contains(prevElemType) || ScalaTokenTypes
            .KEYWORDS
            .contains(prevElemType)
        }
        shift = pars.getTextRange.getStartOffset - inner
          .getTextRange
          .getStartOffset + (
          if (afterWord)
            1
        ...
        if (replaceAsInjection) {
          ...
          val quote =
            if (lit.isMultiLineString)
              ...
              "\""
          val isStart = newRange
            .getStartOffset == lit.contentRange.getStartOffset
          ...
          val startOffset =
            if (isStart)
              literalRange.getStartOffset
            else
              newRange.getStartOffset
          val endOffset =
            if (isEnd)
              literalRange.getEndOffset
            else
              newRange.getEndOffset
          document.replaceString(startOffset, endOffset, text)
          shift =
            if (isStart)
              startOffset - newRange.getStartOffset
            else
              firstPart.length
        }
      case _ =>
  ...
  def replaceOccurences(
      occurences: Array[TextRange],
      newString: String,
      file: PsiFile): Array[TextRange] = {
    val revercedRangeMarkers = occurences
      .reverseMap(replaceOccurence(_, newString, file))
    revercedRangeMarkers
      .reverseMap(rm => new TextRange(rm.getStartOffset, rm.getEndOffset))
  }

  def statementsAndMembersInClass(
      aClass: ScTemplateDefinition): Seq[PsiElement] = {
    ...
    (earlyDefs ++ body)
      .flatMap(_.children)
      .filter(child =>
        child.isInstanceOf[ScBlockStatement] || child.isInstanceOf[ScMember])
      .toSeq
  }
          ...
          ref.getParent match {
            ...
            case _ =>
              val newRef = ScalaPsiElementFactory
                .createExpressionFromText(ref.getText, position)
  ...
  def container(element: PsiElement, file: PsiFile): PsiElement = {
    ...
    if (element == null)
      ...
      val funDef = PsiTreeUtil
        .getParentOfType(element, classOf[ScFunctionDefinition])
      ...
      if (funDef != null && PsiTreeUtil
        ...
        funDef.body.get
      else if (isCaseClausesBlock)
        container(candidate.getContext, file)
      else
        candidate
    }
  }
  ...
  def selectedElements(
      editor: Editor,
      file: ScalaFile,
      trimComments: Boolean): Seq[PsiElement] = {
    trimSpacesAndComments(editor, file, trimComments = trimComments)
    val startElement: PsiElement = file
      .findElementAt(editor.getSelectionModel.getSelectionStart)
    val endElement: PsiElement = file
      .findElementAt(editor.getSelectionModel.getSelectionEnd - 1)
    val elements =
  ...
  def showNotPossibleWarnings(
    ...
    def hasOutsideUsages(elem: PsiElement): Boolean = {
      val scope =
        new LocalSearchScope(
          PsiTreeUtil.getParentOfType(elem, classOf[ScControlFlowOwner], true))
    ...
    if (elements
          .isEmpty || !elements.exists(_.isInstanceOf[ScBlockStatement])) {
      showErrorHint(
