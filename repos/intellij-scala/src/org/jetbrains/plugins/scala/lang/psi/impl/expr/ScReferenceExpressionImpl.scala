...
class ScReferenceExpressionImpl(node: ASTNode)
  ...
  def bindToElement(
    ...
    def tail(qualName: String)(simpleImport: => PsiElement): PsiElement = {
      safeBindToElement(
        qualName,
        {
          ...
          case (qual, false) =>
            ScalaPsiElementFactory.createExpressionFromText(qual, getManager)
              .asInstanceOf[ScReferenceExpression]
        }
      )(simpleImport)
    }
    ...
    element match {
      ...
      case c: PsiClass =>
        if (!ResolveUtils.kindMatches(element, getKinds(incomplete = false)))
          throw new IncorrectOperationException(
            "class does not match expected kind")
        if (refName != c.name)
          throw new IncorrectOperationException(
      ...
      case t: ScTypeAlias =>
        throw new IncorrectOperationException(
          "type does not match expected kind")
      case fun: ScFunction
          if ScalaPsiUtil.hasStablePath(fun) && fun.name == "apply" =>
      ...
      case elem: PsiNamedElement =>
        if (refName != elem.name)
          throw new IncorrectOperationException(
            "named element does not match expected name")
      ...
      case _ =>
        throw new IncorrectOperationException(
          "Cannot bind to element: " + element)
    }
  }

  def getVariants: Array[Object] =
    getVariants(implicits = true, filterNotNamedVariants = false)
  ...
  def getKinds(incomplete: Boolean, completion: Boolean = false) = {
    getContext match {
      ...
      case pref: ScPrefixExpr
          if this == pref.operation || this == pref.getBaseExpr =>
        StdKinds.refExprQualRef
  ...
  protected def convertBindToType(
    ...
    val unresolvedTypeParameters: Seq[TypeParameter] = bind
      .map(_.unresolvedTypeParameters).getOrElse(None).getOrElse(Seq.empty)
                  ...
                  val actualType = tp match {
                    ...
                    case ScCompoundType(comps, _, _) =>
                      if (comps.isEmpty) Any
                        ...
                        ScTypeUtil.removeTypeDesignator(comps.head)
                          .getOrElse(Any)
  ...
  def getPrevTypeInfoParams: Seq[TypeParameter] = {
    qualifier match {
      ...
      case Some(qual) => qual.getNonValueType(TypingContext.empty).map {
