...
class Signature(
    ...
    private val tParams: Array[TypeParameter],
    val substitutor: ScSubstitutor,
    val namedElement: PsiNamedElement,
    val hasRepeatedParam: Seq[Int] = Seq.empty) {
  ...
  def substitutedTypes: List[Seq[() => ScType]] =
    types.map(_.map(f => () => substitutor.subst(f())))

  def typeParams: Array[TypeParameter] =
    tParams.map(_.update(substitutor.subst))
  ...
  def paramTypesEquivExtended(
    ...
    if (paramLength != other.paramLength && !(paramLength.sum == 0 && other
          .paramLength.sum == 0)) return (false, undefSubst)
...
object Signature {
  def unify(
      subst: ScSubstitutor,
      tps1: Array[TypeParameter],
      tps2: Array[TypeParameter]) = {
    var res = subst
