    ...
    val notPassedParams = settings
      ...
      .map { p =>
        val nameAndType = typedName(
          p.oldName,
          p.tp.canonicalText,
          p.fromElement.getProject)
        s"val $nameAndType = ???\n"
      }
    val notPassedParamsText = notPassedParams.mkString

    val elementsToAdd: Iterator[PsiElement] =
