...
object ScalaExtractMethodUtils {
  def createMethodFromSettings(
    ...
    def paramText(param: ExtractMethodParameter): String =
      typedName(
        param.oldName,
        param.tp.canonicalText,
        param.fromElement.getProject,
        param.isCallByNameParameter)

    val parameters =
      settings.parameters.filter(_.passAsParameter).map(paramText)
    val paramsText =
    ...
    val notPassedParams =
      settings.parameters.filter(p => !p.passAsParameter).map { p =>
        val nameAndType =
          typedName(p.oldName, p.tp.canonicalText, p.fromElement.getProject)
        s"val $nameAndType = ???\n"
      }
      ...
      if (settings.lastReturn)
        ...
        def multipleReturnText = {
          ...
          if (!ics.needClass)
            ...
            s"${ics.className}$params"
          else
            s"new ${ics.className}$params"
        }
    ...
    if (!settings.lastReturn) {
      val returnVisitor = new ScalaRecursiveElementVisitor {
        override def visitReturnStatement(ret: ScReturnStmt) {
          if (!ret.returnFunction.contains(method))
            return
    ...
    for ((named, newName) <- bindTo) {
      ...
      id.getParent.getNode.replaceChild(
        id.getNode,
        ScalaPsiElementFactory.createIdentifier(newName, id.getManager))
    }
    method
  }

  @Nullable
  def convertVariableData(
      variable: VariableInfo,
      elements: Array[PsiElement]): ScalaVariableData = {
    if (!variable.element.isInstanceOf[ScTypedDefinition])
      return null
  ...
  /**
    ...
    */
  def calcReturnType(settings: ScalaExtractMethodSettings): String =
    calcReturnTypeExt(settings)._2

  /**
    * @return (isUnit, returnTypePresentableText)
    */
  def calcReturnTypeExt(
      settings: ScalaExtractMethodSettings): (Boolean, String) = {
  ...
  def getParameters(
    ...
    for (input <- myInput) {
      var d: VariableData =
        ScalaExtractMethodUtils.convertVariableData(input, elements)
      if (d != null)
        buffer += d
    }
  ...
  def getReturns(
      myOutput: Array[VariableInfo],
      elements: Array[PsiElement]): Array[ExtractMethodOutput] = {
    val list: util.ArrayList[ExtractMethodOutput] =
      new util.ArrayList[ExtractMethodOutput]
    for (info <- myOutput) {
      val data: ScalaVariableData =
        ScalaExtractMethodUtils.convertVariableData(info, elements)
  ...
  def previewSignatureText(settings: ScalaExtractMethodSettings) = {
    ...
    val classText =
      if (ics.needClass)
        s"${ics.classText(canonTextForTypes = false)}\n\n"
      else
        ""
  ...
  def replaceWithMethodCall(
    ...
    def patternForDeclaration: String = {
      if (ics.needClass)
        return s"$mFreshName: ${ics.className}"

      if (outputTypedNames.length == 0)
        ""
      else if (outputTypedNames.length == 1)
        outputTypedNames(0)
      else
        outputTypedNames.mkString("(", ", ", ")")
    }
    ...
    def insertCallStmt(): PsiElement = {
      ...
      if (settings.lastReturn)
        insertExpression(s"return $methodCallText")
    ...
    def insertAssignsFromMultipleReturn(element: PsiElement) {
      if (!needExtractorsFromMultipleReturn)
        return
      ...
      def addExtractorsFromCaseClass() {
        if (allVals || allVars) {
          ...
          val expr =
            ScalaPsiElementFactory.createExpressionFromText(mFreshName, manager)
