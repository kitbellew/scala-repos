...
object ScalaPsiUtil {
  def nameWithPrefixIfNeeded(c: PsiClass): String = {
    ...
    if (ScalaCodeStyleSettings.getInstance(c.getProject)
  ...
  def typeParamString(param: ScTypeParam): String = {
    ...
    if (param.typeParameters.nonEmpty) {
      paramText += param.typeParameters.map(typeParamString)
        .mkString("[", ", ", "]")
    }
  ...
  def functionArrow(project: Project): String = {
    val useUnicode = ScalaCodeStyleSettings.getInstance(project)
  ...
  def tuplizy(
    ...
    s match {
      ...
      case _ =>
        val exprTypes: Seq[ScType] = s.map(_.getTypeAfterImplicitConversion(
          checkImplicits = true,
          isShape = false,
          None)).map { case (res, _) => res.getOrAny }
        val qual = "scala.Tuple" + exprTypes.length
        val tupleClass = ScalaPsiManager.instance(manager.getProject)
          .getCachedClass(scope, qual).orNull
        if (tupleClass == null) None
        else
          Some(Seq(new Expression(
  ...
  def processImportLastParent(
    ...
    val subst = state.get(ScSubstitutor.key).toOption
      .getOrElse(ScSubstitutor.empty)
    lastParent match {
      case _: ScImportStmt => typeResult match {
          case Success(t, _) => (processor, place) match {
  ...
  def inferMethodTypesArgs(
    ...
    fun match {
      case fun: ScFunction => fun.typeParameters.foldLeft(ScSubstitutor.empty) {
          (subst, tp) =>
            subst.bindT(
  ...
  def findImplicitConversion(
    ...
    lazy val funType =
      Option(ScalaPsiManager.instance(e.getProject).getCachedClass(
        "scala.Function1",
        e.getResolveScope,
    ...
    val isHardCoded = refName == "+" && e.getTypeWithoutImplicits()
      .map(_.isInstanceOf[ValType]).getOrElse(false)
    ...
    def checkImplicits(
      ...
      def args =
        processor match {
          ...
          case m: MethodResolveProcessor =>
            m.argumentClauses.flatMap(_.map(
      ...
      val exprType = ImplicitCollector.exprType(e, fromUnder = false).getOrElse(
        return
      )
                    ...
                    rr.element match {
                      case fun: ScFunction if fun.hasTypeParameters =>
                        ...
                        newRR.resultUndef match {
                          case Some(undef) => undef.getSubstitutor match {
                              case Some(uSubst) =>
                                Some(
                                  rr.copy(
                                    subst = newRR.substitutor.followed(uSubst),
                                    implicitParameterType = rr
                                      .implicitParameterType.map(uSubst.subst)),
    ...
    if (implicitMap.length == 1) {
      ...
      specialExtractParameterType(rr) match {
        case (Some(ScFunctionType(tp, _)), typeParams) => Some(
  ...
  def approveDynamic(
    ...
    val cachedClass = ScalaPsiManager.instance(project)
      .getCachedClass(scope, "scala.Dynamic").orNull
    ...
    val args: Seq[ScExpression] = call.argumentExpressions ++
      (if (isUpdate)
         call.getContext.asInstanceOf[ScAssignStmt].getRExpression match {
           ...
           case None =>
             Seq[ScExpression](ScalaPsiElementFactory.createExpressionFromText(
               "{val x: Nothing = null; x}",
               call.getManager
             )) //we can't to not add something => add Nothing expression
         }
       else Seq.empty)
    val (expr, exprTp, typeArgs: Seq[ScTypeElement]) = call
      ...
      case gen: ScGenericCall =>
        ...
        val referencedType = gen.referencedExpr
          .getNonValueType(TypingContext.empty).getOrNothing
              ...
              case t: ScTypeParameterType =>
                if (typeParameters.exists {
                      case TypeParameter(_, _, _, _, ptp)
                          if ptp == t.param &&
                            ptp.getOwner != ownerPtp.getOwner => true
                      case _                                  => false
                    }) res = None
              case _ =>
            }
  ...
  def getModule(element: PsiElement): Module = {
    val index: ProjectFileIndex = ProjectRootManager
      .getInstance(element.getProject).getFileIndex
  ...
  def collectImplicitObjects(
    ...
    def collectParts(tp: ScType) {
      ...
      def collectSupers(clazz: PsiClass, subst: ScSubstitutor) {
        clazz match {
          case td: ScTemplateDefinition => td.superTypes.foreach { tp =>
              collectParts(subst.subst(tp))
            }
          case clazz: PsiClass => clazz.getSuperTypes.foreach { tp =>
      ...
      tp match {
        ...
        case ScDesignatorType(v: ScFieldId) => v.getType(TypingContext.empty)
            .foreach(collectParts)
        case ScDesignatorType(p: ScParameter) => p.getType(TypingContext.empty)
            .foreach(collectParts)
        ...
        case j: JavaArrayType =>
          ...
          collectParts(parameterizedType.getOrElse(
            return
          ))
        case proj @ ScProjectionType(projected, _, _) =>
          collectParts(projected)
          proj.actualElement match {
            case v: ScBindingPattern => v.getType(TypingContext.empty)
                .map(proj.actualSubst.subst).foreach(collectParts)
            case v: ScFieldId => v.getType(TypingContext.empty)
                .map(proj.actualSubst.subst).foreach(collectParts)
            case v: ScParameter => v.getType(TypingContext.empty)
                .map(proj.actualSubst.subst).foreach(collectParts)
            case _ =>
          }
    ...
    while (parts.nonEmpty) {
      ...
      def collectObjects(tp: ScType) {
        tp match {
          ...
          case ScDesignatorType(ta: ScTypeAliasDefinition) => collectObjects(
              ta.aliasedType.getOrAny)
          case p: ScProjectionType
              if p.actualElement.isInstanceOf[ScTypeAliasDefinition] =>
            collectObjects(p.actualSubst.subst(
              p.actualElement.asInstanceOf[ScTypeAliasDefinition].aliasedType
                .getOrAny))
          ...
          case ScParameterizedType(p: ScProjectionType, args)
              if p.actualElement.isInstanceOf[ScTypeAliasDefinition] =>
            val genericSubst = ScalaPsiUtil.typesCallSubstitutor(
              p.actualElement.asInstanceOf[ScTypeAliasDefinition].typeParameters
                .map(tp => (tp.name, ScalaPsiUtil.getPsiElementId(tp))),
              args)
            val s = p.actualSubst.followed(genericSubst)
            collectObjects(s.subst(
              p.actualElement.asInstanceOf[ScTypeAliasDefinition].aliasedType
                .getOrAny))
          case _ =>
            for {
              ...
              clazz match {
                ...
                case _ => getCompanionModule(clazz) match {
                    case Some(obj: ScObject) => tp match {
    ...
    elems.map(param =>
      () =>
        param match {
          ...
          case p: PsiParameter =>
            val treatJavaObjectAsAny = p.parents
              .findByType(classOf[PsiClass]) match {
    ...
    val visitor = new ScalaRecursiveElementVisitor {
      override def visitExpression(expr: ScExpression) {
        ...
        expr.findImplicitParameters match {
          case Some(results) =>
            for (r <- results if r != null) res = res ++ r.importsUsed
          case _ =>
        }

        //implicit conversions
        def addConversions(fromUnderscore: Boolean) {
          res = res ++ expr.getTypeAfterImplicitConversion(
        ...
        expr match {
          case f: ScForStatement => f.getDesugarizedExpr match {
          ...
          case ref: ScReferenceExpression =>
            ...
            super.visitExpression(expr)
          case _ => super.visitExpression(expr)
        }
      }
    }
    z.accept(visitor)
    ...
    for (i <- 0 to Math.min(tp.length, typeArgs.length) - 1) {
      map +=
        (
          (
            tp(tp.length - 1 - i),
            typeArgs(typeArgs.length - 1 - i).getType(TypingContext.empty)
              .getOrAny))
    }
    new ScSubstitutor(Map(map.toSeq: _*), Map.empty, None)
  }
  ...
  def superTypeMembersAndSubstitutors(
    ...
    (sigs.get(element): @unchecked) match {
      ...
      case Some(x)                                       => x.supers.filter { _.info != element } :+ x
      case None                                          =>
        //this is possible case: private member of library source class.
  ...
  def withCompanionSearchScope(clazz: PsiClass): SearchScope = {
    getBaseCompanionModule(clazz) match {
      case Some(companion) => new LocalSearchScope(clazz)
          .union(new LocalSearchScope(companion))
  ...
  def hasStablePath(o: PsiNamedElement): Boolean = {
    ...
    def hasStablePathInner(m: PsiMember): Boolean = {
      ...
      m.containingClass match {
        case null => false
        case o: ScObject
            if o.isPackageObject || o.qualifiedName == "scala.Predef" => true
      ...
      element.getParent match {
        ...
        case caseClause: ScCaseClause =>
          ...
          if (funTypeToken != null &&
              element.getTextOffset < funTypeToken.getTextOffset) false
  ...
  ********** other cases (1 - need parentheses, 0 - does not need parentheses *****
    ...
    if (parsedDifferently(from, expr)) true
      ...
      (parent, expr) match {
        ...
        case (_: ScSugarCallExpr | _: ScReferenceExpression, elem: PsiElement)
            if ScUnderScoreSectionUtil.isUnderscoreFunction(elem) => true
        case (
              _,
        ...
        case (_, _: ScNewTemplateDefinition | _: ScUnderscoreSection) => false
  ...
  def parameterOf(exp: ScExpression): Option[Parameter] = {
    ...
    exp match {
      case ScAssignStmt(ResolvesTo(p: ScParameter), Some(expr)) => Some(
          new Parameter(p))
      case _ => exp.getParent match {
          ...
          case block: ScBlock if block.statements == Seq(exp) =>
            parameterOf(block)
          case ie: ScInfixExpr
              if exp == (if (ie.isLeftAssoc) ie.lOp else ie.rOp) =>
            ie.operation match {
          ...
          case (_: ScTuple) childOf (inf: ScInfixExpr) => fromMatchedParams(
              inf.matchedParameters)
          case args: ScArgumentExprList => fromMatchedParams(
              args.matchedParameters)
          case _ => None
        }
    }
  }
  ...
  def isReadonly(e: PsiElement): Boolean = {
    e match {
      case classParameter: ScClassParameter => return classParameter.isVal
      case _                                =>
    }
    ...
    e.parentsInFile.takeWhile(!_.isScope)
      .findByType(classOf[ScPatternDefinition]).isDefined
  }
  ...
  def isArgumentOfFunctionType(expr: ScExpression) = {
    isCanonicalArg(expr) && parameterOf(expr)
  ...
  object MethodValue {
    ...
    private def referencedMethod(
      ...
      expr match {
        case ref @ ResolvesTo(f: ScFunctionDefinition)
            if f.isParameterless && !canBeParameterless => None
    ...
    private def isSimpleUnderscore(expr: ScExpression) =
      expr match {
        ...
        case typed: ScTypedStmt => Option(typed.expr).map(_.getText)
            .contains("_")
        case _ => false
      }
  ...
  def syntheticParamClause(
    ...
    val params: Seq[(String, (ScTypeElement) => Unit)] = paramOwner
  ...
  def availableImportAliases(
    ...
    def getSelectors(
      ...
      if (holder != null) {
        val importExprs: Seq[ScImportExpr] = holder.getImportStatements
    ...
    aliases.filter(_._1.getTextRange.getEndOffset < position.getTextOffset)
      .filter(correctResolve).toSet
  }
  ...
  def importAliasFor(
    ...
    val suitableAliases = importAliases.collect {
      case (aliasRef, aliasName)
          if aliasRef.multiResolve(false).exists(rr =>
  ...
  def changeVisibility(
    ...
    val newElem = ScalaPsiElementFactory
      .createModifierFromText(newVisibility, manager).getPsi
  ...
  def kindProjectorPluginEnabled(e: PsiElement): Boolean = {
    val plugins = e.module match {
      ...
      case _ =>
        ScalaCompilerConfiguration.instanceIn(e.getProject).defaultProfile
          .getSettings.plugins
    }
          ...
          case ScFunctionType(retTp, params) =>
            def convertParameter(tpArg: ScType, variance: Int): ScType = {
              tpArg match {
                ...
                case _ => wildcards.find(_.name == tpArg.canonicalText) match {
                    case Some(wildcard) =>
                      (wildcard.lowerBound, wildcard.upperBound) match {
                        case (lo, Any)
                            if variance == ScTypeParam.Contravariant => lo
                        case (Nothing, hi)
                            if variance == ScTypeParam.Covariant => hi
                        case _                                   => tpArg
                      }
                    case _ => tpArg
                  }
