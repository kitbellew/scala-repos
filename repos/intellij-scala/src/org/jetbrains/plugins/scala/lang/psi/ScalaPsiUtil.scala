...
object ScalaPsiUtil {
  ...
  def functionArrow(project: Project): String = {
    val useUnicode = ScalaCodeStyleSettings
      .getInstance(project)
    ...
    s match {
      ...
      case _ =>
        val exprTypes: Seq[ScType] = s
          .map(_.getTypeAfterImplicitConversion(
            checkImplicits = true,
            isShape = false,
            None))
  ...
  def processTypeForUpdateOrApply(
      ...
      PartialFunction.condOpt(candidates) {
        case Array(r @ ScalaResolveResult(fun: PsiMethod, s: ScSubstitutor)) =>
          ...
          val res = fun match {
            ...
            case meth: PsiMethod =>
              (
                update(ResolveUtils.javaPolymorphicType(
                  meth,
                  s,
                  call.getResolveScope)),
                r.importsUsed,
    ...
    checkCandidates(withDynamic = false).orElse(checkCandidates(withDynamic =
      true))
  }
  ...
  ********** other cases (1 - need parentheses, 0 - does not need parentheses *****
  ...
  def kindProjectorPluginEnabled(p: Project): Boolean = {
    ...
    modules.exists { mod =>
      mod.hasScala && mod.scalaCompilerSettings.plugins.exists(_.contains(
        "kind-projector"))
    }
  }
