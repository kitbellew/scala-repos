...
import org.jetbrains.plugins.scala.lang.psi.api.toplevel.packaging.{
  ScPackageContainer,
  ScPackaging
}
...
object ScalaPsiUtil {
  ...
  def functionArrow(project: Project): String = {
    val useUnicode =
      ScalaCodeStyleSettings
        .getInstance(project)
        .REPLACE_CASE_ARROW_WITH_UNICODE_CHAR
    if (useUnicode)
  ...
  def drvTemplate(elem: PsiElement): Option[ScTemplateDefinition] = {
    val template = PsiTreeUtil
      .getContextOfType(elem, true, classOf[ScTemplateDefinition])
    if (template == null)
      return None
    template.extendsBlock.templateParents match {
      case Some(parents)
          if PsiTreeUtil.isContextAncestor(parents, elem, true) =>
        drvTemplate(template)
      case _ =>
        Some(template)
    }
  }
    ...
    element match {
      case st: ScalaStubBasedElementImpl[_] if st.getStub != null =>
        ...
        if (children.size() == 0)
          ...
          children.get(children.size() - 1).getPsi
      case _ =>
        element.getLastChild
    }
  }
  ...
  def tuplizy(
      ...
      manager: PsiManager,
      place: PsiElement): Option[Seq[Expression]] = {
    s match {
      case Seq() =>
      ...
      case _ =>
        ...
        if (tupleClass == null)
          ...
          Some(
            Seq(
              new Expression(
                ScParameterizedType(ScDesignatorType(tupleClass), exprTypes),
                place)))
    }
  }

  def getNextSiblingOfType[T <: PsiElement](
      sibling: PsiElement,
      aClass: Class[T]): T = {
    if (sibling == null)
  ...
  def inferMethodTypesArgs(
      fun: PsiMethod,
      classSubst: ScSubstitutor): ScSubstitutor = {

    fun match {
      case fun: ScFunction =>
        fun
          .typeParameters
          .foldLeft(ScSubstitutor.empty) { (subst, tp) =>
            subst.bindT(
  ...
  def findImplicitConversion(
    ...
    def specialExtractParameterType(
              ...
              case Some(ft) =>
                if (tp.conforms(ft)) {
                  Conformance.undefinedSubst(ft, tp).getSubstitutor match {
                    case Some(subst) =>
                      Some(subst.subst(ft).removeUndefines())
                    case _ =>
                      None
                  }
                } else
      ...
      val typeParams =
        rr.unresolvedTypeParameters match {
          case Some(tParams) if tParams.nonEmpty =>
            tParams
          case _ =>
            Seq.empty
        }
    ...
    if (implicitMap.size > 1) {
      ...
      if (implicitMap.size > 1) {
        ...
        checkImplicits(secondPart = false, noApplicability = true)
    }
  ...
  def approveDynamic(
    ...
    val cachedClass =
      ScalaPsiManager
        .instance(project)
        .getCachedClass(scope, "scala.Dynamic")
        .orNull
    if (cachedClass == null)
      return false
  ...
  def processTypeForUpdateOrApplyCandidates(
    ...
    val (expr, exprTp, typeArgs: Seq[ScTypeElement]) =
      call.getEffectiveInvokedExpr match {
        case gen: ScGenericCall =>
          ...
          val referencedType =
            gen.referencedExpr.getNonValueType(TypingContext.empty).getOrNothing
          referencedType match {
            ...
            case _ =>
              (
                gen.referencedExpr,
                gen.referencedExpr.getType(TypingContext.empty).getOrNothing,
                gen.arguments)
          }
        case expression =>
          (expression, tp, Seq.empty)
      }
    val invoked = call.getInvokedExpr
    val typeParams = invoked
      .getNonValueType(TypingContext.empty)
      .map {
        case ScTypePolymorphicType(_, tps) =>
          tps
        case _ =>
          Seq.empty
      }
    ...
    val emptyStringExpression = ScalaPsiElementFactory
      .createExpressionFromText("\"\"", call.getManager)
    val processor =
      new MethodResolveProcessor(
        expr,
        methodName,
        if (!isDynamic)
          ...
          List(List(emptyStringExpression), args),
        typeArgs,
        typeParams,
    ...
    exprTp match {
      case ScTypePolymorphicType(internal, typeParam)
          if typeParam.nonEmpty && !internal.isInstanceOf[ScMethodType] &&
            !internal.isInstanceOf[ScUndefinedType] =>
        if (!isDynamic ||
            approveDynamic(internal, call.getProject, call.getResolveScope)) {
    ...
    if (candidates.isEmpty &&
      ...
      processor
        .processType(exprTp.inferValueType, call.getEffectiveInvokedExpr, state)
      candidates = processor.candidatesS
    }
    ...
    if (!isDynamic && candidates.forall(!_.isApplicable())) {
      ...
      findImplicitConversion(
        expr,
        methodName,
        call,
        processor,
        noImplicitsForArgs = candidates.nonEmpty) match {
        case Some(res) =>
          ProgressManager.checkCanceled()
          val function = res.element
          var state = ResolveState
            .initial
            .put(ImportUsed.key, res.importUsed)
  ...
  def processTypeForUpdateOrApply(
      tp: ScType,
      call: MethodInvocation,
      isShape: Boolean): Option[(
      ScType,
      collection.Set[ImportUsed],
      Option[PsiNamedElement],
      Option[ScalaResolveResult])] = {

    def checkCandidates(withDynamic: Boolean = false): Option[(
        ScType,
        collection.Set[ImportUsed],
        Option[PsiNamedElement],
        Option[ScalaResolveResult])] = {
      val candidates: Array[ScalaResolveResult] =
        processTypeForUpdateOrApplyCandidates(
          call,
          tp,
          isShape,
          isDynamic = withDynamic)
        ...
        case Array(r @ ScalaResolveResult(fun: PsiMethod, s: ScSubstitutor)) =>
          ...
          val res =
            fun match {
              case fun: ScFun =>
                (
                  update(s.subst(fun.polymorphicType)),
                  r.importsUsed,
                  r.implicitFunction,
                  Some(r))
              case fun: ScFunction =>
              ...
              case meth: PsiMethod =>
                (
                  update(
                    ResolveUtils
                      .javaPolymorphicType(meth, s, call.getResolveScope)),
                  r.importsUsed,
                  r.implicitFunction,
                  Some(r))
            }
              ...
              res.copy(_1 = res._1 match {
                case ScTypePolymorphicType(internal, typeParams2) =>
                  ScalaPsiUtil.removeBadBounds(
                    ScTypePolymorphicType(internal, typeParams ++ typeParams2))
                case _ =>
  ...
  def removeBadBounds(tp: ScType): ScType = {
    tp match {
      case tp @ ScTypePolymorphicType(internal, typeParameters) =>
        def hasBadLinks(
            tp: ScType,
            ownerPtp: PsiTypeParameter): Option[ScType] = {
              ...
              case t: ScTypeParameterType =>
                if (typeParameters.exists {
                      case TypeParameter(_, _, _, _, ptp)
                          if ptp == t.param &&
                            ptp.getOwner != ownerPtp.getOwner =>
                        true
                      case _ =>
                        false
                    })
        ...
        def clearBadLinks(tps: Seq[TypeParameter]): Seq[TypeParameter] = {
          tps.map {
            case t @ TypeParameter(
              ...
              TypeParameter(
                name,
                clearBadLinks(typeParams),
                () => hasBadLinks(lowerType(), ptp).getOrElse(Nothing),
                () => hasBadLinks(upperType(), ptp).getOrElse(Any),
                ptp)
          }
        }
  ...
  def isAnonymousExpression(expr: ScExpression): (Int, ScExpression) = {
    ...
    if (seq.nonEmpty)
      return (seq.length, expr)
    expr match {
      case b: ScBlockExpr =>
        if (b.statements.length != 1)
          (-1, expr)
        else if (b.lastExpr.isEmpty)
          (-1, expr)
        else
          isAnonymousExpression(b.lastExpr.get)
      case p: ScParenthesisedExpr =>
        p.expr match {
          case Some(x) =>
            isAnonymousExpression(x)
          case _ =>
            (-1, expr)
        }
      case f: ScFunctionExpr =>
        (f.parameters.length, expr)
      case _ =>
        (-1, expr)
    }
  }

  def isAnonExpression(expr: ScExpression): Boolean =
    isAnonymousExpression(expr)._1 >= 0

  def getModule(element: PsiElement): Module = {
    val index: ProjectFileIndex =
  ...
  def collectImplicitObjects(
      _tp: ScType,
      project: Project,
      scope: GlobalSearchScope): Seq[ScType] = {
    ...
    def collectParts(tp: ScType) {
      ...
      tp match {
        case ScDesignatorType(v: ScBindingPattern) =>
          v.getType(TypingContext.empty).foreach(collectParts)
        case ScDesignatorType(v: ScFieldId) =>
          v.getType(TypingContext.empty).foreach(collectParts)
        case ScDesignatorType(p: ScParameter) =>
          p.getType(TypingContext.empty).foreach(collectParts)
        case ScCompoundType(comps, _, _) =>
          comps.foreach(collectParts)
        ...
        case ScTypeParameterType(_, _, _, upper, _) =>
          collectParts(upper.v)
        case _ =>
          ScType.extractClassType(tp, Some(project)) match {
            case Some((clazz, subst)) =>
              val packObjects = clazz
                .contexts
                .flatMap {
                  case x: ScPackageLike =>
                    x.findPackageObject(scope).toIterator
                  case _ =>
                    Iterator()
                }
              parts += tp
  ...
  def mapToLazyTypesSeq(elems: Seq[PsiParameter]): Seq[() => ScType] = {
    elems.map(param =>
      () =>
        param match {
          case scp: ScParameter =>
            scp.getType(TypingContext.empty).getOrNothing
          case p: PsiParameter =>
            val treatJavaObjectAsAny =
              p.parents.findByType(classOf[PsiClass]) match {
                case Some(cls) if cls.qualifiedName == "java.lang.Object" =>
                  true // See SCL-3036
                case _ =>
                  false
              }
  ...
  def getPsiElementId(elem: PsiElement): PsiElement = {
    try {
      ...
      case _: PsiInvalidElementAccessException =>
        null
    }
  }
  ...
  def getElementsRange(start: PsiElement, end: PsiElement): Seq[PsiElement] = {
    ...
    if (file == null || file != end.getContainingFile)
      return Nil
    ...
    if (commonParent.getTextRange.getStartOffset == startOffset &&
      ...
      if (parent == null || parent.getTextRange == null)
        return Seq(prev)
      while (parent
               .getTextRange
               .equalsToRange(
  ...
  def isLValue(elem: PsiElement) =
    elem match {
      case e: ScExpression =>
        e.getParent match {
          case as: ScAssignStmt =>
            as.getLExpression eq e
          case _ =>
            false
        }
      case _ =>
  ...
  def getPlaceTd(
    ...
    val td = PsiTreeUtil
      .getContextOfType(placer, true, classOf[ScTemplateDefinition])
    if (ignoreTemplateParents)
      return td
    if (td == null)
      return null
    val res =
      td.extendsBlock.templateParents match {
        case Some(parents) =>
          if (PsiTreeUtil.isContextAncestor(parents, placer, true))
            getPlaceTd(td)
          else
            td
        case _ =>
  ...
  def typesCallSubstitutor(
      tp: Seq[(String, PsiElement)],
      typeArgs: Seq[ScType]): ScSubstitutor = {
  ...
  def genericCallSubstitutor(
      tp: Seq[(String, PsiElement)],
      typeArgs: Seq[ScTypeElement]): ScSubstitutor = {
    val map = new collection.mutable.HashMap[(String, PsiElement), ScType]
    for (i <- 0 to Math.min(tp.length, typeArgs.length) - 1) {
      map +=
        (
  ...
  def genericCallSubstitutor(
      tp: Seq[(String, PsiElement)],
      gen: ScGenericCall): ScSubstitutor = {
  ...
  def superValsSignatures(
      x: PsiNamedElement,
      withSelfType: Boolean = false): Seq[Signature] = {
    val empty = Seq.empty
    val typed =
      x match {
        case x: ScTypedDefinition =>
          x
        case _ =>
          return empty
      }
    val clazz: ScTemplateDefinition =
      nameContext(typed) match {
        case e @ (_: ScValue | _: ScVariable | _: ScObject)
        ...
        case e: ScClassParameter if e.isEffectiveVal =>
          e.containingClass
        case _ =>
          return empty
      }
    ...
    val signatures =
      if (withSelfType)
        TypeDefinitionMembers.getSelfTypeSignatures(clazz)
      else
        TypeDefinitionMembers.getSignatures(clazz)
    val sigs = signatures.forName(x.name)._1
    var res: Seq[Signature] =
      (sigs.get(s): @unchecked) match {
        //partial match
  ...
  def superTypeMembersAndSubstitutors(
    ...
    val types =
      if (withSelfType)
        TypeDefinitionMembers.getSelfTypeTypes(clazz)
      else
        TypeDefinitionMembers.getTypes(clazz)
  ...
  def nameContext(x: PsiNamedElement): PsiElement = {
    ...
    def isAppropriatePsiElement(x: PsiElement): Boolean = {
      x match {
        case _: ScValue | _: ScVariable | _: ScTypeAlias | _: ScParameter |
            _: PsiMethod | _: PsiField | _: ScCaseClause | _: PsiClass |
            _: PsiPackage | _: ScGenerator | _: ScEnumerator | _: ScObject =>
          true
        case _ =>
          false
      }
    }
    if (isAppropriatePsiElement(x))
      return x
  ...
  object inNameContext {
    def unapply(x: PsiNamedElement): Option[PsiElement] =
      nameContext(x).toOption
  }
  ...
  def getMethodPresentableText(
    ...
    method match {
      ...
      case _ =>
        val PARAM_OPTIONS: Int = PsiFormatUtilBase.SHOW_NAME |
        ...
        PsiFormatUtil.formatMethod(
          ...
          PARAM_OPTIONS | PsiFormatUtilBase.SHOW_PARAMETERS,
          PARAM_OPTIONS)
    }
  }
  ...
  def getModifiersPresentableText(modifiers: ScModifierList): String = {
    ...
    modifiers match {
      case st: StubBasedPsiElement[_] if st.getStub != null =>
        for (modifier <- st.getStub.asInstanceOf[ScModifiersStub].getModifiers)
  ...
  def getMethodsForName(
    ...
    for ((n: PhysicalSignature, _) <-
         ...
         if clazz.isInstanceOf[ScObject] ||
           !n.method.hasModifierProperty("static"))
      yield n
  }
  ...
  def getUnapplyMethods(clazz: PsiClass): Seq[PhysicalSignature] = {
    getMethodsForName(clazz, "unapply") ++
      getMethodsForName(clazz, "unapplySeq") ++
      (clazz match {
        case c: ScObject =>
          c.allSynthetics
  ...
  def getParentOfType(
      element: PsiElement,
      classes: Class[_ <: PsiElement]*): PsiElement = {
  ...
  def getParentOfType(
    ...
    while (el != null && !classes.exists(_.isInstance(el)))
      el = el.getParent
    el
  }

  @tailrec
  def getParentWithProperty(
      element: PsiElement,
      strict: Boolean,
      property: PsiElement => Boolean): Option[PsiElement] = {
    if (element == null)
      None
    else if (!strict && property(element))
      Some(element)
    else
      getParentWithProperty(element.getParent, strict = false, property)
  }

  def contextOfType[T <: PsiElement](
      element: PsiElement,
      strict: Boolean,
      clazz: Class[T]): T = {
    var el: PsiElement =
  ...
  def getBaseCompanionModule(clazz: PsiClass): Option[ScTypeDefinition] = {
      ...
      scope match {
        case stub: StubBasedPsiElement[_] if stub.getStub != null =>
          stub
            .getStub
            .getChildrenByType(
              TokenSets.TYPE_DEFINITIONS_SET,
              JavaArrayFactoryUtil.PsiElementFactory)
        case file: PsiFileImpl =>
          val stub = file.getStub
          if (stub != null) {
            file
    ...
    td match {
      case _: ScClass | _: ScTrait =>
        ...
        while (i < length) {
          arrayOfElements(i) match {
            case obj: ScObject if obj.name == name =>
              return Some(obj)
            case _ =>
          }
          i = i + 1
      ...
      case _: ScObject =>
        ...
        while (i < length) {
          arrayOfElements(i) match {
            case c: ScClass if c.name == name =>
              return Some(c)
            case t: ScTrait if t.name == name =>
              return Some(t)
            case _ =>
          }
          i = i + 1
  ...
  object FakeCompanionClassOrCompanionClass {
    def unapply(obj: ScObject): Option[PsiClass] =
  ...
  def withCompanionSearchScope(clazz: PsiClass): SearchScope = {
    getBaseCompanionModule(clazz) match {
      case Some(companion) =>
        new LocalSearchScope(clazz).union(new LocalSearchScope(companion))
      case None =>
  ...
  def hasStablePath(o: PsiNamedElement): Boolean = {
    ...
    def hasStablePathInner(m: PsiMember): Boolean = {
      m.getContext match {
        case f: PsiFile =>
          return true
        case _: ScPackaging | _: PsiPackage =>
          return true
        case _ =>
      }
      m.containingClass match {
        case null =>
          false
        case o: ScObject
            if o.isPackageObject || o.qualifiedName == "scala.Predef" =>
          true
        case o: ScObject =>
          hasStablePathInner(o)
        case j if j.getLanguage.isInstanceOf[JavaLanguage] =>
          true
        case _ =>
          false
      }
  ...
  def getPsiSubstitutor(
      subst: ScSubstitutor,
      project: Project,
      scope: GlobalSearchScope): PsiSubstitutor = {
    case class PseudoPsiSubstitutor(substitutor: ScSubstitutor)
        extends PsiSubstitutor {
      def putAll(
          parentClass: PsiClass,
          mappings: Array[PsiType]): PsiSubstitutor = PsiSubstitutor.EMPTY

      def isValid: Boolean = true

      def put(
          classParameter: PsiTypeParameter,
          mapping: PsiType): PsiSubstitutor = PsiSubstitutor.EMPTY

      def getSubstitutionMap: java.util.Map[PsiTypeParameter, PsiType] =
        new java.util.HashMap[PsiTypeParameter, PsiType]()

      def substitute(`type`: PsiType): PsiType = {
        ScType.toPsi(
          substitutor.subst(ScType.create(`type`, project, scope)),
          project,
          scope)
      }
      ...
      def substitute(typeParameter: PsiTypeParameter): PsiType = {
        ScType.toPsi(
          substitutor
            .subst(new ScTypeParameterType(typeParameter, substitutor)),
          project,
          scope)
      }
      ...
      def substituteWithBoundsPromotion(
          typeParameter: PsiTypeParameter): PsiType = substitute(typeParameter)
    ...
    if (element == null)
      ...
      element.getParent match {
        case block: ScBlock if block.hasRBrace =>
          true
        case _: ScMatchStmt | _: ScalaFile | null =>
          true
        case argList: ScArgumentExprList if argList.isBraceArgs =>
          true

        case argList: ScArgumentExprList if !argList.isBraceArgs =>
          false
        case _: ScParenthesisedExpr | _: ScTuple | _: ScTypeArgs |
            _: ScPatternArgumentList | _: ScParameterClause |
            _: ScTypeParamClause =>
          false
        case caseClause: ScCaseClause =>
          import org.jetbrains.plugins.scala.lang.lexer.ScalaTokenTypes._
          val funTypeToken = caseClause
            .findLastChildByType(TokenSet.create(tFUNTYPE, tFUNTYPE_ASCII))
          if (funTypeToken != null &&
              element.getTextOffset < funTypeToken.getTextOffset)
            false
          else
            newLinesEnabled(caseClause.getParent)

        case other =>
          newLinesEnabled(other.getParent)
      }
    }
  }
  /*
  ...
  ********** other cases (1 - need parentheses, 0 - does not need parentheses *****
  ...
  def needParentheses(from: ScExpression, expr: ScExpression): Boolean = {
    def infixInInfixParentheses(
        parent: ScInfixExpr,
        child: ScInfixExpr): Boolean = {
      ...
      if (parent.lOp == from) {
        ...
        if (priority(lid) < priority(rid))
          true
        else if (priority(rid) < priority(lid))
          false
        else if (associate(lid) != associate(rid))
          true
        else if (associate(lid) == -1)
          true
        else
          false
      } else {
    ...
    def tupleInInfixNeedParentheses(
        parent: ScInfixExpr,
        from: ScExpression,
        expr: ScTuple): Boolean = {
      ...
      if (from == parent.lOp)
        ...
        parent.operation.bind() match {
          case Some(resolveResult: ResolveResult) =>
            ...
            val modifiedParentText = parentText.substring(0, startInParent) +
              from.getText + parentText.substring(endInParent)
              ...
              case ScInfixExpr(_, newOper, tuple: ScTuple) =>
                newOper.bind() match {
                  case Some(newResolveResult) =>
                    newResolveResult.getElement == resolveResult.getElement &&
                      newResolveResult.tuplingUsed
                  case _ =>
                    true
                }
              case _ =>
      ...
      if (newLinesEnabled(from)) {
        expr.getParent match {
          case ScParenthesisedExpr(_) =>
            ...
            val dummyFile = ScalaPsiElementFactory
              .createScalaFile(text, expr.getManager)
            dummyFile.firstChild match {
              case Some(newExpr: ScExpression) =>
                newExpr.getText != text
              case _ =>
                true
            }
          case _ =>
    ...
    if (parsedDifferently(from, expr))
      true
    else {
      val parent = from.getParent
      (parent, expr) match {
        //order of these case clauses is important!
        case (_: ScGuard, _: ScMatchStmt) =>
          true
        case _ if !parent.isInstanceOf[ScExpression] =>
          false
        case _ if expr.getText == "_" =>
          false
        case (_: ScTuple | _: ScBlock | _: ScXmlExpr, _) =>
          false
        case (infix: ScInfixExpr, tuple: ScTuple) =>
          tupleInInfixNeedParentheses(infix, from, tuple)
        case (_: ScSugarCallExpr | _: ScReferenceExpression, elem: PsiElement)
        ...
        case (
              ...
              _: ScSuperReference) =>
          false
        case (_: ScMethodCall | _: ScUnderscoreSection, _) =>
          true
        case (_, _: ScBlock) =>
          false
        case (_: ScGenericCall, _) =>
          true
        case (_: ScReferenceExpression, _: ScNewTemplateDefinition) =>
          val lastChar: Char = expr.getText.last
          lastChar != ')' && lastChar != '}' && lastChar != ']'
        case (_: ScReferenceExpression, _) =>
          true
        case (_, _: ScNewTemplateDefinition | _: ScUnderscoreSection) =>
          false
        case (_: ScPrefixExpr, _) =>
          true
        case (_, _: ScPrefixExpr) =>
          false
        case (par: ScInfixExpr, child: ScInfixExpr) =>
          infixInInfixParentheses(par, child)
        case (_, _: ScInfixExpr) =>
          false
        case (_: ScPostfixExpr | _: ScInfixExpr, _) =>
          true
        case (_, _: ScPostfixExpr) =>
          false
        case (_: ScTypedStmt | _: ScMatchStmt, _) =>
          true
        case _ =>
          false
      }
  ...
  def readAttribute(annotation: PsiAnnotation, name: String): Option[String] = {
    annotation.findAttributeValue(name) match {
      case literal: PsiLiteral =>
        stringValueOf(literal)
  ...
  /**
    ...
    */
  def nthConstructorParam(cls: ScClass, n: Int): Option[ScParameter] =
  ...
  def parameterOf(exp: ScExpression): Option[Parameter] = {
    ...
    exp match {
      case ScAssignStmt(ResolvesTo(p: ScParameter), Some(expr)) =>
        Some(new Parameter(p))
      case _ =>
        exp.getParent match {
          case parenth: ScParenthesisedExpr =>
            parameterOf(parenth)
          case named: ScAssignStmt =>
            parameterOf(named)
          ...
          case (_: ScTuple) childOf (inf: ScInfixExpr) =>
            fromMatchedParams(inf.matchedParameters)
          case args: ScArgumentExprList =>
            fromMatchedParams(args.matchedParameters)
          case _ =>
            None
        }
    }
  ...
  def isReadonly(e: PsiElement): Boolean = {
    ...
    if (parent.isInstanceOf[ScGenerator] || parent.isInstanceOf[ScEnumerator] ||
  ...
  private def isCanonicalArg(expr: ScExpression) =
    expr match {
      case _: ScParenthesisedExpr =>
        false
      case ScBlock(expr: ScExpression) =>
        false
      case _ =>
        true
    }
  ...
  def isArgumentOfFunctionType(expr: ScExpression) = {
    isCanonicalArg(expr) &&
    parameterOf(expr).exists(p => ScFunctionType.isFunctionType(p.paramType))
  }
  ...
  object MethodValue {
    def unapply(expr: ScExpression): Option[PsiMethod] = {
      ...
      expr match {
        ...
        case gc: ScGenericCall
            if !gc.getParent.isInstanceOf[MethodInvocation] =>
        ...
        case mc: ScMethodCall if !mc.getParent.isInstanceOf[ScMethodCall] =>
          referencedMethod(mc, canBeParameterless = false).filter {
            case f: ScFunction
                if f.paramClauses.clauses.size > numberOfArgumentClauses(mc) =>
              true
            case _ =>
              false
          }
    ...
    private def referencedMethod(
        expr: ScExpression,
        canBeParameterless: Boolean): Option[PsiMethod] = {
      expr match {
        case ref @ ResolvesTo(f: ScFunctionDefinition)
            if f.isParameterless && !canBeParameterless =>
          None
        case ref @ ResolvesTo(m: PsiMethod) =>
          Some(m)
        case gc: ScGenericCall =>
        ...
        case m: ScMethodCall =>
          referencedMethod(m.deepestInvokedExpr, canBeParameterless = false)
        case _ =>
          None
      }
    }
  ...
  def syntheticParamClause(
      paramOwner: ScTypeParametersOwner,
      paramClauses: ScParameters,
      classParam: Boolean): Option[ScParameterClause] = {
    if (paramOwner == null)
      return None
    ...
    def synthParams(
      ...
      val bounds = typeParam
        .contextBoundTypeElement
        .map { (cbte: ScTypeElement) =>
    ...
    val params: Seq[(String, (ScTypeElement) => Unit)] =
      paramOwner.typeParameters match {
        case null =>
          Seq()
        case xs =>
          xs.flatMap(synthParams)
      }
    ...
    if (params.isEmpty)
      ...
      val paramClause: ScParameterClause = {
        if (classParam)
          ScalaPsiElementFactory
            .createImplicitClassParamClauseFromTextWithContext(
              fullClauseText,
              paramOwner.getManager,
              paramClauses)
        else
          ScalaPsiElementFactory.createImplicitClauseFromTextWithContext(
            fullClauseText,
            paramOwner.getManager,
            paramClauses)
      }
  ...
  def isPossiblyAssignment(ref: PsiReference): Boolean =
    isPossiblyAssignment(ref.getElement)
  ...
  def isPossiblyAssignment(elem: PsiElement): Boolean =
    elem.getContext match {
      case assign: ScAssignStmt if assign.getLExpression == elem =>
        true
      case infix: ScInfixExpr if infix.isAssignmentOperator =>
        true
  ...
  def availableImportAliases(
    ...
    def getSelectors(
      ...
      if (holder != null) {
        val importExprs: Seq[ScImportExpr] = holder
          .getImportStatements
          .flatMap(_.importExprs)
        importExprs
          .flatMap(_.selectors)
          .filter(_.importedName != "_")
          .foreach { s =>
            if (s.reference.refName != s.importedName)
              result += ((s.reference, s.importedName))
          }
        result.toSet
      } else
        Set.empty
    ...
    def correctResolve(alias: (ScReferenceElement, String)): Boolean = {
      ...
      val ref = ScalaPsiElementFactory
        .createReferenceFromText(text, position.getContext, position)
      val resolves = aliasRef.multiResolve(false)
      resolves.exists {
        case rr: ScalaResolveResult =>
          ref.isReferenceTo(rr.element)
        case _ =>
          false
      }
    }
  ...
  def importAliasFor(
      element: PsiElement,
      refPosition: PsiElement): Option[ScReferenceElement] = {
    ...
    val suitableAliases = importAliases.collect {
      case (aliasRef, aliasName)
          if aliasRef
            .multiResolve(false)
            .exists(rr =>
              ScEquivalenceUtil.smartEquivalence(rr.getElement, element)) =>
        aliasName
    }
    if (suitableAliases.nonEmpty) {
      val newRef: ScStableCodeReferenceElement = ScalaPsiElementFactory
        .createReferenceFromText(suitableAliases.head, refPosition.getManager)
      Some(newRef)
    } else
      None
  }
  ...
  def padWithWhitespaces(element: PsiElement) {
    ...
    for {
      ...
      if (!prevElement.isInstanceOf[PsiWhiteSpace]) {
        parent.addBefore(
          ScalaPsiElementFactory.createWhitespace(element.getManager),
          element)
      }
  ...
  def findInstanceBinding(instance: ScExpression): Option[ScBindingPattern] = {
    ...
    val nameContext = PsiTreeUtil.getParentOfType(
      instance,
    ...
    val (bindings, expr) =
      nameContext match {
        case vd: ScVariableDefinition =>
          (vd.bindings, vd.expr)
        case td: ScPatternDefinition =>
          (td.bindings, td.expr)
        case _ =>
          (Seq.empty[ScBindingPattern], None)
      }
    if (bindings.size == 1 && expr.contains(instance))
      Option(bindings.head)
    else {
      for (bind <- bindings) {
        if (bind.getType(TypingContext.empty).toOption ==
              instance.getType(TypingContext.empty).toOption)
          return Option(bind)
      }
      None
    }
  ...
  def intersectScopes(scope: SearchScope, scopeOption: Option[SearchScope]) = {
    scopeOption match {
      case Some(s) =>
        s.intersectWith(scope)
      case None =>
        scope
    }
  }

  private def addBefore[T <: PsiElement](
      element: T,
      parent: PsiElement,
      anchorOpt: Option[PsiElement]): T = {
    val anchor =
      anchorOpt match {
        case Some(a) =>
          a
        case None =>
          ...
          if (ScalaPsiUtil.isLineTerminator(last.getPrevSibling))
            last.getPrevSibling
          else
            last
      }
    ...
    def newLine: PsiElement =
      ScalaPsiElementFactory.createNewLineNode(element.getManager).getPsi
    ...
    if (!anchorEndsLine)
      ...
      anchor.replace(newLine)

    addedStmt
  }

  def addStatementBefore(
      stmt: ScBlockStatement,
      parent: PsiElement,
      anchorOpt: Option[PsiElement]): ScBlockStatement = {
    addBefore[ScBlockStatement](stmt, parent, anchorOpt)
  }

  def addTypeAliasBefore(
      typeAlias: ScTypeAlias,
      parent: PsiElement,
      anchorOpt: Option[PsiElement]): ScTypeAlias = {
  ...
  def changeVisibility(
    ...
    val newElem =
      ScalaPsiElementFactory
        .createModifierFromText(newVisibility, manager)
        .getPsi
    modifierList.accessModifier match {
      ...
      case None =>
        if (modifierList.children.isEmpty) {
          ...
          modifierList
            .addBefore(ScalaPsiElementFactory.createWhitespace(manager), mod)
        }
    }
  }
  ...
  def kindProjectorPluginEnabled(e: PsiElement): Boolean = {
    val plugins =
      e.module match {
        case Some(mod) =>
          mod.scalaCompilerSettings.plugins
        case _ =>
  ...
  def kindProjectorPluginEnabled(p: Project): Boolean = {
    ...
    modules.exists { mod =>
      mod.hasScala &&
      mod.scalaCompilerSettings.plugins.exists(_.contains("kind-projector"))
    }
  }
  ...
  def isSAMEnabled(e: PsiElement) =
    e.scalaLanguageLevel match {
      ...
      case Some(lang) if lang == Scala_2_11 =>
        ...
        settings.additionalCompilerOptions.contains("-Xexperimental")
      case _ =>
    ...
    def constructorValidForSAM(constructors: Array[PsiMethod]): Boolean = {
      ...
      constructors.length match {
        case 0 =>
          true
        case 1 =>
          constructors.head.getModifierList.hasModifierProperty("public") &&
      ...
      case Some((cl, sub)) =>
        cl match {
          case templDef: ScTemplateDefinition => //it's a Scala class or trait
            val abst: Seq[ScFunction] = templDef
              .allMethods
              .toSeq
              .collect {
                case PhysicalSignature(fun: ScFunction, _)
                    if fun.isAbstractMember =>
                  fun
              }
            val constrValid =
              templDef match { //if it's a class check its constructor
                case cla: ScClass =>
                  constructorValidForSAM(cla.constructors)
                case tr: ScTrait =>
                  true
                case _ =>
                  false
            ...
            val abst: Array[PsiMethod] =
              cl.getMethods
                ...
                case array if array.length > 0 =>
                  array.filterNot(overridesConcreteMethod)
                case any =>
                  any
              }
            //must have exactly one abstract member and SAM must be monomorphic
            val valid = abst.length == 1 && !abst.head.hasTypeParameters &&
              constructorValidForSAM(cl.getConstructors)
            if (valid) {
              //need to generate ScType for Java method
              val method = abst.head
              val project = method.getProject
              val returnType: ScType = ScType
                .create(method.getReturnType, project, scalaScope)
              val params: Array[ScType] = method
                .getParameterList
                .getParameters
                .map { param: PsiParameter =>
                  ScType
                    .create(param.getTypeElement.getType, project, scalaScope)
                }
  ...
  /**
    ...
    * @see SCL-8956
    */
  private def extrapolateWildcardBounds(
      tp: ScType,
      expected: ScType,
      proj: Project,
        ...
        tp match {
          case ScFunctionType(retTp, params) =>
            def convertParameter(tpArg: ScType, variance: Int): ScType = {
              tpArg match {
                case p @ ScParameterizedType(des, tpArgs) =>
                  ScParameterizedType(
                    des,
                    tpArgs.map(convertParameter(_, variance)))
                  ...
                  wildcards.find(_.name == tpArg.canonicalText) match {
                    case Some(wildcard) =>
                      (wildcard.lowerBound, wildcard.upperBound) match {
                        case (lo, Any)
                            if variance == ScTypeParam.Contravariant =>
                          lo
                        case (Nothing, hi)
                            if variance == ScTypeParam.Covariant =>
                          hi
                        case _ =>
                          tpArg
                      }
            ...
            val newParams = params
              .map(convertParameter(_, ScTypeParam.Contravariant))
  ...
  def isImplicit(namedElement: PsiNamedElement) = {
    namedElement match {
      case s: ScModifierListOwner =>
        s.hasModifierProperty("implicit")
