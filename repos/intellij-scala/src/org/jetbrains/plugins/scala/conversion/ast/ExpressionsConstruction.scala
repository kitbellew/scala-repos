...
case class ArrayInitializer(expresions: Seq[IntermediateNode])
    extends IntermediateNode
...
object MethodCallExpression extends IntermediateNode {
  def build(
    ...
    val identifier =
      methodName match {
        case "this" =>
          LiteralExpression(methodName)
        case _ =>
          LiteralExpression(escapeKeyword(methodName))
      }
...
case class ThisExpression(value: Option[IntermediateNode])
    extends IntermediateNode
case class SuperExpression(value: Option[IntermediateNode])
    extends IntermediateNode
...
case class AnonymousClassExpression(anonymousClass: IntermediateNode)
    extends IntermediateNode
