...
class MultiLineStringCopyPasteProcessor extends CopyPastePreProcessor {
    ...
    element match {
      case interpLiteral
          if interpLiteral.getNode.getElementType ==
            ScalaElementTypes.INTERPOLATED_STRING_LITERAL =>
        Some(interpLiteral)
      case string
          if MultiLineStringCopyPasteProcessor
            .SAFE_ELEMENTS
            .contains(string.getNode.getElementType) =>
  ...
  private def checkElement(element: PsiElement): Boolean =
    element == null ||
      !MultiLineStringCopyPasteProcessor
        .SAFE_ELEMENTS
