...
class SimplePrintVisitor extends IntermediateTreeVisitor {
  ...
  def visitAnnotation(
    ...
    if (inAnnotation) { printer.append("new ") }
    else { printer.append("@") }
  ...
  def visitClass(
    ...
    if (primaryConstructor.isDefined) {
      val pc = primaryConstructor.get.asInstanceOf[PrimaryConstruction]
      if (pc.body != null) { printWithSeparator(pc.body, "\n", "", "\n") }
    }
  ...
  def visitVariable(
    ...
    visit(ftype)
    printer.append(" = ")
    if (initalaizer.isDefined) { visit(initalaizer.get) }
    else {
  ...
  def visitConstructor(
    ...
    printer.append("def ")
    printer.append("this")
    if (typeParams.nonEmpty) { printWithSeparator(typeParams, ", ", "[", "]") }
  ...
  def visitPrimaryConstructor(
      params: Seq[(String, IntermediateNode, Boolean)],
  ...
  def visitTryCatch(
    ...
    if (resourcesList.nonEmpty) { printer.append("\n}") }
  }

  def visitSwitchStatement(
      expession: Option[IntermediateNode],
      body: Option[IntermediateNode]) = {
  ...
  def visitSwitchLabelStatement(
      caseValue: Option[IntermediateNode],
      arrow: String) = {
    printer.append("case ")
  ...
  def visitJavaCodeRef(
      statement: JavaCodeReferenceStatement,
