...
class SimplePrintVisitor extends IntermediateTreeVisitor {
  ...
  def visitClass(
    ...
    if (extendsList.isDefined && extendsList.get.nonEmpty) {
      ...
      if (extendsList.get.tail.nonEmpty)
        printer.append(" with ")
  ...
  def visitIfStatement(
    ...
    if (thenBranch.isDefined)
      visit(thenBranch.get)
  ...
  def visitForEach(
    ...
    printer.append(" <- ")
    if (iteratedValue.isDefined)
      visit(iteratedValue.get)
    printer.append(") ")
    if (body.isDefined)
      visit(body.get)
  }
  ...
  def visitJavaCodeRef(
    ...
    if (parametrList.isDefined)
      visit(parametrList.get)
  }
