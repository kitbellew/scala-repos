            ...
            ScalaUtils.runWriteAction(
              new Runnable {
                def run() {
                  ...
                  val name =
                    jFile.getName.substring(0, jFile.getName.length - 5)
                  ...
                  if (existingFile != null) {
                    NotificationUtil.builder(
                      ...
                      s"File $nameWithExtension already exists").setDisplayType(
                      NotificationDisplayType.BALLOON).setNotificationType(
                      NotificationType.ERROR).setGroup(
                      "rename.java.to.scala").setTitle(
                      "Cannot create file").show()
                    return
                  }
                  ...
                  val document = PsiDocumentManager.getInstance(
                    file.getProject).getDocument(file)
                  document.insertString(0, newText)
                  PsiDocumentManager.getInstance(
                    file.getProject).commitDocument(document)
                  ...
                  val settings = CodeStyleSettingsManager.getSettings(
                    file.getProject).getCommonSettings(
