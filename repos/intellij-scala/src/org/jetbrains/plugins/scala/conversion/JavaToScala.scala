...
object JavaToScala {
  ...
  def convertPsiToIntermdeiate(
    ...
    val result: IntermediateNode = element match {
        ...
        m.getMethodExpression.resolve() match {
          case method: PsiMethod
              if method.getName == "parseInt" &&
                ...
                method.getContainingClass.qualifiedName == "java.lang.Integer" =>
            ClassCast(
      ...
      case n: PsiNewExpression =>
        ...
        val mtype =
          TypeConstruction.createStringTypePresentation(n.getType, n.getProject)
    ...
    def handleAsClass(
      ...
      def handleAnonymousClass(clazz: PsiAnonymousClass): IntermediateNode = {
        val tp = TypeConstruction.createStringTypePresentation(
          ...
          clazz.getProject)
        val argList =
          convertPsiToIntermdeiate(clazz.getArgumentList, externalProperties)
        AnonymousClass(
          tp,
          argList,
  ...
  def buildConstructorTargetMap(
    ...
    for (constructor <- constructors) {
      ...
      val refExpr =
        getFirstStatement(constructor).map(_.getExpression).flatMap {
          case mc: PsiMethodCallExpression
              if mc.getMethodExpression.getQualifiedName == "this" =>
      ...
      if (refExpr.isDefined) {
        ...
        if (target.isDefined && target.get.isConstructor) {
          val finalTarget: PsiMethod =
            toTargetConstructorMap.getOrElse(target.get, target.get)
  ...
  def handlePrimaryConstructor(constructors: Seq[PsiMethod])(
          ...
          val updatedField =
            if (fieldInfo.isEmpty) {
              ...
              val p = convertPsiToIntermdeiate(
                fieldInfo.head._1,
                WithReferenceExpression(true))
                .asInstanceOf[FieldConstruction]
    ...
    def GetComplexPrimaryConstructor(): PsiMethod = {
      ...
      def tryFindWithoutParamConstructor(): PsiMethod = {
        val emptyParamsConstructors =
          constructors.filter(_.getParameterList.getParametersCount == 0)
