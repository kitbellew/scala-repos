...
object JavaToScala {
      ...
      element match {
        ...
        case r: PsiReferenceExpression =>
          ...
          val refName =
            if (externalProperties.isInstanceOf[WithReferenceExpression]) {
              fieldParamaterMap
                .getOrElse(r.getReferenceName, r.getReferenceName)
            } else
              r.getReferenceName
        ...
        case be: PsiBinaryExpression =>
          def isOk: Boolean = {
            ...
            be.getROperand match {
              case l: PsiLiteralExpression if l.getText == "null" =>
                return false
              case _ =>
            }
            true
              ...
              getFirstStatement(m)
                ...
                .flatMap {
                  case mc: PsiMethodCallExpression
                    ...
                    Some(
                      convertPsiToIntermdeiate(m.getBody, externalProperties))
                  case _ =>
        ...
        case annot: PsiAnnotation =>
          ...
          for (attribute <- attributes) {
            val value =
              Option(attribute.getValue) match {
          ...
          if (p.isVarArgs) {
            p.getTypeElement.getType match {
              case at: PsiArrayType =>
                val scCompType = TypeConstruction.createStringTypePresentation(
                  at.getComponentType,
                  p.getProject)
  ...
  def buildConstructorTargetMap(
    ...
    for (constructor <- constructors) {
      ...
      val refExpr = getFirstStatement(constructor)
        .map(_.getExpression)
        .flatMap {
          case mc: PsiMethodCallExpression
              if mc.getMethodExpression.getQualifiedName == "this" =>
  ...
  def handleModifierList(owner: PsiModifierListOwner)(implicit
    ...
    def handleAnnotations: Seq[IntermediateNode] = {
      ...
      for {
        ...
        optValue =
          Option(a.getQualifiedName).map(annotationDropList.contains(_))
