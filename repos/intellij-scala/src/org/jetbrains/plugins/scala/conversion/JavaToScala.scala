...
/**
  ...
  */
object JavaToScala {
  private val context: ThreadLocal[mutable.Stack[(Boolean, String)]] =
    new ThreadLocal[mutable.Stack[(Boolean, String)]] {
      override def initialValue(): mutable.Stack[(Boolean, String)] =
        new mutable.Stack[(Boolean, String)]()
    }

  def findVariableUsage(
      elementToFind: PsiElement,
      elementWhereFind: PsiElement): Seq[PsiReferenceExpression] = {
    import scala.collection.JavaConverters._
    ReferencesSearch
      .search(elementToFind, new LocalSearchScope(elementWhereFind)).findAll()
      .asScala.toSeq.filter(_.isInstanceOf[PsiReferenceExpression])
  ...
  def isVar(element: PsiModifierListOwner, parent: PsiElement): Boolean = {
    ...
    val possibleVar = element.hasModifierProperty(PsiModifier.PUBLIC) ||
      element.hasModifierProperty(PsiModifier.PROTECTED)

    val references = findVariableUsage(element, parent)
      .filter((el: PsiReferenceExpression) => PsiUtil.isAccessedForWriting(el))
  ...
  def convertPsiToIntermdeiate(
    ...
    val result: IntermediateNode = element match {
      case f: PsiFile =>
        ...
        m.addChildren(
          f.getChildren.map(convertPsiToIntermdeiate(_, externalProperties)))
        m
      ...
      case f: PsiForeachStatement =>
        ...
        val body = Option(f.getBody)
          .map(convertPsiToIntermdeiate(_, externalProperties))
        ForeachStatement(
          f.getIterationParameter.getName,
          iteratedValue,
          body,
          isJavaCollection)
        ...
        if (r.getQualifierExpression != null) {
          ...
          r.resolve() match {
            case f: PsiMember if f.hasModifierProperty("static") =>
              ...
              if (clazz != null &&
                  context.get().contains((false, clazz.qualifiedName))) {
      ...
      case be: PsiBinaryExpression =>
        def isOk: Boolean = {
          if (be.getLOperand.getType.isInstanceOf[PsiPrimitiveType])
            return false
      ...
      case c: PsiTypeCastExpression => ClassCast(
          ...
          c.getCastType.getType.isInstanceOf[PsiPrimitiveType] &&
            c.getOperand.getType.isInstanceOf[PsiPrimitiveType]
        )
      ...
      case m: PsiMethodCallExpression => m.getMethodExpression.resolve() match {
          case method: PsiMethod
              if method.getName == "parseInt" &&
                m.getArgumentList.getExpressions.length == 1 &&
                method.getContainingClass != null &&
                method.getContainingClass.qualifiedName ==
                "java.lang.Integer" =>
            ClassCast(
              convertPsiToIntermdeiate(
                m.getArgumentList.getExpressions.apply(0),
                externalProperties),
              TypeConstruction("Int"),
              isPrimitive = true)
          case method: PsiMethod
              if method.getName == "parseDouble" &&
                m.getArgumentList.getExpressions.length == 1 &&
                method.getContainingClass != null &&
                method.getContainingClass.qualifiedName == "java.lang.Double" =>
            ClassCast(
              convertPsiToIntermdeiate(
                m.getArgumentList.getExpressions.apply(0),
                externalProperties),
              TypeConstruction("Double"),
              isPrimitive = true)
          case method: PsiMethod
              if method.getName == "round" &&
                m.getArgumentList.getExpressions.length == 1 &&
                method.getContainingClass != null &&
                method.getContainingClass.qualifiedName == "java.lang.Math" =>
      ...
      case l: PsiLocalVariable =>
        val parent = PsiTreeUtil
          .getParentOfType(l, classOf[PsiCodeBlock], classOf[PsiBlockStatement])
        ...
        LocalVariable(
          ...
          l.getName,
          convertPsiToIntermdeiate(l.getTypeElement, externalProperties),
          needVar,
          initalizer)
      case f: PsiField =>
      ...
      case v: PsiArrayInitializerMemberValue =>
        ArrayInitializer(
          v.getInitializers.map(convertPsiToIntermdeiate(_, externalProperties))
            .toSeq)
      case annot: PsiAnnotation =>
        ...
        val attrResult =
          new ArrayBuffer[(Option[String], Option[IntermediateNode])]()
        for (attribute <- attributes) {
          val value = Option(attribute.getValue) match {
            case Some(v: PsiAnnotationMemberValue)
                if isArrayAnnotationParameter(attribute) =>
              ArrayInitializer(
                Seq(convertPsiToIntermdeiate(v, externalProperties)))
            case Some(_) =>
              convertPsiToIntermdeiate(attribute.getValue, externalProperties)
            case _ => null
          }
          attrResult += ((Option(attribute.getName), Option(value)))
        }

        val inAnnotation = PsiTreeUtil
          .getParentOfType(annot, classOf[PsiAnnotation]) != null
        ...
        if (p.isVarArgs) {
          p.getTypeElement.getType match {
            case at: PsiArrayType =>
              val scCompType = TypeConstruction
                .createStringTypePresentation(at.getComponentType, p.getProject)
      ...
      case t: PsiTryStatement =>
        ...
        val finallys = Option(t.getFinallyBlock).map((f: PsiCodeBlock) =>
          f.getStatements.map(convertPsiToIntermdeiate(_, externalProperties))
            .toSeq)
        TryCatchStatement(
          ...
          catches,
          finallys,
          ScalaPsiUtil.functionArrow(t.getProject))
      case p: PsiPrefixExpression =>
        PrefixExpression(
          convertPsiToIntermdeiate(p.getOperand, externalProperties),
          p.getOperationSign.getText,
          canBeSimpified(p))
    ...
    result match {
      case parametrizedConstruction: ParametrizedConstruction =>
        associations ++= parametrizedConstruction.getAssociations.map {
          case (node, path) =>
            AssociationHelper(DependencyKind.Reference, node, Path(path))
        }
  ...
  def createClass(inClass: PsiClass, externalProperties: ExternalProperties)(
    ...
    def extendList: Seq[PsiJavaCodeReferenceElement] = {
      ...
      if (inClass.getExtendsList != null)
        typez ++= inClass.getExtendsList.getReferenceElements
      if (inClass.getImplementsList != null)
        typez ++= inClass.getImplementsList.getReferenceElements
      typez.toSeq
    }
    ...
    def handleObject(objectMembers: Seq[PsiMember]): IntermediateNode = {
      def handleAsEnum(modifiers: IntermediateNode): IntermediateNode = {
        Enum(
          inClass.getName,
          modifiers,
          objectMembers.filter(_.isInstanceOf[PsiEnumConstant])
            .map((el: PsiMember) => el.getName))
      }
      def handleAsObject(modifiers: IntermediateNode): IntermediateNode = {
        val membersOut = objectMembers.filter(!_.isInstanceOf[PsiEnumConstant])
          .map(convertPsiToIntermdeiate(_, externalProperties))
        val initializers = inClass.getInitializers
          .map((x: PsiClassInitializer) =>
      ...
      if (objectMembers.nonEmpty && !inClass.isInstanceOf[PsiAnonymousClass]) {
        ...
        try {
          ...
          val updatedModifiers = modifiers.asInstanceOf[ModifiersConstruction]
    ...
    def handleAsClass(
        classMembers: Seq[PsiMember],
        objectMembers: Seq[PsiMember],
        companionObject: IntermediateNode,
        extendList: Seq[PsiJavaCodeReferenceElement]): IntermediateNode = {

      def handleAnonymousClass(clazz: PsiAnonymousClass): IntermediateNode = {
        val tp = TypeConstruction.createStringTypePresentation(
          clazz.getBaseClassType,
          clazz.getProject)
        val argList = convertPsiToIntermdeiate(
          clazz.getArgumentList,
          externalProperties)
        AnonymousClass(
          tp,
          argList,
          classMembers.map(convertPsiToIntermdeiate(_, externalProperties)),
      ...
      def sortMembers(): Seq[PsiMember] = {
        ...
        val constructorsCallMap = buildConstructorTargetMap(
          inClass.getConstructors.sortBy(_.getTextOffset))
        sort(constructorsCallMap)
      }

      def updateMembersAndConvert(
      ...
      if (classMembers.nonEmpty || objectMembers.isEmpty) {
        ...
        try {
          inClass match {
            ...
            case _ =>
              val typeParams = inClass.getTypeParameters
                .map(convertPsiToIntermdeiate(_, externalProperties))
              val modifiers = handleModifierList(inClass)
  ...
  def getFirstStatement(
    ...
    Option(constructor.getBody).map(_.getStatements).flatMap(_.headOption)
  ...
  def buildConstructorTargetMap(
      constructors: Seq[PsiMethod]): mutable.HashMap[PsiMethod, PsiMethod] = {
    val toTargetConstructorMap = new mutable.HashMap[PsiMethod, PsiMethod]()

    for (constructor <- constructors) {

      val refExpr = getFirstStatement(constructor).map(_.getExpression)
        .flatMap {
          case mc: PsiMethodCallExpression
              if mc.getMethodExpression.getQualifiedName == "this" =>
      ...
      if (refExpr.isDefined) {
        ...
        if (target.isDefined && target.get.isConstructor) {
          val finalTarget: PsiMethod = toTargetConstructorMap
            .getOrElse(target.get, target.get)
  ...
  def handlePrimaryConstructor(constructors: Seq[PsiMethod])(implicit
    ...
    def createPrimaryConstructor(
      ...
      def getCorrespondedFieldInfo(
          param: PsiParameter): Seq[(PsiField, PsiExpressionStatement)] = {
        ...
        for (usage <- usages) {
          ...
          val leftPart = parent.flatMap {
            case ae: PsiAssignmentExpression
          ...
          val field =
            if (leftPart.isDefined) leftPart.get.resolve() match {
              case f: PsiField
                  if f.getContainingClass == constructor.getContainingClass &&
                    f.getInitializer == null => Some(f)
              case _                         => None
            }
            else None
          ...
          var statement: Option[PsiExpressionStatement] =
            if (field.isDefined && parent.isDefined &&
                parent.get.getParent.isInstanceOf[PsiExpressionStatement]) {
              Some(parent.get.getParent.asInstanceOf[PsiExpressionStatement])
            } else None

          if (statement.isDefined &&
              statement.get.getParent != constructor.getBody) {
            statement = None
          }
      ...
      def createContructor: PrimaryConstruction = {
        ...
        val updatedParams =
          new ArrayBuffer[(String, IntermediateNode, Boolean)]()
    ...
    def GetComplexPrimaryConstructor(): PsiMethod = {
      ...
      def tryFindWithoutParamConstructor(): PsiMethod = {
        val emptyParamsConstructors = constructors
          .filter(_.getParameterList.getParametersCount == 0)
    ...
    def handleModifiers: Seq[IntermediateNode] = {
      ...
      owner match {
        case method: PsiMethod =>
          ...
          if (method.findSuperMethods
                .exists(!_.hasModifierProperty("abstract")))
      ...
      if (!owner.hasModifierProperty(PsiModifier.PUBLIC) &&
        ...
        val packageName: String = owner.getContainingFile
          .asInstanceOf[PsiClassOwner].getPackageName
        if (packageName != "")
          modifiers.append(ModifierWithExpression(
      ...
      if (owner.hasModifierProperty(PsiModifier.FINAL) &&
          context.get.nonEmpty && !context.get.top._1) {
        owner match {
  ...
  private def getStatements(m: PsiMethod): Option[Array[PsiStatement]] =
    Option(m.getBody).map(_.getStatements)

  private def serialVersion(c: PsiClass): Option[PsiField] = {
    val serialField = c.findFieldByName("serialVersionUID", false)
    if (serialField != null &&
        serialField.getType.isAssignableFrom(PsiType.LONG) &&
        serialField.hasModifierProperty("static") &&
        serialField.hasModifierProperty("final") &&
