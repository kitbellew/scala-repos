...
/**
  ...
  */
class ScalaBreakpointMethodFilter(
  ...
  override def locationMatches(
    ...
    def signatureMatches(method: Method): Boolean = {
      ...
      sameNameMethods.exists { candidate =>
        candidate != method && candidate.isBridge &&
        expSign == candidate.signature()
      }
    }
  ...
  override def getBreakpointPosition: SourcePosition =
    firstStatementPosition.orNull

  override def getLastStatementLine: Int =
    lastStatementPosition.map(_.getLine).getOrElse(-1)
