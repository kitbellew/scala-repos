...
object FunExpressionTarget {
  def unapply(expr: ScExpression): Option[(Seq[ScBlockStatement], String)] = {
    expr match {
      ...
      case f: ScFunctionExpr =>
        Some(f.result.toSeq.flatMap(blockStmts), text(f))
  ...
  private def shorten(s: String): String = {
    ...
    if (lines.size > 1) lines.head + " ..." else trimmed
  }

  private def text(e: ScExpression) =
    parameterNameAndType(e).getOrElse(shorten(e.getText))
}
