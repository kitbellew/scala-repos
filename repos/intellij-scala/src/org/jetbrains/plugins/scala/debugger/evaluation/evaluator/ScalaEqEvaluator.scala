  ...
  def evaluate(context: EvaluationContextImpl): AnyRef = {
    ...
    (leftResult, rightResult) match {
      ...
      case (v1: PrimitiveValue, v2: PrimitiveValue)
          if DebuggerUtils.isInteger(leftResult) && DebuggerUtils.isInteger(
            rightResult) =>
        DebuggerUtilsEx.createValue(vm, "boolean", v1.longValue == v2.longValue)
      case (v1: PrimitiveValue, v2: PrimitiveValue)
          if DebuggerUtils.isNumeric(leftResult) && DebuggerUtils.isNumeric(
            rightResult) =>
      ...
      case (v1: CharValue, v2: CharValue) =>
        DebuggerUtilsEx.createValue(vm, "boolean", v1 == v2)
      case (v1: ObjectReference, v2: ObjectReference) =>
