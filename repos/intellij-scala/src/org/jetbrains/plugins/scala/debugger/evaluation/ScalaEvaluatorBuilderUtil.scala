...
private[evaluation] trait ScalaEvaluatorBuilderUtil {
      ...
      case Some(p: ScProjectionType) =>
        def exprToEvaluate(p: ScProjectionType): String =
          p.projected match {
            ...
            case ScThisType(cl) =>
              s"${cl.name}.this.${p.actualElement.name}"
            case _ =>
  ...
  def thisEvaluator(resolveResult: ScalaResolveResult): Evaluator = {
    ...
    val (outerClass, iterationCount) = findContextClass(e =>
      e == null || e == containingClass)
  ...
  def thisOrSuperEvaluator(
      refOpt: Option[ScStableCodeReferenceElement],
      isSuper: Boolean): Evaluator = {

    def thisEval(i: Int) =
      if (isSuper)
        new ScalaSuperEvaluator(i)
      else
        new ScalaThisEvaluator(i)

    def stableEvaluator(e: Evaluator) =
      if (isSuper)
        new ScalaSuperDelegate(e)
      else
        e

    def default: Evaluator = {
      val (result, iters) = findContextClass(e =>
        e == null || e.isInstanceOf[PsiClass])
      if (result == null)
        thisEval(0)
      else
        thisEval(iters)
    ...
    refOpt match {
      ...
      case Some(ref) =>
        ...
        val (result, iters) = findContextClass {
          case null =>
            true
          case cl: PsiClass if cl.name != null && cl.name == refName =>
            true
          case _ =>
            false
        }
  ...
  def localMethodEvaluator(
    ...
    val thisEvaluator: Evaluator =
      containingClass match {
        ...
        case _ =>
          val (outerClass, iters) = findContextClass(e =>
            e == null || e == containingClass)

          if (outerClass != null)
            new ScalaThisEvaluator(iters)
          else
            null
      }
  ...
  def stableObjectEvaluator(obj: ScObject): Evaluator = {
    val qualName =
      if (obj.isPackageObject)
        ...
        obj.getQualifiedNameForDebugger
    val qual = qualName
      .split('.')
  ...
  def objectEvaluator(
      obj: ScObject,
      qualEvaluator: () => Evaluator): Evaluator = {
    if (isStable(obj))
      stableObjectEvaluator(obj)
    else {
  ...
  def syntheticFunctionEvaluator(
      synth: ScSyntheticFunction,
    ...
    if (synth.isStringPlusMethod && arguments.length == 1) {
      ...
      val expr = ScalaPsiElementFactory
        .createExpressionWithContextFromText(exprText, ref.getContext, ref)
    ...
    def unaryEval(
        operatorName: String,
        function: Evaluator => Evaluator): Evaluator = {
    ...
    def equalsEval(opName: String): Evaluator = {
      val rawText = JVMNameUtil
        .getJVMRawText("(Ljava/lang/Object;Ljava/lang/Object;)Z")
      binaryEval(
        name,
        (l, r) =>
          new ScalaMethodEvaluator(
        ...
        unaryEval(
          ...
          eval =>
            new ScalaMethodEvaluator(
              BOXES_RUN_TIME,
              "hashFromObject",
              JVMNameUtil.getJVMRawText("(Ljava/lang/Object;)I"),
              Seq(boxEvaluator(eval))))
      case "==" =>
        equalsEval("==")
      case "!=" =>
        unaryEvaluator(equalsEval("!="), "takeNot")
      case "unary_!" =>
        unaryEvalForBoxes("!", "takeNot")
      case "unary_~" =>
        unaryEvalForBoxes("~", "complement")
      case "unary_+" =>
        unaryEvalForBoxes("+", "positive")
      case "unary_-" =>
        unaryEvalForBoxes("-", "negate")
      case "eq" =>
        binaryEval(name, eqEvaluator)
      case "ne" =>
        binaryEval(name, neEvaluator)
      case "<" =>
        binaryEvalForBoxes(name, "testLessThan")
      case ">" =>
        binaryEvalForBoxes(name, "testGreaterThan")
      case ">=" =>
        binaryEvalForBoxes(name, "testGreaterOrEqualThan")
      case "<=" =>
        binaryEvalForBoxes(name, "testLessOrEqualThan")
      case "+" =>
        binaryEvalForBoxes(name, "add")
      case "-" =>
        binaryEvalForBoxes(name, "subtract")
      case "*" =>
        binaryEvalForBoxes(name, "multiply")
      case "/" =>
        binaryEvalForBoxes(name, "divide")
      case "%" =>
        binaryEvalForBoxes(name, "takeModulo")
      case ">>" =>
        binaryEvalForBoxes(name, "shiftSignedRight")
      case "<<" =>
        binaryEvalForBoxes(name, "shiftSignedLeft")
      case ">>>" =>
        binaryEvalForBoxes(name, "shiftLogicalRight")
      case "&" =>
        binaryEvalForBoxes(name, "takeAnd")
      case "|" =>
        binaryEvalForBoxes(name, "takeOr")
      case "^" =>
        binaryEvalForBoxes(name, "takeXor")
      case "&&" =>
        conditionalAnd
      case "||" =>
        conditionalOr
      case "toInt" =>
        unaryEvalForBoxes(name, "toInteger")
      case "toChar" =>
        unaryEvalForBoxes(name, "toCharacter")
      case "toShort" =>
        unaryEvalForBoxes(name, "toShort")
      case "toByte" =>
        unaryEvalForBoxes(name, "toByte")
      case "toDouble" =>
        unaryEvalForBoxes(name, "toDouble")
      case "toLong" =>
        unaryEvalForBoxes(name, "toLong")
      case "toFloat" =>
        unaryEvalForBoxes(name, "toFloat")
      case "synchronized" =>
  ...
  def arrayMethodEvaluator(
    ...
    val message = ScalaBundle
      .message("wrong.number.of.arguments", s"Array.$name")
    name match {
      case "apply" =>
        if (argEvaluators.length == 1)
      ...
      case "length" =>
        if (argEvaluators.isEmpty)
          new ScalaFieldEvaluator(qualEval, "length")
        else
          throw EvaluationException(message)
      case "clone" =>
        if (argEvaluators.isEmpty)
    ...
    fun.getContext match {
      case tb: ScTemplateBody =>
        fun.containingClass match {
          case clazz: ScClass if clazz.qualifiedName == "scala.Array" =>
            true
          case _ =>
            false
        }
  ...
  def isClassOfFunction(fun: ScFunction): Boolean = {
    ...
    fun.getContext match {
      case tb: ScTemplateBody =>
        fun.containingClass match {
          case clazz: PsiClass if clazz.qualifiedName == "scala.Predef" =>
            true
          case _ =>
            false
        }
  ...
  def classOfFunctionEvaluator(ref: ScReferenceExpression) = {
    ...
    if (clazzJVMName != null)
      ...
      new ScalaLiteralEvaluator(null, Null)
  }

  def valueClassInstanceEvaluator(
      value: Evaluator,
      innerType: ScType,
      classType: ScType): Evaluator = {
    val valueClassType =
      new TypeEvaluator(DebuggerUtil.getJVMQualifiedName(classType))
    val innerJvmName = DebuggerUtil
      .getJVMStringForType(innerType, isParam = true)
  ...
  def repeatedArgEvaluator(
    ...
    def seqEvaluator: Evaluator = {
      ...
      val argTypeText =
        if (argTypes.isEmpty)
          expectedType.canonicalText
        else
          Bounds.lub(argTypes).canonicalText
      val argsText =
        if (exprsForP.nonEmpty)
          exprsForP
      ...
      val newExpr = ScalaPsiElementFactory
        .createExpressionWithContextFromText(exprText, context, context)
  ...
  def implicitArgEvaluator(
    ...
    val implicitParameters =
      fun.effectiveParameterClauses.lastOption match {
    ...
    val cannotFindMessage = ScalaBundle
      .message("cannot.find.implicit.parameters")
            ...
            case ScalaResolveResult(elem, _) =>
              ...
              val clazz =
                context.getContext match {
                  case _: ScTemplateBody | _: ScEarlyDefinitions =>
                    ScalaPsiUtil
                      .getContextOfType(context, true, classOf[PsiClass])
              ...
              clazz match {
                ...
                case o: ScObject => //todo: It can cover many cases!
                  throw EvaluationException(
                    ScalaBundle
                      .message("implicit.parameters.from.dependent.objects"))
                case _ =>
                  elem.name //from scope
              }
          }
        val newExpr = ScalaPsiElementFactory
          .createExpressionWithContextFromText(
  ...
  def parameterEvaluator(fun: PsiElement, resolve: PsiElement): Evaluator = {
    val name = NameTransformer
      .encode(resolve.asInstanceOf[PsiNamedElement].name)
    ...
    fun match {
      case funDef: ScFunctionDefinition =>
        def paramIndex(
          ...
          if (locIndex < 0)
            funParams.indexOf(elem)
          else
            locIndex + funParams.size
        }
  ...
  def javaMethodEvaluator(
      method: PsiMethod,
      ref: ScReferenceExpression,
      arguments: Seq[ScExpression]): Evaluator = {

    def boxArguments(
        arguments: Seq[Evaluator],
        method: PsiElement): Seq[Evaluator] = {
      val params =
        method match {
          case fun: ScMethodLike =>
            fun.effectiveParameterClauses.flatMap(_.parameters)
          case m: PsiMethod =>
            m.getParameterList.getParameters.toSeq
          case _ =>
            return arguments
        }
      ...
      arguments
        ...
        .map {
          case (arg, i) =>
            if (params.length <= i || isOfPrimitiveType(params(i)))
              arg
            else
              boxEvaluator(arg)
        }
    ...
    val methodPosition = DebuggerUtil
      .getSourcePositions(method.getNavigationElement)
    ...
    ref.qualifier match {
      ...
      case Some(q) if method.hasModifierPropertyScala("static") =>
        val eval =
          new TypeEvaluator(
            JVMNameUtil.getContextClassJVMQualifiedName(
              SourcePosition.createFromElement(method)))
  ...
  def unresolvedMethodEvaluator(
      ref: ScReferenceExpression,
      args: Seq[ScExpression]): Evaluator = {
    ...
    ref.qualifier match {
      ...
      case _ =>
        new ScalaMethodEvaluator(
          thisOrImportedQualifierEvaluator(ref),
          name,
          null,
          argEvals)
    }
  ...
  def argumentEvaluators(
    ...
    def addForNextClause(
      ...
      def isDefaultExpr(expr: ScExpression) =
        expr match {
          case ChildOf(p: ScParameter) =>
            p.isDefaultParam
          case _ =>
            false
        }
      previousClausesEvaluators ++ clause
        .effectiveParameters
        .map {
          case param =>
            ...
            if (p.isByName)
              throw new NeedCompilationException(
                ScalaBundle.message("method.with.by-name.parameters"))

            val evaluator =
              if (p.isRepeated)
                repeatedArgEvaluator(exprsForP, p.expectedType, call)
              else if (exprsForP.size > 1)
                throw EvaluationException(
                  ScalaBundle.message("wrong.number.of.expressions"))
            ...
            if (!isOfPrimitiveType(param))
              boxEvaluator(evaluator)
            else
              evaluator
        }
    }
    ...
    if (argEvaluators.contains(null))
      arguments.map(arg => evaluatorFor(arg))
    else
      argEvaluators
  }

  def functionEvaluator(
      qualOption: Option[ScExpression],
      ref: ScReferenceExpression,
      funName: String,
      argEvaluators: Seq[Evaluator]): Evaluator = {
    ...
    ref.bind() match {
      ...
      case None =>
        throw EvaluationException(
          ScalaBundle.message("cannot.evaluate.method", funName))
      case Some(r @ privateTraitMethod(tr, fun)) =>
        val traitTypeEval =
          new TypeEvaluator(
            DebuggerUtil.getClassJVMName(tr, withPostfix = true))
        val qualEval = qualEvaluator(r)
        new ScalaMethodEvaluator(
          traitTypeEval,
          name,
          null,
          qualEval +: argEvaluators)
      case Some(r) =>
  ...
  def methodCallEvaluator(
    ...
    val ref =
      call match {
        case hasDeepestInvokedReference(r) =>
          r
        case _ =>
          throw EvaluationException(
      ...
      case fun: ScFunction =>
        ref match {
          case isInsideValueClass(c) if qualOption.isEmpty =>
            ...
            val expr = ScalaPsiElementFactory
              .createExpressionFromText(text, call.getContext)
            evaluatorFor(expr)
          case _ =>
  ...
  def evaluatorForReferenceWithoutParameters(
        ...
        ScalaPsiUtil.nameContext(named) match {
          case param: ScParameter =>
            param.owner match {
              ...
              case _ =>
                throw EvaluationException(
                  ScalaBundle.message("cannot.evaluate.parameter", param.name))
            }
          case caseCl: ScCaseClause =>
            patternEvaluator(caseCl, named)
    ...
    resolve match {
      case Both(isInsideLocalFunction(fun), named: PsiNamedElement)
        ...
        new ScalaDuplexEvaluator(
          calcLocal(named),
          parameterEvaluator(fun, resolve))
      ...
      case cp: ScClassParameter
          if qualifier
            .isEmpty && ValueClassType.isValueClass(cp.containingClass) =>
      ...
      case pack: ScPackage =>
        ...
        val qual = (pack.getQualifiedName + ".package$")
          .split('.')
          .map(NameTransformer.encode)
          .mkString(".")
        stableObjectEvaluator(qual)
      case _ =>
        ...
        val fieldOrVarEval =
          qualifier match {
            case Some(qual) =>
              new ScalaFieldEvaluator(evaluatorFor(qual), name)
            case None =>
              new ScalaLocalVariableEvaluator(name, fileName)
          }
        new ScalaDuplexEvaluator(
          fieldOrVarEval,
          unresolvedMethodEvaluator(ref, Seq.empty))
    }
  }

  def labeledOrSyntheticEvaluator(
      ref: ScReferenceExpression,
      resolve: PsiElement): Option[Evaluator] = {
    if (resolve == null)
      return None

    val labeledValue = resolve
      .getUserData(CodeFragmentFactoryContextWrapper.LABEL_VARIABLE_VALUE_KEY)
  ...
  def patternEvaluator(
      caseCl: ScCaseClause,
      namedElement: PsiNamedElement): Evaluator = {
    ...
    if (caseCl.getParent != null) {
      ...
      caseCl.getParent.getParent match {
        case matchStmt: ScMatchStmt if namedElement.isInstanceOf[ScPattern] =>
          ...
          if (expr.isEmpty)
            throw EvaluationException(
              ScalaBundle.message("cannot.find.expression.of.match"))
          val exprEval = evaluatorFor(expr.get)
          val fromPatternEvaluator = evaluateSubpatternFromPattern(
            exprEval,
            pattern.get,
            namedElement.asInstanceOf[ScPattern])
          new ScalaDuplexEvaluator(
            new ScalaLocalVariableEvaluator(name, fileName),
    ...
    if (stmt.isNamedParameter) {
      ...
      stmt.getLExpression match {
        case call: ScMethodCall =>
          ...
          val args = (call.args.exprs.map(_.getText) :+ rExprText)
            .mkString("(", ", ", ")")
        ...
        case _ =>
          ...
          createAssignEvaluator(leftEvaluator)
            .getOrElse(new AssignmentEvaluator(leftEvaluator, rightEvaluator))
      }
    }
  }
  ...
  def evaluateSubpatternFromPattern(
    ...
    def evaluateConstructorOrInfix(
        exprEval: Evaluator,
        ...
        case fun: ScFunctionDefinition =>
          ...
          val refExpr = ScalaPsiElementFactory
            .createExpressionWithContextFromText(
              ...
              ref.getContext,
              ref)
          val refEvaluator = evaluatorForReferenceWithoutParameters(
            qual,
            elem,
            refExpr.asInstanceOf[ScReferenceExpression])
          ...
          val newEval =
            if (funName == "unapply") {
              ...
              val getEval =
                new ScalaMethodEvaluator(extractEval, "get", null, Seq.empty)
              val indexExpr = ScalaPsiElementFactory.createExpressionFromText(
                "" + nextPatternIndex,
                pattern.getManager)
    ...
    val nextPatternIndex: Int = pattern
      .subpatterns
      .indexWhere(next =>
      ...
      pattern match {
        ...
        case typed: ScTypedPattern =>
          evaluateSubpatternFromPattern(
            exprEval,
            pattern.subpatterns.head,
            subPattern)
        case par: ScParenthesisedPattern =>
          val withoutPars = par
            .subpattern
            .getOrElse(
              throw new IllegalStateException("Empty parentheses pattern"))
        ...
        case tuple: ScTuplePattern =>
          ...
          val newEval =
            new ScalaFieldEvaluator(exprEval, s"_${nextPatternIndex + 1}")
          evaluateSubpatternFromPattern(newEval, nextPattern, subPattern)
        ...
        case _: ScCompositePattern =>
          throw EvaluationException(
            ScalaBundle.message("pattern.alternatives.cannot.bind.vars"))
        case _: ScXmlPattern =>
          throw EvaluationException(
        ...
        parents.constructor match {
          case Some(constr) =>
            ...
            ScType.extractClass(tp, Some(templ.getProject)) match {
              ...
              case _ =>
                throw EvaluationException(
                  ScalaBundle.message("new.expression.without.class.reference"))
            }
  ...
  def constructorArgumentsEvaluators(
      newTd: ScNewTemplateDefinition,
      constr: ScConstructor,
      clazz: PsiClass): Seq[Evaluator] = {
    val constrDef =
      constr.reference match {
        case Some(ResolvesTo(elem)) =>
          elem
        case _ =>
          throw EvaluationException(
    ...
    val explEvaluators =
      for {
        ...
        if (param.exists(!isOfPrimitiveType(_)))
          boxEvaluator(eval)
        else
          eval
      }
        ...
        val outerThis =
          outerClass match {
            ...
            case null =>
              None
            case _ =>
              Some(new ScalaThisEvaluator(iters))
          }
  ...
  def fromLocalArgEvaluator(local: ScTypedDefinition): Evaluator = {
    ...
    val ref = ScalaPsiElementFactory
      .createExpressionWithContextFromText(name, elemAt, elemAt)
  ...
  def expressionFromTextEvaluator(
      ...
      context: PsiElement): Evaluator = {
    val expr = ScalaPsiElementFactory
      .createExpressionWithContextFromText(string, context.getContext, context)
    evaluatorFor(expr)
  }
  ...
  def literalEvaluator(l: ScLiteral): Evaluator = {
    l match {
      case interpolated: ScInterpolatedStringLiteral =>
        val evaluatorOpt = interpolated
          .getStringContextExpression
          .map(evaluatorFor(_))
        evaluatorOpt.getOrElse(ScalaLiteralEvaluator(l))
      case _ if l.isSymbol =>
        val value = l.getValue.asInstanceOf[Symbol].name
        val expr = ScalaPsiElementFactory
          .createExpressionFromText(s"""Symbol("$value")""", l.getContext)
        evaluatorFor(expr)
      case _ =>
        ScalaLiteralEvaluator(l)
    }
  }
  ...
  def infixExpressionEvaluator(infix: ScInfixExpr): Evaluator = {
    ...
    def isUpdate(ref: ScReferenceExpression): Boolean = {
      ...
      (
        ref.resolve() match {
          case n: PsiNamedElement if n.name + "=" == ref.refName =>
            true
          case _ =>
            false
        }
  ...
  def prefixExprEvaluator(p: ScPrefixExpr): Evaluator = {
    ...
    val newExpr = ScalaPsiElementFactory
      .createExpressionWithContextFromText(newExprText, p.getContext, p)
    evaluatorFor(newExpr)
  }
  ...
  def tupleEvaluator(tuple: ScTuple): Evaluator = {
    val exprText = "_root_.scala.Tuple" + tuple.exprs.length + tuple
      .exprs
      .map(_.getText)
      .mkString("(", ", ", ")")
    val expr = ScalaPsiElementFactory
  ...
  def variableDefinitionEvaluator(vd: ScVariableDefinition): Evaluator = {
    vd.expr match {
      ...
      case Some(e) =>
        valOrVarDefinitionEvaluator(vd.pList, e)
    }
  }
  ...
  def patternDefinitionEvaluator(pd: ScPatternDefinition): Evaluator = {
    pd.expr match {
      ...
      case Some(e) =>
        valOrVarDefinitionEvaluator(pd.pList, e)
    }
  }
...
object ScalaEvaluatorBuilderUtil {
  private val BOXES_RUN_TIME =
    new TypeEvaluator(JVMNameUtil.getJVMRawText("scala.runtime.BoxesRunTime"))
  private val BOXED_UNIT =
    new TypeEvaluator(JVMNameUtil.getJVMRawText("scala.runtime.BoxedUnit"))
  ...
  def notEvaluator(eval: Evaluator): Evaluator = {
    val rawText = JVMNameUtil
      .getJVMRawText("(Ljava/lang/Object;)Ljava/lang/Object;")
    unboxEvaluator(
      new ScalaMethodEvaluator(BOXES_RUN_TIME, "takeNot", rawText, boxed(eval)))
  }
  ...
  def binaryEvaluator(
    ...
    val rawText = JVMNameUtil
      .getJVMRawText("(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;")
    unboxEvaluator(
      new ScalaMethodEvaluator(
        BOXES_RUN_TIME,
        boxesRunTimeName,
  ...
  object hasDeepestInvokedReference {
    ...
    final def unapply(expr: ScExpression): Option[ScReferenceExpression] = {
      expr match {
        case call: ScMethodCall =>
          unapply(call.deepestInvokedExpr)
        case genCall: ScGenericCall =>
          unapply(genCall.referencedExpr)
        case ref: ScReferenceExpression =>
          Some(ref)
        case _ =>
          None
  ...
  def classTagText(arg: ScType): String = {
    ...
    arg match {
      case Short =>
        "_root_.scala.reflect.ClassTag.Short"
      case Byte =>
        "_root_.scala.reflect.ClassTag.Byte"
      case Char =>
        "_root_.scala.reflect.ClassTag.Char"
      case Int =>
        "_root_.scala.reflect.ClassTag.Int"
      case Long =>
        "_root_.scala.reflect.ClassTag.Long"
      case Float =>
        "_root_.scala.reflect.ClassTag.Float"
      case Double =>
        "_root_.scala.reflect.ClassTag.Double"
      case Boolean =>
        "_root_.scala.reflect.ClassTag.Boolean"
      case Unit =>
        "_root_.scala.reflect.ClassTag.Unit"
      case Any =>
        "_root_.scala.reflect.ClassTag.Any"
      case AnyVal =>
        "_root_.scala.reflect.ClassTag.AnyVal"
      case Nothing =>
        "_root_.scala.reflect.ClassTag.Nothing"
      case Null =>
        "_root_.scala.reflect.ClassTag.Null"
      case Singleton =>
        "_root_.scala.reflect.ClassTag.Object"
      //todo:
      case _ =>
  ...
  def classManifestText(scType: ScType): String = {
    ...
    scType match {
      case Short =>
        "_root_.scala.reflect.ClassManifest.Short"
      case Byte =>
        "_root_.scala.reflect.ClassManifest.Byte"
      case Char =>
        "_root_.scala.reflect.ClassManifest.Char"
      case Int =>
        "_root_.scala.reflect.ClassManifest.Int"
      case Long =>
        "_root_.scala.reflect.ClassManifest.Long"
      case Float =>
        "_root_.scala.reflect.ClassManifest.Float"
      case Double =>
        "_root_.scala.reflect.ClassManifest.Double"
      case Boolean =>
        "_root_.scala.reflect.ClassManifest.Boolean"
      case Unit =>
        "_root_.scala.reflect.ClassManifest.Unit"
      case Any =>
        "_root_.scala.reflect.ClassManifest.Any"
      case AnyVal =>
        "_root_.scala.reflect.ClassManifest.AnyVal"
      case Nothing =>
        "_root_.scala.reflect.ClassManifest.Nothing"
      case Null =>
        "_root_.scala.reflect.ClassManifest.Null"
      case Singleton =>
        "_root_.scala.reflect.ClassManifest.Object"
  ...
  def isOfPrimitiveType(param: PsiParameter) =
    param match { //todo specialized type parameters
      case p: ScParameter =>
      ...
      implicits._2 match {
        case Some(fun: ScFunction) =>
          ...
          val newExprText =
            fun.containingClass match {
              case o: ScObject if isStable(o) =>
                s"${o.qualifiedName}.$callText"
              case o: ScObject => //todo: It can cover many cases!
                throw EvaluationException(
                  ScalaBundle
                    .message("implicit.conversions.from.dependent.objects"))
              case _ =>
                callText //from scope
            }
          Some(
  ...
  final def isStable(o: ScObject): Boolean = {
    val context = PsiTreeUtil
      .getParentOfType(o, classOf[ScTemplateDefinition], classOf[ScExpression])
    if (context == null)
      return true
    context match {
      case o: ScObject =>
  ...
  def getContextClass(elem: PsiElement, strict: Boolean = true): PsiElement = {
    if (!strict && isGenerateClass(elem))
      elem
    else
  ...
  def isGenerateClass(elem: PsiElement): Boolean = {
    if (ScalaPositionManager.isCompiledWithIndyLambdas(elem.getContainingFile))
      ...
      isGenerateNonAnonfunClass(elem) || isGenerateAnonfun(elem)
  }
  ...
  def isAnonfunInsideSuperCall(elem: PsiElement) = {
    def isInsideSuperCall(td: ScTypeDefinition) = {
      ...
      PsiTreeUtil.getParentOfType(
        ...
        classOf[ScConstructor]) match {
        case ed: ScEarlyDefinitions if ed.getParent == extBlock =>
          true
        case c: ScConstructor if c.getParent.getParent == extBlock =>
          true
        case _ =>
          false
      }
    ...
    val containingClass = PsiTreeUtil
      .getParentOfType(elem, classOf[ScTypeDefinition])
      ...
      def computation =
        elem match {
          ...
          case e: ScExpression
            ...
            true
          case ScalaPsiUtil.MethodValue(_) =>
            true
          case Both(
    ...
    def isGenerateAnonfunSimple: Boolean = {
      elem match {
        case f: ScFunctionExpr =>
          true
        case (_: ScExpression) childOf (_: ScForStatement) =>
          true
        case (cc: ScCaseClauses) childOf (b: ScBlockExpr)
            if b.isAnonymousFunction =>
          true
        case (g: ScGuard) childOf (_: ScEnumerators) =>
          true
        case (g: ScGenerator) childOf (enums: ScEnumerators)
            if !enums.generators.headOption.contains(g) =>
          true
        case e: ScEnumerator =>
          true
        case _ =>
  ...
  def localFunctionIndex(named: PsiNamedElement): Int = {
    elementsWithSameNameIndex(
      named,
      {
        case f: ScFunction if f.isLocal && f.name == named.name =>
          true
  ...
  def elementsWithSameNameIndex(
      ...
      condition: PsiElement => Boolean): Int = {
    val containingClass = getContextClass(named)
    ...
    val depthFirstIterator = containingClass.depthFirst {
      case `containingClass` =>
        true
      case elem if isGenerateClass(elem) =>
        false
      case _ =>
        true
    }
  ...
  def isNotUsedEnumerator(
      named: PsiNamedElement,
      place: PsiElement): Boolean = {
    named match {
      case ScalaPsiUtil
            .inNameContext(enum @ (_: ScEnumerator | _: ScGenerator)) =>
      ...
      def inner(element: PsiElement): Option[ScFunction] = {
        element match {
          ...
          case fun: ScFunction
            ...
            Some(fun)
          case other if other.getContext != null =>
            inner(other.getContext)
          case _ =>
            None
        }
      }
