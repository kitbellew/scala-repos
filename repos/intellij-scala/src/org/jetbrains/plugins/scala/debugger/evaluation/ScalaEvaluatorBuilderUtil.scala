...
private[evaluation] trait ScalaEvaluatorBuilderUtil {
  ...
  def fileName =
    contextClass.toOption.flatMap(_.getContainingFile.toOption).map(_.name)
      .orNull
  ...
  def importedQualifierEvaluator(
    ...
    resolveResult.fromType match {
      case Some(ScDesignatorType(element)) => element match {
  ...
  def thisEvaluator(resolveResult: ScalaResolveResult): Evaluator = {
    ...
    containingClass match {
      case o: ScObject if isStable(o) => return stableObjectEvaluator(o)
      case _                          =>
    }
  ...
  def thisOrSuperEvaluator(
    ...
    refOpt match {
      case Some(ResolvesTo(clazz: PsiClass)) => clazz match {
  ...
  def localMethodEvaluator(
    ...
    val thisEvaluator: Evaluator = containingClass match {
      case obj: ScObject if isStable(obj) => stableObjectEvaluator(obj)
      case t: ScTrait                     => thisOrSuperEvaluator(None, isSuper = true)
      case _ =>
  ...
  def isArrayFunction(fun: ScFunction): Boolean = {
    fun.getContext match {
      case tb: ScTemplateBody => fun.containingClass match {
    ...
    def seqEvaluator: Evaluator = {
      ...
      val argsText =
        if (exprsForP.nonEmpty)
          exprsForP.sortBy(_.getTextRange.getStartOffset).map(_.getText)
            .mkString(".+=(", ").+=(", ").result()")
        else ""
      val exprText =
      ...
      case Some(resolveResults)
        ...
        val exprText = resolveResults(i) match {
          case ScalaResolveResult(clazz: ScTrait, substitutor)
            ...
            argType match {
              case ScParameterizedType(tp, Seq(paramType)) => classManifestText(
                  paramType)
              case _ => throw EvaluationException(cannotFindMessage)
            }
          case ScalaResolveResult(clazz: ScTrait, substitutor)
        ...
        val newExpr = ScalaPsiElementFactory
          .createExpressionWithContextFromText(
            ...
            owner)
        evaluatorFor(newExpr)
      case None => throw EvaluationException(cannotFindMessage)
    }
    ...
    fun match {
      case funDef: ScFunctionDefinition =>
        def paramIndex(
          ...
          val funParams = fun.effectiveParameterClauses
  ...
  def argumentEvaluators(
    ...
    val parameters = clauses.flatMap(_.effectiveParameters)
      .map(new Parameter(_))
    ...
    def addForNextClause(
      ...
      previousClausesEvaluators ++ clause.effectiveParameters.map {
        case param =>
          ...
          val exprsForP = matchedParameters.find(_._1.name == p.name).map(_._2)
            .getOrElse(Seq.empty).filter(_ != null)
          if (p.isByName)
    ...
    val argEvaluators: Seq[Evaluator] = clauses
      .foldLeft(Seq.empty[Evaluator])(addForNextClause)
  ...
  def functionEvaluator(
    ...
    ref.bind() match {
      ...
      case Some(r @ privateTraitMethod(tr, fun)) =>
        val traitTypeEval = new TypeEvaluator(
  ...
  def methodCallEvaluator(
    ...
    resolve match {
      ...
      case fun: ScFunction => ref match {
          case isInsideValueClass(c) if qualOption.isEmpty =>
            val clName = c.name
            val paramName = c.allClauses.flatMap(_.parameters).map(_.name)
      ...
      case method: PsiMethod => javaMethodEvaluator(method, ref, arguments)
      case _                 => unresolvedMethodEvaluator(ref, arguments)
    }
  }
  ...
  def evaluatorForReferenceWithoutParameters(
    ...
    def calcLocal(named: PsiNamedElement): Evaluator = {
      ...
      val localVariableEvaluator: Evaluator = ScalaPsiUtil
        .nameContext(named) match {
        case param: ScParameter => param.owner match {
    ...
    resolve match {
      ...
      case named: PsiNamedElement if isLocalValue => calcLocal(named)
      case obj: ScObject =>
      ...
      case _: ScClassParameter | _: ScBindingPattern =>
        ...
        val name = NameTransformer.encode(named.name)
        val qualEval = qualifierEvaluator(qualifier, ref)
        val withSimpleNameEval = new ScalaMethodEvaluator(
          qualEval,
          name,
          null /* todo */,
          Seq.empty,
        ...
        getContextClass(named) match {
          ...
          case clazz: ScTemplateDefinition
            ...
            val newName = qualName.split('.').map(NameTransformer.encode)
              .mkString("$") + "$$" + name
            val reserveEval = new ScalaMethodEvaluator(
              qualEval,
              newName,
              null /* todo */,
              Seq.empty,
      ...
      case pack: ScPackage =>
        ...
        val qual = (pack.getQualifiedName + ".package$").split('.')
          .map(NameTransformer.encode).mkString(".")
        stableObjectEvaluator(qual)
      case _ =>
  ...
  def constructorArgumentsEvaluators(
    ...
    val explEvaluators = for { arg <- explicitArgs } yield {
  ...
  def literalEvaluator(l: ScLiteral): Evaluator = {
    l match {
      case interpolated: ScInterpolatedStringLiteral =>
        val evaluatorOpt = interpolated.getStringContextExpression
          .map(evaluatorFor(_))
        ...
        case gen: ScGenericCall => gen.referencedExpr match {
            case ref: ScReferenceExpression
              ...
              methodCallEvaluator(
                ...
                call.argumentExpressions ++ collected,
                matchedParameters ++ call.matchedParametersMap)
            case ref: ScReferenceExpression => ref.getType().getOrAny match {
  ...
  def blockExprEvaluator(block: ScBlock): Evaluator = {
    withNewSyntheticVariablesHolder {
      val evaluators = block.statements.filter(!_.isInstanceOf[ScImportStmt])
        .map(evaluatorFor)
...
object ScalaEvaluatorBuilderUtil {
  ...
  private val BOXED_UNIT = new TypeEvaluator(
    JVMNameUtil.getJVMRawText("scala.runtime.BoxedUnit"))
  ...
  def isGenerateNonAnonfunClass(elem: PsiElement): Boolean = {
    elem match {
      case newTd: ScNewTemplateDefinition
          if !DebuggerUtil.generatesAnonClass(newTd) => false
  ...
  def isGenerateAnonfun(elem: PsiElement): Boolean = {
    def isGenerateAnonfunWithCache: Boolean = {
      ...
      def computation =
        elem match {
          case e: ScExpression
              if ScUnderScoreSectionUtil.underscores(e).nonEmpty => true
  ...
  def traitImplementation(elem: PsiElement): Option[JVMName] = {
    ...
    clazz match {
      case t: ScTrait => Some(
          DebuggerUtil.getClassJVMName(t, withPostfix = true))
      case _ => None
    }
  }
      ...
      case ScalaPsiUtil
        ...
        enum.getParent.getParent match {
          case ScForStatement(enums, body) =>
            ...
            def isNotUsed =
              ReferencesSearch.search(named, new LocalSearchScope(body))
                .findFirst() == null
          ...
          v match {
            case mo: ScModifierListOwner
                if mo.getModifierList.accessModifier.exists(am =>
                  am.isPrivate && am.isThis) => Some(bp)
            case _                           => None
          }
        case _ => None
      }
