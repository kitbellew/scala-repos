...
class ScalaPositionManager(val debugProcess: DebugProcess)
  ...
  private def calcPosition(
    ...
    def calcElement(): Option[PsiElement] = {
      ...
      if (possiblePositions.size <= 1) {
        ...
        possiblePositions.find {
          case e: PsiElement if isLambda(e) =>
            false
          case (e: ScExpression) childOf (p: ScParameter) =>
            false
          case _ =>
            true
        }
  ...
  private def getPsiFileByReferenceType(
    ...
    def findFile() = {
      ...
      def topLevelClassName(originalQName: String): String = {
        if (originalQName.endsWith(packageSuffix))
          originalQName
        else
          originalQName.replace(packageSuffix, ".").takeWhile(_ != '$')
      }
  ...
  def findElementByReferenceType(refType: ReferenceType): Option[PsiElement] = {
    ...
    refTypeToElementCache.get(refType) match {
      case Some(Some(p)) if p.getElement != null =>
        Some(p.getElement)
  ...
  private def findElementByReferenceTypeInner(
    ...
    def findCandidates(): Seq[PsiElement] = {
      ...
      if (lastRefTypeLine - firstRefTypeLine >= 2) {
        ...
        val allPositions =
          if (firstRefTypeLine == lastRefTypeLine)
            firstLinePositions
          else
            firstLinePositions ++ positionsOnLine(file, lastRefTypeLine)
        allPositions.distinct.filter(isAppropriateCandidate)
      }
    }

    def filterWithSignature(candidates: Seq[PsiElement]) = {
      val applySignature = refType
        .methodsByName("apply")
        .asScala
        .find(m => !m.isSynthetic)
        .map(_.signature())
    ...
    if (candidates.exists(!isLambda(_)))
      return candidates.headOption
    ...
    if (byContainingClasses.size > 1) {
      findContainingClass(refType) match {
        case Some(e) =>
          return byContainingClasses.get(e).flatMap(_.headOption)
        case None =>
      }
    }
  ...
  private def findByQualName(refType: ReferenceType): Option[PsiClass] = {
    ...
    val withoutSuffix =
      if (endsWithPackageSuffix)
        originalQName.stripSuffix(packageSuffix)
      else
        originalQName.stripSuffix("$").stripSuffix("$class")
    val withDots = withoutSuffix.replace(packageSuffix, ".").replace('$', '.')
    val transformed =
      if (endsWithPackageSuffix)
        withDots + packageSuffix
      else
        withDots
  ...
  private def findByShortName(refType: ReferenceType): Option[PsiClass] = {
    ...
    if (inSameFile.length == 1)
      ...
      if (isScalaObject)
        inSameFile.find(_.isInstanceOf[ScObject])
      else
        inSameFile.find(!_.isInstanceOf[ScObject])
    } else
      None
...
object ScalaPositionManager {
  ...
  def instance(vm: VirtualMachine): Option[ScalaPositionManager] =
    instances.collectFirst {
      case (process, manager) if getVM(process).contains(vm) =>
        manager
    }
  ...
  private def getVM(debugProcess: DebugProcess) = {
    debugProcess.getVirtualMachineProxy match {
      case impl: VirtualMachineProxyImpl =>
        Option(impl.getVirtualMachine)
      case _ =>
        None
    }
  }
  ...
  def positionsOnLine(file: PsiFile, lineNumber: Int): Seq[PsiElement] = {
    ...
    CachedValuesManager
      .getCachedValue(file, cacheProvider)
  ...
  def findGeneratingClassOrMethodParent(element: PsiElement): PsiElement = {
    element match {
      ...
      case InsideMacro(macroCall) =>
        macroCall
      case elem =>
        findGeneratingClassOrMethodParent(elem.getParent)
    }
  }
  ...
  private class MyClassPrepareRequestor(
    ...
    def processClassPrepare(
      ...
      if (!sourceNameOf(referenceType).contains(sourceName))
        return
    ...
    private def partsFor(elem: PsiElement): Seq[String] = {
      elem match {
        case td: ScTypeDefinition =>
          Seq(ScalaNamesUtil.toJavaName(td.name))
        case newTd: ScNewTemplateDefinition
            if DebuggerUtil.generatesAnonClass(newTd) =>
          Seq("$anon")
  ...
  private object NamePattern {
    def forElement(elem: PsiElement): NamePattern = {
      ...
      val cacheProvider =
        new CachedValueProvider[NamePattern] {
