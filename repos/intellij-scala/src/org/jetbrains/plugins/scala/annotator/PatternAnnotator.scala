...
trait PatternAnnotator {
  ...
  def annotatePattern(
    ...
    if (highlightErrors) { PatternAnnotator.checkPattern(pattern, holder) }
  }
}
          ...
          case Some(ref) => ref.bind() match {
              case Some(ScalaResolveResult(fun: ScFunction, _))
                ...
                fun.returnType match {
                  case Success(rt, _) =>
                    val expected = ScPattern.expectedNumberOfExtractorArguments(
                      rt,
                      pattern,
...
object PatternAnnotatorUtil {
  ...
  def matchesPattern(matching: ScType, matched: ScType): Boolean = {
    def abstraction(
      ...
      if (visited.contains(scType)) { return scType }
