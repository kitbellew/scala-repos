...
class ScalaAnnotator
    ...
    val visitor =
      new ScalaElementVisitor {
        ...
        override def visitParameterizedTypeElement(
          ...
          val tp = parameterized
            .typeElement
            .getTypeNoConstructor(TypingContext.empty)
          tp match {
            case Success(res, _) =>
        ...
        override def visitEnumerator(enum: ScEnumerator) {
          enum.valKeyword match {
            case Some(valKeyword) =>
              ...
              annotation
                .registerFix(new RemoveValFromEnumeratorIntentionAction(enum))
            case _ =>
          }
        ...
        override def visitGenerator(gen: ScGenerator) {
          gen.valKeyword match {
            case Some(valKeyword) =>
              ...
              annotation
                .registerFix(new RemoveValFromGeneratorIntentionAction(gen))
            case _ =>
          }
        ...
        private def referencePart(ref: ScReferenceElement) {
          ...
          ref.qualifier match {
            case None =>
              checkNotQualifiedReferenceElement(ref, holder)
            case Some(_) =>
              checkQualifiedReferenceElement(ref, holder)
          }
        }
  ...
  def isAdvancedHighlightingEnabled(element: PsiElement): Boolean = {
    if (!HighlightingAdvisor.getInstance(element.getProject).enabled)
      return false
    element.getContainingFile match {
      case file: ScalaFile =>
        if (file.isCompiled)
        ...
        if (vFile != null && ProjectFileIndex
              .SERVICE
    ...
    val count =
      containingFile.getManager.getModificationTracker.getModificationCount
  ...
  def checkCatchBlockGeneralizedRule(
      block: ScCatchBlock,
      ...
      case Some(expr) =>
        ...
        def checkMember(memberName: String, checkReturnTypeIsBoolean: Boolean) {
          ...
          if (candidates.length != 1) {
            ...
            def error() {
              val error = ScalaBundle
                .message("expected.type.boolean", memberName)
              val annotation = holder.createErrorAnnotation(expr, error)
              annotation.setHighlightType(ProblemHighlightType.GENERIC_ERROR)
            }
            candidates(0) match {
                    ...
                    if (!conformance) {
                      if (typeAware) {
                        ...
                        typeElement match {
                          ...
                          case Some(te)
                              if te.containingFile == t.containingFile =>
                            val fix =
  ...
  private def annotationWithoutHighlighting(
      holder: AnnotationHolder,
      te: PsiElement): Annotation = {
  ...
  private def checkTypeParamBounds(
      sTypeParam: ScTypeBoundsOwner,
      holder: AnnotationHolder) {
    for {
  ...
  private def registerUsedElement(
    ...
    if (named.isValid && named.getContainingFile == file &&
      ...
      val value: ValueUsed =
        element match {
          case ref: ScReferenceExpression
              if checkWrite &&
                ScalaPsiUtil
                  .isPossiblyAssignment(ref.asInstanceOf[PsiElement]) =>
      ...
      resolveResult
        .parentElement
        .foreach(parent => holder.registerValueUsed(ReadValueUsed(parent)))
    }
  }

  def checkBoundsVariance(
      toCheck: PsiElement,
      holder: AnnotationHolder,
      toHighlight: PsiElement,
      checkParentOf: PsiElement,
      varianceOfUpper: Int = ScTypeParam.Covariant,
      checkTypeDeclaredSameBracket: Boolean = true,
      insideParameterized: Boolean = false) {
    toCheck match {
    ...
    toCheck match {
      case paramOwner: ScTypeParametersOwner =>
        val inParameterized =
          if (paramOwner.isInstanceOf[ScTemplateDefinition])
            false
          else
            true
    ...
    def checkAndHighlightBounds(
        boundOption: Option[ScTypeElement],
        expectedVariance: Int) {
      boundOption match {
        case Some(bound)
            if !childHasAnnotation(Some(bound), "uncheckedVariance") =>
          checkVariance(
            bound.calcType,
            expectedVariance,
            toHighlight,
  ...
  private def checkNotQualifiedReferenceElement(
    ...
    def getFix: Seq[IntentionAction] = {
      val classes = ScalaImportTypeFix
        .getTypesToImport(refElement, refElement.getProject)
      if (classes.length == 0)
        return Seq.empty
    ...
    if (resolve.length != 1 && !goodDoc) {
      if (resolve
            .length == 0) { //Let's try to hide dynamic named parameter usage
        refElement match {
      ...
      refElement match {
        ...
        case _ =>
          refElement.getParent match {
            ...
            case _ =>
              processError(countError = true, fixes = getFix)
          }
      }
    } else {
    ...
    if (isAdvancedHighlightingEnabled(refElement) && resolve
      ...
      def addCreateApplyOrUnapplyFix(
          messageKey: String,
          fix: ScTypeDefinition => IntentionAction): Boolean = {
        val refWithoutArgs = ScalaPsiElementFactory.createReferenceFromText(
          refElement.getText,
          parent.getContext,
          parent)
        if (refWithoutArgs != null && refWithoutArgs
          ...
          val annotation = holder
            .createErrorAnnotation(refElement.nameId, error)
          annotation.setHighlightType(ProblemHighlightType.GENERIC_ERROR)
          annotation.registerFix(ReportHighlightingErrorQuickFix)
          refWithoutArgs match {
            case ResolvesTo(obj: ScObject) =>
              annotation.registerFix(fix(obj))
            case InstanceOfClass(td: ScTypeDefinition) =>
              annotation.registerFix(fix(td))
            case _ =>
          }
          true
        } else
      ...
      refElement.getParent match {
        ...
        case Both(
          ...
          if (addCreateApplyOrUnapplyFix(
                messageKey,
                td => new CreateUnapplyQuickFix(td, p)))
            return
  ...
  private def highlightImplicitMethod(
      expr: ScExpression,
      resolveResult: ScalaResolveResult,
      refElement: ScReferenceElement,
      fun: PsiNamedElement,
  ...
  private def highlightImplicitView(
      expr: ScExpression,
      fun: PsiNamedElement,
      typeTo: ScType,
  ...
  private def checkSelfInvocation(
      self: ScSelfInvocation,
      holder: AnnotationHolder) {
    self.bind match {
      case Some(elem) =>
      case None =>
  ...
  private def checkQualifiedReferenceElement(
      refElement: ScReferenceElement,
      holder: AnnotationHolder) {
    AnnotatorHighlighter.highlightReferenceElement(refElement, holder)
  ...
  private def checkForwardReference(
  ...
  private def highlightWrongInterpolatedString(
      l: ScInterpolatedStringLiteral,
      holder: AnnotationHolder) {
    ...
    ref.resolve() match {
      case r: ScFunction =>
        ...
        val shift =
          expr match {
            case ScMethodCall(invoked, _) =>
              invoked.getTextRange.getEndOffset
            case _ =>
              return
          }
  ...
  private def registerAddImportFix(
      refElement: ScReferenceElement,
        ...
        case _ =>
          expr.getParent match {
            case a: ScAssignStmt
                if a.getRExpression.contains(expr) && a
                  .isDynamicNamedAssignment =>
              return
            case args: ScArgumentExprList =>
              return
            case inf: ScInfixExpr if inf.getArgExpr == expr =>
              return
            case tuple: ScTuple
            ...
            case parent @ (_: ScTuple | _: ScParenthesisedExpr) =>
              parent.getParent match {
                case inf: ScInfixExpr if inf.getArgExpr == parent =>
                  return
                case _ =>
              }
            case param: ScParameter =>
              ...
              param.getRealParameterType() match {
                case Success(paramType, _)
                    if !paramType.isGenericType(Option(expr.getProject)) =>
                //do not check generic types. See SCL-3508
                case _ =>
                  return
              }
            case ass: ScAssignStmt if ass.isNamedParameter =>
              return //that's checked in application annotator
            case _ =>
          }
          ...
          expr.expectedTypeEx(fromUnderscore) match {
            ...
            case Some((tp: ScType, typeElement)) =>
              ...
              val conformance = ScalaAnnotator
                .smartCheckConformance(expectedType, exprType)
              if (!conformance) {
                if (typeAware) {
                  val markedPsi =
                  ...
                  val error = ScalaBundle.message(
                    ...
                    expectedTypeText)
                  val annotation: Annotation = holder
                    .createErrorAnnotation(markedPsi, error)
                  annotation.setHighlightType(
                    ProblemHighlightType.GENERIC_ERROR_OR_WARNING)
                  if (WrapInOptionQuickFix
                        .isAvailable(expr, expectedType, exprType)) {
                    val wrapInOptionFix =
  ...
  private def checkExpressionImplicitParameters(
      expr: ScExpression,
      holder: AnnotationHolder) {
      ...
      ScalaPsiUtil.getParentOfType(under, classOf[ScVariableDefinition]) match {
        case varDef @ ScVariableDefinition.expr(expr)
          ...
          if (varDef.containingClass == null) {
            val error = ScalaBundle
              .message("local.variables.must.be.initialized")
            val annotation: Annotation = holder
              .createErrorAnnotation(under, error)
            annotation
              .setHighlightType(ProblemHighlightType.GENERIC_ERROR_OR_WARNING)
  ...
  private def checkExplicitTypeForReturnStatement(
    ...
    fun match {
      case null =>
        ...
        val annotation: Annotation = holder
          .createErrorAnnotation(ret.returnKeyword, error)
        annotation.setHighlightType(ProblemHighlightType.LIKE_UNKNOWN_SYMBOL)
      ...
      case _ =>
        fun.returnTypeElement match {
          case Some(x: ScTypeElement) =>
            ...
            funType match {
              case Success(tp: ScType, _) if tp equiv psi.types.Unit =>
            ...
            val ExpressionTypeResult(_, importUsed, _) =
              ret.expr match {
                case Some(e: ScExpression) =>
                  e.getTypeAfterImplicitConversion()
                case None =>
  ...
  private def checkImportExpr(impExpr: ScImportExpr, holder: AnnotationHolder) {
    if (impExpr.qualifier == null) {
      val annotation: Annotation = holder.createErrorAnnotation(
        impExpr.getTextRange,
        ScalaBundle.message("import.expr.should.be.qualified"))
      annotation.setHighlightType(ProblemHighlightType.GENERIC_ERROR)
    }
  }

  private def checkTypeElementForm(
  ...
  private def checkAnnotationType(
              ...
              annot.constr.reference match {
                case Some(ref) =>
                  Option(ref.resolve()) match {
                    case Some(res: PsiNamedElement) =>
                      res.getName == annotation
                    case _ =>
                      false
                  }
                case _ =>
      ...
      if (!childHasAnnotation(fun.returnTypeElement, "uncheckedVariance")) {
        fun.returnType match {
          case Success(returnType, _) =>
            checkVariance(
              ScType.expandAliases(returnType).getOrType(returnType),
              ScTypeParam.Covariant,
              fun.nameId,
              fun.getParent,
              holder)
      ...
      for (parameter <- fun.parameters) {
        parameter.typeElement match {
          case Some(te) if !childHasAnnotation(Some(te), "uncheckedVariance") =>
            checkVariance(
              ScType.expandAliases(te.calcType).getOrType(te.calcType),
              ScTypeParam.Contravariant,
              parameter.nameId,
              fun.getParent,
              holder)
  ...
  def checkValueAndVariableVariance(
      toCheck: ScDeclaredElementsHolder,
      variance: Int,
        ...
        element.getType() match {
          case Success(tp, _) =>
            ScType.expandAliases(tp) match { //so type alias is highlighted
              case Success(newTp, _) =>
                checkVariance(newTp, variance, element.nameId, toCheck, holder)
              case _ =>
                checkVariance(tp, variance, element.nameId, toCheck, holder)
            }
          case _ =>
        }
      }
  ...
  private def checkVariance(
    ...
    def highlightVarianceError(
      ...
      if (varianceOfPosition != varianceOfElement && varianceOfElement != ScTypeParam
            .Invariant) {
        val pos =
          if (toHighlight.isInstanceOf[ScVariable])
            toHighlight.getText + "_="
          else
            toHighlight.getText
        val place =
          if (toHighlight.isInstanceOf[ScFunction])
            "method"
          else
            "value"
        val elementVariance =
          if (varianceOfElement == 1)
            "covariant"
          else
            "contravariant"
        val posVariance =
          if (varianceOfPosition == 1)
            "covariant"
          else if (varianceOfPosition == -1)
            "contravariant"
          else
            "invariant"
        val annotation = holder.createErrorAnnotation(
          toHighlight,
          ScalaBundle.message(
            s"$elementVariance.type.$posVariance.position.of.$place",
            name,
            typeParam.toString,
            pos))
        annotation.setHighlightType(ProblemHighlightType.GENERIC_ERROR)
              ...
              parentIt
                ...
                case _ =>
                  def findVariance: Int = {
                    if (!checkIfTypeIsInSameBrackets)
                      return i
                    if (PsiTreeUtil.isAncestor(
                      ...
                      if (!scTypeParam
                        ...
                        return i * -1
                    if (toHighlight
                          .getParent == scTypeParam.getParent.getParent)
                      return i * -1
                    i
                  }
                  highlightVarianceError(
                    scTypeParam.variance,
                    findVariance,
                    paramType.name)
              }
            case _ =>
  ...
  //fix for SCL-7176
  private def checkAbstractMemberPrivateModifier(
      element: PsiElement,
      toHighlight: Seq[PsiElement],
      holder: AnnotationHolder) {
    element match {
          ...
          case Some(am) if am.isUnqualifiedPrivateOrThis =>
            for (e <- toHighlight) {
              val annotation = holder.createErrorAnnotation(
                e,
                ScalaBundle
                  .message("abstract.member.not.have.private.modifier"))
              annotation.setHighlightType(ProblemHighlightType.GENERIC_ERROR)
            }
          case _ =>
        }
  ...
  private def checkIntegerLiteral(
    ...
    val isNegative =
      parent match {
        ...
        case prefixExpr: ScPrefixExpr
            if prefixExpr.getChildren.size == 2 && prefixExpr
              .getFirstChild
              .getText == "-" =>
          true
        case _ =>
    ...
    if (base == 8) {
      ...
      scalaVersion match {
        case Some(ScalaLanguageLevel.Scala_2_10) =>
          ...
          val annotation = holder
            .createWarningAnnotation(literal, deprecatedMeaasge)
          annotation.setHighlightType(ProblemHighlightType.LIKE_DEPRECATED)
    ...
    if (status == 2) { // the Integer number is out of range even for Long
      ...
      if (status == 1 && !endsWithL) {
        ...
        val annotation =
          if (isNegative)
            holder.createErrorAnnotation(parent, error)
          else
...
object ScalaAnnotator {
  ...
  val usedImportsKey: Key[mutable.HashSet[ImportUsed]] = Key
    .create("used.imports.key")
