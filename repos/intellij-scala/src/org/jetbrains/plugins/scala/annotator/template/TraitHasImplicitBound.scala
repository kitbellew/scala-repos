...
object TraitHasImplicitBound extends AnnotatorPart[ScTrait] {
  ...
  def annotate(
      ...
      typeAware: Boolean) {
    val contextBoundElements = definition.typeParameters
      .flatMap(p => p.contextBoundTypeElement)
    ...
    val viewBoundElements = definition.typeParameters
      .flatMap(p => p.viewTypeElement)
