...
trait ValueClassAnnotator {
      ...
      case Some(c) =>
        c.parameters match {
          ...
          case Seq(param) =>
            holder.createErrorAnnotation(
              param,
              ScalaBundle.message("value.class.can.have.only.val.parameter"))
          case _ =>
            holder.createErrorAnnotation(
              valueClass.nameId,
              ScalaBundle.message("value.class.can.have.only.one.parameter"))
        }
  ...
  private def annotateValueClassTypeParameters(
    ...
    tp match {
      case Some(tpClause) =>
        tpClause.typeParameters.filter(
          _.hasAnnotation("scala.specialized").isDefined).foreach {
          tpParam =>
