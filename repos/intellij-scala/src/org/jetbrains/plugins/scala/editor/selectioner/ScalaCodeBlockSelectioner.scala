...
class ScalaCodeBlockSelectioner extends ExtendWordSelectionHandlerBase {
  ...
  override def select(
    ...
    if (firstChild.getElementType == ScalaTokenTypes.tLBRACE && lastChild
          .getElementType == ScalaTokenTypes.tRBRACE) {
      while (firstChild.getTreeNext != null && firstChild.getTreeNext
               .getElementType == TokenType.WHITE_SPACE) {
        firstChild = firstChild.getTreeNext
      }
      while (lastChild.getTreePrev != null && lastChild.getTreePrev
               .getElementType == TokenType.WHITE_SPACE) {
