    ...
    val substitutor =
      originalElement match {
        case ref: ScReferenceElement =>
          ref.bind() match {
            case Some(ScalaResolveResult(_, subst)) =>
              subst
            case _ =>
              ScSubstitutor.empty
          }
    ...
    val text =
      element match {
        case clazz: ScTypeDefinition =>
          generateClassInfo(clazz, substitutor)
        case function: ScFunction =>
          generateFunctionInfo(function, substitutor)
        ...
        case alias: ScTypeAlias =>
          generateTypeAliasInfo(alias, substitutor)
        case parameter: ScParameter =>
          generateParameterInfo(parameter, substitutor)
        case b: ScBindingPattern =>
          generateBindingPatternInfo(b, substitutor)
        case _ =>
          null
      }
  ...
  def generateDoc(element: PsiElement, originalElement: PsiElement): String = {
    ...
    if (!containingFile.isInstanceOf[ScalaFile]) {
      if (element.isInstanceOf[ScalaPsiElement])
        debugMessage(
          "Asked to build doc for a scala element, but it is in non scala file (1)",
          element)
    ...
    e match {
      case clazz: ScTypeDefinition =>
        ...
        val pack = {
          ...
          if (lastIndexOf >= 0)
            qualName.substring(0, lastIndexOf)
          else
            ""
        }
        ...
        if (pack != "")
          buffer
            .append("<font size=\"-1\"><b>" + escapeHtml(pack) + "</b></font>")
        ...
        clazz match {
          case par: ScParameterOwner =>
            buffer.append(parseParameters(par, ScType.urlText, end - start - 7))
          case _ =>
        }
        buffer.append("\n")
      ...
      case fun: ScFunction =>
        ...
        "<html><body>" + buffer.toString + "</body></html>"
      case decl: ScDeclaredElementsHolder
          if decl.isInstanceOf[ScValue] || decl.isInstanceOf[ScVariable] =>
        val buffer: StringBuilder = new StringBuilder("")
        decl match {
          case decl: ScMember =>
            buffer.append(parseClassUrl(decl))
          case _ =>
        }
        buffer.append("<PRE>")
        decl match {
          case an: ScAnnotationsHolder =>
            buffer.append(parseAnnotations(an, ScType.urlText))
          case _ =>
        }
        decl match {
          case m: ScModifierListOwner =>
            buffer.append(parseModifiers(m))
          case _ =>
        }
        buffer.append(
          decl match {
        ...
        buffer.append(
          element match {
            case typed: ScTypedDefinition =>
              parseType(typed, ScType.urlText)
            case _ =>
              ": Nothing"
          })
        buffer.append("</PRE>")
        decl match {
          case doc: ScDocCommentOwner =>
            buffer.append(parseDocComment(doc))
          case _ =>
        }
      ...
      case param: ScParameter =>
        ...
        param match {
          case cl: ScClassParameter =>
            buffer.append(parseModifiers(cl))
          case _ =>
        }
        buffer.append(
          param match {
            case c: ScClassParameter if c.isVal =>
              "val "
  ...
  def parseContext(startPoint: PsiElement): Pair[PsiElement, PsiComment] = {
    ...
    def findDocCommentOwner(elem: PsiElement): Option[ScDocCommentOwner] = {
      elem match {
        case null =>
          None
        case d: ScDocCommentOwner =>
          Some(d)
        case _ =>
          findDocCommentOwner(elem.getParent)
      }
    }
...
object ScalaDocumentationProvider {
  ...
  def parseType(
    ...
    val typez =
      elem match {
        ...
        case _ =>
          elem.getType(TypingContext.empty).getOrAny
      }
  ...
  private def parseParameterClause(
    ...
    elem
      .parameters
      .map(parseParameter(_, typeToString))
      .mkString(
        if (elem.isImplicit)
          "(implicit "
        else
  ...
  def createScalaDocStub(commentOwner: PsiDocCommentOwner): String = {
    ...
    def registerInheritedParam(
        allParams: mutable.HashMap[String, PsiDocTag],
        param: PsiDocTag) {
    ...
    def processTypeParams(owner: ScTypeParametersOwner) {
      for (tparam <- owner.typeParameters) {
        if (inheritedTParams.contains(tparam.name)) {
          ...
          val descriptionText = paramTag
            .getText
            .substring(
              paramTag.getValueElement.getTextOffset + paramTag
                .getValueElement
                .getTextLength)
    ...
    commentOwner match {
      ...
      case function: ScFunction =>
        ...
        if (returnTag != null) {
          ...
          buffer
            .append(leadingAsterisks)
            .append(MyScaladocParsing.RETURN_TAG)
            .append(" \n")
        }
  ...
  private def parseTypeParameters(elems: ScTypeParametersOwner): String = {
    ...
    if (typeParameters.nonEmpty)
      escapeHtml(
        typeParameters
          .map(PresentationUtil.presentationString(_))
          .mkString("[", ", ", "]"))
    else
      ""
  }
  ...
  private def parseModifiers(elem: ScModifierListOwner): String = {
    ...
    buffer.append(
      elem.getModifierList.accessModifier match {
        case Some(x: ScAccessModifier) =>
          x.access match {
            case ScAccessModifier.Type.PRIVATE =>
              "private" + accessQualifier(x)
            case ScAccessModifier.Type.PROTECTED =>
              "protected" + accessQualifier(x)
            case ScAccessModifier.Type.THIS_PRIVATE =>
              "private[this] "
            case ScAccessModifier.Type.THIS_PROTECTED =>
              "protected[this] "
          }
        case None =>
          ""
    ...
    Option(elem.getDocComment) match {
      case Some(y: ScDocComment) =>
        ...
        val dummyFile = PsiFileFactory
          ...
          .createFileFromText("dummy", StdFileTypes.JAVA, text)
          .asInstanceOf[PsiJavaFile]
        val javadoc: String =
          elem match {
            case _: ScFunction | _: ScClass =>
              JavaDocumentationProvider.generateExternalJavadoc(
                dummyFile.getClasses.apply(0).getAllMethods.apply(0))
            case _: ScTypeDefinition | _: ScTypeAlias =>
              JavaDocumentationProvider
                .generateExternalJavadoc(dummyFile.getClasses.apply(0))
            case _: PsiMethod =>
              JavaDocumentationProvider.generateExternalJavadoc(
                dummyFile.getClasses.apply(0).getAllMethods.apply(0))
            case _ =>
        ...
        s1 + (
          elem match {
            ...
            case _ =>
              javadoc.substring(110, javadoc.length - 14)
          }
        ) + s2
      case _ =>
        elem match {
  ...
  private def getWikiTextRepresentation(macroFinder: MacroFinder)(
      comment: PsiElement): (mutable.StringBuilder, mutable.StringBuilder) = {
      ...
      element.name match {
        case MyScaladocParsing.TODO_TAG | MyScaladocParsing.NOTE_TAG |
          ...
          element
            .getNode
            .getChildren(null)
            .foreach(node => visitElementInner(node.getPsi))
          commentBody.append("<br/><br/>")
        case MyScaladocParsing.SEE_TAG =>
          element
            .getNode
            .getChildren(null)
            .foreach(node => visitElementInner(node.getPsi, commentBody))
        ...
        case _ =>
          element
            .getNode
            .getChildren(null)
            .foreach(node => visitElementInner(node.getPsi, tagsPart))
      }
    }
    ...
    def visitElementInner(
      ...
      if (element.getFirstChild == null) {
        element.getNode.getElementType match {
          case ScalaDocTokenType.DOC_TAG_NAME =>
            element.getText match {
              ...
              case MyScaladocParsing.NOTE_TAG | MyScaladocParsing.TODO_TAG |
                ...
                result
                  .append("<b>")
                  .append(element.getText.substring(1).capitalize)
                  .append(":</b><br/>")
              case MyScaladocParsing.SEE_TAG =>
              ...
              case MyScaladocParsing.INHERITDOC_TAG =>
                val inherited =
                  element.getParent.getParent.getParent match {
                    case fun: ScFunction =>
          ...
          case ScalaDocTokenType.DOC_LINK_CLOSE_TAG =>
            if (element
              ...
              if (linkText.trim().contains(" ")) {
                ...
                result
                  .append(trimmedText.substring(0, spaceIndex))
                  .append("\">")
          ...
          case ScalaDocTokenType.DOC_COMMENT_DATA
              if element.getPrevSibling != null && element
                .getPrevSibling
                .getNode
                .getElementType == ScalaDocTokenType.DOC_HTTP_LINK_TAG =>
          ...
          case _
              if replaceWikiScheme.contains(element.getText) &&
                (
                  element.getParent.getFirstChild == element || element
                    .getParent
                    .getLastChild == element
                ) =>
            val prefix =
              if (element.getParent.getFirstChild == element)
                "<"
              else
                "</"
        ...
        for (child <- element.getNode.getChildren(null)) {
          child.getPsi match {
            case tag: ScDocTag =>
              visitTags(tag)
            case _ =>
              visitElementInner(child.getPsi, result)
          }
        }
      }
    }
  ...
  private def replaceWikiWithTags(comment: PsiDocComment): PsiDocComment = {
    if (!comment.isInstanceOf[ScDocComment])
      return comment
    val macroFinder =
      new MacroFinderImpl(
  ...
  private def getDocedElement(originalElement: PsiElement): PsiElement = {
    originalElement match {
      ...
      case wrapper: ScFunctionWrapper =>
        wrapper.function
      case _: ScTypeDefinition | _: ScTypeAlias | _: ScValue | _: ScVariable |
          _: ScFunction | _: ScParameter | _: ScBindingPattern =>
        originalElement
      case _ =>
        getDocedElement(originalElement.getParent)
    }
  }
  ...
  private def getMemberHeader(member: ScMember): String = {
    ...
    if (!member.getParent.getParent.getParent.isInstanceOf[ScTypeDefinition])
      return ""
    member.containingClass.name + " " + member
      .containingClass
      .getPresentation
      .getLocationString + "\n"
  }
  ...
  private def getOneLine(s: String): String = {
    ...
    if (i == -1)
      ...
      trimed.substring(0, i) + " ..."
  }

  private def appendTypeParams(
      owner: ScTypeParametersOwner,
  ...
  def generateClassInfo(
    ...
    if (length > 1)
      buffer.append(locationString.substring(1, length - 1))
    if (buffer.nonEmpty)
      buffer.append("\n")
    buffer
      .append(ScalaPsiUtil.getModifiersPresentableText(clazz.getModifierList))
    buffer.append(
      clazz match {
        case _: ScObject =>
          "object "
  ...
  def generateValueInfo(field: ScNamedElement, subst: ScSubstitutor): String = {
    val member =
      ScalaPsiUtil.nameContext(field) match {
        case x: ScMember =>
          x
        case _ =>
          return null
    ...
    buffer
      .append(ScalaPsiUtil.getModifiersPresentableText(member.getModifierList))
    member match {
      case value: ScValue =>
        buffer.append("val ")
        ...
        field match {
          case typed: ScTypedDefinition =>
            ...
            if (typez != null)
              buffer.append(": " + ScType.presentableText(typez))
          case _ =>
        }
        value match {
  ...
  def generateBindingPatternInfo(
    ...
    val typez = subst
      .subst(subst.subst(binding.getType(TypingContext.empty).getOrAny))
    if (typez != null)
      buffer.append(": " + ScType.presentableText(typez))
  ...
  def generateParameterInfo(
      ...
      subst: ScSubstitutor): String = {
    val defaultText =
      s"${parameter.name}: ${ScType.presentableText(
      ...
      parameter match {
        case clParameter: ScClassParameter =>
          val clazz = PsiTreeUtil
            .getParentOfType(clParameter, classOf[ScTypeDefinition])
          ...
          if (clazz == null)
            ...
            clazz.name + " " + clazz.getPresentation.getLocationString + "\n" +
              (
                if (clParameter.isVal)
                  "val "
                else if (clParameter.isVar)
                  "var "
                else
                  ""
              ) + clParameter.name +
