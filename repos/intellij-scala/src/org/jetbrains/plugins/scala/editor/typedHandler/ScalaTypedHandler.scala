...
class ScalaTypedHandler extends TypedHandlerDelegate {
  ...
  override def beforeCharTyped(
    ...
    if ((
        ...
        element.getParent.getLastChild == element && element
          .getText
          .startsWith("" + c) &&
        !(
          elementType == ScalaDocTokenType
            .DOC_ITALIC_TAG && element.getPrevSibling != null
            && element
              .getPrevSibling
              .getNode
              .getElementType == ScalaDocTokenType.DOC_ITALIC_TAG
        )) {
      moveCaret()
      return Result.STOP
      ...
      moveCaret()
      return Result.STOP
    } else if (c == '>' && prevElement != null && prevElement
                 .getNode
  ...
  private def completeScalaDocWikiSyntax(tagToInsert: String)(
    ...
    if (element
          .getNode
          .getElementType
          .isInstanceOf[ScaladocSyntaxElementType] || tagToInsert == "}}}") {
  ...
  private def completeInterpolatedStringBraces(
    ...
    if (element.getNode.getElementType == tLBRACE &&
        ...
        (
          element.getNextSibling == null || element
            .getNextSibling
            .getNode
  ...
  private def indentCase(file: PsiFile)(
    ...
    indentElement(file)(
      ...
      offset,
      elem =>
        elem.getNode.getElementType == ScalaTokenTypes
          .kCASE && elem.getParent.isInstanceOf[ScCaseClause])
  }
  ...
  private def indentElse(file: PsiFile)(
    ...
    indentElement(file)(
      ...
      elem =>
        elem.getNode.getElementType == ScalaTokenTypes
          .kELSE && elem.getParent.isInstanceOf[ScIfStmt])
  }
    ...
    if (condition(element)) {
      ...
      if (prevCondition(anotherElement)) {
        extensions.inWriteAction {
          ...
          CodeStyleManager
            .getInstance(project)
            .adjustLineIndent(file, anotherElement.getTextRange)
        }
      }
    }
  }
  ...
  private def replaceArrowTask(file: PsiFile, editor: Editor)(
    ...
    element.getNode.getElementType match {
      ...
      case ScalaTokenTypes.tIDENTIFIER
          if settings
            .REPLACE_MAP_ARROW_WITH_UNICODE_CHAR && element.getText == "->" =>
    ...
    if (ScalaApplicationSettings.getInstance().UPGRADE_TO_INTERPOLATED) {
      element.getParent match {
        case l: ScLiteral =>
          element.getNode.getElementType match {
            case ScalaTokenTypes.tSTRING | ScalaTokenTypes.tMULTILINE_STRING =>
              if (l.getText.filter(_ == '$').length == 1 && file
                    .getText
                    .charAt(offset - 2) == '$') {
                extensions.inWriteAction {
  ...
  private def completeEmptyXmlTag(editor: Editor)(
    ...
    if (element != null && element.getNode.getElementType == ScalaXmlTokenTypes
        ...
        element.getPrevSibling != null && element
          .getPrevSibling
