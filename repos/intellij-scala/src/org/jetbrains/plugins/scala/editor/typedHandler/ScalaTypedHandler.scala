...
class ScalaTypedHandler extends TypedHandlerDelegate {
  override def charTyped(
    ...
    if (isInDocComment(element)) { //we don't have to check offset >= 3 because "/**" is already has 3 characters
    ...
    } else if (c == ' ' && offset >= 6 && offset < text.length && text.substring(
                 offset - 6,
                 offset) == " case ") {
    ...
    } else if (c == '{' && offset >= 2 && offset < text.length && text.substring(
                 offset - 2,
                 offset) == " {") {
      ...
      if (ltIndex > "case ".length - 1 && element.getPrevSibling.getText.substring(
            0,
            ltIndex).trim() == "case") {
        chooseXmlTask(withAttr = false)
      }
      ...
      if (ScalaPsiUtil.isLineTerminator(prevPositionElement)) {
        prevPositionElement.getPrevSiblingCondition(
          _.getTextLength != 0).foreach(_.getNode.getElementType match {
  ...
  override def beforeCharTyped(
    ...
    if ((c == '"' && Set(
        ...
        isInDocComment(element) && (elementType.isInstanceOf[
      ...
      val indent =
        CodeStyleSettingsManager.getSettings(project).getCommonSettings(
          ScalaLanguage.Instance).getIndentOptions.CONTINUATION_INDENT_SIZE
  ...
  private def isInDocComment(element: PsiElement): Boolean =
    isInPlace(element, classOf[ScDocComment])
  ...
  private def completeScalaDocWikiSyntax(tagToInsert: String)(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
    if (element.getNode.getElementType.isInstanceOf[
          ScaladocSyntaxElementType] || tagToInsert == "}}}") {
      insertAndCommit(offset, tagToInsert, document, project)
    }
  }

  private def completeScalaDocBoldSyntaxElement(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
  ...
  private def completeXmlTag(insert: ScXmlStartTag => String)(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
    ...
    def check(tag: ScXmlStartTag) {
      if (Option(tag.getClosingTag).map(
            _.getTagName != tag.getTagName).getOrElse(true))
        doInsert(tag)
    }
  ...
  private def completeInterpolatedStringBraces(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
  ...
  private def completeMultilineString(
      editor: Editor,
      project: Project,
      element: PsiElement,
      offset: Int) {
  ...
  private def indentCase(file: PsiFile)(
    ...
    indentElement(file)(
      ...
      offset,
      elem =>
        elem.getNode.getElementType == ScalaTokenTypes.kCASE && elem.getParent.isInstanceOf[
          ScCaseClause])
  }
  ...
  private def indentElse(file: PsiFile)(
    ...
    indentElement(file)(
      ...
      offset,
      elem =>
        elem.getNode.getElementType == ScalaTokenTypes.kELSE && elem.getParent.isInstanceOf[
          ScIfStmt])
  }
  ...
  private def indentElement(file: PsiFile)(
      ...
      condition: PsiElement => Boolean = element =>
        element.isInstanceOf[PsiWhiteSpace] || ScalaPsiUtil.isLineTerminator(
          element)) {
      ...
      if (prevCondition(anotherElement)) {
        extensions.inWriteAction {
          ...
          CodeStyleManager.getInstance(project).adjustLineIndent(
            file,
            anotherElement.getTextRange)
        }
      }
    }
  }

  private def replaceArrowTask(file: PsiFile, editor: Editor)(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
    ...
    element.getNode.getElementType match {
      ...
      case ScalaTokenTypes.tIDENTIFIER
          if settings.REPLACE_MAP_ARROW_WITH_UNICODE_CHAR && element.getText == "->" =>
  ...
  private def startAutopopupCompletion(file: PsiFile, editor: Editor)(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
  ...
  private def convertToInterpolated(file: PsiFile, editor: Editor)(
      document: Document,
      project: Project,
      element: PsiElement,
      offset: Int) {
          ...
          element.getNode.getElementType match {
            case ScalaTokenTypes.tSTRING | ScalaTokenTypes.tMULTILINE_STRING =>
              if (l.getText.filter(_ == '$').length == 1 && file.getText.charAt(
                    offset - 2) == '$') {
                extensions.inWriteAction {
                  if (file.getText.charAt(
                        offset) != '}' && CodeInsightSettings.getInstance().AUTOINSERT_PAIR_BRACKET) {
                    document.insertString(offset, "}")
                  }
  ...
  private def completeEmptyXmlTag(editor: Editor)(
    ...
    if (element != null && element.getNode.getElementType == ScalaXmlTokenTypes.XML_DATA_CHARACTERS && element.getText == "/" &&
      ...
      if (xmlLexer.getTokenType != ScalaXmlTokenTypes.XML_EMPTY_ELEMENT_END)
        return
