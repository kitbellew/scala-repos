  ...
  override def postProcessEnter(
    ...
    extensions inWriteAction {
      ...
      val marginCharFromSettings =
        selectBySettings[Option[Char]](None)(Some(marginChar))
      val marginCharOpt =
        marginCharFromSettings match {
          case Some(mChar)
              if hasMarginChars(element, mChar.toString) ||
                (!hasMarginChars(element, mChar.toString) && lines.length > 3) || needAddByType(
                literal) =>
      ...
      if (wasSingleLine || lines.length == 3 &&
        ...
        val indentSize = prevIndent + needInsertIndentInt + interpolatorPrefixLength(
        ...
        val wsPrefix =
          if (isPrevLineFirst)
            prevLinePrefixAfterDelimiter(firstMLQuoteLength) + firstMLQuoteLength
