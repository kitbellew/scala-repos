...
class ScalaImportOptimizer extends ImportOptimizer {
  ...
  def processFile(
    ...
    def isImportUsed(importUsed: ImportUsed): Boolean = {
      ...
      importUsed match {
        case ImportSelectorUsed(sel) if sel.isAliasedImport =>
          true
        case _ =>
          usedImports
            .contains(importUsed) || isLanguageFeatureImport(importUsed) ||
...
object ScalaImportOptimizer {
  ...
  private def importsUsedFor(expr: ScExpression): Set[ImportUsed] = {
    ...
    res ++=
      expr
        .getTypeAfterImplicitConversion(expectedOption = expr
          .smartExpectedType())
        .importsUsed
    ...
    expr match {
      ...
      case f: ScForStatement =>
        res ++= ScalaPsiUtil.getExprImports(f)
      case _ =>
    }
  ...
  private def collectImportsUsed(
    ...
    element match {
      case expression: ScExpression =>
        result ++= importsUsedFor(expression)
      case _ =>
    }
