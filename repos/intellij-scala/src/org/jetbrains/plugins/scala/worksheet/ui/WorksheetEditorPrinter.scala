          ...
          if (differ > 0) {
            ...
            foldingOffsets += (
              (
                ...
                outputBuffer.length - outputBuffer.reverseIterator.takeWhile(
                  _ == '\n').length,
                end - start + 1,
                end)
            )
  ...
  def flushBuffer() {
    ...
    scala.extensions.inReadAction {
      PsiDocumentManager.getInstance(project).getPsiFile(
        originalEditor.getDocument) match {
        case scalaFile: ScalaFile =>
          WorksheetEditorPrinter.saveWorksheetEvaluation(
            scalaFile,
            str,
            worksheetViewer.getUserData(
              WorksheetEditorPrinter.DIFF_SPLITTER_KEY).getProportion)
        case _ =>
      }
    }
//    flushFolding()
...
//    incUpdate(str)
  ...
  private def updateWithPersistentScroll(document: Document, text: String) { //todo - to do
    ...
    extensions.invokeLater {
      extensions.inWriteAction {
        ...
        val worksheetScroll =
          worksheetViewer.getScrollingModel.getVerticalScrollOffset
                ...
                override def run() {
                  foldingOffsetsCopy map {
                    case (start, end, limit, originalEnd) =>
                      val offset = originalDocument getLineEndOffset Math.min(
                        originalEnd,
  ...
  private def getOutputLimit =
    ScalaProjectSettings.getInstance(project).getOutputLimit
        ...
        case group =>
          new CaretAdapter {
            override def caretPositionChanged(e: CaretEvent) {
              if (!e.getEditor.asInstanceOf[
                    EditorImpl].getContentComponent.hasFocus) return
...
//            checkAndAdd(viewerImpl, originalImpl)
            ...
            diffSplitter foreach {
              case splitter =>
                viewerImpl.getScrollPane.getVerticalScrollBar.addAdjustmentListener(
  ...
  def loadWorksheetEvaluation(file: ScalaFile): Option[(String, Float)] = {
    ...
    FileAttributeUtilCache.readAttribute(LAST_WORKSHEET_RUN_RESULT, file).map(
      s => (s, ratio))
  }
  ...
  def newWorksheetUiFor(editor: Editor, virtualFile: VirtualFile) =
    newUiFor(editor, virtualFile, true)

  def newMacrosheetUiFor(editor: Editor, virtualFile: VirtualFile) =
    newUiFor(editor, virtualFile, false)
  ...
  private def createBlankEditorWithLang(
      project: Project,
      lang: Language,
      fileType: LanguageFileType): Editor = {
    val file = PsiFileFactory.getInstance(project).createFileFromText(
      "dummy_" + Random.nextString(10),
      lang,
      "")
