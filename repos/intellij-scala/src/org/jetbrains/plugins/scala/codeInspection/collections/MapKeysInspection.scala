  ...
  override def getSimplification(expr: ScExpression): Option[Simplification] =
    expr match {
      case qual `.map`(`_._1`()) `.toIterator` () if isMap(qual) =>
        ...
        Some(
          replace(expr).withText(invocationText(qual, "keysIterator"))
      ...
      case qual `.map`(`_._1`()) `.toSet` () if isMap(qual) =>
        ...
        Some(
          replace(expr).withText(invocationText(qual, "keySet"))
            .highlightFrom(qual).withHint(setHint))
      case qual `.map`(`_._1`()) if isMap(qual) =>
        Some(replace(expr).withText(invocationText(qual, "keys")).highlightFrom(
          qual))
      case _ => None
    }
}
