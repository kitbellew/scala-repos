...
class DeleteUnmatchedTagQuickFix(s: ScalaPsiElement)
    extends AbstractFixOnPsiElement(
      ScalaBundle.message("xml.delete.unmatched.tag"),
      s) {
...
class RenameOpeningTagQuickFix(s: ScXmlEndTag)
  ...
  def doApplyFix(project: Project) {
    ...
    val attributes = openingTag
      .findChildrenByType(ScalaElementTypes.XML_ATTRIBUTE).map(_.getText)

    elem.getOpeningTag.replace(ScalaPsiElementFactory.createXmlStartTag(
      elem.getTagName,
      elem.getManager,
