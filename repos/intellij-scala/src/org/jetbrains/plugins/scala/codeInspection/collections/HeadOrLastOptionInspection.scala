      ...
      case IfStmt(
        ...
        Some(
          replace(expr).withText(
            invocationText(coll, "headOption")).highlightAll)
      case IfStmt(
            CheckNonEmpty(coll, _, _),
            scalaSome(coll2 `.head` ()),
            scalaNone())
          if PsiEquivalenceUtil.areElementsEquivalent(coll, coll2) =>
        Some(
  ...
  override def getSimplification(expr: ScExpression): Option[Simplification] =
    expr match {
      case IfStmt(
        ...
        Some(
          replace(expr).withText(
            invocationText(coll, "lastOption")).highlightAll)
      case IfStmt(
            CheckNonEmpty(coll, _, _),
            scalaSome(coll2 `.last` ()),
            scalaNone())
  ...
  override def getSimplification(expr: ScExpression): Option[Simplification] =
    expr match {
      case (coll `.lift` ()) `.apply`(literal("0")) =>
        Some(
          replace(expr).withText(
            invocationText(coll, "headOption")).highlightFrom(coll))
      case _ => None
    }
}
  ...
  override def getSimplification(expr: ScExpression): Option[Simplification] =
    expr match {
      case (coll `.lift` ()) `.apply` (coll2 `.sizeOrLength` () `-` literal(
        ...
        Some(
          replace(expr).withText(
            invocationText(coll, "lastOption")).highlightFrom(coll))
      case _ => None
    }
}
