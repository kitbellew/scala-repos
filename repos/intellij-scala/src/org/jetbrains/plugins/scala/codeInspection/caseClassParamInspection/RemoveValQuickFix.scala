...
class RemoveValQuickFix(param: ScClassParameter)
  ...
  def doApplyFix(project: Project) {
    ...
    CodeStyleManager.getInstance(p.getProject).reformatText(
      p.getContainingFile,
...
class RemoveValFromEnumeratorIntentionAction(enum: ScEnumerator)
...
class RemoveValFromGeneratorIntentionAction(enum: ScGenerator)
    extends IntentionAction {
  def getText = "Remove unnecessary 'val'"
