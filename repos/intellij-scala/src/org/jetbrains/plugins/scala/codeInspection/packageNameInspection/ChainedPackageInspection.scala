      ...
      file.asOptionOf[ScalaFile].filter(!_.isScriptFile()).flatMap {
        scalaFile =>
          scalaFile.getPackagings.headOption.flatMap { firstPackaging =>
            val basePackages = ScalaProjectSettings.getInstance(
              file.getProject).getBasePackages.asScala

            basePackages.find(basePackage =>
              firstPackaging.getPackageName != basePackage
                && firstPackaging.getPackageName.startsWith(
                  basePackage)).flatMap { basePackage =>
