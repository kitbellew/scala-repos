      ...
      case qual `.map` (f @ returnsBoolean()) `.getOrElse`(literal(
        ...
        Some(
          replace(expr).withText(
            invocationText(qual, newMethodName, f)).highlightFrom(qual))
      case _ => None
    }
  }
}
...
object MapGetOrElseFalse
    extends MapGetOrElseBoolean("false", "exists", "map.getOrElse.false.hint")

object MapGetOrElseTrue
    extends MapGetOrElseBoolean("true", "forall", "map.getOrElse.true.hint")
