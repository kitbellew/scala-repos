...
class ScalaOptimizeImportsFix extends IntentionAction with HighPriorityAction {
  ...
  def isAvailable(project: Project, editor: Editor, file: PsiFile): Boolean = {
    file.getManager.isInProject(file) && (file.isInstanceOf[
      ScalaFile] || ScalaLanguageDerivative.hasDerivativeOnFile(file))
  }
...
class ScalaEnableOptimizeImportsOnTheFlyFix extends IntentionAction {
  ...
  def invoke(project: Project, editor: Editor, file: PsiFile) {
    ...
    if (file.getManager.isInProject(file) && (file.isInstanceOf[
          ScalaFile] || ScalaLanguageDerivative.hasDerivativeOnFile(file))) {
      if (!FileModificationService.getInstance.prepareFileForWrite(file)) return
...
class MarkImportAsAlwaysUsed(importText: String)
  ...
  def isAvailable(project: Project, editor: Editor, file: PsiFile): Boolean = {
    importText.contains(".") && !ScalaCodeStyleSettings.getInstance(
      project).isAlwaysUsedImport(importText)
  }
