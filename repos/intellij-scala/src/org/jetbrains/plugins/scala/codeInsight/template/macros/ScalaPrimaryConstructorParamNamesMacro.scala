...
class ScalaPrimaryConstructorParamNamesMacro extends Macro {
  override def calculateResult(
    ...
    Option(params.head.calculateResult(context).toString).map(
      MacroUtil.paramPairs(_).map(_._1)) match {
      case Some(head :: tail) =>
  ...
  def getPresentableName: String =
    MacroUtil.scalaPresentablePrefix + "primaryConstructorParamNames"
}
