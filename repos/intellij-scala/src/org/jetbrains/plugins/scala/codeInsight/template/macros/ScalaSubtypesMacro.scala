...
class ScalaSubtypesMacro extends Macro {
  ...
  override def getPresentableName: String =
    MacroUtil.scalaPresentablePrefix + "subtypes(TYPE)"
  ...
  override def calculateQuickResult(
      params: Array[Expression],
      context: ExpressionContext): Result = calculateResult(params, context)
        ...
        ScType.extractClass(scTypeRes.myType, Some(context.getProject)) match {
          case Some(x: ScTypeDefinition) =>
            ...
            ClassInheritorsSearch.search(
              ...
              true).findAll().filter(_.isInstanceOf[ScTypeDefinition])
              .map(_.asInstanceOf[ScTypeDefinition].getType(
                TypingContext.empty)).flatMap(_.toOption)
              .flatMap(MacroUtil.getTypeLookupItem(_, project)).toArray
