...
class StringToMultilineStringIntention extends PsiElementBaseIntentionAction {
  ...
  def isAvailable(
    ...
    val literalExpression: ScLiteral = PsiTreeUtil
      .getParentOfType(element, classOf[ScLiteral], false)
  ...
  def regularToMultiline(literal: ScLiteral, editor: Editor) {
    ...
    literal match {
      case interpolated: ScInterpolatedStringLiteral =>
        ...
        val content = InterpolatedStringFormatter
          .formatContent(parts, toMultiline = true)
        val quote = "\"\"\""
        val text = s"$prefix$quote$content$quote"
        val newLiteral = ScalaPsiElementFactory
          .createExpressionFromText(text, literal.getManager)
  ...
  def multilineToRegular(literal: ScLiteral) {
    literal match {
      case interpolated: ScInterpolatedStringLiteral =>
        ...
        val parts =
          literal match {
            ...
            case _ =>
              InterpolatedStringParser.parse(interpolated).getOrElse(Nil)
          }
        val content = InterpolatedStringFormatter
          .formatContent(parts, toMultiline = false)
        val quote = "\""
        val text = s"$prefix$quote$content$quote"
        val newLiteral = ScalaPsiElementFactory
          .createExpressionFromText(text, literal.getManager)
      ...
      case _ =>
        ...
        parts match {
          case Seq(Text(s)) =>
            val newLiteralText = "\"" + StringUtil
              .escapeStringCharacters(s) + "\""
            val newLiteral = ScalaPsiElementFactory
              .createExpressionFromText(newLiteralText, literal.getManager)
            toReplace.replace(newLiteral)
          case _ =>
        }
    }
