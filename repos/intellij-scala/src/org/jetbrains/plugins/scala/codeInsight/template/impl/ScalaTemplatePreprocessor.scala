...
class ScalaTemplatePreprocessor extends TemplatePreprocessor {
  override def preprocessTemplate(
      editor: Editor,
      file: PsiFile,
      caretOffset: Int,
      textToInsert: String,
    ...
    Option(file.findElementAt(caretOffset))
      .map { caretElem =>
        ...
        do res = res.getPrevSiblingNotWhitespace while (res != null && res.getText.isEmpty)
        res
      }
      .foreach {
        case elem: LeafPsiElement
            if elem.getText == "def" && textToInsert.startsWith("def ") =>
