...
class MergeIfToOrIntention extends PsiElementBaseIntentionAction {
  ...
  def isAvailable(
    ...
    if (!(thenBranch.getTextRange.getEndOffset <= offset && offset <= elseBranch
        ...
        !(ifStmt.getTextRange.getStartOffset <= offset && offset <= ifStmt
          .condition.get.getTextRange.getStartOffset)) return false
  ...
  override def invoke(project: Project, editor: Editor, element: PsiElement) {
    ...
    expr.append("if (").append(outerCondition).append(" || ")
      .append(innerCondition).append(") ").append(ifStmt.thenBranch.get.getText)
