...
class ScalaTargetElementEvaluator extends TargetElementEvaluatorEx {
        ...
        case fakeMethod: FakePsiMethod
          ...
          fakeMethod.navElement match {
            case refPattern: ScReferencePattern
                if ScalaPsiUtil.nameContext(refPattern).isInstanceOf[
                  ScVariable] =>
              Some(refPattern)
            case _ => None
          }
                ...
                case obj: ScObject =>
                  obj.fakeCompanionClassOrCompanionClass match {
                    case cl: ScClass if cl.isCase =>
                      return cl.parameters.find(_.name == p.name)
                    case _ =>
                  }
                case _ =>
              }
  ...
  def isIdentifierPart(
    ...
    child != null && child.getNode != null && ScalaTokenTypes.IDENTIFIER_TOKEN_SET.contains(
          ...
          case a: ScFunctionDefinition if a.isSyntheticApply =>
            a.containingClass match {
              case obj: ScObject =>
                obj.fakeCompanionClassOrCompanionClass match {
                  case cl: ScClass if cl.isCase =>
                    cl.parameters.filter(_.name == p.name).foreach(
                      allRenames.put(_, newName))
                  case _ =>
                }
              case _ =>
            }
