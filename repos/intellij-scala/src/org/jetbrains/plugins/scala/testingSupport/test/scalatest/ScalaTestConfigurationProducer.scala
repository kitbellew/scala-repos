  ...
  def getLocationClassAndTest(
      ...
      call.getInvokedExpr match {
        ...
        case _call: MethodInvocation =>
          checkCallGeneral(
            ...
            case res: SuccessResult =>
              return res.copy(invocation = call)
            case WrongResult =>
              return WrongResult
            case _ =>
          }
          ...
          m match {
            ...
            case call: MethodInvocation =>
              call.getInvokedExpr match {
                case ref: ScReferenceExpression =>
                  ref.qualifier match {
                    case Some(qual: MethodInvocation) =>
                      checkTagged(qual)
                    case Some(qual) =>
                      endupWithLitral(qual)
                    case _ =>
    ...
    def checkFunSuite(fqn: String): Option[String] = {
      ...
      checkCall(
        ...
        case SuccessResult(_, testName, _) =>
    ...
    def checkFreeSpec(fqn: String): Option[String] = {
      ...
      checkFreeSpecInner(".FreeSpecStringWrapper") match {
        case Some(name) =>
          Some(name)
        case None =>
          checkFreeSpecInner(".ResultOfTaggedAsInvocationOnString")
      }
    }
    ...
    def checkWordSpec(fqn: String): Option[String] = {
      ...
      checkWordSpecInner(".WordSpecStringWrapper") match {
        case Some(name) =>
          Some(name)
        case None =>
          checkWordSpecInner(".ResultOfTaggedAsInvocationOnString")
      }
    }
            ...
            ref.refName match {
                ...
                ref.resolve() match {
                  case fun: ScFunction
                      if fun.containingClass != null &&
                        ...
                        fun.containingClass.qualifiedName == shouldFqn2 =>
                    if (result == null) {
                      ref.getParent match {
                        case m: MethodInvocation =>
                          result = infix(m)
                ...
                ref.resolve() match {
                  case fun: ScFunction
                      if fun.containingClass != null &&
                        ...
                        fun.containingClass.qualifiedName == mustFqn2 =>
                    if (result == null) {
                      ref.getParent match {
                        case m: MethodInvocation =>
                          result = infix(m)
                ...
                ref.resolve() match {
                  case fun: ScFunction
                      if fun.containingClass != null &&
                        ...
                        fun.containingClass.qualifiedName == canFqn2 =>
                    if (result == null) {
                      ref.getParent match {
                        case m: MethodInvocation =>
                          result = infix(m)
              ...
              case "of" =>
                ref.resolve() match {
                  case fun: ScFunction
                      if fun.containingClass != null &&
                        fun.containingClass.qualifiedName ==
                        "org.scalatest.FlatSpec.BehaviorWord" =>
                    if (result == null) {
                      ref.getParent match {
                        case m: MethodInvocation =>
                          result = call(m)
    ...
    def checkInfixWithIt(
      ...
      val inv: (MethodInvocation) => Option[String] = {
        case i: ScInfixExpr =>
          i.getBaseExpr match {
            case ref: ScReferenceExpression
                if ref.refName == "it" || ref.refName == "ignore" ||
                  ref.refName == "they" =>
              endupWithIt(ref)
            case _ =>
              endupWithLitral(i.getBaseExpr)
            ...
            base match {
              ...
              case m: MethodInvocation =>
                checkInfixWithIt(
                  ...
                  Map(
                    "should" ->
                      Set(shouldFqn, shouldFqn2, itFqn, igFqn, theyFqn),
    ...
    val oldResult =
      (
        ...
        checkSpec("org.scalatest.fixture.SpecLike") ++
            ...
            getFunSpecBasesPre2_0
              .toStream
              .map(checkFunSpec)
              .find(_.isDefined)
              .getOrElse(None) ++
            //this is intended for scalatest version 2.0
            getFunSpecBasesPost2_0
              .toStream
              .map(checkFunSpec)
              .find(_.isDefined)
              .getOrElse(None) ++
