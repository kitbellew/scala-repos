...
object TestNodeProvider {
  ...
  private def getInnerInfixExprs(expr: ScInfixExpr) = {
    expr.getLastChild.getChildren.filter(_.isInstanceOf[ScInfixExpr])
  ...
  private def processChildren[T <: PsiElement](
    ...
    children.map(element => processFun(element, project)).filter(_.isDefined)
      .map(_.get)
  }

  private def getInnerMethodCalls(expr: ScMethodCall) = {
    expr.args.getLastChild.getChildren.filter(_.isInstanceOf[ScMethodCall])
      ...
      methodExpr.asInstanceOf[ScReferenceExpression].advancedResolve match {
        case Some(resolveResult) =>
          resolveResult.getActualElement.getName == callerName && {
            ...
            funElement.isInstanceOf[ScFunctionDefinition] && checkClauses(
              funElement.asInstanceOf[ScFunctionDefinition].getParameterList
                .clauses,
              paramNames: _*)
          }
        case _ => false
  ...
  private def checkRefExpr(
    ...
    funDef.orElse(refExpr.resolve() match {
      ...
      funDef.getName == funName &&
        paramNames.map(checkClauses(funDef.getParameterList.clauses, _: _*))
          .getOrElse(true)
    })
  }
  ...
  private def checkIgnoreExpr(expr: ScInfixExpr): Boolean = {
    expr.getFirstChild match {
      case infixExpr: ScInfixExpr
        ...
        infixExpr.getFirstChild.asInstanceOf[ScReferenceExpression]
          .resolve() match {
    ...
    expr match {
      case infixExpr: ScInfixExpr =>
        checkScInfixExpr(
          infixExpr,
          "should",
          List("org.specs2.specification.Fragment")) || checkScInfixExpr(
          infixExpr,
          "should",
          List("org.specs2.specification.core.Fragment")) || checkScInfixExpr(
          infixExpr,
          "should",
      ...
      expr.args.findFirstChildByType(ScalaElementTypes.BLOCK_EXPR) match {
        case blockExpr: ScBlockExpr =>
          (for (methodExpr <- blockExpr.children
            ...
            yield extractUTestInner(methodExpr, project)).filter(_.isDefined)
            .map(_.get).toList
        case _ => new util.ArrayList[TreeElement]
      }
