...
class ScalaCompilerSettings(state: ScalaCompilerSettingsState) {
  ...
  private def initFrom0(options: Seq[String]) {
    ...
    debuggingInfoLevel = DebuggingOptions.find(p => options.contains(p._1))
      .map(_._2).getOrElse(DebuggingInfoLevel.Vars)
  ...
  private def normalized(options: Seq[String]): Seq[String] =
    options.flatMap {
      case "-language:macros" => Seq("-language:experimental.macros")
      case option =>
