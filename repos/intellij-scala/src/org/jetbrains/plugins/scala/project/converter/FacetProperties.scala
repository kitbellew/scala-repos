...
private class FacetProperties(element: Element) {
  def option(key: String): Option[String] =
    Option(
      XPath.selectSingleNode(
        element,
        s"configuration/option[@name='$key']/@value").asInstanceOf[
        Attribute]).map(_.getValue)
  ...
  def boolean(key: String, default: Boolean = false): Boolean =
    option(key).map(_.toBoolean).getOrElse(default)
  ...
  def array(key: String): Seq[String] =
    XPath.selectNodes(
      element,
      s"configuration/option[@name='$key']/array/option/@value").asScala.map(
      _.asInstanceOf[Attribute].getValue)
}
