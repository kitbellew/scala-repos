...
import com.intellij.openapi.editor.{
  DefaultLanguageHighlighterColors => DLHC,
  HighlighterColors
}

object HoconHighlighterColors {
  final val BadCharacter =
    key("HOCON_BAD_CHARACTER", HighlighterColors.BAD_CHARACTER)
  final val HashComment = key("HOCON_HASH_COMMENT", DLHC.LINE_COMMENT)
  final val DoubleSlashComment =
    key("HOCON_DOUBLE_SLASH_COMMENT", DLHC.LINE_COMMENT)
  ...
  final val ValidStringEscape =
    key("HOCON_VALID_STRING_ESCAPE", DLHC.VALID_STRING_ESCAPE)
  final val InvalidStringEscape =
    key("HOCON_INVALID_STRING_ESCAPE", DLHC.INVALID_STRING_ESCAPE)
  final val Brackets = key("HOCON_BRACKETS", DLHC.BRACKETS)
  final val Braces = key("HOCON_OBJECT_BRACES", DLHC.BRACES)
  final val IncludeModifierParens =
    key("HOCON_INCLUDE_MODIFIER_PARENS", DLHC.PARENTHESES)
  final val SubBraces = key("HOCON_SUBSTITUTION_BRACES", DLHC.BRACES)
  final val KeyValueSeparator =
    key("HOCON_KEY_VALUE_SEPARATOR", DLHC.OPERATION_SIGN)
  final val Comma = key("HOCON_COMMA", DLHC.COMMA)
  final val Include = key("HOCON_INCLUDE", DLHC.KEYWORD)
  final val IncludeModifier = key(
    "HOCON_INCLUDE_MODIFIER",
    HighlightInfoType.STATIC_METHOD.getAttributesKey)
  final val SubstitutionSign =
    key("HOCON_SUBSTITUTION_SIGN", DLHC.OPERATION_SIGN)
  final val OptionalSubstitutionSign =
    key("HOCON_OPTIONAL_SUBSTITUTION_SIGN", DLHC.OPERATION_SIGN)
