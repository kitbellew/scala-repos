...
class HoconSyntaxHighlightingAnnotator extends Annotator {
  ...
  def annotate(element: PsiElement, holder: AnnotationHolder) {
    ...
    element.getNode.getElementType match {
      case Null =>
        holder.createInfoAnnotation(element, null).setTextAttributes(
          HoconHighlighterColors.Null)

      case Boolean =>
        holder.createInfoAnnotation(element, null).setTextAttributes(
          HoconHighlighterColors.Boolean)

      case Number =>
        holder.createInfoAnnotation(element, null).setTextAttributes(
          HoconHighlighterColors.Number)

      case UnquotedChars if parentType == Include =>
        holder.createInfoAnnotation(element, null).setTextAttributes(
          HoconHighlighterColors.Include)
      ...
      case UnquotedChars if parentType == Included =>
        if (HoconConstants.IncludeQualifiers.contains(element.getText)) {
          ...
          holder.createInfoAnnotation(
            ...
            null).setTextAttributes(HoconHighlighterColors.IncludeModifier)
          holder.createInfoAnnotation(
            TextRange(end - 1, end),
            null).setTextAttributes(
            HoconHighlighterColors.IncludeModifierParens)
      ...
      case Period if parentType == Path || parentType == PrefixedField =>
        holder.createInfoAnnotation(element, null).setTextAttributes(
          HoconHighlighterColors.PathSeparator)
