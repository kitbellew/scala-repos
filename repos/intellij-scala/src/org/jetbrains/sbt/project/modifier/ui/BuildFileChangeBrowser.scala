...
class BuildFileChangeBrowser(
  ...
  override def afterDiffRefresh() {
    ...
    updatedChanges.addAll(getSelectedChanges map { myChange =>
      {
        ...
        fileChangesMap.get(changeSwapped.getVirtualFile).map {
          case (modifiedStatus, modificationStamp) =>
            val newModificationStamp = FileDocumentManager.getInstance()
              .getDocument(changeSwapped.getVirtualFile).getModificationStamp
            ...
            if (newModificationStamp != modificationStamp) {
              ...
              BuildFileChange.swap(new BuildFileChange(
