...
object Play2OldStructureAdapter {
  ...
  def apply(newData: Seq[(ProjectId, File, Play2Data)]): Play2ProjectData = {
    ...
    val oldData = projectKeyValueTriples.groupBy(_._2).mapValues(_.map({
      case (id, _, v) => (id, v)
    }))
  ...
  private def extractProjectKeyValue(
    ...
    val confDirectory = data.confDirectory.map(d =>
      (PLAY_CONF_DIR, new StringParsedValue(d.getCanonicalPath))).toSeq
