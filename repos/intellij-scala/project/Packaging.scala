...
object Packaging {
  ...
  object PackageEntry {
    final case class Directory(source: File, destination: String)
        extends PackageEntry
    final case class Artifact(source: File, destination: String)
        extends PackageEntry
    final case class MergedArtifact(sources: Seq[File], destination: String)
        extends PackageEntry
    final case class Library(source: ModuleID, destination: String)
        extends PackageEntry
  }
  ...
  private def convertEntry(
    ...
    entry match {
      ...
      case Library(libraryId, destination) =>
        val libKey =
          libraryId.organization % libraryId.name % libraryId.revision
