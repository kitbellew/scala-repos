...
object ScalaSigParsers extends RulesWithState with MemoisableRules {
  ...
  lazy val attributes = allEntries(ScalaSigEntryParsers.attributeInfo) as
    "attributes"
...
object ScalaSigEntryParsers extends RulesWithState with MemoisableRules {
  ...
  lazy val entry: EntryParser[Any] = symbol | typeEntry | literal | name |
    attributeInfo | annotInfo | children | get
  ...
  val symbolInfo = nameRef ~ symbolRef ~ nat ~ (symbolRef ?) ~ ref ~ get ^~~~~~^
    SymbolInfo
  ...
  val methodSymbol = symHeader(8) -~
    /*(ref?) -~*/ symbolInfo ~ (ref ?) ^~^ MethodSymbol as "method"
  val extRef = 9 -~ nameRef ~ (symbolRef ?) ~ get ^~~^ ExternalSymbol as
    "extRef"
  val extModClassRef =
  ...
  lazy val annotArgArray = 44 -~ (oneOf(constantRef, constAnnotArgRef) *)
  .map(_.toArray)
  lazy val constAnnotArgRef: Rule[Any, String] = oneOf(
    constantRef,
    refTo(annotArgArray))
  lazy val attributeInfo = 40 -~
    symbolRef ~ typeRef ~ (constAnnotArgRef ?) ~
  ...
  lazy val annotInfo = 43 -~ (nat *) ^^
    AnnotInfo // attarg_Ref {constant_Ref attarg_Ref}
