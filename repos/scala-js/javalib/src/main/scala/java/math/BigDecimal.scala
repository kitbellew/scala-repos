...
object BigDecimal {
  ...
  private final val LongFivePowsBitLength = Array
    .tabulate[Int](LongFivePows.length)(i => bitLength(LongFivePows(i)))
  ...
  private final val LongTenPowsBitLength = Array
    .tabulate[Int](LongTenPows.length)(i => bitLength(LongTenPows(i)))
  ...
  /** An array with the first <code>BigInteger</code> scaled by zero.
    ...
    */
  private final val BigIntScaledByZero = Array
    .tabulate[BigDecimal](BigIntScaledByZeroLength)(new BigDecimal(_, 0))

  /** An array with the zero number scaled by the first positive scales.
    *
    *  (<code>0*10^0, 0*10^1, ..., 0*10^10</code>).
    */
  private final val ZeroScaledBy = Array
    .tabulate[BigDecimal](BigIntScaledByZeroLength)(new BigDecimal(0, _))
  ...
  def valueOf(unscaledVal: Long, scale: Int): BigDecimal = {
    if (scale == 0) valueOf(unscaledVal)
  ...
  private def addAndMult10(
    ...
    if (diffScale < LongTenPows.length && maxLen < 64) {
      ...
      val bi = Multiplication
        .multiplyByTenPow(augend.getUnscaledValue(), diffScale)
  ...
  private def divideBigIntegers(
    ...
    if (qr.rem.signum() == 0) { new BigDecimal(qr.quot, scale) }
    else {
      ...
      val compRem: Int = {
        ...
        if (scaledDivisor.bitLength() < 63) {
          ...
          val compRem = qr.rem.abs().shiftLeftOneBit()
  ...
  private def zeroScaledBy(longScale: Long): BigDecimal = {
    if (longScale == longScale.toInt) valueOf(0, longScale.toInt)
    else if (longScale >= 0) new BigDecimal(0, Int.MaxValue)
...
class BigDecimal() extends Number with Comparable[BigDecimal] {
  ...
  def this(in: Array[Char], offset: Int, len: Int) = {
    ...
    while (index <= last && charNotEqualTo(in(index), '.', 'e', 'E')) {
      if (!wasNonZero) {
        if (in(index) == '0') counter += 1 else wasNonZero = true
      }
      index += 1
    }
    ...
    if ((index <= last) && charEqualTo(in(index), 'e', 'E')) {
      ...
      val newScale: Long = _scale.toLong - java.lang.Integer
        .parseInt(scaleString)
    ...
    } else { setUnscaledValue(new BigInteger(unscaled)) }
  }
  ...
  def this(in: Array[Char]) = { this(in, 0, in.length) }
  ...
  def this(sVal: String) = { this(sVal.toCharArray(), 0, sVal.length) }
  ...
  def this(dVal: Double) = {
    ...
    val mantissa2 = {
      if (_scale > 0) {
        val trailingZeros = Math
          .min(_scale, java.lang.Long.numberOfTrailingZeros(mantissa))
    ...
    if (_scale < 0) {
      ...
      if (_bitLength < 64) _smallValue = mantissa3 << (-_scale)
  ...
  def this(bi: BigInteger) = { this(bi, 0) }
  ...
  def this(lVal: Long) = { this(lVal, 0) }
  ...
  def add(augend: BigDecimal): BigDecimal = {
    ...
    if (this.isZero && diffScale <= 0) { augend }
    else if (augend.isZero && (this.isZero || diffScale >= 0)) { this }
    else if (diffScale == 0) {
      if (Math.max(this._bitLength, augend._bitLength) + 1 < 64)
        ...
        new BigDecimal(
          ...
          this._scale)
    } else if (diffScale > 0) { addAndMult10(this, augend, diffScale) }
    else { addAndMult10(augend, this, -diffScale) }
  }
  ...
  def subtract(subtrahend: BigDecimal): BigDecimal = {
    ...
    if (this.isZero && diffScale <= 0) { subtrahend.negate() }
    else if (subtrahend.isZero && (this.isZero || diffScale >= 0)) { this }
    else if (diffScale == 0) {
  ...
  def multiply(multiplicand: BigDecimal): BigDecimal = {
    ...
    if (this.isZero || multiplicand.isZero) { zeroScaledBy(newScale) }
    else if (this._bitLength + multiplicand._bitLength < 64) {
      valueOf(
        this._smallValue * multiplicand._smallValue,
        safeLongToInt(newScale))
    } else {
      val unscaled = this.getUnscaledValue
        .multiply(multiplicand.getUnscaledValue)
  ...
  def divide(
    ...
    else if (divisor.isZero) throw new ArithmeticException("Division by zero")
  ...
  def divide(divisor: BigDecimal): BigDecimal = {
    ...
    else if (thisUnscaled.signum() == 0) { zeroScaledBy(diffScale) }
    else {
  ...
  def divide(divisor: BigDecimal, mc: MathContext): BigDecimal = {
    ...
    val diffScale: Long = _scale.toLong - divisor._scale
    val trailingZeros = mc.precision + 2L + divisor
    ...
    val (quot, newScale0) = {
      ...
      } else { (getUnscaledValue, diffScale) }
    }
    ...
    val (integerQuot, newScale) = {
      ...
      if (qr.rem.signum() != 0) {
        ...
        val compRem = qr.rem.shiftLeftOneBit()
          .compareTo(divisor.getUnscaledValue)
  ...
  def divideToIntegralValue(divisor: BigDecimal): BigDecimal = {
    ...
    val (integralValue, varScale) = {
      if ((divisor.approxPrecision() + newScale > this
        ...
        val iv = getUnscaledValue
          .divide(divisor.getUnscaledValue.multiply(powerOfTen))
  ...
  def divideToIntegralValue(
    ...
    val (quot, newScale) = {
      if (quotPrecision <= 0) { (BigInteger.ZERO, diffScale) }
        ...
        val ns = Math
          .min(diffScale, Math.max(mcPrecision - quotPrecision + 1, 0))
        (q.multiply(powerOf10(ns)), ns)
      } else {
        /* To calculate the minimum power of ten, such that the quotient
         * (u1 * 10^exp) / u2   has at least 'mc.precision()' digits. */
        val exp = Math
          .min(-diffScale, Math.max(mcPrecision.toLong - diffPrecision, 0))
    ...
    if (quot.signum() == 0) return zeroScaledBy(diffScale)
  ...
  def pow(n: Int, mc: MathContext): BigDecimal = {
    ...
    if (n == 0 || (isZero && n > 0)) { pow(n) }
      ...
      if (n < 0) accum = ONE.divide(accum, newPrecision)
  ...
  def abs(): BigDecimal = { if (signum() < 0) negate() else this }

  def abs(mc: MathContext): BigDecimal = {
    val result =
      if (signum() < 0) negate() else new BigDecimal(getUnscaledValue, _scale)
    result.inplaceRound(mc)
    result
  }
  ...
  def signum(): Int = {
    if (_bitLength < 64) {
      if (_smallValue < 0) -1 else if (_smallValue > 0) 1 else 0
    } else { getUnscaledValue().signum() }
  }
  ...
  def precision(): Int = {
    if (_precision == 0) {
      _precision = {
        if (_bitLength == 0) { 1 }
        else if (_bitLength < 64) { decimalDigitsInLong(_smallValue) }
        else {
      ...
      def loop(
        ...
        if (!strippedBI.testBit(0)) {
          ...
          if (qr.rem.signum() == 0) // To adjust the scale
            loop(if (i < lastPow) i + 1 else i, qr.quot, scale - i)
          else if (i != 1) loop(1, strippedBI, scale)
  ...
  def compareTo(bi: BigDecimal): Int = {
    ...
    if (thisSign == valueSign) {
      if (this._scale == bi._scale && this._bitLength < 64 && bi
            ._bitLength < 64) {
        ...
        if (diffPrecision > diffScale + 1) { thisSign }
          ...
          val (thisUnscaled, valUnscaled) = {
            ...
            if (diffScale < 0) (t.multiply(powerOf10(-diffScale)), v)
            else if (diffScale > 0) (t, v.multiply(powerOf10(diffScale)))
            else (t, v)
          }
          thisUnscaled.compareTo(valUnscaled)
        }
      }
    } else if (thisSign < valueSign) { -1 }
    else { 1 }
  }
  ...
  def min(bd: BigDecimal): BigDecimal = if (compareTo(bd) <= 0) this else bd

  def max(bd: BigDecimal): BigDecimal = if (compareTo(bd) >= 0) this else bd
  ...
  override def toString(): String = {
    if (_toStringImage != null) { _toStringImage }
      ...
      if (_scale == 0) { intString }
        ...
        val result =
          if (_scale > 0 && exponent >= -6) {
            if (exponent >= 0) { intString.insert(end - _scale, ".") }
            else {
            ...
            val r0 =
              if (end - begin >= 1) intString.insert(begin, ".") else intString
            val r1 = r0 + "E"
    ...
    if (_scale == 0) { intString }
      ...
      val result = {
        if ((_scale > 0) && (exponent0 >= -6)) {
          if (exponent0 >= 0) { intString.insert(end - _scale, ".") }
          else {
            intString.insert(begin - 1, "0.")
          ...
          val (e, b) = {
            if (rem != 0) {
              ...
              (exp, beg)
            } else { (exponent0, begin) }
          }
          if (end - b >= 1) res = res.insert(b, ".")
          if (e != 0) {
            res += "E"
            if (e > 0) res += "+"
  ...
  def toPlainString(): String = {
    ...
    if (_scale == 0 || (isZero && _scale < 0)) { intStr }
    else {
      ...
      if (_scale > 0) {
        ...
        if (delta >= 0) {
          ...
          delta = begin - delta
          result += intStr.substring(begin, delta) + "." + intStr
            .substring(delta)
        }
      } else { // (scale <= 0)
  ...
  def toBigInteger(): BigInteger = {
    ...
    else if (_scale < 0) getUnscaledValue.multiply(powerOf10(-_scale.toLong))
  ...
  def toBigIntegerExact(): BigInteger = {
    if (_scale == 0 || isZero) { getUnscaledValue }
      ...
      val integerAndFraction = getUnscaledValue
        .divideAndRemainder(powerOf10(_scale))
  ...
  override def doubleValue(): Double = {
    ...
    if (powerOfTwo < -1074 || sign == 0) {
      ...
      val mantissa = {
        if (_scale <= 0) { mantissa0.multiply(powerOf10(-_scale)) }
        else {
  ...
  private def movePoint(newScale: Long): BigDecimal = {
    ...
    if (isZero) { zeroScaledBy(Math.max(newScale, 0)) }
    else if (newScale >= 0) {
  ...
  private def valueExact(bitLengthOfType: Int): Long = {
    ...
    if (bigInteger.bitLength() < bitLengthOfType) bigInteger.longValue()
    else throw new ArithmeticException("Rounding necessary")
  }
  ...
  private def approxPrecision(): Int = {
    if (_precision > 0) _precision else ((this._bitLength - 1) * Log2).toInt + 1
  }

  private def getUnscaledValue(): BigInteger = {
    if (_intVal == null) _intVal = BigInteger.valueOf(_smallValue)
    _intVal
  }
  ...
  private def setUnscaledValue(unscaledVal: BigInteger): Unit = {
    ...
    if (_bitLength < 64) _smallValue = unscaledVal.longValue()
  }
}
