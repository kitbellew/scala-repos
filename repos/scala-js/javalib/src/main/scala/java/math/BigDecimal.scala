...
class BigDecimal() extends Number with Comparable[BigDecimal] {
  ...
  def this(in: Array[Char], offset: Int, len: Int) = {
    ...
    if ((index <= last) && charEqualTo(in(index), 'e', 'E')) {
      ...
      val newScale: Long = _scale
        .toLong - java.lang.Integer.parseInt(scaleString)
  ...
  def divideToIntegralValue(divisor: BigDecimal): BigDecimal = {
    ...
    if (integralValue.signum() == 0)
      zeroScaledBy(varScale)
    else
      new BigDecimal(integralValue, safeLongToInt(varScale))
  }
  ...
  def pow(n: Int, mc: MathContext): BigDecimal = {
    ...
    if (n == 0 || (isZero && n > 0)) {
      ...
      val newPrecision = {
        if (mcPrec > 0)
          new MathContext(mcPrec + elength + 1, mc.roundingMode)
        else
          mc
      }
  ...
  def abs(mc: MathContext): BigDecimal = {
    val result =
      if (signum() < 0)
        ...
        new BigDecimal(getUnscaledValue, _scale)
    result.inplaceRound(mc)
    result
  }
  ...
  def scaleByPowerOfTen(n: Int): BigDecimal = {
    ...
    if (_bitLength < 64) {
      ...
      if (_smallValue == 0)
        ...
        valueOf(_smallValue, safeLongToInt(newScale))
    } else {
  ...
  def compareTo(bi: BigDecimal): Int = {
    ...
    if (thisSign == valueSign) {
      if (this._scale == bi._scale && this._bitLength < 64 && bi
            ._bitLength < 64) {
        if (_smallValue < bi._smallValue)
          -1
        else if (_smallValue > bi._smallValue)
          1
        else
          0
      } else {
  ...
  override def longValue(): Long = {
     ...
     * could be zero.
     */
    if (_scale <= -64 || _scale > approxPrecision())
      0L
    else
  ...
  override def intValue(): Int = {
     ...
     * could be zero.
     */
    if (_scale <= -32 || _scale > approxPrecision())
      0
    else
    ...
    if (mcPrecGood || discardedPrecision <= 0) {
      ...
      val newScale = {
        if (integerAndFraction(1).signum() != 0) {
          ...
          // To look if there is a carry
          val parityBit =
            if (integerAndFraction(0).testBit(0))
              1
            else
              0
