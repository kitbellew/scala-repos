...
class BigIntegerConstructorsTest {
  ...
  @Test def testConstructorSignBytesNegative6(): Unit = {
    ...
    val aSign = -1
    val rBytes = Array[Byte](
      -13, -57, -101, 1, 75, -90, -46, -92, -4, 14, -24, 101)
    val aNumber = new BigInteger(aSign, aBytes)
    var resBytes = Array.ofDim[Byte](rBytes.length)
    resBytes = aNumber.toByteArray()
    for (i <- 0 until resBytes.length) {
  ...
  @Test def testConstructorSignBytesNegative7(): Unit = {
    ...
    val aSign = -1
    val rBytes = Array[Byte](
      -1, 11, -57, -101, 1, 75, -90, -46, -92, -4, 14, -24, 101)
    val aNumber = new BigInteger(aSign, aBytes)
    var resBytes = Array.ofDim[Byte](rBytes.length)
    resBytes = aNumber.toByteArray()
    for (i <- 0 until resBytes.length) {
  ...
  @Test def testConstructorSignBytesPositive7(): Unit = {
    ...
    val aSign = 1
    val rBytes = Array[Byte](
      0, -12, 56, 100, -2, -76, 89, 45, 91, 3, -15, 23, -101)
    val aNumber = new BigInteger(aSign, aBytes)
    var resBytes = Array.ofDim[Byte](rBytes.length)
    resBytes = aNumber.toByteArray()
    for (i <- 0 until resBytes.length) {
  ...
  @Test def testConstructorStringRadix36(): Unit = {
    ...
    val radix = 36
    val rBytes = Array(
      0, -12, -116, 112, -105, 12, -36, 66, 108, 66, -20, -37, -15, 108, -7, 52,
      -99, -109, -8, -45, -5)
    val aNumber = new BigInteger(value, radix)
    var resBytes = Array.ofDim[Byte](rBytes.length)
    resBytes = aNumber.toByteArray()
