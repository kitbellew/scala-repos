...
private[optimizer] abstract class OptimizerCore(
  ...
  def optimize(
    ...
    try {
      ...
      val newBody =
        if (name.name == "init___") tryElimStoreModule(newBody1) else newBody1
  ...
  private def transform(tree: Tree, isStat: Boolean)(
    ...
    val result = tree match {
      ...
      case tree @ UnaryOp(_, arg) =>
        if (isStat) transformStat(arg) else transformUnaryOp(tree)
    ...
    if (isStat) keepOnlySideEffects(result) else result
  }
  ...
  private def keepOnlySideEffects(stat: Tree): Tree = stat match {
    ...
    case LoadModule(ClassType(moduleClassName)) =>
      if (hasElidableModuleAccessor(moduleClassName)) Skip()(stat.pos) else stat
      ...
      case Boolean_! =>
        arg match {
          ...
          case BinaryOp(innerOp, l, r) =>
            ...
            if (newOp == -1) default else BinaryOp(newOp, l, r)
