...
private[scalajs] final class ScalaJSClassEmitter(
  ...
  private def genES5Constructor(tree: LinkedClass): js.Tree = {
    ...
    val ctorFun =
      if (!isJSClass) {
        val superCtorCall = tree.superClass.fold[js.Tree] { js.Skip() } {
          parentIdent =>
            js.Apply(
    ...
    val chainProto = tree.superClass.fold[js.Tree] { js.Skip() } {
      parentIdent =>
        val (inheritedCtorDef, inheritedCtorRef) =
          if (!isJSClass) { (js.Skip(), envField("h", parentIdent.name)) }
          else {
  ...
  private def genFieldDefs(tree: LinkedClass): List[js.Tree] = {
    ...
    for { field @ FieldDef(name, ftpe, mutable) <- tree.fields } yield {
  ...
  def genInstanceTests(tree: LinkedClass): js.Tree = {
    ...
    if (tree.kind.isClass || tree.kind == ClassKind.Interface ||
      ...
      val createAsStat =
        if (semantics.asInstanceOfs == Unchecked) { js.Skip() }
        else {
          envFieldDef(
            "as",
            className,
    ...
    val (isInstanceFun, isArrayOfFun) = {
      if (isObjectClass) {
        ...
        (
          js.Function(
            List(xParam),
            js.Return { genIsInstanceOf(xParam.ref, ClassType(className)) }),
          js.Undefined())
          ...
          js.If(
            ...
            js.If(
              moduleInstanceVar === js.Null(), {
                ...
                val obj = js
                  .New(encodeClassVar("sjsr_UndefinedBehaviorError"), Nil)
