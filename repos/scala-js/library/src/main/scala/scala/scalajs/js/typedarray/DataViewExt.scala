...
object DataViewExt {
  implicit class DataViewExtOps(val dataView: DataView) extends AnyVal {
    ...
    def getInt64(index: Int, littleEndian: Boolean = false): Long = {
      val high = dataView
        .getInt32(index + (if (littleEndian) 4 else 0), littleEndian)
      val low = dataView
        .getInt32(index + (if (littleEndian) 0 else 4), littleEndian)
    ...
    def setInt64(
      ...
      val low = value.toInt
      dataView
        .setInt32(index + (if (littleEndian) 4 else 0), high, littleEndian)
