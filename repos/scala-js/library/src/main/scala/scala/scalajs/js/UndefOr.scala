...
sealed abstract class UndefOrLowPrioImplicits {
  ...
  /** Upcast `A` to `UndefOr[B1 | B2]`.
    ...
    */
  implicit def any2undefOrUnion[A, B1, B2](a: A)(implicit
      ev: Evidence[A, B1 | B2]): UndefOr[B1 | B2] = {
...
final class UndefOrOps[A](val self: UndefOr[A]) extends AnyVal {
  ...
  /** Necessary to keep $option from being implicitly converted to
    ...
    */
  @inline
  final def withFilter(p: A => Boolean): WithFilter[A] = new WithFilter(self, p)
  ...
  /** Returns a `Some` containing this $option's value
    ...
    */
  @inline
  final def toOption: Option[A] = if (isEmpty) None else Some(this.forceGet)
}
