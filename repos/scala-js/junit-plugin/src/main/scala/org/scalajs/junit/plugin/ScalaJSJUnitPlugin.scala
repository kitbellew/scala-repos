    ...
    global
      ...
      .collectFirst {
        case pl
            if pl
              .getClass
              .getName == "org.scalajs.core.compiler.ScalaJSPlugin" =>
  ...
  object ScalaJSJUnitPluginComponent
    ...
    class ScalaJSJUnitPluginTransformer extends Transformer {
      ...
      def mkBootstrapperClass(
        ...
        val invokeJUnitMethodDef = {
          val annotatedMethods =
            modDefOption.fold(List.empty[MethodSymbol]) { mod =>
      ...
      /** This method generates a method that invokes a test method in the class
        ...
        *  }}}
        */
      def mkInvokeJUnitMethodOnInstanceDef(
          methods: List[MethodSymbol],
          classSym: Symbol,
      ...
      def mkGetJUnitMetadataDef(
        ...
        def liftAnnotations(methodSymbol: Symbol): List[Tree] = {
          ...
          annotations.foreach {
            case ann
                if ann.atp.typeSymbol == TestClass && ann
                  .original
                  .isInstanceOf[Block] =>
              reporter.error(
                ann.pos,
      ...
      private def mkMethodResolutionAndCall(
        ...
        val tree =
          methods.foldRight[Tree](mkMethodNotFound(idParamSym)) {
            (methodSymbol, acc) =>
