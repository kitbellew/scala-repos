...
object ExternalCompile {
  ...
  val scalaJSExternalCompileConfigSettings: Seq[Setting[_]] =
    inTask(compile)(Defaults.runnerTask) ++ Seq(
      ...
      compile := {
        ...
        val allMyDependencies = classpath filterNot
          (_ == classesDirectory) flatMap { cpFile =>
          ...
          dependencies =>
            ...
            def doCompile(sourcesArgs: List[String]): Unit = {
              ...
              run.run(
                ...
                compilerCp,
                "-cp" :: cpStr :: "-d" :: classesDirectory.getAbsolutePath() ::
                  options ++: sourcesArgs,
