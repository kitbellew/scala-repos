...
object Build extends sbt.Build {
  ...
  val commonSettings = Seq(
    ...
    shouldPartest := {
      val testListDir = ((
        resourceDirectory in (partestSuite, Test)
      ).value / "scala"
    ...
    doc in Compile := {
      ...
      val additionalStylesFile =
        (
          root.base / "assets/additional-doc-styles.css"
        ).getCanonicalFile
  ...
  val publishIvySettings =
    (if (Properties.envOrNone("PUBLISH_TO_BINTRAY") == Some("true"))
       ...
       publishToScalaJSRepoSettings) ++ Seq(publishMavenStyle := false)
      ...
      if (isGeneratingEclipse) {
        ...
        project.settings(
          unmanagedSourceDirectories in Compile +=
            (
              scalaSource in (dependency, Compile)
            ).value)
      }
    }
  }
  ...
  lazy val irProjectJS: Project = Project(
    ...
    settings = commonIrProjectSettings ++ myScalaJSSettings ++ Seq(
      ...
      unmanagedSourceDirectories in Compile +=
        (
          scalaSource in Compile in irProject
        ).value)
  ...
  lazy val toolsJS: Project = Project(
    ...
    settings = myScalaJSSettings ++ commonToolsSettings ++ Seq(
      crossVersion := ScalaJSCrossVersion.binary) ++ inConfig(Test) {
    ...
    Project(
      ...
      settings =
        (commonSettings ++ myScalaJSSettings ++ fatalWarningsSettings) ++ Seq(
          ...
          publishArtifact in Compile := false,
          delambdafySetting,
          noClassFilesSettings) ++ (scalaJSExternalCompileSettings)
    ).withScalaJSCompiler.dependsOnLibraryNoJar
      ...
      settings = (
        ...
        Seq(
          ...
          mappings in packageBin := {
            ...
            val otherProducts = ((
              products in javalanglib
            ).value ++
              (
                products in scalalib
              ).value ++
              (
                products in libraryAux
              ).value)
  ...
  lazy val examples: Project = Project(
    id = "examples",
    base = file("examples"),
    settings = commonSettings ++ Seq(name := "Scala.js examples"))
  ...
  lazy val reversi = Project(
    ...
    settings = exampleSettings ++ Seq(
      ...
      moduleName := "reversi")).withScalaJSCompiler.dependsOn(library)
  ...
  val testTagSettings = {
    ...
    Seq(
      testOptionTags := {
        ...
        val envTags = envTagsFor(
          (
            resolvedJSEnv in Test
          ).value)

        val stage =
          (
            scalaJSStage in Test
          ).value
        ...
        val sems =
          stage match {
            case FastOptStage =>
              (
                scalaJSSemantics in (Test, fastOptJS)
              ).value
            case FullOptStage =>
              (
                scalaJSSemantics in (Test, fullOptJS)
              ).value
          }

        val semTags = (if (sems.asInstanceOfs == CheckedBehavior.Compliant)
  ...
  lazy val testSuite: Project = Project(
    ...
    settings = commonSettings ++ myScalaJSSettings ++ testTagSettings ++
      ...
      scalaJSSemantics ~= (_.withRuntimeClassName(
        _.fullName match {
      ...
      scalacOptions in Test ++= {
        if (isGeneratingEclipse) {
          ...
          val jar =
            (
              packageBin in (jUnitPlugin, Compile)
            ).value
          Seq(s"-Xplugin:$jar")
        }
      }
    ...
    settings = commonSettings ++ fatalWarningsSettings ++ Seq(
      ...
      sources in Compile := {
        if (shouldPartest.value) {
          ...
          val toolSrcs =
            (
              sources in (tools, Compile)
            ).value
          // Sources for js-envs
          val jsenvSrcs = {
            val jsenvBase = ((
              scalaSource in (jsEnvs, Compile)
            ).value /
              "org/scalajs/jsenv")
