...
abstract class GenJSCode
  ...
  val jsAddons: JSGlobalAddons { val global: GenJSCode.this.global.type }
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    override def apply(cunit: CompilationUnit): Unit = {
      try {
          ...
          if (!isPrimitive && !isRawJSImplClass) {
            withScopedVars(
              ...
              val tree =
                ...
                } else if (sym.isTraitOrInterface) { genInterface(cd) }
                else if (sym.isImplClass) { genImplClass(cd) }
                else { genClass(cd) }
        ...
        for ((sym, tree) <- generatedClasses) { genIRFile(cunit, sym, tree) }
      } finally {
    ...
    def genScalaJSDefinedJSClass(cd: ClassDef): js.ClassDef = {
      ...
      def gen(tree: Tree): Unit = {
        tree match {
          ...
          case dd: DefDef =>
            ...
            if (sym.isClassConstructor) { constructorTrees += dd }
      ...
      // The complete class definition
      val kind =
        if (isStaticModule(sym)) ClassKind.JSModuleClass else ClassKind.JSClass
    ...
    private def genClassInterfaces(sym: Symbol)(implicit
        pos: Position): List[js.Ident] = {
      for {
        parent <- sym.info.parents
        typeSym = parent.typeSymbol
        _ = assert(typeSym != NoSymbol, "parent needs symbol")
        if typeSym.isTraitOrInterface
      } yield { encodeClassFullNameIdent(typeSym) }
    }
    ...
    def genClassFields(cd: ClassDef): List[js.FieldDef] = {
      ...
      (for {
        ...
        val name =
          if (isExposed(f)) js.StringLiteral(jsNameOf(f)) else encodeFieldSym(f)

        val irTpe =
          if (!isScalaJSDefinedJSClass(classSym)) { toIRType(f.tpe) }
          else {
            val tpeEnteringPosterasure = enteringPhase(
              currentRun.posterasurePhase)(f.tpe)
    ...
    def genJSClassConstructor(
      ...
      if (hasDefaultCtorArgsAndRawJSModule(classSym)) {
        ...
        withNewLocalNameScope {
           ...
           */
          val prePrimaryCtorBody = jsConstructorBuilder
            .mkPrePrimaryCtorBody(overloadIdent)
           ...
           */
          val postPrimaryCtorBody = jsConstructorBuilder
            .mkPostPrimaryCtorBody(overloadIdent)
    ...
    private class ConstructorTree(
      ...
      def get(methodName: String): Option[ConstructorTree] = {
        if (methodName == this.method.name.name) { Some(this) }
        else {
    ...
    private class JSConstructorBuilder(root: ConstructorTree) {
      ...
      private def mkSubPreCalls(
        ...
        val overrideNumss = constructorTree.subConstructors
          .map(_.overrideNumBounds)
    ...
    private def zipMap[T, U, V](xs: List[T], ys: List[U])(
        f: (T, U) => V): List[V] = { for ((x, y) <- xs zip ys) yield f(x, y) }
    ...
    private def mkOverloadSelection(
        ...
        dispatchResolution: js.Tree)(implicit pos: Position): List[js.Tree] = {
      if (!jsConstructorBuilder.hasSubConstructors) {
        dispatchResolution match {
          ...
          case js.ApplyStatic(_, mtd, js.This() :: Nil)
              if ir.Definitions.isConstructorName(mtd.name) => Nil
    ...
    private def mkJSConstructorBuilder(ctors: List[js.MethodDef])(implicit
        pos: Position): JSConstructorBuilder = {
      def findCtorForwarderCall(tree: js.Tree): String =
        tree match {
          case js.ApplyStatic(_, method, js.This() :: _)
              if ir.Definitions.isConstructorName(method.name) => method.name
    ...
    def genMethod(dd: DefDef): Option[js.MethodDef] = {
      withNewLocalNameScope { genMethodWithCurrentLocalNameScope(dd) }
    }
    ...
    def genMethodWithCurrentLocalNameScope(dd: DefDef): Option[js.MethodDef] = {
      ...
      withScopedVars(
        ...
        if (scalaPrimitives.isPrimitive(sym) &&
            !jsPrimitives.shouldEmitPrimitiveBody(sym)) { None }
        ...
        } else if (isJSNativeCtorDefaultParam(sym)) { None }
          ...
          withScopedVars(
            ...
            val optimizerHints = OptimizerHints.empty
              .withInline(shouldMarkInline).withNoinline(shouldMarkNoinline)
    ...
    private def patchMutableFlagOfLocals(
      ...
      val newBody = transformer
        .transform(body, isStat = resultType == jstpe.NoType)
    ...
    private def moveAllStatementsAfterSuperConstructorCall(
      ...
      val (beforeSuper, superCall :: afterSuper) = bodyStats
        .span(!_.isInstanceOf[js.JSSuperConstructorCall])
    ...
    def genMethodDef(
      ...
      def genBody() =
        tree match {
          ...
          case _ => if (bodyIsStat) genStat(tree) else genExpr(tree)
        }
    ...
    def genStatOrExpr(tree: Tree, isStat: Boolean): js.Tree = {
      ...
      tree match {
        ...
        case ValDef(_, name, _, rhs) =>
          ...
          val sym = tree.symbol
          val rhsTree =
            if (rhs == EmptyTree) genZeroOf(sym.tpe) else genExpr(rhs)
        ...
        case Return(expr) => js.Return(toIRType(expr.tpe) match {
        ...
        case This(qual) =>
          if (tree.symbol == currentClassSym.get) { genThis() }
          else {
            assert(
        ...
        case Ident(name) =>
          ...
          if (!sym.hasPackageFlag) {
            ...
            } else { js.VarRef(encodeLocalSym(sym))(toIRType(sym.tpe)) }
          } else { sys.error("Cannot use package as value: " + tree) }

        case Literal(value) => value.tag match {
            ...
            case ClazzTag  => js.ClassOf(toReferenceType(value.typeValue))
    ...
    def genTry(tree: Try, isStat: Boolean): js.Tree = {
      ...
      val handlerAST = {
        if (catches.isEmpty) { js.EmptyTree }
        else {
          ...
          val handler0 = catches.foldRight(elseHandler) { (caseDef, elsep) =>
            ...
            // Generate the test
            if (tpe == ThrowableClass.tpe) { bodyWithBoundVar }
            else {
    ...
    def genApply(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      def isRawJSDefaultParam: Boolean = {
        if (isCtorDefaultParam(sym)) { isRawJSCtorDefaultParam(sym) }
        else {
      ...
      fun match {
        ...
        case _ =>
          if (sym.isLabel) { genLabelApply(tree) }
    ...
    private def genApplyTypeApply(tree: Apply): js.Tree = {
      ...
      if (l.isValueType && r.isValueType) {
        if (cast) genConversion(l, r, source) else js.BooleanLiteral(l == r)
        ...
        if (cast) genAsInstanceOf(source, to) else genIsInstanceOf(source, to)
      }
    }
    ...
    private def genApplyNew(tree: Apply): js.Tree = {
      ...
      if (isStringType(tpe)) { genNewString(tree) }
    ...
    def genTraitImplApply(method: Symbol, arguments: List[js.Tree])(implicit
        pos: Position): js.Tree = { genApplyStatic(method, arguments) }
    ...
    def genIsInstanceOf(value: js.Tree, to: Type)(implicit
      ...
      if (sym == ObjectClass) { js.BinaryOp(js.BinaryOp.!==, value, js.Null()) }
      ...
      } else { js.IsInstanceOf(value, toReferenceType(to)) }
    }
        ...
        def genBody(body: Tree): js.Tree =
          body match {
            ...
            case Block(List(app @ Apply(_, Nil)), _)
                if app.symbol == defaultLabelSym => genDefaultBody
    ...
    private def genBlock(tree: Block, isStat: Boolean): js.Tree = {
      ...
      def translateMatch(expr: LabelDef) = {
        ...
        val translatedMatch = genTranslatedMatch(
          cases.map(_.asInstanceOf[LabelDef]),
          expr)
      ...
      expr match {
        ...
        case Apply(TypeApply(Select(expr: LabelDef, nme.asInstanceOf_Ob), _), _)
            if isCaseLabelDef(expr) => translateMatch(expr)
    ...
    private def genPrimitiveOp(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      else if (code == HASH) genScalaHash(tree, receiver)
      else if (isArrayOp(code)) genArrayOp(tree, code)
      else if (code == SYNCHRONIZED) genSynchronized(tree, isStat)
      else if (isCoercion(code)) genCoercion(tree, receiver, code)
    ...
    private def genSimpleOp(
      ...
      sources match {
        ...
        case List(source) =>
          (code match {
            case POS => source
            case NEG => (resultType: @unchecked) match {
                case jstpe.IntType =>
            ...
            case NOT => (resultType: @unchecked) match {
                case jstpe.IntType =>
          ...
          (code: @switch) match {
            ...
            case _ =>
              ...
              val op = (resultType: @unchecked) match {
                case jstpe.IntType => (code: @switch) match {
                    case ADD => Int_+
                    case SUB => Int_-
                    case MUL => Int_*
                    case DIV => Int_/
                ...
                case jstpe.FloatType => (code: @switch) match {
                    case ADD => Float_+
                    case SUB => Float_-
                    case MUL => Float_*
                    case DIV => Float_/
                ...
                case jstpe.DoubleType => (code: @switch) match {
                    case ADD => Double_+
                    case SUB => Double_-
                    case MUL => Double_*
                    case DIV => Double_/
                ...
                case jstpe.BooleanType => (code: @switch) match {
                    case LT  => Num_<
                    case LE  => Num_<=
                    case GT  => Num_>
                    case GE  => Num_>=
    ...
    def genEqEqPrimitive(ltpe: Type, rtpe: Type, lsrc: js.Tree, rsrc: js.Tree)(
      ...
      if (mustUseAnyComparator) {
        ...
        if (isStringType(ltpe)) {
           ...
           */
          val ltemp = js
            .VarDef(freshLocalIdent(), lsrc.tpe, mutable = false, lsrc)
          val rtemp = js
            .VarDef(freshLocalIdent(), rsrc.tpe, mutable = false, rsrc)
          js.Block(
            ltemp,
            rtemp,
            js.If(
    ...
    private def genArrayOp(tree: Tree, code: Int): js.Tree = {
      ...
      def genSelect() = {
        val elemIRType = toTypeKind(arrayObj.tpe).asInstanceOf[ARRAY].elem
          .toIRType
    ...
    private def genCoercion(tree: Apply, receiver: Tree, code: Int): js.Tree = {
      ...
      def source2int =
        (code: @switch) match {
          ...
          case L2C | L2B | L2S | L2I => js.UnaryOp(js.UnaryOp.LongToInt, source)
          case _                     => source
        }
            ...
            genIsInstanceOf(callTrg, rtClass.tpe), {
              if (implMethodSym.owner == ObjectClass) {
                ...
                if (rtClass == StringClass) {
                  val (rtModuleClass, methodIdent) = encodeRTStringMethodSym(
                    implMethodSym)
                  val retTpe = implMethodSym.tpe.resultType
                  val castCallTrg = fromAny(
                    callTrg,
                    StringClass.toTypeConstructor)
        ...
        case kind: ValueTypeKind =>
          if (kind == CharKind) {
            genApplyMethod(
              ...
              List(expr))
          } else { js.Unbox(expr, kind.primitiveCharCode) }
        case _ => abort(
    ...
    private def lookupModuleClass(name: String) = {
      ...
      if (module == NoSymbol) NoSymbol else module.moduleClass
    }
    ...
    private def genJSPrimitive(
      ...
      if (code == DYNNEW) {
        ...
        def warnIfDuplicatedKey(
          ...
          val keyCounts = allKeys.distinct
            .map(key => key -> allKeys.count(_ == key))
        ...
        (genArgs match {
          ...
          case List(arg) =>
            ...
            def genFunctionToJSFunction(isThisFunction: Boolean): js.Tree = {
              ...
              val fCaptureParam = js.ParamDef(
                ...
                mutable = false,
                rest = false)
              val jsArity = if (isThisFunction) arity - 1 else arity
          ...
          case List(arg1, arg2) => code match {
              case HASPROP =>
      ...
      val boxedResult = sym.name match {
        ...
        case _ =>
          ...
          def genSuperReference(propName: js.Tree): js.Tree = {
            superIn.fold[js.Tree] { js.JSBracketSelect(receiver, propName) } {
              superInSym =>
    ...
    private def genPrimitiveJSClass(sym: Symbol)(implicit
        pos: Position): js.Tree = {
      assert(
    ...
    private def genPrimitiveJSModule(sym: Symbol)(implicit
        pos: Position): js.Tree = {
      assert(
        sym.isModuleClass,
    ...
    private def genPrimitiveJSArgs(sym: Symbol, args: List[Tree])(implicit
      ...
      val wereRepeated = exitingPhase(currentRun.typerPhase) {
        for {
          ...
          param <- params
        } yield { param.name -> isScalaRepeatedParamType(param.tpe) }
      }.toMap
      ...
      def unapply(tree: Tree): Some[Tree] =
        tree match {
          case Apply(TypeApply(asInstanceOf_? @ Select(base, _), _), _)
              if asInstanceOf_?.symbol == Object_asInstanceOf => Some(base)
          case _                                              => Some(tree)
        }
    }
      ...
      def unapply(tree: Apply): Option[Tree] =
        tree match {
          case Apply(wrapArray_?, List(wrapped))
              if isWrapArray(wrapArray_?.symbol) => Some(wrapped)
          case _                                 => None
        }
    }
    ...
    private def tryGenAndRecordAnonFunctionClassGeneric(cd: ClassDef)(
      ...
      def gen(tree: Tree): Unit = {
        tree match {
          ...
          case vd @ ValDef(mods, name, tpt, rhs) =>
            ...
            if (fsym.isPrivate) { paramAccessors ::= fsym }
            else {
      ...
      withNewLocalNameScope {
        ...
        val usedCtorParams =
          if (hasUnusedOuterCtorParam) ctorParams.tail else ctorParams
        ...
        val applyMethod = withScopedVars(
          ...
          try {
            genMethodWithCurrentLocalNameScope(applyDef)
              .getOrElse(abort(s"Oops, $applyDef did not produce a method"))
          } catch { case e: CancelGenMethodAsJSFunction => fail(e.getMessage) }
        }

        // Fourth step: patch the body to unbox parameters and box result

        val js.MethodDef(_, _, params, _, body) = applyMethod
        val (patchedParams, patchedBody) = patchFunBodyWithBoxes(
          applyDef.symbol,
          params,
          body)
        ...
        val functionMaker = { capturedArgs0: List[js.Tree] =>
          val capturedArgs =
            if (hasUnusedOuterCtorParam) capturedArgs0.tail else capturedArgs0
    ...
    private def genAnonFunction(originalFunction: Function): js.Tree = {
      ...
      val Function(
        paramTrees,
        Apply(targetTree @ Select(receiver, _), allArgs0)) = originalFunction

      val captureSyms = global.delambdafy.FreeVarTraverser
        .freeVarsOf(originalFunction)
      ...
      val (allFormalCaptures, body, allActualCaptures) =
        if (!isInImplClass) {
          ...
          val body =
            ...
            } else { genApplyMethod(thisCaptureArg, target, allArgs) }
    ...
    private def patchFunBodyWithBoxes(
        ...
        body: js.Tree,
        useParamsBeforeLambdaLift: Boolean = false)(implicit
        pos: Position): (List[js.ParamDef], js.Tree) = {
      ...
      val (patchedParams, paramsLocal) =
        (for { (param, paramSym) <- params zip paramSyms } yield {
      ...
      val patchedBody = js
        .Block(paramsLocal :+ ensureBoxed(body, methodType.resultType))
    ...
    def genLoadModule(sym0: Symbol)(implicit pos: Position): js.Tree = {
      ...
      val sym = // redirect all static methods of String to RuntimeString
        if (sym1 == StringModule) RuntimeStringModule.moduleClass else sym1

      val isGlobalScope = sym.tpe.typeSymbol isSubClass JSGlobalScopeClass

      if (isGlobalScope) { genLoadGlobal() }
      else if (isJSNativeClass(sym)) { genPrimitiveJSModule(sym) }
      else {
        val moduleClassName = encodeClassFullName(sym)

        val cls = jstpe.ClassType(moduleClassName)
        if (isRawJSType(sym.tpe)) js.LoadJSModule(cls) else js.LoadModule(cls)
      }
    }
    ...
    private def genStaticMember(sym: Symbol)(implicit pos: Position) = {
      ...
      import jsPrimitives._
      if (isPrimitive(sym)) {
        getPrimitive(sym) match { case UNITVAL => js.Undefined() }
      } else {
  ...
  private def patchedLinkedClassOfClass(sym: Symbol): Symbol = {
    ...
    val flatOwnerInfo = {
      ...
      if (sym.needsFlatClasses) sym.info
      sym.owner.rawInfo
    }
    val result = flatOwnerInfo.decl(sym.name).filter(_ isCoDefinedWith sym)
    if (!result.isOverloaded) result else result.alternatives.head
  }
  ...
  protected lazy val isHijackedBoxedClass: Set[Symbol] = HijackedBoxedClasses
    .toSet
