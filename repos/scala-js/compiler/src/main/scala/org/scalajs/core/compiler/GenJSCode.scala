...
abstract class GenJSCode
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    private val translatedAnonFunctions = mutable
      .Map
    ...
    def genClass(cd: ClassDef): js.ClassDef = {
      ...
      def gen(tree: Tree): Unit = {
        tree match {
          ...
          case dd: DefDef =>
            ...
            val isNamedExport = isExport && sym
              .annotations
      ...
      val generatedMembers =
        if (!isHijacked)
          genClassFields(cd) ++ generatedMethods.toList
        else
          generatedMethods.toList // No fields needed
      ...
      val exports = {
        ...
        val exportedConstructorsOrAccessors =
          if (isStaticModule(sym))
            genModuleAccessorExports(sym)
          else
            genConstructorExports(sym)
    ...
    def genRawJSClassData(cd: ClassDef): js.ClassDef = {
      ...
      val superClass =
        if (sym.isTraitOrInterface)
          ...
          Some(encodeClassFullNameIdent(sym.superClass))
      val jsName =
        if (sym.isTraitOrInterface || sym.isModuleClass)
          None
        else
          Some(fullJSNameOf(sym))
    ...
    private class ConstructorTree(
      ...
      lazy val overrideNumBounds: (Int, Int) =
        if (subConstructors.isEmpty)
          (overrideNum, overrideNum)
        else
          (subConstructors.head.overrideNumBounds._1, overrideNum)
      ...
      withScopedVars(
        ...
        def isAbstractMethod =
          if (scalaUsesImplClasses)
            sym.isDeferred || sym.owner.isInterface
          else
            rhs == EmptyTree
        ...
        if (scalaPrimitives.isPrimitive(sym) &&
          ...
          val body =
            if (scalaUsesImplClasses &&
              ...
              val implMethodSym = implClassSym
                .info
                .member(sym.name)
                .suchThat { s =>
                  s.isMethod &&
              ...
              if (isJSClassConstructor) {
                ...
                val body1 =
                  if (!sym.isPrimaryConstructor)
                    ...
                    moveAllStatementsAfterSuperConstructorCall(body0)
                js.MethodDef(
                  static = false,
                  methodName,
                  jsParams,
                  ...
                  (
                    unmutatedMutableLocalVars
                      ...
                      mutatedImmutableLocalVals
                        .map(encodeLocalSym(_).name -> true)
                  ).toMap
    ...
    private def patchMutableFlagOfLocals(
      ...
      val transformer =
        new ir.Transformers.Transformer {
      ...
      tree match {
        ...
        case LabelDef(labelName, labelParams, rhs) =>
          val labelParamSyms = labelParams.map(_.symbol) map { s =>
            if (s == fakeTailJumpParamRepl._1)
              fakeTailJumpParamRepl._2
            else
              s
          }
    ...
    def genTry(tree: Try, isStat: Boolean): js.Tree = {
      ...
      if (handlerAST == js.EmptyTree && finalizerAST == js.EmptyTree)
        ...
        js.Try(blockAST, exceptIdent, handlerAST, finalizerAST)(resultType)
    }
    ...
    private def genApplyTypeApply(tree: Apply): js.Tree = {
      ...
      if (l.isValueType && r.isValueType) {
        ...
        val result =
          if (cast) {
            val ctor = ClassCastExceptionClass
              .info
    ...
    def genMatch(tree: Tree, isStat: Boolean): js.Tree = {
      ...
      val List(defaultBody0) =
        for {
          CaseDef(Ident(nme.WILDCARD), EmptyTree, body) <- cases
        } yield body

      val (defaultBody, defaultLabelSym) =
        defaultBody0 match {
      ...
      sources match {
        ...
        case List(lsrc, rsrc) if isLongOp(args(0).tpe, args(1).tpe) =>
          def toLong(tree: js.Tree, tpe: Type) =
            if (isLongType(tpe))
              ...
              js.UnaryOp(js.UnaryOp.IntToLong, tree)

          def toInt(tree: js.Tree, tpe: Type) =
            if (isLongType(tpe))
              js.UnaryOp(js.UnaryOp.LongToInt, rsrc)
            else
              tree
        ...
        case List(lsrc_in, rsrc_in) =>
          ...
          def convertArg(tree: js.Tree, kind: TypeKind) = {
            ...
            val notLong =
              if (kind == LongKind)
                js.UnaryOp(js.UnaryOp.LongToDouble, tree)
              else
                tree

            if (opType != jstpe.FloatType)
              notLong
            else if (kind == FloatKind)
              notLong
            else
              js.UnaryOp(js.UnaryOp.DoubleToFloat, notLong)
          }
              ...
              case _ =>
                if (eqeq &&
                  ...
                  val body = genEqEqPrimitive(
                    ...
                    rsrc)
                  if (not)
                    js.UnaryOp(js.UnaryOp.Boolean_!, body)
                  else
                    body
                } else {
                  js.BinaryOp(
    ...
    def genEqEqPrimitive(ltpe: Type, rtpe: Type, lsrc: js.Tree, rsrc: js.Tree)(
      ...
      val mustUseAnyComparator: Boolean =
        isRawJSType(ltpe) || isRawJSType(rtpe) || {
          val areSameFinals = ltpe.isFinalType && rtpe
            .isFinalType && (ltpe =:= rtpe)
    ...
    private def genStringConcat(
      ...
      val lhs = {
        ...
        if (!isPrimitiveValueType(receiver.tpe))
          lhs0
        else
          makePrimitiveBox(lhs0, receiver.tpe)
      }
    ...
    private def genPrimitiveJSModule(sym: Symbol)(implicit
      ...
      fullJSNameOf(sym)
        .split('.')
        .foldLeft(genLoadGlobal()) { (memo, chunk) =>
