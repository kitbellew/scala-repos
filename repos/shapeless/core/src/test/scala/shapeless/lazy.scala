...
class LazyStrictTests {
  ...
  def testEffectOrder {
    ...
    implicit def lazyInt: Lazy[Int] =
      Lazy[Int] {
        effects += 3;
        23
      }

    implicit def strictInt: Strict[Int] =
      Strict[Int] {
        effects += 6;
        23
      }
  ...
  def testNotFound {
    ...
    illTyped("lazily[U[String]]", "No U\\[String]")

    illTyped("lazily[V]", "could not find Lazy implicit value of type V")

    illTyped("lazily[W[String, Int]]", "No W\\[String, Int]")
  }
}
