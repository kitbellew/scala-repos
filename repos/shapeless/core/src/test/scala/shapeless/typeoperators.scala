...
class TypeOperatorTests {
  ...
  object Foo {
    implicit def mkFoo: Foo { type T = Int } =
      new Foo { type T = Int; val t = 23 }
  }
  ...
  object Foo2 {
    implicit def mkFoo2: Foo2[Char] { type T = Int } =
      new Foo2[Char] { type T = Int; val t = 23 }
  }
  ...
  def testTheQuantifiers {
    def bar0[T, U0](implicit
        b: Bar[T] { type U = U0 }): Bar[T] { type U = U0 } = {
      val res = the[Bar[T]]
      res
    }
