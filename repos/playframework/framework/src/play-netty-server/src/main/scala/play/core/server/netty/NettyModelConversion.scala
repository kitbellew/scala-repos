...
private[server] class NettyModelConversion(
  ...
  def convertRequest(
    ...
    } else { tryToCreateRequest(request, requestId, remoteAddress, sslHandler) }
  }
  ...
  def convertRequestBody(request: HttpRequest)(implicit
      mat: Materializer): Option[Source[ByteString, Any]] = {
    request match {
        ...
        result.body.contentLength.foreach { contentLength =>
          if (HttpHeaders.isContentLengthSet(response)) {
            ...
            if (manualContentLength == contentLength.toString) {
              logger
                .info(s"Manual Content-Length header, ignoring manual header.")
            } else {
              logger.warn(
  ...
  private def dateHeader: String = {
    ...
    cachedDateHeader match {
      ...
      case _ =>
        val dateHeaderString = ResponseHeader.httpDateFormat
