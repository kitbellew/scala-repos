...
class CachedSpec extends PlaySpecification {
  ...
  "the cached action" should {
    "cache values using injected CachedApi" in
      new WithApplication() {
    ...
    "cache values using named injected CachedApi" in
      new WithApplication(
    ...
    "cache values to disk using injected CachedApi" in
      new WithApplication() {
    ...
    "cache values using Application's Cached" in
      new WithApplication() {
        val invoked = new AtomicInteger()
        val action =
          Cached(_ => "foo") {
    ...
    "use etags for values" in
      new WithApplication() {
        ...
        val result2 = action(
          FakeRequest().withHeaders(IF_NONE_MATCH -> etag.get)).run()
        status(result2) must_== NOT_MODIFIED
        invoked.get() must_== 1
      }

    "support wildcard etags" in
      new WithApplication() {
        val invoked = new AtomicInteger()
        val action =
          Cached(_ => "foo")(Action(Results.Ok("" + invoked.incrementAndGet())))
    ...
    "work with etag cache misses" in
      new WithApplication() {
        val action = Cached(_.uri)(Action(Results.Ok))
        val resultA = action(FakeRequest("GET", "/a")).run()
        status(resultA) must_== 200
        status(
          action(FakeRequest("GET", "/a").withHeaders(IF_NONE_MATCH -> "foo"))
            .run) must_== 200
        status(
          action(
            FakeRequest("GET", "/b")
              .withHeaders(IF_NONE_MATCH -> header(ETAG, resultA).get))
            .run) must_== 200
        status(
          action(FakeRequest("GET", "/c").withHeaders(IF_NONE_MATCH -> "*"))
            .run) must_== 200
      }
  }
  ...
  "Cached EssentialAction composition" should {
    "cache infinite ok results" in
      new WithApplication() {
        ...
        val res2 = contentAsString(
          actionNotFound(FakeRequest("GET", "/b")).run())
        val res3 = contentAsString(
          actionNotFound(FakeRequest("GET", "/b")).run())

        res2 must not equalTo (res3)
      }

    "cache everything for infinite" in
      new WithApplication() {
  ...
  "CacheApi" should {
    ...
    "doesnt give items from cache with wrong type" in
      new WithApplication() {
        val defaultCache = app.injector.instanceOf[CacheApi]
        defaultCache.set("foo", "bar")
        defaultCache.set("int", 31)
  ...
  "EhCacheModule" should {
    "support binding multiple different caches" in
      new WithApplication(
        _.configure("play.cache.bindCaches" -> Seq("custom"))) {
