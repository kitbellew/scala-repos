...
object AllowedHostsFilterSpec extends PlaySpecification {
  ...
  "the allowed hosts filter" should {
    "disallow non-local hosts with default config" in
      withApplication(okWithHost, "") {
        status(request("localhost")) must_== OK
        status(request("typesafe.com")) must_== BAD_REQUEST
        status(request("")) must_== BAD_REQUEST
      }

    "only allow specific hosts specified in configuration" in
      withApplication(
        okWithHost,
        """
    ...
    "allow defining host suffixes in configuration" in
      withApplication(okWithHost, """
    ...
    "support FQDN format for hosts" in
      withApplication(
        okWithHost,
        """
        |play.filters.hosts.allowed = [".example.com", "example.net"]
    ...
    "support allowing empty hosts" in
      withApplication(
        okWithHost,
        """
        |play.filters.hosts.allowed = [".example.com", ""]
    ...
    "support host headers with ports" in
      withApplication(okWithHost, """
        |play.filters.hosts.allowed = ["example.com"]
      """.stripMargin) {
    ...
    "restrict host headers based on port" in
      withApplication(
        okWithHost,
        """
    ...
    "not allow malformed ports" in
      withApplication(okWithHost, """
        |play.filters.hosts.allowed = [".mozilla.org"]
      """.stripMargin) {
        status(request("addons.mozilla.org:@passwordreset.net")) must_==
          BAD_REQUEST
        status(
          request("addons.mozilla.org: www.securepasswordreset.com")) must_==
          BAD_REQUEST
      }

    "validate hosts in absolute URIs" in
      withApplication(okWithHost, """
        |play.filters.hosts.allowed = [".mozilla.org"]
      """.stripMargin) {
        status(
          request(
            ...
            "https://addons.mozilla.org/en-US/firefox/users/pwreset")) must_==
          OK
        status(
          request(
            "addons.mozilla.org",
            "https://www.securepasswordreset.com/en-US/firefox/users/pwreset")) must_==
          BAD_REQUEST
      }

    "not allow bypassing with X-Forwarded-Host header" in
      withServer(okWithHost, """
