...
object CORSFilterSpec extends CORSCommonSpec {
  ...
  "The CORSFilter" should {
    ...
    "pass through a cors request that doesn't match the path prefixes" in
      withApplication(conf = restrictPaths) {
        val result =
          route(
            fakeRequest("GET", "/baz")
...
object CORSWithCSRFSpec extends CORSCommonSpec {
  ...
  "The CORSFilter" should {
    ...
    "Mark CORS requests so the CSRF filter will let them through" in
      withApp() { app =>
    ...
    "Forbid CSRF requests when CORS filter is not installed" in
      withApp(classOf[FiltersWithoutCors]) { app =>
        val result = route(app, corsRequest).get
...
object CORSActionBuilderSpec extends CORSCommonSpec {
  ...
  "The CORSActionBuilder with" should {
    ...
    val restrictOriginsPathConf = Map(
      "myaction.allowedOrigins" ->
        Seq("http://example.org", "http://localhost:9000"))

    "handle a cors request with a subpath of app configuration" in
      withApplicationWithPathConfiguredAction(
        configPath = "myaction",
        conf = restrictOriginsPathConf) {
        val result =
          route(fakeRequest().withHeaders(ORIGIN -> "http://localhost:9000"))
            .get
        ...
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("http://localhost:9000")
...
trait CORSCommonSpec extends PlaySpecification {
  ...
  def commonTests = {
    ...
    "pass through requests without an origin header" in
      withApplication() {
    ...
    "not consider sub domains to be the same origin" in
      withApplication() {
        val result =
          route(
            fakeRequest().withHeaders(
        ...
        status(result) must_== OK
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("http://www.example.com")
      }

    "not consider different ports to be the same origin" in
      withApplication() {
        val result =
          route(
            fakeRequest().withHeaders(
        ...
        status(result) must_== OK
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("http://www.example.com:9000")
      }

    "not consider different protocols to be the same origin" in
      withApplication() {
        val result =
          route(
            fakeRequest().withHeaders(
        ...
        status(result) must_== OK
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("https://www.example.com:9000")
      }

    "forbid an empty origin header" in
      withApplication() {
    ...
    "forbid an invalid origin header" in
      withApplication() {
    ...
    "forbid an unrecognized HTTP method" in
      withApplication() {
        val result =
    ...
    "forbid an empty Access-Control-Request-Method header in a preflight request" in
      withApplication() {
        val result =
          route(
    ...
    "handle a simple cross-origin request with default config" in
      withApplication() {
        val result =
          route(
            fakeRequest("GET", "/").withHeaders(ORIGIN -> "http://localhost"))
            .get
        ...
        header(ACCESS_CONTROL_ALLOW_METHODS, result) must beNone
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("http://localhost")
        header(ACCESS_CONTROL_EXPOSE_HEADERS, result) must beNone
        header(ACCESS_CONTROL_MAX_AGE, result) must beNone
        header(VARY, result) must beSome(ORIGIN)
      }

    "handle simple cross-origin request when the action throws an error" in
      withApplication() {
        val result =
          route(
    ...
    "handle a basic preflight request with default config" in
      withApplication() {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
    ...
    "handle a preflight request with request headers with default config" in
      withApplication() {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
        ...
        status(result) must_== OK
        header(ACCESS_CONTROL_ALLOW_CREDENTIALS, result) must beSome("true")
        header(ACCESS_CONTROL_ALLOW_HEADERS, result) must
          beSome("x-header1,x-header2")
        header(ACCESS_CONTROL_ALLOW_METHODS, result) must beSome("PUT")
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("http://localhost")
        header(ACCESS_CONTROL_EXPOSE_HEADERS, result) must beNone
    ...
    "handle an actual cross-origin request with default config" in
      withApplication() {
        val result =
          route(
            fakeRequest("PUT", "/").withHeaders(ORIGIN -> "http://localhost"))
            .get
    ...
    "handle a preflight request with credentials support off" in
      withApplication(conf = noCredentialsConf) {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
              ORIGIN -> "http://localhost",
    ...
    "handle a simple cross-origin request with credentials support off" in
      withApplication(conf = noCredentialsConf) {
        val result =
          route(
            fakeRequest("GET", "/").withHeaders(ORIGIN -> "http://localhost"))
    ...
    "handle a preflight request with preflight caching off" in
      withApplication(conf = noPreflightCache) {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
              ORIGIN -> "http://localhost",
    ...
    "handle a preflight request with custom preflight cache max age" in
      withApplication(conf = customMaxAge) {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
              ORIGIN -> "http://localhost",
    ...
    "forbid a preflight request with a retricted request method" in
      withApplication(conf = restrictMethods) {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
              ORIGIN -> "http://localhost",
    ...
    "forbid a preflight request with a retricted request header" in
      withApplication(conf = restrictHeaders) {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
              ORIGIN -> "http://localhost",
    ...
    "handle a cors request with exposed headers configured" in
      withApplication(conf = exposeHeaders) {
        val result =
        ...
        header(ACCESS_CONTROL_ALLOW_ORIGIN, result) must
          beSome("http://localhost")
        header(ACCESS_CONTROL_EXPOSE_HEADERS, result) must
          beSome("X-Header1,X-Header2")
        header(ACCESS_CONTROL_MAX_AGE, result) must beNone
        header(VARY, result) must beSome(ORIGIN)
      }
    ...
    "forbid a preflight request with a retricted origin" in
      withApplication(conf = restrictOrigins) {
        val result =
          route(
            fakeRequest("OPTIONS", "/").withHeaders(
              ORIGIN -> "http://localhost",
    ...
    "forbid a cors request with a restricted origin" in
      withApplication(conf = restrictOrigins) {
        val result =
          route(fakeRequest().withHeaders(ORIGIN -> "http://localhost")).get
    ...
    "handle a cors request with a whitelisted origin" in
      withApplication(conf = restrictOrigins) {
        val result =
          route(fakeRequest().withHeaders(ORIGIN -> "http://localhost:9000"))
            .get
