...
class BoneConnectionPool @Inject() (environment: Environment)
  ...
  def create(
    ...
    val isolation = config.getDeprecated[Option[String]](
      "bonecp.isolation",
      "isolation").map {
    ...
    datasource.setDetectUnresolvedTransactions(
      config.getDeprecated[Boolean](
        "bonecp.detectUnresolvedTransactions",
    ...
    config.getDeprecated[Option[String]]("bonecp.initSQL", "initSQL").foreach(
