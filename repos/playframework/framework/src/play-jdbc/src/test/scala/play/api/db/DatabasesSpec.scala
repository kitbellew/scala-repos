...
object DatabasesSpec extends Specification {
  ...
  "Databases" should {
    ...
    "create database with named arguments" in
      new WithDatabase {
        val db = Databases(
          name = "test",
          driver = "org.h2.Driver",
    ...
    "create database with log sql" in
      new WithDatabase {
    ...
    "create default in-memory database" in
      new WithDatabase {
    ...
    "create named in-memory database" in
      new WithDatabase {
    ...
    "create in-memory database with url options" in
      new WithDatabase {
    ...
    "enable autocommit on connections by default" in
      new WithDatabase {
    ...
    "not supply connections after shutdown" in {
      ...
      db.shutdown()
      db.getConnection.close() must
        throwA[SQLException].like {
          case e =>
