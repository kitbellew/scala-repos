...
class ConnectionPoolConfigSpec extends PlaySpecification {
  ...
  "DBModule bindings" should {
    ...
    "use HikariCP when default pool is default" in
    ...
    "use HikariCP when default pool is 'hikaricp'" in
      new WithApplication(_.configure(
    ...
    "use BoneCP when default pool is 'bonecp'" in
      new WithApplication(_.configure(
        "play.db.pool" -> "bonecp",
        "db.default.url" -> "jdbc:h2:mem:default",
        "db.other.driver" -> "org.h2.Driver",
    ...
    "use BoneCP with 'bonecp' specific settings" in
      new WithApplication(_.configure(
        "play.db.pool" -> "bonecp",
        "db.default.url" -> "jdbc:h2:mem:default",
        "db.other.driver" -> "org.h2.Driver",
        ...
        val db = app.injector.instanceOf[DBApi]
        val bonecpDataSource: BoneCPDataSource = db.database("default")
          .dataSource.asInstanceOf[BoneCPDataSource]
        val bonecpConfig = bonecpDataSource.getConfig
        bonecpConfig.getMaxConnectionsPerPartition must be_==(50)
      }

    "use BoneCP when database-specific pool is 'bonecp'" in
      new WithApplication(FakeApplication(additionalConfiguration = Map(
