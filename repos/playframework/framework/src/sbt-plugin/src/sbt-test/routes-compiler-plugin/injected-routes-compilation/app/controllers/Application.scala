...
class Application extends Controller {
  def index = Action { Ok }
  def post = Action { Ok }
  def withParam(param: String) = Action { Ok(param) }
  def takeInt(i: Int) = Action { Ok(s"$i") }
  def takeBool(b: Boolean) = Action { Ok(s"$b") }
  def takeBool2(b: Boolean) = Action { Ok(s"$b") }
  def takeList(x: List[Int]) = Action { Ok(x.mkString(",")) }
  def takeListTickedParam(`b[]`: List[Int]) = Action { Ok(`b[]`.mkString(",")) }
  def takeTickedParams(`b[]`: List[Int], `b%%`: String) =
    Action { Ok(`b[]` mkString (",") + " " + `b%%`) }
  ...
  def interpolatorWarning(parameter: String) = Action { Ok(parameter) }
}
