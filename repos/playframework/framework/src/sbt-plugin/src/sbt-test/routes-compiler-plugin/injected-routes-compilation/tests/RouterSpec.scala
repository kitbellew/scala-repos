...
object RouterSpec extends PlaySpecification {
  ...
  "bind int parameters from the query string as a list" in {
    ...
    "using backticked names on route params" in
      new WithApplication() {
        val Some(result) = route(
          implicitApp,
    ...
    "using backticked names urlencoded on route params" in
      new WithApplication() {
        val Some(result) = route(
          implicitApp,
          FakeRequest(
  ...
  "use a new instance for each instantiated controller" in
    new WithApplication() {
      route(implicitApp, FakeRequest(GET, "/instance")) must
        beSome.like {
          case result =>
  ...
  "URL encoding and decoding works correctly" in
    new WithApplication() {
      def checkDecoding(
  ...
  "allow reverse routing of routes includes" in
    new WithApplication() {
  ...
  "reverse routes complex query params " in
    new WithApplication() {
      controllers
        .routes
        .Application
  ...
  "The assets reverse route support" should {
    ...
    "selected the minified non fingerprinted version" in
      new WithApplication() {
        controllers
          .routes
          .Assets
