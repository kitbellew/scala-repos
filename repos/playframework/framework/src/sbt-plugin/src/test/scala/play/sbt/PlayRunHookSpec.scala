...
object PlayRunHookSpec extends Specification {
  ...
  "PlayRunHook runner" should {
    ...
    "provide implicit `run` which passes every hook to a provided function" in {
      ...
      val executedHooks: HashMap[play.runsupport.RunHook, Boolean] = HashMap
        .empty
    ...
    "combine several thrown exceptions into a RunHookCompositeThrowable" in {
      ...
      def createDummyHooks = new PlayRunHook { executedHooks += ((this, true)) }
      ...
      hooks.run(_.beforeStarted()) must
        throwA[play.runsupport.RunHookCompositeThrowable].like {
          case e: Throwable =>
