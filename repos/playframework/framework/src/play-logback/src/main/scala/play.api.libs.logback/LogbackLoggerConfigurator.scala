...
class LogbackLoggerConfigurator extends LoggerConfigurator {
  ...
  def configure(env: Environment): Unit = {
    ...
    if (!env.resource("application-logger.xml")
          .orElse(env.resource("logger.xml")).isEmpty) {
      System.err.println(
    ...
    def resourceUrl =
      env.resource("logback.xml").orElse(env.resource(
        if (env.mode == Mode.Dev) "logback-play-dev.xml"
        else "logback-play-default.xml"))

    val configUrl = explicitResourceUrl orElse explicitFileUrl orElse
      resourceUrl
