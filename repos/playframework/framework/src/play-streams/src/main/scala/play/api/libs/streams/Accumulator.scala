...
/**
  ...
  * methods for working directly with that future as well as transforming the input.
  */
private class SinkAccumulator[-E, +A](wrappedSink: => Sink[E, Future[A]])
...
private class DoneAccumulator[+A](future: Future[A])
  ...
  def toSink: Sink[Any, Future[A]] =
    Sink.cancelled.mapMaterializedValue(_ => future)
...
object Accumulator {
  ...
  private[streams] def futureToSink[E, A](future: Future[Accumulator[E, A]])(
    ...
    Sink.asPublisher[E](fanout = false).mapMaterializedValue { publisher =>
      future.recover {
        ...
        Source.fromPublisher(publisher).toMat(accumulator.toSink)(Keep.right)
          .run()
      }
    }
  }
  ...
  def done[A](a: A): Accumulator[Any, A] =
    new DoneAccumulator[A](Future.successful(a))
