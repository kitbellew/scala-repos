...
object Probes {
  ...
  def subscriberProbe[T](
      ...
      start: Long = System.nanoTime()): Subscriber[T] = new Subscriber[T]
    with Probe {
    val probeName = name
    val startTime = start
  ...
  def subscriptionProbe(
      ...
      start: Long = System.nanoTime()): Subscription = new Subscription
    with Probe {
    val probeName = name
    val startTime = start
  ...
  def flowProbe[T](
    ...
    Flow[T].transform(() =>
      new PushPullStage[T, T] with Probe {
        ...
        override def onUpstreamFailure(cause: Throwable, ctx: Context[T]) = log(
          "onUpstreamFailure",
