...
object JsPathSpec extends Specification {
  ...
  "JsPath" should {
    "retrieve simple path" in {
      val obj = Json
        .obj("key1" -> Json.obj("key11" -> "value11"), "key2" -> "value2")
    ...
    "retrieve path with array index" in {
      ...
      (JsPath \ "key1" \ 0 \ "key11")(obj) must
        equalTo(Seq(JsString("value11")))
    }
    ...
    "retrieve 1-level recursive path" in {
      ...
      (JsPath \\ "tags")(obj) must
        equalTo(
          Seq(
            Json.arr("alpha1", "beta1", "gamma1"),
    ...
    "retrieve 2-level middle recursive path" in {
      ...
      (JsPath \\ "tags" \ "sub")(obj) must
        equalTo(Seq(JsString("alpha1"), JsString("beta2")))
    }

    "retrieve simple indexed path" in {
      val obj = Json.obj("level1" -> Json.arr(5, "alpha", true))
    ...
    "retrieve 2-level recursive indexed path" in {
      ...
      (JsPath \ "level1" \\ "tags")(1)(obj) must
        equalTo(Seq(JsString("beta1"), JsString("beta2")))
    ...
    "retrieve 2-level recursive indexed path" in {
      ...
      ((JsPath \ "level1" \ "key1")(1) \\ "tags")(obj) must
        equalTo(Seq(Json.arr("alpha1", "beta1", "gamma1")))
    ...
    "retrieve recursive in jsobject and jsarray" in {
      ...
      (JsPath \ "level1" \\ "alpha")(obj) must
        equalTo(Seq(JsString("value11111"), Json.arr("a", "b", "c")))
      ...
      val obj = Json.obj(
        ...
        "array" ->
          Json.arr(
            Json.obj(
              "field" ->
                Json.obj("alpha" -> "v11", "beta" -> "v12", "gamma" -> "v13")),
            Json.obj(
              "field" ->
                Json.obj("alpha" -> "v21", "gamma" -> "v23", "beta" -> "v22")),
            Json.obj(
              "field" ->
                Json.obj("beta" -> "v32", "alpha" -> "v31", "gamma" -> "v33"))
          )
      )

      (JsPath \ "array" \\ "beta")(obj) must
        equalTo(Seq(JsString("v12"), JsString("v22"), JsString("v32")))
    ...
    "get JsPath till last node" in {
      ...
      (__ \ 'level1 \ 'key2 \ 'key21).applyTillLast(res) must
        beEqualTo(
          Right(
            JsSuccess(
              Json.obj("tags" -> Json.arr("alpha2", "beta2", "gamma2")))))

      (__ \ 'level1 \ 'key2 \ 'key23).applyTillLast(res) must
        beEqualTo(
          Right(
            JsError(
              __ \ 'level1 \ 'key2 \ 'key23,
              ValidationError("error.path.missing"))))

      (__ \ 'level2 \ 'key3).applyTillLast(res) must
        beEqualTo(
          Left(
            JsError(
              __ \ 'level2 \ 'key3,
