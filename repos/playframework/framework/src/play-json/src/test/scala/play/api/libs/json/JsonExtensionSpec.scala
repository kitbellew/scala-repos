...
object JsonExtensionSpec extends Specification {
  ...
  "JsonExtension" should {
    ...
    "create a reads[RecUser]" in {
      ...
      Json.fromJson[RecUser](Json.obj(
        "name" -> "bob",
        "cat" -> Json.obj("name" -> "minou"),
        "hobbies" -> Json.arr("bobsleig", "manhunting"),
        "friends" -> Json.arr(Json.obj(
          "name" -> "tom",
          "hobbies" -> Json.arr(),
          "friends" -> Json.arr()))
      )) must beEqualTo(JsSuccess(RecUser(
        "bob",
        Some(Cat("minou")),
    ...
    "create a writes[RecUser]" in {
      ...
      Json.toJson(RecUser(
        ...
        Some(Cat("minou")),
        List("bobsleig", "manhunting"),
        List(RecUser("tom")))) must beEqualTo(Json.obj(
        "name" -> "bob",
        "cat" -> Json.obj("name" -> "minou"),
        "hobbies" -> Json.arr("bobsleig", "manhunting"),
        "friends" -> Json.arr(Json.obj(
    ...
    "VarArgsOnly reads, writes, format" should {
      ...
      "reads should be able to read valid Json and ignore invalid Json" in {
        ...
        Json.fromJson[VarArgsOnly](Json.obj("hello" -> "world"))(reads)
          .isError must beTrue
      }
    ...
    "LastVarArg reads, writes, format" should {
      ...
      "reads should be able to read valid Json and ignore invalid Json" in {
        ...
        Json.fromJson[LastVarArg](Json.obj("hello" -> "world"))(reads)
          .isError must beTrue
      }
    ...
    "test case class 1 field seq[Dog]" in {
      ...
      Json.fromJson[Toto6](js).get must beEqualTo(Toto6(
        Seq(Dog("medor", User(45, "toto")), Dog("brutus", User(23, "tata")))))
    }
    ...
    "test case single-field in companion object" in {
      Json.fromJson[Person2](Json.toJson(Person2(List("bob", "bobby"))))
