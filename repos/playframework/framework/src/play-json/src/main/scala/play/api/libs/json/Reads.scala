...
trait DefaultReads extends LowPriorityDefaultReads {
  ...
  def JsErrorObj(knownValue: JsValue, key: String, args: JsValue*) = {
    Json.obj(
      ...
      "__ERR__" -> key,
      "__ARGS__" ->
        args.foldLeft(JsArray())((acc: JsArray, arg: JsValue) => acc :+ arg))
  }
  ...
  private def parseJDate(
    ...
    catch { case x: java.text.ParseException => None }
  }
  ...
  object TemporalParser {
    ...
    implicit def OffsetDateTimePatternParser(
        pattern: String): TemporalParser[OffsetDateTime] =
      OffsetDateTimeFormatterParser(DateTimeFormatter.ofPattern(pattern))

    /** Instance of offset date/time based on formatter. */
    implicit def OffsetDateTimeFormatterParser(
          ...
          case JsString(str) =>
            enum.values.find(_.toString == str).map(JsSuccess(_))
              .getOrElse(JsError(Seq(
                JsPath() ->
                  Seq(ValidationError("error.expected.validenumvalue")))))
          case _ =>
            JsError(Seq(
