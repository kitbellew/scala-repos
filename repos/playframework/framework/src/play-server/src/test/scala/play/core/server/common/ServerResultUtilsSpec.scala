...
object ServerResultUtilsSpec extends Specification with IterateeSpecification {
  ...
  "ServerResultUtils.cleanFlashCookie" should {
    ...
    "send flash if new" in {
      flashCookieResult(None, Ok.flashing("a" -> "b")) must
    ...
    "leave other cookies untouched when clearing" in {
      flashCookieResult(
        ...
        Ok.withCookies(Cookie("cookie", "value"))) must
        beSome { cookies: Seq[Cookie] =>
          cookies.length must_== 2
          cookies.find(_.name == "PLAY_FLASH") must
            beSome.like {
              case cookie =>
                cookie.value must_== ""
            }
          cookies.find(_.name == "cookie") must
            beSome.like {
              case cookie =>
