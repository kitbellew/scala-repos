...
private[server] class ForwardedHeaderHandler(
  ...
  def remoteConnection(
    ...
    val headerEntries: Iterator[ForwardedEntry] = configuration
      .forwardedHeaders(headers).reverseIterator
  ...
  case class ForwardedHeaderHandlerConfig(
    ...
    private def unquote(s: String): String = {
      if (s.length >= 2 && s.charAt(0) == '"' &&
          s.charAt(s.length - 1) == '"') { s.substring(1, s.length - 1) }
      else s
    }
