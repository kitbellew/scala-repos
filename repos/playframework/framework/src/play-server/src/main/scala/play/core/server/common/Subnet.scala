...
private[common] case class Subnet(ip: InetAddress, cidr: Option[Int] = None) {
  ...
  private def maskBits(leadingBits: Int) =
    leadingBits match {
  ...
  def isInRange(otherIp: InetAddress) = {
    ...
    ip.getClass == otherIp.getClass && ip.getAddress.take(mask / 8)
      .toList.equals(otherIp.getAddress.take(mask / 8).toList) &&
    (for {
