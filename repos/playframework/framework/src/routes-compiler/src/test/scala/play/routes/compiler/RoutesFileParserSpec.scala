...
object RoutesFileParserSpec extends Specification {
  ...
  "route file parser" should {
    ...
    "parse a path with dynamic parts and it should be encodeable" in {
      parseRoute("GET /s/:d/s p.c.m(d)").path must_== PathPattern(Seq(
        StaticPart("s/"),
        DynamicPart("d", "[^/]+", true),
        StaticPart("/s")))
    }
    ...
    "parse method with arguments" in {
      parseRoute("GET /s p.c.m(s1, s2)").call.parameters must_== Some(Seq(
    ...
    "parse argument type" in {
      parseRoute("GET /s p.c.m(i: Int)").call.parameters.get.head
        .typeName must_== "Int"
    }
