...
trait Application {
  ...
  def global: GlobalSettings.Deprecated =
    injector.instanceOf[GlobalSettings.Deprecated]
  ...
  def getExistingFile(relativePath: String): Option[File] =
    Some(getFile(relativePath)).filter(_.exists)
...
trait BuiltInComponents {
  ...
  lazy val injector: Injector = new SimpleInjector(NewInstanceInjector) +
    router + cookieSigner + csrfTokenSigner + httpConfiguration +
    tempFileCreator + global + crypto
  ...
  lazy val csrfTokenSigner: CSRFTokenSigner =
    new CSRFTokenSignerProvider(cookieSigner).get
  ...
  lazy val global: GlobalSettings.Deprecated = play
    .api
