...
object Writeable extends DefaultWriteables {
  ...
  /**
    ...
    */
  def apply[A](transform: A => ByteString)(implicit
      ct: ContentTypeOf[A]): Writeable[A] =
...
trait DefaultWriteables extends LowPriorityWriteables {
  ...
  implicit def writeableOf_NodeBuffer(implicit
      codec: Codec): Writeable[scala.xml.NodeBuffer] = {
    Writeable(xml => codec.encode(xml.toString))
  }
  ...
  implicit def writeableOf_urlEncodedForm(implicit
      codec: Codec): Writeable[Map[String, Seq[String]]] = {
    import java.net.URLEncoder
    Writeable(formData =>
      codec.encode(
        formData.map(item =>
          item._2.map(c =>
            item._1 + "=" + URLEncoder.encode(c, "UTF-8"))).flatten.mkString(
          "&")))
  }
