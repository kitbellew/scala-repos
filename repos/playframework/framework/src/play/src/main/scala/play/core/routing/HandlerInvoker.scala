          ...
          def call(call: => JWebSocket) =
            WebSocket.acceptOrResult[Message, Message] { request =>
              FutureConverters.toScala(
                call(new j.RequestHeaderImpl(request))).map { resultOrFlow =>
                if (resultOrFlow.left.isPresent) {
                  ...
                  Right(Flow[Message].map {
                    ...
                    case CloseMessage(code, reason) =>
                      new JMessage.Close(
                        OptionConverters.toJava(code).asInstanceOf[Optional[
                          Integer]],
                        reason)
                    ...
                    case close: JMessage.Close =>
                      CloseMessage(
                        OptionConverters.toScala(close.code).asInstanceOf[
                          Option[Int]],
                        close.reason)
                  })
                }
