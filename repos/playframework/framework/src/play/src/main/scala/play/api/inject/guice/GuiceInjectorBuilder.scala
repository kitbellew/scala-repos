...
abstract class GuiceBuilder[Self] protected (
  ...
  def createModule(): GuiceModule = {
    ...
    val bindingModules = GuiceableModule.guiced(
      environment,
      configuration,
      binderOptions)(enabledModules) :+ injectorModule
    val overrideModules =
...
sealed abstract class BinderOption(configureBinder: Binder => Unit)
    extends (Binder => Unit) {
...
object BinderOption {
  ...
  case object DisableCircularProxies
      extends BinderOption(_.disableCircularProxies)
  case object RequireAtInjectOnConstructors
      extends BinderOption(_.requireAtInjectOnConstructors)
  case object RequireExactBindingAnnotations
      extends BinderOption(_.requireExactBindingAnnotations)
  case object RequireExplicitBindings
      extends BinderOption(_.requireExplicitBindings)
}
