...
trait BodyParsers {
  ...
  object parse {
    ...
    private def config = Play.privateMaybeApplication
    ...
    /**
      ...
      */
    def raw(
        memoryThreshold: Int = DefaultMaxTextLength,
    ...
    def maxLength[A](maxLength: Long, parser: BodyParser[A])(implicit
        mat: Materializer): BodyParser[Either[MaxSizeExceeded, A]] = BodyParser(
      "maxLength=" + maxLength + ", wrapping=" + parser.toString) { request =>
      import play.api.libs.iteratee.Execution.Implicits.trampoline
...
object BodyParsers extends BodyParsers {
  ...
  private[play] class TakeUpTo(maxLength: Long)
    ...
    override def shape: FlowShape[ByteString, ByteString] =
      FlowShape.of(in, out)
  ...
  private[play] class MaxLengthLimitAttained
