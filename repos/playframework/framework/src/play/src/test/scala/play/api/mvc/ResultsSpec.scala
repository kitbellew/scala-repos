  ...
  "Result" should {
    ...
    "support headers manipulation" in {
      val Result(ResponseHeader(_, headers, _), _) =
        Ok("hello").as("text/html").withHeaders(
          "Set-Cookie" -> "yes",
          "X-YOP" -> "1",
          "X-Yop" -> "2")
    ...
    "provide convenience method for setting cookie header" in withApplication {
      def testWithCookies(
        ...
        result.header.headers.get("Set-Cookie").map(
          Cookies.decodeSetCookieHeader(_).to[Set]) must_== expected
      }
    ...
    "support clearing a language cookie using clearingLang" in withApplication {
      ...
      val cookie = Cookies.decodeSetCookieHeader(
        Ok.clearingLang.header.headers("Set-Cookie")).head
    ...
    "allow discarding a cookie by deprecated names method" in withApplication {
      Cookies.decodeSetCookieHeader(
        Ok.discardingCookies(DiscardingCookie("blah")).header.headers(
          "Set-Cookie")).head.name must_== "blah"
    }

    "allow discarding multiple cookies by deprecated names method" in withApplication {
      val cookies = Cookies.decodeSetCookieHeader(
        Ok.discardingCookies(
          DiscardingCookie("foo"),
          DiscardingCookie("bar")).header.headers("Set-Cookie")).map(_.name)
    ...
    "support redirects for reverse routed calls" in {
      Results.Redirect(Call("GET", "/path")).header must_== Status(
        303).withHeaders(LOCATION -> "/path").header
    }

    "support redirects for reverse routed calls with custom statuses" in {
      Results.Redirect(
        Call("GET", "/path"),
        TEMPORARY_REDIRECT).header must_== Status(
        TEMPORARY_REDIRECT).withHeaders(LOCATION -> "/path").header
    }
  }
}
