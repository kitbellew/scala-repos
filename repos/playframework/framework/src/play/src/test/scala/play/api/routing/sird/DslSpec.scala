...
object DslSpec extends Specification {
  ...
  "Play routing DSL" should {
    ...
    "support extracting query strings" in {
      FakeRequest("GET", "/foo/bar?a=1&b=2&c=3&c=4") must
        beLike {
    ...
    "extract ints from query strings" in {
      "match" in {
        FakeRequest("GET", "/foo?a=1") must
          beLike {
            case GET(p"/foo" ? q"a=${int(a)}") =>
              a must_== 1
          }
      }
      "no match" in {
        FakeRequest("GET", "/foo?a=a") must
          beLike {
            case GET(p"/foo" ? q"a=${int(a)}") =>
              ko
            case _ =>
    ...
    "extract many ints from query strings" in {
      "match" in {
        FakeRequest("GET", "/foo?a=1&a=2") must
          beLike {
            case GET(p"/foo" ? q_s"a=${int(a)}") =>
              a must_== Seq(1, 2)
          }
      }
      "no match" in {
        FakeRequest("GET", "/foo?a=a&a=2") must
          beLike {
            case GET(p"/foo" ? q_s"a=${int(a)}") =>
              ko
            case _ =>
