...
object UrlContextSpec extends Specification {
  ...
  "path interpolation" should {
    ...
    "match a request" in {
      FakeRequest("GET", "/foo/testing/bar") must
        beLike {
          case p"/foo/$id/bar" =>
            id must_== "testing"
        }
    ...
    "match a uri" in {
      URI.create("/foo/testing/bar") must
        beLike {
          case p"/foo/$id/bar" =>
            id must_== "testing"
        }
    ...
    "match a url" in {
      new URL("http://example.com/foo/testing/bar") must
        beLike {
          case p"/foo/$id/bar" =>
            id must_== "testing"
        }
  ...
  "query string interpolation" should {
    ...
    "allow seq parameter extraction" in {
      ...
      "many" in {
        qs("foo" -> "bar1", "foo" -> "bar2", "foo" -> "bar3") must
          beLike {
            case q_s"foo=$foos" =>
              foos must_== Seq("bar1", "bar2", "bar3")
          }
      }
    }
