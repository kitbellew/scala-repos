  ...
  "An F.Promise" should {
    ...
    "be able to be created after a delay (with default ExecutionContext)" in {
      F.Promise.delayed(
        ...
        1,
        MILLISECONDS).get(5, SECONDS) must equalTo(1)
    }
    ...
    "combine a sequence of promises from a vararg" in {
      mustExecute(3) { ec =>
        ...
        F.Promise.sequence[Int](ec, pure(1), pure(2), pure(3)).get(
          5,
          SECONDS) must equalTo(Arrays.asList(1, 2, 3))
      }
    }
    ...
    "combine a sequence of promises from an iterable" in {
      mustExecute(3) { ec =>
        import F.Promise.pure
        F.Promise.sequence[Int](
          Arrays.asList(pure(1), pure(2), pure(3)),
          ec).get(5, SECONDS) must equalTo(Arrays.asList(1, 2, 3))
      }
    }
