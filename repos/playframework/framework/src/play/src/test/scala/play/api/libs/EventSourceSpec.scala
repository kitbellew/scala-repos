...
object EventSourceSpec extends Specification {
  ...
  "EventSource event formatter" should {
    ...
    "format an event with an id" in {
      Event("foo", Some("42"), None)
        .formatted must equalTo("id: 42\ndata: foo\n\n")
    }

    "format an event with a name" in {
      Event("foo", None, Some("message"))
        .formatted must equalTo("event: message\ndata: foo\n\n")
    }
  ...
  "EventSource.Event" should {
    "be writeable as a response body using an enumerator" in {
      val result = Results
        .Ok
        .chunked(Enumerator("foo", "bar", "baz") &> EventSource())
