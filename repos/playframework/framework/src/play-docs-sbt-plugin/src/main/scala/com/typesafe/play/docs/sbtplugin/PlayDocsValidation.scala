...
object PlayDocsValidation {
      ...
      case Some(jarFile) =>
        ...
        val parsedFiles =
          jar
            ...
            .collect {
              case entry
                  if entry.getName.endsWith(".md") && entry
                    .getName
    ...
    val (codeSampleIssues, okFiles) = matchingFiles
      .map {
        case (actualFile, upstreamFile) =>
          val missingCodeSamples = upstreamFile
            .codeSamples
            .filterNot(hasCodeSample(actualFile.codeSamples))
          val introducedCodeSamples = actualFile
            .codeSamples
            .filterNot(hasCodeSample(actualFile.codeSamples))
  ...
  val validateDocsTask = Def.task {
    ...
    def relativeLinkOk(link: LinkRef) = {
      link match {
        case badScalaApi
          ...
          false
        case scalaApi if scalaApi.link.startsWith("api/scala/") =>
          fileExists(scalaApi.link.split('#').head)
        case javaApi if javaApi.link.startsWith("api/java/") =>
    ...
    val (existing, nonExisting) = report
      .codeSamples
  ...
  val validateExternalLinksTask = Def.task {
    ...
    val grouped = report
      ...
      .filterNot { e =>
        e._1.startsWith("http://localhost:") || e
          ._1
          .contains("example.com") || e._1.startsWith("http://127.0.0.1")
      }
      .toSeq
      .sortBy {
        _._1
