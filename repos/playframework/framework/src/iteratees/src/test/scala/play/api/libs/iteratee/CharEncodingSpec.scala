...
object CharEncodingSpec extends Specification {
  ...
  "CharEncodingSpec.decode()" should {
    ...
    "decode US-ASCII" in {
      ...
      val result = Enumerator(input: _*) &> CharEncoding.decode("US-ASCII") |>>>
        Iteratee.consume[String]()
      Await.result(result, Duration.Inf) must be equalTo
        new String(input.flatten.toArray, "US-ASCII")
    }
    ...
    "decode UTF-8" in {
      val input = Seq(
        ...
        Array[Byte](0x6c, 0x6f, 0x20, 0x57),
        Array[Byte](0x6f, 0x72, 0x6c, 0x64))
      val result = Enumerator(input: _*) &> CharEncoding.decode("UTF-8") |>>>
        Iteratee.consume[String]()
      Await.result(result, Duration.Inf) must be equalTo
        new String(input.flatten.toArray, "UTF-8")
    }
    ...
    "decode UTF-16" in {
      ...
      val result = Enumerator(input: _*) &> CharEncoding.decode("UTF-16") |>>>
        Iteratee.consume[String]()
    ...
    "decode UTF-32" in {
      val input = Seq(
        Array[Byte](
          0x00, 0x00, 0x00, 0x48, 0x00, 0x00, 0x00, 0x65, 0x00, 0x00, 0x00,
          0x6c),
        Array[Byte](
          0x00, 0x00, 0x00, 0x6c, 0x00, 0x00, 0x00, 0x6f, 0x00, 0x00, 0x00,
          0x20, 0x00, 0x00, 0x00, 0x57),
        Array[Byte](
          0x00, 0x00, 0x00, 0x6f, 0x00, 0x00, 0x00, 0x72, 0x00, 0x00, 0x00,
          0x6c, 0x00, 0x00, 0x00, 0x64)
      )
      val result = Enumerator(input: _*) &> CharEncoding.decode("UTF-32") |>>>
        Iteratee.consume[String]()
      Await.result(result, Duration.Inf) must be equalTo "Hello World"
    }
    ...
    "fail on invalid ASCII" in {
      val input = Seq(Array[Byte](0x80.toByte))
      val result = Enumerator(input: _*) &> CharEncoding.decode("US-ASCII") |>>>
        Iteratee.skipToEof
      val status = result
        .map { _ =>
    ...
    "fail on invalid UTF-8" in {
      val input = Seq(Array[Byte](0xe2.toByte, 0xe2.toByte, 0xe2.toByte))
      val result = Enumerator(input: _*) &> CharEncoding.decode("UTF-8") |>>>
        Iteratee.skipToEof
      val status = result
        .map { _ =>
  ...
  "CharEncodingSpec.encode()" should {
    ...
    "encode US-ASCII" in {
      val input = Seq("Hel", "lo W", "orld")
      val result = Enumerator(input: _*) &> CharEncoding.encode("US-ASCII") |>>>
        Iteratee.consume[Array[Byte]]()
      Await.result(result, Duration.Inf) must be equalTo
        input.mkString.getBytes("US-ASCII")
    }

    "encode UTF-8" in {
      val input = Seq("Hel", "lo W", "orld")
      val result = Enumerator(input: _*) &> CharEncoding.encode("UTF-8") |>>>
        Iteratee.consume[Array[Byte]]()
      Await.result(result, Duration.Inf) must be equalTo
        input.mkString.getBytes("UTF-8")
    }
    ...
    "encode UTF-16" in {
      val input = Seq("Hel", "lo W", "orld")
      val result = Enumerator(input: _*) &> CharEncoding.encode("UTF-16") |>>>
        Iteratee.consume[Array[Byte]]()
      Await.result(result, Duration.Inf) must be equalTo
        input.mkString.getBytes("UTF-16BE")
    }
    ...
    "encode UTF-32" in {
      val input = Seq("Hel", "lo W", "orld")
      val result = Enumerator(input: _*) &> CharEncoding.encode("UTF-32") |>>>
        Iteratee.consume[Array[Byte]]()
      Await.result(result, Duration.Inf) must be equalTo
        input.mkString.getBytes("UTF-32")
    }
