      ...
      def scan(
        ...
        if (piece.length < needleSize) {
          ...
          if (fullMatch) {
            ...
            val newResults = previousMatches ++ List(
              Unmatched(prefix),
            ...
            Iteratee.flatten(
              inner.fold1(
                ...
                k => {
                  ...
                  val fed = result.filter(!_.content.isEmpty).foldLeft(
                    Future.successful(Array[Byte]() -> Cont(k))) { (p, m) =>
                    p.flatMap(i =>
                      i._2.fold1(
                        (a, e) =>
                          Future.successful((i._1 ++ m.content, Done(a, e))),
