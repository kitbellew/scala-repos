...
trait Enumerator[E] {
  ...
  def andThen(e: Enumerator[E]): Enumerator[E] = new Enumerator[E] {
    def apply[A](i: Iteratee[E, A]): Future[Iteratee[E, A]] = parent
      .apply(i)
      .flatMap(e.apply)(
        dec
  ...
  def onDoneEnumerating(callback: => Unit)(implicit
      ec: ExecutionContext): Enumerator[E] = new Enumerator[E] {
    private val pec = ec.prepare()

    def apply[A](it: Iteratee[E, A]): Future[Iteratee[E, A]] = parent
      .apply(it)
...
object Enumerator {
  ...
  def interleave[E1, E2 >: E1](
      e1: Enumerator[E1],
      e2: Enumerator[E2]): Enumerator[E2] = new Enumerator[E2] {
    ...
    def apply[A](it: Iteratee[E2, A]): Future[Iteratee[E2, A]] = {
      ...
      def iteratee[EE <: E2](
          f: ((Boolean, Boolean)) => (Boolean, Boolean)): Iteratee[EE, Unit] = {
        def step(in: Input[EE]): Iteratee[EE, Unit] = {
  ...
  def repeatM[E](e: => Future[E])(implicit
      ec: ExecutionContext): Enumerator[E] = checkContinue0(new TreatCont0[E] {
    private val pec = ec.prepare()
  ...
  def generateM[E](e: => Future[Option[E]])(implicit
    ...
    def apply[A](
        loop: Iteratee[E, A] => Future[Iteratee[E, A]],
        k: Input[E] => Iteratee[E, A]) = executeFuture(e)(pec).flatMap {
