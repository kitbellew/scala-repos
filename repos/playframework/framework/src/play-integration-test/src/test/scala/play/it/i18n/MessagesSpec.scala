...
object MessagesSpec extends PlaySpecification with Controller {
  ...
  "Messages" should {
    "provide default messages" in
      new WithApplication() {
        val msg = Messages("constraint.email")

        msg must ===("Email")
      }
    "permit default override" in
      new WithApplication() {
  ...
  "Messages@Java" should {
    ...
    "allow translation without parameters" in
      new WithApplication() {
        val msg = Messages.get(enUS, "constraint.email")

        msg must ===("Email")
      }
    "allow translation with any non-list parameter" in
      new WithApplication() {
        val msg = Messages.get(enUS, "constraint.min", "Croissant")

        msg must ===("Minimum value: Croissant")
      }
    "allow translation with any list parameter" in
      new WithApplication() {
        val msg = {
