...
object AnyContentBodyParserSpec extends PlaySpecification {
  ...
  "The anyContent body parser" should {
    ...
    "parse text bodies for DELETE requests" in
      new WithApplication() {
        parse("DELETE", Some("text/plain"), ByteString("bar")) must
          beRight(AnyContentAsText("bar"))
      }

    "parse text bodies for GET requests" in
      new WithApplication() {
        parse("GET", Some("text/plain"), ByteString("bar")) must
          beRight(AnyContentAsText("bar"))
      }

    "parse empty bodies as raw for GET requests" in
      new WithApplication() {
    ...
    "parse text bodies for HEAD requests" in
      new WithApplication() {
        parse("HEAD", Some("text/plain"), ByteString("bar")) must
          beRight(AnyContentAsText("bar"))
      }

    "parse text bodies for OPTIONS requests" in
      new WithApplication() {
        parse("OPTIONS", Some("text/plain"), ByteString("bar")) must
          beRight(AnyContentAsText("bar"))
      }
