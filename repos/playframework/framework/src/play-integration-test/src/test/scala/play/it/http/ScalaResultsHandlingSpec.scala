...
trait ScalaResultsHandlingSpec
  ...
  "scala body handling" should {
    ...
    "add Date header" in
      makeRequest(Results.Ok("Hello world")) { response =>
        response.header(DATE) must beSome
      }

    "add Content-Length for strict results" in
      makeRequest(Results.Ok("Hello world")) { response =>
        response.header(CONTENT_LENGTH) must beSome("11")
        response.body must_== "Hello world"
      }

    "not add a content length header when none is supplied" in
      makeRequest(
        Results
          .Ok
          .sendEntity(
    ...
    "chunk results for chunked streaming strategy" in
      makeRequest(Results.Ok.chunked(Source(List("a", "b", "c")))) { response =>
        response.header(TRANSFER_ENCODING) must beSome("chunked")
        response.header(CONTENT_LENGTH) must beNone
        response.body must_== "abc"
      }

    "chunk results for event source strategy" in
      makeRequest(
        Results
          .Ok
    ...
    "close the connection when no content length is sent" in
      withServer(
        Results
          .Ok
          .sendEntity(
            HttpEntity
              .Streamed(Source.single(ByteString("abc")), None, None))) {
        port =>
          val response =
    ...
    "close the HTTP 1.1 connection when requested" in
      withServer(Results.Ok.withHeaders(CONNECTION -> "close")) { port =>
        val response =
          BasicHttpClient.makeRequests(port, checkClosed = true)(
            BasicRequest("GET", "/", "HTTP/1.1", Map(), ""))(0)
        response.status must_== 200
    ...
    "close the HTTP 1.0 connection when requested" in
      withServer(Results.Ok.withHeaders(CONNECTION -> "close")) { port =>
        val response =
          BasicHttpClient.makeRequests(port, checkClosed = true)(
            BasicRequest(
              ...
              ""))(0)
        response.status must_== 200
        response.headers.get(CONNECTION).map(_.toLowerCase(ENGLISH)) must
          beOneOf(None, Some("close"))
      }

    "close the connection when the connection close header is present" in
      withServer(Results.Ok) { port =>
        BasicHttpClient
    ...
    "close the connection when the connection when protocol is HTTP 1.0" in
      withServer(Results.Ok) { port =>
        BasicHttpClient
          .makeRequests(port, checkClosed = true)(
    ...
    "honour the keep alive header for HTTP 1.0" in
      withServer(Results.Ok) { port =>
        val responses =
        ...
        responses(0).headers.get(CONNECTION) must
          beSome.like {
            case s =>
    ...
    "keep alive HTTP 1.1 connections" in
      withServer(Results.Ok) { port =>
        val responses =
          BasicHttpClient.makeRequests(port)(
    ...
    "close chunked connections when requested" in
      withServer(Results.Ok.chunked(Source(List("a", "b", "c")))) { port =>
    ...
    "keep chunked connections alive by default" in
      withServer(Results.Ok.chunked(Source(List("a", "b", "c")))) { port =>
        val responses =
          BasicHttpClient.makeRequests(port)(
            BasicRequest("GET", "/", "HTTP/1.1", Map(), ""),
    ...
    "return a 500 error on response with null header" in
      withServer(Results.Ok("some body").withHeaders("X-Null" -> null)) {
        port =>
          val response =
            BasicHttpClient
              .makeRequests(port)(
    ...
    "return a 400 error on Header value contains a prohibited character" in
      withServer(Results.Ok) { port =>
    ...
    "split Set-Cookie headers" in {
      ...
      makeRequest {
        ...
        response.allHeaders.get(SET_COOKIE) must
          beSome.like {
    ...
    "not have a message body even when a 204 response with a non-empty body is returned" in
      withServer(
        Result(
    ...
    "not have a message body even when a 304 response with a non-empty body is returned" in
      withServer(
        Result(
    ...
    "not have a message body, nor Content-Length, when a 204 response is returned" in
      withServer(Results.NoContent) { port =>
        val response =
          BasicHttpClient
            .makeRequests(port)(BasicRequest("PUT", "/", "HTTP/1.1", Map(), ""))
            .head
    ...
    "not have a message body, but may have a Content-Length, when a 204 response with an explicit Content-Length is returned" in
      withServer(Results.NoContent.withHeaders("Content-Length" -> "0")) {
        port =>
          val response =
            BasicHttpClient
              .makeRequests(port)(
              ...
              .head
          response.body must beLeft("")
          response.headers.get(CONTENT_LENGTH) must
            beOneOf(None, Some("0")) // Both header values are valid
      }

    "not have a message body, nor a Content-Length, when a 304 response is returned" in
      withServer(Results.NotModified) { port =>
        val response =
          BasicHttpClient
    ...
    "not have a message body, but may have a Content-Length, when a 304 response with an explicit Content-Length is returned" in
      withServer(Results.NotModified.withHeaders("Content-Length" -> "0")) {
        port =>
          val response =
            BasicHttpClient
              .makeRequests(port)(
    ...
    "return a 500 response if a forbidden character is used in a response's header field" in
      withServer(
        ...
        (response.headers -- Set(CONNECTION, CONTENT_LENGTH, DATE, SERVER)) must
          be(Map.empty)
      }
  }
