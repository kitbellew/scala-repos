...
trait AssetsSpec
  ...
  "Assets controller" should {
    ...
    "serve an asset" in
      withServer { client =>
        ...
        result.header(CONTENT_TYPE) must
          beSome.which(_.startsWith("text/plain"))
    ...
    "serve an asset in a subdirectory" in
      withServer { client =>
    ...
    "serve an asset with spaces in the name" in
      withServer { client =>
    ...
    "return not modified when etag matches" in
      withServer { client =>
    ...
    "return asset when etag doesn't match" in
      withServer { client =>
        val result = await(
          client
            .url("/foo.txt")
    ...
    "return not modified when not modified since" in
      withServer { client =>
    ...
    "return asset when modified since" in
      withServer { client =>
        val result = await(
          client
            .url("/foo.txt")
    ...
    "return 200 if the asset is empty" in
      withServer { client =>
    ...
    "return 404 for files that don't exist" in
      withServer { client =>
    ...
    "return not found when the path is a directory" in {
      "if the directory is on the file system" in
        withServer { client =>
