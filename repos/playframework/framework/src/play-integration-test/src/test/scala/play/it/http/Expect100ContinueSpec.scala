...
trait Expect100ContinueSpec
  ...
  "Play" should {
    ...
    "honour 100 continue" in
      withServer(Action(req => Results.Ok)) { port =>
        val responses =
    ...
    "not read body when expecting 100 continue but action iteratee is done" in
      withServer(EssentialAction(_ => Accumulator.done(Results.Ok))) { port =>
        val responses =
          BasicHttpClient.makeRequests(port)(
            BasicRequest(
              "POST",
    ...
    "close the connection after rejecting a Expect: 100-continue body" in
      withServer(EssentialAction(_ => Accumulator.done(Results.Ok))) { port =>
        val responses =
    ...
    "leave the Netty pipeline in the right state after accepting a 100 continue request" in
      withServer(Action(req => Results.Ok)) { port =>
        val responses =
          BasicHttpClient.makeRequests(port)(
            BasicRequest(
