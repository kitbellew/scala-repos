...
object BodyParserSpec
  ...
  "BodyParser.mapM" should {
    ...
    "satisfy lifted functor law 1" in
      prop { (x: Int) =>
    ...
    "satisfy lifted functor law 2" in
      prop { (x: Int) =>
  ...
  "BodyParser.validate" should {
    ...
    "satisfy right-biased functor law 1" in
      prop { (x: Int) =>
    ...
    "satisfy right-biased functor law 2" in
      prop { (x: Int) =>
    ...
    "pass through simple result (case 1)" in
      prop { (s: Result) =>
        mustExecute(1) { implicit ec => // one execution from `validate`
          run {
    ...
    "pass through simple result (case 2)" in
      prop { (s1: Result, s2: Result) =>
        mustExecute(1) { implicit ec => // one execution from `validate`
          run {
  ...
  "BodyParser.validateM" should {
    ...
    "satisfy right-biased, lifted functor law 1" in
      prop { (x: Int) =>
    ...
    "satisfy right-biased, lifted functor law 2" in
      prop { (x: Int) =>
