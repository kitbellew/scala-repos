...
object JsonBodyParserSpec extends PlaySpecification {
  ...
  "The JSON body parser" should {
    ...
    def parse[A](
        ...
        encoding: String,
        bodyParser: BodyParser[A] = BodyParsers.parse.tolerantJson)(implicit
        mat: Materializer) = {
      await(
        bodyParser(
          FakeRequest()
    ...
    "automatically detect the charset" in
      new WithApplication() {
    ...
    "ignore the supplied charset" in
      new WithApplication() {
        parse(
          """{"foo":"bär"}""",
    ...
    "accept all common json content types" in
      new WithApplication() {
        parse(
          """{"foo":"bar"}""",
          Some("application/json"),
          "utf-8",
          BodyParsers.parse.json) must
          beRight.like {
            case json =>
              (json \ "foo").as[String] must_== "bar"
          }
    ...
    "reject non json content types" in
      new WithApplication() {
        parse(
          """{"foo":"bar"}""",
    ...
    "gracefully handle invalid json" in
      new WithApplication() {
        parse(
          """{"foo:}""",
          Some("application/json"),
    ...
    "validate json content using .validate" in
      new WithApplication() {
        import scala.concurrent.ExecutionContext.Implicits.global

        val fooParser = BodyParsers
          .parse
          .json
          .validate {
            _.validate[Foo]
    ...
    "validate json content using implicit reads" in
      new WithApplication() {
        parse(
          """{"a":1,"b":"bar"}""",
          Some("application/json"),
          "utf-8",
          BodyParsers.parse.json[Foo]) must
          beRight.like {
            case foo =>
              foo must_== Foo(1, "bar")
          }
