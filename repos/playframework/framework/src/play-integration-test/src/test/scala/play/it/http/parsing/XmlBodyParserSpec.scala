  ...
  "The XML body parser" should {
    ...
    "parse XML bodies" in new WithApplication() {
      parse(
        ...
        "utf-8") must beRight.like { case xml => xml.text must_== "bar" }
    }
    ...
    "honour the external charset for application sub types" in
      new WithApplication() {
        parse(
          ...
          "iso-8859-1") must beRight.like { case xml => xml.text must_== "bär" }
        parse(
          "<foo>bär</foo>",
          Some("application/xml; charset=utf-16"),
          "utf-16") must beRight.like { case xml => xml.text must_== "bär" }
      }
    ...
    "honour the external charset for text sub types" in new WithApplication() {
      ...
      parse("<foo>bär</foo>", Some("text/xml; charset=utf-16"), "utf-16") must
        beRight.like { case xml => xml.text must_== "bär" }
    }
