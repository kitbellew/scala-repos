...
object XmlBodyParserSpec extends PlaySpecification {
  ...
  "The XML body parser" should {
    ...
    def parse(
        ...
        encoding: String,
        bodyParser: BodyParser[NodeSeq] = BodyParsers
          .parse
    ...
    "honour the external charset for application sub types" in
      new WithApplication() {
        parse(
          "<foo>bär</foo>",
    ...
    "honour the external charset for text sub types" in
      new WithApplication() {
        parse(
          "<foo>bär</foo>",
        ...
        parse("<foo>bär</foo>", Some("text/xml; charset=utf-16"), "utf-16") must
          beRight.like {
            case xml =>
              xml.text must_== "bär"
          }
    ...
    "default to iso-8859-1 for text sub types" in
      new WithApplication() {
        parse("<foo>bär</foo>", Some("text/xml"), "iso-8859-1") must
          beRight.like {
            case xml =>
              xml.text must_== "bär"
          }
    ...
    "default to reading the encoding from the prolog for application sub types" in
      new WithApplication() {
        parse(
    ...
    "default to reading the encoding from the prolog for no content type" in
      new WithApplication() {
        parse(
          """<?xml version="1.0" encoding="utf-16"?><foo>bär</foo>""",
          None,
    ...
    "accept all common xml content types" in
      new WithApplication() {
        parse(
          "<foo>bar</foo>",
          Some("application/xml; charset=utf-8"),
          "utf-8",
          BodyParsers.parse.xml) must
          beRight.like {
            case xml =>
              xml.text must_== "bar"
          }
    ...
    "reject non XML content types" in
      new WithApplication() {
        parse(
          "<foo>bar</foo>",
        ...
        parse(
          ...
          "utf-8",
          BodyParsers.parse.xml) must beLeft
        parse("<foo>bar</foo>", None, "utf-8", BodyParsers.parse.xml) must
          beLeft
      }

    "gracefully handle invalid xml" in
      new WithApplication() {
        parse(
          "<foo",
          Some("text/xml; charset=utf-8"),
    ...
    "parse XML bodies without loading in a related schema" in
      new WithApplication() {
        ...
        parse(xml, Some("text/xml; charset=iso-8859-1"), "iso-8859-1") must
          beLeft
      }
