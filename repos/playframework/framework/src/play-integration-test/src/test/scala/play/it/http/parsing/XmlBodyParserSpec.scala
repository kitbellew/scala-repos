  ...
  "The XML body parser" should {
    ...
    "parse XML bodies" in new WithApplication() {
      parse(
        ...
        "utf-8") must beRight.like { case xml =>
        xml.text must_== "bar"
      }
    }
    ...
    "honour the external charset for application sub types" in new WithApplication() {
      parse(
        ...
        "iso-8859-1") must beRight.like { case xml =>
        xml.text must_== "bär"
      }
      parse(
        "<foo>bär</foo>",
        Some("application/xml; charset=utf-16"),
        "utf-16") must beRight.like { case xml =>
        xml.text must_== "bär"
      }
    }
    ...
    "accept all common xml content types" in new WithApplication() {
      parse(
        ...
        Some("application/xml; charset=utf-8"),
        "utf-8",
        BodyParsers.parse.xml) must beRight.like { case xml =>
        xml.text must_== "bar"
      }
      parse(
        "<foo>bar</foo>",
