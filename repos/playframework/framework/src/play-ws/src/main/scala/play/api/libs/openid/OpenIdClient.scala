...
object UserInfo {
  ...
  private[openid] class UserInfoExtractor(params: Map[String, Seq[String]]) {
    ...
    def id =
      params.get("openid.claimed_id").flatMap(_.headOption)
...
object OpenID {
  ...
  def redirectURL(
    ...
    app.injector.instanceOf[OpenIdClient]
...
private[openid] object Discovery {
  ...
  class XrdsResolver extends Resolver {
    ...
    def resolve(response: WSResponse) =
      for {
        _ <- response.header(HeaderNames.CONTENT_TYPE)
    ...
    private def findUriWithType(xml: Node)(typeId: String) =
      (xml \ "XRD" \ "Service" find
        (node => (node \ "Type").find(inner => inner.text == typeId).isDefined))
        .map { node => (typeId, (node \ "URI").text.trim) }
  }
  ...
  class HtmlResolver extends Resolver {
    ...
    def resolve(response: WSResponse) = {
      val serverUrl: Option[String] = providerRegex.findFirstIn(response.body)
        .orElse(serverRegex.findFirstIn(response.body)).flatMap(extractHref(_))
      serverUrl.map(url => {
    ...
    private def extractHref(link: String): Option[String] =
      new Regex("""href="([^"]*)"""").findFirstMatchIn(link)
        .map(_.group(1).trim)
