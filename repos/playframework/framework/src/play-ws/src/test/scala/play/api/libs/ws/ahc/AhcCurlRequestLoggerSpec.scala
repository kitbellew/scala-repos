...
class AhcCurlRequestLoggerSpec
  ...
  def is(implicit ee: ExecutionEnv) = {
    ...
    "AhcCurlRequestLogger" should {
      ...
      "log a request with custom headers" in
        new WithServer {
          val client = wsUrl("/")
      ...
      "log a request with POST with a hanging quote" in
        new WithServer() {
          val client = wsUrl("/")
          val logger = mock[Logger]
          val requestLogger = AhcCurlRequestLogger(logger)
