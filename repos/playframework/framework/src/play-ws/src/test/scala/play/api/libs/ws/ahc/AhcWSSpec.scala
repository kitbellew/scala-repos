...
object AhcWSSpec extends PlaySpecification with Mockito {
  ...
  "Ahc WS" should {
    ...
    "support direct client instantiation" in
      new WithApplication {
        ...
        try WS.clientUrl("http://example.com/feed") must
          beAnInstanceOf[WSRequest]
        finally sslClient.close()
      }

    "AhcWSClient.underlying" in
      new WithApplication {
        val client = WS.client
    ...
    "support gziped encoding" in
      new WithServer(gzipFakeApp) {
