...
object DebugBuilderSpec extends Specification {
  ...
  "JavaSecurityDebugBuilder" should {
    ...
    "match certpath" in {
      ...
      actual.trim.split("\\s+").toSeq must
        containTheSameElementsAs(Seq("certpath"))
    }
    ...
    "match certpath + ocsp" in {
      ...
      actual.trim.split("\\s+").toSeq must
        containTheSameElementsAs(Seq("certpath", "ocsp"))
    }
  }
  ...
  "JavaxNetDebugBuilder" should {
    ...
    "match some random combinations" in {
      ...
      actual.trim.split("\\s+").toSeq must
        containTheSameElementsAs(Seq("ssl", "defaultctx", "handshake", "data"))
    }
