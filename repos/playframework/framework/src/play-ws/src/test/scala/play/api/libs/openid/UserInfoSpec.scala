...
object UserInfoSpec extends Specification {
  ...
  "UserInfo" should {
    "not include attributes that are not signed" in {
      val requestParams = createDefaultResponseWithAttributeExchange ++
      ...
      val requestParams = createDefaultResponseWithAttributeExchange ++
        Map[String, Seq[String]](
          ...
          "openid.ext1.value.email" ->
            "user@example.com", // the email attribute *is* in the list of signed fields
          "openid.signed" ->
            (defaultSigned +
