...
object FilterArgsSpec extends Specification {
  ...
  "Reloader.filterArgs" should {
    ...
    "support port argument" in { check("1234")(httpPort = Some(1234)) }

    "support disabled port argument" in { check("disabled")(httpPort = None) }
    ...
    "support port property with dev setting" in {
      val devSettings: Seq[(String, String)] = Seq(
        "play.server.http.port" -> "1234")
      val result = Reloader
        .filterArgs(Seq.empty, defaultHttpPort, defaultHttpAddress, devSettings)
    ...
    "support address property with dev setting" in {
      val devSettings: Seq[(String, String)] = Seq(
        "play.server.http.address" -> "not-default-address")
      val result = Reloader
        .filterArgs(Seq.empty, defaultHttpPort, defaultHttpAddress, devSettings)
