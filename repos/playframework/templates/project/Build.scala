      ...
      mappings.foreach {
        case (original, file) =>
          ...
          val templateParams: Map[String, String] = {
            ...
            val templateSources: TemplateSources =
              templateSourcesList.find(_.name == templateName).getOrElse(
      ...
      try {
        ...
        def waitUntilNotPending(
          ...
          val status: Future[TemplateStatus] = for {
            _ <- timeout(2.seconds)
            resp <- clientCall(statusUrl).withHeaders(
              "Accept" -> "application/json,text/html;q=0.9").get()
          } yield {
          ...
          uploaded.map {
            case (name, key) =>
              clientCall("/activator/template/publish")
                .post(s"url=http://downloads.typesafe.com/$key").flatMap {
                  resp =>
                    if (resp.status != 200) {
                    ...
                    val statusUrl = (for {
                      ...
                      status <- (links \ "activator/templates/status").asOpt[
                        JsObject]
