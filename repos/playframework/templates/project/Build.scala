...
object Templates {
  ...
  val templateSettings: Seq[Setting[_]] = s3Settings ++
    Seq(
      templates := Nil,
      ...
      prepareTemplates := {
        ...
        val mappings: Seq[(File, File)] = templateSourcesList.flatMap {
          case templateSources: TemplateSources =>
            val relativeMappings: Seq[(File, String)] = templateSources
              .sourceDirs
                ...
                resp.header("Content-Type") match {
                  ...
                  case _ =>
                    ...
                    body match {
                      case pending
                          if body
                            .contains("This template is being processed.") =>
                        TemplatePending(uuid)
                      case validated
                          if body.contains(
                      ...
                      case failed
                          if body
                            .contains("This template failed to publish.") =>
      ...
      publishTemplates <<=
        (doPublishTemplates, S3.delete, streams).apply { (dpt, s3delete, s) =>
          for {
            streams <- s
            result <- dpt
            _ <- {
  ...
  val templatesCommand =
    Command(
      ...
      (state, args) =>
        ...
        val setBack =
          templates in extracted.currentRef get extracted.structure.data map
            createSetCommand toList
