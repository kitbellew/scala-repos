...
object ScalaTestingWithDatabases extends Specification {
  ...
  "database helpers" should {
    ...
    "allow running static evolutions" in play.api.db.Databases.withInMemory() {
      database =>
        ...
        connection.prepareStatement(
          "insert into test values (10, 'testing')").execute()
        ...
        connection.prepareStatement(
          "select * from test").executeQuery() must throwAn[SQLException]
    }

    "allow running evolutions from a custom path" in play.api.db.Databases.withInMemory() {
      database =>
    ...
    "allow play to manage evolutions for you" in play.api.db.Databases.withInMemory() {
      database =>
        //#with-evolutions
        import play.api.db.evolutions._
