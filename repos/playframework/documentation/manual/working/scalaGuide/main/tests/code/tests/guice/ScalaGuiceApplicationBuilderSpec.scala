  ...
  "Scala GuiceApplicationBuilder" should {
    ...
    "set environment" in {
      ...
      val application = new GuiceApplicationBuilder()
        ...
        .in(Environment(new File("path/to/app"), classLoader, Mode.Test)).build
      // #set-environment
    ...
    "override configuration" in {
      ...
      val application = new GuiceApplicationBuilder()
        .loadConfig(env => Configuration.load(env)).build
      // #override-configuration
    ...
    "add bindings" in {
      // #add-bindings
      val injector = new GuiceApplicationBuilder().bindings(new ComponentModule)
        .bindings(bind[Component].to[DefaultComponent]).injector
      // #add-bindings
    ...
    "load modules" in {
      // #load-modules
      val injector = new GuiceApplicationBuilder().load(
        new play.api.inject.BuiltinModule,
        bind[Component].to[DefaultComponent]).injector
      // #load-modules
    ...
    "injector builder" in {
      // #injector-builder
      val injector = new GuiceInjectorBuilder().configure("key" -> "value")
        .bindings(new ComponentModule)
        .overrides(bind[Component].to[MockComponent]).injector
