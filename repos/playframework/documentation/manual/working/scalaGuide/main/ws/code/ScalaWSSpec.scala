...
class ScalaWSSpec extends PlaySpecification with Results with AfterAll {
  ...
  "WS" should {
    ...
    "allow making an authenticated request" in
      withSimpleServer { ws =>
        val user = "user"
    ...
    "allow setting a query string" in
      withSimpleServer { ws =>
        val response =
          //#query-string
    ...
    "allow setting the content type" in
      withSimpleServer { ws =>
    ...
    "allow setting the virtual host" in
      withSimpleServer { ws =>
        val response =
          //#virtual-host
    ...
    "allow setting the request timeout" in
      withSimpleServer { ws =>
        val response =
          //#request-timeout
    ...
    "when posting data" should {
      ...
      "post with form url encoded body" in
        withServer {
          case ("POST", "/") =>
            Action(BodyParsers.parse.urlFormEncoded)(r =>
              Ok(r.body("key").head))
        } { ws =>
          val response =
            //#url-encoded
      ...
      "post with multipart/form encoded body" in
        withServer {
          case ("POST", "/") =>
    ...
    "when processing a response" should {
      ...
      "handle as JSON with an implicit" in
        withServer {
          case ("GET", "/") =>
            Action {
              import play.api.libs.json._
        ...
        withServer {
          ...
          await(
            downloadFile(FakeRequest()).flatMap(
              _.body.dataStream.runFold(0L)((t, b) => t + b.length))) must_==
            10000L
          file.delete()
        }

      "stream when request is a PUT" in
        withServer {
          case ("PUT", "/") =>
            Action(Ok.chunked(largeSource))
        } { ws =>
    ...
    "allow working with clients directly" in
      withSimpleServer { ws =>
        //#implicit-client
      ...
      new WithApplication() {
        ...
        val logging =
          new AsyncHttpClientConfig.AdditionalChannelInitializer() {
            override def initChannel(
                channel: io.netty.channel.Channel): Unit = {
              channel
                .pipeline
                .addFirst(
                  "log",
    ...
    "grant access to the underlying client" in
      withSimpleServer { ws =>
        //#underlying
