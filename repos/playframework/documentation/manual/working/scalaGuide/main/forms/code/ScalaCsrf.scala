...
object ScalaCsrf extends PlaySpecification {
  ...
  "Play's CSRF protection" should {
    ...
    "allow getting the token" in
      new WithApplication() {
        ...
        contentAsString(result) must be like {
          case t =>
            Crypto.compareSignedTokens(originalToken, t) must_== true
        }
      }
    ...
    "allow rendering a token in a query string" in
      new WithApplication() {
        val originalToken = Crypto.generateSignedToken
        val result =
          tokenFormAction(app)(
        ...
        body must
          find("action=\"/items\\?csrfToken=[a-f0-9]+-\\d+-([a-f0-9]+)\"")
            .withGroup(Crypto.extractSignedToken(originalToken).get)
      }

    "allow rendering a token in a hidden field" in
      new WithApplication() {
        val originalToken = Crypto.generateSignedToken
        val result =
          tokenFormAction(app)(
        ...
        val body = contentAsString(result)
        body must
          find("value=\"[a-f0-9]+-\\d+-([a-f0-9]+)\"")
            .withGroup(Crypto.extractSignedToken(originalToken).get)
      }

    "allow per action checking" in
      new WithApplication() {
    ...
    "allow per action token handling" in
      new WithApplication() {
        import play.api.mvc.Results.Ok
    ...
    "be easy to use with an action builder" in
      new WithApplication() {
        import play.api.mvc.Results.Ok
