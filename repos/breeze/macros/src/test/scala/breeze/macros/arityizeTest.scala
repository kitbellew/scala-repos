...
class arityizeTest extends FunSuite {
  ...
  test("Compiles?") {
    @arityize(5)
    class Foo[@arityize.replicate T](
        @arityize.replicate x: T @arityize.relative(x)) {
      override def toString =
        List((x: @arityize.replicate)).mkString("(", ", ", ")")
    }
  ...
  test("Compiles defs?") {
    @arityize(5)
    def foo[@arityize.replicate T](
        @arityize.replicate x: T @arityize.relative(x)) =
      (x: @arityize.relative(foo))
