    ...
    annottees.head.tree match {
      case tree @ ClassDef(mods, name, targs, impl) =>
        ...
        val results = for (order <- 1 to maxOrder) yield {
          ...
          val newTargs = targs
            .flatMap(arg => expandTypeDef(c, order, bindings)(arg))
          ClassDef(
            mods,
      ...
      case tree @ DefDef(mods, name, targs, vargs, tpt, impl) =>
        ...
        val results = for (order <- 1 to maxOrder) yield {
          ...
          val newVargs = vargs
            .map(_.flatMap(arg => expandValDef(c, order, bindings)(arg)))
          val newTargs = targs
            .flatMap(arg => expandTypeDef(c, order, bindings)(arg))
...
//        println(name + " "+  ret + " " + newVargs)
      ...
      case x @ ClassDef(mods, name, targs, impl) =>
        val newParents = impl.parents
          .flatMap(tree => expandArity(c, order, bindings)(tree))
        val newTemplate = Template(
          newParents,
          impl.self,
          impl.body.flatMap(x => expandArity(c, order, bindings)(x)))
      ...
      case Annotated(ann, tree) => ann match {
          case q"new arityize.relative($sym)" => tree match {
              ...
              case AppliedTypeTree(Ident(nme), targs) =>
                ...
                val newTargs = targs
                  .flatMap(arg => expandArity(c, order, bindings)(arg))
                Seq(AppliedTypeTree(newName, newTargs))
              case _ =>
//                println(tree + " " + tree.getClass); ???
                ???
            }
          case q"new arityize.replicate()" => tree match {
              case Ident(nme) if nme.isTypeName =>
          ...
          case q"new arityize.repeat()" => tree match {
              case Ident(nme) if nme.isTypeName => List.fill(order) { tree }
              case Ident(nme) if nme.isTermName => List.fill(order) { tree }
              case _                            => ???
            }
          case _ =>
//            println("???" + ann + " " + tree)
      ...
      case New(tree) => Seq(New(expandArity(c, order, bindings)(tree).head))
      case _         =>
//        println("???" + tree + " " + tree.getClass)
        Seq(tree)
    }
