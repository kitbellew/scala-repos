...
case class CategoricalPaintScaleFactory[T]() extends PaintScaleFactory[T] {
  override def apply(items: Traversable[T]): PaintScale[T] = {
    ...
    CategoricalPaintScale[T](
      Map() ++ (distinct zip Stream.continually(
        PaintScale.Category20.values.toList).flatten))
  }
}
...
object PaintScaleFactory {
  ...
  /**
    ...
    */
  implicit def singletonFactoryForPaintScale[S, T](paintScale: S)(implicit
      view: S => PaintScale[T]): PaintScaleFactory[T] =
