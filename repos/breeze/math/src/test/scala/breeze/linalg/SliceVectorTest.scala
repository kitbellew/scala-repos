...
class SliceVectorTest extends FunSuite {
  ...
  test("negative indexing tests") {
    ...
    assert(
      tempDV(0 to 3) == DenseVector(0, 1, 2, 3),
      "Failed> tempDV( 0 to 3 ) = " + tempDV(0 to 3))
    assert(
      tempDV(4 to -1) == DenseVector(4, 5, 6),
      "Failed> tempDV( 4 to -1 ) =  " + tempDV(4 to -1))
    assert(
      tempDV(-3 to -1) == DenseVector(4, 5, 6),
      "Failed> tempDV( -3 to -1 ) =  " + tempDV(-3 to -1))
    ...
    assert(
      tempDV(4 to 0 by -2) == DenseVector(4, 2, 0),
