...
class padTest extends FunSuite {
  ...
  test("padRight") {
    ...
    assert(
      padRight(testDVD, dimensions = 7, mode = 11d) == DenseVector(
        1d, 2d, 3d, 4d, 5d, 11d, 11d))
    assert(
      padRight(testDVD, 7, 10d) == DenseVector(1d, 2d, 3d, 4d, 5d, 10d, 10d))
    assert(
      padRight(testDVD, 7, Value(10d)) == DenseVector(
    ...
    assert(
      padLeft(testDVD, dimensions = 7, mode = 11d) == DenseVector(
        11d, 11d, 1d, 2d, 3d, 4d, 5d))
    assert(
      padLeft(testDVD, 7, 10d) == DenseVector(10d, 10d, 1d, 2d, 3d, 4d, 5d))
    assert(
      padLeft(testDVD, 7, Value(10d)) == DenseVector(
