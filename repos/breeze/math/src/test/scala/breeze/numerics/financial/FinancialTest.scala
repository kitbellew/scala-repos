...
class FinancialTest extends FunSuite {
  ...
  test("FutureValue") {
    assert(
      math.abs(futureValue(0.05 / 12, 10 * 12, -100, -100) - 15692.92889) <
        1e-5)
  ...
  test("presentValue") {
    assert(
      math.abs(
        presentValue(0.05 / 12, 10 * 12, -100, 15692.93) - -100.0006713) < 1e-5)
    assert(presentValue(0, 3, 1, 1) == -4.0)
  }
  test("payment") {
    assert(
      math.abs(payment(0.075 / 12, 12 * 15, 200000, 0) - -1854.0247200054619) <
        1e-5)
    assert(math.abs(payment(0.0, 10, 1100, -100) - -100.0) < 1e-5)
  }

  test("principal vs interest payments") {
    val (principal, interest, remainingPrincipal) = principalInterest(
      0.0824 / 12,
      12,
      2500.0)

    val expectedRemainingPrincipal = DenseVector[Double](
      2299.42, 2097.46, 1894.11, 1689.37, 1483.22, 1275.66, 1066.67, 856.25,
      644.38, 431.05, 216.26, -0.00)
    val expectedInterestPayment = DenseVector[Double](
      -17.17, -15.79, -14.40, -13.01, -11.60, -10.18, -8.76, -7.32, -5.88,
      -4.42, -2.96, -1.49)
    val expectedPrincipalPayment = DenseVector[Double](
      -200.58, -201.96, -203.35, -204.74, -206.15, -207.56, -208.99, -210.42,
      -211.87, -213.32, -214.79, -216.26)
  ...
  test("IRR vs MIRR") {
    ...
    assert(
      math.abs(interalRateReturn(expectNormalIRR).get - 0.2809484211599611) <
        DOUBLE_ROUND5_MIN)
    //Some(0.08859833852775578)
    expectNormalIRR = DenseVector[Double](-5, 10.5, 1, -8, 1);
    assert(
      math.abs(interalRateReturn(expectNormalIRR).get - 0.08859833852775578) <
        DOUBLE_ROUND5_MIN)
    //Some(-0.09549583034897258)
    val expectNegativeIRR = DenseVector[Double](-100, 0, 0, 74)
    assert(
      math.abs(interalRateReturn(expectNegativeIRR).get + 0.09549583034897258) <
        DOUBLE_ROUND5_MIN)
