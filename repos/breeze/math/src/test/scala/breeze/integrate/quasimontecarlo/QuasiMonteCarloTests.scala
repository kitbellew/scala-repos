  ...
  "quasiMonteCarloIntegrate" should {
    "approximate the mean of a normal distribution" in {
      val integrationResult =
        quasiMonteCarloIntegrate(identityFunc _)(Gaussian(3, 5).toQuasi)(
          32 * 1024)
      math.abs(integrationResult - 3) should be < 0.001
    }
    "approximate the mean of a gamma distribution, alpha == 1" in {
      val integrationResult =
        quasiMonteCarloIntegrate(identityFunc _)(
          RejectionSampledGammaQuasiRandomVariable(1, 4))(32 * 1024)
    ...
    "approximate the mean of a gamma distribution, alpha > 1" in {
      ...
      val integrationResult =
        quasiMonteCarloIntegrate(identityFunc _)(
          RejectionSampledGammaQuasiRandomVariable(alpha, beta))(128 * 1024)
    ...
    "approximate the mean of a product distribution" in {
      ...
      val sigma = 5.0
      val integrationResult =
        quasiMonteCarloIntegrate(productFunc _)(
          Gaussian(mu, sigma).toQuasi,
          RejectionSampledGammaQuasiRandomVariable(alpha, beta))(
          16 * 1024 * 1024)
      println(
        "Integration result: " + integrationResult + " should be " + alpha * beta * mu)
