...
class FilterTest extends FunSuite {
  ...
  test("BP filtering tested against output from scipy.signal.firwin/ifilter (0.13.2-1)") {
    ...
    val testSignal = DenseVector(-1.27168894, -1.48442932, -0.5481519,
      ...
      0.00401513, -0.52352089, 0.07456588, -0.68864501, 1.24677189, 0.48492816,
      -1.10078968, 0.41359423, 0.13732803, -0.05686372, -0.17045011)
    val spTestSignalFiltered1 = DenseVector(-0.01789505, -0.02088871,
      0.14061118, 0.38912801, 0.17161298, -0.54231618, -0.93826582, -0.69101276,
    ...
    assert(
      norm(
        filtered1(0 to -6) - spTestSignalFiltered1(5 to -1)) < testNormThreshold,
