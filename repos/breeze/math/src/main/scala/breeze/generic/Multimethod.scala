...
trait Multiproc2[Method[AA, BB] <: (AA, BB) => Unit, A <: AnyRef, B]
  ...
  def register[AA <: A, BB <: B](
    ...
    super.register(
      manA.runtimeClass.asInstanceOf[Class[AA]],
...
trait MMRegistry2[R] {
  ...
  def register(a: Class[_], b: Class[_], op: R) {
    ...
    if (a.isPrimitive) {
      ...
      if (b.isPrimitive) {
        ops(
          ReflectionUtil.boxedFromPrimitive(
            a) -> ReflectionUtil.boxedFromPrimitive(b)) = op
      }
    }
    cache.clear()
  }
...
trait MMRegistry3[R] {
  ...
  protected def selectBestOption(
    ...
    for (pair @ (aa, bb, cc) <- options.keys) {
      // if there is no option (aaa,bbb) s.t. aaa <: aa && bbb <: bb, then add it to the list
      if (!bestCandidates.exists(pair =>
            aa.isAssignableFrom(pair._1) && bb.isAssignableFrom(
              pair._2)) && cc.isAssignableFrom(pair._3)) {
        bestCandidates = bestCandidates.filterNot(pair =>
