...
class VectorBuilder[@spec(Double, Int, Float, Long) E](
  ...
  def this(length: Int, initialNonZero: Int = 0)(implicit
      ring: Semiring[E],
      man: ClassTag[E],
  ...
  override def toString = {
    (index.iterator zip data.iterator).take(used)
...
object VectorBuilder extends VectorBuilderOps {
  ...
  def tabulate[@spec(Double, Int, Float, Long) V: ClassTag: Semiring: Zero](
      size: Int)(f: Int => V): VectorBuilder[V] = apply(Array.tabulate(size)(f))
  ...
  implicit def canCopyBuilder[
      @spec(Double, Int, Float, Long) V: ClassTag: Semiring: Zero]
      : CanCopyBuilder[V] = { new CanCopyBuilder[V] }
  implicit def canZerosBuilder[
      @spec(Double, Int, Float, Long) V: ClassTag: Semiring: Zero]
      : CanZerosBuilder[V] = { new CanZerosBuilder[V] }
