...
trait BinaryUpdateRegistry[A <: AnyRef, B, Op <: OpType]
  ...
  def apply(a: A, b: B) {
    ...
    if (cached != null) {
      cached match {
        ...
        case Some(m) => m.asInstanceOf[InPlaceImpl2[Op, A, B]].apply(a, b)
      }
    } else {
  ...
  def register[AA <: A, BB <: B](op: InPlaceImpl2[Op, AA, BB])(implicit
      cA: ClassTag[AA],
      cB: ClassTag[BB]) { super.register(cA.runtimeClass, cB.runtimeClass, op) }
}
