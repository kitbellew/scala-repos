...
/**
  ...
  */
case class BroadcastedRows[T, RowType](underlying: T)
...
object BroadcastedRows {
  ...
  implicit class BroadcastRowsDMToIndexedSeq[T](
      bc: BroadcastedRows[DenseMatrix[T], DenseVector[T]]) {
  ...
  implicit def canForeachRows[T, RowType, ResultRow, Result](implicit
    ...
    new CanForeachValues[BroadcastedRows[T, RowType], RowType] {
      ...
      override def foreach[U](
          from: BroadcastedRows[T, RowType],
          fn: (RowType) => U): Unit = { iter(from.underlying, Axis._1)(fn) }
    }
