...
trait QuasiTensor[@spec(Int) K, @spec(Double, Int, Float, Long) V] {
  ...
  def findAll(f: V => Boolean) =
    activeIterator.filter(p => f(p._2)).map(_._1).toIndexedSeq
...
trait TensorLike[
  ...
  def apply[Slice, Result](slice: Slice)(implicit
      canSlice: CanSlice[This, Slice, Result]) = {
    canSlice(repr, slice)
  }
...
/**
  ...
  * @author dlwh
  */
trait Tensor[@spec(Int) K, @spec(Double, Int, Float, Long) V]
    extends TensorLike[K, V, Tensor[K, V]]
