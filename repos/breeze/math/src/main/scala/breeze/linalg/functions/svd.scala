...
object svd extends UFunc {
  ...
  implicit object Svd_DM_Impl extends Impl[DenseMatrix[Double], DenseSVD] {
    def apply(mat: DenseMatrix[Double]): DenseSVD =
      doSVD_Double(mat)(mode = CompleteSVD)
  }

  implicit object Svd_DM_Impl_Float
      extends Impl[DenseMatrix[Float], SDenseSVD] {
    def apply(mat: DenseMatrix[Float]): SDenseSVD =
      doSVD_Float(mat)(mode = CompleteSVD)
  }
  ...
  object reduced extends UFunc {
    implicit object reduced_Svd_DM_Impl
        extends Impl[DenseMatrix[Double], DenseSVD] {
      def apply(mat: DenseMatrix[Double]): DenseSVD =
        doSVD_Double(mat)(mode = ReducedSVD)
    }

    implicit object reduced_Svd_DM_Impl_Float
        extends Impl[DenseMatrix[Float], SDenseSVD] {
      def apply(mat: DenseMatrix[Float]): SDenseSVD =
        doSVD_Float(mat)(mode = ReducedSVD)
    }
  }
  ...
  type OpMulMatrixDenseVector[Mat] = OpMulMatrix.Impl2[Mat, DenseVector[
