...
trait CSCMatrixOps extends CSCMatrixOps_Ring { this: CSCMatrix.type =>
  ...
  implicit def csc_csc_OpAdd[@expand.args(Int, Double, Float, Long) T](implicit
    ...
    new OpAdd.Impl2[CSCMatrix[T], CSCMatrix[T], CSCMatrix[T]] {
      ...
      implicitly[
        BinaryRegistry[Matrix[T], Matrix[T], OpAdd.type, Matrix[T]]].register(
        this)
    }
  }
...
trait CSCMatrixOpsLowPrio extends SerializableLogging {
  ...
  implicit def canMulM_V_def[T, A, B <: Vector[T]](implicit
    ...
    implicitly[
      OpMulMatrix.Impl2[CSCMatrix[T], Vector[T], Vector[T]]].asInstanceOf[
      breeze.linalg.operators.OpMulMatrix.Impl2[A, B, Vector[T]]]
