...
trait DenseVectorOps extends DenseVector_GenericOps {
  ...
  implicit def dv_v_Op[
    ...
    new BinaryRegistry[DenseVector[T], Vector[T], Op.type, DenseVector[T]] {
      ...
      override protected def bindingMissing(
          a: DenseVector[T],
  ...
  implicit def canZipValues_DV_DV[@expand.args(Int, Double, Float, Long) T]()
    ...
    val res =
      new zipValues.Impl2[DenseVector[T], DenseVector[T], ZippedValues[T, T]] {
  ...
  implicit def dvAddSField[T](implicit
    ...
    binaryOpFromUpdateOp(
      implicitly[CanCopy[DenseVector[T]]],
      dvAddIntoSField,
      ct)
  }
  ...
  implicit def dvSubIntoSField[T](implicit
      field: Ring[T],
  ...
  implicit def dvDivIntoSField[T](implicit
      field: Field[T],
...
trait DenseVector_SpecialOps extends DenseVectorOps {
  ...
  implicit val canAddF: OpAdd.Impl2[DenseVector[Float], DenseVector[
    Float], DenseVector[Float]] = { pureFromUpdate_Float(canAddIntoF) }
  implicitly[
  ...
  implicit val canSubF: OpSub.Impl2[DenseVector[Float], DenseVector[
    Float], DenseVector[Float]] = { pureFromUpdate_Float(canSubIntoF) }
      ...
      def apply(a: DenseVector[T], b: DenseVector[T]): BitVector = {
        if (a.length != b.length)
          throw new ArrayIndexOutOfBoundsException(
            s"Lengths don't match for operator $Op ${a.length} ${b.length}")
        val ad = a.data
        val bd = b.data
        var aoff = a.offset
        var boff = b.offset
...
trait DenseVector_GenericOps {
  ...
  implicit def pureFromUpdate[
      @expand.args(Int, Double, Float, Long) T,
      Other,
      Op <: OpType](op: UFunc.InPlaceImpl2[Op, DenseVector[T], Other])(implicit
  ...
  implicit def implOpSet_DV_V_InPlace[V]
      : OpSet.InPlaceImpl2[DenseVector[V], V] =
  ...
  implicit def implOpSet_DV_DV_InPlace[V]
      : OpSet.InPlaceImpl2[DenseVector[V], DenseVector[V]] =
  ...
  implicit def implOpSet_DV_Vector_InPlace[T, Vec](implicit
      ev: Vec <:< Vector[T]): OpSet.InPlaceImpl2[DenseVector[T], Vec] =
  ...
  implicit def liftDMOpToDVTransposeOp[Tag, V, LHS, R](implicit
    ...
    new UFunc.UImpl2[Tag, LHS, Transpose[DenseVector[V]], R] {
      def apply(v: LHS, v2: Transpose[DenseVector[V]]): R = {
        ...
        val dm: DenseMatrix[V] = DenseMatrix.create(
          data = dv.data,
          offset = dv.offset,
          cols = dv.length,
          rows = 1,
          majorStride = dv.stride)
        op(v, dm)
      }
    }
