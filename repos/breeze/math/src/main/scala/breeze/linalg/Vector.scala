...
trait Vector[@spec(Int, Double, Float) V] extends VectorLike[V, Vector[V]] {
  ...
  def padTo(len: Int, elem: V)(implicit cm: ClassTag[V]): Vector[V] =
    Vector[V](toArray.padTo(len, elem))
  ...
  /** See [[scala.collection.mutable.ArrayOps.foldLeft]].
    */
  def foldLeft[B >: V](z: B)(op: (B, V) => B): B =
    valuesIterator.foldLeft(z)(op)

  /** See [[scala.collection.mutable.ArrayOps.foldRight]].
    */
  def foldRight[B >: V](z: B)(op: (V, B) => B): B =
    valuesIterator.foldRight(z)(op)
...
//  }
  ...
  case class ZippedVectorValues[
      ...
      @spec(Double, Int, Float, Long) U](a: Vector[T], b: Vector[U])
  ...
  implicit def dotField[T](implicit
      field: Semiring[T]): OpMulInner.Impl2[Vector[T], Vector[T], T] = {
...
trait VectorConstructors[Vec[T] <: Vector[T]] {
  ...
  def apply[V: ClassTag](values: V*): Vec[V] = {
    ...
    if (man == manifest[Double])
      apply(values.toArray.asInstanceOf[Array[Double]]).asInstanceOf[Vec[V]]
...
//     apply(values.toArray)
  ...
  def range(start: Int, end: Int, step: Int): Vec[Int] =
    apply[Int](Array.range(start, end, step))
