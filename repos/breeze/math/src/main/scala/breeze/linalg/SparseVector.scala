...
class SparseVector[@spec(Double, Int, Float, Long) V](
  ...
  def this(index: Array[Int], data: Array[V], activeSize: Int, length: Int)(
  ...
  def this(index: Array[Int], data: Array[V], length: Int)(implicit
      value: Zero[V]) = this(index, data, index.length, length)
...
object SparseVector
  ...
  def horzcat[V: Zero: ClassTag](vectors: SparseVector[V]*): CSCMatrix[V] = {
    if (!vectors.forall(_.size == vectors(0).size))
      throw new IllegalArgumentException(
        "vector lengths must be equal, but got: " + vectors.map(
          _.length).mkString(", "))
  ...
  implicit def canCopySparse[@spec(
    ...
    Int,
    Float,
    Long) V: ClassTag: Zero] = new CanCopySparseVector[V]
