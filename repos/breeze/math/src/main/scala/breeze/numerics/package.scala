  ...
  object nextExponent extends UFunc with MappingUFunc {
    ...
    implicit object nextExponentIntImpl2 extends Impl2[Int, Int, Double] {
      def apply(base: Int, v: Int) =
        m.ceil(m.log(v.toDouble) / m.log(base.toDouble))
    }
    implicit object nextExponentDoubleImpl2
...
//          !java.lang.Double.isFinite(v)
  ...
  object lgamma extends UFunc with MappingUFunc {
    ...
    implicit object lgammaImplIntInt extends Impl2[Int, Int, Double] {
      def apply(a: Int, x: Int): Double =
        lgammaImplDoubleDouble(a.toDouble, x.toDouble)
    }
  ...
  object gammp extends UFunc with MappingUFunc {
    implicit object gammpImplIntInt extends Impl2[Int, Int, Double] {
      def apply(v1: Int, v2: Int): Double =
        G.regularizedGammaP(v1.toDouble, v2.toDouble)
    }
    implicit object gammpImplDoubleDouble
        extends Impl2[Double, Double, Double] {
  ...
  object gammq extends UFunc with MappingUFunc {
    ...
    implicit object gammqImplDoubleDouble
        extends Impl2[Double, Double, Double] {
      def apply(v1: Double, v2: Double): Double = G.regularizedGammaP(v1, v2)
    }
  }
  ...
  def closeTo(a: Double, b: Double, relDiff: Double = 1e-4) = {
    a == b ||
    (scala.math.abs(a - b) <
      scala.math.max(scala.math.max(scala.math.abs(a), scala.math.abs(b)), 1) *
      relDiff)
  }
