...
class FeatureVector(val data: Array[Int]) extends NumericOps[FeatureVector] {
  ...
  override def equals(p1: Any): Boolean =
    p1 match {
      case fv: FeatureVector =>
        util.Arrays.equals(fv.data, data)
      case _ =>
        false
    }
}
...
object FeatureVector {
  ...
  object Implicits {
    implicit def fromArrayInt(arr: Array[Int]): FeatureVector =
      new FeatureVector(arr)
  }

  implicit object TransposeFV
      extends CanTranspose[FeatureVector, Transpose[FeatureVector]] {
