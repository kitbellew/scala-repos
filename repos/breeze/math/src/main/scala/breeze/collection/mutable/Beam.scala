...
class Beam[T](val maxSize: Int, xs: T*)(implicit o: Ordering[T])
  ...
  private def trim() { while (queue.size > maxSize) { queue.dequeue() } }
...
object Beam {
  ...
  def apply[T](maxSize: Int, xs: T*)(implicit
      ordering: Ordering[T],
      dummyImplicit: DummyImplicit): Beam[T] = new Beam[T](maxSize, xs: _*)
