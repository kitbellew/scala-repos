...
trait MatrixInnerProduct[M, S] extends MatrixNorms[M, S] {
  ...
  implicit def canInnerProductNorm_Ring(implicit ring: Ring[S]) =
    new norm.Impl[M, Double] {
      override def apply(v: M): Double = sqrt(
        implicitly[Ring[S]].sNorm(canInnerProduct(v, v)))
    }
}
      ...
      new norm.Impl2[M, Int, Double] {
        def apply(v: M, n: Int): Double = {
          ...
          class NormVisitor extends ValuesVisitor[Int] {
            ...
            val (op, opEnd) =
              if (n == 1)
                ((v: Int) => agg += v.abs.toDouble, identity[Double] _)
              else if (n == 2)
                (
                  (v: Int) => {
      ...
      new norm.Impl2[M, Float, Double] {
        def apply(v: M, n: Float): Double = {
          ...
          class NormVisitor extends ValuesVisitor[Float] {
            ...
            val (op, opEnd) =
              if (n == 1)
                ((v: Float) => agg += v.abs.toDouble, identity[Double] _)
              else if (n == 2)
                (
                  (v: Float) => {
                    val nn = v.abs.toDouble
