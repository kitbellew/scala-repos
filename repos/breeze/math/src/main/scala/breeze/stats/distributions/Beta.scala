...
class Beta(a: Double, b: Double)(implicit rand: RandBasis = Rand)
  ...
  def mode = (a - 1) / (a + b - 2)
  def entropy =
    logNormalizer - (a - 1) * digamma(a) - (b - 1) * digamma(
      b) + (a + b - 2) * digamma(a + b)
...
object Beta
    extends ExponentialFamily[Beta, Double]
    with ContinuousDistributionUFuncProvider[Double, Beta] {
  ...
  def sufficientStatisticFor(t: Double) =
    SufficientStatistic(1, math.log(t), math.log1p(-t))
    ...
    new DiffFunction[(Double, Double)] {
      ...
      def calculate(x: (Double, Double)) = {
        ...
        if (a < 0 || b < 0)
          ...
          val obj = n * (lgamma(a) + lgamma(b) - lgamma(
            a + b) - (a - 1) * stats.meanLog - (b - 1) * stats.meanLog1M)
