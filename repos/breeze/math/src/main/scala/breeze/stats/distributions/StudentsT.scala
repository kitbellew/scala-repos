...
case class StudentsT(degreesOfFreedom: Double)(implicit
  ...
  override def probability(x: Double, y: Double): Double = {
    val t =
      new org.apache.commons.math3.distribution.TDistribution(degreesOfFreedom)
    t.probability(x, y)
  }
  ...
  def unnormalizedLogPdf(x: Double): Double =
    -(degreesOfFreedom + 1) / 2 * math.log(1 + (x * x) / degreesOfFreedom)

  lazy val logNormalizer: Double = 0.5 * math
    .log(math.Pi * degreesOfFreedom) + lgamma(degreesOfFreedom / 2) - lgamma(
  ...
  def entropy: Double =
    ((
      degreesOfFreedom + 1
    ) / 2 * (digamma((degreesOfFreedom + 1) / 2) - digamma(degreesOfFreedom))
