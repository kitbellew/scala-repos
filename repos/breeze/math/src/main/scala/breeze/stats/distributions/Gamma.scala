...
case class Gamma(shape: Double, scale: Double)(implicit rand: RandBasis = Rand)
  ...
  override def pdf(x: Double) =
    if (x > 0) { math.exp(logPdf(x)) }
    else {
      if (shape > 1.0) { 0.0 }
      else if (shape == 1.0) { normalizer }
  ...
  def draw() = {
    if (shape == 1.0) { scale * -math.log(rand.uniform.draw()) }
    else if (shape < 1.0) {
...
object Gamma
    extends ExponentialFamily[Gamma, Double]
    with ContinuousDistributionUFuncProvider[Double, Gamma] {
