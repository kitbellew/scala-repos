...
abstract class BaseMetropolisHastings[T](
  ...
  private def getNext(): T = {
    ...
    if (acceptanceRatio > 1.0) { //This is logically unnecessary, but allows us to skip a call to nextDouble
      ...
      maybeNext
    } else if (nextDouble < acceptanceRatio) {
      last = maybeNext
      acceptances += 1
      maybeNext
    } else
...
case class ThreadedBufferedRand[T](wrapped: Rand[T], bufferSize: Int = 1024 * 8)(
  ...
  private val usedArrayQueue =
    new java.util.concurrent.LinkedBlockingQueue[Array[T]](2)
  private val newArrayQueue =
    new java.util.concurrent.LinkedBlockingQueue[Array[T]](2)
