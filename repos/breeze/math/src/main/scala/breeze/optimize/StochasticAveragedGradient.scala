...
class StochasticAveragedGradient[T](
  ...
  protected def determineStepSize(
      ...
      direction: T): Double = state.history.stepSize
    ...
    val newStepSize =
      if (tuneStepFrequency > 0 && (oldState.iter % tuneStepFrequency) == 0) {
        ...
        if ((
              f.valueAt(
                newX,
                IndexedSeq(nextPos)) + l2Regularization / 2 * norm(
                newX) - oldState.adjustedValue
            ) > (
              oldState.adjustedGradient dot xdiff
            ) + (xdiff dot xdiff) / (2 * stepSize)) {
          stepSize / 2
        } else {
          stepSize * 1.5
        }
