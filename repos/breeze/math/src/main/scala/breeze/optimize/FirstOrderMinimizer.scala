    ...
    Iterator.iterate(state) { state =>
      try {
        ...
        val oneOffImprovement = (state.adjustedValue - adjValue) /
          (state.adjustedValue.abs max adjValue.abs max
            1e-6 * state.initialAdjVal.abs)
        logger.info(
...
object FirstOrderMinimizer {
  ...
  case class FunctionValuesConverged[T](
    ...
    override def apply(
      ...
      if (info.length >= 2 && (state.adjustedValue - info.max).abs <=
            tolerance * (if (relative) state.initialAdjVal else 1.0)) {
