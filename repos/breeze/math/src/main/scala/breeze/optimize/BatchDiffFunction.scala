...
trait BatchDiffFunction[T]
  ...
  override def cached(implicit copy: CanCopy[T]) = {
    if (this.isInstanceOf[CachedBatchDiffFunction[_]]) { this }
    else { new CachedBatchDiffFunction[T](this) }
  }
  ...
  def groupItems(groupSize: Int): BatchDiffFunction[T] =
    new BatchDiffFunction[T] {
      ...
      val groups: Array[immutable.IndexedSeq[Int]] = Array
        .tabulate(numGroups)(i =>
