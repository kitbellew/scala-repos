...
class ProbMonadBenchmark extends BreezeBenchmark {
  ...
  def timeMap(reps: Int) =
    run(reps) {
      val mg = gaussian.map(f)
      mg.samplesVector(size)
    }

  def timeMapRepeated(reps: Int) =
    run(reps) {
  ...
  def timeFlatMapRepeated(reps: Int) =
    run(reps) {
      val mg = gaussian.flatMap(fm).flatMap(fm).flatMap(fm)
      mg.samplesVector(size)
    }

  def timeCondition(reps: Int) =
    run(reps) {
  ...
  def timeRepeatCondition(reps: Int) =
    run(reps) {
      val mg =
        gaussian.condition(x => x > 0).condition(x => x < 1).condition(x =>
          x > -1)
      mg.samplesVector(size)
    }

  def timeDrawOpt(reps: Int) =
    run(reps) {
      val mg = gaussian.condition(x => x > 0)
