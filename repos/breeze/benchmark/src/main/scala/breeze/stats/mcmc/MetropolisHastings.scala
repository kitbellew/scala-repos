...
object MetropolisHastingsRunner
    extends MyRunner(classOf[MetropolisHastingsBenchmark])
...
class MetropolisHastingsBenchmark extends BreezeBenchmark {
  ...
  def likelihood(x: Double) =
    2 * math.log1p(1 + epsilon - x) + 3 * math
      .log1p(
        x * x * x + epsilon
  ...
  def pullAllSamples(m: Rand[Double]) = {
    var result = 0.0
    cfor(0)(i => i < numSamples, i => i + 1)(i => { result = m.draw() })
    result
  }
  ...
  def timeMetropolisHastings(reps: Int) =
    run(reps) {
      val m = ArbitraryMetropolisHastings(
        likelihood _,
  ...
  def timeMetropolisHastingsWithWork(reps: Int) =
    run(reps) {
      val m = ArbitraryMetropolisHastings(
        likelihood _,
        (_: Double) => Uniform(0, 1),
  ...
  def timeThreadedBufferedWithWork(reps: Int) =
    run(reps) {
