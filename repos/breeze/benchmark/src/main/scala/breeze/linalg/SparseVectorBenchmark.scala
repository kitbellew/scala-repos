...
class SparseVectorBenchmark extends BreezeBenchmark with BuildsRandomVectors {
  def timeAllocate(reps: Int) = run(reps) { SparseVector.zeros[Double](1024) }
  ...
  def timeDotLargeUneven_10__0_1_%(reps: Int) =
    dotProductBench(reps, 1000000, 0.1, 0.001)
  def timeDotLargeUneven_10_1_%(reps: Int) =
    dotProductBench(reps, 1000000, 0.1, 0.01)
  def timeDotLargeUneven_10_30_%(reps: Int) =
    dotProductBench(reps, 1000000, 0.10, 0.3)
