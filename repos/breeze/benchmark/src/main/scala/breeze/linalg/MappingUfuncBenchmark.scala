...
object harderUfunc extends UFunc with MappingUFunc {
  //A custom stupid ufunc that is very fast to run
  implicit object expDoubleImpl extends Impl[Double, Double] {
    def apply(v: Double) = /*(v+1)/(1+v*v)*/ math.exp(v)
  }
}

class MappingUfuncBenchmark
    extends BreezeBenchmark
    with BuildsRandomMatrices
