...
trait BuildsRandomMatrices {
  ...
  def randomMatrix(
    ...
    if (!transpose) { DenseMatrix.rand[Double](m, n) }
    else { DenseMatrix.rand[Double](m, n).t }
  }
  ...
  def randomIntMatrix(
    ...
    if (!transpose) { DenseMatrix.rand[Int](m, n, rand = Rand.randInt(200)) }
    else { DenseMatrix.rand[Int](m, n, Rand.randInt(200)).t }
  }
}
...
class DenseMatrixBenchmark extends BreezeBenchmark with BuildsRandomMatrices {
  ...
  def timeMapPairsTranspose(reps: Int) =
    runWith(reps, { randomMatrix(2048, 2048, true) })(
