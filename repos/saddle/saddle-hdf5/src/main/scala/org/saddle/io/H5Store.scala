...
object H5Store {
  ...
  private def readAttrLong(node_id: Int, attr: String): Long = {
    ...
    val datatype = H5.H5Aget_type(attrid)
    assertException(
      datatype >= 0 && H5.H5Tget_class(datatype) == HDF5Constants.H5T_INTEGER,
  ...
  private def readPandasSeries[X: ST: ORD, T: ST](
    ...
    readAttrText(idxid, "kind") match {
      ...
      case _ @t =>
        throw new IllegalArgumentException("Bad index type found: %s".format(t))
    }

    val index =
      ixtype.runtimeClass match {
        case x if x == tc => {
