...
class DoubleStats(r: Vec[Double]) extends VecStats[Double] {
  ...
  def min: Option[Double] =
    if (r.count == 0) None
      ...
      val res = r
        .filterFoldLeft(sd.notMissing)(sd.inf)((x: Double, y: Double) =>
          if (x < y) x else y)
      Some(res)
    }
  ...
  def max: Option[Double] =
    if (r.count == 0) None
      ...
      val res: Double = r
        .filterFoldLeft(sd.notMissing)(sd.negInf)((x: Double, y: Double) =>
          if (x > y) x else y)
      Some(res)
    }
...
class IntStats(r: Vec[Int]) extends VecStats[Int] {
  ...
  def max: Option[Int] =
    if (r.count == 0) None
      ...
      val res: Int = r
        .filterFoldLeft(si.notMissing)(si.negInf)((x: Int, y: Int) =>
          if (x > y) x else y)
      Some(res)
    }
...
class LongStats(r: Vec[Long]) extends VecStats[Long] {
  ...
  def min: Option[Long] =
    if (r.count == 0) None
      ...
      val res: Long = r
        .filterFoldLeft(sl.notMissing)(sl.inf)((x: Long, y: Long) =>
          if (x < y) x else y)
      Some(res)
    }
  ...
  def max: Option[Long] =
    if (r.count == 0) None
      ...
      val res: Long = r
        .filterFoldLeft(sl.notMissing)(sl.negInf)((x: Long, y: Long) =>
          if (x > y) x else y)
      Some(res)
    }
