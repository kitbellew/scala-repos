...
object BinOp {
  private final class BinOpImpl[
      ...
      @spec(Boolean, Int, Long, Double) S: ST](f: (Q, R) => S)
  ...
  implicit val powLL: BLLL[Power] =
    new BinOpImpl[Power, Long, Long, Long]((x, y) => fpow(x, y).toLong)
  ...
  implicit val powLI: BLIL[Power] =
    new BinOpImpl[Power, Long, Int, Long]((x, y) => fpow(x, y).toLong)
  ...
  implicit val powIL: BILL[Power] =
    new BinOpImpl[Power, Int, Long, Long]((x, y) => fpow(x, y).toLong)
  ...
  implicit val shlIL: BILL[BitShl] =
    new BinOpImpl[BitShl, Int, Long, Long](_ << _)
  implicit val shrIL: BILL[BitShr] =
    new BinOpImpl[BitShr, Int, Long, Long](_ >> _)
  implicit val ushIL: BILL[BitUShr] =
    new BinOpImpl[BitUShr, Int, Long, Long](_ >>> _)

  // (x, y) => Int ops

  type BIII[T <: OpType] = BinOp[T, Int, Int, Int]
  implicit val powII: BIII[Power] =
    new BinOpImpl[Power, Int, Int, Int]((x, y) => fpow(x, y).toInt)
