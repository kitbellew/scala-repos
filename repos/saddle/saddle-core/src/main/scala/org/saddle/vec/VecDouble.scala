...
class VecDouble(values: Array[Double]) extends Vec[Double] { self =>
  ...
  def rolling[@spec(Boolean, Int, Long, Double) B: ST](
      winSz: Int,
      f: Vec[Double] => B): Vec[B] = VecImpl.rolling(this)(winSz, f)
  ...
  def zipMap[
      ...
      f: (Double, B) => C): Vec[C] = VecImpl.zipMap(this, other)(f)
