...
trait BinOpMat {
  ...
  final class MatSclrElemOp[
      ...
      @spec(Int, Long, Double) C: ST](val op: BinOp[OP, A, B, C])
  ...
  implicit def MatSclrElmOpIII[Op <: ScalarOp](implicit
      op: BinOp[Op, Int, Int, Int]) = new MatSclrElemOp[Op, Int, Int, Int](op)
  ...
  final class MatMatElemOp[
      OP <: ScalarOp, @spec(Int, Long, Double) A, @spec(Int, Long, Double) B,
      @spec(Int, Long, Double) C: ST](op: BinOp[OP, A, B, C])
  ...
  implicit def MatMatElemOpIII[Op <: ScalarOp](implicit
      op: BinOp[Op, Int, Int, Int]) = new MatMatElemOp[Op, Int, Int, Int](op)
  ...
  implicit def matmulOpWithVector[A, B, OP <: InnerProd](implicit
    ...
    new BinOp[InnerProd, Mat[A], Vec[B], Mat[Double]] {
      def apply(m1: Mat[A], m2: Vec[B]): Mat[Double] = { m1.mult(Mat(m2)) }
    }
  ...
  implicit def matmulOpWithMatrix[A, B, OP <: InnerProd](implicit
    ...
    new BinOp[InnerProd, Mat[A], Mat[B], Mat[Double]] {
      def apply(m1: Mat[A], m2: Mat[B]): Mat[Double] = { m1.mult(m2) }
    }
}
