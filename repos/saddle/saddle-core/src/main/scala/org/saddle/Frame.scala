...
class Frame[RX: ST: ORD, CX: ST: ORD, T: ST](
  ...
  def align[U: ST](
    ...
    val lvals: MatCols[T] = cJoin
      .lTake
      .map(locs => values.take(locs))
      .getOrElse(values)
    val rvals: MatCols[U] = cJoin
      .rTake
    ...
    val vecs =
      for (i <- 0 until lvals.length)
        yield {
          val lvec: Vec[T] = rJoin
            .lTake
            .map(locs => lvals(i).take(locs))
            .getOrElse(lvals(i))
          val rvec: Vec[U] = rJoin
            .rTake
  ...
  def rjoinS(
      other: Series[CX, T],
      how: JoinType = LeftJoin): Frame[Int, CX, T] = T.joinS(other, how).T
  ...
  def rjoinAnyS(
      other: Series[CX, _],
      how: JoinType = LeftJoin): Frame[Int, CX, Any] = T.joinAnyS(other, how).T
  ...
  def rjoinAny(
      other: Frame[_, CX, _],
      how: JoinType = LeftJoin): Frame[Int, CX, Any] = T.joinAny(other.T, how).T
