...
private[saddle] object VecImpl {
  ...
  def zipMap[
    ...
    while (i < v1.length) {
      val a = v1(i)
      val b = v2(i)
      if (sca.isMissing(a) || scb.isMissing(b)) { buf(i) = scc.missing }
      else { buf(i) = f(a, b) }
      i += 1
    }
    Vec(buf)
  ...
  def filterFoldLeft[
      @spec(Boolean, Int, Long, Double) A: ST,
      @spec(Boolean, Int, Long, Double) B](vec: Vec[A])(pred: (A) => Boolean)(
      init: B)(f: (B, A) => B): B = {
    val sa = implicitly[ST[A]]
    var acc = init
    var i = 0
    while (i < vec.length) {
      val vi = vec(i)
      if (sa.notMissing(vi) && pred(vi)) { acc = f(acc, vi) }
      i += 1
    }
    acc
  }
  ...
  def seriesfillNA[
      ...
      @spec(Boolean, Int, Long, Double) A: ST](idx: Vec[X], vec: Vec[A])(
      f: X => A): Vec[A] = {
    val buf = vec.contents
    var i = 0
    val l = vec.length
