  ...
  "Series[Int, Double] Tests" in {
    ...
    "head works" in {
      forAll { (s: Series[Int, Double]) =>
        ...
        if (s.length == 1) { s.head(1) must_== s(0) }
        else {
    ...
    "tail works" in {
      forAll { (s: Series[Int, Double]) =>
        ...
        if (s.length == 1) { s.tail(1) must_== s(0) }
        else {
    ...
    "filterAt works" in {
      forAll { (s: Series[Int, Double]) =>
        val idx = Gen.choose(0, s.length - 1)
        forAll(idx) { i =>
          (s.filterAt(_ != i).length == 0 || s.filterAt(_ != i)
    ...
    "pivot works" in {
      ...
      val dat2 = v3.sliceBy(0, 5).toDoubleArray ++ Array(na.to[Double]) ++
        v3.sliceBy(6, 7).toDoubleArray
  ...
  "Series[DateTime, Double] Tests" in {
    ...
    "take works" in {
      forAll { (s: Series[DateTime, Double]) =>
        ...
        forAll(idx) { i =>
          val res = s.take(i.toArray)
          val exp = s.slice(i(0), i(0) + 1) concat
            s.slice(i(1), i(1) + 1) concat s.slice(i(2), i(2) + 1)
          res must_== exp
        }
      }
    }
