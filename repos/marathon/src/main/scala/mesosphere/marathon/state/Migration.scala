...
class Migration @Inject() (
  ...
  def applyMigrationSteps(
    ...
    migrations.filter(_._1 > from).sortBy(_._1)
...
class MigrationTo0_11(
  ...
  def migrateApps(): Future[Unit] = {
    ...
    val rootGroupFuture = groupRepository.rootGroup()
      ...
      (otherStores, appId) =>
        otherStores.flatMap { storedApps =>
          ...
          maybeAppInGroup match {
            case Some(appInGroup) => addVersionInfo(appId, appInGroup)
                .map(storedApps ++ _)
            case None =>
              log.warn(
      ...
      maybeNextApp.map { nextApp =>
        maybeLastApp match {
          ...
          case _ =>
            ...
            nextApp.copy(versionInfo = AppDefinition.VersionInfo
              .forNewConfig(nextApp.version))
        }
      }
    }
...
class MigrationTo0_13(taskRepository: TaskRepository, store: PersistentStore) {
  ...
  def migrateTasks(): Future[Unit] = {
    ...
    entityStore.names().flatMap { keys =>
      ...
      val namesInOldFormat = keys
        .filter(key => oldFormatRegex.pattern.matcher(key).matches)
      log.info(
  ...
  def renameFrameworkId(): Future[Unit] = {
    ...
    store.load(newName).flatMap {
      case Some(_) =>
        ...
        Future.successful(())
      case None => store.load(oldName).flatMap {
          case None =>
...
class MigrationTo0_16(
  ...
  private[this] def updateAllGroupVersions(): Future[Unit] = {
    ...
    groupRepository.listVersions(id).map(d => d.toSeq.sorted).flatMap {
      sortedVersions =>
...
object StorageVersions {
  ...
  def apply(major: Int, minor: Int, patch: Int): StorageVersion = {
    StorageVersion.newBuilder().setMajor(major).setMinor(minor).setPatch(patch)
      .build()
  }
  ...
  implicit class OrderedStorageVersion(val version: StorageVersion)
    ...
    def str: String =
      s"Version(${version.getMajor}, ${version.getMinor}, ${version.getPatch})"
