  ...
  def parent: PathId =
    path match {
      ...
      case head :: Nil =>
        PathId(Nil, absolute)
      case head :: rest =>
        PathId(path.reverse.tail.reverse, absolute)
    }
...
object PathId {
  ...
  def apply(in: String): PathId =
    PathId(
      in.replaceAll("""(^/+)|(/+$)""", "").split("/").filter(_.nonEmpty).toList,
      in.startsWith("/"))
  ...
  private val validPathChars =
    new Validator[PathId] {
      override def apply(pathId: PathId): Result = {
        validate(pathId.path)(validator = pathId
          .path
          .each should matchRegexFully(ID_PATH_SEGMENT_PATTERN.pattern))
      }
    }
