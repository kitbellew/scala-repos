...
class AppsResource @Inject() (
    ...
    implicit identity =>
      withValid(Json.parse(body).as[Seq[AppUpdate]].map(_.withCanonizedIds())) {
        updates =>
          ...
          def updateGroup(root: Group): Group = updates.foldLeft(root) {
            (group, update) =>
              update.id match {
                case Some(id) =>
                  group.updateApp(
  ...
  @Path("{appId:.+}/versions")
  def appVersionsResource(): AppVersionsResource = new AppVersionsResource(
    service,
    groupManager,
    authenticator,
    authorizer,
  ...
  def allAuthorized(implicit identity: Identity): AppSelector =
    new AppSelector {
