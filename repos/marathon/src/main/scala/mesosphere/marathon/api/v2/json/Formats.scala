...
object Formats extends Formats {
  ...
  implicit class FormatWithDefault[A](val m: OFormat[Option[A]])
      extends AnyVal {
  ...
  implicit class FormatAsSeconds(val format: OFormat[Long]) extends AnyVal {
    def asSeconds: OFormat[FiniteDuration] =
      format.inmap(_.seconds, _.toSeconds)
  }
}
...
trait Formats
  ...
  implicit lazy val ipAddressFormat: Format[mesos.NetworkInfo.IPAddress] = {
    def toIpAddress(
        ipAddress: String,
        protocol: mesos.NetworkInfo.Protocol): mesos.NetworkInfo.IPAddress =
      mesos.NetworkInfo.IPAddress.newBuilder().setIpAddress(ipAddress)
        .setProtocol(protocol).build()

    def toTuple(ipAddress: mesos.NetworkInfo.IPAddress)
        : (String, mesos.NetworkInfo.Protocol) =
      (ipAddress.getIpAddress, ipAddress.getProtocol)

    ((__ \ "ipAddress").format[String] ~ (__ \ "protocol")
      .format[mesos.NetworkInfo.Protocol])(toIpAddress, toTuple)
  }

  implicit lazy val TaskIdWrite: Writes[Task.Id] = Writes { id =>
    JsString(id.idString)
  }
  implicit lazy val LocalVolumeIdWrite: Writes[Task.LocalVolumeId] = Writes {
    id =>
      Json.obj(
  ...
  implicit lazy val TaskWrites: Writes[Task] = Writes { task =>
    ...
    val reservation = task.reservationWithVolumes.map { reservation =>
      launched ++ Json.obj("localVolumes" -> reservation.volumeIds)
    }.getOrElse(launched)
  ...
  implicit lazy val EnrichedTaskWrites: Writes[EnrichedTask] = Writes { task =>
    ...
    val enrichedJson = taskJson ++ Json.obj("appId" -> task.appId)
    ...
    if (task.healthCheckResults.nonEmpty)
      withServicePorts ++
        Json.obj("healthCheckResults" -> task.healthCheckResults)
  ...
  implicit lazy val ParameterFormat: Format[Parameter] =
    ((__ \ "key").format[String] ~ (__ \ "value").format[String])(
      Parameter(_, _),
      unlift(Parameter.unapply))

  /*
   * Helpers
   */

  def uniquePorts: Reads[Seq[Int]] =
    Format.of[Seq[Int]].filter(ValidationError("Ports must be unique.")) {
      ports =>
  ...
  def nonEmpty[C <: Iterable[_]](implicit reads: Reads[C]): Reads[C] =
    Reads
      .filterNot[C](ValidationError(s"set must not be empty"))(_.isEmpty)(reads)

  def enumFormat[A <: java.lang.Enum[A]](
      read: String => A,
...
trait ContainerFormats {
  ...
  implicit lazy val DockerNetworkFormat: Format[DockerInfo.Network] =
    enumFormat(
      DockerInfo.Network.valueOf,
      str => s"$str is not a valid network type")

  implicit lazy val PortMappingFormat: Format[Docker.PortMapping] =
    ((__ \ "containerPort").formatNullable[Int]
      .withDefault(AppDefinition.RandomPortValue) ~ (__ \ "hostPort")
      .formatNullable[Int].withDefault(AppDefinition.RandomPortValue) ~
      (__ \ "servicePort").formatNullable[Int]
        .withDefault(AppDefinition.RandomPortValue) ~ (__ \ "protocol")
        .formatNullable[String].withDefault("tcp") ~ (__ \ "name")
        .formatNullable[String] ~ (__ \ "labels")
        .formatNullable[Map[String, String]]
        .withDefault(Map.empty[String, String]))(
      PortMapping(_, _, _, _, _, _),
      unlift(PortMapping.unapply))

  implicit lazy val DockerFormat: Format[Docker] =
    ((__ \ "image").format[String] ~ (__ \ "network")
      .formatNullable[DockerInfo.Network] ~ (__ \ "portMappings")
      .formatNullable[Seq[Docker.PortMapping]] ~ (__ \ "privileged")
      .formatNullable[Boolean].withDefault(false) ~ (__ \ "parameters")
      .formatNullable[Seq[Parameter]].withDefault(Seq.empty) ~
      (__ \ "forcePullImage").formatNullable[Boolean].withDefault(false))(
      Docker(_, _, _, _, _, _),
      unlift(Docker.unapply))

  implicit lazy val ModeFormat: Format[mesos.Volume.Mode] = enumFormat(
    mesos.Volume.Mode.valueOf,
    str => s"$str is not a valid mde")

  implicit lazy val PersistentVolumeInfoFormat: Format[PersistentVolumeInfo] =
    Json.format[PersistentVolumeInfo]

  implicit lazy val VolumeFormat: Format[Volume] =
    ((__ \ "containerPath").format[String] ~ (__ \ "hostPath")
      .formatNullable[String] ~ (__ \ "mode").format[mesos.Volume.Mode] ~
  ...
  implicit lazy val ContainerTypeFormat: Format[mesos.ContainerInfo.Type] =
    enumFormat(
      mesos.ContainerInfo.Type.valueOf,
      str => s"$str is not a valid container type")

  implicit lazy val ContainerFormat: Format[Container] =
    ((__ \ "type").formatNullable[mesos.ContainerInfo.Type]
      .withDefault(mesos.ContainerInfo.Type.DOCKER) ~ (__ \ "volumes")
      .formatNullable[Seq[Volume]].withDefault(Nil) ~ (__ \ "docker")
      .formatNullable[Docker])(Container(_, _, _), unlift(Container.unapply))
}
...
trait IpAddressFormats {
  ...
  private[this] lazy val ValidPortProtocol: Reads[String] = {
    implicitly[Reads[String]].filter(ValidationError(
  ...
  private[this] lazy val ValidPorts: Reads[Seq[DiscoveryInfo.Port]] = {
    ...
    implicitly[Reads[Seq[DiscoveryInfo.Port]]].filter(ValidationError(
      "Port names are not unique."))(hasUniquePortNames).filter(ValidationError(
  ...
  implicit lazy val PortFormat: Format[DiscoveryInfo.Port] =
    ((__ \ "number").format[Int] ~ (__ \ "name").format[String] ~
      (__ \ "protocol").format[String](ValidPortProtocol))(
      DiscoveryInfo.Port(_, _, _),
      unlift(DiscoveryInfo.Port.unapply))

  implicit lazy val DiscoveryInfoFormat: Format[DiscoveryInfo] = Format(
    (__ \ "ports").read[Seq[DiscoveryInfo.Port]](ValidPorts)
      .map(DiscoveryInfo(_)),
  ...
  implicit lazy val IpAddressFormat: Format[IpAddress] =
    ((__ \ "groups").formatNullable[Seq[String]].withDefault(Nil) ~
      (__ \ "labels").formatNullable[Map[String, String]]
        .withDefault(Map.empty[String, String]) ~ (__ \ "discovery")
        .formatNullable[DiscoveryInfo].withDefault(DiscoveryInfo.empty))(
      IpAddress(_, _, _),
      unlift(IpAddress.unapply))
}

trait DeploymentFormats {
  import Formats._

  implicit lazy val ByteArrayFormat: Format[Array[Byte]] = Format(
  ...
  implicit lazy val GroupUpdateFormat: Format[GroupUpdate] =
    ((__ \ "id").formatNullable[PathId] ~ (__ \ "apps")
      .formatNullable[Set[AppDefinition]] ~ (__ \ "groups")
      .lazyFormatNullable(implicitly[Format[Set[GroupUpdate]]]) ~
      (__ \ "dependencies").formatNullable[Set[PathId]] ~ (__ \ "scaleBy")
        .formatNullable[Double] ~ (__ \ "version").formatNullable[Timestamp])(
      GroupUpdate(_, _, _, _, _, _),
      unlift(GroupUpdate.unapply))

  implicit lazy val URLToStringMapFormat: Format[Map[java.net.URL, String]] =
    Format(
      Reads.of[Map[String, String]].map(_.map {
  ...
  implicit lazy val DeploymentActionWrites: Writes[DeploymentAction] = Writes {
    action =>
      Json.obj("type" -> action.getClass.getSimpleName, "app" -> action.app.id)
  }

  implicit lazy val DeploymentStepWrites: Writes[DeploymentStep] = Json
    .writes[DeploymentStep]
}

trait EventFormats {
  import Formats._

  implicit lazy val AppTerminatedEventWrites: Writes[AppTerminatedEvent] = Json
    .writes[AppTerminatedEvent]
  ...
  implicit lazy val DeploymentPlanWrites: Writes[DeploymentPlan] = Writes {
    plan =>
      Json.obj(
        "id" -> plan.id,
  ...
  implicit lazy val UnsubscribeWrites: Writes[Unsubscribe] = Json
    .writes[Unsubscribe]
  implicit lazy val EventStreamAttachedWrites: Writes[EventStreamAttached] =
    Json.writes[EventStreamAttached]
  implicit lazy val EventStreamDetachedWrites: Writes[EventStreamDetached] =
    Json.writes[EventStreamDetached]
  implicit lazy val AddHealthCheckWrites: Writes[AddHealthCheck] = Json
    .writes[AddHealthCheck]
  implicit lazy val RemoveHealthCheckWrites: Writes[RemoveHealthCheck] = Json
    .writes[RemoveHealthCheck]
  implicit lazy val FailedHealthCheckWrites: Writes[FailedHealthCheck] = Json
    .writes[FailedHealthCheck]
  implicit lazy val HealthStatusChangedWrites: Writes[HealthStatusChanged] =
    Json.writes[HealthStatusChanged]
  implicit lazy val GroupChangeSuccessWrites: Writes[GroupChangeSuccess] = Json
    .writes[GroupChangeSuccess]
  implicit lazy val GroupChangeFailedWrites: Writes[GroupChangeFailed] = Json
    .writes[GroupChangeFailed]
  implicit lazy val DeploymentSuccessWrites: Writes[DeploymentSuccess] = Json
    .writes[DeploymentSuccess]
  implicit lazy val DeploymentFailedWrites: Writes[DeploymentFailed] = Json
    .writes[DeploymentFailed]
  implicit lazy val DeploymentStatusWrites: Writes[DeploymentStatus] = Json
    .writes[DeploymentStatus]
  implicit lazy val DeploymentStepSuccessWrites: Writes[DeploymentStepSuccess] =
  ...
  implicit lazy val MesosFrameworkMessageEventWrites
      : Writes[MesosFrameworkMessageEvent] = Json
    .writes[MesosFrameworkMessageEvent]
  implicit lazy val SchedulerDisconnectedEventWrites
      : Writes[SchedulerDisconnectedEvent] = Json
    .writes[SchedulerDisconnectedEvent]
  implicit lazy val SchedulerRegisteredEventWritesWrites
      : Writes[SchedulerRegisteredEvent] = Json.writes[SchedulerRegisteredEvent]
  implicit lazy val SchedulerReregisteredEventWritesWrites
      : Writes[SchedulerReregisteredEvent] = Json
    .writes[SchedulerReregisteredEvent]
...
trait EventSubscribersFormats {
  ...
  implicit lazy val EventSubscribersWrites: Writes[EventSubscribers] = Writes {
    eventSubscribers => Json.obj("callbackUrls" -> eventSubscribers.urls)
  }
}
...
trait HealthCheckFormats {
  ...
  implicit lazy val HealthCheckProtocolFormat: Format[Protocol] = enumFormat(
    Protocol.valueOf,
    str => s"$str is not a valid protocol")

  implicit lazy val HealthCheckFormat: Format[HealthCheck] = {
    import mesosphere.marathon.health.HealthCheck._

    ((__ \ "path").formatNullable[String] ~ (__ \ "protocol")
      .formatNullable[Protocol].withDefault(DefaultProtocol) ~
      (__ \ "portIndex").formatNullable[Int] ~ (__ \ "command")
        .formatNullable[Command] ~ (__ \ "gracePeriodSeconds")
        .formatNullable[Long].withDefault(DefaultGracePeriod.toSeconds)
        .asSeconds ~ (__ \ "intervalSeconds").formatNullable[Long]
        .withDefault(DefaultInterval.toSeconds).asSeconds ~
      (__ \ "timeoutSeconds").formatNullable[Long]
        .withDefault(DefaultTimeout.toSeconds).asSeconds ~
      (__ \ "maxConsecutiveFailures").formatNullable[Int]
        .withDefault(DefaultMaxConsecutiveFailures) ~ (__ \ "ignoreHttp1xx")
        .formatNullable[Boolean].withDefault(DefaultIgnoreHttp1xx) ~
      (
        __ \ "port"
      ).formatNullable[Int])(HealthCheck.apply, unlift(HealthCheck.unapply))
  }
}
...
trait FetchUriFormats {
  ...
  implicit lazy val FetchUriFormat: Format[FetchUri] = {
    ((__ \ "uri").format[String] ~ (__ \ "extract").formatNullable[Boolean]
      .withDefault(true) ~ (__ \ "executable").formatNullable[Boolean]
      .withDefault(false) ~ (__ \ "cache").formatNullable[Boolean]
      .withDefault(false))(FetchUri(_, _, _, _), unlift(FetchUri.unapply))
  }
}
...
trait AppAndGroupFormats {
  ...
  implicit lazy val UpgradeStrategyReads: Reads[UpgradeStrategy] = {
    ...
    ((__ \ "minimumHealthCapacity").readNullable[Double]
      .withDefault(DefaultUpgradeStrategy.minimumHealthCapacity) ~
      (__ \ "maximumOverCapacity").readNullable[Double].withDefault(
        DefaultUpgradeStrategy.maximumOverCapacity))(UpgradeStrategy(_, _))
  }
        ...
        json.asOpt[Seq[String]] match {
          case Some(seq) if seq.size >= 2 && seq.size <= 3 =>
            if (validOperators.contains(seq(1))) {
              val builder = Constraint.newBuilder().setField(seq(0))
                .setOperator(Operator.valueOf(seq(1)))
              if (seq.size == 3) builder.setValue(seq(2))
              JsSuccess(builder.build())
            } else {
              JsError(
                s"Constraint operator must be one of the following: [${validOperators
                  .mkString(", ")}]")
            }
          case _ =>
            JsError(
  ...
  implicit lazy val AppDefinitionReads: Reads[AppDefinition] = {
    ...
    ((__ \ "id").read[PathId].filterNot(_.isRoot) ~
      ...
      (__ \ "env").readNullable[Map[String, String]]
        .withDefault(AppDefinition.DefaultEnv) ~
      (__ \ "instances").readNullable[Int]
        .withDefault(AppDefinition.DefaultInstances) ~
      (__ \ "cpus").readNullable[Double]
        .withDefault(AppDefinition.DefaultCpus) ~
      (__ \ "mem").readNullable[Double].withDefault(AppDefinition.DefaultMem) ~
      (__ \ "disk").readNullable[Double]
        .withDefault(AppDefinition.DefaultDisk) ~
      ...
      (__ \ "constraints").readNullable[Set[Constraint]]
        .withDefault(AppDefinition.DefaultConstraints) ~
      (__ \ "storeUrls").readNullable[Seq[String]]
        .withDefault(AppDefinition.DefaultStoreUrls) ~
      (__ \ "requirePorts").readNullable[Boolean]
        .withDefault(AppDefinition.DefaultRequirePorts) ~
      (__ \ "backoffSeconds").readNullable[Long]
        .withDefault(AppDefinition.DefaultBackoff.toSeconds).asSeconds ~
      (__ \ "backoffFactor").readNullable[Double]
        .withDefault(AppDefinition.DefaultBackoffFactor) ~
      ...
      case class ExtraFields(
        ...
        def upgradeStrategyOrDefault: UpgradeStrategy = {
          ...
          upgradeStrategy
            .getOrElse(if (residency.isDefined) forResidentTasks else empty)
        }
      ...
      val extraReads: Reads[ExtraFields] =
        (
          (__ \ "uris").readNullable[Seq[String]]
            .withDefault(AppDefinition.DefaultUris) ~ (__ \ "fetch")
            .readNullable[Seq[FetchUri]]
            .withDefault(AppDefinition.DefaultFetch) ~ (__ \ "dependencies")
            .readNullable[Set[PathId]]
            .withDefault(AppDefinition.DefaultDependencies) ~ (__ \ "ports")
            .readNullable[Seq[Int]](uniquePorts) ~ (__ \ "upgradeStrategy")
            .readNullable[UpgradeStrategy] ~ (__ \ "labels")
            .readNullable[Map[String, String]]
            .withDefault(AppDefinition.DefaultLabels) ~
            (__ \ "acceptedResourceRoles").readNullable[Set[String]](nonEmpty) ~
            (__ \ "ipAddress").readNullable[IpAddress] ~ (__ \ "version")
              .readNullable[Timestamp].withDefault(Timestamp.now()) ~
            (__ \ "residency").readNullable[Residency] ~
            (__ \ "portDefinitions").readNullable[Seq[PortDefinition]]
        )(ExtraFields).filter(ValidationError(
          "You cannot specify both uris and fetch fields")) { extra =>
          !(extra.uris.nonEmpty && extra.fetch.nonEmpty)
        }.filter(ValidationError(
          "You cannot specify both an IP address and ports")) { extra =>
          val appWithoutPorts = extra.maybePorts.forall(_.isEmpty) &&
            extra.maybePortDefinitions.forall(_.isEmpty)
          appWithoutPorts || extra.ipAddress.isEmpty
        }.filter(ValidationError(
          "You cannot specify both ports and port definitions")) { extra =>
          val portDefinitionsIsEquivalentToPorts = extra.maybePortDefinitions
            .map(_.map(_.port)) == extra.maybePorts
          portDefinitionsIsEquivalentToPorts || extra.maybePorts.isEmpty ||
          extra.maybePortDefinitions.isEmpty
        }
  ...
  /**
    ...
    */
  private[this] def addHealthCheckPortIndexIfNecessary(
      app: AppDefinition): AppDefinition = {
    val hasPortMappings = app.container
      .exists(_.docker.exists(_.portMappings.exists(_.nonEmpty)))
    ...
    app.copy(healthChecks = app.healthChecks.map { healthCheck =>
      def needsDefaultPortIndex =
        healthCheck.port.isEmpty && healthCheck.portIndex.isEmpty &&
  ...
  implicit lazy val taskLostBehaviorReads = Reads[TaskLostBehavior] { json =>
    json.validate[String].flatMap { behaviorString: String =>
      Option(TaskLostBehavior.valueOf(behaviorString)) match {
        ...
        case None => {
          val allowedTaskLostBehaviorString = TaskLostBehavior.values().toSeq
            .map(_.getDescriptorForType.getName).mkString(", ")
  ...
  lazy val TaskCountsWritesWithoutPrefix: Writes[TaskCounts] = Writes {
    counts =>
      Json.obj(
  ...
  implicit lazy val TaskLifeTimeWrites: Writes[TaskLifeTime] = Writes {
    lifeTime =>
      Json.obj(
  ...
  implicit lazy val TaskStatsWrites: Writes[TaskStats] = Writes { stats =>
    val statsJson = Json
      .obj("counts" -> TaskCountsWritesWithoutPrefix.writes(stats.counts))
    Json.obj(
  ...
  implicit lazy val TaskStatsByVersionWrites: Writes[TaskStatsByVersion] =
    Writes { byVersion =>
      ...
      Json.toJson(maybeJsons.iterator.flatMap {
  ...
  implicit lazy val ExtendedAppInfoWrites: Writes[AppInfo] = Writes { info =>
    ...
    val maybeJson = Seq[Option[JsObject]](
      ...
      info.maybeDeployments
        .map(deployments => Json.obj("deployments" -> deployments)),
      info.maybeTasks.map(tasks => Json.obj("tasks" -> tasks)),
      info.maybeLastTaskFailure
        .map(lastFailure => Json.obj("lastTaskFailure" -> lastFailure)),
  ...
  implicit lazy val GroupInfoWrites: Writes[GroupInfo] = Writes { info =>
    val maybeJson = Seq[Option[JsObject]](
      ...
      info.maybeGroups.map(groups => Json.obj("groups" -> groups))).flatten
  ...
  implicit lazy val AppUpdateReads: Reads[AppUpdate] =
    ((__ \ "id").readNullable[PathId].filterNot(_.exists(_.isRoot)) ~
      ...
      (
        __ \ "executor"
      ).readNullable[String](Reads.pattern("^(//cmd)|(/?[^/]+(/[^/]+)*)|$".r)) ~
      (__ \ "constraints").readNullable[Set[Constraint]] ~
      (__ \ "storeUrls").readNullable[Seq[String]] ~
      (__ \ "requirePorts").readNullable[Boolean] ~
      ...
      val extraReads: Reads[ExtraFields] =
        ((__ \ "uris").readNullable[Seq[String]] ~ (__ \ "fetch")
          .readNullable[Seq[FetchUri]] ~ (__ \ "upgradeStrategy")
          .readNullable[UpgradeStrategy] ~ (__ \ "labels")
          .readNullable[Map[String, String]] ~ (__ \ "version")
          .readNullable[Timestamp] ~ (__ \ "acceptedResourceRoles")
          .readNullable[Set[String]](nonEmpty) ~ (__ \ "ipAddress")
          .readNullable[IpAddress] ~ (__ \ "residency")
          .readNullable[Residency] ~ (__ \ "ports")
          .readNullable[Seq[Int]](uniquePorts) ~ (__ \ "portDefinitions")
          .readNullable[Seq[PortDefinition]])(ExtraFields)

      extraReads.filter(ValidationError(
        "You cannot specify both uris and fetch fields")) { extra =>
        !(extra.uris.nonEmpty && extra.fetch.nonEmpty)
      }.filter(ValidationError(
        "You cannot specify both ports and port definitions")) { extra =>
        val portDefinitionsIsEquivalentToPorts = extra.portDefinitions
          .map(_.map(_.port)) == extra.ports
        portDefinitionsIsEquivalentToPorts || extra.ports.isEmpty ||
        extra.portDefinitions.isEmpty
      }.map { extra =>
        update.copy(
          ...
          acceptedResourceRoles = extra.acceptedResourceRoles,
          ipAddress = extra.ipAddress,
          fetch = extra.fetch.orElse(extra.uris.map { seq =>
            seq.map(FetchUri.apply(_))
          }),
  ...
  implicit lazy val GroupFormat: Format[Group] =
    ((__ \ "id").format[PathId] ~ (__ \ "apps")
      .formatNullable[Set[AppDefinition]].withDefault(Group.defaultApps) ~
      (__ \ "groups").lazyFormatNullable(implicitly[Format[Set[Group]]])
        .withDefault(Group.defaultGroups) ~ (__ \ "dependencies")
        .formatNullable[Set[PathId]].withDefault(Group.defaultDependencies) ~
      (__ \ "version").formatNullable[Timestamp].withDefault(
        Group.defaultVersion))(Group(_, _, _, _, _), unlift(Group.unapply))
  ...
  implicit lazy val PortDefinitionFormat: Format[PortDefinition] =
    ((__ \ "port").formatNullable[Int]
      ...
      .formatNullable[Map[String, String]]
      .withDefault(Map.empty[String, String]))(
      PortDefinition(_, _, _, _),
      unlift(PortDefinition.unapply))
}

trait PluginFormats {

  implicit lazy val pluginDefinitionFormat: Writes[PluginDefinition] =
    ((__ \ "id").write[String] ~ (__ \ "plugin").write[String] ~
      (__ \ "implementation").write[String] ~ (__ \ "tags")
        .writeNullable[Set[String]] ~ (__ \ "info").writeNullable[JsObject])(
      d => (d.id, d.plugin, d.implementation, d.tags, d.info))

  implicit lazy val pluginDefinitionsFormat: Writes[PluginDefinitions] = Json
    .writes[PluginDefinitions]
}
