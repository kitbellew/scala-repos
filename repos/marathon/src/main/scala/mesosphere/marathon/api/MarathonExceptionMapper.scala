...
class MarathonExceptionMapper extends ExceptionMapper[Exception] {
  ...
  private def statusCode(exception: Exception): Int =
    exception match {
      //scalastyle:off magic.number
      case e: TimeoutException =>
        503 // Service Unavailable
      case e: UnknownAppException =>
        404 // Not found
      case e: UnknownGroupException =>
        404 // Not found
      case e: AppLockedException =>
        409 // Conflict
      case e: ConflictingChangeException =>
        409 // Conflict
  ...
  private def entity(exception: Exception): JsValue =
    exception match {
      ...
      case e: JsonParseException =>
        Json.obj("message" -> "Invalid JSON", "details" -> e.getOriginalMessage)
      case e: JsonMappingException =>
        Json.obj(
      ...
      case e: JsResultException =>
        val errors = e
          ...
          .map {
            case (path, errs) =>
              Json
                .obj("path" -> path.toString(), "errors" -> errs.map(_.message))
          }
        Json.obj("message" -> s"Invalid JSON", "details" -> errors)
      case ValidationFailedException(obj, failure) =>
        Json.toJson(failure)
      case e: WebApplicationException =>
        //scalastyle:off null
        if (Status.fromStatusCode(e.getResponse.getStatus) != null) {
          Json.obj(
            "message" ->
              Status.fromStatusCode(e.getResponse.getStatus).getReasonPhrase)
        } else {
