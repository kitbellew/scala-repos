  ...
  private[this] def setNewDelay(app: AppDefinition, message: String)(
    ...
    calcDelay(maybeDelay) match {
      case Some(newDelay) =>
        ...
        if (newDelay.deadline <= now) { resetDelay(app) }
        else {
          log.info(
...
private object RateLimiter {
  ...
  private object Delay {
    def apply(clock: Clock, app: AppDefinition): Delay =
      Delay(clock, app.backoff)
    def apply(clock: Clock, delay: FiniteDuration): Delay =
      Delay(clock.now() + delay, delay)
  }

  private case class Delay(deadline: Timestamp, delay: FiniteDuration) {
