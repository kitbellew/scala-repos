...
import mesosphere.marathon.core.matcher.manager.impl.OfferMatcherManagerActor.{
  MatchTimeout,
  OfferData
}
...
private[manager] object OfferMatcherManagerActor {
  ...
  private case class OfferData(
    ...
    def addTasks(added: Seq[TaskOpWithSource]): OfferData = {
      val leftOverOffer =
        added.foldLeft(offer) { (offer, nextOp) =>
...
private[impl] class OfferMatcherManagerActor private (
  ...
  private[this] def offersWanted: Boolean =
    matchers.nonEmpty && launchTokens > 0
  private[this] def updateOffersWanted(): Unit =
    offersWantedObserver.onNext(offersWanted)
    ...
    case OfferMatcher.MatchedTaskOps(offerId, addedOps, resendOffer) =>
      def processAddedTasks(data: OfferData): OfferData = {
        ...
        dataWithTasks.nextMatcherOpt match {
          ...
          case None =>
            log.warning(
              s"Got unexpected matched ops from ${sender()}: $addedOps")
            dataWithTasks
        }
      }
      ...
      offerQueues.get(offerId) match {
        ...
        case None =>
          addedOps
            .foreach(_.reject(s"offer '${offerId.getValue}' already timed out"))
      }
