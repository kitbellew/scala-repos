...
class OfferOperationFactory(
  ...
  def launch(taskInfo: Mesos.TaskInfo): Mesos.Offer.Operation = {
    val launch = Mesos.Offer.Operation.Launch.newBuilder()
      .addTaskInfos(taskInfo).build()

    Mesos.Offer.Operation.newBuilder()
      .setType(Mesos.Offer.Operation.Type.LAUNCH).setLaunch(launch).build()
  }
  ...
  def reserve(
    ...
    val reservedResources = resources.map { resource =>
      Mesos.Resource.newBuilder(resource).setRole(role).setReservation(
        ReservationInfo.newBuilder().setPrincipal(principal)
          .setLabels(TaskLabels.labelsForTask(frameworkId, taskId).mesosLabels))
        .build()
    }

    val reserve = Mesos.Offer.Operation.Reserve.newBuilder()
      .addAllResources(reservedResources.asJava).build()

    Mesos.Offer.Operation.newBuilder()
      .setType(Mesos.Offer.Operation.Type.RESERVE).setReserve(reserve).build()
  }
  ...
  def createVolumes(
    ...
    val volumes: Iterable[Mesos.Resource] = localVolumes.map { vol =>
      val disk = {
        ...
        Mesos.Resource.DiskInfo.newBuilder().setPersistence(persistence)
          .setVolume(volume)
      }
      ...
      Mesos.Resource.newBuilder().setName("disk")
        .setType(Mesos.Value.Type.SCALAR).setScalar(
          Mesos.Value.Scalar.newBuilder()
            .setValue(vol.persistentVolume.persistent.size.toDouble).build())
        .setRole(role).setReservation(reservation).setDisk(disk).build()
    }
    ...
    Mesos.Offer.Operation.newBuilder()
      .setType(Mesos.Offer.Operation.Type.CREATE).setCreate(create).build()
  }
