...
case class IntegrationTestConfig(
  ...
  def marathonUrls: Seq[String] =
    marathonPorts.map(port => s"http://$marathonHost:$port")
...
object IntegrationTestConfig {
  ...
  private[this] def defaultMesosLibConfig: String = {
    ...
    val dirs =
      javaLibraryPath.split(':').toStream #:::
        Stream("/usr/local/lib", "/usr/lib", "/usr/lib64") #::: mesos_dir
  ...
  def apply(config: ConfigMap): IntegrationTestConfig = {
    def string(name: String, default: => String) =
      config.getOptional[String](name).getOrElse(default)
    def int(name: String, default: => Int) =
      config.getOptional[String](name).fold(default)(_.toInt)
    val cwd = string("cwd", ".")
    ...
    val mesosLib = string(
      "mesosLib",
      unusedForExternalSetup(defaultMesosLibConfig))
