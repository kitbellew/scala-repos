...
class TaskOpFactoryImplTest
  ...
  test("Copy SlaveID from Offer to Task") {
    ...
    val offer = MarathonTestHelper.makeBasicOffer().setHostname("some_host")
      .setSlaveId(SlaveID("some slave ID")).build()
    ...
    val expectedTask = Task.LaunchedEphemeral(
      ...
      status = Task.Status(stagedAt = f.clock.now()),
  ...
  test("Resident app -> Launch succeeds") {
    ...
    val localVolumeIdLaunched = LocalVolumeId(
      ...
      "uuidLaunched")
    val localVolumeIdUnwanted = LocalVolumeId(
      app.id,
      "persistent-volume-unwanted",
      "uuidUnwanted")
    val localVolumeIdMatch = LocalVolumeId(
      app.id,
      "persistent-volume",
      "uuidMatch")
    ...
    val taskInfoResources = taskOp.get.offerOperations.head.getLaunch
      .getTaskInfos(0).getResourcesList.asScala
  ...
  test("Resident app -> None (enough launched tasks)") {
    ...
    val f = new Fixture
    val app = f.residentApp
    val usedVolumeId = LocalVolumeId(
      app.id,
      "unwanted-persistent-volume",
      "uuid1")
    val offeredVolumeId = LocalVolumeId(
      app.id,
      "unwanted-persistent-volume",
      "uuid2")
