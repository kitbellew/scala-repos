...
class TaskOpFactoryImplTest
  ...
  test("Copy SlaveID from Offer to Task") {
    ...
    val request =
      TaskOpFactory.Request(app, offer, runningTasks, additionalLaunches = 1)
  ...
  test("Resident app -> Launch succeeds") {
    ...
    val localVolumeIdLaunched =
      LocalVolumeId(app.id, "persistent-volume-launched", "uuidLaunched")
    val localVolumeIdUnwanted =
      LocalVolumeId(app.id, "persistent-volume-unwanted", "uuidUnwanted")
    val localVolumeIdMatch =
  ...
  test("Resident app -> None (enough launched tasks)") {
    ...
    val app = f.residentApp
    val usedVolumeId =
      LocalVolumeId(app.id, "unwanted-persistent-volume", "uuid1")
    val offeredVolumeId =
      LocalVolumeId(app.id, "unwanted-persistent-volume", "uuid2")
    val runningTasks = Seq(f.residentLaunchedTask(app.id, usedVolumeId))
    val offer =
      f.offerWithVolumes(runningTasks.head.taskId.idString, offeredVolumeId)
  ...
  class Fixture {
    ...
    val config: MarathonConf =
      MTH.defaultConfig(mesosRole = Some("test"), principal = Some("principal"))
