...
class PortWithRoleRandomPortsFromRangesTest extends MarathonSpec {
  ...
  test("works for one element range") {
    assert(
      PortWithRole.lazyRandomPortsFromRanges()(Seq(portRange("role", 10, 10)))
  ...
  test("works for multiple ranges") {
    withRandomSeeds(
      ...
      expectedOutput = (10 to 13).map(PortWithRole("role", _)) ++ (14 to 20)
        .map(PortWithRole("marathon", _)) ++ (21 to 30)
        .map(PortWithRole("role", _))
    )
  }
  ...
  test("is lazy with multiple ranges") {
    ...
    def performTest(seed: Int): Unit = {
      val rand = new Random(new util.Random(seed.toLong))
      val ports = PortWithRole
        .lazyRandomPortsFromRanges(rand)((0 to numberOfRanges).map { i =>
