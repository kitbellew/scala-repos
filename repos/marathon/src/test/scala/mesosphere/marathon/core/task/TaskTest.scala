...
class TaskTest extends FunSuite with Mockito with GivenWhenThen with Matchers {
  ...
  class Fixture {
    ...
    val ipAddress1 = MesosProtos.NetworkInfo.IPAddress.newBuilder()
      .setIpAddress(ipString1).build()

    val ipString2 = "123.123.123.124"
    val ipAddress2 = MesosProtos.NetworkInfo.IPAddress.newBuilder()
      .setIpAddress(ipString2).build()

    val networkWithOneIp1 = MesosProtos.NetworkInfo.newBuilder
      .addIpAddresses(ipAddress1).build()
    val networkWithOneIp2 = MesosProtos.NetworkInfo.newBuilder
      .addIpAddresses(ipAddress2).build()

    val networkWithMultipleIps = MesosProtos.NetworkInfo.newBuilder
      .addAllIpAddresses(Seq(ipAddress1, ipAddress2).asJava).build()
    ...
    val taskWithMultipleNetworkAndMultipleIPs = MarathonTestHelper
      .runningTaskForApp(appWithoutIpAddress.id)
      .withAgentInfo(_.copy(host = host)).withNetworking(Task.NetworkInfoList(
        networkWithOneIp1,
        networkWithOneIp2))
  }
  ...
  test(
    ...
    f.taskWithoutIp.effectiveIpAddress(f.appWithoutIpAddress) should
      equal(f.host)
  }
  ...
  test(
    ...
    val f = new Fixture
    f.taskWithOneIp.effectiveIpAddress(f.appWithIpAddress) should
      equal(f.ipString1)
  }
  ...
  test(
    ...
    val f = new Fixture
    f.taskWithOneIp.effectiveIpAddress(f.appWithoutIpAddress) should
      equal(f.host)
  }
  ...
  test(
    ...
    val f = new Fixture
    f.taskWithMultipleNetworkAndMultipleIPs.ipAddresses should
      equal(Seq(f.ipAddress1, f.ipAddress2))
  }
  ...
  test(
    ...
    val f = new Fixture
    f.taskWithMultipleNetworksAndOneIp.ipAddresses should
      equal(Seq(f.ipAddress1))
  }
