...
class TaskCreationHandlerAndUpdaterDelegateTest
  ...
  test("Created succeeds") {
    ...
    f.taskTrackerProbe.expectMsg(TaskTrackerActor.ForwardTaskOp(
      f.timeoutFromNow,
      task.taskId,
  ...
  test("StatusUpdate succeeds") {
    ...
    val update = TaskStatus.newBuilder()
      .setTaskId(TaskID.newBuilder().setValue(taskId)).buildPartial()
  ...
  test("StatusUpdate fails") {
    ...
    val update = TaskStatus.newBuilder().setTaskId(taskId.mesosTaskId)
      .buildPartial()
