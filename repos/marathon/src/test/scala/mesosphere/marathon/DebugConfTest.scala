...
class DebugConfTest extends MarathonSpec {
  ...
  test("metrics can be enabled") {
    val conf = MarathonTestHelper
      .makeConfig("--master", "127.0.0.1:5050", "--metrics")
    assert(conf.metrics())
  }

  test("the deprecated --enable_metrics is accepted") {
    val conf = MarathonTestHelper
      .makeConfig("--master", "127.0.0.1:5050", "--enable_metrics")
    assert(conf.metrics())
  }

  test("metrics can be disabled") {
    val conf = MarathonTestHelper
      .makeConfig("--master", "127.0.0.1:5050", "--disable_metrics")
    assert(!conf.metrics())
  }
}
