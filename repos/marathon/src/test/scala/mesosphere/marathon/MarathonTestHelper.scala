...
object MarathonTestHelper {
  ...
  def dummyTaskBuilder(appId: PathId) = MarathonTask
    .newBuilder()
  ...
  def dummyTaskProto(taskId: String) = MarathonTask
    .newBuilder()
    .setId(taskId)
    .setHost("host.some")
    .build()

  def mininimalTask(appId: PathId): Task = mininimalTask(
  ...
  def startingTaskProto(appId: PathId): Protos.MarathonTask = startingTaskProto(
    Task.Id.forApp(appId).idString)
  ...
  def stagedTaskProto(appId: PathId): Protos.MarathonTask = stagedTaskProto(
    Task.Id.forApp(appId).idString)
  def stagedTaskProto(
      taskId: String,
      appVersion: Timestamp = Timestamp(1),
  ...
  def runningTaskProto(appId: PathId): Protos.MarathonTask = runningTaskProto(
    Task.Id.forApp(appId).idString)
  ...
  def healthyTask(appId: PathId): Task = healthyTask(
    Task.Id.forApp(appId).idString)
  def healthyTask(taskId: String): Task =
    TaskSerializer.fromProto(healthyTaskProto(taskId))

  def healthyTaskProto(appId: PathId): Protos.MarathonTask = healthyTaskProto(
    Task.Id.forApp(appId).idString)
  ...
  def unhealthyTask(appId: PathId): Task = unhealthyTask(
    Task.Id.forApp(appId).idString)
