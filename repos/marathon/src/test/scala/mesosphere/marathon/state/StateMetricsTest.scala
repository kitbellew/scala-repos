...
class StateMetricsTest
  ...
  private[this] def testCrashingCall(read: Boolean): Unit = {
    ...
    metrics.metrics.registry.getMeters.get(untested.errorMeterName)
      .getCount should be(0)
    metrics.metrics.registry.getMeters.get(tested.errorMeterName)
      .getCount should be(1)
    metrics.metrics.registry.getMeters.get(untested.requestsMeterName)
      .getCount should be(0)
    metrics.metrics.registry.getMeters.get(tested.requestsMeterName)
      .getCount should be(1)
    metrics.metrics.registry.getHistograms.get(untested.durationHistogramName)
      .getCount should be(0)
  ...
  private[this] def testDelayedSuccesfulFuture(read: Boolean): Unit = {
    ...
    metrics.metrics.registry.getHistograms.get(tested.durationHistogramName)
      .getCount should be(0)
    metrics.metrics.registry.getHistograms.get(tested.durationHistogramName)
      .getSnapshot.getMax should be(0)
