...
class ContainerTest extends MarathonSpec with Matchers {
  ...
  test("ToProto") {
    ...
    assert(
      f.container.docker.get.forcePullImage ==
        proto.getDocker.getForcePullImage)
    ...
    assert(
      f.container2.docker.get.portMappings ==
        Some(portMappings.map(PortMappingSerializer.fromProto)))
    assert(proto2.getDocker.hasForcePullImage)
    assert(
      f.container2.docker.get.forcePullImage ==
        proto2.getDocker.getForcePullImage)
    ...
    assert(
      f.container3.docker.get.parameters.map(_.key) ==
        proto3.getDocker.getParametersList.asScala.map(_.getKey))
    assert(
      f.container3.docker.get.parameters.map(_.value) ==
        proto3.getDocker.getParametersList.asScala.map(_.getValue))
    assert(proto3.getDocker.hasForcePullImage)
    assert(
      f.container3.docker.get.forcePullImage ==
        proto3.getDocker.getForcePullImage)
  ...
  test("ToMesos") {
    ...
    val expectedPortMappings = Seq(
      mesos.ContainerInfo.DockerInfo.PortMapping.newBuilder
        .setContainerPort(8080).setHostPort(32001).setProtocol("tcp").build,
      mesos.ContainerInfo.DockerInfo.PortMapping.newBuilder
        .setContainerPort(8081).setHostPort(32002).setProtocol("udp").build
    )
  ...
  test("ConstructFromProto") {
    ...
    val containerInfo = Protos.ExtendedContainerInfo.newBuilder
      ...
      .setDocker(DockerSerializer.toProto(f.container.docker.get)).build
    ...
    val containerInfo2 = Protos.ExtendedContainerInfo.newBuilder
      .setType(mesos.ContainerInfo.Type.DOCKER)
      .setDocker(DockerSerializer.toProto(f.container2.docker.get)).build
    ...
    val containerInfo3 = Protos.ExtendedContainerInfo.newBuilder
      .setType(mesos.ContainerInfo.Type.DOCKER)
      .setDocker(DockerSerializer.toProto(f.container3.docker.get)).build
  ...
  test("""FromJSON with Mesos ContainerInfo should parse successfully""") {
    val f = new Fixture
    val appDef = Json.parse(f.mesosContainerWithPersistentVolumeJsonStr)
      .as[AppDefinition]
