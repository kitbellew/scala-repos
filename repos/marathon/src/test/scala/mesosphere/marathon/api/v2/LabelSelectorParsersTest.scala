...
class LabelSelectorParsersTest extends MarathonSpec with Matchers {
  ...
  test("A valid existence label query can be parsed") {
    ...
    existence
      ...
      true)
    existence
      .matches(AppDefinition(labels = Map("none" -> "one"))) should be(false)
  }
  ...
  test("A valid combined label query can be parsed") {
    ...
    combined.matches(
      AppDefinition(labels = Map(
        "foo" -> "one",
        "bla" -> "four",
        "existence" -> "true"))) should be(true)
    combined
      .matches(AppDefinition(labels = Map("foo" -> "one"))) should be(false)
    combined
      .matches(AppDefinition(labels = Map("bla" -> "four"))) should be(false)
  }
  ...
  test(
    ...
    combined.matches(
      AppDefinition(labels = Map(
        ...
        "###" -> "&&&"))) should be(true)
    combined
      .matches(AppDefinition(labels = Map("^^^" -> "---"))) should be(false)
    combined
      .matches(AppDefinition(labels = Map("###" -> "four"))) should be(false)
  }
