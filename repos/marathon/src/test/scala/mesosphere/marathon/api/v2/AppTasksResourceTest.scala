...
class AppTasksResourceTest
  ...
  test("deleteMany") {
    ...
    groupManager.app(appId.toRootPath) returns
      Future.successful(Some(AppDefinition(appId.toRootPath)))

    val response = appsTaskResource
      .deleteMany(appId, host, scale = false, force = false, auth.request)
    response.getStatus shouldEqual 200
    JsonTestHelper.assertThatJsonString(response.getEntity.asInstanceOf[String])
  ...
  test("get tasks") {
    ...
    healthCheckManager.statuses(appId) returns
      Future.successful(collection.immutable.Map.empty)
    groupManager.app(appId) returns
      Future.successful(Some(AppDefinition(appId)))
    ...
    JsonTestHelper.assertThatJsonString(response.getEntity.asInstanceOf[String])
      .correspondsToJsonOf(Json.obj(
        "tasks" -> Seq(task1, task2).map(toEnrichedTask)))
  }
  ...
  test(
    ...
    groupManager.app("/app".toRootPath) returns
      Future.successful(Some(AppDefinition("/app".toRootPath)))
  ...
  test(
    ...
    groupManager.group(groupPath) returns
      Future.successful(Some(Group(groupPath)))
