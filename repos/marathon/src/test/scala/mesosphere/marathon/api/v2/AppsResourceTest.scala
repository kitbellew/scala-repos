...
class AppsResourceTest
  ...
  test("Replace an existing application") {
    ...
    val response = appsResource
      .replace(app.id.toString, body, false, auth.request)
  ...
  test(
    ...
    When("The application is updated")
    val response = appsResource
      .replace(app.id.toString, body, force = false, auth.request)
  ...
  test("access without authentication is denied") {
    ...
    val replaceMultiple = appsResource
      .replaceMultiple(false, s"[$app]".getBytes("UTF-8"), req)
    Then("we receive a NotAuthenticated response")
