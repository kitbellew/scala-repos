...
class DeploymentPlanRevertTest
  ...
  private[this] def testWithConcurrentChange(
      originalBeforeChanges: Group,
      changesBeforeTest: Deployment*)(deployments: Deployment*)(
    ...
    test(s"Reverting ${firstDeployment.name} after deploying ${deployments.tail
      .map(_.name).mkString(", ")}") {
      Given("an existing group with apps")
  ...
  private[this] def removeGroup(groupId: String) =
    Deployment(s"remove group '$groupId'", _.remove(groupId.toRootPath))
  ...
  private[this] def changeGroupDependencies(
    ...
    def setDependencies(group: Group): Group =
      group.copy(dependencies = group.dependencies ++ addedIds -- removedIds)
