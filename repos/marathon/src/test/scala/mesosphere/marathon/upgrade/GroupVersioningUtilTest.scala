...
class GroupVersioningUtilTest
  ...
  val nestedApp = Group.empty.copy(
    groups = Set(Group(
      id = PathId("/nested"),
      apps = Set(AppDefinition(PathId("/nested/app"), cmd = Some("sleep 123"))),
      version = Timestamp(2))),
    version = Timestamp(2)
  )
  ...
  val nestedAppUpdated = Group.empty.copy(
    groups = Set(Group(
      id = PathId("/nested"),
      apps = Set(AppDefinition(PathId("/nested/app"), cmd = Some("sleep 234"))),
      version = Timestamp(2))),
    version = Timestamp(2)
  )
  ...
  test("A scaled app should get proper versionInfo") {
    ...
    Then("The timestamp of the app and groups are updated appropriately")
    def update(maybeApp: Option[AppDefinition]): AppDefinition =
      maybeApp.map(_.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(0))
        .withScaleOrRestartChange(Timestamp(10)))).get
    updated.toString should be(
      nestedAppScaled.updateApp(PathId("/nested/app"), update, Timestamp(10))
        .toString)
  }
  ...
  test("A updated app should get proper versionInfo") {
    ...
    def update(maybeApp: Option[AppDefinition]): AppDefinition =
      maybeApp
        .map(_.copy(versionInfo = VersionInfo.forNewConfig(Timestamp(10)))).get
    updated.toString should be(
      nestedAppUpdated.updateApp(PathId("/nested/app"), update, Timestamp(10))
        .toString)
  }
}
