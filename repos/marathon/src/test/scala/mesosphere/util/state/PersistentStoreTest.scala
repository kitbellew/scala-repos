...
trait PersistentStoreTest
  ...
  test("Multiple creates should create only the first time") {
    ...
    entity should be('empty)
    persistentStore.create("foo", "Hello".getBytes).futureValue.bytes should
      be("Hello".getBytes)
  ...
  test("Multiple updates will update only with correct version") {
    ...
    persistentStore.update(read.get.withNewContent("Hello again".getBytes))
      .futureValue.bytes should be("Hello again".getBytes)
    whenReady(
      persistentStore.update(read2.get.withNewContent("Will be None".getBytes))
