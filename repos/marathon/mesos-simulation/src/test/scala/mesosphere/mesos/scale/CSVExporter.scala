...
object CSVExporter {
  ...
  def toRows(directory: File): Seq[Row] = {
    val metrics = directory.listFiles().toSeq.filter(_.getName.endsWith("json"))
      .sortBy(_.getName).map { f => MetricsFormat.readMetric(f.toURI.toURL) }
    ...
    Seq(
      countr.map { name =>
        Row(
          name + ".count",
          metrics.map(_.counters.find(_.name == name).map(_.count.toDouble)))
      },
      gauges.map { name =>
        Row(
          name + ".value",
          metrics.map(_.gauges.find(_.name == name).map(_.value)))
      },
      histos.map { name =>
        Row(
          name + ".count",
          metrics.map(_.histograms.find(_.name == name).map(_.count.toDouble)))
      },
      histos.map { name =>
        Row(
          name + ".mean",
          metrics.map(_.histograms.find(_.name == name).map(_.mean)))
      },
      histos.map { name =>
        Row(
          name + ".p50",
          metrics.map(_.histograms.find(_.name == name).map(_.p50)))
      },
      histos.map { name =>
        Row(
          name + ".p75",
          metrics.map(_.histograms.find(_.name == name).map(_.p75)))
      },
      histos.map { name =>
        Row(
          name + ".p95",
          metrics.map(_.histograms.find(_.name == name).map(_.p95)))
      },
      histos.map { name =>
        Row(
          name + ".p98",
          metrics.map(_.histograms.find(_.name == name).map(_.p98)))
      },
      histos.map { name =>
        Row(
          name + ".p99",
          metrics.map(_.histograms.find(_.name == name).map(_.p99)))
      },
      histos.map { name =>
        Row(
          name + ".p999",
          metrics.map(_.histograms.find(_.name == name).map(_.p999)))
      },
      histos.map { name =>
        Row(
          name + ".min",
          metrics.map(_.histograms.find(_.name == name).map(_.min)))
      },
      histos.map { name =>
        Row(
          name + ".max",
          metrics.map(_.histograms.find(_.name == name).map(_.max)))
      },
      histos.map { name =>
        Row(
          name + ".stddev",
          metrics.map(_.histograms.find(_.name == name).map(_.stddev)))
      },
      meters.map { name =>
        Row(
          name + ".count",
          metrics.map(_.meters.find(_.name == name).map(_.count.toDouble)))
      },
      meters.map { name =>
        Row(
          name + ".mean_rate",
          metrics.map(_.meters.find(_.name == name).map(_.mean_rate)))
      },
      meters.map { name =>
        Row(
          name + ".m1",
          metrics.map(_.meters.find(_.name == name).map(_.m1_rate)))
      },
      meters.map { name =>
        Row(
          name + ".m5",
          metrics.map(_.meters.find(_.name == name).map(_.m5_rate)))
      },
      meters.map { name =>
        Row(
          name + ".m15",
          metrics.map(_.meters.find(_.name == name).map(_.m15_rate)))
      },
      timers.map { name =>
        Row(
          name + ".count",
      ...
      timers.map { name =>
        Row(
          name + ".meanRate",
          metrics.map(_.timers.find(_.name == name).map(_.mean_rate)))
      },
      timers.map { name =>
        Row(
          name + ".m1_rate",
          metrics.map(_.timers.find(_.name == name).map(_.m1_rate)))
      },
      timers.map { name =>
        Row(
          name + ".m5_rate",
          metrics.map(_.timers.find(_.name == name).map(_.m5_rate)))
      },
      timers.map { name =>
        Row(
          name + ".m15_rate",
