  ...
  s"Cluster sharding ($mode)" must {
    ...
    "use second node" in within(20 seconds) {
      ...
      runOn(first) {
        ...
        expectMsg(3)
        lastSender
          .path should ===(node(second) / "user" / "counterRegion" / "2" / "2")
        ...
        expectMsg(1)
        lastSender
          .path should ===(node(second) / "user" / "counterRegion" / "0" / "12")
      }
      ...
      runOn(second) {
        ...
        region ! GetCurrentRegions
        expectMsg(
          CurrentRegions(Set(Cluster(system).selfAddress, node(first).address)))
      }
    ...
    "use third and fourth node" in within(15 seconds) {
      ...
      runOn(first) {
        ...
        expectMsg(11)
        lastSender
          .path should ===(node(third) / "user" / "counterRegion" / "3" / "3")

        region ! EntityEnvelope(4, Increment)
        region ! Get(4)
        expectMsg(21)
        lastSender
          .path should ===(node(fourth) / "user" / "counterRegion" / "4" / "4")
      }
      enterBarrier("first-update")
