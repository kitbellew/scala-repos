...
abstract class ClusterShardingSpec(config: ClusterShardingSpecConfig)
  ...
  s"Cluster sharding ($mode)" must {
    ...
    "failover shards on crashed node" in
      within(30 seconds) {
    ...
    "recover coordinator state after coordinator crash" in
      within(60 seconds) {
        join(fifth, fourth)
    ...
    "rebalance to nodes with less shards" in
      within(60 seconds) {
        runOn(fourth) {
          for (n ← 1 to 10) {
  ...
  "easy to use with extensions" in
    within(50.seconds) {
      ...
      runOn(fifth) {
        //#counter-usage
        val counterRegion: ActorRef = ClusterSharding(system)
          .shardRegion("Counter")
  ...
  "easy API for starting" in
    within(50.seconds) {
      runOn(first) {
        ...
        val counterRegionViaGet: ActorRef = ClusterSharding(system)
          .shardRegion("ApiTest")
  ...
  "Persistent Cluster Shards" must {
    "recover entities upon restart" in
      within(50.seconds) {
        ...
        runOn(fourth) {
          ...
          val secondCounter1 = system
            .actorSelection(lastSender.path.parent / "1")
    ...
    "permanently stop entities which passivate" in
      within(15.seconds) {
        runOn(third, fourth, fifth) {
          persistentRegion
        }
    ...
    "restart entities which stop without passivating" in
      within(50.seconds) {
        runOn(third, fourth) {
          persistentRegion
        }
    ...
    "be migrated to new regions upon region failure" in
      within(15.seconds) {
        ...
        runOn(fourth) {
          ...
          lastSender.path should
            ===(
              node(third) / "user" / "AutoMigrateRememberRegionTestRegion" /
                "1" / "1")
    ...
    "ensure rebalance restarts shards" in
      within(50.seconds) {
        runOn(fourth) {
          for (i ← 2 to 12) {
        ...
        runOn(fifth) {
          awaitAssert {
            var count = 0
            for (n ← 2 to 12) {
              val entity = system.actorSelection(
                rebalancingPersistentRegion.path /
                  (n % 12).toString / n.toString)
              entity ! Identify(n)
