...
class Hakker(name: String, chair: Int) extends Actor {
  ...
  def waiting_for(
    ...
    case Busy(chopstick) =>
      ...
      self ! Eat
    case Identify =>
      identify("Waiting for Chopstick")
  }
  ...
  def managementEvents: Receive = {
    case state: CurrentClusterState =>
      state.leader foreach updateTable
    case LeaderChanged(Some(leaderAddress)) =>
      updateTable(leaderAddress)
  ...
  def updateTable(leaderAdress: Address): Unit = {
    ...
    context.actorSelection(RootActorPath(leaderAdress) / "user" / "table") !
      chair
  }
