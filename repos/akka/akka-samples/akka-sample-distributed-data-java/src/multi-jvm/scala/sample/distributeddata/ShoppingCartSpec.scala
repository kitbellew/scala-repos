  ...
  "Demo of a replicated shopping cart" must {
    ...
    "handle updates directly after start" in
      within(15.seconds) {
        runOn(node2) {
          shoppingCart !
            new ShoppingCart.AddItem(new LineItem("1", "Apples", 2))
          shoppingCart !
            new ShoppingCart.AddItem(new LineItem("2", "Oranges", 3))
        }
        ...
        awaitAssert {
          ...
          cart.items.asScala.toSet should
            be(
              Set(
                new LineItem("1", "Apples", 2),
    ...
    "handle updates from different nodes" in
      within(5.seconds) {
        runOn(node2) {
          shoppingCart !
            new ShoppingCart.AddItem(new LineItem("1", "Apples", 5))
          shoppingCart ! new ShoppingCart.RemoveItem("2")
        }
        runOn(node3) {
          shoppingCart !
            new ShoppingCart.AddItem(new LineItem("3", "Bananas", 4))
        }
        enterBarrier("updates-done")
