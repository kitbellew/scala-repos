...
class ORSetSpec extends WordSpec with Matchers {
  ...
  "A ORSet" must {
    ...
    "be able to have its user set correctly merged with another ORSet with overlapping user sets" in {
      ...
      val c1 = ORSet().add(node1, user1).add(node1, user2).add(node1, user3)
        .remove(node1, user1).remove(node1, user3)
      ...
      // set 2
      val c2 = ORSet().add(node2, user1).add(node2, user2).add(node2, user3)
        .add(node2, user4).remove(node2, user3)
  ...
  "ORSet unit test" must {
    ...
    "verify mergeCommonKeys" in {
      ...
      ORSet.mergeCommonKeys(commonKeys, thisSet, thatSet) should
        be(expectedDots)
    }
    ...
    "verify mergeDisjointKeys" in {
      ...
      ORSet.mergeDisjointKeys(keys, elements, vvector, acc) should
        be(expectedDots)
    }
