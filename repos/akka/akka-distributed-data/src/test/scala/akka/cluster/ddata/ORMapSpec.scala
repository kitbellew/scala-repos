...
class ORMapSpec extends WordSpec with Matchers {
  ...
  "A ORMap" must {
    ...
    "be able to remove entry" in {
      val m = ORMap().put(node1, "a", GSet() + "A").put(
        node1,
        "b",
        GSet() + "B").remove(node1, "a")
    ...
    "be able to have its entries correctly merged with another ORMap with overlapping entries" in {
      val m1 = ORMap().put(node1, "a", GSet() + "A1").put(
        node1,
        "b",
        GSet() + "B1").remove(node1, "a").put(node1, "d", GSet() + "D1")
      val m2 = ORMap().put(node2, "c", GSet() + "C2").put(
        node2,
        "a",
        GSet() + "A2").put(node2, "b", GSet() + "B2").remove(node2, "b").put(
        node2,
        "d",
        GSet() + "D2")
    ...
    "illustrate the danger of using remove+put to replace an entry" in {
      val m1 = ORMap.empty.put(node1, "a", GSet.empty + "A").put(
        node1,
        "b",
        GSet.empty + "B")
    ...
    "be able to update entry" in {
      val m1 =
        ORMap.empty[ORSet[String]].put(node1, "a", ORSet.empty.add(node1, "A"))
          .put(
            node1,
            "b",
    ...
    "be able to update ORSet entry with remove+put" in {
      val m1 = ORMap.empty[ORSet[String]].put(
        node1,
        "a",
        ORSet.empty.add(node1, "A01"))
    ...
    "be able to update ORSet entry with remove -> merge -> put" in {
      ...
      val m5 =
        merged2.updated(node2, "c", ORSet.empty[String])(_.add(node2, "C2"))
