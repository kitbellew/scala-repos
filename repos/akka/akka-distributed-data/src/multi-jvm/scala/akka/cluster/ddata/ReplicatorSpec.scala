...
class ReplicatorSpec
  ...
  "replicate values to new node" in {
    ...
    runOn(second) {
      ...
      val c = changedProbe.expectMsgPF() { case c @ Changed(KeyA) ⇒
        c.get(KeyA)
      }
      c.value should be(6)
    }
  ...
  "be replicated after succesful update" in {
    ...
    runOn(first) {
      ...
      changedProbe.expectMsgPF() { case c @ Changed(KeyC) ⇒
        c.get(KeyC).value
      } should be(30)
    ...
    runOn(second) {
      ...
      expectMsgPF() { case g @ GetSuccess(KeyZ, _) ⇒
        g.get(KeyZ).value
      } should be(30)
    }
      ...
      within(10.seconds) {
        awaitAssert {
          for (n ← 1 to 30) {
            ...
            expectMsgPF() { case g @ GetSuccess(KeyDn, _) ⇒
              g.get(KeyDn)
            }.value should be(n)
          }
        }
  ...
  "support majority quorum write and read with 3 nodes with 1 unreachable" in {
    ...
    runOn(first) {
      ...
      val c152 = probe3.expectMsgPF() { case g @ GetSuccess(KeyE, _) ⇒
        g.get(KeyE)
      }
    ...
    runOn(second) {
      ...
      val c155 = probe5.expectMsgPF() { case g @ GetSuccess(KeyE, _) ⇒
        g.get(KeyE)
      }
  ...
  "read-repair happens before GetSuccess" in {
    ...
    runOn(second) {
      ...
      expectMsgPF() { case g @ GetSuccess(KeyG, _) ⇒
        g.get(KeyG).elements
      } should be(Set("a", "b"))
  ...
  "check that a remote update and a local update both cause a change event to emit with the merged data" in {
    ...
    runOn(second) {
      ...
      changedProbe.expectMsgPF() { case c @ Changed(KeyH) ⇒
        c.get(KeyH).entries
  ...
  "avoid duplicate change events for same data" in {
    ...
    within(5.seconds) { // gossip to third
      changedProbe.expectMsgPF() { case c @ Changed(KeyI) ⇒
        c.get(KeyI).elements
      } should be(Set("a"))
    }
