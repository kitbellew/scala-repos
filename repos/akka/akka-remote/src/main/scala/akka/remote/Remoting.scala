...
private[remote] object Remoting {
  ...
  def localAddressForRemote(
    ...
    transportMapping.get(remote.protocol) match {
      ...
      case None ⇒
        throw new RemoteTransportException(
          s"No transport is loaded for protocol: [${remote.protocol}], available protocols: [${transportMapping
            .keys.mkString(", ")}]",
          null)
    }
  }
  ...
  private[Remoting] class TransportSupervisor
    ...
    override def supervisorStrategy =
      OneForOneStrategy() { case NonFatal(e) ⇒ Restart }
    ...
    endpointManager match {
      case None ⇒
        ...
        try {
          ...
          val transports: Seq[(AkkaProtocolTransport, Address)] = Await
            .result(addressesPromise.future, StartupTimeout.duration)
    ...
    def unregisterEndpoint(endpoint: ActorRef): Unit =
      if (isWritable(endpoint)) {
        ...
        addressToWritable.get(address) match {
          case Some(policy)
              if policy
                .isTombstone ⇒ // There is already a tombstone directive, leave it there
...
private[remote] class EndpointManager(conf: Config, log: LoggingAdapter)
  ...
  override val supervisorStrategy = OneForOneStrategy(loggingEnabled = false) {
    case e @ InvalidAssociation(
      ...
      keepQuarantinedOr(remoteAddress) {
        ...
        endpoints
          .markAsFailed(sender(), Deadline.now + settings.RetryGateClosedFor)
      }
      ...
      case ia @ InboundAssociation(handle: AkkaProtocolHandle) ⇒
        endpoints.readOnlyEndpointFor(handle.remoteAddress) match {
          case Some((endpoint, _)) ⇒
            ...
            endpoints
              .writableEndpointWithPolicyFor(handle.remoteAddress) match {
  ...
  private def listens: Future[Seq[
    ...
    val transports: Seq[AkkaProtocolTransport] =
      for ((fqn, adapters, config) ← settings.Transports) yield {
        ...
        //   Driver
        val driver = extendedSystem.dynamicAccess.createInstanceFor[Transport](
          fqn,
          args).recover({
  ...
  private def removePendingReader(
    ...
    if (pendingReadHandoffs.get(takingOverFrom)
  ...
  private def createEndpoint(
    ...
    require(
      transportMapping contains localAddress,
