...
private[remote] class EndpointManager(conf: Config, log: LoggingAdapter)
  ...
  val pruneTimerCancellable: Cancellable = context
    .system
    .scheduler
    ...
    case ShutdownAndFlush ⇒
      ...
      (
        for {
          ...
          flushStatus ←
            shutdownAll(endpoints.allEndpoints)(
      ...
      pendingReadHandoffs
        .valuesIterator foreach (_.disassociate(AssociationHandle.Shutdown))
