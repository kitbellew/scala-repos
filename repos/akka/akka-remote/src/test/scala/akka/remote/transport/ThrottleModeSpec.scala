...
class ThrottleModeSpec extends AkkaSpec {
  ...
  "ThrottleMode" must {
    ...
    "in tokenbucket mode allow consuming tokens up to capacity" in {
      ...
      val (bucket2, success2) =
        bucket1.tryConsumeTokens(nanoTimeOfSend = 0L, 40)
      bucket2 should ===(TokenBucket(100, 100, 0, 50))
      success2 should ===(true)

      val (bucket3, success3) =
        bucket2.tryConsumeTokens(nanoTimeOfSend = 0L, 50)
    ...
    "accurately replenish tokens" in {
      ...
      val (bucket2, success2) =
        bucket1.tryConsumeTokens(nanoTimeOfSend = halfSecond, 0)
      bucket2 should ===(TokenBucket(100, 100, halfSecond, 50))
      success2 should ===(true)

      val (bucket3, success3) =
        bucket2.tryConsumeTokens(nanoTimeOfSend = 2 * halfSecond, 0)
      bucket3 should ===(TokenBucket(100, 100, 2 * halfSecond, 100))
      success3 should ===(true)

      val (bucket4, success4) =
        bucket3.tryConsumeTokens(nanoTimeOfSend = 3 * halfSecond, 0)
    ...
    "accurately interleave replenish and consume" in {
      ...
      val (bucket2, success2) =
        bucket1.tryConsumeTokens(nanoTimeOfSend = halfSecond, 60)
      bucket2 should ===(TokenBucket(100, 100, halfSecond, 0))
      success2 should ===(true)

      val (bucket3, success3) =
        bucket2.tryConsumeTokens(nanoTimeOfSend = 2 * halfSecond, 40)
      bucket3 should ===(TokenBucket(100, 100, 2 * halfSecond, 10))
      success3 should ===(true)

      val (bucket4, success4) =
        bucket3.tryConsumeTokens(nanoTimeOfSend = 3 * halfSecond, 70)
    ...
    "allow oversized packets through by loaning" in {
      ...
      val (bucket2, success2) =
        bucket1.tryConsumeTokens(nanoTimeOfSend = halfSecond, 110)
      bucket2 should ===(TokenBucket(100, 100, halfSecond, -40))
      success2 should ===(true)

      val (bucket3, success3) =
        bucket2.tryConsumeTokens(nanoTimeOfSend = 2 * halfSecond, 20)
      bucket3 should ===(TokenBucket(100, 100, halfSecond, -40))
      success3 should ===(false)

      val (bucket4, success4) =
        bucket3.tryConsumeTokens(nanoTimeOfSend = 3 * halfSecond, 20)
