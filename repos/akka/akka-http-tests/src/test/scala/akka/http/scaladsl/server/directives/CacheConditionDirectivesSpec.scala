...
class CacheConditionDirectivesSpec extends RoutingSpec {
  ...
  "the `conditional` directive" should {
    ...
    def taggedAndTimestamped =
      conditional(tag, timestamp) {
        completeOk
      }
    def weak =
      conditional(tag.copy(weak = true), timestamp) {
        completeOk
      }

    "return OK for new resources" in {
      Get() ~> taggedAndTimestamped ~>
        check {
    ...
    "return OK for non-matching resources" in {
      Get() ~> `If-None-Match`(EntityTag("old")) ~> taggedAndTimestamped ~>
        check {
          status shouldEqual OK
          headers should contain theSameElementsAs (responseHeaders)
        }
      Get() ~> `If-Modified-Since`(timestamp - 1000) ~> taggedAndTimestamped ~>
        check {
          status shouldEqual OK
          headers should contain theSameElementsAs (responseHeaders)
        }
      Get() ~> `If-None-Match`(EntityTag("old")) ~>
        `If-Modified-Since`(timestamp - 1000) ~> taggedAndTimestamped ~>
        check {
          status shouldEqual OK
          headers should contain theSameElementsAs (responseHeaders)
        }
    ...
    "ignore If-Modified-Since if If-None-Match is defined" in {
      Get() ~> `If-None-Match`(tag) ~> `If-Modified-Since`(timestamp - 1000) ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
        }
      Get() ~> `If-None-Match`(EntityTag("old")) ~> ifModifiedSince ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual OK
        }
    ...
    "return PreconditionFailed for matched but unsafe resources" in {
      Put() ~> `If-None-Match`(tag) ~> ifModifiedSince ~>
        taggedAndTimestamped ~>
        check {
    ...
    "return NotModified for matching resources" in {
      Get() ~> `If-None-Match`.`*` ~> ifModifiedSince ~> taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
        }
      Get() ~> `If-None-Match`(tag) ~> ifModifiedSince ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
      ...
      Get() ~> `If-None-Match`(tag.copy(weak = true)) ~> ifModifiedSince ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
        }
      Get() ~> `If-None-Match`(tag, EntityTag("some"), EntityTag("other")) ~>
        ifModifiedSince ~> taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
        }
    ...
    "return NotModified when only one matching header is set" in {
      Get() ~> `If-None-Match`.`*` ~> taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
        }
      Get() ~> `If-None-Match`(tag) ~> taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
        }
      Get() ~> ifModifiedSince ~> taggedAndTimestamped ~>
        check {
          status shouldEqual NotModified
          headers should contain theSameElementsAs (responseHeaders)
        }
    ...
    "return normally for matching If-Match/If-Unmodified" in {
      Put() ~> `If-Match`.`*` ~> taggedAndTimestamped ~>
        check {
          status shouldEqual OK
          headers should contain theSameElementsAs (responseHeaders)
        }
      Put() ~> `If-Match`(tag) ~> taggedAndTimestamped ~>
        check {
          status shouldEqual OK
          headers should contain theSameElementsAs (responseHeaders)
        }
      Put() ~> ifUnmodifiedSince ~> taggedAndTimestamped ~>
        check {
          status shouldEqual OK
          headers should contain theSameElementsAs (responseHeaders)
        }
    ...
    "return PreconditionFailed for non-matching If-Match/If-Unmodified" in {
      Put() ~> `If-Match`(EntityTag("old")) ~> taggedAndTimestamped ~>
        check {
          status shouldEqual PreconditionFailed
          headers shouldEqual Nil
        }
      Put() ~> `If-Unmodified-Since`(timestamp - 1000) ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual PreconditionFailed
          headers shouldEqual Nil
    ...
    "ignore If-Unmodified-Since if If-Match is defined" in {
      Put() ~> `If-Match`(tag) ~> `If-Unmodified-Since`(timestamp - 1000) ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual OK
        }
      Put() ~> `If-Match`(EntityTag("old")) ~> ifModifiedSince ~>
        taggedAndTimestamped ~>
        check {
          status shouldEqual PreconditionFailed
        }
