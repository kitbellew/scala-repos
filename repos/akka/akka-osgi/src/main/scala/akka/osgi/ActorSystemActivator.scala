...
abstract class ActorSystemActivator extends BundleActivator {
  ...
  def addLogServiceListener(context: BundleContext, system: ActorSystem) {
    ...
    Option(context.getServiceReference(classOf[LogService].getName))
      .foreach(x ⇒ {
        logServiceListner
          .serviceChanged(new ServiceEvent(ServiceEvent.REGISTERED, x))
      })
  }
  ...
  def serviceForReference[T](
      ...
      reference: ServiceReference[_]): T =
