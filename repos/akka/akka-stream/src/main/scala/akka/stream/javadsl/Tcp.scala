...
class Tcp(system: ExtendedActorSystem) extends akka.actor.Extension {
  ...
  def bind(
    ...
    Source.fromGraph(
      delegate.bind(
        ...
        halfClose,
        idleTimeout).map(new IncomingConnection(_))
  ...
  def bind(
    ...
    Source.fromGraph(
      delegate.bind(interface, port).map(new IncomingConnection(_))
        .mapMaterializedValue(_.map(new ServerBinding(_))(ec).toJava))
  ...
  def outgoingConnection(
    ...
    Flow.fromGraph(
      delegate.outgoingConnection(
        ...
        halfClose,
        connectTimeout,
        idleTimeout).mapMaterializedValue(
        _.map(new OutgoingConnection(_))(ec).toJava))
