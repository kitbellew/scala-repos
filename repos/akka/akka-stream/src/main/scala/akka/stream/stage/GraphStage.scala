...
abstract class GraphStageLogic private[stream] (
  ...
  final protected def setHandler(in: Inlet[_], handler: InHandler): Unit = {
    ...
    if (_interpreter != null)
      _interpreter.setHandler(conn(in), handler)
  }

  /**
    * Assign callbacks for linear stage for both [[Inlet]] and [[Outlet]]
    */
  final protected def setHandlers(
      in: Inlet[_],
      out: Outlet[_],
  ...
  final protected def setHandler(out: Outlet[_], handler: OutHandler): Unit = {
    ...
    if (_interpreter != null)
      _interpreter.setHandler(conn(out), handler)
  }
  ...
  final protected def emitMultiple[T](
      out: Outlet[T],
      elems: immutable.Iterable[T]): Unit = emitMultiple(out, elems, DoNothing)
  ...
  final protected def emitMultiple[T](
      out: Outlet[T],
      elems: java.util.Iterator[T]): Unit = {
    import collection.JavaConverters._
  ...
  final protected def emitMultiple[T](
      out: Outlet[T],
      elems: Iterator[T]): Unit = emitMultiple(out, elems, DoNothing)
  ...
  private class EmittingIterator[T](
      _out: Outlet[T],
      elems: Iterator[T],
      _previous: OutHandler,
      _andThen: () ⇒ Unit)
