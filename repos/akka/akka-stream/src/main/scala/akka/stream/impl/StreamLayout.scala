...
object StreamLayout {
  ...
  final def validate(
    ...
    if (!isSealed && (inset -- allIn).nonEmpty)
      problems ::= s"foreign inlets ${ins(inset -- allIn)}"
    if (!isSealed && (outset -- allOut).nonEmpty)
      problems ::= s"foreign outlets ${outs(outset -- allOut)}"
    ...
    final def wire(from: OutPort, to: InPort): Module = {
      ...
      require(
        ...
        if (downstreams.contains(from))
          ...
          s"The output port [$from] is not part of the underlying graph."
      )
      require(
        inPorts(to),
      ...
      CompositeModule(
        ...
        Transform(
          ...
          if (this.isSealed)
            ...
            this.materializedValueComputation),
        if (this.isSealed)
          Attributes.none
        else
          attributes
    ...
    def compose[A, B, C](that: Module, f: (A, B) ⇒ C): Module = {
      ...
      val matCompRight =
        if (that.isSealed)
          ...
          that.materializedValueComputation

      val mat = {
        val comp =
          if (f == scaladsl.Keep.left) {
            if (IgnorableMatValComp(matCompRight))
              matCompLeft
            else
              null
          } else if (f == scaladsl.Keep.right) {
            if (IgnorableMatValComp(matCompLeft))
              matCompRight
            else
              null
          } else
  ...
  final case class CopiedModule(
    ...
    override def withAttributes(attr: Attributes): Module =
      if (attr ne attributes)
        this.copy(attributes = attr)
      else
        this
...
private[stream] final class VirtualProcessor[T]
  ...
  override def subscribe(s: Subscriber[_ >: T]): Unit = {
    ...
    if (s == null) {
      ...
      rec(s.asInstanceOf[Subscriber[Any]])
  }
  ...
  override final def onComplete(): Unit =
    get() match {
      case null =>
        if (!compareAndSet(null, EmptyPublisher))
          onComplete()
      case s: Subscription =>
        if (!compareAndSet(s, EmptyPublisher))
          onComplete()
      case Both(s) =>
        set(Inert)
        tryOnComplete(s)
...
private[stream] abstract class MaterializerSession(
    val topLevel: StreamLayout.Module,
  ...
  protected def materializeModule(
    ...
    while (!matValSrc.isEmpty) {
      ...
      if (MaterializerSession.Debug)
        println(s"  delayed computation of $node")
  ...
  private def resolveMaterialized(
      matNode: MaterializedValueNode,
  ...
  final protected def assignPort(
      in: InPort,
      subscriberOrVirtual: AnyRef): Unit = {
