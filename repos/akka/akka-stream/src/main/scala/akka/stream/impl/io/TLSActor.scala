...
private[akka] class TLSActor(
  ...
  class ChoppingBlock(idx: Int, name: String) extends TransferState {
    override def isReady: Boolean =
      buffer.nonEmpty || inputBunch.isPending(idx) || inputBunch.isDepleted(idx)
  ...
  val bidirectional =
    TransferPhase(outbound || inbound) { () ⇒
      if (tracing)
  ...
  val flushingOutbound =
    TransferPhase(outboundHalfClosed) { () ⇒
      if (tracing)
  ...
  val outboundClosed =
    TransferPhase(outboundHalfClosed || inbound) { () ⇒
      if (tracing)
  ...
  val inboundClosed =
    TransferPhase(outbound || inboundHalfClosed) { () ⇒
      if (tracing)
