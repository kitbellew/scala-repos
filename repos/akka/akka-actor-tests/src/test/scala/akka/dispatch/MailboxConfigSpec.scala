  ...
  def testEnqueueDequeue(
    ...
    within(10 seconds) {
      ...
      EventFilter.warning(
        ...
        def createConsumer: Future[Vector[Envelope]] =
          spawn {
...
class SingleConsumerOnlyMailboxVerificationSpec
  ...
  "A SingleConsumerOnlyMailbox" should {
    "support pathological ping-ponging for the unbounded case" in
      within(30.seconds) { pathologicalPingPong("test-unbounded-dispatcher") }

    "support pathological ping-ponging for the bounded case" in
      within(30.seconds) { pathologicalPingPong("test-bounded-dispatcher") }
  }
}
