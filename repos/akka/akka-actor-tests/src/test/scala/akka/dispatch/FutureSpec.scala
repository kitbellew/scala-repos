...
class FutureSpec
  ...
  "A Future" when {
    ...
    "from an Actor" which {
      ...
      "throws an exception" must {
        behave like
          futureWithException[RuntimeException] { test ⇒
    ...
    "using flatMap with an Actor" which {
      ...
      "will throw an exception" must {
        behave like
          futureWithException[ArithmeticException] { test ⇒
            filterException[ArithmeticException] {
              val actor1 = system.actorOf(Props[TestActor])
              val actor2 = system.actorOf(
                Props(
      ...
      "will throw a NoSuchElementException when matching wrong type" must {
        behave like
          futureWithException[NoSuchElementException] { test ⇒
    ...
    "being tested" must {
      ...
      "zip" in {
        ...
        intercept[IllegalStateException] {
          Await.result(
            Promise.failed[String](f).future zip
              Promise.successful("foo").future,
            timeout)
        } should ===(f)

        intercept[IllegalStateException] {
          Await.result(
            Promise.successful("foo").future zip
              Promise.failed[String](f).future,
            timeout)
        } should ===(f)
        ...
        Await.result(
          Promise.successful("foo").future zip Promise.successful("foo").future,
  ...
  def emptyFuture(f: (Future[Any] ⇒ Unit) ⇒ Unit) {
    "not be completed" in {
      f(_ should not be ('completed))
    }
    "not contain a value" in {
      f(_.value should ===(None))
    }
  }

  def futureWithResult(f: ((Future[Any], Any) ⇒ Unit) ⇒ Unit) {
    "be completed" in {
      f((future, _) ⇒ future should be('completed))
    }
    "contain a value" in {
    ...
    "not timeout" in {
      f((future, _) ⇒ FutureSpec.ready(future, 0 millis))
    }
    "filter result" in {
      f { (future, result) ⇒
    ...
    "always cast successfully using mapTo" in {
      f((future, message) ⇒
        (
          ...
          } should produce[java.lang.Exception]
        ).getMessage should ===(message))
    }
  }
