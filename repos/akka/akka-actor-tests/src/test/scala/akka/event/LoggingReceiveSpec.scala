  ...
  "A LoggingReceive" must {
    ...
    "decorate a Receive" in {
      new TestKit(appLogging) {
        ...
        val a = system.actorOf(Props(new Actor {
          def receive = new LoggingReceive(
            Some("funky"),
            { case null ⇒
            })
        }))
    ...
    "be added on Actor if requested" in {
      new TestKit(appLogging) with ImplicitSender {
        ...
        val actor = TestActorRef(new Actor {
          def switch: Actor.Receive = { case "becomenull" ⇒
            context.become(r, false)
          }
          def receive = switch orElse LoggingReceive { case x ⇒
            sender() ! "x"
          }
        })
        ...
        within(500 millis) {
          actor ! "bah"
          expectMsgPF() { case UnhandledMessage("bah", testActor, `actor`) ⇒
            true
          }
        }
      }
    ...
    "not duplicate logging" in {
      new TestKit(appLogging) with ImplicitSender {
        system.eventStream.subscribe(testActor, classOf[Logging.Debug])
        val actor = TestActorRef(new Actor {
          def receive = LoggingReceive(LoggingReceive { case _ ⇒
            sender() ! "x"
          })
        })
        actor ! "buh"
