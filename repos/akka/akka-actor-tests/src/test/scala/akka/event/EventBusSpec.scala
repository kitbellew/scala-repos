...
class ActorEventBusSpec(conf: Config)
  ...
  def disposeSubscriber(
      system: ActorSystem,
      subscriber: BusType#Subscriber): Unit = system.stop(subscriber)
  ...
  private def expectUnsubscribedByUnsubscriber(p: TestProbe, a: ActorRef) {
    ...
    p.fishForMessage(1 second, hint = expectedMsg) {
      case Logging.Debug(_, _, msg) if msg equals expectedMsg ⇒
        true
      case other ⇒
        false
    }
...
object LookupEventBusSpec {
  class MyLookupEventBus extends EventBus with LookupClassification {
    ...
    override protected def compareSubscribers(
        a: Procedure[Int],
        b: Procedure[Int]): Int = akka.util.Helpers.compareIdentityHash(a, b)
    override protected def mapSize = 32
    override protected def publish(
        event: Int,
        subscriber: Procedure[Int]): Unit = subscriber(event)
  }
}
