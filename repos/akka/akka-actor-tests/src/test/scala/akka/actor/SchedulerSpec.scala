  ...
  "A Scheduler" must {
    ...
    "schedule more than once" taggedAs TimingTest in {
      ...
      collectCancellable(
        system
          .scheduler
          .schedule(0 milliseconds, 50 milliseconds)(tickActor2 ! Tick))
    ...
    "schedule once" taggedAs TimingTest in {
      ...
      collectCancellable(
        system
          .scheduler
          .scheduleOnce(300 milliseconds)(countDownLatch.countDown()))
    ...
    "be cancellable" taggedAs TimingTest in {
      for (_ ← 1 to 10)
        system.scheduler.scheduleOnce(1 second, testActor, "fail").cancel()
    ...
    "pick up schedule after actor restart" taggedAs TimingTest in {
      ...
      val actor = Await
        .result((supervisor ? props).mapTo[ActorRef], timeout.duration)
    ...
    "not be affected by long running task" taggedAs TimingTest in {
      ...
      system
        .scheduler
        .schedule(225.millis, 225.millis) {
          Thread.sleep(100)
          latch.countDown()
        }
      Await.ready(latch, 6.seconds)
...
class LightArrayRevolverSchedulerSpec
  ...
  "A LightArrayRevolverScheduler" must {
    ...
    "survive vicious enqueueing" in {
      withScheduler(config = ConfigFactory
        .parseString("akka.scheduler.ticks-per-wheel=2")) { (sched, driver) ⇒
        import driver._
        import system.dispatcher
        val counter = new AtomicInteger
        val terminated = future {
          var rounds = 0
          while (Try(sched.scheduleOnce(Duration.Zero)(())(localEC))
                   .isSuccess) {
            Thread.sleep(1)
            driver.wakeUp(step)
            rounds += 1
  ...
  def withScheduler(start: Long = 0L, config: Config = ConfigFactory.empty)(
    ...
    val lbq =
      new AtomicReference[LinkedBlockingQueue[Long]](
