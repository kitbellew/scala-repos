      ...
      def check(target: ActorRef) {
        ...
        if (target != root)
          askNode(
            c1,
            SelectString(
              "../.." + target.path.elements.mkString("/", "/", "/"))) should
            ===(Some(target))
      }
  ...
  "An ActorSelection" must {
    ...
    "resolve non-existing with Failure" in {
      intercept[ActorNotFound] {
        Await.result(
          system.actorSelection(system / "none").resolveOne(1.second.dilated),
          timeout.duration)
      }
    }
    ...
    "compare equally" in {
      ...
      ActorSelection(c21, "../*/hello").## should
        ===(ActorSelection(c21, "../*/hello").##)
      ActorSelection(c2, "../*/hello") should not be
        ActorSelection(c21, "../*/hello")
      ActorSelection(c2, "../*/hello").## should not be
        ActorSelection(c21, "../*/hello").##
      ActorSelection(c21, "../*/hell") should not be
        ActorSelection(c21, "../*/hello")
      ActorSelection(c21, "../*/hell").## should not be
        ActorSelection(c21, "../*/hello").##
    }
