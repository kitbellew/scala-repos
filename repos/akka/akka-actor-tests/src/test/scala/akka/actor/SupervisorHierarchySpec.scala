...
object SupervisorHierarchySpec {
    ...
    override def preStart {
      ...
      val kidInfo: Map[ActorPath, Int] =
        if (s > 0) {
          ...
          (1 to kids).map { (id) ⇒
            val kidSize = if (rest > 0) { rest -= 1; sizes + 1 }
            else sizes
            val props = Props(
              new Hierarchy(
                kidSize,
                ...
                listener,
                myLevel + 1,
                random)).withDeploy(propsTemplate.deploy)
          ...
          if (context.child(name).isEmpty) {
            ...
            val props = Props(
              new Hierarchy(
                ...
                listener,
                myLevel + 1,
                random)).withDispatcher("hierarchy")
  ...
  class StressTest(testActor: ActorRef, size: Int, breadth: Int)
    ...
    when(Idle) {
      case Event(Init, _) ⇒
        hierarchy = context.watch(
          context.actorOf(
            Props(new Hierarchy(size, breadth, self, 0, random)).withDispatcher(
              "hierarchy"),
            "head"))
    ...
    whenUnhandled {
      ...
      case Event(StateTimeout, _) ⇒
        println(
          "pingChildren:\n" + pingChildren.view.map(
            _.path.toString).toSeq.sorted.mkString("\n"))
        ignoreNotResumedLogs = false
...
class SupervisorHierarchySpec
  ...
  "A Supervisor Hierarchy" must {
    ...
    "restart manager and workers in AllForOne" in {
      ...
      val workerProps =
        Props(new CountDownActor(countDown, SupervisorStrategy.defaultStrategy))
          ...
          Props(new Actor {
            ...
            val child = context.actorOf(
              Props(new Actor {
                ...
                if (ca <= 6 && ca % 3 == 0)
                  context.actorOf(
                    Props(new Actor { override def receive = { case _ ⇒ } }),
                    "workingChild")
