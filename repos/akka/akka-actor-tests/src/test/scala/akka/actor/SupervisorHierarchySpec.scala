...
object SupervisorHierarchySpec {
  ...
  class Hierarchy(
    ...
    val unwrap: PartialFunction[Throwable, (Throwable, Throwable)] = {
      case x @ PostRestartException(_, f: Failure, _) ⇒
        (f, x)
      case x @ ActorInitializationException(_, _, f: Failure) ⇒
        (f, x)
      case x ⇒
        (x, x)
    }
      ...
      new Receive {
        val handler: Receive = {
          case f: Failure ⇒
            ...
            throw f
          case "ping" ⇒ {
            Thread.sleep((random.nextFloat * 1.03).toLong);
            sender() ! "pong"
          }
          case Dump(0) ⇒
            abort("dump")
          case Dump(level) ⇒
            context.children foreach (_ ! Dump(level - 1))
          case Terminated(ref) ⇒
            /*
  ...
  class StressTest(testActor: ActorRef, size: Int, breadth: Int)
    ...
    override val supervisorStrategy =
      OneForOneStrategy() {
        ...
        case OriginalRestartException(f: Failure) ⇒
          f.directive
        case ActorInitializationException(_, _, f: Failure) ⇒
          f.directive
        case _ ⇒
          Stop
      }
    ...
    val nextJob = Iterator.continually(
    ...
    private def bury(path: ActorPath): Unit = {
      ...
      val isChild =
        (other: ActorRef) ⇒ other.path.elements.take(deadGuySize) == deadGuy
    ...
    whenUnhandled {
      case Event(Ready(ref), _) ⇒
        ...
        stay
      case Event(e: ErrorLog, _) ⇒
        if (e.msg.startsWith("not resumed"))
          stay
        else {
          errors :+= sender() -> e
        ...
        Props(
          new Actor {
            override def supervisorStrategy =
              OneForOneStrategy() {
                case _ ⇒
                  Await.ready(latch, 4.seconds.dilated);
                  SupervisorStrategy.Resume
              }
            def receive = {
              case "spawn" ⇒
