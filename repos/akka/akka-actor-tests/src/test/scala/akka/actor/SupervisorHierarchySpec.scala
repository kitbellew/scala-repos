...
object SupervisorHierarchySpec {
  ...
  class StressTest(testActor: ActorRef, size: Int, breadth: Int)
    ...
    override def postStop { testActor ! "stressTestStopped" }
    ...
    when(Init) {
      case Event(Ready(ref), _) ⇒
        if (children contains ref) {
          ...
          children :+= ref
          if (children.size == size) goto(Stress) else stay
        }
    ...
    onTransition { case Stress -> Finishing ⇒ ignoreFailConstr = true }
          ...
          Props(new Actor {
            ...
            val child = context.actorOf(
              Props(new Actor {
                ...
                if (ca < 6) { throw new IllegalArgumentException("OH NO!") }
                override def preStart() = { preStartCalled.incrementAndGet() }
                override def postRestart(reason: Throwable) = {
                  postRestartCalled.incrementAndGet()
                }
                override def receive = { case m ⇒ sender() ! m }
              }),
              "failChild"
            )

            override def receive = { case m ⇒ child.forward(m) }
          }),
          "failResumer"
        )
