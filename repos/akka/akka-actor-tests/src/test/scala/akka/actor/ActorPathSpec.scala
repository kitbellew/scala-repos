...
class ActorPathSpec extends WordSpec with Matchers {
  ...
  "An ActorPath" must {
    ...
    "throw exception upon malformed paths" in {
      ...
      intercept[MalformedURLException] {
        ActorPath.fromString("://hallo")
      }
      intercept[MalformedURLException] {
        ActorPath.fromString("s://dd@:12")
      }
      intercept[MalformedURLException] {
        ActorPath.fromString("s://dd@h:hd")
      }
      intercept[MalformedURLException] {
        ActorPath.fromString("a://l:1/b")
      }
    }
    ...
    "create correct toString" in {
      ...
      (RootActorPath(a) / "user" / "foo")
        .toString should ===("akka.tcp://mysys/user/foo")
      (RootActorPath(a) / "user" / "foo" / "bar")
        .toString should ===("akka.tcp://mysys/user/foo/bar")
    }

    "have correct path elements" in {
      (RootActorPath(Address("akka.tcp", "mysys")) / "user" / "foo" / "bar")
        .elements
        .toSeq should ===(Seq("user", "foo", "bar"))
    }
    ...
    "create correct toStringWithoutAddress" in {
      ...
      (RootActorPath(a) / "user" / "foo")
        .toStringWithoutAddress should ===("/user/foo")
      (RootActorPath(a) / "user" / "foo" / "bar")
        .toStringWithoutAddress should ===("/user/foo/bar")
    }

    "validate path elements" in {
      intercept[InvalidActorNameException](ActorPath.validatePathElement(""))
    ...
    "create correct toStringWithAddress" in {
      ...
      (root / "user")
        .toStringWithAddress(a) should ===("akka.tcp://mysys@aaa:2552/user")
      (root / "user" / "foo")
        .toStringWithAddress(a) should ===("akka.tcp://mysys@aaa:2552/user/foo")

      //      root.toStringWithAddress(b) should ===("akka.tcp://mysys@bb:2552/")
      (root / "user")
        .toStringWithAddress(b) should ===("akka.tcp://mysys@bb:2552/user")
      (root / "user" / "foo")
        .toStringWithAddress(b) should ===("akka.tcp://mysys@bb:2552/user/foo")

      root.toStringWithAddress(c) should ===("akka.tcp://mysys@cccc:2552/")
      (root / "user")
        .toStringWithAddress(c) should ===("akka.tcp://mysys@cccc:2552/user")
      ...
      (rootA / "user")
        .toStringWithAddress(b) should ===("akka.tcp://mysys@aaa:2552/user")
      (rootA / "user" / "foo")
        .toStringWithAddress(b) should ===("akka.tcp://mysys@aaa:2552/user/foo")
    }
