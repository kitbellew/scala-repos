...
case class JavaUri(uri: sm.Uri) extends jm.Uri {
  ...
  def rawQueryString(rawQuery: String): jm.Uri = t(
    _.withRawQueryString(rawQuery))
  ...
  def fragment(fragment: Optional[String]): jm.Uri = t(
    _.copy(fragment = fragment.asScala))
