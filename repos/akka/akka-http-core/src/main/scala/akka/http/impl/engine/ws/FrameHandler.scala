...
private[http] object FrameHandler {
  ...
  private class HandlerStage(server: Boolean)
    ...
    private abstract class CollectingMessageFrame(expectedOpcode: Opcode)
      ...
      def handleRegularFrameStart(start: FrameStart)(implicit
          ctx: Ctx): SyncDirective = {
        if ((expectFirstHeader && start.header
              .opcode == expectedOpcode) // first opcode must be the expected
  ...
  final case class ActivelyCloseWithCode(code: Option[Int], reason: String = "")
      extends MessagePart
      with BypassEvent {
    def isMessageEnd: Boolean = true
  }
