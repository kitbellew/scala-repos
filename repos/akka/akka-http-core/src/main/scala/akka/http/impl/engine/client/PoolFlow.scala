...
private object PoolFlow {
  ...
  def apply(
    ...
    Flow
      .fromGraph(GraphDSL.create[FlowShape[RequestContext, ResponseContext]]() {
        implicit b ⇒
          import settings._
          ...
          val slots = Vector.tabulate(maxConnections)(
            PoolSlot(_, connectionFlow, remoteAddress, settings)).map(b.add(_))
