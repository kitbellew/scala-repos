...
class Http(system: ExtendedActorSystem) extends akka.actor.Extension {
  ...
  /**
    ...
    */
  def bind(connect: ConnectHttp, materializer: Materializer)
      : Source[IncomingConnection, CompletionStage[ServerBinding]] = {
    val connectionContext = connect
      .effectiveConnectionContext(defaultServerHttpContext).asScala
    new Source(
      delegate.bind(connect.host, connect.port, connectionContext)(materializer)
  ...
  def bind(
    ...
    new Source(
      delegate.bind(
        ...
        connectionContext,
        settings.asScala,
        log)(materializer).map(new IncomingConnection(_))
        .mapMaterializedValue(_.map(new ServerBinding(_))(ec).toJava))
  }
  ...
  def bindAndHandle(
    ...
    delegate.bindAndHandle(
      ...
      connect.host,
      connect.port,
      connectionContext)(materializer).map(new ServerBinding(_))(ec).toJava
  }
  ...
  def bindAndHandle(
    ...
    delegate.bindAndHandle(
      ...
      connectionContext,
      settings.asScala,
      log)(materializer).map(new ServerBinding(_))(ec).toJava
  }
  ...
  def clientLayer(
      ...
      settings: ClientConnectionSettings): BidiFlow[
    HttpRequest,
    SslTlsOutbound,
    SslTlsInbound,
    HttpResponse,
    NotUsed] =
    adaptClientLayer(
      delegate.clientLayer(JavaMapping.toScala(hostHeader), settings.asScala))
      ...
      if (to.isHttps)
        delegate.outgoingConnectionHttps(
          ...
          to.effectiveConnectionContext(defaultClientHttpsContext)
            .asInstanceOf[HttpsConnectionContext].asScala,
  ...
  def newHostConnectionPool[T](
    ...
    HttpResponse], T], HostConnectionPool] =
    adaptTupleFlow(
      delegate.newHostConnectionPool[T](to.host, to.port)(materializer)
    ...
    adaptTupleFlow {
      to.effectiveHttpsConnectionContext(defaultClientHttpsContext) match {
        case https: HttpsConnectionContext ⇒
          delegate.newHostConnectionPoolHttps[T](
            to.host,
            to.port,
            https.asScala,
            settings.asScala,
            log)(materializer).mapMaterializedValue(_.toJava)
        case _ ⇒
          delegate
            .newHostConnectionPool[T](to.host, to.port, settings.asScala, log)(
              materializer).mapMaterializedValue(_.toJava)
      }
    }
  ...
  def cachedHostConnectionPool[T](
    ...
    HttpResponse], T], HostConnectionPool] =
    adaptTupleFlow(
      delegate.cachedHostConnectionPool[T](to.host, to.port)(materializer)
        .mapMaterializedValue(_.toJava))
  ...
  def cachedHostConnectionPool[T](
    ...
    HttpResponse], T], HostConnectionPool] =
    adaptTupleFlow(
      delegate.cachedHostConnectionPoolHttps[T](
        to.host,
        to.port,
        to.effectiveHttpsConnectionContext(defaultClientHttpsContext).asScala,
        settings.asScala,
  ...
  def superPool[T](
      ...
      : Flow[Pair[HttpRequest, T], Pair[Try[HttpResponse], T], NotUsed] =
    adaptTupleFlow(
      delegate.superPool[T](connectionContext.asScala, settings.asScala, log)(
        materializer))
  ...
  def superPool[T](
    ...
    adaptTupleFlow(
      delegate
        .superPool[T](defaultClientHttpsContext.asScala, settings.asScala, log)(
          materializer))
  ...
  /**
    ...
    */
  def webSocketClientLayer(request: WebSocketRequest): BidiFlow[
    Message,
    SslTlsOutbound,
    SslTlsInbound,
    Message,
  ...
  def setDefaultClientHttpsContext(context: HttpsConnectionContext): Unit =
    delegate.setDefaultClientHttpsContext(
      context.asInstanceOf[akka.http.scaladsl.HttpsConnectionContext])
    ...
    new BidiFlow(
      JavaMapping
        .adapterBidiFlow[
          ...
          sm.HttpRequest,
          sm.HttpResponse,
          HttpResponse].atop(clientLayer))

  private def adaptWsBidiFlow(
      wsLayer: scaladsl.Http.WebSocketClientLayer): BidiFlow[
    Message,
    SslTlsOutbound,
    SslTlsInbound,
    Message,
  ...
  private def adaptWsFlow[Mat](
    ...
    scaladsl.model.ws.Message,
    Mat] =
    stream.scaladsl.Flow[scaladsl.model.ws.Message].map(Message.adapt)
      .viaMat(javaFlow.asScala)(Keep.right).map(_.asScala)
