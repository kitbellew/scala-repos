...
private[parser] trait IpAddressParsing { this: Parser ⇒
  ...
  def `ip-v6-address`: Rule1[Array[Byte]] = {
    ...
    def h8(ix: Int) = rule {
      HEXDIG ~ HEXDIG ~ run(a(ix) = (hv(charAt(-2)) * 16 + hv(lastChar)).toByte)
    }
    def h16(ix: Int) = rule {
      h8(ix) ~ h8(ix + 1) | h4(ix) ~ h8(ix + 1) | zero(ix) ~ h8(ix + 1) | zero(
        ix) ~ h4(ix + 1)
    }
    def h16c(ix: Int) = rule {
      h16(ix) ~ ':' ~ !':'
    }
    def ch16o(ix: Int) = rule {
  ...
  def `ip-v6-reference`: Rule1[String] = rule {
