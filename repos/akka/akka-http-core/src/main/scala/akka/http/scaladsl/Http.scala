...
object Http extends ExtensionId[HttpExt] with ExtensionIdProvider {
  ...
  /**
    ...
    * }}}
    */
  type WebSocketClientLayer = BidiFlow[
    Message,
    SslTlsOutbound,
    SslTlsInbound,
    Message,
  ...
  final case class IncomingConnection(
    ...
    def handleWith[Mat](handler: Flow[HttpRequest, HttpResponse, Mat])(implicit
        fm: Materializer): Mat = flow.joinMat(handler)(Keep.right).run()

    /**
      * Handles the connection with the given handler function.
      */
    def handleWithSyncHandler(handler: HttpRequest ⇒ HttpResponse)(implicit
        fm: Materializer): Unit = handleWith(Flow[HttpRequest].map(handler))
