...
private[parser] trait LinkHeader {
  ...
  def `link` =
    rule { zeroOrMore(`link-value`).separatedBy(listSep) ~ EOI ~> (Link(_)) }
  ...
  def `relation-types` =
    rule(
      ws('"') ~ oneOrMore(`relation-type`).separatedBy(oneOrMore(SP)) ~> (_
        .mkString(" ")) ~ ws('"')
        | `relation-type` ~ OWS)
  ...
  def `reg-rel-type` =
    rule { capture(LOWER_ALPHA ~ zeroOrMore(`reg-rel-type-octet`)) ~ !VCHAR }
