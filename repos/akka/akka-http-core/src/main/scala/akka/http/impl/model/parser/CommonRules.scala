...
private[parser] trait CommonRules {
  ...
  def `IMF-fixdate` =
    rule { // mixture of the spec-ed `IMF-fixdate` and `rfc850-date`
      (`day-name-l` | `day-name`) ~ ", " ~ (date1 | date2) ~ ' ' ~
        `time-of-day` ~ ' ' ~ ("GMT" | "UTC") ~> {
  ...
  // ******************************************************************************************
  def language =
    rule { `primary-tag` ~ zeroOrMore('-' ~ `sub-tag`) ~> (Language(_, _: _*)) }
  ...
  def `oauth2-bearer-token` =
    rule { ignoreCase("bearer") ~ OWS ~ `token68` ~> OAuth2BearerToken }
  ...
  def `cookie-pair`: Rule1[HttpCookiePair] =
    rule { `cookie-name` ~ ws('=') ~ `cookie-value` ~> (createCookiePair _) }
  ...
  def `cookie-value-raw` =
    rule { capture(zeroOrMore(`cookie-octet-raw`)) ~ OWS }
  ...
  def `extension-av` =
    rule {
      !(ignoreCase("expires=") | ignoreCase("max-age=") |
        ...
        ignoreCase("httponly")) ~ capture(zeroOrMore(`av-octet`)) ~ OWS ~> {
        (c: HttpCookie, s: String) ⇒ c.copy(extension = Some(s))
      }
    }
  ...
  def `byte-range-spec` =
    rule {
      `first-byte-pos` ~ ws('-') ~
        (`last-byte-pos` ~> (ByteRange(_: Long, _)) |
  ...
  def products =
    rule { `product-or-comment` ~ zeroOrMore(`product-or-comment`) ~> (_ +: _) }
