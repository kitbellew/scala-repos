...
private[parser] trait CommonRules { this: Parser with StringBuilding ⇒
  ...
  def `complete-length` = rule {
    longNumberCapped
  }

  def `first-byte-pos` = rule {
    longNumberCapped
  }

  def `last-byte-pos` = rule {
    longNumberCapped
  }

  def `other-content-range` = rule {
  ...
  def `other-range-set` = rule {
    oneOrMore(VCHAR) ~ OWS
  }

  def `other-range-unit` = rule {
