...
private[parser] trait CommonRules {
  ...
  def RWS =
    rule {
      oneOrMore(optional(CRLF) ~ oneOrMore(WSP))
    } // extended with `obs-fold`
  ...
  def token: Rule1[String] =
    rule {
      capture(token0) ~ OWS
    }

  def `quoted-string`: Rule1[String] =
    rule {
      DQUOTE ~ clearSB() ~ zeroOrMore(qdtext ~ appendSB() | `quoted-pair`) ~
        push(sb.toString) ~ DQUOTE ~ OWS
    }

  def qdtext =
    rule {
      `qdtext-base` | `obs-text`
    }
  ...
  def comment: Rule0 =
    rule {
      ws('(') ~ clearSB() ~
        zeroOrMore(ctext | `quoted-cpair` | `nested-comment`) ~ ws(')')
    }
  ...
  def `nested-comment` = {
    ...
    rule {
      &('(') ~ run(saved = sb.toString) ~
        (comment ~ prependSB(saved + " (") ~ appendSB(')') |
  ...
  def date1 =
    rule {
      day ~ `date-sep` ~ month ~ `date-sep` ~ year
    }
  ...
  def month =
    rule(
      "Jan" ~ push(1) | "Feb" ~ push(2) | "Mar" ~ push(3) | "Apr" ~ push(4) |
        "May" ~ push(5) | "Jun" ~ push(6) | "Jul" ~ push(7) | "Aug" ~ push(8) |
        "Sep" ~ push(9) | "Oct" ~ push(10) | "Nov" ~ push(11) |
        "Dec" ~ push(12))
  ...
  def `day-name-l` =
    rule(
      "Sunday" ~ push(0) | "Monday" ~ push(1) | "Tuesday" ~ push(2) |
        "Wednesday" ~ push(3) | "Thursday" ~ push(4) | "Friday" ~ push(5) |
        "Saturday" ~ push(6))
  ...
  def qvalue =
    rule { // a bit more lenient than the spec
      capture(
        '0' ~ optional('.' ~ zeroOrMore(DIGIT)) | '.' ~ oneOrMore(DIGIT) |
  ...
  def `primary-tag` =
    rule {
      capture(oneOrMore(ALPHA)) ~ OWS
    }

  def `sub-tag` =
    rule {
      capture(oneOrMore(ALPHANUM)) ~ OWS
    }
  ...
  def `cookie-value-rfc-6265` =
    rule {
      ('"' ~ capture(zeroOrMore(`cookie-octet-rfc-6265`)) ~ '"' |
        capture(zeroOrMore(`cookie-octet-rfc-6265`))) ~ OWS
    }
  ...
  def `cookie-av` =
    rule {
      `expires-av` | `max-age-av` | `domain-av` | `path-av` | `secure-av` |
  ...
  def `extension-av` =
    rule {
      !(
        ignoreCase("expires=") | ignoreCase("max-age=") |
          ignoreCase("domain=") | ignoreCase("path=") | ignoreCase("secure") |
          ignoreCase("httponly")
      ) ~ capture(zeroOrMore(`av-octet`)) ~ OWS ~> {
        (c: HttpCookie, s: String) ⇒
  ...
  def `origin-list` =
    rule {
      oneOrMore(capture(oneOrMore(VCHAR)) ~> (HttpOrigin(_))).separatedBy(SP) ~
        OWS // offload to URL parser
    }
  ...
  def `byte-range-set` =
    rule {
      zeroOrMore(ws(',')) ~
        oneOrMore(`byte-range-spec` | `suffix-byte-range-spec`)
          .separatedBy(listSep)
    }
  ...
  def `byte-range-spec` =
    rule {
      `first-byte-pos` ~ ws('-') ~
        (`last-byte-pos` ~>
          (ByteRange(_: Long, _)) | run(ByteRange.fromOffset(_)))
    }
  ...
  def `bytes-unit` =
    rule {
      "bytes" ~ OWS ~ push(RangeUnits.Bytes)
    }
  ...
  def `other-content-range` =
    rule {
      `other-range-unit` ~ `other-range-resp`
    }
  ...
  def `other-range-unit` =
    rule {
      token ~> RangeUnits.Other
    }
  ...
  def `product-or-comment` =
    rule(
      product ~ comment ~>
        (ProductVersion(_, _, sb.toString)) |
        product ~>
        (ProductVersion(_, _)) |
  ...
  def `transfer-extension` =
    rule {
      token ~ zeroOrMore(ws(';') ~ `transfer-parameter`) ~>
        (_.toMap) ~>
  ...
  def longNumberCappedAtIntMaxValue =
    rule {
      capture((1 to 11).times(DIGIT)) ~>
        (s ⇒ math.min(s.toLong, Int.MaxValue)) ~ zeroOrMore(DIGIT) ~ OWS
    }
  ...
  def longNumberCapped =
    rule(
      (capture((1 to 18).times(DIGIT)) ~ !DIGIT ~>
        (_.toLong) | oneOrMore(DIGIT) ~ push(999999999999999999L)) ~ OWS)
