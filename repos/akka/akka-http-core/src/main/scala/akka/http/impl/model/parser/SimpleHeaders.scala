...
private[parser] trait SimpleHeaders {
  ...
  def `accept-ranges` =
    rule {
      ("none" ~ push(Nil) |
        zeroOrMore(ws(',')) ~ oneOrMore(`range-unit`).separatedBy(listSep)) ~
        EOI ~>
        (`Accept-Ranges`(_))
    }
  ...
  def `if-modified-since` =
    rule {
      `HTTP-date` ~ EOI ~> (`If-Modified-Since`(_))
    }
  ...
  def `if-unmodified-since` =
    rule {
      `HTTP-date` ~ EOI ~> (`If-Unmodified-Since`(_))
    }
  ...
  def `proxy-authorization` =
    rule {
      credentials ~ EOI ~> (`Proxy-Authorization`(_))
    }
  ...
  def `set-cookie` =
    rule {
      `cookie-pair` ~>
        (_.toCookie) ~ zeroOrMore(ws(';') ~ `cookie-av`) ~ EOI ~>
        (`Set-Cookie`(_))
    }
    ...
    def addr =
      rule {
        (`ip-v4-address` | `ip-v6-address`) ~>
          (RemoteAddress(_)) | "unknown" ~ push(RemoteAddress.Unknown)
      }
    rule {
