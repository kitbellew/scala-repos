...
private[parser] trait SimpleHeaders {
  ...
  def `access-control-allow-origin` =
    rule(
      ws('*') ~ EOI ~ push(`Access-Control-Allow-Origin`.`*`)
        | `origin-list-or-null` ~ EOI ~> (origins ⇒
      ...
      oneOrMore(
        token ~> (x ⇒
          HttpEncodings.getForKeyCaseInsensitive(x) getOrElse HttpEncoding
            .custom(x))).separatedBy(listSep) ~ EOI ~> (`Content-Encoding`(_))
    }
  ...
  def `if-match` =
    rule(
      ws('*') ~ EOI ~ push(`If-Match`.`*`)
        | oneOrMore(`entity-tag`).separatedBy(listSep) ~ EOI ~> (tags ⇒
          `If-Match`(EntityTagRange(tags: _*))))
  ...
  def `if-none-match` =
    rule {
      ws('*') ~ EOI ~ push(`If-None-Match`.`*`) |
        oneOrMore(`entity-tag`).separatedBy(listSep) ~ EOI ~> (tags ⇒
          `If-None-Match`(EntityTagRange(tags: _*)))
    }
  ...
  def `x-real-ip` =
    rule {
      (`ip-v4-address` | `ip-v6-address`) ~ EOI ~> (b ⇒
        `X-Real-Ip`(RemoteAddress(b)))
    }
