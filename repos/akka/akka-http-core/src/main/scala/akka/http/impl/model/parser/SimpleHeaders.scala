...
private[parser] trait SimpleHeaders {
  ...
  def `access-control-max-age` =
    rule { `delta-seconds` ~ EOI ~> (`Access-Control-Max-Age`(_)) }
  ...
  def `access-control-request-method` =
    rule { httpMethodDef ~ EOI ~> (`Access-Control-Request-Method`(_)) }
  ...
  def allow =
    rule { zeroOrMore(httpMethodDef).separatedBy(listSep) ~ EOI ~> (Allow(_)) }
  ...
  def connection =
    rule { oneOrMore(token).separatedBy(listSep) ~ EOI ~> (Connection(_)) }
  ...
  def `content-encoding` =
    rule {
      oneOrMore(
        token ~>
          (x ⇒
            HttpEncodings.getForKeyCaseInsensitive(x) getOrElse
              HttpEncoding.custom(x))).separatedBy(listSep) ~ EOI ~>
        (`Content-Encoding`(_))
    }

  // http://tools.ietf.org/html/rfc7230#section-3.3.2
  def `content-length` =
    rule { longNumberCapped ~> (`Content-Length`(_)) ~ EOI }
  ...
  def `if-match` =
    rule(
      ws('*') ~ EOI ~ push(`If-Match`.`*`) |
        oneOrMore(`entity-tag`).separatedBy(listSep) ~ EOI ~>
  ...
  def `strict-transport-security` =
    rule {
      ignoreCase("max-age=") ~ `delta-seconds` ~
        optional(ws(";") ~ ignoreCase("includesubdomains") ~ push(true)) ~
        EOI ~> (`Strict-Transport-Security`(_, _))
    }
  ...
  def upgrade =
    rule { oneOrMore(protocol).separatedBy(listSep) ~ EOI ~> (Upgrade(_)) }

  def protocol =
    rule { token ~ optional(ws("/") ~ token) ~> (UpgradeProtocol(_, _)) }
  ...
  def `x-forwarded-for` = {
    def addr =
      rule {
        (`ip-v4-address` | `ip-v6-address`) ~> (RemoteAddress(_)) |
          "unknown" ~ push(RemoteAddress.Unknown)
      }
    rule {
