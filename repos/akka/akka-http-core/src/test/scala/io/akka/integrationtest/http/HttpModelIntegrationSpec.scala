...
class HttpModelIntegrationSpec
  ...
  "External HTTP libraries" should {
    ...
    "be able to get String headers and an Array[Byte] body out of an HttpRequest" in {
      ...
      val partialTextHeaders: Seq[(String, String)] = request.headers
      ...
      val entityBytes: Array[Byte] = Await
        .result(request.entity.toStrict(1.second), 2.seconds).data.toArray
