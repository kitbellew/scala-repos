...
class One2OneBidiFlowSpec extends AkkaSpec {
  ...
  "A One2OneBidiFlow" must {
    ...
    "trigger an `OutputTruncationException` if the wrapped stream completes early" in
      assertAllStagesStopped {
        val flowInProbe = TestSubscriber.probe[Int]()
        val flowOutProbe = TestPublisher.probe[Int]()

        val testSetup = One2OneBidiFlow[Int, Int](-1) join
        ...
        Source.fromPublisher(upstreamProbe).via(testSetup)
    ...
    "backpressure the input side if the maximum number of pending output elements has been reached" in
      assertAllStagesStopped {
        ...
        Source(1 to 1000).log("", seen.set).via(
          One2OneBidiFlow[Int, Int](MAX_PENDING) join
            Flow.fromSinkAndSourceMat(Sink.ignore, Source.fromPublisher(out))(
              Keep.left)).runWith(Sink.ignore)
