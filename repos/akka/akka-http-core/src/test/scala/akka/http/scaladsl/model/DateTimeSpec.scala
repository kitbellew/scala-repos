...
class DateTimeSpec extends WordSpec with Matchers {
  ...
  "DateTime.toRfc1123DateTimeString" should {
    "properly print a known date" in {
      DateTime(specificClicks).toRfc1123DateTimeString shouldEqual
        "Tue, 12 Jul 2011 14:08:12 GMT"
      DateTime(2011, 7, 12, 14, 8, 12).toRfc1123DateTimeString shouldEqual
        "Tue, 12 Jul 2011 14:08:12 GMT"
    }
  ...
  "DateTime.toIsoDateTimeString" should {
    "properly print a known date" in {
      DateTime(specificClicks).toIsoDateTimeString shouldEqual
        "2011-07-12T14:08:12"
    }
  }
  ...
  "DateTime.fromIsoDateTimeString" should {
    "properly parse a legal string" in {
      DateTime.fromIsoDateTimeString("2011-07-12T14:08:12") shouldBe
        Some(DateTime(specificClicks))
    }
    "properly parse a legal extended string" in {
      DateTime.fromIsoDateTimeString("2011-07-12T14:08:12.123Z") shouldBe
        Some(DateTime(specificClicks))
    }
  ...
  "The two DateTime implementations" should {
    "allow for transparent round-trip conversions" in {
      ...
      val roundTripOk: Matcher[DateTime] = Matcher { dt: DateTime ⇒
        MatchResult(
          ...
          dt.toRfc1123DateTimeString + " != " + roundTrip(dt)
    ...
    "properly represent DateTime.MinValue" in {
      ...
      DateTime(DateTime.MinValue.clicks).toString shouldEqual
        "1800-01-01T00:00:00"
    }
    "properly represent DateTime.MaxValue" in {
      DateTime.MaxValue.toString shouldEqual "2199-12-31T23:59:59"
      DateTime(DateTime.MaxValue.clicks).toString shouldEqual
        "2199-12-31T23:59:59"
    }
  }
}
