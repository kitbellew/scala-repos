...
class ResponseParserSpec extends FreeSpec with Matchers with BeforeAndAfterAll {
  ...
  "The response parsing logic should" - {
    "properly parse" - {
      ...
      "a response with a simple body" in
        new Test {
          collectBlocking(
            rawParse(
              GET,
      ...
      "a response with a missing reason phrase" in
        new Test {
          "HTTP/1.1 200 \r\nContent-Length: 0\r\n\r\n" should
      ...
      "a response funky `Transfer-Encoding` header" in
        new Test {
          override def parserSettings: ParserSettings =
            super.parserSettings.withCustomStatusCodes(ServerOnTheMove)
    ...
    "properly parse a chunked" - {
      ...
      "response with additional transfer encodings" in
        new Test {
          Seq("""HTTP/1.1 200 OK
    ...
    "reject a response with" - {
      ...
      "with a missing reason phrase and no trailing space" in
        new Test {
          Seq("HTTP/1.1 200\r\nContent-Length: 0\r\n\r\n") should
            generalMultiParseTo(
              Left(
