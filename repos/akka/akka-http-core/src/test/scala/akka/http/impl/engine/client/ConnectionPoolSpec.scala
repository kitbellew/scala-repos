...
class ConnectionPoolSpec
  ...
  "The host-level client infrastructure" should {
    ...
    "properly complete a simple request/response cycle" in
      new TestSetup {
        ...
        response.headers should
          contain(RawHeader("Req-Host", s"$serverHostName:$serverPort"))
      }

    "open a second connection if the first one is loaded" in
      new TestSetup {
        val (requestIn, responseOut, responseOutSub, hcp) =
          cachedHostConnectionPool[Int]()
    ...
    "open a second connection if the request on the first one is dispatch but not yet completed" in
      new TestSetup {
        val (requestIn, responseOut, responseOutSub, hcp) =
          cachedHostConnectionPool[Int]()
    ...
    "not open a second connection if there is an idle one available" in
      new TestSetup {
        val (requestIn, responseOut, responseOutSub, hcp) =
          cachedHostConnectionPool[Int]()
    ...
    "be able to handle 500 pipelined requests against the test server" in
      new TestSetup {
    ...
    "properly surface connection-level errors" in
      new TestSetup(autoAccept = true) {
        val (requestIn, responseOut, responseOutSub, hcp) =
    ...
    "respect the configured `maxRetries` value" in
      new TestSetup(autoAccept = true) {
        val (requestIn, responseOut, responseOutSub, hcp) =
    ...
    "automatically shutdown after configured timeout periods" in
      new TestSetup() {
        val (_, _, _, hcp) = cachedHostConnectionPool[Int](idleTimeout = 1
          .second)
    ...
    "transparently restart after idle shutdown" in
      new TestSetup() {
        val (requestIn, responseOut, responseOutSub, hcp) =
  ...
  "The single-request client infrastructure" should {
    ...
    "transform absolute request URIs into relative URIs plus host header" in
      new LocalTestSetup {
        val request = HttpRequest(uri =
          s"http://$serverHostName:$serverPort/abc?query#fragment")
        val responseFuture = Http().singleRequest(request)
        val responseHeaders = Await.result(responseFuture, 1.second).headers
        responseHeaders should
          contain(RawHeader("Req-Raw-Request-URI", "/abc?query"))
        responseHeaders should
          contain(RawHeader("Req-Host", s"$serverHostName:$serverPort"))
      }

    "support absolute request URIs without path component" in
      new LocalTestSetup {
    ...
    "support absolute request URIs with a double slash path component" in
      new LocalTestSetup {
        val request = HttpRequest(uri =
        ...
        responseHeaders should
          contain(RawHeader("Req-Raw-Request-URI", "//foo"))
      }
  ...
  "The superPool client infrastructure" should {
    ...
    "route incoming requests to the right cached host connection pool" in
      new TestSetup(autoAccept = true) {
  ...
  class TestSetup(
    ...
    def responseHeaders(r: HttpRequest, connNr: Int) =
      ConnNrHeader(connNr) +: RawHeader("Req-Uri", r.uri.toString) +:
        r.headers.map(h ⇒ RawHeader("Req-" + h.name, h.value))
