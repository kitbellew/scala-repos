...
class RequestParserSpec extends FreeSpec with Matchers with BeforeAndAfterAll {
  ...
  "The request parsing logic should" - {
    "properly parse a request" - {
      ...
      "with no headers and no body but remaining content" in
        new Test {
          ...
          |POST /foo HTTP/1.0
          |
          |TRA""") /* beginning of TRACE request */ should
            generalMultiParseTo(
      ...
      "with 3 headers and a body" in
        new Test {
          ...
          |Content-length:    17
          |
          |Shake your BOODY!""" should
            parseTo {
              HttpRequest(
                POST,
                "/resource/yes",
                List(`User-Agent`("curl/7.19.7 xyz"), Connection("keep-alive")),
                "Shake your BOODY!",
                `HTTP/1.0`)
            }
          closeAfterResponseCompletion shouldEqual Seq(false)
          ...
          |""" should
            parseTo {
              HttpRequest(
                ...
                "/abc",
                List(
                  `User-Agent`("curl/7.19.7 abc xyz"),
                  Accept(MediaRanges.`*/*`),
      ...
      "with a funky `Transfer-Encoding` header" in
        new Test {
          """PUT / HTTP/1.1
    ...
    "properly parse a chunked request" - {
      ...
      "message end with extension and trailer" in
        new Test {
      ...
      "don't overflow the stack for large buffers of chunks" in
        new Test {
    ...
    "support `rawRequestUriHeader` setting" in
      new Test {
    ...
    "reject a request with" - {
      "an illegal HTTP method" in
        new Test {
          ...
          "GETX " should
            parseToError(
              NotImplemented,
              ErrorInfo("Unsupported HTTP method", "GETX"))
        }

      "a too long HTTP method" in
        new Test {
      ...
      "a too-long URI" in
        new Test {
          "GET /23456789012345678901 HTTP/1.1" should
            parseToError(
              RequestUriTooLong,
              ErrorInfo(
      ...
      "with a too-long header-value" in
        new Test {
          """|GET / HTTP/1.1
          |Fancy: 123456789012345678901234567890123""" should
            parseToError(
              BadRequest,
              ErrorInfo(
      ...
      "with an illegal entity using CONNECT" in
        new Test {
      ...
      "with an illegal entity using HEAD" in
        new Test {
      ...
      "with an illegal entity using TRACE" in
        new Test {
