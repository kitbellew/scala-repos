  ...
  "The Java HTTP extension" should {
    ...
    "properly bind a server (with three parameters)" in {
      ...
      val binding = http.bind(toHost(host, port), materializer)
        .toMat(Sink.fromSubscriber(probe), Keep.left).run(materializer)
    ...
    "properly bind and handle a server with a flow (with four parameters)" in {
      ...
      val flow: Flow[HttpRequest, HttpResponse, NotUsed] = akka.stream.scaladsl
        .Flow[HttpRequest].map(req ⇒ HttpResponse.create()).asJava
      val binding = http.bindAndHandle(flow, toHost(host, port), materializer)

      val (_, completion) = http.outgoingConnection(toHost(host, port)).runWith(
    ...
    "properly bind and handle a server with a synchronous function (with four parameters)" in {
      ...
      val response = http.singleRequest(
        HttpRequest.create(s"http://$host:$port/").withMethod(HttpMethods.GET),
        materializer)
    ...
    "create a cached connection pool (with a ConnectToHttp and a materializer)" in {
      ...
      val pair: Pair[HostConnectionPool, CompletionStage[
        Pair[Try[HttpResponse], NotUsed]]] = Source.single(new Pair(
        HttpRequest.GET(s"http://$host:$port/"),
        NotUsed.getInstance()))
        .viaMat(poolFlow, Keep.right[NotUsed, HostConnectionPool]).toMat(
          Sink.head(),
          Keep
            .both[HostConnectionPool, CompletionStage[
              Pair[Try[HttpResponse], NotUsed]]]).run(materializer)
    ...
    "have client layer methods" in {
      ...
      http.clientLayer(
        headers.Host.create("example.com"),
        connectionSettings,
        loggingAdapter)
    }
    ...
    "create an outgoing connection (with a ConnectHttp)" in {
      ...
      val response = Source.single(get(host, port)).via(flow)
    ...
    "interact with a websocket through a flow (with with one parameter)" in {
      ...
      val pair = Source.single(TextMessage.create("hello")).viaMat(
        flow,
        Keep.right[NotUsed, CompletionStage[WebSocketUpgradeResponse]]).toMat(
        Sink.head[Message](),
        Keep
