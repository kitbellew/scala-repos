...
class HttpHeaderSpec extends FreeSpec with Matchers {
  ...
  "The HTTP header model must correctly parse and render the headers" - {
    ...
    "Accept-Charset" in {
      ...
      "Accept-Charset: utf16;q=1" =!=
        `Accept-Charset`(`UTF-16`).renderedTo("UTF-16")
    ...
    "Cookie (RFC 6265)" in {
      ...
      "Cookie: a=1;b=2" =!=
        Cookie("a" -> "1", "b" -> "2").renderedTo("a=1; b=2")
      "Cookie: a=1 ;b=2" =!=
        Cookie("a" -> "1", "b" -> "2").renderedTo("a=1; b=2")

      "Cookie: z=0;a=1,b=2" =!= Cookie("z" -> "0").renderedTo("z=0")
      """Cookie: a=1;b="test"""" =!=
        Cookie("a" -> "1", "b" -> "test").renderedTo("a=1; b=test")

      "Cookie: a=1; b=f\"d\"c\"; c=xyz" =!=
        Cookie("a" -> "1", "c" -> "xyz").renderedTo("a=1; c=xyz")
      "Cookie: a=1; b=ä; c=d" =!=
        Cookie("a" -> "1", "c" -> "d").renderedTo("a=1; c=d")
    ...
    "Expires" in {
      ...
      "Expires: 0" =!=
        Expires(DateTime.MinValue).renderedTo("Wed, 01 Jan 1800 00:00:00 GMT")
    }
    ...
    "If-Modified-Since" in {
      ...
      "If-Modified-Since: 0" =!=
        `If-Modified-Since`(DateTime.MinValue)
    ...
    "X-Forwarded-For" in {
      ...
      "X-Forwarded-For: 234.123.5.6, 8.8.8.8" =!=
        `X-Forwarded-For`(
  ...
  implicit class TestLine(line: String) {
    ...
    def =!=>(expectedRendering: String) = {
      ...
      val HttpHeader.ParsingResult.Ok(header, Nil) = HttpHeader
        .parse(name, value)
      header.toString shouldEqual
        header.renderedTo(expectedRendering).rendering("")
    }
  }
  ...
  def renderFromHeaderTo(
      header: TestHeader,
      line: String): Matcher[HttpHeader.ParsingResult] =
    Matcher {
