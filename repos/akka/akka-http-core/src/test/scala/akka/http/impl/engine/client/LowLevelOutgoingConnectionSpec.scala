...
class LowLevelOutgoingConnectionSpec
  ...
  "The connection-level client implementation" should {
    ...
    "handle a request/response round-trip" which {
      ...
      "has a request with empty entity" in
        new TestSetup {
          sendStandardRequest()
      ...
      "has a request with default entity" in
        new TestSetup {
      ...
      "has a response with a chunked entity" in
        new TestSetup {
          sendStandardRequest()
          sendWireData("""HTTP/1.1 200 OK
      ...
      "exhibits eager request stream completion" in
        new TestSetup {
    ...
    "handle several requests on one persistent connection" which {
      "has a first response that was chunked" in
        new TestSetup {
          requestsSub.sendNext(HttpRequest())
          expectWireData("""GET / HTTP/1.1
            |Host: example.com
    ...
    "produce proper errors" which {
      ...
      "catch illegal response starts" in
        new TestSetup {
          sendStandardRequest()
      ...
      "catch illegal response chunks" in
        new TestSetup {
          sendStandardRequest()
          sendWireData("""HTTP/1.1 200 OK
            |Transfer-Encoding: chunked
      ...
      "catch a response start truncation" in
        new TestSetup {
          sendStandardRequest()
          sendWireData("HTTP/1.1 200 OK")
          netInSub.sendComplete()
    ...
    "support response length verification" which
      isDefinedVia {
        import HttpEntity._
    ...
    "ignore interim 1xx responses" in
      new TestSetup {
        sendStandardRequest()
