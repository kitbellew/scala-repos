  ...
  "The server implementation" should {
    ...
    "proceed to next request once previous request's entity has beed drained" in
      new TestSetup {
        ...
        twice {
          ...
          val whenComplete = expectRequest().entity.dataBytes
            .runWith(Sink.ignore)
    ...
    "not emit entities when responding to HEAD requests if transparent-head-requests is enabled (with Default)" in
      new TestSetup {
        ...
        inside(expectRequest()) {
          case HttpRequest(GET, _, _, _, _) ⇒
            responses.sendNext(HttpResponse(entity = HttpEntity.Default(
    ...
    "not emit entities when responding to HEAD requests if transparent-head-requests is enabled (with Chunked)" in
      new TestSetup {
        ...
        inside(expectRequest()) {
          case HttpRequest(GET, _, _, _, _) ⇒
            responses.sendNext(HttpResponse(entity = HttpEntity.Chunked(
              ContentTypes.`text/plain(UTF-8)`,
              Source.fromPublisher(data))))
            val dataSub = data.expectSubscription()
            dataSub.expectCancellation()
    ...
    "respect Connection headers of HEAD requests if transparent-head-requests is enabled" in
      new TestSetup {
        ...
        inside(expectRequest()) {
          case HttpRequest(GET, _, _, _, _) ⇒
            responses.sendNext(HttpResponse(entity = CloseDelimited(
              ContentTypes.`text/plain(UTF-8)`,
              Source.fromPublisher(data))))
            val dataSub = data.expectSubscription()
            dataSub.expectCancellation()
    ...
    "support request timeouts" which {
      ...
      "are programmatically increased (not expiring)" in
        new RequestTimeoutTestSetup(10.millis) {
          send("GET / HTTP/1.1\r\nHost: example.com\r\n\r\n")
          expectRequest().header[`Timeout-Access`]
    ...
    "support request length verification" which afterWord("is defined via") {
          ...
          def expectDefaultEntityWithSizeError(limit: Int, actualSize: Int) =
            inside(request) {
              case HttpRequest(
                    ...
                    _) ⇒
                val error = the[Exception].thrownBy(
                  entity.dataBytes.runFold(ByteString.empty)(_ ++ _)
                    .awaitResult(100.millis)).getCause
                error shouldEqual
      ...
      "a smaller programmatically-set limit (strict entity)" in
        new LengthVerificationTest(maxContentLength = 12) {
          sendStrictRequestWithLength(10)
          expectRequest().mapEntity(_ withSizeLimit 10)
      ...
      "the config setting applied before another attribute (default entity)" in
        new LengthVerificationTest(maxContentLength = 10) {
          ...
          sendDefaultRequestWithLength(10)
          expectRequest().mapEntity(nameDataSource("foo"))
            .expectEntity[HttpEntity.Default](10)
