...
class HttpServerSpec
  ...
  "The server implementation" should {
    ...
    "deliver a request as soon as all headers are received" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Content-Length: 12
    ...
    "deliver an error response as soon as a parsing error occurred" in
      new TestSetup {
        send("""GET / HTTP/1.2
             |Host: example.com
             |
    ...
    "report an invalid Chunked stream" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
    ...
    "deliver the request entity as it comes in for a Default entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Content-Length: 12
    ...
    "deliver the request entity as it comes in for a chunked entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Transfer-Encoding: chunked
    ...
    "deliver the second message properly after a Strict entity" in
      new TestSetup {
        send("""POST /strict HTTP/1.1
             |Host: example.com
             |Content-Length: 12
    ...
    "deliver the second message properly after a Default entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Content-Length: 12
    ...
    "deliver the second message properly after a Chunked entity" in
      new TestSetup {
    ...
    "close the request entity stream when the entity is complete for a Default entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Content-Length: 12
    ...
    "close the request entity stream when the entity is complete for a Chunked entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Transfer-Encoding: chunked
    ...
    "close the connection if request entity stream has been cancelled" in
      new TestSetup {
    ...
    "proceed to next request once previous request's entity has beed drained" in
      new TestSetup {
        ...
        twice {
          ...
          val whenComplete = expectRequest()
            .entity
            .dataBytes
            .runWith(Sink.ignore)
    ...
    "translate HEAD request to GET request when transparent-head-requests are enabled" in
      new TestSetup {
    ...
    "keep HEAD request when transparent-head-requests are disabled" in
      new TestSetup {
        override def settings =
    ...
    "respect Connection headers of HEAD requests if transparent-head-requests is enabled" in
      new TestSetup {
        send("""HEAD / HTTP/1.1
             |Host: example.com
             |Connection: close
    ...
    "produce a `100 Continue` response when requested by a `Default` entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Expect: 100-continue
        ...
        inside(expectRequest()) {
          case HttpRequest(
                ...
                _,
                Default(
                  ContentType(`application/octet-stream`, None),
                  16,
                  data),
                _) ⇒
            val dataProbe = TestSubscriber.manualProbe[ByteString]
    ...
    "produce a `100 Continue` response when requested by a `Chunked` entity" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Expect: 100-continue
    ...
    "render a closing response instead of `100 Continue` if request entity is not requested" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Expect: 100-continue
    ...
    "render a 500 response on response stream errors from the application" in
      new TestSetup {
        send("""GET / HTTP/1.1
             |Host: example.com
             |
    ...
    "correctly consume and render large requests and responses" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
    ...
    "deliver a request with a non-RFC3986 request-target" in
      new TestSetup {
    ...
    "use default-host-header for HTTP/1.0 requests" in
      new TestSetup {
    ...
    "fail an HTTP/1.0 request with 400 if no default-host-header is set" in
      new TestSetup {
        send("""GET /abc HTTP/1.0
             |
             |""")
    ...
    "support remote-address-header" in
      new TestSetup {
        ...
        request.headers should
          contain(`Remote-Address`(RemoteAddress(theAddress, Some(8080))))
      }

    "support request timeouts" which {

      "are defined via the config" in
      ...
      "are programmatically increased (not expiring)" in
        new RequestTimeoutTestSetup(10.millis) {
          send("GET / HTTP/1.1\r\nHost: example.com\r\n\r\n")
          expectRequest()
      ...
      "are programmatically increased (expiring)" in
        new RequestTimeoutTestSetup(10.millis) {
          send("GET / HTTP/1.1\r\nHost: example.com\r\n\r\n")
          expectRequest()
            .header[`Timeout-Access`]
      ...
      "are programmatically decreased" in
      ...
      "have a programmatically set timeout handler" in
        new RequestTimeoutTestSetup(10.millis) {
          send("GET / HTTP/1.1\r\nHost: example.com\r\n\r\n")
    ...
    "add `Connection: close` to early responses" in
      new TestSetup {
        send("""POST / HTTP/1.1
             |Host: example.com
             |Content-Length: 100000
        ...
        val HttpRequest(POST, _, _, entity, _) = expectRequest()
        responses
          .sendNext(HttpResponse(status = StatusCodes.InsufficientStorage))
    ...
    "support request length verification" which
      afterWord("is defined via") {
                ...
                case HttpRequest(
                  ...
                  val error =
                    the[Exception]
                      ...
                      .getCause
                  error shouldEqual
                    EntityStreamSizeException(limit, Some(actualSize))
                  error.getMessage should
                    include("exceeded content length limit")
        ...
        "the config setting applied before another attribute (default entity)" in
          new LengthVerificationTest(maxContentLength = 10) {
