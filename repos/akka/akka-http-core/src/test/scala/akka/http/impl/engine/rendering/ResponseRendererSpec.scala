...
class ResponseRendererSpec
  ...
  "The response preparation logic should properly render" - {
    "a response with no body," - {
      "status 200 and no headers" in
        new TestSetup() {
          HttpResponse(200) should
            renderTo {
              """HTTP/1.1 200 OK
      ...
      "a custom Date header" in
        new TestSetup() {
          HttpResponse(
            200,
            List(Date(DateTime(2011, 8, 26, 10, 11, 59)))) should
            renderTo {
              """HTTP/1.1 200 OK
      ...
      "status 304 and a few headers" in
        new TestSetup() {
          HttpResponse(
            304,
            List(RawHeader("X-Fancy", "of course"), Age(0))) should
            renderTo {
      ...
      "a custom status code and no headers" in
        new TestSetup() {
          HttpResponse(ServerOnTheMove) should
            renderTo {
        ...
        new TestSetup() {
          ResponseRenderingContext(
            ...
            response = HttpResponse(
              ...
              entity = "Small f*ck up overhere!")) should
            renderTo(
              """HTTP/1.1 200 OK
              |Age: 30
              |Server: akka-http/1.0.0
    ...
    "a response with a Strict body," - {
      "status 400 and a few headers" in
        new TestSetup() {
          HttpResponse(
            400,
            List(Age(30), Connection("Keep-Alive")),
            "Small f*ck up overhere!") should
            renderTo {
        ...
        new TestSetup() {
          HttpResponse(
            ...
            HttpEntity(
              ...
              ByteString("Small f*ck up overhere!"))) should
            renderTo {
              """HTTP/1.1 400 Bad Request
              |Age: 30
              |Server: akka-http/1.0.0
      ...
      "status 200 and a custom Transfer-Encoding header" in
        new TestSetup() {
          HttpResponse(
            headers = List(
              `Transfer-Encoding`(TransferEncodings.Extension("fancy"))),
            entity = "All good") should
            renderTo {
              """HTTP/1.1 200 OK
    ...
    "a response with a Default (streamed with explicit content-length body," - {
        ...
        new TestSetup() {
          HttpResponse(
            ...
            entity = Default(
              ContentTypes.`text/plain(UTF-8)`,
              23,
              source(ByteString("Small f*ck up overhere!")))) should
            renderTo {
              """HTTP/1.1 400 Bad Request
              |Age: 30
              |Server: akka-http/1.0.0
      ...
      "one chunk and incorrect (too large) Content-Length" in
        new TestSetup() {
      ...
      "one chunk and incorrect (too small) Content-Length" in
        new TestSetup() {
          the[RuntimeException] thrownBy {
            HttpResponse(
              200,
          ...
          ResponseRenderingContext(
            HttpResponse(
              ...
              entity = CloseDelimited(
                ContentTypes.`application/json`,
                source(ByteString.empty)))) should
            renderTo(
              """HTTP/1.1 200 OK
              |Server: akka-http/1.0.0
              |Date: Thu, 25 Aug 2011 09:10:29 GMT
        ...
        new TestSetup() {
          ResponseRenderingContext(
            HttpResponse(
              ...
              entity = CloseDelimited(
                ContentTypes.`application/json`,
                source(ByteString("abc"), ByteString("defg"))))) should
            renderTo(
              """HTTP/1.1 200 OK
              |Server: akka-http/1.0.0
              |Date: Thu, 25 Aug 2011 09:10:29 GMT
    ...
    "a chunked response" - {
      "with empty entity" in
        new TestSetup() {
          pending // Disabled until #15981 is fixed
          HttpResponse(
            200,
            List(Age(30)),
            Chunked(ContentTypes.NoContentType, source())) should
            renderTo {
              """HTTP/1.1 200 OK
              |Age: 30
              |Server: akka-http/1.0.0
      ...
      "with empty entity but non-default Content-Type" in
        new TestSetup() {
          pending // Disabled until #15981 is fixed
          HttpResponse(
            200,
            List(Age(30)),
            Chunked(ContentTypes.`application/json`, source())) should
            renderTo {
              """HTTP/1.1 200 OK
              |Age: 30
              |Server: akka-http/1.0.0
      ...
      "with one chunk and no explicit LastChunk" in
        new TestSetup() {
          HttpResponse(entity = Chunked(
            ContentTypes.`text/plain(UTF-8)`,
            source("Yahoooo"))) should
            renderTo {
              """HTTP/1.1 200 OK
            |Server: akka-http/1.0.0
            |Date: Thu, 25 Aug 2011 09:10:29 GMT
      ...
      "with one chunk and an explicit LastChunk" in
        new TestSetup() {
          HttpResponse(entity = Chunked(
            ContentTypes.`text/plain(UTF-8)`,
            source(
      ...
      "with a custom Transfer-Encoding header" in
        new TestSetup() {
          HttpResponse(
            headers = List(
              `Transfer-Encoding`(TransferEncodings.Extension("fancy"))),
        ...
        new TestSetup() {
          ResponseRenderingContext(
            ...
            response = HttpResponse(entity = Chunked(
              ContentTypes.`application/json`,
              source(Chunk("abc"), Chunk("defg"))))) should
            renderTo(
              """HTTP/1.1 200 OK
              |Server: akka-http/1.0.0
              |Date: Thu, 25 Aug 2011 09:10:29 GMT
    ...
    "properly handle the Server header" - {
      ...
      "if a default is set but an explicit Server header given" in
        new TestSetup() {
          HttpResponse(200, List(Server("server/1.0"))) should
            renderTo {
              """HTTP/1.1 200 OK
            |Server: server/1.0
            |Date: Thu, 25 Aug 2011 09:10:29 GMT
    ...
    "render a CustomHeader header" - {
      "if renderInResponses = true" in
        new TestSetup(None) {
          ...
          HttpResponse(200, List(MyHeader(5))) should
            renderTo {
              """HTTP/1.1 200 OK
            |X-My-Header: No5
            |Date: Thu, 25 Aug 2011 09:10:29 GMT
      ...
      "not if renderInResponses = false" in
        new TestSetup(None) {
          ...
          HttpResponse(200, List(MyInternalHeader(5))) should
            renderTo {
              """HTTP/1.1 200 OK
            |Date: Thu, 25 Aug 2011 09:10:29 GMT
            |Content-Length: 0
