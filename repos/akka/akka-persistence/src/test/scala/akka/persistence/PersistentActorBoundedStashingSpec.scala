...
object PersistentActorBoundedStashingSpec {
  ...
  class StashOverflowStrategyFromConfigPersistentActor(name: String)
    ...
    val updateState: Receive = { case Evt(data) ⇒ events = data :: events }

    val commonBehavior: Receive = { case GetState ⇒ sender() ! events.reverse }

    def receiveRecover = updateState

    override def receiveCommand: Receive =
      commonBehavior orElse { case Cmd(x: Any) ⇒ persist(Evt(x))(updateState) }
  }
