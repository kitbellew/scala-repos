...
abstract class AtLeastOnceDeliverySpec(config: Config)
  ...
  "AtLeastOnceDelivery" must {
    ...
    "re-deliver lost messages after restart" taggedAs
      (TimingTest) in {
        val probe = TestProbe()
        val probeA = TestProbe()
        val dst = system.actorOf(destinationProps(probeA.ref))
    ...
    "restore state from snapshot" taggedAs
      (TimingTest) in {
        val probe = TestProbe()
        val probeA = TestProbe()
        val dst = system.actorOf(destinationProps(probeA.ref))
    ...
    "warn about unconfirmed messages" taggedAs
      (TimingTest) in {
        val probe = TestProbe()
        val probeA = TestProbe()
    ...
    "re-deliver many lost messages" taggedAs
      (TimingTest) in {
        val probe = TestProbe()
        val probeA = TestProbe()
        val probeB = TestProbe()
