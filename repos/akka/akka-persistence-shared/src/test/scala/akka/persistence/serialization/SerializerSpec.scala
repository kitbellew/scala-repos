    ...
    def receive = {
      case m ⇒
        context.actorSelection(
          s"akka.tcp://remote@127.0.0.1:${port}/user/remote")
...
class MyPayloadSerializer extends Serializer {
  ...
  def toBinary(o: AnyRef): Array[Byte] =
    o match { case MyPayload(data) ⇒ s".${data}".getBytes(UTF_8) }
...
class MyPayload2Serializer extends SerializerWithStringManifest {
  ...
  def toBinary(o: AnyRef): Array[Byte] =
    o match { case MyPayload2(data, n) ⇒ s".$data:$n".getBytes(UTF_8) }
...
class MySnapshotSerializer extends Serializer {
  ...
  def toBinary(o: AnyRef): Array[Byte] =
    o match { case MySnapshot(data) ⇒ s".${data}".getBytes(UTF_8) }
...
class MySnapshotSerializer2 extends SerializerWithStringManifest {
  ...
  def toBinary(o: AnyRef): Array[Byte] =
    o match { case MySnapshot2(data) ⇒ s".${data}".getBytes(UTF_8) }
