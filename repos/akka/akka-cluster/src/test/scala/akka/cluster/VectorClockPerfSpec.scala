...
class VectorClockPerfSpec extends WordSpec with Matchers {
  ...
  val clockSize = sys.props.get("akka.cluster.VectorClockPerfSpec.clockSize")
    .getOrElse("1000").toInt
  val iterations = sys.props.get("akka.cluster.VectorClockPerfSpec.iterations")
