...
class HeartbeatNodeRingSpec extends WordSpec with Matchers {
  ...
  "A HashedNodeRing" must {
    ...
    "pick specified number of nodes + unreachable as receivers" in {
      ...
      ring
        .receivers(aa) should ===(Set(bb, cc, dd, ff)) // unreachable ee skipped
      ring
        .receivers(bb) should ===(Set(cc, dd, ee, ff)) // unreachable aa skipped
      ring
        .receivers(cc) should ===(Set(dd, ee, ff, bb)) // unreachable aa skipped
    ...
    "pick all except own as receivers when less than total number of nodes" in {
      ...
      HeartbeatNodeRing(cc, nodes, Set.empty, 5)
        .myReceivers should ===(expected)
      HeartbeatNodeRing(cc, nodes, Set.empty, 6)
        .myReceivers should ===(expected)
      HeartbeatNodeRing(cc, nodes, Set.empty, 7)
        .myReceivers should ===(expected)
    }
