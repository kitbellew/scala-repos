...
class ReachabilityPerfSpec extends WordSpec with Matchers {
  ...
  val nodesSize = sys.props.get("akka.cluster.ReachabilityPerfSpec.nodesSize")
    .getOrElse("250").toInt
  val iterations = sys.props.get("akka.cluster.ReachabilityPerfSpec.iterations")
    .getOrElse("10000").toInt
  ...
  private def createReachabilityOfSize(
      base: Reachability,
      size: Int): Reachability = (base /: (1 to size)) {
    case (r, i) ⇒
  ...
  private def checkThunkFor(
      ...
      times: Int): Unit = {
    for (i ← 1 to times) { thunk(Reachability(r1.records, r1.versions)) }
  }
