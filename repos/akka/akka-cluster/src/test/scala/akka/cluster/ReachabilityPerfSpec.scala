...
class ReachabilityPerfSpec extends WordSpec with Matchers {
  ...
  val nodesSize = sys.props.get(
    "akka.cluster.ReachabilityPerfSpec.nodesSize").getOrElse("250").toInt
  val iterations = sys.props.get(
    "akka.cluster.ReachabilityPerfSpec.iterations").getOrElse("10000").toInt
