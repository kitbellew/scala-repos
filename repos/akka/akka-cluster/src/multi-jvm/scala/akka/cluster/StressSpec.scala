...
private[cluster] object StressMultiJvmSpec extends MultiNodeConfig {
  ...
  val totalNumberOfNodes =
    System.getProperty("MultiJvm.akka.cluster.Stress.nrOfNodes") match {
      case null ⇒
        13
      case value ⇒
        value.toInt requiring (_ >= 10, "nrOfNodes should be >= 10")
    }

  for (n ← 1 to totalNumberOfNodes)
    role("node-" + n)
      ...
      cluster.failureDetector match {
        case reg: DefaultFailureDetectorRegistry[Address] ⇒
          reg.failureDetector(address) match {
            case Some(fd: PhiAccrualFailureDetector) ⇒
              fd.phi
            case _ ⇒
              0.0
          }
  ...
  class Master(
    ...
    val payload =
      Array.fill(settings.payloadSize)(
    ...
    def createJob(): Job = {
      if (tree)
        ...
        SimpleJob(idCounter.next(), payload)
    }
...
abstract class StressSpec
    ...
    runOn(roles.head) {
      clusterResultAggregator match {
        case Some(r) ⇒
          watch(r)
          expectMsgPF() {
            case Terminated(a) if a.path == r.path ⇒
              true
          }
  ...
  def joinSeveral(numberOfNodes: Int, toSeedNodes: Boolean): Unit =
    within(
      ...
      val title =
        s"join ${numberOfNodes} to ${if (toSeedNodes)
          "seed nodes"
        else
          "one node"}, in ${nbrUsedRoles} nodes cluster"
      createResultAggregator(
        title,
        expectedResults = currentRoles.size,
        includeInHistory = true)
    ...
    within(25.seconds + convergenceWithin(3.seconds, nbrUsedRoles - 1)) {
      ...
      val title =
        s"${if (shutdown)
          ...
          "remove"} one from ${nbrUsedRoles} nodes cluster"
      createResultAggregator(
        title,
        expectedResults = currentRoles.size,
        includeInHistory = true)
      ...
      runOn(roles.head) {
        ...
        expectMsgPF() {
          case Terminated(a) if a.path == expectedPath ⇒
            true
        }
      }
    ...
    within(
      ...
      val title =
        s"${if (shutdown)
          ...
          "leave"} ${numberOfNodes} in ${nbrUsedRoles} nodes cluster"
      createResultAggregator(
        title,
        expectedResults = currentRoles.size,
        includeInHistory = true)
