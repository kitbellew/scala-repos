...
private[cluster] object StressMultiJvmSpec extends MultiNodeConfig {
  ...
  val totalNumberOfNodes = System
    .getProperty("MultiJvm.akka.cluster.Stress.nrOfNodes") match {
    case null ⇒ 13
  ...
  class Settings(conf: Config) {
    ...
    val numberOfNodesJoiningToOneNode = getInt("nr-of-nodes-joining-to-one") *
      nFactor
    ...
    val numberOfNodesJoiningToSeedNodes =
      (totalNumberOfNodes - numberOfSeedNodes -
        ...
        numberOfNodesJoiningOneByOneSmall - numberOfNodesJoiningOneByOneLarge -
    ...
    require(
      numberOfSeedNodes + numberOfNodesJoiningToSeedNodesInitially +
        numberOfNodesJoiningOneByOneSmall + numberOfNodesJoiningOneByOneLarge +
        numberOfNodesJoiningToOneNode + numberOfNodesJoiningToSeedNodes <=
        totalNumberOfNodes,
    ...
    require(
      numberOfNodesLeavingOneByOneSmall + numberOfNodesLeavingOneByOneLarge +
        ...
        numberOfNodesShutdownOneByOneLarge + numberOfNodesShutdown <=
    ...
    override def toString: String = {
      testConfig.withFallback(
        ConfigFactory.parseString(s"nrOfNodes=${totalNumberOfNodes}")).root
        .render
    }
  }
  ...
  class PhiObserver extends Actor with ActorLogging {
    ...
    val emptyPhiByNode = Map.empty[Address, PhiValue]
    ...
    import context.dispatcher
    val checkPhiTask = context.system.scheduler
    ...
    def receive = {
      ...
      case Terminated(ref) ⇒ reportTo match {
  ...
  /**
    ...
    */
  class Master(
      settings: StressMultiJvmSpec.Settings,
      batchInterval: FiniteDuration,
      tree: Boolean)
      extends Actor {
    val workers = context.actorOf(FromConfig.props(Props[Worker]), "workers")
    val payload = Array
      .fill(settings.payloadSize)(ThreadLocalRandom.current.nextInt(127).toByte)
    ...
    import context.dispatcher
    val resendTask = context.system.scheduler
  ...
  class Worker extends Actor with ActorLogging {
    def receive = {
      ...
      case TreeJob(id, payload, idx, levels, width) ⇒
        ...
        val totalActors = ((width * math.pow(width, levels) - 1) / (width - 1))
          .toInt
        log.debug(
...
abstract class StressSpec
  ...
  def jvmInfo(): String = {
    ...
    sb.append("Operating system: ").append(os.getName).append(", ")
      .append(os.getArch).append(", ").append(os.getVersion)
    sb.append("\n")
    sb.append("JVM: ").append(runtime.getVmName).append(" ")
      .append(runtime.getVmVendor).append(" ").append(runtime.getVmVersion)
    sb.append("\n")
    ...
    sb.append("Heap: ").append((heap.getUsed.toDouble / 1024 / 1024).form)
      ...
      .append(" - ").append((heap.getMax.toDouble / 1024 / 1024).form)
      .append(")").append(" MB")
    sb.append("\n")
  ...
  def clusterResultAggregator: Option[ActorRef] = {
    system.actorSelection(node(roles.head) / "user" / ("result" + step))
  ...
  def joinSeveral(numberOfNodes: Int, toSeedNodes: Boolean): Unit =
    within(
      10.seconds + convergenceWithin(3.seconds, nbrUsedRoles + numberOfNodes)) {
  ...
  def removeOne(shutdown: Boolean): Unit =
    within(25.seconds + convergenceWithin(3.seconds, nbrUsedRoles - 1)) {
      ...
      runOn(roles.head) {
        system.actorSelection(node(removeRole) / "user" / "watchee")
  ...
  def removeSeveral(numberOfNodes: Int, shutdown: Boolean): Unit =
    within(
      25.seconds + convergenceWithin(5.seconds, nbrUsedRoles - numberOfNodes)) {
      val currentRoles = roles.take(nbrUsedRoles - numberOfNodes)
  ...
  def exerciseJoinRemove(title: String, duration: FiniteDuration): Unit = {
    ...
    val loopDuration = 10.seconds +
      convergenceWithin(4.seconds, nbrUsedRoles + activeRoles.size)
    val rounds = ((duration - loopDuration).toMillis / loopDuration.toMillis)
      .max(1).toInt
      ...
      if (counter > rounds) previousAS
        ...
        val (nextAS, nextAddresses) = within(loopDuration) {
          reportResult {
            ...
            val nextAddresses = clusterView.members.map(_.address) diff
              usedAddresses
            runOn(usedRoles: _*) {
  ...
  def master: Option[ActorRef] = {
    system.actorSelection("/user/" + masterName)
  ...
  "A cluster under stress" must {
    ...
    "join seed nodes" taggedAs LongRunningTest in within(30 seconds) {
      ...
      val otherNodesJoiningSeedNodes = roles.slice(
        numberOfSeedNodes,
        numberOfSeedNodes + numberOfNodesJoiningToSeedNodesInitially)
    ...
    "start routers that are running while nodes are joining" taggedAs
      LongRunningTest in {
    ...
    "end routers that are running while nodes are joining" taggedAs
    ...
    "start routers that are running while nodes are removed" taggedAs
      LongRunningTest in {
        if (exerciseActors) {
          runOn(roles.take(3): _*) {
            system.actorOf(
    ...
    "end routers that are running while nodes are removed" taggedAs
      LongRunningTest in within(30.seconds) {
        if (exerciseActors) {
          runOn(roles.take(3): _*) {
            master match {
