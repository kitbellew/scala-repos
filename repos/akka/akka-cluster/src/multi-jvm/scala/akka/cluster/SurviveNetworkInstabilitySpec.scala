...
abstract class SurviveNetworkInstabilitySpec
  ...
  "A network partition tolerant cluster" must {
    ...
    "heal after one isolated node" taggedAs LongRunningTest in
      within(45.seconds) {
    ...
    "heal after unreachable when ring is changed" taggedAs LongRunningTest in
      within(60.seconds) {
    ...
    "down and remove quarantined node" taggedAs LongRunningTest in
      within(60.seconds) {
        ...
        runOn(second) {
          val sysMsgBufferSize = system.asInstanceOf[ExtendedActorSystem]
            .provider.asInstanceOf[RemoteActorRefProvider].remoteSettings
            .SysMsgBufferSize
          val refs = Vector
            .fill(sysMsgBufferSize + 1)(system.actorOf(Props[Echo])).toSet
          system.actorSelection(node(third) / "user" / "watcher") !
            Targets(refs)
    ...
    "continue and move Joining to Up after downing of one half" taggedAs
