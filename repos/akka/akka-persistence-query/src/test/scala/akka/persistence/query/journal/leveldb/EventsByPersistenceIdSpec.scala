...
class EventsByPersistenceIdSpec
  ...
  "Leveldb query EventsByPersistenceId" must {
    ...
    "find existing events" in {
      ...
      src.map(_.event).runWith(TestSink.probe[Any]).request(2)
        .expectNext("a-1", "a-2").expectNoMsg(500.millis).request(2)
        .expectNext("a-3").expectComplete()
    }

    "find existing events up to a sequence number" in {
      val ref = setup("b")
      val src = queries.currentEventsByPersistenceId("b", 0L, 2L)
      src.map(_.event).runWith(TestSink.probe[Any]).request(5)
        .expectNext("b-1", "b-2").expectComplete()
    }

    "not see new events after demand request" in {
      val ref = setup("f")
      val src = queries.currentEventsByPersistenceId("f", 0L, Long.MaxValue)
      val probe = src.map(_.event).runWith(TestSink.probe[Any]).request(2)
        .expectNext("f-1", "f-2").expectNoMsg(100.millis)

      ref ! "f-4"
      expectMsg("f-4-done")

      probe.expectNoMsg(100.millis).request(5).expectNext("f-3")
    ...
    "return remaining values after partial journal cleanup" in {
      ...
      src.map(_.event).runWith(TestSink.probe[Any]).request(1).expectNext("h-3")
        .expectComplete()
    }
  ...
  "Leveldb live query EventsByPersistenceId" must {
    ...
    "find new events after demand request" in {
      ...
      val probe = src.map(_.event).runWith(TestSink.probe[Any]).request(2)
        .expectNext("e-1", "e-2").expectNoMsg(100.millis)

      ref ! "e-4"
      expectMsg("e-4-done")

      probe.expectNoMsg(100.millis).request(5).expectNext("e-3")
        .expectNext("e-4")
    }
  }
