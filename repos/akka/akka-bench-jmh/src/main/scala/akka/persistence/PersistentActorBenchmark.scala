...
class PersistentActorThroughputBenchmark {
  ...
  def persistentActor_persist_reply(): Unit = {
    for (i <- data10k)
      persistPersistentActor.tell(i, probe.ref)
  ...
  def persistentActor_persistAsync_reply(): Unit = {
    for (i <- data10k)
      persistAsync1PersistentActor.tell(i, probe.ref)
  ...
  def persistentActor_noPersist_reply(): Unit = {
    for (i <- data10k)
      noPersistPersistentActor.tell(i, probe.ref)
  ...
  def persistentActor_persistAsync_replyRightOnCommandReceive(): Unit = {
    for (i <- data10k)
      persistAsyncQuickReplyPersistentActor.tell(i, probe.ref)
