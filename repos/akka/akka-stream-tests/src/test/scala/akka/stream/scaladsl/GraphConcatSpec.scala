  ...
  "Concat" must {
    ...
    "work with one immediately failed and one nonempty publisher" in
      assertAllStagesStopped {
        ...
        if (!errorSignalled)
          errorSignalled ||= subscriber2.expectNextOrError(1, TestException)
            .isLeft
        if (!errorSignalled)
          errorSignalled ||= subscriber2.expectNextOrError(2, TestException)
            .isLeft
        if (!errorSignalled)
          errorSignalled ||= subscriber2.expectNextOrError(3, TestException)
            .isLeft
        if (!errorSignalled)
          errorSignalled ||= subscriber2.expectNextOrError(4, TestException)
            .isLeft
    ...
    "work with one nonempty and one delayed failed publisher" in
      assertAllStagesStopped {
        ...
        var errorSignalled = false
        if (!errorSignalled)
          errorSignalled ||= subscriber.expectNextOrError(1, TestException)
            .isLeft
        if (!errorSignalled)
          errorSignalled ||= subscriber.expectNextOrError(2, TestException)
            .isLeft
        if (!errorSignalled)
          errorSignalled ||= subscriber.expectNextOrError(3, TestException)
            .isLeft
        if (!errorSignalled)
          errorSignalled ||= subscriber.expectNextOrError(4, TestException)
            .isLeft
