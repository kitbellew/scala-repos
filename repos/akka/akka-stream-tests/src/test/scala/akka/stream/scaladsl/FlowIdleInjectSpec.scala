  ...
  "keepAlive" must {
    ...
    "not emit additional elements if upstream is fast enough" in
      Utils.assertAllStagesStopped {
        Await.result(
          Source(1 to 10).keepAlive(1.second, () ⇒ 0).grouped(1000)
            .runWith(Sink.head),
    ...
    "emit elements periodically after silent periods" in
      Utils.assertAllStagesStopped {
        ...
        val result = Await.result(
          sourceWithIdleGap.keepAlive(0.6.seconds, () ⇒ 0).grouped(1000)
            .runWith(Sink.head),
    ...
    "immediately pull upstream" in {
      ...
      Source.fromPublisher(upstream).keepAlive(1.second, () ⇒ 0)
    ...
    "immediately pull upstream after busy period" in {
      ...
      (Source(1 to 10) ++ Source.fromPublisher(upstream))
        .keepAlive(1.second, () ⇒ 0).runWith(Sink.fromSubscriber(downstream))
