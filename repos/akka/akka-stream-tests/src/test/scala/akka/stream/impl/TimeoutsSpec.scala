...
class TimeoutsSpec extends AkkaSpec {
  ...
  "InitialTimeout" must {
    ...
    "pass through elements unmodified" in assertAllStagesStopped {
      Await.result(
        Source(1 to 100).initialTimeout(2.seconds).grouped(200)
          .runWith(Sink.head),
      ...
      a[TE] shouldBe thrownBy {
        Await.result(
          Source(1 to 100).concat(Source.failed(TE("test")))
            .initialTimeout(2.seconds).grouped(200).runWith(Sink.head),
          3.seconds)
      }
    }
    ...
    "fail if no initial element passes until timeout" in
      assertAllStagesStopped {
        ...
        Source.maybe[Int].initialTimeout(1.second)
  ...
  "CompletionTimeout" must {
    ...
    "pass through elements unmodified" in assertAllStagesStopped {
      Await.result(
        Source(1 to 100).completionTimeout(2.seconds).grouped(200)
          .runWith(Sink.head),
        3.seconds) should ===(1 to 100)
    }
    ...
    "pass through error unmodified" in assertAllStagesStopped {
      a[TE] shouldBe thrownBy {
        Await.result(
          Source(1 to 100).concat(Source.failed(TE("test")))
            .completionTimeout(2.seconds).grouped(200).runWith(Sink.head),
          3.seconds)
      }
    }
    ...
    "fail if not completed until timeout" in assertAllStagesStopped {
      ...
      val downstreamProbe = TestSubscriber.probe[Int]()
      Source.fromPublisher(upstreamProbe).completionTimeout(2.seconds)
        .runWith(Sink.fromSubscriber(downstreamProbe))
  ...
  "IdleTimeout" must {
    ...
    "pass through error unmodified" in assertAllStagesStopped {
      a[TE] shouldBe thrownBy {
        Await.result(
          Source(1 to 100).concat(Source.failed(TE("test")))
            .idleTimeout(2.seconds).grouped(200).runWith(Sink.head),
          3.seconds)
      }
    }
    ...
    "fail if time between elements is too large" in assertAllStagesStopped {
      val upstreamProbe = TestPublisher.probe[Int]()
      val downstreamProbe = TestSubscriber.probe[Int]()
      Source.fromPublisher(upstreamProbe).idleTimeout(1.seconds)
        .runWith(Sink.fromSubscriber(downstreamProbe))
    ...
    "not signal error in simple loopback case and pass through elements unmodified" in
      assertAllStagesStopped {
        val timeoutIdentity = BidiFlow
          .bidirectionalIdleTimeout[Int, Int](2.seconds).join(Flow[Int])
    ...
    "not signal error if traffic is one-way" in assertAllStagesStopped {
      ...
      val assembly: RunnableGraph[(Future[Done], Future[Done])] = upstream
        .joinMat(BidiFlow.bidirectionalIdleTimeout[Int, String](2.seconds))(
          Keep.left).joinMat(downstream)(Keep.both)
    ...
    "be able to signal timeout once no traffic on either sides" in
      assertAllStagesStopped {
        ...
        RunnableGraph.fromGraph(GraphDSL.create() { implicit b ⇒
          ...
          val timeoutStage = b
            .add(BidiFlow.bidirectionalIdleTimeout[String, Int](2.seconds))
        ...
        error1.getMessage should
          ===("No elements passed in the last 2 seconds.")
