  ...
  "A StatefulMapConcat" must {
    ...
    "be able to restart" in {
      Source(List(2, 1, 3, 4, 1)).statefulMapConcat(() ⇒ {
        ...
        Supervision.restartingDecider)).runWith(TestSink.probe[Int]).request(2)
        .expectNext(1, 1).request(4).expectNext(1, 1, 1, 1).expectComplete()
    }
    ...
    "be able to resume" in {
      Source(List(2, 1, 3, 4, 1)).statefulMapConcat(() ⇒ {
        ...
        Supervision.resumingDecider)).runWith(TestSink.probe[Int]).request(2)
        .expectNext(1, 1).requestNext(4).request(4).expectNext(1, 1, 1, 1)
        .expectComplete()
    }
