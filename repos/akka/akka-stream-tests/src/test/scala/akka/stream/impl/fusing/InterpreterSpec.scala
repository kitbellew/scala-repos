...
class InterpreterSpec extends AkkaSpec with GraphInterpreterSpecKit {
  ...
  "Interpreter" must {
    ...
    "implement chain of maps correctly" in
    ...
    "work with only boundary ops" in
      new OneBoundedSetup[Int](Seq.empty) {
        lastEvents() should be(Set.empty)
    ...
    "implement take" in
      new OneBoundedSetup[Int](Seq(Take(2))) {
    ...
    "implement take inside a chain" in
      new OneBoundedSetup[Int](
        Seq(
    ...
    "implement fold with proper cancel" in
      new OneBoundedSetup[Int](
        Seq(Fold(0, (agg: Int, x: Int) ⇒ agg + x, stoppingDecider))) {
    ...
    "implement grouped" in
      new OneBoundedSetup[Int](Seq(Grouped(3))) {
        lastEvents() should be(Set.empty)
    ...
    "work with batch-batch (conflate-conflate)" in
      new OneBoundedSetup[Int](
        Batch(
          1L,
          ConstantFun.zeroLong,
    ...
    "implement batch-expand (conflate-expand)" in
      new OneBoundedSetup[Int](
        Batch(
          1L,
          ConstantFun.zeroLong,
    ...
    "implement doubler-conflate (doubler-batch)" in
      new OneBoundedSetup[Int](
        Doubler().toGS,
        Batch(
          1L,
        ...
        Seq(
          new PushPullStage[Any, Any] {
            ...
            override def onUpstreamFinish(ctx: Context[Any])
                : TerminationDirective = ctx.absorbTermination()
          })) {
        lastEvents() should be(Set.empty)
