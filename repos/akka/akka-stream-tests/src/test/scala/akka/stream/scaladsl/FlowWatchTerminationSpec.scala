...
class FlowWatchTerminationSpec extends AkkaSpec {
  ...
  "A WatchTermination" must {
    ...
    "fail future when stream is failed" in assertAllStagesStopped {
      ...
      val (p, future) = TestSource.probe[Int].watchTermination()(Keep.both)
        .to(Sink.ignore).run()
      p.sendNext(1)
      p.sendError(ex)
    ...
    "complete the future for an empty stream" in assertAllStagesStopped {
      val (future, p) = Source.empty[Int].watchTermination()(Keep.right)
        .toMat(TestSink.probe[Int])(Keep.both).run()
      p.request(1)
      future.futureValue should ===(Done)
    }
    ...
    "complete future for graph" in assertAllStagesStopped {
      ...
      val ((sourceProbe, future), sinkProbe) = TestSource.probe[Int]
        .watchTermination()(Keep.both).concat(Source(2 to 5))
        .toMat(TestSink.probe[Int])(Keep.both).run()
