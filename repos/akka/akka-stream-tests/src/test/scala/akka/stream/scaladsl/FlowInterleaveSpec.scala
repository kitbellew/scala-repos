...
class FlowInterleaveSpec extends BaseTwoStreamsSetup {
  ...
  override def setup(p1: Publisher[Int], p2: Publisher[Int]) = {
    ...
    Source.fromPublisher(p1).interleave(Source.fromPublisher(p2), 2)
  ...
  "An Interleave for Flow " must {
    ...
    "work in the happy case" in assertAllStagesStopped {
      ...
      Source(0 to 3).interleave(Source(4 to 6), 2)
        .interleave(Source(7 to 11), 3).runWith(Sink.fromSubscriber(probe))
    ...
    "work with segmentSize = 1" in assertAllStagesStopped {
      ...
      Source(0 to 2).interleave(Source(3 to 5), 1)
        .runWith(Sink.fromSubscriber(probe))
      probe.expectSubscription().request(10)
    ...
    "pass along early cancellation" in assertAllStagesStopped {
      ...
      val (graphSubscriber1, graphSubscriber2) = Source.asSubscriber[Int]
        ...
        .toMat(Sink.fromSubscriber(down))(Keep.left).run
