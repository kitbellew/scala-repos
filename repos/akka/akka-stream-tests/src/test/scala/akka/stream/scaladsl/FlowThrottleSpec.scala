...
class FlowThrottleSpec extends AkkaSpec {
  ...
  "Throttle for single cost elements" must {
    ...
    "emit single element per tick" in Utils.assertAllStagesStopped {
      ...
      Source.fromPublisher(upstream).throttle(
        1,
        300.millis,
        0,
        Shaping).runWith(Sink.fromSubscriber(downstream))
    ...
    "cancel when downstream cancels" in Utils.assertAllStagesStopped {
      val downstream = TestSubscriber.probe[Int]()
      Source(1 to 10).throttle(1, 300.millis, 0, Shaping).runWith(
        Sink.fromSubscriber(downstream))
      downstream.cancel()
    }

    "send elements downstream as soon as time comes" in Utils.assertAllStagesStopped {
      val probe = Source(1 to 10).throttle(2, 500.millis, 0, Shaping).runWith(
        TestSink.probe[Int])
        .request(5)
    ...
    "throw exception when exceeding throughtput in enforced mode" in Utils.assertAllStagesStopped {
      an[RateExceededException] shouldBe thrownBy {
        Await.result(
          Source(1 to 5).throttle(1, 200.millis, 5, Enforcing).runWith(
            Sink.ignore),
          2.seconds)
      }
    }
  ...
  "Throttle for various cost elements" must {
    ...
    "cancel when downstream cancels" in Utils.assertAllStagesStopped {
      val downstream = TestSubscriber.probe[Int]()
      Source(1 to 10).throttle(2, 200.millis, 0, identity, Shaping).runWith(
        Sink.fromSubscriber(downstream))
      downstream.cancel()
    }
    ...
    "send elements downstream as soon as time comes" in Utils.assertAllStagesStopped {
      val probe =
        Source(1 to 10).throttle(4, 500.millis, 0, _ ⇒ 2, Shaping).runWith(
          TestSink.probe[Int])
          .request(5)
      probe.receiveWithin(600.millis) should be(Seq(1, 2))
      probe.expectNoMsg(100.millis)
    ...
    "throw exception when exceeding throughtput in enforced mode" in Utils.assertAllStagesStopped {
      an[RateExceededException] shouldBe thrownBy {
        Await.result(
          Source(1 to 5).throttle(
            2,
            200.millis,
            0,
            identity,
            Enforcing).runWith(Sink.ignore),
          2.seconds)
      }
    }
