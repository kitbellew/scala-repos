  ...
  "SslTls" must {
    ...
    object ClientInitiatesViaTcp extends CommunicationSetup {
      ...
      def decorateFlow(
        ...
        clientTls(leftClosing) join
          Tcp().outgoingConnection(binding.localAddress)
      }
    ...
    "emit an error if the TLS handshake fails certificate checks" in
      assertAllStagesStopped {
        ...
        val clientErr = simple
          ...
          .join(
            Tcp()
              .outgoingConnection(Await.result(server, 1.second).localAddress))
          .run()
