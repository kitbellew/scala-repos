...
class LifecycleInterpreterSpec extends AkkaSpec with GraphInterpreterSpecKit {
  ...
  "Interpreter" must {
    ...
    "call postStop in order on stages - when upstream completes" in
      new OneBoundedSetup[String](Seq(
    ...
    "call postStop in order on stages - when upstream onErrors" in
      new OneBoundedSetup[String](Seq(PreStartAndPostStopIdentity(
    ...
    "call postStop in order on stages - when downstream cancels" in
      new OneBoundedSetup[String](Seq(
    ...
    "call preStart before postStop" in
      new OneBoundedSetup[String](Seq(PreStartAndPostStopIdentity(
    ...
    "onError when preStart fails" in
      new OneBoundedSetup[String](Seq(PreStartFailer(() ⇒ throw TE("Boom!")))) {
        lastEvents() should ===(Set(Cancel, OnError(TE("Boom!"))))
      }

    "not blow up when postStop fails" in
      new OneBoundedSetup[String](Seq(PostStopFailer(() ⇒ throw TE("Boom!")))) {
        upstream.onComplete()
        lastEvents() should ===(Set(OnComplete))
      }

    "onError when preStart fails with stages after" in
      new OneBoundedSetup[String](Seq(
    ...
    "continue with stream shutdown when postStop fails" in
      new OneBoundedSetup[String](Seq(PostStopFailer(() ⇒ throw TE("Boom!")))) {
    ...
    "postStop when pushAndFinish called if upstream completes with pushAndFinish" in
      new OneBoundedSetup[String](Seq(
  ...
  private[akka] case class PostStopFailer[T](ex: () ⇒ Throwable)
      extends PushStage[T, T] {
