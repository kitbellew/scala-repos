...
class FlowDelaySpec extends AkkaSpec {
  ...
  "A Delay" must {
    ...
    "add delay to initialDelay if exists upstream" in {
      Source(1 to 10).initialDelay(1.second).delay(1.second).runWith(
        TestSink.probe[Int])
        .request(10)
    ...
    "drop tail for internal buffer if it's full in DropTail mode" in assertAllStagesStopped {
      Await.result(
        Source(1 to 20).delay(
          1.seconds,
          DelayOverflowStrategy.dropTail).withAttributes(inputBuffer(16, 16))
          .grouped(100)
          .runWith(Sink.head),
    ...
    "drop head for internal buffer if it's full in DropHead mode" in assertAllStagesStopped {
      Await.result(
        Source(1 to 20).delay(
          1.seconds,
          DelayOverflowStrategy.dropHead).withAttributes(inputBuffer(16, 16))
          .grouped(100)
          .runWith(Sink.head),
    ...
    "clear all for internal buffer if it's full in DropBuffer mode" in assertAllStagesStopped {
      Await.result(
        Source(1 to 20).delay(
          1.seconds,
          DelayOverflowStrategy.dropBuffer).withAttributes(inputBuffer(16, 16))
          .grouped(100)
          .runWith(Sink.head),
    ...
    "pass elements with delay through normally in backpressured mode" in assertAllStagesStopped {
      Source(1 to 3).delay(
        300.millis,
        DelayOverflowStrategy.backpressure).runWith(TestSink.probe[Int])
        .request(5)
        .expectNoMsg(200.millis)
    ...
    "emit early when buffer is full and in EmitEarly mode" in assertAllStagesStopped {
      ...
      Source.fromPublisher(p).delay(
        10.seconds,
        DelayOverflowStrategy.emitEarly).withAttributes(inputBuffer(16, 16)).to(
        Sink.fromSubscriber(c)).run()
      val cSub = c.expectSubscription()
      val pSub = p.expectSubscription()
      cSub.request(20)
