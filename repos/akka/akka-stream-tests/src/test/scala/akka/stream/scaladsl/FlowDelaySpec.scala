...
class FlowDelaySpec extends AkkaSpec {
  ...
  "A Delay" must {
    ...
    "add delay to initialDelay if exists upstream" in {
      Source(1 to 10).initialDelay(1.second).delay(1.second)
        .runWith(TestSink.probe[Int]).request(10).expectNoMsg(1800.millis)
        .expectNext(300.millis, 1).expectNextN(2 to 10).expectComplete()
    }

    "deliver element after time passed from actual receiving element" in {
      Source(1 to 3).delay(300.millis).runWith(TestSink.probe[Int]).request(2)
        .expectNoMsg(200.millis) //delay
        .expectNext(200.millis, 1) //delayed element
        .expectNext(100.millis, 2) //buffered element
        .expectNoMsg(200.millis).request(1).expectNext(3) //buffered element
        .expectComplete()
    }
    ...
    "fail on overflow in Fail mode" in assertAllStagesStopped {
      Source(1 to 20).delay(300.millis, DelayOverflowStrategy.fail)
        .withAttributes(inputBuffer(16, 16)).runWith(TestSink.probe[Int])
        .request(100).expectError(new BufferOverflowException(
