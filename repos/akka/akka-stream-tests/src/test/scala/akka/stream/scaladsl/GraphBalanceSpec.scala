...
class GraphBalanceSpec extends AkkaSpec {
  ...
  "A balance" must {
    ...
    "support waiting for demand from all downstream subscriptions" in {
      ...
      val p2 =
        RunnableGraph.fromGraph(GraphDSL.create(Sink.asPublisher[Int](false)) {
          implicit b ⇒ p2Sink ⇒
    ...
    "work with 5-way balance" in {
      ...
      val (s1, s2, s3, s4, s5) = RunnableGraph.fromGraph(
    ...
    "balance between all three outputs" in {
      ...
      val results = RunnableGraph.fromGraph(
        GraphDSL.create(outputs, outputs, outputs)(List(_, _, _)) {
          implicit b ⇒ (o1, o2, o3) ⇒
            val balance = b.add(Balance[Int](3, waitForAllDownstreams = true))
    ...
    "fairly balance between three outputs" in {
      ...
      val (p1, p2, p3) = RunnableGraph.fromGraph(
        GraphDSL.create(probe, probe, probe)(Tuple3.apply) {
          implicit b ⇒ (o1, o2, o3) ⇒
