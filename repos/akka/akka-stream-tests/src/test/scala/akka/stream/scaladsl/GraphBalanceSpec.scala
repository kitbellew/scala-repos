  ...
  "A balance" must {
    ...
    "support waiting for demand from all downstream subscriptions" in {
      ...
      val p2 = RunnableGraph
        .fromGraph(
          GraphDSL.create(Sink.asPublisher[Int](false)) { implicit b ⇒ p2Sink ⇒
          ...
          .fromGraph(
            GraphDSL.create(
              ...
              implicit b ⇒ (p2Sink, p3Sink) ⇒
                val balance = b
                  .add(Balance[Int](3, waitForAllDownstreams = true))
                Source(List(1, 2, 3)) ~> balance.in
                balance.out(0) ~> Sink.fromSubscriber(s1)
                balance.out(1) ~> p2Sink
    ...
    "work with 5-way balance" in {
      ...
      Set(s1, s2, s3, s4, s5) flatMap
        (Await.result(_, 3.seconds)) should be((0 to 14).toSet)
    }
