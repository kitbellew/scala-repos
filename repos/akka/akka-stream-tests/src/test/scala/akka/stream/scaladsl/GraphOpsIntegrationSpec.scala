...
class GraphOpsIntegrationSpec extends AkkaSpec {
  ...
  "FlowGraphs" must {
    ...
    "support broadcast - merge layouts" in {
      val resultFuture =
        RunnableGraph.fromGraph(GraphDSL.create(Sink.head[Seq[Int]]) {
          implicit b ⇒ (sink) ⇒
    ...
    "support wikipedia Topological_sorting 2" in {
      ...
      val (resultFuture2, resultFuture9, resultFuture10) =
        RunnableGraph.fromGraph(
          GraphDSL.create(seqSink, seqSink, seqSink)(Tuple3.apply) {
            implicit b ⇒ (sink2, sink9, sink10) ⇒
              ...
              m11.out ~> b11.in
              b11.out(0).grouped(
                1000) ~> sink2.in // Vertex 2 is omitted since it has only one in and out
              b11.out(1) ~> m9.in(0)
              b11.out(2) ~> m10.in(1)
