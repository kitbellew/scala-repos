...
class FusingSpec extends AkkaSpec {
  ...
  def graph(async: Boolean) =
    Source.unfold(1)(x ⇒ Some(x -> x)).filter(_ % 2 == 1).alsoTo(
      Flow[Int].fold(0)(_ + _).to(Sink.head.named("otherSink")).addAttributes(
  ...
  "SubFusingActorMaterializer" must {
    ...
    "work with asynchronous boundaries in the subflows" in {
      ...
      Source(0 to 9).map(_ * 10)
        .flatMapMerge(5, i ⇒ Source(i to (i + 9)).via(async)).grouped(1000)
        .runWith(Sink.head).futureValue.sorted should ===(0 to 198 by 2)
    }
    ...
    "use multiple actors when there are asynchronous boundaries in the subflows (manual)" in {
      ...
      Source(0 to 9).map(x ⇒ { testActor ! ref; x })
        .flatMapMerge(5, i ⇒ Source.single(i).via(async)).grouped(1000)
        .runWith(Sink.head).futureValue.sorted should ===(0 to 9)
    ...
    "use multiple actors when there are asynchronous boundaries in the subflows (combinator)" in {
      ...
      Source(0 to 9).map(x ⇒ { testActor ! ref; x })
        .flatMapMerge(5, i ⇒ Source.single(i).via(flow.async)).grouped(1000)
        .runWith(Sink.head).futureValue.sorted should ===(0 to 9)
      val refs = receiveN(20)
      withClue(s"refs=\n${refs.mkString("\n")}") {
        refs.toSet.size should ===(11) // main flow + 10 subflows
