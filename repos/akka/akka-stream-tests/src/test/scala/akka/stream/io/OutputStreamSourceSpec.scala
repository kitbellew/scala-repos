...
class OutputStreamSourceSpec extends AkkaSpec(UnboundedMailboxConfig) {
  ...
  "OutputStreamSource" must {
    "read bytes from OutputStream" in assertAllStagesStopped {
      val (outputStream, probe) = StreamConverters.asOutputStream().toMat(
        TestSink.probe[ByteString])(Keep.both).run
    ...
    "block writes when buffer is full" in assertAllStagesStopped {
      val (outputStream, probe) = StreamConverters.asOutputStream().toMat(
        TestSink.probe[ByteString])(Keep.both)
    ...
    "use dedicated default-blocking-io-dispatcher by default" in assertAllStagesStopped {
      ...
      try {
        StreamConverters.asOutputStream().runWith(TestSink.probe[ByteString])(
          materializer)
        ...
        val ref = expectMsgType[Children].children.find(
          _.path.toString contains "outputStreamSource").get
