  ...
  "FlowGraphs" when {
    ...
    "turned into sources" should {
      ...
      "be reusable multiple times" in {
        ...
        val source = Source.fromGraph(GraphDSL.create(Source(1 to 5)) {
          implicit b ⇒ s ⇒
            import GraphDSL.Implicits._
            SourceShape(s.out.map(_ * 2).outlet)
        })

        RunnableGraph.fromGraph(GraphDSL.create(source, source)(Keep.both) {
          implicit b ⇒ (s1, s2) ⇒
            import GraphDSL.Implicits._
    ...
    "used together" should {
      "materialize properly" in {
        ...
        val (m1, m2, m3) = RunnableGraph.fromGraph(
      ...
      "allow connecting source to sink directly" in {
        ...
        val (m1, m2) =
          RunnableGraph.fromGraph(GraphDSL.create(source, sink)(Keep.both) {
            implicit b ⇒ (src, snk) ⇒
              import GraphDSL.Implicits._
              src.out ~> snk.in
              ClosedShape
