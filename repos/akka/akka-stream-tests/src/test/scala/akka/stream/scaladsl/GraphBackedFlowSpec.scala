...
class GraphFlowSpec extends AkkaSpec {
  ...
  def validateProbe(
      probe: TestSubscriber.ManualProbe[Int],
      requests: Int,
      result: Set[Int]): Unit = {
    val subscription = probe.expectSubscription()

    val collected = (1 to requests)
      .map { _ ⇒
  ...
  "FlowGraphs" when {
    "turned into flows" should {
      ...
      "work with another GraphFlow" in {
        ...
        val flow2 = Flow.fromGraph(
          GraphDSL.create(Flow[String].map(_.toInt)) { implicit b ⇒ importFlow ⇒
    ...
    "turned into sources" should {
      ...
      "be reusable multiple times" in {
        ...
        val source = Source.fromGraph(
          GraphDSL.create(Source(1 to 5)) { implicit b ⇒ s ⇒
            import GraphDSL.Implicits._
        ...
        RunnableGraph
          .fromGraph(
            GraphDSL.create(source, source)(Keep.both) { implicit b ⇒ (s1, s2) ⇒
              import GraphDSL.Implicits._
    ...
    "turned into sinks" should {
      ...
      "work with a Source when having KeyedSink inside" in {
        ...
        val sink = Sink.fromGraph(
          GraphDSL.create(pubSink) { implicit b ⇒ p ⇒
            SinkShape(p.in)
          })
      ...
      "be transformable with a Pipe" in {
        ...
        val sink = Sink.fromGraph(
          GraphDSL.create(partialGraph, Flow[String].map(_.toInt))(Keep.both) {
    ...
    "used together" should {
      "materialize properly" in {
        ...
        val (m1, m2, m3) = RunnableGraph
          .fromGraph(
            GraphDSL.create(source, flow, sink)(Tuple3.apply) {
      ...
      "allow connecting source to sink directly" in {
        ...
        val source = Source.fromGraph(
          GraphDSL.create(inSource) { implicit b ⇒ src ⇒
            SourceShape(src.out)
          })

        val sink = Sink.fromGraph(
          GraphDSL.create(outSink) { implicit b ⇒ snk ⇒
            SinkShape(snk.in)
          })

        val (m1, m2) = RunnableGraph
          .fromGraph(
            GraphDSL.create(source, sink)(Keep.both) { implicit b ⇒ (src, snk) ⇒
              import GraphDSL.Implicits._
              src.out ~> snk.in
              ClosedShape
            })
