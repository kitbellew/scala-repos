...
class InterpreterStressSpec extends AkkaSpec with GraphInterpreterSpecKit {
  ...
  "Interpreter" must {
    ...
    "work with a massive chain of maps" in
      new OneBoundedSetup[Int](Vector.fill(chainLength)(map): _*) {
        ...
        info(
          s"Chain finished in $time seconds ${(chainLength * repetition) /
            (time * 1000 * 1000)} million maps/s")
      }

    "work with a massive chain of maps with early complete" in
      new OneBoundedSetup[Int](
        Vector.fill(halfLength)(map) ++ Seq(Take(repetition / 2).toGS) ++
    ...
    "work with a massive chain of takes" in
      new OneBoundedSetup[Int](Vector.fill(chainLength / 10)(Take(1))) {
        lastEvents() should be(Set.empty)
    ...
    "work with a massive chain of drops" in
      new OneBoundedSetup[Int](Vector.fill(chainLength / 1000)(dropOne): _*) {
        lastEvents() should be(Set.empty)
