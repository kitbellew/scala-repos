...
class FlowZipWithSpec extends BaseTwoStreamsSetup {
  ...
  override def setup(p1: Publisher[Int], p2: Publisher[Int]) = {
    ...
    Source.fromPublisher(p1).zipWith(Source.fromPublisher(p2))(_ + _).runWith(
  ...
  "A ZipWith for Flow " must {
    ...
    "work in the happy case" in {
      ...
      Source(1 to 4).zipWith(Source(10 to 40 by 10))(
        (_: Int) + (_: Int)).runWith(Sink.fromSubscriber(probe))
    ...
    "work in the sad case" in {
      val probe = TestSubscriber.manualProbe[Outputs]()
      Source(1 to 4).zipWith(Source(-2 to 2))((_: Int) / (_: Int)).runWith(
