  ...
  "Sliding" must {
    ...
    def check(gen: Gen[(Int, Int, Int)]): Unit =
      forAll(gen, MinSize(1000), MaxSize(1000)) {
        case (len, win, step) ⇒
          val af = Source.fromIterator(() ⇒ Iterator.from(0).take(len)).sliding(
            win,
            step).runFold(Seq.empty[Seq[Int]])(_ :+ _)
          val cf = Source.fromIterator(() ⇒
            Iterator.from(0).take(len).sliding(win, step)).runFold(
            Seq.empty[Seq[Int]])(_ :+ _)
    ...
    "work with empty sources" in assertAllStagesStopped {
      Source.empty.sliding(1).runForeach(testActor ! _).map(_ ⇒
        "done") pipeTo testActor
      expectMsg("done")
    }
  }
}
