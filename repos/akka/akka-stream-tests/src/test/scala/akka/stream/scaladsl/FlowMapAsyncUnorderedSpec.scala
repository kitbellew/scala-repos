...
class FlowMapAsyncUnorderedSpec extends AkkaSpec {
  ...
  "A Flow with mapAsyncUnordered" must {
    ...
    "resume after multiple failures" in assertAllStagesStopped {
      ...
      Await.result(
        Source(futures)
          .mapAsyncUnordered(2)(identity).withAttributes(
    ...
    "signal NPE when future is completed with null" in {
      ...
      val p = Source(List("a", "b")).mapAsyncUnordered(4)(elem ⇒
    ...
    "handle cancel properly" in assertAllStagesStopped {
      ...
      Source.fromPublisher(pub).mapAsyncUnordered(4)(Future.successful).runWith(
