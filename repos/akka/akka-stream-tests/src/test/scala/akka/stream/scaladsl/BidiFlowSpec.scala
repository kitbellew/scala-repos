...
class BidiFlowSpec extends AkkaSpec {
  ...
  "A BidiFlow" must {
    ...
    "work as a Flow that is open on the right" in {
      ...
      val result = Source(List(ByteString("1"), ByteString("2"))).via(f)
    ...
    "work when reversed" in {
      ...
      val f = Flow[Int].map(_.toString).join(inverse.reversed)
        .join(bidi.reversed)
      val result = Source(List(1, 2, 3)).via(f).limit(10).runWith(Sink.seq)
      Await.result(result, 1.second) should ===(Seq("5", "6", "7"))
    }
    ...
    "combine materialization values" in assertAllStagesStopped {
      ...
      val right = Flow
        .fromGraph(GraphDSL.create(Sink.head[immutable.Seq[Long]]) {
          implicit b ⇒ sink ⇒
            val flow = b.add(Flow[Long].grouped(10))
            flow ~> sink
            FlowShape(flow.in, b.add(Source.single(ByteString("10"))).out)
        })
      val ((l, m), r) = left.joinMat(bidiMat)(Keep.both)
        .joinMat(right)(Keep.both).run()
    ...
    "suitably override attribute handling methods" in {
      ...
      val b: BidiFlow[Int, Long, ByteString, String, NotUsed] = bidi
        .withAttributes(name("")).async.named("")
    }
