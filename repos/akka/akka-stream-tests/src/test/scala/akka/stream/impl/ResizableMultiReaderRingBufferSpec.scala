...
class ResizableMultiReaderRingBufferSpec extends WordSpec with ShouldMatchers {
  ...
  "A ResizableMultiReaderRingBuffer" should {
    ...
    "initially be empty (1)" in
      new Test(iSize = 2, mSize = 4, cursorCount = 1) {
        inspect shouldEqual "0 0 (size=0, writeIx=0, readIx=0, cursors=1)"
      }

    "initially be empty (2)" in
      new Test(iSize = 4, mSize = 4, cursorCount = 3) {
        inspect shouldEqual "0 0 0 0 (size=0, writeIx=0, readIx=0, cursors=3)"
      }

    "fail reads if nothing can be read" in
      new Test(iSize = 4, mSize = 4, cursorCount = 3) {
    ...
    "fail writes if there is no more space" in
      new Test(iSize = 4, mSize = 4, cursorCount = 2) {
        write(1) shouldEqual true
        write(2) shouldEqual true
        write(3) shouldEqual true
    ...
    "automatically grow if possible" in
      new Test(iSize = 2, mSize = 8, cursorCount = 2) {
        write(1) shouldEqual true
        ...
        write(7) shouldEqual true
        inspect shouldEqual
          "3 4 5 6 7 0 0 0 (size=5, writeIx=5, readIx=0, cursors=2)"
        read(0) shouldEqual 4
        read(0) shouldEqual 5
        read(0) shouldEqual 6
        read(0) shouldEqual 7
        ...
        read(1) shouldEqual null
        inspect shouldEqual
          "0 0 0 0 0 0 0 0 (size=0, writeIx=5, readIx=5, cursors=2)"
      }
      ...
      class StressTestCursor(cursorNr: Int, run: Int) extends Cursor {
        ...
        def tryReadAndReturnTrueIfDone(buf: TestBuffer): Boolean = {
          ...
          try {
            val x = buf.read(this)
            log("OK\n")
            if (x != counter) fail(s"""|Run $run, cursorNr $cursorNr, counter $counter: got unexpected $x
                         |  Buf: ${buf.inspect}
