...
class GraphStageTimersSpec extends AkkaSpec {
  ...
  "GraphStage timer support" must {
    ...
    def setupIsolatedStage: SideChannel = {
      ...
      val stopPromise = Source.maybe[Nothing]
        .via(new TestStage(testActor, channel)).to(Sink.ignore).run()
    ...
    "resubmit single-shot timer" in {
      ...
      within(2.5.seconds) {
        ...
        within(1.second) { expectMsg(Tick(2)) }
        expectNoMsg(1.second)
      }
    ...
    "correctly cancel a named timer" in {
      ...
      within(500.millis) { expectMsg(TestCancelTimerAck) }
      within(300.millis, 1.second) { expectMsg(Tick(1)) }
      expectNoMsg(1.second)
    ...
    "receive and cancel a repeated timer" in {
      ...
      val seq = receiveWhile(2.seconds) { case t: Tick ⇒ t }
    ...
    "produce scheduled ticks as expected" in assertAllStagesStopped {
      ...
      Source.fromPublisher(upstream).via(new TestStage2)
