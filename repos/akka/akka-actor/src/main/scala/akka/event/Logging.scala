...
trait LoggingBus extends ActorEventBus {
  ...
  private def setUpStdoutLogger(config: Settings) {
    ...
    AllLogLevels filter (level >= _) foreach
  ...
  private[akka] def startDefaultLoggers(system: ActorSystemImpl) {
    ...
    try {
        ...
        if (system.settings.DebugUnhandledMessage)
          subscribe(
            system.systemActorOf(
              Props(new Actor {
                def receive = {
                  case UnhandledMessage(msg, sender, rcp) ⇒ publish(Debug(
                      rcp.path.toString,
                      rcp.getClass,
      ...
      case e: Exception ⇒
        ...
        e.printStackTrace()
        throw new ConfigurationException(
          "Could not start logger due to [" + e.toString + "]")
    }
  }
  ...
  private[akka] def stopDefaultLoggers(system: ActorSystem) {
    ...
    publish(
      Debug(simpleName(this), this.getClass, "all default loggers stopped"))
  }
...
object Logging {
  ...
  /**
    ...
    */
  class LogEventException(val event: LogEvent, cause: Throwable)
  ...
  case class Error(
      ...
      message: Any = "")
      extends LogEvent {
    def this(logSource: String, logClass: Class[_], message: Any) =
  ...
  object Error {
    ...
    def apply(
        ...
        logClass: Class[_],
        message: Any,
        mdc: MDC) = new Error2(cause, logSource, logClass, message, mdc)
  ...
  /**
    ...
    */
  final case class InitializeLogger(bus: LoggingBus)
...
trait LoggingAdapter {
  ...
  def format(t: String, arg: Any*): String = {
    ...
    while (p < arg.length) {
      ...
      if (index == -1) {
        sb.append(rest).append(" WARNING arguments left: ")
          .append(arg.length - p)
        rest = ""
        p = arg.length
      } else {
...
object NoLogging extends LoggingAdapter {
  ...
  final protected override def notifyError(
      cause: Throwable,
      message: String): Unit = ()
