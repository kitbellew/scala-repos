...
object Helpers {
  ...
  val isWindows: Boolean = System.getProperty("os.name", "").toLowerCase(
    Locale.ROOT).indexOf("win") >= 0
  ...
  def compareIdentityHash(a: AnyRef, b: AnyRef): Int = {
     ...
     */
    val diff = ((System.identityHashCode(
      a) & 0xFFFFFFFFL) - (System.identityHashCode(b) & 0xFFFFFFFFL))
