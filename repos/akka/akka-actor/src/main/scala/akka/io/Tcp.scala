...
class TcpExt(system: ExtendedActorSystem) extends IO.Extension {
  ...
  class Settings private[TcpExt] (_config: Config)
    ...
    val NrOfSelectors
        : Int = getInt("nr-of-selectors") requiring (_ > 0, "nr-of-selectors must be > 0")

    val BatchAcceptLimit
        : Int = getInt("batch-accept-limit") requiring (_ > 0, "batch-accept-limit must be > 0")
    ...
    val ReceivedMessageSizeLimit
        : Int = getString("max-received-message-size") match {
    ...
    val FinishConnectRetries
        : Int = getInt("finish-connect-retries") requiring (_ > 0,
