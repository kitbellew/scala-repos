...
class AkkaConsoleReporter(
  ...
  def printMetrics[T <: Metric](
      ...
      printer: T ⇒ Unit)(implicit clazz: ClassTag[T]) {
    if (!metrics.isEmpty) {
  ...
  private def printMeter(meter: Meter) {
    ...
    output.print(
      "         mean rate = %2.2f events/%s%n".format(
        convertRate(meter.getMeanRate),
        getRateUnit))
    output.print(
      "     1-minute rate = %2.2f events/%s%n".format(
        convertRate(meter.getOneMinuteRate),
        getRateUnit))
    output.print(
      "     5-minute rate = %2.2f events/%s%n".format(
        convertRate(meter.getFiveMinuteRate),
        getRateUnit))
    output.print(
      "    15-minute rate = %2.2f events/%s%n".format(
  ...
  private def printTimer(timer: Timer) {
    ...
    output.print(
      "         mean rate = %2.2f calls/%s%n".format(
        convertRate(timer.getMeanRate),
        getRateUnit))
    output.print(
      "     1-minute rate = %2.2f calls/%s%n".format(
        convertRate(timer.getOneMinuteRate),
        getRateUnit))
    output.print(
      "     5-minute rate = %2.2f calls/%s%n".format(
        convertRate(timer.getFiveMinuteRate),
        getRateUnit))
    output.print(
      "    15-minute rate = %2.2f calls/%s%n".format(
    ...
    output.print(
      "               max = %2.2f %s%n".format(
        ...
        getDurationUnit))
    output.print(
      "              mean = %2.2f %s%n".format(
        convertDuration(snapshot.getMean),
        getDurationUnit))
    output.print(
      "            stddev = %2.2f %s%n".format(
        convertDuration(snapshot.getStdDev),
        getDurationUnit))
    output.print(
      "            median = %2.2f %s%n".format(
