...
object TestActorRef {
    ...
    apply[T](
      Props({
        system.asInstanceOf[ExtendedActorSystem].dynamicAccess
          .createInstanceFor[T](t.runtimeClass, Nil).recover(
            dynamicCreateRecover).get
      }),
      name)
    ...
    apply[T](
      Props({
        system.asInstanceOf[ExtendedActorSystem]
          .dynamicAccess.createInstanceFor[T](t.runtimeClass, Nil).recover(
            dynamicCreateRecover).get
      }),
      supervisor,
      name
  ...
  def create[T <: Actor](
      ...
      props: Props,
      supervisor: ActorRef,
      name: String): TestActorRef[T] = apply(props, supervisor, name)(system)
  ...
  def create[T <: Actor](
      system: ActorSystem,
      props: Props,
      supervisor: ActorRef): TestActorRef[T] = apply(props, supervisor)(system)
  ...
  def create[T <: Actor](
      system: ActorSystem,
      props: Props,
      name: String): TestActorRef[T] = apply(props, name)(system)
