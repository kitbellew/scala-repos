...
class MarshalSpec extends AkkaSpec {
  ...
  "use marshal" in {
    ...
    val entity = Await
      .result(entityFuture, 1.second) // don't block in non-test code!
    ...
    val response = Await
      .result(responseFuture, 1.second) // don't block in non-test code!
    ...
    val request = HttpRequest(headers = List(
      headers.Accept(MediaTypes.`application/json`)))
    val responseText = "Plaintext"
    val respFuture = Marshal(responseText)
      .toResponseFor(request) // with content negotiation!
