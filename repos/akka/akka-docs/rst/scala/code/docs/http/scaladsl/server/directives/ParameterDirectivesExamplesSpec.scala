...
class ParameterDirectivesExamplesSpec
  ...
  "required-1" in {
    ...
    // tests:
    Get("/?color=blue&backgroundColor=red") ~> route ~>
      check {
        responseAs[String] shouldEqual
          "The color is 'blue' and the background is 'red'"
      }
    Get("/?color=blue") ~> Route.seal(route) ~>
      check {
        status shouldEqual StatusCodes.NotFound
        responseAs[String] shouldEqual
  ...
  "required-value" in {
    ...
    // tests:
    Get("/?color=blue&action=true") ~> route ~>
      check {
        responseAs[String] shouldEqual "The color is 'blue'."
      }

    Get("/?color=blue&action=false") ~> Route.seal(route) ~>
      check {
        status shouldEqual StatusCodes.NotFound
        responseAs[String] shouldEqual
  ...
  "mapped-value" in {
    ...
    // tests:
    Get("/?color=blue&count=42") ~> route ~>
      check {
        responseAs[String] shouldEqual
          "The color is 'blue' and you have 42 of it."
      }

    Get("/?color=blue&count=blub") ~> Route.seal(route) ~>
      check {
  ...
  "repeated" in {
    ...
    Get("/?color=blue&city=Chicago") ~> Route.seal(route) ~>
      check {
        responseAs[String] === "The color is 'blue' and the city is Chicago."
      }

    Get("/?color=blue&city=Chicago&city=Boston") ~> Route.seal(route) ~>
      check {
        responseAs[String] ===
  ...
  "mapped-repeated" in {
    ...
    Get("/?color=blue&distance=5") ~> Route.seal(route) ~>
      check {
        responseAs[String] === "The color is 'blue' and the distance is 5."
      }

    Get("/?color=blue&distance=5&distance=14") ~> Route.seal(route) ~>
      check {
        responseAs[String] ===
          "The color is 'blue' and the distances are 5, 14."
      }
  }
  "parameterMap" in {
