...
class DebuggingDirectivesExamplesSpec extends RoutingSpec {
  ...
  "logRequestResult" in {
    ...
    def printRequestMethodAndResponseStatus(req: HttpRequest)(res: Any): Unit =
      println(
        requestMethodAndResponseStatusAsInfo(req)(res).map(_.obj.toString)
          .getOrElse(""))
    val logRequestResultPrintln = DebuggingDirectives
  ...
  "logResult" in {
    ...
    val logResultPrintln = DebuggingDirectives
