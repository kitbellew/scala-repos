...
object SimplestCustomSerializer {
  ...
  class SimplestPossiblePersonSerializer extends SerializerWithStringManifest {
    ...
    override def toBinary(obj: AnyRef): Array[Byte] =
      obj match {
        case p: Person =>
          s"""${p.name}|${p.surname}""".getBytes(Utf8)
        case _ =>
          throw new IllegalArgumentException(
...
class RemovedEventsAwareSerializer extends SerializerWithStringManifest {
  ...
  override def toBinary(o: AnyRef): Array[Byte] =
    o match {
      case _ =>
        o.toString.getBytes(utf8) // example serialization
    }
...
class SkippedEventsAwareAdapter extends EventAdapter {
  ...
  override def fromJournal(event: Any, manifest: String) =
    event match {
      case EventDeserializationSkipped =>
        EventSeq.empty
      case _ =>
        EventSeq(event)
    }
...
class RenamedEventAwareSerializer extends SerializerWithStringManifest {
  ...
  override def toBinary(o: AnyRef): Array[Byte] =
    o match {
      case SamplePayload(data) =>
        s"""$data""".getBytes(Utf8)
  ...
  override def fromBinary(bytes: Array[Byte], manifest: String): AnyRef =
    manifest match {
      case OldPayloadClassName =>
        SamplePayload(new String(bytes, Utf8))
