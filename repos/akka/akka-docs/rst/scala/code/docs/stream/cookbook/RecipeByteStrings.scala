...
class RecipeByteStrings extends RecipeSpec {
  ...
  "Recipes for bytestring streams" must {
    ...
    "have a working chunker" in {
      ...
      chunks.fold(ByteString())(_ ++ _) should
        be(ByteString(1, 2, 3, 4, 5, 6, 7, 8, 9))
    }
    ...
    "have a working bytes limiter" in {
      ...
      val bytes1 = Source(List(
        ByteString(1, 2),
        ByteString(3),
        ByteString(4, 5, 6),
        ByteString(7, 8, 9)))
      val bytes2 = Source(List(
        ByteString(1, 2),
        ByteString(3),
        ByteString(4, 5, 6),
        ByteString(7, 8, 9, 10)))
    ...
    "demonstrate compacting" in {
      ...
      Await.result(compacted.limit(10).runWith(Sink.seq), 3.seconds)
