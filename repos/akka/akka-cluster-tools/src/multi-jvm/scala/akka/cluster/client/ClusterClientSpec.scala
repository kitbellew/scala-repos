...
class ClusterClientSpec
  ...
  "A ClusterClient" must {
    ...
    "re-establish connection to another receptionist when server is shutdown" in
      within(30 seconds) {
        ...
        runOn(client) {
          val c = system.actorOf(
            ClusterClient
              .props(ClusterClientSettings(system).withInitialContacts(
                initialContacts)),
            "client2")
    ...
    "re-establish connection to receptionist after partition" in
      within(30 seconds) {
        runOn(client) {
          val c = system.actorOf(
            ClusterClient
          ...
          testConductor
            .passThrough(client, receptionistRoleName, Direction.Both).await
    ...
    "re-establish connection to receptionist after server restart" in
      within(30 seconds) {
        runOn(client) {
        ...
        runOn(remainingServerRoleNames.toSeq: _*) {
          ...
          val sys2 = ActorSystem(
            ...
            ConfigFactory.parseString(
              "akka.remote.netty.tcp.port=" + Cluster(system)
                .selfAddress.port.get).withFallback(system.settings.config))
