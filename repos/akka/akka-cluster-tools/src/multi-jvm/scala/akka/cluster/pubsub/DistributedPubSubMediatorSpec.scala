  ...
  class Sender extends Actor {
    ...
    def receive = {
      case in: String ⇒
        val out = in.toUpperCase
        mediator !
          Send(path = "/user/destination", msg = out, localAffinity = true)
    }
  }
  //#sender
...
class DistributedPubSubMediatorSpec
  ...
  "A DistributedPubSubMediator" must {
    ...
    "replicate users to new node" in
      within(20 seconds) {
        join(third, first)
    ...
    "keep track of removed users" in
      within(15 seconds) {
        runOn(first) {
    ...
    "demonstrate usage of Publish" in
      within(15 seconds) {
        def later(): Unit = {
          awaitCount(10)
        }
    ...
    "send-all to all other nodes" in
      within(15 seconds) {
        ...
        runOn(third) {
          chatUser("u5") !
            TalkToOthers("/user/u11", "hi") // sendToAll to all other nodes
        }
    ...
    "send one message to each group" in
      within(20 seconds) {
        runOn(first) {
    ...
    "transfer delta correctly" in {
      ...
      runOn(first) {
        ...
        mediator !
          Status(versions = deltaBuckets1.map(b ⇒ b.owner -> b.version).toMap)
        val deltaBuckets2 = expectMsgType[Delta].buckets
        deltaBuckets1.map(_.content.size).sum should ===(500)

        mediator !
          Status(versions = deltaBuckets2.map(b ⇒ b.owner -> b.version).toMap)
    ...
    "remove entries when node is removed" in
      within(30 seconds) {
        mediator ! Count
    ...
    "receive proper unsubscribeAck message" in
      within(15 seconds) {
        runOn(first) {
    ...
    "get topics after simple publish" in
      within(15 seconds) {
        runOn(first) {
