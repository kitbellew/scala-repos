...
class StreamTestKitSpec extends AkkaSpec {
  ...
  "A TestSink Probe" must {
    ...
    "#toStrict with failing source" in {
      val error = intercept[AssertionError] {
        Source.fromIterator(() ⇒
          ...
          }).runWith(TestSink.probe).toStrict(300.millis)
      }
    ...
    "#expectNextOrError with right exception" in {
      Source.failed[Int](ex).runWith(TestSink.probe).request(4)
    ...
    "#expectError" in {
      Source.failed[Int](ex).runWith(TestSink.probe).request(1)
    ...
    "#expectNextOrComplete with completion" in {
      Source.single(1).runWith(TestSink.probe).request(4)
        .expectNextOrComplete(1).expectNextOrComplete(1337)
    }
