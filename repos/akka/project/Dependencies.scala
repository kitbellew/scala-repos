...
object Dependencies {
  ...
  val Versions = Seq(
    ...
    scalaStmVersion := sys.props.get("akka.build.scalaStmVersion").getOrElse(
      "0.7"),
    scalaCheckVersion := sys.props.get(
      "akka.build.scalaCheckVersion").getOrElse("1.11.6"),
    scalaTestVersion := (
  ...
  object Compile {
    ...
    val camelCore =
      "org.apache.camel" % "camel-core" % "2.13.4" exclude ("org.slf4j", "slf4j-api") // ApacheV2
    ...
    val uncommonsMath =
      "org.uncommons.maths" % "uncommons-maths" % "1.2.2a" exclude ("jfree", "jcommon") exclude ("jfree", "jfreechart") // ApacheV2
    ...
    // ssl-config
    val sslConfigAkka =
      "com.typesafe" %% "ssl-config-akka" % "0.1.3" // ApacheV2
...
object DependencyHelpers {
  ...
  def versionDependentDeps(
    ...
    libraryDependencies <++= scalaVersion(version =>
      modules.flatMap(m => m.modules(version)))
