  ...
  override lazy val buildSettings = Seq(
    sourceBranch in Global in ValidatePR := {
      sys.env.get(SourceBranchEnvVarName) orElse
        sys.env.get(SourcePullIdJenkinsEnvVarName).map(
          "pullreq/" + _) getOrElse // Set by "GitHub pull request builder plugin"
        "HEAD"
    },
      ...
      sys.env.get(PullIdEnvVarName).map(_.toInt) flatMap {
        prId =>
          ...
          try {
            ...
            val gh = GitHubBuilder.fromEnvironment().withOAuthToken(
              GitHub.envTokenOrThrow).build()
            val comments =
    ...
    changedDirectories in Global in ValidatePR := {
      ...
      val dirtyModuleNames: Set[String] =
        if (runningOnJenkins) Set.empty
          ...
          val dirtyDirectories = statusOutput
            ...
            .toSet
          log.info(
            "Detected uncomitted changes in directories (including in dependency analysis): " + dirtyDirectories.mkString(
              "[",
              ",",
              "]"))
          dirtyDirectories
      ...
      log.info(
        "Detected changes in directories: " + allModuleNames.mkString(
          "[",
          ", ",
          "]"))
      allModuleNames
  ...
  override lazy val projectSettings =
    inConfig(ValidatePR)(Defaults.testTasks) ++ Seq(
      ...
      validatePullRequest := Def.taskDyn {
        ...
        validationTasks.map(taskKey => Def.task { taskKey.value }).foldLeft(
          zero) { (acc, current) =>
          acc.zipWith(current) {
            case (taskSeq, task) =>
