...
object Release {
  ...
  def buildReleaseCommand =
    Command.command("buildRelease") { state =>
      ...
      val (state2, Seq(api, japi)) = extracted
        .runTask(unidoc in Compile, state1)
  ...
  def uploadReleaseCommand =
    Command.command("uploadRelease") { state =>
      val extracted = Project.extract(state)
