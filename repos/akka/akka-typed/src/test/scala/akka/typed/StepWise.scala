...
object StepWise {
  ...
  private final case class Message(
      ...
      trace: Trace)
      extends AST
  private final case class MultiMessage(
      timeout: FiniteDuration,
      count: Int,
      f: (Seq[Any], Any) ⇒ Any,
      trace: Trace)
  ...
  final case class Steps[T, U](ops: List[AST], keepTraces: Boolean) {
    ...
    def expectFailureKeep(timeout: FiniteDuration)(
      ...
      copy(ops = Failure(
        timeout,
        (failed, value) ⇒
          f.asInstanceOf[(Failed, Any) ⇒ Failed.Decision](failed, value) ->
            value,
        getTrace()) :: ops)
      ...
      case Termination(t, f, trace) :: tail ⇒
        ...
        Full {
          case Sig(_, ReceiveTimeout) ⇒
            throwTimeout(
              trace,
              s"timeout of $t expired while waiting for termination")
          case Sig(_, t: Terminated) ⇒
            run(ctx, tail, f(t, value))
          case other ⇒
            throwIllegalState(
              trace,
              s"unexpected $other while waiting for termination")
        }
      case Nil ⇒
        Stopped
    }
