...
class TypedSpec(config: Config)
  ...
  val blackhole = await(
    system ?
      Create(Props(ScalaDSL.Full[Any] { case _ ⇒ ScalaDSL.Same }), "blackhole"))
...
object TypedSpec {
  ...
  def guardian(outstanding: Map[ActorRef[_], ActorRef[Status]] = Map.empty)
      : Behavior[Command] =
    FullTotal {
      case Sig(ctx, f @ t.Failed(ex, test)) ⇒ outstanding get test match {
          case Some(reply) ⇒
            reply ! Failed(ex)
      ...
      case Sig(ctx, Terminated(test)) ⇒ outstanding get test match {
          case Some(reply) ⇒
            reply ! Success
            guardian(outstanding - test)
          case None ⇒ Same
