...
object RejectionHandler {
  ...
  /**
    ...
    */
  def default =
    newBuilder().handleAll[SchemeRejection] { rejections ⇒
      val schemes = rejections.map(_.supported).mkString(", ")
      complete(
        (BadRequest, "Uri scheme not allowed, supported schemes: " + schemes))
    }.handleAll[MethodRejection] { rejections ⇒
      val (methods, names) = rejections.map(r ⇒ r.supported -> r.supported.name)
        .unzip
      complete((
        MethodNotAllowed,
        List(Allow(methods)),
