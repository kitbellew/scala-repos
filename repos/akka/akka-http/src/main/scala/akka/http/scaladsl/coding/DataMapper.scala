...
object DataMapper {
  ...
  implicit val mapRequest: DataMapper[HttpRequest] = mapMessage(
    mapRequestEntity)((m, f) ⇒ m.withEntity(f(m.entity)))
  implicit val mapResponse: DataMapper[HttpResponse] = mapMessage(
    mapResponseEntity)((m, f) ⇒ m.withEntity(f(m.entity)))
