  ...
  def withRequestTimeout(
    ...
    Directive { inner ⇒ ctx ⇒
      ctx.request.header[`Timeout-Access`] match {
        ...
        case _ ⇒
          ctx.log
            .warning("withRequestTimeout was used in route however no request-timeout is set!")
      }
      inner()(ctx)
    }
  ...
  def withRequestTimeoutResponse(
    ...
    Directive { inner ⇒ ctx ⇒
      ctx.request.header[`Timeout-Access`] match {
        ...
        case _ ⇒
          ctx.log
            .warning("withRequestTimeoutResponse was used in route however no request-timeout is set!")
      }
      inner()(ctx)
    }
