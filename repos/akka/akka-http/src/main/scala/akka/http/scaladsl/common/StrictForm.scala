...
object StrictForm {
  ...
  object Field {
    ...
    def unmarshallerFromFSU[T](
      ...
      Unmarshaller.withMaterializer(implicit ec ⇒
        implicit mat ⇒ {
          ...
          case FromPart(value) ⇒
            val charsetName = value.entity.contentType.asInstanceOf[
              ContentType.NonBinary].charset.nioCharset.name
    ...
    object FieldUnmarshaller extends LowPrioImplicits {
      implicit def fromBoth[T](implicit
    ...
    Unmarshaller.withMaterializer { implicit ec ⇒ implicit fm ⇒ entity ⇒
      ...
      def tryUnmarshalToMultipartForm: Future[StrictForm] =
        for {
          ...
          strictMultiPartFD ← multiPartFD.toStrict(
            10.seconds).fast // TODO: make timeout configurable
        } yield {
          new StrictForm {
