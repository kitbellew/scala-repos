...
private[pref] final class DataForm {
  ...
  val pref = Form(
    mapping(
      ...
      "autoThreefold" ->
        number.verifying(Pref.AutoThreefold.choices.toMap contains _),
      "takeback" -> number.verifying(Pref.Takeback.choices.toMap contains _),
      "clockTenths" ->
        number.verifying(Pref.ClockTenths.choices.toMap contains _),
      ...
      "submitMove" ->
        number.verifying(Pref.SubmitMove.choices.toMap contains _),
      "insightShare" -> number.verifying(Set(0, 1, 2) contains _),
      "confirmResign" ->
        number.verifying(Pref.ConfirmResign.choices.toMap contains _),
