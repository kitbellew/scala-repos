...
private[report] final class ReportApi {
            ...
            !isAlreadySlain(report, user) ?? {
              ...
              if (by.id == UserRepo.lichessId)
                reportTube.coll.update(
                  ...
                  Json.obj(
                    "$set" ->
                      (reportTube.toMongo(report).get - "processedBy" -
                        "_id"))) flatMap { res =>
  ...
  private def monitorUnprocessed =
    nbUnprocessed foreach { nb => lila.mon.mod.report.unprocessed(nb) }
  ...
  def autoBoostReport(userId: String, accompliceId: String): Funit = {
    UserRepo.byId(userId) zip UserRepo.byId(accompliceId) zip
  ...
  def process(id: String, by: User): Funit =
    $find byId id flatMap {
      _ ?? { report =>
        $update(
          Json.obj("user" -> report.user, "reason" -> report.reason) ++
            unprocessedSelect,
