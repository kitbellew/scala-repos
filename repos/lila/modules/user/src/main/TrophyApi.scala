...
final class TrophyApi(coll: lila.db.Types.Coll) {
  ...
  private implicit val trophyKindBSONHandler =
    new BSONHandler[BSONString, Trophy.Kind] {
      def read(bsonString: BSONString): Trophy.Kind =
        Trophy.Kind byKey bsonString.value err
          s"No such trophy kind: ${bsonString.value}"
  ...
  def awardMarathonWinner(userId: String): Funit =
    award(userId, Trophy.Kind.MarathonWinner)

  def findByUser(user: User, max: Int = 12): Fu[List[Trophy]] =
    coll.find(BSONDocument("user" -> user.id)).cursor[Trophy]()
      .collect[List](max)
}
