...
object UserRepo {
  ...
  def byEmail(email: String): Fu[Option[User]] =
    $find one Json.obj(F.email -> email)
  ...
  def nameds(usernames: List[String]): Fu[List[User]] =
    $find byIds usernames.map(normalize)
  ...
  def byIdsSortRating(ids: Iterable[ID], nb: Int) =
    coll.find(
      ...
      .cursor[User](ReadPreference.secondaryPreferred).collect[List](nb)
  ...
  def idsByIdsSortRating(ids: Iterable[ID], nb: Int): Fu[List[User.ID]] =
    coll.find(
      ...
      .sort(BSONDocument(s"perfs.standard.gl.r" -> -1))
      .cursor[BSONDocument](ReadPreference.secondaryPreferred).collect[List](nb)
  ...
  def orderByGameCount(u1: String, u2: String): Fu[Option[(String, String)]] = {
    coll.find(
      ...
      BSONDocument(s"${F.count}.game" -> true)).cursor[BSONDocument]()
  ...
  def firstGetsWhite(u1O: Option[String], u2O: Option[String]): Fu[Boolean] =
    (u1O |@| u2O).tupled.fold(fuccess(scala.util.Random.nextBoolean)) {
      case (u1, u2) =>
        coll.find(
          BSONDocument("_id" -> BSONDocument("$in" -> BSONArray(u1, u2))),
          BSONDocument("_id" -> true)).sort(BSONDocument(F.colorIt -> 1))
  ...
  def engineSelect(v: Boolean) =
    Json.obj(F.engine -> v.fold(JsBoolean(true), $ne(true)))
  def trollSelect(v: Boolean) =
  ...
  def isEngine(id: ID): Fu[Boolean] =
    $count.exists($select(id) ++ engineSelect(true))

  def isTroll(id: ID): Fu[Boolean] =
    $count.exists($select(id) ++ trollSelect(true))
  ...
  def passwd(id: ID, password: String): Funit =
    $primitive.one($select(id), "salt")(_.asOpt[String]) flatMap { saltOption =>
      saltOption ?? { salt =>
        $update(
          $select(id),
          $set(Json.obj("password" -> hash(password, salt), "sha512" -> false)))
      }
    }

  def email(id: ID, email: String): Funit = $update.field(id, F.email, email)

  def email(id: ID): Fu[Option[String]] =
    $primitive.one($select(id), F.email)(_.asOpt[String])
  ...
  def idsSumToints(ids: Iterable[String]): Fu[Int] =
    ids.nonEmpty ?? coll.aggregate(
      ...
      .map(_.documents.headOption flatMap { _.getAs[Int](F.toints) }).map(~_)
  ...
  def countEngines(userIds: List[String]): Fu[Int] =
    coll.count(
      BSONDocument("_id" -> BSONDocument("$in" -> userIds), F.engine -> true)
        .some)
  ...
  private def newUser(
    ...
    BSONDocument(
      ...
      F.username -> username,
      F.email -> email,
      F.mustConfirmEmail -> (email.isDefined && mobileApiVersion.isEmpty)
        .option(DateTime.now),
