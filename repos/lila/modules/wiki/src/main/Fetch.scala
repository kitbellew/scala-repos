...
private[wiki] final class Fetch(gitUrl: String, markdownPath: String)(implicit
  ...
  def apply: Funit =
    getFiles flatMap { files =>
      val (defaultPages, langPages) = files.map(filePage).flatten partition
        (_.isDefaultLang)
      val newLangPages =
        (
          langPages map { page =>
            defaultPages find
              (_.number == page.number) map { default =>
                page.copy(slug = default.slug)
              }
          }
        ).flatten
      $remove($select.all) >>
        (newLangPages ::: defaultPages).map($insert(_)).sequenceFu.void
    }
  ...
  private def filePage(file: File): Option[Page] = {
    ...
    if (name == "Home")
      ...
      Page.make(name, toHtml(file))
  }

  private def getFiles: Fu[List[File]] =
    Future {
