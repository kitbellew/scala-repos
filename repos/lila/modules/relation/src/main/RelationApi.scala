...
final class RelationApi(
  ...
  def fetchRelation(u1: ID, u2: ID): Fu[Option[Relation]] =
    coll.find(
      ...
      BSONDocument("r" -> true, "_id" -> false)).one[BSONDocument].map {
  ...
  def fetchFollows(u1: ID, u2: ID) =
    coll.count(BSONDocument("_id" -> makeId(u1, u2), "r" -> Follow).some)
      .map(0 !=)

  def fetchBlocks(u1: ID, u2: ID) =
    coll.count(BSONDocument("_id" -> makeId(u1, u2), "r" -> Block).some)
      .map(0 !=)
