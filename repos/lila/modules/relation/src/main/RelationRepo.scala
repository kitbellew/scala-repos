...
private[relation] object RelationRepo {
  ...
  private def relaters(userId: ID, relation: Relation): Fu[Set[ID]] =
    coll.distinct("u1", BSONDocument("u2" -> userId, "r" -> relation).some) map
      lila.db.BSON.asStringSet

  private def relating(userId: ID, relation: Relation): Fu[Set[ID]] =
    coll.distinct("u2", BSONDocument("u1" -> userId, "r" -> relation).some) map
      lila.db.BSON.asStringSet
  ...
  private def save(u1: ID, u2: ID, relation: Relation): Funit =
    $save(makeId(u1, u2), Json.obj("u1" -> u1, "u2" -> u2, "r" -> relation))
