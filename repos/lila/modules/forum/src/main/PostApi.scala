...
final class PostApi(
  ...
  def makePost(categ: Categ, topic: Topic, data: DataForm.PostData)(implicit
    ...
    lastNumberOf(topic) zip detectLanguage(data.text) zip userIds(
      topic) flatMap { case ((number, lang), topicUserIds) =>
      val post = Post.make(
        topicId = topic.id,
        author = data.author,
  ...
  def urlData(postId: String, troll: Boolean): Fu[Option[PostUrlData]] =
    get(postId) flatMap {
      case Some((topic, post)) if (!troll && post.troll) =>
        fuccess(none[PostUrlData])
      case Some((topic, post)) =>
        PostRepo(troll).countBeforeNumber(topic.id, post.number) map { nb =>
