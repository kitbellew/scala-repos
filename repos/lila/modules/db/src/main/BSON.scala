...
object BSON {
  ...
  def stringAnyValHandler[A](from: A => String, to: String => A) =
    new BSONHandler[BSONString, A] {
    ...
    implicit def MapReader[V](implicit
      ...
      new BSONDocumentReader[Map[String, V]] {
        def read(bson: BSONDocument): Map[String, V] = {
          ...
          val b = collection.immutable.Map.newBuilder[String, V]
          for (tuple <- bson.elements)
            b += (tuple._1 -> valueReader.read(tuple._2))
          b.result
        }
      }
  ...
  implicit def bsonArrayToListHandler[T](implicit
    ...
    new BSONHandler[BSONArray, List[T]] {
      def read(array: BSONArray) =
        readStream(array, reader.asInstanceOf[BSONReader[BSONValue, T]]).toList
  ...
  final class Reader(val doc: BSONDocument) {
    ...
    def getD[A](k: String, default: A)(implicit
        reader: BSONReader[_ <: BSONValue, A]): A =
