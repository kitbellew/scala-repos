...
object JsTube {
  ...
  object Helpers {
    ...
    def rename(from: Symbol, to: Symbol) =
      __.json update
        ((__ \ to).json copyFrom (__ \ from).json.pick) andThen
        (__ \ from).json.prune

    def readDate(field: Symbol) = (__ \ field)
      .json
      .update(
        of[JsObject] map { o =>
    ...
    def writeDate(field: Symbol) = (__ \ field)
      .json
      .update(
    ...
    def writeDateOpt(field: Symbol) = (__ \ field)
      .json
      .update(
        of[JsNumber] map { millis =>
          Json.obj("$date" -> millis)
