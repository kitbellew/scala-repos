...
private final class ChallengeRepo(coll: Coll, maxPerUser: Int) {
  ...
  def createdByChallengerId(userId: String): Fu[List[Challenge]] =
    coll.find(selectCreated ++ BSONDocument("challenger.id" -> userId))
      .sort(BSONDocument("createdAt" -> 1)).cursor[Challenge]().collect[List]()
  ...
  def removeByUserId(userId: String): Funit =
    coll.remove(BSONDocument(
      "$or" -> BSONArray(
        ...
        BSONDocument("destUser.id" -> userId)))).void
  ...
  private[challenge] def realTimeUnseenSince(
    ...
    coll.find(
      selectCreated ++ selectClock ++ BSONDocument(
        "seenAt" -> BSONDocument("$lt" -> date))).cursor[Challenge]()
      .collect[List](max)
    ...
    coll.update(
      ...
      BSONDocument(
        "$set" -> BSONDocument(
          ...
          "seenAt" -> DateTime.now,
          "expiresAt" -> inTwoWeeks))).void

  def setSeen(id: Challenge.ID) =
    coll.update(
      selectId(id),
      BSONDocument("$set" -> BSONDocument("seenAt" -> DateTime.now))).void
  ...
  def statusById(id: Challenge.ID) =
    coll.find(selectId(id), BSONDocument("status" -> true, "_id" -> false))
