...
private[timeline] final class EntryRepo(coll: Coll, userMax: Int) {
  ...
  private def userEntries(userId: String, max: Int): Fu[List[Entry]] =
    coll.find(BSONDocument("users" -> userId)).sort(BSONDocument("date" -> -1))
      .cursor[Entry]().collect[List](max)

  def findRecent(typ: String, since: DateTime) =
    coll
      .find(BSONDocument("typ" -> typ, "date" -> BSONDocument("$gt" -> since)))
      .cursor[Entry]().collect[List]()
