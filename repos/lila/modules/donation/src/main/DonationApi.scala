...
final class DonationApi(
  ...
  private def donatedByUser(userId: String): Fu[Int] =
    coll.aggregate(
      ...
      List(Group(BSONNull)("net" -> SumField("net")))).map {
  ...
  def list(nb: Int) =
    coll.find(decentAmount).sort(BSONDocument("date" -> -1)).cursor[Donation]()
      .collect[List](nb)
  ...
  def top(nb: Int): Fu[List[lila.user.User.ID]] =
    coll.aggregate(
      ...
      List(
        ...
        Limit(nb))).map { _.documents.flatMap { _.getAs[String]("_id") } }
  ...
  def progress: Fu[Progress] = {
    ...
    coll.find(
      ...
      BSONDocument("net" -> true, "_id" -> false)).cursor[BSONDocument]()
