...
object Event {
  ...
  case class Enpassant(pos: Pos, color: Color) extends Event {
    def typ = "enpassant"
    def data = Json.obj("key" -> pos.key, "color" -> color)
  }
  ...
  case class Clock(white: Float, black: Float) extends Event {
    def typ = "clock"
    def data =
      Json.obj("white" -> truncateAt(white, 2), "black" -> truncateAt(black, 2))
  }
  object Clock {
    def apply(clock: ChessClock): Clock =
      Clock(clock remainingTime Color.White, clock remainingTime Color.Black)
    def tenths(white: Int, black: Int): Clock =
      Clock(white.toFloat / 10, black.toFloat / 10)
  }
  ...
  case class Crowd(white: Boolean, black: Boolean, watchers: JsValue)
      extends Event {
    def typ = "crowd"
    def data =
      Json.obj("white" -> white, "black" -> black, "watchers" -> watchers)
  }
