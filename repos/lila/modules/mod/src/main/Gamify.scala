...
final class Gamify(logColl: Coll, reportColl: Coll, historyColl: Coll) {
  ...
  def history(orCompute: Boolean = true): Fu[List[HistoryMonth]] = {
    ...
    historyColl.find(BSONDocument())
      .sort(BSONDocument("year" -> -1, "month" -> -1)).cursor[HistoryMonth]()
      .collect[List]().flatMap { months =>
  ...
  private def actionLeaderboard(
    ...
    logColl.aggregate(
      Match(
        BSONDocument("date" -> dateRange(after, before), "mod" -> notLichess)),
