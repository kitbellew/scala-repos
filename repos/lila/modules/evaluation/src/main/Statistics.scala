...
object Statistics {
  ...
  def average[T](a: NonEmptyList[T])(implicit n: Numeric[T]): Double = {
    ...
    def average(a: List[T], sum: T, depth: Int): Double = {
      a match {
        ...
        case x :: xs =>
          average(xs, n.plus(sum, x), depth + 1)
      }
    }
  ...
  def noFastMoves(pov: lila.game.Pov): Boolean =
    pov.game.moveTimes(pov.color).count(2 >) <= 2
  ...
  def listDeviation[T](x: List[T])(implicit n: Numeric[T]): Double =
    x match {
      ...
      case _ :: Nil =>
        0
      case a :: b =>
        deviation(NonEmptyList.nel(a, b))
    }
}
