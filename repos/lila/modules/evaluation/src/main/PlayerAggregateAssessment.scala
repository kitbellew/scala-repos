...
case class PlayerAggregateAssessment(
  ...
  def action = {
    val markable: Boolean = !isGreatUser && isWorthLookingAt &&
      ...
      &&
        ...
        || (cheatingSum + likelyCheatingSum).toDouble /
          assessmentsCount >= 0.20 - relationModifier))
    ...
    val reportable: Boolean = isWorthLookingAt &&
      ...
      &&
        ...
        || (cheatingSum + likelyCheatingSum).toDouble /
          assessmentsCount >= 0.10 - relationModifier))
  ...
  def reportText(maxGames: Int = 10): String = {
    val gameLinks: String = (playerAssessments.sortBy(-_.assessment.id)
      .take(maxGames).map { a =>
