...
private final class AggregationPipeline {
  ...
  private lazy val movetimeIdDispatcher =
    MovetimeRange.reversedNoInf.foldLeft[BSONValue](
      BSONInteger(MovetimeRange.MTRInf.id)) { case (acc, mtr) =>
      BSONDocument(
        "$cond" -> BSONArray(
          BSONDocument(
  ...
  private lazy val materialIdDispatcher = BSONDocument(
    "$cond" -> BSONArray(
      ...
      MaterialRange.reversedButEqualAndLast.foldLeft[BSONValue](
        BSONInteger(MaterialRange.Up4.id)) { case (acc, mat) =>
        BSONDocument(
          "$cond" -> BSONArray(
            BSONDocument(
  ...
  private def group(d: Dimension[_], f: GroupFunction) =
      ...
      /* sortDate :: */ sampleGames :: ((metric match {
        ...
        case M.Opportunism =>
          List(
            ...
            unwindMoves,
            matchMoves(
              BSONDocument(F.moves("o") -> BSONDocument("$exists" -> true))),
            sampleMoves,
            group(
              dimension,
              GroupFunction(
            ...
            Project(
              BSONDocument(
                "_id" -> true,
                "v" -> BSONDocument(
                  "$multiply" -> BSONArray(100, BSONDocument("$avg" -> "$v"))),
                "nb" -> true,
                "ids" -> true
              )).some
          )
        case M.Luck =>
          List(
            ...
            unwindMoves,
            matchMoves(
              BSONDocument(F.moves("l") -> BSONDocument("$exists" -> true))),
            sampleMoves,
            group(
              dimension,
              GroupFunction(
