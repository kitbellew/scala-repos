...
object Chart {
  ...
  def fromAnswer[X](getLightUser: String => Option[LightUser])(
    ...
    def series =
      clusters.foldLeft(Map.empty[String, Serie]) {
        case (acc, cluster) => cluster.insight match {
      ...
      }.map { case (_, serie) => serie.copy(data = serie.data.reverse) }.toList
