...
private final class FishnetRepo(analysisColl: Coll, clientColl: Coll) {
  ...
  def lichessClients =
    clientColl.find(BSONDocument(
      "enabled" -> true,
      "userId" -> BSONDocument("$regex" -> "^lichess-"))).cursor[Client]()
      .collect[List]()
  ...
  def updateAnalysis(ana: Work.Analysis) =
    analysisColl.update(selectWork(ana.id), ana).void
  def deleteAnalysis(ana: Work.Analysis) =
    analysisColl.remove(selectWork(ana.id)).void
  ...
  def getSimilarAnalysis(work: Work.Analysis): Fu[Option[Work.Analysis]] =
    analysisColl.find(BSONDocument("game.id" -> work.game.id))
      .one[Work.Analysis]
