...
final class Tv(actor: ActorRef) {
  ...
  def getGame(channel: Tv.Channel): Fu[Option[Game]] =
    (
      actor ? TvActor.GetGameId(channel) mapTo manifest[Option[String]]
    ) recover {
      case e: Exception =>
  ...
  def getGames(channel: Tv.Channel, max: Int): Fu[List[Game]] =
    (
      actor ? TvActor.GetGameIds(channel, max) mapTo manifest[List[String]]
    ) recover { case e: Exception => Nil } flatMap GameRepo.games
...
object Tv {
  ...
  private def variant(variant: chess.variant.Variant) =
