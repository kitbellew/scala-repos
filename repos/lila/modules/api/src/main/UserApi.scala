...
private[api] final class UserApi(
  ...
  def list(
    ...
    lila.team.MemberRepo userIdsByTeam teamId map (_ take makeNb(nb)) flatMap
      UserRepo.enabledByIds map { users =>
    ...
    UserRepo named username flatMap {
      ...
      case Some(u) =>
        GameRepo mostUrgentGame u zip
          ...
          }) zip relationApi.countFollowing(u.id) zip
          relationApi.countFollowers(u.id) zip ctx.isAuth.?? {
            prefApi followable u.id
          } zip ctx.userId.?? { relationApi.fetchRelation(_, u.id) } zip
