...
private[video] final class VideoApi(videoColl: Coll, viewColl: Coll) {
  ...
  private def videoViews(userOption: Option[User])(
    ...
    userOption match {
      case None => fuccess { videos map { VideoView(_, false) } }
      case Some(user) =>
  ...
  object video {
    ...
    def search(
      ...
      Paginator(
        adapter = new BSONAdapter[Video](
          ...
          selector = BSONDocument("$text" -> BSONDocument("$search" -> q)),
          projection = textScore,
          sort = textScore) mapFutureList videoViews(user),
        currentPage = page,
    ...
    def removeNotIn(ids: List[Video.ID]) =
      videoColl.remove(BSONDocument("_id" -> BSONDocument("$nin" -> ids))).void
    ...
    def similar(
      ...
      videoColl.find(BSONDocument(
        ...
        .sort(BSONDocument("metadata.likes" -> -1)).cursor[Video]()
  ...
  object view {
    ...
    def seenVideoIds(user: User, videos: Seq[Video]): Fu[Set[Video.ID]] =
      viewColl.distinct(
        ...
        BSONDocument("_id" -> BSONDocument("$in" -> videos.map { v =>
