  ...
  def updateAll: Funit =
    fetch flatMap { entries =>
      entries.map { entry =>
        ...
        ).recover { case e: Exception => logger.warn("update all youtube", e) }
      }.sequenceFu.void
    }
...
object Youtube {
  ...
  private[video] case class ContentDetails(duration: String) {
    def seconds: Int =
      iso8601Formatter.parsePeriod(duration).toStandardSeconds.getSeconds
  }
}
