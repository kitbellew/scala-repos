...
final class JsonView(
  ...
  def playerJson(
    ...
    getSocketStatus(pov.game.id) zip
      (pov.opponent.userId ?? UserRepo.byId) zip canTakeback(pov.game) zip
            ...
            .obj(
              ...
              "player" ->
                Json
                  .obj(
                    ...
                    "proposingTakeback" ->
                      player.isProposingTakeback.option(true),
              ...
              "opponent" ->
                Json
                  .obj(
                    ...
                    "offeringRematch" ->
                      opponent.isOfferingRematch.option(true),
                    "offeringDraw" -> opponent.isOfferingDraw.option(true),
                    "proposingTakeback" ->
                      opponent.isProposingTakeback.option(true),
                    "onGame" ->
              ...
              "pref" ->
                Json.obj(
                  ...
                  "replay" -> pref.replay,
                  "autoQueen" ->
                    (pov.game.variant == chess.variant.Antichess)
  ...
  def watcherJson(
    ...
    getSocketStatus(pov.game.id) zip getWatcherChat(pov.game, user) zip
    ...
    (pov.game.pgnMoves.nonEmpty ?? GameRepo.initialFen(pov.game)) map {
      initialFen =>
        ...
        Json.obj(
          ...
          "player" ->
            Json.obj("id" -> owner.option(pov.playerId), "color" -> color.name),
          "opponent" ->
            Json.obj("color" -> opponent.color.name, "ai" -> opponent.aiLevel),
  ...
  private def getWatcherChat(
    ...
    forUser ?? { user =>
      chatApi.userChat find s"${game.id}/w" map
        (_ forUser user.some) map
        (_.some)
    }

  private def getUsers(game: Game) =
    UserRepo.pair(game.whitePlayer.userId, game.blackPlayer.userId)
