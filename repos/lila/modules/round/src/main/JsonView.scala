...
final class JsonView(
      ...
      case (((socket, opponentUser), takebackable), chat) =>
        ...
        Json.obj(
          ...
          "pref" -> Json.obj(
            ...
            "coords" -> pref.coords,
            "replay" -> pref.replay,
            "autoQueen" -> (pov.game.variant == chess.variant.Antichess).fold(
            ...
            "confirmResign" -> (pref.confirmResign == Pref.ConfirmResign.YES).option(
              true)
          ),
          "chat" -> chat.map { c =>
            JsArray(c.lines map {
  ...
  private def animationDuration(pov: Pov, pref: Pref) =
    math.round {
      animationFactor(
        pref) * baseAnimationDuration.toMillis * pov.game.finished.fold(
        1,
        math.max(
          0,
