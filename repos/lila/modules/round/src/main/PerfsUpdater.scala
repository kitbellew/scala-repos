...
final class PerfsUpdater(historyApi: HistoryApi, rankingApi: RankingApi) {
  ...
  private def updateRatings(
    ...
    catch { case e: Exception => logger.error("update ratings", e) }
  }
  ...
  private def mkPerfs(ratings: Ratings, perfs: Perfs, game: Game): Perfs = {
    ...
    val perfs1 = perfs.copy(
      chess960 = addRatingIf(
        ...
        perfs.chess960,
        ratings.chess960),
      kingOfTheHill = addRatingIf(
        game.ratingVariant.kingOfTheHill,
        perfs.kingOfTheHill,
        ratings.kingOfTheHill),
      threeCheck = addRatingIf(
        game.ratingVariant.threeCheck,
        perfs.threeCheck,
        ratings.threeCheck),
      antichess = addRatingIf(
