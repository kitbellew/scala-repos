...
object BSONHandlers {
  ...
  private implicit val leaderboardRatio =
    new BSONHandler[BSONInteger, LeaderboardApi.Ratio] {
  ...
  implicit val tournamentHandler =
    new BSON[Tournament] {
  ...
  implicit val playerBSONHandler =
    new BSON[Player] {
      def reads(r: BSON.Reader) =
        Player(
          _id = r str "_id",
  ...
  implicit val leaderboardEntryHandler =
