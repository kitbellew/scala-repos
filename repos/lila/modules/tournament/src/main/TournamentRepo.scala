...
object TournamentRepo {
  ...
  def nonEmptyEnterable: Fu[List[Tournament]] =
    coll.find(enterableSelect ++ nonEmptySelect).cursor[Tournament]().collect[
      List]()
  ...
  def started: Fu[List[Tournament]] =
    coll.find(startedSelect).sort(BSONDocument("createdAt" -> -1)).toList[
      Tournament](None)
  ...
  def allCreated(aheadMinutes: Int): Fu[List[Tournament]] =
    coll.find(allCreatedSelect(aheadMinutes)).cursor[Tournament]().collect[
      List]()
  ...
  def scheduledDedup: Fu[List[Tournament]] =
    scheduledCreated map {
      ...
      _.flatMap { tour => tour.schedule map (tour -> _) }.foldLeft(
        List[Tournament]() -> none[Freq]) {
