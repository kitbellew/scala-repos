...
object TournamentRepo {
  ...
  def recentAndNext: Fu[List[Tournament]] =
    coll.find(sinceSelect(DateTime.now minusDays 1)).cursor[Tournament]()
      .collect[List]()

  def byIdAndPlayerId(id: String, userId: String): Fu[Option[Tournament]] =
    coll.find(selectId(id) ++ BSONDocument("players.id" -> userId))
      .one[Tournament]
  ...
  def nonEmptyEnterable: Fu[List[Tournament]] =
    coll.find(enterableSelect ++ nonEmptySelect).cursor[Tournament]()
      .collect[List]()

  def createdIncludingScheduled: Fu[List[Tournament]] =
    coll.find(createdSelect).toList[Tournament](None)

  def started: Fu[List[Tournament]] =
    coll.find(startedSelect).sort(BSONDocument("createdAt" -> -1))
  ...
  def finished(limit: Int): Fu[List[Tournament]] =
    coll.find(finishedSelect).sort(BSONDocument("startsAt" -> -1))
    ...
    coll.find(
      finishedSelect ++ BSONDocument(
        "$or" -> BSONArray(
          ...
          scheduledSelect))).sort(BSONDocument("startsAt" -> -1))
      .cursor[Tournament]().collect[List](limit)
  ...
  def setStatus(tourId: String, status: Status) =
    coll.update(
      selectId(tourId),
      BSONDocument("$set" -> BSONDocument("status" -> status.id))).void

  def setNbPlayers(tourId: String, nb: Int) =
    coll.update(
      selectId(tourId),
      BSONDocument("$set" -> BSONDocument("nbPlayers" -> nb))).void

  def setWinnerId(tourId: String, userId: String) =
    coll.update(
      selectId(tourId),
      BSONDocument("$set" -> BSONDocument("winner" -> userId))).void

  def setFeaturedGameId(tourId: String, gameId: String) =
    coll.update(
      selectId(tourId),
      BSONDocument("$set" -> BSONDocument("featured" -> gameId))).void

  def featuredGameId(tourId: String) =
    coll.find(selectId(tourId), BSONDocument("featured" -> true))
  ...
  def allCreated(aheadMinutes: Int): Fu[List[Tournament]] =
    coll.find(allCreatedSelect(aheadMinutes)).cursor[Tournament]()
      .collect[List]()
  ...
  def promotable: Fu[List[Tournament]] =
    stillWorthEntering zip publicCreatedSorted(24 * 60) map {
      case (started, created) =>
        (started ::: created).foldLeft(List.empty[Tournament]) {
  ...
  def update(tour: Tournament) =
    coll.update(BSONDocument("_id" -> tour.id), tour)
