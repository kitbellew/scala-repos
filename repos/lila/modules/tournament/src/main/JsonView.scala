...
final class JsonView(
            ...
            povs.map { pov =>
              Json
                .obj(
                  ...
                  "op" ->
                    gameUserJson(pov.opponent.userId, pov.opponent.rating),
                  "win" -> pov.win,
  ...
  private val firstPageCache = lila
    .memo
    .AsyncCache[String, JsObject](
      (id: String) =>
  ...
  private val cachableData = lila
    .memo
    .AsyncCache[String, CachableData](
      id =>
        for {
          ...
          .flatMap {
            _.map {
                case rp @ RankedPlayer(_, player) =>
                  ...
                  } yield playerJson(sheet.some, tour, rp) ++
                    Json.obj(
