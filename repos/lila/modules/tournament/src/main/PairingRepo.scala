...
object PairingRepo {
  ...
  def recentByTour(tourId: String, nb: Int): Fu[Pairings] =
    coll.find(selectTour(tourId)).sort(recentSort).cursor[Pairing]()
      .collect[List](nb)
  ...
  def lastOpponents(
    ...
    coll.find(
      ...
      BSONDocument("_id" -> false, "u" -> true)).sort(recentSort)
      .cursor[BSONDocument]().enumerate(nb) |>>>
      Iteratee.fold(scala.collection.immutable.Map.empty[String, String]) {
        (acc, doc) =>
  ...
  def recentIdsByTourAndUserId(
    ...
    coll.find(selectTourUser(tourId, userId), BSONDocument("_id" -> true))
  ...
  def countByTourIdAndUserIds(tourId: String): Fu[Map[String, Int]] = {
    ...
    coll.aggregate(
      ...
      List(
        ...
        Unwind("u"),
        GroupField("u")("nb" -> SumValue(1)))).map {
  ...
  def findPlaying(tourId: String): Fu[Pairings] =
    coll.find(selectTour(tourId) ++ selectPlaying).cursor[Pairing]()
      .collect[List]()
  ...
  def finishedByPlayerChronological(
    ...
    coll.find(selectTourUser(tourId, userId) ++ selectFinished).sort(chronoSort)
      .cursor[Pairing]().collect[List]()
  ...
  def playingUserIds(tour: Tournament): Fu[Set[String]] =
    ...
    ).map(_.documents.headOption.flatMap(_.getAs[Set[String]]("ids")).getOrElse(
      Set.empty[String]))
