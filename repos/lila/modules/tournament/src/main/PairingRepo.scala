...
object PairingRepo {
  ...
  def recentByTour(tourId: String, nb: Int): Fu[Pairings] =
    coll.find(selectTour(tourId)).sort(recentSort).cursor[Pairing]().collect[
      List](nb)
  ...
  def opponentsOf(tourId: String, userId: String): Fu[Set[String]] =
    coll.find(
      ...
      _.flatMap { doc => ~doc.getAs[List[String]]("u").filter(userId !=) }.toSet
    }
  ...
  def findPlaying(tourId: String): Fu[Pairings] =
    coll.find(selectTour(tourId) ++ selectPlaying).cursor[Pairing]().collect[
      List]()
  ...
  def playingGameIds(tourId: String): Fu[List[String]] =
    coll.aggregate(
      ...
      _.documents.headOption.flatMap(_.getAs[List[String]]("ids")).getOrElse(
        List.empty[String]))
}
