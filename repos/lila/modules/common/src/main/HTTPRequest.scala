...
object HTTPRequest {
  ...
  private def uaContains(req: RequestHeader, str: String) =
    userAgent(req).exists(_ contains str)
  def isTrident(req: RequestHeader) = uaContains(req, "Trident/")
  def isChrome(req: RequestHeader) = uaContains(req, "Chrome/")
  def isSafari(req: RequestHeader) =
  ...
  def isFacebookBot(req: RequestHeader) =
    userAgent(req) ?? (_ contains "facebookexternalhit")
