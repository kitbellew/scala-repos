...
object Tournament extends LilaController {
  ...
  private def tournamentNotFound(implicit ctx: Context) =
    NotFound(html.tournament.notFound())
      ...
      negotiate(
        html = Reasonable(page, 20) {
          ...
          if (HTTPRequest isXhr ctx.req) finishedPaginator map { pag =>
            ...
            env.api.fetchVisibleTournaments zip repo.scheduledDedup zip
              finishedPaginator zip UserRepo.allSortToints(10) map {
  ...
  def userGameNbMini(id: String, user: String, nb: Int) =
    Open { implicit ctx =>
      withUserGameNb(id, user, nb) { pov => Ok(html.tournament.miniGame(pov)) }
    }

  def userGameNbShow(id: String, user: String, nb: Int) =
    Open { implicit ctx =>
  ...
  def terminate(id: String) =
    Secure(_.TerminateTournament) { implicit ctx => me =>
