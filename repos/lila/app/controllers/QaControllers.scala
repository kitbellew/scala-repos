...
trait QaController extends LilaController {
  ...
  protected def renderQuestion(q: Question, answerForm: Option[Form[_]] = None)(
      implicit ctx: Context): Fu[Result] = (api.answer popular q.id) zip
    fetchPopular zip api.relation.questions(q, 10) zip
    (QaAuth.canAsk ?? { forms.anyCaptcha map (_.some) }) flatMap {
      case (((answers, popular), related), captcha) => fuccess {
          Ok(views.html.qa.questionShow(
            q,
            answers,
            popular,
            related,
