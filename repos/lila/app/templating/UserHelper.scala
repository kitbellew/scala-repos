...
trait UserHelper { self: I18nHelper with StringHelper with NumberHelper =>
  ...
  def usernameOrAnon(userId: Option[String]) =
    lightUser(userId).fold(User.anonymous)(_.titleName)
  ...
  def userGameFilterTitle(info: UserInfo, filter: GameFilter)(implicit
      ctx: UserContext) = splitNumber(userGameFilterTitleNoTag(info, filter))
  ...
  private val donorBadge =
    """<span data-icon="&#xe001;" class="donor is-gold" title="Lichess donor"></span>"""
}
