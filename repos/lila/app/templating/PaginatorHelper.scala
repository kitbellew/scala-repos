...
trait PaginatorHelper {
  ...
  implicit def toRichPager[A](pager: Paginator[A]) =
    new {
      ...
      def sliding(length: Int): List[Option[Int]] = {
        ...
        val post =
          toPage match {
            case x if x == pager.nbPages =>
              Nil
            case x if x == pager.nbPages - 1 =>
              List(pager.nbPages.some)
            case x =>
              List(none, pager.nbPages.some)
          }
      ...
      def pageNbResults = pager.currentPageResults.size
    }
}
