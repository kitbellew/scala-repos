...
trait LogicLaws[A] extends Laws {
  ...
  def heyting(implicit A: Heyting[A]) =
    new DefaultRuleSet(
      ...
      "x∧y → y = 1" -> forAll { (x: A, y: A) =>
        ((x & y) imp y) === A.one
      },
      "x → y → (x∧y) = 1" -> forAll { (x: A, y: A) =>
        (x imp (y imp (x & y))) === A.one
      },
      "x → x∨y" -> forAll { (x: A, y: A) =>
        (x imp (x | y)) === A.one
      },
      "y → x∨y" -> forAll { (x: A, y: A) =>
        (y imp (x | y)) === A.one
      },
      "(x → z) → ((y → z) → ((x | y) → z)) = 1" -> forAll {
        (x: A, y: A, z: A) =>
