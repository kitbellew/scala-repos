...
trait BaseLaws[A] extends Laws {
  ...
  def signed(implicit A: Signed[A]) =
    new SimpleRuleSet(
      name = "signed",
  ...
  def metricSpace[R](implicit
    ...
    new SimpleRuleSet(
      name = "metricSpace",
      "non-negative" →
        forAll((a1: A, a2: A) => MSA.distance(a1, a2).sign != Sign.Negative),
      ...
      "symmetry" →
        forAll((a1: A, a2: A) => MSA.distance(a1, a2) === MSA.distance(a2, a1)),
      "triangleInequality" →
        forAll((a1: A, a2: A, a3: A) =>
          (MSA.distance(a1, a2) + MSA.distance(a2, a3)) >=
            MSA.distance(a1, a3)))
