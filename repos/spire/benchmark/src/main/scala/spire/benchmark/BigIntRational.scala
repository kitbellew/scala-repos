...
object BigIntRational {
  ...
  def apply(n: BigInt, d: BigInt): BigIntRational = {
    val gcd = n.gcd(d)
    if (d < 0) { new BigIntRational(-n / gcd, -d / gcd) }
    else { new BigIntRational(n / gcd, d / gcd) }
  }
}
...
final class BigIntRational private (val n: BigInt, val d: BigInt) {
  ...
  def +(r: BigIntRational): BigIntRational = {
    ...
    if (dgcd == 1) { new BigIntRational(r.d * n + r.n * d, r.d * d) }
    else {
  ...
  def /(r: BigIntRational): BigIntRational = {
    ...
    val num = (n / a) * (r.d / b)
    val den = (d / b) * (r.n / a)
    if (den < BigInt(0)) { new BigIntRational(-num, -den) }
    else { new BigIntRational(num, den) }
  }

  def pow(exp: Int): BigIntRational =
    if (exp == 0) { BigIntRational.One }
    else if (exp < 0) { new BigIntRational(d pow exp.abs, n pow exp.abs) }
    else { new BigIntRational(n pow exp, d pow exp) }
