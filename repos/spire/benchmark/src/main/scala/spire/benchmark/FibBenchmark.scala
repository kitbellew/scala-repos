...
class FibBenchmarks extends MyBenchmark {
  ...
  def timeBigIntegerFibJava(reps: Int) =
    run(reps) {
  ...
  def timeBigIntegerFibGen(reps: Int) =
    run(reps) {
  ...
  def timeBigIntFibScala(reps: Int) =
    run(reps) {
  ...
  def timeNaturalFibGen(reps: Int) =
    run(reps) {
  ...
  def scalaGenFib[@sp(Int, Long) A](n: Int)(implicit r: Rig[A]): A = {
    @tailrec
    def loop(n: Int, a: A, b: A): A = if (n == 0) a else loop(n - 1, b, a + b)
    loop(n, r.zero, r.one)
  }
