...
object IntervalTrie {
  ...
  private final case class IntervalTrieImpl[T](belowAll: Boolean, tree: Tree)(
    ...
    def aboveAll: Boolean =
      if (tree eq null) belowAll else belowAll ^ tree.sign
    ...
    def below(value: T): Boolean =
      SampleBelow(belowAll, tree, toPrefix(ise.toLong(value)))

    def at(value: T): Boolean =
      SampleAt(belowAll, tree, toPrefix(ise.toLong(value)))

    def above(value: T): Boolean =
      SampleAbove(belowAll, tree, toPrefix(ise.toLong(value)))
