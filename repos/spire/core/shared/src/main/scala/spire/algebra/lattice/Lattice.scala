...
object Lattice {
  def min[@sp(Boolean, Byte, Short, Int, Long, Float, Double) A](implicit
      ev: Order[A]): Lattice[A] =
    new MinMaxLattice[A]

  def gcd[@sp(Byte, Short, Int, Long) A](implicit
      ev: EuclideanRing[A]): Lattice[A] =
    new GcdLcmLattice[A]
}

class MinMaxLattice[@sp(Boolean, Byte, Short, Int, Long, Float, Double) A: Order]
    extends Lattice[A] {
...
trait BoundedJoinSemilattice[@sp(Boolean, Byte, Short, Int, Long, Float, Double) A]
    extends Any
    with JoinSemilattice[A] {
  def zero: A
...
trait BoundedMeetSemilattice[@sp(Boolean, Byte, Short, Int, Long, Float, Double) A]
    extends Any
    with MeetSemilattice[A] {
  def one: A
