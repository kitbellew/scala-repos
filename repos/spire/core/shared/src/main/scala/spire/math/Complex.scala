...
object Complex extends ComplexInstances {
  ...
  def rootsOfUnity[@sp(Float, Double) T](n: Int)(implicit
      f: Field[T],
      t: Trig[T],
      r: IsReal[T]): Array[Complex[T]] = {
...
final case class Complex[@sp(Float, Double) T](real: T, imag: T)
  ...
  def asPolarTuple(implicit
      f: Field[T],
      n: NRoot[T],
      t: Trig[T],
      o: IsReal[T]): (T, T) = (abs, arg)

  def isZero(implicit o: IsReal[T]): Boolean =
    real.isSignZero && imag.isSignZero
  ...
  def %(rhs: T)(implicit f: Field[T], o: IsReal[T]): Complex[T] =
    this - (this /~ rhs) * rhs
  def /%(
      rhs: T)(implicit f: Field[T], o: IsReal[T]): (Complex[T], Complex[T]) = {
    val q = this /~ rhs
    (q, this - q * rhs)
  }
  ...
  def /%(b: Complex[T])(implicit
      f: Field[T],
      o: IsReal[T]): (Complex[T], Complex[T]) = {
    val q = this /~ b
    (q, this - q * b)
  }
  ...
  def pow(b: Complex[T])(implicit
      f: Field[T],
      n: NRoot[T],
      t: Trig[T],
      o: IsReal[T]): Complex[T] =
    if (b.isZero) { Complex.one[T] }
    else if (this.isZero) {
    ...
    } else { Complex.polar(abs fpow b.real, arg * b.real) }
  ...
  def ceil(implicit o: IsReal[T]): Complex[T] =
    new Complex(real.ceil, imag.ceil)
...
object FastComplex {
  ...
  final def apply(real: Double, imag: Double): Long =
  ...
  // addition
  final def add(a: Long, b: Long): Long =
    encode(real(a) + real(b), imag(a) + imag(b))

  // subtraction
  final def subtract(a: Long, b: Long): Long =
    encode(real(a) - real(b), imag(a) - imag(b))
