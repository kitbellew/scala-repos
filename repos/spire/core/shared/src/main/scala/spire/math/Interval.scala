...
sealed abstract class Interval[A](implicit order: Order[A]) {
  ...
  protected[this] def lowerPairBelow(
      ...
      lower2: A,
      flags2: Int): Boolean =
    lower1 < lower2 || lower1 === lower2 && (
      isClosedLower(flags1) || isOpenLower(flags2)
    )

  protected[this] def upperPairAbove(
      upper1: A,
      flags1: Int,
      upper2: A,
      flags2: Int): Boolean =
    upper1 > upper2 || upper1 === upper2 && (
      isClosedUpper(flags1) || isOpenUpper(flags2)
    )
  ...
  def isSupersetOf(rhs: Interval[A]): Boolean =
    (lhs, rhs) match {
      ...
      case (Point(lhsval), Point(rhsval)) =>
        lhsval === rhsval
      case (Point(_), _) =>
        false // rhs cannot be Empty or Point
      case (_, Point(rhsval)) =>
        lhs.contains(rhsval)

      // remaining cases are Above, Below and Bounded, we deal first with the obvious false

      case (Above(_, _), Below(_, _)) =>
        false
      case (Below(_, _), Above(_, _)) =>
        false
      case (Bounded(_, _, _), Below(_, _)) =>
        false
      case (Bounded(_, _, _), Above(_, _)) =>
        false
  ...
  def *(rhs: Interval[A])(implicit ev: Semiring[A]): Interval[A] = {
    ...
    def aboveAbove(lower1: A, lf1: Int, lower2: A, lf2: Int): Interval[A] = {
      ...
      if (lower1s < 0 || lower2s < 0)
        ...
        val strongZero = (lower1s == 0 && isClosedLower(lf1)) || (
          lower2s == 0 && isClosedLower(lf2)
        )
        val flags =
          if (strongZero)
            0
    ...
    def belowBelow(upper1: A, uf1: Int, upper2: A, uf2: Int): Interval[A] = {
      ...
      if (upper1s > 0 || upper2s > 0)
        ...
        val strongZero = (upper1s == 0 && isClosedUpper(uf1)) || (
          upper2s == 0 && isClosedUpper(uf2)
        )
        val flags =
          if (strongZero)
            0
    ...
    def aboveBounded(
      ...
      if (hasBelowZero2 && hasAboveZero2)
        ...
        if (hasBelowZero1) // the minimal point is lower1(-) * upper2(+)
          ...
          val strongZero = (lower1s == 0 && isClosedLower(lf1)) || (
            lower2s == 0 && isClosedLower(flags2)
          )
          val flags =
            if (strongZero)
              0
        ...
        if (hasBelowZero1) { // the maximal point is lower1(-) * lower2(-)
          ...
          val strongZero = (lower1s == 0 && isClosedLower(lf1)) || (
            upper2s == 0 && isClosedUpper(flags2)
          )
          val flags =
            if (strongZero)
              0
    ...
    (lhs, rhs) match {
      ...
      case (_, Empty()) =>
        rhs
      case (Point(lv), _) =>
        rhs * lv // use multiplication by scalar
      case (_, Point(rv)) =>
        lhs * rv
      ...
      case (Above(lower1, lf1), Above(lower2, lf2)) =>
        aboveAbove(lower1, lf1, lower2, lf2)
      case (Above(lower1, lf1), Below(upper2, uf2)) =>
        aboveBelow(lower1, lf1, upper2, uf2)
      case (Below(upper1, uf1), Above(lower2, lf2)) =>
        aboveBelow(lower2, lf2, upper1, uf1)
      case (Below(upper1, uf1), Below(upper2, uf2)) =>
        belowBelow(upper1, uf1, upper2, uf2)
      ...
      case Above(lower, lf) =>
        (lower.compare(z), isClosedLower(lf)) match {
          ...
          case (0, false) =>
            this
          case _ =>
            Bounded(z, lower.reciprocal, 1 | lowerFlagToUpper(lf))
        }
      ...
      case Below(upper, uf) =>
        (upper.compare(z), isClosedUpper(uf)) match {
          ...
          case (0, false) =>
            this
          case _ =>
            Bounded(upper.reciprocal, z, 2 | upperFlagToLower(uf))
        }
      ...
      case Bounded(lower, upper, flags) =>
        (
          ...
          case (_, 0, _, true) =>
            error // contains zero
          case (0, _, false, _) =>
            Above(upper.reciprocal, upperFlagToLower(flags))
          case (_, 0, _, false) =>
            Below(lower.reciprocal, lowerFlagToUpper(flags))
          case _ =>
            Bounded(upper.reciprocal, lower.reciprocal, swapFlags(flags))
        }
    }
  }
  ...
  def +(rhs: A)(implicit ev: AdditiveSemigroup[A]): Interval[A] =
    this match {
      case Point(v) =>
        Point(v + rhs)
      case Bounded(l, u, flags) =>
        Bounded(l + rhs, u + rhs, flags)
      case Above(l, lf) =>
        Above(l + rhs, lf)
      case Below(u, uf) =>
        Below(u + rhs, uf)
  ...
  def unary_-()(implicit ev: AdditiveGroup[A]): Interval[A] =
    this match {
      case Point(v) =>
        Point(-v)
      case Bounded(l, u, f) =>
        Bounded(-u, -l, swapFlags(f))
      case Above(l, lf) =>
        Below(-l, lowerFlagToUpper(lf))
      case Below(u, uf) =>
        Above(-u, upperFlagToLower(uf))
      case All() | Empty() =>
        this
    }
    ...
    if (rhs < ev.zero) {
      this match {
        case Point(v) =>
          Point(v * rhs)
        case Bounded(l, u, f) =>
          Bounded(u * rhs, l * rhs, swapFlags(f))
        case Above(l, lf) =>
          Below(l * rhs, lowerFlagToUpper(lf))
        case Below(u, uf) =>
          Above(u * rhs, upperFlagToLower(uf))
        case All() | Empty() =>
          this
      }
    } else if (rhs === ev.zero) {
    ...
    if (k == 1) {
      ...
      this match {
        ...
        case Below(u, uf) =>
          Below(u.nroot(k), uf)
        case Bounded(l, u, flags) =>
          Bounded(l.nroot(k), u.nroot(k), flags)
      }
    }
  }
  ...
  // step.
  private[this] def getStart(bound: Bound[A], step: A, unboundError: String)(
      implicit ev: AdditiveMonoid[A]): A =
    bound match {
      case EmptyBound() =>
        ev.zero
      case Open(x) =>
    ...
    if (step === ev.zero) {
      ...
      upperBound match {
        case EmptyBound() =>
          Iterator.empty
        case Unbound() =>
          iter(x, false, _ => true, test)
        case Closed(y) =>
          iter(x, y + step > y, _ <= y, test)
        case Open(y) =>
          iter(x, y + step > y, _ < y, test)
      }
    } else {
...
object Interval {
  ...
  private[spire] def fromOrderedBounds[A: Order](
      lower: Bound[A],
      upper: Bound[A]): Interval[A] =
    (lower, upper) match {
      case (EmptyBound(), EmptyBound()) =>
        empty
      case (Closed(x), Closed(y)) =>
        Bounded(x, y, closedLowerFlags | closedUpperFlags)
      case (Open(x), Open(y)) =>
        Bounded(x, y, openLowerFlags | openUpperFlags)
      case (Unbound(), Open(y)) =>
        below(y)
      case (Open(x), Unbound()) =>
        above(x)
      case (Unbound(), Closed(y)) =>
        atOrBelow(y)
      case (Closed(x), Unbound()) =>
        atOrAbove(x)
      case (Closed(x), Open(y)) =>
        Bounded(x, y, closedLowerFlags | openUpperFlags)
      case (Open(x), Closed(y)) =>
        Bounded(x, y, openLowerFlags | closedUpperFlags)
