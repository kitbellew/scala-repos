...
sealed abstract class Rational
  ...
  def /~(rhs: Rational): Rational =
    Rational(SafeLong((this / rhs).toBigInt), SafeLong.one)
  ...
  def limitTo(max: SafeLong): Rational =
    if (this.signum < 0) { -((-this).limitTo(max)) }
    else {
      require(max.signum > 0, "Limit must be a positive integer.")

      val half = max >> 1
      val floor = SafeLong(this.toBigInt)
      if (floor >= max) { Rational(max) }
      else if (floor >= (max >> 1)) { Rational(floor.toLong) }
      else if (this.compareToOne < 0) { limitDenominatorTo(max) }
      else { limitDenominatorTo(max * denominator / numerator) }
    }
  ...
  def limitDenominatorTo(limit: SafeLong): Rational = {
    ...
    def closest(l: Rational, u: Rational): Rational = {
      ...
      } else if (mediant == this) { mediant }
      else if (mediant < this) { closest(mediant, u) }
      else { closest(l, mediant) }
    }
...
object Rational extends RationalInstances {
  ...
  private[math] def toDouble(n: SafeLong, d: SafeLong): Double =
    n.signum match {
      ...
      case 1  =>
        ...
        val addBit =
          if (nShared < dShared || (nShared == dShared && d.toBigInteger
                .getLowestSetBit < dLowerLength)) { 1 }
          else { 0 }
  ...
  def apply(n: Long, d: Long): Rational = {
    def build0(n: Long, d: Long) =
      if (n == 0L) zero
        ...
        if (divisor == 1L) longRational(n, d)
        else longRational(n / divisor, d / divisor)
      }
    if (d == 0) throw new IllegalArgumentException("0 denominator")
    else if (d > 0) build0(n, d)
    else if (n == Long.MinValue || d == Long.MinValue)
  ...
  private[math] def buildWithDiv(
    ...
    val n = num / ngcd
    val d = rd / ngcd
    Checked.tryOrReturn { apply(n, lden * d) } {
  ...
  implicit def apply(x: Long): Rational =
    if (x == 0L) Rational.zero else longRational(x, 1L)
  ...
  implicit def apply(x: Double): Rational =
    if (x == 0d) { zero }
      ...
      if (exp > 10) { apply(SafeLong(value) << exp, SafeLong.one) }
      else if (exp >= 0) { apply(value << exp, 1L) }
      else if (exp >= -52 && (~((-1L) << (-exp)) & value) == 0L) {
        apply(value >> (-exp), 1L)
      } else { apply(SafeLong(value), SafeLong.one << (-exp)) }
    }

  implicit def apply(x: BigDecimal): Rational = {
    if (x.ulp >= 1) { apply(x.toBigInt, 1) }
    else {
  ...
  private final class LongRational(val n: Long, val d: Long)
    ...
    def reciprocal: Rational =
      if (n == 0L) throw new ArithmeticException("reciprocal called on 0/1")
        ...
        bigRational(-SafeLong(d), -SafeLong(n))
    ...
    override def isValidChar: Boolean =
      isWhole && new scala.runtime.RichLong(n).isValidChar

    override def isValidByte: Boolean =
      isWhole && new scala.runtime.RichLong(n).isValidByte

    override def isValidShort: Boolean =
      isWhole && new scala.runtime.RichLong(n).isValidShort

    override def isValidInt: Boolean =
      isWhole && new scala.runtime.RichLong(n).isValidInt
        ...
        case r: LongRational =>
          ...
          if (dgcd == 1L) {
            ...
            Checked.tryOrReturn {
              ...
              if (ngcd == 1L) Rational(num, lden * r.d)
              else Rational.buildWithDiv(num, ngcd, r.d, lden)
            } {
              val num: SafeLong = SafeLong(n) * rden + SafeLong(r.n) * lden

              val ngcd: Long = spire.math.gcd(dgcd, (num % dgcd).toLong)

              if (ngcd == 1L) Rational(num, SafeLong(lden) * r.d)
              else Rational(num / ngcd, SafeLong(lden) * (r.d / ngcd))
            }
          }
        case r: BigRational =>
          ...
          if (dgcd == 1L) {
            ...
            if (ngcd == 1L) Rational(num, SafeLong(lden) * r.d)
            else Rational(num / ngcd, r.d / ngcd * lden)
      ...
      if (n == 0L) Rational.zero
        ...
        r match {
          case r: LongRational =>
            ...
            val d1 = d / b
            val d2 = r.d / a
            Checked.tryOrReturn[Rational] { longRational(n1 * n2, d1 * d2) } {
    ...
    def /(r: Rational): Rational =
      r match {
        case r: LongRational =>
          ...
          Checked.tryOrReturn[Rational] { longRational(n1 * d2, d1 * n2) } {
    ...
    def pow(exp: Int): Rational =
      if (exp == 0) Rational.one
      else if (exp < 0) reciprocal.pow(-exp)
      else Rational(SafeLong(n).pow(exp), SafeLong(d).pow(exp))

    def compareToOne: Int = n compare d

    def compare(r: Rational): Int =
      r match {
        case r: LongRational => Checked.tryOrElse {
            LongAlgebra.compare(n * r.d, r.n * d)
          } {
            val dgcd = spire.math.gcd(d, r.d)
            if (dgcd == 1L) (SafeLong(n) * r.d) compare (SafeLong(r.n) * d)
            else
        ...
        case r: BigRational =>
          ...
          if (dgcd == 1L) (SafeLong(n) * r.d) compare (r.n * d)
    ...
    override def hashCode: Int =
      if (d == 1L) unifiedPrimitiveHashcode else 29 * (37 * n.## + d.##)

    override def toString: String = if (isWhole) n.toString else s"$n/$d"
  }
  private def longRational(n: Long, d: Long): LongRational =
    new LongRational(n, d)
  ...
  private final class BigRational(val n: SafeLong, val d: SafeLong)
    ...
    def reciprocal: Rational =
      if (signum < 0) Rational(-d, -n) else bigRational(d, n)
    ...
    def +(r: Rational): Rational =
      r match {
        ...
        case r: BigRational =>
          ...
          if (dgcd.isOne) { Rational(r.d * n + r.n * d, r.d * d) }
          else {
            val lden: SafeLong = d / dgcd
            val rden: SafeLong = r.d / dgcd
            val num: SafeLong = rden * n + r.n * lden
            val ngcd: SafeLong = num.gcd(dgcd)
            if (ngcd.isOne) Rational(num, lden * r.d)
            else Rational(num / ngcd, (r.d / ngcd) * lden)
          }
      }
    ...
    def gcd(r: Rational): Rational =
      r match {
        ...
        case r: BigRational =>
          val dgcd: SafeLong = d.gcd(r.d)
          if (dgcd.isOne) { Rational(n.abs gcd r.n.abs, d * r.d) }
          else {
            val lm = d / dgcd
            val rm = r.d / dgcd
    ...
    def pow(exp: Int): Rational =
      if (exp == 0) Rational.one
      else if (exp < 0) Rational(d pow -exp, n pow -exp)
      else Rational(n pow exp, d pow exp)
    ...
    def compare(r: Rational): Int =
      r match {
        case r: LongRational =>
          ...
          if (dgcd == 1L) (n * r.d) compare (SafeLong(r.n) * d)
          else (n * (r.d / dgcd)) compare (SafeLong(r.n) * (d / dgcd))
        case r: BigRational =>
          val dgcd = d.gcd(r.d)
          if (dgcd.isOne) (n * r.d) compare (r.n * d)
          else ((r.d / dgcd) * n) compare ((d / dgcd) * r.n)
      }
  ...
  private def bigRational(n: SafeLong, d: SafeLong): BigRational =
    new BigRational(n, if (d.isOne) SafeLong.one else d)
}
