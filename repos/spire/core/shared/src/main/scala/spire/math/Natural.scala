...
sealed abstract class Natural
  ...
  def getNumBits: Int = {
    ...
    @tailrec
    def recur(next: Natural, b: Int): Int =
      next match {
  ...
  def getDigitLength: Int = {
    @tailrec
    def recur(next: Natural, n: Int): Int =
      next match {
        case End(d)         => n + 1
  ...
  def toList: List[UInt] = {
    @tailrec
    def recur(next: Natural, sofar: List[UInt]): List[UInt] =
      next match {
        case End(d)         => d :: sofar
  ...
  def toArray: Array[Int] = {
    ...
    @tailrec
    def recur(next: Natural, i: Int): Unit =
      next match {
        case End(d) => arr(i) = d.signed
        case Digit(d, tail) =>
          arr(i) = d.signed
          recur(tail, i - 1)
      }
  ...
  def reversed: Natural = {
    @tailrec
    def recur(next: Natural, sofar: Natural): Natural =
      next match {
  ...
  def trim: Natural = {
    ...
    def recur(next: Natural): Natural = {
      next match {
        case Digit(n, tail) => if (n == UInt(0)) recur(tail) else next
        case End(n)         => next
      }
    }
  ...
  override def toString: String = {
    ...
    def recur(next: Natural, s: String): String = {
      next match {
        case End(d) => d.toLong.toString + s
        case Digit(d, tail) =>
  ...
  def isZero: Boolean = {
    @tailrec
    def recur(next: Natural): Boolean =
      next match {
        case End(n)         => n == UInt(0)
        case Digit(n, tail) => if (n == UInt(0)) recur(tail) else false
      }
    recur(this)
  }
  ...
  def isOne: Boolean =
    this match {
      case End(n)         => n == UInt(1)
      case Digit(n, tail) => n == UInt(1) && tail.isZero
    }
  ...
  def powerOfTwo: Int = {
    ...
    @tailrec
    def recur(next: Natural, shift: Int, bit: Int): Int =
      next match {
        case End(n) =>
          val t = test(n)
  ...
  def compare(rhs: UInt): Int =
    this match {
      case End(d) => if (d < rhs) -1 else if (d > rhs) 1 else 0
      case Digit(d, tail) =>
  ...
  def compare(rhs: Natural): Int = {
    ...
    @tailrec
    def recur(lhs: Natural, rhs: Natural, d: Int): Int =
      lhs match {
  ...
  def +(rhs: Natural): Natural = {
    def recur(left: Natural, right: Natural, carry: Long): Natural =
      left match {
        case End(ld) => right match {
            case End(rd) => Natural(ld.toLong + rd.toLong + carry)
  ...
  def <<(n: Int): Natural = {
    val m: Int = n & 0x1f
    def recur(next: Natural, carry: Long): Natural =
      next match {
        case End(d) => Natural((d.toLong << m) | carry)
        case Digit(d, tail) =>
  ...
  def &(rhs: Natural): Natural = {
    def and(lhs: Natural, rhs: Natural): Natural =
      lhs match {
        case End(ld) => rhs match {
            case End(rd)          => End(ld & rd)
  ...
  def ^(rhs: Natural): Natural = {
    def xor(lhs: Natural, rhs: Natural): Natural =
      lhs match {
        case End(ld) => rhs match {
            case End(rd)          => End(ld ^ rd)
            case Digit(rd, rtail) => Digit(ld ^ rd, rtail)
...
object Natural extends NaturalInstances {
  ...
  def apply(s: String): Natural = {
    def parse(sofar: Natural, s: String, m: Natural): Natural =
      if (s.length <= 18) { Natural(s.toLong) * m + sofar }
      else {
    ...
    def /%(n: UInt): (Natural, Natural) = {
      ...
      def recur(
        ...
        next match {
          ...
          case Natural.Digit(d, tail) =>
            recur(tail, UInt(r), Digit(UInt(q), sofar))
        }
      }

      if (n == UInt(0)) { throw new IllegalArgumentException("/ by zero") }
      else if (n == UInt(1)) { (this, Natural(UInt(0))) }
      else {
        reversed match {
          case Digit(d, tail) =>
            val q = d / n
            val r = d % n
            recur(tail, r, End(q))
          case _ => throw new IllegalArgumentException("bug in reversed")
        }
      }
    }
  }
