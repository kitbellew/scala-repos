...
object Algebraic extends AlgebraicInstances {
  ...
  final class BitBound(val bitBound: Long) extends AnyVal {
    ...
    def +(that: BitBound): BitBound = new BitBound(
...
private[math] trait AlgebraicIsReal extends IsAlgebraic[Algebraic] {
  ...
  def ceil(a: Algebraic): Algebraic = Algebraic(
    a.toBigDecimal(0, RoundingMode.CEILING))
  def floor(a: Algebraic): Algebraic = Algebraic(
    a.toBigDecimal(0, RoundingMode.FLOOR))
  def round(a: Algebraic): Algebraic = Algebraic(
    a.toBigDecimal(0, RoundingMode.HALF_EVEN))
