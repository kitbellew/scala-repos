...
final class Algebraic private (val expr: Algebraic.Expr)
  ...
  def toBigDecimal(mc: MathContext): BigDecimal = {
    ...
    def rec(e: Expr, digits: Int): JBigDecimal =
      e match {
        case ConstantLong(n) =>
...
object Algebraic extends AlgebraicInstances {
  ...
  sealed abstract class Expr extends Serializable {
    ...
    private def radicalNodes(): Set[KRoot] = {
      val childRadicals =
        children.foldLeft(Set.empty[KRoot]) { (acc, child) =>
      ...
      if (cachedDegreeBound == 0L) {
        cachedDegreeBound =
          radicals.foldLeft(1L) { (acc, kroot) =>
            checked {
              acc * kroot.k
            }
          }
  ...
  case object BFMSS extends ZeroBoundFunction {
    ...
    private def mul(lhs: Bound, rhs: Bound): Bound =
      checked {
        Bound(lhs.l + rhs.l, lhs.u + rhs.u)
      }

    private def div(lhs: Bound, rhs: Bound): Bound =
      checked {
        Bound(lhs.l + rhs.u, lhs.u + rhs.l)
      }

    private def nroot(sub: Bound, k: Int): Bound =
      checked {
        if (sub.u < sub.l) {
          Bound((sub.l + (k - 1) * sub.u) / k, sub.u)
        } else {
          Bound(sub.l, (sub.u * (k - 1) * sub.l) / k)
        }
      }
    ...
    private def pow(sub: Bound, k: Int): Bound = {
      ...
      if (k > 1) {
        Bound(sum(sub.l, k - 1, sub.l), sum(sub.u, k - 1, sub.u))
      } else if (k == 1) {
        sub
      } else if (k == 0) {
