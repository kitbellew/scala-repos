...
object Numeric {
  ...
  implicit final val BigDecimalIsNumeric: Numeric[BigDecimal] =
    new BigDecimalIsNumeric
  implicit final val AlgebraicIsNumeric: Numeric[Algebraic] =
    new AlgebraicIsNumeric
  implicit final val RealIsNumeric: Numeric[Real] = new RealIsNumeric
  implicit final val RationalIsNumeric: Numeric[Rational] =
    new RationalIsNumeric

  implicit def complexIsNumeric[A: Fractional: Trig: IsReal]: ComplexIsNumeric[A] =
...
@SerialVersionUID(0L)
private[math] class RealIsNumeric
    extends Numeric[Real]
    with RealIsFractional
    with Serializable {
...
class ComplexIsNumeric[A](implicit
  ...
  override def fromDouble(n: Double): Complex[A] =
    Complex[A](algebra.fromDouble(n))

  override def eqv(x: Complex[A], y: Complex[A]): Boolean = x == y
  override def nroot(a: Complex[A], n: Int): Complex[A] =
