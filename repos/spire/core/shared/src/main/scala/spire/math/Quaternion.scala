...
object Quaternion extends QuaternionInstances {
  ...
  def apply[@sp(Float, Double) A](a: A)(implicit
      f: Semiring[A]): Quaternion[A] = Quaternion(a, f.zero, f.zero, f.zero)
...
private[math] trait QuaternionAlgebra[A]
  ...
  def fpow(a: Quaternion[A], b: Quaternion[A]): Quaternion[A] =
    a.fpow(b.r) //FIXME
...
trait QuaternionInstances {
  implicit def QuaternionAlgebra[A](implicit
      fr: Fractional[A],
      tr: Trig[A],
...
final case class Quaternion[@sp(Float, Double) A](r: A, i: A, j: A, k: A)
  ...
  def isReal(implicit o: IsReal[A]): Boolean =
    i.isSignZero && j.isSignZero && k.isSignZero
  ...
  def quaternionSignum(implicit
      f: Field[A],
      o: IsReal[A],
      n: NRoot[A]): Quaternion[A] = if (isZero) this else this / abs
  ...
  def /%(rhs: Complex[A])(implicit
      f: Field[A],
      o: IsReal[A]): (Quaternion[A], Quaternion[A]) = {
    val q = lhs /~ rhs
    (q, lhs - q)
  }
  def /%(rhs: Quaternion[A])(implicit
      f: Field[A],
      o: IsReal[A]): (Quaternion[A], Quaternion[A]) = {
    val q = lhs /~ rhs
    (q, lhs - q)
