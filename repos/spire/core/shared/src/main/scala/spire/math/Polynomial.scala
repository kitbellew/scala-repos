    ...
    def loop(p: Polynomial[C], xs: List[C], pts: List[(C, C)]): Polynomial[C] =
      pts match {
        ...
        case (x, y) :: tail =>
          ...
          val prod =
            xs.foldLeft(Polynomial.one[C]) { (prod, xn) =>
...
trait Polynomial[@sp(Double) C] {
  ...
  def data(implicit ring: Semiring[C], eq: Eq[C]): Map[Int, C] = {
    val bldr =
      new scala.collection.mutable.MapBuilder[Int, C, Map[Int, C]](
        Map.empty[Int, C])
    foreachNonZero { (e, c) =>
      bldr += ((e, c))
    }
  ...
  def compose(
      y: Polynomial[C])(implicit ring: Rig[C], eq: Eq[C]): Polynomial[C] = {
...
trait PolynomialEuclideanRing[@sp(Double) C]
  ...
  override def quotmod(
      x: Polynomial[C],
      y: Polynomial[C]): (Polynomial[C], Polynomial[C]) = x /% y
