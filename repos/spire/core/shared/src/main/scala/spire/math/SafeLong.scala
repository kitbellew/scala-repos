...
sealed abstract class SafeLong
  ...
  final def min(that: SafeLong): SafeLong = if (this < that) this else that

  final def max(that: SafeLong): SafeLong = if (this > that) this else that
...
object SafeLong extends SafeLongInstances {
  ...
  implicit def apply(x: BigInt): SafeLong =
    if (x.isValidLong) SafeLongLong(x.toLong)
