...
class PartialSyntaxTest
  ...
  implicit val SeqIntEq: Eq[Seq[Int]] =
    spire.optional.genericEq.generic[Seq[Int]]
  ...
  def testPartialActionSyntax(seq: Seq[Int], perm: Map[Int, Int]) = {
    ...
    ((perm ?|+|> seq) === PartialAction[Seq[Int], Map[Int, Int]].partialActl(
      perm,
      seq)) &&
    ((seq <|+|? perm) === PartialAction[Seq[Int], Map[Int, Int]].partialActr(
      seq,
      perm)) &&
    ((perm ??|+|> seq) === PartialAction[Seq[Int], Map[Int, Int]].actlIsDefined(
      perm,
      seq)) &&
    ((seq <|+|?? perm) === PartialAction[Seq[Int], Map[Int, Int]].actrIsDefined(
      seq,
      perm))
  }
}
