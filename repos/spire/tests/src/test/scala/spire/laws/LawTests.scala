...
class LawTests extends FunSuite with Discipline {
  ...
  checkAll(
    "List[Rational]",
    VectorSpaceLaws[List[Rational], Rational].normedVectorSpace(
      max,
      implicitly,
      implicitly))
  ...
  checkAll(
    "Set[Int]",
    GroupLaws[Set[Int]](spire.optional.genericEq.generic, implicitly).monoid)
  ...
  checkAll(
    "Map[Int, Int]",
    PartialActionLaws.apply[Map[Int, Int], Seq[Int]](
      implicitly,
      Arbitrary(arbitrary[Perm].map(_.map)),
      implicitly,
      implicitly).groupPartialAction
  )
}
