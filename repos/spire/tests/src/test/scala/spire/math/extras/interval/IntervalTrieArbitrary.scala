...
object IntervalTrieArbitrary {
  ...
  def makeProfileXor(
      initial: Boolean,
      support: Array[Long],
      kind: Array[Int]): IntervalTrie[Long] = {
  ...
  private def randomProfileXor(
    ...
    for {
      ...
      edges <-
        Gen.resize(count, Gen.containerOf[Array, Long](Gen.choose(min, max)))
  ...
  implicit val arbIntervalTrie = Arbitrary[IntervalTrie[Long]](randomProfileGen(
    3))
}
