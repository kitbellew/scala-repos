...
class NaturalTest
  ...
  property("x - y") {
    forAll { (x: N, y: N) =>
      ...
      if (x.num >= y.num) { z shouldBe Try(Natural(x.num - y.num)) }
      else { z.isFailure shouldBe true }
    }
  }
  ...
  property("x /% y") {
    forAll { (x: N, y: Positive[BigInt]) =>
      (
        Natural(x.num) /% Natural(y.num)
      ) shouldBe ((Natural(x.num / y.num), Natural(x.num % y.num)))
    }
  }
