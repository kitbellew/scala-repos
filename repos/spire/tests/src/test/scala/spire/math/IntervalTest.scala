...
class IntervalTest extends FunSuite {
  ...
  test("c.contains(0.0) is false") {
    assert(c.contains(0.0) === false)
  }
  test("c.crosses(0.0) is false") {
    assert(c.crosses(0.0) === false)
  }
...
class RingIntervalTest extends FunSuite {
  ...
  val a = cc(0.0, 4.0)
  test("a + a") {
    assert(a + a === cc(0.0, 8.0))
  }
  test("a - a") {
    assert(a - a === cc(-4.0, 4.0))
  }
  test("a * a") {
    assert(a * a === cc(0.0, 16.0))
  }

  val b = cc(-8.0, 2.0)
  test("b + b") {
    assert(b + b === cc(-16.0, 4.0))
  }
  test("b - b") {
    assert(b - b === cc(-10.0, 10.0))
  }
  test("b * b") {
    assert(b * b === cc(-16.0, 64.0))
  }
...
class IntervalGeometricPartialOrderTest extends FunSuite {
  ...
  test("[2, 3) < [3, 4]") {
    assert(openUpper(2, 3) < closed(3, 4))
  }
  test("[2, 3] < (3, 4]") {
    assert(closed(2, 3) < openLower(3, 4))
  }
  test("[2, 3] cannot be compared to [3, 4]") {
    assert(closed(2, 3).partialCompare(closed(3, 4)).isNaN)
  }
  test("[3, 4] > [2, 3)") {
    assert(closed(3, 4) > openUpper(2, 3))
  }
  test("[2, 3) <= [3, 4]") {
    assert(openUpper(2, 3) <= closed(3, 4))
  }
  ...
  test("empty cannot be compared to [2, 3]") {
    assert(open(2, 2).partialCompare(closed(2, 3)).isNaN)
  }
  test("[2, 3] cannot be compared to empty") {
    assert(closed(2, 3).partialCompare(open(2, 2)).isNaN)
  }
...
class ContinuousIntervalTest extends FunSuite {
  ...
  test("[-a,b] / [c,d]") {
    assert(cc(-a, b) / cc(c, d) === cc(-a / c, b / c))
  }
  test("[-a,b] / [-d,-c]") {
  ...
  test("[a,b] / [c,d]") {
    assert(cc(a, b) / cc(c, d) === cc(a / d, b / c))
  }

  // numerator interval is negative
  test("[-b,-a] / [-d,-c]") {
    assert(cc(-b, -a) / cc(-d, -c) === cc(-a / -d, -b / -c))
  }
  test("[-b,-a] / [c,d]") {
    assert(cc(-b, -a) / cc(c, d) === cc(-b / c, -a / d))
  }
}
...
class IntervalCheck
            ...
            () =>
              rng.nextInt(5) match {
                case 0 =>
                  x
                case _ =>
                  x + (Rational(rng.nextGaussian).abs * Long.MaxValue)
              }
          case (_, ValueBound(y)) =>
            () =>
              rng.nextInt(5) match {
                case 4 =>
                  y
                case _ =>
                  y - (Rational(rng.nextGaussian).abs * Long.MaxValue)
              }
          case (_, _) =>
  ...
  def testUnop(f: Interval[Rational] => Interval[Rational])(
    ...
    forAll { (a: Interval[Rational]) =>
      ...
      sample(a, tries).foreach { x =>
        ...
        if (!ok)
          println("%s failed on %s" format (a, x.toString))
        ok shouldBe true
      }
    }
  }
  ...
  property("sampled unop abs") {
    testUnop(_.abs)(_.abs)
  }
  property("sampled unop -") {
    testUnop(-_)(-_)
  }
  property("sampled unop pow(2)") {
    testUnop(_.pow(2))(_.pow(2))
  }
  property("sampled unop pow(3)") {
    testUnop(_.pow(3))(_.pow(3))
  }

  property("sampled binop +") {
    testBinop(_ + _)(_ + _)
  }
  property("sampled binop -") {
    testBinop(_ - _)(_ - _)
  ...
  property("sampled binop vmin") {
    testBinop(_ vmin _)(_ min _)
  }
  property("sampled binop vmax") {
    testBinop(_ vmax _)(_ max _)
  }
