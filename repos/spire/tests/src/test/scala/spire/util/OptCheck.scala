...
class OptCheck extends FunSuite {
  ...
  test("Opt.empty") {
    ...
    assertResult("Opt.empty") {
      Opt.empty[Int].toString
    }
    assertResult("Opt.empty") {
      Opt.empty[String].toString
    }

    assertResult("Opt.empty") {
      customToString(Opt.empty[Int])
    }
    assertResult("Opt.empty") {
      customToString(Opt.empty[String])
    }
    assertResult("Opt.empty") {
      intToString(Opt.empty[Int])
    }
    assertResult("Opt.empty") {
      strToString(Opt.empty[String])
    }
  ...
  test("Opt(value)") {
    ...
    assertResult("Opt(abc)") {
      Opt("abc").toString
    }

    assertResult("Opt(1)") {
      customToString(Opt(1))
    }
    assertResult("Opt(abc)") {
      customToString(Opt("abc"))
    }
    assertResult("Opt(1)") {
      intToString(Opt(1))
    }
