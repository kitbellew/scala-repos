...
class DebugManager(broadcaster: ActorRef, config: EnsimeConfig)
  ...
  private val pendingBreaksBySourceName =
    new mutable.HashMap[String, mutable.HashSet[Breakpoint]].withDefault { _ =>
      new mutable.HashSet
    }
  ...
  def clearBreakpoint(file: File, line: Int): Unit = {
    ...
    val toRemove = activeBreakpoints.filter {
      _ == clearBp
    }
    for (vm <- maybeVM) {
