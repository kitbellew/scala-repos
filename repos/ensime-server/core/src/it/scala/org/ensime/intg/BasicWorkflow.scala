    ...
    withEnsimeConfig { implicit config =>
      withTestKit { implicit testkit =>
        withProject { (project, asyncHelper) =>
          ...
          designations.syms should
            contain(SymbolDesignation(12, 19, PackageSymbol))
          // expected Symbols
          ...
          project !
            UsesOfSymbolAtPointReq(Left(fooFile), 119) // point on testMethod
          expectMsgPF() {
