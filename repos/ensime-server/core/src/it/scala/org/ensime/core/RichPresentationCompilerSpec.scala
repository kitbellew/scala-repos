...
class RichPresentationCompilerSpec
    ...
    "round-trip between typeFullName and askTypeInfoByName" in {
      withPresCompiler { (config, cc) =>
        ...
        def roundtrip(label: String, expectedFullName: String) = {
          ...
          val tpe =
            cc.askTypeInfoAt(new OffsetPosition(file, index + comment.length))
              .get
  ...
  it should "get symbol info by name" in
    withPresCompiler { (config, cc) =>
      def verify(
          fqn: String,
  ...
  it should "handle askSymbolByName" in
    withPresCompiler { (config, cc) =>
      val file = srcFile(
        config,
        "abc.scala",
  ...
  it should "get completions on an object name" in
    withPosInCompiledSource(
      "package com.example",
  ...
  it should "get members for infix method call" in
    withPosInCompiledSource(
      "package com.example",
      "object Abc { def aMethod(a: Int) = a }",
  ...
  it should "complete new construction" in
    withPosInCompiledSource(
      "package com.example",
  ...
  it should "complete symbol in logical op" in
    withPosInCompiledSource(
      "package com.example",
  ...
  it should "complete infix method of Set." in
    withPosInCompiledSource(
      "package com.example",
  ...
  it should "get type info for imports" in
    withPresCompiler { (config, cc) =>
      val expected = Map(
        "1.0" -> Some("java$"),
        "1.1" -> Some("java$"),
  ...
  it should "get symbol positions for compiled files" in
    withPresCompiler { (config, cc) =>
