...
class RefactoringHandlerSpec
  ...
  it should "format files from FileSourceFileInfo with handleFormatFile and handle encoding" in {
    withAnalyzer { (dir, analyzerRef) =>
      ...
      val formatted = analyzer.handleFormatFile(SourceFileInfo(
        new File(file.path)))
      val expectedContents = contents("package blah", "class Something {}", "")
  ...
  it should "organize imports when 3 imports exist" in withAnalyzer {
    (dir, analyzerRef) =>
      ...
      val procId = 1
      analyzer.handleRefactorPrepareRequest(new PrepareRefactorReq(
        procId,
        'Ignored,
  ...
  it should "organize imports" in {
    withAnalyzer { (dir, analyzerRef) =>
      ...
      val procId = 1
      val result = analyzer.handleRefactorRequest(new RefactorReq(
        procId,
        OrganiseImportsRefactorDesc(new File(file.path)),
        false))
