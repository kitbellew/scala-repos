...
class SemanticHighlightingSpec
  ...
  "SemanticHighlighting" should "highlight classes" in
  ...
  it should "highlight constructors" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight function calls" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight deprecated symbols" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "support custom deprecated symbol names" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
      ...
      sds should ===(List((DeprecatedSymbol, "BadTrait")))
    }

  it should "highlight imported names" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight objects" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight operators" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight packages" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(config, cc, """
             package com.example
             package other
          """, List(PackageSymbol))
      sds should
  ...
  it should "highlight typeParams" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight vals" in
    withPresCompiler { (config, cc) =>
      ...
      sds should ===(List((ValSymbol, "u"), (ValSymbol, "v"), (ValSymbol, "u")))
    }

  it should "highlight valFields" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight vars" in
    withPresCompiler { (config, cc) =>
      ...
      sds should ===(List((VarSymbol, "u"), (VarSymbol, "v"), (VarSymbol, "u")))
    }

  it should "highlight varFields" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight lazy val fields correctly as vals" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
      ...
      sds should ===(List((ValFieldSymbol, "u"), (ValFieldSymbol, "v")))
    }

  it should "highlight setter operators" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
      ...
      sds should ===(List((OperatorFieldSymbol, "value")))
    }

  it should "not be confused by whitespace" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(config, cc, """
            package com.  example
          """, List(PackageSymbol))
      // only part of "example" is highlighted
      sds should ===(List((PackageSymbol, "com"), (PackageSymbol, "example")))
    }

  it should "highlight negation operators" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
  ...
  it should "highlight implicit conversions" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
      ...
      sds should ===(List((ImplicitConversionSymbol, "\"sample\"")))
    }

  it should "highlight implicit parameters" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
      ...
      sds should ===(List((ImplicitParamsSymbol, "zz(1)")))
    }

  it should "highlight method calls after operators" in
    withPresCompiler { (config, cc) =>
      val sds = getSymbolDesignations(
        config,
        cc,
