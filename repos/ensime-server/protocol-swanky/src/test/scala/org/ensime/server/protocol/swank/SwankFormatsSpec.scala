...
class SwankFormatsSpec extends EnsimeSpec with EnsimeTestData {
  ...
  "SWANK Formats" should "unmarshal startup messages" in {
    unmarshal("(swank:connection-info)", ConnectionInfoReq: RpcRequest)
  }
  ...
  it should "unmarshal RpcAnalyserRequests" in {
    ...
    unmarshal(
      ...
      TypecheckFilesReq(List(
        Right(SourceFileInfo(file1)),
        Right(SourceFileInfo(file2, Some("xxx"), None)))): RpcRequest
    )

    unmarshal("""(swank:unload-all)""", UnloadAllReq: RpcRequest)

    unmarshal("""(swank:typecheck-all)""", TypecheckAllReq: RpcRequest)
    ...
    unmarshal("""(swank:cancel-refactor 1)""", CancelRefactorReq(1): RpcRequest)
  ...
  it should "unmarshal RpcDebugRequests" in {
    unmarshal("""(swank:debug-active-vm)""", DebugActiveVmReq: RpcRequest)
    ...
    unmarshal("""(swank:debug-stop)""", DebugStopReq: RpcRequest)
    ...
    unmarshal(s"""(swank:debug-run)""", DebugRunReq: RpcRequest)

    unmarshal(
      s"""(swank:debug-continue "13")""",
      DebugContinueReq(dtid): RpcRequest)

    unmarshal(s"""(swank:debug-step "13")""", DebugStepReq(dtid): RpcRequest)

    unmarshal(s"""(swank:debug-next "13")""", DebugNextReq(dtid): RpcRequest)
  ...
  it should "marshal EnsimeGeneralEvent as EnsimeEvent" in {
    ...
    marshal(AnalyzerReadyEvent: EnsimeEvent, "(:compiler-ready)")

    marshal(
      FullTypeCheckCompleteEvent: EnsimeEvent,
      "(:full-typecheck-finished)")

    marshal(IndexerReadyEvent: EnsimeEvent, "(:indexer-ready)")
    ...
    marshal(ClearAllScalaNotesEvent: EnsimeEvent, "(:clear-all-scala-notes)")
  }
  ...
  it should "marshal DebugEvent as EnsimeEvent" in {
    ...
    marshal(DebugVMStartEvent: EnsimeEvent, """(:debug-event (:type start))""")
    marshal(
  ...
  it should "marshal DebugValue" in {
    ...
    marshal(sourcePos3: SourcePosition, "(:type empty)")
    marshal(
      sourcePos4: SourcePosition,
      s"""(:type offset :file "$file1" :offset 456)""")

    marshal(breakPoint1: Breakpoint, s"""(:file "$file1" :line 57)""")
  ...
  it should "marshal various informational types" in {
    ...
    marshal(entityInfo: EntityInfo, entityInfoStr)

    marshal(typeInfo: EntityInfo, typeInfoStr)
  ...
  it should "marshal ranges and semantic highlighting" in {
    ...
    marshal(
      SymbolDesignations(
        ...
        List(
          ...
          SymbolDesignation(11, 22, ClassSymbol))): SymbolDesignations,
    ...
    marshal(
      ImplicitInfos(List(ImplicitParamInfo(
        5,
        6,
        symbolInfo,
  ...
  it should "marshal legacy raw response types" in {
    marshal(FalseResponse, "nil")

    marshal(TrueResponse, "t")

    marshal(StringResponse("wibble"), """"wibble"""")

    marshal(VoidResponse, """nil""")
