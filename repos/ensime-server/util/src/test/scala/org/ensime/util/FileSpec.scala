...
class FileSpec extends FlatSpec with Matchers {
  ...
  it should "provide an output stream" in
    withTempFile { file =>
  ...
  it should "create Files with parents if necessary" in
    withTempDir { dir =>
  ...
  it should "read lines from a File with UNIX line endings" in
    withTempFile { file =>
      val out = file.outputStream()
      out.write(unix)
      out.close()
  ...
  it should "read lines from a File with Windows line endings" in
    withTempFile { file =>
      val out = file.outputStream()
      out.write(windows)
      out.close
  ...
  it should "read a UNIX File as a String" in
    withTempFile { file =>
      val out = file.outputStream()
      out.write(unix)
      out.close()
  ...
  it should "read a File as a String preserving CR" in
    withTempFile { file =>
      val out = file.outputStream()
      out.write(windows)
      out.close()
  ...
  it should "calculate children" in
    withTempDir { dir =>
