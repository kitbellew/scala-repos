...
object Serbench extends Benchmark {
  ...
  class JValueBench {
    benchmark("lift-json (ser compact(render(jvalue))") {
      compact(render(bigJValue))
    }
    benchmark("lift-json (ser compactRender(jvalue)") {
      compactRender(bigJValue)
    }
  }
  ...
  def deserialize(array: Array[Byte]) =
    new ObjectInputStream(
      new ByteArrayInputStream(array)).readObject.asInstanceOf[Project]
