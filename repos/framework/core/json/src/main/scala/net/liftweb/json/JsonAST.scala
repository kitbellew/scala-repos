  ...
  sealed abstract class JValue extends Diff.Diffable {
    ...
    private def findDirectByName(xs: List[JValue], name: String): List[JValue] =
      xs.flatMap {
        case JObject(l) =>
          l.collect {
            case JField(n, value) if n == name =>
              value
          }
        case JArray(l) =>
          findDirectByName(l, name)
        case _ =>
          Nil
      }
    ...
    def foldField[A](z: A)(f: (A, JField) => A): A = {
      def rec(acc: A, v: JValue) = {
        v match {
          ...
          case JArray(l) =>
            l.foldLeft(acc)((a, e) => e.foldField(a)(f))
          case _ =>
            acc
        }
      }
    ...
    def findField(p: JField => Boolean): Option[JField] = {
      def find(json: JValue): Option[JField] =
        json match {
      ...
      def append(value1: JValue, value2: JValue): JValue =
        (value1, value2) match {
          ...
          case (JArray(xs), JArray(ys)) =>
            JArray(xs ::: ys)
          case (JArray(xs), v: JValue) =>
            JArray(xs ::: List(v))
          case (v: JValue, JArray(xs)) =>
            JArray(v :: xs)
          case (x, y) =>
            JArray(x :: y :: Nil)
        }
  ...
  private def bufRender(
    ...
    value match {
      ...
      case JString(null) =>
        buf.append("null")
      case JString(s) =>
        bufQuote(s, buf, settings)
      case JArray(arr) =>
        bufRenderArr(arr, buf, settings, indentLevel)
      case JObject(obj) =>
...
trait JsonDSL extends Implicits {
  ...
  class JsonListAssoc(left: List[JField]) {
    def ~(right: (String, JValue)) =
      JObject(left ::: List(JField(right._1, right._2)))
...
trait Printer {
  ...
  def compact(d: JsonAST.RenderIntermediaryDocument): String =
    JsonAST.compactRender(d.value)
