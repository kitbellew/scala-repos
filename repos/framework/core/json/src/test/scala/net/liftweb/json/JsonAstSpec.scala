...
object JsonAstSpec extends Specification with JValueGen with ScalaCheck {
  ...
  "escape bad JSON characters by default" in {
    val allCharacters: String = ('\u0000' to '\uffff').mkString("")

    val rendered = JsonAST
      .render(JString(allCharacters), JsonAST.RenderSettings.compact)

    "[\u0000-\u0019]".r.pattern.matcher(rendered).find() must beFalse
  }

  "allow escaping bad JavaScript characters when serializing" in {
    val allCharacters = ('\u0000' to '\uffff').mkString("")

    val rendered = JsonAST
      .render(JString(allCharacters), JsonAST.RenderSettings.compactJs)
