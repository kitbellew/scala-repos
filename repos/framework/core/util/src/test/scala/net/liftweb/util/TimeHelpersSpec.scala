...
object TimeHelpersSpec
  ...
  "the TimeHelpers" should {
    ...
    "provide a millisToDays function returning the number of days since the epoch time" in
      forAllTimeZones {
        ...
        millisToDays(
          today.setYear(1970).setMonth(0).setDay(1).getTime.getTime) must_==
          0 // the epoch time
    ...
    "provide a daysSinceEpoch function returning the number of days since the epoch time" in
      forAllTimeZones { daysSinceEpoch must_== millisToDays(now.getTime) }
    "provide a time function creating a new Date object from a number of millis" in
      forAllTimeZones { time(1000) must_== new Date(1000) }
    ...
    "provide a parseInternetDate function to parse a string formatted using the internet format" in
      forAllTimeZones {
        parseInternetDate(internetDateFormatter.format(now)).getTime.toLong must
          beCloseTo(now.getTime.toLong, 1000L)
      }
    "provide a parseInternetDate function returning new Date(0) if the input date cant be parsed" in
      forAllTimeZones { parseInternetDate("unparsable") must_== new Date(0) }
    "provide a toInternetDate function formatting a date to the internet format" in
      forAllTimeZones {
        toInternetDate(now) must
          beMatching("..., \\d* ... \\d\\d\\d\\d \\d\\d:\\d\\d:\\d\\d .*")
      }
  ...
  "The Calendar class" should {
    ...
    "have a setTimezone method to setting the time zone and returning the updated Calendar" in
      forAllTimeZones { today.setTimezone(utc).getTimeZone must_== utc }
    "have a noTime method to setting the time to 00:00:00 and returning the updated Calendar" in
      forAllTimeZones { hourFormat(today.noTime.getTime) must_== "00:00:00" }
  }
}
...
object forAllTimeZones extends Around {
  ...
  override def around[T: AsResult](f: => T) =
    synchronized { // setDefault is on static context so tests should be sequenced
      ...
      val commonJavaAndJodaTimeZones =
        (TimeZone.getAvailableIDs.toSet & DateTimeZone.getAvailableIDs.toSet)
          .filter { timeZoneId =>
            TimeZone.getTimeZone(timeZoneId).getOffset(millis) ==
              DateTimeZone.forID(timeZoneId).getOffset(millis)
          }
