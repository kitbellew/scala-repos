...
object ListHelpersSpec extends Specification with ListHelpers {
  ...
  "ListHelpers.delta" should {
    "insert after 2" in {
      val ret =
        delta(List(1, 2, 4, 5), List(1, 2, 3, 4, 5)) {
          case InsertAfterDelta(3, 2) =>
            "ok"
          case _ =>
            "not ok"
        }
    ...
    "prepend and append 2,4, 99" in {
      val ret =
        delta(List(2, 4, 99), List(1, 2, 3, 4, 5)) {
          case InsertAfterDelta(3, 2) =>
            "ok"
          case AppendDelta(5) =>
            "ok5"
          case RemoveDelta(99) =>
            "99"
          case InsertAtStartDelta(1) =>
            "1"
          case InsertAfterDelta(5, 4) =>
            "ok5"
          case _ =>
            "fail"
        }
    ...
    "prepend and append" in {
      val ret =
        delta(List(4, 2, 99), List(1, 2, 3, 4, 5)) {
          case InsertAfterDelta(3, 2) =>
            "ok"
          case InsertAfterDelta(4, 3) =>
            "ok3"
          case RemoveDelta(4) =>
            "r4"
          case AppendDelta(5) =>
  ...
  "The ListHelpers rotateList function (rotate method on a List object)" should {
    "create a List of all the circular permutations of a given list" in {
      List(1, 2, 3)
        .rotate must_== List(List(1, 2, 3), List(2, 3, 1), List(3, 1, 2))
    }
  }
