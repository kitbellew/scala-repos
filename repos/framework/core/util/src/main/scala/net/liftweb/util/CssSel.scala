...
private class SelectorMap(binds: List[CssBind])
  ...
  private val (
    ...
    val selThis: Box[CssBind] = binds.flatMap {
      b =>
        b.css.openOrThrowException(
          "Guarded with test before calling this method").subNodes match {
    ...
    final def applyAttributeRules(bindList: List[CssBind], elem: Elem): Elem = {
      bindList.map(b =>
        (
          b,
          b.css.openOrThrowException(
            "Guarded with test before calling this method").subNodes.openOrThrowException(
            "Guarded with test before calling this method"))).foldLeft(elem) {
        ...
        case (elem, (bind, AttrRemoveSubNode(attr))) => {
          ...
          if (calced.isEmpty || org.isEmpty) {
            ...
            val flat: Box[NodeSeq] = if (attr == "class") {
              ...
              SuperString(org.text).charSplit(' ').toList.filter(
                _.length > 0).filter(s => !set.contains(s)) match {
                case Nil => Empty
  ...
  final def apply(in: NodeSeq): NodeSeq =
    selectThis match {
      case Full(_) => {
        try {
          run(in, true, 0)
      ...
      cssSelector.collect {
        case EnclosedSelector(a, b) =>
          new CssBindPromoter(stringSelector, Full(a)) #> nsFunc({ ns =>
            new CssBindPromoter(stringSelector, Full(b)).#>(replacement)(
              converter)(ns)
...
trait CssBind extends CssSel {
  ...
  /**
    ...
    */
  private[util] def selectThis_? : Boolean =
    css match {
      case Full(sel) => {
        sel.subNodes match {
          case Full(SelectThisNode(_)) => true
  ...
  /**
    ...
    */
  private[util] def attrSel_? : Boolean =
    css match {
      case Full(sel) => {
        sel.subNodes match {
...
object CanBind extends CssBindImplicits {
  ...
  implicit def nodeSeqFuncTransform[A](implicit
      view: A => NodeSeq => NodeSeq): CanBind[A] =
    new CanBind[A] {
  ...
  implicit def nodeFuncTransform: CanBind[NodeSeq => Node] =
    new CanBind[NodeSeq => Node] {
      def apply(func: => NodeSeq => Node)(ns: NodeSeq): Seq[NodeSeq] =
        List(func(ns))
    }

  implicit def iterableNodeTransform[NST](implicit
      f2: NST => NodeSeq): CanBind[Iterable[NST]] =
  ...
  implicit def boxNodeTransform[NST](implicit
      f2: NST => NodeSeq): CanBind[Box[NST]] =
    new CanBind[Box[NST]] {
  ...
  implicit def optionNodeTransform[NST](implicit
      f2: NST => NodeSeq): CanBind[Option[NST]] =
  ...
  implicit def stringFuncTransform: CanBind[NodeSeq => String] =
    new CanBind[NodeSeq => String] {
      def apply(info: => NodeSeq => String)(ns: NodeSeq): Seq[NodeSeq] =
        List(Text(info(ns)))
    }
