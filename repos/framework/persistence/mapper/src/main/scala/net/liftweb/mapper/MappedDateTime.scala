...
abstract class MappedDateTime[T <: Mapper[T]](val fieldOwner: T)
  ...
  /**
    ...
    */
  def parse(s: String): Box[Date] =
    LiftRules.dateTimeConverter().parseDateTime(s)
  ...
  def real_convertToJDBCFriendly(value: Date): Object =
    if (value == null) null else new java.sql.Timestamp(value.getTime)
  ...
  def buildSetActualValue(
    ...
    (inst, v) =>
      doField(
        ...
        accessor,
        {
          case f: MappedDateTime[T] => f.st(toDate(v))
        })
  ...
  def buildSetDateValue(
    ...
    (inst, v) =>
      doField(
        ...
        accessor,
        {
          case f: MappedDateTime[T] => f.st(Full(v))
        })
  ...
  def buildSetBooleanValue(
    ...
    (inst, v, isNull) =>
      doField(
        ...
        accessor,
        {
          case f: MappedDateTime[T] => f.st(Empty)
        })
