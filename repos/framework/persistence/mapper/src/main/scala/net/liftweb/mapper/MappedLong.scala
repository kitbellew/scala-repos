...
abstract class MappedEnumList[T <: Mapper[T], ENUM <: Enumeration](
    val fieldOwner: T,
    val enum: ENUM)(implicit val manifest: TypeTag[Seq[ENUM#Value]])
  ...
  def real_convertToJDBCFriendly(value: Seq[ENUM#Value]): Object =
    new java.lang.Long(Helpers.toLong(value))
  ...
  override def _toForm: Box[NodeSeq] =
    Full(
      SHtml.checkbox[ENUM#Value](
        enum.values.iterator.toList,
        get,
        this(_)).toForm)
}
...
abstract class MappedLong[T <: Mapper[T]](val fieldOwner: T)
  ...
  def real_convertToJDBCFriendly(value: Long): Object =
