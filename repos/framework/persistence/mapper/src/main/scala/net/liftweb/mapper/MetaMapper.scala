...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  def findAllByInsecureSql(
    ...
    findAllByInsecureSqlDb(dbDefaultConnectionIdentifier, query, checkedBy)
  ...
  def findAllByPreparedStatement(
      dbId: ConnectionIdentifier,
      stmt: PreparedStatement): List[A] =
    findAllByPreparedStatementDb(dbId, stmt)(a => Full(a))
  ...
  type OtherMapper = KeyedMapper[
    _,
    _
  ] // T forSome {type T <: KeyedMapper[KeyDude, T]}
  type OtherMetaMapper = KeyedMetaMapper[
    _,
    _
  ...
  def findAllFields(
      fields: scala.collection.Seq[SelectableField],
      by: QueryParam[A]*): List[A] =
    findMapFieldDb(dbDefaultConnectionIdentifier, fields, by: _*)(v => Full(v))

  def findAllFieldsDb(
      dbId: ConnectionIdentifier,
      fields: Seq[SelectableField],
      by: QueryParam[A]*): List[A] =
    findMapFieldDb(dbId, fields, by: _*)(v => Full(v))

  private def dealWithPrecache(
      ret: List[A],
      by: Seq[QueryParam[A]]): List[A] = {

    val precache: List[PreCache[A, _, _]] = by.toList.flatMap {
      case j: PreCache[A, _, _] => List[PreCache[A, _, _]](j)
      case _                    => Nil
    }
    for (j <- precache) {
      ...
      val ol: List[MT] =
        if (!j.deterministic) {
          ...
          val lst: Set[FT] = Set(
            filter(ret.map(v =>
              v.getSingleton.getActualField(v, j.field).get
                .asInstanceOf[FT])): _*)
          ...
          j.field.dbKeyToTable.asInstanceOf[MetaMapper[A]].findAll(
            new InThing[A] {
              ...
              val outerField: MappedField[JoinType, A] = j.field.dbKeyToTable
                .primaryKeyField.asInstanceOf[MappedField[JoinType, A]]
              val innerField: MappedField[JoinType, A] = j.field
      ...
      val map: Map[FT, MT] = Map(ol.map(v => (v.primaryKeyField.get, v)): _*)

      for (i <- ret) {
        val field: MappedForeignKey[FT, A, _] = getActualField(i, j.field)
          .asInstanceOf[MappedForeignKey[FT, A, _]]
  ...
  def findMapDb[T](dbId: ConnectionIdentifier, by: QueryParam[A]*)(
      f: A => Box[T]): List[T] = findMapFieldDb(dbId, mappedFields, by: _*)(f)
  ...
  def buildSelectString(
    ...
    val bl = by.toList ::: addlQueryParams.get
    val selectStatement = "SELECT " + distinct(by) +
      fields.map(_.dbSelectString).mkString(", ") + " FROM " +
      MapperRules.quoteTableName.vend(_dbTableNameLC) + "  "

    val (str, start, max) = addEndStuffs(
      addFields(selectStatement, false, bl, conn),
      bl,
      conn)
    (str, start, max, bl)
  }

  def findMapFieldDb[T](
      dbId: ConnectionIdentifier,
      fields: Seq[SelectableField],
  ...
  object addlQueryParams
      extends net.liftweb.http.RequestVar[List[QueryParam[A]]](Nil) {
    override val __nameSalt = randomString(10)
  }

  private[mapper] def addFields(
      what: String,
      whereAdded: Boolean,
      by: List[QueryParam[A]],
      conn: SuperConnection): String = {
        ...
        x match {
            ...
            (1 to field.dbColumnCount).foreach { cn =>
              updatedWhat = updatedWhat + whereOrAnd +
                ...
                MapperRules.quoteColumnName
                  .vend(otherField.dbColumnNames(otherField.name)(cn - 1))
            }
          ...
          case ByList(field, orgVals) =>
            ...
            if (vals.isEmpty) updatedWhat = updatedWhat + whereOrAnd + " 0 = 1 "
              ...
              updatedWhat = updatedWhat + vals.map(v =>
          ...
          case in: InRaw[A, _] =>
            updatedWhat = updatedWhat + whereOrAnd +
              (in.rawSql match {
                ...
                case sql =>
                  " " +
                    MapperRules.quoteColumnName.vend(in.field._dbColumnNameLC) +
                    " IN ( " + sql + " ) "
              })
          ...
          case (in: InThing[A]) =>
            updatedWhat = updatedWhat + whereOrAnd +
              ...
              in.inKeyword + "(" + in.innerMeta.addEndStuffs(
                in.innerMeta.addFields(
                  "SELECT " + in.distinct +
                    MapperRules.quoteColumnName
                      .vend(in.innerField._dbColumnNameLC) + " FROM " +
                    MapperRules.quoteTableName
                      .vend(in.innerMeta._dbTableNameLC) + " ",
                  false,
                  in.queryParams,
                  conn
                ),
  ...
  private[mapper] def setStatementFields(
      st: PreparedStatement,
      by: List[QueryParam[A]],
      curPos: Int,
      conn: SuperConnection): Int = {
    by match {
      case Nil => curPos
      ...
      case (in: InThing[A]) :: xs =>
        val newPos = in.innerMeta
          .setStatementFields(st, in.queryParams, curPos, conn)
        setStatementFields(st, xs, newPos, conn)
      ...
      case BySql(query, who, params @ _*) :: xs => {
        params.toList match {
          ...
          case p :: ps =>
            setStatementFields(
              st,
              BySql[A](query, who, p) :: BySql[A](query, who, ps: _*) :: xs,
              curPos,
              conn)
        }
      }
  ...
  private def _addOrdering(in: String, params: List[QueryParam[A]]): String = {
    params.flatMap {
      case OrderBy(field, order, nullOrder) =>
        List(
          MapperRules.quoteColumnName.vend(field._dbColumnNameLC) + " " +
            order.sql + " " + (nullOrder.map(_.getSql).openOr("")))
    ...
    if (conn.brokenLimit_?) (tmp, start, max)
      ...
      val ret = (max, start) match {
        case (Full(max), Full(start)) =>
          tmp + " LIMIT " + max + " OFFSET " + start
        case (Full(max), _) => tmp + " LIMIT " + max
        case (_, Full(start)) =>
          tmp + " LIMIT " + conn.driverType.maxSelectLimit + " OFFSET " + start
        case _ => tmp
      }
      (ret, Empty, Empty)
    }
  ...
  private[mapper] def ??(meth: Method, inst: A) =
    meth.invoke(inst).asInstanceOf[MappedField[AnyBound, A]]

  def dirty_?(toTest: A): Boolean =
    mappedFieldList.exists(mft => ??(mft.method, toTest).dirty_?)
    ...
    toSave match {
      case x: MetaMapper[_] =>
        throw new MapperException(
          "Cannot test the MetaMapper singleton for saved status")
  ...
  /**
    ...
    */
  protected def decodeFromJSON_!(
      json: JsonAST.JObject,
      markFieldsAsDirty: Boolean): A = {
  ...
  final def validate(toValidate: A): List[FieldError] = {
    ...
    logger.debug("Validated dbName=%s, entity=%s, result=%s".format(
      dbName,
      toValidate,
      ret))
  ...
  def clean_?(toCheck: A): Boolean =
    mappedColumns
      .foldLeft(true)((bool, ptr) => bool && !(??(ptr._2, toCheck).dirty_?))
            ...
            val ret =
              if (saved_?(toSave)) {
                ...
                val query = "INSERT INTO " +
                  MapperRules.quoteTableName.vend(_dbTableNameLC) + " (" +
                  columnNamesForInsert + ") VALUES (" + columnQueriesForInsert +
                  ")"
                ...
                val generatedColumns =
                  (mappedColumnInfo.filter(_._2.dbAutogenerated_?).map(_._1))
                    .toList
  ...
  def createInstances(
      ...
      rs: ResultSet,
      start: Box[Long],
      omax: Box[Long]): List[A] =
    createInstances(dbId, rs, start, omax, v => Full(v))

  def createInstances[T](
      dbId: ConnectionIdentifier,
      rs: ResultSet,
      start: Box[Long],
      omax: Box[Long],
  ...
  def appendFieldToStrings(in: A): String =
    mappedFieldList.map(p => ??(p.method, in).asString).mkString(",")

  private val columnNameToMappee =
    new HashMap[String, Box[(ResultSet, Int, A) => Unit]]
      ...
      for {
        ...
        case None =>
          ...
          Box(mappedColumns.get(colName)).flatMap { fieldInfo =>
            val setTo = {
              val tField = fieldInfo.invoke(this)
                .asInstanceOf[MappedField[AnyRef, A]]
  ...
  def createInstance(
      dbId: ConnectionIdentifier,
      rs: ResultSet,
      mapFuncs: List[Box[(ResultSet, Int, A) => Unit]]): A = {
  ...
  private def createApplier(
    ...
    val accessor = mappedColumns.get(name) orElse
      mappedColumns.get(name.toLowerCase)
    if ((accessor eq null) || accessor == None) { null }
    else {
      (accessor.get.invoke(this).asInstanceOf[MappedField[AnyRef, A]])
  ...
  def fieldMapperTransforms(
      fieldHtml: (BaseOwnedMappedField[A] => NodeSeq),
      mappedObject: A): Seq[CssSel] = {
  ...
  private val mappedAppliers = new HashMap[
  ...
  private var mappedColumnInfo: SortedMap[String, MappedField[AnyRef, A]] =
    TreeMap()
  ...
  this.runSafe {
    ...
    def isLifecycle(m: Method) =
      classOf[LifecycleCallbacks].isAssignableFrom(m.getReturnType)

    val mapperAccessMethods = new FieldFinder[MappedField[_, _]](this, logger)
      .accessorMethods

    mappedCallbacks = mapperAccessMethods.filter(isLifecycle)
      .map(v => (v.getName, v))
      ...
      v.invoke(this) match {
        case untypedMf: MappedField[_, _] if !untypedMf.ignoreField_? =>
          ...
          for (colName <- mf.dbColumnNames(v.getName)
                 .map(MapperRules.quoteColumnName.vend).map(_.toLowerCase)) {
            mappedColumnInfo += colName -> mf
            mappedColumns += colName -> v
          }
          if (mf.dbPrimaryKey_?) {
            thePrimaryKeyField = Full(
              MapperRules.quoteColumnName.vend(mf._dbColumnNameLC))
    ...
    fieldOrder
      .foreach(f => findPos(f).foreach(pos => resArray += tArray.remove(pos)))
    ...
    logger.trace("Mapped fields for %s: %s".format(
      dbName,
      mappedFieldList.map(_.name).mkString(",")))
  }

  val columnNamesForInsert = (mappedColumnInfo
    .filter(c => !(c._2.dbPrimaryKey_? && c._2.dbAutogenerated_?)).map(_._1))
    .toList.mkString(",")
  ...
  /**
    ...
    * htmlHeades method
    */
  var displayNameToHeaderElement: String => NodeSeq = MapperRules
  ...
  /**
    * The mapped fields
    */
  lazy val mappedFields: Seq[BaseMappedField] = mappedFieldList
    .map(f => f.field)

  /**
    * the mapped fields as MappedField rather than BaseMappedField
    */
  lazy val mappedFieldsForModel: List[MappedField[_, A]] = mappedFieldList
    .map(_.field)
  ...
  /**
    ...
    * doHtmlLine method.
    */
  var displayFieldAsLineElement: NodeSeq => NodeSeq = MapperRules
  ...
  def asJs(actual: A): JsExp = {
    JE.JsObj(
      ("$lift_class", JE.Str(dbTableName)) ::
        mappedFieldList.map(f => ??(f.method, actual)).filter(_.renderJs_?)
          .flatMap(_.asJs).toList ::: actual.suplementalJs(Empty): _*)
  }
  ...
  /**
    ...
    */
  lazy val doAllFieldNames: Seq[(String, SourceFieldMetadata)] = mappedFieldList
    .map(fh => fh.name.toLowerCase -> fh.field.sourceInfoMetadata())

  /**
    * Get a list of all the fields as a map
    * @return a list of all the fields
    */
  lazy val fieldNamesAsMap: Map[String, SourceFieldMetadata] = Map(
    doAllFieldNames: _*)
  ...
  def asHtml(toLine: A): NodeSeq =
    Text(internalTableName_$_$) :: Text("={ ") ::
      ...
      } yield { <span>{field.displayName}={field.asHtml}&nbsp;</span> }) :::
      List(Text(" }"))
  ...
  /**
    ...
    * you can change the function to display something else.
    */
  var formatFormElement: (NodeSeq, NodeSeq) => NodeSeq = MapperRules
    .formatFormElement
  ...
  def toForm(toMap: A): NodeSeq =
    mappedFieldList.map(e => ??(e.method, toMap))
      .filter(f => f.dbDisplay_? && f.dbIncludeInForm_?).flatMap(field =>
        field.toForm.toList
  ...
  def flatMapFieldTitleForm[T](
    ...
    formFields(toMap).flatMap(field =>
      field.toForm.toList
        .flatMap(fo => func(field.displayHtml, field.fieldId, fo)))
  ...
  /**
    ...
    */
  def getActualField[T](
      actual: A,
      protoField: MappedField[T, A]): MappedField[T, A] =
  ...
  final def _dbTableNameLC = {
    ...
    if (conn.isDefined) {
      val rc = conn
        .openOrThrowException("We just checked that this is a Full Box")
  ...
  private def _beforeValidationOnCreate(what: A) {
    eachField(what, beforeValidationOnCreate) { field =>
      field.beforeValidationOnCreate
    }
  }
  private def _beforeValidationOnUpdate(what: A) {
    eachField(what, beforeValidationOnUpdate) { field =>
      field.beforeValidationOnUpdate
    }
  }
  ...
  private def _afterValidationOnCreate(what: A) {
    eachField(what, afterValidationOnCreate) { field =>
      field.afterValidationOnCreate
    }
  }
  private def _afterValidationOnUpdate(what: A) {
    eachField(what, afterValidationOnUpdate) { field =>
      field.afterValidationOnUpdate
    }
  }
  ...
  implicit def fieldToItem[T](in: MappedField[T, A]): IndexItem[A] =
    IndexField(in)
  implicit def boundedFieldToItem(
      in: (MappedField[String, A], Int)): BoundedIndexField[A] =
    BoundedIndexField(in._1, in._2)
...
final case class Index[A <: Mapper[A]](columns: List[IndexItem[A]])
    extends BaseIndex[A] // (columns :_*)

object Index {
  def apply[A <: Mapper[A]](cols: IndexItem[A]*): Index[A] =
    new Index[A](cols.toList)
}

/**
  *  Represents a unique index on the given columns
  */
final case class UniqueIndex[A <: Mapper[A]](columns: List[IndexItem[A]])
    extends BaseIndex[A] // (uniqueColumns : _*)

object UniqueIndex {
  def apply[A <: Mapper[A]](cols: IndexItem[A]*): UniqueIndex[A] =
    new UniqueIndex[A](cols.toList)
}
...
/**
  ...
  */
final case class GenericIndex[A <: Mapper[A]](
    createFunc: (String, List[String]) => String,
    validated: IHaveValidatedThisSQL,
    columns: List[IndexItem[A]])
    extends BaseIndex[A] // (indexColumns : _*)

object GenericIndex {
  def apply[A <: Mapper[A]](
      createFunc: (String, List[String]) => String,
      validated: IHaveValidatedThisSQL,
      cols: IndexItem[A]*): GenericIndex[A] =
...
case class IndexField[A <: Mapper[A], T](field: MappedField[T, A])
    extends IndexItem[A] {
  def indexDesc: String =
    MapperRules.quoteColumnName.vend(field._dbColumnNameLC)
}
case class BoundedIndexField[A <: Mapper[A]](
    field: MappedField[String, A],
    len: Int)
    extends IndexItem[A] {
  def indexDesc: String =
    MapperRules.quoteColumnName.vend(field._dbColumnNameLC) + "(" + len + ")"
}

sealed trait QueryParam[O <: Mapper[O]]
final case class Cmp[O <: Mapper[O], T](
    field: MappedField[T, O],
    opr: OprEnum.Value,
    value: Box[T],
    otherField: Box[MappedField[T, O]],
    dbFunc: Box[String])
    extends QueryParam[O]

final case class OrderBy[O <: Mapper[O], T](
    field: MappedField[T, O],
    order: AscOrDesc,
    nullOrder: Box[NullOrder])
    extends QueryParam[O]
...
object OrderBy {
  def apply[O <: Mapper[O], T](
      field: MappedField[T, O],
      order: AscOrDesc): OrderBy[O, T] = new OrderBy[O, T](field, order, Empty)

  def apply[O <: Mapper[O], T](
      field: MappedField[T, O],
      order: AscOrDesc,
      no: NullOrder): OrderBy[O, T] = new OrderBy[O, T](field, order, Full(no))
}
...
final case class OrderBySql[O <: Mapper[O]](
    sql: String,
...
/**
  ...
  */
final case class BySql[O <: Mapper[O]](
    query: String,
    checkedBy: IHaveValidatedThisSQL,
    params: Any*)
    extends QueryParam[O]
...
sealed abstract class InThing[OuterType <: Mapper[OuterType]]
...
/**
  ...
  * be generated
  */
final case class PreCache[TheType <: Mapper[
  TheType], FieldType, OtherType <: KeyedMapper[FieldType, OtherType]](
    field: MappedForeignKey[FieldType, TheType, OtherType],
    deterministic: Boolean)
    extends QueryParam[TheType]

object PreCache {
  def apply[TheType <: Mapper[TheType], FieldType, OtherType <: KeyedMapper[
    FieldType,
    OtherType]](field: MappedForeignKey[FieldType, TheType, OtherType]) =
    new PreCache(field, true)
}

final case class InRaw[TheType <: Mapper[TheType], T](
    field: MappedField[T, TheType],
    rawSql: String,
    checkedBy: IHaveValidatedThisSQL)
    extends QueryParam[TheType]

object NotIn {
  def fk[InnerMapper <: Mapper[InnerMapper], JoinTypeA, Zoom <% QueryParam[
    InnerMapper], OuterMapper <: KeyedMapper[JoinTypeA, OuterMapper]](
    ...
    new InThing[OuterMapper] {
      ...
      val outerField: MappedField[JoinType, OuterMapper] = fielda.dbKeyToTable
        .primaryKeyField
  ...
  def apply[InnerMapper <: Mapper[InnerMapper], JoinTypeA, Zoom <% QueryParam[
    InnerMapper], OuterMapper <: Mapper[OuterMapper]](
    ...
    new InThing[OuterMapper] {
      ...
      val innerMeta: MetaMapper[InnerMapper] = innerField.fieldOwner
        .getSingleton
...
object By {
  ...
  def apply[O <: Mapper[O], T, U <% T](field: MappedField[T, O], value: U) =
  ...
  def apply[O <: Mapper[O], T, Q <: KeyedMapper[T, Q]](
    ...
    value match {
      case Full(v) =>
        Cmp[O, T](field, Eql, Full(v.primaryKeyField.get), Empty, Empty)
...
object By_>= {
  ...
  def apply[O <: Mapper[O], T, U <% T](field: MappedField[T, O], value: U) =
    Cmp[O, T](field, >=, Full(value), Empty, Empty)

  def apply[O <: Mapper[O], T](
      field: MappedField[T, O],
      otherField: MappedField[T, O]) =
    Cmp[O, T](field, >=, Empty, Full(otherField), Empty)
}
...
object NotBy {
  ...
  def apply[O <: Mapper[O], T, Q <: KeyedMapper[T, Q]](
    ...
    value match {
      case Full(v) =>
        Cmp[O, T](field, <>, Full(v.primaryKeyField.get), Empty, Empty)
...
object ByRef {
  ...
  def apply[O <: Mapper[O], T](
      field: MappedField[T, O],
      otherField: MappedField[T, O]) =
    Cmp[O, T](field, Eql, Empty, Full(otherField), Empty)
}
...
object NullRef {
  ...
  def apply[O <: Mapper[O], T](field: MappedField[T, O]) =
    Cmp(field, IsNull, Empty, Empty, Empty)
}

object NotNullRef {
  import OprEnum._
  def apply[O <: Mapper[O], T](field: MappedField[T, O]) =
    Cmp(field, IsNotNull, Empty, Empty, Empty)
}
...
trait KeyedMetaMapper[Type, A <: KeyedMapper[Type, A]]
  ...
  type Q =
    MappedForeignKey[AnyBound, A, OO] with MappedField[AnyBound, A] forSome {
  ...
  def asSafeJs(actual: A, f: KeyObfuscator): JsExp = {
    ...
    JE.JsObj(
      first :: ("$lift_class", JE.Str(dbTableName)) ::
        mappedFieldList.map(f => this.??(f.method, actual))
  ...
  def find(key: Any): Box[A] =
    key match {
      ...
      case prod: Product if (testProdArity(prod)) =>
        find(convertToQPList(prod): _*)
      case key => anyToFindString(key) flatMap (find(_))
    }

  def findDb(dbId: ConnectionIdentifier, key: Any): Box[A] =
    key match {
      case qp: QueryParam[A] =>
        findDb(dbId, List(qp.asInstanceOf[QueryParam[A]]): _*)
      case prod: Product if (testProdArity(prod)) =>
        findDb(dbId, convertToQPList(prod): _*)
  ...
  def find(key: String): Box[A] =
    dbStringToKey(key) flatMap
      (realKey => findDbByKey(selectDbForKey(realKey), realKey))

  def find(dbId: ConnectionIdentifier, key: String): Box[A] =
    dbStringToKey(key) flatMap (realKey => findDbByKey(dbId, realKey))
      ...
      DB.prepareStatement(
        ...
        if (field.dbIgnoreSQLType_?)
          ...
          st.setObject(
            ...
            conn.driverType
              .columnTypeMap(field.targetSQLType(field._dbColumnNameLC)))
        DB.exec(st) { rs =>
  ...
  def viewTransform(html: NodeSeq): NodeSeq = {
    ...
    val otherTransforms =
      obj.fieldMapperTransforms(_.asHtml).reverse ++ obj.fieldTransforms.reverse

    otherTransforms.foldRight(PassThru: (NodeSeq) => NodeSeq)(_ andThen _) apply
      html
  }
...
class KeyObfuscator {
  ...
  def obscure[KeyType, MetaType <: KeyedMapper[KeyType, MetaType]](
      ...
      key: KeyType): String =
    synchronized {
      val local: Map[Any, String] = to
        .getOrElse(theType._dbTableNameLC, Map.empty)
      local.get(key) match {
        case Some(s) => s
        case _ =>
          ...
          val lf: Map[String, Any] = from
            .getOrElse(theType._dbTableNameLC, Map.empty) + ((ret -> key))
          // lf(ret) = key
  ...
  def apply[KeyType, MetaType <: KeyedMapper[KeyType, MetaType], Q <% KeyType](
      theType: KeyedMetaMapper[KeyType, MetaType],
      key: Q): String = {
    val k: KeyType = key
    obscure(theType, k)
