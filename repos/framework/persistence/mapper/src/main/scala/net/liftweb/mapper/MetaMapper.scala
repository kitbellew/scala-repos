...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  private[mapper] def addFields(
    ...
    implicit def dbfToFunc(in: Box[String]): DBFuncWrapper = new DBFuncWrapper(
      in)
  ...
  /**
    ...
    */
  def clean_?(toCheck: A): Boolean = mappedColumns.foldLeft(true)((bool, ptr) =>
  ...
  /**
    ...
    * it is autogenerated
    */
  def columnPrimaryKey_?(name: String) = mappedColumnInfo
    .get(name)
  ...
  implicit def fieldToItem[T](in: MappedField[T, A]): IndexItem[A] = IndexField(
    in)
...
trait KeyedMetaMapper[Type, A <: KeyedMapper[Type, A]]
  ...
  def find(key: String): Box[A] = dbStringToKey(key) flatMap (realKey =>
