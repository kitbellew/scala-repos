...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  private def dealWithPrecache(
    ...
    val precache: List[PreCache[A, _, _]] = by
      .toList
      .flatMap {
        case j: PreCache[A, _, _] =>
          List[PreCache[A, _, _]](j)
        case _ =>
          Nil
      }
    for (j <- precache) {
        ...
        if (!j.deterministic) {
          def filter(in: Seq[FT]): Seq[FT] =
            in.flatMap {
              case null =>
                Nil
              case x: Number if x.longValue == 0L =>
                Nil
              case x =>
                List(x)
            }
          ...
          j.field
            ...
            .findAll(
              new InThing[A] {
                ...
                val outerField: MappedField[JoinType, A] = j
                  .field
                  .dbKeyToTable
                  .primaryKeyField
                  .asInstanceOf[MappedField[JoinType, A]]
                val innerField: MappedField[JoinType, A] = j
                  .field
                  .asInstanceOf[MappedField[JoinType, A]]
    ...
    params.flatMap {
      case OrderBy(field, order, nullOrder) =>
        List(
          MapperRules.quoteColumnName.vend(field._dbColumnNameLC) + " " +
            order.sql + " " +
            (nullOrder.map(_.getSql).openOr("")))
      case OrderBySql(sql, _) =>
        List(sql)
      case _ =>
        Nil
    } match {
  ...
  protected def addEndStuffs(
    ...
    val max =
      params.foldRight(Empty.asInstanceOf[Box[Long]]) { (a, b) =>
        a match {
          case MaxRows(n) =>
            Full(n);
          case _ =>
      ...
      val ret =
        (max, start) match {
          ...
          case (Full(max), _) =>
            tmp + " LIMIT " + max
          case (_, Full(start)) =>
            tmp + " LIMIT " + conn.driverType.maxSelectLimit + " OFFSET " +
              start
          case _ =>
            tmp
        }
      ...
      case Types.BOOLEAN =>
        value match {
          ...
          case b: java.lang.Boolean =>
            st.setBoolean(index, b.booleanValue)
          // If we can't figure it out, maybe the driver can
          case other =>
            setObj(st, index, other, mappedColumnType)
        }

      // In all other cases, delegate to the driver
      case _ =>
        setObj(st, index, value, mappedColumnType)
    }
  }
            ...
            if (rs.next) {
              ...
              toSave.runSafe {
                for {
                  ...
                  auto <- primaryKeyAutogenerated
                  if auto
                } {
                  findApplier(indexMap, rs.getObject(1)) match {
                    case Full(ap) =>
                      ap.apply(toSave, rs.getObject(1))
                    case _ =>
                  }
                }
              }
      ...
      val look =
        (
          ...
          if (clz ne null)
            ...
            Empty)
      Box(
        mappedAppliers.get(look) orElse {
  ...
  private[mapper] def checkFieldNames(in: A): Unit = {
    mappedFieldList.foreach(f =>
      ??(f.method, in) match {
        case field if (field.i_name_! eq null) =>
          field.setName_!(f.name)
        case _ =>
      })
  }
  ...
  this.runSafe {
    ...
    val mapperAccessMethods =
      new FieldFinder[MappedField[_, _]](this, logger).accessorMethods

    mappedCallbacks = mapperAccessMethods
      .filter(isLifecycle)
      .map(v => (v.getName, v))
  ...
  val columnNamesForInsert = (mappedColumnInfo.filter(c => !(c._2.dbPrimaryKey_? && c._2.dbAutogenerated_?)).map(_._1))
    .toList
    .mkString(",")
    ...
    JE.JsObj(
      ("$lift_class", JE.Str(dbTableName)) ::
        mappedFieldList
          ...
          .filter(_.renderJs_?)
          .flatMap(_.asJs)
          .toList ::: actual.suplementalJs(Empty): _*)
  }
...
object NotIn {
  def fk[InnerMapper <: Mapper[InnerMapper], JoinTypeA, Zoom <% QueryParam[
    ...
    new InThing[OuterMapper] {
      ...
      val outerField: MappedField[JoinType, OuterMapper] =
        fielda.dbKeyToTable.primaryKeyField
  ...
  def apply[InnerMapper <: Mapper[InnerMapper], JoinTypeA, Zoom <% QueryParam[
    ...
    new InThing[OuterMapper] {
      ...
      val innerMeta: MetaMapper[InnerMapper] =
        innerField.fieldOwner.getSingleton
...
object By {
  ...
  def apply[O <: Mapper[O], T](
    ...
    value match {
      case Full(x) =>
        Cmp[O, Box[T]](field, Eql, Full(value), Empty, Empty)
      case _ =>
        NullRef(field)
    }
...
object NotBy {
  ...
  def apply[O <: Mapper[O], T](
    ...
    value match {
      case Full(x) =>
        Cmp[O, Box[T]](field, <>, Full(value), Empty, Empty)
      case _ =>
        NotNullRef(field)
    }
...
trait KeyedMetaMapper[Type, A <: KeyedMapper[Type, A]]
  ...
  private def anyToFindString(in: Any): Box[String] =
    in match {
      case Empty | None | null | Failure(_, _, _) =>
        Empty
      case Full(n) =>
        anyToFindString(n)
      case Some(n) =>
  ...
  def find(key: Any): Box[A] =
    key match {
      case qp: QueryParam[A] =>
        find(List(qp.asInstanceOf[QueryParam[A]]): _*)
      case prod: Product if (testProdArity(prod)) =>
        find(convertToQPList(prod): _*)
      case key =>
        anyToFindString(key) flatMap (find(_))
    }
  ...
  def findDb(dbId: ConnectionIdentifier, key: Any): Box[A] =
    key match {
      ...
      case key =>
        anyToFindString(key) flatMap (find(dbId, _))
    }
  ...
  def crudSnippets: LiftRules.SnippetPF = {
    ...
    NamedPF("crud " + Name) {
      case Name :: "addForm" :: _ =>
        addFormSnippet
      case Name :: "editForm" :: _ =>
        editFormSnippet
      case Name :: "viewTransform" :: _ =>
        viewTransform
    }
  }
