...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  def buildSelectString(
    ...
    val (str, start, max) =
      addEndStuffs(addFields(selectStatement, false, bl, conn), bl, conn)
    (str, start, max, bl)
  }
    ...
    for (v <- mapperAccessMethods) {
      v.invoke(this) match {
        case untypedMf: MappedField[_, _] if !untypedMf.ignoreField_? =>
          ...
          if (mf.dbPrimaryKey_?) {
            thePrimaryKeyField =
              Full(MapperRules.quoteColumnName.vend(mf._dbColumnNameLC))
  ...
  /**
    * The mapped fields
    */
  lazy val mappedFields: Seq[BaseMappedField] =
    mappedFieldList.map(f => f.field)

  /**
    * the mapped fields as MappedField rather than BaseMappedField
    */
  lazy val mappedFieldsForModel: List[MappedField[_, A]] =
    mappedFieldList.map(_.field)
  ...
  def flatMapFieldTitleForm2[T](
    ...
    formFields(toMap).flatMap(field =>
      field.toForm.toList
        .flatMap(fo => func(field.displayHtml, field, fo)))
...
trait KeyedMetaMapper[Type, A <: KeyedMapper[Type, A]]
  ...
  def asSafeJs(actual: A, f: KeyObfuscator): JsExp = {
    ...
    JE.JsObj(
      first :: ("$lift_class", JE.Str(dbTableName)) :: mappedFieldList
  ...
  def formSnippet(html: NodeSeq, obj: A, cleanup: (A => Unit)): NodeSeq = {
    ...
    val otherTransforms =
      obj
        .fieldMapperTransforms(_.toForm openOr Text(""))
        .reverse ++ obj.fieldTransforms.reverse
