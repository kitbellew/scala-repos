...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  private def _addOrdering(in: String, params: List[QueryParam[A]]): String =
    params.flatMap {
      case OrderBy(field, order, nullOrder) =>
        List(
          MapperRules.quoteColumnName
            .vend(field._dbColumnNameLC) + " " + order.sql + " " + (nullOrder
            .map(_.getSql)
            .openOr("")))
  ...
  this.runSafe {
    ...
    mappedCallbacks =
      mapperAccessMethods.filter(isLifecycle).map(v => (v.getName, v))
  ...
  def asHtml(toLine: A): NodeSeq =
    Text(internalTableName_$_$) :: Text("={ ") ::
      ...
      } yield <span>{field.displayName}={field.asHtml}&nbsp;</span>) ::: List(
      Text(" }"))
    ...
    DB.use(dbId) { conn =>
      ...
      DB.prepareStatement(
        "SELECT " +
          ...
          .vend(_dbTableNameLC) + " WHERE " + MapperRules.quoteColumnName.vend(
