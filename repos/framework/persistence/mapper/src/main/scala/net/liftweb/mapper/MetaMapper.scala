...
trait MetaMapper[A <: Mapper[A]] extends BaseMetaMapper with Mapper[A] {
  ...
  def countDb(dbId: ConnectionIdentifier, by: QueryParam[A]*): Long = {
    DB.use(dbId) { conn =>
      ...
      val (query, start, max) = addEndStuffs(
        addFields(
          "SELECT COUNT(*) FROM " + MapperRules.quoteTableName
            .vend(_dbTableNameLC) + "  ",
          false,
          bl,
          conn),
  ...
  def bulkDelete_!!(dbId: ConnectionIdentifier, by: QueryParam[A]*): Boolean = {
    DB.use(dbId) { conn =>
      ...
      val (query, start, max) = addEndStuffs(
        addFields(
          "DELETE FROM " + MapperRules.quoteTableName
            .vend(_dbTableNameLC) + " ",
          false,
          bl,
          conn),
  ...
  def findDbByKey(
    ...
    DB.use(dbId) { conn =>
      ...
      DB.prepareStatement(
        "SELECT " +
          fields.map(_.dbSelectString).mkString(", ") +
          " FROM " + MapperRules.quoteTableName
          .vend(_dbTableNameLC) + " WHERE " + MapperRules.quoteColumnName
          .vend(field._dbColumnNameLC) + " = ?",
        conn
      ) { st =>
