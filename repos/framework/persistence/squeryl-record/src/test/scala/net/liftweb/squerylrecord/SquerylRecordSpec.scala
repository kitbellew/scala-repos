...
class SquerylRecordSpec extends Specification with AroundExample {
  ...
  "SquerylRecord" should {
    ...
    "support order by" in {
      transaction {
        val orderedCompanies = from(companies)(c => select(c) orderBy (c.name))
      ...
      transaction {
        S.initIfUninitted(session) {
          val companiesWithEmployees = join(companies, employees.leftOuter)(
            (c, e) => select(c, e) on (c.id === e.map(_.companyId)))
          ...
          companiesAndEmployeesWithSameName.filter(ce => ce.measures == 0) must
            haveSize(2)
    ...
    "support updates" in {
      ...
      transactionWithRollback {
        S.initIfUninitted(session) {
          ...
          update(companies)(c => where(c.id === id) set (c.name := "Name2"))
    ...
    "support inner queries" >> {
      ...
      transaction {
        ...
        val companyId: Long = from(companies)(c =>
          where(
            c.id in from(companies)(c2 =>
              where(c2.id === td.c1.id) select (c2.id))) select (c.id)).single
        ...
        val companyIdField: LongField[Company] = from(companies)(c =>
          where(
            c.idField in from(companies)(c2 =>
        ...
        val companyIdByName: Long = from(companies)(c =>
          where(
            c.name in from(companies)(c2 =>
              where(c2.name === td.c1.name) select (c2.name))) select (c.id))
          .single
        ...
        val companyIdByCreated: DateTimeField[Company] = from(companies)(c =>
          where(
            c.created in from(companies)(c2 =>
        ...
        val empSalary: DecimalField[Employee] = from(employees)(e =>
          where(
            e.salary in from(employees)(e2 =>
              where(e2.id === td.e1.id) select (e2.salary))) select (e.salary))
          .single
        ...
        val empEmail: EmailField[Employee] = from(employees)(e =>
          where(
            e.email in from(employees)(e2 =>
              where(e2.id === td.e1.id) select (e2.email))) select (e.email))
          .single
        empSalary.get must_== td.e1.salary.get
        ...
        val empAdmin: BooleanField[Employee] = from(employees)(e =>
          where(
            e.admin in from(employees)(e2 =>
              where(e2.id === td.e2.id) select (e2.admin))) select (e.admin))
          .single
        ...
        val empRoleQuery = from(employees)(e =>
          where(
            e.role in from(employees)(e2 =>
              where(e2.id === td.e2.id) select (e2.role))) select (e.role.get))
  ...
  class ToChar(
      d: DateExpression[Timestamp],
      e: StringExpression[String],
      m: OutMapper[String])
  ...
  def toChar(d: DateExpression[Timestamp], e: StringExpression[String])(implicit
      m: OutMapper[String]) = new ToChar(d, e, m)
