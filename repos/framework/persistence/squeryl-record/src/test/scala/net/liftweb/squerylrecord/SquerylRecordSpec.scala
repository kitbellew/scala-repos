  ...
  "SquerylRecord" should {
      ...
      transaction {
        S.initIfUninitted(session) {
          ...
          val companiesAndEmployeesWithSameName =
            join(companies, employees.leftOuter)((c, e) =>
              groupBy(c.id) compute
                (countDistinct(e.map(_.id))) on
                (c.name === e.map(_.name)))
    ...
    "support inner queries" >> {
      ...
      transaction {
        ...
        // ...And DateTime-Fields:
        val companyIdByCreated: DateTimeField[Company] =
          from(companies)(c =>
            where(
              c.created in
                from(companies)(c2 =>
        ...
        // Decimal Fields:
        val empSalary: DecimalField[Employee] =
          from(employees)(e =>
            where(
              e.salary in
                from(employees)(e2 =>
        ...
        // Email fields:
        val empEmail: EmailField[Employee] =
          from(employees)(e =>
            where(
              e.email in
                from(employees)(e2 =>
        ...
        // Boolean fields:
        val empAdmin: BooleanField[Employee] =
          from(employees)(e =>
            where(
              e.admin in
                from(employees)(e2 =>
    ...
    "Properly reset the dirty_? flag after loading entities" >>
      inTransaction {
        val company =
