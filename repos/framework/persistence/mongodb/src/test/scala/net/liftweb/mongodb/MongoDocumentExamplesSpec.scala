...
class MongoDocumentExamplesSpec extends Specification with MongoTestKit {
  ...
  "Mongo examples" in {
    ...
    MongoDB.use(db => {
      ...
      val lstjobj2 = SessCollection.findAll(
        ("name" -> (("$regex" -> "^Mongo") ~ ("$flags" -> 0))) ~ ("count" -> 3))
  ...
  "Pattern example" in {
    ...
    PatternDoc.find(pdoc1._id) must beLike { case Some(pdoc) =>
  ...
  "Issue 586 Date test" in {
    ...
    fromDb must beLike { case Some(sdd) =>
      sdd._id must_== newId
      sdd.dt must_== newDt
      sdd.save

      StringDateDoc.find(newId) must beLike { case Some(sdd2) =>
