...
class MongoDirectSpec extends Specification with MongoTestKit {
  ...
  "Mongo tutorial 2 example" in {
    ...
    MongoDB.useCollection("iDoc")(coll => {
      ...
      // limiting result set
      val cur5 = coll.find(new BasicDBObject("i", new BasicDBObject("$gt", 50)))
        .limit(3)
      ...
      // skip
      val cur6 = coll.find(new BasicDBObject("i", new BasicDBObject("$gt", 50)))
        .skip(10)
  ...
  "Mongo more examples" in {
    ...
    MongoDB.use(db => {
      ...
      val cur2 = coll.find(
        BasicDBObjectBuilder.start.add(key, Pattern.compile(regex))
          .add("count", 1).get)
      cur2.count must_== 1
