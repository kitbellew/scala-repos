  ...
  private def serialize(a: Any, formats: Formats): JValue = {
    ...
    a.asInstanceOf[AnyRef] match {
      ...
      case x if datetype_?(x.getClass) =>
        datetype2jvalue(x)(formats)
      case x if mongotype_?(x.getClass) =>
        mongotype2jvalue(x)(formats)
      case x: BasicDBList =>
  ...
  object Parser {
      ...
      trimArr(arr).foreach { a =>
        a match {
          ...
          case JsonDate(date) =>
            dbl.add(date)
          case JArray(arr) =>
            dbl.add(parseArray(arr))
          case JObject(jo) =>
            dbl.add(parseObject(jo))
          case jv: JValue =>
            dbl.add(renderValue(jv))
        }
      }
      dbl
    }
    ...
    private def parseObject(obj: List[JField])(implicit
      ...
      trimObj(obj).foreach { jf =>
        jf.value match {
          case JsonObjectId(objectId) =>
            dbo.put(jf.name, objectId)
          case JsonRegex(regex) =>
            dbo.put(jf.name, regex)
          case JsonUUID(uuid) =>
            dbo.put(jf.name, uuid)
          case JsonDate(date) =>
            dbo.put(jf.name, date)
          case JArray(arr) =>
            dbo.put(jf.name, parseArray(arr))
          case JObject(jo) =>
            dbo.put(jf.name, parseObject(jo))
          case jv: JValue =>
            dbo.put(jf.name, renderValue(jv))
        }
      }
      dbo
    }

    private def renderValue(jv: JValue)(implicit formats: Formats): Object =
      jv match {
        case JBool(b) =>
        ...
        case JInt(n) =>
          renderInteger(n)
        case JDouble(n) =>
          new java.lang.Double(n)
        case JNull =>
          null
        case JNothing =>
          sys.error("can't render 'nothing'")
        case JString(null) =>
          "null"
        case JString(s) =>
          stringProcessor.vend(s)
        case _ =>
          ""
      }
    ...
    private def renderInteger(i: BigInt): Object = {
      ...
      } else if (i <= java.lang.Long.MAX_VALUE &&
