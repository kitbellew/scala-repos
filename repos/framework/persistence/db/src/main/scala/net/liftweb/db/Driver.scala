...
abstract class DriverType(val name: String) {
   ...
   */
  protected def performInsertWithGenKeys[T](
      conn: SuperConnection,
      query: String,
      setter: PreparedStatement => Unit,
      tableName: String,
...
object H2Driver extends DriverType("H2") {
  ...
  /**
    ...
    */
  override def pkDefinedByIndexColumn_? : Boolean =
    false //changed to false by nafg
...
object OracleDriver extends DriverType("Oracle") {
  ...
  override def primaryKeySetup(
    ...
    super.primaryKeySetup(tableName, columnName) ::: List(
      ...
      "CREATE OR REPLACE TRIGGER " + tableName + "_trigger BEFORE INSERT ON " +
        tableName + " " + "FOR EACH ROW " + "WHEN (new." + columnName +
        " is null) " + "BEGIN " + "SELECT " + tableName +
        "_sequence.nextval INTO :new." + columnName + " FROM DUAL; " + "END;"
    )
  }
