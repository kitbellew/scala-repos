...
trait DBLog {
  ...
  protected def logMeta[T](description: String)(f: => T): T = logMeta({
    ignore: T => description
  })(f)
  ...
  def statementEntries: List[DBStatementEntry] = executedStatements
  ...
  def metaEntries: List[DBMetaEntry] = executedStatements
