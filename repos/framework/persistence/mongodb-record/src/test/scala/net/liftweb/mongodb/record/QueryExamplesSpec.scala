...
object QueryExamplesSpec extends Specification with MongoTestKit {
  ...
  "Query examples" in {
    ...
    val fred = Person.createRecord.name("Flinstone, Fred")
      .birthDate(fredsBirthDate.getTime).childId(pebblesId).petId(dinoId).save()
    val wilma = Person.createRecord.name("Flinstone, Wilma")
      .birthDate(wilmasBirthDate.getTime).childId(pebblesId).petId(dinoId)
      .save()
    val barney = Person.createRecord.name("Rubble, Barney")
      .birthDate(barneysBirthDate.getTime).childId(bammbammId).save()
    val betty = Person.createRecord.name("Rubble, Betty")
      .birthDate(bettysBirthDate.getTime).childId(bammbammId).save()
    ...
    people.map(_.id.get).filterNot(
      List(wilma.id.get, barney.id.get, betty.id.get)
    ...
    people3.map(_.id.get) must_== List(
      betty.id.get,
      barney.id.get,
      wilma.id.get,
      fred.id.get)
    ...
    people4.map(_.id.get) must_== List(
      fred.id.get,
      wilma.id.get,
      barney.id.get,
      betty.id.get)
