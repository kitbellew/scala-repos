...
trait MongoTestKit extends Specification with BeforeAfterEach {
  ...
  def dbName =
    "lift_record_" + this.getClass.getName.replace("$", "")
      .replace("net.liftweb.mongodb.record.", "").replace(".", "_").toLowerCase
  ...
  def before = {
    // define the dbs
    dbs foreach { case (id, db) => MongoDB.defineDb(id, mongo, db) }
  }
  ...
  def isMongoRunning: Boolean =
    try {
      if (dbs.length < 1) false
        ...
        dbs foreach { case (id, _) => MongoDB.use(id)(db => { db.getName }) }
        true
      }
  ...
  def after = {
    if (!debug && isMongoRunning) {
      // drop the databases
      dbs foreach { case (id, _) => MongoDB.use(id) { db => db.dropDatabase } }
    }
