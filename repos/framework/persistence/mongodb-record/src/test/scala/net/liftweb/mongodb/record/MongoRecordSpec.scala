  ...
  "MongoRecord" should {
    ...
    "save and retrieve 'standard' type fields" in {
      ...
      S.initIfUninitted(session) {
        ...
        bftrFromDb must beLike { case Full(tr) =>
          tr mustEqual bftr
        }
      }
    }
    ...
    "handle null" in {
      ...
      ntrFromDb must beLike { case Full(n) =>
        // goes in as
    ...
    "handle Box using JsonBoxSerializer" in {
      ...
      btrFromDb must beLike { case Full(b) =>
    ...
    "use defaultValue when field is not present in the database" in {
      S.initIfUninitted(session) {
        ...
        recFromDb must beLike { case Full(r) =>
    ...
    "update dirty fields for a FieldTypeTestRecord" in {
      S.initIfUninitted(session) {
        ...
        fromDb2 must beLike { case Full(rec) =>
          rec must_== fttr2
          rec.dirty_? must_== false
        }
      }
    ...
    "update dirty fields for a PatternFieldTestRecord" in {
      ...
      fromDb must beLike { case Full(rec) =>
        rec must_== pftrd
        rec.dirty_? must_== false
      }
    }
