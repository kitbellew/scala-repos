  ...
  **/
  ...
  def ajaxEditable(
    ...
    def setAndSwap(
      ...
      (SHtml.ajaxCall(
        Str("ignore"),
        { ignore: String => SetHtml(show, showContents) })._2.cmd &
        swapJsCmd(show, hide))

    def displayMarkup: NodeSeq =
      displayContents ++ Text(" ") ++ <input value={
        S.?("edit")
      } type="button" onclick={
    ...
    def editMarkup: NodeSeq = {
      val formData: NodeSeq =
        editForm ++ <input type="submit" value={S.?("ok")} /> ++
          hidden(onSubmit) ++ <input type="button" onclick={
  ...
  private def optionToElem(option: SelectableOption[String]): Elem =
    option.attrs
      .foldLeft(<option value={option.value}>{option.label}</option>)(_ % _)
  ...
  def checkbox[T](
    ...
    fmapFunc {
      LFuncHolder((selectedChoiceValues: List[String]) => {
        val validSelectedIndicies = selectedChoiceValues.map(_.toInt)
  ...
  def checkbox_*(
    ...
    fmapFunc(func)(name =>
      (attrs
        .foldLeft(<input type="checkbox" name={name} value="true"/>)(_ % _) %
