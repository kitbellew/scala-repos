...
class LiftSession(
  ...
  /**
    ...
    */
  private var ajaxRequests = scala
    .collection
    .mutable
    ...
    value match {
      ...
      case node: scala.xml.Node =>
        currentSourceContext
          .doWith(node)(processSurroundAndInclude("Source", xform(ns)))
      ...
      case x =>
        currentSourceContext
          .doWith(x)(processSurroundAndInclude("Source", xform(ns)))
    }
  }
      ...
      try {
        ...
        snippetName
          .map { snippet =>
            ...
            S.doSnippet(snippet)(
              runWhitelist(snippet, cls, method, kids) {
                (
              ...
              in.!(
                xlate(p) match {
                  ...
                  case Failure(msg, _, _) =>
                    logger.error(
                      "Failed to deserialize JSON message " + p + ". Error " +
                        msg);
                    p
                })
              JsCmds.Noop
