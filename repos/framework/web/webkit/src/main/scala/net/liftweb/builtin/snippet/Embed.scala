...
object Embed extends DispatchSnippet {
  ...
  def render(kids: NodeSeq): NodeSeq = {
    for {
      ...
      what <- S.attr ~ ("what") ?~ (
        "FIX" + "ME The 'what' attribute not defined. In order to embed a template, the 'what' attribute must be specified"
      )
    ...
    case Full((templateName, template)) => {
      val bindings: Seq[CssSel] = kids.collect {
        case BindAtWithName(element, name) => s"#$name" #> element.child
      }
