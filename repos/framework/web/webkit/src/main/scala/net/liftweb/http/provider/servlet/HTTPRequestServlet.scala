...
class HTTPRequestServlet(
  ...
  /**
    ...
    */
  lazy val userAgent
      : Box[String] = headers find (_.name equalsIgnoreCase "user-agent") flatMap (_.values.headOption)
...
private class OfflineRequestSnapshot(
  ...
  def headers(name: String): List[String] =
    _headers.filter(_.name == name).map(_.name)
  ...
  def param(name: String): List[String] =
    _params.filter(_.name == name).map(_.name)
