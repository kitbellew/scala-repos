...
case class JsonCall(funcId: String) {
  ...
  def apply(command: String, target: String, params: JsExp) =
    JsCmds.Run(
      funcId + "({'command': " + command.encJs + ", 'target': " + target.encJs +
        ", 'params':" + params.toJsCmd + "});")
  ...
  def apply(command: JsExp, target: JsExp, params: JsExp) =
    JsCmds.Run(
      funcId + "({'command': " + command.toJsCmd + ", 'target': " +
        target.toJsCmd + ", 'params':" + params.toJsCmd + "});")
...
object JsExp {
  ...
  implicit def jValueToJsExp(jv: JValue): JsExp =
    new JsExp {
  ...
  implicit def numToJValue(in: JE.Num): JValue =
    in match { case JE.Num(n) => JDouble(n.doubleValue()) }
...
trait JsExp extends HtmlFixer with ToJsCmd {
  ...
  def appendToParent(parentName: String): JsCmd = {
    ...
    JsCmds.JsCrVar(ran, this) & JE.JsRaw(
      "if (" + ran + ".parentNode) " + ran + " = " + ran + ".cloneNode(true)")
      .cmd & JE.JsRaw(
      "if (" + ran + ".nodeType) {" + parentName + ".appendChild(" + ran +
        ");} else {" + parentName + ".appendChild(document.createTextNode(" +
        ran + "));}").cmd
  }
...
object JE {
  ...
  case class ValById(id: String) extends JsExp {
    def toJsCmd =
      "(function() {if (document.getElementById(" + id.encJs +
        ")) {return document.getElementById(" + id.encJs +
          ...
          JsRaw(
            name + ".appendChild(lift$.swappable(" + AnonFunc(
              JsCmds.JsCrVar("df", JsRaw("document.createDocumentFragment()")) &
                addToDocFrag("df", visible.toList) & JE.JsRaw("return df").cmd)
              .toJsCmd + "(), " + AnonFunc(
              JsCmds.JsCrVar("df", JsRaw("document.createDocumentFragment()")) &
                addToDocFrag("df", hidden.toList) & JE.JsRaw("return df").cmd)
              .toJsCmd + "()))").cmd
      }
    }
  }
  ...
  protected trait MostLjFuncs {
    ...
    def apply(obj: String, func: String): JsExp =
      new JsExp {
        def toJsCmd = "lift$." + funcName + "(" + obj + ", " + func.encJs + ")"
      }

    def apply(obj: JsExp, func: JsExp): JsExp =
      new JsExp {
        def toJsCmd =
  ...
  object LjAlt {
    def apply(obj: String, func: String, alt: String): JsExp =
      new JsExp {
        def toJsCmd =
    ...
    def apply(obj: JsExp, func: JsExp, alt: JsExp): JsExp =
      new JsExp {
        def toJsCmd =
          "lift$.alt(" + obj.toJsCmd + ", " + func.toJsCmd + ", " +
            alt.toJsCmd + ")"
      }
  }
  ...
  object LjMagicUpdate {
    def apply(
      ...
      new JsExp {
        def toJsCmd =
          "lift$.magicUpdate(" + obj + ", " + field.encJs + ", " +
            idField.encJs + ", " + toUpdate.toJsCmd + ")"
      }
    ...
    def apply(
      ...
      new JsExp {
        def toJsCmd =
          "lift$.magicUpdate(" + obj.toJsCmd + ", " + field.encJs + ", " +
            idField.encJs + ", " + toUpdate.toJsCmd + ")"
      }
  }
  ...
  object LjFold {
    def apply(what: JsExp, init1: JsExp, func: String): JsExp =
      new JsExp {
        def toJsCmd =
          "lift$.fold(" + what.toJsCmd + ", " + init1.toJsCmd + ", " +
            func.encJs + ")"
      }
  ...
  object LjSort extends MostLjFuncs {
    ...
    def apply(obj: String): JsExp =
      new JsExp {
        def toJsCmd = "lift$." + funcName + "(" + obj + ")"
      }

    def apply(obj: JsExp): JsExp =
      new JsExp {
  ...
  trait AnonFunc extends JsExp {
    ...
    def applied(params: JsExp*): JsExp =
      new JsExp {
        def toJsCmd =
          "(" + AnonFunc.this.toJsCmd + ")" + params.map(_.toJsCmd)
            .mkString("(", ",", ")")
      }
  ...
  object AnonFunc {
    def apply(in: JsCmd): AnonFunc =
      new JsExp with AnonFunc {
        def toJsCmd = "function() {" + in.toJsCmd + "}"
      }

    def apply(params: String, in: JsCmd): AnonFunc =
      new JsExp with AnonFunc {
...
trait HtmlFixer {
  ...
  def fixHtmlFunc(uid: String, content: NodeSeq)(f: String => String) =
    fixHtmlAndJs(uid, content) match {
      case (str, Nil) => f(str)
      case (str, cmds) =>
        "((function() {" + cmds.reduceLeft { _ & _ }.toJsCmd + " return " +
          f(str) + ";})())"
    }
  ...
  protected def fixHtmlAndJs(
      uid: String,
      content: NodeSeq): (String, List[JsCmd]) = {
    import Helpers._
...
""")
  ...
  case class SetValueAndFocus(id: String, value: String) extends JsCmd {
    def toJsCmd =
      "if (document.getElementById(" + id.encJs +
        ")) {document.getElementById(" + id.encJs + ").value = " + value.encJs +
        "; document.getElementById(" + id.encJs + ").focus();};"
  }
  ...
  object Function {
    def apply(name: String, params: List[String], body: JsCmd): JsCmd =
      new JsCmd {
        def toJsCmd = "function " + name + "(" + params.mkString(", ") + """) {
    """ + body.toJsCmd + """
    }
"""
      }
  ...
  case class SetValById(id: String, right: JsExp) extends JsCmd {
    def toJsCmd =
      "if (document.getElementById(" + id.encJs +
        ")) {document.getElementById(" + id.encJs + ").value = " +
        right.toJsCmd + ";};"
  }
  ...
  case class JsTry(what: JsCmd, alert: Boolean) extends JsCmd {
    def toJsCmd =
      "try { " + what.toJsCmd + " } catch (e) {" +
        (if (alert) "alert(e);" else "") + "}"
  }
  ...
  case object JsIf {
    ...
    def apply(condition: JsExp, bodyTrue: JsCmd, bodyFalse: JsCmd): JsCmd =
      JE.JsRaw(
        "if ( " + condition.toJsCmd + " ) { " + bodyTrue.toJsCmd +
          " } else { " + bodyFalse.toJsCmd + " }")
  ...
  case class JsFor(
      ...
      extends JsCmd {
    def toJsCmd =
      "for ( " + initialExp.toJsCmd + "; " + condition.toJsCmd + "; " +
  ...
  case class JsForIn(initialExp: JsExp, reference: String, body: JsCmd)
      extends JsCmd {
    def toJsCmd =
      "for ( " + initialExp.toJsCmd + " in " + reference + ") { " +
        body.toJsCmd + " }"
  }
