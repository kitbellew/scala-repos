...
trait AbstractScreen extends Factory with Loggable {
  ...
  def validate: List[FieldError] =
    screenFields.filter(_.shouldDisplay_?).filter(_.show_?)
  ...
  protected final case class OtherValueInitializerImpl[T](f: () => T)
  ...
  protected class FieldBuilder[T](
    ...
    def make: Field { type ValueType = T } = {
      ...
      val newShow: Box[BaseField => Boolean] =
  ...
  protected def builder[T](
    ...
    new FieldBuilder[T](
      ...
      stuff.toList.collect { case AFilter(v) => v.asInstanceOf[T => T] },
      stuff)
  }
  ...
  protected object FilterOrValidate {
    ...
    implicit def promoteValidate[T](
        v: T => List[FieldError]): FilterOrValidate[T] = AVal(v)

    implicit def promoteToFormParam(
        a: SHtml.ElemAttr): FilterOrValidate[Nothing] = FormParam(a)
  ...
  protected final case class FormParam(fp: SHtml.ElemAttr)
      extends FilterOrValidate[Nothing]

  protected final case class FormFieldId(id: String)
      extends FilterOrValidate[Nothing]

  protected final case class AFilter[T](f: T => T) extends FilterOrValidate[T]

  protected final case class AVal[T](v: T => List[FieldError])
      extends FilterOrValidate[T]

  protected final case class AFieldBinding(binding: FieldBinding)
      extends FilterOrValidate[Nothing]

  protected final case class Help(ns: NodeSeq) extends FilterOrValidate[Nothing]

  protected final case class FieldTransform(
      func: BaseField => NodeSeq => NodeSeq)
      extends FilterOrValidate[Nothing]
  ...
  protected def field[T](
    ...
    val newHelp: Box[NodeSeq] = (stuff.collect { case Help(ns) => ns })
      .headOption
  ...
  protected def field[T](
    ...
    new Field {
      ...
      override def set(v: T) =
        underlying.openOrThrowException("Legacy code")
  ...
  protected def radio(
    ...
    makeField[String, Seq[String]](
      ...
      field =>
        Full(
          SHtml.radio(field.otherValue, Full(field.is), field.set _, eAttr: _*)
            .toForm),
...
trait ScreenWizardRendered extends Loggable {
  ...
  protected def renderAll(
    ...
    val notices: List[(NoticeType.Value, NodeSeq, Box[String])] = S
      .getAllNotices
    ...
    def bindField(f: ScreenFieldInfo): NodeSeq => NodeSeq = {
      ...
      val theForm = theFormEarly.map { fe =>
        {
          ...
          val id = Helpers
            .findBox(f)(_.attribute("id").map(_.text).filter(_ == curId))
      ...
      def bindLabel(): CssBindFunc = {
        ...
        myNotices match {
          ...
          case _ =>
            val maxN =
              myNotices.map(_._1).sortWith { _.id > _.id }
  ...
  protected def redirectBack(): JsCmd = {
    if (ajaxForms_?) { AjaxOnDone.get }
    else { S.seeOther(Referer.get) }
  }
...
trait LiftScreen
  ...
  protected def mapLocalAction[T](func: () => JsCmd)(f: String => T): T = {
    ...
    ref.synchronized { ref.set(ref.get + (name -> func)) }
    f(name)
  }
    ...
    val finishButton = theScreen.finishButton %
      ("onclick" ->
        (if (ajaxForms_?) {
         ...
         } else { "document.getElementById(" + finishId.encJs + ").submit()" }))
