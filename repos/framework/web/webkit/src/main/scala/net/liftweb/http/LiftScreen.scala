...
trait AbstractScreen extends Factory with Loggable {
  ...
  protected class FieldBuilder[T](
    ...
    def make: Field {
      ...
      val newTransforms: List[BaseField => NodeSeq => NodeSeq] =
        stuff
          .collect({
            case FieldTransform(func) =>
              func
          })
          .toList
  ...
  protected def valMinLen(
    ...
    s =>
      s match {
        case str if (null ne str) && str.length >= len =>
          Nil
        case _ =>
          List(
            FieldError(
  ...
  protected def valMaxLen(
    ...
    s =>
      s match {
        case str if (null eq str) || str.length <= len =>
          Nil
        case _ =>
          List(
            FieldError(
  ...
  protected def valRegex(
    ...
    s =>
      s match {
        case str if (null ne str) && pat.matcher(str).matches =>
          Nil
        case _ =>
          List(
            FieldError(
  ...
  protected def renderAll(
      ...
      logger.trace(
        ...
        (
          for {
            ...
            if bindingInfo.bindingStyle == style
    ...
    def bindErrors: CssBindFunc =
      notices.filter(_._3.isEmpty) match {
        case Nil =>
          remove(_.globalErrors)
        case xs =>
          replaceChildren(_.globalErrors) #>
            xs.map {
              case (noticeType, msg, _) =>
                val metaData: MetaData = noticeTypeToAttr(theScreen)
                  .map(_(noticeType)) openOr Null
    ...
    val processed = S.session map
      (_.runTemplate("css-bound-screen", allTemplate)) openOr
      (allTemplate)
