...
object UserAgentCalculator extends Factory {
  ...
  def defaultIeCalcFunction(userAgent: Box[String]): Box[Double] =
    for {
      ...
      ieVersionString <- Box.legacyNullTest(
        ieMatch.group(2)) or Box.legacyNullTest(ieMatch.group(3))
...
object Req {
  ...
  def apply(
    ...
    val paramCalcInfo: AvoidGAL = new AvoidGAL(() => {
      ...
      if ((reqType.post_? ||
        ...
        val params = localParams ++ (request.params.sortWith { (s1, s2) =>
          s1.name < s2.name
  ...
  def parsePath(in: String): ParsePath = {
    ...
    val orgLst = p1.replaceAll("/$", "/index").split("/").toList.map(
      _.trim).filter(_.length > 0)
...
class Req(
    ...
    try {
      ...
      def charSet: String =
        contentType.flatMap(ct =>
          r.findFirstIn(ct).flatMap(r2.findFirstIn)).getOrElse("UTF-8")
  ...
  lazy val accepts: Box[String] = {
    request.headers.toList.filter(_.name equalsIgnoreCase "accept").flatMap(
      _.values) match {
      case Nil => Empty
