...
trait UserAgentCalculator {
  lazy val ieVersion: Box[Int] = UserAgentCalculator.ieCalcFunction.vend
    .apply(userAgent).map(_.toInt)
  ...
  lazy val safariVersion: Box[Int] = UserAgentCalculator.safariCalcFunction.vend
    .apply(userAgent).map(_.toInt)
  ...
  lazy val firefoxVersion: Box[Double] = UserAgentCalculator.firefoxCalcFunction
    .vend.apply(userAgent)

  lazy val isFirefox2: Boolean = firefoxVersion
    .map(v => v >= 2d && v < 3d) openOr false
  lazy val isFirefox3: Boolean = firefoxVersion
    .map(v => v >= 3d && v < 3.5d) openOr false
  lazy val isFirefox35: Boolean = firefoxVersion
    .map(v => v >= 3.5d && v < 3.6d) openOr false
  lazy val isFirefox36: Boolean = firefoxVersion
    .map(v => v >= 3.6d && v < 4d) openOr false
  ...
  lazy val chromeVersion: Box[Double] = UserAgentCalculator.chromeCalcFunction
    .vend.apply(userAgent)
...
class OnDiskFileParamHolder(
  ...
  protected override def finalize { tryo(localFile.delete) }
}
...
object Req {
  ...
  def apply(
      request: HTTPRequest,
      rewrite: List[LiftRules.RewritePF],
      nanoStart: Long): Req = this.apply(request, rewrite, Nil, Nil, nanoStart)
  ...
  def apply(
    ...
    val contextPath = LiftRules.calculateContextPath() openOr request
      .contextPath
    val turi =
    ...
    val localParams: Map[String, List[String]] = Map(
      rewritten.params.toList
        .map { case (name, value) => name -> List(value) }: _*)
    ...
    lazy val queryStringParam: (List[String], Map[String, List[String]]) = {
      val params: List[(String, String)] = for {
        ...
        nameVal <- queryString.split("&").toList.map(_.trim)
          .filter(_.length > 0)
      ...
      val nvp: Map[String, List[String]] = params
    ...
    val paramCalculator: () => ParamCalcInfo = () => { paramCalcInfo.thunk }

    val wholePath = rewritten.path.wholePath

    val stateless = NamedPF
      .applyBox(StatelessReqTest(wholePath, request), otherStatelessTest) or
      NamedPF.applyBox(wholePath, statelessTest)
  ...
  def parsePath(in: String): ParsePath = {
    ...
    val orgLst = p1.replaceAll("/$", "/index").split("/").toList.map(_.trim)
      .filter(_.length > 0)
...
final case class ContentType(
  ...
  def compare(that: ContentType): Int =
    ((that.q openOr 1d) compare (q openOr 1d)) match {
      case 0 =>
        def doDefault = { order compare that.order }
...
class Req(
  ...
  def withNewPath(newPath: ParsePath): Req = {
    ...
    new Req(
      ...
      /**
        ...
        */
      override lazy val weightedAccept: List[ContentType] = outer.weightedAccept
  ...
  def destroyServletSession() {
    for { httpReq <- Box !! request } httpReq.destroyServletSession()
  }
  ...
  lazy val forcedBodyAsJson: Box[JsonAST.JValue] = {
    try {
      ...
      def charSet: String =
        contentType.flatMap(ct => r.findFirstIn(ct).flatMap(r2.findFirstIn))
          .getOrElse("UTF-8")

      body.map(b =>
        JsonParser
          .parse(new InputStreamReader(new ByteArrayInputStream(b), charSet)))
    } catch {
  ...
  /**
    ...
    */
  lazy val hostName: String = containerRequest.map(_.serverName) openOr Req
    .localHostName
  ...
  lazy val accepts: Box[String] = {
    request.headers.toList.filter(_.name equalsIgnoreCase "accept")
