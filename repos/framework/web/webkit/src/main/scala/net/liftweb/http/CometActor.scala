...
/**
  ...
  *
  */
trait ListenerManager { self: SimpleActor[Any] =>
...
/**
  ...
  * @see ListenerManager
  */
trait CometListener extends BaseCometActor { self: BaseCometActor =>
...
trait BaseCometActor
  ...
  @volatile private var _lastRenderTime =
    if (partialUpdateStream_?) 0
    else Helpers.nextNum
  ...
  protected implicit def nodeSeqFuncToBoxNodeSeq(
      f: NodeSeq => NodeSeq): Box[NodeSeq] = Full(f(defaultHtml))
