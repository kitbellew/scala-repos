...
trait MathLibModule[M[+_]]
  ...
  trait MathLib extends ColumnarTableLib with InfixLib {
    ...
    object log10
        extends Op1DD("log10", n => doubleIsDefined(n) && n > 0.0, Math.log10)
    ...
    object log1p
        extends Op1DD("log1p", n => doubleIsDefined(n) && n > -1.0, Math.log1p)

    object sqrt
        extends Op1DD("sqrt", n => doubleIsDefined(n) && n >= 0.0, Math.sqrt)
