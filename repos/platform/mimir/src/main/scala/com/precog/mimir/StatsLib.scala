...
trait StatsLibModule[M[+_]]
  ...
  trait StatsLib extends ColumnarTableLib with ReductionLib {
    ...
    object Median extends Morphism1(EmptyNamespace, "median") {
      ...
      def apply(table: Table, ctx: MorphContext) = { //TODO write tests for the empty table case
        ...
        for {
          ...
          median <-
            if (count % 2 == 0) {
              val middleValues = sortedTable
                .takeRange((count.toLong / 2) - 1, 2)
    ...
    object Mode extends Morphism1(EmptyNamespace, "mode") {
      ...
      implicit def setMonoid[A] =
        new Monoid[Set[A]] { //TODO this is WAY WRONG - it needs to deal with slice boundaries properly!!
          ...
          def reduce(schema: CSchema, range: Range): Result = {
            schema.columns(JNumberT) flatMap {
              case col: LongColumn =>
                ...
                if (mapped.isEmpty) { Set.empty[BigDecimal] }
                else {
                  val foldedMapped: (
                      Option[BigDecimal],
                      BigDecimal,
                      Set[BigDecimal],
                      BigDecimal) = mapped.foldLeft(
    ...
    abstract class Smoothing(name: String)
        extends Morphism2(StatsNamespace, name) {
                      ...
                      (
                        ...
                        Map(
                          ColumnRef(CPath.Identity, CNum) -> ArrayNumColumn(
                            defined,
                            smoothed)))
                    } getOrElse { (FindSecond(x0), Map.empty) }
                }
    ...
    object Covariance extends Morphism2(StatsNamespace, "cov") {
        ...
        new Reducer[Result] {
          def reduce(schema: CSchema, range: Range): Result = {
            ...
            val result = cross map {
              case (c1: LongColumn, c2: LongColumn) =>
                val mapped = range filter (r =>
                  c1.isDefinedAt(r) && c2.isDefinedAt(r)) map { i =>
                  (c1(i), c2(i))
                }
                if (mapped.isEmpty) { None }
                else {
      ...
      def extract(res: Result): Table = {
        val res2 = res filter { case (count, _, _, _) => count != 0 }
    ...
    object LinearRegression extends Morphism2(StatsNamespace, "linReg") {
      ...
      def extract(res: Result): Table = {
        val res2 = res filter { case (count, _, _, _, _) => count != 0 }
    ...
    trait BaseRankScanner extends CScanner {
      ...
      def buildRankContext(
          m: Map[ColumnRef, Column],
          lastRow: Int,
          curr: Long,
          next: Long): RankContext = {
        val items = m.filter { case (k, v) => v.isDefinedAt(lastRow) }.map {
    ...
    class IndexedRankScanner extends BaseRankScanner {
      def buildRankArrayIndexed(
          defined: BitSet,
          r: Range,
          ctxt: RankContext): (Array[Long], Long, Int) = {
        var curr = ctxt.next
      ...
      def scan(
          ctxt: RankContext,
          _m: Map[ColumnRef, Column],
          range: Range): (RankContext, Map[ColumnRef, Column]) = {
    ...
    trait UniqueRankScanner extends BaseRankScanner {
      ...
      private def isDuplicate(
          ...
          row: Int,
          index: Int): Boolean = {
        if (index >= cols.length) { true }
        else {
          val dc = definedCols(index)
          val wasDefined = dc.get(lastRow)
          if (wasDefined != dc.get(row)) { false }
      ...
      /**
        ...
        * return false.
        */
      def isDuplicateFromContext(
          ctxt: RankContext,
          refs: Array[ColumnRef],
          cols: Array[Column],
          row: Int): Boolean = {
      ...
      def buildRankArrayUnique(
          defined: BitSet,
          duplicateRows: BitSet,
          r: Range,
    ...
    final class DummyScanner(length: Long) extends CScanner {
      ...
      def scan(
        ...
        val defined = cols.values.foldLeft(BitSetUtil.create()) {
          (bitset, col) =>
    ...
    abstract class BaseRank(name: String, retType: JType = JNumberT)
        extends Morphism1(StatsNamespace, name) {
