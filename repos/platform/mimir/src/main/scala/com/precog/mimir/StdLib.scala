...
trait TableLibModule[M[+_]] extends TableModule[M] with TransSpecModule {
  ...
  object TableLib {
    private val defaultMorphism1Opcode =
      new java.util.concurrent.atomic.AtomicInteger(0)
    private val defaultMorphism2Opcode =
      new java.util.concurrent.atomic.AtomicInteger(0)
    private val defaultUnaryOpcode =
      new java.util.concurrent.atomic.AtomicInteger(0)
    private val defaultBinaryOpcode =
      new java.util.concurrent.atomic.AtomicInteger(0)
    private val defaultReductionOpcode =
      new java.util.concurrent.atomic.AtomicInteger(0)
  }
  ...
  trait TableLib extends Library {
    ...
    object MorphismAlignment {
      ...
      case class Custom(
          alignment: IdentityPolicy,
          f: (Table, Table) => M[(Table, Morph1Apply)])
          extends MorphismAlignment
    }

    abstract class Morphism1(val namespace: Vector[String], val name: String)
    ...
    abstract class Morphism2(val namespace: Vector[String], val name: String)
    ...
    abstract class Op1(namespace: Vector[String], name: String)
        extends Morphism1(namespace, name)
        with Op1Like {
      ...
      def apply(table: Table, ctx: MorphContext) =
        sys.error("morphism application of an op1 is wrong")
    }
...
object StdLib {
  ...
  object StrFrom {
    ...
    class SN(
        c1: StrColumn,
        c2: NumColumn,
        defined: (String, BigDecimal) => Boolean,
  ...
  object DoubleFrom {
    ...
    class L(c: LongColumn, defined: Double => Boolean, f: Double => Double)
      ...
      override def isDefinedAt(row: Int) =
        super.isDefinedAt(row) && defined(c(row).toDouble) && doubleIsDefined(
          apply(row))
    ...
    class DD(
      ...
      override def isDefinedAt(row: Int) =
        super.isDefinedAt(row) && defined(c1(row), c2(row)) && doubleIsDefined(
          apply(row))
    ...
    class DL(
      ...
      override def isDefinedAt(row: Int) =
        super.isDefinedAt(row) && defined(c1(row), c2(row).toDouble) && doubleIsDefined(
          apply(row))
    ...
    class LD(
      ...
      override def isDefinedAt(row: Int) =
        super.isDefinedAt(row) && defined(c1(row).toDouble, c2(row)) && doubleIsDefined(
          apply(row))
  ...
  object BoolFrom {
    ...
    class LN(
        c1: LongColumn,
        c2: NumColumn,
        defined: (Long, BigDecimal) => Boolean,
    ...
    class NL(
        c1: NumColumn,
        c2: LongColumn,
        defined: (BigDecimal, Long) => Boolean,
