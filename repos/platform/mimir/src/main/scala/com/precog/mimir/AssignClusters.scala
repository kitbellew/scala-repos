          ...
          def scanner(modelSet: ModelSet): CScanner =
            new CScanner {
              ...
              def scan(
                ...
                def filteredRange(cols: Map[ColumnRef, Column]) =
                  range.filter(defined(cols).apply)

                val result: Set[Map[ColumnRef, Column]] = {

                  val modelsResult: Set[Map[ColumnRef, Column]] = modelSet
                    .models map {
                    case model =>
                      val includedModel = included(model)
                      val definedModel = defined(includedModel)

                      val clusterIds: Array[String] = model.clusters map {
                        _.name
                      }
                      val clusterCenters: Array[Array[Double]] = (model
                        .clusters).map {
                      ...
                      val centerPaths: Array[CPath] = model
                        .clusters collectFirst {
                        case (m: ModelCluster) =>
                          m.featureValues.keys.toArray.sorted
                      ...
                      def transposeResults(values: Array[Array[Double]]) = {
                        var k = 0
                        val acc = Array
                          .fill(centerPaths.length)(Array.empty[Double])
                ...
                val reduced: Map[ColumnRef, Column] = result.toSet
                  .suml(monoidCols)
