...
trait TimeLibModule[M[+_]] extends ColumnarTableLibModule[M] {
  trait TimeLib extends ColumnarTableLib {
      ...
      def scanner =
        new CScanner {
          ...
          def scan(
            ...
            def dateCol(idx: Int): Array[DateTime] =
              dateTimes map { arr =>
                if (arr.length > idx)
                  arr(idx)
                else
    ...
    trait TimeFraction extends Op1F1 {
      val tpe = UnaryOperationType(StrAndDateT, JNumberT)
      def f1(ctx: MorphContext): F1 =
        CF1P("builtin::time::timeFraction") {
          case (c: StrColumn) =>
            val dateCol = createDateCol(c)
    ...
    object DateHourMinuteSecond
        extends Op1F1(TimeNamespace, "dateHourMinuteSecond")
        with TimeTruncation {
      val fmt = ISODateTimeFormat.dateHourMinuteSecond()
    }

    object DateHourMinSecMilli
        extends Op1F1(TimeNamespace, "dateHourMinSecMilli")
        with TimeTruncation {
      val fmt = ISODateTimeFormat.dateHourMinuteSecondMillis()
      override val deprecation = Some("use dateHourMinuteSecondMillis instead")
    }

    object DateHourMinuteSecondMillis
        extends Op1F1(TimeNamespace, "dateHourMinuteSecondMillis")
        with TimeTruncation {
      val fmt = ISODateTimeFormat.dateHourMinuteSecondMillis()
    }
