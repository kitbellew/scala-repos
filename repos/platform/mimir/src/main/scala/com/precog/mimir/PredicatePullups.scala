...
trait PredicatePullupsModule[M[+_]] extends TransSpecableModule[M] {
  trait PredicatePullups extends TransSpecable {
    ...
    case class GroupEdit(
        group: BucketSpec,
        graphEdit: (DepGraph, DepGraph),
        specEdit: (dag.BucketSpec, dag.BucketSpec))
        ...
        case s @ Split(g @ Group(id, target, gchild), schild, _) =>
          ...
          extractFilter(gchild) match {
            case Some((booleans @ (_ :: _), newChildren @ (_ :: _))) => {
              val newChild = newChildren
                .reduceLeft(dag.IntersectBucketSpec(_, _))
              val boolean = booleans
                .reduceLeft(Join(And, IdentitySort, _, _)(s.loc))
