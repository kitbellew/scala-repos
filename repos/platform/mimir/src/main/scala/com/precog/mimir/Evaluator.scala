...
trait EvaluatorModule[M[+_]]
  ...
  abstract class EvaluatorLike[N[+_]](N0: Monad[N])(implicit
    ...
    private val evalLogger = LoggerFactory
      .getLogger("com.precog.mimir.Evaluator")
    ...
    private def MorphLogger(loc: instructions.Line): MorphLogger =
      new MorphLogger {
    ...
    def fullRewriteDAG(
        optimize: Boolean,
        ctx: EvaluationContext): DepGraph => DepGraph = {
      stagedRewriteDAG(optimize, ctx) andThen (orderCrosses _) andThen
        composeOptimizations(
          optimize,
    ...
    def eval(
      ...
      def resolveTopLevelGroup(
        ...
        spec match {
          case UnionBucketSpec(left, right) => for {
          ...
          case IntersectBucketSpec(left, right) => for {
              leftSpec <- resolveTopLevelGroup(left, splits)
              rightSpec <- resolveTopLevelGroup(right, splits)
      ...
      def prepareEval(graph: DepGraph, splits: Map[Identifier, Int => N[Table]])
        ...
        def cross(
          ...
          (prepareEval(left, splits) |@| prepareEval(right, splits))
            ...
            case (ptLeft, ptRight) =>
              ...
              val resultM = Table.cross(
                lTable.compact(valueSpec),
              ...
              transState liftM mn(resultM map {
                case (joinOrder, table) =>
                  val sort = joinOrder match {
                    ...
                    case CrossRight => ptRight.sort match {
                        case IdentityOrder(ids) =>
                          val lIds = ptLeft.sort match {
                            case IdentityOrder(lIds)
                                if graph.uniqueIdentities => lIds
                            case _                        => Vector.empty
                          }
                          IdentityOrder(
        ...
        def join(
          ...
          val (leftKeySpec, rightKeySpec) = joinKey match {
            ...
            case ValueJoin(id) =>
              val valueKeySpec = trans
                .DerefObjectStatic(Leaf(Source), CPathField("sort-" + id))
          ...
          def join0(
            ...
            val leftResult = pendingTableLeft.table
              .transform(liftToValues(pendingTableLeft.trans))
            val rightResult = pendingTableRight.table
              .transform(liftToValues(pendingTableRight.trans))
        ...
        def evalNotTransSpecable(
          ...
          graph match {
            case dag.Observe(data, samples) => for {
                ...
                result <- {
                  ...
                  val keySpec = trans.WrapObject(
                    trans.DerefObjectStatic(TransSpec1.Id, paths.Key),
                    paths.Key.name)
                  val valueSpec = trans.WrapObject(
                    trans.DerefObjectStatic(TransSpec1.Id, paths.Value),
                    paths.Value.name)

                  val keyTable = tableData.transform(keySpec)
                    .canonicalize(maxSliceSize)
                  val valueTable = tableSamples.transform(valueSpec)
                    .canonicalize(maxSliceSize)
            ...
            case s: SplitParam => sys.error("Inlining of SplitParam failed")
            ...
            case dag.AbsoluteLoad(parent, jtpe) => for {
                pendingTable <- prepareEval(parent, splits)
                ...
                trans2 = trans.Map1(
                  ...
                  f1)
                loaded = pendingTable.table.transform(trans2)
                  .load(ctx.apiKey, jtpe).fold(
                    {
                      ...
                      case fatal =>
                        report.error(
                          ...
                          report.die() >> Table.empty.point[N]
                    },
                    table => table.point[N]
                  )
            ...
            case dag.RelativeLoad(parent, jtpe) => for {
                pendingTable <- prepareEval(parent, splits)
            ...
            case dag.Morph1(mor, parent) => for {
                pendingTable <- prepareEval(parent, splits)
            ...
            case dag.Morph2(mor, left, right) =>
              val spec: (TransSpec2, TransSpec2) => TransSpec2 = {
                (srcLeft, srcRight) =>
                  trans.InnerArrayConcat(
                    trans.WrapArray(srcLeft),
                    trans.WrapArray(srcRight))
              }

              val joined
                  : StateT[N, EvaluatorState, (Morph1Apply, PendingTable)] = mor
                .alignment match {
                case MorphismAlignment.Cross(morph1) =>
                  ((transState liftM mn(morph1)) |@|
                    cross(graph, left, right, None)(spec)).tupled

                case MorphismAlignment.Match(morph1)
                    if areJoinable(left, right) =>
                  ((transState liftM mn(morph1)) |@|
                    join(graph, left, right, IdentitySort)(spec)).tupled
                ...
                case MorphismAlignment.Match(morph1) =>
                  ...
                  ((transState liftM mn(morph1)) |@|
                    cross(graph, left, right, Some(hint))(spec)).tupled
                ...
                case MorphismAlignment.Custom(idPolicy, f) =>
                  ...
                  pair flatMap {
                    case (ptLeft, ptRight) =>
                      val leftTable = ptLeft.table
                        .transform(liftToValues(ptLeft.trans))
                      val rightTable = ptRight.table
                        .transform(liftToValues(ptRight.trans))
              ...
              joined flatMap {
                case (morph1, PendingTable(joinedTable, _, _, sort)) =>
                  transState liftM
                    mn(morph1(joinedTable, MorphContext(ctx, graph))) map {
                      table =>
                        PendingTable(
                          table,
                          graph,
          ...
          returns an array (to be dereferenced later) containing the result of each reduction
            ...
            case m @ MegaReduce(reds, parent) =>
              ...
              val original = firstCoalesce.zipWithIndex map {
                case (red, idx) => (red, Some(makeJArray(idx) _))
              }
              ...
              for {
                ...
                result = mn(
                  pendingTable.table.transform(liftedTrans)
                    .transform(DerefObjectStatic(Leaf(Source), paths.Value))
                    .transform(spec).reduce(reduction.reducer(
                      MorphContext(ctx, graph)))(reduction.monoid))
            ...
            case r @ dag.Reduce(red, parent) => for {
                pendingTable <- prepareEval(parent, splits)
                liftedTrans = liftToValues(pendingTable.trans)
              ...
              if (parent.valueKeys contains id) { prepareEval(parent, splits) }
                ...
                for {
                  ...
                  result = {
                    ...
                    val wrappedValue = trans
                      .WrapObject(valueSpec, paths.Value.name)
                    ...
                    val spec = InnerObjectConcat(
                      InnerObjectConcat(
                        ObjectDelete(
                          Leaf(Source),
                          Set(CPathField("sort-" + id), paths.Value) ++
                            oldSortFields),
                        wrappedSort),
                      wrappedValue)
        ...
        val timingM = transState liftM
          report.timing(graph.loc, endTime - startTime)
      ...
      def fullEval(
          graph: DepGraph,
          splits: Map[Identifier, Int => N[Table]],
          parentSplits: List[Identifier]): StateT[N, EvaluatorState, Table] = {
          ...
          for {
            ...
            optBack = optPoint map { point =>
              for {
                ...
                toEval =
                  listStagingPoints(Queue(rewritten)) filter
                    referencesOnlySplit(parentSplits)
        ...
        val toEval = listStagingPoints(Queue(graph)) filter
          referencesOnlySplit(parentSplits)
      ...
      resultTable map {
        _ paged maxSliceSize compact
          DerefObjectStatic(Leaf(Source), paths.Value)
      }
    }

    private[this] def stagedOptimizations(
        graph: DepGraph,
        ctx: EvaluationContext,
        optimize: Boolean) =
      for {
    ...
    def inlineNodeValue(graph: DepGraph, from: DepGraph, result: RValue) = {
      val replacements = graph.foldDown(true) {
        case join @ Join(
              ...
              Const(CLong(index2))) => List((join, Const(result)(from.loc)))
      }
    ...
    private def referencesOnlySplit(parentSplits: List[Identifier])(
      ...
      val currentIsReferenced = parentSplits.headOption
        .map(referencedSplits.contains(_)).getOrElse(true)
    ...
    private def enumerateGraphs(forest: BucketSpec): Set[DepGraph] =
      forest match {
        case UnionBucketSpec(left, right) =>
          enumerateGraphs(left) ++ enumerateGraphs(right)
        case IntersectBucketSpec(left, right) =>
          enumerateGraphs(left) ++ enumerateGraphs(right)
    ...
    private def flip[A, B, C](f: (A, B) => C)(b: B, a: A): C =
      f(a, b) // is this in scalaz?
    ...
    private object IdentityOrder {
      def apply(node: DepGraph): IdentityOrder =
        IdentityOrder(Vector.range(0, node.identities.length))
      def empty: IdentityOrder = IdentityOrder(Vector.empty)
      def single: IdentityOrder = IdentityOrder(Vector(0))
    }

    private case class PendingTable(
        table: Table,
        graph: DepGraph,
        trans: TransSpec1,
        sort: TableOrder)
