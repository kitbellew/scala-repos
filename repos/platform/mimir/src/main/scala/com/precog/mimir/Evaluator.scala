...
trait EvaluatorModule[M[+_]]
  ...
  abstract class EvaluatorLike[N[+_]](N0: Monad[N])(implicit
      ...
      def prepareEval(graph: DepGraph, splits: Map[Identifier, Int => N[Table]])
        ...
        def cross(
          ...
          (prepareEval(left, splits) |@| prepareEval(
            right,
            splits)).tupled flatMap { case (ptLeft, ptRight) =>
                  ...
                  ptLeft.sort match {
                    case IdentityOrder(ids) =>
                      val rIds = ptRight.sort match {
                        case IdentityOrder(rIds) if graph.uniqueIdentities =>
                          rIds
                        case _ => Vector.empty
                      }
                  ...
                  ptRight.sort match {
                    case IdentityOrder(ids) =>
                      val lIds = ptLeft.sort match {
                        case IdentityOrder(lIds) if graph.uniqueIdentities =>
                          lIds
                        case _ => Vector.empty
                      }
        ...
        def evalNotTransSpecable(
          ...
          graph match {
            ...
            case m @ MegaReduce(reds, parent) =>
              ...
              def makeJArray(idx: Int)(tpe: JType): JType = JArrayFixedT(
                Map(idx -> tpe))
    ...
    private object IdentityOrder {
      def apply(node: DepGraph): IdentityOrder = IdentityOrder(
        Vector.range(0, node.identities.length))
