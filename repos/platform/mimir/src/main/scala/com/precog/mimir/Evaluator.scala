...
trait EvaluatorModule[M[+_]]
  ...
  abstract class EvaluatorLike[N[+_]](N0: Monad[N])(implicit
      ...
      def prepareEval(graph: DepGraph, splits: Map[Identifier, Int => N[Table]])
        ...
        def set0(pt: PendingTable, tg: (TransSpec1, DepGraph))
          ...
          for {
            _ <-
              monadState.modify { state =>
                state
                  .copy(assume = state.assume + (tg._2 -> (pt.table, pt.sort)))
              }
        ...
        def join(
          ...
          def join0(
            ...
            val leftResult = pendingTableLeft
              .table
              .transform(liftToValues(pendingTableLeft.trans))
            val rightResult = pendingTableRight
              .table
            ...
            val resultM =
              (isSorted(leftSort), isSorted(rightSort)) match {
                case (true, true) =>
                  M point
                    (KeyOrder ->
                      simpleJoin(leftResult, rightResult)(
                        leftKeySpec,
                        rightKeySpec,
                        joinSpec))
              ...
              val joined
                ...
                mor.alignment match {
                  case MorphismAlignment.Cross(morph1) =>
                    (
                      (transState liftM mn(morph1)) |@|
                        cross(graph, left, right, None)(spec)
                    ).tupled
                  ...
                  case MorphismAlignment.Match(morph1)
                    ...
                    (
                      (transState liftM mn(morph1)) |@|
                        join(graph, left, right, IdentitySort)(spec)
                    ).tupled
                  ...
                  case MorphismAlignment.Match(morph1) =>
                    ...
                    (
                      (transState liftM mn(morph1)) |@|
                        cross(graph, left, right, Some(hint))(spec)
                    ).tupled
                  ...
                  case MorphismAlignment.Custom(idPolicy, f) =>
                    ...
                    pair flatMap {
                      case (ptLeft, ptRight) =>
                        val leftTable = ptLeft
                          .table
                          .transform(liftToValues(ptLeft.trans))
                        val rightTable = ptRight
                          .table
    ...
    private def referencesOnlySplit(parentSplits: List[Identifier])(
      ...
      val currentIsReferenced = parentSplits
        .headOption
    ...
    private def simpleJoin(left: Table, right: Table)(
      ...
      val result =
        left.cogroup(leftKey, rightKey, right)(
          emptySpec,
          emptySpec,
