...
trait LinearRegressionLibModule[M[+_]]
  ...
  trait LinearRegressionLib
    ...
    override def _libMorphism2 =
      super._libMorphism2 ++ Set(MultiLinearRegression, LinearPrediction)
    ...
    object MultiLinearRegression
      ...
      def coefficientReducer: Reducer[CoeffAcc] =
        new Reducer[CoeffAcc] {
          def reduce(schema: CSchema, range: Range): CoeffAcc = {
            ...
            val weightedRes0 = res map {
              _ * count
            }

            val removed = cleaned map {
              _._2
        ...
        new Reducer[StdErrorAcc] {
          def reduce(schema: CSchema, range: Range): StdErrorAcc = {
            ...
            def removeColumns(
                ...
                colDim: Int,
                idx: Set[Int]): Matrix = {
              val columnIndices = (0 until colDim).toArray filterNot {
                idx.contains(_)
              }
            ...
            val matrixProduct = matrixX map { matrix =>
              matrix.transpose() times matrix
            } getOrElse {
              stdErrorMonoid.zero.product
            }

            val actualY0 = y0 map { arr =>
              (new Matrix(Array(arr))).transpose()
            }

            val retainedBeta = removeAt(acc.beta, acc.removed.toArray)
            val weightedBeta = retainedBeta map {
              _ / acc.count
            }
      ...
      def extract(
        ...
        val weightedBeta = coeffs.beta map {
          _ / coeffs.count
        }
        ...
        val arr = resultCoeffs.zip(resultErrors) map {
          case (beta, error) =>
