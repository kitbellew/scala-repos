...
trait DAG extends Instructions {
  ...
  def decorate(stream: Vector[Instruction]): Either[StackError, DepGraph] = {
    ...
    def loop(
      ...
      def processJoinInstr(instr: JoinInstr) = {
        val maybeOpSort = Some(instr) collect {
          case instructions.Map2Match(op) =>
            (op, IdentitySort)
          case instructions.Map2Cross(op) =>
            (op, Cross(None))
        }
        ...
        val eitherRootsAbom = Some(instr) collect {
          case instr @ instructions.Observe =>
            continue {
              case Right(samples) :: Right(data) :: tl =>
                Right(Right(Observe(data, samples)(loc)) :: tl)
              case Left(_) :: _ | _ :: Left(_) :: _ =>
                Left(OperationOnBucket(instr))
              case _ =>
                Left(StackUnderflow(instr))
          ...
          case instr @ instructions.Assert =>
            continue {
              case Right(child) :: Right(pred) :: tl =>
                Right(Right(Assert(pred, child)(loc)) :: tl)
              case Left(_) :: _ | _ :: Left(_) :: _ =>
                Left(OperationOnBucket(instr))
              case _ =>
                Left(StackUnderflow(instr))
          ...
          case instructions.SetDifference =>
            continue {
              case Right(right) :: Right(left) :: tl =>
                Right(Right(Diff(left, right)(loc)) :: tl)
              case Left(_) :: _ | _ :: Left(_) :: _ =>
                Left(OperationOnBucket(instr))
              case _ =>
                Left(StackUnderflow(instr))
      ...
      val tail: Option[Trampoline[Either[StackError, DepGraph]]] = stream
        ...
        case instr @ Map1(op) => {
          continue {
            case Right(hd) :: tl =>
              Right(Right(Operate(op, hd)(loc)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instr))
            case _ =>
              Left(StackUnderflow(instr))
        ...
        case instr: JoinInstr =>
          processJoinInstr(instr)

        case instr @ instructions.Morph1(BuiltInMorphism1(m1)) => {
          continue {
            case Right(hd) :: tl =>
              Right(Right(Morph1(m1, hd)(loc)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instr))
            case _ =>
              Left(StackUnderflow(instr))
        ...
        case instr @ instructions.Reduce(BuiltInReduction(red)) => {
          continue {
            case Right(hd) :: tl =>
              Right(Right(Reduce(red, hd)(loc)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instr))
            case _ =>
              Left(StackUnderflow(instr))
        ...
        case instructions.Distinct => {
          continue {
            case Right(hd) :: tl =>
              Right(Right(Distinct(hd)(loc)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instructions.Distinct))
            case _ =>
              Left(StackUnderflow(instructions.Distinct))
          }
        }
        ...
        case instructions.Extra => {
          continue {
            case Right(parent) :: tl =>
              Right(Left(Extra(parent)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instructions.Extra))
            case _ =>
              Left(StackUnderflow(instructions.Extra))
          }
        }
            ...
            splits match {
              case (open @ OpenSplit(loc, spec, oldTail, id)) :: splitsTail => {
                roots match {
                  ...
                  case _ =>
                    (Left(StackUnderflow(Merge)), splitsTail)
                }
              }
        ...
        case instr @ FilterMatch =>
          processFilter(instr, IdentitySort)
        case instr @ FilterCross =>
          processFilter(instr, Cross(None))

        case Dup => {
          roots match {
            case hd :: tl =>
              loop(loc, hd :: hd :: tl, splits, stream.tail)
            case _ =>
              Left(StackUnderflow(Dup)).point[Trampoline]
          }
        }
        ...
        case Drop => {
          roots match {
            case hd :: tl =>
              loop(loc, tl, splits, stream.tail)
            case _ =>
              Left(StackUnderflow(Drop)).point[Trampoline]
          }
        }

        case loc: Line =>
          loop(loc, roots, splits, stream.tail)

        case instr @ instructions.AbsoluteLoad => {
          continue {
            case Right(hd) :: tl =>
              Right(Right(AbsoluteLoad(hd)(loc)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instr))
            case _ =>
              Left(StackUnderflow(instr))
        ...
        case instr @ instructions.RelativeLoad => {
          continue {
            case Right(hd) :: tl =>
              Right(Right(RelativeLoad(hd)(loc)) :: tl)
            case Left(_) :: _ =>
              Left(OperationOnBucket(instr))
            case _ =>
              Left(StackUnderflow(instr))
    ...
    def findFirstRoot(line: Option[Line], stream: Vector[Instruction])
      ...
      val back = stream.headOption collect {
        case ln: Line =>
          findFirstRoot(Some(ln), stream.tail)
        ...
        case i: PushNum =>
          buildConstRoot(i)
        case PushTrue =>
          buildConstRoot(PushTrue)
        case PushFalse =>
          buildConstRoot(PushFalse)
        case PushNull =>
          buildConstRoot(PushNull)
        case PushObject =>
          buildConstRoot(PushObject)
        case PushArray =>
          buildConstRoot(PushArray)
  ...
  private def findGraphWithId(id: Int)(spec: dag.BucketSpec): Option[DepGraph] =
    spec match {
      ...
      case dag.IntersectBucketSpec(left, right) =>
        findGraphWithId(id)(left) orElse findGraphWithId(id)(right)
      case dag.Group(`id`, target, _) =>
        Some(target)
      case dag.Group(_, _, child) =>
        findGraphWithId(id)(child)
      case dag.UnfixedSolution(`id`, target) =>
        Some(target)
      case dag.UnfixedSolution(_, _) =>
        None
      case dag.Extra(_) =>
        None
    }

  private case class OpenSplit(
      loc: Line,
      spec: dag.BucketSpec,
      oldTail: List[Either[dag.BucketSpec, DepGraph]],
      id: Identifier)

  sealed trait Identities {
    def ++(other: Identities): Identities =
      (this, other) match {
        case (Identities.Undefined, _) =>
          Identities.Undefined
        case (_, Identities.Undefined) =>
          Identities.Undefined
        case (Identities.Specs(a), Identities.Specs(b)) =>
          Identities.Specs(a ++ b)
      }
  ...
  sealed trait DepGraph {
        ...
        def inner(graph: DepGraph): DepGraph =
          graph match {
            ...
            case graph @ dag.New(parent) =>
              dag.New(memoized(parent))(graph.loc)

            case graph @ dag.Morph1(m, parent) =>
              dag.Morph1(m, memoized(parent))(graph.loc)

            case graph @ dag.Morph2(m, left, right) =>
              dag.Morph2(m, memoized(left), memoized(right))(graph.loc)

            case graph @ dag.Distinct(parent) =>
              dag.Distinct(memoized(parent))(graph.loc)

            case graph @ dag.AbsoluteLoad(parent, jtpe) =>
              dag.AbsoluteLoad(memoized(parent), jtpe)(graph.loc)

            case graph @ dag.RelativeLoad(parent, jtpe) =>
              dag.RelativeLoad(memoized(parent), jtpe)(graph.loc)

            case graph @ dag.Operate(op, parent) =>
              dag.Operate(op, memoized(parent))(graph.loc)

            case graph @ dag.Reduce(red, parent) =>
              dag.Reduce(red, memoized(parent))(graph.loc)
            ...
            case graph @ dag.IUI(union, left, right) =>
              dag.IUI(union, memoized(left), memoized(right))(graph.loc)

            case graph @ dag.Diff(left, right) =>
              dag.Diff(memoized(left), memoized(right))(graph.loc)

            case graph @ dag.Join(op, joinSort, left, right) =>
              dag.Join(op, joinSort, memoized(left), memoized(right))(graph.loc)

            case graph @ dag.Filter(joinSort, target, boolean) =>
              dag.Filter(joinSort, memoized(target), memoized(boolean))(
                graph.loc)
            ...
            case dag.Memoize(parent, priority) =>
              dag.Memoize(memoized(parent), priority)
          }

        def memoizedSpec(spec: dag.BucketSpec): dag.BucketSpec =
          spec match { //TODO generalize?
            case dag.UnionBucketSpec(left, right) =>
    ...
    object ScopeUpdate {
      ...
      implicit def depGraphScopeUpdate =
        new ScopeUpdate[DepGraph] {
          override def update(node: DepGraph) = Some(node)
        }
      implicit def bucketSpecScopeUpdate =
        new ScopeUpdate[dag.BucketSpec] {
    ...
    object EditUpdate {
      ...
      implicit def depGraphEditUpdate =
        new EditUpdate[DepGraph] {
          def edit[T](
              inScope: Boolean,
              from: DepGraph,
          ...
          def edit[T](
              ...
              edit: (DepGraph, DepGraph),
              replace: dag.BucketSpec => T,
              retain: dag.BucketSpec => T): T = retain(from)

          def bimap[T](
              e: DepGraph)(fg: DepGraph => T, fs: dag.BucketSpec => T): T =
            fg(e)
        }
      implicit def bucketSpecEditUpdate =
        new EditUpdate[dag.BucketSpec] {
          def edit[T](
              inScope: Boolean,
              from: DepGraph,
              edit: (dag.BucketSpec, dag.BucketSpec),
              replace: DepGraph => T,
              retain: DepGraph => T): T = retain(from)
          def edit[T](
              inScope: Boolean,
              from: dag.BucketSpec,
              edit: (dag.BucketSpec, dag.BucketSpec),
          ...
          def bimap[T](e: dag.BucketSpec)(
              fg: DepGraph => T,
              fs: dag.BucketSpec => T): T = fs(e)
        }
    }
    ...
    def foldDown[Z](enterSplitChild: Boolean)(f0: PartialFunction[DepGraph, Z])(
      ...
      def foldThroughSpec(spec: dag.BucketSpec, acc: Z): Z =
        spec match {
          case dag.UnionBucketSpec(left, right) =>
          ...
          case dag.UnfixedSolution(_, solution) =>
            foldDown0(solution, acc |+| f(solution))
          case dag.Extra(expr) =>
            foldDown0(expr, acc |+| f(expr))
        }

      def foldDown0(node: DepGraph, acc: Z): Z =
        node match {
          case dag.SplitParam(_, _) =>
            acc

          case dag.SplitGroup(_, identities, _) =>
            acc

          case node @ dag.Const(_) =>
            acc
          ...
          case node @ dag.Reduce(_, parent) =>
            foldDown0(parent, acc |+| f(parent))

          case node @ dag.MegaReduce(_, parent) =>
            foldDown0(parent, acc |+| f(parent))
  ...
  object dag {
    ...
    object ConstString {
      def unapply(graph: Const): Option[String] =
        graph match {
    ...
    object ConstDecimal {
      def unapply(graph: Const): Option[BigDecimal] =
        graph match {
          case Const(CNum(d)) =>
            Some(d)
          case Const(CLong(d)) =>
            Some(d)
          case Const(CDouble(d)) =>
            Some(d)
          case _ =>
            None
        }
    ...
    //tic variable node
    case class SplitParam(id: Int, parentId: Identifier)(val loc: Line)
        extends DepGraph {
    ...
    case class Operate(op: UnaryOperation, parent: DepGraph)(val loc: Line)
        extends DepGraph {
    ...
    case class Split(spec: BucketSpec, child: DepGraph, id: Identifier)(
        val loc: Line)
        extends DepGraph
        with StagingPoint {
      lazy val identities = Identities.Specs(Vector(SynthIds(IdGen.nextInt())))
      ...
      lazy val containsSplitArg = {
        def loop(spec: BucketSpec): Boolean =
          spec match {
    ...
    case class Assert(pred: DepGraph, child: DepGraph)(val loc: Line)
        extends DepGraph {
    ...
    case class Observe(data: DepGraph, samples: DepGraph)(val loc: Line)
        extends DepGraph {
    ...
    case class Join(
      ...
      def uniqueIdentities =
        joinSort match {
          case Cross(_) | IdentitySort =>
            left.uniqueIdentities && right.uniqueIdentities
          case _ =>
            false
        }
    ...
    case class Filter(joinSort: JoinSort, target: DepGraph, boolean: DepGraph)(
      ...
      lazy val identities =
        joinSort match {
          case Cross(_) =>
            target.identities ++ boolean.identities
          case _ =>
            IdentityMatch(target, boolean).identities
        }

      def uniqueIdentities =
        joinSort match {
          case IdentitySort | Cross(_) =>
            target.uniqueIdentities && boolean.uniqueIdentities
          case _ =>
            false
        }
    ...
    case class UnionBucketSpec(left: BucketSpec, right: BucketSpec)
        extends BucketSpec
    case class IntersectBucketSpec(left: BucketSpec, right: BucketSpec)
        extends BucketSpec

    case class Group(id: Int, target: DepGraph, forest: BucketSpec)
        extends BucketSpec
    ...
    case class CoproductIds(left: IdentitySpec, right: IdentitySpec)
        extends IdentitySpec {
    ...
    case class IdentityMatch(left: DepGraph, right: DepGraph) {
      ...
      private def canonicalize(identities: Identities) =
        identities match {
      ...
      private def union(a: IdentitySpec, b: IdentitySpec): IdentitySpec =
        (a, b) match {
          case (CoproductIds(_, _), CoproductIds(_, _)) =>
            (a.possibilities ++ b.possibilities).reduceRight(CoproductIds(_, _))
          case (CoproductIds(_, _), _) =>
            a
          case (_, CoproductIds(_, _)) =>
            b
          case _ =>
            a
        }
      ...
      private def matches =
        (leftIdentities, rightIdentities) match {
          case (Identities.Specs(a), Identities.Specs(b)) =>
  ...
  case class MultiplePredicateStackValuesAtEnd(instr: Instruction)
      extends StackError
  ...
  case class DerefObjectAppliedToCompoundOperand(instr: Instruction)
      extends StackError
  case class DerefArrayAppliedToCompoundOperand(instr: Instruction)
      extends StackError
