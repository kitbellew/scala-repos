...
trait ReductionFinderSpecs[M[+_]]
  ...
  "mega reduce" should {
    ...
    "in a reduction of a singleton" in {
      ...
      val megaR =
        dag.MegaReduce(
          List((trans.Leaf(trans.Source), List(input.red))),
          Const(CString("alpha"))(line))
    ...
    "in joins where transpecs are eq, wrap object, operate, filter" in {
      ...
      val nonEqTrans =
        trans.Map1(
          trans
            .Equal(fooDerefTrans, trans.ConstLiteral(CNum(5), fooDerefTrans)),
      ...
      val filterTrans =
        trans.Filter(
          ...
          trans
            .Equal(bazDerefTrans, trans.ConstLiteral(CNum(12), bazDerefTrans)))
    ...
    "where three reductions use three different trans specs" in {
      ...
      val input =
        Join(Add, Cross(None), r1, Join(Add, Cross(None), r2, r3)(line))(line)
  ...
  "reduction finder" should {
    ...
    "in a join where only one side is a reduction" in {
      ...
      "left" in {
        ...
        val input =
          Join(Add, Cross(Some(CrossRight)), r, dag.Operate(Neg, load)(line))(
            line)
    ...
    "in a split that contains two reductions of the same dataset #2" in {
      ...
      lazy val parent =
        SplitGroup(1, Identities.Specs(Vector(LoadIds("/clicks"))), id)(line)
  ...
  def joinDeref(left: DepGraph, first: Int, second: Int, line: Line): DepGraph =
    Join(
      ...
      Join(DerefArray, Cross(Some(CrossLeft)), left, Const(CLong(first))(line))(
        line),
