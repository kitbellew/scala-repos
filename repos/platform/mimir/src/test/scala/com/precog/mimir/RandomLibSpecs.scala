...
trait RandomLibSpecs[M[+_]]
  ...
  "return observed set given 22 heterogeneous events with identities" in {
    ...
    val expected = Set(0.5782602543202314, 0.8725670189895508,
      0.9089304564449439, 0.5659348849912463, 0.5658210600064535,
      0.5260245794435036, 0.5616114285770709, 0.29996900265360527,
      0.1760490091508321, 0.6328197512142251, 0.8796105992305842,
      0.4216360283269056, 0.5529979602103697, 0.07161817845821428,
      0.9674312488914959, 0.4195344677504308, 0.644432398131965,
      0.7187289610972881, 0.18631410431791728, 0.6891140970302626,
  ...
  "return observed set joined with original" in {
    ...
    val randObj =
      dag.Join(WrapObject, Cross(None), Const(CString("rand"))(line), observe)(
        line)

    val numbersObj =
      dag.Join(WrapObject, Cross(None), Const(CString("data"))(line), numbers)(
        line)
    ...
    val expected = Set(0.39249760038657855, 0.8928583546656026,
      0.2502047859103521, 0.7861686432357328, 0.6239576284000388,
    ...
    result must haveAllElementsLike {
      case (ids, SObject(obj)) =>
        ...
        obj("rand") must beLike { case SDecimal(d) => expected must contain(d) }
    }
  }
}
