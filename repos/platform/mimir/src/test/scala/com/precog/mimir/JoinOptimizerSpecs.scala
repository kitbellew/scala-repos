...
trait JoinOptimizerSpecs[M[+_]]
  ...
  "join optimization" should {
    ...
    "eliminate cartesian with a new and a filter inside an array join" in {
      ...
      val medalsPcountry = Join(DerefObject, Cross(None), medalsP, country)(
        line)
      val medalsPPcountry = Join(DerefObject, Cross(None), medalsPP, country)(
        line)
    ...
    "eliminate cartesian in medal winners query" in {
      ...
      val medalsPrime = Join(
        JoinObject,
        IdentitySort,
        medals,
        wrapName(medals))(line)

      val athletesPrime = Join(
        JoinObject,
        IdentitySort,
        athletes,
