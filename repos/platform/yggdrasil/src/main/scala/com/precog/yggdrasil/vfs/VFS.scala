  ...
  object Resource {
    def asQuery(path: Path, version: Version)(implicit
      ...
      resource =>
        def notAQuery =
          notFound(
            "Requested resource at %s version %s cannot be interpreted as a Quirrel query.".format(
              path.path,
              version))
        EitherT {
          resource.fold(
    ...
    def asProjection(path: Path, version: Version)(implicit
      ...
      resource =>
        def notAProjection =
          notFound(
            "Requested resource at %s version %s cannot be interpreted as a Quirrel projection.".format(
              path.path,
              version))
        resource.fold(
