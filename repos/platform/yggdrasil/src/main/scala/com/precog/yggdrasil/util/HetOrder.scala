...
trait HetOrderLow {
  implicit def reverse[
      @spec(Boolean, Long, Double, AnyRef) A,
      @spec(Boolean, Long, Double, AnyRef) B](implicit ho: HetOrder[A, B]) =
    new HetOrder[B, A] {
      def compare(b: B, a: A) = {
  ...
  implicit def fromOrder[@spec(Boolean, Long, Double, AnyRef) A](implicit
      o: Order[A]) =
    new HetOrder[A, A] {
...
object ExtraOrders {
  implicit object BooleanOrder extends Order[Boolean] {
    ...
    def compare(a: Boolean, b: Boolean) =
      if (a == b)
        0
      else if (a)
        1
