...
object HomogeneousArrayColumn {
  def unapply[A](col: HomogeneousArrayColumn[A]): Option[CValueType[A]] = Some(
    col.tpe.elemType)
...
object BoolColumn {
  ...
  def Either(definedAt: BitSet, values: BitSet) = new BitsetColumn(definedAt)
    with BoolColumn {
...
object EmptyArrayColumn {
  def apply(definedAt: BitSet) = new BitsetColumn(definedAt)
