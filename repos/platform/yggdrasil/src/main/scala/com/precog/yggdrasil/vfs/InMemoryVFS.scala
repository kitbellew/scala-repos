...
trait InMemoryVFSModule[M[+_]] extends VFSModule[M, Slice] {
  ...
  class Projection(slices0: Vector[Slice]) extends ProjectionLike[M, Slice] {
    ...
    def getBlockAfter(id: Option[Key], columns: Option[Set[ColumnRef]] = None)(
        implicit M: Monad[M]): M[Option[BlockProjectionData[Key, Slice]]] =
      M point {
        id match {
          case Some(i) =>
  ...
  case class InMemoryProjectionResource(
    ...
    def append(data: NIHDB.Batch): IO[PrecogUnit] =
      IO { proj.append(Slice.fromJValues(data.values.toStream)) }
  ...
  case class InMemoryBlobResource(
    ...
    def asByteStream(mimeType: MimeType)(implicit M: Monad[M]) =
      OptionT { M.point { Some(data :: StreamT.empty[M, Array[Byte]]) } }
  }
  ...
  object VFS extends VFSCompanion {
    ...
    case class BinaryRecord(resource: InMemoryBlobResource, versionId: UUID)
        extends Record
    object BinaryRecord {
      def apply(
          data: (Array[Byte], MimeType),
          authorities: Authorities,
          uuid: UUID) =
        new BinaryRecord(
    ...
    case class JsonRecord(resource: InMemoryProjectionResource, versionId: UUID)
        extends Record
    object JsonRecord {
      def apply(data: Vector[JValue], authorities: Authorities, uuid: UUID) =
        new JsonRecord(
          new InMemoryProjectionResource(
            new Projection(Vector(Slice.fromJValues(data.toStream))),
            authorities),
          uuid)
    }
    ...
    private[InMemoryVFSModule] def newVersion: UUID =
      new UUID(0L, vid.getAndIncrement())
  }
  ...
  class InMemoryVFS(
    ...
    def writeAll(events: Seq[(Long, EventMessage)]): IO[PrecogUnit] = {
      def updated(
          acc: Map[(Path, Version), Record],
          appendTo: Option[Record],
          key: (Path, Version),
          writeAs: Authorities,
          values: Seq[JValue]) = {
        val path = key._1
