...
trait TransSpecModule extends FNModule {
  ...
  object trans {
    ...
    case class MapWith[+A <: SourceType](source: TransSpec[A], mapper: Mapper)
        extends TransSpec[A]

    case class Map1[+A <: SourceType](source: TransSpec[A], f: F1)
        extends TransSpec[A] //done

    case class DeepMap1[+A <: SourceType](source: TransSpec[A], f: F1)
        extends TransSpec[A] //done
    ...
    case class InnerObjectConcat[+A <: SourceType](objects: TransSpec[A]*)
        extends ObjectSpec[A] //done

    case class OuterObjectConcat[+A <: SourceType](objects: TransSpec[A]*)
        extends ObjectSpec[A] //done
    ...
    case class OuterArrayConcat[+A <: SourceType](arrays: TransSpec[A]*)
        extends ArraySpec[A] //done
    ...
    case class Typed[+A <: SourceType](source: TransSpec[A], tpe: JType)
        extends TransSpec[A] // done
    ...
    case class Equal[+A <: SourceType](left: TransSpec[A], right: TransSpec[A])
        extends TransSpec[A] //done
    ...
    object TransSpec {
      ...
      def mapSources[A <: SourceType, B <: SourceType](spec: TransSpec[A])(
        ...
        spec match {
          ...
          case DerefObjectStatic(source, field) =>
            DerefObjectStatic(mapSources(source)(f), field)
          case DerefObjectDynamic(left, right) =>
            DerefObjectDynamic(mapSources(left)(f), mapSources(right)(f))
          case DerefArrayStatic(source, element) =>
            DerefArrayStatic(mapSources(source)(f), element)
          case DerefArrayDynamic(left, right) =>
            DerefArrayDynamic(mapSources(left)(f), mapSources(right)(f))
          ...
          case trans.EqualLiteral(source, value, invert) =>
      ...
      def deepMap[A <: SourceType](spec: TransSpec[A])(
        ...
        spec match {
          ...
          case DerefObjectStatic(source, field) =>
            DerefObjectStatic(deepMap(source)(f), field)
          case DerefObjectDynamic(left, right) =>
            DerefObjectDynamic(deepMap(left)(f), deepMap(right)(f))
          case DerefArrayStatic(source, element) =>
            DerefArrayStatic(deepMap(source)(f), element)
          case DerefArrayDynamic(left, right) =>
            DerefArrayDynamic(deepMap(left)(f), deepMap(right)(f))
          ...
          case trans.EqualLiteral(source, value, invert) =>
            trans.EqualLiteral(deepMap(source)(f), value, invert)
    ...
    object TransSpec2 {
      ...
      def DerefArray1(source: Source2) =
    ...
    /**
      ...
      */
    case class GroupKeySpecSource(key: CPathField, spec: TransSpec1)
        extends GroupKeySpec

    case class GroupKeySpecAnd(left: GroupKeySpec, right: GroupKeySpec)
        extends GroupKeySpec
