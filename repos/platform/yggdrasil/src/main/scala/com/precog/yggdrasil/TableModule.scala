...
case class ExactSize(minSize: Long) extends TableSize {
  ...
  def +(other: TableSize) =
    other match {
      case ExactSize(n) =>
        ExactSize(minSize + n)
      case EstimateSize(n1, n2) =>
        EstimateSize(minSize + n1, minSize + n2)
      case UnknownSize =>
        UnknownSize
      case InfiniteSize =>
        InfiniteSize
...
case class EstimateSize(minSize: Long, maxSize: Long) extends TableSize {
  def +(other: TableSize) =
    other match {
      case ExactSize(n) =>
        EstimateSize(minSize + n, maxSize + n)
      case EstimateSize(n1, n2) =>
        EstimateSize(minSize + n1, maxSize + n2)
      case UnknownSize =>
        UnknownSize
      case InfiniteSize =>
        InfiniteSize
...
trait TableModule[M[+_]] extends TransSpecModule {
    ...
    def sorted: M[GroupingSource] =
      for {
        t <-
          table.sort(
            trans
              .DerefObjectStatic(trans.Leaf(trans.Source), CPathField("key")))
      } yield {
  ...
  final case class GroupingAlignment(
      groupKeyLeftTrans: trans.TransSpec1,
