...
trait Slice { source =>
  ...
  def wrap(wrapper: CPathNode): Slice = new Slice {
    ...
    val columns = wrapper match {
      case CPathArray =>
        source.columns map { case (ColumnRef(CPath(nodes @ _*), ctype), col) =>
  ...
  def take(sz: Int): Slice = if (sz >= source.size) source
  else {
    new Slice {
      val size = sz
      val columns = source.columns lazyMapValues { col =>
  ...
  def drop(sz: Int): Slice = if (sz <= 0) source
  else {
    new Slice {
...
object Slice {
  ...
  def fromJValues(values: Stream[JValue]): Slice = fromRValues(
    values.map(RValue.fromJValue))
