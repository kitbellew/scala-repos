...
trait Slice {
  ...
  lazy val valueColumns: Set[Column] =
    columns collect {
  ...
  def mapColumns(f: CF1): Slice =
    new Slice {
      ...
      val columns: Map[ColumnRef, Column] = {
        val resultColumns: Seq[(ColumnRef, Column)] =
          for {
      ...
      def flattenDeleteTree[A](
        ...
        (jType, cType, cPath) match {
          ...
          case (
            ...
            fields get name map
              (flattenDeleteTree(_, cType, CPath(cPath: _*))) getOrElse
              (retain)
  ...
  def isType(jtpe: JType): Slice =
    new Slice {
      ...
      val pathsAndTypes: Seq[(CPath, CType)] = source.columns.toSeq map {
        case (ColumnRef(selector, ctype), _) =>
          (selector, ctype)
      }
      ...
      val definedBits = (
        source.columns
      ).values.map(_.definedAt(0, size)).reduceOption(_ | _) getOrElse
        new BitSet
    ...
    new Slice {
      ...
      val columns = source
        ...
        .collect {
          ...
          case c @ (ColumnRef(CPath(CPathIndex(i), xs @ _*), ctype), col) =>
            c
        }
    }
    ...
    new Slice {
      lazy val retained: ArrayIntList = {
        ...
        (lastDefined, firstDefined) match {
          case (Some((prev, i)), Some(j)) =>
            findStraddlingDistinct(prev, i, j)
          case (_, Some(j)) =>
            acc.add(j);
  ...
  def order: spire.math.Order[Int] =
    if (columns.size == 1) {
      ...
      val grouped =
        columns.foldLeft(Left(Map.empty): GroupedCols) {
          ...
          case (Right(acc), (ColumnRef(path, _), col)) =>
            Right(
              acc + (path -> (acc.getOrElse(path, Set.empty[Column]) + col)))
        }
    ...
    new Slice {
      ...
      val columns = source.columns lazyMapValues {
        case col: BoolColumn =>
          ...
          val values =
            BitSetUtil.filteredRange(0, source.size) { row =>
              defined(row) && col(row)
            }
      ...
      val optSchema = {
          ...
          selector.nodes match {
            case CPathField(name) :: tail => {
              target match {
                ...
                case node =>
                  SchemaNode
                    .Union(Set(node, insert(SchemaNode.Obj(Map()), ref, col)))
              }
            }
            ...
            case CPathIndex(idx) :: tail => {
              target match {
                ...
                case node =>
                  SchemaNode
                    .Union(Set(node, insert(SchemaNode.Arr(Map()), ref, col)))
              }
            }
            ...
            case Nil => {
              ...
              target match {
                case SchemaNode.Union(nodes) =>
                  SchemaNode.Union(nodes + node)
                case oldNode =>
                  SchemaNode.Union(Set(oldNode, node))
              }
            }
          }
        }
        ...
        def normalize(schema: SchemaNode): Option[SchemaNode] =
          schema match {
            ...
            case lf: SchemaNode.Leaf =>
              Some(lf)
          }

        val schema =
          columns.foldLeft(SchemaNode.Union(Set()): SchemaNode) {
            case (acc, (ref, col)) =>
              insert(acc, ref, col)
          }
  ...
  def toJValue(row: Int) = {
    columns.foldLeft[JValue](JUndefined) {
      case (jv, (ColumnRef(selector, _), col)) if col.isDefinedAt(row) =>
        CPathUtils
          .cPathToJPaths(selector, col.cValue(row))
          .foldLeft(jv) {
            case (jv, (path, value)) =>
              jv.unsafeInsert(path, value.toJValue)
          }
  ...
  override def toString = (0 until size)
    .map(toString(_).getOrElse(""))
    .mkString("\n")
}
...
object Slice {
    ...
    rv.flattenWithPath
      .foldLeft(into) {
        ...
        case (acc, (cpath, cvalue)) =>
          ...
          val updatedColumn: ArrayColumn[_] =
            cvalue match {
              case CBoolean(b) =>
                acc
  ...
  def rowComparatorFor(s1: Slice, s2: Slice)(
    ...
    val traversal = CPathTraversal(paths)
    val lCols = s1.columns groupBy
      (_._1.selector) map {
        case (path, m) =>
          path -> m.values.toSet
      }
    val rCols = s2.columns groupBy
      (_._1.selector) map {
        case (path, m) =>
          path -> m.values.toSet
      }
            ...
            v match {
              ...
              case JNum(d) =>
                ctype match {
                  ...
                  case _ =>
                    sys.error("non-numeric type reached")
                }
              ...
              case _ =>
                sys.error("non-flattened value reached")
            }
