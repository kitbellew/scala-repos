...
sealed trait SValue {
      ...
      this match {
        case SObject(obj) =>
          (selector.nodes: @unchecked) match {
            case JPathField(name) :: Nil =>
              obj.get(name)
            case JPathField(name) :: xs =>
              obj.get(name).flatMap(_ \ JPath(xs))
          }
  ...
  def set(selector: JPath, value: SValue): Option[SValue] =
    this match {
      ...
      case SNull if (selector == JPath.Identity) =>
        Some(value)
  ...
  lazy val toJValue: JValue =
    this match {
      ...
      case SArray(arr) =>
        JArray(arr.map(_.toJValue)(collection.breakOut): _*)
      case SString(s) =>
        JString(s)
      case STrue =>
        JBool(true)
  ...
  lazy val toRValue: RValue =
    this match {
      ...
      case SArray(arr) =>
        RArray(arr.map(_.toRValue)(collection.breakOut): _*)
      case SString(s) =>
        CString(s)
      case STrue =>
        CBoolean(true)
...
trait SValueInstances {
  case class paired(sv1: SValue, sv2: SValue) {
    ...
    def fold[A](default: => A)(
        ...
        nul: => A) = {
      (sv1, sv2) match {
        case (SObject(o1), SObject(o2)) =>
          obj(o1)(o2)
        case (SArray(a1), SArray(a2)) =>
          arr(a1)(a2)
        case (SString(s1), SString(s2)) =>
          str(s1)(s2)
        case (SBoolean(b1), SBoolean(b2)) =>
          bool(b1)(b2)
  ...
  implicit val StructureOrdering: scala.math.Ordering[(JPath, CType)] =
    implicitly[Order[(JPath, CType)]].toScalaOrdering
}

object SValue extends SValueInstances {
  @inline
  def fromCValue(cv: CValue): SValue =
    cv match {
      case CString(s) =>
        SString(s)
      case CBoolean(b) =>
...
object SType {
  @inline
  def fromCType(ct: CType): SType =
    ct match {
      case CString =>
        SString
      case CBoolean =>
        SBoolean
      case (_: CNumericType[_]) =>
        SDecimal
      case CDate =>
        sys.error("todo")
      case CPeriod =>
...
case object SBoolean extends SType with (Boolean => SValue) {
  def apply(v: Boolean) =
    if (v)
      ...
      SFalse
  def unapply(v: SValue): Option[Boolean] =
    v match {
      case STrue =>
        Some(true)
      case SFalse =>
