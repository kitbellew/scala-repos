...
trait ProjectionModule[M[+_], Block] {
  ...
  trait ProjectionCompanionLike[M0[+_]] { self =>
    ...
    def liftM[T[_[+_], +_]](implicit T: Hoist[T], M0: Monad[M0]) =
      new ProjectionCompanionLike[({ type λ[+α] = T[M0, α] })#λ] {
...
trait ProjectionLike[M[+_], Block] {
  ...
  def getBlockStream(columns: Option[Set[ColumnRef]])(implicit
      M: Monad[M]): StreamT[M, Block] = {
    StreamT.unfoldM[M, Block, Option[Key]](None) { key =>
      getBlockAfter(key, columns) map {
        _ map { case BlockProjectionData(_, maxKey, block) =>
          (block, Some(maxKey))
        }
      }
    }
