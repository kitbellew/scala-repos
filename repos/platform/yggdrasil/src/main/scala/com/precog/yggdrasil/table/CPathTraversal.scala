...
sealed trait CPathTraversal { self =>
  ...
  def rowOrder(
      ...
      optRight: Option[Map[CPath, Set[Column]]] = None): spire.math.Order[Int] = {
    val right = optRight getOrElse left

    def plan0(
        t: CPathTraversal,
        paths: List[(List[CPathNode], List[CPathNode])],
        idx: Int): CPathComparator =
      t match {
        case Done =>
...
object CPathTraversal {
  ...
  case class Select(path: CPathNode, next: CPathTraversal)
      extends CPathTraversal
  case class Loop(start: Int, end: Option[Int], tail: CPathTraversal)
      extends CPathTraversal

  def apply(p: CPath): CPathTraversal =
    p.nodes.foldRight(Done: CPathTraversal)(Select(_, _))
