    ...
    EitherT {
      IO {
        if (currentFile.exists) {
          for {
            jv <- JParser.parseFromFile(currentFile).leftMap(
              ioError).disjunction
            version <- jv match {
              case JString(`unsetSentinel`) =>
                \/.left(
                  NotFound(
                    "No current data for the path %s exists; it has been archived.".format(
                      dir)))
              case other =>
    ...
    IO {
      ...
      val completedVersions: Validation[Error, Set[UUID]] =
        if (completedFile.exists) {
          for {
            jvs <- JParser.parseManyFromFile(completedFile).leftMap(
              Error.thrown)
            versions <- jvs.toList.traverse[
              ({ type λ[α] = Validation[Error, α] })#λ,
