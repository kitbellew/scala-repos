...
trait SecureVFSModule[M[+_], Block] extends VFSModule[M, Block] {
  ...
  class SecureVFS(
      ...
      resource =>
        ...
        val permissions: Set[Permission] = resource.authorities.accountIds map {
          accountId =>
            ...
            readMode match {
              case Read =>
                ReadPermission(path, writtenBy)
              case Execute =>
                ExecutePermission(path, writtenBy)
              case ReadMetadata =>
                ReducePermission(path, writtenBy)
            }
        }
    ...
    def executeAndCache(
      ...
      import EvaluationError._
      logger
        .debug("Executing query for %s and caching to %s".format(path, cacheAt))
      for {
        executor <-
    ...
    def persistingStream(
      ...
      StreamT.unfoldM((0, stream)) {
        case (pseudoOffset, s) =>
          s.uncons flatMap {
            case Some((x, xs)) =>
              val ingestRecords = VFS
                .toJsonElements(blockf(x))
                .zipWithIndex map {
                case (v, i) =>
                  IngestRecord(EventId(pseudoOffset, i), v)
              }
