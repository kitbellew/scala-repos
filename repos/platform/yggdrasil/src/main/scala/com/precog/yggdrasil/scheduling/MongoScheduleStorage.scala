...
object MongoScheduleStorage {
  def apply(config: Configuration)(implicit
      executor: ExecutionContext): (MongoScheduleStorage, Stoppable) = {
...
class MongoScheduleStorage private[MongoScheduleStorage] (
    mongo: Mongo,
    database: Database,
    settings: MongoScheduleStorageSettings)(implicit executor: ExecutionContext)
  ...
  private def insertTask(task: ScheduledTask \/ JObject, collection: String) =
    database(
      insert(task.valueOr { st => st.serialize.asInstanceOf[JObject] }).into(
        collection))
  ...
  def reportRun(report: ScheduledRunReport) =
    database(
      insert(report.serialize.asInstanceOf[JObject]).into(
  ...
  def statusFor(id: UUID, limit: Option[Int]) = {
    database(
      ...
      taskOpt =>
        database(
          selectAll.from(settings.reports).where(
            ".id" === id.toString
