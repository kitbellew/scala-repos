...
case class FilesystemIngestFailureLog(
    failureLog: Map[EventMessage, FilesystemIngestFailureLog.LogRecord],
    restoreFrom: YggCheckpoint,
    persistDir: File)
  ...
  def apply(
    ...
    def readAll(
      ...
      if (line == null) into
        ...
        val logRecord =
          ((Thrown(_: Throwable)) <-: JParser.parseFromString(line)).flatMap(
            _.validated[LogRecord]).valueOr(err => sys.error(err.message))
    ...
    implicit val decomposer: Decomposer[LogRecord] = new Decomposer[LogRecord] {
      def decompose(rec: LogRecord) =
        JObject(
          ...
          "messageType" -> rec.message.fold(
            _ => "ingest",
            _ => "archive",
            _ => "storeFile").serialize,
...
/**
  ...
  */
abstract class KafkaShardIngestActor(
    shardId: String,
  ...
  def receive = {
    ...
    case complete @ BatchComplete(requestor, checkpoint) =>
      ...
      logger.debug(
        "BatchComplete insert. Head = %s, completed = %s".format(
          ingestCache.headOption.map(_._1),
          checkpoint))
      ...
      if (totalConsecutiveFailures < maxConsecutiveFailures) {
        ...
        for (messages <- ingestCache.get(checkpoint)) {
          val batchHandler = context.actorOf(
            Props(new BatchHandler(self, requestor, checkpoint, ingestTimeout)))
        ...
        logger.error(
          "Skipping ingest batch due to excessive consecutive failures at Kafka offsets: " + ingestCache.keys.map(
            _.offset).mkString("[", ", ", "]"))
        logger.error(
    ...
    case GetMessages(requestor) =>
      try {
        logger.trace(
          "Responding to GetMessages from %s starting from checkpoint %s. Running batches = %d/%d".format(
            requestor,
            lastCheckpoint,
            runningBatches.get,
            maxCacheSize))
        if (runningBatches.get < maxCacheSize) {
          ...
          logger.warn(
            "Concurrent ingest window full (%d). Cannot start new ingest batch".format(
              runningBatches.get))
          requestor ! IngestData(Nil)
        }
      } catch {
  ...
  private def readRemote(fromCheckpoint: YggCheckpoint): Future[
    ...
    def buildBatch(
      ...
      input match {
        ...
        case (offset, ArchiveMessage(_, _, _, eventId, _)) :: tail
            if batch.nonEmpty =>
          logger.debug(
            "Batch stopping on receipt of ArchiveMessage at offset/id %d/%d".format(
              offset,
              eventId.uid))
          (batch, checkpoint)
    ...
    val read = M.point {
      ...
      val rawMessages = msTime({ t =>
        logger.debug(
          "Kafka fetch from %s:%d in %d ms".format(
            topic,
      ...
      val eventMessages: List[
        ...
        msTime({ t =>
          logger.debug(
            "Raw kafka deserialization of %d events in %d ms".format(
              rawMessages.size,
              t))
        }) {
      ...
      val batched: Validation[
        ...
        eventMessages.sequence[
          ...
          val apiKeys: List[(APIKey, Path)] = msTime({ t =>
            logger.debug(
              "Collected api keys from %d messages in %d ms".format(
                messageSet.size,
                t))
          }) {
            messageSet collect {
              ...
              messageSet.flatMap {
                ...
                case (offset, -\/((apiKey, path, genMessage))) =>
                  authorityCache.get((apiKey, path)) map { authorities =>
                    ...
                    logger.warn(
                      "Discarding event at offset %d with apiKey %s for path %s because we could not determine the account".format(
                        offset,
                        apiKey,
                        path))
                    None
