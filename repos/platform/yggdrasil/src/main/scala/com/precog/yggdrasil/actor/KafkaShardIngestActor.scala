...
object FilesystemIngestFailureLog {
  ...
  def apply(
      ...
      if (line == null) into
        ...
        val logRecord =
          ((Thrown(_: Throwable)) <-: JParser.parseFromString(line))
            .flatMap(_.validated[LogRecord])
            .valueOr(err => sys.error(err.message))
    ...
    if (logFiles.isEmpty)
      ...
      val reader = new BufferedReader(new FileReader(logFiles.maxBy(
    ...
    implicit val decomposer: Decomposer[LogRecord] = new Decomposer[LogRecord] {
      def decompose(rec: LogRecord) =
        JObject(
          ...
          "messageType" ->
            rec.message.fold(_ => "ingest", _ => "archive", _ => "storeFile")
              .serialize,
    ...
    case GetMessages(requestor) =>
      try {
        ...
        if (runningBatches.get < maxCacheSize) {
          ...
          logger.warn(
            "Concurrent ingest window full (%d). Cannot start new ingest batch"
              .format(runningBatches.get))
          requestor ! IngestData(Nil)
        }
      } catch {
        case t: Throwable =>
  ...
  private def readRemote(fromCheckpoint: YggCheckpoint): Future[
    ...
    def buildBatch(
      ...
      input match {
        ...
        case (offset, ArchiveMessage(_, _, _, eventId, _)) :: tail
            if batch.nonEmpty =>
          logger.debug(
            "Batch stopping on receipt of ArchiveMessage at offset/id %d/%d"
    ...
    val read = M.point {
      ...
      val eventMessages: List[
        ...
        msTime({ t =>
          logger.debug("Raw kafka deserialization of %d events in %d ms".format(
            rawMessages.size,
            t))
        }) {
