...
trait ColumnarTableModuleSpec[M[+_]]
  ...
  "a table dataset" should {
    ...
    "handle special cases of renderJson" >> {
      ...
      "undefined row" >> { testRenderJson(JObject(Nil) :: JNum(42) :: Nil) }
      ...
      "split a cross that would exceed maxSliceSize boundaries" in {
        val sample: List[JValue] = List(
          ...
          JObject(
            JField(
              "key",
              JArray(JNum(-3918416808128018609L) :: JNum(-1L) :: Nil)) ::
              JField("value", JNum(-1.0)) :: Nil),
          JObject(
            JField(
              "key",
              JArray(JNum(-3918416898128018609L) :: JNum(-2L) :: Nil)) ::
              JField("value", JNum(-1.0)) :: Nil),
          JObject(
            JField(
              "key",
              JArray(JNum(-3918426808128018609L) :: JNum(-3L) :: Nil)) ::
              JField("value", JNum(-1.0)) :: Nil)
        )
