...
trait TransformSpec[M[+_]]
  ...
  def testMap1ArrayObject = {
    val data: Stream[JValue] = Stream(
      ...
      JObject(
        JField("value", JArray(JNum(30) :: Nil)) :: JField(
          "key",
          JArray(JNum(1) :: Nil)) :: Nil),
      JObject(
        JField("value", JNum(20)) :: JField(
          "key",
          JArray(JNum(1) :: Nil)) :: Nil)
    )
  ...
  def testDeepMap1CoerceToDouble = {
    ...
    val expected = Stream(
      ...
      JNum(34.5),
      JNum(31.9),
      JObject(JField("baz", JNum(31)) :: Nil),
      JNum(20))
      ...
      val results = toJson(table.transform {
        Filter(
          ...
          Equal(
            Leaf(Source),
            Leaf(Source)
          ) //Map1 now makes undefined all columns not a the root-identity level
        )
      })
  ...
  def checkMap2Eq = {
    implicit val gen = sample(_ =>
      Seq(JPath("value1") -> CDouble, JPath("value2") -> CLong))
    check { (sample: SampleData) =>
      val table = fromSample(sample)
      val results = toJson(table.transform {
        Map2(
  ...
  def checkMap2Add = {
    implicit val gen = sample(_ =>
      Seq(JPath("value1") -> CLong, JPath("value2") -> CLong))
    check { (sample: SampleData) =>
      val table = fromSample(sample)
      val results = toJson(table.transform {
        Map2(
  ...
  def checkEqualSelfArray = {
    ...
    val data2: Stream[JValue] = Stream(JObject(List(
      JField("value", JArray(List(JNum(9), JNum(10), JNum(11)))),
      JField("key", JArray(List())))))
      ...
      genBase map { sd =>
        SampleData(sd.data.zipWithIndex map {
          case (jv, i) if i % 2 == 0 =>
            if (hasVal1Val2(jv)) {
              jv.set(
                ...
                jv(JPath(JPathField("value"), JPathField("value2"))))
            } else { jv }
          ...
          case (jv, i) if i % 5 == 3 =>
            if (hasVal1Val2(jv)) {
              jv.set(
                JPath(JPathField("value"), JPathField("value2")),
                JUndefined)
            } else { jv }
  ...
  def testInnerObjectConcatEmptyObject = {
    ...
    val expected: Stream[JValue] = Stream(
      ...
      JObject(
        JField("ook", JNum(99)) :: JField("ick", JNum(100)) :: JField(
          "ack",
          JNum(102)) :: Nil),
      JObject(Nil),
  ...
  def checkArrayConcat = {
    implicit val gen = sample(_ =>
      Seq(JPath("[0]") -> CLong, JPath("[1]") -> CLong))
  ...
  def testIsTypeUnion = {
    ...
    val jtpe = JObjectFixedT(Map(
      "value" -> JObjectFixedT(
        Map("foo" -> JUnionT(JNumberT, JTextT), "bar" -> JObjectUnfixedT)),
      "key" -> JArrayUnfixedT))
    val results = toJson(table.transform { IsType(Leaf(Source), jtpe) })
  ...
  def testIsType(sample: SampleData) = {
    ...
    val schemas = schemas0 map {
      _ map { case (jpath, ctype) => (CPath(jpath), ctype) }
    }
  ...
  def testTypedAtSliceBoundary = {
    ...
    val sample = SampleData(
      data.toStream,
      Some((3, List((JPath(".n"), CEmptyObject)))))
  ...
  def testTypedObject = {
    ...
    val results = toJson(table.transform {
      Typed(
        Leaf(Source),
        JObjectFixedT(Map(
          "value" -> JObjectFixedT(Map("foo" -> JNumberT)),
          "key" -> JArrayUnfixedT)))
    })
  ...
  def testTypedObject2 = {
    val data: Stream[JValue] = Stream(JObject(List(
      JField(
        "value",
    ...
    val results = toJson(table.transform {
      Typed(
        Leaf(Source),
        JObjectFixedT(Map(
          "value" -> JObjectFixedT(Map("bar" -> JNumberT)),
          "key" -> JArrayUnfixedT)))
    })

    val expected = Stream(JObject(List(
      JField("value", JObject(List(JField("bar", JNum(77))))),
      JField("key", JArray(List(JNum(1)))))))
    results.copoint must_== expected
  }
  ...
  def testTypedArray = {
    ...
    val results = toJson(table.transform {
      Typed(
        Leaf(Source),
        JObjectFixedT(Map(
          "value" -> JArrayFixedT(Map(0 -> JNumberT, 1 -> JBooleanT)),
          "key" -> JArrayUnfixedT)))
    })
  ...
  def testTypedArray3 = {
    ...
    val jtpe = JObjectFixedT(Map(
      "value" -> JArrayFixedT(
        Map(0 -> JArrayFixedT(Map()), 1 -> JArrayFixedT(Map()), 2 -> JNullT)),
      "key" -> JArrayUnfixedT))
  ...
  def testTypedArray4 = {
    val data: Stream[JValue] = Stream(
      JObject(List(
        JField(
          "value",
          JArray(List(JNum(2.4), JNum(12), JBool(true), JArray(List())))),
        JField("key", JArray(List(JNum(1)))))),
      JObject(List(
        JField("value", JArray(List(JNum(3.5), JNull, JBool(false)))),
        JField("key", JArray(List(JNum(2))))))
    )
    val sample = SampleData(data)
    val table = fromSample(sample)
  ...
  def testTypedNumber2 = {
    val data: Stream[JValue] = Stream(
      JObject(List(
        JField("value", JNum(23)),
        JField("key", JArray(List(JNum(1), JNum(3)))))),
      JObject(List(
        JField("value", JNum(12.5)),
        JField("key", JArray(List(JNum(2), JNum(4))))))
    )
    val sample = SampleData(data)
    val table = fromSample(sample)
  ...
  def testDerefObjectDynamic = {
    val data = JObject(
      JField("foo", JNum(1)) :: JField("ref", JString("foo")) :: Nil) #::
      JObject(
        JField("bar", JNum(2)) :: JField("ref", JString("bar")) :: Nil) #::
      JObject(
        JField("baz", JNum(3)) :: JField(
          "ref",
          JString("baz")) :: Nil) #:: Stream.empty[JValue]
  ...
  def checkConst = {
    ...
    check { (sample: SampleData) =>
      val table = fromSample(sample)
      val results = toJson(table.transform(ConstLiteral(
        CString("foo"),
        DerefObjectStatic(
          DerefObjectStatic(Leaf(Source), CPathField("value")),
          CPathField("field")))))
