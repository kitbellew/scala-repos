...
trait CogroupSpec[M[+_]]
  ...
  implicit val cogroupData = Arbitrary(for {
    depth <- choose(1, 2)
    cschema <- Gen.oneOf(arraySchema(depth, 2), objectSchema(depth, 2))
    (idCount, data) <- genEventColumns(cschema)
  } yield {
    val (lschema, rschema) = Bifunctor[Tuple2]
      .umap(cschema.splitAt(cschema.size / 2)) { _.map(_._1).toSet }
    val (l, r) = data map {
      case (ids, values) =>
  ...
  def testCogroup(l: SampleData, r: SampleData) = {
    ...
    val result: Table = ltable
      .cogroup(SourceKey.Single, SourceKey.Single, rtable)(
        Leaf(Source),
        Leaf(Source),
        OuterObjectConcat(
  ...
  def testAnotherSimpleCogroup = {
    ...
    val rtable = fromSample(
      SampleData(Stream(recr(0), recr(1), recr(5), recr(6), recr(7))))
  ...
  def testAnotherSimpleCogroupSwitched = {
    ...
    val ltable = fromSample(
      SampleData(Stream(recl(0), recl(1), recl(5), recl(6), recl(7))))
  ...
  def testPartialUndefinedCogroup = {
    ...
    val ltable = fromSample(
      SampleData(Stream(parseUnsafe("""{ "id" : "foo", "val" : 4 }"""))))
