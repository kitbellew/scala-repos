...
trait CompactSpec[M[+_]]
  ...
  def tableStats(table: Table): List[(Int, Int)] =
    table match {
  ...
  def undefineTable(fullTable: Table): Table =
    fullTable match {
      case cTable: ColumnarTable =>
        val slices =
  ...
  def undefineColumn(fullTable: Table, path: CPath): Table =
    fullTable match {
      case cTable: ColumnarTable =>
        val slices =
          cTable.slices.toStream.copoint // fuzzing must be done strictly otherwise sadness will ensue
