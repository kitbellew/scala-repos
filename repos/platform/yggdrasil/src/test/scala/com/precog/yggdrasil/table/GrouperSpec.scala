...
trait GrouperSpec[M[+_]]
  ...
  def testHistogramByValue(set: Stream[Int]) = {
    ...
    val result =
      Table.merge(spec) { (key: RValue, map: GroupId => M[Table]) =>
        for {
          gs1 <- map(groupId)
          gs1Json <- gs1.toJson
        } yield {
  ...
  def testCtrPartialJoinAnd(
    ...
    val result =
      Table.merge(intersection) { (key, map) =>
        for {
          gs1 <- map(groupId1)
          gs2 <- map(groupId2)
          gs1Json <- gs1.toJson
            ...
            Stream(
              JObject(
                JField(
                  "key",
                  JArray(
                    key.toJValue(tic_aj) :: key.toJValue(tic_bj) :: Nil)) ::
                  JField("value", JNum(gs1Json.size + gs2Json.size)) :: Nil)))
        }
      }
