...
trait Solver extends parser.AST with typer.Binder {
  ...
  def solve(tree: Expr, sigma: Sigma)(
    ...
    lazy val Rules: Set[PartialFunction[Expr, Set[Expr]]] = Set(
      {
        case Add(loc, left, right) if left equalsIgnoreLoc right =>
          Set(Mul(loc, NumLit(loc, "2"), left))
      },
      {
        case Sub(loc, left, right) if left equalsIgnoreLoc right =>
          Set(NumLit(loc, "0"))
      },
      {
      ...
      },
      {
        case Add(loc, Mul(loc2, x, y), z) if y equalsIgnoreLoc z =>
          Set(Mul(loc2, Add(loc, x, NumLit(loc, "1")), y))
      },
      {
      ...
      },
      {
        case Sub(loc, Mul(loc2, x, y), z) if y equalsIgnoreLoc z =>
          Set(Mul(loc2, Sub(loc, x, NumLit(loc, "1")), y))
      },
      {
      ...
      },
      {
        case Add(loc, Div(loc2, w, x), Div(loc3, y, z)) =>
          Set(Div(loc2, Add(loc, Mul(loc2, w, z), Mul(loc3, y, x)), Mul(loc2, x, z)))
      },
      {
      ...
      },
      {
        case Sub(loc, Div(loc2, w, x), Div(loc3, y, z)) =>
          Set(Div(loc2, Sub(loc, Mul(loc2, w, z), Mul(loc3, y, x)), Mul(loc2, x, z)))
      },
      {
    ...
    def solveBinary(tree: Expr, left: Expr, right: Expr)(
        ...
        invertRight: Expr => Expr => Expr): Expr => Option[Expr] = {
