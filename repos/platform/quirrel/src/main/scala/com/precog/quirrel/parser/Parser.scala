...
trait Parser extends RegexParsers with Filters with AST {
  ...
  private[quirrel] lazy val numLiteralRegex =
    """[0-9]+(\.[0-9]+)?([eE][0-9]+)?""".r
  ...
  private def buildDeepRelate(
      ...
      e: Expr): Expr = {
    val builders = relations zip
      (relations drop 1) map {
        case (e1, e2) => { e3: Expr =>
          Relate(loc, e1, e2, e3)
        }
      }
  ...
  private[quirrel] def canonicalizeRelPath(str: String): String =
    str substring 2
