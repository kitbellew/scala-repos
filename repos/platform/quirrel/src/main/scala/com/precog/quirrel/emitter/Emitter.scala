...
trait Emitter
  ...
  case class EmitterError(expr: Option[Expr], message: String)
      extends Exception(message)

  private def nullProvenanceError[A](): A =
    throw EmitterError(None, "Expression has null provenance")
  ...
  private case class MarkDispatch(let: ast.Let, actuals: Vector[Expr])
      extends MarkType
  ...
  private implicit val EmitterStateMonoid: Monoid[EmitterState] =
    new Monoid[EmitterState] {
      val zero = StateT.stateT[Id, Emission, Unit](())

      def append(v1: EmitterState, v2: => EmitterState): EmitterState = v1 >> v2
    }

  private def reduce[A](xs: Iterable[A])(implicit m: Monoid[A]) =
    xs.foldLeft(mzero[A])(_ |+| _)

  def emit(expr: Expr): Vector[Instruction] = {
    lazy val trace = expr.trace

    def insertInstrAtMulti(is: Seq[Instruction], _idx: Int): EmitterState =
    ...
    def insertInstrAt(i: Instruction, idx: Int): EmitterState =
      insertInstrAtMulti(i :: Nil, idx)

    def nextId(f: Int => EmitterState): EmitterState =
      StateT.apply[Id, Emission, Unit] { e =>
    ...
    def emitAndMark(markType: MarkType)(f: => EmitterState): EmitterState =
      StateT.apply[Id, Emission, Unit] { e =>
        f(e) match {
          case (e, _) =>
    ...
    def labelTicVar(solve: ast.Solve, name: TicId)(
        state: => EmitterState): EmitterState = {
      StateT.apply[Id, Emission, Unit] { e =>
        (e.copy(ticVars = e.ticVars + ((solve, name) -> state)), ())
      }
    }

    def labelFormal(id: Identifier, let: ast.Let)(
        state: => EmitterState): EmitterState = {
      StateT.apply[Id, Emission, Unit] { e =>
    ...
    def emitDup(markType: MarkType): EmitterState =
      StateT.apply[Id, Emission, Unit] { e =>
        ...
        for (const <- expr.constrainingExpr if !(const equalsIgnoreLoc expr))
          yield {
            if (expr.children exists { _.constrainingExpr == Some(const) }) None
              ...
              Some(
                emitExpr(const, dispatches) >> emitInstr(Dup) >>
                  emitInstr(Map2Match(Eq)) >> emitInstr(FilterMatch))
            }
          }
    ...
    def emitBucketSpec(
      ...
      spec match {
        case buckets.UnionBucketSpec(left, right) =>
          emitBucketSpec(solve, left, contextualDispatches, dispatches) >>
            emitBucketSpec(solve, right, contextualDispatches, dispatches) >>
            emitInstr(MergeBuckets(false))

        case buckets.IntersectBucketSpec(left, right) =>
          emitBucketSpec(solve, left, contextualDispatches, dispatches) >>
            emitBucketSpec(solve, right, contextualDispatches, dispatches) >>
            emitInstr(MergeBuckets(true))
        ...
        case buckets.Group(origin, target, forest, dtrace) => {
          nextId { id =>
            ...
            emitBucketSpec(solve, forest, contextualDispatches, dispatches) >>
              ...
              (origin map { labelGroup(_, id) } getOrElse
                mzero[EmitterState]) >> emitInstr(Group(id))
          }
        }
        ...
        .foldLeft((provs.head.possibilities, Vector.empty[EmitterState])) {
          case ((provAcc, instrAcc), prov) => {
            val joinInstr = StateT.apply[Id, Emission, Unit] { e =>
              ...
              val intersection = resolvedProv.possibilities
    ...
    def emitExpr(
        expr: Expr,
        dispatches: Set[ast.Dispatch]): StateT[Id, Emission, Unit] = {
        ...
        (expr match {
          ...
          case expr @ ast.Solve(loc, _, body) =>
            ...
            emitBucketSpec(expr, spec, contextualDispatches, dispatches) >>
              emitInstr(Split) >> emitExpr(body, dispatches) >> emitInstr(Merge)

          case ast.Import(_, _, child) => emitExpr(child, dispatches)

          case ast.Assert(_, pred, child) =>
            emitExpr(pred, dispatches) >> emitExpr(child, dispatches) >>
              emitInstr(Assert)

          case ast.Observe(_, data, samples) =>
            emitExpr(data, dispatches) >> emitExpr(samples, dispatches) >>
              emitInstr(Observe)
          ...
          case ast.ObjectDef(_, props) => {
            def fieldToObjInstr(t: (String, Expr)) =
              emitInstr(PushString(t._1)) >> emitExpr(t._2, dispatches) >>
                emitInstr(Map2Cross(WrapObject))
            ...
            val groups = provToField.foldLeft(Vector.empty[EmitterState]) {
              case (stateAcc, (provenance, fields)) =>
                ...
                val joinInstr = StateT.apply[Id, Emission, Unit] { e =>
                  ...
                  emitInstr(
                    if (resolved == ValueProvenance) Map2Cross(JoinObject)
          ...
          case ast.ArrayDef(_, values) => {
              ...
              .foldLeft((Vector.empty[EmitterState], Vector.empty[Int])) {
                case ((allStates, allIndices), (provenance, elements)) => {
                  ...
                  val joinInstr = StateT.apply[Id, Emission, Unit] { e =>
                    val resolved = e.subResolve(provenance)
                    emitInstr(
                      if (resolved == ValueProvenance) Map2Cross(JoinArray)
            ...
            val (_, swaps) = indices.zipWithIndex
                ...
                case ((remap, state), (j, i)) if resolve(remap)(i) != j => {
                  ...
                  val state2 = {
                    if (j == 0) { //required for correctness
                      emitInstr(PushNum(i2.toString)) >>
                        emitInstr(Map2Cross(ArraySwap))
                    } else if (i2 == 0) { //not required for correctness, but nice simplification
                      emitInstr(PushNum(j.toString)) >>
                        emitInstr(Map2Cross(ArraySwap))
                    } else {
                      val swps = (i2 :: j :: i2 :: Nil)
                      swps map { idx =>
                        emitInstr(PushNum(idx.toString)) >>
                          emitInstr(Map2Cross(ArraySwap))
                      } reduce { _ >> _ }
                    }
                  }
          ...
          case d @ ast.Dispatch(_, name, actuals) => {
            d.binding match {
              ...
              case ExpandGlobBinding =>
                emitExpr(actuals.head, dispatches) >>
                  emitInstr(Morph1(BuiltInMorphism1(expandGlob)))

              case Morphism1Binding(m) =>
                emitExpr(actuals.head, dispatches) >>
                  emitInstr(Morph1(BuiltInMorphism1(m)))

              case Morphism2Binding(m) =>
                emitExpr(actuals(0), dispatches) >>
                  emitExpr(actuals(1), dispatches) >>
                  emitInstr(Morph2(BuiltInMorphism2(m)))

              case ReductionBinding(f) =>
                emitExpr(actuals.head, dispatches) >>
                  emitInstr(Reduce(BuiltInReduction(f)))
          ...
          case ast.Cond(_, pred, left, right) =>
            ...
            emitFilter(left, pred, dispatches) >> emitFilterState(
              emitExpr(right, dispatches),
              right.provenance,
          ...
          case ast.Union(_, left, right) =>
            emitExpr(left, dispatches) >> emitExpr(right, dispatches) >>
              emitInstr(IUnion)

          case ast.Intersect(_, left, right) =>
            emitExpr(left, dispatches) >> emitExpr(right, dispatches) >>
              emitInstr(IIntersect)

          case ast.Difference(_, left, right) =>
            emitExpr(left, dispatches) >> emitExpr(right, dispatches) >>
              emitInstr(SetDifference)
  ...
  private def collapseLines(
      bytecode: Vector[Instruction]): Vector[Instruction] = {
