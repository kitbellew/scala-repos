...
trait Emitter
  ...
  private implicit val EmitterStateMonoid: Monoid[EmitterState] =
    new Monoid[EmitterState] {
      ...
      def append(v1: EmitterState, v2: => EmitterState): EmitterState =
        v1 >> v2
    }
  ...
  def emit(expr: Expr): Vector[Instruction] = {
    ...
    def emitDup(markType: MarkType): EmitterState =
      StateT.apply[Id, Emission, Unit] { e =>
        ...
        val pushDown =
          if (offset > 0) (1 to (offset + 1)).reverse map Swap
          else Vector()
        ...
        (insertInstrAtMulti((pullUp :+ Dup) ++ pushDown ++ saveSwaps, insertIdx) >> insertInstrAtMulti(
          restoreSwaps,
        ...
        (expr match {
          ...
          case expr @ ast.Solve(loc, _, body) =>
            ...
            emitBucketSpec(expr, spec, contextualDispatches, dispatches) >> emitInstr(
              Split) >> emitExpr(body, dispatches) >> emitInstr(Merge)
