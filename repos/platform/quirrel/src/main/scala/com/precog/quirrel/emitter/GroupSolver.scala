...
trait GroupSolver
    ...
    val results =
      forest map {
        case (sigma, where, dtrace) => {
          ...
          val fullyResolvedRightProv = resolveUnifications(
            where.right.relations)(resolvedRightProv.makeCanonical)
  ...
  private def pred(
    ...
    case t @ TicVar(_, `tv`) =>
      !free && t.binding == SolveBinding(b) || free &&
        t.binding == FreeBinding(b)
  }
