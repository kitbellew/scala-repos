...
trait GroupSolver
  ...
  override def inferBuckets(tree: Expr): Set[Error] = {
    def loopSpec(dispatches: Set[Dispatch])(spec: BucketSpec): Set[Error] =
      spec match {
        ...
        case UnfixedSolution(_, solution, _) =>
          loop(dispatches)(solution)

        case FixedSolution(_, solution, expr, _) =>
          loop(dispatches)(solution) ++ loop(dispatches)(expr)

        case Extra(expr, _) =>
          loop(dispatches)(expr)
      }

    def loop(dispatches: Set[Dispatch])(tree: Expr): Set[Error] =
      tree match {
        case Let(_, _, _, _, right) =>
          loop(dispatches)(right)
          ...
          val sigma: Sigma = dispatches flatMap { dispatch =>
            val (subs, letM) =
              dispatch.binding match {
                case LetBinding(let) =>
                  (let.substitutions, Some(let))
                case _ =>
                  (Map[Dispatch, Map[String, Expr]](), None)
              }
          ...
          val finalErrors =
            specM match {
              case Some(forest) => {
                ...
                rem map UnableToSolveTicVariable map {
                  Error(expr, _)
                }
              }

              case None =>
                expr.vars map UnableToSolveTicVariable map {
                  Error(expr, _)
                }
            }
        ...
        case NaryOp(_, values) =>
          (values map loop(dispatches)).fold(Set[Error]()) {
            _ ++ _
          }
      }
  ...
  private def solveForest(
    ...
    val results = forest map {
      case (sigma, where, dtrace) => {
        ...
        val resolvedLeftProv =
          orderedSigma.foldLeft(leftProv) {
            case (prov, formal @ (id, let)) =>
              substituteParam(id, let, prov, sigma(formal).provenance)
          }

        val resolvedRightProv =
          orderedSigma.foldLeft(rightProv) {
            case (prov, formal @ (id, let)) =>
              substituteParam(id, let, prov, sigma(formal).provenance)
          }

        val fullyResolvedLeftProv =
          resolveUnifications(where.left.relations)(
            resolvedLeftProv.makeCanonical)
        val fullyResolvedRightProv =
          resolveUnifications(where.right.relations)(
            resolvedRightProv.makeCanonical)
  ...
  private def resolveExpr(sigma: Sigma, expr: Expr): Expr =
    expr match {
      case expr @ Dispatch(_, id, _) => {
        expr.binding match {
          case FormalBinding(let) =>
            resolveExpr(sigma, sigma((id, let)))
          case _ =>
            expr
        }
      }
  ...
  private def solveConstraint(
    ...
    val contribErrors =
      if (!back.isDefined) {
        val vars = listTicVars(Some(b), constraint, sigma) map {
          _._2
        }
  ...
  private def solveGroupCondition(
      b: Solve,
      expr: Expr,
      free: Boolean,
      sigma: Sigma,
        ...
        expr.binding match {
          ...
          case FormalBinding(let) => {
            ...
            val resultM = actualM map {
              solveGroupCondition(b, _, free, sigma, dtrace)
            }
  ...
  private def pred(
      ...
      tv: TicId,
      free: Boolean,
      sigma: Sigma): PartialFunction[Node, Boolean] = {
  ...
  private def mergeSpecs(
      specs: TraversableOnce[(Option[BucketSpec], Set[Error])])
      : (Option[BucketSpec], Set[Error]) = {
    val (back, errors) =
  ...
  private def isTranspecable(to: Expr, from: Expr, sigma: Sigma): Boolean = {
    to match {
      case _ if to equalsIgnoreLoc from =>
        true

      case Let(_, _, _, _, right) =>
        isTranspecable(right, from, sigma)

      case Relate(_, _, _, in) =>
      ...
      case to @ Dispatch(_, id, actuals) => {
        to.binding match {
          ...
          case Op1Binding(_) | Op2Binding(_) =>
            true
      ...
      case Eq(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Eq(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case NotEq(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case NotEq(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)
      ...
      case Descent(_, child, _) =>
        isTranspecable(child, from, sigma)
      ...
      case Where(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Where(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case With(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case With(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Add(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Add(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Sub(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Sub(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Mul(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Mul(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Div(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Div(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Mod(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Mod(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Pow(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Pow(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Lt(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Lt(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case LtEq(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case LtEq(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Gt(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Gt(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case GtEq(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case GtEq(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case And(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case And(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      case Or(_, left, right) if isPrimitive(left, sigma) =>
        isTranspecable(right, from, sigma)
      case Or(_, left, right) if isPrimitive(right, sigma) =>
        isTranspecable(left, from, sigma)

      // non-primitive cases

      case Deref(_, left, right) =>
        isTranspecable(left, from, sigma) && isTranspecable(right, from, sigma)
  ...
  private def isPrimitive(expr: Expr, sigma: Sigma): Boolean =
    expr match {
      case Literal(_) =>
        true
      ...
      case Let(_, _, _, _, right) =>
        isPrimitive(right, sigma)

      case Relate(_, _, _, in) =>
        isPrimitive(in, sigma)

      case _: Union | _: Intersect | _: Difference | _: Cond =>
        false
  ...
  private def listTicVars(
      b: Option[Solve],
      expr: Expr,
      sigma: Sigma): Set[(Option[Solve], TicId)] =
    expr match {
      ...
      case Assert(_, pred, child) =>
        listTicVars(b, pred, sigma) ++ listTicVars(b, child, sigma)

      case Observe(_, data, samples) =>
        listTicVars(b, data, sigma) ++ listTicVars(b, samples, sigma)
      ...
      case d @ Dispatch(_, id, actuals) => {
        val leftSet =
          d.binding match {
            ...
            case FormalBinding(let) =>
              listTicVars(b, sigma((id, let)), sigma)
            case _ =>
              Set[(Option[Solve], TicId)]()
          }
        (
          actuals map {
            listTicVars(b, _, sigma)
  ...
  private def listSolvedVars(spec: BucketSpec): Set[TicId] =
    spec match {
      case UnionBucketSpec(left, right) =>
        listSolvedVars(left) ++ listSolvedVars(right)
      case IntersectBucketSpec(left, right) =>
        listSolvedVars(left) ++ listSolvedVars(right)
      case Group(_, _, forest, _) =>
        listSolvedVars(forest)
      case UnfixedSolution(id, _, _) =>
        Set(id)
      case FixedSolution(id, _, _, _) =>
        Set(id)
      case Extra(_, _) =>
        Set()
    }

  private def listSolutionExprs(spec: BucketSpec): Set[Expr] =
    spec match {
      case UnionBucketSpec(left, right) =>
        listSolutionExprs(left) ++ listSolutionExprs(right)
      case IntersectBucketSpec(left, right) =>
        listSolutionExprs(left) ++ listSolutionExprs(right)
      case Group(_, _, forest, _) =>
        listSolutionExprs(forest)
      case UnfixedSolution(_, expr, _) =>
        Set(expr)
      case FixedSolution(_, solution, _, _) =>
        Set(solution)
      case Extra(expr, _) =>
        Set(expr)
    }

  private def findCommonality(
      solve: Solve,
      nodes: Set[Expr],
      sigma: Sigma): Option[Expr] = {
    case class Kernel(
        nodes: Set[ExprWrapper],
        sigma: Sigma,
        seen: Set[ExprWrapper])
  ...
  private def enumerateParents(sigma: Sigma)(expr: Expr): (Set[Expr], Sigma) =
    expr match {
      case Let(_, _, _, _, right) =>
        (Set(right), sigma)

      case _: Solve =>
        (Set(), sigma) // TODO will this do the right thing?

      case Assert(_, pred, child) =>
        (Set(pred, child), sigma)

      case Observe(_, data, samples) =>
        (Set(data, samples), sigma)
  ...
  sealed trait BucketSpec {
    ...
    final def derive(id: TicId, expr: Expr): BucketSpec =
      this match {
        case UnionBucketSpec(left, right) =>
    ...
    final def exprs: Set[Expr] =
      this match {
        case UnionBucketSpec(left, right) =>
          left.exprs ++ right.exprs
        case IntersectBucketSpec(left, right) =>
          left.exprs ++ right.exprs
        case Group(_, target, forest, _) =>
          forest.exprs + target
        case UnfixedSolution(_, solution, _) =>
          Set(solution)
        case FixedSolution(_, solution, expr, _) =>
