...
trait ProvenanceChecker extends parser.AST with Binder {
  ...
  override def checkProvenance(expr: Expr): Set[Error] = {
    ...
    def handleNary(
      ...
      if (values.isEmpty) { (ValueProvenance, (Set(), Set())) }
      else {
    ...
    def loop(
      ...
      val back: (Set[Error], Set[ProvConstraint]) = expr match {
        ...
        case Relate(_, from, to, in) => {
          ...
          val constraints2 = constraints + (from.provenance -> from) +
            (to.provenance -> to)
          ...
          if (from.provenance == NullProvenance ||
            ...
            expr.provenance = InfiniteProvenance
          } else { expr.provenance = in.provenance }
        ...
        case expr @ Dispatch(_, name, actuals) => {
          expr.binding match {
            ...
            case ReductionBinding(_) => {
              ...
              if (actuals.head.provenance == InfiniteProvenance) {
                expr.provenance = NullProvenance
              } else { expr.provenance = ValueProvenance }
                  ...
                  def rec(policy: IdentityPolicy): Provenance =
                    policy match {
                      case IdentityPolicy.Product(left, right) => {
                        ...
                        unifyProvenance(relations)(recLeft, recRight) getOrElse
              ...
              def rec(policy: IdentityPolicy)
                ...
                policy match {
                  case IdentityPolicy.Product(left0, right0) =>
                    ...
                    val prov =
                      unifyProvenance(relations)(leftProv, rightProv) getOrElse
  ...
  def unifyProvenance(relations: Map[Provenance, Set[Provenance]])(
    ...
    (p1, p2) match {
      ...
      case (ProductProvenance(left, right), p2) => {
        ...
        val unionP = (leftP |@| rightP) { case (p1, p2) => p1 & p2 }
  ...
  def substituteParam(
    ...
    target match {
      ...
      case DerivedIntersectProvenance(left, right) => {
        ...
        } else { DerivedIntersectProvenance(left2, right2) }
      }
    ...
    private def associateLeft: Provenance =
      this match {
        ...
        case DerivedIntersectProvenance(_, _) =>
          findChildren(this, false).toList sorted
            Provenance.order.toScalaOrdering reduceLeft
  ...
  case class CoproductProvenance(left: Provenance, right: Provenance)
    ...
    def cardinality: Option[Int] =
      if (left.cardinality == right.cardinality) { left.cardinality }
      else { sys.error("cardinality invariant not upheld") }
  }
