...
trait ProvenanceChecker extends parser.AST with Binder {
  ...
  override def checkProvenance(expr: Expr): Set[Error] = {
    def handleBinary(
      ...
      val (provenance, contribErrors, contribConstr) = {
        if ((left.provenance == InfiniteProvenance ||
            right.provenance == InfiniteProvenance) && expr.disallowsInfinite) {
      ...
      (
        provenance,
        (
          leftErrors ++ rightErrors ++ contribErrors,
          leftConstr ++ rightConstr ++ contribConstr))
    }
    ...
    def handleNary(
      ...
      if (values.isEmpty) {
        ...
        val (prov, constrContrib, errorsContrib) = provenances reduce {
          (pair1, pair2) =>
            ...
            if ((prov1 == InfiniteProvenance || prov2 == InfiniteProvenance) &&
    ...
    def handleCond(
      ...
      val (provenance, errors, constr) = {
        if (pred.provenance == NullProvenance ||
            left.provenance == NullProvenance ||
            right.provenance == NullProvenance) {
          (NullProvenance, Set(), Set())
        } else if (pred.provenance == InfiniteProvenance ||
    ...
    def handleUnion(
      ...
      val (provenance, errors, constr) =
        if (left.provenance == InfiniteProvenance ||
          ...
          handleUnionLike(UnionProvenanceDifferentLength)(
            left.provenance,
            right.provenance)
        }
    ...
    def hasCommonality(left: Provenance, right: Provenance): Boolean =
      (left, right) match {
    ...
    def dynamicPossibility(prov: Provenance) =
      prov.possibilities exists {
    ...
    def handleIntersect(
      ...
      val (prov, errors, constr) = {
        if (left.provenance == right.provenance)
        ...
        else if (left.provenance == NullProvenance ||
                 right.provenance == NullProvenance)
          (NullProvenance, Set(), Set())
        else if (left.provenance == UndefinedProvenance &&
                 right.provenance == UndefinedProvenance)
          ...
          (
            ...
            Set())
        else if (left.provenance == InfiniteProvenance ||
                 right.provenance == InfiniteProvenance)
          (
            NullProvenance,
            Set(Error(expr, CannotUseDistributionWithoutSampling)),
    ...
    def handleDifference(
      ...
      val (prov, errors, constr) = {
        if (left.provenance == right.provenance)
          ...
          (
            NullProvenance,
            Set(Error(expr, DifferenceProvenanceDifferentLength)),
            Set())
    ...
    def loop(
      ...
      val back: (Set[Error], Set[ProvConstraint]) =
        expr match {
          ...
          case Assert(_, pred, child) => {
            ...
            if (pred.provenance != NullProvenance &&
                pred.provenance != InfiniteProvenance)
          ...
          case Observe(_, data, samples) => {
            ...
            (
              dataErrors ++ samplesErrors ++ observeDataErrors ++
                observeSamplesErrors,
              dataConst ++ samplesConst)
          }
          ...
          case Relate(_, from, to, in) => {
            ...
            val (contribErrors, contribConstr) =
              if (from.provenance == InfiniteProvenance ||
                  to.provenance == InfiniteProvenance) {
                (Set(Error(expr, CannotUseDistributionWithoutSampling)), Set())
              } else if (from.provenance.isParametric ||
                         to.provenance.isParametric) {
            ...
            if (from.provenance == NullProvenance ||
                to.provenance == NullProvenance ||
                from.provenance == InfiniteProvenance ||
                to.provenance == InfiniteProvenance) {
              expr.provenance = NullProvenance
            ...
            val finalErrors = fromErrors ++ toErrors ++ inErrors ++
              contribErrors
            val finalConstrs = fromConstr ++ toConstr ++ inConstr ++
              contribConstr
          ...
          case expr @ Dispatch(_, name, actuals) => {
            expr.binding match {
              case LetBinding(let) => {
                ...
                val actualErrors =
                  errorsVec.fold(Set[Error]()) {
                    _ ++ _
                  }
                val actualConstr =
                  constrVec.fold(Set[ProvConstraint]()) {
                    _ ++ _
                  }
              ...
              case DistinctBinding => {
                ...
                if (actuals.head.provenance == InfiniteProvenance) {
                  ...
                  expr.provenance = DynamicProvenance(
                    currentId.getAndIncrement())
                }
              ...
              case LoadBinding |
                  RelLoadBinding => { // FIXME not the same as StaticProvenance!
                val (errors, constr) = loop(
                  actuals.head,
                  relations,
                  constraints)
              ...
              case Morphism1Binding(morph1) => {
                ...
                expr.provenance = {
                  if (morph1.isInfinite) {
                    ...
                    def rec(policy: IdentityPolicy): Provenance =
                      policy match {
                        case IdentityPolicy.Product(left, right) => {
                          ...
                          unifyProvenance(relations)(
                            recLeft,
                            recRight) getOrElse
                            ProductProvenance(recLeft, recRight)
                        }
              ...
              case Morphism2Binding(morph2) => {
                ...
                val (leftErrors, leftConstr) = loop(
                  left,
                  relations,
                  constraints)
                  ...
                  policy match {
                    case IdentityPolicy.Product(left0, right0) =>
                      ...
                      val prov = unifyProvenance(relations)(
                        leftProv,
                        rightProv) getOrElse
                        ProductProvenance(leftProv, rightProv)
                    ...
                    case IdentityPolicy.Synthesize => {
                      val paramProv = ParametricDynamicProvenance(
  ...
  def unifyProvenance(relations: Map[Provenance, Set[Provenance]])(
      p1: Provenance,
      p2: Provenance): Option[Provenance] =
    (p1, p2) match {
      case (p1, p2) if p1 == p2 =>
        Some(p1)

      case (p1, p2)
          if pathExists(relations, p1, p2) || pathExists(relations, p2, p1) =>
        Some(p1 & p2)

      case (NullProvenance, p) =>
        Some(NullProvenance)
      case (p, NullProvenance) =>
        Some(NullProvenance)

      case (UndefinedProvenance, _) =>
        Some(UndefinedProvenance)
      case (_, UndefinedProvenance) =>
        Some(UndefinedProvenance)

      case (ValueProvenance, p) =>
        Some(p)
      case (p, ValueProvenance) =>
        Some(p)
  ...
  private def pathExists(
      graph: Map[Provenance, Set[Provenance]],
      from: Provenance,
  ...
  def components(prov: Provenance, target: Provenance): Boolean = {
    ...
    prov match {
      ...
      case CoproductProvenance(left, right) =>
        recursePair(left, right)
      case ParametricDynamicProvenance(p, _) =>
        components(p, target)
      case _ =>
        prov == target
    }
  ...
  def substituteParam(
    ...
    target match {
      case ParamProvenance(`id`, `let`) =>
        sub

      case ParametricDynamicProvenance(prov, _)
          if components(prov, ParamProvenance(`id`, `let`)) =>
        DynamicProvenance(currentId.getAndIncrement())

      case UnifiedProvenance(left, right) =>
        UnifiedProvenance(
          substituteParam(id, let, left, sub),
          substituteParam(id, let, right, sub))
  ...
  def resolveUnifications(relations: Map[Provenance, Set[Provenance]])(
  ...
  sealed trait Provenance {
    ...
    private def associateLeft: Provenance =
      this match {
        case UnifiedProvenance(_, _) =>
          findChildren(this, true).toList sorted
            Provenance.order.toScalaOrdering reduceLeft UnifiedProvenance

        case ProductProvenance(_, _) =>
          findChildren(this, false).toList sorted
            Provenance.order.toScalaOrdering reduceLeft ProductProvenance

        case CoproductProvenance(_, _) =>
          findChildren(this, false).toList sorted
            Provenance.order.toScalaOrdering reduceLeft CoproductProvenance

        case DerivedUnionProvenance(_, _) =>
          findChildren(this, false).toList sorted
            Provenance.order.toScalaOrdering reduceLeft DerivedUnionProvenance

        case DerivedIntersectProvenance(_, _) =>
          findChildren(this, false)
            .toList sorted Provenance.order.toScalaOrdering reduceLeft
            DerivedIntersectProvenance

        case DerivedDifferenceProvenance(_, _) =>
          findChildren(this, false)
            .toList sorted Provenance.order.toScalaOrdering reduceLeft
            DerivedDifferenceProvenance
    ...
    private def findChildren(
      ...
      prov match {
        ...
        case ProductProvenance(left, right) if !unified =>
          findChildren(left, unified) ++ findChildren(right, unified)
        case CoproductProvenance(left, right) if !unified =>
          findChildren(left, unified) ++ findChildren(right, unified)
        case DerivedUnionProvenance(left, right) if !unified =>
          findChildren(left, unified) ++ findChildren(right, unified)
        case DerivedIntersectProvenance(left, right) if !unified =>
          findChildren(left, unified) ++ findChildren(right, unified)
        case DerivedDifferenceProvenance(left, right) if !unified =>
          findChildren(left, unified) ++ findChildren(right, unified)
        case _ =>
          Set(prov)
      }
    ...
    def makeCanonical: Provenance = {
      this match {
        case UnifiedProvenance(left, right) =>
          UnifiedProvenance(left.makeCanonical, right.makeCanonical)
            .associateLeft
        case ProductProvenance(left, right) =>
          ProductProvenance(left.makeCanonical, right.makeCanonical)
            .associateLeft
        case CoproductProvenance(left, right) =>
          CoproductProvenance(left.makeCanonical, right.makeCanonical)
            .associateLeft
        case DerivedUnionProvenance(left, right) =>
          DerivedUnionProvenance(left.makeCanonical, right.makeCanonical)
        case DerivedIntersectProvenance(left, right) =>
          DerivedIntersectProvenance(left.makeCanonical, right.makeCanonical)
        case DerivedDifferenceProvenance(left, right) =>
  ...
  object Provenance {
    implicit def order: Order[Provenance] =
      new Order[Provenance] {
        def order(p1: Provenance, p2: Provenance): Ordering =
          (p1, p2) match {
            ...
            case (ParamProvenance(_, _), _) =>
              GT
            case (_, ParamProvenance(_, _)) =>
              LT
            ...
            case (ParametricDynamicProvenance(_, _), _) =>
              GT
            case (_, ParametricDynamicProvenance(_, _)) =>
              LT
            ...
            case (DerivedUnionProvenance(_, _), _) =>
              GT
            case (_, DerivedUnionProvenance(_, _)) =>
              LT
            ...
            case (
                  ...
                  DerivedIntersectProvenance(left2, right2)) =>
              (left1 ?|? left2) |+| (right1 ?|? right2)
            case (DerivedIntersectProvenance(_, _), _) =>
              GT
            case (_, DerivedIntersectProvenance(_, _)) =>
              LT
            ...
            case (
                  ...
                  DerivedDifferenceProvenance(left2, right2)) =>
              (left1 ?|? left2) |+| (right1 ?|? right2)
            case (DerivedDifferenceProvenance(_, _), _) =>
              GT
            case (_, DerivedDifferenceProvenance(_, _)) =>
              LT
            ...
            case (
                  ...
                  UnifiedProvenance(left2, right2)) =>
              (left1 ?|? left2) |+| (right1 ?|? right2)
            case (UnifiedProvenance(_, _), _) =>
              GT
            case (_, UnifiedProvenance(_, _)) =>
              LT
            ...
            case (
              ...
              (left1 ?|? left2) |+| (right1 ?|? right2)
            case (ProductProvenance(_, _), _) =>
              GT
            case (_, ProductProvenance(_, _)) =>
              LT
            ...
            case (
                  ...
                  CoproductProvenance(left2, right2)) =>
              (left1 ?|? left2) |+| (right1 ?|? right2)
            case (CoproductProvenance(_, _), _) =>
              GT
            case (_, CoproductProvenance(_, _)) =>
              LT
            ...
            case (StaticProvenance(_), _) =>
              GT
            case (_, StaticProvenance(_)) =>
              LT
            ...
            case (DynamicProvenance(_), _) =>
              GT
            case (_, DynamicProvenance(_)) =>
              LT

            case (ValueProvenance, _) =>
              GT
            case (_, ValueProvenance) =>
              LT

            case (UndefinedProvenance, UndefinedProvenance) =>
              EQ
            case (UndefinedProvenance, _) =>
              GT
            case (_, UndefinedProvenance) =>
            ...
            case (InfiniteProvenance, InfiniteProvenance) =>
              EQ
            case (InfiniteProvenance, _) =>
              GT
            case (_, InfiniteProvenance) =>
              LT

            case (NullProvenance, NullProvenance) =>
              EQ
          }
      }
  }
  ...
  case class DerivedUnionProvenance(left: Provenance, right: Provenance)
      extends Provenance {
    override val toString = "@@U<" + left.toString + "|" + right.toString + ">"

    val isParametric = left.isParametric || right.isParametric

    override def possibilities =
      left.possibilities ++ right.possibilities + this

    def cardinality: Option[Int] = None
  }

  case class DerivedIntersectProvenance(left: Provenance, right: Provenance)
      extends Provenance {
  ...
  case class DerivedDifferenceProvenance(left: Provenance, right: Provenance)
      extends Provenance {
  ...
  case class UnifiedProvenance(left: Provenance, right: Provenance)
      extends Provenance {
  ...
  case class ProductProvenance(left: Provenance, right: Provenance)
      extends Provenance {
    override val toString = "(%s & %s)".format(left, right)
  ...
  case class CoproductProvenance(left: Provenance, right: Provenance)
      extends Provenance {
    override val toString = "(%s | %s)".format(left, right)
  ...
  case class ParametricDynamicProvenance(prov: Provenance, id: Int)
      extends Provenance {
  ...
  case class NotRelated(left: Provenance, right: Provenance)
      extends ProvConstraint
  case class SameCard(left: Provenance, right: Provenance, tpe: ErrorType)
      extends ProvConstraint
  case class Commonality(left: Provenance, right: Provenance, tpe: ErrorType)
      extends ProvConstraint
}
