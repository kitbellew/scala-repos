...
object ParserSpecs
  ...
  "uncomposed expression parsing" should {
    ...
    "accept an assertion with a compound expression" in {
      parseSingle("assert a < 12 [a]") must beLike {
        case Assert(
              ...
              ArrayDef(
                _,
                Vector(Dispatch(_, Identifier(Vector(), "a"), Vector())))) => ok
      }
    }
    ...
    "accept a single solve expression" in {
      parseSingle("solve 'a 'a + 42") must beLike {
        case Solve(
              _,
              Vector(TicVar(_, "'a")),
              Add(_, TicVar(_, "'a"), NumLit(_, "42"))) => ok
      }
    }
      ...
      "with Add" >> {
        parseSingle("solve 'a, 'b 'a + 'b") must beLike {
          case Solve(
                ...
                Add(_, TicVar(_, "'a"), TicVar(_, "'b"))) => ok
        }
      }
      ...
      "with Union" >> {
        parseSingle("solve 'a, 'b 'a union 'b") must beLike {
          case Solve(
                _,
                Vector(TicVar(_, "'a"), TicVar(_, "'b")),
                Union(_, TicVar(_, "'a"), TicVar(_, "'b"))) => ok
        }
      }
      ...
      "with Difference" >> {
        parseSingle("solve 'a, 'b 'a difference 'b") must beLike {
          case Solve(
                _,
                Vector(TicVar(_, "'a"), TicVar(_, "'b")),
                Difference(_, TicVar(_, "'a"), TicVar(_, "'b"))) => ok
        }
      }
        ...
        parseSingle(
          ...
          case Solve(
                ...
                And(
                  ...
                  Paren(_, Where(_, TicVar(_, "'b"), BoolLit(_, false))))) => ok
        }
      }
    }
      ...
      parseSingle("solve 'a foo(b) := 1 + 'a foo") must beLike {
        case Solve(
              ...
              Let(
                ...
                Dispatch(_, Identifier(Vector(), "foo"), Vector()))) => ok
      }
    }
      ...
      parseSingle(
        ...
        case ObjectDef(
              ...
              Vector(
                ...
                ("test ` ing \\ with $%^&*!@#$ me!", NumLit(_, "2")))) => ok
      }
    }
    ...
    "accept an array definition with multiple actuals" in {
      parseSingle("[1, 2, 3]") must beLike {
        case ArrayDef(
              _,
              Vector(NumLit(_, "1"), NumLit(_, "2"), NumLit(_, "3"))) => ok
      }
    }
    ...
    "accept an array dereference" in {
      ...
      parseSingle("x[y]") must beLike {
        case Deref(
              ...
              Dispatch(_, Identifier(Vector(), "y"), Vector())) => ok
      }
    }
    ...
    "accept a dispatch with one actual" in {
      parseSingle("x(1)") must beLike {
        case Dispatch(_, Identifier(Vector(), "x"), Vector(NumLit(_, "1"))) =>
          ok
      }
    }
  ...
  "operator precedence" should {
    ...
    "favor negation/complement over multiplication/division" in {
      parseSingle("!a * b") must beLike {
        case Mul(
              ...
              Dispatch(_, Identifier(Vector(), "b"), Vector())) => ok
      }
      ...
      parseSingle("a + b * c") must beLike {
        case Add(
              ...
              Mul(
                ...
                Dispatch(_, Identifier(Vector(), "c"), Vector()))) => ok
      }
      parseSingle("a - b * c") must beLike {
        case Sub(
              _,
    ...
    "favor multiplication/division according to left/right ordering" in {
      parseSingle("1 * 2 / 3") must beLike {
        case Div(_, Mul(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 / 2 * 3") must beLike {
        case Mul(_, Div(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }

      parseSingle("1 * 2 % 3") must beLike {
        case Mod(_, Mul(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 % 2 * 3") must beLike {
        case Mul(_, Mod(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }

      parseSingle("1 % 2 / 3") must beLike {
        case Div(_, Mod(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 / 2 % 3") must beLike {
        case Mod(_, Div(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
    }
    ...
    "favor power according to left/right ordering" in {
      parseSingle("1 / 2 ^ 3") must beLike {
        case Div(_, NumLit(_, "1"), Pow(_, NumLit(_, "2"), NumLit(_, "3"))) =>
          ok
      }

      parseSingle("1 ^ 2 ^ 3") must beLike {
        case Pow(_, Pow(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
    }
    ...
    "favor addition/subtraction according to left/right ordering" in {
      parseSingle("1 + 2 - 3") must beLike {
        case Sub(_, Add(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 - 2 + 3") must beLike {
        case Add(_, Sub(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
    }
    ...
    "favor inequality operators according to left/right ordering" in {
      parseSingle("1 < 2 <= 3") must beLike {
        case LtEq(_, Lt(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 < 2 > 3") must beLike {
      ...
      parseSingle("1 <= 2 < 3") must beLike {
        case Lt(_, LtEq(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 > 2 < 3") must beLike {
    ...
    "favor equality operators according to left/right ordering" in {
      parseSingle("1 = 2 != 3") must beLike {
        case NotEq(_, Eq(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
      parseSingle("1 != 2 = 3") must beLike {
        case Eq(_, NotEq(_, NumLit(_, "1"), NumLit(_, "2")), NumLit(_, "3")) =>
          ok
      }
    }
    ...
    "favor where over relate" in {
      parseSingle("a where b ~ c d") must beLike {
        case Relate(
              ...
              Dispatch(_, Identifier(Vector(), "d"), Vector())) => ok
      }
    ...
    "favor where over let" in {
      parseSingle("a := 1 2 where 3") must beLike {
        case Let(
              ...
              Vector(),
              NumLit(_, "1"),
              Where(_, NumLit(_, "2"), NumLit(_, "3"))) => ok
      }
    }
    ...
    "correctly nest multiple binds" in {
      ...
      parseSingle(input) must beLike {
        case Let(
              ...
              Dispatch(_, Identifier(Vector(), "e"), Vector())) => ok
      }
    }
    ...
    "associate paired consecutive parentheses" in {
      parseSingle("a := b := c (d) (e)") must beLike {
        case Let(
              ...
              Paren(_, Dispatch(_, Identifier(Vector(), "e"), Vector()))) => ok
      }
    }
    ...
    "correctly disambiguate chained array dereferences" in {
      parseSingle("a := b [c] [d]") must beLike {
        case Let(
              ...
              ArrayDef(
                _,
                Vector(Dispatch(_, Identifier(Vector(), "d"), Vector())))) => ok
      }
    }
...

      {
        forest.head must beLike {
          case Let(
                ...
                Vector(),
                Dispatch(_, Identifier(Vector(), "c"), Vector()),
                Deref(_, Paren(_, NumLit(_, "42")), NumLit(_, "1"))) => ok
        }
