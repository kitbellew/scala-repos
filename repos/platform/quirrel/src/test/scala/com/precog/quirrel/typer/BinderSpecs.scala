...
object BinderSpecs
  ...
  "let binding" should {
    ...
    "bind all formals in expression scope" in {
...

      {
        val e @ Let(
          ...
          _) = parseSingle("a(b, c, d) := b + c + d a(1, 2, 3)")
    ...
    "bind formals in the constraint set" in {
      val e @ Let(_, _, _, Solve(_, Vector(Add(_, _, d: Dispatch)), _), _) =
        parseSingle("f(x) := solve 'a + x 'a 42")
      d.binding mustEqual FormalBinding(e)
      d.errors must beEmpty
    }

    "bind names in the constraint set" in {
      val e @ Let(_, _, _, _, Solve(_, Vector(Add(_, _, d: Dispatch)), _)) =
        parseSingle("f := 42 solve 'a + f 'a")
      d.binding mustEqual LetBinding(e)
      d.errors must beEmpty
    }
    ...
    "attribute call sites onto functions" in {
      val e1 @ Let(_, _, _, _, Add(_, d1: Dispatch, d2: Dispatch)) =
        parseSingle("f(x) := x f(12) + f(16)")
  ...
  "inherited scoping" should {
    ...
    "forward binding through let" in {
      val e1 @ Let(_, _, _, _, e2 @ Let(_, _, _, d1: Dispatch, d2: Dispatch)) =
        parseSingle("a := 42 b := a a")
    ...
    "forward binding through object definition" in {
      val e @ Let(_, _, _, _, ObjectDef(_, Vector((_, d: Dispatch)))) =
        parseSingle("a := 42 { a: a }")
      d.binding mustEqual LetBinding(e)
      d.isReduction mustEqual false
      d.errors must beEmpty
    }
    ...
    "forward binding through dispatch" in {
      forall(libReduction) { f =>
        val e @ Let(_, _, _, _, Dispatch(_, _, Vector(d: Dispatch))) =
          parseSingle("a := 42 %s(a)".format(f.fqn))
        d.binding mustEqual LetBinding(e)
        d.isReduction mustEqual false
        d.errors must beEmpty
      }
