...
object SolverSpecs
  ...
  "compound binary expression solution" should {
    "solve chained addition and multiplication" in {
      solve("'a * 2 + 3", 'a) must
        beLike {
          case Some(
                Div(_, Sub(_, someFunction, NumLit(_, "3")), NumLit(_, "2"))) =>
            ok
        }
      ...
      solve("'a * 2 + 'a + 'a", 'a) must
        beLike {
          ...
          case Some(
            ...
            ok
          case Some(
                Div(_, someFunction, Add(_, NumLit(_, "2"), NumLit(_, "2")))) =>
            ok
        }

      solve("'a + 'a * 2 + 'a", 'a) must
        beLike {
          case Some(Div(_, someFunction, NumLit(_, "4"))) =>
            ok
          case Some(
    ...
    "solve addition across dividend" in {
      solve("'a / 2 + 'a", 'a) must
        beLike {
          case Some(
                Div(_, Mul(_, someFunction, NumLit(_, "2")), NumLit(_, "3"))) =>
            ok
          case Some(
                Div(
                  _,
    ...
    "solve addition of multiplicands with common factors" in {
      solve("2 * 'a + 3 * 'a", 'a) must
        beLike {
          case Some(
                Div(_, someFunction, Add(_, NumLit(_, "2"), NumLit(_, "3")))) =>
            ok
          case Some(
                Div(_, someFunction, Add(_, NumLit(_, "3"), NumLit(_, "2")))) =>
            ok
        }

      solve("3 * 'a + 2 * 'a", 'a) must
        beLike {
          case Some(
                Div(_, someFunction, Add(_, NumLit(_, "2"), NumLit(_, "3")))) =>
            ok
    ...
    "solve addition with negation" in {
      solve("2 * 'a + neg 'a", 'a) must
        beLike {
          case Some(
                Div(_, someFunction, Sub(_, NumLit(_, "2"), NumLit(_, "1")))) =>
            ok
        }

      solve("neg 'a + 2 * 'a", 'a) must
        beLike {
          case Some(
                Div(_, someFunction, Sub(_, NumLit(_, "2"), NumLit(_, "1")))) =>
            ok
    ...
    "solve multiplication of dividends" in {
      solve("(2 / 'a) * ('a * 'a / 3)", 'a) must
        beLike {
          case Some(
                Div(_, Mul(_, someFunction, NumLit(_, "3")), NumLit(_, "2"))) =>
            ok
        }

      solve("('a * 'a / 3) * (2 / 'a)", 'a) must
        beLike {
          case Some(
                Div(_, Mul(_, someFunction, NumLit(_, "3")), NumLit(_, "2"))) =>
            ok
  ...
  "simple relation" should {
    "solve when target is in lhs of equality relation" in {
      solveRelation("(2 / 'a) / (3 / ('a * 'a)) = 0", 'a) must
        beLike {
          case Some(
                Div(
                  _,
                  Mul(_, NumLit(_, "0"), NumLit(_, "3")),
                  NumLit(_, "2"))) =>
            ok
          case Some(
                Div(
    ...
    "solve when target is in rhs of equality relation" in {
      solveRelation("0 = (2 / 'a) / (3 / ('a * 'a))", 'a) must
        beLike {
          case Some(
                Div(
                  _,
