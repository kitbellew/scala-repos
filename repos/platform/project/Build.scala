...
object PlatformBuild extends Build {
  ...
  val commonSettings = Seq(
    ...
    scalacOptions ++= {
      ...
      Seq("-unchecked", "-g:none") ++
        Option(System.getProperty("com.precog.build.optimize")).map { _ =>
          Seq("-optimize")
        }.getOrElse(Seq())
    },
  ...
  lazy val logging = Project(id = "logging", base = file("logging")).settings(
    commonNexusSettings: _*)

  lazy val standalone = Project(
    id = "standalone",
    base = file("standalone")).settings(
  ...
  lazy val platform = Project(id = "platform", base = file(".")).settings(
    ScctPlugin.mergeReportSettings ++ ScctPlugin.instrumentSettings: _*).aggregate(
    quirrel,
    mirror,
    yggdrasil,
    bytecode,
  ...
  lazy val util = Project(id = "util", base = file("util")).settings(
    commonNexusSettings: _*) dependsOn (logging % "test->test")

  lazy val common = Project(id = "common", base = file("common")).settings(
    commonNexusSettings: _*) dependsOn (util, logging % "test->test")

  lazy val bytecode =
    Project(id = "bytecode", base = file("bytecode")).settings(
      commonNexusSettings: _*) dependsOn (logging % "test->test")

  lazy val quirrel = Project(id = "quirrel", base = file("quirrel")).settings(
    commonNexusSettings: _*) dependsOn (bytecode % "compile->compile;test->test", util, logging % "test->test")

  lazy val mirror = Project(id = "mirror", base = file("mirror")).settings(
    commonNexusSettings: _*) dependsOn (quirrel)

  lazy val niflheim = Project(
    id = "niflheim",
    base = file("niflheim")).settings(commonAssemblySettings: _*).dependsOn(
  ...
  lazy val yggdrasil = Project(
    id = "yggdrasil",
    base = file("yggdrasil")).settings(commonAssemblySettings: _*).dependsOn(
    common % "compile->compile;test->test",
    bytecode,
    util,
    niflheim,
  ...
  lazy val yggdrasilProf =
    Project(id = "yggdrasilProf", base = file("yggdrasilProf")).settings(
      commonNexusSettings ++ jprofilerSettings ++ Seq(
        fullRunInputTask(
          profileTask,
          Test,
          "com.precog.yggdrasil.test.Run")): _*).dependsOn(
      yggdrasil % "compile->compile;compile->test",
      logging % "test->test")

  lazy val mongo = Project(id = "mongo", base = file("mongo")).settings(
    commonAssemblySettings: _*).dependsOn(
    standalone,
    muspelheim % "compile->compile;test->test")

  lazy val jdbc = Project(id = "jdbc", base = file("jdbc")).settings(
    commonAssemblySettings: _*).dependsOn(
    standalone,
    muspelheim % "compile->compile;test->test")

  lazy val desktop = Project(id = "desktop", base = file("desktop")).settings(
    commonAssemblySettings: _*).dependsOn(standalone, bifrost)

  lazy val mimir = Project(id = "mimir", base = file("mimir")).settings(
    commonNexusSettings: _*).dependsOn(
  ...
  lazy val muspelheim = Project(
    id = "muspelheim",
    base = file("muspelheim")).settings(
  ...
  lazy val ragnarok = Project(
    id = "ragnarok",
    base = file("ragnarok")).settings(commonAssemblySettings: _*).dependsOn(
    quirrel,
    mimir,
    yggdrasil,
    ingest,
  ...
  lazy val gjallerhorn = Project(
    id = "gjallerhorn",
    base = file("gjallerhorn")).settings(commonAssemblySettings: _*).dependsOn(
    quirrel,
    mimir,
  ...
  lazy val performance = Project(
    id = "performance",
    base = file("performance")).settings(commonNexusSettings: _*).dependsOn(
    ingest,
    common % "compile->compile;test->test",
    quirrel,
    mimir,
  ...
  lazy val jprofiler =
    Project(id = "jprofiler", base = file("jprofiler")).settings(
      jprofilerSettings ++ commonNexusSettings ++ Seq(
        fullRunInputTask(
          profileTask,
          Test,
          "com.precog.jprofiler.Run")): _*).dependsOn(
      ragnarok,
      logging % "test->test")

  /// Services ///

  lazy val auth = Project(id = "auth", base = file("auth")).settings(
    commonAssemblySettings: _*).dependsOn(
    common % "compile->compile;test->test",
    logging % "test->test")
  ...
  lazy val accounts = Project(
    id = "accounts",
    base = file("accounts")).settings(
    commonAssemblySettings: _*) dependsOn (common % "compile->compile;test->test", auth, logging % "test->test")

  lazy val ingest = Project(id = "ingest", base = file("ingest")).settings(
    commonAssemblySettings: _*).dependsOn(
    common % "compile->compile;test->test",
    yggdrasil,
    logging % "test->test")

  lazy val dvergr = Project(id = "dvergr", base = file("dvergr")).settings(
    commonAssemblySettings: _*).dependsOn(
    common % "compile->compile;test->test",
    util,
    logging % "test->test")

  lazy val bifrost = Project(id = "bifrost", base = file("bifrost")).settings(
    commonAssemblySettings: _*).dependsOn(
    common % "compile->compile;test->test",
    muspelheim,
    surtr % "test->test")
  ...
  lazy val ratatoskr =
    Project(id = "ratatoskr", base = file("ratatoskr")).settings(
      commonAssemblySettings: _*).dependsOn(mimir, ingest, auth, accounts)
}
