...
trait TestEventService
  ...
  def configureEventService(config: Configuration): EventService.State = {
    ...
    val eventStore = new EventStore[Future] {
      def save(action: Event, timeout: Timeout) =
        M.point { stored += action; \/-(PrecogUnit) }
    }
    val jobManager =
    ...
    val tmpdir = java.io.File.createTempFile("test.ingest.tmpdir", null)
      .getParentFile()
  ...
  def track[A](
    ...
    val svc = client.contentType[A](contentType).query("receipt", sync.toString)
      .query("mode", if (batch) "batch" else "stream").path("/ingest/v2/fs/")
    ...
    for {
      ...
      content <-
        response.content map (a => bi(a) map (Some(_))) getOrElse Future(None)
    } yield {
      (
