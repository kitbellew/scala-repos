...
object IdentityPolicy {
  ...
  /** Both identity policies are adhered to, and then concatenated uniquely.
    ...
    */
  case class Product(left: IdentityPolicy, right: IdentityPolicy)
      extends IdentityPolicy
}
...
trait Morphism1Like extends FunctionLike {
  ...
  val idPolicy: IdentityPolicy =
    IdentityPolicy.Strip // TODO remove this default
}
...
object Op1Like {
  def unapply(
      op1: Op1Like): Option[(Vector[String], String, Int, UnaryOperationType)] =
...
object ReductionLike {
  def unapply(red: ReductionLike): Option[(Vector[String], String, Int)] =
    Some(red.namespace, red.name, red.opcode)
}
