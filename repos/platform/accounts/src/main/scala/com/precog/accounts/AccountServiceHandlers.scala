...
class AccountServiceHandlers(
    val accountManager: AccountManager[Future],
    apiKeyFinder: APIKeyFinder[Future],
    clock: Clock,
    rootAccountId: String,
    rootAPIKey: APIKey,
    emailer: TemplateEmailer)(implicit executor: ExecutionContext)
    extends Logging {
  import ServiceHandlerUtil._

  def withAccountAdmin[A](
      accountId: String,
      auth: Account,
    ...
    accountManager.findAccountById(accountId) flatMap {
      case Some(account) =>
        accountManager.hasAncestor(account, auth) flatMap {
          ...
          case false => {
            logger.warn(
              "Unauthorized access attempt to account %s from account %s (%s)".format(
                accountId,
                auth.accountId,
      ...
      (request: HttpRequest[Future[JValue]]) => {
        Success { (auth: Account) =>
          ...
          def followIssuers(
            ...
            logger.debug(
              "Finding account for %s with remaining %s".format(
                currentKey,
                remaining))
          ...
          accountManager.findAccountByAPIKey(keyToFind).flatMap {
            ...
            HttpResponse[JValue](
              OK,
              content = accountId.map(id => WrappedAccountId(id).serialize))
          }
        }
      }
                      ...
                      existingAccountOpt map {
                        account =>
                          ...
                          Promise.successful(
                            Responses.failure(
                              Conflict,
                              "An account already exists with the email address %s. If you feel this is in error, please contact support@precog.com".format(
                                email)))
                      } getOrElse {
            ...
            request.content.map { futureContent =>
              futureContent.flatMap { jvalue =>
                (jvalue \ "email").validated[String] match {
                    ...
                    accountManager.findAccountById(accountId).flatMap {
                      case Some(account) =>
                        if (account.email == requestEmail) {
                          ...
                          logger.warn(
                            "Password reset request for account %s did not match email on file (%s provided)".format(
                              accountId,
                              requestEmail))
                          Future(
                            HttpResponse[JValue](
                  ...
                  case Failure(error) =>
                    logger.warn(
                      "Password reset request for account %s without body".format(
                        accountId))
                    Future(
                      HttpResponse[JValue](
  ...
  object PasswordResetHandler
    ...
    val service: HttpRequest[Future[JValue]] => Validation[
      ...
      {
        Success {
          (request.parameters.get('accountId).toSuccess(
            NonEmptyList("Missing account ID in request URI")) |@|
            request.parameters.get('resetToken).toSuccess(
              ...
              futureContent.flatMap { jvalue =>
                jvalue.validated[String]("password") match {
                  case Success(newPassword) =>
                    accountManager.resetAccountPassword(
                      accountId,
                      resetToken,
                      newPassword).map {
                      case \/-(true) =>
                        logger.info(
                          "Password for account %s successfully reset by %s".format(
                            accountId,
                            remoteIpFrom(request)))
                      ...
                      case -\/(error) =>
                        logger.error(
                          "Password reset for account %s from %s failed: %s".format(
                            accountId,
                            remoteIpFrom(request),
                            error))
                        Responses.failure(
                  ...
                  case Failure(error) =>
                    logger.warn(
                      "Password reset request for account %s without new password".format(
                        accountId))
                    Future(
                      Responses.failure(
                        BadRequest,
                ...
                (jvalue \ "password").validated[String] match {
                  case Success(newPassword) =>
                    accountManager.updateAccountPassword(
                      ...
                      case true =>
                        logger.info(
                          "Password for account %s successfully updated by %s".format(
                            account.accountId,
                            remoteIpFrom(request)))
                        HttpResponse[JValue](OK, content = None)
                      case _ =>
                        logger.error(
                          "Password update for account %s from %s failed".format(
                            account.accountId,
                            remoteIpFrom(request)))
                        Responses.failure(
                          InternalServerError,
                  ...
                  case Failure(error) =>
                    logger.warn(
                      "Invalid password update body \"%s\" for account %s from %s: %s".format(
                ...
                (jvalue \ "type").validated[String] match {
                  case Success(planType) =>
                    accountManager.updateAccount(
                      ...
                      case _ =>
                        logger.error(
                          "Plan change to %s for account %s by %s failed".format(
                            planType,
                            account.accountId,
                            remoteIpFrom(request)))
                        Responses.failure(
  ...
  object DeleteAccountPlanHandler
        ...
        Success { (auth: Account) =>
          withAccountAdmin(request, auth) { account =>
            accountManager.updateAccount(
              ...
              case true =>
                logger.info(
                  "Account plan for %s deleted (converted to free plan) by %s".format(
                    account.accountId,
                    remoteIpFrom(request)))
                HttpResponse[JValue](
                  OK,
              ...
              case _ =>
                logger.error(
                  "Account plan for %s deletion by %s failed".format(
                    account.accountId,
                    remoteIpFrom(request)))
                Responses.failure(
                  InternalServerError,
    ...
    val metadata = DescriptionMetadata(
      "Downgrades the specified account to the free plan.")
  }
        ...
        Success { (auth: Account) =>
          withAccountAdmin(request, auth) { account =>
            accountManager.deleteAccount(account.accountId).map {
              case Some(_) =>
                logger.warn(
                  "Account %s deleted by %s".format(
                    account.accountId,
                    remoteIpFrom(request)))
                HttpResponse[JValue](HttpStatus(NoContent))
              case None =>
                logger.error(
                  "Account %s deletion by %s failed".format(
                    account.accountId,
                    remoteIpFrom(request)))
                HttpResponse[JValue](
                  HttpStatus(InternalServerError),
