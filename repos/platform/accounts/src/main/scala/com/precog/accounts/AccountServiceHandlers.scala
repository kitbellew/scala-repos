          ...
          case false => {
            ...
            Future(
              HttpResponse[JValue](
                ...
                content = Some(JString("You do not have access to account " +
                  accountId))))
          }
        }
    ...
    val service: HttpRequest[Future[JValue]] => Validation[
      ...
      (request: HttpRequest[Future[JValue]]) => {
        Success { (auth: Account) =>
          ...
          logger.debug(
            "Looking up account ids with account: " +
              auth.accountId + " for API key: " + keyToFind)
          ...
          accountManager
            .findAccountByAPIKey(keyToFind)
            .flatMap {
              case Some(accountId) =>
                logger.debug("Found account for API key: " + keyToFind +
                  " = " + accountId)
                Promise.successful(Some(accountId))
                ...
                (jvalue \ "type").validated[String] match {
                  case Success(planType) =>
                    accountManager.updateAccount(
                      ...
                      case true =>
                        logger.info(
                          "Plan changed for %s to %s from %s"
                            .format(
                              account.accountId,
                              planType,
                              remoteIpFrom(request)))
