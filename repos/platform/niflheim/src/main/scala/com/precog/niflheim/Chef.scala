...
final case class Chef(blockFormat: CookedBlockFormat, format: SegmentFormat)
    extends Actor {
  ...
  def cook(
      root: File,
      reader: StorageReader): ValidationNel[IOException, File] = {
    assert(root.exists)
  ...
  def receive = {
    case Prepare(blockid, seqId, root, source) => cook(root, source) match {
        case Success(file) => sender ! Cooked(blockid, seqId, root, file)
        case Failure(_)    => sender ! Spoilt(blockid, seqId)
      }
  }
}
