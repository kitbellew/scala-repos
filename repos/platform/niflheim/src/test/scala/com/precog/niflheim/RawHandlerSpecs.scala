...
object RawHandlerSpecs extends Specification with ScalaCheck {
  ...
  "raw handler" should {
    ...
    /**
      ...
      */
    val tmp1 = tempfile()
    "generate snapshots from writes" in
      new cleanup(tmp1) {
...
""".trim
    ...
    /**
      ...
      */
    val tmp4 = tempfile()
    "throw an exception when loading empty logs" in
      new cleanup(tmp4) {
    ...
    /**
      ...
      */
    val tmp5 = tempfile()
    "throw an exception when creating already-present logs " in
      new cleanup(tmp5) {
    ...
    /**
      ...
      */
    val tmp7 = tempfile()
    "throw errors when totally corrupted" in
      new cleanup(tmp7) {
        val ps = makeps(tmp7)
    ...
    /**
      ...
      */
    val tmp8 = tempfile()
    "recover when partially corrupted" in
      new cleanup(tmp8) {
    ...
    val tmp9 = tempfile()
    "make sure we defensively copy" in
      new cleanup(tmp9) {
        ...
        struct2.toSet must_== Set(ColumnRef(cpa, CString))
        struct3.toSet must_==
          Set(ColumnRef(cpa, CString), ColumnRef(cpb, CString))
