...
trait ArbitraryJValue {
  ...
  def genSimple: Gen[JValue] =
    oneOf(value(JNull), genJNum, genJBool, genJString)

  def genSimpleNotNull: Gen[JValue] = oneOf(genJNum, genJBool, genJString)

  def genArray(listSize: Int): Gen[JValue] = for (l <- genList(listSize))
    yield JArray(l)
  def genObject(listSize: Int): Gen[JObject] = for (l <- genFieldList(listSize))
    yield JObject(l)
  ...
  implicit def arbJObject: Arbitrary[JObject] = Arbitrary(
    choose(0, 5) flatMap genObject)
  implicit def arbJValueClass: Arbitrary[Class[_ <: JValue]] = Arbitrary(
    genJValueClass)
