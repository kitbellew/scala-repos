...
trait ArbitraryEventMessage extends ArbitraryJValue {
  def genStreamId: Gen[Option[UUID]] =
    Gen.oneOf(Gen.resultOf[Int, Option[UUID]](_ => Some(UUID.randomUUID)), None)
  ...
  def genRandomIngest: Gen[Ingest] =
    for {
      ...
      content <-
        containerOf[List, JValue](genContentJValue).map(l => Vector(l: _*))
      if !content.isEmpty
  ...
  def genRandomIngestMessage: Gen[IngestMessage] =
    for {
      ...
      eventIds <-
        containerOfN[List, EventId](ingest.data.size, genEventId).map(l =>
          Vector(l: _*))
      streamRef <- genStreamRef
    } yield {
