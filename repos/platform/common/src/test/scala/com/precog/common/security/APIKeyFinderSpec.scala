          ...
          for {
            ...
            grant <-
              mgr.createGrant(
                ...
                permissions,
                None)
          } yield (key0.apiKey, key1.apiKey, grant.grantId, mgr)
        ).copoint
    ...
    "find all child API Keys" in {
      val (parent, keys, mgr) =
        (
          ...
          } yield ((key0.apiKey, Set(key1, key2) map (_.apiKey), mgr))
        ).copoint
    ...
    "not return grand-child API keys or self API key when finding children" in {
      val (parent, child, mgr) =
        (
          ...
          } yield ((key0.apiKey, key1.apiKey, mgr))
        ).copoint
    ...
    "return issuer details when a proper root key is passed to findAPiKey" in {
      val (rootKey, key0, key1, mgr) =
        (
          ...
          } yield (rootKey, key0.apiKey, key1.apiKey, mgr)
        ).copoint
