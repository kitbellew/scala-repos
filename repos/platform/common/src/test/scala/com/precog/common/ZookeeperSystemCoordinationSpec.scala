...
class ZookeeperSystemCoordinationSpec extends Specification {
  ...
  "zookeeper system coordination " should {
    ...
    "save and restore relay agent state" in TestZookeeperClient() {
    ...
    "restore relay agent state" in TestZookeeperClient() {
      factory: ClientFactory =>
        val client1 = factory()
        val sc1 = newSystemCoordination(client1)
  ...
  def newSystemCoordination(client: ZkClient) = new ZookeeperSystemCoordination(
    client,
  ...
  case class TestZookeeperClient(zkHosts: String = "127.0.0.1:2181")
