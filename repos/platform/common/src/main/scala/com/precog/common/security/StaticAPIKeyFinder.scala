...
class StaticAPIKeyFinder[M[+_]](apiKey: APIKey)(implicit val M: Monad[M])
  ...
  val rootGrant = v1.GrantDetails(
    ...
    new Instant(0L),
    None)
  val rootAPIKeyRecord = v1
    .APIKeyDetails(apiKey, Some("Static api key"), None, Set(rootGrant), Nil)
  ...
  def findAPIKey(apiKey: APIKey, rootKey: Option[APIKey]) =
    M.point(
      if (apiKey == self.apiKey)
        Some(rootAPIKeyRecord)
      else
        None)
  def findGrant(grantId: GrantId) =
    M.point(
      if (rootGrant.grantId == grantId)
        Some(rootGrant)
      else
        None)

  def findAllAPIKeys(fromRoot: APIKey): M[Set[v1.APIKeyDetails]] =
    findAPIKey(fromRoot, None) map {
      _.toSet
    }
