...
class FileJobManager[M[+_]] private[FileJobManager] (
  ...
  def findJob(jobId: JobId): M[Option[Job]] =
    M.point { loadJob(jobId).map(_.job) }
  ...
  def listChannels(jobId: JobId): M[Seq[ChannelId]] =
    M.point { loadJob(jobId).map(_.messages.keys.toSeq).getOrElse(Seq.empty) }
  ...
  def getStatus(jobId: JobId): M[Option[Status]] =
    M.point { loadJob(jobId).flatMap(_.status) }
