...
trait Codec[@spec(Boolean, Long, Double) A] { self =>
  ...
  def writeAll(a: A)(
    ...
    @inline @tailrec def loop(
        s: Option[S],
        buffers: List[ByteBuffer]): List[ByteBuffer] = s match {
      case None => buffers
      case Some(s) =>
        val buf = acquire()
...
object Codec {
  ...
  def sizePackedInt(n: Int, size: Int = 1): Int =
    if ((n & ~0x7F) != 0) { sizePackedInt(n >>> 7, size + 1) }
    else size
  ...
  trait FixedWidthCodec[@spec(Boolean, Long, Double) A] extends Codec[A] {
    ...
    def writeInit(a: A, b: ByteBuffer): Option[A] =
      if (b.remaining >= size) {
        writeUnsafe(a, b)
        None
      } else { Some(a) }
    ...
    override def skip(buf: ByteBuffer) { buf.position(buf.position() + size) }
  }
  ...
  implicit def arrayCodec[@spec(Boolean, Long, Double) A: Codec: Manifest]
      : Codec[Array[A]] = ArrayCodec(Codec[A])
  ...
  case class CValueCodec[A](cType: CValueType[A])(implicit val codec: Codec[A])
    ...
    def writeUnsafe(a: CWrappedValue[A], sink: ByteBuffer) =
      codec.writeUnsafe(a.value, sink)
    def writeInit(a: CWrappedValue[A], sink: ByteBuffer) =
      codec.writeInit(a.value, sink)
  ...
  case class SparseBitSetCodec(size: Int) extends Codec[BitSet] {
    ...
    def writeUnsafe(bs: BitSet, sink: ByteBuffer) { sink.put(writeBitSet(bs)) }
    ...
    def readBitSet(src: ByteBuffer): RawBitSet = {
      ...
      def read(l: Int, r: Int, offset: Int): Int = {
        if (l == r) { offset }
          ...
          if (get(offset)) { RawBitSet.set(bits, l) }
          offset + 2
        } else {
          val c = (l + r) / 2
          (get(offset), get(offset + 1)) match {
