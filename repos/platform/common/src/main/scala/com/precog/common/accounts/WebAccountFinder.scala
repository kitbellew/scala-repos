  ...
  def apply(config: Configuration)(implicit executor: ExecutionContext)
    ...
    serviceConfig.get[String]("hardcoded_account") map { accountId =>
      ...
      (serviceConfig.get[String]("protocol").toSuccess(
        ...
        serviceConfig.get[String]("user").toSuccess(
          nels("Configuration property service.user is required")) |@|
        serviceConfig.get[String]("password").toSuccess(
          nels("Configuration property service.password is required"))) {
        (protocol, host, port, path, user, password) =>
          logger.info(
            "Creating new WebAccountFinder with properties %s://%s:%s/%s %s:%s".format(
              protocol,
              host,
              port.toString,
              path,
