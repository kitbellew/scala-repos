...
sealed trait CPath { self =>
  ...
  def combine(paths: Seq[CPath]): Seq[CPath] = {
    if (paths.isEmpty) Seq(this)
      ...
      paths map { path => CPath(this.nodes ++ path.nodes) }
  }
...
object CPath {
  ...
  case class FieldNode[A](field: CPathField, children: Seq[CPathTree[A]])
      extends CPathTree[A]
  case class IndexNode[A](index: CPathIndex, children: Seq[CPathTree[A]])
      extends CPathTree[A]
