...
object WrittenByPermission {
  def unapply(perm: WrittenByPermission): Option[(Path, WrittenBy)] =
    Some((perm.path, perm.writtenBy))
}
...
object Permission {
  ...
  object WriteAs {
    def all(accountIds: NonEmptyList[AccountId]): WriteAs =
      WriteAsAll(accountIds.list.toSet)
