...
abstract class BrowseServiceSpecs[M[+_]](implicit
  ...
  def colSizeMetadata(descriptor: ColumnRef, size: Long): ColumnMetadata =
    Map(descriptor -> Map(StringValueStats -> StringValueStats(size, "a", "z")))
  ...
  "browse" should {
    "find child paths" in {
      client.browse("", Path("/foo/")).valueOr(e => sys.error(e.toString))
        .copoint must beLike {
        case JArray(results) =>
  ...
  "size" should {
    "find correct size for single-column path" in {
      client.size("", Path("/foo/bar1/baz/quux1"))
        .valueOr(e => sys.error(e.toString)).copoint must beLike {
        case JNum(result) => result mustEqual 10
      }
    }

    "find correct size for multi-column path" in {
      client.size("", Path("/foo/bar")).valueOr(e => sys.error(e.toString))
        .copoint must beLike { case JNum(result) => result mustEqual 60 }
    }

    "find default (0) size for non-existent path" in {
      client.size("", Path("/not/really")).valueOr(e => sys.error(e.toString))
  ...
  "structure" should {
    "find correct node information" in {
      client.structure("", Path("/foo/bar"), CPath.Identity)
        .valueOr(e => sys.error(e.toString)).copoint must beLike {
        case result =>
    ...
    "find correct leaf types" in {
      client.structure("", Path("/foo/bar"), CPath("bar"))
        .valueOr(e => sys.error(e.toString)).copoint must beLike {
        case result =>
          result must_== JObject(
            "children" -> JArray(),
    ...
    "find default empty result for non-existent path" in {
      client.structure("", Path("/bar/foo"), CPath.Identity)
        .valueOr(e => sys.error(e.toString)).copoint must beLike {
