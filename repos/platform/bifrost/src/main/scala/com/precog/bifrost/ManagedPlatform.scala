...
trait ManagedPlatform
    extends ManagedExecution
    with Platform[Future, Slice, StreamT[Future, Slice]]
...
trait ManagedExecution
  ...
  def errorReport[A](implicit
      shardQueryMonad: JobQueryTFMonad,
      decomposer0: Decomposer[A]): QueryLogger[JobQueryTF, A] = {
  ...
  trait ManagedQueryExecutor[+A] extends QueryExecutor[Future, A] {
    ...
    def execute(
      ...
      EitherT.right(
        createQueryJob(context.apiKey, Some(userQuery.serialize), opts.timeout)(
          executionContext)) flatMap {
        implicit shardQueryMonad: JobQueryTFMonad =>
          import JobQueryState._

          complete(
            EitherT.eitherTHoist[EvaluationError].hoist(sink)
        ...
        resultE map { result =>
          ...
          jobManager.setResult(
            ...
            case Left(error) =>
              jobManager.abort(
                jobId,
                "Error occured while storing job results: " + error,
                yggConfig.clock.now())
            case Right(_) =>
