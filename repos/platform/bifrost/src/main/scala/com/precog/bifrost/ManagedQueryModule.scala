...
trait ManagedQueryModule extends YggConfigComponent with Logging {
  ...
  trait JobQueryTFMonad
      extends QueryTMonad[JobQueryState, Future]
      with QueryTHoist[JobQueryState] {
  ...
  private final case class JobQueryStateManager(
    ...
    def hasExpired(): Boolean = { yggConfig.clock.now() isAfter expiresAt }
