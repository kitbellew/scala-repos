  ...
  val service = (request: HttpRequest[A]) =>
    Success {
      ...
      (apiKey: APIKey, path: Path) => {
        ...
        platform.vfs.readResource(
          apiKey,
          path,
          Version.Current,
          AccessMode.Read).run flatMap {
          _.fold(
            resourceError =>
              M point {
                resourceError match {
                  case Corrupt(message) =>
                    logger.error(
                      "Corrupt resource found for current version of path %s: %s".format(
                        path.path,
                        message))
                  ...
                  case IOError(throwable) =>
                    logger.error(
                      "An IO error was encountered reading data from path %s".format(
                        path.path),
                      throwable)
                    HttpResponse(InternalServerError)
                  ...
                  case IllegalWriteRequestError(message) =>
                    logger.error(
                      "Unexpected write response to readResource of path %s: %s".format(
                        path.path,
                        message))
                    HttpResponse(InternalServerError)
                  ...
                  case PermissionsError(message) =>
                    logger.warn(
                      "Access denied after auth combinator check to path %s for %s: %s".format(
                        path.path,
                        apiKey,
                        message))
                  ...
                  case multiple: ResourceErrors =>
                    multiple.fold(
                      fatal => {
                        logger.error(
                          "Multiple errors encountered serving readResource of path %s: %s".format(
                            path.path,
                      ...
                      userError =>
                        HttpResponse(
                          HttpStatus(
                            BadRequest,
                            "Multiple errors encountered reading resource at path %s".format(
                              path.path)),
                          ...
                          content = Some(Left(JObject(
                            "errors" -> JArray(userError.messages.list.map(
                              JString(_)): _*)).renderPretty.getBytes("UTF-8")))
                        )
                    )
                }
              },
