...
class StatusTest extends FunSuite {
  ...
  test("matches unknown status") {
    Status(99) match {
      case Unknown(_) =>
      case status =>
        fail(s"$status should be UnknownStatus")
    }
  }
  ...
  test("matches informational status") {
    100 until 200 foreach { code =>
      Status(code) match {
        ...
        case status =>
          fail(s"$status should be Informational")
      }
    }
  }
  ...
  test("matches successful status") {
    200 until 300 foreach { code =>
      Status(code) match {
        case Successful(_) =>
        case status =>
          fail(s"$status should be Successful")
      }
    }
  }
  ...
  test("matches redirection status") {
    300 until 400 foreach { code =>
      Status(code) match {
        case Redirection(_) =>
        case status =>
          fail(s"$status should be Redirection")
      }
    }
  }
  ...
  test("match client error status") {
    400 until 500 foreach { code =>
      Status(code) match {
        case ClientError(_) =>
        case status =>
          fail(s"$status should be ClientError")
      }
    }
  }
  ...
  test("match server error status") {
    500 until 600 foreach { code =>
      Status(code) match {
        case ServerError(_) =>
        case status =>
          fail(s"$status should be ServerError")
      }
    }
  }
