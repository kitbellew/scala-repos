...
class RequestTest extends FunSuite {
  ...
  test("path") {
    ...
    tests.foreach {
      case (input, expected) =>
        assert(Request(input).path == expected)
    }
  }
  ...
  test("file extension") {
    ...
    tests.foreach {
      case (input, expected) =>
        assert(Request(input).fileExtension == expected)
    }
  }
  ...
  test("queryString") {
    ...
    assert(
      Request.queryString("/search.json", Map.empty[String, String]) ==
        "/search.json")

    assert(
      Request.queryString("/search.json", "q" -> "twitter") ==
        "/search.json?q=twitter")
    assert(
      Request.queryString("/search.json", Map("q" -> "twitter")) ==
        "/search.json?q=twitter")
