...
class HttpNackFilterTest extends FunSuite {
  ...
  test("automatically retries with HttpNack") {
    new ClientCtx {
      val server = Http.server.configured(Stats(serverSr))
        .configured(Label("myservice"))
        .serve(new InetSocketAddress(0), flakyService)
      val client = Http.client.configured(Stats(clientSr)).newService(
        Name
  ...
  test("HttpNack works with ClientBuilder") {
    new ClientCtx {
      val server = Http.server.configured(Stats(serverSr))
        .configured(Label("myservice"))
        .serve(new InetSocketAddress(0), flakyService)
      val client = ClientBuilder().name("http")
        .hosts(server.boundAddress.asInstanceOf[InetSocketAddress])
        .codec(com.twitter.finagle.http.Http()).reportTo(clientSr)
  ...
  test("HttpNack works with ServerBuilder") {
    new ClientCtx {
      ...
      val server = ServerBuilder()
        ...
        .bindTo(new InetSocketAddress(0)).name(serverLabel).reportTo(serverSr)
        .build(flakyService)
      val client = Http.client.configured(Stats(clientSr)).newService(
        Name
          .bound(Address(server.boundAddress.asInstanceOf[InetSocketAddress])),
  ...
  test("a server that doesn't support HttpNack fails the request") {
    new ClientCtx {
      val server = Http.server.withStack(StackServer.newStack)
        .configured(Stats(serverSr)).configured(Label("myservice"))
        .serve(new InetSocketAddress(0), flakyService)
