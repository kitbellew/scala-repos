...
object Bijections {
  ...
  implicit val methodToNetty =
    new Injection[Method, HttpMethod] {
      import HttpMethod._
      import Method._
  ...
  implicit val methodFromNetty =
    new Injection[HttpMethod, Method] {
      import HttpMethod._
      import Method._
  ...
  implicit val statusToNetty =
    new Injection[Status, HttpResponseStatus] {
      def apply(s: Status) = HttpResponseStatus.valueOf(s.code)
    }

  implicit val statusFromNetty =
    new Injection[HttpResponseStatus, Status] {
      def apply(s: HttpResponseStatus) = Status.fromCode(s.getCode)
    }

  // Request

  implicit val requestToNetty =
    new Injection[Request, HttpRequest] {
      def apply(r: Request): HttpRequest = r.httpRequest
    }

  implicit val requestFromNetty =
    new Injection[HttpRequest, Request] {
  ...
  implicit val responseFromNetty =
    new Injection[HttpResponse, Response] {
      def apply(r: HttpResponse): Response = Response(r)
    }

  implicit val responseToNetty =
    new Injection[Response, HttpResponse] {
