...
class ConnectionManager {
  ...
  def observeMessage(message: Any) =
    synchronized {
      message match {
        ...
        case chunk: HttpChunk =>
          observeChunk(chunk)
        case _ =>
          isKeepAlive = false // conservative
      }
    }

  def observeRequest(request: HttpRequest) =
    synchronized {
      isIdle = false
  ...
  def observeResponse(response: HttpResponse) =
    synchronized {
      if (!response.isChunked && !response
            .headers
            .contains(HttpHeaders.Names.CONTENT_LENGTH))
        isKeepAlive = false
  ...
  def observeChunk(chunk: HttpChunk) =
    synchronized {
      require(!isIdle)
      if (chunk.isLast)
        chunks -= 1
