...
class CookieMap(message: Message)
  ...
  private[this] val underlying = mutable.Map[String, Set[Cookie]]()
  ...
  private[this] def decodeCookies(header: String): Iterable[Cookie] = {
    ...
    try { decoder.decode(header).asScala map { new Cookie(_) } }
    catch {
  ...
  protected def rewriteCookieHeaders() {
    ...
    if (message.isRequest) {
      ...
      foreach { case (_, cookie) => encoder.addCookie(cookie.underlying) }
  ...
  /**
    ...
    */
  override def foreach[U](f: ((String, Cookie)) => U): Unit =
    iterator.foreach(f)
  ...
  /**
    ...
    */
  def +=(cookie: Cookie): CookieMap = { this += ((cookie.name, cookie)) }
  ...
  /**
    ...
    * @param cookie the ''Cookie'' to add
    */
  def add(cookie: Cookie) { add(cookie.name, cookie) }
