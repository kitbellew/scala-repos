...
private[finagle] trait HttpMessageProxy extends Proxy {
  ...
  protected[finagle] def headers(): HttpHeaders = httpMessage.headers()

  protected[finagle] def getContent(): ChannelBuffer = httpMessage.getContent()
  ...
  def setChunked(chunked: Boolean): Unit = httpMessage.setChunked(chunked)
}
