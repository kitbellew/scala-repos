...
class StabilizerTest extends FunSuite {
  ...
  test("Pending resolutions don't tick out successful results")(
    new Ctx {
      setVa(Addr.Bound(addr1))
      assertStabilized(Addr.Bound(addr1))
      setVa(Addr.Bound(addr2))
  ...
  test("Removes are delayed while failures are observed")(
    new Ctx {
  ...
  test("Reflect additions while addrs are unstable")(
    new Ctx {
      setVa(Addr.Bound(addr1, addr2))
      assertStabilized(Addr.Bound(addr1, addr2))
  ...
  test("Merge WeightedSocketAddresses")(
    new Ctx {
  ...
  test("Adds and removes are batched by batchEpoch")(
    new Ctx {
  ...
  test("First update does not wait for epoch to turn")(
    new Ctx {
