...
class ZkSessionTest extends FunSuite with Eventually with IntegrationPatience {
  ...
  test("ops retry safely") {
    Time.withCurrentTimeFrozen { tc =>
      ...
      watchedZk.value.opq(0).res() = Throw(
        new KeeperException.ConnectionLoss(None))
      assert(timer.tasks.size == 1)
      tc.advance(20.milliseconds)
      timer.tick()
      assert(
        watchedZk.value.opq ==
          Seq(ExistsWatch("/foo/bar"), ExistsWatch("/foo/bar")))
      assert(ref.get == Activity.Pending)

      watchedZk.value.opq(1).res() = Throw(
        new KeeperException.SessionExpired(None))
      assert(
        watchedZk.value.opq ==
          Seq(ExistsWatch("/foo/bar"), ExistsWatch("/foo/bar")))
  ...
  test("factory authenticates and closes on expiry") {
    Time.withCurrentTimeFrozen { tc =>
      ...
      implicit val timer = new MockTimer
      val zkState: Var[WatchState] with Updatable[WatchState] = Var(
        WatchState.Pending)
      ...
      eventually {
        assert(
          watchedZk.value.opq ==
            Seq(AddAuthInfo("digest", Buf.Utf8(authInfo)), Close(Time.Bottom)))
      }
