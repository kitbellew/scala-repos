...
class Zk2Resolver(
  ...
  private[this] val discoverers = Memoize.snappable[String, ServiceDiscoverer] {
    hosts =>
  ...
  private[this] val addrOf_ =
    Memoize[(ServiceDiscoverer, String, Option[String]), Var[Addr]] {
      case (discoverer, path, endpointOption) =>
        ...
        scoped.provideGauge("limbo") {
          nlimbo
        }
        scoped.provideGauge("size") {
          size
        }
        ...
        val stabilizedVa = Var.async(Addr.Pending: Addr) { u =>
          ...
          val reg =
            (discoverer.health.changes joinLast states).register(Witness {
              tuple =>
