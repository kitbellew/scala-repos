...
private[serverset2] object Stabilizer {
  ...
  private val notifyMs = FinagleStatsReceiver.scope("serverset2")
    .scope("stabilizer").stat("notify_ms")
  ...
  def apply(va: Var[Addr], removalEpoch: Epoch, batchEpoch: Epoch): Var[Addr] =
    Var.async(Addr.Pending: Addr) { u =>
      ...
      val batchedUpdates = (addrs select batchEpoch.event).foldLeft(init) {
        case (st, ev) =>
          val now = Time.now
          ev match {
            case Left(newAddr) =>
