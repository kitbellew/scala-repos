...
case class ZCount(key: ChannelBuffer, min: ZInterval, max: ZInterval)
    extends StrictKeyCommand {
...
case class ZRem(key: ChannelBuffer, members: Seq[ChannelBuffer])
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.ZREM, key) ++ members)
}
object ZRem {
  def apply(args: Seq[Array[Byte]]) = {
    RequireClientProtocol(
...
case class ZRemRangeByRank(key: ChannelBuffer, start: Long, stop: Long)
    extends StrictKeyCommand {
...
case class ZRevRank(key: ChannelBuffer, member: ChannelBuffer)
    extends ZRankCmd {
  ...
  override def validate() {
    ...
    withScores.map { s =>
      s match {
        case WithScores =>
        case _ =>
          throw ClientError("withScores must be an instance of WithScores")
      }
    }
...
trait ZRangeCmdCompanion {
  def get(
      ...
      start: Long,
      stop: Long,
      withScores: Option[CommandArgument]): ZRangeCmd
  ...
  protected def safeInt(i: String) =
    RequireClientProtocol.safe { NumberFormat.toInt(i) }

  protected def safeDouble(i: String) =
    RequireClientProtocol.safe { NumberFormat.toDouble(i) }

  protected def safeLong(i: String) =
    RequireClientProtocol.safe { NumberFormat.toLong(i) }
}
...
abstract class ZRankCmd extends StrictKeyCommand with StrictMemberCommand {
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(this.commandBytes, key, member))
}
