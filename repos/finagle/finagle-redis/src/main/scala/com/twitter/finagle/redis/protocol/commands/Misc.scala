...
object Info {
  def apply(section: Seq[Array[Byte]]) = {
    new Info(section.headOption.map { ChannelBuffers.wrappedBuffer }.getOrElse(
...
case class ConfigSet(param: ChannelBuffer, value: ChannelBuffer)
    extends Config(ConfigSet.channelBuffer, Seq(param, value))
...
case class ConfigGet(param: ChannelBuffer)
    extends Config(ConfigGet.channelBuffer, Seq(param))
...
case class ConfigResetStat()
    extends Config(sub = ConfigResetStat.channelBuffer, args = Seq())
...
abstract class Config(sub: ChannelBuffer, args: Seq[ChannelBuffer])
    extends Command {
  def command = Commands.CONFIG
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.CONFIG, sub) ++ args)
...
case class SlaveOf(host: ChannelBuffer, port: ChannelBuffer) extends Command {
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.SLAVEOF, host, port))
}
