...
case class SAdd(key: ChannelBuffer, values: Seq[ChannelBuffer])
...
case class SRem(key: ChannelBuffer, values: List[ChannelBuffer])
    extends StrictKeyCommand {
...
case class SInter(keys: Seq[ChannelBuffer]) extends StrictKeysCommand {
  ...
  override def toChannelBuffer =
    RedisCodec.toUnifiedFormat(CommandBytes.SINTER +: keys)
}
