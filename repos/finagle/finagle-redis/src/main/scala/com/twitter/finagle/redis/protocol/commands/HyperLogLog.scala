...
case class PFAdd(key: ChannelBuffer, elements: Seq[ChannelBuffer])
    extends StrictKeyCommand {
  val command = Commands.PFADD

  override def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.PFADD, key) ++ elements)
}
...
case class PFCount(keys: Seq[ChannelBuffer]) extends StrictKeysCommand {
  ...
  override def toChannelBuffer =
    RedisCodec.toUnifiedFormat(CommandBytes.PFCOUNT +: keys)
}

object PFCount {
  def apply(args: => Seq[Array[Byte]]): PFCount = PFCount(
    args map ChannelBuffers.wrappedBuffer)
}
...
case class PFMerge(destKey: ChannelBuffer, srcKeys: Seq[ChannelBuffer])
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.PFMERGE, destKey) ++ srcKeys)
}
