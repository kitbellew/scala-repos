...
case class BAdd(key: ChannelBuffer, field: ChannelBuffer, value: ChannelBuffer)
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.BADD, key, field, value))
}
...
case class BRem(key: ChannelBuffer, fields: Seq[ChannelBuffer])
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.BREM, key) ++ fields)
}
...
case class BGet(key: ChannelBuffer, field: ChannelBuffer)
    extends StrictKeyCommand {
  def command = Commands.BGET
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.BGET, key, field))
}
...
case class BRange(
  ...
  val request: Seq[ChannelBuffer] =
    if (startField.isEmpty && endField.isEmpty)
      ...
      Seq(key, StringToChannelBuffer("end"), endField.get)
    else
      Seq(key, StringToChannelBuffer("startend"), startField.get, endField.get)
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.BRANGE) ++ request)
}
