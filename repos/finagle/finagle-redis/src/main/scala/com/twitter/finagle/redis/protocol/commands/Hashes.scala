...
case class HDel(key: ChannelBuffer, fields: Seq[ChannelBuffer])
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HDEL, key) ++ fields)
}
...
case class HExists(key: ChannelBuffer, field: ChannelBuffer)
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HEXISTS, key, field))
}
...
case class HGet(key: ChannelBuffer, field: ChannelBuffer)
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HGET, key, field))
}
...
case class HGetAll(key: ChannelBuffer) extends StrictKeyCommand {
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HGETALL, key))
}
...
case class HKeys(key: ChannelBuffer) extends StrictKeyCommand {
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HKEYS, key))
}
...
case class HMGet(key: ChannelBuffer, fields: Seq[ChannelBuffer])
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HMGET, key) ++ fields)
}

case class HMSet(key: ChannelBuffer, fv: Map[ChannelBuffer, ChannelBuffer])
    extends StrictKeyCommand {
...
case class HSet(key: ChannelBuffer, field: ChannelBuffer, value: ChannelBuffer)
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HSET, key, field, value))
}
...
case class HSetNx(
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HSETNX, key, field, value))
}
...
case class HVals(key: ChannelBuffer) extends StrictKeyCommand {
  def command = Commands.HVALS
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.HVALS, key))
}
