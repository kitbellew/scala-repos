...
case class GetBit(key: ChannelBuffer, offset: Int) extends StrictKeyCommand {
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(
      Seq(CommandBytes.GETBIT, key, StringToChannelBuffer(offset.toString)))
}
object GetBit {
  def apply(args: Seq[Array[Byte]]) = {
...
case class GetRange(key: ChannelBuffer, start: Long, end: Long)
    extends StrictKeyCommand {
...
object GetRange {
  def apply(args: Seq[Array[Byte]]) = {
    ...
    val end = RequireClientProtocol.safe {
      NumberFormat.toLong(BytesToString(list(2)))
    }
...
case class SetBit(key: ChannelBuffer, offset: Int, value: Int)
    extends StrictKeyCommand {
...
case class Strlen(key: ChannelBuffer) extends StrictKeyCommand {
  ...
  def toChannelBuffer =
    RedisCodec.toUnifiedFormat(Seq(CommandBytes.STRLEN, key))
}
object Strlen {
  def apply(args: Seq[Array[Byte]]) = {
