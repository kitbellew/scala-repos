...
final class SetClientIntegrationSuite extends RedisClientTest {
  ...
  test(
    "Correctly add, then pop members from a set while counting them",
    RedisTest,
    ClientTest) {
    withRedisClient { client =>
  ...
  test(
    ...
    withRedisClient { client =>
      assert(
        Await.result(client.sAdd(key, List(foo))) == oneElemAdded,
        oneElemAddErrorMessage)
      assert(
        Await.result(client.sIsMember(key, foo)) == true,
        "Foo was not a member of the set")

      assert(
        Await.result(client.sIsMember(key, baz)) == false,
        "Baz was found in the set")
      assert(
        Await.result(client.sAdd(key, List(baz))) == oneElemAdded,
        oneElemAddErrorMessage)
      assert(
        Await.result(client.sIsMember(key, baz)) == true,
        "Baz was not found in the set")
      assert(
        Await.result(client.sIsMember(key, foo)) == true,
        "Foo was not a member of the set")
      ...
      assert(
        ...
        "Baz was found in the set")
      assert(
        Await.result(client.sIsMember(key, foo)) == false,
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      val strings: CollectionSet[String] = Await.result(client.sMembers(key))
      ...
      Await.result(client.sPop(key))
      assert(
        Await.result(client.sMembers(key)) == CollectionSet(),
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await.result(client.sRem(key, List(moo))) == 1,
        "Could not remove one Element")
      assert(
        Await.result(client.sRem(key, List(boo))) == 1,
        "Could not remove one Element")
      assert(
        Await.result(client.sRem(key, List(moo))) == 0,
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      allMembers.foreach(m => {
        assert(
          Await.result(client.sAdd(key, List(m))) == oneElemAdded,
          oneElemAddErrorMessage)
      })
      ...
      assert(
        oneMember.size == 1,
        "The one member set had an incorrect number of members")
      assert(allMembers.contains(oneMember.head) == true)

      val twoMembers = Await.result(client.sRandMember(key, count = Some(2)))
      assert(
        twoMembers.size == 2,
        "The two member set had an incorrect number of members")
      assert(
        twoMembers.forall(allMembers.contains(_)) == true,
        "The two member set did not " + "match the original Sequence")
      ...
      assert(
        ...
        "The set members did not match " + "the original Sequence")
