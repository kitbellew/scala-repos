...
final class ListClientIntegrationSuite extends RedisClientTest {
  ...
  test("Correctly pop elements off a list", RedisTest, ClientTest) {
    withRedisClient { client =>
      ...
      assert(Await.result(client.lPush(foo, List(baz))) == 2)
      assert(
        Await.result(client.lPop(foo)).getOrElse(
          fail(PopFailureMessage)) == baz)
      assert(
        Await.result(client.lPop(foo)).getOrElse(
          fail(PopFailureMessage)) == bar)
    }
  }
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await.result(client.lPop(key)).getOrElse(fail(PopFailureMessage)) ==
          string2ChanBuf("bar"))
      assert(Await.result(client.lLen(key)) == 0)
    }
  }
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await.result(client.lIndex(key, 0)).getOrElse(
          fail(IndexFailureMessage)) == bar)
      assert(Await.result(client.lPush(key, List(baz))) == 2)
      assert(
        Await.result(client.lIndex(key, 0)).getOrElse(
          fail(IndexFailureMessage)) == baz)
      assert(
        Await.result(client.lPop(key)).getOrElse(
          fail(PopFailureMessage)) == baz)
      assert(
        Await.result(client.lIndex(key, 0)).getOrElse(
          fail(IndexFailureMessage)) == bar)
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(
        Await.result(client.lInsertAfter(key, moo, bar)).getOrElse(
          fail(PivotFailureMessage)) == 2)
      assert(
        Await.result(client.lInsertBefore(key, moo, foo)).getOrElse(
          fail(PivotFailureMessage)) == 3)
      assert(
        Await.result(client.lPop(key)).getOrElse(
          fail(PopFailureMessage)) == foo)
      assert(
        Await.result(client.lPop(key)).getOrElse(
          fail(PopFailureMessage)) == moo)
      assert(
        Await.result(client.lPop(key)).getOrElse(
          fail(PopFailureMessage)) == bar)
    }
  ...
  test(
    ...
    withRedisClient { client =>
      ...
      assert(Await.result(client.lPush(key, List(baz))) == 2)
      assert(
        Await.result(client.rPop(key)).getOrElse(
          fail(PopFailureMessage)) == bar)
      assert(
        Await.result(client.rPop(key)).getOrElse(
          fail(PopFailureMessage)) == baz)
