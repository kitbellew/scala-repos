...
final class StringClientIntegrationSuite extends RedisClientTest {
  ...
  test(
    "Correctly perform mGet, mSet, and mSetNx commands",
    RedisTest,
    ClientTest) {
    withRedisClient { client =>
      Await.result(client.mSet(Map(foo -> bar, bar -> baz)))
      assert(
        Await.result(client.mGet(Seq(foo, bar, baz))) ==
          Seq(Some(bar), Some(baz), None))
      assert(
        Await.result(client.mSetNx(Map(foo -> bar, baz -> foo, boo -> moo))) ==
          false)
