...
final class SortedSetClientIntegrationSuite extends RedisClientTest {
  ...
  test("Correctly get the zRangeByScore", RedisTest, ClientTest) {
    withRedisClient { client =>
      ...
      for (left <- Await.result(client.zRangeByScore(
             ...
             ZInterval(0),
             true,
             Some(Limit(0, 5)))).left) { assert(left.asTuples == Seq()) }
    }
  }
  ...
  test("Correctly get zRevRangeByScore", RedisTest, ClientTest) {
    withRedisClient { client =>
      assert(Await.result(client.zAdd(foo, 10, bar)) == 1)
      assert(Await.result(client.zAdd(foo, 20, baz)) == 1)
      for (left <- Await.result(client.zRevRangeByScore(
             foo,
             ZInterval(10),
             ZInterval(0),
             true,
      ...
      for (left <- Await.result(client.zRevRangeByScore(
             ...
             ZInterval(10),
             true,
             Some(Limit(0, 1)))).left) { assert(left.asTuples == Seq()) }
      for (left <- Await.result(client.zRevRangeByScore(
             foo,
             ZInterval(0),
             ZInterval(0),
  ...
  test(
    "Correctly add members and zIncr, then zIncr a nonmember",
    RedisTest,
    ClientTest) {
    withRedisClient { client =>
      assert(Await.result(client.zAdd(foo, 10, bar)) == 1)
