...
final class SortedSetCodecSuite extends RedisRequestTest {
    ...
    unwrap(codec(wrap("ZADD nums 3.14159 pi 2.71828 e\r\n"))) {
      case ZAdd(nums, members) =>
        members.toList match {
          ...
          case _ =>
            fail("Expected two elements in list")
        }
    }
  }
  ...
  test(
    "Throw a ClientError for ZINTERSTORE and ZUNIONSTORE with invalid arguments",
    CodecTest) {
    val bad = List(
      "%s",
      "%s foo",
      "%s foo 1 a b",
      "%s foo 2 a b WEIGHTS 2",
      "%s foo 2 a b WEIGHTS 1",
      "%s foo 2 a b WEIGHTS 2 2 AGGREGATE foo",
      "%s foo 2 a b c WEIGHTS 2 2 2",
      "%s foo 1 b WEIGHTS 2 AGGREGATE",
  ...
  private def verifyIU(cmd: String, k: String, n: Int)(
    ...
    cmd match {
      case "ZINTERSTORE" => {
        case ZInterStore(k, n, keys, w, a) =>
          f(keys, w, a)
      }
      case "ZUNIONSTORE" => {
        case ZUnionStore(k, n, keys, w, a) =>
          f(keys, w, a)
      }
      case _ =>
  ...
  private def testZRangeBytes(key: Array[Byte]) = {
    ...
    val keyBack =
      zrange match {
        case ZRange(key, start, stop, withScores) =>
          key
      }
  ...
  test(
    ...
    val bad = List(
      ...
      "%s key 0 1 NOSCORES",
      "%s key 0 1 LIMOT 1 2",
      "%s key 0 1 LIMIT foo 1",
      "%s key 0 1 LIMIT 1 foo",
      "%s key 0 1 WITHSCORES WITHSCORES",
  ...
  def verifyRangeByScore(
    ...
    cmd match {
      case "ZRANGEBYSCORE" => {
        case ZRangeByScore(k, min, max, s, l) =>
          f(s, l)
      }
      case "ZREVRANGEBYSCORE" => {
        case ZRevRangeByScore(k, min, max, s, l) =>
          f(s, l)
      }
    }
  ...
  test("Correctly encode ZRANGEBYSCORE and ZREVRANGEBYSCORE from 1 to 2") {
    List("ZRANGEBYSCORE", "ZREVRANGEBYSCORE").foreach { cmd =>
      unwrap(doCmd(cmd, "%s myzset 1 2\r\n")) {
        verifyRangeByScore(cmd, "myzset", ZInterval(1), ZInterval(2)) {
          (s, l) =>
            assert(s == None)
            assert(l == None)
        }
