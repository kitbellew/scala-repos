...
final class SortedSetClientServerIntegrationSuite
  ...
  test("ZADD should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      assert(
        Await.result(client(ZAdd("zadd1", List(ZMember(1, "one"))))) ==
          IntegerReply(1))
      assert(
        Await.result(client(ZAdd("zadd1", List(ZMember(2, "two"))))) ==
          IntegerReply(1))
      assert(
        Await.result(client(ZAdd("zadd1", List(ZMember(3, "two"))))) ==
          IntegerReply(0))
  ...
  test("ZCOUNT should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      initialize(client)
      assert(
        Await.result(client(ZCount(ZKEY, ZInterval.MIN, ZInterval.MAX))) ==
          IntegerReply(3))
      assert(
        Await
  ...
  test(
    "ZINTERSTORE and ZUNIONSTORE should work correctly",
    ClientServerTest,
    RedisTest) {
    withRedisClient { client =>
      val key = "zstore1"
  ...
  test(
    "ZRANGE and ZREVRANGE should work correctly",
    ClientServerTest,
    RedisTest) {
    withRedisClient { client =>
      zAdd(
      ...
      assertMBulkReply(
        ...
        List("three", "two", "one"))
      assertMBulkReply(client(ZRevRange("zrange1", 2, 3)), List("one"))
      assertMBulkReply(client(ZRevRange("zrange1", -2, -1)), List("two", "one"))
    }
  }

  test(
    "ZRANGEBYSCORE and ZREVRANGEBYSCORE should work correctly",
    ClientServerTest,
    RedisTest) {
    withRedisClient { client =>
      ...
      assertMBulkReply(
        client(
          ZRangeByScore(key, ZInterval.exclusive(1f), ZInterval.exclusive(2f))),
        List())
      assertMBulkReply(
  ...
  test(
    "ZRANK and ZREVRANK should work correctly",
    ClientServerTest,
    RedisTest) {
    withRedisClient { client =>
      val key = "zrank1"
  ...
  test("ZREM should work correctly", ClientServerTest, RedisTest) {
    withRedisClient { client =>
      ...
      assert(
        Await.result(client(ZRem(key, List("nosuchmember")))) ==
          IntegerReply(0))
      assertMBulkReply(
