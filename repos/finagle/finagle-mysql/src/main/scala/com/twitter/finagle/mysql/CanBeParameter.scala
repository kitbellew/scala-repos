...
object CanBeParameter {
  ...
  implicit val valueCanBeParameter = {
    new CanBeParameter[Value] {
      ...
      def typeCode(param: Value) =
        param match {
          case RawValue(typ, _, _, _) =>
            typ
          case StringValue(_) =>
            Type.VarChar
          case ByteValue(_) =>
      ...
      def write(writer: BufferWriter, param: Value) =
        param match {
          // allows for generic binary values as params to a prepared statement.
          case RawValue(_, _, true, bytes) =>
          ...
          case StringValue(s) =>
            writer.writeLengthCodedString(s)
          case _ =>
            ()
        }
    }
