...
class NumericTypeTest extends FunSuite with IntegrationClient {
  for (c <- client) {
    ...
    val textEncoded = Await.result(
      c.query("SELECT * FROM `numeric`") map {
        case rs: ResultSet if rs.rows.size > 0 =>
          rs.rows(0)
        case v =>
  ...
  def testRow(row: Row) {
    ...
    test("extract %s from %s".format("tinyint", rowType)) {
      row("tinyint") match {
        case Some(ByteValue(b)) =>
          assert(b == 2)
        case v =>
          fail("expected ByteValue but got %s".format(v))
      }
    }

    test("extract %s from %s".format("smallint", rowType)) {
      row("smallint") match {
        case Some(ShortValue(s)) =>
          assert(s == 1)
        case v =>
          fail("expected ShortValue but got %s".format(v))
      }
    }

    test("extract %s from %s".format("mediumint", rowType)) {
      row("mediumint") match {
        case Some(IntValue(i)) =>
          assert(i == 3)
        case v =>
          fail("expected IntValue but got %s".format(v))
      }
    }
    ...
    test("extract %s from %s".format("bigint", rowType)) {
      row("bigint") match {
        case Some(LongValue(l)) =>
          assert(l == 5)
        case v =>
          fail("expected LongValue but got %s".format(v))
      }
    }
    ...
    test("extract %s from %s".format("float", rowType)) {
      row("float") match {
        ...
        case v =>
          fail("expected FloatValue but got %s".format(v))
      }
    }
    ...
    test("extract %s from %s".format("double", rowType)) {
      row("double") match {
        ...
        case v =>
          fail("expected DoubleValue but got %s".format(v))
      }
    }

    test("extract %s from %s".format("decimal", rowType)) {
      row("decimal") match {
        case Some(BigDecimalValue(bd)) =>
          assert(bd == BigDecimal(1.61803398875))
        case v =>
          fail("expected BigDecimalValue but got %s".format(v))
      }
    }
    ...
    test("extract %s from %s".format("bit", rowType)) {
      row("bit") match {
        ...
        case v =>
          fail("expected a RawValue but got %s".format(v))
      }
    }
  }
}
...
class BlobTypeTest extends FunSuite with IntegrationClient {
  ...
  def testRow(row: Row) {
    ...
    test("extract %s from %s".format("char", rowType)) {
      row("char") match {
        case Some(StringValue(s)) =>
          assert(s == "a")
        case a =>
          fail("Expected StringValue but got %s".format(a))
      }
    }
    ...
    test("extract %s from %s".format("tinyblob", rowType)) {
      row("tinyblob") match {
        case Some(RawValue(_, _, _, bytes)) =>
          assert(bytes.toList == List(0x66))
        case a =>
          fail("Expected RawValue but got %s".format(a))
      }
    }
    ...
    test("extract %s from %s".format("binary", rowType)) {
      row("binary") match {
        case Some(RawValue(_, _, _, bytes)) =>
          assert(bytes.toList == List(0x69, 0x70))
        case a =>
          fail("Expected RawValue but got %s".format(a))
      }
    }
...
class DateTimeTypeTest extends FunSuite with IntegrationClient {
  ...
  def testRow(row: Row) {
    ...
    test("extract %s from %s".format("date", rowType)) {
      row("date") match {
        case Some(DateValue(d)) =>
          assert(d.toString() == "2013-11-02")
        case a =>
          fail("Expected DateValue but got %s".format(a))
      }
    }
      ...
      test("extract %s from %s in local time".format(repr, rowType)) {
        row(repr) match {
          case Some(timestampValueLocal(t)) =>
            ...
            assert(t == timestamp)
          case a =>
            fail("Expected TimestampValue but got %s".format(a))
        }
      }
      ...
      test("extract %s from %s in UTC".format(repr, rowType)) {
        row(repr) match {
          case Some(timestampValueUTC(t)) =>
            ...
            val timestamp =
              new Timestamp(format.parse("2013-11-02 19:56:" + secs).getTime)
            assert(t == timestamp)
          case a =>
            fail("Expected TimestampValue but got %s".format(a))
        }
    ...
    test("extract %s from %s".format("year", rowType)) {
      row("year") match {
        case Some(ShortValue(s)) =>
          assert(s == 2013)
        case a =>
          fail("Expected ShortValue but got %s".format(a))
      }
    }
  }
}
