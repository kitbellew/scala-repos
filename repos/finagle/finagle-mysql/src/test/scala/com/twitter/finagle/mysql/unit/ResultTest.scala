...
trait HexDump {
  ...
  lazy val packets: Seq[Packet] = {
    ...
    val asBytes = tokens
      .map(_.map(s => ((s(0).asDigit << 4) + s(1).asDigit).toByte))
...
class HandshakeInitTest extends FunSuite {
  val authPluginHex =
    test("decode protocol version 10")(
      new HexDump {
        ...
        assert(
          h.salt === Array[Byte](
            100, 118, 72, 64, 73, 45, 67, 74, 42, 52, 100, 124, 99, 90, 119,
            107, 52, 94, 93, 58))
      })

  test("decode protocol version 10 with auth plugin name")(
    new HexDump {
