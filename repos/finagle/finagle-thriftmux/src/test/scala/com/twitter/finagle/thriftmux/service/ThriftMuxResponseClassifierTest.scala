...
class ThriftMuxResponseClassifierTest extends FunSuite {
  ...
  test("ThriftExceptionsAsFailures") {
    ...
    assert(
      "ThriftMux.usingDeserializeCtx(ThriftExceptionsAsFailures)" == classifier
        .toString())
  ...
  test(
    ...
    Contexts
      ...
      .let(DeserializeCtx.Key, ctx) {
        ...
        assert(
          ThriftMuxResponseClassifier.DeserializeCtxOnly.isDefinedAt(reqRep))
        assert(
          Success == ThriftMuxResponseClassifier.DeserializeCtxOnly(reqRep))
      }
  }
