...
class DarkTrafficFilter[Req, Rep](
  ...
  private[this] val requestsForwardedCounter =
    scopedStatsReceiver.counter("forwarded")
  ...
  override def apply(request: Req, service: Service[Req, Rep]): Future[Rep] = {
    if (forwardAfterService) {
      service(request).ensure { darkRequest(request) }
    } else {
  ...
  private[this] def darkRequest(request: Req): Unit = {
    ...
    } else { requestsSkippedCounter.incr() }
  }
}
