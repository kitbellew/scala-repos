...
class MigrationClientTest
  ...
  override def beforeEach() {
    ...
    // set-up old pool
    val oldPoolCluster = new ZookeeperServerSetCluster(ServerSets.create(
      zookeeperClient,
    ...
    // set-up new pool
    val newPoolCluster = new ZookeeperServerSetCluster(ServerSets.create(
      zookeeperClient,
      ZooKeeperUtils.OPEN_ACL_UNSAFE,
      newPoolPath))
    (0 to 1) foreach { _ =>
    ...
    val migrationDataArray = MigrationConstants.jsonMapper
      .writeValueAsString(migrationConfig).getBytes()
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1731
    test("dark read w/ read repair") {
      ...
      val Buf.Utf8(mcRes) = Await.result(migrationClient.get("foo"), TIMEOUT)
        .get
      assert(mcRes == "bar")
