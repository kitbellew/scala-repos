...
class MemcachedTest
  ...
  test("Memcached.Client has expected stack and params") {
    ...
    assert(
      markDeadFor.take(10).force.toSeq ===
        (0 until 10 map { _ => 1.second }))
    assert(
      params[Transporter.ConnectTimeout] ==
        Transporter.ConnectTimeout(100.milliseconds))
    assert(
      params[Memcached.param.EjectFailedHost] ==
        Memcached.param.EjectFailedHost(false))
  ...
  test("Memcache.newPartitionedClient enables FactoryToService") {
    ...
    val client = Memcached.client.configured(Stats(st))
