...
import com.twitter.finagle.memcached.{CasResult, KetamaClientBuilder}
...
class ReplicationClientTest extends FunSuite with BeforeAndAfterEach {
  ...
  override def beforeEach() {
    ...
    val firstPoolCluster = new ZookeeperServerSetCluster(ServerSets.create(
      zookeeperClient,
    ...
    val secondPoolCluster = new ZookeeperServerSetCluster(ServerSets.create(
      zookeeperClient,
      ZooKeeperUtils.EVERYONE_READ_CREATOR_ALL,
      secondPoolPath))
    (0 to 1) foreach { _ =>
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client set & getOne") {
      ...
      val mycluster1 = CachePoolCluster
        .newZkCluster(firstPoolPath, zookeeperClient)
      Await
        .result(
          mycluster1.ready
        ) // give it sometime for the cluster to get the initial set of memberships
      val mycluster2 = CachePoolCluster
        .newZkCluster(secondPoolPath, zookeeperClient)
      Await
        .result(
          mycluster2.ready
        ) // give it sometime for the cluster to get the initial set of memberships

      val client1 = KetamaClientBuilder().clientBuilder(
        ClientBuilder().hostConnectionLimit(1).codec(Memcached())
          .failFast(false)).group(Group.fromCluster(mycluster1))
        .failureAccrualParams(Int.MaxValue, 0.seconds).build()
      val client2 = KetamaClientBuilder().clientBuilder(
        ClientBuilder().hostConnectionLimit(1).codec(Memcached())
          .failFast(false)).group(Group.fromCluster(mycluster2))
        .failureAccrualParams(Int.MaxValue, 0.seconds).build()
      ...
      intercept[WriteException] { Await.result(replicatedClient.getOne("foo")) }
    }
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client delete") {
      ...
      assert(
        Await
          .result(
            replicatedClient.delete("empty-key")) == ConsistentReplication(
          false))
      ...
      assert(Await.result(replicatedClient.delete("client2-only")) match {
        case InconsistentReplication(
              Seq(Return(JBoolean.FALSE), Return(JBoolean.TRUE))) => true
        case _                                                    => false
      })
    ...
    test("base replication client getsAll & cas") {
      ...
      assert(
        Await.result(replicatedClient.checkAndSet(
          ...
          case InconsistentReplication(
                Seq(Throw(_), Return(CasResult.NotFound))) => true
          case _                                           => false
        })
      Await.result(client1.set("foo", Buf.Utf8("bar")))
      assert(
      ...
      assert(
        Await.result(replicatedClient.checkAndSet(
          ...
          case InconsistentReplication(
                Seq(Throw(_), Return(CasResult.Stored))) => true
          case _                                         => false
        })
    ...
    test("base replication client add & replace") {
        ...
        Await
          .result(
            replicatedClient
              .replace(
                "no-such-key",
                Buf.Utf8("test"))) == ConsistentReplication(false))
      ...
      assert(
        Await
          ...
          case InconsistentReplication(
                Seq(Return(JBoolean.TRUE), Return(JBoolean.FALSE))) => true
          case _                                                    => false
        })
      assert(
        Await
    ...
    test("base replication client incr & decr") {
      ...
      assert(
        Await
          .result(
            replicatedClient
              .set("foo", Buf.Utf8("1"))) == ConsistentReplication(()))
      assert(
        Await.result(replicatedClient.getAll("foo")) == ConsistentReplication(
          Some(Buf.Utf8("1"))))
      assert(
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client many keys") {
      ...
      (0 until count).foreach { n =>
        {
          val ConsistentReplication(Some(Buf.Utf8(res))) = Await
            .result(replicatedClient.getAll("foo" + n))
          assert(res == "bar" + n)
        }
      }
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client non supported operation") {
      ...
      intercept[UnsupportedOperationException] {
        Await
          .result(replicatedClient.append("not-supported", Buf.Utf8("value")))
      }
      intercept[UnsupportedOperationException] {
        Await
          .result(replicatedClient.prepend("not-supported", Buf.Utf8("value")))
      }
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("simple replication client get & set") {
      ...
      intercept[WriteException] { Await.result(replicatedClient.get("foo")) }
    }
  ...
  if (Option(System.getProperty("USE_EXTERNAL_MEMCACHED")).isDefined)
    test("simple replication client gets & cas") {
      ...
      intercept[SimpleReplicationFailure] {
        Await
          .result(replicatedClient.cas("foo", Buf.Utf8("baz"), Buf.Utf8("2|3")))
      }
      intercept[SimpleReplicationFailure] {
    ...
    test("simple replication client add & replace") {
      ...
      assert(
        Await
          .result(
            replicatedClient
              .replace("client1-only", Buf.Utf8("test"))) == false)
