...
class ReplicationClientTest extends FunSuite with BeforeAndAfterEach {
  ...
  if (Option(System.getProperty("USE_EXTERNAL_MEMCACHED")).isDefined)
    test("base replication client getsAll & cas") {
      ...
      assert(
        Await.result(
          replicatedClient.getsAll("foo")) == InconsistentReplication(Seq(
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client incr & decr") {
      ...
      assert(
        Await.result(
          replicatedClient.incr("foo", 1)) == InconsistentReplication(
          Seq(Return(Some(4L)), Return(Some(3L)))))
      assert(Await.result(client2.decr("foo", 1)) == Some(2L))
      assert(
        Await.result(
          replicatedClient.decr("foo", 1)) == InconsistentReplication(
  ...
  if (Option(System.getProperty("USE_EXTERNAL_MEMCACHED")).isDefined)
    test("simple replication client gets & cas") {
      ...
      assert(
        Await.result(replicatedClient.cas(
          "foo",
          Buf.Utf8("baz"),
          Buf.Utf8("1|1"))) == true)
