...
class ReplicationClientTest extends FunSuite with BeforeAndAfterEach {
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client set & getOne") {
      ...
      assert(Await.result(
        replicatedClient.set("foo", Buf.Utf8("bar"))) == ConsistentReplication(
        ()))
      assert(
  ...
  if (Option(System.getProperty("USE_EXTERNAL_MEMCACHED")).isDefined)
    test("base replication client getsAll & cas") {
      ...
      assert(
        Await.result(
          replicatedClient.getsAll("foo")) == InconsistentReplication(
          Seq(
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1712
    test("base replication client add & replace") {
      ...
      // consistent
      assert(Await.result(
        replicatedClient.add("foo", Buf.Utf8("bar"))) == ConsistentReplication(
        true))
      assert(
        Await.result(replicatedClient.getAll("foo")) == ConsistentReplication(
          Some(Buf.Utf8("bar"))))
      ...
      assert(
        Await.result(
          replicatedClient.replace(
            "foo",
            Buf.Utf8("baz"))) == ConsistentReplication(true))
      assert(
        Await.result(replicatedClient.getAll("foo")) == ConsistentReplication(
          Some(Buf.Utf8("baz"))))
  ...
  if (!sys.props.contains("SKIP_FLAKY")) // CSL-1731
    test("base replication client replica down") {
      ...
      assert(Await.result(
        replicatedClient.set("foo", Buf.Utf8("baz"))) == ConsistentReplication(
        ()))
    }
