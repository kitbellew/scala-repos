...
trait BaseClient[T] {
  ...
  /**
    ...
    * @return true if stored, false if not stored
    */
  def replace(key: String, value: T): Future[JBoolean] =
...
protected class ConnectedClient(
    protected val service: Service[Command, Response])
    extends Client {
  import ClientConstants._
    ...
    service(command).map {
      ...
      case other =>
        throw new IllegalStateException(
          "Invalid response type from get: %s"
...
trait PartitionedClient extends Client {
  ...
  private[this] def withKeysGroupedByClient[A](keys: Iterable[String])(
    ...
    Future.collect(keys.groupBy(clientOf).map(Function.tupled(f)).toSeq)
  }
...
object KetamaClientKey {
  private[memcached] case class HostPortBasedKey(
      host: String,
      port: Int,
      weight: Int)
...
private[finagle] case class NodeMarkedDead(key: KetamaClientKey)
    extends NodeHealth
private[finagle] case class NodeRevived(key: KetamaClientKey) extends NodeHealth

class FailureAccrualException(message: String)
...
private[finagle] object KetamaFailureAccrualFactory {
  ...
  def module[Req, Rep](
      key: KetamaClientKey,
      healthBroker: Broker[NodeHealth]): Stackable[ServiceFactory[Req, Rep]] =
    new Stack.Module5[
...
private[finagle] class KetamaPartitionedClient(
  ...
  private[this] var nodes = mutable.Map[KetamaClientKey, Node]() ++ {
    ketamaNodeSnap
      .toMap
      .mapValues { kn: KetamaNode[Client] =>
  ...
  private[this] val deadNodeGauge =
...
case class KetamaClientBuilder private[memcached] (
  ...
  def dest(
      name: Name,
      useOnlyResolvedAddress: Boolean = false): KetamaClientBuilder = {
    val Name.Bound(va) =
  ...
  def failureAccrualParams(
      numFailures: Int,
      markDeadFor: Duration): KetamaClientBuilder =
...
object KetamaClientBuilder {
  def apply(): KetamaClientBuilder =
    KetamaClientBuilder(Group.empty, Some("ketama"), None)
  def get() = apply()
}
...
case class RubyMemCacheClientBuilder(
  ...
  def nodes(hostPortWeights: String): RubyMemCacheClientBuilder =
    copy(_nodes = CacheNodeGroup(hostPortWeights)
      .members
      .map { node: CacheNode =>
...
object PHPMemCacheClientBuilder {
  def apply(): PHPMemCacheClientBuilder =
    PHPMemCacheClientBuilder(Nil, Some("crc32-itu"), None)
  def get() = apply()
}
