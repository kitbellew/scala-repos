...
trait KeyValidation {
  ...
  def badKey(key: Buf): Boolean = {
    if (key == null) true else { tooLong(key) || invalidByteIndex(key) != -1 }
  }
}
...
// arithmetic commands
case class Incr(key: Buf, value: Long)
    extends ArithmeticCommand(key, value, "Incr")
case class Decr(key: Buf, value: Long)
