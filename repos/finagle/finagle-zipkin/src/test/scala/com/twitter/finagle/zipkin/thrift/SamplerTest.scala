...
class SamplerTest extends FunSuite with MockitoSugar {
  ...
  test("Sampler should drop all by setting samplerate") {
    ...
    for (i <- 1 until 100) {
      assert(
        sampler.sampleTrace(TraceId(None, None, SpanId(i), None)) ==
          Some(false))
    }
  }
  ...
  test("Sampler should drop none by setting samplerate") {
    ...
    for (i <- 1 until 100) {
      assert(
        sampler.sampleTrace(TraceId(None, None, SpanId(i), None)) == Some(true))
    }
  }
  ...
  test("Sampler should drop all by providing samplerate") {
    ...
    for (i <- 1 until 100) {
      assert(
        sampler.sampleTrace(TraceId(None, None, SpanId(i), None), 0f) ==
          Some(false))
    }
  }
  ...
  test("Sampler should drop none by providing samplerate") {
    ...
    for (i <- 1 until 100) {
      assert(
        sampler.sampleTrace(TraceId(None, None, SpanId(i), None), 1f) ==
          Some(true))
    }
  }
  ...
  test("Sampler should sample record if sample is none and sample rate 1") {
    ...
    sampler.setSampleRate(1f)
    assert(
      sampler.sampleRecord(Record(traceId, Time.now, Annotation.ClientSend())))
  }
}
