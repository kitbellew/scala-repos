...
private[finagle] object TrafficDistributor {
  ...
  /**
    ...
    */
  private def safelyScanLeft[T, U](init: U, stream: Event[Activity.State[T]])(
    ...
    stream.foldLeft(initState) {
      ...
      case (Activity.Failed(_), Activity.Ok(update)) =>
        Activity.Ok(f(init, update))
      case (Activity.Ok(state), Activity.Ok(update)) =>
        Activity.Ok(f(state, update))
      case (stale @ Activity.Ok(state), Activity.Failed(_)) if init != state =>
        stale
...
private[finagle] class TrafficDistributor[Req, Rep](
  ...
  /**
    ...
    */
  private[this] def weightEndpoints(addrs: Event[Activity.State[Set[Address]]])
