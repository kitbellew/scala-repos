...
abstract class TransportParams[A <: Stack.Parameterized[A]](
  ...
  def sendBufferSize(size: StorageUnit): A =
    self.configured(
      self.params[Transport.BufferSizes].copy(send = Some(size.inBytes.toInt)))
  ...
  def receiveBufferSize(size: StorageUnit): A =
    self.configured(
      self.params[Transport.BufferSizes].copy(recv = Some(size.inBytes.toInt)))
  ...
  def writeTimeout(timeout: Duration): A =
    self
      .configured(self.params[Transport.Liveness].copy(writeTimeout = timeout))
