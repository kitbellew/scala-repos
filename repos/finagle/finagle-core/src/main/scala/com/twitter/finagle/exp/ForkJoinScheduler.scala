...
private class ForkJoinScheduler(
  ...
  private[this] val gauges = Seq(
    ...
    statsReceiver.addGauge("active_blocks") {
      activeBlocks.get
    },
    // Returns an estimate of the number of threads that are
    // currently stealing or executing tasks.
    statsReceiver.addGauge("active_threads") {
      pool.getActiveThreadCount()
    },
    // Returns the targeted parallelism level of this pool.
    statsReceiver.addGauge("parallelism") {
      pool.getParallelism()
    },
    ...
    statsReceiver.addGauge("queued_tasks") {
    ...
    statsReceiver.addGauge("steals") {
      pool.getStealCount()
    },
    // The number of tasks that were split off a local schedule.
    statsReceiver.addGauge("splits") {
      splitCount.get
    }
  )
