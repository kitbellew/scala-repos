...
private object TrafficDistributorTest {
  ...
  case class AddressFactory(addr: Address) extends ServiceFactory[Int, Int] {
    def apply(conn: ClientConnection) =
      Future.value(Service.mk(i => Future.value(i)))
...
class TrafficDistributorTest extends FunSuite {
  ...
  test("distributes when weights are uniform")(
    new Ctx {
  ...
  test("distributes according to non-uniform weights")(
    new Ctx {
      val R = 10 * 10 * 1000
  ...
  test("memoize calls to newEndpoint and newBalancer")(
    new Ctx {
  ...
  test("transitions between activity states")(
    new Ctx {
  ...
  test("transitions to failure if failure comes first")(
    new Ctx {
      val init: Activity.State[Set[Address]] = Activity.Pending
      val dest = Var(init)
      val dist = newDist(dest)
  ...
  test("status is bestOf all weight classes")(
    new Ctx {
  ...
  test("handles replicated addresses")(
    new Ctx {
      val init: Set[Address] = (1 to 5).map(Address(_)).toSet
      val dest = Var(Activity.Ok(init))
  ...
  test("increment weights on a shard")(
    new StringClient with StringServer {
