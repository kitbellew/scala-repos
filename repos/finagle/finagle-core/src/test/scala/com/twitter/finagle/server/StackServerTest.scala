...
class StackServerTest extends FunSuite {
  test("Deadline isn't changed until after it's recorded") {
    ...
    val stack = StackServer.newStack[Unit, Deadline] ++
      Stack.Leaf(Endpoint, echo)
    val statsReceiver = new InMemoryStatsReceiver
    val factory = stack.make(
      StackServer.defaultParams + TimeoutFilter.Param(1.second) +
        Stats(statsReceiver))
    ...
    Time.withCurrentTimeFrozen { ctl =>
      Contexts.broadcast.let(Deadline, Deadline.ofTimeout(5.seconds)) {
        ...
        assert(
          statsReceiver.stats(
            Seq("admission_control", "deadline", "transit_latency_ms"))(0) ==
            1.second.inMilliseconds.toFloat)
