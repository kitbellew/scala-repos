...
class StackTest extends FunSuite {
  class TestCtx extends StringClient with StringServer {
    ...
    val newClientStack = StackClient.newStack[String, String].replace(
  ...
  test(
    ...
    new TestCtx {
      ...
      val client = stringClient.withStack(newClientStack).newService(
        Name
  ...
  test(
    ...
    new TestCtx {
      val server = ServerBuilder().codec(StringCodec)
        .bindTo(new InetSocketAddress(InetAddress.getLoopbackAddress, 0))
        .name("server").build(failService)
