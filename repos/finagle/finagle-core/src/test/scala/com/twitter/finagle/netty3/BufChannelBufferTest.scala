...
class BufChannelBufferTest extends FunSuite with BeforeAndAfter {
  ...
  test("compareTo") {
    ...
    assertEquals(
      0,
      bcb.compareTo(ChannelBuffers.wrappedBuffer(ByteOrder.LITTLE_ENDIAN, value)))

    value(0) = (value(0) + 1).asInstanceOf[Byte]
    assert(
      bcb.compareTo(ChannelBuffers.wrappedBuffer(ByteOrder.BIG_ENDIAN, value)) < 0)
    assert(
      bcb.compareTo(ChannelBuffers.wrappedBuffer(ByteOrder.LITTLE_ENDIAN, value)) < 0)
