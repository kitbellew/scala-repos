...
class BufChannelBufferTest extends FunSuite with BeforeAndAfter {
  ...
  test("getByteArray boundary check 3") {
    ...
    intercept[IndexOutOfBoundsException] { bcb.getBytes(0, dst, -1, 4) }
  ...
  test("discardReadBytes") {
    ...
    bcb.readByte()
    intercept[ReadOnlyBufferException] { bcb.discardReadBytes() }
  }
  ...
  test("copy") {
    ...
    intercept[ReadOnlyBufferException] { copy.setByte(1, 1) }
  }
  ...
  test("compareTo") {
    ...
    assert(
      bcb.compareTo(
        ChannelBuffers.wrappedBuffer(ByteOrder.LITTLE_ENDIAN, value, 0, 31)) >
        0)
    assert(
      bcb.slice(0, 31)
        .compareTo(ChannelBuffers.wrappedBuffer(ByteOrder.BIG_ENDIAN, value)) <
        0)
    assert(
  ...
  test("indexOf") {
    val bcb =
      new BufChannelBuffer(Buf.ByteArray.Owned(Array[Byte](1, 2, 3, 2, 1)))
