...
/**
  ...
  */
case class SeqMismatchException(id: Int, expected: Int)
    extends TransportException {
  override def toString =
    "SeqMismatchException: got %d, expected %d".format(id, expected)
}
    ...
    if (req.oneway)
      ...
      service(newReq) flatMap { resBuf =>
        ...
        getAndSetId(resBuf, givenId) match {
          ...
          case Return(badId) =>
            Future.exception(SeqMismatchException(badId, id))
