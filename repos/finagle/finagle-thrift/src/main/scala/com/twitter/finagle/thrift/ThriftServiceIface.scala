...
object ThriftServiceIface {
  private val resetCounter = ClientStatsReceiver.scope(
    "thrift_service_iface").counter("reusable_buffer_resets")
        ...
        service(args).onSuccess { result =>
          if (result.successField.isDefined) {
            ...
            result.firstException.map { ex =>
              ...
              methodStats.failuresScope.counter(
                Throwables.mkString(ex): _*).incr()
            }
          }
        }
      }
