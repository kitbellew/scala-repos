...
class TFinagleBinaryProtocolTest
  ...
  private def assertSerializedBytes(expected: String, trans: TMemoryBuffer) {
  ...
  test("writeString fallsback on encoding failure") {
    ...
    val str =
      new String(
        Array.fill(TFinagleBinaryProtocol.OutBufferSize) {
          '\u2603'
        })
  ...
  test("writeString same as TBinaryProtocol") {
    def compare(str: String) {
      ...
      val optProto =
        new TFinagleBinaryProtocol(optTrans, NullCounter, NullCounter)
