...
class ClientTest extends FunSuite {
  ignore(
    ...
    val serviceFactory = ClientBuilder().hosts("localhost:22133")
      .codec(Kestrel()).hostConnectionLimit(1).buildFactory()
    ...
    val rep = Await.result(client.get("foo")) map { case Buf.Utf8(s) => s }
    assert(rep == Some("bar"))
  }
  ...
  ignore(
    ...
    val serviceFactory = ClientBuilder().hosts("localhost:2229")
