...
object Hasher {
   ...
   * Instances below
   */
  implicit val unit: Hasher[Unit] =
    new Hasher[Unit] {
      @inline
      def hash(i: Unit) = 0
    }
  implicit val boolean: Hasher[Boolean] =
    new Hasher[Boolean] {
  ...
  implicit val byte: Hasher[Byte] =
    new Hasher[Byte] {
      @inline
      def hash(i: Byte) = hashInt(i.toInt)
    }
  implicit val char: Hasher[Char] =
    new Hasher[Char] {
      @inline
  ...
  implicit val short: Hasher[Short] =
    new Hasher[Short] {
      @inline
      def hash(i: Short) = hashInt(i.toInt)
    }

  implicit val int: Hasher[Int] =
    new Hasher[Int] {
      @inline
