...
class DateTest extends WordSpec {
  ...
  "A RichDate" should {
    ...
    "know the most recent time units" in {
      ...
      assert(
        Hours(1).floorOf("2011-10-25 10:01") === RichDate("2011-10-25 10:00"))
    }
    ...
    "correctly calculate upperBound" in {
      assert(
        Seconds(1).floorOf(RichDate.upperBound("20101001")) === Seconds(
          1).floorOf(RichDate("2010-10-01 23:59:59")))
      assert(
        Seconds(1).floorOf(RichDate.upperBound("2010100114")) === Seconds(
      ...
      assert(
        Seconds(1).floorOf(RichDate.upperBound("2010-10-01")) === Seconds(
          1).floorOf(RichDate("2010-10-01 23:59:59")))
      assert(
  ...
  "A DateRange" should {
    "correctly iterate on each duration" in {
      ...
      assert(
        DateRange("201010010000", RichDate("2010-10-02") - Millisecs(1)).each(
          Hours(1)).size === 24)
      assert(
        DateRange(
          "2010-10-01 00:00",
          RichDate("2010-10-02") - Millisecs(1)).each(Hours(1)).size === 24)
      assert(
        DateRange(
          "2010-10-01 00:00",
          RichDate("2010-10-02") + Millisecs(1)).each(Hours(1)).size === 25)
      assert(
        DateRange("2010-10-01", RichDate.upperBound("2010-10-20")).each(
          Days(1)).size === 20)
      assert(
        DateRange("2010-10-01", RichDate.upperBound("2010-10-01")).each(
          Hours(1)).size === 24)
      assert(
        DateRange("2010-10-31", RichDate.upperBound("2010-10-31")).each(
          Hours(1)).size === 24)
      assert(
        DateRange("2010-10-31", RichDate.upperBound("2010-10-31")).each(
          Days(1)).size === 1)
      assert(
        DateRange(
          "2010-10-31 12:00",
          RichDate.upperBound("2010-10-31 13")).each(Minutes(1)).size === 120)
    }
