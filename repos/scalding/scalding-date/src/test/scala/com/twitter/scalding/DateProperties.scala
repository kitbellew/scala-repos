...
object DateProperties extends Properties("Date Properties") {
  ...
  implicit val durationArb: Arbitrary[Duration] = Arbitrary {
    choose(0, 10000).map { Millisecs(_) }
  }
  ...
  property("DateRange.exclusiveUpper works") = forAll {
    (a: RichDate, b: RichDate) =>
  ...
  property("Globifying produces matching patterns") = forAll {
    (dr: DateRange) =>
