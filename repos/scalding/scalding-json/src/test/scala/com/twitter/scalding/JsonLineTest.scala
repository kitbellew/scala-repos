...
class JsonLineInputJob(args: Args) extends Job(args) {
  try {
    JsonLine("input0", ('foo, 'bar)).read.project('foo, 'bar)
      .write(Tsv("output0"))
...
class JsonLineInputJobSkipEmptyLines(args: Args) extends Job(args) {
  try {
    JsonLine("input0", ('foo, 'bar), failOnEmptyLines = false).read
      .project('foo, 'bar).write(Tsv("output0"))
...
class JsonLineNestedInputJob(args: Args) extends Job(args) {
  try {
    JsonLine("input0", (Symbol("foo.too"), 'bar)).read
      .rename((Symbol("foo.too") -> ('foo))).project('foo, 'bar)
      .write(Tsv("output0"))
...
class JsonLineTest extends WordSpec {
  ...
  "A JsonLine sink" should {
    JobTest(new JsonLineJob(_)).source(
    ...
    JobTest(new JsonLineRestrictedFieldsJob(_)).source(
      Tsv("input0", ('query, 'queryStats)),
      List(("doctor's mask", List(42.1f, 17.1f))))
    ...
    JobTest(new JsonLineNestedInputJob(_)).source(
      JsonLine("input0", (Symbol("foo.too"), 'bar)),
      List((0, json), (1, json3))).sink[(Int, String)](Tsv("output0")) {
      outBuf =>
    ...
    "fail on empty lines by default" in {
      intercept[FlowException] {
        JobTest(new JsonLineInputJob(_)).source(
          JsonLine("input0", ('foo, 'bar)),
          List((0, json), (1, json2), (2, ""), (3, "   ")))
          .sink[(Int, String)](Tsv("output0")) { outBuf => outBuf.toList }.run
          .finish
      }
    }

    JobTest(new JsonLineInputJobSkipEmptyLines(_)).source(
      JsonLine("input0", ('foo, 'bar)),
      List((0, json), (1, json2), (2, ""), (3, "   ")))
      .sink[(Int, String)](Tsv("output0")) { outBuf =>
