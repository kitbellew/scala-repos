...
import cascading.avro.local.{
  AvroScheme => LAvroScheme,
  PackedAvroScheme => LPackedAvroScheme
}
...
object UnpackedAvroSource {
  ...
  def apply[T](path: String) = new UnpackedAvroSource[T](Seq(path), None)

  def apply[T](paths: Seq[String], schema: Schema) =
    new UnpackedAvroSource[T](paths, Some(schema))

  def apply[T](paths: Seq[String]) = new UnpackedAvroSource[T](paths, None)
