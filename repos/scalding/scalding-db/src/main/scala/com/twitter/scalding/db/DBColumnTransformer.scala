...
object DBColumnTransformer {
  def columnDefnToDefinition(
    ...
    val sizeStr = preparedCol
      .sizeOpt
      .map { siz =>
        s"($siz)"
      }
      .getOrElse("")
    val defStr = preparedCol
      .defaultValue
      .map { default =>
        s" DEFAULT '${default}' "
      }
  ...
  private def defaultColumnMutator
    ...
    case t @ DBColumnDefinition(INT, _, _, None, _, _) =>
      t.copy(sizeOpt = Some(11))
    case t @ DBColumnDefinition(SMALLINT, _, _, None, _, _) =>
      t.copy(sizeOpt = Some(6))
    case t @ DBColumnDefinition(TINYINT, _, _, None, _, _) =>
      t.copy(sizeOpt = Some(6))
  ...
  def mutateColumns(
    ...
    columns.map(c =>
      columnMutator.orElse(defaultColumnMutator)(DBColumnDefinition(c)))

  def columnDefnsToCreate(
      columnMutator: PartialFunction[DBColumnDefinition, DBColumnDefinition],
      columns: Iterable[ColumnDefinition]): Iterable[Definition] =
    columns.map(c =>
      columnDefnToDefinition(c, columnMutator.orElse(defaultColumnMutator)))
