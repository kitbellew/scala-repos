  ...
  implicit def arbitraryTestSets: Arbitrary[TestSets] =
    Arbitrary {
      for {
        ...
        aListSet <- Gen.listOf(Gen.listOf(Gen.alphaStr).map(l =>
          l.to[collection.Seq])).map(_.to[collection.Set])
        aSetSet <- Gen.listOf(Gen.listOf(Gen.alphaStr).map(l =>
          l.to[collection.Set])).map(_.to[collection.Set])
        aMapSet <- Gen.listOf(Gen.listOf(arb[(Int, Int)]).map(l =>
          l.toMap.asInstanceOf[collection.Map[Int, Int]])).map(
          _.to[collection.Set])
      } yield TestSets(
        aBoolSet,
        aByteSet,
    ...
    Arbitrary {
      for {
        ...
        aMapMap <- Gen.listOf(arb[(Map[Int, Int], Map[Int, Int])]).map(
          _.toMap.map {
            case (k, v) =>
              k.asInstanceOf[collection.Map[Int, Int]] -> v.asInstanceOf[
