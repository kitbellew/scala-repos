...
object ScroogeOrderedBuf {
  ...
  def apply(c: Context)(
    ...
    val fieldNames: List[String] = companionSymbol.asModule.moduleClass.asType
      .toType.declarations.filter(_.name.decoded.endsWith("Field ")).collect {
        case s: TermSymbol => s
      }.filter(_.isStatic).filter(_.isVal).map { t =>
        val decodedName = t.name.decoded // Looks like "MethodNameField "
        decodedName.dropRight(6)
    ...
    val elementData: List[(c.universe.Type, TermName, TreeOrderedBuf[c.type])] =
      outerType.declarations.collect { case m: MethodSymbol => m }.filter(m =>
        fieldNames.contains(m.name.toTermName.toString.toLowerCase)).map {
        accessorMethod =>
    ...
    new TreeOrderedBuf[c.type] {
      ...
      override val lazyOuterVariables: Map[String, ctx.Tree] = elementData
        .map(_._3.lazyOuterVariables).reduceLeftOption(_ ++ _).getOrElse(Map())
