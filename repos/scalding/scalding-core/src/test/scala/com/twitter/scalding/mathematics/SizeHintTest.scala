...
object SizeHintProps extends Properties("SizeHint") {
  ...
  implicit val sparseArb: Arbitrary[SparseHint] = Arbitrary {
    sparseHintGen
  }
  ...
  property("ordering makes sense") = forAll { (a: SizeHint, b: SizeHint) =>
    (
      List(a, b).max.total.getOrElse(BigInt(-1L)) >=
        a.total.getOrElse(BigInt(-1L))
    )
  }

  property("addition increases sparsity fraction") = forAll {
  ...
  property("squaring a finite hint preserves size") = forAll {
    (a: FiniteHint) =>
