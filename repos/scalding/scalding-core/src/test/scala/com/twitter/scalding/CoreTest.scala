...
class MapToGroupBySizeSumMaxJob(args: Args) extends Job(args) {
  ...
  mapTo(1 -> ('kx, 'x)) { line: String =>
    val x = line.toDouble
    ((x > 0.5), x)
  }.groupBy('kx) { _.size.sum[Double]('x -> 'sx).max('x) }.write(
    Tsv(args("output")))
}
...
class Function2Job(args: Args) extends Job(args) {
  ...
  Tsv("in", ('x, 'y)).mapTo(('x, 'y) -> 'xy) { (x: String, y: String) =>
    x + y
  }.write(Tsv("output"))
}
...
class DailySuffixTsvJob(args: Args) extends Job(args) with UtcDateRangeJob {
  import TDsl._
  DailySuffixTsvJob.source("input0").read.toTypedPipe[(String, Int)](
    (0, 1)).write(TypedTsv[(String, Int)]("output0"))
}
