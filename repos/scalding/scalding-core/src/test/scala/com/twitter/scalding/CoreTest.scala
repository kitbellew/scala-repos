...
class NumberJoinerJob(args: Args) extends Job(args) {
  ...
  val in1 =
    Tsv("input1")
      .read
      .mapTo((0, 1) -> ('x1, 'y1)) { input: (Long, Long) =>
        input
      }
  ...
  "A MRMJob" should {
    ...
    JobTest(new MRMJob(_))
      ...
      .sink[Int](Tsv("outputSetTo")) { outBuf =>
        "use flattenTo" in {
          outBuf.toList.sorted shouldBe (input
            .map {
              _._2
            }
            .sorted)
...
class LeftJoinJob(args: Args) extends Job(args) {
  val p1 =
    Tsv(args("input1")).mapTo((0, 1) -> ('k1, 'v1)) { v: (String, Int) =>
      v
    }
  val p2 =
...
class ForceReducersJob(args: Args) extends Job(args) {
  TextLine("in")
    ...
    .groupBy('num) {
      _.toList[String]('words -> 'wordList).forceToReducers
    }
...
class ToListJob(args: Args) extends Job(args) {
  TextLine(args("in"))
    ...
    .groupBy('offset) {
      _.toList[String]('words -> 'wordList)
    }
    .map('wordList -> 'wordList) { w: List[String] =>
      w.mkString(" ")
    }
      ...
      .sink[(Int, Int, Int)](Tsv("output")) { outBuf =>
        "keep all the columns" in {
          val correct =
            (
              1 to 100
            ).map(i => (i, i * i % 5, i * i * i)).toList.sortBy(_._2)
          outBuf.toList shouldBe correct
        }
      }
      .run
  ...
  "A CounterJob" should {
    ...
    "have the right counter and output values" in {
      JobTest(new CounterJob(_))
        ...
        .counter("age_group_older_than_18") {
          _ shouldBe 3
        }
        .counter("reduce_hit") {
          _ shouldBe 2
        }
        .counter("bad_group_bad_counter") {
          _ shouldBe 0
        }
