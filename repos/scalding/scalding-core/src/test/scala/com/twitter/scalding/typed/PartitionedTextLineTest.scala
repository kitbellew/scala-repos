...
class PartitionedTextLineTest extends WordSpec with Matchers {
  ...
  "PartitionedTextLine" should {
    ...
    "be able to split output by multiple  partitions" in {
      ...
      directory.listFiles.flatMap(d =>
        d.listFiles.map(d.getName + "/" + _.getName)).toSet shouldBe Set(
        "A/X",
        "A/Y",
        "B/Z")
      ...
      val aySource =
        ScalaSource.fromFile(new File(directory, "A/Y/part-00000-00001"))
      val bzSource =
        ScalaSource.fromFile(new File(directory, "B/Z/part-00000-00002"))
