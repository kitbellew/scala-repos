    ...
    JobTest(new SortWithTakeJob(_))
      ...
      .sink[(String, List[(Long, Double)])](Tsv("output0")) { buf =>
        "grouped list" in {
          ...
          whatWeGet.get("a").getOrElse("apples") shouldBe (whatWeWant.get(
            "a").getOrElse("oranges"))
          whatWeGet.get("b").getOrElse("apples") shouldBe (whatWeWant.get(
            "b").getOrElse("oranges"))
        }
      }
      .runHadoop
    ...
    JobTest(new ApproximateUniqueCountJob(_))
      ...
      .sink[(String, Long)](Tsv("output0")) { buf =>
        "grouped OS count" in {
          ...
          whatWeGet.get("laptop").getOrElse("apples") shouldBe (whatWeWant.get(
            "laptop").getOrElse("oranges"))
          whatWeGet.get("mobile").getOrElse("apples") shouldBe (whatWeWant.get(
            "mobile").getOrElse("oranges"))
        }
      }
      .runHadoop
      .finish
