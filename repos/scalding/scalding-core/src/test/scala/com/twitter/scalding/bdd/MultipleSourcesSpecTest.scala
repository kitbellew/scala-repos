  ...
  "A test with two sources" should {
    "accept an operation with two input pipes" in {
      Given {
        ...
        List(
          ("Stefano", "home1"),
          ("Rajah", "home2")) withSchema ('name, 'address)
      } When { (pipe1: RichPipe, pipe2: RichPipe) =>
        {
          pipe1
            .joinWithSmaller('name -> 'name, pipe2)
            .map('address -> 'address_transf) { address: String =>
              address + "_transf"
            }
        }
    ...
    "accept an operation with two input pipes using Tuples" in {
      Given {
        List(
          new Tuple("Stefano", "110"),
          new Tuple("Rajah", "220")) withSchema ('name, 'points)
      } And {
        List(
          new Tuple("Stefano", "home1"),
          new Tuple("Rajah", "home2")) withSchema ('name, 'address)
      } When { (pipe1: RichPipe, pipe2: RichPipe) =>
        {
          pipe1
            .joinWithSmaller('name -> 'name, pipe2)
  ...
  "A test with four sources" should {
    "compile mixing an operation with inconsistent number of input pipes but fail at runtime" in {
      an[IllegalArgumentException] should be thrownBy {
        Given {
          ...
          List(
            ("col1_1", "col2_1"),
            ("col1_2", "col2_2")) withSchema ('col1, 'col5)
        } When { (pipe1: RichPipe, pipe2: RichPipe, pipe3: RichPipe) =>
          {
            pipe1
              .joinWithSmaller('col1 -> 'col1, pipe2)
    ...
    "be used with a function accepting a list of sources because there is no implicit for functions with more than three input pipes" in {
      Given {
        ...
        List(
          ("col1_1", "col2_1"),
          ("col1_2", "col2_2")) withSchema ('col1, 'col6)
