...
class TypedApiTest extends WordSpec with Matchers with TBddDsl {
  "A test with a single source" should {
    "accept an operation from working with a single tuple-typed pipe" in {
      Given { List(("Joe", "M", 40), ("Sarah", "F", 22)) } When {
    ...
    "accept an operation from single case class-typed pipe" in {
      Given {
        ...
        in.map { person =>
          person match {
            case UserInfo(name, "M", age) =>
              EstimatedContribution(name, (1000.0 / (72 - age)))
            case UserInfo(name, _, age) =>
              EstimatedContribution(name, (1000.0 / (80 - age)))
          }
        }
  ...
  "A test with a two sources" should {
    ...
    "accept an operation from two case classes-typed pipes" in {
      ...
      } And { List(UserWithAge("Joe", 40), UserWithAge("Sarah", 22)) } When {
        (gender: TypedPipe[UserWithGender], age: TypedPipe[UserWithAge]) =>
          gender.groupBy(_.name).join(age.groupBy(_.name)).mapValues {
            value: (UserWithGender, UserWithAge) =>
  ...
  "A test with a list of sources" should {
    ...
    "be created when adding a source to four sources" in {
      Given { List(("Joe", "user1"), ("Sarah", "user2")) } And {
        List(("user1", "M"), ("user2", "F"))
      } And { List(("user1", 40), ("user2", 22)) } And {
        List(("user1", 1000L), ("user2", 800L))
      } And { List(("user1", true), ("user2", false)) } When {
        pipes: List[TypedPipe[_]] =>
          ...
          withUserID.swap.group.join(withGender.group).join(withAge.group)
            .join(withIncome.group).join(withSmoker.group).flatMapValues {
              case (
