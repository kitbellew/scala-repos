...
object Grouped {
  ...
  def valueSorting[V](ord: Ordering[V]): Fields =
    Field.singleOrdered[V]("value")(ord)
  ...
  protected def groupOpWithValueSort[V2](valueSort: Option[Ordering[_ >: V1]])(
    ...
    TypedPipeFactory({ (fd, mode) =>
            ...
            vs =>
              vs match {
                case ordser: OrderedSerialization[V1] =>
                  ...
                  val (boxfn, boxordSer) = Grouped
                    .getBoxFnAndOrder[V1](ordser, fd)
                  val valueF = new Fields("value")
                  valueF.setComparator(
                    "value",
                    new CascadingBinaryComparator(boxordSer))
                  val ts2 = tupleSetter
                    .asInstanceOf[TupleSetter[(K, Boxed[V1])]].contraMap {
                      kv1: (K, V1) => (kv1._1, boxfn(kv1._2))
                    }
                  (Some(valueF), ts2)
      ...
      TypedPipe.from(pipe, Grouped.kvFields)(
        fd,
        mode,
        Grouped.tuple2Conv[K, V2](keyOrdering))
    })
}
...
case class IdentityReduce[K, V1](
   ...
   */
  private def toUIR =
    UnsortedIdentityReduce(keyOrdering, mapped, reducers, descriptions)
  ...
  /**
    ...
    */
  override def bufferedTake(n: Int) = toUIR.bufferedTake(n)

  override def withSortOrdering[U >: V1](
      so: Ordering[U]): IdentityValueSortedReduce[K, V1] =
  ...
  override def filterKeys(fn: K => Boolean) = toUIR.filterKeys(fn)
  ...
  override def mapValues[V2](fn: V1 => V2) = toUIR.mapValues(fn)
...
case class UnsortedIdentityReduce[K, V1](
  ...
  override def withDescription(
      description: String): UnsortedIdentityReduce[K, V1] =
    copy(descriptions = descriptions :+ description)
...
case class IdentityValueSortedReduce[K, V1](
  ...
  override def withDescription(
      description: String): IdentityValueSortedReduce[K, V1] =
    IdentityValueSortedReduce[K, V1](
      keyOrdering,
      mapped,
      valueSort,
  ...
  override lazy val toTypedPipe = groupOpWithValueSort[V1](valueSort = Some(
    valueSort)) { gb =>
...
case class ValueSortedReduce[K, V1, V2](
  ...
  override lazy val toTypedPipe = {
    ...
    groupOpWithValueSort(optVOrdering) {
      ...
      _.every(new cascading.pipe.Every(
        _,
        Grouped.valueField,
...
case class IteratorMappedReduce[K, V1, V2](
  ...
  override def withDescription(
      description: String): IteratorMappedReduce[K, V1, V2] =
    copy(descriptions = descriptions :+ description)
