...
/**
  ...
  * in each operation.
  */
trait ReduceOperations[+Self <: ReduceOperations[Self]]
  ...
  def approximateUniqueCount[T <% Array[Byte]: TupleConverter](
    ...
    hyperLogLogMap[T, Double](f, errPercent) {
      _.estimatedSize
    }
  }
  ...
  def mkString(
    ...
    mapList[String, String](fieldDef) {
      _.mkString(start, sep, end)
    }
  }
  ...
  def sum[T](fs: Symbol*)(implicit
      sg: Semigroup[T],
      tconv: TupleConverter[T],
      tset: TupleSetter[T]): Self = sum[T](fs -> fs)(sg, tconv, tset)
