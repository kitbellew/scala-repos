...
private[scalding] case class HadoopFlowPCounterImpl(
    ...
    statKey: StatKey)
    extends CounterImpl {
  private[this] val cntr = fp.getReporter()
...
object Stats {
  ...
  def getCounterValue(key: StatKey)(implicit
      cascadingStats: CascadingStats): Long =
  ...
  def getAllCustomCounters()(implicit
      cascadingStats: CascadingStats): Map[String, Long] = {
    val counts = for {
...
class StatsFlowListener(f: Map[StatKey, Long] => Try[Unit])
  ...
  override def onCompleted(flow: Flow[_]): Unit = {
    if (success) {
      ...
      val values = keys.map(k => (k, stats.getCounterValue(k.group, k.counter)))
        .toMap
      f(values).get
    }
  }
