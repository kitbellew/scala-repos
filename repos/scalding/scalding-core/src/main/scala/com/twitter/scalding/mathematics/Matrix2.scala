...
sealed trait Matrix2[R, C, V] extends Serializable {
  ...
  def +(that: Matrix2[R, C, V])(implicit mon: Monoid[V]): Matrix2[R, C, V] =
    Sum(this, that, mon)
  def -(that: Matrix2[R, C, V])(implicit g: Group[V]): Matrix2[R, C, V] =
    Sum(this, that.negate, g)
  ...
  // Matrix product
  def *[C2](that: Matrix2[C, C2, V])(implicit
      ring: Ring[V],
      mj: MatrixJoiner2): Matrix2[R, C2, V] = Product(this, that, ring)

  def *(that: Scalar2[V])(implicit
      ring: Ring[V],
      mj: MatrixJoiner2): Matrix2[R, C, V] = that * this
  ...
  def propagate[C2, VecV](vec: Matrix2[C, C2, VecV])(implicit
    ...
    lazy val joinedBool = mj
      .join(this.asInstanceOf[Matrix2[R, C, Boolean]], vec)
    ...
    lazy val resultPipe = joinedBool.flatMap {
      ...
      .sum.filter { kv => mon.isNonZero(kv._2) }.map {
        case ((r, c2), v) => (r, c2, v)
      }
    MatrixLiteral(resultPipe, this.sizeHint)
  }

  def propagateRow[C2](mat: Matrix2[C, C2, Boolean])(implicit
      ev: =:=[R, Unit],
      mon: Monoid[V],
      mj: MatrixJoiner2): Matrix2[Unit, C2, V] =
    mat.transpose.propagate(this.transpose.asInstanceOf[Matrix2[C, Unit, V]])
      .transpose
  ...
  def getRow(index: R): Matrix2[Unit, C, V] =
    MatrixLiteral(
      toTypedPipe.filter { case (r, c, v) => Ordering[R].equiv(r, index) }.map {
        case (r, c, v) => ((), c, v)
      },
  ...
  def getColumn(index: C): Matrix2[R, Unit, V] =
    MatrixLiteral(
      toTypedPipe.filter { case (r, c, v) => Ordering[C].equiv(c, index) }.map {
        case (r, c, v) => (r, (), v)
      },
  ...
  def trace(implicit mon: Monoid[V], ev: =:=[R, C]): Scalar2[V] =
    Scalar2(
      toTypedPipe.asInstanceOf[TypedPipe[(R, R, V)]].filter {
        case (r1, r2, _) => Ordering[R].equiv(r1, r2)
      }.map { case (_, _, x) => x }.sum(mon))

  def write(sink: TypedSink[(R, C, V)])(implicit
      fd: FlowDef,
...
/**
  ...
  */
case class OneC[R, V](implicit override val rowOrd: Ordering[R])
    extends Matrix2[R, Unit, V] {
...
/**
  ...
  */
case class OneR[C, V](implicit override val colOrd: Ordering[C])
    extends Matrix2[Unit, C, V] {
...
case class Product[R, C, C2, V](
  ...
  lazy val toOuterSum: TypedPipe[(R, C2, V)] = {
    if (optimal) {
      if (isSpecialCase) { specialCase }
        ...
        val localRing = ring
        joiner.join(left, right).map {
          case (key, ((l1, lv), (r2, rv))) => (l1, r2, localRing.times(lv, rv))
        }
      }
    } else {
  ...
  private def computePipe(
    ...
    if (isSpecialCase) { joined }
      ...
      val localRing = ring
      joined.groupBy(w => (w._1, w._2)).mapValues { _._3 }.sum(localRing)
        .filter { kv => localRing.isNonZero(kv._2) }.map {
          case ((r, c), v) => (r, c, v)
        }
    }
  }
  ...
  override def trace(implicit mon: Monoid[V], ev1: =:=[R, C2]): Scalar2[V] = {
    val (cost1, plan1) = Matrix2
      .optimize(this.asInstanceOf[Matrix2[Any, Any, V]])
...
case class Sum[R, C, V](
  ...
  override lazy val toTypedPipe: TypedPipe[(R, C, V)] = {
    if (left.equals(right)) {
      ...
      collectAddends(this).reduce((x, y) => x ++ y).groupBy(x => (x._1, x._2))
        .mapValues { _._3 }.sum(mon).filter { kv => mon.isNonZero(kv._2) }.map {
          case ((r, c), v) => (r, c, v)
        }
    }
  }
  ...
  override def sumColVectors(implicit
      ring: Ring[V],
      mj: MatrixJoiner2): Matrix2[R, Unit, V] =
  ...
  override def trace(implicit mon: Monoid[V], ev: =:=[R, C]): Scalar2[V] =
    Scalar2(collectAddends(this).map { pipe =>
      pipe.asInstanceOf[TypedPipe[(R, R, V)]].filter {
        case (r, c, v) => Ordering[R].equiv(r, c)
      }.map { _._3 }
    }.reduce(_ ++ _).sum)
}
...
case class MatrixLiteral[R, C, V](
    ...
    override val sizeHint: SizeHint)(implicit
    override val rowOrd: Ordering[R],
...
trait Scalar2[V] extends Serializable {
  ...
  def *[R, C](that: Matrix2[R, C, V])(implicit
      ring: Ring[V],
    ...
    that match {
      ...
      case x @ OneC() => Product(OneC[Unit, V](), toMatrix, ring)
          .asInstanceOf[Matrix2[R, C, V]]
      case x @ OneR() => Product(toMatrix, OneR[Unit, V](), ring)
          .asInstanceOf[Matrix2[R, C, V]]
    }
  ...
  def divMatrix[R, C](that: Matrix2[R, C, V])(implicit
    ...
    MatrixLiteral(
      that.toTypedPipe.mapWithValue(value) {
...
case class ValuePipeScalar[V](override val value: ValuePipe[V])
    extends Scalar2[V]
...
object Scalar2 {
  ...
  implicit def const[V](v: V): Scalar2[V] = from(LiteralValue(v))

  def apply[V](v: V): Scalar2[V] = from(LiteralValue(v))
}

object Matrix2 {
  def apply[R: Ordering, C: Ordering, V](
      t: TypedPipe[(R, C, V)],
      hint: SizeHint): Matrix2[R, C, V] = MatrixLiteral(t, hint)
  ...
  def J[R, C, V](implicit
      ordR: Ordering[R],
      ordC: Ordering[C],
      ring: Ring[V],
      mj: MatrixJoiner2) = Product(OneC[R, V]()(ordR), OneR[C, V]()(ordC), ring)
  ...
  def optimizeProductChain[V](
    ...
    val sharedMap = HashMap
      .empty[Matrix2[Any, Any, V], TypedPipe[(Any, Any, V)]]
      ...
      mf match {
        ...
        case Sum(left, right, mon) => {
          ...
          val (cost1, newLeft) = optimizeProductChain(
            lastLChain.toIndexedSeq,
            pair(ringL, joinerL))
          val (cost2, newRight) = optimizeProductChain(
            lastRChain.toIndexedSeq,
            pair(ringR, joinerR))
          (
