...
sealed trait Matrix2[R, C, V] extends Serializable {
  ...
  def propagate[C2, VecV](vec: Matrix2[C, C2, VecV])(implicit
    ...
    lazy val joinedBool = mj.join(
      this.asInstanceOf[Matrix2[R, C, Boolean]],
      vec)
...
class DefaultMatrixJoiner(sizeRatioThreshold: Long) extends MatrixJoiner2 {
  def join[R, C, V, C2, V2](
    ...
    val one =
      left.toTypedPipe.map {
        case (r, c, v) =>
          (c, (r, v))
      }.group
    val two =
      right.toTypedPipe.map {
        case (c, c2, v2) =>
          (c, (c2, v2))
      }.group
...
case class Product[R, C, C2, V](
  ...
  override def equals(obj: Any): Boolean =
    obj match {
      case Product(tl, tr, _, _) =>
        left.equals(tl) && right.equals(tr)
      case _ =>
        false
    }
  ...
  private lazy val isSpecialCase: Boolean =
    right.isInstanceOf[OneC[_, _]] || left.isInstanceOf[OneR[_, _]]
  ...
  override def trace(implicit mon: Monoid[V], ev1: =:=[R, C2]): Scalar2[V] = {
    val (cost1, plan1) = Matrix2.optimize(
      this.asInstanceOf[Matrix2[Any, Any, V]])
...
case class Sum[R, C, V](
  ...
  def collectAddends(sum: Sum[R, C, V]): List[TypedPipe[(R, C, V)]] = {
    def getLiteral(mat: Matrix2[R, C, V]): TypedPipe[(R, C, V)] = {
      mat match {
        case x @ Product(_, _, _, _) =>
          x.toOuterSum
        case x @ MatrixLiteral(_, _) =>
          x.toTypedPipe
        case x @ HadamardProduct(_, _, _) =>
          x.optimizedSelf.toTypedPipe
        case _ =>
          sys.error("Invalid addend")
      }
    }
...
trait Scalar2[V] extends Serializable {
  ...
  def +(that: Scalar2[V])(implicit sg: Semigroup[V]): Scalar2[V] = {
    (value, that.value) match {
      case (EmptyValue, _) =>
        that
      case (LiteralValue(v1), _) =>
        that.map(sg.plus(v1, _))
      case (_, EmptyValue) =>
        this
      case (_, LiteralValue(v2)) =>
        map(sg.plus(_, v2))
      // TODO: optimize sums of scalars like sums of matrices:
      // only one M/R pass for the whole Sum.
      case (_, ComputedValue(v2)) =>
        Scalar2((value ++ v2).sum(sg))
    }
  }
  ...
  def *[R, C](that: Matrix2[R, C, V])(implicit
    ...
    that match {
      ...
      case s @ Sum(left, right, mon) =>
        Sum(this * left, this * right, mon)
      case m @ MatrixLiteral(_, _) =>
        timesLiteral(m) // handle literals here
      case x @ OneC() =>
