...
trait TBddDsl extends FieldConversions with TypedPipeOperationsConversions {
  ...
  abstract class TypedTestSource[T] {
    ...
    def asSource: Source = IterableSource(data map { Tuple1(_) }, 'tuple)
    ...
    def addSourceDataToJobTest(jobTest: JobTest) =
      jobTest.source(asSource, data)
  }

  class SimpleTypedTestSource[T](val data: Iterable[T])
      extends TypedTestSource[T] {
  ...
  case class TestCaseGiven3[TypeIn1, TypeIn2, TypeIn3](
    ...
    def And(next: TypedTestSource[_]) =
  ...
  case class TestCaseGivenList(sources: List[TypedTestSource[_]]) {
    ...
    def When[TypeOut: Manifest](
        op: ListOfTypedPipesOperations[TypeOut]): TestCaseWhen[TypeOut] =
  ...
  case class CompleteTestCase[OutputType: Manifest](
    ...
    class DummyJob(args: Args) extends Job(args) {
      ...
      implicit val td: TypeDescriptor[OutputType] =
        new TypeDescriptor[OutputType] {
