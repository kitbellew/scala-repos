...
class Matrix[RowT, ColT, ValT](
  ...
  def mapValues[ValU](fn: (ValT) => ValU)(
    ...
    val newPipe = pipe.flatMap(valSym -> valSym) { imp: Tuple1[ValT] =>
      //Ensure an arity of 1
  ...
  def filterValues(fn: (ValT) => Boolean): Matrix[RowT, ColT, ValT] = {
    val newPipe = pipe.filter(valSym) { imp: Tuple1[ValT] =>
      //Ensure an arity of 1
      //This annoying Tuple1 wrapping ensures we can handle ValT that may itself be a Tuple.
      fn(imp._1)
    }
...
class RowVector[ColT, ValT](
  ...
  def mapValues[ValU](fn: (ValT) => ValU)(
    ...
    val newPipe = pipe.flatMap(valS -> valS) { imp: Tuple1[ValT] =>
      // Ensure an arity of 1
      //This annoying Tuple1 wrapping ensures we can handle ValT that may itself be a Tuple.
      mon.nonZeroOption(fn(imp._1)).map { Tuple1(_) }
    }
