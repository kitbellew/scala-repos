...
trait TypedPipe[+T] extends Serializable {
  ...
  /**
    ...
    */
  final def toPipe[U >: T](fieldNames: Fields)(
      implicit flowDef: FlowDef,
      mode: Mode,
  ...
  /**
    ...
    */
  private[typed] def asPipe[U >: T](fieldNames: Fields)(
      implicit flowDef: FlowDef,
      mode: Mode,
      setter: TupleSetter[U]): Pipe
  ...
  def sumByLocalKeys[K, V](
    ...
    TypedPipeFactory({ (fd, mode) =>
      ...
      val msr = new MapsideReduce(sg, 'key, 'value, None)(
        singleConverter[V],
        singleSetter[V])
      TypedPipe.from[(K, V)](pipe.eachTo(fields -> fields)(_ => msr), fields)(
        fd,
        mode,
        tuple2Converter)
    })
  ...
  def unpackToPipe[U >: T](fieldNames: Fields)(
      implicit fd: FlowDef,
      mode: Mode,
...
final case object EmptyTypedPipe extends TypedPipe[Nothing] {
  ...
  override def asPipe[U >: Nothing](fieldNames: Fields)(
      implicit fd: FlowDef,
      mode: Mode,
      setter: TupleSetter[U]): Pipe =
  ...
  override def sumByLocalKeys[K, V](
      implicit ev: Nothing <:< (K, V),
      sg: Semigroup[V]) = this
