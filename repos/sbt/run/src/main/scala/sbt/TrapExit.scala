...
object TrapExit {
  ...
  private def isDone(t: Thread): Boolean =
  ...
  private final class TrapInterrupt(
...
private final class TrapExit(delegateManager: SecurityManager)
  ...
  private[this] def runManaged0(f: xsbti.F0[Unit], xlog: xsbti.Logger): Int = {
    ...
    try {
      ...
      case e: InterruptedException =>
        // here, the thread that started the run has been interrupted, not the main thread of the executing code
  ...
  /**
    ...
    */
  private final class App(val execute: xsbti.F0[Unit], val log: Logger)
      extends Runnable {
    ...
    private[this] val groups =
      new TrieMap[ThreadID, WeakReference[ThreadGroup]]
  ...
  /**
    ...
    */
  private[this] def isEventQueue(t: Thread): Boolean =
