...
final case class Task[T](info: Info[T], work: Action[T]) {
  ...
  private[sbt] def mapTask(g: Task ~> Task): Task[T] = g(
    Task(info, work.mapTask(g)))
  def tag(tags: Tag*): Task[T] = tagw(tags.map(t => (t, 1)): _*)
  def tagw(tags: (Tag, Int)*): Task[T] = copy(info =
...
final case class Info[T](
  ...
  def set[T](key: AttributeKey[T], value: T) =
    copy(attributes = this.attributes.put(key, value))
