...
object TaskRunnerCircularTest extends Properties("TaskRunner Circular") {
  ...
  property("Allows references to completed tasks") =
    forAllNoShrink(MaxTasksGen, MaxWorkersGen) { allowedReference _ }
  ...
  def cyclic(i: Incomplete) =
    Incomplete.allExceptions(i).exists(
      _.isInstanceOf[Execute[Task]#CyclicException[_]])
}
