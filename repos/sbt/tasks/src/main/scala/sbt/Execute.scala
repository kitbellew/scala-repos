...
private[sbt] final class Execute[A[_] <: AnyRef](
  ...
  def run[T](root: A[T])(implicit strategy: Strategy): Result[T] = try {
  ...
  def addReverse(node: A[_], dependent: A[_]): Unit =
  ...
  def cyclic[T](caller: A[T], target: A[T], msg: String) = throw new Incomplete(
    Some(caller),
    message = Some(msg),
