...
object KeyIndex {
  ...
  def combine(indices: Seq[KeyIndex]): KeyIndex = new KeyIndex {
    ...
    def tasks(proj: Option[ResolvedReference], conf: Option[String]) = concat(
      _.tasks(proj, conf))
    def tasks(
    ...
    def keys(proj: Option[ResolvedReference], conf: Option[String]) = concat(
      _.keys(proj, conf))
    def keys(
        proj: Option[ResolvedReference],
        conf: Option[String],
...
trait KeyIndex {
  ...
  def tasks(
      proj: Option[ResolvedReference],
      conf: Option[String]): Set[AttributeKey[_]]
  def tasks(
      proj: Option[ResolvedReference],
      conf: Option[String],
...
// task axis <-> key
private final class AKeyIndex(
    val data: Relation[Option[AttributeKey[_]], String]) {
  def add(task: Option[AttributeKey[_]], key: AttributeKey[_]): AKeyIndex =
    new AKeyIndex(data + (task, key.rawLabel) + (task, key.label))
...
private final class KeyIndex0(val data: BuildIndex) extends ExtendableKeyIndex {
  ...
  def configs(project: Option[ResolvedReference]): Set[String] = confIndex(
    project).configs
  def tasks(
      proj: Option[ResolvedReference],
      conf: Option[String]): Set[AttributeKey[_]] = keyIndex(proj, conf).tasks
  def tasks(
      proj: Option[ResolvedReference],
      conf: Option[String],
  ...
  private[this] def add1(
    ...
    new KeyIndex0(
      data.add(uri, id, config.toOption.map(_.name), task.toOption, key))
}
