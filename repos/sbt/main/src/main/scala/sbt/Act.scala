...
object Act {
  ...
  def projectRef(
      index: KeyIndex,
      currentBuild: URI): Parser[ParsedAxis[ResolvedReference]] = {
  ...
  def optProjectRef(
      index: KeyIndex,
      current: ProjectRef): Parser[ParsedAxis[ResolvedReference]] =
  ...
  private[this] def actionParser: Parser[ActAction] =
    token(
      ((ShowCommand ^^^ ShowAction) | (MultiTaskCommand ^^^ MultiAction)) <~ Space
    ) ?? SingleAction
