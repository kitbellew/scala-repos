...
object BuiltinCommands {
  ...
  val spacedModeParser: State => Parser[Inspect.Mode] =
    Inspect.spacedModeParser
  ...
  def projectsParser(s: State): Parser[List[URI] => List[URI]] = {
    ...
    val removeBase =
      token(Space ~> "remove") ~> token(
        Space ~> Uri(Project.extraBuilds(s).toSet)).+
    addBase.map(toAdd => (xs: List[URI]) => (toAdd.toList ::: xs).distinct) | removeBase
