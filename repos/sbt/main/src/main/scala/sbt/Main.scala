...
object BuiltinCommands {
  ...
  def aboutPlugins(e: Extracted): String = {
    def list(b: BuildUnit) =
      b.plugins.detected.autoPlugins.map(
        _.value.label) ++ b.plugins.detected.plugins.names
    val allPluginNames =
  ...
  def aboutScala(s: State, e: Extracted): String = {
    ...
    val instance = e.getOpt(Keys.scalaInstance.task).flatMap(_ =>
      quiet(e.runTask(Keys.scalaInstance, s)._2))
  ...
  private[this] def selectScalaVersion(
    ...
    sv match { case Some(si.version) => si.version; case _ => si.actualVersion }
  ...
  private[this] def loadedEval(s: State, arg: String): Unit = {
    val extracted = Project extract s
    import extracted._
    val result = session.currentEval().eval(
      arg,
      srcName = "<eval>",
      imports = autoImports(extracted))
  ...
  private[this] def rawEval(s: State, arg: String): Unit = {
    ...
    val result = Load.mkEval(classpath, s.baseDir, Nil).eval(
      arg,
      srcName = "<eval>",
      imports = new EvalImports(Nil, ""))
  ...
  def project =
    Command.make(ProjectCommand, projectBrief, projectDetailed)(
  ...
  private[this] def doLoadFailed(s: State, loadArg: String): State = {
    val result = (SimpleReader.readLine(
      "Project loading failed: (r)etry, (q)uit, (l)ast, or (i)gnore? ") getOrElse Quit).toLowerCase(
      Locale.ENGLISH)
