...
object Load {
    ...
    ((loadedBuild in GlobalScope :== loaded) +:
      ...
      loaded.units.toSeq.flatMap {
        case (uri, build) =>
          ...
          val pluginBuildSettings = plugins.flatMap(_.buildSettings) ++ loaded.autos
            .buildSettings(uri)
  ...
  def loadUnit(
    ...
    val hasRoot = loadedProjectsRaw.projects.exists(_.base == normBase) || defsScala
      .exists(_.rootProject.isDefined)
