...
object Load {
  ...
  def injectGlobal(state: State): Seq[Setting[_]] =
    (appConfiguration in GlobalScope :== state.configuration) +: LogManager
      .settingsLogger(state) +: EvaluateTask.injectSettings
  ...
  def loadGlobalSettings(
    ...
    config.copy(
      injectSettings = config.injectSettings.copy(projectLoaded = compiled))
  }
  ...
  def loadUnit(
    ...
    val initialProjects =
      defsScala.flatMap(b => projectsFromBuild(b, normBase))
    ...
    val hasRoot =
      loadedProjectsRaw.projects.exists(_.base == normBase) ||
  ...
  private[this] def resolveProject(
    ...
    val allSettings = {
      ...
      def pluginSettings(f: Plugins) = {
        val included =
          loadedPlugins.detected.plugins.values
            .filter(f.include) // don't apply the filter to AutoPlugins, only Plugins
        included.flatMap(p =>
  ...
  def activateGlobalPlugin(
    ...
    config.globalPlugin match {
      case Some(gp) =>
        config.copy(
          injectSettings = config.injectSettings.copy(project = gp.inject))
      case None => config
    }
  def plugins(
      dir: File,
  ...
  def binaryPlugins(classpath: Seq[File], loader: ClassLoader): Seq[String] =
    PluginDiscovery.binaryModuleNames(
      classpath,
      loader,
      PluginDiscovery.Paths.Plugins)
