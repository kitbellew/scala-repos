...
object Load {
  ...
  def loadGlobalSettings(
    ...
    config.copy(injectSettings = config
      .injectSettings
  ...
  def configInheritRef(
    ...
    configurationOpt(lb.units, ref.build, ref.project, config)
      .toList
  ...
  def reapply(newSettings: Seq[Setting[_]], structure: sbt.BuildStructure)(
    ...
    val newData =
      Def.make(transformed)(structure.delegates, structure.scopeLocal, display)
  ...
  def loadUnit(
    ...
    val valDefinitions =
      memoSettings
        .values
        .foldLeft(DefinedSbtValues.empty) { (prev, sbtFile) =>
  ...
  private[this] def resolveProject(
    ...
    val transformedProject =
      configFiles
        .flatMap(_.manipulations)
        .foldLeft(rawProject) { (prev, t) =>
          t(prev)
        }
  ...
  val autoPluginSettings: Seq[Setting[_]] =
    inScope(GlobalScope in LocalRootProject)(
      Seq(
        sbtPlugin :== true,
        pluginData := {
