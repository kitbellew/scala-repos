...
object Load {
  ...
  def defaultLoad(
      ...
      topLevelExtras: List[URI] = Nil): (() => Eval, sbt.BuildStructure) = {
  ...
  def defaultPreGlobal(
      ...
      globalBase: File,
      log: Logger): sbt.LoadBuildConfiguration = {
    val provider = state.configuration.provider
    val scalaProvider = provider.scalaProvider
    val stagingDirectory = getStagingDirectory(state, globalBase)
      .getCanonicalFile
    val loader = getClass.getClassLoader
    val classpath = Attributed
      .blankSeq(provider.mainClasspath ++ scalaProvider.jars)
    val localOnly = false
    val lock = None
    val checksums = Nil
    ...
    new sbt.LoadBuildConfiguration(
      stagingDirectory,
      classpath,
      loader,
      compilers,
  ...
  def defaultWithGlobal(
      state: State,
      base: File,
      rawConfig: sbt.LoadBuildConfiguration,
      globalBase: File,
      log: Logger): sbt.LoadBuildConfiguration = {
    ...
    val globalSettings = configurationSources(
      getGlobalSettingsDirectory(state, globalBase))
  ...
  def apply(
      rootBase: File,
      s: State,
      config: sbt.LoadBuildConfiguration): (() => Eval, sbt.BuildStructure) = {
    ...
    def setDefining[T] =
      (key: ScopedKey[T], value: T) =>
        value match {
          ...
          case ik: InputTask[t] => ik.mapTask(tk => setDefinitionKey(tk, key))
              .asInstanceOf[T]
          case _ => value
        }
  ...
  def structureIndex(
      ...
      projects: Map[URI, LoadedBuildUnit]): sbt.StructureIndex = {
  ...
  def reapply(newSettings: Seq[Setting[_]], structure: sbt.BuildStructure)(
      implicit display: Show[ScopedKey[_]]): sbt.BuildStructure = {
    val transformed = finalTransforms(newSettings)
    val newData = Def
      .make(transformed)(structure.delegates, structure.scopeLocal, display)
    ...
    ((loadedBuild in GlobalScope :== loaded) +:
      ...
      loaded.units.toSeq.flatMap {
        case (uri, build) =>
          ...
          val pluginBuildSettings = plugins.flatMap(_.buildSettings) ++ loaded
            .autos.buildSettings(uri)
          ...
          val buildSettings = transformSettings(
            ...
            uri,
            rootProject,
            pluginNotThis ++ pluginBuildSettings ++ (buildBase +: build
              .buildSettings))
  ...
  def extractSettings(plugins: Seq[Plugin])
    ...
    (
      ...
      plugins.flatMap(_.buildSettings))
  ...
  def load(
      file: File,
      s: State,
      config: sbt.LoadBuildConfiguration): sbt.PartBuild =
    load(
      file,
      builtinLoader(
        s,
  ...
  def checkProjectBase(buildBase: File, projectBase: File): Unit = {
    ...
    assert(
      buildBase == projectBase || IO.relativize(buildBase, projectBase)
        .isDefined,
    ...
    builds map {
      case (uri, unit) =>
        (
          uri,
          unit
            .resolveRefs(ref => Scope.resolveProjectRef(uri, rootProject, ref)))
    }
  }
  def checkAll(
    ...
    for ((uri, refs) <- referenced; ref <- refs) {
      val ProjectRef(refURI, refID) = Scope
        .resolveProjectRef(uri, rootProject, ref)
  ...
  def resolveProjects(
    ...
    IO.assertAbsolute(uri)
    val resolve = (_: Project)
      .resolve(ref => Scope.resolveProjectRef(uri, rootProject, ref))
  ...
  def projects(unit: sbt.BuildUnit): Seq[Project] = {
    ...
    val resolveBuild = (_: Project)
      .resolveBuild(ref => Scope.resolveProjectBuild(unit.uri, ref))
  ...
  def emptyBuild(uri: URI) =
    sys.error(s"No root project defined for build unit '$uri'")
  def noBuild(uri: URI) = sys.error(s"Build unit '$uri' not defined.")
  def noProject(uri: URI, id: String) =
    sys.error(s"No project '$id' defined in '$uri'.")
  ...
  def loadUnit(
      ...
      localBase: File,
      s: State,
      config: sbt.LoadBuildConfiguration): sbt.BuildUnit = {
    ...
    val hasRoot = loadedProjectsRaw.projects
  ...
  private[this] def loadTransitive(
    ...
    def discoverAndLoad(p: Project): (Project, Seq[Project], Seq[File]) = {
      val (root, discovered, files, generated) =
        discover(p.auto, p.base) match {
          ...
          case DiscoveredProjects(None, rest, files, generated) =>
            (p, rest, files, generated)
        }
    ...
    newProjects match {
      ...
      case Nil if makeOrDiscoverRoot =>
        ...
        discover(AddSettings.defaultSbtFiles, buildBase) match {
          case DiscoveredProjects(Some(root), discovered, files, generated) =>
            log.debug(s"[Loading] Found root project ${root
          ...
          case DiscoveredProjects(None, discovered, files, generated) =>
            ...
            val root0 =
              if (discovered.isEmpty || java.lang.Boolean
      ...
      case Nil =>
        log.debug(s"[Loading] Done in ${buildBase}, returning: ${acc.map(_.id)
          .mkString("(", ", ", ")")}")
  ...
  private[this] def resolveProject(
    ...
    val transformedProject = configFiles.flatMap(_.manipulations)
      .foldLeft(rawProject) { (prev, t) => t(prev) }
    ...
    val allSettings = {
      ...
      def pluginSettings(f: Plugins) = {
        val included = loadedPlugins.detected.plugins.values
          .filter(
            f.include
          ) // don't apply the filter to AutoPlugins, only Plugins
        included
          .flatMap(p => p.settings.filter(isProjectThis) ++ p.projectSettings)
      }
    ...
    transformedProject.copy(settings = allSettings).setAutoPlugins(autoPlugins)
  ...
  def enableSbtPlugin(
    ...
    config.copy(injectSettings = config.injectSettings.copy(
      ...
      project = config.pluginManagement.inject ++ config.injectSettings
        .project))
  ...
  def noPlugins(
    ...
    loadPluginDefinition(
      dir,
      config,
      PluginData(config.globalPluginClasspath, None, None))
  def buildPlugins(
      dir: File,
      s: State,
      config: sbt.LoadBuildConfiguration): sbt.LoadedPlugins =
  ...
  def pluginDefinitionLoader(
      config: sbt.LoadBuildConfiguration,
      pluginData: PluginData): (Seq[Attributed[File]], ClassLoader) =
    pluginDefinitionLoader(
      config,
      pluginData.dependencyClasspath,
      pluginData.definitionClasspath)
  def pluginDefinitionLoader(
      config: sbt.LoadBuildConfiguration,
  ...
  def buildPluginDefinition(
    ...
    val pluginState = Project
      .setProject(Load.initialSession(pluginDef, eval), pluginDef, s)
  ...
  def loadPlugins(
      dir: File,
      data: PluginData,
      loader: ClassLoader): sbt.LoadedPlugins =
    new sbt.LoadedPlugins(
      dir,
      data,
      loader,
  ...
  @deprecated(
    "Replaced by the more general PluginDiscovery.binarySourceModuleNames and will be made private.",
    "0.13.2")
  def getPluginNames(
      classpath: Seq[Attributed[File]],
      loader: ClassLoader): Seq[String] =
    PluginDiscovery.binarySourceModuleNames(
      classpath,
      loader,
      PluginDiscovery.Paths.Plugins,
      classOf[Plugin].getName)
  ...
  def findPlugins(analysis: Analysis): Seq[String] =
    discover(analysis, "sbt.Plugin")

  @deprecated("No longer used.", "0.13.2")
  def findDefinitions(analysis: Analysis): Seq[String] =
    discover(analysis, "sbt.Build")
  ...
  def checkCycles(units: Map[URI, sbt.LoadedBuildUnit]): Unit =
  ...
  def importAllRoot(values: Seq[String]): Seq[String] =
    BuildUtil.importAllRoot(values)
  ...
  def buildUtil(
      root: URI,
      units: Map[URI, sbt.LoadedBuildUnit],
      keyIndex: KeyIndex,
