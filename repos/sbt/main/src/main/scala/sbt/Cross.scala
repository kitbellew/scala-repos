...
object Cross {
  ...
  lazy val switchVersion =
    Command.arb(requireSession(switchParser), switchHelp) {
      case (state, (arg, command)) =>
        ...
        val (add, exclude) =
          if (home.exists) {
            ...
            state
              .log
              .info(
                "Setting Scala home to " + home + " with actual version " +
        ...
        val isForceGc = getOpt(Keys.forcegc in Global) getOrElse
  ...
  private[this] def excludeKeys(
      keys: Set[AttributeKey[_]]): Setting[_] => Boolean =
    _.key match {
