...
object BuildPaths {
  ...
  val stagingDirectory = AttributeKey[File](
    "staging-directory",
    "The directory for staging remote projects.",
    DSetting)
  ...
  private[this] def defaultDependencyBase(globalBase: File) =
    globalBase / "dependency"

  def configurationSources(base: File): Seq[File] =
    (base * (GlobFilter("*.sbt") - ".sbt")).get
  def pluginDirectory(definitionBase: File) =
    definitionBase / PluginsDirectoryName
  ...
  @deprecated(
    "Use projectStandard.  The alternative project directory location has been removed.",
    "0.13.0")
