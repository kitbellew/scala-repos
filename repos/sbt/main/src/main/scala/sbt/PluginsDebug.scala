...
private[sbt] class PluginsDebug(
  ...
  def debug(notFoundKey: String, context: Context): String = {
    val (activated, deactivated) = Util
      .separate(toEnable(notFoundKey, context)) {
  ...
  private[this] def debugDeactivated(
    ...
    if (possible.nonEmpty) {
      ...
      explanations.mkString(
        s"Plugins are available that could provide $notFoundKey, but they are impossible to add:\n\t",
        "\n\t",
        "")
    }
  }
...
private[sbt] object PluginsDebug {
  def helpAll(s: State): String =
    if (Project.isProjectLoaded(s)) {
      ...
      def helpBuild(uri: URI, build: LoadedBuildUnit): String = {
        val pluginStrings = for (plugin <- availableAutoPlugins(build)) yield {
          val activatedIn = build.defined.values.toList
            .filter(_.autoPlugins.contains(plugin)).map(_.id)
          val actString =
      ...
      val buildStrings =
        for ((uri, build) <- structure.units) yield helpBuild(uri, build)
  ...
  def autoPluginMap(s: State): Map[String, AutoPlugin] = {
    val extracted = Project.extract(s)
    import extracted._
    structure.units.values.toList.flatMap(availableAutoPlugins)
      .map(plugin => (plugin.label, plugin)).toMap
  }
  ...
  def help(plugin: AutoPlugin, s: State): String = {
    ...
    val perBuild: Map[URI, Set[AutoPlugin]] = structure.units
      .mapValues(unit => availableAutoPlugins(unit).toSet)
    val pluginsThisBuild = perBuild.getOrElse(currentRef.build, Set.empty)
      .toList
    ...
    if (!pluginsThisBuild.contains(plugin)) {
      val availableInBuilds: List[URI] = perBuild.toList.filter(_._2(plugin))
        .map(_._1)
      ...
      val thisAggregated = BuildUtil.dependencies(structure.units)
        .aggregateTransitive.getOrElse(currentRef, Nil)
      val definedInAggregated = thisAggregated
        .filter(ref => definesPlugin(projectForRef(ref)))
      if (definedInAggregated.nonEmpty) {
        val projectNames = definedInAggregated
          .map(
            _.project
        ...
        val aggNote =
          if (thisAggregated.nonEmpty)
            "Note: This project aggregates other projects and this"
          else "Note: This"
  ...
  def apply(available: List[AutoPlugin]): PluginsDebug = {
    ...
    val nameToKey: Map[String, AttributeKey[_]] = keyR._2s.toList
      .map(key => (key.label, key)).toMap
  ...
  final case class PluginActivated(plugin: AutoPlugin, context: Context)
      extends PluginEnable
  ...
  private[this] def enableDeactivated(
      context: Context,
      plugin: AutoPlugin): PluginEnable = {
    ...
    if (contradictions.nonEmpty)
      ...
      val modelForMin = context.deducePlugin(
        and(includeAll(minRequiredPlugins), excludeAll(minAbsentPlugins)),
        context.log)
      ...
      val incrementalModel = context
        .deducePlugin(incrementalInputs, context.log).toSet
      ...
      val extraPlugins = incrementalModel.toSet -- minRequiredPlugins --
        initialModel
  ...
  def explainPluginEnable(ps: PluginEnable): String =
    ps match {
      case PluginRequirements(
        ...
        val parts =
          indent(excludedError(false /* TODO */, blockingExcludes.toList)) ::
            ...
            note(willRemove(plugin, toBeRemoved.toList)) :: Nil
      ...
      case PluginActivated(plugin, context) =>
        s"Plugin ${plugin.label} already activated."
    }
  ...
  private[this] def excludedError(
      transitive: Boolean,
  ...
  private[this] def excludedPluginsError(transitive: Boolean)(
      dependencies: List[AutoPlugin]) =
  ...
  private[this] def willAddPlugins(base: AutoPlugin)(
      plugins: List[AutoPlugin]) =
    s"Enabling ${base.label} will also enable:\n\t${labels(plugins).mkString("\n\t")}"

  private[this] def willRemove(
      base: AutoPlugin,
      plugins: List[AutoPlugin]): String =
  ...
  private[this] def willRemovePlugins(base: AutoPlugin)(
      plugins: List[AutoPlugin]) =
  ...
  private[this] def needToDeactivate(
