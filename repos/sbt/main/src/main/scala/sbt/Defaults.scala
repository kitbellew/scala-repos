...
object Defaults extends BuildCommon {
  ...
  def thisBuildCore: Seq[Setting[_]] =
    inScope(GlobalScope.copy(project = Select(ThisBuild)))(
      Seq(managedDirectory := baseDirectory.value / "lib_managed"))
  ...
  def paths =
    Seq(
      ...
      cacheDirectory := crossTarget
        .value / CacheDirectoryName / thisProject.value.id / "global"
    )
  ...
  lazy val outputConfigPaths = Seq(
    ...
    classDirectory := crossTarget
      .value / (prefix(configuration.value.name) + "classes"),
    target in doc := crossTarget
      .value / (prefix(configuration.value.name) + "api")
  )
  def addBaseSources =
    Seq(
      unmanagedSources := {
  ...
  def compilersSetting =
    compilers := Compiler.compilers(
      ...
      scalaCompilerBridgeSource
        .value)(appConfiguration.value, streams.value.log)
  ...
  lazy val configTasks = docTaskSettings(doc) ++ inTask(compile)(
    ...
    selectMainClass := mainClass
      .value orElse askForMainClass(discoveredMainClasses.value),
    ...
    Def.task {
      ...
      def files(id: String) =
        for {
          ...
          if art.`type` == Artifact.DefaultType
        } yield file
      def file(id: String) =
  ...
  lazy val testTasks: Seq[Setting[_]] =
    testTaskOptions(test) ++ testTaskOptions(testOnly) ++ testTaskOptions(
      ...
      loadedTestFrameworks := testFrameworks
        .value
        .flatMap(f =>
          f.create(testLoader.value, streams.value.log)
            .map(x => (f, x))
            .toIterable)
        .toMap,
      definedTests <<= detectTests,
      definedTestNames <<= definedTests map (
        _.map(_.name).distinct
      ) storeAs definedTestNames triggeredBy compile,
  ...
  def testTaskOptions(key: Scoped): Seq[Setting[_]] =
    inTask(key)(
      Seq(
        ...
        testOptions := Tests
          .Listeners(testListeners.value) +: (testOptions in TaskGlobal).value,
        testExecution <<= testExecutionTask(key)
      )) ++ inScope(GlobalScope)(
      Seq(derive(testGrouping <<= singleTestGroupDefault)))
  ...
  private[this] lazy val inputTests0: Initialize[InputTask[Unit]] = {
    val parser =
      loadForParser(definedTestNames)((s, i) =>
  ...
  def packageBinMappings =
    products map {
      _ flatMap Path.allSubpaths
    }
  def packageDocMappings =
    doc map {
  ...
  def sourceMappings =
    (unmanagedSources, unmanagedSourceDirectories, baseDirectory) map {
      (srcs, sdirs, base) =>
        (srcs --- sdirs --- base) pair (
          relativeTo(sdirs) | relativeTo(base) | flat
        )
    }
...
object Classpaths {
  ...
  def findClasspathConfig(
    ...
    val search = map(thisConfig) +: (
      delegated.toList ++ Seq(Compile, Configurations.Default)
    )
    def notFound =
      sys.error(
  ...
  val ivyPublishSettings: Seq[Setting[_]] = publishGlobalDefaults ++ Seq(
    ...
    packagedArtifact in makePom := (
      (artifact in makePom).value -> makePom.value
    ),
  ...
  val jvmBaseSettings: Seq[Setting[_]] = Seq(
    libraryDependencies ++= autoLibraryDependency(
      autoScalaLibrary
        .value && scalaHome.value.isEmpty && managedScalaInstance.value,
      sbtPlugin.value,
    ...
    allDependencies := {
      ...
      val pluginAdjust =
        if (sbtPlugin.value)
          ...
          base
      if (scalaHome.value.isDefined || ivyScala
            .value
            .isEmpty || !managedScalaInstance.value)
        pluginAdjust
      else {
        val version = scalaVersion.value
  ...
  def warnResolversConflict(ress: Seq[Resolver], log: Logger): Unit = {
    ...
    for ((name, r) <- resset groupBy (_.name)
         if r.size > 1) {
      log.warn(
  ...
  private[this] def sbtClassifiersGlobalDefaults =
    Defaults.globalDefaults(
      Seq(
        transitiveClassifiers in updateSbtClassifiers ~= (
          _.filter(_ != DocClassifier)
        )))
  ...
  def withExcludes(out: File, classifiers: Seq[String], lock: xsbti.GlobalLock)(
    ...
    lock(
      ...
      new Callable[UpdateReport] {
        def call = {
          val excludes =
            CacheIO.fromFile[Map[ModuleID, Set[String]]](
              excludeMap,
  ...
  def updateTask: Initialize[Task[UpdateReport]] =
    Def.task {
      ...
      val uc =
        (logLevel in update).?.value orElse st.get(logLevel.key) match {
  ...
  private[sbt] def dependencyPositionsTask
    ...
    Def.task {
      ...
      val f =
        Tracked.inputChanged(cacheFile / "input_dsp") {
  ...
  def exportProductsTask: Initialize[Task[Classpath]] =
    Def.task {
      ...
      for (f <- productsImplTask.value)
        yield APIMappings
          .store(analyzed(f, compile.value), apiURL.value)
  ...
  def interDependencies(
    ...
    for ((dep, c) <- visited)
      if (includeSelf || (dep != projectRef) || (
            conf.name != c && self.name != c
          ))
        tasks += f(dep, c, data)
  ...
  def autoPlugins(
    ...
    val pluginClasspath = report.matching(
      configurationFilter(CompilerPlugin.name)) ++ internalPluginClasspath
    val plugins = sbt
      .internal
      .inc
      .classpath
...
trait BuildExtra extends BuildCommon with DefExtra {
  ...
  def addArtifact(a: Artifact, taskDef: TaskKey[File]): SettingsDefinition = {
    val pkgd = packagedArtifacts := packagedArtifacts
      .value updated (a, taskDef.value)
  ...
  def addArtifact(
    ...
    val pkgd = packagedArtifacts := packagedArtifacts
      .value updated (artLocal.value, taskLocal.value)
