...
object Defaults extends BuildCommon {
  ...
  private[sbt] lazy val globalSbtCore: Seq[Setting[_]] = globalDefaults(
    Seq(
      ...
      fork :== false,
      initialize :== {},
      forcegc :== sys.props.get("sbt.task.forcegc").map(
        java.lang.Boolean.parseBoolean).getOrElse(
  ...
  def scalaInstanceFromUpdate: Initialize[Task[ScalaInstance]] =
    Def.task {
      ...
      val otherJars =
        allFiles.filterNot(x => x == libraryJar || x == compilerJar)
      new ScalaInstance(
        scalaVersion.value,
  ...
  lazy val testTasks: Seq[Setting[_]] =
    testTaskOptions(test) ++ testTaskOptions(testOnly) ++ testTaskOptions(
      ...
      loadedTestFrameworks := testFrameworks.value.flatMap(f =>
        f.create(testLoader.value, streams.value.log).map(x =>
          (f, x)).toIterable).toMap,
  ...
  def testTaskOptions(key: Scoped): Seq[Setting[_]] =
    inTask(key)(
      Seq(
        ...
        testOptions := Tests.Listeners(
          testListeners.value) +: (testOptions in TaskGlobal).value,
      ...
      Seq(
        packageOptions <<= (
          ...
          Package.addSpecManifestAttributes(
            name,
            ver,
            orgName) +: Package.addImplManifestAttributes(
            name,
            ver,
            h,
            org,
            orgName) +: main.map(Package.MainClass.apply) ++: p
        })) ++
      inTask(packageSrc)(
        Seq(
  ...
  def doClean(clean: Seq[File], preserve: Seq[File]): Unit =
    IO.withTemporaryDirectory { temp =>
      val (dirs, files) = preserve.filter(_.exists).flatMap(
        _.allPaths.get).partition(_.isDirectory)
      ...
      Seq(
        ...
        key in TaskGlobal := {
          ...
          (hasScala, hasJava) match {
            ...
            case (_, true) =>
              val javadoc = sbt.inc.Doc.cachedJavadoc(
                label,
                s.cacheDirectory / "java",
                cs.javac)
              javadoc.run(
                srcs.toList,
                cp,
  ...
  def consoleTask(
    ...
    (
      ...
      case (
        ...
        val loader = sbt.internal.inc.classpath.ClasspathUtilities.makeLoader(
          fullcp,
          si,
          IO.createUniqueDirectory(temp))
  ...
  def sbtPluginExtra(m: ModuleID, sbtV: String, scalaV: String): ModuleID =
    m.extra(
      ...
      PomExtraDependencyAttributes.ScalaVersionKey -> scalaV).copy(
      crossVersion = CrossVersion.Disabled)
  ...
  def writePluginsDescriptor(plugins: Set[String], dir: File): Seq[File] =
    PluginDiscovery.writeDescriptor(
      plugins.toSeq,
      dir,
      PluginDiscovery.Paths.Plugins).toList
  ...
  lazy val compileSettings: Seq[Setting[_]] =
    configSettings ++ (mainRunMainTask +: mainRunTask +: addBaseSources) ++ Classpaths.addUnmanagedLibrary
...
object Classpaths {
  ...
  val ivyBaseSettings: Seq[Setting[_]] =
    baseGlobalDefaults ++ sbtClassifiersTasks ++ Seq(
      conflictWarning := conflictWarning.value.copy(label =
  ...
  private[sbt] def defaultProjectID: Initialize[ModuleID] =
    Def.setting {
      val base =
        ModuleID(organization.value, moduleName.value, version.value).cross(
          crossVersion in projectID value).artifacts(artifacts.value: _*)
      apiURL.value match {
        case Some(u) =>
  ...
  def sbtClassifiersTasks =
    sbtClassifiersGlobalDefaults ++ inTask(updateSbtClassifiers)(
      Seq(
        externalResolvers := {
          val explicit = buildStructure.value.units(
            thisProjectRef.value.build).unit.plugins.pluginData.resolvers
  ...
  def exportProductsTask: Initialize[Task[Classpath]] =
    Def.task {
      ...
      for (f <- productsImplTask.value)
        yield APIMappings.store(analyzed(f, compile.value), apiURL.value).put(
          artifact.key,
          art).put(moduleID.key, module).put(configuration.key, config)
    }
  ...
  def confOpt(
      ...
      conf: String): Option[Configuration] =
  ...
  def managedJars(
    ...
    up.filter(
      configurationFilter(config.name) && artifactFilter(`type` =
        jarTypes)).toSeq.map {
      case (conf, module, art, file) =>
        Attributed(file)(
          AttributeMap.empty.put(artifact.key, art).put(
            moduleID.key,
            module).put(configuration.key, config))
    } distinct;
  ...
  def autoPlugins(
    ...
    val plugins = sbt.internal.inc.classpath.ClasspathUtilities.compilerPlugins(
      pluginClasspath)
  ...
  def substituteScalaFiles(scalaOrg: String, report: UpdateReport)(
    ...
    report.substitute { (configuration, module, arts) =>
      if (module.organization == scalaOrg) {
        ...
        val replaceWith =
          scalaJars(module.revision).filter(_.getName == jarName).map(f =>
            (Artifact(f.getName.stripSuffix(".jar")), f))
...
trait BuildExtra extends BuildCommon with DefExtra {
  ...
  def addArtifact(
    ...
    val pkgd =
      packagedArtifacts := packagedArtifacts.value updated (artLocal.value, taskLocal.value)
