...
object Defaults extends BuildCommon {
  ...
  def analysisMap[T](cp: Seq[Attributed[T]]): T => Option[CompileAnalysis] = {
    val m =
      (for (a <- cp; an <- a.metadata get Keys.analysis) yield (a.data, an))
        .toMap
    m.get _
  }
  ...
  def thisBuildCore: Seq[Setting[_]] =
    inScope(GlobalScope.copy(project = Select(ThisBuild)))(Seq(
      managedDirectory := baseDirectory.value / "lib_managed"))
  @deprecated("Use AutoPlugins and globalSbtCore instead.", "0.13.2")
  lazy val globalCore: Seq[Setting[_]] = globalDefaults(
    defaultTestTasks(test) ++ defaultTestTasks(testOnly) ++ defaultTestTasks(
      testQuick) ++ Seq(
      excludeFilter :== HiddenFileFilter) ++ globalIvyCore ++ globalJvmCore) ++ globalSbtCore

  private[sbt] lazy val globalJvmCore: Seq[Setting[_]] = Seq(
    compilerCache := state.value get Keys
      .stateCompilerCache getOrElse CompilerCache.fresh,
    sourcesInBase :== true,
  ...
  private[sbt] lazy val globalIvyCore: Seq[Setting[_]] = Seq(
  ...
  private[sbt] lazy val globalSbtCore: Seq[Setting[_]] = globalDefaults(Seq(
    ...
    fork :== false,
    initialize :== {},
    forcegc :== sys.props.get("sbt.task.forcegc")
  ...
  def paths =
    Seq(
      ...
      cacheDirectory := crossTarget.value / CacheDirectoryName / thisProject
        .value.id / "global"
    )

  lazy val configPaths =
    sourceConfigPaths ++ resourceConfigPaths ++ outputConfigPaths
  ...
  lazy val outputConfigPaths = Seq(
    cacheDirectory := crossTarget.value / CacheDirectoryName / thisProject.value
      .id / configuration.value.name,
  ...
  def compileBase =
    inTask(console)(compilersSetting :: Nil) ++ compileBaseGlobal ++ Seq(
      incOptions := incOptions.value
        .withClassfileManagerType(Maybe.just(new TransactionalManagerType(
      ...
      scalaCompilerBridgeSource := Compiler
        .defaultCompilerBridgeSource(scalaVersion.value)
    )
  ...
  private[this] lazy val configGlobal = globalDefaults(
    Seq(initialCommands :== "", cleanupCommands :== ""))
  ...
  def inAllConfigurations[T](key: TaskKey[T]): Initialize[Task[Seq[T]]] =
    (state, thisProjectRef) flatMap { (state, ref) =>
      val structure = Project structure state
      val configurations = Project.getProject(ref, structure).toList
        .flatMap(_.configurations)
      configurations
        .flatMap { conf => key in (ref, conf) get structure.data } join
    }
  ...
  def watchSetting: Initialize[Watched] =
    (pollInterval, thisProjectRef, watchingMessage, triggeredMessage) {
      (interval, base, msg, trigMsg) =>
        new Watched {
    ...
    Def.taskDyn {
      ...
      scalaHome.value match {
        ...
        case None =>
          ...
          if (version == scalaProvider
                .version) // use the same class loader as the Scala classes used by sbt
  ...
  private[sbt] def unmanagedScalaInstanceOnly
      : Initialize[Task[Option[ScalaInstance]]] =
    Def.taskDyn {
  ...
  def scalaInstanceFromUpdate: Initialize[Task[ScalaInstance]] =
    Def.task {
      val toolReport = update.value
        .configuration(Configurations.ScalaTool.name) getOrElse
      ...
      val otherJars = allFiles
        .filterNot(x => x == libraryJar || x == compilerJar)
      new ScalaInstance(
        scalaVersion.value,
        makeClassLoader(state.value)(
          libraryJar :: compilerJar :: otherJars.toList),
        libraryJar,
        compilerJar,
        otherJars.toArray,
        None)
  ...
  lazy val testTasks: Seq[Setting[_]] =
    testTaskOptions(test) ++ testTaskOptions(testOnly) ++ testTaskOptions(
      ...
      loadedTestFrameworks := testFrameworks.value.flatMap(f =>
        f.create(testLoader.value, streams.value.log).map(x => (f, x))
          .toIterable).toMap,
      ...
      executeTests <<= (
        ...
        testExecution in test,
        fullClasspath in test,
        javaHome in test,
        testForkedParallel,
        javaOptions in test) flatMap allTestGroupsTask,
      testResultLogger in (Test, test) :== TestResultLogger
        .SilentWhenNoTests, // https://github.com/sbt/sbt/issues/1185
      test := {
      ...
      testListeners := {
        TestLogger.make(
          ...
          closeableTestLogger(
            ...
            logBuffered.value)) +:
          new TestStatusReporter(succeededFile(
            streams.in(test).value.cacheDirectory)) +:
          testListeners.in(TaskGlobal).value
      },
      testOptions := Tests.Listeners(testListeners.value) +: (
        testOptions in TaskGlobal
      ).value,
  ...
  def testLogger(manager: Streams, baseKey: Scoped)(
  ...
  def singleTestGroupDefault: Initialize[Task[Seq[Tests.Group]]] =
    Def.task {
  ...
  private[this] lazy val inputTests0: Initialize[InputTask[Unit]] = {
    val parser = loadForParser(definedTestNames)((s, i) =>
      testOnlyParser(s, i getOrElse Nil))
    Def.inputTaskDyn {
      ...
      val modifiedOpts = Tests.Filters(filter(selected)) +: Tests
        .Argument(frameworkOptions: _*) +: config.options
    ...
    val groupTasks = groups map {
      case Tests.Group(name, tests, runPolicy) => runPolicy match {
          case Tests.SubProcess(opts) =>
            ...
            val forkedConfig = config
              .copy(parallel = config.parallel && forkedParallelExecution)
            s.log
              .debug(s"Forking tests - parallelism = ${forkedConfig.parallel}")
            ForkTests(
              runners,
              tests.toList,
              forkedConfig,
  ...
  lazy val packageConfig: Seq[Setting[_]] =
    inTask(packageBin)(Seq(
      packageOptions <<= (
        ...
        organizationName,
        mainClass,
        packageOptions) map { (name, ver, h, org, orgName, main, p) =>
        Package.addSpecManifestAttributes(name, ver, orgName) +: Package
          .addImplManifestAttributes(name, ver, h, org, orgName) +: main
          .map(Package.MainClass.apply) ++: p
      })) ++
  ...
  def collectFiles(
    ...
    (dirs, filter, excludes) map { (d, f, excl) =>
      d.descendantsExcept(f, excl).get
    }

  def artifactPathSetting(art: SettingKey[Artifact]) =
    (
      crossTarget,
      projectID,
      art,
  ...
  def artifactSetting =
    ((artifact, artifactClassifier).identity zipWith configuration.?) {
      case ((a, classifier), cOpt) =>
        ...
        if (combined.isEmpty)
          a.copy(classifier = None, configurations = cOpt.toList)
        else {
          val classifierString = combined mkString "-"
          val confs = cOpt.toList flatMap { c =>
            artifactConfigurations(a, c, classifier)
          }
          a.copy(
            classifier = Some(classifierString),
            `type` = Artifact.classifierType(classifierString),
  ...
  def doClean(clean: Seq[File], preserve: Seq[File]): Unit =
    IO.withTemporaryDirectory { temp =>
      val (dirs, files) = preserve.filter(_.exists).flatMap(_.allPaths.get)
  ...
  def runMainTask(
    ...
    val parser = loadForParser(discoveredMainClasses)((s, names) =>
  ...
  def runTask(
    ...
    Def.inputTask {
      val mainClass = mainClassTask.value getOrElse sys
        .error("No main class detected.")
      toError(scalaRun.value.run(
        mainClass,
    ...
    inTask(key)(Seq(
      apiMappings ++= {
        if (autoAPIMappings.value)
          APIMappings.extract(dependencyClasspath.value, streams.value.log)
            .toMap
      ...
      key in TaskGlobal := {
        val s = streams.value
        val cs: IncrementalCompilerImpl.Compilers = compilers.value match {
        ...
        (hasScala, hasJava) match {
          case (true, _) =>
            ...
            val runDoc = Doc.scaladoc(
              label,
              s.cacheDirectory / "scala",
              cs.scalac.onArgs(exported(s, "scaladoc")),
              fiOpts)
            runDoc(srcs, cp, out, options, maxErrors.value, s.log)
          case (_, true) =>
            val javadoc = sbt.inc.Doc
              .cachedJavadoc(label, s.cacheDirectory / "java", cs.javac)
            javadoc.run(
              srcs.toList,
              cp,
              out,
  ...
  def mainRunTask =
    run <<= runTask(fullClasspath in Runtime, mainClass in run, runner in run)
  def mainRunMainTask =
    runMain <<= runMainTask(fullClasspath in Runtime, runner in run)

  def discoverMainClasses(analysis: CompileAnalysis): Seq[String] =
    Discovery.applications(Tests.allDefs(analysis)).collect({
  ...
  def consoleTask(
      ...
      task: TaskKey[_]): Initialize[Task[Unit]] =
    (
      compilers in task,
      classpath in task,
      scalacOptions in task,
      initialCommands in task,
      cleanupCommands in task,
      taskTemporaryDirectory in task,
      ...
      case (
        ...
        )(cpFiles, options, loader, initCommands, cleanup)()(s.log)
  ...
  def compileTaskSettings: Seq[Setting[_]] =
    inTask(compile)(compileInputsSettings)

  def compileTask: Initialize[Task[CompileAnalysis]] =
    Def.task {
  ...
  private[this] def compileIncrementalTaskImpl(
      s: TaskStreams,
      ci: Inputs): CompileResult = {
    lazy val x = s.text(ExportStream)
    def onArgs(cs: IncrementalCompilerImpl.Compilers) =
      cs.copy(
        scalac = cs.scalac.onArgs(exported(x, "scalac")),
        javac = cs.javac /*.onArgs(exported(x, "javac"))*/ )
    val compilers: IncrementalCompilerImpl.Compilers = ci.compilers match {
  ...
  def compileAnalysisSettings: Seq[Setting[_]] =
    Seq(previousCompile := {
      val setup = compileIncSetup.value
      val store = MixedAnalyzingCompiler.staticCachedStore(setup.cacheFile)
      store.get() match {
        case Some((an, setup)) =>
          new PreviousResult(Maybe.just(an), Maybe.just(setup))
        case None =>
          new PreviousResult(
  ...
  def printWarningsTask: Initialize[Task[Unit]] =
    (streams, compile, maxErrors, sourcePositionMappers) map {
      case (s, analysis: Analysis, max, spms) =>
        val problems = analysis.infos.allInfos.values
          .flatMap(i => i.reportedProblems ++ i.unreportedProblems)
        val reporter =
          new LoggerReporter(max, s.log, Compiler.foldMappers(spms))
        problems foreach { p =>
  ...
  def writePluginsDescriptor(plugins: Set[String], dir: File): Seq[File] =
    PluginDiscovery
      .writeDescriptor(plugins.toSeq, dir, PluginDiscovery.Paths.Plugins).toList
  ...
  def copyResourcesTask =
    (classDirectory, resources, resourceDirectories, streams) map {
      (target, resrcs, dirs, s) =>
        val cacheFile = s.cacheDirectory / "copy-resources"
        val mappings =
          (resrcs --- dirs) pair (rebase(dirs, target) | flat(target))
        s.log.debug(
  ...
  @deprecated("Use the new <key>.all(<ScopeFilter>) API", "0.13.0")
  def inDependencies[T](
      key: SettingKey[T],
      default: ProjectRef => T,
      includeRoot: Boolean = true,
      classpath: Boolean = true,
      aggregate: Boolean = false): Initialize[Seq[T]] =
    forDependencies[T, T](
      ref => (key in ref) ?? default(ref),
      includeRoot,
      classpath,
      aggregate)

  @deprecated("Use the new <key>.all(<ScopeFilter>) API", "0.13.0")
  def forDependencies[T, V](
      init: ProjectRef => Initialize[V],
      includeRoot: Boolean = true,
      classpath: Boolean = true,
  ...
  def transitiveDependencies(
      ...
      includeRoot: Boolean,
      classpath: Boolean = true,
      aggregate: Boolean = false): Seq[ProjectRef] = {
    def tdeps(
        enabled: Boolean,
    ...
    def fullCp =
      tdeps(
        classpath,
        getDependencies(structure, classpath = true, aggregate = false))
    def fullAgg =
      tdeps(
        aggregate,
  ...
  def noAggregation: Seq[Scoped] =
    Seq(run, runMain, console, consoleQuick, consoleProject)
  lazy val disableAggregation = Defaults
    .globalDefaults(noAggregation map disableAggregate)
  def disableAggregate(k: Scoped) = aggregate in k :== false

  lazy val runnerSettings: Seq[Setting[_]] = Seq(runnerTask)
  lazy val baseTasks: Seq[Setting[_]] = projectTasks ++ packageBase

  lazy val baseClasspaths: Seq[Setting[_]] = Classpaths
    .publishSettings ++ Classpaths.baseSettings
  lazy val configSettings: Seq[Setting[_]] = Classpaths
    .configSettings ++ configTasks ++ configPaths ++ packageConfig ++ Classpaths
    .compilerPluginConfig

  lazy val compileSettings: Seq[Setting[_]] = configSettings ++ (
    mainRunMainTask +: mainRunTask +: addBaseSources
  ...
  lazy val projectBaseSettings: Seq[Setting[_]] =
    projectCore ++ runnerSettings ++ paths ++ baseClasspaths ++ baseTasks ++ compileBase ++ disableAggregation
  ...
  lazy val coreDefaultSettings: Seq[Setting[_]] =
    projectCore ++ disableAggregation ++ Seq(
      ...
      target := baseDirectory.value / "target")
  @deprecated(
    "Default settings split into coreDefaultSettings and IvyModule/JvmModule plugins.",
    "0.13.2")
  lazy val defaultSettings: Seq[Setting[_]] =
    projectBaseSettings ++ defaultConfigs
...
object Classpaths {
  ...
  def concatDistinct[T](
      ...
      b: ScopedTaskable[Seq[T]]): Initialize[Task[Seq[T]]] =
    (a, b) map { (x, y) => (x ++ y).distinct }
  def concat[T](
      a: ScopedTaskable[Seq[T]],
      b: ScopedTaskable[Seq[T]]): Initialize[Task[Seq[T]]] = (a, b) map (_ ++ _)
  def concatSettings[T](
      a: SettingKey[Seq[T]],
      b: SettingKey[Seq[T]]): Initialize[Seq[T]] = (a, b)(_ ++ _)
  ...
  def packaged(
      pkgTasks: Seq[TaskKey[File]]): Initialize[Task[Map[Artifact, File]]] =
  ...
  val ivyPublishSettings: Seq[Setting[_]] = publishGlobalDefaults ++ Seq(
    ...
    packagedArtifact in makePom := ((artifact in makePom).value -> makePom
      .value),
  ...
  @deprecated(
    "This has been split into jvmPublishSettings and ivyPublishSettings.",
    "0.13.2")
  val publishSettings: Seq[Setting[_]] =
    ivyPublishSettings ++ jvmPublishSettings
  ...
  private[this] def baseGlobalDefaults =
    Defaults.globalDefaults(Seq(
      ...
      sbtDependency := {
        ...
        val cross =
          if (id.crossVersioned) CrossVersion.binary else CrossVersion.Disabled
        val base = ModuleID(
          id.groupID,
          id.name,
          sbtVersion.value,
  ...
  val ivyBaseSettings: Seq[Setting[_]] =
    baseGlobalDefaults ++ sbtClassifiersTasks ++ Seq(
      ...
      appResolvers := {
        ...
        appRepositories(ac) map { ars =>
          Resolver.reorganizeAppResolvers(ars, uj, true)
        }
      },
      ...
      ivyScala <<= ivyScala or (
        ...
        Some(new IvyScala(
          ...
          checkExplicit = true,
          overrideScalaVersion = plugin,
          scalaOrganization = so))
      },
      artifactPath in makePom <<= artifactPathSetting(artifact in makePom),
      publishArtifact in makePom := publishMavenStyle.value && publishArtifact
        .value,
      ...
      updateConfiguration := {
        ...
        val specialArtifactTypes = sourceArtifactTypes
          .value union docArtifactTypes.value
        // By default, to retrieve all types *but* these (it's assumed that everything else is binary/resource)
        new UpdateConfiguration(
          retrieveConfiguration.value,
          false,
      ...
      retrieveConfiguration := {
        if (retrieveManaged.value)
          Some(new RetrieveConfiguration(
      ...
      ivyConfigurations := {
        ...
        (confs ++ confs.map(internalConfigurationMap.value) ++ (
          if (autoCompilerPlugins.value) CompilerPlugin :: Nil else Nil
        )).distinct
      },
      ...
      classifiersModule in updateClassifiers := {
        ...
        val externalModules = update.value.allModules
          .filterNot(m => projectDeps contains key(m))
        GetClassifiersModule(
          projectID.value,
          externalModules,
          ivyConfigurations.in(updateClassifiers).value,
          transitiveClassifiers.in(updateClassifiers).value)
      },
  ...
  val jvmBaseSettings: Seq[Setting[_]] = Seq(
    libraryDependencies ++= autoLibraryDependency(
      autoScalaLibrary.value && scalaHome.value.isEmpty && managedScalaInstance
        .value,
      sbtPlugin.value,
    ...
    allDependencies := {
      ...
      if (scalaHome.value.isDefined || ivyScala.value
            .isEmpty || !managedScalaInstance.value) pluginAdjust
      else {
        val version = scalaVersion.value
        val isDotty = ScalaInstance.isDotty(version)
        ScalaArtifacts.toolDependencies(
          scalaOrganization.value,
          version,
          isDotty) ++ pluginAdjust
      }
    }
  )
  ...
  private[sbt] def defaultProjectID: Initialize[ModuleID] =
    Def.setting {
      val base = ModuleID(organization.value, moduleName.value, version.value)
        .cross(crossVersion in projectID value).artifacts(artifacts.value: _*)
      apiURL.value match {
        case Some(u) =>
  ...
  def sbtClassifiersTasks =
    sbtClassifiersGlobalDefaults ++ inTask(updateSbtClassifiers)(Seq(
      externalResolvers := {
        val explicit = buildStructure.value.units(thisProjectRef.value.build)
          .unit.plugins.pluginData.resolvers
      ...
      classifiersModule <<= (
        ...
        val pluginJars = pluginClasspath
          .filter(
            _.data.isFile
        ...
        withExcludes(out, mod.classifiers, lock(app)) {
          excludes =>
            val noExplicitCheck = ivyScala.value
              .map(_.copy(checkExplicit = false))
      ...
      bootIvyConfiguration := (ivyConfiguration in updateSbtClassifiers).value)

  def deliverTask(
      config: TaskKey[DeliverConfiguration]): Initialize[Task[File]] =
    (ivyModule, config, update, streams) map { (module, config, _, s) =>
      IvyActions.deliver(module, config, s.log)
    }
  def publishTask(
  ...
  def updateTask: Initialize[Task[UpdateReport]] =
    Def.task {
      ...
      val forceUpdateByTime = forceUpdate match {
        ...
        case Some(period) =>
          val elapsedDuration = new FiniteDuration(
            System.currentTimeMillis() - fullUpdateOutput.lastModified(),
      ...
      val subScalaJars: String => Seq[File] = Defaults
        .unmanagedScalaInstanceOnly.value match {
    ...
    def work =
      (_: In) match {
        case conf :+: settings :+: config :+: HNil =>
          ...
          ew.lines foreach { log.warn(_) }
          ew.infoAllTheThings foreach { log.info(_) }
          val cw = CompatibilityWarning
            .run(compatWarning, module, mavenStyle, log)
          result
      }
  ...
  private[sbt] def dependencyPositionsTask
      : Initialize[Task[Map[ModuleID, SourcePosition]]] =
    Def.task {
      ...
      val f = Tracked.inputChanged(cacheFile / "input_dsp") {
        (inChanged: Boolean, in: Seq[ModuleID]) =>
          val outCache = Tracked
            .lastOutput[Seq[ModuleID], Map[ModuleID, SourcePosition]](
              outCacheFile) {
  ...
  def defaultRepositoryFilter =
    (repo: MavenRepository) => !repo.root.startsWith("file:")
  ...
  def deliverConfig(
      ...
      logging: UpdateLogging.Value = UpdateLogging.DownloadOnly) =
    new DeliverConfiguration(
      deliverPattern(outputDirectory),
      status,
      None,
      logging)
  @deprecated(
    "Previous semantics allowed overwriting cached files, which was unsafe. Please specify overwrite parameter.",
    "0.13.2")
  def publishConfig(
      ...
      resolverName: String,
      logging: UpdateLogging.Value): PublishConfiguration =
    publishConfig(
      artifacts,
      ivyFile,
      checksums,
  ...
  def depMap: Initialize[Task[Map[ModuleRevisionId, ModuleDescriptor]]] =
    (thisProjectRef, settingsData, buildDependencies, streams) flatMap {
      (root, data, deps, s) =>
  ...
  def exportProductsTask: Initialize[Task[Classpath]] =
    Def.task {
      ...
      for (f <- productsImplTask.value)
        yield APIMappings.store(analyzed(f, compile.value), apiURL.value)
          .put(artifact.key, art).put(moduleID.key, module)
  ...
  def mkIvyConfiguration: Initialize[Task[IvyConfiguration]] =
    (
      ...
      updateOptions,
      streams) map {
      (rs, paths, other, moduleConfs, off, check, app, t, uo, s) =>
  ...
  def interDependencies(
    ...
    for ((dep, c) <- visited)
      if (includeSelf || (dep != projectRef) || (conf.name != c && self
            .name != c)) tasks += f(dep, c, data)
  ...
  def mapped(
    ...
    lazy val defaultMap = parseMapping(
      defaultMapping,
      masterConfs,
      depConfs,
      _ :: Nil)
  ...
  def parseMapping(
    ...
    union(
      confString.split(";") map parseSingleMapping(
        masterConfs,
        depConfs,
        default))
  def parseSingleMapping(
    ...
    val ms: Seq[(String, Seq[String])] = trim(confString.split("->", 2)) match {
      case x :: Nil =>
  ...
  def getConfigurations(
      p: ResolvedReference,
      data: Settings[Scope]): Seq[Configuration] =
    ivyConfigurations in p get data getOrElse Nil
  def confOpt(
      configurations: Seq[Configuration],
      conf: String): Option[Configuration] = configurations.find(_.name == conf)
  def productsTask(
  ...
  def defaultConfigurationTask(
      p: ResolvedReference,
      data: Settings[Scope]): Configuration =
  ...
  def addUnmanagedLibrary: Seq[Setting[_]] =
    Seq(unmanagedJars in Compile <++= unmanagedScalaLibrary)
    ...
    up.filter(
      ...
      .toSeq.map {
        case (conf, module, art, file) =>
          Attributed(file)(
            AttributeMap.empty.put(artifact.key, art).put(moduleID.key, module)
              .put(configuration.key, config))
      } distinct;
  ...
  def autoPlugins(
      report: UpdateReport,
      internalPluginClasspath: Seq[File]): Seq[String] = {
    val pluginClasspath = report.matching(configurationFilter(
      CompilerPlugin.name)) ++ internalPluginClasspath
    val plugins = sbt.internal.inc.classpath.ClasspathUtilities
      .compilerPlugins(pluginClasspath)
  ...
  lazy val compilerPluginConfig = Seq(scalacOptions := {
  ...
  def appRepositories(app: xsbti.AppConfiguration): Option[Seq[Resolver]] =
    try {
      Some(
        app.provider.scalaProvider.launcher.appRepositories
          .toSeq map bootRepository)
    } catch { case _: NoSuchMethodError => None }

  def bootRepositories(app: xsbti.AppConfiguration): Option[Seq[Resolver]] =
    try {
      Some(
        app.provider.scalaProvider.launcher.ivyRepositories
          .toSeq map bootRepository)
    } catch { case _: NoSuchMethodError => None }
...
trait BuildExtra extends BuildCommon with DefExtra {
  ...
  def addSbtPlugin(
      dependency: ModuleID,
      sbtVersion: String): Setting[Seq[ModuleID]] =
    libraryDependencies <+= (scalaBinaryVersion in update) { scalaV =>
  ...
  def addSbtPlugin(dependency: ModuleID): Setting[Seq[ModuleID]] =
    libraryDependencies <+= (
      sbtBinaryVersion in update,
      scalaBinaryVersion in update) { (sbtV, scalaV) =>
  ...
  def addArtifact(a: Artifact, taskDef: TaskKey[File]): SettingsDefinition = {
    val pkgd =
      packagedArtifacts := packagedArtifacts.value updated (a, taskDef.value)
  ...
  def addArtifact(
    ...
    val pkgd = packagedArtifacts := packagedArtifacts.value updated (
      artLocal.value, taskLocal.value
    )
  ...
  @deprecated(
    ...
    "0.13.0")
  def seq(settings: Setting[_]*): SettingsDefinition =
    new Def.SettingList(settings)
  ...
  def externalIvySettingsURI(
      ...
      addMultiResolver: Boolean = true): Setting[Task[IvyConfiguration]] = {
    val other =
      (baseDirectory, appConfiguration, projectResolver, updateOptions, streams)
        .identityMap
  ...
  private[this] def inBase(name: String): Initialize[File] =
    Def.setting { baseDirectory.value / name }

  def externalIvyFile(
      file: Initialize[File] = inBase("ivy.xml"),
      iScala: Initialize[Option[IvyScala]] = ivyScala)
      : Setting[Task[ModuleSettings]] =
    moduleSettings := new IvyFileConfiguration(
      file.value,
      iScala.value,
      ivyValidate.value,
  ...
  def externalPom(
      ...
      iScala: Initialize[Option[IvyScala]] = ivyScala)
      : Setting[Task[ModuleSettings]] =
    moduleSettings := new PomConfiguration(
      file.value,
      ivyScala.value,
      ivyValidate.value,
      managedScalaInstance.value)
  ...
  def fullRunInputTask(
      ...
      config: Configuration,
      mainClass: String,
      baseArguments: String*): Setting[InputTask[Unit]] =
    scoped <<= inputTask { result =>
      (initScoped(scoped.scopedKey, runnerInit) zipWith (
  ...
  def noTestCompletion(config: Configuration = Test): Setting[_] =
    inConfig(config)(Seq(definedTests <<= detectTests)).head
...
trait BuildCommon {
  ...
  def getFromContext[T](
      task: TaskKey[T],
      context: ScopedKey[_],
      s: State): Option[T] =
    SessionVar
      .get(SessionVar.resolveContext(task.scopedKey, context.scope, s), s)

  def loadFromContext[T](task: TaskKey[T], context: ScopedKey[_], s: State)(
      implicit f: sbinary.Format[T]): Option[T] =
    SessionVar
      .load(SessionVar.resolveContext(task.scopedKey, context.scope, s), s)
