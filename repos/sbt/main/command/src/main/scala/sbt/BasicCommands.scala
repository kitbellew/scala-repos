...
object BasicCommands {
  ...
  def stashOnFailure =
    Command.command(StashOnFailure)(s =>
      s.copy(onFailure = None).update(OnFailureStack)(
        s.onFailure :: _.toList.flatten))
  def popOnFailure =
  ...
  def call =
    Command(ApplyCommand, Help.more(ApplyCommand, ApplyDetailed))(_ =>
      ...
      case (state, (cp, args)) =>
        ...
        state.log.info("Applying State transformations " + args.mkString(
          ", ") + (if (cp.isEmpty) ""
      ...
      line match {
        case Some(line) =>
          val newState = s.copy(
            ...
            remainingCommands =
              line +: Shell +: s.remainingCommands).setInteractive(true)
  ...
  def alias =
    Command.make(AliasCommand, Help.more(AliasCommand, AliasDetailed)) { s =>
