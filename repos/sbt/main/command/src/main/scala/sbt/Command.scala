...
object Command {
  ...
  def command(name: String, briefHelp: String, detail: String)(
      f: State => State): Command =
    command(name, Help(name, (name, briefHelp), detail))(f)
  def command(name: String, help: Help = Help.empty)(
      f: State => State): Command =
  ...
  def make(name: String, help: Help = Help.empty)(
      parser: State => Parser[() => State]): Command =
  ...
  def custom(
      ...
      help: Help = Help.empty): Command = customHelp(parser, const(help))
  def customHelp(
      parser: State => Parser[() => State],
  ...
  def suggestions(
    ...
    bs.map { b => (b, distance(a, b)) } filter (_._2 <= maxDistance) sortBy (_
      ._2) take (maxSuggestions) map (_._1)
...
object Help {
  ...
  def briefOnly(help: Seq[(String, String)]): Help =
    apply(help, Map.empty[String, String])
