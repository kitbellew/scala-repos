...
object Tests {
  ...
  final case class Listeners(listeners: Iterable[TestReportListener])
      extends TestOption
  ...
  final case class Filters(filterTest: Seq[String => Boolean])
      extends TestOption
  ...
  def foldTasks(results: Seq[Task[Output]], parallel: Boolean): Task[Output] =
    if (results.isEmpty)
      task {
        Output(TestResult.Passed, Map.empty, Nil)
      }
    else if (parallel)
      reduced(
