...
object CacheIvy {
    ...
    implicit val inlineWithExcludesToHL =
      (c: InlineConfigurationWithExcludes) =>
        c.module :+: c.dependencies :+: c.ivyXML :+: c.configurations :+: c.defaultConfiguration
          .map(_.name) :+: c.ivyScala :+: c.validate :+: c.overrides :+: c.excludes :+: HNil
  ...
  object L3 {
    ...
    implicit def mavenRToHL =
      (m: MavenRepository) => m.name :+: m.root :+: HNil
    implicit def fileRToHL =
      (r: FileRepository) => r.name :+: r.configuration :+: r.patterns :+: HNil
    implicit def urlRToHL =
      (u: URLRepository) => u.name :+: u.patterns :+: HNil
