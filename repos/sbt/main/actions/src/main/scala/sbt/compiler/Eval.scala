...
final class Eval(
  ...
  private[this] def evalCommon[T](
    ...
    def unlinkAll(): Unit = for ((sym, _) <- run.symSource)
  ...
  def loadPlain(dir: File, moduleName: String): ClassLoader => Any = parent =>
    getValue[Any](
      moduleName,
  ...
  private[this] def parseDefinitions(
      parser: syntaxAnalyzer.UnitParser): Seq[Tree] = {
  ...
  private[this] def makeModuleName(hash: String): String =
    "$" + Hash.halve(hash)
...
private[sbt] object Eval {
  ...
  def filesModifiedBytes(fs: Array[File]): Array[Byte] = if (fs eq null)
