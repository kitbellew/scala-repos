...
object Sync {
    ...
    mappings => {
      ...
      def outofdate(source: File, target: File): Boolean =
        !previousRelation.contains(source, target) ||
          (previousInfo get source) !=
          (currentInfo get source) || !target.exists ||
  ...
  implicit def relationFormat[A, B](implicit
    ...
    asProduct2[Relation[A, B], Map[A, Set[B]], Map[B, Set[A]]](Relation.make _)(
      r => (r.forwardMap, r.reverseMap))(af, bf)
  ...
  def readInfo[F <: FileInfo](file: File)(implicit
      infoFormat: Format[F]): RelationInfo[F] =
    try {
