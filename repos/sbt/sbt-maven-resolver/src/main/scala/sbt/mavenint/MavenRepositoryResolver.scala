...
/**
  ...
  */
abstract class MavenRepositoryResolver(settings: IvySettings)
  ...
  override def getDependency(
    ...
    try {
        ...
        if (MakePom.isDependencyVersionRange(
          ...
          val coords =
            s"${dd.getDependencyRevisionId.getOrganisation}:${aetherArtifactIdFromMrid(
              dd.getDependencyRevisionId)}:${revision}"
          ...
          if (result.getVersions.isEmpty)
            throw new MavenResolutionException(
              s"Did not find any versions for $dd")
      ...
      Message.debug(s"Requesting conf [${dd.getModuleConfigurations
        .mkString(",")}] from Aether module ${drid} in resolver ${getName}")
  ...
  def addDependenciesFromAether(
    ...
    for (d <- result.getDependencies.asScala) {
      ...
      if ((d.getArtifact.getClassifier != null) || ((d.getArtifact
            .getExtension != null) && !("jar" == d.getArtifact.getExtension))) {
        val tpe: String =
  ...
  private def getPackagingFromPomProperties(
