...
object Formatting {
  ...
  lazy val prefs: Seq[Setting[_]] = {
    ...
    Seq(
      sr.preferences :=
        sr.preferences.value.setPreference(AlignSingleLineCaseStatements, true))
  }
  lazy val sbtFilesSettings: Seq[Setting[_]] = Seq() ++ scalariformSettings ++
    prefs ++ inConfig(BuildConfig)(configScalariformSettings) ++
    ...
    Seq(
      ...
      includeFilter in (BuildConfig, sr.format) :=
        ("*.scala": FileFilter),
      includeFilter in (BuildSbtConfig, sr.format) :=
        ("*.sbt": FileFilter),
