...
trait Exprs extends Core with Types with Xml {
  ...
  class WsCtx(curlyBlock: Boolean) {
    ...
    val Enumerators = {
      ...
      val Assign = P(`=` ~/ Expr)
      val Enumerator = P(
        Semis ~ `val`.? ~ TypeOrBindPattern ~/ (Generator | Assign) | Semis.? ~ Guard)
    ...
    val Expr: P0 = {
      ...
      val For = {
        val Body = P(
          "(" ~/ ExprCtx.Enumerators ~ ")" | "{" ~/ StatCtx.Enumerators ~ "}")
      ...
      val ParenedLambda = P(
        Parened ~~ (WL ~ `=>` ~ LambdaRhs.? | ExprSuffix ~~ PostfixSuffix))
    ...
    val ExprSuffix = P(
      (WL ~ "." ~/ Id | WL ~ TypeArgs | NoSemis ~ ArgList).repX ~~ (NoSemis ~ `_`).?)
    ...
    val InfixSuffix = P(
      NoSemis ~~ WL ~~ Id ~ TypeArgs.? ~~ OneSemiMax ~ PrefixExpr ~~ ExprSuffix)
    val PostFix = P(NoSemis ~~ WL ~~ Id ~ Newline.?)

    val PostfixSuffix = P(
      InfixSuffix.repX ~~ PostFix.? ~ (`=` ~/ Expr).? ~ MatchAscriptionSuffix.?)
  ...
  val CaseClauses: P0 = {
    ...
    val CaseClause: P0 = P(
      `case` ~ !(`class` | `object`) ~/ Pattern ~ ExprCtx.Guard.? ~ `=>` ~ Block)
    P(CaseClause.rep(1) ~ "}")
  }
}
