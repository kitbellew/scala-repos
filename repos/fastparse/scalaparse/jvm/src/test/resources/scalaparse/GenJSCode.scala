...
abstract class GenJSCode
  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    override def apply(cunit: CompilationUnit) {
      try {
        val generatedClasses = ListBuffer
          .empty[(Symbol, js.ClassDef, ClassInfo)]
          ...
          if (!isPrimitive && !isRawJSImplClass) {
            withScopedVars(
              ...
              val tree =
                ...
                } else if (sym.isInterface) { genInterface(cd) }
                else if (sym.isImplClass) { genImplClass(cd) }
                else { genClass(cd) }

              currentClassInfoBuilder.setEncodedName(encodeClassFullName(sym))
    ...
    def genClass(cd: ClassDef): js.ClassDef = {
      ...
      val reflProxies = if (isHijacked) Nil else genReflCallProxies(sym)

      // Hashed definitions of the class
      val hashedDefs = Hashers
        .hashDefs(generatedMembers ++ exports ++ reflProxies)
    ...
    def genRawJSClassData(cd: ClassDef): js.ClassDef = {
      ...
      val jsName =
        if (sym.isInterface || sym.isModuleClass) None else Some(jsNameOf(sym))
    ...
    def genMethodWithInfoBuilder(
      ...
      val result = withScopedVars(
        ...
        def createInfoBuilder() = {
          new MethodInfoBuilder().setEncodedName(methodIdent.name)
        ...
        if (scalaPrimitives.isPrimitive(sym)) { None }
          ...
          Some(
            ...
            infoBuilder)
        } else if (isRawJSCtorDefaultParam(sym)) { None }
        else if (isTrivialConstructor(sym, params, rhs)) { None }
    ...
    private def isTrivialConstructor(
        ...
        params: List[Symbol],
        rhs: Tree): Boolean = {
      if (!sym.isClassConstructor) { false }
      else {
        rhs match {
          ...
          case Block(
            ...
            if (encodeMethodSym(sym) == encodeMethodSym(callee)) {
              ...
              params.zip(args) forall {
                case (param, arg) => arg.symbol == param
              }
            } else { false }
    ...
    private def genReflCallProxy(sym: Symbol): js.MethodDef = {
      ...
      withNewLocalNameScope {
        withScopedVars(currentMethodInfoBuilder := new MethodInfoBuilder) {
          ...
          val resTpeEnteringPosterasure = enteringPhase(
            currentRun.posterasurePhase) {
            sym.tpe match {
            ...
            if (methodTailJumpThisSym.get == NoSymbol) { innerBody }
              ...
              js.Block(
                js.VarDef(
                  ...
                  currentClassType,
                  methodTailJumpThisSym.isMutable,
    ...
    def genApply(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      fun match {
        ...
        case Select(Super(_, _), _) => genSuperCall(tree)
    ...
    private def addAllInfoOfAnonFunction(funInfo: MethodInfo): Unit = {
      ...
      for (cls <- funInfo.instantiatedClasses) builder.addInstantiatedClass(cls)

      for (cls <- funInfo.accessedModules) builder.addAccessedModule(cls)

      for (cls <- funInfo.usedInstanceTests) builder.addUsedInstanceTest(cls)

      for (cls <- funInfo.accessedClassData) builder.addAccessedClassData(cls)
    }
    ...
    def genTranslatedMatch(cases: List[LabelDef], matchEnd: LabelDef)(implicit
      ...
      js.Labeled(
        encodeLabelSym(matchEnd.symbol),
    ...
    private def genPrimitiveJSCall(tree: Apply, isStat: Boolean): js.Tree = {
      ...
      val boxedResult = sym.name match {
        ...
        case nme.apply
            if receiver0.tpe.typeSymbol.isSubClass(JSThisFunctionClass) =>
        ...
        case _ =>
          ...
          if (sym.hasFlag(reflect.internal.Flags.DEFAULTPARAM)) {
            ...
            args match {
              case List(keyArg) => js.JSBracketSelect(receiver, keyArg)
      ...
      if (compMembers.isEmpty) {
        ...
        assert(
          ...
          s"""For constructor with type ${ctor.tpe} on java.lang.String,
                                                     |found multiple companion module members."""
            .stripMargin
        )
    ...
    private def genPrimitiveJSClass(sym: Symbol)(implicit
        pos: Position): js.Tree = { genGlobalJSObject(sym) }
    ...
    /** Gen JS code representing a JS object (class or module) in global scope
      */
    private def genGlobalJSObject(sym: Symbol)(implicit
        pos: Position): js.Tree = {
    ...
    private def genPrimitiveJSArgs(sym: Symbol, args: List[Tree])(implicit
      ...
      val paramTpes = enteringPhase(currentRun.posterasurePhase) {
        for (param <- sym.tpe.params) yield param.tpe
      }
    ...
    private def tryGenAndRecordAnonFunctionClassGeneric(cd: ClassDef)(
      ...
      withNewLocalNameScope {
        ...
        val (applyMethod, methodInfoBuilder) = withScopedVars(
          ...
          try {
            genMethodWithInfoBuilder(applyDef)
    ...
    private def genAnonFunction(originalFunction: Function): js.Tree = {
      ...
      val (allFormalCaptures, body, allActualCaptures) =
        if (!isInImplClass) {
          ...
          val (thisActualCapture :: actualArgs, actualCaptures) = allArgs
            .splitAt(formalArgs.size + 1)
          val (
            thisFormalCapture :: formalCaptures,
            thisCaptureArg :: captureArgs) = makeCaptures(
            thisActualCapture :: actualCaptures)
      ...
      val (patchedFormalArgs, patchedBody) = patchFunBodyWithBoxes(
        target,
        formalArgs,
        body)
    ...
    private def patchFunBodyWithBoxes(
        ...
        body: js.Tree)(implicit pos: Position): (List[js.ParamDef], js.Tree) = {
      val methodType = enteringPhase(currentRun.posterasurePhase)(methodSym.tpe)

      val (patchedParams, paramsLocal) =
        (for { (param, paramSym) <- params zip methodType.params } yield {
          val paramTpe = enteringPhase(currentRun.posterasurePhase)(
            paramSym.tpe)
