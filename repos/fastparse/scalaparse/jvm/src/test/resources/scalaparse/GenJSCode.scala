  ...
  class JSCodePhase(prev: Phase) extends StdPhase(prev) with JSExportsPhase {
    ...
    def genRawJSClassData(cd: ClassDef): js.ClassDef = {
      ...
      val superClass =
        if (sym.isInterface)
          ...
          Some(encodeClassFullNameIdent(sym.superClass))
      val jsName =
        if (sym.isInterface || sym.isModuleClass)
          None
        else
          Some(jsNameOf(sym))
        ...
        withScopedVars(
          ...
          if (scalaPrimitives.isPrimitive(sym)) {
            ...
            None
                       ...
                       .isClassConstructor && isHijackedBoxedClass(sym.owner)) {
            None
          } else {
            withScopedVars(
      ...
      sources match {
        ...
        case List(lsrc_in, rsrc_in) =>
          def convertArg(tree: js.Tree, tpe: Type) = {
            ...
            if (resultType != jstpe.FloatType)
              fromLong
            else if (kind == FloatKind)
              fromLong
            else
              js.UnaryOp(js.UnaryOp.DoubleToFloat, fromLong)
          }
    ...
    private def tryGenAndRecordAnonFunctionClassGeneric(cd: ClassDef)(
      ...
      withNewLocalNameScope {
        ...
        val (applyMethod, methodInfoBuilder) =
          withScopedVars(
      ...
      val (patchedParams, paramsLocal) =
        (
          for {
            ...
            val paramTpe =
              enteringPhase(currentRun.posterasurePhase)(paramSym.tpe)
