...
object Combinators {
    ...
    def parseRec(cfg: ParseCtx, index: Int) = {
      p.parseRec(cfg, index) match {
        ...
        case f: Mutable.Failure if f.cut =>
          failMore(f, index, cfg.logDepth)
        case _ =>
          success(cfg.success, ev.none, index, Set.empty, false)
      }
    }
  ...
  object Sequence {
    ...
    /**
      ...
      */
    case class Chain[R](p: Parser[R], cut: Boolean)(
        val ev: Implicits.Sequencer[R, R, R])
    case class Flat[R](p0: Parser[R], ps: Vector[Chain[R]]) extends Parser[R] {
      def parseRec(cfg: ParseCtx, index: Int): Mutable[R] = {
        ...
        def rec(
          ...
          if (vIndex >= ps.length)
            success(cfg.success, r1, rIndex, traceParsers, rCut)
          else {
            val c = ps(vIndex)
        ...
        p0.parseRec(cfg, index) match {
          case f: Mutable.Failure =>
            failMore(f, index, cfg.logDepth, cut = f.cut)
          case Mutable.Success(value0, index0, traceParsers0, cut0) =>
    ...
    def flatten[R](s: Sequence[R, R, R]): Flat[R] = {
      def rec(s: Sequence[R, R, R]): Flat[R] = {
        ...
        s.p1 match {
          ...
          case p =>
            Flat(p, Vector(Chain[R](s.p2, s.cut)(ev2)))
        }
      }
      rec(s)
    }
  ...
  case class Cut[T](p: Parser[T]) extends Parser[T] {
    def parseRec(cfg: ParseCtx, index: Int) = {
      p.parseRec(cfg, index) match {
        case f: Mutable.Failure =>
          failMore(f, index, cfg.logDepth, cut = false)
        case s: Mutable.Success[T] =>
