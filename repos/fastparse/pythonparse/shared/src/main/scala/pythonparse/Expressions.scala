...
object Expressions {
  ...
  val NUMBER: P[Ast.expr.Num] = P(
    Lexical.floatnumber | Lexical.longinteger | Lexical.integer |
      Lexical.imagnumber).map(Ast.expr.Num)
  ...
  val atom: P[Ast.expr] = {
    ...
    P(
      empty_tuple | empty_list | empty_dict |
        ...
        "[" ~
        (list_comp | list) ~ "]" | "{" ~ dictorsetmaker ~ "}" |
        "`" ~
        testlist1.map(x =>
          Ast.expr.Repr(Ast.expr.Tuple(x, Ast.expr_context.Load))) ~ "`" |
        STRING.rep(1).map(_.mkString).map(Ast.expr.Str) |
        NAME.map(Ast.expr.Name(_, Ast.expr_context.Load)) | NUMBER)
  }
  ...
  val subscript: P[Ast.slice] = {
    ...
    val multi = P(test.? ~ ":" ~ test.? ~ sliceop.?).map {
      case (lower, upper, step) =>
        Ast.slice.Slice(
          lower,
          upper,
  ...
  val fpdef: P[Ast.expr] = P(
    NAME.map(Ast.expr.Name(_, Ast.expr_context.Param)) | "(" ~ fplist ~ ")")
  val fplist: P[Ast.expr] = P(fpdef.rep(sep = ",") ~ ",".?)
    .map(Ast.expr.Tuple(_, Ast.expr_context.Param))
}
