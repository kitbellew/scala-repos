...
object Ast {
  ...
  object stmt {
    ...
    case class Print(dest: Option[expr], values: Seq[expr], nl: bool)
        extends stmt
    ...
    case class Exec(body: expr, globals: Option[expr], locals: Option[expr])
        extends stmt
  ...
  object expr {
    ...
    case class DictComp(key: expr, value: expr, generators: Seq[comprehension])
        extends expr
    case class GeneratorExp(elt: expr, generators: Seq[comprehension])
        extends expr
    ...
    case class Attribute(value: expr, attr: identifier, ctx: expr_context)
        extends expr
    case class Subscript(value: expr, slice: slice, ctx: expr_context)
        extends expr
