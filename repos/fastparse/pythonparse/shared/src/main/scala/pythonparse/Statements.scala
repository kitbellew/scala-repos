...
class Statements(indent: Int) {
  ...
  val expr_stmt: P[Ast.stmt] = {
    ...
    P(
      aug
        .map { case (a, b, c) => Ast.stmt.AugAssign(tuplize(a), b, c) } | assign
        .map {
  ...
  val augassign: P[Ast.operator] = P(
    "+=".!.map(_ => Ast.operator.Add) | "-=".!.map(_ => Ast.operator.Sub) | "*=".!.map(
      _ =>
        Ast.operator.Mult) | "/=".!.map(_ => Ast.operator.Div) | "%=".!.map(_ =>
      Ast.operator.Mod) | "&=".!.map(_ => Ast.operator.BitAnd) | "|=".!.map(_ =>
      Ast.operator.BitOr) | "^=".!.map(_ => Ast.operator.BitXor) | "<<=".!.map(_ =>
