...
object RegressionTests extends TestSuite {
  ...
  val tests = TestSuite {
    'multiple_comments -
      TestUtils.check(Statements.file_input, Seq(Ast.stmt.Pass), """# a
        |# b
        |pass""".stripMargin)

    'multiple_newlines -
      TestUtils.check(Statements.file_input, Seq(Expr('a), Expr('b)), """a
        |
        |b""".stripMargin)
    ...
    'backslash_breaks -
      TestUtils
        ...
        |""".stripMargin)
    'multiline_string -
      TestUtils.check(Statements.file_input, Seq(Expr(Str("\n"))), """'''
        |'''
        |""".stripMargin)

    'try_finally_no_except -
      TestUtils.check(
        Statements.file_input,
    ...
    'indented_try_except_with_space -
      TestUtils.check(
        Statements.file_input,
        Seq(
          FunctionDef(
    ...
    'indented_block_with_spaces_and_offset_comments -
      TestUtils.check(
        Statements.file_input,
        Seq(
          FunctionDef(
          ...
          While(
            ...
            Seq(
              TryExcept(
                ...
                Seq(
                  excepthandler
                    .ExceptHandler(None, None, Seq(Return(Some('a)))),
    ...
    'weird_comments -
      TestUtils.check(
        Statements.file_input,
        Seq(While(Num(1), Seq(Expr('a)), Nil)),
        """while 1:
        |    #
        |    a
        |""".stripMargin)
    'ident_looking_string -
      TestUtils.check(
        Statements.file_input,
        Seq(
          If(
    ...
    'same_line_comment -
      TestUtils.check(
        Statements.file_input,
        Seq(If('b, Seq(If('c, Seq(Pass), Nil)), Nil)),
        """if b:  #
        |    if c:
        |        pass
        |""".stripMargin)
          ...
          While(
            ...
            Seq(
              If(
                'a,
                Seq(Expr('a)),
                Seq(If('b, Seq(Expr('b)), Seq(If('c, Seq(Expr('c)), Nil)))))),
            Nil)),
        """while 1:
        |    if a:
        |        a
