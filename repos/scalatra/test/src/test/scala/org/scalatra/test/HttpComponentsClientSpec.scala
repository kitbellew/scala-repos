...
class HttpComponentsClientSpec
    ...
    new HttpServlet {
      override def service(req: HttpServletRequest, resp: HttpServletResponse) {
        ...
        req.getParameterMap.foreach {
          case (name, values) =>
            resp
              .setHeader("Request-Param-%s".format(name), values.mkString(", "))
        }
  ...
  "client" should {
    ...
    "submit body for POST/PUT/PATCH requests" in {
      (
        doReqWithBody("POST", "post test") must equalTo("received: post test")
      ) and
        (
          doReqWithBody("PUT", "put test") must equalTo("received: put test")
        ) and
        (
