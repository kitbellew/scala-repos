...
class FileUploadSupportTestServlet
  ...
  post("""/multipart.*""".r) {
    ...
    params.get("file") foreach {
      response.setHeader("file-as-param", _)
    }
    params("utf8-string")
  }
...
class MaxSizeTestServlet extends ScalatraServlet with FileUploadSupport {
  ...
  error {
    case e: FileUploadBase.SizeLimitExceededException =>
      halt(413, "boom")
  }
...
class FileUploadSupportTest extends ScalatraFunSuite {
  ...
  test("sets multiple file params") {
    multipartResponse().getHeader("file-two-with-brackets") should
      equal("twothree")
  }

  test("looks for params with [] suffix, Ruby style") {
    multipartResponse().getHeader("file-two-without-brackets") should
      equal("twothree")
  }
  ...
  test("query parameters don't shadow post parameters") {
    multipartResponse("/multipart-param?string=bar").getHeader("string") should
      equal("bar;foo")
  }
