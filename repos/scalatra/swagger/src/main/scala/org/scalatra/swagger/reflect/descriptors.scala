...
class ManifestScalaType(val manifest: Manifest[_]) extends ScalaType {
  ...
  lazy val fullName: String =
    rawFullName +
      (if (typeArgs.nonEmpty) typeArgs.map(_.fullName).mkString("[", ", ", "]")
       else "")
  ...
  def copy(
    ...
    if (erasure == classOf[Int] || erasure == classOf[java.lang.Integer])
      ...
      ManifestScalaType.FloatType
    else if (erasure == classOf[Double] ||
             erasure == classOf[java.lang.Double]) ManifestScalaType.DoubleType
      ...
      ManifestScalaType.BooleanType
    else if (erasure == classOf[String] ||
             erasure == classOf[java.lang.String]) ManifestScalaType.StringType
