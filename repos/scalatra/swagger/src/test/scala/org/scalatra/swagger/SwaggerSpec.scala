      ...
      bd must beSome[JValue] and {
        ...
        val props = (j \ "models" \ "Pet" \ "properties")
          ...
          .map {
            case (x, y) ⇒
              x →
                y.asInstanceOf[Map[String, BigInt]]
                  .get("position")
...
class SwaggerTestServlet(protected val swagger: Swagger)
  ...
  val getPet =
    (apiOperation[Pet]("getPetById") summary "Find pet by ID" notes
      ...
      ("application/json", "application/xml") authorizations
      ("oauth2"))
  ...
  val findByStatus =
    (apiOperation[List[Pet]]("findPetsByStatus")
      .deprecate summary "Finds Pets by status" notes
  ...
  val findByTags =
    (apiOperation[List[Pet]]("findPetsByTags")
      .deprecate summary "Finds Pets by tags" notes
