...
class NotFoundSpec extends ScalatraSpec {
  ...
  def customNotFound =
    get("/custom/matches-nothing") { body must_== "custom not found" }

  def customNotFoundStatus =
    get("/custom/matches-nothing") { status must_== 200 }

  def servletNotFoundSends404 =
    get("/default/matches-nothing") { status must_== 404 }

  def filterNotFoundInvokesChain =
    get("/filtered/fall-through") { body must_== "fell through" }

  def customMethodNotAllowed =
    get("/custom/no-get") { body must_== "custom method not allowed" }

  def defaultMethodNotAllowedSends405 =
    get("/default/no-get") { status must_== 405 }
  ...
  def passFromNotAllowed =
    get("/pass-from-not-allowed/no-get") { body must_== "fell through" }

  def methodNotAllowedFilterPass =
    get("/filtered/get") { body must_== "servlet get" }
}
