...
class RoutePrecedenceTestBaseServlet extends ScalatraServlet {
  get("/override-route") { "base" }
}

class RoutePrecedenceTestChildServlet extends RoutePrecedenceTestBaseServlet {
  get("/override-route") { "child" }

  get("/hide-route") { "hidden by later route" }

  get("/hide-route") { "visible" }

  get("/pass") { response.getWriter.write("3") }
  ...
  get("/do-not-pass") { response.getWriter.write("2") }

  get("/do-not-pass") { response.getWriter.write("1") }

  notFound { response.getWriter.write("c") }
}
...
class RoutePrecedenceTest extends ScalatraFunSuite {
  ...
  test("Routes in child should override routes in base") {
    get("/override-route") { body should equal("child") }
  }
