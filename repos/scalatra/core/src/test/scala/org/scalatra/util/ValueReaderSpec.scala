...
class ValueReaderSpec extends Specification with DefaultImplicitConversions {
  ...
  "A StringMapValueReader" should {
    ...
    "read a long value" in {
      new StringMapValueReader(params)
        .read("long") must beRight(Some("1239485775745309200"))
    }
  ...
  "A MultiParamsValueReader" should {
    "read a string value" in {
      new MultiParamsValueReader(multiParams)
        .read("hello") must beRight(Some(Seq("world")))
    }
    "read an int value" in {
      new MultiParamsValueReader(multiParams)
        .read("int") must beRight(Some(Seq("1")))
    }
    "read a long value" in {
      new MultiParamsValueReader(multiParams)
        .read("long") must beRight(Some(Seq("1239485775745309200")))
    }
    "read a boolean value" in {
      new MultiParamsValueReader(multiParams)
        .read("bool") must beRight(Some(Seq("true")))
    }
    "read a stringlist value" in {
      new MultiParamsValueReader(multiParams)
        .read("stringlist") must beRight(Some(Seq("hello", "world")))
    }
    "read an intlist value" in {
      new MultiParamsValueReader(multiParams)
        .read("intlist") must beRight(Some(Seq("1", "2")))
    }
    ...
    "read a booleanlist value" in {
      new MultiParamsValueReader(multiParams)
        .read("booleanlist") must beRight(Some(Seq("false", "true")))
    }
  }
}
