...
class UrlSupportTest extends ScalatraFunSuite {
  ...
  test("a should expand an option") {
    url("page-relative", Map("id" -> "the-id")) should
      equal("page-relative?id=the-id")
  }
  ...
  test("a '/' should come out as /context") {
    get("/context/strip-context?url=/") {
      body should equal("/context")
    }
  }
  ...
  test("params should url encode both keys and values in UTF-8") {
    url("de-to-ru", Map("fünf" -> "пять")) should
      equal("de-to-ru?f%C3%BCnf=%D0%BF%D1%8F%D1%82%D1%8C")
  }
