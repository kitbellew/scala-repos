...
class RailsLikeUrlGeneratorTest extends FunSuite with Matchers {
  ...
  test("dynamic segment") {
    url(":foo.example.com", "foo" -> "vanilla") should
      equal("vanilla.example.com")
  }

  test("dynamic segment with leading underscore") {
    url(":_foo.example.com", "_foo" -> "vanilla") should
      equal("vanilla.example.com")
  }
  ...
  test("static string and dynamic segment inside optional segment") {
    url("foo(/bar.:extension)", "extension" -> "json") should
      equal("foo/bar.json")
  ...
  test("glob segment at the beginning") {
    url("*files/foo.txt", "files" -> "/home/thib") should
      equal("/home/thib/foo.txt")
  }

  test("glob segment in the middle") {
    url("src/*files/foo.txt", "files" -> "a/b/c") should
      equal("src/a/b/c/foo.txt")
  }
