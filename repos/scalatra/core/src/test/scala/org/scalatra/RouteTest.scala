...
class RouteTestServlet extends ScalatraServlet {
  get("/foo") { "matched simple string route" }
  ...
  get("/dot-in-named-param/:foo/:bar") { params("foo") }

  get("/dot-outside-named-param/:file.:ext") {
    List("file", "ext") foreach { x => response.setHeader(x, params(x)) }
  }

  get("/literal.dot.in.path") { "matched literal dot" }

  get("/test$") { "test$" }

  get("/te+st") { "te+st" }

  get("/test(bar)") { "test(bar)" }
  ...
  get("""^/foo.../bar$""".r) { "regex match" }

  get("""/reg(ular)?-ex(pression)?""".r) { "regex: false" }

  get(
    """/reg(ular)?-ex(pression)?""".r,
    params.getOrElse("condition", "false") == "true") { "regex: true" }

  post() { "I match any post!" }

  get("/nothing", false) { "shouldn't return" }
  ...
  get("/encoded-uri-3/%C3%B6") { "ö" }

  get("/semicolon/?") { "semicolon" }

  get("/semicolon/document") { "document" }
}
...
class RouteTest extends ScalatraFunSuite {
  ...
  test("supports optional named params") {
    get("/optional/hello/world") { body should equal("foo=hello;bar=world") }

    get("/optional/hello") { body should equal("foo=hello") }

    get("/optional") { body should equal("") }

    get("/optional-ext.json") { body should equal("ext=json") }

    get("/optional-ext") { body should equal("") }
  }

  test("supports single splat params") {
    get("/single-splat/foo") { body should equal("foo") }

    get("/single-splat/foo/bar/baz") { body should equal("foo/bar/baz") }
  }
  ...
  test("literally matches . in paths") {
    get("/literal.dot.in.path") { body should equal("matched literal dot") }
  }

  test("literally matches $ in paths") {
    get("/test$") { body should equal("test$") }
  }

  test("literally matches + in paths") {
    get("/te+st") { body should equal("te+st") }
  }

  test("literally matches () in paths") {
    get("/test(bar)") { body should equal("test(bar)") }
  }

  test("literally matches ; in paths") {
    get("/foo;123") { status should equal(200) }
  }

  test("supports conditional path routes") {
    get("/conditional", "condition" -> "true") { body should equal("true") }

    get("/conditional") { body should equal("false") }
  }

  test("supports regular expressions") {
    get("/foooom/bar") { body should equal("regex match") }
  }

  test("makes regular expression captures available in params(\"captures\")") {
    get("/foorooomma/baf") { body should equal("orooomma:f") }
  }
  ...
  test("a route with no matchers matches all requests to that method") {
    post("/an-arbitrary-path") { body should equal("I match any post!") }
  }

  test("matchers should not execute if one before it fails") {
    get("/fail") { body should not include ("shouldn't return") }
  }

  test("trailing slash is optional in a subcontext-mapped servlet") {
    get("/subcontext") { body should equal("root") }

    get("/subcontext/") { body should equal("root") }
  }

  test("handles encoded characters in uri") {
    get("/encoded-uri/ac/dc") { status should equal(405) }
    ...
    get("/encoded-uri-3/%25C3%25B6") { status should equal(200) }
  }
