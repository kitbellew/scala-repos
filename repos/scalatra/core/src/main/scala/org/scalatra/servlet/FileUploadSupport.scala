...
trait FileUploadSupport extends ServletBase with HasMultipartConfig {
    ...
    val wrapped = new HttpServletRequestWrapper(req) {
      ...
      override def getParameterMap: JMap[String, Array[String]] = {
        (new JHashMap[String, Array[String]].asScala ++
          (formMap transform { (k, v) => v.toArray })).asJava
      }
    }
    wrapped
  }
  ...
  def fileParams(implicit
      request: HttpServletRequest): MultiMapHeadView[String, FileItem] = {
  ...
  def fileParams(key: String)(implicit
      request: HttpServletRequest): FileItem = { fileParams(request)(key) }
}
