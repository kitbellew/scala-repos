...
object ScentrySpec extends Specification with Mockito {
  ...
  "The scentry" should {
    ...
    val context =
      new ScalatraFilter {
        private[this] val sessionMap = scala
          .collection
          .mutable
        ...
        mockSession.getAttribute(anyString) answers { k =>
          sessionMap
            .getOrElse(k.asInstanceOf[String], null)
            .asInstanceOf[AnyRef]
        }
    ...
    "run all set user callbacks" in {
      ...
      there was
        atLeastOne(req).setAttribute("scentry.auth.default.user", User("6789"))
