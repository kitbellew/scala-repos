...
trait CommandSupport extends ParamsValueReaderProperties with CommandExecutors {
  ...
  private[this] val commandFactories: ConcurrentMap[Class[_], () => Command] =
    new ConcurrentHashMap[Class[_], () => Command].asScala
  ...
  def command[T <: CommandType](implicit
      ...
      mf: Manifest[T]): T = {
    def createCommand =
      commandFactories.get(mf.erasure).map(_())
        .getOrElse(mf.erasure.newInstance()).asInstanceOf[T]
  ...
  /**
    ...
    */
  def ifValid[T <: CommandType](implicit mf: Manifest[T]): RouteMatcher =
