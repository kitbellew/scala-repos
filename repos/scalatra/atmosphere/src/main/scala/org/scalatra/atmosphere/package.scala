...
package object atmosphere {
  ...
  abstract class ClientFilter(val uuid: String)
      extends Function[AtmosphereResource, Boolean]
  ...
  implicit def atmoResourceWithClient(res: AtmosphereResource) =
    new {
      def clientOption =
        res.session.get(AtmosphereClientKey)
  ...
  private[atmosphere] implicit def jucFuture2akkaFuture[T](
      javaFuture: java.util.concurrent.Future[T])(implicit
