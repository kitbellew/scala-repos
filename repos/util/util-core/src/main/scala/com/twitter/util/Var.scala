...
trait Var[+T] {
  ...
  lazy val changes: Event[T] = new Event[T] {
    def register(s: Witness[T]) = self.observe { newv => s.notify(newv) }
  }
...
object Var {
  ...
  private[util] class Observer[-T](observe: T => Unit) {
    ...
    /**
      ...
      */
    def claim(newOwner: AnyRef): Unit =
      synchronized {
    ...
    /**
      ...
      */
    def publish(owner: AnyRef, value: T, version: Long): Unit =
      synchronized {
  ...
  /**
    ...
    */
  def collect[T: ClassTag, CC[X] <: Traversable[X]](vars: CC[Var[T]])(implicit
      newBuilder: CanBuildFrom[CC[T], T, CC[T]]): Var[CC[T]] = {
    val vs = vars.toArray
...
private object UpdatableVar {
  ...
  implicit def order[T] =
    new Ordering[Party[T]] {
