...
object Duration extends TimeLikeOps[Duration] {
  ...
  override def fromMilliseconds(millis: Long): Duration =
    super.fromMilliseconds(millis)
  override def fromMicroseconds(micros: Long): Duration =
    super.fromMicroseconds(micros)
  ...
  private val FullDurationRegex =
    ("(" + SingleDurationRegex.pattern.pattern + """)+\s*""").r
...
sealed class Duration private[util] (protected val nanos: Long) extends {
    ...
    def multiplyNanos(a: Long, b: Long): Duration =
      if (a < 0L)
        if (b < 0L && a < Long.MaxValue / b)
          overflowedDuration(a, b)
        else if (b > 0L && Long.MinValue / b > a)
          overflowedDuration(a, b)
        else
          Duration.fromNanoseconds(a * b)
      else if (a > 0L && a > Long.MaxValue / b)
        overflowedDuration(a, b)
      else
        Duration.fromNanoseconds(a * b)

    if (nanos > x)
      multiplyNanos(x, nanos)
    else
  ...
  def *(x: Double): Duration = (nanos * x) match {
    case product if java.lang.Double.isNaN(product) =>
      Undefined
    ...
    case product =>
      ...
      if (productLong == Long.MaxValue)
        Top
      else if (productLong == Long.MinValue)
        Bottom
      else
        fromNanoseconds(productLong)
  }
  ...
  def %(x: Duration): Duration =
    x match {
      case Undefined | Nanoseconds(0) =>
        Undefined
      case Nanoseconds(ns) =>
        fromNanoseconds(nanos % ns)
      case Top | Bottom =>
        this
    }
  ...
  def unary_- : Duration =
    if (inNanoseconds == Long.MinValue)
      Top
    else
      ops.fromNanoseconds(-inNanoseconds)
