...
object AsyncMeter {
  ...
  def perSecond(permits: Int, maxWaiters: Int)(implicit
      timer: Timer): AsyncMeter = new AsyncMeter(permits, 1.second, maxWaiters)
  ...
  def extraWideAwait(permits: Int, meter: AsyncMeter): Future[Unit] = {
    if (permits > meter.burstSize) {
      ...
      val seqWithoutLast: Seq[Future[Unit]] = (0 until num)
        .map(_ => meter.await(meter.burstSize))
      val seq =
...
class AsyncMeter private[concurrent] (
  ...
  private[this] val bucket: TokenBucket = TokenBucket
    .newBoundedBucket(burstSize)
