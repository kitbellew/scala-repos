...
class VarTest extends FunSuite with GeneratorDrivenPropertyChecks {
  ...
  test("version ordering") {
    ...
    val o1 = v1.changes.register(
      Witness({ i =>
        result = result + i
      })
    ) // result = 2
    val o2 = v1.changes.register(
      Witness({ i =>
        result = result * i * i
      })
    ) // result = 2 * 2 * 2 = 8
    val o3 = v1.changes.register(
      Witness({ i =>
  ...
  test("Var observers coming and going") {
    ...
    val c1 = f.changes.register(
      Witness({ i =>
        assert(i == 22)
      }))
    val c2 = f.changes.register(
      Witness({ i =>
        assert(i == 22)
      }))
