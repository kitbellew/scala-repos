...
class TxTest extends WordSpec {
  "Tx.twoParty" should {
    ...
    "complain on ack ack" in {
      ...
      assert(intercept[Exception] { rtx.ack() } == Tx.AlreadyAckd)
    }
    ...
    "complain on nack ack" in {
      ...
      assert(intercept[Exception] { rtx.ack() } == Tx.AlreadyNackd)
    }
    ...
    "complain on nack nack" in {
      ...
      assert(intercept[Exception] { rtx.nack() } == Tx.AlreadyNackd)
    }
    ...
    "complain when already done" in {
      ...
      assert(intercept[Exception] { stx.ack() } == Tx.AlreadyDone)
    }
  }
}
