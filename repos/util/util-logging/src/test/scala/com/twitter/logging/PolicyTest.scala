...
class PolicyTest extends FunSuite {
  ...
  test("Policy.parse: never") { assert(parse("never") == Never) }

  test("Policy.parse: hourly") { assert(parse("hourly") == Hourly) }

  test("Policy.parse: daily") { assert(parse("daily") == Daily) }

  test("Policy.parse: sighup") { assert(parse("sighup") == SigHup) }

  test("Policy.parse: weekly") { assert(parse("weekly(3)") == Weekly(3)) }
