...
protected[zk] object LiftableFuture {
  implicit def liftableFuture[T](f: Future[T]): LiftableFuture[T] =
    new LiftableFuture(f)
}
...
protected[zk] class LiftableFuture[T](f: Future[T]) {
  ...
  def liftKeeperException =
    liftSuccess handle { case e: KeeperException => Throw(e) }

  /** Lift failures when a watch would have been successfully installed */
  def liftNoNode =
    liftSuccess handle { case e: KeeperException.NoNodeException => Throw(e) }
}
