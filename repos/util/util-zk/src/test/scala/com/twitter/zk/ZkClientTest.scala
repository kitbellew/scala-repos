...
class ZkClientTest extends WordSpec with MockitoSugar {
  ...
  class ZkClientHelper {
    ...
    def create(
        ...
        mode: CreateMode = zkClient.mode)(wait: => Future[String]) {
      when(zk.create(
        meq(path),
        meq(data),
        meq(acls.asJava),
    ...
    def delete(path: String, version: Int)(wait: => Future[Unit]) {
      when(zk.delete(
        ...
        meq(null))) thenAnswer answer[AsyncCallback.VoidCallback](2) {
        cbValue =>
          wait onSuccess { _ =>
    ...
    def exists(path: String)(stat: => Future[Stat]) {
      when(zk.exists(
        ...
        cbValue =>
          stat onSuccess { cbValue.processResult(0, path, null, _) } onFailure {
            case ke: KeeperException =>
              cbValue.processResult(ke.code.intValue, path, null, null)
          }
      }
    ...
    def watch(path: String)(stat: => Future[Stat])(
      ...
      when(
        zk.exists(
            ...
            cb.capture(),
            meq(null))) thenAnswer
        answer[Watcher, AsyncCallback.StatCallback](1, 2) {
    ...
    def watchChildren(path: String)(children: => Future[ZNode.Children])(
        update: => Future[WatchedEvent]) {
      ...
      when(
        zk.getChildren(
            ...
            cb.capture(),
            meq(null))) thenAnswer
        answer[Watcher, AsyncCallback.Children2Callback](1, 2) {
    ...
    def getData(path: String)(result: => Future[ZNode.Data]) {
      when(zk.getData(
        ...
        meq(null))) thenAnswer answer[AsyncCallback.DataCallback](2) {
        cbValue =>
          result onSuccess { z =>
    ...
    def watchData(path: String)(result: => Future[ZNode.Data])(
        update: => Future[WatchedEvent]) {
      val w = ArgumentCaptor.forClass(classOf[Watcher])
      val cb = ArgumentCaptor.forClass(classOf[AsyncCallback.DataCallback])
      when(
        zk.getData(meq(path), w.capture(), cb.capture(), meq(null))) thenAnswer
        answer[Watcher, AsyncCallback.DataCallback](1, 2) {
    ...
    def setData(path: String, data: Array[Byte], version: Int)(
      ...
      when(zk.setData(
        ...
        meq(version),
        any[AsyncCallback.StatCallback],
        meq(null))) thenAnswer answer[AsyncCallback.StatCallback](3) {
        cbValue =>
  ...
  "ZkClient" should {
    ...
    "transform" should {
      ...
      "chained" in {
        val transformed = zkClient.withAcl(acl).withMode(mode)
  ...
  "ZNode" should {
    ...
    "create" should {
      ...
      "sequential" in {
        ...
        create(path, data, mode = CreateMode.EPHEMERAL_SEQUENTIAL)(Future(
          path + "0000"))
      ...
      "a child" should {
        ...
        "sequential, with an empty name" in {
          ...
          assert(
            Await.result(zkClient(path).create(
              data,
              child = Some(""),
      ...
      "monitor" should {
        ...
        "handles session events properly" should {
          ...
          "Disconnected" in {
            ...
            watch(znode.path)(Future(new Stat))(Future(
              StateEvent.Disconnected()))
            val offer = znode.exists.monitor()
            assert(offer.syncWait().get() == result)

            watch(znode.path)(Future(new Stat))(Future(
              NodeEvent.Deleted(znode.path)))
            assert(offer.syncWait().get() == result)
    ...
    "getChildren" should {
      ...
      "watch" in {
        watchChildren(znode.path)(Future(result))(Future(
          NodeEvent.ChildrenChanged(znode.path)))
      ...
      "monitor" in {
        ...
        results foreach { r =>
          watchChildren(znode.path)(Future(r))(Future(
            NodeEvent.ChildrenChanged(znode.path)))
        }
    ...
    "monitorTree" should {
      ...
      val treeChildren = treeRoot +: ('a' to 'e').map { c =>
        ZNode
          .Children(treeRoot(c.toString), new Stat, 'a' to c map { _.toString })
      }.flatMap { z =>
      ...
      val updatesByPath = updateTree.map { z =>
        val prior: Set[ZNode] = expectedByPath.get(z.path).map { _.added }
          .getOrElse(Set.empty)
        z.path ->
          ZNode
            .TreeUpdate(z, z.children.toSet -- prior, prior -- z.children.toSet)
      }.toMap
      ...
      def okUpdates(event: String => WatchedEvent) {
        ...
        intercept[TimeoutException] { Await.result(offer.sync(), 1.second) }
      }
      ...
      "stop on session expiration" in {
        treeChildren foreach { z =>
          ...
          watchChildren(z.path)(Future.exception(
    ...
    connectString =>
      "A live server @ %s".format(connectString) should {
        ...
        "have 'zookeeper' in '/'" in {
          Await.result(znode.getChildren(), 2.seconds).children.map { _.name }
            .contains("zookeeper")
        }
      }
  }
