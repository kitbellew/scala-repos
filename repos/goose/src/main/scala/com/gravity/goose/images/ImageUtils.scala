  ...
  def fetchEntity(
    ...
    URLHelper.tryToHttpGet(imageSrc) match {
      case Some(httpget) =>
        ...
        val response =
          try config.getHtmlFetcher.getHttpClient.execute(httpget, localContext)
          catch {
