...
trait ContentExtractor {
  ...
  private def getMetaContent(doc: Document, metaName: String): String = {
    ...
    if (meta.size > 0) { content = meta.first.attr("content") }
  ...
  def getCanonicalLink(article: Article): String = {
    ...
    } else { article.finalUrl }
  }

  def getDomain(url: String): String = { new URL(url).getHost }
  ...
  def calculateBestNodeBasedOnClustering(article: Article): Option[Element] = {
    ...
    trace(
      logPrefix + "About to inspect num of nodes with text: " + numberOfNodes)
    ...
    for (node <- nodesWithText) {
      ...
      if (numberOfNodes > 15) {
        if ((numberOfNodes - i) <= bottomNodesForNegativeScore) {
          val booster: Float = bottomNodesForNegativeScore.asInstanceOf[Float] -
      ...
      trace(
        logPrefix + "Location Boost Score: " + boostScore + " on interation: " +
          i + "' id='" + node.parent.id + "' class='" +
      ...
      if (!parentNodes.contains(node.parent)) { parentNodes.add(node.parent) }
  ...
  private def isHighLinkDensity(e: Element): Boolean = {
    ...
    for (link <- links) { sb.append(link.text) }
    ...
    trace(
      logPrefix + "Calulated link density score as: " + score + " for node: " +
        getShortText(e.text, 50))
  ...
  private def updateScore(node: Element, addToScore: Int) {
    ...
    } catch { case e: NumberFormatException => { currentScore = 0 } }
  ...
  def extractVideos(node: Element): List[Element] = {
    ...
    try {
      ...
      case e: NullPointerException => { warn(e.toString, e) }
      case e: Exception            => { warn(e.toString, e) }
    }
  ...
  def isTableTagAndNoParagraphsExist(e: Element): Boolean = {
    ...
    for (p <- subParagraphs) { if (p.text.length < 25) { p.remove() } }
  ...
  def postExtractionCleanup(targetNode: Element): Element = {
    ...
    for { e <- node.children if (e.tagName != "p") } {
      ...
      if (isHighLinkDensity(e) || isTableTagAndNoParagraphsExist(e) ||
          !isNodeScoreThreshholdMet(node, e)) {
        try { e.remove() }
        catch {
  ...
  def isNodeScoreThreshholdMet(node: Element, e: Element): Boolean = {
    ...
    trace(
      logPrefix + "topNodeScore: " + topNodeScore + " currentNodeScore: " +
  ...
  def getSiblingContent(
    ...
    if (currentSibling.tagName == "p" && currentSibling.text.length() > 0) {
      ...
      if (potentialParagraphs.first == null) { None }
      else {
  ...
  private def getBaselineScoreForSiblings(topNode: Element): Int = {
    ...
    if (numberOfParagraphs > 0) {
      ...
      if (logger.isDebugEnabled) {
        logger.debug(
          "The base score for siblings to beat is: " + base + " NumOfParas: " +
