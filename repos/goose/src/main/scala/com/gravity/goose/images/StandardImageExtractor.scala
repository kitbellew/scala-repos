        ...
        scoredImages.foreach {
          case (key, value) =>
            if (highScoreImage == null)
              highScoreImage = key
            else if (value > scoredImages.get(highScoreImage).get)
              highScoreImage = key
        }
      ...
      try {
          ...
          try {
            ...
            if (depthLevel > 1)
              if (width < 300) {
                if (logger.isDebugEnabled)
                  logger.debug(
                    "going depthlevel: " + depthLevel + " and img was only: " + width + " wide: " + localSrcPath)
                continueVar = false
              }
          } catch {
            case e: IOException =>
        ...
        if (continueVar)
          if (width < 50) {
            if (logger.isDebugEnabled)
              logger.debug(image
                .attr("src") + " is too small width: " + width + " removing..")
            image.remove()
            continueVar = false
          }
        if (continueVar) {
          ...
          trace(
            logPrefix + imageSource + " Area is: " + area + " sequence score: " + sequenceScore + " totalScore: " + totalScore)
          cnt += 1;
