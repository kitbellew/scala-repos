...
class Configuration {
  ...
  var publishDateExtractor: PublishDateExtractor = new PublishDateExtractor {
    ...
    def extract(rootElement: Element): java.util.Date = {
      ...
      def bestPubDate =
        pubSelectors.flatMap(extractCandidate(rootElement, _))
          .reduceOption(minDate)
      def bestModDate =
        modSelectors.flatMap(extractCandidate(rootElement, _))
          .reduceOption(minDate)
  ...
  def getPublishDateExtractor: PublishDateExtractor = { publishDateExtractor }
