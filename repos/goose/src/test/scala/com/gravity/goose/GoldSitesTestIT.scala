...
class GoldSitesTestIT {
  ...
  def testArticleElementedArticle() { // to verify issue #56 is resolved
    ...
    val content =
      "UN PIANO per l'equità e la crescita destinato in primo luogo al Sud. L'ha varato ieri il Consiglio dei ministri."
    val title =
  ...
  def techCrunch() {
    ...
    val image =
      "http://tctechcrunch2011.files.wordpress.com/2011/08/screen-shot-2011-08-13-at-6-43-20-pm1.png?w=640"
  ...
  def cnn2() {
    val url =
      "http://www.cnn.com/2011/POLITICS/10/06/tea.party.left/index.html?hpt=hp_t1"
    implicit val config = TestUtils.DEFAULT_CONFIG
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def businessWeek() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.businessweek.com/magazine/content/10_34/b4192066630779.htm"
  ...
  def businessWeek2() {
    ...
    val article: Article = TestUtils.getArticle(url)
    val content =
      "There's discord on Wall Street: Strategists at major American investment banks see a"
  ...
  def businessWeek3() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.businessinsider.com/ben-and-jerrys-schweddy-balls-one-million-moms-american-family-association-boycott-2011-9"
    val article: Article = TestUtils.getArticle(url)
    //    if (article == null) println("NULL ARTICLE!") else println("TEXT: \n" + article.cleanedArticleText)
    val content =
      "Not everyone's a fan of Ben & Jerry's new \"Schweddy Balls\" -- the Saturday Night Live-inspired flavor it rolled out a few weeks ago"
    val image =
      "http://static7.businessinsider.com/image/4e68c8c36bb3f7d80a000016/conservative-moms-are-now-calling-for-a-boycott-of-ben-and-jerrys-schweddy-balls-flavor.jpg"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def desertNews() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url =
      "http://www.deseretnews.com/article/705388385/High-school-basketball-Top-Utah-prospects-representing-well.html"
    val article = TestUtils.getArticle(url)
    val content =
      "Utah isn't known nationally for producing top basketball talent"
  ...
  def foxNews() {
    ...
    val image =
      "http://a57.foxnews.com/static/managed/img/Politics/396/223/startsign.jpg"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def foxNews2() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.foxnews.com/politics/2011/10/06/obama-defends-528-million-federal-loan-to-bankrupt-solyndra/"
    val article = TestUtils.getArticle(url)
    val content =
      "The director of the controversial loan program that cleared the way for a $535"
    val image =
      "http://a57.foxnews.com/static/managed/img/Politics/396/223/silver_jonathan.jpg"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def msnbc() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://bottomline.msnbc.msn.com/_news/2011/10/06/8190264-even-without-jobs-apple-still-shines-analysts-say"
    val article = TestUtils.getArticle(url)
    val content =
      "The death of technology titan Steve Jobs, co-founder and former CEO of Apple"
    val image =
      "http://msnbcmedia.msn.com/j/MSNBC/Components/Photo/_new/tz-biz-11106-applefuture-108p.nv_auth_landscape.jpg"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def laTimes() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.latimes.com/business/la-fi-jobs-legacy-hiltzik-20111006,0,5186643.column"
    val article = TestUtils.getArticle(url)
    val content =
      "Everyone knows Steve Jobs pulled off one of the outstanding corporate turnarounds in U.S. history"
  ...
  def aolNews() {
    ...
    val article = TestUtils.getArticle(url)
    val content =
      "WASHINGTON (Aug. 13) -- Declaring \"the maritime soul of the Marine Corps\" is"
    val image =
      "http://o.aolcdn.com/photo-hub/news_gallery/6/8/680919/1281734929876.JPEG"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def wallStreetJournal() {
    ...
    val image =
      "http://s.wsj.net/public/resources/images/OB-JO759_0814st_A_20100814143158.jpg"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def usaToday() {
    ...
    val image =
      "http://i.usatoday.net/communitymanager/_photos/the-huddle/2010/08/18/favrespeaksx-inset-community.jpg"
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart = content,
      expectedImage = image)
  ...
  def usaToday3() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.usatoday.com/money/perfi/funds/story/2011-10-05/3q-mutual-fund-report/50674776/1"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def washingtonpost() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.washingtonpost.com/wp-dyn/content/article/2010/12/08/AR2010120803185.html"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def time404() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://newsfeed.time.com/2011/08/24/washington-monument-closes-to-repair-earthquake-induced-FOO-BAR/"
    val article = TestUtils.getArticle(url)
  ...
  def ap() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://hosted2.ap.org/APDEFAULT/bbd825583c8542898e6fa7d440b9febc/Article_2011-10-06-Kids-Concussions/id-6cb44517aaec4303936fa07d5490dce6"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def businessInsider() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.businessinsider.com/closing-bell-september-20-2011-9"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def financialTimes() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.ft.com/intl/cms/s/2/4e268022-e472-11e0-92a3-00144feabdc0.htm"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def huffpoBusiness() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.huffingtonpost.com/david-macaray/labor-union-membership_b_973038.html"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def huffpoBusiness2() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://www.huffingtonpost.com/2011/09/21/us-sees-challenges-in-s_n_974724.html"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def nyTimes1() {
    ...
    val url: String =
      "http://www.nytimes.com/2011/09/20/arts/design/preserving-the-american-folk-art-museums-place-in-new-york.html?_r=1&ref=arts"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def yahooFinance() {
    val url =
      "http://finance.yahoo.com/news/Mulling-Meg-Whitman-HP-apf-4116866737.html?x=0"
    implicit val config = TestUtils.DEFAULT_CONFIG
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def time3() {
    ...
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart =
        "The hemline could no longer be held. With wartime controls on",
      expectedImage = null)
    TestUtils.printReport()
  }
  ...
  def businessinsider() {
    val url =
      "http://www.businessinsider.com/meanwhile-developments-in-greece-2011-9"
    implicit val config = TestUtils.DEFAULT_CONFIG
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def businessinsider2() {
    val url =
      "http://www.businessinsider.com/goldman-on-the-fed-announcement-2011-9"
    implicit val config = TestUtils.DEFAULT_CONFIG
    val article = TestUtils.getArticle(url)
  ...
  def buzznetImages() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://newageamazon.buzznet.com/user/journal/17025056/doubt-gives-hope-new-album/"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def timeImages() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://swampland.time.com/2012/01/09/hecklers-and-hostile-crowds-stymie-santorum-in-new-hampshire/"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
  ...
  def cnnMoneyImages() {
    implicit val config = TestUtils.DEFAULT_CONFIG
    val url: String =
      "http://money.cnn.com/2012/01/09/pf/suze_orman_prepaid_card/index.htm?iid=HP_LN"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
      expectedStart =
  ...
  def yahooVoices() {
    ...
    val url: String =
      "http://voices.yahoo.com/article/9330101/lovess-demise-10882501.html"
    val article = TestUtils.getArticle(url)
    TestUtils.runArticleAssertions(
      article = article,
      expectedTitle = "Love's Demise",
      expectedStart =
        "Do we not love like lovers in demise? We both know our love has faded away;")
  }
}
