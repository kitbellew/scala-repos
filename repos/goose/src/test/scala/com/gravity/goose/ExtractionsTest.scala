...
class ExtractionsTest {
  ...
  def testHuffingtonPost() {
    ...
    val content =
      "A top regional Federal Reserve official sharply criticized Friday"
    val keywords =
  ...
  def wiredPubDate() {
    ...
    config.setPublishDateExtractor(
  ...
  def guardian1() {
    ...
    val url: String =
      "http://www.guardian.co.uk/film/2011/nov/18/kristen-wiig-bridesmaids"
    val article = TestUtils.getArticle(url, html)
    TestUtils.runArticleAssertions(article = article, expectedStart = expected)
