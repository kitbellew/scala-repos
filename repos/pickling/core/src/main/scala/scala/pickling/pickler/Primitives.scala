...
trait PrimitivePicklers {
  ...
  implicit val shortPickler: Pickler[Short] with Unpickler[Short] =
  ...
  implicit val booleanPickler: Pickler[Boolean] with Unpickler[Boolean] =
    PrimitivePickler[Boolean]
  implicit val floatPickler: Pickler[Float] with Unpickler[Float] =
    PrimitivePickler[Float]
  implicit val doublePickler: Pickler[Double] with Unpickler[Double] =
    PrimitivePickler[Double]
  implicit val nullPickler: Pickler[Null] with Unpickler[Null] =
    PrimitivePickler[Null]
  implicit val stringPickler: Pickler[String] with Unpickler[String] =
...
object PrimitivePickler {
  def apply[A: FastTypeTag]: Pickler[A] with Unpickler[A] =
    new PrimitivePickler[A](FastTypeTag.valueTypeName(
      implicitly[FastTypeTag[A]]))
}
