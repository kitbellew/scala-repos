...
package object internal {
  ...
  private[pickling] def typeFromString(mirror: Mirror, stpe: String): Type = {
    ...
    if (typeFromStringCache.contains(stpe)) typeFromStringCache(stpe)
      ...
      val result = AppliedType.parseFull(stpe) match {
  ...
  def lookupPicklee(picklee: Any): Int =
    currentRuntime.refRegistry.pickle.registerPicklee(picklee)
  ...
  def lookupUnpicklee(index: Int): Any =
    currentRuntime.refRegistry.unpickle.lookupUnpicklee(index)
  def preregisterUnpicklee() =
    currentRuntime.refRegistry.unpickle.preregisterUnpicklee()
  def registerUnpicklee(unpicklee: Any, index: Int) =
    currentRuntime.refRegistry.unpickle.regsiterUnpicklee(index, unpicklee)
