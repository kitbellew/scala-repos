...
object Compat {
  ...
  def PickleMacros_pickleTo[T: c.WeakTypeTag, S](c: Context)(output: c.Expr[S])(
      format: c.Expr[PickleFormat]): c.Expr[Unit] = {
    val c0: c.type = c
    val bundle = new {
      val c: c0.type = c0
    } with PickleMacros
  ...
  def FastTypeTagMacros_apply(c: Context)(
      key: c.Expr[String]): c.Expr[FastTypeTag[t]] forSome {
    type t
  } = {
    val c0: c.type = c
    val bundle = new {
