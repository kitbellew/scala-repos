...
trait FastTypeTag[T] extends Equals {
  ...
  def reflectType(otherMirror: ru.Mirror): ru.Type =
    typeFromString(otherMirror, key)
...
object FastTypeTag {
  ...
  def mkRawArrayTypeAndKey(
    ...
    val (elemTpe, elemKey) =
      if (elemClass.isArray) { mkRawArrayTypeAndKey(elemClass, mirror) }
      else {
        val elemClassSymbol =
        ...
        raw.getOrElse(
          clazz, {
            ...
            if (clazz.isArray) mkRawArray(clazz, mirror)
              ...
              val clazzName =
                if (clazzName0.contains("anonfun$") || clazzName0
                      .contains("$colon$colon") || clazzName0
                      .endsWith("$") || clazzName0.endsWith("$sp")) clazzName0
