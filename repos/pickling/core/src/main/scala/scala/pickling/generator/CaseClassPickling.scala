...
class CaseClassPickling(
  ...
  def allVars(cls: IrClass): Seq[IrMethod] = {
    (cls.methods.filter(_.isParamAccessor) ++
      IrSymbol.allDeclaredMethodIncludingSubclasses(cls).filter(x =>
        x.isVar || x.isVal)).groupBy(_.methodName).map(
      _._2.head).toList.filterNot(_.isMarkedTransient)
  }
              ...
              def reflectionErrorMessage(ast: IrAst): List[String] =
                ast match {
                  case x: SetField if x.requiresReflection =>
                    List(s"field ${x.name} has no public setter/accces")
                  case x: GetField if x.requiresReflection =>
                    List(s"field ${x.name} has no public getter/access")
                  case x: CallConstructor if x.requiresReflection =>
                    List(s"constructor is not public")
                  case x: CallModuleFactory if x.requiresReflection =>
                    List(s"factory method (apply) is not public")
                  case x: PickleEntry =>
                    x.ops.toList.flatMap(reflectionErrorMessage)
                  case x: PickleBehavior =>
  ...
  def checkFactoryImpl(
    ...
    (for {
      ...
      factoryMethod <- tpe.methods.filter(_.methodName == "apply").sortBy(
        _.parameterNames.flatten.size).headOption
    } yield {
      val vars = allVars(tpe)
