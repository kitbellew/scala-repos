...
class Tools[C <: Context](val c: C) {
  ...
  def allStaticallyKnownConcreteSubclasses(
    ...
    def sourcepathAndClasspathScan(): List[Symbol] = {
      ...
      lazy val classpathCache = Tools.subclassCache(
        mirror, {
          ...
          if (visited(currTpe)) {
            if (tpe <:< currTpe)
              true // TODO: make sure this sanely works for polymorphic types
                       ...
                       .isModuleClass) loop(rest, visited)
...
abstract class Macro extends RichTypes {
  ...
  def shouldBotherAboutCleaning(tpe: Type) =
    shareAnalyzer.shouldBotherAboutCleaning(tpe)
  ...
  def compileTimeDispatcheesNotSelf(tpe: Type): List[Type] =
    tools.compileTimeDispatchees(tpe, rootMirror, true)

  def compileTimeDispatchees(tpe: Type): List[Type] =
    tools.compileTimeDispatchees(tpe, rootMirror, false)
  ...
  def syntheticBaseQualifiedName(tpe: Type): TypeName =
    newTypeName(syntheticPackageName + "." + syntheticBaseName(tpe).toString)

  def syntheticPicklerName(tpe: Type): TypeName =
    syntheticBaseName(tpe) + syntheticPicklerSuffix()
  def syntheticPicklerQualifiedName(tpe: Type): TypeName =
    syntheticBaseQualifiedName(tpe) + syntheticPicklerSuffix()
  def syntheticPicklerSuffix(): String = "Pickler"

  def syntheticUnpicklerName(tpe: Type): TypeName =
    syntheticBaseName(tpe) + syntheticUnpicklerSuffix()
  def syntheticUnpicklerQualifiedName(tpe: Type): TypeName =
    syntheticBaseQualifiedName(tpe) + syntheticUnpicklerSuffix()
  def syntheticUnpicklerSuffix(): String = "Unpickler"

  def syntheticPicklerUnpicklerName(tpe: Type): TypeName =
    syntheticBaseName(tpe) + syntheticPicklerUnpicklerSuffix()
  def syntheticPicklerUnpicklerQualifiedName(tpe: Type): TypeName =
    syntheticBaseQualifiedName(tpe) + syntheticPicklerUnpicklerSuffix()
  ...
  def reflectivelyWithoutGetter(target: String, fir: FieldIR)(
      body: Tree => Tree): List[Tree] = {
  ...
  /**
    ...
    */
  def reflectively(target: TermName, fir: FieldIR)(
      body: Tree => Tree): List[Tree] = {
    val prologue = {
    ...
    val wrappedBody =
      q"""
        val $ownerSymbol = implicitly[scala.pickling.FastTypeTag[${owner.asClass
        .toType.erasure}]].tpe
