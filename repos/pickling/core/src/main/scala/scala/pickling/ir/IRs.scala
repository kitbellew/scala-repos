...
class IRs[U <: Universe with Singleton](val uni: U) {
  ...
  case class FieldIR(
    ...
    def hasGetter =
      param.isDefined || getter.isDefined // either case class val or getter
  ...
  def nonParamFieldIRsOf(tpe: Type): Iterable[FieldIR] = {
    ...
    val goodAccessorsNotParamsVars = goodAccessorsNotParams.filter(acc =>
      acc.isSetter && acc.accessed != NoSymbol) // 2.10 compat: !acc.isAbstract

    goodAccessorsNotParamsVars.map { symSetter: MethodSymbol =>
      val sym = symSetter.getter.asMethod
      val rawSymTpe =
        sym.typeSignatureIn(rawTpe) match {
  ...
  def newClassIR(tpe: Type): ClassIR = {
      ...
      tpe.declaration(nme.CONSTRUCTOR) match {
        case overloaded: TermSymbol =>
          overloaded
            .alternatives
            .head
            .asMethod // NOTE: primary ctor is always the first in the list
    ...
    val (filteredAccessors, transientAccessors) = allAccessors
      .partition(notMarkedTransient)
    ...
    def fieldIRsUsingCtor(): List[FieldIR] = {
      // collect:
      // (a) all ctor params
      val ctorFieldIRs = primaryCtorParamsOpt
        .get
        .map { s =>
          ...
          val baseSymTpe = baseSym
            .typeSignature
            .asSeenFrom(rawTpe, rawTpe.typeSymbol.asClass)
    ...
    def fieldIRsUsingAllocateInstance(): List[FieldIR] = {
      ...
      val fieldIRs1 = baseClasses.flatMap { baseClass =>
        ...
        allGetters.map { getter =>
          val rawSymTpe =
            getter.typeSignatureIn(rawTpe) match {
              case NullaryMethodType(ntpe) =>
                ntpe;
              case ntpe =>
                ntpe
  ...
  private def fields(tp: Type): Q = {
    val ctor =
      tp.declaration(nme.CONSTRUCTOR) match {
        case overloaded: TermSymbol =>
          overloaded
            .alternatives
            .head
    ...
    val ctorParams =
      if (ctor != NoSymbol)
        ctor
          ...
          .flatten
          .flatMap { sym =>
            if (transientAccessors
                  .exists(acc => acc.name.toString == sym.name.toString))
              List()
            else
              List(sym.asTerm)
          }
    ...
    val (paramAccessors, otherAccessors) = allAccessors
      .partition(_.isParamAccessor)

    def mkFieldIR(
        sym: TermSymbol,
        param: Option[TermSymbol],
        accessor: Option[MethodSymbol]) = {
      val (quantified, rawTp) =
        tp match {
          case ExistentialType(quantified, tpe) =>
            (quantified, tpe);
          case tpe =>
            (Nil, tpe)
        }
      val rawSymTp =
        accessor.getOrElse(sym).typeSignatureIn(rawTp) match {
  ...
  private val compose = composition(f1, f2, f3)

  private val flatten: C => C =
    (c: C) =>
      if (c.parent != null)
