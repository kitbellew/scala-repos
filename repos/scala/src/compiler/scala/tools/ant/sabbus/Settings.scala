...
class Settings {
  ...
  def toArgs: List[String] =
    (if (!gBf.isEmpty) "-g:" + g :: Nil else Nil) :::
      ...
      (if (!classpathBf.isEmpty) "-classpath" :: classpath.toString :: Nil
       else Nil) :::
      (if (!sourcepathBf.isEmpty) "-sourcepath" :: sourcepath.toString :: Nil
       else Nil) :::
      (if (!sourcedirBf.isEmpty) "-Xsourcedir" :: sourcedir.toString :: Nil
       else Nil) :::
