...
\*============================================================================*/
  ...
  def createCompilerPath: Path =
    createNewPath(compilerPath _, p => compilerPath = p)
  ...
  def createSourcepath(): Path =
    createNewPath(sourcepath _, p => sourcepath = p)
  ...
  def createBootclasspath(): Path =
    createNewPath(bootclasspath _, p => bootclasspath = p)
...
\*============================================================================*/
  ...
  protected def getBootclasspath: List[File] =
    pathAsList(bootclasspath, "bootclasspath")
...
\*============================================================================*/
  ...
  protected def executeInternal(settings: Settings, sourceFiles: List[File]) {
    ...
    if (reporter.hasErrors) {
      val msg =
        "Compile failed with %d error%s; see the compiler error output for details.".format(
          reporter.ERROR.count,
          plural(reporter.ERROR.count))
      if (failonerror) buildError(msg) else log(msg)
    } else if (reporter.WARNING.count > 0)
      log(
        "Compile succeeded with %d warning%s; see the compiler output for details.".format(
          reporter.WARNING.count,
          plural(reporter.WARNING.count)))
  }
}
