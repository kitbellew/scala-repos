...
class FastTrack[MacrosAndAnalyzer <: Macros with Analyzer](
  ...
  private implicit def context2taggers(c0: MacroContext): Taggers {
    val c: c0.type
  } =
    new {
      val c: c0.type = c0
  ...
  private implicit def context2macroimplementations(
      c0: MacroContext): FormatInterpolator {
    val c: c0.type
  } =
    new {
      val c: c0.type = c0
    } with FormatInterpolator
  private implicit def context2quasiquote(c0: MacroContext): QuasiquoteImpls {
    val c: c0.type
  } =
    new {
      val c: c0.type = c0
  ...
  private val fastTrackCache = perRunCaches
    .newGeneric[Map[Symbol, FastTrackEntry]] {
