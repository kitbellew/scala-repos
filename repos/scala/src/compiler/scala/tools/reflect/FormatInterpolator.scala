...
abstract class FormatInterpolator {
  ...
  sealed trait Conversion {
    ...
    def noFlags =
      flags.isEmpty || falsely { errorAt(Flags, "flags not allowed") }
    def noWidth =
      width.isEmpty || falsely { errorAt(Width, "width not allowed") }
    def noPrecision =
    ...
    def pickAcceptable(arg: Tree, variants: Type*): Option[Type] =
      variants find (arg.tpe <:< _) orElse (variants find (c
