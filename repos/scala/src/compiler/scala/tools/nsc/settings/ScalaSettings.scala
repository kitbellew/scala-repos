...
trait ScalaSettings
  ...
  protected def experimentalSettings =
    List[BooleanSetting](YmethodInfer, overrideObjects, overrideVars)
  ...
  /*val toolcp =*/
  PathSetting("-toolcp", "Add to the runner classpath.", "")
  ...
  val noassertions = BooleanSetting(
    "-Xdisable-assertions",
    "Generate no assertions or assumptions.") andThen
    (flag => if (flag) elidebelow.value = elidable.ASSERTION + 1)
  ...
  val Ydumpclasses = StringSetting(
    ...
    "")
  val stopAfter = PhasesSetting("-Ystop-after", "Stop after") withAbbreviation
    ("-stop") // backward compat
  ...
  val Ymemberpos = StringSetting(
    "-Yshow-member-pos",
    "output style",
    "Show start and end positions of members",
  ...
  object YoptChoices extends MultiChoiceEnumeration {
    ...
    val inlineProject = Choice(
      "inline-project",
      "Inline only methods defined in the files being compiled. Enables unreachable-code.")
    val inlineGlobal = Choice(
      "inline-global",
    ...
    val lMethod = Choice(
      ...
      "Enable intra-method optimizations: " +
        methodChoices.mkString("", ",", "."),
      expandsTo = methodChoices)

    private val projectChoices = List(lMethod, inlineProject)
    val lProject = Choice(
      "l:project",
      "Enable cross-method optimizations within the current project: " +
        projectChoices.mkString("", ",", "."),
  ...
  def YoptAddToBytecodeRepository =
    YoptBuildCallGraph || YoptInlinerEnabled || YoptClosureInvocations
  ...
  def YoptWarningsSummaryOnly =
    YoptWarnings.value subsetOf
      Set(YoptWarningsChoices.none, YoptWarningsChoices.atInlineFailedSummary)
  ...
  val YoptTrace = StringSetting(
    ...
    "Trace the optimizer progress for a specific method.",
    "")
  ...
  val Ystatistics = {
    ...
    MultiChoiceSetting(
      ...
      default = Some(List("_"))) withPostSetHook { _ =>
  ...
  val optimise = BooleanSetting(
    "-optimise",
    "Compiler flag for the optimizer in Scala 2.11").withAbbreviation(
    "-optimize").withDeprecationMessage(
  ...
  val Xexperimental = BooleanSetting(
    "-Xexperimental",
    "Enable experimental extensions.") enablingIfNotSetByUser
