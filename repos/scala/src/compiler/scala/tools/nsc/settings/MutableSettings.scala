...
class MutableSettings(val errorFn: String => Unit)
  ...
  protected def parseParams(args: List[String]): List[String] = {
    ...
    def tryToSetIfExists(
      ...
      lookupSetting(cmd) match {
        ...
        case None =>
          None //error reported in processArguments
        case Some(cmd) =>
          setter(cmd)(args)
      }
    ...
    def clearIfExists(cmd: String): Option[List[String]] =
      lookupSetting(cmd) match {
        case Some(c: Clearable) =>
          ...
          Some(Nil)
        case Some(s) =>
          s.errorAndValue(s"Missing argument to $cmd", None)
        case None =>
          None
      }
  ...
  def MultiStringSetting(name: String, arg: String, descr: String) =
    add(new MultiStringSetting(name, arg, descr))
  ...
  def StringSetting(name: String, arg: String, descr: String, default: String) =
    add(new StringSetting(name, arg, descr, default))
  def ScalaVersionSetting(
      name: String,
      arg: String,
      descr: String,
  ...
  class OutputDirs {
    ...
    def srcFilesFor(
      ...
      singleOutDir match {
        case Some(d) =>
          d match {
            case _: VirtualDirectory | _: io.ZipArchive =>
              Nil
            case _ =>
              List(d.lookupPathUnchecked(srcPath, directory = false))
          }
        case None =>
  ...
  class BooleanSetting private[nsc] (name: String, descr: String)
    ...
    override def tryToSetColon(args: List[String]) =
      args match {
        ...
        case _ =>
          errorAndValue(s"'$name' accepts only one boolean value", None)
      }
  }
  ...
  class MultiChoiceSetting[E <: MultiChoiceEnumeration] private[nsc] (
    ...
    object ChoiceOrVal {
      def unapply(
        ...
        a match {
          case c: domain.Choice =>
            Some((c.name, c.help, c.expandsTo))
          case v: domain.Value =>
            Some((v.toString, "", Nil))
        }
    }
    ...
    def compute() = {
      def simple(v: domain.Value) =
        v match {
          case ChoiceOrVal(_, _, Nil) =>
            true
          case _ =>
            false
        }
      ...
      /**
        ...
        */
      def expand(vs: domain.ValueSet): domain.ValueSet =
        vs flatMap {
          case c @ ChoiceOrVal(_, _, Nil) =>
            domain.ValueSet(c)
          case ChoiceOrVal(_, _, others) =>
            expand(domain.ValueSet(others: _*))
        }
    ...
    private def tryToSetArgs(args: List[String], halting: Boolean) = {
      ...
      def tryArg(arg: String) =
        arg match {
          case "help" =>
            sawHelp = true
          case s if isChoice(s) =>
            added += s // this case also adds "_"
          case s =>
            badChoice(s)
        }
      def loop(args: List[String]): List[String] =
        args match {
          case arg :: _ if halting && (!isPos(arg) || !isChoice(arg)) =>
            args
          case arg :: rest =>
            tryArg(arg);
            loop(rest)
  ...
  class ChoiceSetting private[nsc] (
      ...
      extends Setting(
        name,
        descr + choices.mkString(" (", ",", ") default:" + default)) {
    type T = String
    protected var v: T = default
    def indexOfChoice: Int = choices indexOf value

    private def usageErrorMessage =
      f"Usage: $name:<$helpArg>%n where <$helpArg> choices are ${choices mkString ", "} (default: $default)%n"
    ...
    override def tryToSetColon(args: List[String]) =
      args match {
        case Nil =>
          errorAndValue(usageErrorMessage, None)
