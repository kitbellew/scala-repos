...
trait JavaParsers extends ast.parser.ParsersCommon with JavaScanners {
  ...
  class JavaUnitParser(val unit: global.CompilationUnit) extends JavaParser {
    ...
    def deprecationWarning(off: Int, msg: String) =
      currentRun.reporting.deprecationWarning(off, msg)
  ...
  abstract class JavaParser extends ParserCommon {
      ...
      if (in.token == LT) {
        ...
        atPos(t1.pos) {
          ...
          if (wildcards.isEmpty)
            ...
            ExistentialTypeTree(t2, wildcards.toList)
        }
      } else
        t
    }
        ...
        in.token match {
          ...
          case _ =>
            val privateWithin: TypeName =
              if (isPackageAccess && !inInterface)
                thisPackageName
              else
                tpnme.EMPTY
    ...
    def bound(): Tree =
      atPos(in.currentPos) {
        ...
        if (ts.tail.isEmpty)
          ...
          CompoundTypeTree(Template(ts, noSelfType, List()))
      }
    ...
    def termDecl(mods: Modifiers, parentToken: Int): List[Tree] = {
      ...
      if (in.token == LPAREN && rtptName != nme.EMPTY && !inInterface) {
        ...
        if (in.token == LPAREN) {
          ...
          if (!isConcreteInterfaceMethod)
            mods1 |= Flags.DEFERRED
          List {
            atPos(pos) {
          ...
          if (inInterface)
            mods1 |= Flags.FINAL | Flags.STATIC
    ...
    def memberDecl(mods: Modifiers, parentToken: Int): List[Tree] =
      in.token match {
        case CLASS | ENUM | INTERFACE | AT =>
          typeDecl(
            if (definesInterface(parentToken))
              mods | Flags.STATIC
            else
              mods)
        case _ =>
    ...
    def addCompanionObject(statics: List[Tree], cdef: ClassDef): List[Tree] = {
      def implWithImport(importStmt: Tree) =
        deriveTemplate(cdef.impl)(importStmt :: _)
      ...
      val cdefNew =
        if (statics.isEmpty)
          ...
          deriveClassDef(cdef)(_ => implWithImport(importCompanionObject(cdef)))
    ...
    def importDecl(): List[Tree] = {
      ...
      if (names.length < 2) {
        ...
        val selector =
          lastname match {
            case nme.WILDCARD =>
              ImportSelector(lastname, lastnameOffset, null, -1)
            case _ =>
