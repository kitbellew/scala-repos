...
abstract class SymbolicXMLBuilder(p: Parsers#Parser, preserveWS: Boolean) {
  ...
  protected def mkXML(
    ...
    def starArgs =
      if (children.isEmpty)
        ...
        List(Typed(makeXMLseq(pos, children), wildStar))
        ...
        List(
          List(
            ...
            if (empty)
              ...
              Literal(Constant(false))) ::: starArgs))
  ...
  final def text(pos: Position, txt: String): Tree =
    atPos(pos) {
      val t =
        if (isPattern)
          makeTextPat(const(txt))
        else
          makeText1(const(txt))
      if (coalescing)
        t updateAttachment TextAttache(pos, txt)
      else
        t
    }
  ...
  def charData(pos: Position, txt: String) =
    if (coalescing)
      ...
      atPos(pos) {
        if (isPattern)
          Apply(_scala_xml(xmlterms._PCData), List(const(txt)))
        else
          New(_scala_xml(_PCData), LL(const(txt)))
      }
  ...
  def element(
    ...
    def mkAttributeTree(pre: String, key: String, value: Tree) =
      atPos(pos.makeTransparent) {
        ...
        val (clazz, attrArgs) =
          if (pre == null)
            (_scala_xml_UnprefixedAttribute, baseArgs)
          else
            (_scala_xml_PrefixedAttribute, const(pre) :: baseArgs)
