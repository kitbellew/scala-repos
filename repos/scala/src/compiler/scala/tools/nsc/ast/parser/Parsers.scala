...
trait Parsers extends Scanners with MarkupParsers with ParsersCommon {
  ...
  class SourceFileParser(val source: SourceFile) extends Parser {
    ...
    /** The parse starting point depends on whether the source file is self-contained:
      ...
      */
    def parseStartRule =
      if (source.isSelfContained)() => compilationUnit() else () => scriptBody()
  ...
  class UnitParser(
    ...
    def showSyntaxErrors() =
      for ((offset, msg) <- syntaxErrors) reporter.error(o2p(offset), msg)
  ...
  abstract class Parser extends ParserCommon {
    ...
    def makeBinop(
      ...
      if (isExpr) {
        if (treeInfo.isLeftAssoc(op)) { Apply(mkSelection(left), arguments) }
        else {
          val x = freshTermName()
          Block(
            List(ValDef(
      ...
      } else { Apply(Ident(op.encode), stripParens(left) :: arguments) }
    }
    ...
    def reduceStack(
      ...
      if (samePrecedence) checkHeadAssoc(leftAssoc)
    ...
    trait PatternContextSensitive {
      ...
      private def tupleInfixType(start: Offset) = {
        ...
        if (in.token == RPAREN) {
          ...
          accept(RPAREN)
          if (in.token == ARROW) atPos(start, in.skipToken()) {
      ...
      /** {{{
        ...
        */
      def annotType(): Tree =
        placeholderTypeBoundary { annotTypeRest(simpleType()) }
      ...
      def compoundType(): Tree =
        compoundTypeRest(
          if (in.token == LBRACE) atInPos(scalaAnyRefConstr) else annotType())
      ...
      def compoundTypeRest(t: Tree): Tree = {
        ...
        ts.toList match {
          ...
          case tps =>
            atPos(t.pos.start)(CompoundTypeTree(
              Template(tps, noSelfType, refinements)))
        }
      }
      ...
      def infixTypeRest(t: Tree, mode: InfixMode.Value): Tree = {
        ...
        def asInfix = {
          ...
          def mkOp(t1: Tree) =
            atPos(t.pos.start, opOffset) { AppliedTypeTree(tycon, List(t, t1)) }
          if (leftAssoc) infixTypeRest(mkOp(compoundType()), InfixMode.LeftOp)
          else mkOp(infixType(InfixMode.RightOp))
        }
        if (isIdent) checkRepeatedParam orElse asInfix else t
      }
    ...
    def mixinQualifierOpt(): TypeName =
      if (in.token == LBRACKET) inBrackets(identForType()) else tpnme.EMPTY
    ...
    def typeOrInfixType(location: Location): Tree =
      if (location == Local) typ() else startInfixType()
            ...
            if (in.token == EQUALS) {
              ...
              if (in.token == USCORE) {
                ...
                t = atPos(t.pos.start, colonPos) {
                  ...
                  if (isWildcard(t)) (placeholderParams: @unchecked) match {
    ...
    def postfixExpr(): Tree = {
      ...
      def loop(top: Tree): Tree =
        if (!isIdent) top
          ...
          if (isExprIntro) prefixExpr() match {
      ...
      if (isUnaryOp) {
        atPos(in.offset) {
          if (lookingAhead(isSimpleExprIntro)) {
            ...
            else Select(stripParens(simpleExpr()), uname)
          } else simpleExpr()
        }
      } else simpleExpr()
    }
    ...
    def blockExpr(): Tree =
      atPos(in.offset) {
        inBraces {
          if (in.token == CASE) Match(EmptyTree, caseClauses()) else block()
        }
      }
    ...
    def generator(eqOK: Boolean, allowNestedIf: Boolean = true): List[Tree] = {
      ...
      if (hasEq && eqOK) in.nextToken() else accept(LARROW)
      val rhs = expr()

      def loop(): List[Tree] =
        if (in.token != IF) Nil else makeFilter(in.offset, guard()) :: loop()

      val tail = if (allowNestedIf) loop() else Nil
    ...
    def argumentPatterns(): List[Tree] =
      inParens { if (in.token == RPAREN) Nil else seqPatterns() }
    ...
    def accessQualifierOpt(mods: Modifiers): Modifiers = {
      ...
      if (in.token == LBRACKET) {
        ...
        result =
          if (in.token == THIS) { in.nextToken(); mods | Flags.LOCAL }
    ...
    def annotationExpr(): Tree =
      atPos(in.offset) {
        ...
        if (in.token == LPAREN) New(t, multipleArgumentExprs()) else New(t, Nil)
      }
    ...
    def importExpr(): Tree = {
        ...
        val selectors: List[ImportSelector] = in.token match {
          ...
          case _ =>
            ...
            if (in.token == DOT) {
              ...
              val t = atPos(
                start,
                if (name == nme.ERROR) in.offset else nameOffset)(Select(
                expr,
                name))
              in.nextToken()
              return loop(t)
      ...
      loop(in.token match {
        ...
        case _ =>
          val id = atPos(start)(Ident(ident()))
          accept(DOT)
          if (in.token == THIS) thisDotted(id.name.toTypeName) else id
      })
    }
    ...
    def funDefRest(
      ...
      val result = atPos(
        start,
        if (name.toTermName == nme.ERROR) start else nameOffset) {
        var newmods = mods
      ...
      atPos(start, if (name == tpnme.ERROR) start else nameOffset) {
        savingClassContextBounds {
          ...
          if (!classContextBounds.isEmpty && mods.isTrait) {
            val viewBoundsExist =
              if (settings.future) "" else " nor view bounds `<% ...'"
            syntaxError(
    ...
    def makePackageObject(start: Offset, objDef: ModuleDef): PackageDef =
      objDef match {
        case ModuleDef(mods, name, impl) =>
          makePackaging(
            start,
            atPos(o2p(objDef.pos.start)) { Ident(name) },
      ...
      checkNoEscapingPlaceholders {
        ...
        while (!isStatSeqEnd && !isCaseDefEnd) {
          if (in.token == IMPORT) {
            ...
            } else { stats ++= localDef(0) }
            acceptStatSepOpt()
          } else if (isExprIntro) {
            stats += statement(InBlock)
            if (!isCaseDefEnd) acceptStatSep()
          } else if (isStatSep) { in.nextToken() }
          else {
            val addendum =
              if (isModifier) " (no modifiers allowed here)" else ""
            syntaxErrorOrIncomplete(
          ...
          case stats =>
            val start =
              if (stats forall (_ == EmptyTree)) 0
                ...
                if (wpos.isDefined) wpos.start else 0
              }
