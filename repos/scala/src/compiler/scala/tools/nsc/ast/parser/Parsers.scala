...
trait Parsers extends Scanners with MarkupParsers with ParsersCommon {
  ...
  class SourceFileParser(val source: SourceFile) extends Parser {
    ...
    object symbXMLBuilder
        extends SymbolicXMLBuilder(this, preserveWS = true) { // DEBUG choices
  ...
  class UnitParser(
    ...
    override def incompleteInputError(msg: String): Unit = {
      ...
      if (smartParsing)
        ...
        currentRun.parsing.incompleteInputError(o2p(offset), msg)
    }
  ...
  lazy val ScalaValueClassNames = Seq(
    tpnme.AnyVal,
    tpnme.Unit,
    tpnme.Boolean,
    tpnme.Byte,
  ...
  abstract class Parser extends ParserCommon {
    ...
    def scriptBody(): Tree = {
      ...
      def searchForMain(): Option[Tree] = {
         ...
         * via import, etc.
         */
        def isMainMethod(t: Tree) =
          t match {
            case DefDef(_, nme.main, Nil, List(_), _, _) =>
              true
            case _ =>
              false
          }
        ...
        val newStmts = stmts collect {
          ...
          case md @ ModuleDef(mods, name, template)
              if !seenModule && (md exists isMainMethod) =>
            seenModule = true
            ...
            if (name == mainModuleName)
              ...
              treeCopy.ModuleDef(md, mods, mainModuleName, template)
          case _ =>
      ...
      def mainParameter =
        List(ValDef(Modifiers(Flags.PARAM), nme.args, mainParamType, EmptyTree))
      def mainDef =
        DefDef(
          NoMods,
          nme.main,
    ...
    def placeholderTypeBoundary(op: => Tree): Tree = {
      ...
      if (!placeholderTypes.isEmpty && t.isInstanceOf[AppliedTypeTree]) {
        ...
        t =
          atPos(expos) {
            ExistentialTypeTree(t, placeholderTypes.reverse)
          }
        placeholderTypes = List()
      }
    ...
    protected def skip(targetToken: Token): Unit = {
      ...
      while (true) {
        ...
        if (targetToken == in.token && nparens == 0 && nbraces == 0)
          return
        in.nextToken()
      }
    }
    ...
    def isTemplateIntro: Boolean =
      in.token match {
        case OBJECT | CASEOBJECT | CLASS | CASECLASS | TRAIT =>
          true
        case _ =>
          false
      }
      ...
      atPos(tree.pos) {
        ...
        tree match {
          ...
          case build.SyntacticTuple(as) =>
            val arity = as.length
            val example = analyzer.exampleTuplePattern(
              as map {
                case Ident(name) =>
                  name;
                case _ =>
            ...
            val msg = sm"""|not a legal formal parameter.
    ...
    def makeSafeTupleType(elems: List[Tree], offset: Offset) = {
      if (checkTupleSize(elems, offset))
        makeTupleType(elems)
      else
        makeTupleType(
          Nil
    ...
    def stripParens(t: Tree) =
      t match {
        case Parens(ts) =>
          atPos(t.pos) {
            makeSafeTupleTerm(ts, t.pos.point)
          }
        case _ =>
          t
      }
    ...
    def makeBinop(
      ...
      require(
        isExpr || targs.isEmpty || targs.exists(_.isErroneous),
      ...
      def mkSelection(t: Tree) = {
        ...
        if (targs.isEmpty)
          ...
          atPos(left.pos)(TypeApply(sel, targs))
      }
      def mkNamed(args: List[Tree]) =
        if (isExpr)
          args map treeInfo.assignmentToMaybeNamedArg
        else
          args
      val arguments =
        right match {
      ...
      if (isExpr) {
        if (treeInfo.isLeftAssoc(op)) {
          ...
          Block(
            List(
              ValDef(
                Modifiers(symtab.Flags.SYNTHETIC | symtab.Flags.ARTIFACT),
                x,
                TypeTree(),
                stripParens(left))),
    ...
    trait PatternContextSensitive {
      ...
      private def makeExistentialTypeTree(t: Tree) = {
        ...
        ExistentialTypeTree(
          t,
          refinement() flatMap {
            case t @ TypeDef(_, _, _, TypeBoundsTree(_, _)) =>
              Some(t)
            case t @ ValDef(_, _, _, EmptyTree) =>
              Some(t)
            case EmptyTree =>
              None
            case _ =>
      ...
      def typ(): Tree =
        placeholderTypeBoundary {
          ...
          val t =
            if (in.token == LPAREN)
              tupleInfixType(start)
            else
              infixType(InfixMode.FirstOp)
      ...
      def infixTypeRest(t: Tree, mode: InfixMode.Value): Tree = {
        ...
        def checkRepeatedParam =
          if (isRawStar) {
            lookingAhead(
              in.token match {
                case RPAREN | COMMA | EQUALS | RBRACE =>
                  t
                case _ =>
                  EmptyTree
              })
        ...
        if (isIdent)
          checkRepeatedParam orElse asInfix
        else
          t
      }
    ...
    def selector(t: Tree): Tree = {
      val point =
        if (isIdent)
          ...
          in.lastOffset //SI-8459
      //assert(t.pos.isDefined, t)
      if (t != EmptyTree)
        Select(t, ident(skipIt = false)) setPos r2p(
          t.pos.start,
          point,
          in.lastOffset)
      else
    ...
    def path(thisOK: Boolean, typeOK: Boolean): Tree = {
      ...
      if (in.token == THIS) {
        ...
        t =
          atPos(start) {
            Super(This(tpnme.EMPTY), mixinQualifierOpt())
          }
        accept(DOT)
        t = selector(t)
        if (in.token == DOT)
    ...
    def selectors(t: Tree, typeOK: Boolean, dotOffset: Offset): Tree =
      if (typeOK && in.token == TYPE) {
        in.nextToken()
        atPos(t.pos.start, dotOffset) {
          SingletonTypeTree(t)
        }
      } else {
        val t1 = selector(t)
        if (in.token == DOT) {
          selectors(t1, typeOK, in.skipToken())
        } else
          t1
      }
    ...
    /** {{{
      ...
      *  }}}
      */
    def stableId(): Tree = path(thisOK = false, typeOK = false)
    ...
    private def pkgQualId() = {
      ...
      if (currentPackage == "")
        ...
        currentPackage = currentPackage + "." + pkg
          ...
          finish(
            in.token match {
              case CHARLIT =>
                in.charVal
              case INTLIT =>
                in.intVal(isNegated).toInt
              case LONGLIT =>
                in.intVal(isNegated)
              case FLOATLIT =>
                in.floatVal(isNegated).toFloat
              case DOUBLELIT =>
              ...
              case NULL =>
                null
              case _ =>
                syntaxErrorOrIncompleteAnd("illegal literal", skipIt = true)(
                  null)
            })
      }
    ...
    def newLineOptWhenFollowedBy(token: Offset): Unit = {
      ...
      if (in.token == NEWLINE && in.next.token == token)
        newLineOpt()
    }

    def newLineOptWhenFollowing(p: Token => Boolean): Unit = {
      // note: next is defined here because current == NEWLINE
      if (in.token == NEWLINE && p(in.next.token))
        newLineOpt()
    }
      ...
      (in.token: @scala.annotation.switch) match {
        ...
        case FOR =>
          ...
          def parseFor =
            atPos(start) {
              val enums =
                if (in.token == LBRACE)
                  inBracesOrNil(enumerators())
                else
                  inParensOrNil(enumerators())
              newLinesOpt()
              if (in.token == YIELD) {
                in.nextToken()
        ...
        case _ =>
          def parseOther = {
            ...
            def lhsIsTypedParamList() =
              t match {
                case Parens(xs) if xs.forall(isTypedParam) =>
                  true
                case _ =>
                  false
              }
            if (in.token == ARROW && (
                  location != InTemplate || lhsIsTypedParamList
                )) {
              t =
                atPos(t.pos.start, in.skipToken()) {
                  Function(
      ...
      def loop(top: Tree): Tree =
        if (!isIdent)
          ...
          if (isExprIntro)
            prefixExpr() match {
              case EmptyTree =>
                reduceExprStack(base, top)
              case next =>
                loop(next)
            }
          else
            finishPostfixOp(start, base, popOpInfo())
        }
    ...
    def argumentExprs(): List[Tree] = {
      def args(): List[Tree] =
        commaSeparated(
          if (isIdent)
            treeInfo.assignmentToMaybeNamedArg(expr())
          else
            expr())
      in.token match {
        case LBRACE =>
    ...
    def multipleArgumentExprs(): List[List[Tree]] = {
      if (in.token != LPAREN)
        ...
        argumentExprs() :: multipleArgumentExprs()
    }
    ...
    def caseBlock(): Tree = atPos(accept(ARROW))(block())
    ...
    def enumerator(
      ...
      if (in.token == IF && !isFirst)
        makeFilter(in.offset, guard()) :: Nil
      else
        generator(!isFirst, allowNestedIf)
    ...
    def generator(eqOK: Boolean, allowNestedIf: Boolean = true): List[Tree] = {
      ...
      if (hasVal) {
        if (hasEq)
          ...
          syntaxError(
            in.offset,
            "val in for comprehension must be followed by assignment")
      }
    ...
    trait SeqContextSensitive extends PatternContextSensitive {
      ...
      def pattern(): Tree = {
        ...
        loop() match {
          case pat :: Nil =>
            pat
          case xs =>
            atPos(start)(makeAlternative(xs))
        }
      }
      ...
      def pattern3(): Tree = {
        ...
        def loop(top: Tree): Tree =
          reducePatternStack(base, top) match {
      ...
      def badPattern3(): Tree = {
        ...
        val (isUnderscore, isStar) =
          opstack match {
            case OpInfo(Ident(nme.WILDCARD), nme.STAR, _, _) :: _ =>
              (true, true)
            case OpInfo(_, nme.STAR, _, _) :: _ =>
              (false, true)
            case _ =>
              (false, false)
          }
        ...
        val subtext =
          (isUnderscore, isStar, isSequenceOK) match {
    ...
    /** {{{
      ...
      *  }}}
      */
    def accessModifierOpt(): Modifiers =
      normalizeModifiers {
        in.token match {
    ...
    def constructorAnnotations(): List[Tree] =
      readAnnots {
        ...
        case _ =>
          ...
          if (isRawStar) {
            ...
            if (useStartAsPosition)
              atPos(start)(repeatedApplication(t))
            else
              atPos(t.pos.start, t.pos.point)(repeatedApplication(t))
          } else
            t
      }
    }
    ...
    def param(owner: Name, implicitmod: Int, caseParam: Boolean): ValDef = {
      ...
      if (owner.isTypeName) {
        ...
        in.token match {
          case v @ (VAL | VAR) =>
            ...
            in.nextToken()
          case _ =>
            if (mods.flags != Flags.PARAMACCESSOR)
              accept(VAL)
            if (!caseParam)
              mods |= Flags.PrivateLocal
        }
        if (caseParam)
      ...
      atPos(
        ...
        if (name == nme.ERROR)
          ...
          nameOffset) {
        ValDef(
          (mods | implicitmod.toLong | bynamemod) withAnnotations annots,
          name.toTermName,
          tpt,
          default)
      }
    ...
    def importExpr(): Tree = {
                ...
                val t =
                  atPos(
                    ...
                    if (name == nme.ERROR)
                      ...
                      nameOffset)(Select(expr, name))
                in.nextToken()
                return loop(t)
              }
              // import foo.bar.Baz;
              else
                List(makeImportSelector(name, nameOffset))
          }
      ...
      loop(
        in.token match {
          case THIS =>
            thisDotted(tpnme.EMPTY)
          case _ =>
            val id = atPos(start)(Ident(ident()))
            accept(DOT)
            if (in.token == THIS)
              thisDotted(id.name.toTypeName)
            else
              id
        })
    }
    ...
    def patDefOrDcl(pos: Int, mods: Modifiers): List[Tree] = {
        ...
        val trees = {
          val pat =
            if (tp.isEmpty)
              ...
              Typed(p, tp) setPos (p.pos union tp.pos)
      ...
      val trees = (
        lhs.toList.init flatMap (mkDefs(_, tp.duplicate, rhs.duplicate))
      ) ::: mkDefs(lhs.last, tp, rhs)
      val hd = trees.head
    ...
    def constrExpr(vparamss: List[List[ValDef]]): Tree =
      if (in.token == LBRACE)
        ...
        Block(selfInvocation(vparamss) :: Nil, literalUnit)
    ...
    def tmplDef(pos: Offset, mods: Modifiers): Tree = {
      if (mods.isLazy)
        syntaxError("classes cannot be lazy", skipIt = false)
      in.token match {
        case TRAIT =>
          classDef(
            pos,
    ...
    def makePackageObject(start: Offset, objDef: ModuleDef): PackageDef =
      objDef match {
        case ModuleDef(mods, name, impl) =>
          makePackaging(
            start,
            atPos(o2p(objDef.pos.start)) {
              Ident(name)
            },
            List(ModuleDef(mods, nme.PACKAGEkw, impl)))
      }
    ...
    def ensureEarlyDef(tree: Tree): Tree =
      tree match {
    ...
    def templateOpt(
      ...
      val tstart1 =
        if (body.isEmpty && in.lastOffset < tstart)
          in.lastOffset
        else
          tstart
      ...
      atPos(tstart1) {
        ...
        if (inScalaRootPackage && ScalaValueClassNames.contains(name))
          ...
          gen.mkTemplate(
            gen.mkParents(mods, parents, parentPos),
            self,
            constrMods,
            vparamss,
            body,
      ...
      if (in.token == LBRACE) {
        ...
        if (in.token == LPAREN) {
          if (parenMeansSyntaxError)
            syntaxError(
              s"traits or objects may not have parameters",
              skipIt = true)
          else
            abort("unexpected opening parenthesis")
        }
        (noSelfType, List())
      }
    }
    ...
    def makePackaging(start: Offset, pkg: Tree, stats: List[Tree]): PackageDef =
      pkg match {
        case x: RefTree =>
          atPos(start, pkg.pos.point)(PackageDef(x, stats))
      }
    ...
    def statSeq(
      ...
      def default(tok: Token) =
        if (isStatSep)
          ...
          syntaxErrorOrIncompleteAnd(errorMsg, skipIt = true)(Nil)
      while (!isStatSeqEnd) {
        ...
        while (!isStatSeqEnd && !isCaseDefEnd) {
          if (in.token == IMPORT) {
            ...
            if (in.token == IMPLICIT) {
              val start = in.skipToken()
              if (isIdent)
                stats += implicitClosure(start, InBlock)
              else
                stats ++= localDef(Flags.IMPLICIT)
            } else {
              stats ++= localDef(0)
            }
            acceptStatSepOpt()
    ...
    def compilationUnit(): PackageDef =
      checkNoEscapingPlaceholders {
        ...
        resetPackage()
        topstats() match {
          case (stat @ PackageDef(_, _)) :: Nil =>
            stat
          case stats =>
            val start =
              if (stats forall (_ == EmptyTree))
                0
              else {
