...
// ------------------ Debugging -------------------------------------
  ...
  @inline
  final def assert(assertion: Boolean) {
    assert(assertion, "")
  }
  @inline
  ...
  @inline
  final def require(requirement: Boolean) {
    require(requirement, "")
  }

  @inline
  final def ifDebug(body: => Unit) {
    if (settings.debug)
      body
  }
  ...
  @inline
  final override def debuglog(msg: => String) {
    if (settings.debug)
      log(msg)
  }
...
// ------------ Phases -------------------------------------------}
  ...
  def phaseDescriptions: String =
    phaseHelp("description", elliptically = true, phasesDescMap)
  ...
  def extendCompilerClassPath(urls: URL*): Unit = {
    if (settings.YclasspathImpl.value == ClassPathRepresentationType.Flat)
      throw new UnsupportedOperationException(
        "Flat classpath doesn't support extending the compiler classpath")
  ...
  private def isSystemPackageClass(pkg: Symbol) =
    pkg == RootClass ||
      (pkg.hasTransOwner(definitions.ScalaPackageClass) &&
        !pkg.hasTransOwner(
  ...
  def invalidateClassPathEntries(paths: String*): Unit = {
    if (settings.YclasspathImpl.value == ClassPathRepresentationType.Flat)
      throw new UnsupportedOperationException(
        "Flat classpath doesn't support the classpath invalidation")

    implicit object ClassPathOrdering extends Ordering[PlatformClassPath] {
      def compare(a: PlatformClassPath, b: PlatformClassPath) =
        a.asClassPathString compare b.asClassPathString
    }
  ...
  /** Merges new classpath entries into the symbol table
    ...
    */
  private def mergeNewEntries(
      newEntries: PlatformClassPath,
      root: ClassSymbol,
      allEntries: OptClassPath,
      oldEntries: OptClassPath,
      invalidated: mutable.ListBuffer[ClassSymbol],
      failed: mutable.ListBuffer[ClassSymbol]) {
  ...
  class Run extends RunContextApi with RunReporting with RunParsing {
    ...
    private class SyncedCompilationBuffer {
      ...
      def head: CompilationUnit =
        synchronized {
          underlying.head
        }
      def apply(i: Int): CompilationUnit =
        synchronized {
          underlying(i)
        }
      ...
      def toList: List[CompilationUnit] =
        synchronized {
          underlying.toList
        }
    }
    ...
    private def warnDeprecatedAndConflictingSettings(unit: CompilationUnit) {
      ...
      settings.userSetSettings filter
        (_.isDeprecated) foreach { s =>
          currentRun
            .reporting
            .deprecationWarning(
              NoPosition,
    ...
    def compileLate(unit: CompilationUnit) {
      ...
      if (firstPhase ne
            null) { // we might get here during initialization, is a source is newer than the binary
        val maxId = math.max(globalPhase.id, typerPhase.id)
        firstPhase.iterator takeWhile
          (_.id < maxId) foreach
          (ph =>
  ...
  def getFile(clazz: Symbol, suffix: String): File =
    getFile(clazz.sourceFile, clazz.fullName split '.', suffix)
...
object Global {
  ...
  def apply(settings: Settings): Global =
    new Global(settings, reporter(settings))
