...
trait ContextErrors {
  ...
  case class DivergentImplicitTypeError(
      underlyingTree: Tree,
      pt0: Type,
      sym: Symbol)
      extends TreeTypeError {
  ...
  object ErrorUtils {
    def issueNormalTypeError(tree: Tree, msg: String)(implicit
        context: Context) { issueTypeError(NormalTypeError(tree, msg)) }

    def issueSymbolTypeError(sym: Symbol, msg: String)(implicit
        context: Context) { issueTypeError(SymbolTypeError(sym, msg)) }
  ...
  private def MacroIncompatibleEngineError(
  ...
  trait TyperContextErrors {
    ...
    object TyperErrorGen {
      ...
      def AdaptTypeError(tree: Tree, found: Type, req: Type) = {
        ...
        issueNormalTypeError(
          callee,
          withAddendum(callee.pos)(typeErrorMsg(foundType, req)))
      ...
      def AppliedTypeWrongNumberOfArgsError(
          tree: Tree,
          tpt: Tree,
          tparams: List[Symbol]) = {
      ...
      def WrongNumberOfParametersError(tree: Tree, argpts: List[Type]) = {
        issueNormalTypeError(
          tree,
          "wrong number of parameters; expected = " + argpts.length)
        setError(tree)
      }
      ...
      def QualifyingClassError(tree: Tree, qual: Name) = {
        issueNormalTypeError(
          tree,
          if (qual.isEmpty)
            tree + " can be used only in a class, object, or template"
      ...
      def DefDefinedTwiceError(sym0: Symbol, sym1: Symbol) = {
        ...
        val isBug = sym0.isAbstractType && sym1.isAbstractType && (sym0
          .name startsWith "_$")
        val addendums = List(
          ...
          else if ((sym0.associatedFile ne NoAbstractFile) && (sym1
                     .associatedFile ne NoAbstractFile))
      ...
      protected def macroExpansionError(
        ...
        if (msg != null)
          context
            .error(
              if (pos.isDefined) pos else expandee.pos,
              msg
            ) // issueTypeError(PosAndMsgTypeError(..)) won't work => swallows positions
        setError(expandee)
      ...
      def MacroGeneratedException(expandee: Tree, ex: Throwable) = {
        ...
        val message = {
          try {
            ...
            val relevancyThreshold = realex.getStackTrace()
              .indexWhere(_.getMethodName endsWith "macroExpandWithRuntime")
            if (relevancyThreshold == -1) None
            else {
              var relevantElements = realex.getStackTrace()
                .take(relevancyThreshold + 1)
              def isMacroInvoker(este: StackTraceElement) =
                este.isNativeMethod || (este.getClassName != null && (este
                  .getClassName contains "fastTrack"))
              var threshold = relevantElements.reverse
  ...
  trait InferencerContextErrors {
    ...
    object InferErrorGen {
      ...
      private def issueAmbiguousTypeErrorUnlessErroneous(
          pos: Position,
          pre: Type,
          sym1: Symbol,
          sym2: Symbol,
      ...
      // TODO: no test case
      def NoConstructorInstanceError(
          tree: Tree,
          restpe: Type,
          pt: Type,
          msg: String) = {
      ...
      def NoBestMethodAlternativeError(
          tree: Tree,
          argtpes: List[Type],
          pt: Type,
          lastTry: Boolean) = {
      ...
      def AmbiguousMethodAlternativeError(
          tree: Tree,
          pre: Type,
          best: Symbol,
      ...
      def NoBestExprAlternativeError(tree: Tree, pt: Type, lastTry: Boolean) = {
        issueNormalTypeError(
          tree,
          withAddendum(tree.pos)(typeErrorMsg(tree.symbol.tpe, pt)))
        setErrorOnLastTry(lastTry, tree)
      }

      def AmbiguousExprAlternativeError(
          tree: Tree,
          pre: Type,
          best: Symbol,
          firstCompeting: Symbol,
      ...
      private[scala] def NotWithinBoundsErrorMessage(
        ...
        prefix + "type arguments " + targs.mkString("[", ",", "]") +
          " do not conform to " + tparams.head
      ...
      //substExpr
      def PolymorphicExpressionInstantiationError(
          tree: Tree,
      ...
      def IllegalModifierCombination(sym: Symbol, flag1: Int, flag2: Int) =
        issueSymbolTypeError(
          ...
          "illegal combination of modifiers: %s and %s for: %s".format(
            Flags.flagsToString(flag1.toLong),
            Flags.flagsToString(flag2.toLong),
            sym))
      ...
      if (!info1.tpe.isErroneous && !info2.tpe.isErroneous) {
        ...
        def viewMsg = {
          ...
          def explanation = {
            ...
            if (AnyRefTpe <:< req)
              (if (sym == AnyClass || sym == UnitClass) (sm"""|Note: ${sym
