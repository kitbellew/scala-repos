...
trait Contexts {
  ...
  def warnUnusedImports(unit: CompilationUnit) = {
    for (imps <- allImportInfos.remove(unit)) {
      for (imp <- imps.reverse.distinct) {
        ...
        imp.tree.selectors filterNot
          (s => isMask(s) || used(s)) foreach { sel =>
  ...
  class Context private[typechecker] (
    ...
    def makeSilent(
        reportAmbiguousErrors: Boolean = ambiguousErrors,
      ...
      def accessWithin(ab: Symbol) = {
        ...
        if (sym.isJavaDefined) {
          ...
          def abEnclosesStopAtPkg(o: Symbol): Boolean =
            (o eq ab) ||
              (!o.isPackageClass &&
                (o ne NoSymbol) && abEnclosesStopAtPkg(o.owner))
    ...
    /** A symbol `sym` qualifies as an implicit if it has the IMPLICIT flag set,
      ...
      */
    private def isQualifyingImplicit(
        name: Name,
        sym: Symbol,
        pre: Type,
        imported: Boolean) =
      sym.isImplicit && isAccessible(sym, pre) &&
        !(
          imported && {
    ...
    /** It's possible that seemingly conflicting identifiers are
      ...
      */
    private def resolveAmbiguousImport(
        name: Name,
        imp1: ImportInfo,
    ...
    /** The symbol with name `name` imported via the import in `imp`,
      ...
      */
    private def importedAccessibleSymbol(
        imp: ImportInfo,
        name: Name,
  ...
  abstract class ContextReporter(
    ...
    final def propagatingErrorsTo[T](target: ContextReporter)(expr: => T): T = {
      ...
      if ((this ne target) &&
          hasErrors) { // `this eq target` in e.g., test/files/neg/divergent-implicit.scala
    ...
    final private[typechecker] def ++=(
        errors: Traversable[AbsTypeError]): Unit = errorBuffer ++= errors
  ...
  class ImportInfo(val tree: Import, val depth: Int) {
    ...
    def importedSymbol(
      ...
      if (record && settings.warnUnusedImport && selectors.nonEmpty &&
          result != NoSymbol && pos != NoPosition)
