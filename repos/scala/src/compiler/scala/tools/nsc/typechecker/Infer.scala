...
trait Infer extends Checkable {
  ...
  private[typechecker] def isFullyDefined(tp: Type): Boolean =
    tp match {
      ...
      case TypeVar(_, constr) if constr.inst == NoType =>
        false
      case _ =>
        falseIfNoInstance({
          instantiate(tp);
  ...
  abstract class Inferencer
    ...
    def checkAccessible(
      ...
      def malformed(ex: MalformedType, instance: Type): Type = {
        val what =
          if (ex.msg contains "malformed type")
            "is malformed"
          else
            s"contains a ${ex.msg}"
    ...
    def makeFullyDefined(tp: Type): Type = {
      ...
      if (tp eq tp1)
        ...
        existentialAbstraction(tparams.reverse, tp1)
    }
    def ensureFullyDefined(tp: Type): Type =
      if (isFullyDefined(tp))
        tp
      else
        makeFullyDefined(tp)
    ...
    private def exprTypeArgs(
      ...
      def conforms =
        if (useWeaklyCompatible)
          isWeaklyCompatible(restpeInst, pt)
        else
          isCompatible(restpeInst, pt)
          ...
          Some(
            toLists {
              ...
              (
                okArgs,
                okTparams,
                m.values.map(_.getOrElse(NothingTpe)),
                nok.keys)
            })
      }
    ...
    def methTypeArgs(
      ...
      def argumentPosition(idx: Int): Position =
        context.tree match {
          case x: ValOrDefDef =>
            x.rhs match {
              case Apply(fn, args) if idx < args.size =>
                args(idx).pos
              case _ =>
                context.tree.pos
            }
      ...
      tp match {
        ...
        case _ =>
          ...
          tp nonPrivateMember nme.apply match {
            ...
            case sym =>
              OverloadedType(tp, sym.filter(_.isPublic).alternatives)
          }
      }
    ...
    def eligibleForTupleConversion(
      ...
      def canSendTuple =
        argsCount match {
          case 0 =>
            !varargsTarget // avoid () to (()) conversion - SI-3224
          case 1 =>
            false // can't tuple a single argument
          case n =>
            n <= MaxTupleArity // <= 22 arguments
        }
      def canReceiveTuple =
        paramsCount match {
          case 1 =>
...
//                                 (!ftpe2.isInstanceOf[OverloadedType] || ftpe1.isInstanceOf[OverloadedType]) &&
    ...
    def inferExprInstance(
      ...
      val treeTp =
        if (treeTp0 eq null)
          ...
          treeTp0 // can't refer to tree in default for treeTp0
      val tvars = tparams map freshVar
          ...
          try {
            ...
            val variances =
              if (ctorTp.paramTypes.isEmpty)
                ...
                undetparams map varianceInTypes(ctorTp.paramTypes)
    ...
    def instBounds(tvar: TypeVar): TypeBounds = {
      ...
      val (loBounds, hiBounds) =
        if (isFullyDefined(instType))
          (List(instType), List(instType))
        else
          (tvar.constr.loBounds, tvar.constr.hiBounds)
    ...
    def inferTypedPattern(
       ...
       */
      if (tpparams.isEmpty && ptparams.nonEmpty)
        intersect(pattp, pt)
      else
        intersect(pt, pattp)
    }
    ...
    def freeTypeParamsOfTerms(tp: Type): List[Symbol] = {
      ...
      val syms = tp collect {
        case TypeRef(_, sym, _) if isFreeTypeParamOfTerm(sym) =>
          sym
      }
      syms.distinct
    }
    ...
    private def containsNamedType(argtpes: List[Type]): Boolean =
      argtpes match {
        case Nil =>
          false
        case NamedType(_, _) :: _ =>
          true
        case _ :: rest =>
          containsNamedType(rest)
      }
