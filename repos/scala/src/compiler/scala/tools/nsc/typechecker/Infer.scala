...
trait Infer extends Checkable {
  ...
  private lazy val stdErrorClass = rootMirror.RootClass
  ...
  abstract class Inferencer
      ...
      if (settings.debug) {
        ...
        Console.println(
          "" + pre + " " + sym.owner + " " + context.owner + " " + context.outer
            .enclClass.owner + " " + sym.owner.thisType + (pre =:= sym.owner
            .thisType))
      }
      ...
      if (sym.isError) tree setSymbol sym setType ErrorType
        ...
        accessible match {
          ...
          case sym =>
            ...
            tree setSymbol sym1 setType (pre match {
              case _: SuperType =>
    ...
    def protoTypeArgs(
      ...
      def instantiateToBound(tvar: TypeVar, variance: Variance): Type = {
        ...
        else if (hiBounds.nonEmpty && (variance.isPositive || loBounds
                   .nonEmpty && upper <:< lower)) setInst(upper)
        else WildcardType
      }
    ...
    def adjustTypeArgs(
      ...
      foreach3(tparams, tvars, targs) { (tparam, tvar, targ) =>
        val retract = (targ.typeSymbol == NothingClass // only retract Nothings
          && (restpe.isWildcard || !varianceInType(restpe)(tparam)
          ...
          (
            ...
            if (retract) None
              ...
              Some(
                ...
                else if (targ.typeSymbol.isModuleClass || tvar.constr
                           .avoidWiden) targ
                else targ.widen)))
      }
      buf.result()
    ...
    private[typechecker] def isApplicableSafe(
      ...
      def applicableExpectingPt(pt: Type): Boolean = {
        ...
        val result = newTyper(silent).infer
    ...
    def inferConstructorInstance(
      ...
      def inferForApproxPt =
        if (isFullyDefined(pt)) {
          inferFor(pt.instantiateTypeParams(
            ptparams,
      ...
      inferFor(pt) orElse inferForApproxPt match {
        ...
        case _ =>
          ...
          devWarning(s"failed inferConstructorInstance for $tree: ${tree
