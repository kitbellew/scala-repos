  ...
  abstract class Namer(val context: Context)
    ...
    def createNamer(tree: Tree): Namer = {
      ...
      def isConstrParam(vd: ValDef) = {
        (sym hasFlag PARAM | PRESUPER) && !vd.mods.isJavaDefined &&
    ...
    def createPrimaryConstructorParameterNamer
      ...
      val paramContext = outerContext
        .makeNewScope(outerContext.tree, outerContext.owner)
    ...
    def enclosingNamerWithScope(scope: Scope) = {
      var cx = context
      while (cx != NoContext && cx.scope != scope)
        cx = cx.outer
      if (cx == NoContext || cx == context)
        thisNamer
      else
        newNamer(cx)
    }
    ...
    def deriveAccessors(vd: ValDef) =
      vd.mods.isLazy || (owner.isClass && deriveAccessorsInClass(vd))
    ...
    private def deriveAccessorsInClass(vd: ValDef) =
      !vd.mods.isPrivateLocal && // note, private[this] lazy vals do get accessors -- see outer disjunction of deriveAccessors
        ...
        ) && // outer accessors are added later, in explicitouter
          !isEnumConstant(
            vd
    ...
    def isEnumConstant(vd: ValDef) = {
      val ownerHasEnumFlag =
        ...
        if (context.unit.isJava)
          owner.companionClass.hasJavaEnumFlag
        else
          owner.hasJavaEnumFlag
    ...
    def setPrivateWithin[T <: Symbol](tree: Tree, sym: T, mods: Modifiers): T =
      if (sym.isPrivateLocal || !mods.hasAccessBoundary)
        ...
        sym setPrivateWithin
          typer.qualifyingClass(tree, mods.privateWithin, packageOK = true)

    def setPrivateWithin(tree: MemberDef, sym: Symbol): Symbol =
      setPrivateWithin(tree, sym, tree.mods)

    def inConstructorFlag: Long = {
      val termOwnedContexts: List[Context] = context
        .enclosingContextChain
        .takeWhile(c => c.owner.isTerm && !c.owner.isAnonymousFunction)
      val constructorNonSuffix = termOwnedContexts exists
        (c => c.owner.isConstructor && !c.inConstructorSuffix)
      val earlyInit = termOwnedContexts exists (_.owner.isEarlyInitialized)
      if (constructorNonSuffix || earlyInit)
        INCONSTRUCTOR
      else
        0L
    }
    ...
    protected def conflict(newS: Symbol, oldS: Symbol) =
      ((!oldS.isSourceMethod || nme.isSetterName(newS.name) ||
        newS.isTopLevel) &&
    ...
    private def inCurrentScope(m: Symbol): Boolean = {
      if (owner.isClass)
        ...
        m.owner.isClass && context.scope == m.owner.info.decls
    }
      ...
      tree match {
        ...
        case ValDef(_, _, _, _) =>
          if (isParameter)
            ...
            owner.newValue(name.toTermName, pos, flags)
      }
    }
    ...
    def enterClassSymbol(tree: ClassDef): Symbol = {
      ...
      val isRedefinition =
        (existing.isType && existing.isTopLevel &&
      ...
      val clazz: Symbol = {
        if (isRedefinition) {
          ...
          assignAndEnterSymbol(tree) setFlag inConstructorFlag
      }
      clazz match {
    ...
    def validateCompanionDefs(tree: ImplDef) {
      ...
      val module =
        if (sym.isModule)
          ...
          ctx.scope lookupModule tree.name
      val clazz =
        if (sym.isClass)
          sym
        else
          ctx.scope lookupClass tree.name
      val fails =
        (module.isModule && clazz.isClass && !module.isSynthetic &&
          !clazz.isSynthetic &&
          (clazz.sourceFile ne null) &&
          (module.sourceFile ne null) && !(module isCoDefinedWith clazz) &&
          module.exists && clazz.exists)
      if (fails) {
        reporter.error(
          tree.pos,
          (
    ...
    def enterModuleSymbol(tree: ModuleDef): Symbol = {
      ...
      if (m.isModule && !m.hasPackageFlag && inCurrentScope(m) &&
    ...
    def applicableTypeParams(owner: Symbol): List[Symbol] =
      if (owner.isTerm || owner.isPackageClass)
        Nil
      else
        applicableTypeParams(owner.owner) ::: owner.typeParams
    ...
    /** Default implementation of `ensureCompanionObject`.
      ...
      */
    def standardEnsureCompanionObject(
        cdef: ClassDef,
        creator: ClassDef => Tree = companionModuleDef(_)): Symbol = {
      ...
      if (m != NoSymbol && currentRun.compiles(m))
        m
      else
        enterSyntheticSym(atPos(cdef.pos.focus)(creator(cdef)))
    }
    ...
    def enterPackage(tree: PackageDef) {
      ...
      newNamer(context.make(tree, sym.moduleClass, sym.info.decls)) enterSyms
        tree.stats
    }
    ...
    def enterClassDef(tree: ClassDef) {
      ...
      if (settings.warnPackageObjectClasses && owner.isPackageObjectClass &&
          !mods.isImplicit) {
        reporter.warning(
          tree.pos,
...
// --- Lazy Type Assignment --------------------------------------------------
    ...
    def accessorTypeCompleter(tree: ValDef, isSetter: Boolean) =
      mkTypeCompleter(tree) { sym =>
        logAndValidate(sym) {
          sym setInfo {
            val tp =
    ...
    def selfTypeCompleter(tree: Tree) =
      mkTypeCompleter(tree) { sym =>
        ...
        sym setInfo {
          if (selftpe.typeSymbol isNonBottomSubClass sym.owner)
            ...
            intersectionType(List(sym.owner.tpe, selftpe))
        }
      }
    ...
    private def widenIfNecessary(sym: Symbol, tpe: Type, pt: Type): Type = {
      ...
      val shouldWiden =
        (!tpe
          .typeSymbolDirect
          .isModuleClass // Infer Foo.type instead of "object Foo"
        &&
          (tpe.widen <:< pt) // Don't widen our way out of conforming to pt
          &&
          (sym.isVariable || sym.isMethod && !sym.hasAccessorFlag ||
            isHidden(tpe)))
      ...
      if (clazz.isCaseClass && !clazz.hasModuleFlag) {
        ...
        modClass.attachments.get[ClassForCaseCompanionAttachment] foreach {
          cma =>
            val cdef = cma.caseClass
            def hasCopy =
              (decls containsName nme.copy) ||
                parents.exists(_ member nme.copy exists)
    ...
    private def classSig(cdef: ClassDef): Type = {
      ...
      if (clazz.isDerivedValueClass) {
        log(
          "Ensuring companion for derived value class " + cdef.name + " at " +
            cdef.pos.show)
        clazz setFlag FINAL
    ...
    private def methodSig(ddef: DefDef): Type = {
      ...
      def thisMethodType(restpe: Type) = {
        if (vparamSymss.lengthCompare(0) >
              0) { // OPT fast path for methods of 0-1 parameter lists
        ...
        val res = GenPolyType(
          ...
          if (vparamSymss.isEmpty)
            NullaryMethodType(restpe)
          // vparamss refer (if they do) to skolemized tparams
          else
            (vparamSymss :\ restpe)(makeMethodType)
        )
      ...
      val resTpFromOverride =
        if (methOwner.isClass &&
            (tpt.isEmpty || mexists(vparamss)(_.tpt.isEmpty))) {
      ...
      val overridden = {
        ...
        if (isConstr || !methOwner.isClass)
          NoSymbol
        else
          overriddenSymbol(methResTp)
      }
      val hasDefaults = mexists(vparamss)(_.symbol.hasDefault) ||
        mexists(overridden.paramss)(_.hasDefault)
      if (hasDefaults)
      ...
      vparamss.foldLeft(Nil: List[List[ValDef]]) { (previous, vparams) =>
        ...
        map2(vparams, rvparams)((vparam, rvparam) => {
          ...
          if (sym.hasDefault) {
            ...
            val defVparamss =
              mmap(rvparamss.take(previous.length)) { rvp =>
                copyValDef(rvp)(
                  mods = rvp.mods &~ DEFAULTPARAM,
                  rhs = EmptyTree)
              }
            ...
            val parentNamer =
              if (isConstr) {
                val (cdef, nmr) = moduleNamer.getOrElse {
                  ...
                  module
                    .initialize // call type completer (typedTemplate), adds the
                ...
                defTparams = rtparams.map(rt =>
                  copyTypeDef(rt)(mods = rt.mods &~
                    (COVARIANT | CONTRAVARIANT)))
                nmr
              } else
                ownerNamer getOrElse {
                  val ctx = context
                    .nextEnclosing(c => c.scope.toList.contains(meth))
    ...
    def typeSig(tree: Tree): Type = {
      ...
      def annotate(annotated: Symbol) = {
        ...
        if (!annotated.isInitialized)
          tree match {
            case defn: MemberDef =>
              val ainfos = defn.mods.annotations filterNot
                (_ eq null) map { ann =>
    ...
    def includeParent(tpe: Type, parent: Symbol): Type =
      tpe match {
        case PolyType(tparams, restpe) =>
          PolyType(tparams, includeParent(restpe, parent))
        case ClassInfoType(parents, decls, clazz) =>
          if (parents exists (_.typeSymbol == parent))
            tpe
          else
            ClassInfoType(parents :+ parent.tpe, decls, clazz)
        case _ =>
          tpe
      }
    ...
    private val logDefinition =
      new LogTransitions[Symbol](
    ...
    def validate(sym: Symbol) {
      ...
      if (sym.info.typeSymbol == FunctionClass(0) && sym.isValueParameter &&
  ...
  def mkTypeCompleter(t: Tree)(c: Symbol => Unit) =
    new LockingTypeCompleter with FlagAgnosticCompleter {
      val tree = t
