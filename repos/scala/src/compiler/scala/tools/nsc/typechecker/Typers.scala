...
trait Typers
  ...
  final def forArgMode(fun: Tree, mode: Mode) =
    if (treeInfo.isSelfOrSuperConstrCall(fun))
      mode | SCCmode
    else
      mode
  ...
  abstract class Typer(context0: Context)
    ...
    def phasedAppliedType(sym: Symbol, args: List[Type]) = {
      ...
      if (phase.erasedTypes)
        erasure.specialScalaErasure(tp)
      else
        tp
    }
    ...
    def checkNonCyclic(pos: Position, tp: Type, lockedSym: Symbol): Boolean =
      try {
        if (!lockedSym.lock(CyclicReferenceError(pos, tp, lockedSym)))
          false
        else
    ...
    def checkNonCyclic(sym: Symbol) {
      if (!checkNonCyclic(sym.pos, sym.tpe_*))
    ...
    object checkNoEscaping extends TypeMap {
      ...
      def addHidden(sym: Symbol) =
        if (!(
              hiddenSymbols contains sym
            ))
          hiddenSymbols = sym :: hiddenSymbols

      override def apply(t: Type): Type = {
        def checkNoEscape(sym: Symbol) {
          if (sym.isPrivate && !sym.hasFlag(SYNTHETIC_PRIVATE)) {
            var o = owner
            while (o != NoSymbol && o != sym
                     .owner && o != sym.owner.linkedClassOfClass &&
    ...
    private def isStableContext(tree: Tree, mode: Mode, pt: Type) = {
      ...
      def expectsStable =
        (pt.isStable
          ...
          || !(
            tree.tpe <:< pt
          ) && (ptSym.isAbstractType && pt.bounds.lo.isStable || ptSym
            .isRefinementClass))
    ...
    protected def stabilize(
      ...
      def checkStable(tree: Tree): Tree =
        if (treeInfo.isStableIdentifierPattern(tree))
          tree
        else
          UnstableTreeError(tree)
    ...
    protected def adapt(
              ...
              (
                typer1 silent { tpr =>
                  ...
                  if (tpr.context.reporter.hasErrors)
                    ...
                    tpr.typed(withImplicitArgs, mode, pt)
                }
                  orElse { _ =>
      ...
      def adaptMismatchedSkolems() = {
        ...
        val boundOrSkolems =
          if (canIgnoreMismatch)
            bound ++ pt.skolemsExceptMethodTypeParams
          else
            Nil
          ...
          if (tree.tpe <:< AnyTpe)
            pt.dealias match {
              ...
              case TypeRef(_, sym, _)
                  if isNumericValueClass(sym) && isNumericSubType(
                    tree.tpe,
                    pt) =>
      ...
      def vanillaAdapt(tree: Tree) = {
        ...
        if (tree.isType)
          adaptType()
        else if (mode.typingExprNotFun && treeInfo.isMacroApplication(
                   tree) && !isMacroExpansionSuppressed(tree))
      ...
      if (isMacroImplRef(tree)) {
        ...
        tree.tpe match {
          ...
          case PolyType(tparams, restpe)
            ...
            val tree1 = (if (tree.isType)
                           tree
                         else
                           TypeApply(
                             tree,
                             tparams1 map (tparam =>
                               TypeTree(tparam.tpeHK) setPos tree
                                 .pos
                                 .focus)) setPos tree.pos)
          ...
          case mt: MethodType
              if mode
                ...
                .isMacroApplicationOrBlock(tree) =>
    ...
    def instantiatePossiblyExpectingUnit(
        ...
        mode: Mode,
        pt: Type): Tree = {
      if (mode.typingExprNotFun && pt
            .typeSymbol == UnitClass && !tree.tpe.isInstanceOf[MethodType])
    ...
    private def typedParentType(
      ...
      if (probe.isTrait || inMixinPosition) {
        ...
        val supertparams =
          if (supertpt.hasSymbolField)
            supertpt.symbol.typeParams
          else
            Nil
        ...
        val supertptWithTargs =
          if (supertparams.isEmpty || context.unit.isJava)
            supertpt
          else
            inferParentTypeArgs
        ...
        if (argssAreTrivial)
          ...
          supertptWithTargs updateAttachment SuperArgsAttachment(argss)
      }
    }
      ...
      treeInfo.firstConstructor(templ.body) match {
        case ctor @ DefDef(
          ...
          val cbody2 = { // called both during completion AND typing.
            ...
            clazz
              .unsafeTypeParams foreach (sym => typer1.context.scope.enter(sym))
      ...
      def explode0(parents: List[Tree]): List[Tree] = {
        ...
        if (supertpt.tpe.typeSymbol == AnyClass) {
          ...
          if (supertpt1.isErrorTyped)
            ...
            explode0(supersuper :: supertpt1 :: rest)
        } else
          parents
      }

      def explode(parents: List[Tree]) =
        if (treeInfo isTraitRef parents.head)
          explode0(parents)
        else
          parents
    ...
    def validateParentClasses(parents: List[Tree], selfType: Type) {
      ...
      def validateDynamicParent(parent: Symbol, parentPos: Position) =
        if (parent == DynamicClass)
          checkFeature(parentPos, DynamicsFeature)
      ...
      def validateParentClass(parent: Tree, superclazz: Symbol) =
        if (!parent.isErrorTyped) {
          ...
          if (!isPastTyper && psym.hasDeprecatedInheritanceAnnotation &&
              !sameSourceFile && !context
                .owner
                .ownerChain
                .exists(x => x.isDeprecated || x.hasBridgeAnnotation)) {
            val suffix = psym
              .deprecatedInheritanceMessage map (": " + _) getOrElse ""
            val msg =
          ...
          if (!(
            ...
            if (settings.explaintypes)
              explainTypes(selfType, parentTypeOfThis)
          }
      ...
      if (!parents.isEmpty && parents.forall(!_.isErrorTyped)) {
        ...
        for (p <- parents)
          validateParentClass(p, superclazz)
      }
    ...
    def typedClassDef(cdef: ClassDef): Tree = {
      ...
      if (clazz.isTrait && clazz.info.parents.nonEmpty && clazz
            .info
            .firstParent
            .typeSymbol == AnyClass)
    ...
    private def ensurePredefParentsAreInSameSourceFile(template: Template) = {
      val parentSyms = template
        .parents map (_.symbol) filterNot (_ == AnyRefClass)
    ...
    def typedTemplate(templ0: Template, parents1: List[Tree]): Template = {
      ...
      val selfType = (if (clazz.isAnonymousClass && !phase.erasedTypes)
    ...
    def typedValDef(vdef: ValDef): ValDef = {
      ...
      val valDefTyper = {
        val maybeConstrCtx =
          if ((sym.isParameter || sym
                .isEarlyInitialized) && sym.owner.isConstructor)
              ...
              val subst =
                new SubstTypeMap(tparams, tparams map (_ => WildcardType)) {
                  override def matches(sym: Symbol, sym1: Symbol) =
                    if (sym.isSkolem)
                      matches(sym.deSkolemize, sym1)
                    else if (sym1.isSkolem)
                      matches(sym, sym1.deSkolemize)
                    else
                      super[SubstTypeMap].matches(sym, sym1)
                }
      ...
      if (superConstr.symbol.isPrimaryConstructor && !superClazz
        ...
        for ((superAcc, superArg @ Ident(name)) <-
          ...
          if (mexists(vparamss)(_.symbol == superArg.symbol)) {
            ...
            if (alias
                  .exists && !alias.accessed.isVariable && !isRepeatedParamType(
                  alias.accessed.info)) {
              val ownAcc =
    ...
    def typedDefDef(ddef: DefDef): DefDef = {
      ...
      var rhs1 =
        if (ddef.name == nme.CONSTRUCTOR && !ddef
              .symbol
              .hasStaticFlag) { // need this to make it possible to generate static ctors
      ...
      if (meth.isClassConstructor && !isPastTyper && !meth
            .owner
      ...
      if (tpt1.tpe.typeSymbol != NothingClass && !context
            .returnsSeen && rhs1.tpe.typeSymbol != NothingClass)
        rhs1 = checkDead(rhs1)
      ...
      if (!isPastTyper) {
        ...
        for (p <- allParams) {
          for (n <- p.deprecatedParamName) {
            if (allParams.exists(p1 =>
                  p != p1 && (p1
                    .name == n || p1.deprecatedParamName.exists(_ == n))))
        ...
        if (meth.isImplicit && !meth.isSynthetic)
          meth.info.paramss match {
    ...
    def typedBlock(block0: Block, mode: Mode, pt: Type): Block = {
      ...
      try {
        ...
        treeCopy
          .Block(block, stats2, expr1)
          .setType(
            if (treeInfo.isExprSafeToInline(block))
              expr1.tpe
            else
              expr1.tpe.deconst)
      } finally {
    ...
    def typedCase(cdef: CaseDef, pattpe: Type, pt: Type): CaseDef = {
      ...
      val guard1: Tree =
        if (cdef.guard == EmptyTree)
          EmptyTree
        else
          typed(cdef.guard, BooleanTpe)
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
    ...
    def virtualizedMatch(match_ : Match, mode: Mode, pt: Type) = {
      ...
      val matchStrategy: Tree =
        if (!(
              settings.Xexperimental && context.isNameInScope(vpmName._match)
            ))
    ...
    def synthesizePartialFunction(
      ...
      val casesTrue = cases map (c =>
        deriveCaseDef(c)(x => atPos(x.pos.focus)(TRUE))
          .duplicate
      ...
      def applyOrElseMethodDef = {
        ...
        if (!paramSynthetic)
          methodBodyTyper.context.scope enter x
    ...
    def synthesizeSAMFunction(
      ...
      val funArgTps =
        foreach2(samMethTp.paramTypes, fun.vparams)((tp, vpar) =>
          vpar.tpt setType tp)
      ...
      val serializableParentAddendum =
        if (typeIsSubTypeOfSerializable(samClassTp))
          Nil
        else
          List(TypeTree(SerializableTpe))
      ...
      val block =
        nestedTyper.typedPos(sampos, mode, samClassTpFullyDefined) {
          Block(
            ...
            classDef,
            Apply(
              Select(New(Ident(tpnme.ANON_FUN_NAME)), nme.CONSTRUCTOR),
              Nil))
        }
    ...
    private def typedFunction(fun: Function, mode: Mode, pt: Type): Tree = {
       ...
       */
      val sam =
        if (pt.typeSymbol == FunctionSymbol)
          NoSymbol
        else
          samOf(pt)
    ...
    def typedStats(stats: List[Tree], exprOwner: Symbol): List[Tree] = {
      ...
      def includesTargetPos(tree: Tree) =
        tree.pos.isRange && context.unit.exists && (tree
          .pos includes context.unit.targetPos)
      val localTarget = stats exists includesTargetPos
      def typedStat(stat: Tree): Tree = {
        if (context.owner.isRefinementClass && !treeInfo
              .isDeclarationOrTypeDef(stat))
              ...
              if (localTarget && !includesTargetPos(stat)) {
                ...
                val localTyper =
                  if (inBlock || (stat.isDef && !stat.isInstanceOf[LabelDef])) {
                    ...
                    newTyper(context.make(stat, exprOwner))
                ...
                if (treeInfo.isSelfOrSuperConstrCall(result)) {
                  ...
                  if (treeInfo.isSelfConstrCall(result) && result
                        .symbol
                        .pos
                        .pointOrElse(0) >= exprOwner
                        .enclMethod
                        .pos
                        .pointOrElse(0))
        ...
        while ((e ne null) && e.owner == scope) {
          ...
          while ((e1 ne null) && e1.owner == scope) {
            if (!accesses(e.sym, e1.sym) && !accesses(e1.sym, e.sym) &&
              ...
              if (!e.sym.isErroneous && !e1.sym.isErroneous && !e
                    .sym
                    .hasDefault &&
                  !e.sym.hasAnnotation(BridgeClass) && !e1
                    .sym
          ...
          def matches(stat: Tree, synt: Tree) =
            (stat, synt) match {
              ...
              case (
                ...
                mods
                  .hasDefault && syntName.toString.startsWith(statName.toString)
          ...
          if (!sameLength(formals, args) || // wrong nb of arguments
            ...
            if (tparams.isEmpty) { // all type params are defined
              def handleMonomorphicCall: Tree = {
                 ...
                 */
                if (args.isEmpty && canTranslateEmptyListToNil && fun
                      .symbol
                      .isInitialized && ListModule
                      .hasCompleteInfo && (fun.symbol == List_apply))
              ...
              def handlePolymorphicCall = {
                ...
                val lenientTargs = protoTypeArgs(
                  ...
                  pt)
                val strictTargs =
                  map2(lenientTargs, tparams)((targ, tparam) =>
                ...
                def typedArgToPoly(
                  ...
                  val newmode =
                    if (isByNameParamType(remainingParams.head))
                      POLYmode
                    else
                      POLYmode | BYVALmode
    ...
    def typedAnnotation(ann: Tree, mode: Mode = EXPRmode): AnnotationInfo = {
      ...
      def tree2ConstArg(tree: Tree, pt: Type): Option[ClassfileAnnotArg] =
        tree match {
          ...
          case Apply(fun, args) =>
            ...
            if (typedFun.symbol.owner == ArrayModule
                  .moduleClass && typedFun.symbol.name == nme.apply)
              pt match {
      ...
      def trees2ConstArg(trees: List[Tree], pt: Type): Option[ArrayAnnotArg] = {
        ...
        if (args.exists(_.isEmpty))
          ...
          Some(ArrayAnnotArg(args.flatten.toArray))
      }
          ...
          if (argss.length > 1) {
            ...
            val nvPairs = args map {
              case arg @ AssignOrNamedArg(Ident(name), rhs) =>
                val sym =
                  if (isJava)
                    annScope.lookup(name)
                  else
                    findSymbol(typedFun.tpe.params)(_.name == name)
                if (sym == NoSymbol) {
    ...
    def packedType(tree: Tree, owner: Symbol): Type = {
      ...
      def containsDef(owner: Symbol, sym: Symbol): Boolean =
        (!sym.hasPackageFlag) && {
          var o = sym.owner
          while (o != owner && o != NoSymbol && !o.hasPackageFlag)
            o = o.owner
      ...
      def isLocal(sym: Symbol): Boolean =
        if (sym == NoSymbol || sym.isRefinementClass || sym.isLocalDummy)
          false
        else if (owner == NoSymbol)
              ...
              case TypeRef(pre, sym, args) =>
                if (sym.isAliasType && containsLocal(tp) && (tp.dealias ne tp))
                  ...
                  if (pre.isVolatile)
                    pre match {
                      case SingleType(_, sym)
                          if sym.isSynthetic && isPastTyper =>
                        debuglog(
      ...
      def isApplyDynamicNamed(fun: Tree): Boolean =
        fun match {
          case DynamicApplicationNamed(qual, _)
              if acceptsApplyDynamic(qual.tpe.widen) =>
            true
          case _ =>
            false
          // look deeper?
        ...
        acceptsApplyDynamicWithType(qual, name) map { tp =>
          ...
          def findSelection(t: Tree): Option[(TermName, Tree)] =
            t match {
              ...
              case _ =>
                (
                  t.children flatMap findSelection
                ).headOption
            }
    ...
    def typed1(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def lookupInOwner(owner: Symbol, name: Name): Symbol =
        if (mode.inQualMode)
          rootMirror.missingHook(owner, name)
        else
          NoSymbol
          ...
          def setInlineAttachment(
            ...
            t match {
              case TypeApply(fun, _) =>
                setInlineAttachment(fun, att)
              case _ =>
                t.updateAttachment(att)
            }
      ...
      def typedArrayValue(tree: ArrayValue) = {
        ...
        val elems1 = tree
          .elems mapConserve (elem => typed(elem, mode, elemtpt1.tpe))
        // see run/t6126 for an example where `pt` does not suffice (tagged types)
        val tpe1 =
          if (isFullyDefined(pt) && !phase.erasedTypes)
            pt
          else
            arrayType(elemtpt1.tpe)
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
        if (varsym.isVariable || varsym.isValue && phase.erasedTypes) {
          ...
          val t =
            atPos(lhs1.pos.withEnd(rhs1.pos.end)) {
          ...
          if (pt.typeSymbol == PartialFunctionClass)
            ...
            val arity =
              if (isFunctionType(pt))
                pt.dealiasWiden.typeArgs.length - 1
              else
                1
            val params =
            ...
            val body = treeCopy.Match(
              ...
              selector1,
              (
                cases map duplicateAndKeepPositions
      ...
      def typedNew(tree: New) = {
          ...
          if (checkStablePrefixClassType(tpt0))
            if (tpt0.hasSymbolField && !tpt0.symbol.typeParams.isEmpty) {
              ...
              TypeTree()
                .setOriginal(tpt0)
                .setType(
                  appliedType(tpt0.tpe, context.undetparams map (_.tpeHK))
            ...
            case ValDef(mods, _, _, Apply(Select(`tree`, _), _))
              ...
              val sym1 =
                if (sym.owner.isClass && sym.getterIn(sym.owner) != NoSymbol)
                  ...
                  sym.lazyAccessorOrSelf
              val pre =
                if (sym1.owner.isClass)
                  sym1.owner.thisType
                else
                  NoPrefix
        ...
        if (sym.isAbstractType || sym.hasAbstractFlag)
          ...
          setError(tpt)
                     ...
                     tp == sym.typeOfThis // when there's no explicit self type -- with (#3612) or without self variable
      ...
      def normalTypedApply(tree: Tree, fun: Tree, args: List[Tree]) = {
        ...
        silentResult match {
          case SilentResultValue(fun1) =>
            val fun2 =
              if (stableApplication)
                stabilizeFun(fun1, mode, pt)
              else
                fun1
            if (Statistics.canEnable)
      ...
      def typedSuper(tree: Super) = {
        ...
        treeCopy
          .Super(tree, qual1, mix) setType SuperType(clazz.thisType, owntype)
      }
      ...
      def typedThis(tree: This) =
        tree.symbol orElse qualifyingClass(
          ...
          case clazz =>
            ...
            if (isStableContext(tree, mode, pt))
      ...
      def typedSelectInternal(tree: Tree, qual: Tree, name: Name): Tree = {
        ...
        if (phase.erasedTypes && qual
              .isInstanceOf[Super] && tree.symbol != NoSymbol)
        ...
        if (!reallyExists(sym)) {
          ...
          val (result, accessibleError) =
            silent(_.makeAccessible(tree1, sym, qual.tpe, qual)) match {
        ...
        tree match {
          ...
          case Select(qual, name) =>
            ...
            val qualStableOrError =
              (if (qualTyped.isErrorTyped || !name
                     .isTypeName || treeInfo.admitsTypeSelection(qualTyped))
                 qualTyped
               else
      ...
      def typedIdent(tree: Tree, name: Name): Tree = {
        ...
        def inEmptyPackage =
          if (settings.exposeEmptyPackage)
            lookupInEmpty(name)
          else
            NoSymbol
        ...
        def issue(err: AbsTypeError) = {
          ...
          val suppress = reporter
            .hasErrors && (name startsWith tpnme.ANON_CLASS_NAME)
          if (!suppress)
        ...
        nameLookup match {
          ...
          case LookupSucceeded(qual, sym) =>
            ( // this -> Foo.this
              ...
              else if (isPredefClassOf(sym) && pt
                         .typeSymbol == ClassClass && pt.typeArgs.nonEmpty) {
      ...
      def typedCompoundTypeTree(tree: CompoundTypeTree) = {
        ...
        if ((parents1 ++ templ.body) exists (_.isErrorTyped))
          ...
          tree setType (if (templ.exists(_.isErroneous))
                          ErrorType
                        else
                          self) // Being conservative to avoid SI-5361
        }
      }
      ...
      def defDefTyper(ddef: DefDef) = {
        val isConstrDefaultGetter = ddef.mods.hasDefault && sym
          .owner
          .isModuleClass &&
        ...
        if (!isPastTyper) {
          ...
          def requiresNoArgs(tp: Type): Boolean =
            tp match {
              ...
              case _ =>
                true // catches all others including NullaryMethodType
            }
      ...
      def typedLiteral(tree: Literal) = {
        if (settings.warnMissingInterpolator)
          warnMissingInterpolator(tree)

        tree setType (if (tree.value.tag == UnitTag)
                        UnitTpe
                      else
                        ConstantType(tree.value))
      }
      ...
      def typedTypeBoundsTree(tree: TypeBoundsTree) = {
        val lo1 =
          if (tree.lo.isEmpty)
            ...
            typedType(tree.lo, mode)
        val hi1 =
          if (tree.hi.isEmpty)
            TypeTree(AnyTpe)
          else
            typedType(tree.hi, mode)
        treeCopy
          .TypeBoundsTree(tree, lo1, hi1) setType TypeBounds(lo1.tpe, hi1.tpe)
      }
    ...
    def typedPattern(tree: Tree, pt: Type): Tree = {
      ...
      typingInPattern(
        context
          .withImplicitsDisabledAllowEnrichment(typed(tree, PATTERNmode, pt)))
    }
    ...
    def typedHigherKindedType(tree: Tree, mode: Mode, pt: Type): Tree =
      if (pt != Kind.Wildcard && pt.typeParams.isEmpty)
        ...
        context withinTypeConstructorAllowed typed(tree, NOmode, pt)
...
object TypersStats {
  ...
  val byTypeNanos =
    Statistics.newByClass("time spent by tree node", "typer")(
