...
trait Typers
  ...
  abstract class Typer(context0: Context)
    ...
    def applyImplicitArgs(fun: Tree): Tree = fun.tpe match {
      case MethodType(params, _) =>
        ...
        for (param <- params) {
          ...
          val res =
            if (paramFailed ||
                (paramTp.isErroneous && { paramFailed = true; true }))
              SearchFailure
            else
              inferImplicit(
                fun,
    ...
    def viewExists(from: Type, to: Type): Boolean =
      (!from.isError &&
        !to.isError && context.implicitsEnabled &&
        (inferView(
          context.tree,
          from,
          to,
        ...
        def checkNoEscape(sym: Symbol) {
          if (sym.isPrivate && !sym.hasFlag(SYNTHETIC_PRIVATE)) {
            ...
            while (o != NoSymbol && o != sym.owner &&
                   o != sym.owner.linkedClassOfClass && !o.isLocalToBlock &&
                   !o.isPrivate &&
    ...
    private def isStableContext(tree: Tree, mode: Mode, pt: Type) = {
      ...
      def expectsStable =
        (pt.isStable ||
          mode.inQualMode && !tree.symbol.isConstant || !(tree.tpe <:< pt) &&
          (ptSym.isAbstractType && pt.bounds.lo.isStable ||
            ptSym.isRefinementClass))
    ...
    def stabilizeFun(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def stabilizable =
        (pre.isStable && sym.tpe.params.isEmpty &&
    ...
    def samToFunctionType(tp: Type, sam: Symbol = NoSymbol): Type = {
      ...
      if (samSym.exists &&
          samSym.owner != correspondingFunctionSymbol) // don't treat Functions as SAMs
    ...
    protected def adapt(
      ...
      def adaptToImplicitMethod(mt: MethodType): Tree = {
        if (hasUndets) {
          // (9) -- should revisit dropped condition `hasUndetsInMonoMode`
      ...
      def instantiateToMethodType(mt: MethodType): Tree = {
        ...
        if (!meth.isConstructor && (isFunctionType(pt) || shouldEtaExpandToSam)) {
          // (4.2)
      ...
      def adaptType(): Tree = {
        ...
        def properTypeRequired =
          (tree.hasSymbolField &&
            !context.inTypeConstructorAllowed && !context.unit.isJava)
        ...
        else if (kindArityMismatch &&
                 !kindArityMismatchOk) // (7.1) @M: check kind-arity
      ...
      def fallbackAfterVanillaAdapt(): Tree = {
        ...
        if (mode.typingExprNotFun) {
          ...
          if (tree.tpe <:< AnyTpe)
            pt.dealias match {
              ...
              case TypeRef(_, sym, _)
                  if isNumericValueClass(sym) &&
                    isNumericSubType(tree.tpe, pt) =>
          ...
          if (pt.dealias.annotations.nonEmpty &&
              canAdaptAnnotations(tree, this, mode, pt)) // (13)
          ...
          if (context.implicitsEnabled && !pt.isError && !tree.isErrorTyped) {
            ...
            inferView(tree, tree.tpe, pt, reportAmbiguous = true) match {
              ...
              case coercion =>
                def msg =
                  "inferred view from " + tree.tpe + " to " + pt + " = " +
                    coercion + ":" + coercion.tpe
      ...
      if (isMacroImplRef(tree)) {
        ...
        tree.tpe match {
          ...
          case ct @ ConstantType(value)
              if mode.inNone(TYPEmode | FUNmode) && (ct <:< pt) &&
          ...
          case PolyType(tparams, restpe)
              if mode.inNone(TAPPmode | PATTERNmode) &&
                !context.inTypeConstructorAllowed => // (3)
          ...
          case mt: MethodType
              if mode.typingExprNotFunNotLhs && !hasUndetsInMonoMode &&
                !treeInfo.isMacroApplicationOrBlock(tree) =>
    ...
    private def isAdaptableWithView(qual: Tree) = {
      ...
      (!isPastTyper && qual.isTerm && !qual.isInstanceOf[Super] &&
      ((qual.symbol eq null) || !qual.symbol.isTerm ||
      qual.symbol.isValue) && !qtpe.isError &&
      !qtpe.typeSymbol.isBottomClass && qtpe != WildcardType &&
        ...
        override def traverse(tree: Tree): Unit = if (isValueClass) {
          tree match {
            ...
            case cd: ClassDef if !cd.symbol.isAnonymousClass =>
              // Don't warn about partial functions, etc. SI-7571
            ...
            case Select(sup @ Super(qual, mix), selector)
                if selector != nme.CONSTRUCTOR && qual.symbol == clazz &&
                  mix != tpnme.EMPTY =>
    ...
    private def typedPrimaryConstrBody(templ: Template)(
      ...
      treeInfo.firstConstructor(templ.body) match {
        case ctor @ DefDef(
          ...
          val cbody2 = {
            // called both during completion AND typing.
    ...
    def typedParentTypes(templ: Template): List[Tree] = templ.parents match {
      ...
      case first :: rest =>
        try {
          val supertpts =
            fixDuplicateSyntheticParents(
              normalizeFirstParent(
                typedParentType(first, templ, inMixinPosition = false) +:
                  (rest map
                  (typedParentType(_, templ, inMixinPosition = true)))))
    ...
    def validateParentClasses(parents: List[Tree], selfType: Type) {
      ...
      def validateParentClass(parent: Tree, superclazz: Symbol) =
        if (!parent.isErrorTyped) {
          ...
          if (!(selfType <:< parentTypeOfThis) && !phase.erasedTypes &&
              !context.owner.isSynthetic &&
              // don't check synthetic concrete classes for virtuals (part of DEVIRTUALIZE)
    ...
    def typedClassDef(cdef: ClassDef): Tree = {
      ...
      if (clazz.isTrait && clazz.info.parents.nonEmpty &&
          clazz.info.firstParent.typeSymbol == AnyClass)
    ...
    def typedModuleDef(mdef: ModuleDef): Tree = {
      ...
      val noSerializable =
        ((linkedClass eq NoSymbol) || linkedClass.isErroneous ||
          !linkedClass.isSerializable || clazz.isSerializable)
    ...
    def typedValDef(vdef: ValDef): ValDef = {
      ...
      val valDefTyper = {
        val maybeConstrCtx =
          if ((sym.isParameter || sym.isEarlyInitialized) &&
    ...
    private def typedValDefImpl(vdef: ValDef) = {
      ...
      val rhs1 =
        if (vdef.rhs.isEmpty) {
          if (sym.isVariable && sym.owner.isTerm && !sym.isLazy &&
              !isPastTyper) LocalVarUninitializedError(vdef)
          vdef.rhs
        } else {
          val tpt2 =
            if (sym.hasDefault) {
    ...
    def computeParamAliases(
      ...
      if (superConstr.symbol.isPrimaryConstructor &&
          !superClazz.isJavaDefined &&
          sameLength(superParamAccessors, superArgs)) {
          ...
          if (mexists(vparamss)(_.symbol == superArg.symbol)) {
            val alias =
              (superAcc.initialize.alias orElse
                ...
                (alias => superClazz.info.nonPrivateMember(alias.name) == alias))
          ...
          def checkAbstract(tp0: Type, what: String): Boolean = {
            def check(sym: Symbol): Boolean = !sym.isAbstractType || {
              ...
              ((!sym.hasTransOwner(meth.owner) && failStruct(
                ...
                what)) ||
              (!sym.hasTransOwner(meth) &&
              failStruct(paramPos, "a type member of that refinement", what)) ||
    ...
    def typedDefDef(ddef: DefDef): DefDef = {
      ...
      var rhs1 =
        if (ddef.name == nme.CONSTRUCTOR && !ddef.symbol.hasStaticFlag) {
          // need this to make it possible to generate static ctors
          if (!meth.isPrimaryConstructor &&
              (!meth.owner.isClass || meth.owner.isModuleClass ||
      ...
      if (meth.isClassConstructor && !isPastTyper &&
          !meth.owner.isSubClass(AnyValClass)) {
    ...
    def typedLabelDef(ldef: LabelDef): LabelDef =
      if (!nme.isLoopHeaderLabel(ldef.symbol.name) || isPastTyper) {
        ...
        val restpe = rhs1.tpe
        if (restpe == initpe) {
          // stable result, no need to check again
          ldef.params foreach (param => param setType param.symbol.tpe)
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
    ...
    def synthesizePartialFunction(
      ...
      def applyOrElseMethodDef = {
        ...
        val B1 =
          methodSym newTypeParameter (newTypeName("B1")) setInfo TypeBounds.empty
        val default =
          methodSym newValueParameter
            (newTermName("default"), tree.pos.focus,
            SYNTHETIC) setInfo functionType(List(A1.tpe), B1.tpe)
        ...
        // case
        val match0 =
          methodBodyTyper.typedMatch(selector(x), cases, mode, resTp)
      ...
      def isDefinedAtMethod = {
        ...
        val methodBodyTyper = newTyper(context.makeNewScope(
          context.tree,
          methodSym)) // should use the DefDef for the context's tree, but it doesn't exist yet (we need the typer we're creating to create it)
              ...
              if (isFullyDefined(argpt)) argpt
                ...
                fun match {
                  case etaExpansion(vparams, fn, args) =>
                    silent(_.typed(fn, mode.forFunMode, pt)) filter
                      (_ => context.undetparams.isEmpty) map { fn1 =>
    ...
    def typedStats(stats: List[Tree], exprOwner: Symbol): List[Tree] = {
      ...
      def typedStat(stat: Tree): Tree =
        if (context.owner.isRefinementClass &&
            !treeInfo.isDeclarationOrTypeDef(stat)) OnlyDeclarationsError(stat)
        else
          stat match {
            case imp @ Import(_, _) =>
            ...
            case _ =>
              if (localTarget && !includesTargetPos(stat)) {
                ...
                if (!isPastTyper &&
                    treeInfo.isPureExprForWarningPurposes(result))
                  context.warning(
                    stat.pos,
      ...
      def accesses(looker: Symbol, accessed: Symbol) =
        accessed.isLocalToThis &&
          ((accessed.isParamAccessor) ||
            (looker.hasAccessorFlag && !accessed.hasAccessorFlag &&
              accessed.isPrivate))
          ...
          while ((e1 ne null) && e1.owner == scope) {
            if (!accesses(e.sym, e1.sym) && !accesses(e1.sym, e.sym) &&
              ...
              if (!e.sym.isErroneous && !e1.sym.isErroneous &&
                  !e.sym.hasDefault && !e.sym.hasAnnotation(BridgeClass) &&
                  !e1.sym.hasAnnotation(BridgeClass)) {
                log(
        ...
        if (newStats.isEmpty) stats
          ...
          def matches(stat: Tree, synt: Tree) = (stat, synt) match {
            ...
            case (
              ...
              mods.hasDefault &&
                syntName.toString.startsWith(statName.toString)
            ...
            (formal baseType FunctionClass(vparams.length) match {
              case TypeRef(_, _, formalargs) =>
                (exists2(formalargs, vparams)((formal, vparam) =>
                  vparam.tpt.isEmpty &&
                    (tparams exists formal.contains)) &&
      ...
      fun.tpe match {
            ...
            val (args1, argTpes) = context.savingUndeterminedTypeParams() {
              ...
              args.map {
                case arg @ AssignOrNamedArg(Ident(name), rhs) =>
                  ...
                  val argWithTypedRhs =
                    treeCopy.AssignOrNamedArg(arg, arg.lhs, typedRhs)
        ...
        case mt @ MethodType(params, _) =>
          ...
          def tryTupleApply: Tree =
            if (eligibleForTupleConversion(paramTypes, argslen) &&
                !phase.erasedTypes) {
              val tupleArgs = List(
          ...
          def tryNamesDefaults: Tree = {
            ...
            if (mt.isErroneous) duplErrTree
              ...
              if (namelessArgs exists (_.isErroneous)) {
                duplErrTree
              } else if (!allArgsArePositional(argPos) &&
                         !sameLength(formals, params))
              ...
              if (fun1.isErroneous) duplErrTree
                ...
                if (!sameLength(allArgs, args) &&
                    callToCompanionConstr(context, funSym)) {
                  duplErrorTree(
          ...
          if (!sameLength(formals, args) || // wrong nb of arguments
              ...
              isNamedApplyBlock(fun)) {
            // fun was transformed to a named apply block =>
            ...
            if (tparams.isEmpty) {
              ...
              def handleMonomorphicCall: Tree = {
                ...
                def noExpectedType =
                  !phase.erasedTypes && fun.symbol.isLabel &&
                    treeInfo.isSynthCaseSymbol(fun.symbol)
                 ...
                 */
                if (args.isEmpty && canTranslateEmptyListToNil &&
                    fun.symbol.isInitialized && ListModule.hasCompleteInfo &&
              ...
              def handlePolymorphicCall = {
                ...
                if (args1 exists { _.isErrorTyped }) duplErrTree
                  ...
                  debuglog(
                    "infer method inst " + fun + ", tparams = " + tparams +
                      ", args = " + args1.map(_.tpe) + ", pt = " + pt +
                      ", lobounds = " + tparams.map(_.tpe.bounds.lo) +
                      ", parambounds = " + tparams.map(_.info)) //debug
    ...
    def typedAnnotation(ann: Tree, mode: Mode = EXPRmode): AnnotationInfo = {
      ...
      val typedFunPart =
        ...
        if (targs.isEmpty &&
            typedFun0.exists(t => t.tpe != null && isDummyAppliedType(t.tpe)))
        ...
        if (typedFun.isErroneous || annType == null) ErroneousAnnotation
          ...
          if (argss.length > 1) {
            ...
            for (sym <- names) {
              ...
              sym.initialize
              if (!sym.hasAnnotation(AnnotationDefaultAttr) &&
                  !sym.hasDefault)
                reportAnnotationError(
          ...
          val typedAnn: Tree = {
            ...
            val localTyper =
              newTyper(context.make(ann, context.owner.newLocalDummy(ann.pos)))
    ...
    def isReferencedFrom(ctx: Context, sym: Symbol): Boolean =
      (ctx.owner.isTerm &&
        ...
        var ctx1 = ctx.outer
        while ((ctx1 != NoContext) && (ctx1.scope eq ctx.scope)) ctx1 = ctx1
          .outer
    ...
    def packedType(tree: Tree, owner: Symbol): Type = {
      def defines(tree: Tree, sym: Symbol) =
        (sym.isExistentialSkolem &&
          sym.unpackLocation == tree || tree.isDef && tree.symbol == sym)
      def isVisibleParameter(sym: Symbol) =
        (sym.isParameter &&
          (sym.owner == owner) && (sym.isType || !owner.isAnonymousFunction))
    ...
    protected def typedTypeApply(
      ...
      fun.tpe match {
        case OverloadedType(pre, alts) =>
          ...
          //
          val fun1 =
            adaptAfterOverloadResolution(fun, mode.forFunMode | TAPPmode)
        ...
        case PolyType(tparams, restpe) if tparams.nonEmpty =>
          if (sameLength(tparams, args)) {
            ...
            if (isPredefClassOf(fun.symbol))
              ...
              if (!isPastTyper && fun.symbol == Any_isInstanceOf &&
                  targs.nonEmpty) {
    ...
    object dyna {
      ...
      def mkInvoke(
        ...
        acceptsApplyDynamicWithType(qual, name) map { tp =>
          ...
          def matches(t: Tree) =
            isDesugaredApply ||
              treeInfo.dissectApplied(t).core == treeSelection
      ...
      def typedAnnotated(atd: Annotated): Tree = {
        ...
        if (arg1.isType) {
          ...
          def setInlineAttachment(
              t: Tree,
              att: InlineAnnotatedAttachment): Unit = t match {
      ...
      def typedBind(tree: Bind) = {
        ...
        name match {
          case name: TypeName =>
            assert(
              body == EmptyTree,
              context.unit + " typedBind: " + name.debugString + " " +
                body + " " + body.getClass)
            val sym =
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def typedIf(tree: If): If = {
        ...
        def samePackedTypes =
          (!isPastTyper &&
            thenp1.tpe.annotations.isEmpty && elsep1.tpe.annotations.isEmpty &&
              ...
              if (typed(expr).tpe.typeSymbol != UnitClass)
                context.warning(
                  ...
                  "enclosing method " + name +
                    " has result type Unit: return value discarded")
            }
            val res = treeCopy
          ...
          if (checkStablePrefixClassType(tpt0))
            if (tpt0.hasSymbolField && !tpt0.symbol.typeParams.isEmpty) {
              ...
              TypeTree()
                ...
                .setType(appliedType(
                  tpt0.tpe,
                  context.undetparams map (_.tpeHK))) // @PP: tpeHK! #3343, #4018, #4347.
            } else tpt0
          else tpt0
        }
      ...
      def tryTypedApply(fun: Tree, args: List[Tree]): Tree = {
        ...
        def onError(
          ...
          val retry =
            (typeErrors.forall(_.errPos != null)) &&
              (fun :: tree :: args exists errorInResult)
          typingStack.printTyping({
            val funStr =
              ptTree(fun) + " and " + (args map ptTree mkString ", ")
          ...
          if (retry) {
            ...
            tryTypedArgs(args, forArgMode(fun, mode)) match {
              case Some(args1)
                ...
                if (qual1 ne qual) {
                  val tree1 =
                    Apply(Select(qual1, name) setPos fun.pos, args1) setPos tree.pos
      ...
      def normalTypedApply(tree: Tree, fun: Tree, args: List[Tree]) = {
        ...
        def onError(reportError: => Tree): Tree = fun match {
          case Select(qual, name)
              if !mode.inPatternMode &&
                nme.isOpAssignmentName(newTermName(name.decode)) =>
        ...
        silentResult match {
          case SilentResultValue(fun1) =>
            ...
            val noSecondTry =
              (isPastTyper || context.inSecondTry || (fun2.symbol ne null) &&
                fun2.symbol.isConstructor || isImplicitMethodType(fun2.tpe))
      ...
      def typedSuper(tree: Super) = {
        ...
        def findMixinSuper(site: Type): Type = {
          ...
          if (ps.isEmpty) {
            debuglog(
              "Fatal: couldn't find site " + site + " in " +
                site.parents.map(_.typeSymbol.name))
      ...
      def typedSelectInternal(tree: Tree, qual: Tree, name: Name): Tree = {
        ...
        val sym =
          tree.symbol orElse member(qual, name) orElse {
            // symbol not found? --> try to convert implicitly to a type that does have the required
            // member.  Added `| PATTERNmode` to allow enrichment in patterns (so we can add e.g., an
            // xml member to StringContext, which in turn has an unapply[Seq] method)
            if (name != nme.CONSTRUCTOR &&
                mode.inAny(EXPRmode | PATTERNmode)) {
        ...
        if (phase.erasedTypes && qual.isInstanceOf[Super] &&
            tree.symbol != NoSymbol) qual setType tree.symbol.owner.tpe
        ...
        if (!reallyExists(sym)) {
          ...
          result match {
            ...
            case SelectFromTypeTree(qual @ TypeTree(), name)
                if qual.tpe.typeArgs.nonEmpty =>
              // TODO: somehow the new qual is not checked in refchecks
      ...
      def qualifies(sym: Symbol) =
        (sym.hasRawInfo && reallyExists(sym) &&
      ...
      def typedIdent(tree: Tree, name: Name): Tree = {
        ...
        nameLookup match {
          ...
          case LookupSucceeded(qual, sym) =>
            ...
            else if (isPredefClassOf(sym) && pt.typeSymbol == ClassClass &&
                     pt.typeArgs.nonEmpty) {
        ...
        if (treeInfo isWildcardStarType tree.tpt)
          ...
          tree match {
            ...
            case Typed(expr, tpt) =>
              ...
              val expr1 =
                typed(expr, mode.onlySticky, tpt1.tpe.deconst) // then type the expression with tpt1 as the expected type
      ...
      def warnMissingInterpolator(lit: Literal): Unit = if (!isPastTyper) {
        ...
        def maybeWarn(s: String): Unit = {
          ...
          def suspiciousIdents =
            InterpolatorIdentRegex findAllIn s map
              (s => suspiciousSym(TermName(s drop 1)))
      ...
      def typedOutsidePatternMode(tree: Tree): Tree = tree match {
        ...
        case tree: AssignOrNamedArg =>
          typedAssign(tree.lhs, tree.rhs) // called by NamesDefaults in silent typecheck
    ...
    private def typedInternal(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def retypingOk =
        (context.retyping && (tree.tpe ne null) &&
      ...
      def runTyper(): Tree = {
        ...
        val tree1: Tree =
          if (alreadyTyped) tree else typed1(tree, mode, ptWild)
    ...
    def computeMacroDefType(ddef: DefDef, pt: Type): Type = {
      ...
      val isMacroBodyOkay =
        !ddef.symbol.isErroneous && !(rhs1 exists (_.isErroneous)) &&
          rhs1 != EmptyTree
...
object TypersStats {
  ...
  val rawTypeFailed =
    Statistics.newSubCounter("  of which in failed", rawTypeCount)
  val subtypeFailed =
    Statistics.newSubCounter("  of which in failed", subtypeCount)
  val findMemberFailed =
    Statistics.newSubCounter("  of which in failed", findMemberCount)
  val failedSilentNanos =
    Statistics.newSubTimer("time spent in failed", typerNanos)
  ...
  val isReferencedNanos =
    Statistics.newSubTimer("time spent ref scanning", typerNanos)
