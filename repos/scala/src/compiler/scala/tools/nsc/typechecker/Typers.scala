...
trait Typers
  ...
  abstract class Typer(context0: Context)
    ...
    private def isNarrowable(tpe: Type): Boolean = unwrapWrapperTypes(
      tpe) match {
    ...
    private def checkEphemeral(clazz: Symbol, body: List[Tree]) = {
      ...
      def where = if (isValueClass) "value class"
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def tryTypedApply(fun: Tree, args: List[Tree]): Tree = {
        ...
        def onError(
          ...
          def errorInResult(tree: Tree) = treesInResult(tree) exists (err =>
      ...
      def tryWithFilterAndFilter(tree: Select, qual: Tree): Tree = {
        def warn(sym: Symbol) = context.deprecationWarning(
          tree.pos,
          sym,
      ...
      def warnMissingInterpolator(lit: Literal): Unit = if (!isPastTyper) {
        ...
        def maybeWarn(s: String): Unit = {
          ...
          def suspiciousIdents = InterpolatorIdentRegex findAllIn s map (s =>
