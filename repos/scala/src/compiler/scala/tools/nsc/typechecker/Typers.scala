...
trait Typers
  ...
  abstract class Typer(context0: Context)
    ...
    def checkNonCyclic(pos: Position, tp: Type, lockedSym: Symbol): Boolean =
      try if (!lockedSym.lock(CyclicReferenceError(pos, tp, lockedSym))) false
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
    ...
    def synthesizePartialFunction(
      ...
      def applyOrElseMethodDef = {
        ...
        val default =
          methodSym newValueParameter (newTermName("default"), tree.pos.focus, SYNTHETIC) setInfo functionType(
            List(A1.tpe),
            B1.tpe)
