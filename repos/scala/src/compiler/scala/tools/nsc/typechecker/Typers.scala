...
trait Typers
  ...
  private final val InterpolatorIdentRegex =
    """\$[$\w]+""".r // note that \w doesn't include $
  ...
  abstract class Typer(context0: Context)
      ...
      fun.tpe match {
        case MethodType(params, _) =>
          ...
          for (param <- params) {
            ...
            if (res.isSuccess) { argBuff += mkArg(param.name, res.tree) }
            else {
              mkArg =
                gen
                  .mkNamedArg // don't pass the default argument (if any) here, but start emitting named arguments for the following args
              if (!param.hasDefault && !paramFailed) {
                context.reporter
                  .reportFirstDivergentError(fun, param, paramTp)(context)
                paramFailed = true
              }
              /* else {
        ...
        from match {
          ...
          case _ =>
            def wrapImplicit(from: Type): Tree = {
              val result = inferImplicit(
                tree,
                functionType(from.withoutAnnotations :: Nil, to),
                reportAmbiguous,
                isView = true,
                context,
                saveAmbiguousDivergent = saveErrors)
    ...
    def dropExistential(tp: Type): Type =
      tp match {
        case ExistentialType(tparams, tpe) => new SubstWildcardMap(tparams)
            .apply(tp)
        case TypeRef(_, sym, _) if sym.isAliasType =>
          val tp0 = tp.dealias
          if (tp eq tp0) {
            devWarning(
              s"dropExistential did not progress dealiasing $tp, see SI-7126")
            tp
          } else {
    ...
    object checkNoEscaping extends TypeMap {
      ...
      private def check[T <: Tree](
        ...
        else if (hiddenSymbols exists (_.isErroneous))
          HiddenSymbolWithError(tree)
      ...
      override def apply(t: Type): Type = {
        def checkNoEscape(sym: Symbol) {
          if (sym.isPrivate && !sym.hasFlag(SYNTHETIC_PRIVATE)) {
            var o = owner
            while (o != NoSymbol && o != sym.owner && o != sym.owner
                     .linkedClassOfClass &&
    ...
    def reenterValueParams(vparamss: List[List[ValDef]]) {
      for (vparams <- vparamss)
        for (vparam <- vparams) context.scope enter vparam.symbol
    }
    ...
    def qualifyingClass(tree: Tree, qual: Name, packageOK: Boolean) =
      context.enclClass.owner.ownerChain
        .find(o => qual.isEmpty || o.isClass && o.name == qual) match {
    ...
    final def typerWithCondLocalContext[T](c: => Context)(cond: Boolean)(
        f: Typer => T): T = if (cond) typerWithLocalContext(c)(f) else f(this)
    ...
    private def isStableContext(tree: Tree, mode: Mode, pt: Type) = {
      ...
      def expectsStable =
        (pt.isStable
          ...
          || !(tree.tpe <:< pt) && (ptSym.isAbstractType && pt.bounds.lo
            .isStable || ptSym.isRefinementClass))
              ...
              case Ident(_) =>
                val packageObject =
                  if (!sym.isOverloaded && sym.owner.isModuleClass)
                    sym.owner
                      .sourceModule // historical optimization, perhaps no longer needed
    ...
    def stabilizeFun(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      tree.tpe match {
        case MethodType(_, _) if stabilizable =>
          tree setType MethodType(
            Nil,
            singleType(pre, sym)
          ) // TODO: should this be a NullaryMethodType?
        case _ => tree
      }
    }
    ...
    def silent[T](
      ...
      val rawTypeStart =
        if (Statistics.canEnable) Statistics.startCounter(rawTypeFailed)
        else null
      val findMemberStart =
        if (Statistics.canEnable) Statistics.startCounter(findMemberFailed)
        else null
      val subtypeStart =
        if (Statistics.canEnable) Statistics.startCounter(subtypeFailed)
        else null
      val failedSilentStart =
        if (Statistics.canEnable) Statistics.startTimer(failedSilentNanos)
        else null
      def stopStats() = {
      ...
      try {
        if (context.reportErrors ||
          ...
          assert(
            context.bufferErrors || isPastTyper,
    ...
    def checkFeature(
      ...
      if (isPastTyper) true
        ...
        val nestedOwners = featureTrait.owner.ownerChain
          .takeWhile(_ != languageFeatureModule.moduleClass).reverse
        val featureName = (nestedOwners map (_.name + "."))
          .mkString + featureTrait.name
        def action(): Boolean = {
          def hasImport =
            inferImplicit(
              EmptyTree: Tree,
          ...
          if (!OK) {
            val Some(
              AnnotationInfo(
                _,
                List(
                  Literal(Constant(featureDesc: String)),
                  Literal(Constant(required: Boolean))),
                _)) =
              featureTrait getAnnotation LanguageFeatureAnnot
            context.featureWarning(
              pos,
              featureName,
              featureDesc,
              featureTrait,
    ...
    def samToFunctionType(tp: Type, sam: Symbol = NoSymbol): Type = {
      ...
      if (samSym.exists && samSym
            .owner != correspondingFunctionSymbol) // don't treat Functions as SAMs
    ...
    protected def adapt(
      ...
      def instantiateToMethodType(mt: MethodType): Tree = {
        ...
        if (!meth.isConstructor && (isFunctionType(
              pt) || shouldEtaExpandToSam)) { // (4.2)
        ...
        if (mode.typingExprNotFun) {
          ...
          if (tree.tpe <:< AnyTpe) pt.dealias match {
            ...
            case TypeRef(_, sym, _)
                if isNumericValueClass(sym) && isNumericSubType(tree.tpe, pt) =>
          ...
          if (pt.dealias.annotations.nonEmpty && canAdaptAnnotations(
                tree,
                this,
                mode,
                pt
          ...
          if (context.implicitsEnabled && !pt.isError && !tree.isErrorTyped) {
            ...
            inferView(tree, tree.tpe, pt, reportAmbiguous = true) match {
              ...
              case coercion =>
                def msg =
                  "inferred view from " + tree
                    .tpe + " to " + pt + " = " + coercion + ":" + coercion.tpe
                if (settings.logImplicitConv) context.echo(tree.pos, msg)

                debuglog(msg)
                val silentContext = context
                  .makeImplicit(context.ambiguousErrors)
                val res = newTyper(silentContext).typed(
                  new ApplyImplicitView(coercion, List(tree)) setPos tree.pos,
                  mode,
                  pt)
      ...
      def vanillaAdapt(tree: Tree) = {
        ...
        def shouldInsertApply(tree: Tree) =
          mode.typingExprFun && {
            tree.tpe match {
      ...
      if (isMacroImplRef(tree)) {
        ...
        tree.tpe match {
          ...
          case ct @ ConstantType(value)
              if mode.inNone(TYPEmode | FUNmode) && (
                ct <:< pt
              ) && canAdaptConstantTypeToLiteral => // (0)
            adaptConstant(value)
          ...
          case PolyType(tparams, restpe)
              if mode.inNone(TAPPmode | PATTERNmode) && !context
                .inTypeConstructorAllowed => // (3)
            ...
            val tree1 =
              (if (tree.isType) tree
                 ...
                 TypeApply(
                   tree,
                   tparams1 map (tparam =>
                     TypeTree(tparam.tpeHK) setPos tree.pos.focus)) setPos tree
                   .pos)
            context.undetparams ++= tparams1
            notifyUndetparamsAdded(tparams1)
            adapt(
              tree1 setType restpe.substSym(tparams, tparams1),
              mode,
              pt,
              original)
          ...
          case mt: MethodType
              if mode
                .typingExprNotFunNotLhs && !hasUndetsInMonoMode && !treeInfo
                .isMacroApplicationOrBlock(tree) => instantiateToMethodType(mt)
          case _                                 => vanillaAdapt(tree)
        }
    }
    ...
    def adaptAfterOverloadResolution(
        tree: Tree,
        mode: Mode,
        pt: Type = WildcardType,
        original: Tree = EmptyTree): Tree = { adapt(tree, mode, pt, original) }
    ...
    def instantiateExpectingUnit(tree: Tree, mode: Mode): Tree = {
      ...
      silent(_.instantiate(tree, mode, UnitTpe)) orElse { _ =>
        ...
        val valueDiscard = atPos(tree.pos)(Block(
    ...
    def instantiatePossiblyExpectingUnit(
      ...
      if (mode.typingExprNotFun && pt.typeSymbol == UnitClass && !tree.tpe
            .isInstanceOf[MethodType]) instantiateExpectingUnit(tree, mode)
      else instantiate(tree, mode, pt)
    }
    ...
    def adaptToArguments(
      ...
      def doAdapt(restpe: Type) =
        ...
        adaptToMember(
          qual,
          HasMethodMatching(name, args map (_.tpe), restpe),
          reportAmbiguous,
          saveErrors)
        ...
        override def traverse(tree: Tree): Unit =
          if (isValueClass) {
            tree match {
              ...
              case cd: ClassDef
                  if !cd.symbol
                    .isAnonymousClass => // Don't warn about partial functions, etc. SI-7571
                implRestriction(
                  tree,
                  "nested class"
                ) // avoiding Type Tests that might check the $outer pointer.
              case Select(sup @ Super(qual, mix), selector)
                  if selector != nme.CONSTRUCTOR && qual
                    .symbol == clazz && mix != tpnme.EMPTY =>
      ...
      for (stat <- body) {
        ...
        stat match {
          ...
          case DefDef(_, name, _, _, _, rhs) =>
            ...
            else if (isValueClass && (name == nme.equals_ || name == nme
                       .hashCode_) && !stat.symbol.isSynthetic)
              notAllowed(
                s"redefinition of $name method. See SIP-15, criterion 4.")
            else if (stat.symbol != null && stat.symbol.isParamAccessor)
              notAllowed("additional parameter")
            checkEphemeralDeep.traverse(rhs)
          case _: ValDef    => notAllowed("field definition")
          case _: ModuleDef =>
            //see https://issues.scala-lang.org/browse/SI-6359
            implRestriction(stat, "nested object")
          case _ => notAllowed("this statement")
        }
      }
    }
      ...
      if (!clazz.isPrimitiveValueClass) {
        clazz.primaryConstructor.paramss match {
          case List(List(param)) =>
            ...
            val accessor = decls.toList
              .find(x => x.isMethod && x.accessedOrSelf == paramAccessor)
            accessor match {
              case None =>
                context.error(
                  paramAccessor.pos,
              ...
              case Some(acc) =>
                ...
                checkEphemeral(
                  clazz,
                  body filterNot (stat =>
                    stat.symbol != null && stat.symbol
                      .accessedOrSelf == paramAccessor))
            }
          case _ =>
            context.error(
              clazz.pos,
      ...
      if (probe.isTrait || inMixinPosition) {
        ...
        def inferParentTypeArgs: Tree = {
          typedPrimaryConstrBody(templ) {
            ...
            case tpt =>
              TypeTree(tpt.tpe) setPos supertpt
                .pos // SI-7224: don't .focus positions of the TypeTree of a parent that exists in source
          }
        }
      ...
      treeInfo.firstConstructor(templ.body) match {
        case ctor @ DefDef(
          ...
          val (preSuperStats, superCall) = {
            val (stats, rest) =
              cstats span (x => !treeInfo.isSuperConstrCall(x))
            (
    ...
    private def normalizeFirstParent(parents: List[Tree]): List[Tree] = {
      ...
      def explode0(parents: List[Tree]): List[Tree] = {
        val supertpt :: rest =
          parents // parents is always non-empty here - it only grows
      ...
      def explode(parents: List[Tree]) =
        if (treeInfo isTraitRef parents.head) explode0(parents) else parents
        ...
        case first :: rest =>
          try {
            ...
            supertpts mapConserve (tpt =>
              checkNoEscaping.privates(context.owner, tpt))
          } catch {
            case ex: TypeError =>
      ...
      def validateParentClass(parent: Tree, superclazz: Symbol) =
        if (!parent.isErrorTyped) {
          ...
          if (psym != superclazz) {
            if (psym.isTrait) {
              ...
              if (!ps.isEmpty && !superclazz.isSubClass(ps.head.typeSymbol))
                pending += ParentSuperSubclassError(
                  parent,
                  superclazz,
                  ps.head.typeSymbol,
                  psym)
          ...
          if (!isPastTyper && psym.hasDeprecatedInheritanceAnnotation &&
              !sameSourceFile && !context.owner.ownerChain
                .exists(x => x.isDeprecated || x.hasBridgeAnnotation)) {
            val suffix =
              psym.deprecatedInheritanceMessage map (": " + _) getOrElse ""
            val msg =
              s"inheritance from ${psym.fullLocationString} is deprecated$suffix"
            context.deprecationWarning(parent.pos, psym, msg)
          }

          if (psym.isSealed && !phase.erasedTypes)
            if (sameSourceFile) psym addChild context.owner
            else pending += ParentSealedInheritanceError(parent, psym)
          ...
          if (!(selfType <:< parentTypeOfThis) &&
              !phase.erasedTypes &&
              !context.owner
                .isSynthetic && // don't check synthetic concrete classes for virtuals (part of DEVIRTUALIZE)
    ...
    def typedClassDef(cdef: ClassDef): Tree = {
      ...
      if (clazz.isTrait && clazz.info.parents.nonEmpty && clazz.info.firstParent
            .typeSymbol == AnyClass) checkEphemeral(clazz, impl2.body)
      ...
      if ((clazz isNonBottomSubClass ClassfileAnnotationClass) && (
        ...
        else if (clazz != SerialVersionUIDAttr)
          restrictionWarning(
            cdef.pos,
            unit,
    ...
    def typedModuleDef(mdef: ModuleDef): Tree = {
      ...
      val noSerializable = ((linkedClass eq NoSymbol)
    ...
    private def ensurePredefParentsAreInSameSourceFile(template: Template) = {
      val parentSyms =
        template.parents map (_.symbol) filterNot (_ == AnyRefClass)
      if (parentSyms exists (_.associatedFile != PredefModule.associatedFile))
        context.error(
          template.pos,
          s"All parents of Predef must be defined in ${PredefModule.associatedFile}.")
    }
    ...
    /** In order to override this in the TreeCheckers Typer so synthetics aren't re-added
      ...
      */
    protected def finishMethodSynthesis(
        templ: Template,
        clazz: Symbol,
    ...
    protected def enterSym(txt: Context, tree: Tree): Context =
      if (txt eq context) namer enterSym tree else newNamer(txt) enterSym tree
    ...
    def typedTemplate(templ0: Template, parents1: List[Tree]): Template = {
      ...
      if (!phase.erasedTypes && !clazz.info.resultType
            .isError) // @S: prevent crash for duplicated type members
      ...
      val body2 = {
        val body2 =
          if (isPastTyper || reporter.hasErrors) body1
            ...
            body1 flatMap rewrappingWrapperTrees(
              namer.addDerivedTrees(Typer.this, _))
        ...
        val primaryCtor1 = primaryCtor match {
          case DefDef(
            ...
            val pos =
              wrappingPos(parents1.head.pos, primaryCtor :: argss.flatten)
                .makeTransparent
    ...
    def typedValDef(vdef: ValDef): ValDef = {
      ...
      val valDefTyper = {
        val maybeConstrCtx =
          if ((sym.isParameter || sym.isEarlyInitialized) && sym.owner
                .isConstructor) context.makeConstructorContext
          else context
    ...
    private def typedValDefImpl(vdef: ValDef) = {
      ...
      treeCopy
        .ValDef(
          ...
          vdef.name,
          tpt1,
          checkDead(rhs1)) setType NoType
    }

    /** Enter all aliases of local parameter accessors.
      */
    def computeParamAliases(
        clazz: Symbol,
      ...
      def decompose(call: Tree): (Tree, List[Tree]) =
        call match {
          ...
          case Apply(fn, args) =>
            ...
            val applyArgs =
              if (args.length < params.length) args :+ EmptyTree
          ...
          case Block(_ :+ superCall, _) => decompose(superCall)
          case _                        => (call, Nil)
        }
      ...
      if (superConstr.symbol.isPrimaryConstructor && !superClazz
        ...
        for ((superAcc, superArg @ Ident(name)) <-
          ...
          if (mexists(vparamss)(_.symbol == superArg.symbol)) {
            ...
            if (alias.exists && !alias.accessed
                  .isVariable && !isRepeatedParamType(alias.accessed.info)) {
              val ownAcc =
    ...
    def checkMethodStructuralCompatible(ddef: DefDef): Unit = {
       ...
       */
      def paramssTypes(tp: Type): List[List[Type]] =
        tp match {
        ...
        foreachWithIndex(paramList) { (paramType, paramIdx) =>
          ...
          def checkAbstract(tp0: Type, what: String): Boolean = {
            def check(sym: Symbol): Boolean =
              !sym.isAbstractType || {
                log(s"""checking $tp0 in refinement$parentString at ${meth.owner
                  .owner.fullLocationString}""")
    ...
    def typedDefDef(ddef: DefDef): DefDef = {
      ...
      if (!isPastTyper && meth.isPrimaryConstructor) {
        for (vparams <- ddef.vparamss; vd <- vparams) {
          if (vd.mods.isParamAccessor) { namer.validateParam(vd) }
        }
      }
      ...
      var rhs1 =
        ...
        } else { transformedOrTyped(ddef.rhs, EXPRmode, tpt1.tpe) }

      if (meth.isClassConstructor && !isPastTyper && !meth.owner
        ...
        else checkSelfConstructorArgs(ddef, meth.owner)
      }

      if (tpt1.tpe.typeSymbol != NothingClass && !context.returnsSeen && rhs1
            .tpe.typeSymbol != NothingClass) rhs1 = checkDead(rhs1)
        ...
        for (p <- allParams) {
          for (n <- p.deprecatedParamName) {
            if (allParams.exists(p1 =>
                  p != p1 && (p1.name == n || p1.deprecatedParamName
                    .exists(_ == n)))) DeprecatedParamNameError(p, n)
          }
        }
    ...
    def typedTypeDef(tdef: TypeDef): TypeDef =
      typerWithCondLocalContext(context.makeNewScope(tdef, tdef.symbol))(
        tdef.tparams.nonEmpty) { _.typedTypeDefImpl(tdef) }
    ...
    def typedBlock(block0: Block, mode: Mode, pt: Type): Block = {
      ...
      try {
        ...
        if (phaseId(currentPeriod) <= currentRun.typerPhase.id) {
          ...
          block match {
            case Block(
              ...
              def matchesVisibleMember(member: Symbol) =
                visibleMembers exists { vis =>
                  (member.name == vis.name) &&
                  (member.tpe <:< vis.tpe.substThis(vis.owner, classDef.symbol))
                }
              // The block is an anonymous class definitions/instantiation pair
              //   -> members that are hidden by the type of the block are made private
              val toHide = (classDecls filter (member =>
                member.isTerm
                  && member.isPossibleInRefinement
                  && member.isPublic
                  && !matchesVisibleMember(member)) map (member =>
                member
        ...
        val stats1 =
          if (isPastTyper) block.stats
          else
            block.stats.flatMap(stat =>
              stat match {
        ...
        treeCopy.Block(block, stats2, expr1).setType(
    ...
    def typedCase(cdef: CaseDef, pattpe: Type, pt: Type): CaseDef = {
      ...
      for (bind @ Bind(name, _) <- cdef.pat) {
        ...
        if (name.toTermName != nme.WILDCARD && sym != null) {
          ...
          } else namer.enterIfNotThere(sym)
        }
      }
...
//    body1 = checkNoEscaping.locals(context.scope, pt, body1)
    ...
    def typedMatch(
      ...
      if (isFullyDefined(pt)) finish(casesTyped, pt)
      else
    ...
    def virtualizedMatch(match_ : Match, mode: Mode, pt: Type) = {
      ...
      val matchStrategy: Tree =
        if (!(settings.Xexperimental && context.isNameInScope(vpmName._match)))
          null // fast path, avoiding the next line if there's no __match to be seen
        else
          newTyper(context.makeImplicit(reportAmbiguousErrors = false)).silent(
            _.typed(Ident(vpmName._match)),
            reportAmbiguousErrors = false) orElse (_ => null)

      if (matchStrategy ne null) // virtualize
        typed(
          (new PureMatchTranslator(
            this.asInstanceOf[patmat.global.analyzer.Typer] /*TODO*/,
            matchStrategy)).translateMatch(match_),
          mode,
          pt)
      else match_ // will be translated in phase `patmat`
    }
    ...
    def synthesizePartialFunction(
      ...
      val anonClass = context.owner newAnonymousFunctionClass tree
        .pos addAnnotation SerialVersionUIDAnnotation
      ...
      def applyOrElseMethodDef = {
        val methodSym = anonClass
          .newMethod(nme.applyOrElse, tree.pos, FINAL | OVERRIDE)

        // create the parameter that corresponds to the function's parameter
        val A1 =
          methodSym newTypeParameter (newTypeName("A1")) setInfo TypeBounds
            .upper(argTp)
        val x = mkParam(methodSym, A1.tpe)

        // applyOrElse's default parameter:
        val B1 =
          methodSym newTypeParameter (newTypeName("B1")) setInfo TypeBounds
            .empty
        val default = methodSym newValueParameter (
          newTermName("default"), tree.pos.focus, SYNTHETIC
        ...
        val match_ = {
          val cdef = mkDefaultCase(
            methodBodyTyper.typed1(REF(default) APPLY (REF(x)), mode, B1.tpe)
              .setType(B1.tpe))
          val List(defaultCase) = methodBodyTyper
            .typedCases(List(cdef), argTp, B1.tpe)
        ...
        val defdef = newDefDef(methodSym, rhs)(
      ...
      def isDefinedAtMethod = {
        val methodSym = anonClass
          .newMethod(nme.isDefinedAt, tree.pos.makeTransparent, FINAL)
        ...
        val match_ = methodBodyTyper.typedMatch(
          selector(paramSym),
        ...
        DefDef(
          methodSym,
          methodBodyTyper.virtualizedMatch(match_, mode, BooleanTpe))
      }
      ...
      def applyMethod = {
        val methodSym = anonClass
          .newMethod(nme.apply, tree.pos, FINAL | OVERRIDE)
        val paramSym = mkParam(methodSym)
        ...
        val match_ = methodBodyTyper
          .typedMatch(selector(paramSym), cases, mode, resTp)
        ...
        (
          DefDef(
            methodSym,
            methodBodyTyper.virtualizedMatch(match_, mode, matchResTp)),
          matchResTp)
      }

      def parents(resTp: Type) =
        addSerializable(appliedType(
          AbstractPartialFunctionClass.typeConstructor,
          List(argTp, resTp)))
      ...
      val typedBlock = typedPos(tree.pos, mode, pt) {
        Block(
          ...
          atPos(tree.pos.focus)(Apply(
            Select(
              New(Ident(anonClass.name).setSymbol(anonClass)),
              nme.CONSTRUCTOR),
            List()))
        )
      }
    ...
    def synthesizeSAMFunction(
      ...
      val samBodyDef = DefDef(
        NoMods,
        bodyName,
        Nil,
        List(
          fun.vparams.map(_.duplicate)
        ), // must duplicate as we're also using them for `samDef`
      ...
      val samClassTpFullyDefined =
        if (isFullyDefined(samClassTp)) samClassTp
          ...
          try {
            ...
            val nestedCtx = enterSym(
              context.makeNewScope(context.tree, context.owner).makeSilent(),
              samBodyDef)
            ...
            val targs = solvedTypes(
              tvars,
              tparams,
              tparams map varianceInType(sam.info),
              upper = false,
            ...
            case _: NoInstance | _: TypeError =>
              devWarning(
                sampos,
                s"Could not define type $samClassTp using ${samBodyDef.symbol
      ...
      val funArgTps = foreach2(samMethTp.paramTypes, fun.vparams)((tp, vpar) =>
        vpar.tpt setType tp)

      // `final override def ${sam.name}($p1: $T1', ..., $pN: $TN'): ${samMethTp.finalResultType} = ${sam.name}\$body'($p1, ..., $pN)`
      val samDef = DefDef(
        Modifiers(FINAL | OVERRIDE | SYNTHETIC),
        sam.name.toTermName,
        Nil,
        List(fun.vparams),
      ...
      val classDef = ClassDef(
        ...
        gen.mkTemplate(
          parents =
            TypeTree(samClassTpFullyDefined) :: serializableParentAddendum,
          self = noSelfType,
          constrMods = NoMods,
          vparamss = ListOfNil,
          body = List(samDef),
      ...
      val block = nestedTyper.typedPos(sampos, mode, samClassTpFullyDefined) {
        Block(
          samBodyDef,
          classDef,
      ...
      if (block exists (_.isErroneous))
        context.error(
          fun.pos,
          s"Could not derive subclass of $samClassTp\n (with SAM `def $sam$samMethTp`)\n based on: $fun.")
      ...
      if (!FunctionSymbol.exists) MaxFunctionArityError(fun)
              ...
              if (isFullyDefined(argpt)) argpt
                ...
                fun match {
                  case etaExpansion(vparams, fn, args) =>
                    silent(_.typed(fn, mode.forFunMode, pt)) filter (_ =>
                      context.undetparams.isEmpty) map { fn1 =>
                      // if context.undetparams is not empty, the function was polymorphic,
                      // so we need the missing arguments to infer its type. See #871
                      //println("typing eta "+fun+":"+fn1.tpe+"/"+context.undetparams)
                      val ftpe =
                        normalize(fn1.tpe) baseType FunctionClass(numVparams)
                ...
                MissingParameterTypeError(
                  ...
                  vparam,
                  pt,
                  withTupleAddendum = !issuedMissingParameterTypeError)
        ...
        fun.body match {
          ...
          case Match(sel, cases)
              if (sel ne EmptyTree) && (pt
                .typeSymbol == PartialFunctionClass) =>
            ...
            outerTyper.synthesizePartialFunction(
              p.name,
              p.pos,
    ...
    def typedStats(stats: List[Tree], exprOwner: Symbol): List[Tree] = {
      ...
      def includesTargetPos(tree: Tree) =
        tree.pos.isRange && context.unit.exists && (tree.pos includes context
          .unit.targetPos)
      val localTarget = stats exists includesTargetPos
      def typedStat(stat: Tree): Tree = {
        if (context.owner.isRefinementClass && !treeInfo
              .isDeclarationOrTypeDef(stat)) OnlyDeclarationsError(stat)
        else
          stat match {
            case imp @ Import(_, _) =>
              ...
              if (localTarget && !includesTargetPos(stat)) {
                ...
                if (treeInfo.isSelfOrSuperConstrCall(result)) {
                  ...
                  if (treeInfo.isSelfConstrCall(result) && result.symbol.pos
                        ...
                        .pointOrElse(0)) ConstructorsOrderError(stat)
                }

                if (!isPastTyper && treeInfo
                      .isPureExprForWarningPurposes(result))
                  context.warning(
                    stat.pos,
      ...
      def accesses(looker: Symbol, accessed: Symbol) =
        accessed.isLocalToThis && ((accessed.isParamAccessor)
          || (looker.hasAccessorFlag && !accessed.hasAccessorFlag && accessed
            .isPrivate))
        ...
        while ((e ne null) && e.owner == scope) {
          ...
          while ((e1 ne null) && e1.owner == scope) {
            if (!accesses(e.sym, e1.sym) && !accesses(e1.sym, e.sym) &&
              ...
              if (!e.sym.isErroneous && !e1.sym.isErroneous && !e.sym
                    .hasDefault &&
                  !e.sym.hasAnnotation(BridgeClass) && !e1.sym
                    .hasAnnotation(BridgeClass)) {
                log(
                ...
                scope
                  .unlink(
                    e1
                  ) // need to unlink to avoid later problems with lub; see #2779
              }
      ...
      def addSynthetics(stats: List[Tree]): List[Tree] = {
        ...
        if (newStats.isEmpty) stats
          ...
          def matches(stat: Tree, synt: Tree) =
            (stat, synt) match {
              // synt is default arg for stat
              case (
                    DefDef(_, statName, _, _, _, _),
                    DefDef(mods, syntName, _, _, _, _)) =>
                mods.hasDefault && syntName.toString
                  .startsWith(statName.toString)

              // synt is companion module
              case (
                    ClassDef(_, className, _, _),
                    ModuleDef(_, moduleName, _)) =>
              ...
              case (
                ...
                cmods.isImplicit && dmods.isImplicit && cname
                  .toTermName == dname
    ...
    /** Does function need to be instantiated, because a missing parameter
      ...
      */
    def needsInstantiation(
        tparams: List[Symbol],
        formals: List[Type],
        args: List[Tree]) = {
      def isLowerBounded(tparam: Symbol) =
        !tparam.info.bounds.lo.typeSymbol.isBottomClass
    ...
    def callToCompanionConstr(context: Context, calledFun: Symbol) = {
      calledFun.isConstructor && {
        ...
        (methCtx != NoContext) && {
          ...
          companionSymbolOf(calledFun.owner, context).moduleClass == contextFun
            .owner
        }
      }
    }
          ...
          def shapeType(arg: Tree): Type =
            arg match {
              case Function(vparams, body) =>
                functionType(
                  vparams map (_ => AnyTpe),
                  shapeType(body)
                ) // TODO: should this be erased when retyping during erasure?
      ...
      fun.tpe match {
          ...
          def handleOverloaded = {
            ...
            val (args1, argTpes) = context.savingUndeterminedTypeParams() {
              ...
              def typedArg0(tree: Tree) =
                typedArg(tree, amode, BYVALmode, WildcardType)
              args.map {
                case arg @ AssignOrNamedArg(Ident(name), rhs) =>
                  // named args: only type the righthand sides ("unknown identifier" errors otherwise)
                  // the assign is untyped; that's ok because we call doTypedApply
                  val typedRhs = typedArg0(rhs)
                  val argWithTypedRhs = treeCopy
                    .AssignOrNamedArg(arg, arg.lhs, typedRhs)
        ...
        case _ if isPolymorphicSignature(fun.symbol) =>
          ...
          val cloneParams = pts map (pt =>
            clone.newValueParameter(currentUnit.freshTermName()).setInfo(pt))
        ...
        case mt @ MethodType(params, _) =>
          ...
          def tryTupleApply: Tree = {
            if (eligibleForTupleConversion(paramTypes, argslen) && !phase
                  .erasedTypes) {
              val tupleArgs = List(
              ...
              silent(_.doTypedApply(tree, fun, tupleArgs, mode, pt)) map { t =>
                ...
                val keepTree = (!mode
                  .typingExprNotFun // why? introduced in 4e488a60, doc welcome
                  || t.symbol == null // ditto
          ...
          def tryNamesDefaults: Tree = {
            ...
            if (mt.isErroneous) duplErrTree
              ...
              tryTupleApply orElse duplErrorTree(
                TooManyArgsNamesDefaultsError(tree, fun))
            } else if (lencmp == 0) {
              // we don't need defaults. names were used, so this application is transformed
              // into a block (@see transformNamedApplication in NamesDefaults)
              val (namelessArgs, argPos) = removeNames(Typer.this)(args, params)
              if (namelessArgs exists (_.isErroneous)) { duplErrTree }
              else if (!allArgsArePositional(argPos) && !sameLength(
                         formals,
                         params))
                // !allArgsArePositional indicates that named arguments are used to re-order arguments
                duplErrorTree(MultipleVarargError(tree))
              else if (allArgsArePositional(argPos) && !isNamedApplyBlock(
                         fun)) {
              ...
              val symsOwnedByContextOwner = tree.collect {
                case t @ (_: DefTree | _: Function)
                    if ownerOf(t.symbol) == context.owner => t.symbol
              }
              def rollbackNamesDefaultsOwnerChanges() {
                symsOwnedByContextOwner foreach (_.owner = context.owner)
              }

              val fun1 = transformNamedApplication(Typer.this, mode, pt)(
                fun,
                x => x)
              if (fun1.isErroneous) duplErrTree
              else {
                assert(isNamedApplyBlock(fun1), fun1)
                val NamedApplyInfo(qual, targs, previousArgss, _) = context
                ...
                val (allArgs, missing) = addDefaults(
                  args,
                  qual,
                  targs,
                  previousArgss,
          ...
          if (!sameLength(formals, args) || // wrong nb of arguments
            ...
            if (tparams.isEmpty) { // all type params are defined
              def handleMonomorphicCall: Tree = {
                ...
                def ifPatternSkipFormals(tp: Type) =
                  tp match {
                 ...
                 */
                if (args.isEmpty && canTranslateEmptyListToNil && fun.symbol
                      .isInitialized && ListModule.hasCompleteInfo && (fun
                      .symbol == List_apply))
              ...
              def handlePolymorphicCall = {
                assert(
                  !mode.inPatternMode,
                  mode
                ) // this case cannot arise for patterns
                val lenientTargs = protoTypeArgs(
                  tparams,
                  formals,
                  mt.resultApprox,
                  pt)
                ...
                def typedArgToPoly(arg: Tree, formal: Type): Tree = { //TR TODO: cleanup
                  val lenientPt = formal
                    .instantiateTypeParams(tparams, lenientTargs)
                  val newmode =
                  ...
                  if (!argtparams.isEmpty) {
                    val strictPt = formal
                      .instantiateTypeParams(tparams, strictTargs)
        ...
        case ErrorType => if (!tree.isErrorTyped) setError(tree) else tree
    ...
    def typedAnnotation(ann: Tree, mode: Mode = EXPRmode): AnnotationInfo = {
      ...
      def tree2ConstArg(tree: Tree, pt: Type): Option[ClassfileAnnotArg] =
        tree match {
          ...
          case Apply(fun, args) =>
            ...
            if (typedFun.symbol.owner == ArrayModule.moduleClass && typedFun
                  .symbol.name == nme.apply) pt match {
      ...
      val typedFunPart =
        (
          ...
          if (targs.isEmpty && typedFun0
                .exists(t => t.tpe != null && isDummyAppliedType(t.tpe)))
            logResult(s"Retyped $typedFun0 to find type args")(typed(
              argss.foldLeft(fun0)(Apply(_, _))))
          else typedFun0)
      ...
      finish(
        if (typedFun.isErroneous || annType == null) ErroneousAnnotation
        else if (annType
          ...
          if (argss.length > 1) {
            ...
            val annScope = annType.decls.filter(sym =>
            ...
            val nvPairs = args map {
              case arg @ AssignOrNamedArg(Ident(name), rhs) =>
                val sym =
                  if (isJava) annScope.lookup(name)
          ...
          def annInfo(t: Tree): AnnotationInfo =
            t match {
              case Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
                AnnotationInfo(annType, args, List()).setOriginal(typedAnn)
                  .setPos(t.pos)
    ...
    def isReferencedFrom(ctx: Context, sym: Symbol): Boolean =
      (ctx.owner.isTerm && (ctx.scope
        .exists { dcl => dcl.isInitialized && (dcl.info contains sym) }) || {
        var ctx1 = ctx.outer
    ...
    def isCapturedExistential(sym: Symbol) =
      ((sym hasAllFlags EXISTENTIAL | CAPTURED) && {
        val start =
          if (Statistics.canEnable) Statistics.startTimer(isReferencedNanos)
          else null
    ...
    def packCaptured(tpe: Type): Type = {
      ...
      for (tp <- tpe)
        if (isCapturedExistential(tp.typeSymbol)) captured += tp.typeSymbol
    ...
    def packedType(tree: Tree, owner: Symbol): Type = {
      ...
      def isLocal(sym: Symbol): Boolean =
        if (sym == NoSymbol || sym.isRefinementClass || sym.isLocalDummy) false
          ...
          containsDef(owner, sym) || isRawParameter(
            sym) || isCapturedExistential(sym)
              ...
              if (sym.isAliasType && containsLocal(tp) && (tp.dealias ne tp))
                ...
                if (pre.isVolatile) pre match {
                  case SingleType(_, sym) if sym.isSynthetic && isPastTyper =>
                    debuglog(
                      s"ignoring volatility of prefix in pattern matcher generated inferred type: $tp"
                    ) // See pos/t7459c.scala
    ...
    protected def typedExistentialTypeTree(
    ...
    protected def typedTypeApply(
      ...
      fun.tpe match {
        case OverloadedType(pre, alts) =>
          ...
          //
          val fun1 = adaptAfterOverloadResolution(
            fun,
            mode.forFunMode | TAPPmode)

          val tparams =
            fun1.symbol
              .typeParams //@M TODO: fun.symbol.info.typeParams ? (as in typedAppliedTypeTree)
          val args1 =
        ...
        case PolyType(tparams, restpe) if tparams.nonEmpty =>
          if (sameLength(tparams, args)) {
            ...
            if (isPredefClassOf(fun.symbol))
              ...
              if (!isPastTyper && fun.symbol == Any_isInstanceOf && targs
                    .nonEmpty) {
        ...
        case ErrorType => setError(treeCopy.TypeApply(tree, fun, args))
        case _ => fun match {
    ...
    object dyna {
      ...
      def isDynamicallyUpdatable(tree: Tree) =
        tree match {
      ...
      def isApplyDynamicNamed(fun: Tree): Boolean =
        fun match {
          case DynamicApplicationNamed(qual, _)
              if acceptsApplyDynamic(qual.tpe.widen) => true
          case _                                     => false
          // look deeper?
      ...
      def typedNamedApply(
          ...
          mode: Mode,
          pt: Type): Tree = {
        def argToBinding(arg: Tree): Tree =
          arg match {
      ...
      def mkInvoke(
        ...
        acceptsApplyDynamicWithType(qual, name) map { tp =>
          ...
          def hasNamed(args: List[Tree]): Boolean =
            args exists (_.isInstanceOf[AssignOrNamedArg])
          // not supported: foo.bar(a1,..., an: _*)
          def hasStar(args: List[Tree]) = treeInfo.isWildcardStarArgList(args)
          def applyOp(args: List[Tree]) =
            if (hasNamed(args)) nme.applyDynamicNamed else nme.applyDynamic
          def matches(t: Tree) =
           ...
           */
          def findSelection(t: Tree): Option[(TermName, Tree)] =
            t match {
          ...
          findSelection(cxTree) match {
            case Some((opName, treeInfo.Applied(_, targs, _))) =>
              ...
              if (opName == nme.updateDynamic)
                suppressMacroExpansion(fun) // SI-7617
              val nameStringLit = atPos(
                treeSelection.pos.withStart(treeSelection.pos.point)
                  .makeTransparent) { Literal(Constant(name.decode)) }
              markDynamicRewrite(
    ...
    def typed1(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def typedBind(tree: Bind) = {
        ...
        name match {
          case name: TypeName =>
            assert(
              body == EmptyTree,
              context.unit + " typedBind: " + name
                .debugString + " " + body + " " + body.getClass)
            val sym =
      ...
      def typedArrayValue(tree: ArrayValue) = {
        ...
        val elems1 =
          tree.elems mapConserve (elem => typed(elem, mode, elemtpt1.tpe))
      ...
      def typedAssign(lhs: Tree, rhs: Tree): Tree = {
        ...
        def fail() =
          if (lhs1.isErrorTyped) lhs1 else AssignmentError(tree, varsym)
...
//        (phase.erasedTypes && varsym.isValue && !varsym.isMethod)) {
      ...
      def typedIf(tree: If): If = {
        ...
        def samePackedTypes =
          (!isPastTyper
            ...
            && packedType(thenp1, context.owner) =:= packedType(
              elsep1,
              context.owner))
        ...
        if (isFullyDefined(pt)) finish(pt)
        ...
        if (selector == EmptyTree) {
          if (pt.typeSymbol == PartialFunctionClass)
            synthesizePartialFunction(
              newTermName(context.unit.fresh.newName("x")),
              tree.pos,
              paramSynthetic = true,
              tree,
              mode,
            ...
            val arity =
              if (isFunctionType(pt)) pt.dealiasWiden.typeArgs.length - 1 else 1
            val params =
      ...
      def typedReturn(tree: Return) = {
        ...
        if (enclMethod == NoContext ||
          ...
          if (restpt.tpe eq null) {
            ...
            val expr1 =
              context withinReturnExpr typedByValueExpr(expr, restpt.tpe)
            ...
            val res = treeCopy.Return(tree, checkDead(expr1))
        ...
        val tpt1 = {
          ...
          if (checkStablePrefixClassType(tpt0))
            if (tpt0.hasSymbolField && !tpt0.symbol.typeParams.isEmpty) {
              ...
              TypeTree().setOriginal(tpt0).setType(appliedType(
                tpt0.tpe,
                context.undetparams map (_.tpeHK)
              )) // @PP: tpeHK! #3343, #4018, #4347.
            } else tpt0
          else tpt0
        }
      ...
      def typedEta(expr1: Tree): Tree =
        expr1.tpe match {
          case TypeRef(_, ByNameParamClass, _) =>
            ...
            new ChangeOwnerTraverser(context.owner, expr2.symbol)
              .traverse(expr2)
          ...
          case ErrorType => expr1
          case _         => UnderscoreEtaError(expr1)
        }
      ...
      def tryTypedApply(fun: Tree, args: List[Tree]): Tree = {
        val start =
          if (Statistics.canEnable) Statistics.startTimer(failedApplyNanos)
          else null

        def onError(
            typeErrors: Seq[AbsTypeError],
          ...
          def errorInResult(tree: Tree) =
            treesInResult(tree) exists (err =>
              typeErrors.exists(_.errPos == err.pos))

          val retry = (typeErrors.forall(_.errPos != null)) && (
            fun :: tree :: args exists errorInResult
          )
          typingStack.printTyping({
            val funStr = ptTree(fun) + " and " + (args map ptTree mkString ", ")
            if (retry) "second try: " + funStr
            else
              "no second try: " + funStr + " because error not in result: " + typeErrors
                .head.errPos + "!=" + tree.pos
          })
          if (retry) {
            ...
            tryTypedArgs(args, forArgMode(fun, mode)) match {
              case Some(args1)
                ...
                if (qual1 ne qual) {
                  val tree1 =
                    Apply(Select(qual1, name) setPos fun.pos, args1) setPos tree
                      .pos
      ...
      def normalTypedApply(tree: Tree, fun: Tree, args: List[Tree]) = {
        ...
        val stableApplication = (fun.symbol ne null) && fun.symbol
          .isMethod && fun.symbol.isStable
        val funpt = if (mode.inPatternMode) pt else WildcardType
        val appStart =
          if (Statistics.canEnable) Statistics.startTimer(failedApplyNanos)
          else null
        val opeqStart =
          if (Statistics.canEnable) Statistics.startTimer(failedOpEqNanos)
          else null

        def onError(reportError: => Tree): Tree =
          fun match {
            case Select(qual, name)
                if !mode.inPatternMode && nme
                  .isOpAssignmentName(newTermName(name.decode)) =>
        ...
        silentResult match {
          case SilentResultValue(fun1) =>
            ...
            if (isFirstTry) tryTypedApply(fun2, args)
            else doTypedApply(tree, fun2, args, mode, pt)
          case err: SilentTypeError => onError({
      ...
      object ArrayInstantiation {
        def unapply(tree: Apply) =
          tree match {
            case Apply(Select(New(tpt), name), arg :: Nil)
                if tpt.tpe != null && tpt.tpe.typeSymbol == ArrayClass =>
              Some(tpt.tpe) collect {
                case erasure.GenericArray(level, componentType) =>
                  val tagType = (1 until level)
                    .foldLeft(componentType)((res, _) => arrayType(res))

                  resolveClassTag(tree.pos, tagType) match {
                    case EmptyTree => MissingClassTagError(tree, tagType)
                    case tag =>
                      atPos(tree.pos)(new ApplyToImplicitArgs(
                        Select(tag, nme.newArray),
                        arg :: Nil))
                  }
              }
            case _ => None
        ...
        tree match {
          case Apply(Block(stats, expr), args) =>
            typed1(
              ...
              mode,
              pt)
          case Apply(fun, args) => normalTypedApply(tree, fun, args) match {
      ...
      def convertToAssignment(
        ...
        def mkAssign(vble: Tree): Tree =
          Assign(
            vble,
            Apply(
              Select(vble.duplicate, prefix) setPos fun.pos.focus,
              args) setPos tree.pos.makeTransparent) setPos tree.pos
            ...
            case tab :: is =>
              def mkCall(name: Name, extraArgs: Tree*) =
                (Apply(
                  ...
                  is.map(i => i()) ++ extraArgs) setPos tree.pos)
              mkCall(
                nme.update,
                Apply(
                  Select(mkCall(nme.apply), prefix) setPos fun.pos,
        ...
        val tree1 = qual match {
          ...
          case Apply(fn, indices) => fn match {
      ...
      def typedSuper(tree: Super) = {
        ...
        def findMixinSuper(site: Type): Type = {
          ...
          if (ps.isEmpty) ps = site.parents filter (_.typeSymbol.name == mix)
          if (ps.isEmpty) {
            debuglog(
              "Fatal: couldn't find site " + site + " in " + site.parents
      ...
      def typedThis(tree: This) =
        tree.symbol orElse qualifyingClass(
          tree,
          tree.qual,
          packageOK = false) match {
          case NoSymbol => tree
          case clazz =>
      ...
      def typedSelect(tree: Tree, qual: Tree, name: Name): Tree = {
        ...
        if (isPastTyper) t.tpe match {
          case OverloadedType(pre, alts) =>
            if (alts forall (s =>
                  (s.owner == ObjectClass) || (s
                    .owner == AnyClass) || isPrimitiveValueClass(s.owner))) ()
      ...
      def typedSelectInternal(tree: Tree, qual: Tree, name: Name): Tree = {
        def asDynamicCall =
          dyna.mkInvoke(context, tree, qual, name) map { t =>
            dyna.wrapErrors(t, (_.typed1(t, mode, pt)))
          }
        ...
        val sym = tree.symbol orElse member(qual, name) orElse {
          ...
          if (name != nme.CONSTRUCTOR && mode.inAny(EXPRmode | PATTERNmode)) {
            val qual1 = adaptToMemberWithArgs(
              tree,
              qual,
              name,
              mode,
        ...
        if (phase.erasedTypes && qual.isInstanceOf[Super] && tree
              .symbol != NoSymbol) qual setType tree.symbol.owner.tpe
        ...
        if (!reallyExists(sym)) {
          ...
          val (result, accessibleError) = silent(
            ...
            case SilentTypeError(err: AccessTypeError) => (tree1, Some(err))
          ...
          result match {
            ...
            case SelectFromTypeTree(qual @ TypeTree(), name)
                if qual.tpe.typeArgs
                  .nonEmpty => // TODO: somehow the new qual is not checked in refchecks
      ...
      def tryWithFilterAndFilter(tree: Select, qual: Tree): Tree = {
        def warn(sym: Symbol) =
          context.deprecationWarning(
            tree.pos,
            sym,
            s"`withFilter' method does not yet exist on ${qual.tpe.widen}, using `filter' method instead")
      ...
      def typedIdent(tree: Tree, name: Name): Tree = {
        ...
        def issue(err: AbsTypeError) = {
          ...
          val suppress =
            reporter.hasErrors && (name startsWith tpnme.ANON_CLASS_NAME)
        ...
        nameLookup match {
          case LookupAmbiguous(msg) => issue(
              AmbiguousIdentError(tree, name, msg))
          case LookupInaccessible(sym, msg) => issue(
              AccessError(tree, sym, context, msg))
          case LookupNotFound =>
          ...
          case LookupSucceeded(qual, sym) =>
            ( // this -> Foo.this
              if (sym.isThisSym)
              ...
              else if (isPredefClassOf(sym) && pt.typeSymbol == ClassClass && pt
                         .typeArgs.nonEmpty) {
                ...
                val tree1 =
                  if (qual == EmptyTree) tree
                    ...
                    atPos(tree.pos)(
                      Select(atPos(tree.pos.focusStart)(qual), name))
      ...
      def typedCompoundTypeTree(tree: CompoundTypeTree) = {
        ...
        if ((parents1 ++ templ.body) exists (_.isErrorTyped))
          ...
          templ updateAttachment CompoundTypeTreeOriginalAttachment(
            parents1,
            Nil
          ) // stats are set elsewhere
          tree setType (if (templ.exists(_.isErroneous)) ErrorType
                        else self) // Being conservative to avoid SI-5361
        }
      }
        ...
        if (tpt1.isErrorTyped) { tpt1 }
          ...
          if (sameLength(tparams, args)) {
            ...
            val args1 = map2Conserve(args, tparams) { (arg, tparam) =>
              ...
              val pt =
                if (mode.typingPatternOrTypePat) { tparam.initialize; ptParams }
            ...
            val result =
              TypeTree(appliedType(tpt1.tpe, argtypes)) setOriginal original
            ...
            if (settings.debug)
              Console
                .println(
                  tpt1 + ":" + tpt1.symbol + ":" + tpt1.symbol.info
                ) //debug
      ...
      def typedPackageDef(pdef0: PackageDef) = {
        ...
        val stats1 = newTyper(
          context.make(tree, sym.moduleClass, sym.info.decls))
      ...
      def defDefTyper(ddef: DefDef) = {
        val isConstrDefaultGetter = ddef.mods.hasDefault && sym.owner
          .isModuleClass &&
      ...
      def typedAlternative(alt: Alternative) = {
        context withinPatAlternative (treeCopy.Alternative(
          tree,
      ...
      def typedTyped(tree: Typed) = {
        ...
        else if (mode.inPatternMode) typedInPattern(tree, mode.onlySticky, pt)
        else
          tree match {
      ...
      def typedTypeApply(tree: TypeApply) = {
        ...
        val args1 =
          if (sameLength(args, tparams)) map2Conserve(args, tparams) {
            (arg, tparam) =>
              typedHigherKindedType(
                arg,
                mode,
      ...
      def warnMissingInterpolator(lit: Literal): Unit =
        if (!isPastTyper) {
          ...
          def isRecognizablyNotForInterpolation =
            context.enclosingApply.tree match {
              case Apply(
                    Select(Apply(RefTree(_, nme.StringContext), _), _),
                    _) => true
              case Apply(
                    Select(New(RefTree(_, tpnme.implicitNotFound)), _),
                    _) => true
              case _   => isMacroExpansion
            }
          def requiresNoArgs(tp: Type): Boolean =
            tp match {
              case PolyType(_, restpe) => requiresNoArgs(restpe)
              case MethodType(Nil, restpe) =>
                requiresNoArgs(
                  restpe
                ) // may be a curried method - can't tell yet
          ...
          def maybeWarn(s: String): Unit = {
            ...
            if (suspiciousExpr.nonEmpty)
              ...
              suspiciousIdents find isPlausible foreach (sym =>
                warn(
                  s"detected interpolated identifier `$$${sym.name}`"
                )) // "$id"
          }
          lit match {
            case Literal(Constant(s: String))
                if !isRecognizablyNotForInterpolation => maybeWarn(s)
            case _                                    =>
          }
        }
      ...
      def typedSingletonTypeTree(tree: SingletonTypeTree) = {
        ...
        if (refTyped.isErrorTyped) { setError(tree) }
          ...
          if (refTyped.isErrorTyped || treeInfo.admitsTypeSelection(refTyped))
            tree
          else UnstableTreeError(tree)
        }
      }
      ...
      def typedSelectFromTypeTree(tree: SelectFromTypeTree) = {
        ...
        if (qual1.isErrorTyped)
          setError(treeCopy.SelectFromTypeTree(tree, qual1, tree.name))
        else if (qual1.tpe.isVolatile)
          TypeSelectionFromVolatileTypeError(tree, qual1)
      ...
      def typedTypeBoundsTree(tree: TypeBoundsTree) = {
        ...
        treeCopy.TypeBoundsTree(tree, lo1, hi1) setType TypeBounds(
          lo1.tpe,
          hi1.tpe)
      }
      ...
      def typedExistentialTypeTree(tree: ExistentialTypeTree) = {
        val tree1 = typerWithLocalContext(
          ...
          else typer.typedExistentialTypeTree(tree, mode)
        }
      ...
      def typedInPatternMode(tree: Tree): Tree =
        tree match {
          case tree: Alternative => typedAlternative(tree)
          case tree: Star        => typedStar(tree)
          case _ => abort(
              s"unexpected tree in pattern mode: ${tree.getClass}\n$tree")
        }
      ...
      def typedTypTree(tree: TypTree): Tree =
        tree match {
          ...
          case _ => abort(
              s"unexpected type-representing tree: ${tree.getClass}\n$tree")
        }
      ...
      def typedMemberDef(tree: MemberDef): Tree =
        tree match {
          ...
          case tree: ClassDef => newTyper(context.makeNewScope(tree, sym))
              .typedClassDef(tree)
          case tree: ModuleDef =>
      ...
      def typedOutsidePatternMode(tree: Tree): Tree =
        tree match {
          case tree: Block =>
          ...
          case tree: AssignOrNamedArg =>
            typedAssign(
              tree.lhs,
              tree.rhs
            ) // called by NamesDefaults in silent typecheck
      ...
      def typedInAnyMode(tree: Tree): Tree =
        tree match {
          ...
          case tree: UnApply =>
            abort(
              s"unexpected UnApply $tree"
            ) // turns out UnApply never reaches here
          case _ =>
            if (mode.inPatternMode) typedInPatternMode(tree)
            else typedOutsidePatternMode(tree)
        }
    ...
    def typed(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def body =
        (if (printTypings && !phase.erasedTypes && !noPrintTyping(tree))
           typingStack
             .nextTyped(tree, mode, pt, context)(typedInternal(tree, mode, pt))
    ...
    private def typedInternal(tree: Tree, mode: Mode, pt: Type): Tree = {
      ...
      def runTyper(): Tree = {
        ...
        val ptWild =
          ...
          else dropExistential(ptPlugins) // FIXME: document why this is done.
        ...
        val result =
          if (tree1.isEmpty) tree1
            ...
            if (hasPendingMacroExpansions) macroExpandAll(this, result)
            else result
          }
      ...
      try runTyper()
        ...
        case ex: TypeError =>
          ...
          typingStack.printTyping(
            tree,
            "caught %s: while typing %s".format(ex, tree)
          ) //DEBUG
    ...
    def typedPattern(tree: Tree, pt: Type): Tree = {
      ...
      typingInPattern(context.withImplicitsDisabledAllowEnrichment(
    ...
    def computeMacroDefType(ddef: DefDef, pt: Type): Type = {
      ...
      if (isMacroBodyOkay && shouldInheritMacroImplReturnType) {
        ...
        def reportWarning(inferredType: Type) = {
          val explanation =
            s"inference of $inferredType from macro impl's c.Expr[$inferredType] is deprecated and is going to stop working in 2.12"
          context.deprecationWarning(
            ddef.pos,
            ddef.symbol,
            s"$commonMessage ($explanation)")
        }
...
object TypersStats {
  ...
  val rawTypeFailed = Statistics
    .newSubCounter("  of which in failed", rawTypeCount)
  val subtypeFailed = Statistics
    .newSubCounter("  of which in failed", subtypeCount)
  val findMemberFailed = Statistics
    .newSubCounter("  of which in failed", findMemberCount)
  val failedSilentNanos = Statistics
    .newSubTimer("time spent in failed", typerNanos)
  ...
  val isReferencedNanos = Statistics
    .newSubTimer("time spent ref scanning", typerNanos)
  val visitsByType = Statistics
    .newByClass("#visits by tree node", "typer")(Statistics.newCounter(""))
  val byTypeNanos = Statistics.newByClass("time spent by tree node", "typer")(
    Statistics.newStackableTimer("", typerNanos))
