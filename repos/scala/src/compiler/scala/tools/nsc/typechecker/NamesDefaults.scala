...
trait NamesDefaults { self: Analyzer =>
  ...
  class ConstructorDefaultsAttachment(
      val classWithDefault: ClassDef,
  ...
  def transformNamedApplication(typer: Typer, mode: Mode, pt: Type)(
    ...
    def baseFunBlock(baseFun: Tree): Tree = {
      ...
      def blockWithQualifier(qual: Tree, selected: Name) = {
        val sym =
          blockTyper.context.owner.newValue(
    ...
    if (isNamedApplyBlock(tree)) {
      ...
      tree match {
        ...
        case baseFun =>
          // also treats "case TypeApply(fun, targs)" and "case Select(New(..), <init>)"
  ...
  def missingParams[T](
    ...
    val namedArgsOnChangedPosition =
      args.zip(params) dropWhile {
        case (arg, param) =>
          val n = argName(arg)
    ...
    def matchesName(param: Symbol, name: Name, argIndex: Int) = {
      ...
      def checkName = {
        ...
        if (res && checkDeprecation(true))
          warn(s"naming parameter $name has been deprecated.")
        res
      }
      def checkAltName = {
