...
trait PatternTypers {
  ...
  trait PatternTyper {
    ...
    // more common place)
    private def hasUnapplyMember(tpe: Type): Boolean = reallyExists(
      unapplyMember(tpe))
    private def hasUnapplyMember(sym: Symbol): Boolean = hasUnapplyMember(
      sym.tpe_*)
    private def hasUnapplyMember(fun: Tree): Boolean =
      hasUnapplyMember(fun.symbol) || hasUnapplyMember(fun.tpe)
    ...
    private class VariantToSkolemMap extends TypeMap(trackVariance = true) {
      ...
      def skolems = try skolemBuffer.toList
     ...
     */
    private def convertToCaseConstructor(
        tree: Tree,
        caseClass: Symbol,
        ptIn: Type): Tree = {
