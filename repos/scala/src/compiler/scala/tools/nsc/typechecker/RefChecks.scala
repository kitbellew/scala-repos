...
abstract class RefChecks
    ...
    with scala.reflect.internal.transform.RefChecks {
  ...
  val toJavaRepeatedParam =
    new SubstSymMap(RepeatedParamClass -> JavaRepeatedParamClass)
  val toScalaRepeatedParam =
    new SubstSymMap(JavaRepeatedParamClass -> RepeatedParamClass)

  def accessFlagsToString(sym: Symbol) =
    flagsToString(
      sym getFlag (PRIVATE | PROTECTED),
      if (sym.hasAccessBoundary)
        "" + sym.privateWithin.name
      else
        "")
  ...
  class RefCheckTransformer(unit: CompilationUnit) extends Transformer {
    ...
    private def checkOverloadedRestrictions(
        clazz: Symbol,
      ...
      val defaultGetters = defaultClass
        .info
      ...
      defaultMethodNames.toList.distinct foreach { name =>
        ...
        if (haveDefaults.lengthCompare(1) > 0) {
          ...
          if (haveDefaults.exists(x => !x.isConstructor) || owners
                .distinct
                .size < haveDefaults.size) {
            reporter.error(
              clazz.pos,
              "in " + clazz +
      ...
      if (clazz isSubClass DynamicClass) {
        for ((_, m1 :: m2 :: _) <- (
                 clazz.info member nme.applyDynamic
             ).alternatives groupBy (_.typeParams.length)) {
          reporter.error(
            m1.pos,
...
// Override checking ------------------------------------------------------------
    ...
    private def checkAllOverrides(clazz: Symbol, typesOnly: Boolean = false) {
      ...
      def checkOverride(pair: SymbolPair) {
        ...
        debuglog(
          "Checking validity of %s overriding %s"
            .format(member.fullLocationString, other.fullLocationString))
        ...
        def objectOverrideErrorMsg =
          ("overriding " + high
            .fullLocationString + " with " + low.fullLocationString + ":\n" +
        ...
        def overrideErrorMsg(msg: String): String = {
          val isConcreteOverAbstract =
            (other.owner isSubClass member.owner) && other.isDeferred && !member
              .isDeferred
          val addendum =
        ...
        def emitOverrideError(fullmsg: String) {
          if (member.owner == clazz)
            ...
            mixinOverrideErrors += new MixinOverrideError(member, fullmsg)
        }
          ...
          if (noErrorType) {
            emitOverrideError(
              if (member.isModule && other.isModule)
                objectOverrideErrorMsg
              else
                overrideErrorMsg("has incompatible type"))
          }
        }
        ...
        if (typesOnly)
          ...
          def isOverrideAccessOK =
            member
              .isPublic || { // member is public, definitely same or relaxed access
              (!other.isProtected || member
                .isProtected) && // if o is protected, so is m
              ((!isRootOrNone(ob) && ob
                .hasTransOwner(mb)) || // m relaxes o's access boundary
              other
                .isJavaDefined) // overriding a protected java member, see #3946
            }
          if (!isOverrideAccessOK) {
          ...
          } else if (!other.isDeferredOrJavaDefault && !other
                       .hasFlag(JAVA_DEFAULTMETHOD) && !member
                       .isAnyOverride && !member.isSynthetic) { // (*)
            // (*) Synthetic exclusion for (at least) default getters, fixes SI-5178. We cannot assign the OVERRIDE flag to
            // the default getter: one default getter might sometimes override, sometimes not. Example in comment on ticket.
            if (isNeitherInClass && !(
                  other.owner isSubClass member.owner
                ))
              emitOverrideError(
          ...
          } else if (member.isAnyOverride && (other hasFlag ACCESSOR) && other
                       .accessed
                       .isVariable && !other.accessed.isLazy) {
            ...
            overrideError("needs to be a stable, immutable value")
                     ...
                     other.isValue && !other.isSourceMethod && !other
                       .isDeferred && !other.isLazy) {
            overrideError("cannot override a concrete non-lazy value")
          } else if (other.isValue && other.isLazy && !other
                       .isSourceMethod && !other.isDeferred &&
                     member.isValue && !member.isLazy) {
            overrideError(
              "must be declared lazy to override a concrete lazy value")
          } else if (other.isDeferred && member.isTermMacro && member
        ...
        def checkOverrideAlias() {
          ...
          if (pair.sameKind && lowType
            ...
            overrideTypeError() // (1.6)
        }
        //if (!member.typeParams.isEmpty) // (1.7)  @MAT
        //  overrideError("may not be parameterized");
        def checkOverrideAbstract() {
          if (!(
                highInfo.bounds containsType lowType
              )) { // (1.7.1)
          ...
          typer
            ...
            case kindErrors =>
              reporter.error(
                member.pos,
                "The kind of " + member
                  .keyString + " " + member.varianceString + member.nameString +
                  " does not conform to the expected kind of " + other
                  .defString + other.locationString + "." +
          ...
          if (low.isAliasType) {
            typer
              ...
              case kindErrors =>
                reporter.error(
                  member.pos,
                  "The kind of the right-hand side " + lowType
                    .normalize + " of " + low.keyString + " " +
                    low.varianceString + low
                    .nameString + " does not conform to its expected kind." +
                    kindErrors.toList.mkString("\n", ", ", "")
                )
            }
          } else if (low.isAbstractType && lowType
                       .isVolatile && !highInfo.bounds.hi.isVolatile)
            overrideError(
        ...
        def checkOverrideTypes() {
          if (high.isAliasType)
            ...
            checkOverrideAbstract()
          else if (high.isTerm)
            checkOverrideTerm()
        }

        def checkOverrideDeprecated() {
          if (other.hasDeprecatedOverridingAnnotation && !member
                .ownerChain
                .exists(x => x.isDeprecated || x.hasBridgeAnnotation)) {
            val suffix = other
              .deprecatedOverridingMessage map (": " + _) getOrElse ""
            val msg =
      ...
      if (clazz.isConcreteClass && !typesOnly) {
        ...
        def abstractErrorMessage =
          ...
          if (abstractErrors.size <= 2)
            abstractErrors mkString " "
          else
        ...
        def abstractClassError(mustBeMixin: Boolean, msg: String) {
          def prelude =
            (if (clazz.isAnonymousClass || clazz.isModuleClass)
               ...
               clazz + " needs to be a mixin"
             else
               clazz + " needs to be abstract") + ", since"

          if (abstractErrors.isEmpty)
            abstractErrors ++= List(prelude, msg)
          else
            abstractErrors += msg
        }
        ...
        def javaErasedOverridingSym(sym: Symbol): Symbol =
          clazz
            ...
            .filter(other =>
              !other.isDeferred && other.isJavaDefined && !sym
                .enclClass
        ...
        def checkNoAbstractMembers(): Unit = {
          ...
          val missingMethods = grouped.toList flatMap {
            case (name, syms) =>
              if (syms exists (_.isSetter))
                syms filterNot (_.isGetter)
              else
                syms
          }
            ...
            if (regrouped.tail.isEmpty)
              ...
              (
                regrouped.sortBy("" + _._1.name) flatMap {
                  case (owner, members) =>
                    ("// Members declared in " + owner
                      .fullName) +: membersStrings(members) :+ ""
                }
              ).init
          }
          ...
          for (member <- missing) {
            ...
            if (underlying.isVariable) {
              ...
              if (member.isSetter && isMultiple)
                ...
                undefined(
                  if (member.isSetter)
                    ...
                    analyzer.abstractVarMessage(member))
              ...
              matchingArity match {
                ...
                case Scope(concrete) =>
                  val mismatches = abstractParams zip concrete
                    .tpe
                    .paramTypes filterNot {
                    case (x, y) =>
                      x =:= y
                  }
                  mismatches match {
                    ...
                    case (pa, pc) :: Nil =>
                      ...
                      val addendum = (if (abstractSym == concreteSym) {
                                        ...
                                        // about forcing types.
                                        if (underlying.isJavaDefined && pa
                                              .typeArgs
                                              .isEmpty && abstractSym
                                              .typeParams
                                              .nonEmpty)
                                          ". To implement a raw type, use %s[_]"
                                            .format(pa)
      ...
      def hasMatchingSym(inclazz: Symbol, member: Symbol): Boolean = {
        ...
        def javaAccessCheck(sym: Symbol) =
          (!inclazz.isJavaDefined // not a java defined member
            ...
            || sym
              .isProtected // marked protected in java, thus accessible to subclasses
            || sym.privateWithin == member
              .enclosingPackageClass // exact package match
          )
...
//            println("validate base type "+tp)
        ...
        if (baseClass.isClass) {
          ...
          if (index >= 0) {
            if (seenTypes(index) forall (tp1 =>
                  !(
                    tp1 <:< tp
                  )))
              seenTypes(index) =
                tp :: (seenTypes(index) filter (tp1 =>
                  !(
                    tp <:< tp1
                  )))
          }
      ...
      for (stat <- stats) {
        ...
        stat match {
          ...
          case ClassDef(_, _, _, _) | DefDef(_, _, _, _, _, _) |
              ModuleDef(_, _, _) | ValDef(_, _, _, _) =>
...
// Comparison checking -------------------------------------------------------
    ...
    def checkImplicitViewOptionApply(
        ...
        fn: Tree,
        args: List[Tree]): Unit =
      if (settings.warnOptionImplicit)
        (fn, args) match {
    ...
    private def checkSensibleEquals(
      ...
      def underlyingClass(tp: Type): Symbol = {
        ...
        if (sym.isAbstractType)
          underlyingClass(sym.info.bounds.hi)
        else
          sym
      }
      ...
      if (nullCount == 2) // null == null
        ...
        if (isNew(qual)) // new X == y
          nonSensiblyNew()
        else if (isNew(other) && (receiver
                   .isEffectivelyFinal || isReferenceOp)) // object X ; X == new Y
          nonSensiblyNew()
        else if (receiver.isEffectivelyFinal && !(
                   receiver isSubClass actual
                 ) && !actual.isRefinementClass) { // object X, Y; X == Y
          if (isEitherNullable)
      ...
      else if (isWarnable && nullCount == 0 && !(
                 isSpecial(receiver) && isSpecial(actual)
               )) {
        // better to have lubbed and lost
        def warnIfLubless(): Unit = {
          val common = global.lub(List(actual.tpe, receiver.tpe))
          if (ObjectTpe <:< common && !(
                ObjectTpe <:< actual.tpe && ObjectTpe <:< receiver.tpe
              ))
            unrelatedTypes()
        }
    ...
    def checkInfiniteLoop(valOrDef: ValOrDefDef) {
      ...
      if (trivialInfiniteLoop)
        reporter.warning(
          valOrDef.rhs.pos,
          s"${valOrDef.symbol.fullLocationString} does nothing other than call itself recursively")
    }
...
// Transformation ------------------------------------------------------------
    ...
    /** Eliminate ModuleDefs. In all cases the ModuleDef (carrying a module symbol) is
      ...
      */
    private def eliminateModuleDefs(moduleDef: Tree): List[Tree] =
      exitingRefchecks {
        ...
        def matchingInnerObject() = {
          ...
          val accessor = site
            .newMethod(moduleName, module.pos, newFlags) setInfoAndEnter newInfo

          DefDef(accessor, Select(This(site), module)) :: Nil
        }
        val newTrees = cdef :: (if (module.isStatic)
                                  // trait T { def f: Object }; object O extends T { object f }. Need to generate method f in O.
                                  if (module.isOverridingSymbol)
                                    matchingInnerObject()
                                  else
                                    Nil
                                else
      ...
      if (site.isTrait)
        ...
        def mkAccessorSymbol =
          site
            ...
            .andAlso(self =>
              if (module.isPrivate)
                self.expandName(module.owner))

        val accessor =
          if (module.owner == site)
            module
          else
            mkAccessorSymbol
        val accessorDef = DefDef(
          accessor,
          gen
            .mkAssignAndReturn(moduleVar, gen.newModule(module, moduleVar.tpe))
            .changeOwner(moduleVar -> accessor))
      ...
      try typer.infer.checkBounds(tree0, pre, owner, tparams, argtps, "")
        ...
        case ex: TypeError =>
          ...
          if (settings.explaintypes) {
            ...
            (argtps, bounds)
              .zipped map ((targ, bound) => explainTypes(bound.lo, targ))
            (argtps, bounds)
              .zipped map ((targ, bound) => explainTypes(targ, bound.hi))
            ()
          }
      }
    ...
    private def checkUndesiredProperties(sym: Symbol, pos: Position) {
      ...
      if (sym.isDeprecated && !currentOwner
            .ownerChain
            .exists(x => x.isDeprecated || x.hasBridgeAnnotation))
      ...
      if (sym.hasMigrationAnnotation && settings
            .Xmigration
            .value != NoScalaVersion) {
        val changed =
    ...
    private def checkAccessibilityOfReferencedTypes(tree: Tree) {
      ...
      def checkAccessibilityOfType(tpe: Type) {
        ...
        // otherwise warn about the inaccessible syms in the unnormalized type
          ...
          inaccessible foreach (sym => warnLessAccessible(sym, member))
      }
    ...
    private def checkByNameRightAssociativeDef(tree: DefDef) {
      tree match {
        case DefDef(_, name, _, params :: _, _, _) =>
          if (settings.warnByNameRightAssociative && !treeInfo.isLeftAssoc(
                name.decodedName) && params.exists(p => isByName(p.symbol)))
            reporter.warning(
              tree.pos,
    ...
    private def checkDeprecatedOvers(tree: Tree) {
      ...
      if (symbol.isDeprecated) {
        val concrOvers = symbol
          .allOverriddenSymbols
          .filter(sym =>
            !sym.isDeprecated && !sym.isDeferred && !sym
              .hasDeprecatedOverridingAnnotation && !sym
              .enclClass
              .hasDeprecatedInheritanceAnnotation)
    ...
    override def transform(tree: Tree): Tree = {
      ...
      try {
          ...
          tree match {
            ...
            case ValDef(_, _, _, _) | DefDef(_, _, _, _, _, _) =>
              ...
              if (settings.warnInaccessible) {
                if (!sym.isConstructor && !sym
                      .isEffectivelyFinalOrNotOverridden && !sym.isSynthetic)
            ...
            case Template(parents, self, body) =>
              ...
              val moduleDesugared =
                if (currentOwner.isTrait)
                  ...
                  mixinModuleDefs(currentOwner)
              checkAllOverrides(currentOwner)
              checkAnyValSubclass(currentOwner)
              if (currentOwner.isDerivedValueClass)
                currentOwner
                  .primaryConstructor makeNotPrivate NoSymbol // SI-6601, must be done *after* pickler!
              if (bridges.nonEmpty || moduleDesugared.nonEmpty)
                deriveTemplate(tree)(_ ::: bridges ::: moduleDesugared)
              else
                tree
            ...
            case Ident(name) =>
              ...
              if (name != nme.WILDCARD && name != tpnme.WILDCARD_STAR) {
                assert(
                  sym != NoSymbol,
                  "transformCaseApply: name = " + name
                    .debugString + " tree = " + tree + " / " + tree.getClass
                ) //debug
        ...
        result = result match {
          ...
          case Apply(fun, args)
              if fun.symbol.isLabel && treeInfo.isSynthCaseSymbol(fun.symbol) =>
            savingInPattern {
