  ...
  def computeMacroDefTypeFromMacroImplRef(
    ...
    macroImplRef match {
      case MacroImplReference(_, _, _, macroImpl, targs) =>
        ...
        runtimeType = runtimeType.substituteTypes(
          macroImpl.typeParams,
          targs map (_.tpe))
        ...
        def unsigma(tpe: Type): Type =
          transformTypeTagEvidenceParams(
            ...
            case (implCtxParam :: Nil) :: implParamss =>
              val implToDef =
                flatMap2(implParamss, macroDdef.vparamss)(
                  map2(_, _)((_, _))).toMap
    ...
    override def onSuccess(expanded0: Tree) = {
      ...
      def typecheck(label: String, tree: Tree, pt: Type): Tree = {
        if (tree.isErrorTyped)
          ...
          val result = typer.context.withImplicitsEnabled(
...
object MacrosStats {
  ...
  val macroExpandNanos = Statistics.newSubTimer(
