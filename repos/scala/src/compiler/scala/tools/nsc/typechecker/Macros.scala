...
/**
  ...
  *    (TypeTag(Int))
  */
trait Macros extends MacroRuntimes with Traces with Helpers { self: Analyzer =>
                  ...
                  case TypeRef(pre, sym, args) =>
                    val pre1 = pre match {
                      case SingleType(
                          ...
                          value)
                          if c == implCtxParam &&
                            prefix == MacroContextPrefix && value == ExprValue =>
    ...
    protected def expand(desugared: Tree): Tree = {
      ...
      try {
        withInfoLevel(nodePrinters.InfoLevel.Quiet) {
          // verbose printing might cause recursive macro expansions
