...
trait Macros extends MacroRuntimes with Traces with Helpers {
  ...
  object MacroImplBinding {
    ...
    def unpickle(pickle: Tree): MacroImplBinding = {
      val (wrapped, targs) = pickle match {
  ...
  def macroContext(
    ...
    new {
      ...
      val expandee = universe.analyzer.macroExpanderAttachment(expandeeTree)
  ...
  def standardMacroArgs(typer: Typer, expandee: Tree): MacroArgs = {
    ...
    val context = expandee.attachments.get[MacroRuntimeAttachment]
      .flatMap(_.macroContext).getOrElse(macroContext(typer, prefix, expandee))
              ...
              val tpe =
                if (targ.isTypeParameterOrSkolem) {
                  if (targ.owner == macroDef) {
                    ...
                    val argPos = macroDef.typeParams
                      .indexWhere(_.name == targ.name)
                    targs(argPos).tpe
                  } else
                    targ.tpe.asSeenFrom(
                      if (prefix == EmptyTree) macroDef.owner.tpe
                      else prefix.tpe,
                      macroDef.owner)
                } else targ.tpe
  ...
  def enclosingMacroPosition =
    openMacros map (_.macroApplication.pos) find (_ ne NoPosition) getOrElse
      NoPosition
      ...
      override def transform(tree: Tree) =
        super.transform(tree match {
          ...
          case tree
            ...
            val context = tree.attachments.get[MacroRuntimeAttachment].get
              .typerContext
            delayed -= tree
