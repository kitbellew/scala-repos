...
trait Macros extends MacroRuntimes with Traces with Helpers {
  ...
  case class MacroImplBinding(
    ...
    def is_??? = {
      ...
      className == Predef_???.owner.javaClassName &&
      methName == Predef_???.name.encoded
    }
    ...
    def unpickle(pickle: Tree): MacroImplBinding = {
      ...
      def unpickle[T](field: String, clazz: Class[T]): T = {
        ...
        if (!payload.contains(field))
          failField("is supposed to be there")
        val raw: Any = payload(field)
        if (raw == null)
          failField(s"is not supposed to be null")
  ...
  def bindMacroImpl(macroDef: Symbol, macroImplRef: Tree): Unit = {
    ...
    macroDef withAnnotation
      AnnotationInfo(MacroImplAnnotation.tpe, List(pickle), Nil)
  }
    ...
    macroImplRef match {
      case MacroImplReference(_, _, _, macroImpl, targs) =>
        ...
        runtimeType = runtimeType
          .substituteTypes(macroImpl.typeParams, targs map (_.tpe))
        ...
        def unsigma(tpe: Type): Type =
          transformTypeTagEvidenceParams(
            ...
            case (implCtxParam :: Nil) :: implParamss =>
              val implToDef =
                flatMap2(implParamss, macroDdef.vparamss)(map2(_, _)((_, _)))
                  .toMap
                      ...
                      val pre1 =
                        pre match {
                          case SingleType(
                                ...
                                value)
                              if c == implCtxParam &&
                                prefix == MacroContextPrefix &&
                                value == ExprValue =>
                            ThisType(macroDdef.symbol.owner)
                          case SingleType(
                                SingleType(NoPrefix, implParam),
                                value) if value == ExprValue =>
                            implToDef get implParam map
                              (defParam =>
                                SingleType(NoPrefix, defParam.symbol)) getOrElse
                              pre
                          case _ =>
                            pre
                        }
  ...
  def standardMacroArgs(typer: Typer, expandee: Tree): MacroArgs = {
    ...
    if (paramss.length < argss.length)
      MacroTooManyArgumentListsError(expandee)
    if (paramss.length > argss.length && !isNullaryArgsEmptyParams)
      MacroTooFewArgumentListsError(expandee)

    val macroImplArgs: List[Any] =
      if (fastTrack contains macroDef) {
        // Take a dry run of the fast track implementation
        if (fastTrack(macroDef) validate expandee)
          argss.flatten
        else
          MacroTooFewArgumentListsError(expandee)
      } else {
        def calculateMacroArgs(binding: MacroImplBinding) = {
          ...
          val trees =
            map3(argss, paramss, signature)((args, defParams, implParams) => {
              ...
              if (isVarargs) {
                val (normal, varargs) = wrappedArgs splitAt
                  (defParams.length - 1)
                normal :+
                  varargs // pack all varargs into a single Seq argument (varargs Scala style)
              } else
                wrappedArgs
            })
  ...
  def enclosingMacroPosition =
    openMacros map
      (_.macroApplication.pos) find
      (_ ne NoPosition) getOrElse NoPosition
  ...
  abstract class MacroExpander(val typer: Typer, val expandee: Tree) {
    ...
    def apply(desugared: Tree): Tree = {
      if (isMacroExpansionSuppressed(desugared))
        onSuppressed(expandee)
      else
        expand(desugared)
    }
    ...
    protected def expand(desugared: Tree): Tree = {
      ...
      def summary() =
        s"expander = $this, expandee = ${showDetailed(expandee)}, desugared = ${if (expandee == desugared)
          ...
          showDetailed(desugared)}"
      if (macroDebugVerbose)
        println(s"macroExpand: ${summary()}")
      linkExpandeeAndDesugared(expandee, desugared)

      val start =
        if (Statistics.canEnable)
          Statistics.startTimer(macroExpandNanos)
        else
          null
      if (Statistics.canEnable)
      ...
      try {
        withInfoLevel(
          nodePrinters
            .InfoLevel
            .Quiet) { // verbose printing might cause recursive macro expansions
            ...
            try {
              ...
              expanded match {
                case Success(expanded) =>
                  ...
                  if (settings.Ymacroexpand.value ==
                        settings.MacroExpand.Discard) {
  ...
  class DefMacroExpander(
    ...
    lazy val innerPt = {
      val tp =
        if (isNullaryInvocation(expandee))
          expandee.tpe.finalResultType
        else
          expandee.tpe
    ...
    override def onSuccess(expanded0: Tree) = {
      ...
      def typecheck(label: String, tree: Tree, pt: Type): Tree = {
        if (tree.isErrorTyped)
          ...
          if (macroDebugVerbose)
            println(s"$label (against pt = $pt): $tree")
          // `macroExpandApply` is called from `adapt`, where implicit conversions are disabled
          // therefore we need to re-enable the conversions back temporarily
          val result = typer
            .context
            .withImplicitsEnabled(typer.typed(tree, mode, pt))
      ...
      if (isBlackbox(expandee)) {
        val expanded1 =
          atPos(enclosingMacroPosition.makeTransparent)(
        ...
        val expanded1 = typecheck(
          "whitebox typecheck #0",
          expanded0,
          WildcardType)
    ...
    override def onDelayed(delayed: Tree) = {
      ...
      val shouldInstantiate = typer.context.undetparams.nonEmpty &&
        !mode.inPolyMode
  ...
  def macroExpandWithRuntime(
    ...
    (wasDelayed, nowDelayed) match {
      ...
      case (true, false) =>
        ...
        if (expanded exists (_.isErroneous))
          Failure(expandee)
        else
          Skip(expanded)
      case (false, true) =>
        ...
        expandee updateAttachment
          MacroRuntimeAttachment(
            delayed = true,
      ...
      case (false, false) =>
        ...
        try {
          ...
          if (hasNewErrors)
            MacroGeneratedTypeError(expandee)
          ...
          expanded match {
            case expanded: Expr[_] if expandee.symbol.isTermMacro =>
  ...
  def macroExpandWithoutRuntime(typer: Typer, expandee: Tree): MacroStatus = {
    import typer.TyperErrorGen._
    val fallbackSym = expandee.symbol.nextOverriddenSymbol orElse
      MacroImplementationNotFoundError(expandee)
  ...
  private val delayed = perRunCaches
    .newWeakMap[Tree, scala.collection.mutable.Set[Int]]()
  ...
  def notifyUndetparamsInferred(
      undetNoMore: List[Symbol],
      inferreds: List[Type]): Unit = {
    undetparams --= undetNoMore map (_.id)
    if (macroDebugVerbose)
      (undetNoMore zip inferreds) foreach {
        case (sym, tpe) =>
      ...
      override def transform(tree: Tree) =
        super.transform(
          tree match {
            ...
            case tree
                if (delayed contains tree) &&
                  calculateUndetparams(tree).isEmpty && !tree.isErroneous =>
              val context =
...
object MacrosStats {
  ...
  val macroExpandNanos = Statistics
    .newSubTimer("time spent in macroExpand", typerNanos)
}
