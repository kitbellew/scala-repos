...
abstract class TreeCheckers extends Analyzer {
  ...
  def diff(t1: Tree, t2: Tree): String =
    (t1, t2) match {
      case (_: Literal, _: Literal) =>
        ""
      case (t1: ImplDef, t2: ImplDef) =>
        diff(t1.impl, t2.impl)
      case (t1: Template, t2: Template) =>
        diffTemplates(t1, t2)
      case _ =>
        diffTrees(t1, t2).toString // "<error: different tree classes>"
    }
  ...
  private def ownersString(s: Symbol) =
    nonPackageOwnersPlusOne(s) match {
      case Nil =>
        "NoSymbol"
      case xs =>
        xs mkString " -> "
  ...
  class TreeChecker(context0: Context) extends Typer(context0) {
    ...
    override protected def finishMethodSynthesis(
        templ: Template,
        clazz: Symbol,
        context: Context): Template = templ
    ...
    private def passThrough(tree: Tree) =
      tree match {
    ...
    object precheck extends TreeStackTraverser {
      ...
      override def traverse(tree: Tree): Unit =
        tree match {
          case md: MemberDef =>
            pushMemberDef(md)(traverseInternal(tree))
          case _ =>
          ...
          tree match {
            ...
            case _ =>
              ...
              if (sym.owner != currentOwner) {
                val expected = currentOwner.ownerChain find
                  (x => cond(x)) getOrElse {
                ...
                if (sym.owner != expected)
                  fail(
                    sm"""|
                            | currentOwner chain: ${currentOwner.ownerChain take
                      3 mkString " -> "}
      ...
      private def checkSymbolRefsRespectScope(
        ...
        def isOk(sym: Symbol) =
          treeSym hasTransOwner
            sym.enclosingSuchThat(x =>
        ...
        def encls =
          enclosingMemberDefs.filterNot(_.symbol == treeSym).zipWithIndex map {
            case (md, i) =>
              mk(s"encl(${i + 1})", md.symbol, signature)
          }
              ...
              if (tree.tpe eq null)
                errorFn(
                  s"tree.tpe=null for " + tree.shortClass + " (symbol: " +
                    classString(tree.symbol) + " " + signature(tree.symbol) +
                    "), last seen tpe was " + oldtpe)
