...
trait Implicits {
  ...
  object HasMember {
    ...
    def apply(name: Name): Type = hasMemberCache.getOrElseUpdate(
      name,
  ...
  object HasMethodMatching {
    ...
    def templateArgType(argtpe: Type) = new BoundedWildcardType(
      TypeBounds.lower(argtpe))
  ...
  class ImplicitSearch(
    ...
    def undet_s = if (undetParams.isEmpty) ""
    ...
    private def typedImplicit(
      ...
      (context.openImplicits find { case OpenImplicit(info, tp, tree1) =>
    ...
    class ImplicitComputation(iss: Infoss, isLocalToCallsite: Boolean) {
      ...
      /** Returns all eligible ImplicitInfos and their SearchResults in a map.
        */
      def findAll() = linkedMapFrom(eligible)(x =>
  ...
  class Message(sym: Symbol, msg: String, annotationName: String) {
    ...
    def format(paramName: Name, paramTp: Type): String = format(
      typeArgsAtSym(paramTp) map (_.toString))
