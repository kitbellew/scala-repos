...
trait Implicits {
  ...
  def inferImplicit(
    ...
    context.undetparams =
      ((context.undetparams ++ result.undetparams) filterNot
        result.subst.from.contains).distinct
  ...
  def inferImplicit(
    ...
    val result = wrapper(inferImplicit(
      tree,
      pt,
        ...
        case RefinedType(List(WildcardType), decls) => decls.toList match {
            case List(sym) => sym.tpe match {
                case MethodType(params, restpe)
                    if (params forall
                      (_.tpe.isInstanceOf[BoundedWildcardType])) =>
  ...
  class ImplicitSearch(
    ...
    private def dominates(dtor: Type, dted: Type): Boolean = {
      def core(tp: Type): Type =
        tp.dealiasWiden match {
          ...
          case ExistentialType(tparams, result) => core(result)
              .subst(tparams, tparams map (t => core(t.info.bounds.hi)))
          case PolyType(tparams, result) => core(result)
              .subst(tparams, tparams map (t => core(t.info.bounds.hi)))
          case _ => tp
        }
    ...
    private def matchesArgRes(
        ...
        ptarg: Type,
        ptres: Type,
        undet: List[Symbol]): Boolean = (ptarg weak_<:< tparg) && {
      ptres match {
      ...
      try {
        ...
        if (context.reporter.hasErrors)
          ...
          if (matchesPt(itree3.tpe, ptInstantiated, undetParams)) {
            ...
            fail("incompatible: %s does not match expected type %s".format(
              itree3.tpe,
              ptInstantiated))
        }
    ...
    def isValid(sym: Symbol) = {
      ...
      def comesBefore(sym: Symbol, owner: Symbol) = {
        ...
        (if (sym.hasAccessorFlag) {
           ...
           !(owner hasTransOwner
             sym)) // faster than owner.ownerChain contains sym
      }
          ...
          case TypeRef(pre, sym, args) => infoMap get sym match {
              case Some(infos1) =>
                if (infos1.nonEmpty && !(pre =:= infos1.head.pre.prefix)) {
                  log(
                    s"Ignoring implicit members of $pre#$sym as it is also visible via another prefix: ${infos1
                      .head.pre.prefix}")
                  infoMap(sym) =
              ...
              case None =>
                if (pre.isStable && !pre.typeSymbol.isExistentiallyBound) {
                  ...
                  val infos = pre1.implicitMembers.iterator
                    .map(mem => new ImplicitInfo(mem.name, pre1, mem)).toList
    ...
    private def tagOfType(
      ...
      val materializer = atPos(pos.focus)(gen.mkMethodCall(
    ...
    def allImplicits: List[SearchResult] = {
      ...
      (search(context.implicitss, isLocalToCallsite = true) ++
        search(implicitsOfExpectedType, isLocalToCallsite = false)).toList
        .filter(_.tree ne EmptyTree)
    }
  ...
  class Message(sym: Symbol, msg: String, annotationName: String) {
    ...
    private lazy val typeParamNames: List[String] = sym.typeParams
      .map(_.decodedName)
