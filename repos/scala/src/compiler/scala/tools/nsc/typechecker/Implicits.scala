  ...
  class ImplicitInfo(val name: Name, val pre: Type, val sym: Symbol) {
    ...
    override def equals(other: Any) = other match {
      case that: ImplicitInfo =>
        this.name == that.name && this.pre =:= that.pre && this.sym == that.sym
      case _ => false
    }
  ...
  class ImplicitSearch(
    ...
    def failure(
      ...
      if (settings.XlogImplicits)
        reporter.echo(
          pos,
          what + " is not a valid implicit value for " + pt +
            " because:\n" + reason)
      SearchFailure
    }
    ...
    private def typedImplicit1(
      ...
      try {
        ...
        context.reporter.firstError match {
          // using match rather than foreach to avoid non local return.
          case Some(err) =>
    ...
    class ImplicitComputation(iss: Infoss, isLocalToCallsite: Boolean) {
      ...
      private def isIneligible(info: ImplicitInfo) =
        (info.isCyclicOrErroneous || isView && (info.sym eq Predef_conforms) // as an implicit conversion, Predef.$conforms is a no-op, so exclude it
      ...
      if (eligible.nonEmpty)
        printTyping(
          tree,
          eligible.size +
            s" eligible for pt=$pt at ${fullSiteString(context)}")
    ...
    def allImplicits: List[SearchResult] = {
      ...
      (search(context.implicitss, isLocalToCallsite = true) ++ search(
        ...
        isLocalToCallsite = false)).toList
    ...
    def allImplicitsPoly(
      ...
      eligibleInfos(context.implicitss, isLocalToCallsite = true) ++ eligibleInfos(
        implicitsOfExpectedType,
...
object ImplicitsStats {
  ...
  val implicitSearchCount =
    Statistics.newCounter("#implicit searches", "typer")
  ...
  val typedImplicits =
    Statistics.newSubCounter("  #typed", implicitSearchCount)
  val foundImplicits =
    Statistics.newSubCounter("  #found", implicitSearchCount)
  val improvesCount =
  ...
  val inscopeFailNanos =
    Statistics.newSubTimer("  failed in scope", typerNanos)
