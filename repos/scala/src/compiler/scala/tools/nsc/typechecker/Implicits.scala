...
trait Implicits {
  ...
  def inferImplicit(
    ...
    val result =
      new ImplicitSearch(tree, pt, isView, implicitSearchContext, pos)
        .bestImplicit
    ...
    context.undetparams =
      (
        (context.undetparams ++ result.undetparams) filterNot
          result.subst.from.contains
      ).distinct
  ...
  private def isInvalidConversionSource(tpe: Type): Boolean =
    tpe match {
      case Function1(in, _) =>
        in <:< NullClass.tpe
      case _ =>
  ...
  class ImplicitInfo(val name: Name, val pre: Type, val sym: Symbol) {
    ...
    def isCyclicOrErroneous: Boolean = {
      if (!isCyclicOrErroneousCache.isKnown)
        isCyclicOrErroneousCache = computeIsCyclicOrErroneous
    ...
    override def equals(other: Any) =
      other match {
        case that: ImplicitInfo =>
          this.name == that.name && this.pre =:= that.pre &&
            this.sym == that.sym
        case _ =>
          false
      }
  ...
  def memberWildcardType(name: Name, tp: Type) = {
    ...
    name match {
      case x: TermName =>
        result.typeSymbol.newMethod(x) setInfoAndEnter tp
      case x: TypeName =>
  ...
  object HasMethodMatching {
    val dummyMethod = NoSymbol.newTermSymbol(TermName("typer$dummy")) setInfo
      NullaryMethodType(AnyTpe)
  ...
  object Function1 {
    ...
    def unapply(tp: Type) =
      tp match {
        case TypeRef(_, Sym, arg1 :: arg2 :: _) =>
          Some((arg1, arg2))
        case _ =>
          None
      }
  ...
  class ImplicitSearch(
      ...
      def complexity(tp: Type): Int =
        tp.dealias match {
          ...
          case RefinedType(parents, _) =>
            (parents map complexity).sum + 1
          case _ =>
            1
        }
      def overlaps(tp1: Type, tp2: Type): Boolean =
        (tp1, tp2) match {
          case (RefinedType(parents, _), _) =>
            parents exists (overlaps(_, tp2))
          case (_, RefinedType(parents, _)) =>
            parents exists (overlaps(tp1, _))
          case _ =>
            tp1.typeSymbol == tp2.typeSymbol
        }
    ...
    private def typedImplicit(
      ...
      (
        ...
        case None =>
          try {
            context.openImplicits = OpenImplicit(info, pt, tree) ::
        ...
        ptres match {
          case HasMethodMatching(name, argtpes, restpe) =>
            (tpres.member(name) filter
              (m => isApplicableSafe(undet, m.tpe, argtpes, restpe))) !=
              NoSymbol
          case _ =>
            tpres <:< ptres
        }
              ...
              pt match {
                case tr @ TypeRef(pre, sym, args) =>
                  if (sym.isAliasType)
                    ...
                    hasLength(params, len) && sym == FunctionClass(len) && {
                      var ps = params
                      var as = args
                      if (fast) {
    ...
    /** This expresses more cleanly in the negative: there's a linear path
      ...
      */
    private def isPlausiblySubType(tp1: Type, tp2: Type) =
      !isImpossibleSubType(tp1, tp2)
    ...
    private def typedImplicit1(
      ...
      try {
        ...
        context
          .reporter
          .firstError match { // using match rather than foreach to avoid non local return.
          case Some(err) =>
        ...
        def hasMatchingSymbol(tree: Tree): Boolean =
          (tree.symbol == info.sym) || {
            tree match {
              case Apply(fun, _) =>
                hasMatchingSymbol(fun)
              case TypeApply(fun, _) =>
                hasMatchingSymbol(fun)
              case Select(pre, nme.apply) =>
                pre.symbol == info.sym
              case _ =>
                false
            }
        ...
        if (context.reporter.hasErrors)
          fail(
            "hasMatchingSymbol reported error: " +
              context.reporter.firstError.get.errMsg)
          ...
          if (matchesPt(itree3.tpe, ptInstantiated, undetParams)) {
            ...
            context.reporter.firstError match {
              case Some(err) =>
                return fail(
                  "type parameters weren't correctly instantiated outside of the implicit tree: " +
                    err.errMsg)
              case None =>
            }
    ...
    def isValid(sym: Symbol) = {
      def hasExplicitResultType(sym: Symbol) = {
        ...
        sym.rawInfo match {
          case tc: TypeCompleter =>
            hasExplicitRT(tc.tree)
          case PolyType(_, tc: TypeCompleter) =>
            hasExplicitRT(tc.tree)
          case _ =>
            true
        }
      ...
      hasExplicitResultType(sym) || comesBefore(sym, context.owner)
    }
    ...
    class ImplicitComputation(iss: Infoss, isLocalToCallsite: Boolean) {
      ...
      private def isIneligible(info: ImplicitInfo) =
        (info.isCyclicOrErroneous ||
          isView &&
          (info.sym eq
            Predef_conforms) // as an implicit conversion, Predef.$conforms is a no-op, so exclude it
          ||
        ...
        def apply(
          ...
          if (search.isDivergent && divergentError.isEmpty) {
            ...
            if (search.isFailure) {
              ...
              context
                .reporter
                .retainDivergentErrorsExcept(divergentError.getOrElse(null))
            }
            search
          }
        }
      ...
      val eligible = {
        val matches = iss flatMap { is =>
          val result = is filter
            (info => checkValid(info.sym) && survives(info))
          ...
          case firstPending :: otherPending =>
            ...
            DivergentImplicitRecovery(
              ...
              case sr if sr.isFailure =>
                rankImplicits(otherPending, acc)
              case newBest =>
                best =
      ...
      def success(arg: Tree) = {
        ...
        try {
          ...
          context.reporter.firstError match {
            ...
            case None =>
              new SearchResult(tree1, EmptyTreeTypeSubstituter, Nil)
          }
        } catch {
          case ex: TypeError =>
    ...
    private def manifestOfType(tp: Type, flavor: Symbol): SearchResult = {
      ...
      def manifestFactoryCall(
        ...
        if (args contains EmptyTree)
          ...
          typedPos(tree.pos.focus) {
            val mani = gen
              .mkManifestFactoryCall(full, constructor, tparg, args.toList)
            if (settings.debug)
      ...
      def findSingletonManifest(name: String) =
        typedPos(tree.pos.focus) {
      ...
      if (full) {
        ...
        if (tagInScope.isEmpty)
          ...
          if (resolveClassTag(pos, tp, allowMaterialization = true) ==
                EmptyTree) {
            throw new TypeError(
              pos,
              sm"""to create a manifest here, it is necessary to interoperate with the type tag `$tagInScope` in scope.
    ...
    private def materializeImplicit(pt: Type): SearchResult =
      pt match {
        ...
        case pt @ TypeRef(pre, sym, arg :: Nil) =>
          sym match {
            case sym if ManifestSymbols(sym) =>
              manifestOfType(arg, sym)
            case sym if TagSymbols(sym) =>
            ...
            case sym if sym.isAliasType =>
              materializeImplicit(pt.betaReduce)
            case _ =>
              SearchFailure
          }
        case _ =>
    ...
    def bestImplicit: SearchResult = {
      ...
      if (result.isFailure &&
          settings.debug) // debuglog is not inlined for some reason
        log(
  ...
  class ImplicitAnnotationMsg(
      f: Symbol => Option[String],
      clazz: Symbol,
      annotationName: String) {
    def unapply(sym: Symbol): Option[(Message)] =
      f(sym) match {
        case Some(m) =>
          Some(new Message(sym, m, annotationName))
    ...
    def check(sym: Symbol): Option[String] =
      sym
        ...
        .flatMap(
          _.stringArg(0) match {
            case Some(m) =>
              new Message(sym, m, annotationName).validate
            case None =>
  ...
  object ImplicitNotFoundMsg
      extends ImplicitAnnotationMsg(
        _.implicitNotFoundMsg,
        ImplicitNotFoundClass,
        "implicitNotFound")

  object ImplicitAmbiguousMsg
      extends ImplicitAnnotationMsg(
        _.implicitAmbiguousMsg,
...
object ImplicitsStats {
  ...
  val rawTypeImpl = Statistics
    .newSubCounter("  of which in implicits", rawTypeCount)
  val subtypeImpl = Statistics
    .newSubCounter("  of which in implicit", subtypeCount)
  val findMemberImpl = Statistics
    .newSubCounter("  of which in implicit", findMemberCount)
  val subtypeAppInfos = Statistics
    .newSubCounter("  of which in app impl", subtypeCount)
  val implicitSearchCount = Statistics.newCounter("#implicit searches", "typer")
  val plausiblyCompatibleImplicits = Statistics
    .newSubCounter("  #plausibly compatible", implicitSearchCount)
  val matchingImplicits = Statistics
    .newSubCounter("  #matching", implicitSearchCount)
  ...
  val improvesCount = Statistics
    .newSubCounter("implicit improves tests", implicitSearchCount)
  val improvesCachedCount = Statistics
    .newSubCounter("#implicit improves cached ", implicitSearchCount)
  val inscopeImplicitHits = Statistics
    .newSubCounter("#implicit inscope hits", implicitSearchCount)
  val oftypeImplicitHits = Statistics
    .newSubCounter("#implicit oftype hits ", implicitSearchCount)
  val implicitNanos = Statistics
    .newSubTimer("time spent in implicits", typerNanos)
  val inscopeSucceedNanos = Statistics
    .newSubTimer("  successful in scope", typerNanos)
  val inscopeFailNanos = Statistics.newSubTimer("  failed in scope", typerNanos)
  val oftypeSucceedNanos = Statistics
    .newSubTimer("  successful of type", typerNanos)
  ...
  val implicitCacheAccs = Statistics
    .newCounter("implicit cache accesses", "typer")
  val implicitCacheHits = Statistics
