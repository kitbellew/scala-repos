...
abstract class ClassfileParser {
  ...
  def parseClass() {
    ...
    def parseParents: List[Type] = {
      if (isScala) {
        ...
        raiseLoaderLevel {
          ...
          if (jflags.isAnnotation)
            ifaces ::= ClassfileAnnotationClass.tpe
          superType :: ifaces
        }
    }
    ...
    if (isTopLevel) {
      if (c != clazz) {
        if ((clazz eq NoSymbol) && (c ne NoSymbol))
          clazz = c
        else
          mismatchError(c)
      }
  ...
  def parseMethod() {
    ...
    if (jflags.isPrivate && !optimized) {
      ...
      if ((sflags & PRIVATE) != 0L &&
          optimized) { // TODO this should be !optimized, no? See c4181f656d.
        in.skip(4);
        skipAttributes()
      } else {
        val name = readName()
            ...
            tp match {
              case TypeRef(pre, classSym, args) =>
                ...
                // or we'll create a boatload of needless existentials.
                         ...
                         classSym.unsafeTypeParams.isEmpty)
                  tp
                else
      ...
      attrName match {
        ...
        case tpnme.ConstantValueATTR =>
          ...
          if (c1 ne null)
            ...
            devWarning(s"failure to convert $c to $symtype")
      ...
      tag match {
        ...
        case ARRAY_TAG =>
          ...
          if (hasError)
            ...
            Some(ArrayAnnotArg(arr.toArray))
        case ANNOTATION_TAG =>
    ...
    def parseAnnotations(len: Int): Option[AnnotationInfo] = {
      ...
      for (n <- 0 until nAttr)
        parseAnnotation(u2) match {
          case Some(scalaSig)
              if (scalaSig.atp == ScalaSignatureAnnotation.tpe) =>
  ...
  case class InnerClassEntry(
    ...
    private def enclClass =
      if (isInner)
        innerClasses innerSymbol strippedOuter
      else
        classNameToSymbol(strippedOuter)
