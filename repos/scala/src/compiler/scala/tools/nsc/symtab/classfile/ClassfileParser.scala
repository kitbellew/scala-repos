...
abstract class ClassfileParser {
  ...
  protected class ConstantPool {
    ...
    def firstExpecting(index: Int, expected: Int): Int = {
      ...
      if (first == expected) start + 1 else this errorBadTag start
    }
  ...
  def parseField() {
    ...
    if ((sflags & PRIVATE) != 0L && !optimized) { in.skip(4); skipAttributes() }
      ...
      sym setInfo { if (jflags.isEnum) ConstantType(Constant(sym)) else info }
    ...
    def sig2type(
      ...
      tag match {
            ...
            tp match {
              case TypeRef(pre, classSym, args) =>
                ...
                if (sig.charAt(index) == '<') {
                  ...
                  debuglogResult("new existential")(newExistentialType(
                    existentials.toList,
        ...
        case 'T' =>
          ...
          index += 1
          if (skiptvs) AnyTpe else tparams(n).typeConstructor
      }
    } // sig2type(tparams, skiptvs)
          ...
          def readParamNames(): Unit = {
            ...
            while (i < paramCount) {
              ...
              if ((
                  ...
                  ) != ACC_SYNTHETIC) { // name not synthetic
                val params =
