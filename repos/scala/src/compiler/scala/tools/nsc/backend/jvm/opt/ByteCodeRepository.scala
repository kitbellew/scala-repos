...
class ByteCodeRepository[BT <: BTypes](
    val classPath: ClassFileLookup[AbstractFile],
    val btypes: BT) {
  import btypes._
  ...
  /**
    ...
    */
  val parsedClasses
      : concurrent.Map[InternalName, Either[ClassNotFound, (ClassNode, Long)]] =
    recordPerRunCache(concurrent.TrieMap.empty)
  ...
  def methodNode(
    ...
    def findInParents(
        parents: List[InternalName],
        failedClasses: List[ClassNotFound])
        : Either[List[ClassNotFound], (MethodNode, InternalName)] =
      parents match {
        case x :: xs =>
    ...
    if (ownerInternalNameOrArrayDescriptor.charAt(0) == '[')
      Left(
        MethodNotFound(name, descriptor, ownerInternalNameOrArrayDescriptor, Nil))
    else
