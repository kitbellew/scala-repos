      ...
      callGraph.callsites(methodNode).valuesIterator foreach {
        case callsite @ Callsite(
          ...
          inlineRequest(callsite) match {
            ...
            case Some(Left(w)) =>
              if ((calleeAnnotatedInline && bTypes.compilerSettings.YoptWarningEmitAtInlineFailed) || w.emitWarning(
                    compilerSettings)) {
                val annotWarn =
                  if (calleeAnnotatedInline) " is annotated @inline but" else ""
                val msg =
            ...
            case None =>
              if (canInlineFromSource && calleeAnnotatedInline && !callsite.annotatedNoInline && bTypes.compilerSettings.YoptWarningEmitAtInlineFailed) {
                ...
                def warnMsg =
                  callsiteWarning.map(" Possible reason:\n" + _).getOrElse("")
              ...
              } else if (callsiteWarning.isDefined && callsiteWarning.get.emitWarning(
                           compilerSettings)) {
