...
//    rewriteFinalTraitMethodInvocations()
    ...
    for (request <- collectAndOrderInlineRequests) {
      val Right(callee) =
        request.callsite
          .callee // collectAndOrderInlineRequests returns callsites with a known callee
  ...
  def rewriteFinalTraitMethodInvocation(callsite: Callsite): Unit = {
    ...
    val selfParamTypeV: Either[OptimizerWarning, ClassBType] =
      calleeDeclarationClass.info
        .map(_.inlineInfo.traitImplClassSelfType match {
    ...
    val res = for {
      ...
      if (!selfTypeOk) {
        ...
        val receiverValue = analyzer.frameAt(callsite.callsiteInstruction)
    ...
    for (warning <- res.left) {
      ...
      val newCallee = callee
        .copy(calleeInfoWarning = Some(RewriteTraitCallToStaticImplMethodFailed(
          ...
          check match {
            case x :: xs =>
              if (x == goal) true
                ...
                val callees = nonElidedRequests(x)
                  .map(_.callsite.callee.get.callee)
  ...
  def adaptPostRequestForMainCallsite(
    ...
    def impl(post: InlineRequest, at: Callsite): List[InlineRequest] = {
      post.callsite.inlinedClones.find(_.clonedWhenInlining == at) match {
        ...
        case None => post.post.flatMap(impl(_, post.callsite))
            .flatMap(impl(_, at))
      }
    }
  ...
  def inline(request: InlineRequest): List[CannotInlineWarning] =
    canInlineBody(request.callsite) match {
      ...
      case None =>
        ...
        val postRequests = request.post
          .flatMap(adaptPostRequestForMainCallsite(_, request.callsite))
  ...
  def inlineCallsite(callsite: Callsite): Unit = {
    ...
    localOpt
      .minimalRemoveUnreachableCode(callee, calleeDeclarationClass.internalName)
    ...
    for (argTp <- calleeParamTypes) {
      val opc = argTp
        .getOpcode(ISTORE) // returns the correct xSTORE instruction for argTp
      argStores
        .insert(
  ...
  def canInlineBody(callsite: Callsite): Option[CannotInlineWarning] = {
    ...
    assert(
      !isConstructor(callee),
      s"Constructors cannot be inlined: $calleeDesc")
    assert(
      !BytecodeUtils.isAbstractMethod(callee),
      s"Callee is abstract: $calleeDesc")
    assert(
      callsiteMethod.instructions.contains(callsiteInstruction),
      s"Callsite ${textify(callsiteInstruction)} is not an instruction of $calleeDesc")
      ...
      instruction match {
        ...
        case mi: MethodInsnNode =>
          if (mi.owner.charAt(0) == '[')
            Right(true) // array methods are accessible
          else {
            def canInlineCall(
                opcode: Int,
                methodFlags: Int,
