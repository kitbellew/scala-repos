...
class CoreBTypes[BTFS <: BTypesFromSymbols[_ <: Global]](val bTypes: BTFS) {
  ...
  lazy val srBoxesRuntimeBoxToMethods: Map[BType, MethodNameAndType] =
  ...
  private def staticRefMethods(
    ...
    allRefClasses.map(refClass =>
      (
        ...
        methodNameAndType(refClass, name, static = true)))(collection.breakOut)
  }

  // scala/runtime/BooleanRef -> MethodNameAndType(create,(Z)Lscala/runtime/BooleanRef;)
  lazy val srRefCreateMethods: Map[InternalName, MethodNameAndType] =
    staticRefMethods(nme.create)

  // scala/runtime/BooleanRef -> MethodNameAndType(zero,()Lscala/runtime/BooleanRef;)
  lazy val srRefZeroMethods: Map[InternalName, MethodNameAndType] =
  ...
  lazy val srRefConstructors: Map[InternalName, MethodNameAndType] =
  ...
  lazy val tupleClassConstructors: Map[InternalName, MethodNameAndType] = {
    val tupleClassSymbols = TupleClass
      .seq ++ specializedSubclasses(TupleClass(1)) ++
  ...
  lazy val srJFunctionRefs: Set[InternalName] = functionClasses(
    "scala.runtime.java8.JFunction").map(classBTypeFromSymbol(_).internalName)
...
/**
  ...
  */
final class CoreBTypesProxy[BTFS <: BTypesFromSymbols[_ <: Global]](
    val bTypes: BTFS)
    extends CoreBTypesProxyGlobalIndependent[BTFS] {
  import bTypes._
  import global._
  ...
  def primitiveTypeToBType: Map[Symbol, PrimitiveBType] =
    _coreBTypes.primitiveTypeToBType

  def boxedClasses: Set[ClassBType] = _coreBTypes.boxedClasses
  def boxedClassOfPrimitive: Map[PrimitiveBType, ClassBType] =
    _coreBTypes.boxedClassOfPrimitive
  ...
  def jliMethodHandlesLookupRef: ClassBType =
    _coreBTypes.jliMethodHandlesLookupRef
  ...
  def srBoxesRuntimeBoxToMethods: Map[BType, MethodNameAndType] =
    _coreBTypes.srBoxesRuntimeBoxToMethods
  def srBoxesRuntimeUnboxToMethods: Map[BType, MethodNameAndType] =
    _coreBTypes.srBoxesRuntimeUnboxToMethods

  def javaBoxMethods: Map[InternalName, MethodNameAndType] =
    _coreBTypes.javaBoxMethods
  def javaUnboxMethods: Map[InternalName, MethodNameAndType] =
    _coreBTypes.javaUnboxMethods

  def predefAutoBoxMethods: Map[String, MethodBType] =
    _coreBTypes.predefAutoBoxMethods
  def predefAutoUnboxMethods: Map[String, MethodBType] =
    _coreBTypes.predefAutoUnboxMethods

  def srRefCreateMethods: Map[InternalName, MethodNameAndType] =
    _coreBTypes.srRefCreateMethods
  def srRefZeroMethods: Map[InternalName, MethodNameAndType] =
    _coreBTypes.srRefZeroMethods

  def primitiveBoxConstructors: Map[InternalName, MethodNameAndType] =
    _coreBTypes.primitiveBoxConstructors
  def srRefConstructors: Map[InternalName, MethodNameAndType] =
    _coreBTypes.srRefConstructors
  def tupleClassConstructors: Map[InternalName, MethodNameAndType] =
    _coreBTypes.tupleClassConstructors
  ...
  def AndroidParcelableInterface: Symbol =
    _coreBTypes.AndroidParcelableInterface
  ...
  def lambdaMetaFactoryBootstrapHandle =
    _coreBTypes.lambdaMetaFactoryBootstrapHandle
  def lambdaDeserializeBootstrapHandle =
    _coreBTypes.lambdaDeserializeBootstrapHandle
}
