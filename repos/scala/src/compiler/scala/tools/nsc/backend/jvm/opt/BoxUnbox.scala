...
class BoxUnbox[BT <: BTypes](val btypes: BT) {
  ...
  def boxUnboxElimination(method: MethodNode, owner: InternalName): Boolean = {
    AsmAnalyzer.sizeOKForSourceValue(method) && {
      ...
      def replaceBoxOperationsSingleCreation(
        ...
        if (canRewrite) {
          val localSlots: Vector[(Int, Type)] = boxKind.boxedTypes
            ...
            for (extraction <- allConsumers) {
              ...
              val replacementOps =
                if (valueIndex == 0) {
                  ...
                  val consumeStack = boxKind.boxedTypes
  ...
  def allCreationsConsumers(
    ...
    def addCreations(boxConsumer: BoxConsumer): Boolean = {
      val newProds = boxConsumer.boxProducers(prodCons)
    ...
    def addBoxConsumers(creation: BoxCreation): Boolean = {
      val newCons = creation.boxConsumers(prodCons, ultimate = true)
            ...
            if (receiverProds == Set(dupOp)) {
              ...
              if (initializedInstanceCons == dupConsWithoutInit &&
                  prodCons.producersForValueAt(afterInit, stackTopAfterInit) ==
                    Set(dupOp)) { return Some((dupOp, initCall)) }
            }
          }
        }
      }
      ...
      insn match {
        case mi: MethodInsnNode =>
          if ((isScalaUnbox(mi) || isJavaUnbox(mi)) && typeOK(mi))
            ...
            BoxKind.checkReceiverPredefLoad(mi, prodCons)
  ...
  object Tuple {
    ...
    private val specializedTupleClassR = "scala/Tuple[12]\\$mc[IJDCZ]{1,2}\\$sp"
      .r
