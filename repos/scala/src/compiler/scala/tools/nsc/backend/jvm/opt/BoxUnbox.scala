...
class BoxUnbox[BT <: BTypes](val btypes: BT) {
  ...
  def boxUnboxElimination(method: MethodNode, owner: InternalName): Boolean = {
    AsmAnalyzer.sizeOKForSourceValue(method) && {
      val toInsertBefore = mutable.Map
      ...
      def replaceBoxOperationsSingleCreation(
          creation: BoxCreation,
        ...
        def copyOpsToEliminate: Option[Set[AbstractInsnNode]] = {
          ...
          while (replaceOK && copyOps.hasNext)
            copyOps.next() match {
              case vi: VarInsnNode =>
                elidableCopyOps += vi
        ...
        if (canRewrite) {
          val localSlots: Vector[(Int, Type)] =
            boxKind.boxedTypes
              .map(tp => (getLocal(tp.getSize), tp))(collection.breakOut)
          ...
          val storeInitialValues =
            creation.loadInitialValues match {
              case Some(ops) =>
                ops ::: storeOps
              case None =>
                storeOps
          ...
          finalCons foreach {
            ...
            case extraction =>
              ...
              if (keepBox)
                ...
                toReplace(extraction.consumer) = loadOps
      ...
      def replaceBoxOperationsMultipleCreations(
          allCreations: Set[BoxCreation],
        ...
        def updateLocalVariableTypes(reTypedLocals: Map[Int, Type]): Unit = {
          lazy val localsByIndex = method.localVariables.asScala
            .groupBy(_.index)
            ...
            case Some((boxCreation, boxKind)) =>
              allCreationsConsumers(boxCreation, boxKind, prodCons) match {
                case Some((allCreations, allConsumers)) =>
                  val (escapingConsumers, boxConsumers) = allConsumers
                    .partition(_.isEscaping)
                  if (boxConsumers.nonEmpty) {
                    for (c <- allCreations)
                      knownHandled ++= c.allInsns
                    for (e <- allConsumers)
                      knownHandled ++= e.allInsns
  ...
  def allCreationsConsumers(
    ...
    if (addBoxConsumers(initialCreation))
      Some((creations, consumers))
    else
      None
  }
  ...
  /**
    ...
    */
  def checkCopyOpReplacements(
      initialProds: Set[BoxCreation],
      finalCons: Set[BoxConsumer],
    ...
    if (replaceOK)
      Some((replacements, nextCopyOpLocal, reTypedLocals))
    else
      None
  }
  ...
  /**
    ...
    */
  class CopyOpsIterator(
      initialCreations: Set[BoxCreation],
      finalCons: Set[BoxConsumer],
      prodCons: ProdConsAnalyzer)
        ...
        if (prodCons.producersForInputsOf(dupOp) == Set(newOp)) {
          ...
          if (initCalls.size == 1) {
            ...
            if (receiverProds == Set(dupOp)) {
              ...
              val initializedInstanceCons = prodCons.consumersOfValueAt(
                afterInit,
                stackTopAfterInit)
  ...
  object PrimitiveBox {
    ...
    def checkPrimitiveBox(
      ...
      def checkKind(mi: MethodInsnNode) =
        expectedKind match {
          case Some(kind) =>
            if (kind.boxClass == boxClass(mi))
              expectedKind
            else
              None
          case None =>
            Some(PrimitiveBox(boxedType(mi), boxClass(mi)))
        }
  ...
  object Ref {
    ...
    def checkRefCreation(
      ...
      def checkKind(mi: MethodInsnNode): Option[Ref] =
        expectedKind match {
          case Some(kind) =>
            if (kind.refClass == refClass(mi))
              expectedKind
            else
              None
          case None =>
            Some(Ref(boxedType(mi), refClass(mi)))
        }
      ...
      insn match {
        case fi: FieldInsnNode
          ...
          if (fi.getOpcode == GETFIELD)
            Some(StaticGetterOrInstanceRead(fi))
  ...
  case class Tuple(boxedTypes: List[Type], tupleClass: InternalName)
    ...
    def extractedValueIndex(extraction: BoxConsumer): Int =
      extraction match {
        ...
        case PrimitiveBoxingGetter(mi) =>
          tupleGetterIndex(mi.name)
        case PrimitiveUnboxingGetter(mi, _) =>
          tupleGetterIndex(mi.name)
        case _ =>
  ...
  object Tuple {
    ...
    def checkTupleCreation(
      ...
      def checkKind(mi: MethodInsnNode): Option[Tuple] =
        expectedKind match {
          case Some(kind) =>
            if (kind.tupleClass == tupleClass(mi))
              expectedKind
            else
              None
          case None =>
            Some(Tuple(boxedTypes(mi), tupleClass(mi)))
        }
  ...
  sealed trait BoxCreation {
    ...
    def valuesConsumer: MethodInsnNode =
      this match {
        case StaticFactory(call, _) =>
          call
        case ModuleFactory(_, call) =>
          call
        case InstanceCreation(_, _, initCall) =>
          initCall
      }

    def allInsns: Set[AbstractInsnNode] =
      this match {
        case StaticFactory(c, _) =>
          Set(c)
        case ModuleFactory(m, c) =>
          Set(m, c)
        case InstanceCreation(n, d, i) =>
          Set(n, d, i)
      }
    ...
    def boxConsumers(
      ...
      val startInsn =
        this match {
          ...
          case InstanceCreation(_, dupOp, _) =>
            dupOp
          case _ =>
            producer
        }
      val cons =
        if (ultimate)
          prodCons.ultimateConsumersOfOutputsFrom(startInsn)
        else
          prodCons.consumersOfOutputsFrom(startInsn)
      this match {
        case InstanceCreation(_, _, initCall) =>
          cons - initCall
        case _ =>
  ...
  sealed trait BoxConsumer {
    ...
    def isWrite =
      this match {
        case _: StaticSetterOrInstanceWrite =>
          true
        case _ =>
          false
      }
    ...
    def postExtractionAdaptationOps(
      ...
      this match {
        case PrimitiveBoxingGetter(_) =>
          List(getScalaBox(typeOfExtractedValue))
  ...
  case class PrimitiveUnboxingGetter(
