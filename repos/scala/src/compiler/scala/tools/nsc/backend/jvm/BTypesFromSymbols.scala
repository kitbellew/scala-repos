...
class BTypesFromSymbols[G <: Global](val global: G) extends BTypes {
  ...
  val byteCodeRepository: ByteCodeRepository[this.type] =
    new ByteCodeRepository(global.classPath, this)
  ...
  def staticHandleFromSymbol(sym: Symbol): asm.Handle = {
    ...
    val descriptor = methodBTypeFromMethodType(sym.info, isConstructor = false)
      .descriptor
    new asm.Handle(
      ...
      case tp =>
        ...
        tp match {
          ...
          case RefinedType(parents, _) =>
            parents.map(typeToBType(_).asClassBType)
  ...
  def implementedInterfaces(classSym: Symbol): List[Symbol] = {
    ...
    def isInterfaceOrTrait(sym: Symbol) =
      sym.isInterface || sym.isTrait || sym.hasJavaAnnotationFlag
    ...
    val allParents = classParents ++
      classSym.annotations.flatMap(newParentForAnnotation)
    ...
    def javaClassfileFlags(classSym: Symbol): Int = {
      ...
      def enumFlags =
        ACC_ENUM | {
          ...
          val hasAbstractMethod = classSym.info.decls
        ...
        if (considerAsTopLevelImplementationArtifact(classSym)) Nil
          ...
          val valueClassCompanionMembers = {
            if (linkedClass != NoSymbol &&
                exitingPickler(classSym.isDerivedValueClass)) {
    ...
    if (isTopLevel || considerAsTopLevelImplementationArtifact(innerClassSym))
      ...
      val enclosingClass = {
        ...
        if ((innerClassSym.isJavaDefined &&
            ...
            isTopLevelModuleClass(innerClassSym.rawowner))) { // (2)
      ...
      val innerName: Option[String] = {
        ...
        if (exitingPickler(
              innerClassSym.isAnonymousClass ||
  ...
  def mirrorClassClassBType(moduleClassSym: Symbol): ClassBType = {
    ...
    classBTypeFromInternalName.getOrElse(
      internalName, {
        ...
        val nested =
          exitingPickler(memberClassesForInnerClassTable(moduleClassSym)) map
            classBTypeFromSymbol
        c.info = Right(ClassInfo(
          superClass = Some(ObjectRef),
          interfaces = Nil,
          flags = asm.Opcodes.ACC_SUPER | asm.Opcodes.ACC_PUBLIC |
  ...
  final def javaFlags(sym: Symbol): Int = {
    ...
    GenBCode.mkFlags(
      ...
      if ((sym.isDeferred && !sym.hasFlag(symtab.Flags.JAVA_DEFAULTMETHOD)) ||
