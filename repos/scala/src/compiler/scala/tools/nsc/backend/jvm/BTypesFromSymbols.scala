...
class BTypesFromSymbols[G <: Global](val global: G) extends BTypes {
  ...
  def implementedInterfaces(classSym: Symbol): List[Symbol] = {
    ...
    val allParents = classParents ++ classSym.annotations
  ...
  private def buildNestedInfo(innerClassSym: Symbol): Option[NestedInfo] = {
    ...
    if (isTopLevel || considerAsTopLevelImplementationArtifact(innerClassSym))
      ...
      val enclosingClass = {
        ...
        if ((innerClassSym.isJavaDefined && innerClassSym.rawowner
              .isModuleClass) || // (1)
    ...
    classBTypeFromInternalName.getOrElse(
      internalName, {
        ...
        c.info = Right(ClassInfo(
          ...
          interfaces = Nil,
          flags = asm.Opcodes.ACC_SUPER | asm.Opcodes.ACC_PUBLIC | asm.Opcodes
            .ACC_FINAL,
  ...
  final def javaFlags(sym: Symbol): Int = {
    ...
    val finalFlag = ((((sym.rawflags & symtab.Flags
      .FINAL) != 0) || isTopLevelModuleClass(sym))
    ...
    GenBCode.mkFlags(
      ...
      if ((sym.isDeferred && !sym
            .hasFlag(symtab.Flags.JAVA_DEFAULTMETHOD)) || sym.hasAbstractFlag)
        ACC_ABSTRACT
      else 0,
