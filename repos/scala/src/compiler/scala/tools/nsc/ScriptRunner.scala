...
class ScriptRunner extends HasCompileSocket {
  ...
  private def compileWithDaemon(
    ...
    val compSettingNames = new Settings(sys.error).visibleSettings.toList map
      (_.name)
    val compSettings = settings.visibleSettings.toList filter
      (compSettingNames contains _.name)
    val coreCompArgs = compSettings flatMap (_.unparse)
    val compArgs = coreCompArgs ++
