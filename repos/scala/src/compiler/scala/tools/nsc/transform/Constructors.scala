...
abstract class Constructors extends Statics with Transform with ast.TreeDSL {
  ...
  private trait OmittablesHelper {
    def computeOmittableAccessors(
          ...
          if (omittables.nonEmpty) {
            ...
            tree match {
              ...
              case _: Select if omittables(sym) =>
                ...
                bodyOfOuterAccessor get sym foreach
                  traverse // recurse to mark as needed the field supporting the outer-accessor-method
                super.traverse(tree)
              case _ =>
                super.traverse(tree)
            }
      ...
      if (omittables.nonEmpty)
        (defs.iterator ++ auxConstructors.iterator) foreach
          detectUsages.traverse
  ...
  private trait DelayedInitHelper extends ConstructorTransformerBase {
    ...
    private def delayedInitClosure(
      ...
      val satelliteClass = localTyper.typed {
        atPos(impl.pos) {
          ...
          closureClass setInfoAndEnter
            new ClassInfoType(closureParents, newScope, closureClass)
          ...
          val applyMethod: MethodSymbol =
            (closureClass newMethod
              (nme.apply, impl.pos, FINAL) setInfoAndEnter
              MethodType(Nil, ObjectTpe))
    ...
    def delayedInitDefsAndConstrStats(
      ...
      val hookCallerClass = (new ConstructorTransformer(unit)) transform
        delayedInitClosure(delayedHookSym)
      val delayedInitCall =
        localTyper.typedPos(impl.pos) {
          gen.mkMethodCall(
            This(clazz),
            delayedInitMethod,
            Nil,
    ...
    private def mergeConstructors(
      ...
      for (s <- originalStats;
        ...
        yield {
          ...
          if (stat1 eq stat) {
            assert(
              ctorParams(genericClazz).length == primaryConstrParams.length)
...
//        println("residual specialized constructor statements: " + specBuf)
    ...
    def guardSpecializedInitializer(stats: List[Tree]): List[Tree] =
      if (settings.nospecialization.value)
        ...
        if (guardSpecializedFieldInit && usesSpecializedField &&
            stats.nonEmpty) {
    ...
    object intoConstructor extends Transformer {
        ...
        tree match {
          case Apply(Select(This(_), _), List()) =>
            ...
            if (clazz.isTrait)
              ...
              gen.mkAttributedIdent(parameter(tree.symbol.accessed)) setPos
                tree.pos
      ...
      def apply(oldOwner: Symbol, newOwner: Symbol)(tree: Tree) =
        if (tree eq EmptyTree)
          ...
          transform(tree.changeOwner(oldOwner -> newOwner))
    }
      ...
      for (stat <- stats) {
        ...
        def moveEffectToCtor(
          ...
          if (initializingRhs ne EmptyTree) {
            val initPhase =
              if (mods hasFlag STATIC)
                classInitStatBuf
              else if (mods hasFlag PRESUPER | PARAMACCESSOR)
                constrPrefixBuf
              else
                constrStatBuf
        ...
        stat match {
          ...
          case _ =>
            constrStatBuf +=
              intoConstructor(impl.symbol, primaryConstr.symbol)(stat)
        }
      }
    ...
    def transformed = {
      ...
      val omittableAccessor: Set[Symbol] =
        if (isDelayedInitSubclass)
          ...
          computeOmittableAccessors(clazz, defs, auxConstructors)
      ...
      val paramInits = paramAccessors filterNot omittableSym map { acc =>
        ...
        val conflict = clazz.info.nonPrivateMember(acc.name) filter
      ...
      val primaryConstructor =
        deriveDefDef(primaryConstr)(_ => {
          treeCopy.Block(
            primaryConstrBody,
            paramInits ::: constructorPrefix ::: uptoSuperStats :::
      ...
      if (classInitStats.isEmpty)
        deriveTemplate(impl)(_ => prunedStats)
      else {
        val staticCtor =
