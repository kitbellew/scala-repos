  ...
  class CleanUpTransformer(unit: CompilationUnit) extends StaticsTransformer {
    ...
    def transformApplyDynamic(ad: ApplyDynamic) = {
      ...
      def getPrimitiveReplacementForStructuralCall(
        ...
        val methodName = (if (params.isEmpty)
          ...
          val (mparams, resType) =
            ad.symbol.tpe match {
              ...
              case tpe @ OverloadedType(pre, alts) =>
                ...
                alts filter
                  (_.paramss.flatten.size == params.length) map
                  (_.tpe) match {
      ...
      tree match {
        ...
        case Apply(
              ...
              List(
                ...
                _))
            if wrapRefArrayMeth
              .symbol == currentRun.runDefinitions.Predef_wrapRefArray &&
