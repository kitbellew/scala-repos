    ...
    def transformApplyDynamic(ad: ApplyDynamic) = {
          ...
          BLOCK(
            ...
            IF(REF(methodSym) OBJ_NE NULL).THEN(Return(REF(methodSym)))
              ELSE {
                ...
                BLOCK(
                  REF(methodSym) === (REF(
                    currentRun.runDefinitions.ensureAccessibleMethod) APPLY (
                    methodSymRHS
                  )),
                  cacheAdd,
      ...
      def callAsReflective(paramTypes: List[Type], resType: Type): Tree = {
        ...
        gen.evalOnce(qual, currentOwner, unit) { qual1 =>
          ...
          localTyper typed (if (isMaybeBoxed && isJavaValueMethod)
      ...
      tree match {
        ...
        case Apply(
              ...
              List(
                ...
                _))
            if wrapRefArrayMeth.symbol == currentRun.runDefinitions
              .Predef_wrapRefArray && appMeth
              .symbol == ArrayModule_genericApply => super.transform(arg)
        case Apply(
              appMeth,
              List(
                elem0,
