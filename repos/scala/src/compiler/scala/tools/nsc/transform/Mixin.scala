  ...
  private def rebindSuper(
    ...
    exitingSpecialize {
      ...
      while (!bcs.isEmpty && sym == NoSymbol) {
        ...
        sym = member
          .matchingSymbol(bcs.head, base.thisType)
...
// --------- type transformation -----------------------------------------------
  ...
  def cloneBeforeErasure(
    ...
    val newSym = enteringErasure {
      ...
      if (mixinClass.typeParams.isEmpty)
        sym
      else
...
// --------- term transformation -----------------------------------------------
  ...
  class MixinTransformer(unit: CompilationUnit) extends Transformer {
    ...
    def bitmapCategory(field: Symbol): Name = {
      import nme._
      val isNormal = (if (isFieldWithBitmap(field))
                        true
    ...
    private def addNewDefs(clazz: Symbol, stats: List[Tree]): List[Tree] = {
      ...
      def bitmapFor(clazz0: Symbol, offset: Int, field: Symbol): Symbol = {
        ...
        val bitmapName =
          nme.newBitmapName(category, offset / flagsPerBitmap(field)).toTermName
      ...
      def mkSlowPathDef(
        ...
        val rhs: Tree = gen
          .mkSynchronizedCheck(attrThis, cond, syncBody, stats)
      ...
      def buildBitmapOffsets() {
        def fold(fields: List[Symbol], category: Name) = {
          ...
          if (idx == 0)
            ...
            bitmapKindForCategory(category) = BooleanClass
          else if (idx < 9)
            bitmapKindForCategory(category) = ByteClass
          else if (idx < 33)
            bitmapKindForCategory(category) = IntClass
          else
      ...
      tree match {
        ...
        case Select(qual, name) if sym.owner.isTrait && !sym.isMethod =>
          ...
          if (ifaceGetter == NoSymbol)
            ...
            typedPos(tree.pos)((qual DOT ifaceGetter)())
