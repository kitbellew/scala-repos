...
abstract class OverridingPairs extends SymbolPairs {
  ...
  class Cursor(base: Symbol) extends super.Cursor(base) {
    ...
    override protected def exclude(sym: Symbol) =
      (sym.isPrivateLocal || sym.isArtifact || sym.isConstructor ||
        (sym.isPrivate &&
          sym.owner !=
          base) // Privates aren't inherited. Needed for pos/t7475a.scala
      )
    ...
    override protected def matches(lo: Symbol, high: Symbol) =
      lo.isType ||
        ((lo.owner !=
          high.owner) // don't try to form pairs from overloaded members
        &&
          !high
