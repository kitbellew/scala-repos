          ...
          if (grouped) {
            ...
            val subclasses =
              debug.patmatResult(s"enum $sym sealed, subclasses")(
                ...
                sym.sealedDescendants.toList sortBy
                  (_.sealedSortName) filterNot
                  (x =>
    ...
    class TreeMakersToProps(val root: Symbol) {
      ...
      abstract class TreeMakerToProp extends (TreeMaker => Prop) {
        ...
        def updateSubstitution(subst: Substitution): Unit = {
          ...
          val (boundSubst, unboundSubst) =
            (subst.from zip subst.to) partition {
              case (f, t) =>
...
trait MatchAnalysis extends MatchApproximation {
  ...
  trait MatchAnalyzer extends MatchApproximator {
    ...
    object CounterExample {
      def prune(examples: List[CounterExample]): List[CounterExample] = {
        ...
        for (example <- examples
             if (!result.exists(example coveredBy _)))
          result += example
        result.toList
      }
    }
      ...
      override def coveredBy(other: CounterExample): Boolean =
        other match {
          case other @ ListExample(_) =>
            this == other ||
              ((elems.length == other.elems.length) &&
      ...
      override def coveredBy(other: CounterExample): Boolean =
        other match {
          case TupleExample(otherArgs) =>
            this == other ||
              ((ctorArgs.length == otherArgs.length) &&
    ...
    def expandModel(
      ...
      val groupedByVar: Map[Var, List[Sym]] = solution
        .unassigned
        .groupBy(_.variable)
