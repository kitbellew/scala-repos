...
// -------- erasure on types --------------------------------------------------------
      ...
      if (!result) {
        tp match {
          ...
          case TypeRef(pre, sym, args) =>
            ...
            else if (sym.isTypeParameterOrSkolem || sym
                       .isExistentiallyBound || !args.isEmpty)
              result = true
            else if (sym.isClass)
              traverse(rebindInnerClass(pre, sym)) // #2585
  ...
  def needsJavaSig(tp: Type, throwsArgs: List[Type]) =
    !settings.Ynogenericsig && {
      NeedsSigCollector.collect(tp) || throwsArgs
        .exists(NeedsSigCollector.collect)
    }
        ...
        tp.dealiasWiden match {
          case t @ RefinedType(parents, decls) =>
            ...
            if (parents1 eq parents)
              ...
              RefinedType(parents1, decls)
          case t @ ExistentialType(tparams, tpe) =>
            val tpe1 = squashBoxed(tpe)
            if (tpe1 eq tpe)
              t
            else
              ExistentialType(tparams, tpe1)
          case t =>
            if (boxedClass contains t.typeSymbol)
              ObjectTpe
            else
              tp
        }
        ...
        tp.dealiasWiden match {
          ...
          case tp1 @ TypeRef(pre, sym, args) =>
            def argApply(tp: Type) = {
              val tp1 = apply(tp)
              if (tp1.typeSymbol == UnitClass)
                ObjectTpe
              else
                squashBoxed(tp1)
            }
            if (sym == ArrayClass && args.nonEmpty)
              if (unboundedGenericArrayLevel(tp1) == 1)
                ObjectTpe
              else
                mapOver(tp1)
          ...
          case tp1 @ MethodType(params, restpe) =>
            ...
            val restpe1 =
              if (restpe.typeSymbol == UnitClass)
                ...
                apply(restpe)
            if ((params1 eq params) && (restpe1 eq restpe))
              tp1
            else
              MethodType(params1, restpe1)
  ...
  def minimizeParents(parents: List[Type]): List[Type] =
    if (parents.isEmpty)
      ...
      while (rest.nonEmpty) {
        ...
        val nonLeaf = leaves exists { t =>
          t.typeSymbol isSubClass candidate.typeSymbol
        }
        if (!nonLeaf) {
          leaves = leaves filterNot { t =>
            isInterfaceOrTrait(t.typeSymbol) && (
              candidate.typeSymbol isSubClass t.typeSymbol
            )
          }
          leaves += candidate
        }
  ...
  def javaSig(sym0: Symbol, info: Type): Option[String] =
    enteringErasure {
      ...
      def jsig(
          ...
          primitiveOK: Boolean = true): String = {
        val tp = tp0.dealias
        tp match {
          case st: SubType =>
          ...
          case TypeRef(pre, sym, args) =>
            ...
            if (sym == ArrayClass && args.nonEmpty) {
              if (unboundedGenericArrayLevel(tp) == 1)
                ...
                ARRAY_TAG.toString + (args map (jsig(_))).mkString
              ...
              def unboxedMsg =
                if (unboxed == unboxedSeen)
                  ...
                  s", seen within ${sym.simpleName} as $unboxedSeen"
              logResult(
          ...
          case BoundedWildcardType(bounds) =>
            println(
              "something's wrong: " + sym0 + ":" + sym0
                .tpe + " has a bounded wildcard type")
...
// -------- erasure on trees ------------------------------------------
  ...
  class ComputeBridges(unit: CompilationUnit, root: Symbol) {
    ...
    def checkBridgeOverrides(
        member: Symbol,
        other: Symbol,
        bridge: Symbol): Seq[(Position, String)] = {
    ...
    def checkPair(pair: SymbolPair) {
      ...
      val bridgeNeeded = exitingErasure(
        !member.isMacro &&
          ...
          while ((e ne null) && !(
                   (e.sym.tpe =:= otpe) && (bridgeTarget(e.sym) == member)
                 ))
      ...
      var newFlags = (member.flags | BRIDGE | ARTIFACT) & ~(
        ACCESSOR | DEFERRED | LAZY | lateDEFERRED
      )
      ...
      if (member.isModule)
        newFlags = (newFlags | METHOD) & ~(MODULE | lateMETHOD | STABLE)
...
//                }
        ...
        case Select(qual, name) =>
          if (tree.symbol == NoSymbol) {
            ...
            if ((
                  isPrimitiveValueType(qual1.tpe) && !isPrimitiveValueMember(
                    tree.symbol)
                ) ||
            ...
            if (isPrimitiveValueMember(tree.symbol) && !isPrimitiveValueType(
              ...
              val applied = Apply(qual1, List()) setPos qual1
                .pos setType qual1.tpe.resultType
              adaptMember(selectFrom(applied))
            } else if (!(
                         qual1.isInstanceOf[Super] || (
        ...
        case SelectFromArray(qual, name, erasure) =>
          var qual1 = typedQualifier(qual)
          if (!(qual1.tpe <:< erasure))
            qual1 = cast(qual1, erasure)
    ...
    override def typed1(tree: Tree, mode: Mode, pt: Type): Tree = {
      val tree1 =
        try {
          ...
          case er: TypeError =>
            Console
              .println("exception when typing " + tree + "/" + tree.getClass)
  ...
  class ErasureTransformer(unit: CompilationUnit) extends Transformer {
    ...
    private def doubleDefError(pair: SymbolPair) {
      ...
      if (!pair.isErroneous) {
        val what = (if (low.owner == high.owner)
                      "double definition"
        ...
        val when =
          if (exitingRefchecks(lowType matches highType))
            ...
            " after erasure: " + exitingPostErasure(highType)
    ...
    private def sameTypeAfterErasure(sym1: Symbol, sym2: Symbol) =
      exitingPostErasure(sym1.info =:= sym2.info) && !sym1.isMacro && !sym2
        .isMacro
      ...
      while (e ne null) {
        if (e.sym.isTerm) {
          ...
          while (e1 ne null) {
            assert(
              e.sym ne e1.sym,
              s"Internal error: encountered ${e.sym.debugLocationString} twice during scope traversal. This might be related to SI-8010.")
    ...
    def addBridges(stats: List[Tree], base: Symbol): List[Tree] =
      if (base.isTrait)
        ...
        if (bridges.isEmpty)
          ...
          (
            stats filterNot (stat => toBeRemoved contains stat.symbol)
          ) ::: bridges
      }
    ...
    private val preTransformer =
      new TypingTransformer(unit) {
        ...
        private def preEraseNormalApply(tree: Apply) = {
            ...
            fn match {
              case TypeApply(sel @ Select(qual, name), List(targ)) =>
                if (qual.tpe != null && isPrimitiveValueClass(
                      qual.tpe.typeSymbol) && targ.tpe != null && targ
                      .tpe <:< AnyRefTpe)
                  reporter.error(
                    sel.pos,
                ...
                targ.tpe match {
                  ...
                  case RefinedType(parents, decls) if (parents.length >= 2) =>
                    gen.evalOnce(qual, currentOwner, unit) { q =>
                      ...
                      if (parentTests.isEmpty)
                        Literal(Constant(true))
                      else
                        atPos(tree.pos) {
                          parentTests map mkIsInstanceOf(q) reduceRight gen
                            .mkAnd
                        }
                    }
                  case _ =>
          ...
          } else if (fn.symbol.isMethodWithExtension && !fn
                       .symbol
                       .tpe
                       .isErroneous) {
            Apply(
        ...
        private def preEraseApply(tree: Apply) = {
          tree.fun match {
            case TypeApply(fun @ Select(qual, name), args @ List(arg))
                if (
                  (
                    fun.symbol == Any_isInstanceOf || fun
                      .symbol == Object_isInstanceOf
                  ) &&
            ...
            case fn @ Select(qual, name) =>
              ...
              if (fn.symbol.owner == ArrayClass) {
                ...
                if (unboundedGenericArrayLevel(qual.tpe.widen) == 1 || qual
                      .tpe
                      .typeSymbol
                      .isAbstractType) {
          ...
          tree match {
            ...
            case Select(qual, name) =>
              ...
              if (owner.isRefinementClass) {
                sym.allOverriddenSymbols filterNot (
              ...
              def isJvmAccessible(sym: Symbol) =
                (sym.isClass && !sym.isJavaDefined) || localTyper
                  .context
              ...
              if (!isJvmAccessible(owner) && qual.tpe != null) {
                qual match {
                  case Super(_, _) =>
                    ...
                    reporter.error(
                      tree.pos,
                      s"Unable to access ${tree.symbol.fullLocationString} with a super reference.")
                    tree
                  case _ =>
                    ...
                    if (isJvmAccessible(qualSym) && !qualSym
                          .isPackageClass && !qualSym.isPackageObjectClass) {
            ...
            case ClassDef(_, _, _, _) =>
              debuglog(
                "defs of " + tree.symbol + " = " + tree.symbol.info.decls)
            ...
            tree1 match {
              ...
              case ArrayValue(elemtpt, trees) =>
                treeCopy
                  .ArrayValue(
                    tree1,
                    elemtpt setType specialScalaErasure
                      .applyInArray(elemtpt.tpe),
                    trees map transform)
                  .clearType()
