...
// -------- erasure on types --------------------------------------------------------
  ...
  def needsJavaSig(tp: Type, throwsArgs: List[Type]) =
    !settings.Ynogenericsig && {
      NeedsSigCollector.collect(tp) ||
      throwsArgs.exists(NeedsSigCollector.collect)
    }
        ...
        tp match {
          ...
          case TypeRef(pre, sym, args) =>
            ...
            if (sym == ArrayClass && args.nonEmpty) {
              ...
              val unboxedSeen = (tp memberType sym.derivedValueClassUnbox)
                .finalResultType
              def unboxedMsg =
          ...
          case BoundedWildcardType(bounds) =>
            println(
              "something's wrong: " + sym0 + ":" + sym0.tpe +
  ...
  private lazy val interceptedMethods = poundPoundMethods ++
    primitiveGetClassMethods
...
//                }
        ...
        case Select(qual, name) =>
          if (tree.symbol == NoSymbol) {
            ...
            if (isPrimitiveValueMember(tree.symbol) &&
                !isPrimitiveValueType(qual1.tpe)) {
              tree.symbol = NoSymbol
              selectFrom(qual1)
            } else if (isMethodTypeWithEmptyParams(qual1.tpe)) {
              assert(qual1.symbol.isStable, qual1.symbol)
              val applied = Apply(qual1, List()) setPos qual1.pos setType
                qual1.tpe.resultType
  ...
  class ErasureTransformer(unit: CompilationUnit) extends Transformer {
    ...
    private def checkNoDoubleDefs(root: Symbol) {
      ...
      def isErasureDoubleDef(pair: SymbolPair) = {
        ...
        !exitingRefchecks(lowType matches highType) &&
        sameTypeAfterErasure(low, high)
      }
          ...
          def preEraseIsInstanceOf = {
            fn match {
              case TypeApply(sel @ Select(qual, name), List(targ)) =>
                if (qual.tpe != null &&
                    isPrimitiveValueClass(qual.tpe.typeSymbol) &&
                    targ.tpe != null && targ.tpe <:< AnyRefTpe)
                  reporter.error(
                    sel.pos,
                  ...
                  case RefinedType(parents, decls) if (parents.length >= 2) =>
                    gen.evalOnce(qual, currentOwner, unit) { q =>
                      ...
                      if (parentTests.isEmpty)
                        ...
                        atPos(tree.pos) {
                          parentTests map mkIsInstanceOf(q) reduceRight
                            gen.mkAnd
                        }
                    }
                  case _ =>
            ...
            case fn @ Select(qual, name) =>
              ...
              if (fn.symbol.owner == ArrayClass) {
                ...
                if (unboundedGenericArrayLevel(qual.tpe.widen) == 1 ||
                    qual.tpe.typeSymbol.isAbstractType) {
            ...
            case Select(qual, name) =>
              ...
              if (owner.isRefinementClass) {
                sym.allOverriddenSymbols filterNot
                  (_.owner.isRefinementClass) match {
                  case overridden :: _ =>
                    log(
              ...
              if (!isJvmAccessible(owner) && qual.tpe != null) {
                qual match {
                  ...
                  case _ =>
                    ...
                    if (isJvmAccessible(qualSym) && !qualSym.isPackageClass &&
              ...
              case ArrayValue(elemtpt, trees) =>
                treeCopy
                  .ArrayValue(
                    tree1,
                    elemtpt setType
                      specialScalaErasure.applyInArray(elemtpt.tpe),
                    trees map transform)
                  .clearType()
              case DefDef(_, _, _, _, tpt, _) =>
                try super.transform(tree1).clearType()
                finally tpt setType
                  specialErasure(tree1.symbol)(tree1.symbol.tpe).resultType
              case ApplyDynamic(
                    qual,
