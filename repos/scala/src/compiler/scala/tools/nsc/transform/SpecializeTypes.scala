...
abstract class SpecializeTypes extends InfoTransform with TypingTransformers {
  ...
  private implicit val typeOrdering: Ordering[Type] =
    Ordering[String] on ("" + _.typeSymbol.name)
  ...
  private val anyrefSpecCache = perRunCaches
    .newMap[Symbol, mutable.Map[Symbol, Symbol]]()
      ...
      t1 forall {
        case (sym, tpe) =>
          t2 get sym exists { t2tp =>
            (tpe == t2tp) || !(
              isPrimitiveValueType(tpe) || isPrimitiveValueType(t2tp)
      ...
      env forall {
        case (tvar, tpe) =>
          tvar.isSpecialized && (concreteTypes(tvar) contains tpe) && {
            ...
            (sym.owner != rootMirror
              .RootClass && (sym.owner.typeParams contains tvar))
          }
      }
    }
  }
  ...
  case class NormalizedMember(target: Symbol) extends SpecializedInfo {
    ...
    override def typeBoundsIn(env: TypeEnv): Boolean = {
      target.info.typeParams exists { tvar =>
        tvar.isSpecialized && (specializedTypeVars(tvar.info.bounds) exists env
          .isDefinedAt)
      }
    }
      ...
      override def apply(tp: Type): Type =
        tp match {
          case TypeRef(pre, sym, args) if args.nonEmpty =>
            ...
            val args1 =
              map2(args, sym.info.typeParams)((tp, orig) =>
                if (isSpecializedAnyRefSubtype(tp, orig))
                  AnyRefTpe
                else
                  tp)
            specializedClass
  ...
  private def specializedName(
      name: Name,
      tvars: immutable.Set[Symbol],
      env: TypeEnv): TermName = {
    val (methparams, others) = tvars
      .toList sortBy ("" + _.name) partition (_.owner.isMethod)
  ...
  private def specializations(tps: List[Symbol]): List[TypeEnv] = {
    ...
    def loop(ctypes: List[List[Type]]): List[List[Type]] =
      ctypes match {
        case Nil =>
          Nil
        case set :: Nil =>
  ...
  def specializedTypeVars(tpes: List[Type]): immutable.Set[Symbol] = {
    ...
    def loop(
      ...
      if (xs.isEmpty)
        ...
        loop(result ++ specializedTypeVars(xs.head), xs.tail)
    }
  ...
  def specializedTypeVars(sym: Symbol): immutable.Set[Symbol] =
    (if (neverHasTypeParameters(sym))
       ...
       enteringTyper(specializedTypeVars(sym.info)))
  ...
  def specializedTypeVars(tpe: Type): immutable.Set[Symbol] =
    tpe match {
      case TypeRef(pre, sym, args) =>
        if (sym.isAliasType)
        ...
        else if (sym.isTypeParameter && sym.isSpecialized || (sym
                   .isTypeSkolem && sym.deSkolemize.isSpecialized))
          Set(sym)
          ...
          specializedTypeVars(
            sym.typeParams zip args collect {
              case (tp, arg) if tp.isSpecialized =>
                arg
            })

      case PolyType(tparams, resTpe) =>
        specializedTypeVars(resTpe :: mapList(tparams)(symInfo)) // OPT
      ...
      case MethodType(argSyms, resTpe) =>
        specializedTypeVars(resTpe :: mapList(argSyms)(symTpe)) // OPT
      case ExistentialType(_, res) =>
        specializedTypeVars(res)
      case AnnotatedType(_, tp) =>
        specializedTypeVars(tp)
      case TypeBounds(lo, hi) =>
        specializedTypeVars(lo :: hi :: Nil)
      case RefinedType(parents, _) =>
        parents flatMap specializedTypeVars toSet
      case _ =>
        immutable.Set.empty
    }
  ...
  private def typeParamSubAnyRef(tparam: Symbol, sClass: Symbol): Type = {
    val sClassMap = anyrefSpecCache
      .getOrElseUpdate(sClass, mutable.Map[Symbol, Symbol]())
  ...
  /** Maps AnyRef bindings from a raw environment (holding AnyRefs) into type parameters from
    ...
    */
  private def mapAnyRefsInSpecSym(
      env: TypeEnv,
      origsym: Symbol,
      specsym: Symbol): TypeEnv =
    env map {
  ...
  private def mapAnyRefsInOrigCls(env: TypeEnv, origcls: Symbol): TypeEnv =
    env map {
      case (sym, AnyRefTpe) if sym.owner == origcls =>
        (sym, sym.tpe)
      case x =>
        x
    }
  ...
  def specializeClass(clazz: Symbol, outerEnv: TypeEnv): List[Symbol] = {
    def specializedClass(env0: TypeEnv, normMembers: List[Symbol]): Symbol = {
      ...
      sClass.setAnnotations(
        clazz.annotations
      ) // SI-8574 important that the subclass picks up @SerialVersionUID, @strictfp, etc.

      def cloneInSpecializedClass(
          member: Symbol,
          flagFn: Long => Long,
          newName: Name = null) =
      ...
      currentRun.symSource(sClass) =
        clazz.sourceFile // needed later on by mixin
      ...
      val decls1 =
        newScope // declarations of the newly specialized class 'sClass'
      var oldClassTParams: List[Symbol] =
        Nil // original unspecialized type parameters
      var newClassTParams: List[Symbol] =
        Nil // unspecialized type parameters of 'specializedClass' (cloned)
      ...
      val specializedInfoType: Type = {
        ...
        newClassTParams =
          produceTypeParameters(oldClassTParams, sClass, env) map subst(env)
        ...
        def specializedParents(parents: List[Type]): List[Type] = {
          ...
          for (p <- parents) {
            ...
            if (stp != p)
              if (p.typeSymbol.isTrait)
                res ::= stp
              else if (currentRun.compiles(clazz))
                reporter.warning(
                  clazz.pos,
                  p.typeSymbol + " must be a trait. Specialized version of "
      ...
      for (m <- normMembers;
        ...
        if (m.isConstructor) {
          ...
          // }
        } else if (!sClass.isTrait && m.isMethod && !m
                     .hasAccessorFlag) { // other concrete methods
          ...
          def mkAccessor(field: Symbol, name: Name) = {
            val newFlags = (SPECIALIZED | m.getterIn(clazz).flags) & ~(
              LOCAL | CASEACCESSOR | PARAMACCESSOR
            )
          ...
          def overrideIn(clazz: Symbol, sym: Symbol) = {
            val newFlags = (sym.flags | OVERRIDE | SPECIALIZED) & ~(
              DEFERRED | CASEACCESSOR | PARAMACCESSOR
            )
          ...
          if (nme.isLocalName(m.name)) {
            val specGetter = mkAccessor(
              specVal,
              specVal.getterName) setInfo MethodType(Nil, specVal.info)
  ...
  private def normalizeMember(
    ...
    sym :: (if (!sym.isMethod || enteringTyper(sym.typeParams.isEmpty))
              Nil
              ...
              val unusedStvars =
                specializingOn filterNot specializedTypeVars(sym.info)
              ...
              for (env0 <- specializations(specializingOn)
                ...
                yield {
                  ...
                  val specMember = sym.cloneSymbol(
                    owner,
                    (sym.flags | SPECIALIZED) & ~DEFERRED
                  ) // <-- this needs newName = ...
                  val env = mapAnyRefsInSpecSym(env0, sym, specMember)
                  val (keys, vals) = env.toList.unzip

                  specMember setName specializedName(
                    sym,
                    env
                  ) // <-- but the name is calculated based on the cloned symbol
  ...
  private def specializeMember(
    ...
    def specializeOn(tparams: List[Symbol]): List[Symbol] =
      specializations(tparams) map { spec0 =>
        ...
        if (wasSpec.nonEmpty)
          debuglog(
            "specialized overload for %s in %s"
              .format(specMember, pp(typeEnv(specMember))))
      ...
      if (hasUnspecializableAnnotation(sym)) {
        ...
        val tps1 =
          if (sym.isConstructor)
            tps filter (sym.info.paramTypes contains _)
          else
            tps
  ...
  private def specializedOverload(
      owner: Symbol,
      sym: Symbol,
      env: TypeEnv,
      nameSymbol: Symbol = NoSymbol): Symbol = {
    val newFlags = (sym.flags | SPECIALIZED) & ~(
      DEFERRED | CASEACCESSOR | LAZY
    )
        ...
        def checkOverriddenTParams(overridden: Symbol) {
          foreach2(overridden.info.typeParams, overriding.info.typeParams) {
            (baseTvar, derivedTvar) =>
              val missing = concreteTypes(baseTvar)
                .toSet -- concreteTypes(derivedTvar).toSet
              if (missing.nonEmpty) {
                reporter.error(
                  derivedTvar.pos,
        ...
        if (!overriding.isParamAccessor) {
          for (overridden <- overriding.allOverriddenSymbols) {
            ...
            if (stvars.nonEmpty) {
              ...
              if (TypeEnv.restrict(env, stvars).nonEmpty && TypeEnv
                    .isValid(env, overridden) && atNext != NoSymbol) {
          ...
          needsSpecialOverride(overriding) match {
            case (NoSymbol, _) =>
              if (overriding.isSuperAccessor) {
                ...
                debuglog(
                  "checking special overload for super accessor: %s, alias for %s"
                    .format(overriding.fullName, alias.fullName))
                needsSpecialOverride(alias) match {
                  case nope @ (NoSymbol, _) =>
                    None
                  case (overridden, env) =>
                    val om = specializedOverload(
                      clazz,
                      overriding,
                      env,
                      overridden)
            ...
            case (overridden, env) =>
              ...
              debuglog(
                "specialized overload %s for %s in %s: %s"
                  .format(om, overriding.name.decode, pp(env), om.info))
              if (overriding.isAbstractOverride)
                om.setFlag(ABSOVERRIDE)
              typeEnv(om) = env
              addConcreteSpecMethod(overriding)
              if (overriding.isDeferred) { // abstract override
                debuglog(
                  "abstract override " + overriding
                    .fullName + " with specialized " + om.fullName)
              ...
              ifDebug(
                exitingSpecialize(
                  assert(
                    ...
                    "Could not find " + om
                      .name + " in " + overridden.owner.info.decls)))
              Some(om)
          }
        }
      ).toList
  ...
  private def unify(
    ...
    (tp1, tp2) match {
      ...
      case (TypeRef(_, sym1, args1), TypeRef(_, sym2, args2)) =>
        ...
        if (strict && args1.length != args2.length)
          unifyError(tp1, tp2)
      ...
      case (MethodType(params1, res1), MethodType(params2, res2)) =>
        if (strict && params1.length != params2.length)
          unifyError(tp1, tp2)
        debuglog("Unify methods " + tp1 + " and " + tp2)
        unify(
          res1 :: (params1 map (_.tpe)),
      ...
      case (ThisType(_), ThisType(_)) =>
        env
      case (_, SingleType(_, _)) =>
        unify(tp1, tp2.underlying, env, strict)
      case (SingleType(_, _), _) =>
        unify(tp1.underlying, tp2, env, strict)
      case (ThisType(_), _) =>
        unify(tp1.widen, tp2, env, strict)
      case (_, ThisType(_)) =>
        unify(tp1, tp2.widen, env, strict)
      case (RefinedType(_, _), RefinedType(_, _)) =>
        env
      case (AnnotatedType(_, tp1), tp2) =>
        unify(tp2, tp1, env, strict)
      case (ExistentialType(_, res1), _) =>
        unify(tp2, res1, env, strict)
      case (TypeBounds(lo1, hi1), TypeBounds(lo2, hi2)) =>
        unify(List(lo1, hi1), List(lo2, hi2), env, strict)
      case _ =>
        debuglog(
          "don't know how to unify %s [%s] with %s [%s]"
            .format(tp1, tp1.getClass, tp2, tp2.getClass))
        env
    }
  ...
  private def unify(
    ...
    if (tp1.isEmpty || tp2.isEmpty)
      env
    else
      (
        tp1 zip tp2
      ).foldLeft(env) { (env, args) =>
        if (!strict)
          unify(args._1, args._2, env, strict)
        else {
          val nenv = unify(args._1, args._2, emptyEnv, strict)
          if (env.keySet intersect nenv.keySet isEmpty)
            env ++ nenv
          else {
            debuglog(
              "could not unify: u(" + args._1 + ", " + args
                ._2 + ") yields " + nenv + ", env: " + env)
            unifyError(tp1, tp2)
          }
        }
      }
      ...
      override def mapOver(tp: Type): Type =
        tp match {
          case ClassInfoType(parents, decls, clazz) =>
            ...
            if ((parents1 eq parents) && (decls1 eq decls))
              tp
            else
    ...
    env forall {
      case (tvar, tpe) =>
        matches(tvar.info.bounds.lo, tpe) && matches(
          ...
          if (warnings)
            reporter
              .warning(tvar.pos, "Bounds prevent specialization of " + tvar)
      ...
      override def castType(tree: Tree, pt: Type): Tree = {
        ...
        val ntree =
          if (tree.tpe != null && !(
            ...
            if (casttpe <:< pt)
              gen.mkCast(tree, casttpe)
            else if (casttpe <:< CastMap(pt))
              gen.mkCast(tree, pt)
            else
              tree
          } else
  ...
  class ImplementationAdapter(
    ...
    override val symSubst =
      new SubstSymMap(from, to) {
    ...
    private def shouldMakePublic(sym: Symbol): Boolean =
      sym.hasFlag(PRIVATE | PROTECTED) && (addressFields || !nme
        .isLocalName(sym.name))
  ...
  def illegalSpecializedInheritance(clazz: Symbol): Boolean =
    (clazz.isSpecialized
      && originalClass(clazz)
        .parentSymbols
        .exists(p => hasSpecializedParams(p) && !p.isTrait))

  def specializeCalls(unit: CompilationUnit) =
    new TypingTransformer(unit) {
        ...
        override def traverse(tree: Tree) =
          tree match {
            case DefDef(_, _, _, vparams :: Nil, _, rhs) =>
              if (concreteSpecMethods(tree.symbol) || tree
                    .symbol
                    .isConstructor) {
        ...
        (treeType =:= memberType) || { // anyref specialization
          memberType match {
            case PolyType(_, resTpe) =>
              debuglog(
                "Conformance for anyref - polytype with result type: " + resTpe + " and " + treeType + "\nOrig. sym.: " + origSymbol)
              try {
      ...
      def transform1(tree: Tree) = {
        ...
        def specSym(qual: Tree): Symbol = {
          ...
          if (env.isEmpty)
            ...
            qual.tpe member specializedName(symbol, env) suchThat isMatch
        }
        ...
        def matchingSymbolInPrefix(
            ...
            member: Symbol,
            env: TypeEnv): Symbol = {
          pre member specializedName(member, env) suchThat (_
            .tpe matches subst(env, member.tpe))
        }
        ...
        def transformSelect(sel: Select) = {
          ...
          def typedOp(member: Symbol) =
            localTyper typedOperator newSelect(member)
          def typedTree(member: Symbol) = localTyper typed newSelect(member)

          val ignoreEnv = specializedTypeVars(symbol.info)
            .isEmpty || name == nme.CONSTRUCTOR
          if (ignoreEnv)
        ...
        def computeResidualTypeVars(
          ...
          ifDebug(
            assert(
              residualTargs.length == specMember.info.typeParams.length,
          ...
          localTyper.typedOperator(
            atPos(tree.pos)(tree1)
          ) // being polymorphic, it must be a method
        }
        ...
        tree match {
          case Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
            def transformNew = {
              debuglog(
                "Attempting to specialize new %s(%s)"
                  .format(tpt, args.mkString(", ")))
              val found = specializedType(tpt.tpe)
              if (found.typeSymbol ne tpt
                    .tpe
                    .typeSymbol) { // the ctor can be specialized
                val inst = New(found, transformTrees(args): _*)
                reportError(localTyper.typedPos(tree.pos)(inst))(_ =>
                  super.transform(tree))
              } else
                super.transform(tree)
            }
          ...
          case Apply(sel @ Select(sup @ Super(qual, name), name1), args)
            ...
            def transformSuperApply = {
              ...
              val tree1 = Apply(
                Select(sup1, name1) setPos sel.pos,
                transformTrees(args))
          ...
          case TypeApply(sel @ Select(qual, name), targs)
              if (
                specializedTypeVars(symbol.info)
                  .nonEmpty && name != nme.CONSTRUCTOR
              ) =>
            debuglog(
              "checking typeapp for rerouting: " + tree + " with sym.tpe: " + symbol
                .tpe + " tree.tpe: " + tree.tpe)
            val qual1 = transform(qual)
            ...
            specSym(qual1) match {
              ...
              case specMember =>
                ...
                ifDebug(
                  assert(
                    symbol.info.typeParams.length == targs.length,
                    symbol.info.typeParams + " / " + targs))

                val env = typeEnv(specMember)
                computeResidualTypeVars(
                  tree,
                  specMember,
          ...
          case Select(Super(_, _), _)
            ...
            val pos = tree.pos
            debuglog(
              pos.source.file.name + ":" + pos
                .line + ": not specializing call to super inside illegal specialized inheritance class.\n" + pos
                .lineContent)
            tree
          ...
          case Template(parents, self, body) =>
            def transformTemplate = {
              val specMembers = makeSpecializedMembers(
                tree.symbol.enclClass) ::: (implSpecClasses(body) map localTyper
                .typed)
              if (!symbol.isPackageClass)
                (new CollectMethodBodies)(tree)
              val parents1 =
                map2(currentOwner.info.parents, parents)((tpe, parent) =>
            ...
            def transformDefDef = {
              if (symbol.isConstructor) {
                ...
                if (symbol.isPrimaryConstructor)
                  localTyper.typedPos(symbol.pos)(
                    deriveDefDef(tree)(_ =>
                      Block(List(t), Literal(Constant(())))))
                else // duplicate the original constructor
                  reportError(duplicateBody(ddef, info(symbol).target))(_ =>
                    ddef)
              } else
                info(symbol) match {
                  case Implementation(target) =>
                    ...
                    val tree1 =
                      reportError(duplicateBody(ddef, target))(_ => ddef)
                  ...
                  case NormalizedMember(target) =>
                    logResult("constraints")(
                      ...
                      case Some(constraint) if !target.isDeferred =>
                        // we have an rhs, specialize it
                        val tree1 =
                          reportError(duplicateBody(ddef, target, constraint))(
                            _ => ddef)
                        debuglog("implementation: " + tree1)
                        deriveDefDef(tree1)(transform)
                      case _ =>
                        deriveDefDef(tree)(_ =>
                          localTyper typed gen.mkSysErrorCall(
                  ...
                  case SpecialOverload(original, env) =>
                    debuglog(
                      "completing specialized " + symbol
                        .fullName + " calling " + original)
                    ...
                    val t = DefDef(
                      ...
                      { vparamss: List[List[Symbol]] =>
                        ...
                        debuglog(
                          "inside defdef: " + symbol + "; type: " + symbol
                            .tpe + "; owner: " + symbol.owner)
                  ...
                  case fwd @ Forward(_) =>
                    ...
                    val rhs1 = forwardCall(
                      ...
                      vparamss)
                    debuglog(
                      "-->d completed forwarder to specialized overload: " + fwd
                        .target + ": " + rhs1)
                    reportError {
      ...
      /**
        ...
        * @see duplicateBody
        */
      private def expandInnerNormalizedMembers(tree: Tree) =
        tree match {
      ...
      private def addBody(tree: DefDef, source: Symbol): DefDef = {
        ...
        copyDefDef(tree)(
          vparamss = List(newSyms map ValDef.apply),
          rhs = newBody)
      }
...
//        for (m <- specialOverrides(specializedClass)) specializedClass.info.decls.enter(m)
        ...
        for (m <- sClass.info.decls
          ...
          if (m.isMethod) {
            if (info(m).target.hasAccessorFlag)
            ...
            if (m.isClassConstructor) {
              ...
              if (m.isPrimaryConstructor) {
                for (param <- vparams;
                  ...
                  val acc = param
                    .cloneSymbol(sClass, param.flags | PARAMACCESSOR | PRIVATE)
...
//            log("created synthetic class: " + m.fullName)
        ...
        if (hasSpecializedFields) {
          val isSpecializedInstance = sClass :: sClass
            .parentSymbols exists (_ hasFlag SPECIALIZED)
          val sym = sClass.newMethod(
            nme.SPECIALIZED_INSTANCE,
            sClass.pos) setInfoAndEnter MethodType(Nil, BooleanTpe)
    ...
    override def transform(tree: Tree) = {
      val resultTree =
        if (settings.nospecialization)
          ...
          exitingSpecialize(specializeCalls(unit).transform(tree))
