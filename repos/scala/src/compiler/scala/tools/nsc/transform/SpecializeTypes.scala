...
abstract class SpecializeTypes extends InfoTransform with TypingTransformers {
      ...
      env forall {
        case (tvar, tpe) =>
          tvar.isSpecialized && (concreteTypes(tvar) contains tpe) && {
            ...
            (sym.owner != rootMirror.RootClass && (sym.owner
              .typeParams contains tvar))
          }
      }
    }
  }
  ...
  def survivingArgs(sym: Symbol, args: List[Type]): List[Type] =
    for ((tvar, tpe) <- sym.info.typeParams.zip(args)
         if !tvar.isSpecialized || !isPrimitiveValueType(tpe)) yield tpe
      ...
      tp match {
        case TypeRef(pre, sym, args) if args.nonEmpty =>
          ...
          val args1 = map2(args, sym.info.typeParams)((tp, orig) =>
            if (isSpecializedAnyRefSubtype(tp, orig)) AnyRefTpe else tp)
          specializedClass
  ...
  private def specializedName(
    ...
    val (methparams, others) =
      tvars.toList sortBy ("" + _.name) partition (_.owner.isMethod)
  ...
  private def specializedName(
    ...
    (if (name == nme.CONSTRUCTOR || (types1.isEmpty && types2.isEmpty))
       ...
       newTermName(
         base.toString + "$"
           + "m" + ms + types1.map(t => abbrvTag(t.typeSymbol))
           .mkString("", "", "")
           + "c" + cs + types2.map(t => abbrvTag(t.typeSymbol))
           .mkString("", "", "$sp"))
     })
  ...
  def concreteTypes(sym: Symbol): List[Type] = {
    val types =
      if (!sym.isSpecialized) Nil // no @specialized Annotation
      else specializedOn(sym) map (s => specializesClass(s).tpe) sorted
  ...
  def specializedTypeVars(tpe: Type): immutable.Set[Symbol] =
    tpe match {
      case TypeRef(pre, sym, args) =>
        if (sym.isAliasType) specializedTypeVars(tpe.dealiasWiden)
        else if (sym.isTypeParameter && sym.isSpecialized || (sym
                   .isTypeSkolem && sym.deSkolemize.isSpecialized)) Set(sym)
        else if (sym == ArrayClass) specializedTypeVars(args)
        else if (args.isEmpty) Set()
        else
          specializedTypeVars(sym.typeParams zip args collect {
            case (tp, arg) if tp.isSpecialized => arg
          })

      case PolyType(tparams, resTpe) => specializedTypeVars(
          resTpe :: mapList(tparams)(symInfo)
        ) // OPT
      ...
      case MethodType(argSyms, resTpe) => specializedTypeVars(
          resTpe :: mapList(argSyms)(symTpe)
        ) // OPT
  ...
  private def typeParamSubAnyRef(tparam: Symbol, sClass: Symbol): Type = {
    ...
    sClassMap.getOrElseUpdate(
      tparam,
      tparam.cloneSymbol(
        sClass,
        tparam.flags,
        tparam.name append tpnme.SPECIALIZED_SUFFIX)
        modifyInfo (info => TypeBounds(info.bounds.lo, AnyRefTpe))).tpe
  }
  ...
  def specializeClass(clazz: Symbol, outerEnv: TypeEnv): List[Symbol] = {
    def specializedClass(env0: TypeEnv, normMembers: List[Symbol]): Symbol = {
      ...
      val specializedInfoType: Type = {
        ...
        if (extraSpecializedMixins.nonEmpty)
          debuglog("extra specialized mixins for %s: %s".format(
            clazz.name.decode,
      ...
      def forwardToOverload(m: Symbol): Symbol = {
        val specMember = enterMember(cloneInSpecializedClass(
          m,
      ...
      for (m <- normMembers;
        ...
        if (m.isPrivate) m.resetFlag(PRIVATE).setFlag(PROTECTED)
        ...
        if (m.isConstructor) {
          ...
          } else debuglog("conflicting env for " + m + " env: " + env)
    ...
    subclasses foreach { env =>
      ...
      if (existing != NoSymbol) clazz.owner.info.decls.unlink(existing)
  ...
  private def normalizeMember(
    ...
    sym :: (if (!sym.isMethod || enteringTyper(sym.typeParams.isEmpty)) Nil
  ...
  private def pp(env: TypeEnv): String = {
    env.toList.sortBy(_._1.name) map {
      case (k, v) =>
        ...
        if (k == vsym) "" + k.name else k.name + ":" + vsym.name
  ...
  private def specializeMember(
    ...
    def specializeOn(tparams: List[Symbol]): List[Symbol] =
      specializations(tparams) map { spec0 =>
        ...
        if (wasSpec.nonEmpty)
          debuglog("specialized overload for %s in %s".format(
            specMember,
    ...
    if (sym.isMethod) {
      if (hasUnspecializableAnnotation(sym)) { List() }
        ...
        if (stvars.nonEmpty)
          debuglog("specialized %s on %s".format(
      ...
      def needsSpecialOverride(overriding: Symbol): (Symbol, TypeEnv) = {
        def checkOverriddenTParams(overridden: Symbol) {
          foreach2(overridden.info.typeParams, overriding.info.typeParams) {
            (baseTvar, derivedTvar) =>
              val missing =
                concreteTypes(baseTvar).toSet -- concreteTypes(derivedTvar)
                  .toSet
              if (missing.nonEmpty) {
                reporter.error(
                  derivedTvar.pos,
        ...
        if (!overriding.isParamAccessor) {
          for (overridden <- overriding.allOverriddenSymbols) {
            ...
            if (stvars.nonEmpty) {
              debuglog("specialized override of %s by %s%s".format(
          ...
          case (overridden, env) =>
            ...
            if (overriding.isDeferred) { // abstract override
              ...
              info get overriding match {
                ...
                case _ => info(om) = SpecialOverride(overriding)
              }
            ...
            ifDebug(exitingSpecialize(assert(
              ...
              "Could not find " + om.name + " in " + overridden.owner.info
                .decls)))
            Some(om)
        }
      }).toList
    ...
    (tp1, tp2) match {
      case (TypeRef(_, sym1, _), _) if sym1.isSpecialized =>
        ...
        if (isPrimitiveValueClass(tp2.typeSymbol) || isSpecializedAnyRefSubtype(
              tp2,
              sym1)) env + ((sym1, tp2))
        else if (isSpecializedAnyRefSubtype(tp2, sym1)) env + ((sym1, tp2))
        else if (strict) unifyError(tp1, tp2)
        else env
      case (TypeRef(_, sym1, args1), TypeRef(_, sym2, args2)) =>
        ...
        e
      case (TypeRef(_, sym1, _), _) if sym1.isTypeParameterOrSkolem => env
      ...
      case (PolyType(tparams1, res1), PolyType(tparams2, res2)) =>
        ...
        if (strict && tparams1.length != tparams2.length) unifyError(tp1, tp2)
          ...
          unify(
            ...
            env,
            strict)
        else unify(res1, res2, env, strict)
  ...
  def illegalSpecializedInheritance(clazz: Symbol): Boolean =
    (clazz.isSpecialized
      && originalClass(clazz).parentSymbols
        ...
        override def traverse(tree: Tree) =
          tree match {
            case DefDef(_, _, _, vparams :: Nil, _, rhs) =>
              if (concreteSpecMethods(tree.symbol) || tree.symbol
                    .isConstructor) {
            ...
            case ValDef(mods, name, tpt, rhs)
                if concreteSpecMethods(tree.symbol) => body(tree.symbol) = rhs
      ...
      def transform1(tree: Tree) = {
        ...
        def transformSelect(sel: Select) = {
          ...
          if (ignoreEnv) overloads(symbol) find (_ matchesSym symbol) match {
            ...
            overloads(symbol) find (_ matchesEnv env) match {
              case Some(Overload(member, _)) => typedOp(member)
              case _ => matchingSymbolInPrefix(qual1.tpe, symbol, env) match {
        ...
        tree match {
          case Apply(Select(New(tpt), nme.CONSTRUCTOR), args) =>
            def transformNew = {
              debuglog("Attempting to specialize new %s(%s)".format(
                tpt,
                args.mkString(", ")))
          ...
          case TypeApply(sel @ Select(qual, name), targs)
              if (specializedTypeVars(symbol.info).nonEmpty && name != nme
                .CONSTRUCTOR) =>
            debuglog(
          ...
          case sel @ Select(_, _) => transformSelect(sel)
          ...
          case Template(parents, self, body) =>
            def transformTemplate = {
              val specMembers = makeSpecializedMembers(
                ...
                .typed)
              if (!symbol.isPackageClass) (new CollectMethodBodies)(tree)
              val parents1 = map2(currentOwner.info.parents, parents)(
                (tpe, parent) => TypeTree(tpe) setPos parent.pos)
          ...
          case ddef @ DefDef(_, _, _, vparamss, _, _)
            ...
            def transformDefDef = {
              if (symbol.isConstructor) {
                val t = atOwner(symbol)(forwardCtorCall(
                  tree.pos,
                  gen.mkSuperInitCall,
                  vparamss,
                  symbol.owner))
                ...
                info(symbol) match {
                  case Implementation(target) =>
                    ...
                    val tree1 = reportError(duplicateBody(ddef, target))(_ =>
                      ddef)
                    debuglog("implementation: " + tree1)
                    deriveDefDef(tree1)(transform)

                  case NormalizedMember(target) =>
                    logResult("constraints")(satisfiabilityConstraints(
                      typeEnv(symbol))) match {
                      case Some(constraint) if !target.isDeferred =>
                        // we have an rhs, specialize it
                        val tree1 = reportError(
                          duplicateBody(ddef, target, constraint))(_ => ddef)
                        debuglog("implementation: " + tree1)
                        deriveDefDef(tree1)(transform)
                      case _ =>
                        deriveDefDef(tree)(_ =>
                  ...
                  case SpecialOverride(target) =>
                    ...
                    log("%s is param accessor? %b".format(
                      ddef.symbol,
                      ddef.symbol.isParamAccessor))
                    // we have an rhs, specialize it
                    ...
                    val t = DefDef(
                      ...
                      { vparamss: List[List[Symbol]] =>
                        ...
                        gen.maybeMkAsInstanceOf(
                          fun,
                          symbol.owner.thisType.memberType(symbol)
                            .finalResultType,
                          symbol.owner.thisType.memberType(original)
                            .finalResultType)
                      }
                    )
                  ...
                  case SpecializedAccessor(target) =>
                    val rhs1 =
                      if (symbol.isGetter) gen.mkAttributedRef(target)
                      else
                        Assign(
...
//            log("created synthetic class: " + m.fullName)
        ...
        if (hasSpecializedFields) {
          val isSpecializedInstance =
            sClass :: sClass.parentSymbols exists (_ hasFlag SPECIALIZED)
          val sym = sClass
            .newMethod(
