      ...
      case ClassInfoType(parents, decls, clazz)
        ...
        val parents1 = parents match {
          ...
          case hd :: tl =>
            ...
            if (clazz.isTrait) ObjectTpe :: tl else parents
        }
        if (clazz.isTrait) {
          decls foreach { sym =>
            if (!sym.isType)
...
// Tree transformation --------------------------------------------------------------
  ...
  private def mkAssign(clazz: Symbol, assignSym: Symbol, rhs: Tree): Tree = {
    ...
    if (assignSym.isSetter) Apply(qual, List(rhs)) else Assign(qual, rhs)
  }
  ...
  private def addMixinConstructorCalls(tree: Tree, clazz: Symbol): Tree = {
    def mixinConstructorCall(mc: Symbol): Tree =
      atPos(tree.pos) { Apply(SuperSelect(clazz, mc.primaryConstructor), Nil) }
    ...
    tree match {
      ...
      case Block(stats, expr) =>
        ...
        val (presuper, supercall :: rest) = stats span
          (t => t.hasSymbolWhich(_ hasFlag PRESUPER))
        treeCopy.Block(
          tree,
    ...
    override def transform(tree: Tree): Tree = {
      ...
      val tree1 = tree match {
        ...
        case Template(parents, self, body) =>
          val parents1 = sym.owner.info.parents map
