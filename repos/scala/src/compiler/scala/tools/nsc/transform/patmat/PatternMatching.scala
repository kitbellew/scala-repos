    ...
    override def transform(tree: Tree): Tree =
      tree match {
        case Match(sel, cases) =>
          ...
          val translated = translator.translateMatch(treeCopy.Match(
            tree,
            transform(sel),
...
trait Debugging {
  ...
  object debug {
    ...
    @inline
    final def patmat(s: => String) = if (printPatmat) Console.err.println(s)
    @inline
  ...
  trait TypedSubstitution extends MatchMonadInterface {
    ...
    class Substitution(val from: List[Symbol], val to: List[Tree]) {
      ...
      def >>(other: Substitution): Substitution = {
        val (fromFiltered, toFiltered) = (from, to).zipped filter { (f, t) =>
          !other.from.contains(f)
        }
        new Substitution(
          other.from ++ fromFiltered,
          other.to.map(apply) ++ toFiltered
