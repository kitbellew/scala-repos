  ...
  trait TreeMakers extends TypedSubstitution with CodegenCore {
    ...
    sealed trait PreserveSubPatBinders extends TreeMaker {
      ...
      lazy val storedBinders: Set[Symbol] =
        (if (debugInfoEmitVars) subPatBinders.toSet else Set.empty) ++
      ...
      override def subPatternsAsSubstitution =
        Substitution(subPatBinders, subPatRefs) >>
          super.subPatternsAsSubstitution
    ...
    object TypeTestTreeMaker {
      ...
      trait TypeTestCondStrategy {
        ...
        def withOuterTest(
            orig: Result)(testedBinder: Symbol, expectedTp: Type): Result = orig
      ...
      object treeCondStrategy extends TypeTestCondStrategy {
        ...
        override def withOuterTest(
            orig: Tree)(testedBinder: Symbol, expectedTp: Type): Tree = {
          ...
          def definedInStaticLocation(tp: Type): Boolean = {
            def isStatic(tp: Type): Boolean =
              if (tp == NoType || tp.typeSymbol.isPackageClass ||
                  tp == NoPrefix) true
    ...
    **/
    ...
    def propagateSubstitution(
        treeMakers: List[TreeMaker],
    ...
    def combineCases(
      ...
      val casesNoSubstOnly = casesRaw map
        (propagateSubstitution(_, EmptySubstitution))
    ...
    def combineCasesNoSubstOnly(
      ...
      fixerUpper(owner, scrut.pos) {
        ...
        emitSwitch(
          ...
          if (casesNoSubstOnly nonEmpty) {
            ...
            val synthCatchAll =
              if (casesNoSubstOnly.nonEmpty && {
                    ...
                    nonTrivLast.nonEmpty &&
                    nonTrivLast.head.isInstanceOf[BodyTreeMaker]
                  }) None
              else matchFailGen
          ...
          } else { codegen.matcher(scrut, scrutSym, pt)(Nil, matchFailGen) }
        }
      }
    ...
    protected def fixerUpper(origOwner: Symbol, pos: Position) =
      new Traverser {
        ...
        override def traverse(t: Tree) {
          if (t != EmptyTree && t.pos == NoPosition) { t.setPos(pos) }
          t match {
            ...
            case Function(_, _)
              ...
              debug.patmat(
                "fundef: " +
                  ((t, t.symbol.ownerChain, currentOwner.ownerChain)))
              t.symbol.owner = currentOwner
            case d: DefTree
                if (d.symbol != NoSymbol) &&
                  ((d.symbol.owner == NoSymbol) ||
                    (d.symbol.owner ==
                      origOwner)) => // don't indiscriminately change existing owners! (see e.g., pos/t3440, pos/t3534, pos/unapplyContexts2)
              debug.patmat(
