...
object ClassPath {
  ...
  abstract class ClassPathContext[T]
      extends classpath.ClassPathFactory[ClassPath[T]] {
    ...
    def validPackage(name: String) =
      (name != "META-INF") && (name != "") && (name.charAt(0) != '.')
  ...
  def toSourceName(f: AbstractFile): String =
    FileUtils.stripSourceExtension(f.name)
}
...
/**
  ...
  */
class SourcePath[T](dir: AbstractFile, val context: ClassPathContext[T])
    extends ClassPath[T] {
...
class MergedClassPath[T](
  ...
  override def toString() =
    "merged classpath " + entries.mkString("(", "\n", ")")
}
