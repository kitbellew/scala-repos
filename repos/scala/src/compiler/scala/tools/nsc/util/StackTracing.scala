...
private[util] trait StackTracing extends Any {
  ...
  def stackTracePrefixString(e: Throwable)(
    ...
    def because(e: Throwable): String =
      e.getCause match { case null => null; case c => header(c) }
    def msg(e: Throwable): String =
      e.getMessage match { case null => because(e); case s => s }
    def txt(e: Throwable): String =
      msg(e) match { case null => ""; case s => s": $s" }
      ...
      if (unseen(e)) {
        ...
        val frames =
          (if (share.nonEmpty) {
             ...
             val trimmed = trace.reverse dropWhile
               (spare.hasNext && spare.next == _)
             trimmed.reverse
           } else trace)
