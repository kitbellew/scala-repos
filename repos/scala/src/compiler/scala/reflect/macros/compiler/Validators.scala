  ...
  trait Validator {
    ...
    private def sanityCheck() = {
      ...
      val effectiveOwner =
        if (isImplMethod)
          ...
          macroImplOwner.owner
      val effectivelyStatic = effectiveOwner.isStaticOwner ||
        effectiveOwner.moduleClass.isStaticOwner
      val correctBundleness =
        if (isImplMethod)
          macroImplOwner.isModuleClass
        else
          macroImplOwner.isClass && !macroImplOwner.isModuleClass
    ...
    private def checkMacroDefMacroImplCorrespondence() = {
      ...
      try {
        ...
        val maxLubDepth = lubDepth(aparamss.flatten map (_.tpe)) max
          lubDepth(rparamss.flatten map (_.tpe))
        ...
        case ex: NoInstance =>
          MacroImplTparamInstantiationError(atparams, ex)
      }
    }
    ...
    private lazy val referenceMacroImplSig: MacroImplSig = {
      ...
      object SigGenerator {
        ...
        val paramss =
          if (isImplMethod)
            ...
            mmap(macroDdef.vparamss)(param)
        val macroDefRet =
          if (!macroDdef.tpt.isEmpty)
            typer.typedType(macroDdef.tpt).tpe
          else
            computeMacroDefTypeFromMacroImplRef(macroDdef, macroImplRef) orElse
              AnyTpe
      ...
      val result = MacroImplSig(
        macroDdef.tparams map (_.symbol),
        paramss,
        implReturnType)
