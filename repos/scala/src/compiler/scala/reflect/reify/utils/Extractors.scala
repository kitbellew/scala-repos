    ...
    val reifierBody = {
      def gc(symtab: SymbolTable): SymbolTable = {
        def loop(symtab: SymbolTable): SymbolTable = {
          def extractNames(tree: Tree) = tree.collect { case ref: RefTree =>
            ref.name
          }.toSet
      ...
      val universeAlias = ValDef(
        ...
        nme.UNIVERSE_SHORT,
        Ident(tparamu),
        Select(Ident(nme.MIRROR_UNTYPED), nme.universe))
      val mirrorAlias = ValDef(
        NoMods,
        nme.MIRROR_SHORT,
        Select(Ident(nme.UNIVERSE_SHORT), tpnme.Mirror),
        TypeApply(
          Select(Ident(nme.MIRROR_UNTYPED), nme.asInstanceOf_),
  ...
  object ReifiedTree {
    def apply(
      ...
      val tagCtor = TypeApply(
        ...
        List(mkTarg(tpe)))
      val exprCtor = TypeApply(
        Select(Select(Ident(nme.UNIVERSE_SHORT), nme.Expr), nme.apply),
        List(mkTarg(tpe)))
      val tagArgs = List(
        Ident(nme.MIRROR_SHORT),
    ...
    def unapply(tree: Tree): Option[(Tree, TermName, Tree, Long, String)] = {
      ...
      tree match {
        case ValDef(
            ...
            if uref1.name == nme.UNIVERSE_SHORT && internal1 == nme.internal && rs1 == nme.reificationSupport && acceptFreeTermFactory(
              freeTermFactory) &&
