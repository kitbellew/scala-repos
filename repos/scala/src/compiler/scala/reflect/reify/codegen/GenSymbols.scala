...
trait GenSymbols {
  ...
  def reifySymRef(sym: Symbol): Tree = {
    ...
    if (sym == NoSymbol)
       ...
       */
      val hasPackagelessParent = sym.ownerChain.tail.tail exists
        (_.isEmptyPackageClass)
      if (sym.isStatic &&
          (sym.isClass || sym.isModule) && !hasPackagelessParent) {
  ...
  private def reifyIntoSymtab(sym: Symbol)(
    ...
    if (fromSymtab == EmptyTree) {
      ...
      val tree = reification.tree updateAttachment
        ReifyBindingAttachment(binding)
