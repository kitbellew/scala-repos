  ...
  val reshape =
    new Transformer {
        ...
        val preTyper =
          tree match {
            ...
            case template @ Template(parents, self, body) =>
              ...
              val body1 = reshapeLazyVals(
                trimSyntheticCaseClassCompanions(body))
              Template(parents1, self, body1).copyAttrs(template)
            case block @ Block(stats, expr) =>
              val stats1 = reshapeLazyVals(
                trimSyntheticCaseClassCompanions(stats))
          ...
          case ty @ Typed(expr1, tpt) =>
            ...
            if (annotatedArg != EmptyTree) {
              if (annotatedArg.isType) {
                ...
                if (reifyDebug)
                  println(
                    "verdict: was an annotated value, equivalent is " +
                      original)
      ...
      private def toPreTyperLazyVal(ddef: DefDef): ValDef = {
        ...
        val mods1 = Modifiers(flags1, privateWithin0, annotations0) setPositions
          mods0.positions
          ...
          case vdef @ ValDef(mods, name, tpt, rhs) if !mods.isLazy =>
            val mods1 =
              if (accessors.contains(vdef)) {
                ...
                Modifiers(flags1, privateWithin1, annotations1) setPositions
                  mods.positions
              } else {
                mods
              }
      ...
      private def trimSyntheticCaseClassMembers(
          deff: Tree,
