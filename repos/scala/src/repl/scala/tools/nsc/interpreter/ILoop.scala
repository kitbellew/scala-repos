...
class ILoop(in0: Option[BufferedReader], protected val out: JPrintWriter)
  ...
  protected def newJavap() =
    JavapClass(addToolsJarToLoader(), new IMain.ReplStrippingWriter(intp), intp)
  ...
  private def changeSettings(line: String): Result = {
    def showSettings() =
      for (s <- settings.userSetSettings.toSeq.sorted)
        echo(s.toString)
    if (line.isEmpty)
      showSettings()
    else {
  ...
  private def phaseCommand(name: String): Result = {
    ...
    if (name == "clear") {
      ...
      if (what.isEmpty || !phased.set(what))
        ...
        val activeMessage =
          if (what.toString.length == name.length)
            "" + what
          else
            "%s (%s)".format(what, name)
        ...
        file applyReader { reader =>
          in =
            if (verbose)
              ...
              SimpleReader(reader, out, interactive = false)
          echo(s"Loading $file...")
          loop()
        }
      }
  ...
  def editCommand(what: String): Result =
    editCommand(what, Properties.envOrNone("EDITOR"))
  ...
  def withFile[A](filename: String)(action: File => A): Option[A] = {
    ...
    if (res.isEmpty)
      echo("That file does not exist") // courtesy side-effect
    res
  }

  def loadCommand(arg: String): Result = {
    def run(file: String, verbose: Boolean) =
      withFile(file) { f =>
  ...
  def require(arg: String): Unit = {
    ...
    if (!f.exists)
      echo(s"The path '$f' doesn't seem to exist.")
    else if (exists)
      echo(
      ...
      def mkReader(maker: ReaderMaker) =
        maker { () =>
          if (settings.noCompletion)
            ...
            new PresentationCompilerCompleter(intp)
        }
