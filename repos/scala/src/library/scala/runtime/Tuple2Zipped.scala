...
final class Tuple2Zipped[El1, Repr1, El2, Repr2](
  ...
  def map[B, To](f: (El1, El2) => B)(implicit cbf: CBF[Repr1, B, To]): To = {
    ...
    for (el1 <- colls._1) {
      if (elems2.hasNext) b += f(el1, elems2.next())
  ...
  def exists(@deprecatedName('f) p: (El1, El2) => Boolean): Boolean = {
    ...
    for (el1 <- colls._1) {
      if (elems2.hasNext) {
        if (p(el1, elems2.next())) return true
      } else return false
    }
    false
  }
  ...
  def foreach[U](f: (El1, El2) => U): Unit = {
    ...
    for (el1 <- colls._1) {
      if (elems2.hasNext) f(el1, elems2.next())
      else return
    }
  }

  override def toString =
    "(%s, %s).zipped".format(colls._1.toString, colls._2.toString)
}
