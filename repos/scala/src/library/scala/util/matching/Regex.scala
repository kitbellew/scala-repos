...
class Regex private[matching] (val pattern: Pattern, groupNames: String*)
  ...
  /** Create a new Regex with the same pattern, but no requirement that
    ...
    */
  def unanchored: UnanchoredRegex = new Regex(pattern, groupNames: _*)
    with UnanchoredRegex { override def anchored = outer }
...
object Regex {
  ...
  class MatchIterator(
    ...
    private[matching] def replacementData = new AbstractIterator[Match]
      with Replacement {
