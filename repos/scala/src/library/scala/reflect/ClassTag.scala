...
trait ClassTag[T]
  ...
  override def newArray(len: Int): Array[T] =
    runtimeClass match {
      ...
      case _ =>
        java.lang.reflect.Array.newInstance(runtimeClass, len).asInstanceOf[
          Array[T]]
    }
  ...
  def unapply(x: Any): Option[T] =
    if (null != x && (
          ...
          || (x.isInstanceOf[Char] && runtimeClass.isAssignableFrom(
            classOf[Char]))
          || (x.isInstanceOf[Int] && runtimeClass.isAssignableFrom(
            classOf[Int]))
  ...
  override def equals(x: Any) =
    x.isInstanceOf[ClassTag[_]] && this.runtimeClass == x.asInstanceOf[ClassTag[
      _]].runtimeClass
