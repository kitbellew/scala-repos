...
@deprecated(
  ...
  "2.11.0")
trait SeqProxyLike[+A, +Repr <: SeqLike[A, Repr] with Seq[A]]
    extends SeqLike[A, Repr]
    with IterableProxyLike[A, Repr] {
  ...
  override def indexOfSlice[B >: A](that: GenSeq[B]): Int =
    self.indexOfSlice(that)
  override def indexOfSlice[B >: A](that: GenSeq[B], from: Int): Int =
    self.indexOfSlice(that)
  override def lastIndexOfSlice[B >: A](that: GenSeq[B]): Int =
    self.lastIndexOfSlice(that)
  override def lastIndexOfSlice[B >: A](that: GenSeq[B], end: Int): Int =
    self.lastIndexOfSlice(that, end)
  ...
  override def union[B >: A, That](that: GenSeq[B])(implicit
      bf: CanBuildFrom[Repr, B, That]): That = self.union(that)(bf)
