...
trait GenericTraversableTemplate[+A, +CC[X] <: GenTraversable[X]]
  ...
  private def sequential: TraversableOnce[A] =
    this.asInstanceOf[GenTraversableOnce[A]].seq
  ...
  def unzip3[A1, A2, A3](implicit
      asTriple: A => (A1, A2, A3)): (CC[A1], CC[A2], CC[A3]) = {
    val b1 = genericBuilder[A1]
    val b2 = genericBuilder[A2]
  ...
  def flatten[B](implicit
      asTraversable: A => /*<:<!!!*/ GenTraversableOnce[B]): CC[B] = {
  ...
  def transpose[B](implicit
    ...
    val bs: IndexedSeq[Builder[B, CC[B]]] = IndexedSeq
      .fill(headSize)(genericBuilder[B])
    for (xs <- sequential) {
      var i = 0
