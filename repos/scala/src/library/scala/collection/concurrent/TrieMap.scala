...
private[collection] final class INode[K, V](bn: MainNode[K, V], g: Gen)
  ...
  def rec_lookup(
    ...
    m match {
      case cn: CNode[K, V] => // 1) a multinode
        ...
        val bmp = cn.bitmap
        if ((bmp & flag) == 0)
          null // 1a) bitmap shows no binding
...
private[collection] final class LNode[K, V](
    final val listmap: immutable.ListMap[K, V])
    extends MainNode[K, V] {
  def this(k: K, v: V) = this(immutable.ListMap(k -> v))
  def this(k1: K, v1: V, k2: K, v2: V) =
    this(immutable.ListMap(k1 -> v1, k2 -> v2))
...
private[collection] final class CNode[K, V](
    val bitmap: Int,
  ...
  def dual[K, V](
    ...
    if (lev < 35) {
      ...
      if (xidx == yidx) {
        ...
        if (xidx < yidx)
          new CNode(bmp, Array(x, y), gen)
        else
          new CNode(bmp, Array(y, x), gen)
      }
    } else {
...
final class TrieMap[K, V] private (
  ...
  private var hashingobj =
    if (hashf.isInstanceOf[Hashing.Default[_]])
      new TrieMap.MangledHashing[K]
    else
      hashf
  ...
  override def clear() {
    val r = RDCSS_READ_ROOT()
    if (!RDCSS_ROOT(r, r.gcasRead(this), INode.newRootNode[K, V]))
      clear()
  }
  ...
  def iterator: Iterator[(K, V)] =
    if (nonReadOnly)
      readOnlySnapshot().iterator
    else
      new TrieMapIterator(0, this)
...
private[collection] class TrieMapIterator[K, V](
    var level: Int,
...
private[concurrent] object Debug {
  ...
  def flush() {
    for (s <- JavaConversions.asScalaIterator(logbuffer.iterator()))
