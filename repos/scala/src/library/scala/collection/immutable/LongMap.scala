...
object LongMap {
  ...
  implicit def canBuildFrom[A, B] =
    new CanBuildFrom[LongMap[A], (Long, B), LongMap[B]] {
      def apply(from: LongMap[A]): Builder[(Long, B), LongMap[B]] = apply()
      def apply(): Builder[(Long, B), LongMap[B]] =
        new MapBuilder[Long, B, LongMap[B]](empty[B])
    }
...
sealed abstract class LongMap[+T]
  ...
  /**
    ...
    */
  override final def foreach[U](f: ((Long, T)) => U): Unit =
    this match {
      case LongMap.Bin(_, _, left, right) => {
        left.foreach(f);
        right.foreach(f)
  ...
  final override def apply(key: Long): T =
    this match {
      case LongMap.Bin(prefix, mask, left, right) =>
        if (zero(key, mask))
          left(key)
  ...
  override def updated[S >: T](key: Long, value: S): LongMap[S] =
    this match {
      case LongMap.Bin(prefix, mask, left, right) =>
