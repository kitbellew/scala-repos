...
private[immutable] object IntMapUtils extends BitOperations.Int {
  ...
  def bin[T](
    ...
    (left, right) match {
      case (left, IntMap.Nil) =>
        left
      case (IntMap.Nil, right) =>
        right
      case (left, right) =>
        IntMap.Bin(prefix, mask, left, right)
    }
}
...
object IntMap {
  ...
  implicit def canBuildFrom[A, B] =
    new CanBuildFrom[IntMap[A], (Int, B), IntMap[B]] {
      def apply(from: IntMap[A]): Builder[(Int, B), IntMap[B]] = apply()
      def apply(): Builder[(Int, B), IntMap[B]] =
        new MapBuilder[Int, B, IntMap[B]](empty[B])
    }
    ...
    override def equals(that: Any) =
      that match {
        case _: this.type =>
          true
        case _: IntMap[_] =>
          false // The only empty IntMaps are eq Nil
        case _ =>
          super.equals(that)
      }
  }
...
sealed abstract class IntMap[+T]
  ...
  final override def apply(key: Int): T =
    this match {
      case IntMap.Bin(prefix, mask, left, right) =>
        if (zero(key, mask))
          left(key)
      ...
      case (IntMap.Tip(key, value), that) =>
        that.get(key) match {
          case None =>
            IntMap.Nil
          case Some(value2) =>
            IntMap.Tip(key, f(key, value, value2))
        }
      case (_, IntMap.Tip(key, value)) =>
        this.get(key) match {
          case None =>
            IntMap.Nil
          case Some(value2) =>
            IntMap.Tip(key, f(key, value2, value))
        }
      case (_, _) =>
        IntMap.Nil
    }
  ...
  def ++[S >: T](that: IntMap[S]) = this.unionWith[S](that, (key, x, y) => y)
