...
class HashMap[A, +B]
  ...
  protected def filter0(
      ...
      negate: Boolean,
      level: Int,
      buffer: Array[HashMap[A, B @uV]],
      offset0: Int): HashMap[A, B] = null
  ...
  private[collection] def updated0[B1 >: B](
      key: A,
      hash: Int,
      level: Int,
      value: B1,
  ...
  /** Creates a new map which is the merge of this and the argument hash map.
    ...
    */
  def merged[B1 >: B](that: HashMap[A, B1])(
      mergef: MergeFunction[A, B1]): HashMap[A, B1] =
    merge0(that, 0, liftMerger(mergef))

  protected def merge0[B1 >: B](
      that: HashMap[A, B1],
      level: Int,
      merger: Merger[A, B1]): HashMap[A, B1] = that
...
object HashMap extends ImmutableMapFactory[HashMap] with BitOperations.Int {
  ...
  private def liftMerger[A1, B1](
      mergef: MergeFunction[A1, B1]): Merger[A1, B1] =
    if (mergef == null) defaultMerger.asInstanceOf[Merger[A1, B1]]
    else liftMerger0(mergef)
  ...
  private def makeHashTrieMap[A, B](
      hash0: Int,
      elem0: HashMap[A, B],
      hash1: Int,
      elem1: HashMap[A, B],
      level: Int,
      size: Int): HashTrieMap[A, B] = {
  ...
  class HashMap1[A, +B](
      private[collection] val key: A,
      private[collection] val hash: Int,
      private[collection] val value: (B @uV),
      private[collection] var kv: (A, B @uV))
      extends HashMap[A, B] {
    override def size = 1
  ...
  class HashTrieMap[A, +B](
      ...
      if ((bitmap & mask) != 0) {
        ...
        if (subNew eq sub) this
          ...
          if (bitmapNew != 0) {
            ...
            if (elemsNew.length == 1 && !elemsNew(0).isInstanceOf[
                  HashTrieMap[_, _]])
              elemsNew(0)
            else
            ...
            HashMap.empty[A, B]
        } else if (elems.length == 1 && !subNew.isInstanceOf[
                     HashTrieMap[_, _]]) {
          subNew
        } else {
    ...
    override protected def filter0(
      ...
      if (offset == offset0) {
        ...
        // unchanged
        this
      } else if (offset == offset0 + 1 && !buffer(offset0).isInstanceOf[
                   HashTrieMap[A, B]]) {
        // leaf
        buffer(offset0)
      } else {
    ...
    override def iterator: Iterator[(A, B)] =
      new TrieIterator[(A, B)](elems.asInstanceOf[Array[Iterable[(A, B)]]]) {
        final override def getElem(cc: AnyRef): (A, B) =
          cc.asInstanceOf[HashMap1[A, B]].ensurePair
      }
