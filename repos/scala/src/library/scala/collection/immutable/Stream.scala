...
abstract class Stream[+A]
  ...
  override def ++[B >: A, That](that: GenTraversableOnce[B])(implicit
      bf: CanBuildFrom[Stream[A], B, That]): That =
  ...
  /**
    ...
    * of `op`.
    */
  override final def scanLeft[B, That](z: B)(op: (B, A) => B)(implicit
      bf: CanBuildFrom[Stream[A], B, That]): That =
    if (isStreamBuilder(bf))
      asThat(
        if (isEmpty)
  ...
  /** Returns the stream resulting from applying the given function `f` to each
    ...
    */
  override final def map[B, That](f: A => B)(implicit
      bf: CanBuildFrom[Stream[A], B, That]): That = {
    if (isStreamBuilder(bf))
      asThat(
        if (isEmpty)
  ...
  override final def collect[B, That](pf: PartialFunction[A, B])(implicit
      bf: CanBuildFrom[Stream[A], B, That]): That = {
    if (!isStreamBuilder(bf))
      super.collect(pf)(bf)
    else {
      ...
      while (rest.nonEmpty && !runWith(rest.head))
        rest = rest.tail
      ...
      if (rest.isEmpty)
        Stream.Empty.asInstanceOf[That]
      else
        Stream.collectedTail(newHead, rest, pf, bf).asInstanceOf[That]
    }
  }
  ...
  override private[scala] def filterImpl(
    ...
    var rest = this
    while (!rest.isEmpty && p(rest.head) == isFlipped)
      rest = rest.tail
    // private utility func to avoid `this` on stack (would be needed for the lazy arg)
    if (rest.nonEmpty)
      Stream.filteredTail(rest, p, isFlipped)
    else
      Stream.Empty
  }
  ...
  override final def withFilter(p: A => Boolean): FilterMonadic[A, Stream[A]] =
    new Stream.StreamWithFilter(this, p)
  ...
  override final def zip[A1 >: A, B, That](
      that: scala.collection.GenIterable[B])(implicit
      bf: CanBuildFrom[Stream[A], (A1, B), That]): That =
  ...
  override def zipWithIndex[A1 >: A, That](implicit
      bf: CanBuildFrom[Stream[A], (A1, Int), That]): That =
  ...
  override def dropRight(n: Int): Stream[A] = {
    ...
    def advance(stub0: List[A], stub1: List[A], rest: Stream[A]): Stream[A] = {
      if (rest.isEmpty)
        Stream.empty
      else if (stub0.isEmpty)
        advance(stub1.reverse, Nil, rest)
      else
        cons(stub0.head, advance(stub0.tail, rest.head :: stub1, rest.tail))
    }
    if (n <= 0)
      this
    else
      advance((this take n).toList, Nil, this drop n)
  }
  ...
  override def distinct: Stream[A] = {
    ...
    def loop(seen: Set[A], rest: Stream[A]): Stream[A] = {
      if (rest.isEmpty)
        rest
      else if (seen(rest.head))
        loop(seen, rest.tail)
      else
        cons(rest.head, loop(seen + rest.head, rest.tail))
    }
    loop(Set(), this)
  }
  ...
  override def padTo[B >: A, That](len: Int, elem: B)(implicit
    ...
    def loop(len: Int, these: Stream[A]): Stream[B] =
      if (these.isEmpty)
        Stream.fill(len)(elem)
      else
        cons(these.head, loop(len - 1, these.tail))

    if (isStreamBuilder(bf))
      asThat(loop(len, this))
    else
  ...
  override def flatten[B](implicit
      asTraversable: A => /*<:<!!!*/ GenTraversableOnce[B]): Stream[B] = {
...
object Stream extends SeqFactory[Stream] {
  ...
  private[immutable] final class StreamWithFilter[A](
    ...
    def map[B, That](f: A => B)(implicit
        bf: CanBuildFrom[Stream[A], B, That]): That = filtered map f
