...
object Iterator {
  ...
  /** With the advent of `TraversableOnce` and `Iterator`, it can be useful to have a builder which
    ...
    */
  implicit def IteratorCanBuildFrom[A] =
    new TraversableOnce.BufferedCanBuildFrom[A, Iterator] {
  ...
  def iterate[T](start: T)(f: T => T): Iterator[T] =
    new AbstractIterator[T] {
      ...
      def next(): T = {
        if (first) first = false else acc = f(acc)
  ...
  /** Avoid stack overflows when applying ++ to lots of iterators by
    ...
    */
  private[scala] final class ConcatIterator[+A](
...
trait Iterator[+A] extends TraversableOnce[A] {
  ...
  protected def sliceIterator(from: Int, until: Int): Iterator[A] = {
    ...
    if (rest == 0) empty else new Iterator.SliceIterator(this, lo, rest)
  }
  ...
  def ++[B >: A](that: => GenTraversableOnce[B]): Iterator[B] =
    new Iterator.JoinIterator(self, that)
  ...
  /** Tests whether every element of this iterator relates to the
    ...
    */
  def corresponds[B](that: GenTraversableOnce[B])(
      p: (A, B) => Boolean): Boolean = {
    val that0 = that.toIterator
    while (hasNext && that0.hasNext) if (!p(next(), that0.next())) return false
  ...
  def scanLeft[B](z: B)(op: (B, A) => B): Iterator[B] =
    new AbstractIterator[B] {
      ...
      def next() =
        if (hasNext) {
          val res = elem
          if (self.hasNext) elem = op(elem, self.next()) else hasNext = false
          res
        } else Iterator.empty.next()
    }
    ...
    new AbstractIterator[A] {
      ...
      def hasNext =
        hdDefined || tail.hasNext && {
          hd = tail.next()
          if (p(hd)) hdDefined = true else tail = Iterator.empty
          hdDefined
        }
      def next() =
        if (hasNext) { hdDefined = false; hd }
  ...
  def partition(p: A => Boolean): (Iterator[A], Iterator[A]) = {
    ...
    class PartitionIterator(p: A => Boolean) extends AbstractIterator[A] {
      ...
      def next() =
        if (!lookahead.isEmpty) lookahead.dequeue() else { skip(); self.next() }
    }
  ...
  def zipAll[B, A1 >: A, B1 >: B](
    ...
    new AbstractIterator[(A1, B1)] {
      ...
      def next(): (A1, B1) =
        ...
        } else { if (that.hasNext) (thisElem, that.next()) else empty.next() }
    }
  ...
  class GroupedIterator[B >: A](self: Iterator[A], size: Int, step: Int)
    ...
    def next = {
      ...
      if (!filled) throw new NoSuchElementException("next on empty iterator")
      filled = false
      buffer.toList
    }
  }
  ...
  def duplicate: (Iterator[A], Iterator[A]) = {
    ...
    class Partner extends AbstractIterator[A] {
      def hasNext: Boolean =
        self.synchronized { (this ne ahead) && !gap.isEmpty || self.hasNext }
      def next(): A =
        self.synchronized {
  ...
  def patch[B >: A](
    ...
    new AbstractIterator[B] {
      ...
      private var i =
        (if (from > 0) from
         else 0) // Counts down, switch to patch on 0, -1 means use patch first
      def hasNext: Boolean = {
        if (i == 0) {
      ...
      def next(): B = {
        ...
        if (i < 0) {
          if (patchElems.hasNext) patchElems.next() else origElems.next()
        } else {
