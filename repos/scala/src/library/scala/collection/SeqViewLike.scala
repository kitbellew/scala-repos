...
trait SeqViewLike[
  ...
  trait FlatMapped[B] extends super.FlatMapped[B] with Transformed[B] {
    ...
    protected[this] def findRow(idx: Int, lo: Int, hi: Int): Int = {
      val mid = (lo + hi) / 2
      if (idx < index(mid))
        findRow(idx, lo, mid - 1)
      else if (idx >= index(mid + 1))
        findRow(idx, mid + 1, hi)
      else
        mid
    }
  ...
  trait Prepended[B >: A] extends super.Prepended[B] with Transformed[B] {
    ...
    def apply(idx: Int): B =
      if (idx < fstSeq.length)
        fstSeq(idx)
      else
        self.apply(idx - fstSeq.length)
  }
  ...
  trait Zipped[B] extends super.Zipped[B] with Transformed[(A, B)] {
    ...
    def length =
      if ((thatSeq lengthCompare self.length) <= 0)
        thatSeq.length
      else
        self.length
  ...
  trait Patched[B >: A] extends Transformed[B] {
    ...
    def apply(idx: Int): B = {
      val actualFrom =
        if (self.lengthCompare(from) < 0)
          self.length
        else
          from
      if (idx < actualFrom)
        self.apply(idx)
      else if (idx < actualFrom + plen)
        patch.apply(idx - actualFrom)
      else
        self.apply(idx - plen + replaced)
    }
  ...
  protected override def newZipped[B](
      that: GenIterable[B]): Transformed[(A, B)] =
    new {
      val other = that
  ...
  protected override def newTaken(n: Int): Transformed[A] =
    newSliced(SliceInterval(0, n))
  protected override def newDropped(n: Int): Transformed[A] =
...
//    else super.patch[B, That](from, patch, replaced)(bf)
  ...
  override def reverseMap[B, That](f: A => B)(implicit
      bf: CanBuildFrom[This, B, That]): That = reverse map f
  ...
  override def :+[B >: A, That](elem: B)(implicit
      bf: CanBuildFrom[This, B, That]): That = ++(Iterator.single(elem))(bf)
