...
trait ParIterableLike[
  ...
  def map[S, That](f: T => S)(implicit bf: CanBuildFrom[Repr, S, That]): That =
    if (bf(repr).isCombiner) {
      ...
      setTaskSupport(seq.map(f)(bf2seq(bf)), tasksupport)
  /*bf ifParallel { pbf =>
  ...
  def collect[S, That](pf: PartialFunction[T, S])(
    ...
    if (bf(repr).isCombiner) {
      ...
      setTaskSupport(seq.collect(pf)(bf2seq(bf)), tasksupport)
  /*bf ifParallel { pbf =>
  ...
  def flatMap[S, That](f: T => GenTraversableOnce[S])(
    ...
    if (bf(repr).isCombiner) {
      ...
      setTaskSupport(seq.flatMap(f)(bf2seq(bf)), tasksupport)
  /*bf ifParallel { pbf =>
  ...
  def take(n: Int): Repr = {
    val actualn =
      if (size > n)
        ...
        size
    if (actualn < MIN_FOR_COPY)
      take_sequential(actualn)
    else
      tasksupport.executeAndWaitResult(
  ...
  def drop(n: Int): Repr = {
    val actualn =
      if (size > n)
        ...
        size
    if ((size - actualn) < MIN_FOR_COPY)
      drop_sequential(actualn)
    else
      tasksupport.executeAndWaitResult(
  ...
  override def slice(unc_from: Int, unc_until: Int): Repr = {
    ...
    if ((until - from) <= MIN_FOR_COPY)
      slice_sequential(from, until)
    else
      tasksupport.executeAndWaitResult(
    ...
    if (bf(repr).isCombiner) {
      if (tasksupport.parallelismLevel > 1) {
        if (size > 0)
          ...
          setTaskSupport((bf(repr) += z).result(), tasksupport)
      } else
        setTaskSupport(seq.scan(z)(op)(bf2seq(bf)), tasksupport)
    } else
      setTaskSupport(seq.scan(z)(op)(bf2seq(bf)), tasksupport)
  ...
  def zip[U >: T, S, That](that: GenIterable[S])(
    ...
    if (bf(repr).isCombiner && that.isParSeq) {
      ...
      setTaskSupport(seq.zip(that)(bf2seq(bf)), tasksupport)
  ...
  protected[this] class Aggregate[S](
    ...
    override def merge(that: Aggregate[S]) =
      result = combop(result, that.result)
  }
  ...
  protected[this] class Product[U >: T](
    ...
    override def merge(that: Product[U]) =
      result = num.times(result, that.result)
  }
  ...
  protected[this] class Min[U >: T](
    ...
    override def merge(that: Min[U]) =
      if (this.result == None)
        ...
        result =
          if (ord.lteq(result.get, that.result.get))
            result
          else
            that.result
    override def requiresStrictSplitters = true
  ...
  protected[this] class Max[U >: T](
    ...
    override def merge(that: Max[U]) =
      if (this.result == None)
        ...
        result =
          if (ord.gteq(result.get, that.result.get))
            result
          else
            that.result
    override def requiresStrictSplitters = true
  ...
  protected[this] class Map[S, That](
    ...
    def leaf(prev: Option[Combiner[S, That]]) =
      result = pit.map2combiner(f, reuse(prev, cbf()))
  ...
  protected[this] class Collect[S, That](
    ...
    @volatile var result: Combiner[S, That] = null
    def leaf(prev: Option[Combiner[S, That]]) =
      result = pit.collect2combiner[S, That](pf, pbf())
    protected[this] def newSubtask(p: IterableSplitter[T]) =
      new Collect(pf, pbf, p)
    override def merge(that: Collect[S, That]) =
      ...
      for ((p, untilp) <- pits zip sizes; if untilp <= n)
        yield {
          if (untilp + p.remaining < n)
            ...
            new Take(n - untilp, cbf, p)
        }
    }
    override def merge(that: Take[U, This]) =
      result = result combine that.result
    override def requiresStrictSplitters = true
  }
  ...
  protected[this] class Drop[U >: T, This >: Repr](
    ...
    @volatile var result: Combiner[U, This] = null
    def leaf(prev: Option[Combiner[U, This]]) =
      result = pit.drop2combiner(n, reuse(prev, cbf()))
    protected[this] def newSubtask(p: IterableSplitter[T]) =
      throw new UnsupportedOperationException
    override def split = {
      val pits = pit.splitWithSignalling
      ...
      for ((p, withp) <- pits zip sizes.tail; if withp >= n)
        yield {
          if (withp - p.remaining > n)
            ...
            new Drop(n - withp + p.remaining, cbf, p)
        }
    }
    override def merge(that: Drop[U, This]) =
      result = result combine that.result
    override def requiresStrictSplitters = true
  }
  ...
  protected[this] class Slice[U >: T, This >: Repr](
    ...
    @volatile var result: Combiner[U, This] = null
    def leaf(prev: Option[Combiner[U, This]]) =
      result = pit.slice2combiner(from, until, reuse(prev, cbf()))
    protected[this] def newSubtask(p: IterableSplitter[T]) =
      throw new UnsupportedOperationException
    override def split = {
      val pits = pit.splitWithSignalling
      ...
      for ((p, untilp) <- pits zip sizes;
           if untilp + p.remaining >= from || untilp <= until)
        yield {
    ...
    override def merge(that: Slice[U, This]) =
      result = result combine that.result
    override def requiresStrictSplitters = true
  }
  ...
  protected[this] class TakeWhile[U >: T, This >: Repr](
    ...
    def leaf(prev: Option[(Combiner[U, This], Boolean)]) =
      if (pos < pit.indexFlag) {
        ...
        if (!result._2)
          pit.setIndexFlagIfLesser(pos)
      } else
        result = (reuse(prev.map(_._1), cbf()), false)
    protected[this] def newSubtask(p: IterableSplitter[T]) =
      throw new UnsupportedOperationException
    override def split = {
      val pits = pit.splitWithSignalling
  ...
  protected[this] class Zip[U >: T, S, That](
    ...
    def leaf(prev: Option[Result]) =
      result = pit.zip2combiner[U, S, That](othpit, pbf())
    protected[this] def newSubtask(p: IterableSplitter[T]) =
      throw new UnsupportedOperationException
    override def split = {
      val pits = pit.splitWithSignalling
  ...
  protected[this] class ToParCollection[U >: T, That](
    ...
    override def merge(that: ToParCollection[U, That]) =
      result = result combine that.result
  }
  ...
  protected[this] class ToParMap[K, V, That](
    ...
    override def merge(that: ToParMap[K, V, That]) =
      result = result combine that.result
  }
  ...
  protected[this] class CreateScanTree[U >: T](
    ...
    override def merge(that: CreateScanTree[U]) =
      if (this.result != null) {
        if (that.result != null)
          result = ScanNode(result, that.result)
      } else
        result = that.result
    override def requiresStrictSplitters = true
  }
