...
trait ParMap[K, V]
  ...
  override def updated[U >: V](key: K, value: U): ParMap[K, U] =
    this + ((key, value))
  ...
  /** The same map with a given default function.
    ...
    */
  def withDefault(d: K => V): scala.collection.parallel.mutable.ParMap[K, V] =
    new ParMap.WithDefault[K, V](this, d)
...
object ParMap extends ParMapFactory[ParMap] {
  ...
  class WithDefault[K, V](underlying: ParMap[K, V], d: K => V)
    ...
    /** If these methods aren't overridden to thread through the underlying map,
      ...
      */
    override def withDefault(d: K => V): ParMap[K, V] =
