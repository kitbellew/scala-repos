...
private[mutable] abstract class ParHashMapCombiner[K, V](
  ...
  def result: ParHashMap[K, V] =
    if (size >= (ParHashMapCombiner.numblocks * sizeMapBucketSize)) {
      // 1024
      // construct table
  ...
  /** A hash table which will never resize itself. Knowing the number of elements in advance,
    ...
    *  in the table.
    */
  private[ParHashMapCombiner] class AddingHashTable(
      numelems: Int,
      lf: Int,
      _seedvalue: Int)
  ...
  class FillBlocks(
      buckets: Array[Unrolled[DefaultEntry[K, V]]],
      table: AddingHashTable,
      offset: Int,
      howmany: Int)
