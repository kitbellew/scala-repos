...
/** Augments iterators with additional methods, mostly transformers,
  ...
  */
private[collection] trait AugmentedIterableIterator[+T]
  ...
  def filterNot2combiner[U >: T, This](
      pred: T => Boolean,
      cb: Combiner[U, This]): Combiner[U, This] = {
    while (hasNext) {
      val curr = next()
  ...
  def partition2combiners[U >: T, This](
      pred: T => Boolean,
      btrue: Combiner[U, This],
  ...
  def splitAt2combiners[U >: T, This](
      at: Int,
      before: Combiner[U, This],
      after: Combiner[U, This]) = {
    before.sizeHint(at)
    if (isRemainingCheap)
  ...
  def zip2combiner[U >: T, S, That](
      otherpit: RemainsIterator[S],
  ...
  def zipAll2combiner[U >: T, S, That](
      that: RemainsIterator[S],
      thiselem: U,
      thatelem: S,
      cb: Combiner[(U, S), That]): Combiner[(U, S), That] = {
    ...
    while (this.hasNext && that.hasNext)
      cb += ((this.next(), that.next()))
    while (this.hasNext)
      cb += ((this.next(), thatelem))
    while (that.hasNext)
      cb += ((thiselem, that.next()))
    cb
  }

}

private[collection] trait AugmentedSeqIterator[+T]
  ...
  def reverseMap2combiner[S, That](
      f: T => S,
      cb: Combiner[S, That]): Combiner[S, That] = {
    //val cb = cbf(repr)
    if (isRemainingCheap)
  ...
  def updated2combiner[U >: T, That](
      index: Int,
      elem: U,
      cb: Combiner[U, That]): Combiner[U, That] = {
    //val cb = cbf(repr)
    if (isRemainingCheap)
      cb.sizeHint(remaining)
    var j = 0
...
trait IterableSplitter[+T]
  ...
  def splitWithSignalling: Seq[IterableSplitter[T]] = {
    val pits = split
    pits foreach {
      _.signalDelegate = signalDelegate
    }
    pits
  }
  ...
  class Mapped[S](f: T => S) extends IterableSplitter[S] {
    ...
    def split: Seq[IterableSplitter[S]] = self.split.map {
      _ map f
    }
  }
  ...
  class Appended[U >: T, PI <: IterableSplitter[U]](protected val that: PI)
  ...
  def appendParIterable[U >: T, PI <: IterableSplitter[U]](that: PI) =
    new Appended[U, PI](that)

  class Zipped[S](protected val that: SeqSplitter[S])
      extends IterableSplitter[(T, S)] {
    signalDelegate = self.signalDelegate
  ...
  class ZippedAll[U >: T, S](
      protected val that: SeqSplitter[S],
      protected val thiselem: U,
      protected val thatelem: S)
  ...
  def zipAllParSeq[S, U >: T, R >: S](
      that: SeqSplitter[S],
      thisElem: U,
      thatElem: R) = new ZippedAll[U, R](that, thisElem, thatElem)
}
...
trait SeqSplitter[+T]
  ...
  class Mapped[S](f: T => S) extends super.Mapped[S](f) with SeqSplitter[S] {
    ...
    def psplit(sizes: Int*): Seq[SeqSplitter[S]] = self.psplit(sizes: _*).map {
      _ map f
    }
  }
  ...
  class Appended[U >: T, PI <: SeqSplitter[U]](it: PI)
    ...
    def psplit(sizes: Int*): Seq[SeqSplitter[U]] =
      if (firstNonEmpty) {
        ...
        curr.asInstanceOf[SeqSplitter[U]].psplit(sizes: _*)
  }
  ...
  class ZippedAll[U >: T, S](ti: SeqSplitter[S], thise: U, thate: S)
      extends super.ZippedAll[U, S](ti, thise, thate)
      with SeqSplitter[(U, S)] {
  ...
  override def zipAllParSeq[S, U >: T, R >: S](
      that: SeqSplitter[S],
      thisElem: U,
      thatElem: R) = new ZippedAll[U, R](that, thisElem, thatElem)
  ...
  class Patched[U >: T](from: Int, patch: SeqSplitter[U], replaced: Int)
      extends SeqSplitter[U] {
    signalDelegate = self.signalDelegate
  ...
  def patchParSeq[U >: T](
      from: Int,
      patchElems: SeqSplitter[U],
      replaced: Int) = new Patched(from, patchElems, replaced)
