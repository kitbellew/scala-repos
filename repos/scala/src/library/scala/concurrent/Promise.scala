...
trait Promise[T] {
  ...
  /** Completes the promise with a value.
    ...
    *  $promiseCompletion
    */
  def success(@deprecatedName('v) value: T): this.type = complete(
    Success(value))
  ...
  /** Completes the promise with an exception.
    ...
    *  $promiseCompletion
    */
  def failure(@deprecatedName('t) cause: Throwable): this.type = complete(
    Failure(cause))
  ...
  /** Tries to complete the promise with an exception.
    ...
    *  @return    If the promise has already been completed returns `false`, or `true` otherwise.
    */
  def tryFailure(@deprecatedName('t) cause: Throwable): Boolean = tryComplete(
    Failure(cause))
}
