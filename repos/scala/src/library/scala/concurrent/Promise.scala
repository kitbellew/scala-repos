...
trait Promise[T] {
  ...
  final def tryCompleteWith(other: Future[T]): this.type = {
    if (other ne
          this.future) { // this tryCompleteWith this doesn't make much sense
  ...
  /** Completes the promise with a value.
    ...
    *  $promiseCompletion
    */
  def success(@deprecatedName('v) value: T): this.type =
  ...
  /** Completes the promise with an exception.
    ...
    *  $promiseCompletion
    */
  def failure(@deprecatedName('t) cause: Throwable): this.type =
