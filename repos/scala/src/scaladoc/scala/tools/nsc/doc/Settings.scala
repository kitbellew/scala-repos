...
class Settings(error: String => Unit, val printMsg: String => Unit = println(_))
  ...
  lazy val uncompilableFiles =
    docUncompilable.value match {
      case "" =>
        Nil
      case path =>
  ...
  val docAuthor = BooleanSetting("-author", "Include authors.")
  ...
  def skipPackage(qname: String) = skipPackageNames(qname.toLowerCase)

  lazy val hiddenImplicits: Set[String] = {
    if (docImplicitsHide.value.isEmpty)
      hardcoded.commonConversionTargets
    else
  ...
  lazy val extUrlMapping: Map[String, String] =
    docExternalDoc.value flatMap { s =>
      val idx = s.indexOf("#")
      if (idx > 0) {
  ...
  object hardcoded {
    ...
    private val excludedClassnamePatterns = Set(
      ...
      """^scala.Product.*""",
      """^scala.Function.*""",
      """^scala.runtime.AbstractFunction.*""") map (_.r)
