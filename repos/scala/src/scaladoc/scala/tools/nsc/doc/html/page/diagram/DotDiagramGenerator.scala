...
class DotDiagramGenerator(settings: doc.Settings, dotRunner: DotRunner)
  ...
  private def generateDot(d: Diagram) = {
    ...
    d match {
      case InheritanceDiagram(
        ...
        superClasses =
          if (_superClasses.length >
                settings.docDiagramsMaxNormalClasses.value) {
        ...
        incomingImplicits =
          if (_incomingImplicits.length >
                settings.docDiagramsMaxImplicitClasses.value) {
        ...
        node2Index =
          (thisNode :: subClasses ::: superClasses ::: incomingImplicits :::
            outgoingImplicits).zipWithIndex.toMap
    ...
    val implicitsDot = {
      if (!isInheritanceDiagram) ""
          ...
          if (incomingImplicits.isEmpty) ""
            ...
            "subgraph clusterIncoming {\n" + "style=\"invis\"\n" +
              ...
              (if (incomingImplicits.size > 1)
                 incomingImplicits.map(n => "node" + node2Index(n))
                   .mkString(" -> ") +
          ...
          if (outgoingImplicits.isEmpty) ""
            ...
            "subgraph clusterOutgoing {\n" + "style=\"invis\"\n" +
              ...
              (if (outgoingImplicits.size > 1)
                 outgoingImplicits.map(n => "node" + node2Index(n))
                   .mkString(" -> ") +
                   " [constraint=\"false\", style=\"invis\", minlen=\"0.0\"];\n"
               else "") + "}"
        }
        ...
        val outgoingTooltip = thisNode.name +
          " can be implicitly converted to " + outgoingImplicits.map(_.name)
            .mkString(", ")
        "subgraph clusterAll {\n" + "style=\"invis\"\n" + outgoingCluster +
          "\n" + thisCluster + "\n" + incomingCluster + "\n" +
    ...
    val graph = "digraph G {\n" +
      ...
      edges.map {
        case (from, tos) =>
          tos.map(to => {
            val id = "graph" + counter + "_" + node2Index(to) + "_" +
              node2Index(from)
  ...
  private def node2Dot(node: Node) = {
    ...
    node.doctpl match {
      case Some(tpl) =>
        attr += "URL" -> (page.relativeLinkTo(tpl) + "#inheritance-diagram")
      case _ =>
    }
    ...
    if (!img.equals("")) {
      img = "<TD><IMG SCALE=\"TRUE\" SRC=\"" + settings.outdir.value + "/lib/" +
        img + "\" /></TD>"
      name = name + " "
    }
    val label = "<<TABLE BORDER=\"0\" CELLBORDER=\"0\">" + "<TR>" + img +
        ...
        try {
          ...
          case exc: Exception =>
            if (settings.docDiagramsDebug) {
              ...
              settings.printMsg(
                "Use " + settings.docDiagramsDebug.name +
  ...
  /**
    ...
    */
  private def transform(e: scala.xml.Node): scala.xml.Node =
    e match {
