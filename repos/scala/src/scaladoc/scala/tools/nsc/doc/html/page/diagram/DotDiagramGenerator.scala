...
class DotDiagramGenerator(settings: doc.Settings, dotRunner: DotRunner)
  ...
  private def generateDot(d: Diagram) = {
    ...
    d match {
      case InheritanceDiagram(
        ...
        superClasses =
          if (_superClasses.length > settings.docDiagramsMaxNormalClasses
                .value) {
        ...
        incomingImplicits =
          if (_incomingImplicits.length > settings.docDiagramsMaxImplicitClasses
                .value) {
        ...
        outgoingImplicits =
          if (_outgoingImplicits.length > settings.docDiagramsMaxImplicitClasses
                .value) {
    ...
    val implicitsDot = {
      if (!isInheritanceDiagram) ""
        ...
        val incomingTooltip = incomingImplicits.map(_.name)
  ...
  private def node2Dot(node: Node) = {
    ...
    if (!img.equals("")) {
      img = "<TD><IMG SCALE=\"TRUE\" SRC=\"" + settings.outdir
        .value + "/lib/" + img + "\" /></TD>"
      name = name + " "
    }
