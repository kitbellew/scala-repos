...
trait EntityPage extends HtmlPage {
        ...
        <li class={
          ...
          mbr match {
            case dtpl: DocTemplateEntity =>
              dtpl
                .companion
                .fold(<span class="separator"></span>) { c: DocTemplateEntity =>
                  <a class="object" href={
                    relativeLinkTo(c)
                  } title={
  ...
  val constructors = (tpl match {
    case cls: Class =>
      (cls.constructors: List[MemberEntity]).sorted
    case _ =>
      Nil
  })
  ...
  def memberToCommentBodyHtml(
    ...
    val paramComments = {
      val prs: List[ParameterEntity] =
        mbr match {
          case cls: Class =>
            cls.typeParams ::: cls.valueParams.flatten
          case trt: Trait =>
            trt.typeParams
          case dfe: Def =>
            dfe.typeParams ::: dfe.valueParams.flatten
          case ctr: Constructor =>
            ctr.valueParams.flatten
          case _ =>
            Nil
        }
      ...
      mbr
        ...
        .fold(NodeSeq.Empty) { comment =>
          val cmtedPrs = prs filter {
            case tp: TypeParam =>
              comment.typeParams isDefinedAt tp.name
            case vp: ValueParam =>
              comment.valueParams isDefinedAt vp.name
          }
      ...
      mbr.byConversion match {
        case Some(conv) =>
          <dt class="implicit">Implicit</dt> ++ {
            ...
            val conversionOwnerQualifiedNane = conv
              .convertorOwner
              .qualifiedName
              .stripSuffix(".package")
    ...
    def bound0(bnd: Option[TypeEntity], pre: String): NodeSeq =
      bnd match {
        case None =>
          NodeSeq.Empty
        case Some(tpe) =>
          scala.xml.Text(pre) ++ typeToHtml(tpe, hasLinks)
      }
    bound0(lo, " >: ") ++ bound0(hi, " <: ")
  }
    ...
    def inside(hasLinks: Boolean, nameLink: String = ""): NodeSeq =
      ...
      <span class="symbol">
        ...
        if (isReduced)
          ...
          mbr match {
            case cls: Class =>
              paramsToHtml(cls.valueParams)
            case ctr: Constructor =>
              paramsToHtml(ctr.valueParams)
            case dfe: Def =>
              paramsToHtml(dfe.valueParams)
            case _ =>
              NodeSeq.Empty
          }
        }
    ...
    def codeStringToXml(text: String): NodeSeq = {
      ...
      for (c <- text)
        c match {
          ...
          case _ =>
            goodLookingXml ++= Text(c.toString)
        }
