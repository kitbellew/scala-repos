...
class ModelFactory(val global: Global, val settings: doc.Settings) {
  ...
  abstract class EntityImpl(val sym: Symbol, val inTpl: TemplateImpl)
    ...
    def annotations =
      sym.annotations.filterNot(_.tpe =:= typeOf[macroImpl]).map(makeAnnotation)
  ...
  abstract class DocTemplateImpl(sym: Symbol, inTpl: DocTemplateImpl)
    ...
    def sourceUrl = {
      ...
      if (!settings.docsourceurl.isDefault) inSource map {
        case (file, _) =>
          val filePath = fixPath(file.path)
            .replaceFirst("^" + assumedSourceRoot, "").stripSuffix(".scala")
    ...
    def registerSubClass(sc: DocTemplateEntity): Unit = {
      if (subClassesCache != null) subClassesCache += sc
    }
    ...
    lazy val memberSyms = sym.info.members
      .filter(s => membersShouldDocument(s, this)).toList

    // the inherited templates (classes, traits or objects)
    val memberSymsLazy = memberSyms
      .filter(t => templateShouldDocument(t, this) && !inOriginalOwner(t, this))
  ...
  abstract class NonTemplateMemberImpl(
    ...
    override def isShadowedImplicit =
      isImplicitlyInherited && inTpl.implicitsShadowing.get(this)
        .map(_.isShadowed).getOrElse(false)
    override def isAmbiguousImplicit =
      isImplicitlyInherited && inTpl.implicitsShadowing.get(this)
        .map(_.isAmbiguous).getOrElse(false)
    ...
    def lo =
      sym.info.bounds match {
        case TypeBounds(lo, hi) if lo.typeSymbol != NothingClass =>
          Some(makeTypeInTemplateContext(
  ...
  object modelCreation {
    ...
    def createTemplate(
      ...
      def createDocTemplate(
        ...
        else if (bSym.isModule) new DocTemplateImpl(bSym, inTpl) with Object {}
        else if (bSym.isTrait) new DocTemplateImpl(bSym, inTpl) with Trait {}
      ...
      if (bSym == RootPackage) // (1)
        ...
        if (settings.skipPackage(makeQualifiedName(bSym))) None
        else
          inTpl match {
    ...
    def createLazyTemplateMember(
      ...
      if (docTemplatesCache isDefinedAt bSym) docTemplatesCache(bSym)
      else
          ...
          if (bSym == definitions.Object_synchronized) {
            val cSymInfo = (bSym.info: @unchecked) match {
              case PolyType(ts, MethodType(List(bp), mt)) =>
                val cp = bp.cloneSymbol.setPos(bp.pos)
    ...
    new ParameterImpl(aSym, inTpl) with ValueParam {
      ...
      def defaultValue =
        if (aSym.hasDefault) {
          ...
          (currentRun.units filter (_.source.file == aSym.sourceFile))
            .toList match {
            case List(unit) =>
  ...
  def isPureBridge(sym: Symbol) =
    sym.isBridge && sym.allOverriddenSymbols.forall(_.isBridge)
