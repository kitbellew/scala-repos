...
class ModelFactory(val global: Global, val settings: doc.Settings) {
  ...
  abstract class MemberImpl(sym: Symbol, inTpl: DocTemplateImpl)
    ...
    def deprecation =
      if (sym.isDeprecated)
        Some(
          (sym.deprecationMessage, sym.deprecationVersion) match {
    ...
    def migration =
      if (sym.hasMigrationAnnotation)
        Some(
          (sym.migrationMessage, sym.migrationVersion) match {
            case (Some(msg), Some(ver)) =>
              parseWiki(
    ...
    def isAbstract =
      ...
      (
        (
          ...
          sym.isAbstractClass || sym.isAbstractType
      ) && !sym.isSynthetic
        ...
        mbr match {
          case hk: HigherKinded if !hk.typeParams.isEmpty =>
            ...
            "[" + hk
              ...
              .map(tp =>
                tp.variance + tp
                  .name + tParams(tp) + boundsToString(tp.hi, tp.lo))
              .mkString(", ") + "]"
          case _ =>
            ""
        }
  ...
  def makeMember(
    ...
    if (!localShouldDocument(aSym) || aSym.isModuleClass || aSym
          .isPackageObject || aSym.isMixinConstructor)
      Nil
    else {
  ...
  def templateShouldDocument(aSym: Symbol, inTpl: DocTemplateImpl): Boolean =
    (
      aSym.isTrait || aSym.isClass || aSym
        .isModule || typeShouldDocument(aSym, inTpl)
    ) &&
