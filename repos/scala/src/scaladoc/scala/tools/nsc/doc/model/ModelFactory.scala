...
class ModelFactory(val global: Global, val settings: doc.Settings) {
  ...
  abstract class DocTemplateImpl(sym: Symbol, inTpl: DocTemplateImpl)
    ...
    def groupSearch[T](extractor: Comment => Option[T]): Option[T] = {
      val comments = comment +:
        linearizationTemplates.collect {
  ...
  def makeMember(
    ...
    if (!localShouldDocument(aSym) || aSym.isModuleClass ||
        aSym.isPackageObject || aSym.isMixinConstructor)
      Nil
    else {
        ...
        if (aSym.hasDefault) {
          ...
          (currentRun.units filter (_.source.file == aSym.sourceFile))
            ...
            case List(unit) =>
              ...
              unit.body find
                (t => isCorrespondingParam(t.symbol)) collect {
