...
abstract class ScaladocModelTest extends DirectTest {
  ...
  private[this] def newDocFactory: DocFactory = {
    ...
    settings.scaladocQuietRun =
      true // yaay, no more "model contains X documentable templates"!
    val args = extraSettings + " " + scaladocSettings
    new ScalaDoc.Command(
      (CommandLineParser tokenize (args)),
      settings
    ) // side-effecting, I think
  ...
  object access {
    ...
    implicit class TemplateAccess(tpl: DocTemplateEntity) {
      ...
      def _classes(name: String): List[DocTemplateEntity] = tpl.templates
        .filter(_.name == name)
        .collect({ case c: DocTemplateEntity with Class => c })

      def _classMbr(name: String): MemberTemplateEntity = getTheFirst(
        _classesMbr(name),
        tpl.qualifiedName + ".classMember(" + name + ")")
      def _classesMbr(name: String): List[MemberTemplateEntity] = tpl.templates
        .filter(_.name == name)
      ...
      def _traits(name: String): List[DocTemplateEntity] = tpl.templates
        .filter(_.name == name)
        .collect({ case t: DocTemplateEntity with Trait => t })

      def _traitMbr(name: String): MemberTemplateEntity = getTheFirst(
        _traitsMbr(name),
        tpl.qualifiedName + ".traitMember(" + name + ")")
      def _traitsMbr(name: String): List[MemberTemplateEntity] = tpl.templates
        .filter(_.name == name)
      ...
      def _objects(name: String): List[DocTemplateEntity] = tpl.templates
        .filter(_.name == name)
        .collect({ case o: DocTemplateEntity with Object => o })

      def _objectMbr(name: String): MemberTemplateEntity = getTheFirst(
        _objectsMbr(name),
        tpl.qualifiedName + ".objectMember(" + name + ")")
      def _objectsMbr(name: String): List[MemberTemplateEntity] = tpl.templates
        .filter(_.name == name)
      ...
      def _conversion(name: String): ImplicitConversion = getTheFirst(
        _conversions(name),
      ...
      def _absType(name: String): MemberEntity = getTheFirst(
        _absTypes(name),
      ...
      def _absTypeTpl(name: String): DocTemplateEntity = getTheFirst(
        _absTypeTpls(name),
        tpl.qualifiedName + ".abstractType(" + name + ")")
      ...
      def _aliasType(name: String): MemberEntity = getTheFirst(
        _aliasTypes(name),
      ...
      def _aliasTypeTpl(name: String): DocTemplateEntity = getTheFirst(
        _aliasTypeTpls(name),
        tpl.qualifiedName + ".aliasType(" + name + ")")
    ...
    implicit class PackageAccess(pack: Package) extends TemplateAccess(pack) {
      def _package(name: String): Package = getTheFirst(
        _packages(name),
