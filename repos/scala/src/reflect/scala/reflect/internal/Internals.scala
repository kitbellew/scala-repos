...
trait Internals extends api.Internals {
  ...
  trait SymbolTableInternal extends MacroInternalApi {
    ...
    def substituteSymbols(
        tree: Tree,
        from: List[Symbol],
        to: List[Symbol]): Tree = tree.substituteSymbols(from, to)
    ...
    def refinedType(
        parents: List[Type],
        decls: Scope,
        clazz: Symbol): RefinedType = self.RefinedType(parents, decls, clazz)
    ...
    def refinedType(
        ...
        owner: Symbol,
        decls: Scope,
        pos: Position): Type = self.refinedType(parents, owner, decls, pos)
    ...
    def subpatterns(tree: Tree): Option[List[Tree]] =
      tree.attachments.get[SubpatternsAttachment].map(
        _.patterns.map(duplicateAndKeepPositions))
