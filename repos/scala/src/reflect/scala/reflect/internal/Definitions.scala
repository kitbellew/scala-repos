...
trait Definitions extends api.StandardDefinitions {
  ...
  abstract class DefinitionsClass
    ...
    def repeatedToSingle(tp: Type): Type = elementExtract(
      RepeatedParamClass,
      tp) orElse elementExtract(JavaRepeatedParamClass, tp) orElse tp
    ...
    def ArrayModule_genericApply = ArrayModule_overloadedApply.suchThat(
    ...
    def SymbolLiteral_dummy = NoSymbol
      .newMethodSymbol(nme.apply)
    ...
    private def Context_210 = if (settings.isScala211) NoSymbol
    else
      getClassIfDefined(
    ...
    def typeOfMemberNamedGet(tp: Type) = typeArgOfBaseTypeOr(tp, OptionClass)(
    ...
    def typeOfMemberNamedApply(tp: Type) = typeArgOfBaseTypeOr(tp, SeqClass)(
      resultOfMatchingMethod(tp, nme.apply)(IntTpe))
    def typeOfMemberNamedDrop(tp: Type) = typeArgOfBaseTypeOr(tp, SeqClass)(
    ...
    final class RunDefinitions {
      ...
      def isPredefClassOf(sym: Symbol) = if (PredefModule.hasCompleteInfo)
        sym == Predef_classOf
