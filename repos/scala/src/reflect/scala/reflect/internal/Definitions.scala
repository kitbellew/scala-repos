...
trait Definitions extends api.StandardDefinitions {
  ...
  private def enterNewClass(
      owner: Symbol,
      name: TypeName,
      parents: List[Type],
      flags: Long = 0L): ClassSymbol = {
    val clazz = owner.newClassSymbol(name, NoPosition, flags)
    clazz setInfoAndEnter
  ...
  trait ValueClassDefinitions {
    ...
    private def symbolsMap[T](
        syms: List[Symbol],
        f: Name => T): Map[Symbol, T] = mapFrom(syms)(x => f(x.name))
    private def symbolsMapFilt[T](
        syms: List[Symbol],
        p: Name => Boolean,
        f: Name => T) = symbolsMap(syms filter (x => p(x.name)), f)
    ...
    lazy val allRefClasses: Set[Symbol] = {
      refClass.values.toSet ++ volatileRefClass.values.toSet ++
        Set(VolatileObjectRefClass, ObjectRefClass)
    }
    ...
    def isGetClass(sym: Symbol) =
      (sym.name ==
        nme.getClass_ // this condition is for performance only, this is called from `Typer#stabilize`.
        && getClassMethods(sym))
    ...
    lazy val ScalaNumericValueClasses = ScalaValueClasses filterNot
      Set[Symbol](UnitClass, BooleanClass)
    lazy val ScalaValueClassesNoUnit = ScalaValueClasses filterNot
      (_ eq UnitClass)
  ...
  abstract class DefinitionsClass
    ...
    def isUnimportable(sym: Symbol) =
      ((sym eq NoSymbol) || sym.isConstructor || sym.isPrivateLocal)
    ...
    def isTrivialTopType(tp: Type) =
      (tp =:= AnyTpe || tp =:= AnyValTpe || tp =:= AnyRefTpe)
    ...
    lazy val UninitializedFieldConstructor = UninitializedErrorClass
      .primaryConstructor
    ...
    lazy val UnqualifiedOwners = UnqualifiedModules.toSet ++
      UnqualifiedModules.map(_.moduleClass)
    ...
    lazy val ComparableClass = requiredClass[java.lang.Comparable[_]] modifyInfo
      fixupAsAnyTrait
    ...
    lazy val ByNameParamClass = specialPolyClass(
      tpnme.BYNAME_PARAM_CLASS_NAME,
      COVARIANT)(_ => AnyTpe)
    lazy val JavaRepeatedParamClass = specialPolyClass(
      tpnme.JAVA_REPEATED_PARAM_CLASS_NAME,
      COVARIANT)(tparam => arrayType(tparam.tpe))
    lazy val RepeatedParamClass = specialPolyClass(
      tpnme.REPEATED_PARAM_CLASS_NAME,
      COVARIANT)(tparam => seqType(tparam.tpe))
    ...
    def repeatedToSingle(tp: Type): Type =
      elementExtract(RepeatedParamClass, tp) orElse
        elementExtract(JavaRepeatedParamClass, tp) orElse tp
    ...
    lazy val StructuralCallSite_dummy = NoSymbol.newMethodSymbol(nme.apply)
    ...
    def SymbolLiteral_dummy =
      NoSymbol.newMethodSymbol(nme.apply)
    ...
    def isJavaMainMethod(sym: Symbol) = (sym.name == nme.main) &&
      (sym.info match {
    ...
    def hasJavaMainMethod(sym: Symbol): Boolean = (sym.tpe member nme.main)
      .alternatives exists isJavaMainMethod
    ...
    def functionType(formals: List[Type], restpe: Type) =
      FunctionClass.specificType(formals, restpe)
    ...
    def isFunctionSymbol(sym: Symbol) =
      FunctionClass.seq contains unspecializedSymbol(sym)
    ...
    private def macroBundleParamInfo(tp: Type) = {
      ...
      ctor.paramss match {
        case List(List(c)) =>
          ...
          val isContextCompatible = sym
            .isNonBottomSubClass(BlackboxContextClass) ||
            sym.isNonBottomSubClass(WhiteboxContextClass)
    ...
    def isBlackboxMacroBundleType(tp: Type) = {
      ...
      val unwrappedContext = MacroContextType.unapply(macroBundleParamInfo(tp))
        .getOrElse(NoType)
    ...
    def typeOfMemberNamedDrop(tp: Type) =
      typeArgOfBaseTypeOr(tp, SeqClass)(
    ...
    def hasSelectors(tp: Type) =
      ((tp.members containsName nme._1) && (tp.members containsName nme._2))
    ...
    lazy val Any_getClass = enterNewMethod(
      ...
      DEFERRED)
    lazy val Any_isInstanceOf = newT1NullaryMethod(
      AnyClass,
      nme.isInstanceOf_,
      FINAL)(_ => BooleanTpe)
    ...
    lazy val getClassMethods: Set[Symbol] = primitiveGetClassMethods +
      Object_getClass
    ...
    lazy val Object_synchronized = newPolyMethod(
      1,
      ObjectClass,
      nme.synchronized_,
      FINAL)(tps =>
    ...
    private def fatalMissingSymbol(
        owner: Symbol,
        name: Name,
        what: String = "member",
        addendum: String = "") = {
    ...
    /** Lists symbols that are synthesized or hijacked by the compiler.
      ...
      */
    lazy val symbolsNotPresentInBytecode = syntheticCoreClasses ++
      syntheticCoreMethods ++ hijackedCoreClasses

    /** Is the symbol that of a parent which is added during parsing? */
    lazy val isPossibleSyntheticParent = ProductClass.seq.toSet[Symbol] +
      ProductRootClass + SerializableClass

    private lazy val boxedValueClassesSet = boxedClass.values.toSet[Symbol] +
      BoxedUnitClass
    ...
    final class RunDefinitions {
      ...
      lazy val WeakTypeTagClass = TypeTagsClass
        .map(sym => getMemberClass(sym, tpnme.WeakTypeTag))
      lazy val WeakTypeTagModule = TypeTagsClass
        .map(sym => getMemberModule(sym, nme.WeakTypeTag))
      lazy val TypeTagClass = TypeTagsClass
        .map(sym => getMemberClass(sym, tpnme.TypeTag))
      lazy val TypeTagModule = TypeTagsClass
        .map(sym => getMemberModule(sym, nme.TypeTag))
      ...
      lazy val materializeWeakTypeTag = ReflectApiPackage
        .map(sym => getMemberMethod(sym, nme.materializeWeakTypeTag))
      lazy val materializeTypeTag = ReflectApiPackage
        .map(sym => getMemberMethod(sym, nme.materializeTypeTag))
      ...
      lazy val ApiUniverseReify = ApiUniverseClass
        .map(sym => getMemberMethod(sym, nme.reify))

      lazy val ReflectRuntimeUniverse = DefinitionsClass.this
        .ReflectRuntimeUniverse
      lazy val ReflectRuntimeCurrentMirror = DefinitionsClass.this
        .ReflectRuntimeCurrentMirror

      lazy val TreesTreeType = TreesClass
        .map(sym => getTypeMember(sym, tpnme.Tree))
      object TreeType {
      ...
      lazy val Scala_Java8_CompatPackage = rootMirror
        .getPackageIfDefined("scala.runtime.java8")
      lazy val Scala_Java8_CompatPackage_JFunction = (0 to MaxFunctionArity)
        .toArray map
        (i =>
          getMemberIfDefined(
