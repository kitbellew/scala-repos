    ...
    def elimSub0(ts: List[Type]): List[Type] =
      ts match {
        ...
        case t :: ts1 =>
          ...
          if (rest exists (t1 => isSubType(t, t1, depth.decr))) rest
          else t :: rest
      }
    val ts0 = elimSub0(ts)
          ...
          def lubsym(proto: Symbol): Symbol = {
            ...
            if (syms contains NoSymbol) NoSymbol
              ...
              if (proto.isTerm) // possible problem: owner of info is still the old one, instead of new refinement class
                proto.cloneSymbol(lubRefined.typeSymbol).setInfoOwnerAdjusted(
                  lub(symtypes, depth.decr))
                ...
                lubRefined.typeSymbol.newAbstractType(
                  proto.name.toTypeName,
                  proto.pos)
