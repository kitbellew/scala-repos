...
private[internal] trait GlbLubs {
  ...
  def lubList(ts: List[Type], depth: Depth): List[Type] = {
    ...
    def isHotForTs(xs: List[Type]) =
      ts exists (_.typeParams == xs.map(_.typeSymbol))
    ...
    def loop(pretypes: List[Type], tsBts: List[List[Type]]): List[Type] = {
      ...
      if (tsBts.isEmpty || (tsBts exists typeListIsEmpty))
        pretypes.reverse
      else if (tsBts.tail.isEmpty)
        pretypes.reverse ++ tsBts.head
      else {
  ...
  private def elimSub(ts: List[Type], depth: Depth): List[Type] = {
    ...
    val ts0 = elimSub0(ts)
    if (ts0.isEmpty || ts0.tail.isEmpty)
      ts0
    else {
  ...
  def numericLub(ts: List[Type]) =
    ts reduceLeft
      ((t1, t2) =>
        if (isNumericSubType(t1, t2))
          t2
        else if (isNumericSubType(t2, t1))
          t1
        else
          IntTpe)
            ...
            val syms = narrowts map
              (t =>
                ...
                t.nonPrivateMember(proto.name)
                  .filter(sym =>
                    sym.tpe matches
                      prototp.substThis(lubThisType.typeSymbol, t)))
  ...
  private def matchingBounds(
    ...
    def getBounds(tp: Type): List[Type] =
      tp match {
