...
private[internal] trait TypeMaps {
    ...
    private def correspondingTypeArgument(lhs: Type, rhs: Type): Type = {
      ...
      if (argIndex < 0 && rhs.parents.exists(typeIsErroneous))
        ...
        if (!rhsArgs.isDefinedAt(argIndex))
          ...
          def msg = s"Created $result, though could not find ${own_s(
  ...
  abstract class SubstMap[T](from: List[Symbol], to: List[T]) extends TypeMap {
    ...
    def apply(tp0: Type): Type = if (from.isEmpty) tp0
    else {
  ...
  class SubstSymMap(from: List[Symbol], to: List[Symbol])
    ...
    object mapTreeSymbols extends TypeMapTransformer {
      ...
      def transformIfMapped(tree: Tree)(trans: Symbol => Tree) = termMapsTo(
        tree.symbol) match {
