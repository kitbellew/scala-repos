...
private[internal] trait TypeMaps {
      ...
      tp match {
        ...
        case tp1 @ RefinedType(parents, decls) =>
          parents filter (_.typeSymbol != SingletonClass) match {
            case Nil =>
              AnyTpe
            case p :: Nil if decls.isEmpty =>
              mapOver(p)
            case ps =>
              mapOver(copyRefinedType(tp1, ps, decls))
          }
        case tp1 =>
          mapOver(tp1)
      }
    ...
    def apply(tp: Type): Type =
      tp match {
        ...
        case AnnotatedType(_, _) =>
          mapOver(tp)
        case _ =>
          tp // no recursion - top level only
      }
  }
    ...
    def apply(tp: Type): Type =
      tp match {
        case MethodType(params, restpe) =>
          ...
          if (restpe eq restpe1)
            tp
          else
            MethodType(params, restpe1)
        case TypeRef(_, RepeatedParamClass, arg :: Nil) =>
          seqType(arg)
        case _ =>
          if (etaExpandKeepsStar)
            tp
          else
            mapOver(tp)
      }
  ...
  abstract class TypeMap(trackVariance: Boolean) extends (Type => Type) {
      ...
      tp match {
        case tr @ TypeRef(pre, sym, args) =>
          ...
          if ((pre1 eq pre) && (args1 eq args))
            ...
            copyTypeRef(tp, pre1, tr.coevolveSym(pre1), args1)
        case ThisType(_) =>
          tp
        ...
        case MethodType(params, result) =>
          ...
          if ((params1 eq params) && (result1 eq result))
            tp
          else
            copyMethodType(tp, params1, result1.substSym(params, params1))
        case PolyType(tparams, result) =>
          val tparams1 = flipped(mapOver(tparams))
          val result1 = this(result)
          if ((tparams1 eq tparams) && (result1 eq result))
            tp
          else
            PolyType(tparams1, result1.substSym(tparams, tparams1))
        case NullaryMethodType(result) =>
          val result1 = this(result)
          if (result1 eq result)
            tp
          else
            NullaryMethodType(result1)
        case ConstantType(_) =>
          tp
        case SuperType(thistp, supertp) =>
          val thistp1 = this(thistp)
          val supertp1 = this(supertp)
          if ((thistp1 eq thistp) && (supertp1 eq supertp))
            tp
          else
            SuperType(thistp1, supertp1)
        ...
        case BoundedWildcardType(bounds) =>
          ...
          if (bounds1 eq bounds)
            tp
          else
            BoundedWildcardType(bounds1.asInstanceOf[TypeBounds])
        case rtp @ RefinedType(parents, decls) =>
        ...
        case ExistentialType(tparams, result) =>
          ...
          if ((tparams1 eq tparams) && (result1 eq result))
            ...
            newExistentialType(tparams1, result1.substSym(tparams, tparams1))
        case OverloadedType(pre, alts) =>
          val pre1 =
            if (pre.isInstanceOf[ClassInfoType])
              pre
            else
              this(pre)
          if (pre1 eq pre)
        ...
        case AnnotatedType(annots, atp) =>
          ...
          val atp1 = this(atp)
          if ((annots1 eq annots) && (atp1 eq atp))
            tp
          else if (annots1.isEmpty)
            atp1
          else
            AnnotatedType(annots1, atp1)
        /*
            case ErrorType => tp
      ...
      def loop(syms: List[Symbol]): Boolean =
        syms match {
          case Nil =>
            true
          case x :: xs =>
            (x.info eq applyToSymbolInfo(x)) && loop(xs)
        }
      loop(origSyms)
    }
    ...
    def mapOver(annot: AnnotationInfo): AnnotationInfo = {
      ...
      if ((args eq args1) && (atp eq atp1))
        ...
        AnnotationInfo(atp1, args1, assocs) setPos annot.pos
    }

    def mapOverAnnotations(
        annots: List[AnnotationInfo]): List[AnnotationInfo] = {
      val annots1 = annots mapConserve mapOver
      if (annots1 eq annots)
        annots
      else
        annots1 filterNot (_ eq UnmappableAnnotation)
    }
    ...
    def mapOverAnnotArgs(args: List[Tree]): List[Tree] = {
      ...
      if (args1 contains UnmappableTree)
        Nil
      else
        args1
    }
  ...
  class ExistentialExtrapolation(tparams: List[Symbol])
          ...
          case TypeRef(pre, sym, args) if tparams contains sym =>
            val repl =
              if (variance.isPositive)
                dropSingletonType(tp1.bounds.hi)
              else
                tp1.bounds.lo
    ...
    override def mapOver(tree: Tree) =
      tree match {
        case Ident(_) if tree.tpe.isStable =>
          tree
        case _ =>
          super.mapOver(tree)
      }
    ...
    private object annotationArgRewriter extends TypeMapTransformer {
      ...
      override def transform(tree: Tree): Tree =
        super.transform(tree) match {
          case This(_) if matchesThis(tree.symbol) =>
            newThis()
          case tree =>
            tree
        }
  ...
  abstract class SubstMap[T](from: List[Symbol], to: List[T]) extends TypeMap {
    ...
    protected def renameBoundSyms(tp: Type): Type =
      tp match {
        ...
        tp match {
          ...
          case ClassInfoType(parents, decls, sym) =>
            ...
            if (parents1 eq parents)
              ...
              ClassInfoType(parents1, decls, sym)
          case _ =>
            tp
        }
      }
    ...
    protected def toType(fromtp: Type, sym: Symbol) =
      fromtp match {
        case TypeRef(pre, _, args) =>
          copyTypeRef(fromtp, pre, sym, args)
  ...
  class InstantiateDependentMap(params: List[Symbol], actuals0: List[Type])
    ...
    def existentialsNeeded: List[Symbol] =
      existentials.iterator.filter(_ ne null).toList
    ...
    def apply(tp: Type): Type =
      tp match {
        case SingleType(NoPrefix, StabilizedArgTp(tp)) =>
          tp
        case _ =>
          mapOver(tp)
      }
    ...
    override def mapOver(arg: Tree, giveup: () => Nothing): Tree = {
      ...
      object treeTrans extends Transformer {
        override def transform(tree: Tree): Tree =
          tree.symbol match {
            case StableArgTp(tp) =>
              gen.mkAttributedQualifier(tp, tree.symbol)
    ...
    def traverse(tp: Type) {
      if (!result) {
        tp match {
          ...
          case _ =>
            tp.normalize match {
              case TypeRef(_, sym1, _) if (sym == sym1) =>
                result = true
              case SingleType(_, sym1) if (sym == sym1) =>
                result = true
              case _ =>
                mapOver(tp)
            }
  ...
  object adaptToNewRunMap extends TypeMap {
    ...
    private def adaptToNewRun(pre: Type, sym: Symbol): Symbol = {
      if (phase.flatClasses || sym
        ...
        sourceModule1
          .moduleClass orElse sourceModule1.initialize.moduleClass orElse {
          val msg =
