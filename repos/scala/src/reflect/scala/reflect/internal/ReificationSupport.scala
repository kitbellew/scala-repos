  ...
  class ReificationSupportImpl extends ReificationSupportApi {
    ...
    def selectTerm(owner: Symbol, name: String): TermSymbol = {
      ...
      if (result.isOverloaded)
        result.suchThat(!_.isMethod).asTerm
      else
        result
    }
    ...
    def toStats(tree: Tree): List[Tree] =
      tree match {
        case EmptyTree =>
          Nil
        case SyntacticBlock(stats) =>
          stats
        case defn if defn.isDef =>
          defn :: Nil
        case imp: Import =>
          imp :: Nil
        case _ =>
          throw new IllegalArgumentException(s"can't flatten $tree")
      }
    ...
    object ImplicitParams extends ImplicitParamsExtractor {
      def apply(
        ...
        if (implparams.nonEmpty)
          paramss :+ mkImplicitParam(implparams)
        else
          paramss
          ...
          case treeInfo.Applied(fun, targs, argss) =>
            fun match {
              ...
              case _ =>
                val callee =
                  if (fun.isTerm)
                    SyntacticTypeApplied(fun, targs)
                  else
                    SyntacticAppliedType(fun, targs)
                Some((callee, argss))
            }
        }
    }
      ...
      def unapply(tree: Tree): Option[
        ...
        tree match {
          case ModuleDef(
                mods,
                name,
                UnMkTemplate(parents, selfType, _, _, earlyDefs, body)) =>
    ...
    object SyntacticBlock extends SyntacticBlockExtractor {
      ...
      def unapply(tree: Tree): Option[List[Tree]] =
        tree match {
          ...
          case SyntheticUnit() =>
            Some(Nil)
          case _ if tree.isTerm && tree.nonEmpty =>
            Some(tree :: Nil)
          case _ =>
            None
        }
          ...
          if (name != nme.CONSTRUCTOR)
            ...
            rhs match {
              case Block(_, _) =>
                rhs
              case _ =>
                Block(List(rhs), gen.mkSyntheticUnit)
            }
        }
    ...
    def UnliftListOfListsElementwise[T](unliftable: Unliftable[T]) =
      new UnliftListOfListsElementwise[T] {
        def unapply(lst: List[List[Tree]]): Option[List[List[T]]] = {
          ...
          if (unlifted.flatten.length == lst.flatten.length)
            Some(unlifted)
          else
            None
        }
    ...
    protected object UnPatSeq {
      def unapply(trees: List[Tree]): Option[List[(Tree, Tree)]] = {
        ...
        val patvalues = imploded.flatMap {
          case SyntacticPatDef(_, pat, EmptyTree, rhs) =>
            Some((pat, rhs))
    ...
    object implodePatDefs extends Transformer {
      override def transform(tree: Tree) =
        tree match {
          case templ: Template =>
            deriveTemplate(templ)(transformStats)
          case block: Block =>
    ...
    object SyntacticPatDef extends SyntacticPatDefExtractor {
      def apply(
        ...
        tpt match {
          ...
          case _ =>
            gen.mkPatDef(mods, Typed(pat, tpt), rhs)
        }
    ...
    object SyntacticFor extends SyntacticForExtractor {
      ...
      def unapply(tree: Tree) =
        tree match {
          case UnFor(enums, gen.Yield(body)) =>
            None
          case UnFor(enums, body) =>
            Some((enums, body))
          case _ =>
            None
        }
    ...
    object SyntacticTypeIdent extends SyntacticTypeIdentExtractor {
      ...
      def unapply(tree: Tree): Option[TypeName] =
        tree match {
          case MaybeTypeTreeOriginal(Ident(name: TypeName)) =>
            Some(name)
          case _ =>
            None
        }
    ...
    object SyntacticImport extends SyntacticImportExtractor {
      ...
      private object WildcardSelector {
        ...
        def unapply(sel: ImportSelector): Option[Int] =
          sel match {
            case ImportSelector(nme.WILDCARD, offset, null, -1) =>
              Some(offset)
            case _ =>
              None
          }
