...
trait Types
  ...
  abstract class Type extends TypeApiImpl with Annotatable[Type] {
    ...
    def dealiasWiden: Type =
      (if (this ne widen)
         widen.dealiasWiden
       else if (this ne dealias)
         dealias.dealiasWiden
       else
         this)
    ...
    def dealiasWidenChain: List[Type] =
      this :: (if (this ne widen)
                 widen.dealiasWidenChain
               else if (this ne betaReduce)
                 betaReduce.dealiasWidenChain
               else
                 Nil)
    ...
    def subst(from: List[Symbol], to: List[Type]): Type =
      if (from.isEmpty)
        ...
        substTypeMapCache(from, to)(this)
    ...
    @inline
    final def orElse(alt: => Type): Type =
      if (this ne NoType)
        this
      else
        alt
    ...
    def matchesPattern(that: Type): Boolean =
      (this <:< that) || (that match {
        case ArrayTypeRef(elem2) if elem2.typeConstructor.isHigherKinded =>
          this match {
            case ArrayTypeRef(elem1) =>
              elem1 matchesPattern elem2
            case _ =>
              false
          }
    ...
    def weak_<:<(that: Type): Boolean = {
      ...
      val result =
        ((this eq that) ||
          (if (explainSwitch)
             explain("weak_<:", isWeakSubType, this, that)
           else
             isWeakSubType(this, that)))
      if (Statistics.canEnable)
        Statistics.popTimer(typeOpsStack, start)
      result
    }
    ...
    def =:=(that: Type): Boolean =
      ((this eq that) ||
        (if (explainSwitch)
           explain("=", isSameType, this, that)
         else
           isSameType(this, that)))
    ...
    def baseTypeIndex(sym: Symbol): Int = {
      ...
      while (lo <= hi) {
        ...
        if (sym == btssym)
          return mid
        else if (sym isLess btssym)
          hi = mid - 1
        else if (btssym isLess sym)
          lo = mid + 1
        else
          abort(
      ...
      if (str == "type")
        ...
        widen match {
          case RefinedType(_, _) =>
            "" + widen
          case _ =>
            if (widen.toString.trim == "")
              str
            else
              s"$str (with underlying type $widen)"
        }
      else
        str
    }
    ...
    private def findDecl(name: Name, excludedFlags: Int): Symbol = {
      ...
      if (alts.isEmpty)
        ...
        (baseClasses.head.newOverloaded(this, alts))
    }
    ...
    def findMembers(excludedFlags: Long, requiredFlags: Long): Scope = {
      ...
      if (this.isGround)
        ...
        suspendingTypeVars(typeVarsInType(this))(findMembersInternal)
    }
...
// Subclasses ------------------------------------------------------------
  ...
  abstract case class ThisType(sym: Symbol)
    ...
    override def prefixString =
      if (settings.debug)
        sym.nameString + ".this."
      else if (sym.isAnonOrRefinementClass)
        "this."
  ...
  abstract case class SingleType(pre: Type, sym: Symbol)
    ...
    override def isTrivial: Boolean = {
      if (trivial == UNKNOWN)
        trivial = fromBoolean(pre.isTrivial)
      toBoolean(trivial)
    }
  ...
  object SuperType extends SuperTypeExtractor {
    def apply(thistp: Type, supertp: Type): Type = {
      if (phase.erasedTypes)
        ...
        unique(new UniqueSuperType(thistp, supertp))
    }
  }
  ...
  abstract case class TypeBounds(lo: Type, hi: Type)
    ...
    private[internal] def starNotation(typeString: Type => String): String = {
      if (emptyLowerBound && emptyUpperBound)
        ""
      else if (emptyLowerBound)
        "(" + typeString(hi) + ")"
      else
        "(%s, %s)" format (typeString(lo), typeString(hi))
    }
  ...
  abstract class CompoundType extends Type with CompoundTypeApi {
    ...
    override def baseClasses: List[Symbol] = {
      ...
      if (baseClassesPeriod == currentPeriod && cached != null)
        cached
      else {
    ...
    private def normalizeImpl = {
      ...
      def flatten(tps: List[Type]): List[Type] = {
        def dealiasRefinement(tp: Type) =
          if (tp.dealias.isInstanceOf[RefinedType])
            tp.dealias
          else
            tp
        tps map dealiasRefinement flatMap {
          case RefinedType(parents, ds) if ds.isEmpty =>
            flatten(parents)
          case tp =>
            List(tp)
        }
  ...
  class ModuleTypeRef(pre0: Type, sym0: Symbol)
    ...
    override def prefixString =
      if (sym.isOmittablePrefix)
        ...
        prefix.prefixString + sym.nameString + "."
  }
  ...
  class NoArgsTypeRef(pre0: Type, sym0: Symbol)
    ...
    override def narrow =
      if (sym.isModuleClass)
        singleType(pre, sym.sourceModule)
      else
        super.narrow
  ...
  trait NonClassTypeRef extends TypeRef {
    ...
    final override protected def relativeInfo = {
      ...
      if ((relativeInfoCache eq null) || (
            relativeInfoCacheValidForSymInfo ne symInfo
          ) || (relativeInfoCacheValidForPeriod != currentPeriod)) {
        relativeInfoCache = super.relativeInfo

        if (this.isInstanceOf[AbstractTypeRef])
    ...
    private def validateRelativeInfo(): Unit =
      relativeInfoCache match {
  ...
  trait AliasTypeRef extends NonClassTypeRef {
    ...
    override def prefix =
      if (this ne normalize)
        normalize.prefix
      else
        pre
    override def termSymbol =
      if (this ne normalize)
        normalize.termSymbol
      else
        super.termSymbol
    override def typeSymbol =
      if (this ne normalize)
        normalize.typeSymbol
      else
        sym
    ...
    override protected def normalizeImpl =
      if (typeParamsMatchArgs)
        ...
        if (overriddenSym != NoSymbol)
          pre.memberType(overriddenSym).normalize
        else
          ErrorType
      }
  ...
  private def embeddedSymbol(tp: Type, name: Name): Symbol =
    ...
    if (tp.typeSymbol.isRefinementClass)
      tp.normalize.decls lookup name
    else {
      debuglog(
  ...
  abstract case class TypeRef(pre: Type, sym: Symbol, args: List[Type])
    ...
    final def relativize(tp: Type): Type =
      if (tp.isTrivial)
        ...
        tp match {
          ...
          case _ =>
            seenFromOwnerInstantiated(tp)
        }
      }

    private def argsOrDummies =
      if (args.isEmpty)
        dummyArgs
      else
        args
    ...
    final override def baseType(clazz: Symbol): Type =
      ...
      // NOTE: this first goes to requested base type, *then* does asSeenFrom prefix & instantiates args
        ...
        relativize(sym.info.baseType(clazz))
      else
        baseTypeOfNonClassTypeRef(clazz)
    ...
    override def parents: List[Type] = {
      ...
      if (parentsPeriod == currentPeriod && cache != null)
        cache
      else {
    ...
    private def tupleTypeString: String =
      args match {
        case Nil =>
          noArgsString
        case arg :: Nil =>
    ...
    def apply(pre: Type, sym: Symbol, args: List[Type]): Type =
      unique({
        if (args ne Nil) {
          ...
          if (sym.isAliasType)
            new AliasNoArgsTypeRef(pre, sym)
          else if (sym.isAbstractType)
            new AbstractNoArgsTypeRef(pre, sym)
          else if (sym.isRefinementClass)
            new RefinementTypeRef(pre, sym)
          else if (sym.isPackageClass)
            new PackageTypeRef(pre, sym)
          else if (sym.isModuleClass)
            new ModuleTypeRef(pre, sym)
          else
            new ClassNoArgsTypeRef(pre, sym)
        }
      })
  }
  ...
  case class MethodType(
    ...
    override def isTrivial: Boolean = {
      if (trivial == UNKNOWN)
        trivial = fromBoolean(isTrivialResult && areTrivialParams(params))
      toBoolean(trivial)
    }
    ...
    private def areTrivialParams(ps: List[Symbol]): Boolean =
      ps match {
        case p :: rest =>
          p.tpe.isTrivial && !typesContain(paramTypes, p) && !(
    ...
    override def resultType(actuals: List[Type]) =
      if (isTrivial || phase.erasedTypes)
        resultType
        ...
        existentialAbstraction(params, resultType)
  ...
  case class ExistentialType(
    ...
    override def params =
      underlying.params mapConserve { param =>
        val tpe1 = rewrap(param.tpeHK)
        if (tpe1 eq param.tpeHK)
          param
        else
          param.cloneSymbol.setInfo(tpe1)
      }
...
//      maybeRewrap(underlying.instantiateTypeParams(formals, actuals))
      ...
      if ((quantified1 eq quantified) && (underlying1 eq underlying))
        this
      else
        existentialAbstraction(
          quantified1,
    ...
    private def isStraightApplication =
      (
        quantified corresponds underlying.typeArgs
      ) { (q, a) =>
        q.tpe =:= a
      }
    ...
    override def skolemizeExistential(owner0: Symbol, origin: AnyRef) = {
      ...
      def newSharpenedSkolem(quant: Symbol, tparam: Symbol): Symbol = {
        ...
        val canSharpen = (emptyBounds(quant) && !emptyBounds(tparam)
          && (
            existentialsInType(tparam.info) intersect quantified
          ).isEmpty)
        ...
        val skolemInfo =
          if (!canSharpen)
            ...
            tparam.info.substSym(tpars, quantified)
      ...
      val canSharpenBounds = (underlying
        .typeSymbol
      ...
      if (canSharpenBounds)
        ...
        deriveType(quantified, newSkolem)(underlying)
    }

    private def wildcardArgsString(
        qset: Set[Symbol],
        args: List[Type]): List[String] =
      args map {
        case TypeRef(_, sym, _)
            if (
              qset contains sym
    ...
    override def safeToString: String = {
      def clauses = {
        val str = quantified map (_
          .existentialToString) mkString (" forSome { ", "; ", " }")
    ...
    override def atOwner(owner: Symbol) =
      if (!allSymbolsHaveOwner(quantified, owner))
        cloneInfo(owner)
      else
        this
  ...
  def overloadedType(pre: Type, alternatives: List[Symbol]): Type =
    alternatives match {
      ...
      case alt :: Nil =>
        pre memberType alt
      case _ =>
        OverloadedType(pre, alternatives)
    }
  ...
  object TypeVar {
    ...
    private def deriveConstraint(tparam: Symbol): TypeConstraint = {
      ...
      if (propagateParameterBoundsToTypeVars) {
        ...
        if (exclude)
          ...
          TypeVar.trace("constraint", "For " + tparam.fullLocationString)(
    ...
    /** This is the only place TypeVars should be instantiated.
      */
    private def createTypeVar(
        origin: Type,
        constr: TypeConstraint,
        args: List[Type],
        params: List[Symbol],
        untouchable: Boolean): TypeVar = {
      val tv = (if (args.isEmpty && params.isEmpty) {
                  if (untouchable)
  ...
  /** A class representing a type variable: not used after phase `typer`.
    ...
    */
  abstract case class TypeVar(origin: Type, var constr: TypeConstraint)
      extends Type {
    ...
    private def shouldRepackType =
      enableTypeVarExperimentals && encounteredHigherLevel
    ...
    private[Types] def suspended: Boolean =
      (_suspended: @unchecked) match {
        case ConstantFalse =>
          false
        case ConstantTrue =>
          true
    ...
    def registerBound(
      ...
      def addBound(tp: Type) {
        if (isLowerBound)
          addLoBound(tp, isNumericBound)
        else
          addHiBound(tp, isNumericBound)
      }
...
//      println("regTypeEq: "+(safeToString, debugString(tp), tp.getClass, if (typeVarLHS) "in LHS" else "in RHS", if (suspended) "ZZ" else if (instValid) "IV" else "")) //@MDEBUG
    ...
    private def tparamsOfSym(sym: Symbol) =
      sym.info match {
    ...
    def originLocation = {
      ...
      List(
        ...
        if (encl.isMethod)
          ...
          None,
        if (sym.owner.isTerm && (sym.owner != encl))
          Some(sym.owner)
        else
    ...
    private def levelString =
      if (settings.explaintypes)
        ...
        ""
    override def safeToString =
      (if ((constr eq null) || (inst eq null))
         "TVar<" + originName + "=null>"
       else if (inst ne NoType)
         "=?" + inst
       else
         (if (untouchable)
            "!?"
          else
            "?") + levelString + originName)
  ...
  case class AnnotatedType(
    ...
    override def safeToString =
      annotations.mkString(underlying + " @", " @", "")
    ...
    /** Drop the annotations on the bounds, unless the low and high
      ...
      */
    override def bounds: TypeBounds =
      underlying.bounds match {
        case TypeBounds(_: this.type, _: this.type) =>
          TypeBounds(this, this)
        case oftp =>
          oftp
      }
    ...
    override def instantiateTypeParams(
      ...
      val underlying1 = underlying.instantiateTypeParams(formals, actuals)
      if ((annotations1 eq annotations) && (underlying1 eq underlying))
        this
      else
        AnnotatedType(annotations1, underlying1)
    }
  ...
  def annotatedType(annots: List[AnnotationInfo], underlying: Type): Type =
    if (annots.isEmpty)
      ...
      AnnotatedType(annots, underlying)
    ...
    def unapply(tp: Type): Option[(List[AnnotationInfo], Type)] =
      tp.staticAnnotations match {
        case Nil =>
          None
        case annots =>
          Some((annots, tp.withoutAnnotations))
      }
  }
  ...
  abstract class LazyPolyType(override val typeParams: List[Symbol])
    ...
    override def safeToString =
      (if (typeParams.isEmpty)
         ...
         typeParamsString(this)) + super.safeToString
  }
...
// Creators ---------------------------------------------------------------
  ...
  private def removeSuper(tp: Type, sym: Symbol): Type =
    tp match {
      case SuperType(thistp, _) =>
        if (sym.isEffectivelyFinal || sym.isDeferred)
          thistp
        else
          tp
      case _ =>
  ...
  def copyRefinedType(
      original: RefinedType,
      parents: List[Type],
      decls: Scope) =
    if ((parents eq original.parents) && (decls eq original.decls))
      original
    else {
      ...
      for (sym <- syms2)
        sym modifyInfo (
          _ substThisAndSym (original.typeSymbol, resultThis, syms1, syms2)
        )
  ...
  def intersectionType(tps: List[Type]): Type =
    tps match {
      case tp :: Nil =>
        tp
      case _ =>
        refinedType(tps, commonOwner(tps))
    }
  ...
  object GenPolyType {
    def apply(tparams: List[Symbol], tpe: Type): Type = {
      ...
      if (tparams.nonEmpty)
        ...
        tpe // it's okay to be forgiving here
    }
...
// Helper Classes ---------------------------------------------------------
  ...
  class TypeUnwrapper(
    ...
    def apply(tp: Type): Type =
      tp match {
        case AnnotatedType(_, underlying) if annotated =>
          apply(underlying)
        case ExistentialType(_, underlying) if existential =>
          apply(underlying)
        case PolyType(_, underlying) if poly =>
          apply(underlying)
        case NullaryMethodType(underlying) if nullary =>
          apply(underlying)
        case tp =>
          tp
      }
  ...
  def elementExtract(container: Symbol, tp: Type): Type = {
    ...
    unwrapWrapperTypes(tp baseType container).dealiasWiden match {
      case TypeRef(_, `container`, arg :: Nil) =>
        arg
      case _ =>
        NoType
    }
  ...
  def repackExistential(tp: Type): Type =
    (if (tp == NoType)
       ...
       existentialAbstraction(existentialsInType(tp), tp))
  ...
  def isDummyAppliedType(tp: Type) =
    tp.dealias match {
      case tr @ TypeRef(_, _, args) =>
        args exists isDummyOf(tr)
      case _ =>
        false
    }
  ...
  def nestedMemberType(sym: Symbol, pre: Type, owner: Symbol): Type = {
    def loop(tp: Type): Type =
      if (tp.isTrivial)
        ...
        loop(tp.prefix) memberType tp.typeSymbol
...
// Helper Methods  -------------------------------------------------------------
  ...
  def normalizePlus(tp: Type): Type = {
    if (isRawType(tp))
      ...
      tp.normalize match {
        ...
        case st @ SingleType(_, sym) if sym.isModule =>
          st.underlying.normalize
  ...
  final def compareLengths(xs1: List[_], xs2: List[_]): Int =
    if (xs1.isEmpty) {
      ...
      compareLengths(xs1.tail, xs2.tail)
  ...
  def isEligibleForPrefixUnification(tp: Type): Boolean =
    tp match {
      case SingleType(pre, sym) =>
        !(
          sym hasFlag PACKAGE
        ) && isEligibleForPrefixUnification(pre)
  ...
  /** This appears to be equivalent to tp.isInstanceof[SingletonType],
    ...
    */
  def isSingleType(tp: Type) =
    tp match {
      case ThisType(_) | SuperType(_, _) | SingleType(_, _) =>
        true
      case _ =>
        false
    }

  def isConstantType(tp: Type) =
    tp match {
      case ConstantType(_) =>
        true
      case _ =>
  ...
  def isExistentialType(tp: Type): Boolean =
    tp match {
      case _: ExistentialType =>
        true
      case tp: Type if tp.dealias ne tp =>
        isExistentialType(tp.dealias)
      case _ =>
        false
    }

  def isImplicitMethodType(tp: Type) =
    tp match {
      case mt: MethodType =>
        mt.isImplicit
      case _ =>
  ...
  private def isHKTypeRef(tp: Type) =
    tp match {
      case TypeRef(_, sym, Nil) =>
        tp.isHigherKinded
      case _ =>
  ...
  final def isUseableAsTypeArgs(tps: List[Type]): Boolean =
    tps match {
      case Nil =>
        true
      case x :: xs =>
        isUseableAsTypeArg(x) && isUseableAsTypeArgs(xs)
    }
  ...
  ****/
  ...
  private def isAlwaysValueType(tp: Type) =
    tp match {
      case RefinedType(_, _) =>
        true
  ...
  private def isAlwaysNonValueType(tp: Type) =
    tp match {
  ...
  /** Should be called only with types for which a clear true/false answer
    ...
    */
  private def isValueElseNonValue(tp: Type): Boolean =
    tp match {
      case tp if isAlwaysValueType(tp) =>
        true
      case tp if isAlwaysNonValueType(tp) =>
        false
      case AnnotatedType(_, underlying) =>
        isValueElseNonValue(underlying)
      case SingleType(_, sym) =>
        sym.isValue // excludes packages and statics
      case TypeRef(_, _, _) if tp.isHigherKinded =>
        false // excludes type constructors
      case ThisType(sym) =>
        !sym.isPackageClass // excludes packages
      case TypeRef(_, sym, _) =>
        !sym.isPackageClass // excludes packages
      case PolyType(_, _) =>
        true // poly-methods excluded earlier
      case tp =>
        sys.error("isValueElseNonValue called with third-way type " + tp)
    }
  ...
  /** Does member `symLo` of `tpLo` have a stronger type
    ...
    */
  protected[internal] def specializesSym(
      preLo: Type,
      symLo: Symbol,
      preHi: Type,
      symHi: Symbol,
    ...
    (symHi.isAliasType || symHi.isTerm || symHi.isAbstractType) && {
      ...
      if (symHi.isTerm)
        (
          ...
          (!symHi.isStable || symLo
            .isStable) && // sub-member must remain stable
  ...
  final def matchesType(
    ...
    def matchesQuantified(
        tparams1: List[Symbol],
      ...
      case sym1 :: rest1 =>
        syms2 match {
          ...
          case sym2 :: rest2 =>
            ...
            (tp1 =:= tp2 ||
            syms1isJava && tp2
              .typeSymbol == ObjectClass && tp1.typeSymbol == AnyClass ||
  ...
  def isWithinBounds(
    ...
    (
      bounds corresponds targs
    )(boundsContainType)
  }

  def instantiatedBounds(
      pre: Type,
      owner: Symbol,
      tparams: List[Symbol],
      ...
      case ExistentialType(tparams, quantified) :: rest =>
        mergePrefixAndArgs(quantified :: rest, variance, depth) match {
          case NoType =>
            NoType
          case tpe =>
            existentialAbstraction(tparams, tpe)
        }
      case _ =>
...
// Errors and Diagnostics -----------------------------------------------------
  ...
  def explainTypes(found: Type, required: Type) {
    if (settings.explaintypes)
      withTypesExplained(found <:< required)
  }
  ...
  def explainTypes(op: (Type, Type) => Any, found: Type, required: Type) {
    if (settings.explaintypes)
      withTypesExplained(op(found, required))
  }
  ...
  def isUnboundedGeneric(tp: Type) =
    tp match {
  ...
  def addSerializable(ps: Type*): List[Type] =
    (if (ps exists typeIsSubTypeOfSerializable)
       ps.toList
     else
       (
         ps :+ SerializableTpe
  ...
  final def uncheckedBounds(tp: Type): Type = {
    if (tp.typeArgs.isEmpty || UncheckedBoundsClass == NoSymbol)
      ...
      tp.withAnnotation(AnnotationInfo marker UncheckedBoundsClass.tpe)
  }
  ...
  def objToAny(tp: Type): Type =
    if (!phase.erasedTypes && tp.typeSymbol == ObjectClass)
      AnyTpe
    else
      tp
...
// ----- Hoisted closures and convenience methods, for compile time reductions -------
  ...
  def typeDepth(tp: Type): Depth =
    tp match {
      case TypeRef(pre, sym, args) =>
        typeDepth(pre) max typeDepth(args).incr
      case RefinedType(parents, decls) =>
        typeDepth(parents) max symTypeDepth(decls.toList).incr
      case TypeBounds(lo, hi) =>
  ...
  private[scala] def maxDepth(tps: List[Type]): Depth = {
    ...
    def loop(tps: List[Type], acc: Depth): Depth =
      tps match {
        case tp :: rest =>
          loop(rest, acc max typeDepth(tp))
        case _ =>
          acc
      }
    loop(tps, Depth.Zero)
  ...
  private def typesContain(tps: List[Type], sym: Symbol): Boolean =
    tps match {
      case tp :: rest =>
        (tp contains sym) || typesContain(rest, sym)
      case _ =>
        false
    }
  ...
  private def areTrivialTypes(tps: List[Type]): Boolean =
    tps match {
      case tp :: rest =>
        tp.isTrivial && areTrivialTypes(rest)
      case _ =>
        true
    }
...
// -------------- Statistics --------------------------------------------------------
  ...
  Statistics.newView("#unique types") {
    if (uniques == null)
      0
    else
      uniques.size
