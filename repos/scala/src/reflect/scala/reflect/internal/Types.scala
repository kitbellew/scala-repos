...
trait Types
  ...
  trait RewrappingTypeProxy extends SimpleTypeProxy {
    ...
    override def resultType(actuals: List[Type]) = maybeRewrap(
      underlying.resultType(actuals))
    ...
    override def cloneInfo(owner: Symbol) = maybeRewrap(
      underlying.cloneInfo(owner))
    ...
    override def withAnnotations(annots: List[AnnotationInfo]) = maybeRewrap(
      underlying.withAnnotations(annots))
...
// Subclasses ------------------------------------------------------------
  ...
  class ModuleTypeRef(pre0: Type, sym0: Symbol)
    ...
    override def prefixString = if (sym.isOmittablePrefix) ""
  ...
  abstract case class TypeRef(pre: Type, sym: Symbol, args: List[Type])
    ...
    private def refinementDecls = fullyInitializeScope(decls) filter (sym =>
...
//      maybeRewrap(underlying.instantiateTypeParams(formals, actuals))
    ...
    override def baseType(clazz: Symbol) = maybeRewrap(
      underlying.baseType(clazz))
    ...
    private def quantifierOwner = quantified collectFirst {
  ...
  abstract case class TypeVar(
    ...
    private[Types] def suspended_=(b: Boolean): Unit = _suspended =
      if (b) ConstantTrue else ConstantFalse
    // SI-7785 Link the suspended attribute of a TypeVar created in, say, a TypeMap (e.g. AsSeenFrom) to its originator
    private[Types] def linkSuspended(origin: TypeVar): Unit = _suspended =
      origin
...
// Helper Methods  -------------------------------------------------------------
  ...
  private def symTypeDepth(syms: List[Symbol]): Depth = typeDepth(
    syms map (_.info))
  private def typeDepth(tps: List[Type]): Depth = maxDepth(tps)
  private def baseTypeSeqDepth(tps: List[Type]): Depth = maxbaseTypeSeqDepth(
    tps)
