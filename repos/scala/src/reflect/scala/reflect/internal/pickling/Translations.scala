...
trait Translations {
  ...
  def picklerTag(ref: AnyRef): Int =
    ref match {
      ...
      case const: Constant =>
        LITERAL + const.tag
      case _: Tree =>
        TREE // its sub tag more precisely identifies it
      case _: TermName =>
        TERMname
      case _: TypeName =>
        TYPEname
      case _: ArrayAnnotArg =>
        ANNOTARGARRAY // an array of annotation arguments
      ...
      case _: Modifiers =>
        MODIFIERS
      case _ =>
        sys.error(s"unpicklable entry ${shortClassOfInstance(ref)} $ref")
    }
  ...
  def picklerTag(sym: Symbol): Int =
    sym match {
      ...
      case _: ClassSymbol =>
        CLASSsym
      case _: TypeSymbol if sym.isAbstractType =>
        TYPEsym
      case _: TypeSymbol =>
        ALIASsym
      case _: TermSymbol if sym.isModule =>
        MODULEsym
      case _: TermSymbol =>
        VALsym
    }
  ...
  def picklerTag(tpe: Type): Int =
    tpe match {
      ...
      case _: ExistentialType =>
        EXISTENTIALtpe
      case StaticallyAnnotatedType(_, _) =>
        ANNOTATEDtpe
      case _: AnnotatedType =>
        picklerTag(tpe.underlying)
    }
