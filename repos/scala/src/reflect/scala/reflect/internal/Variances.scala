...
trait Variances {
  ...
  class VarianceValidator extends Traverser {
    ...
    private object ValidateVarianceMap extends TypeMap(trackVariance = true) {
      ...
      def isUncheckedVariance(tp: Type) =
        tp match {
      ...
      override def mapOver(decls: Scope): Scope = {
        decls foreach (sym =>
          withVariance(if (sym.isAliasType) Invariant else variance)(this(
            sym.info)))
        decls
      }
    ...
    override def traverse(tree: Tree) {
      ...
      tree match {
        ...
        case ValDef(_, _, _, _) => validateVariance(sym)
        case DefDef(_, _, tparams, vparamss, _, _) =>
          validateVariance(sym)
          traverseTrees(tparams)
          traverseTreess(vparamss)
        case Template(_, _, _)       => super.traverse(tree)
        case CompoundTypeTree(templ) => super.traverse(tree)
        ...
        case tt: TypTree => super.traverse(tt)
