...
class ModifierFlags {
  ...
  final val FINAL = 1 <<
    5 // May not be overridden. Note that java final implies much more than scala final.
  ...
  final val ABSTRACT = 1 <<
    3 // abstract class, or used in conjunction with abstract override.
  // Note difference to DEFERRED!
  final val DEFERRED = 1 << 4 // was `abstract' for members | trait is virtual
  final val INTERFACE = 1 <<
    7 // symbol is an interface (i.e. a trait which defines only abstract methods)
  ...
  final val LOCAL = 1 <<
    19 // symbol is local to current class (i.e. private[this] or protected[this]
  // pre: PRIVATE or PROTECTED are also set
  final val JAVA = 1 << 20 // symbol was defined by a Java class
  final val STATIC = 1 << 23 // static field, method or class
  final val CASEACCESSOR = 1 <<
    24 // symbol is a case parameter (or its accessor, or a GADT skolem)
  final val TRAIT = 1 << 25 // symbol is a trait
  final val DEFAULTPARAM = 1 << 25 // the parameter has a default value
  final val PARAMACCESSOR = 1 <<
    29 // for field definitions generated for primary constructor
  ...
  final val LAZY = 1L <<
    31 // symbol is a lazy val. can't have MUTABLE unless transformed by typer
  final val PRESUPER = 1L << 37 // value is evaluated before super call
  final val DEFAULTINIT = 1L <<
    41 // symbol is initialized to the default value: used by -Xcheckinit
  final val ARTIFACT = 1L <<
    46 // symbol should be ignored when typechecking; will be marked ACC_SYNTHETIC in bytecode
...
class Flags extends ModifierFlags {
  ...
  final val CAPTURED = 1 <<
    16 // variable is accessed from nested function.  Set by LambdaLift.
  final val LABEL = 1 << 17 // method symbol is a label. Set by TailCall
  final val INCONSTRUCTOR = 1 <<
    17 // class symbol is defined in this/superclass constructor.
  final val SYNTHETIC = 1 <<
    21 // symbol is compiler-generated (compare with ARTIFACT)
  ...
  final val ACCESSOR = 1 <<
    27 // a value or variable accessor (getter or setter)

  final val SUPERACCESSOR = 1 << 28 // a super accessor
  final val MODULEVAR = 1 <<
    30 // for variables: is the variable caching a module value
  ...
  final val TRANS_FLAG = 1L <<
    38 // transient flag guaranteed to be reset after each phase.
  ...
  final val SYNCHRONIZED = 1L <<
    45 // symbol is a method which should be marked ACC_SYNCHRONIZED
  ...
  // 25:  DEFAULTPARAM/M       TRAIT/M
  // 35:     EXISTENTIAL       MIXEDIN
  val OverloadedFlagsMask = 0L | BYNAMEPARAM | CONTRAVARIANT | DEFAULTPARAM |
    EXISTENTIAL
  ...
  final val ExplicitFlags =
    PRIVATE | PROTECTED | ABSTRACT | FINAL | SEALED | OVERRIDE | CASE |
      IMPLICIT | ABSOVERRIDE | LAZY | JAVA_DEFAULTMETHOD
  ...
  final val PrintableFlags =
    ExplicitFlags | BridgeFlags | LOCAL | SYNTHETIC | STABLE | CASEACCESSOR |
      MACRO | ACCESSOR | SUPERACCESSOR | PARAMACCESSOR | STATIC | SPECIALIZED |
  ...
  final val ModuleToClassFlags = AccessFlags | TopLevelCreationFlags | CASE |
    SYNTHETIC

  /** These flags are not pickled */
  final val FlagsNotPickled = IS_ERROR | OVERLOADED | LIFTED | TRANS_FLAG |
    LOCKED | TRIEDCOOKING
  ...
  /** If we have a top-level class or module
    ...
    */
  final val TopLevelPickledFlags = PickledFlags &
    ~(MODULE | METHOD | PACKAGE | PARAM | EXISTENTIAL)
  ...
  @annotation.switch
  override def flagToString(flag: Long): String =
    flag match {
      case PROTECTED =>
        "protected" // (1L << 0)
      case OVERRIDE =>
      ...
      case MACRO =>
        "<macro>" // (1L << 15)
      case BYNAMEPARAM =>
        "<bynameparam/captured/covariant>" // (1L << 16)
      case CONTRAVARIANT =>
        "<contravariant/inconstructor/label>" // (1L << 17)
      case ABSOVERRIDE =>
        "absoverride" // (1L << 18)
      case LOCAL =>
        "<local>" // (1L << 19)
      ...
      case ARTIFACT =>
        "<artifact>" // (1L << 46)
      case JAVA_DEFAULTMETHOD =>
        "<defaultmethod>" // (1L << 47)
      case JAVA_ENUM =>
        "<enum>" // (1L << 48)
      case JAVA_ANNOTATION =>
        "<annotation>" // (1L << 49)
      case 0x4000000000000L =>
        "" // (1L << 50)
      case `lateDEFERRED` =>
        "<latedeferred>" // (1L << 51)
      case `lateFINAL` =>
      ...
      case `notPRIVATE` =>
        "<notprivate>" // (1L << 58)
      case 0x800000000000000L =>
        "" // (1L << 59)
      case 0x1000000000000000L =>
        "" // (1L << 60)
      case 0x2000000000000000L =>
        "" // (1L << 61)
