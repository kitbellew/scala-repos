...
abstract class TreeGen {
  ...
  def mkAttributedRef(sym: Symbol): RefTree =
    if (sym.owner.isStaticOwner) {
      ...
      mkAttributedRef(sym.owner.thisType, sym)
    else
      mkAttributedIdent(sym)
  ...
  def mkAttributedSelect(qual: Tree, sym: Symbol): RefTree = {
    ...
    if (qual.symbol != null && (
      ...
      if (pkgQualifier.tpe == null)
        ...
        tree setType (qual.tpe memberType sym)
    }
  }
  ...
  def mkNamedArg(lhs: Tree, rhs: Tree): Tree =
    atPos(rhs.pos)(AssignOrNamedArg(lhs, rhs))
  ...
  def mkTemplate(
    ...
    val constr = {
      if (constrMods.isTrait) {
        if (body forall treeInfo.isInterfaceMember)
          None
        else
          Some(
  ...
  def mkParents(
    ...
    if (ownerMods.isCase)
      ...
      atPos(parentPos)(scalaAnyRefConstr) :: Nil
    else
      parents
  ...
  def mkBlock(stats: List[Tree], doFlatten: Boolean = true): Tree =
    if (stats.isEmpty)
      mkSyntheticUnit()
    else if (!stats.last.isTerm)
      Block(stats, mkSyntheticUnit())
    else if (stats.length == 1 && doFlatten)
      stats.head
    else
      ...
      case (t @ ValFrom(pat, rhs)) :: rest =>
        ...
        val pos1 =
          if (t.pos == NoPosition)
            ...
            rangePos(t.pos.source, t.pos.start, t.pos.point, rhs1.pos.end)
        val vfrom1 = ValFrom(
  ...
  def mkPatDef(pat: Tree, rhs: Tree)(implicit
      fresh: FreshNameCreator): List[ValDef] = mkPatDef(Modifiers(0), pat, rhs)
  ...
  def mkCheckIfRefutable(pat: Tree, rhs: Tree)(implicit
    ...
    if (treeInfo.isVarPatternDeep(pat))
      rhs
    else {
      val cases = List(
  ...
  class GetVarTraverser extends Traverser {
    ...
    def namePos(tree: Tree, name: Name): Position =
      if (!tree.pos.isRange || name.containsName(nme.raw.DOLLAR))
        tree.pos.focus
      else {
