...
abstract class TreeGen {
  ...
  def mkTemplate(
    ...
    var vparamss1 = mmap(vparamss) { vd =>
      val param = atPos(vd.pos.makeTransparent) {
        val mods = Modifiers(
          vd.mods.flags & (IMPLICIT | DEFAULTPARAM | BYNAMEPARAM) | PARAM |
            PARAMACCESSOR)
        ValDef(
  ...
  def mkFor(enums: List[Tree], sugarBody: Tree)(implicit
    ...
    def makeValue(pat: Tree): Tree =
      pat match { case Bind(name, _) => Ident(name) setPos pat.pos.focus }
    ...
    enums match {
      ...
      case (t @ ValFrom(pat, rhs)) :: rest =>
        ...
        val rhs1 = mkFor(
          ...
          Yield(
            Block(pdefs, atPos(wrappingPos(ids)) { mkTuple(ids) }) setPos
              wrappingPos(pdefs)))
  ...
  class GetVarTraverser extends Traverser {
    ...
    override def traverse(tree: Tree): Unit = {
      ...
      def add(name: Name, t: Tree) =
        if (!seenName(name)) buf += ((name, t, namePos(tree, name)))
      val bl = buf.length
