...
trait TypeDebugging {
  ...
  object noPrint extends (Tree => Boolean) {
    def skipScalaName(name: Name) =
      name match {
        case tpnme.Any | tpnme.Nothing | tpnme.AnyRef =>
          true
        case _ =>
          false
      }
    ...
    def skip(t: Tree): Boolean =
      t match {
        ...
        case Apply(fn, Nil) =>
          skip(fn)
        case Block(stmt :: Nil, expr) =>
          skip(stmt) && skip(expr)
        case DefDef(_, nme.CONSTRUCTOR, Nil, ListOfNil, _, rhs) =>
          skip(rhs)
        ...
        case TypeBoundsTree(lo, hi) =>
          ...
          val hi_s =
            if (noPrint(hi))
              ...
              " <: " + ptTree(hi)
          lo_s + hi_s
        case _ if (t.symbol eq null) || (t.symbol eq NoSymbol) =>
          to_s(t)
        case _ =>
          "" + t.symbol.tpe
      }
