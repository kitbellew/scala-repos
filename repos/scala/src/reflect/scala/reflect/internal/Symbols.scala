...
trait Symbols extends api.Symbols { self: SymbolTable =>
  ...
  def recursionTable_=(value: immutable.Map[Symbol, Int]) = _recursionTable =
    value

  private var existentialIds = 0
  protected def nextExistentialId() = { existentialIds += 1; existentialIds }
  protected def freshExistentialName(suffix: String) = newTypeName(
    "_" + nextExistentialId() + suffix)
  ...
  abstract class SymbolContextApiImpl extends SymbolApi {
    ...
    def asFreeTerm: FreeTermSymbol = throw new ScalaReflectionException(
      s"$this is not a free term")
    def isFreeType: Boolean = false
    def asFreeType: FreeTypeSymbol = throw new ScalaReflectionException(
      s"$this is not a free type")
  ...
  abstract class Symbol protected[Symbols] (
    ...
    private def isSynchronized = this.isInstanceOf[
...
// ----- tests ----------------------------------------------------------------------
    ...
    def isStrictFP = hasAnnotation(
      ScalaStrictFPAttr) || (enclClass hasAnnotation ScalaStrictFPAttr)
    def isSerializable = info.baseClasses.exists(p =>
    ...
    def deprecatedParamName = getAnnotation(
      DeprecatedNameAttr) flatMap (_ symbolArg 0 orElse Some(nme.NO_NAME))
    def hasDeprecatedInheritanceAnnotation = hasAnnotation(
      DeprecatedInheritanceAttr)
    def deprecatedInheritanceMessage =
      getAnnotation(DeprecatedInheritanceAttr) flatMap (_ stringArg 0)
    def hasDeprecatedOverridingAnnotation = hasAnnotation(
      DeprecatedOverridingAttr)
    ...
    /** Does this symbol denote a stable value, ignoring volatility?
      ...
      */
    final def isStable = isTerm && !isMutable && !(hasFlag(
...
// ------ toString -------------------------------------------------------------------
  ...
  class TermSymbol protected[Symbols] (
    ...
    override def isSetter = isAccessor && nme.isSetterName(
      name
  ...
  class MethodSymbol protected[Symbols] (
    ...
    override def exceptions = for (ThrownException(tp) <- annotations)
      yield tp.typeSymbol
  }
  ...
  abstract class TypeSymbol protected[Symbols] (
    ...
    def existentialBound: Type = abort(
      "unexpected type: " + this.getClass + " " + debugLocationString)
  ...
  class NoSymbol protected[Symbols] ()
    ...
    def cloneSymbolImpl(owner: Symbol, newFlags: Long) = abort(
      "NoSymbol.clone()")
  }
