...
trait Symbols extends api.Symbols {
  ...
  protected def saveOriginalOwner(sym: Symbol): Unit = {
    ...
    if (sym.owner != NoSymbol) {
      if (originalOwnerMap contains sym)
        ...
        defineOriginalOwner(sym, sym.rawowner)
    }
  }
  ...
  abstract class SymbolContextApiImpl extends SymbolApi {
    ...
    def isImplementationArtifact: Boolean =
      (this hasFlag BRIDGE) || (this hasFlag VBRIDGE) || (this hasFlag ARTIFACT)
    def isJava: Boolean = isJavaDefined
    def isVal: Boolean = isTerm && !isModule && !isMethod && !isMutable
    def isVar: Boolean =
      isTerm && !isModule && !isMethod && !isLazy && isMutable
    ...
    def companion: Symbol = {
      if (isModule && !hasPackageFlag)
        companionSymbol
      else if (isModuleClass && !isPackageClass)
        sourceModule.companionSymbol
      else if (isClass && !isModuleClass && !isPackageClass)
        companionSymbol
      else
        NoSymbol
    }
  ...
  abstract class Symbol protected[Symbols] (
    ...
    assert(
      ...
      s"unsafe symbol $initName (child of $initOwner) in runtime reflection universe")
    ...
    private[this] var _rawowner =
      if (initOwner eq null)
        this
      else
        initOwner
    ...
    override def flagMask =
      if (settings.debug && !isAbstractType)
        AllFlags
      else if (owner.isRefinementClass)
        ExplicitFlags & ~OVERRIDE
      else
        ExplicitFlags
...
// ----- tests ----------------------------------------------------------------------
    ...
    final def flags: Long = {
      if (Statistics.hotEnabled)
        Statistics.incCounter(flagsCount)
      val fs = _rawflags & phase.flagMask
      (fs | ((fs & LateFlags) >>> LateShift)) & ~(
        (fs & AntiFlags) >>> AntiShift
      )
    }
    ...
    final def isDelambdafyTarget =
      isArtifact && isMethod && (name containsName tpnme.ANON_FUN_NAME)
    final def isDefinedInPackage = effectiveOwner.isPackageClass
    final def needsFlatClasses =
      phase.flatClasses && rawowner != NoSymbol && !rawowner.isPackageClass
    ...
    def packageObject: Symbol =
      if (isPackageClass)
        ...
        moduleClass.packageObject
      else
        NoSymbol

    /** If this is a constructor, its owner: otherwise this.
      */
    final def skipConstructor: Symbol =
      if (isConstructor)
        owner
      else
        this
    ...
    final def isOmittablePrefix =
      /*!settings.debug.value &&*/ (UnqualifiedOwners(skipPackageObject)
        || isEmptyPrefix)
    def isEmptyPrefix =
    ...
    def migrationMessage =
      getAnnotation(MigrationAnnotationClass) flatMap {
        _.stringArg(0)
      }
    def migrationVersion =
      getAnnotation(MigrationAnnotationClass) flatMap {
        _.stringArg(1)
      }
    def elisionLevel =
      getAnnotation(ElidableMethodClass) flatMap {
        _.intArg(0)
      }
    def implicitNotFoundMsg =
      getAnnotation(ImplicitNotFoundClass) flatMap {
        _.stringArg(0)
      }
    def implicitAmbiguousMsg =
      getAnnotation(ImplicitAmbiguousClass) flatMap {
        _.stringArg(0)
      }
...
// ------ name attribute --------------------------------------------------------------
    ...
    protected def createTypeSkolemSymbol(
        name: TypeName,
        origin: AnyRef,
        pos: Position,
    ...
    def accessBoundary(base: Symbol): Symbol = {
      if (hasFlag(PRIVATE) || isLocalToBlock)
        owner
      else if (hasAllFlags(PROTECTED | STATIC | JAVA))
        enclosingRootClass
...
//          activeLocks += 1
          ...
          if (cnt == 3)
            abort(s"no progress in completing $this: $tp")
        }
        rawInfo
      } catch {
    ...
    def substInfo(syms0: List[Symbol], syms1: List[Symbol]): this.type =
      if (syms0.isEmpty)
        ...
        modifyInfo(_.substSym(syms0, syms1))
        ...
        if (validTo < curPeriod) {
          ...
          try {
            ...
            if (validTo < curPeriod) {
              ...
              _validTo =
                if (itr.pid == NoPhase.id)
                  curPeriod
                else
                  period(currentRunId, itr.pid)
            }
          } finally {
            phase = current
          }
    ...
    def unsafeTypeParams: List[Symbol] =
      if (isMonomorphicType)
        ...
        enteringPhase(unsafeTypeParamPhase)(rawInfo.typeParams)
...
// ----- setters implemented in selected subclasses -------------------------------------
    ...
    def typeOfThis_=(tp: Type) {
      throw new UnsupportedOperationException(
        "typeOfThis_= inapplicable for " + this)
    }
    def sourceModule_=(sym: Symbol) {
      throw new UnsupportedOperationException(
...
// ------ comparisons ----------------------------------------------------------------
    ...
    final def isLess(that: Symbol): Boolean = {
      def baseTypeSeqLength(sym: Symbol) =
        if (sym.isAbstractType)
          1 + sym.info.bounds.hi.baseTypeSeq.length
        else
          sym.info.baseTypeSeq.length
      if (this.isType)
        (
          that.isType && {
    ...
    def isBottomSubClass(that: Symbol) =
      ((this eq NothingClass)
        || (this eq NullClass) && that.isClass && (that ne NothingClass) && !(
          that isNonBottomSubClass AnyValClass
        ))
    ...
    def filter(cond: Symbol => Boolean): Symbol =
      if (isOverloaded) {
        ...
        if (!changed)
          ...
          NoSymbol
        else if (alts1.tail.isEmpty)
          alts1.head
        else
          owner.newOverloaded(info.prefix, alts1.reverse)
      } else if (cond(this))
        this
      else
        NoSymbol
...
// ------ access to related symbols --------------------------------------------------
    ...
    final def outerClass: Symbol =
      if (this == NoSymbol) {
        ...
        owner
      else if (isClassLocalToConstructor)
        owner.enclClass.outerClass
      else
        owner.outerClass
    ...
    def enclosingTopLevelClassOrDummy: Symbol =
      if (isTopLevel) {
        if (isClass)
          ...
          moduleClass.orElse(this)
      } else
        owner.enclosingTopLevelClassOrDummy
    ...
    private def matchingSymbolInternal(
        site: Type,
      ...
      if (candidate.isOverloaded)
        candidate filter qualifies
      else if (qualifies(candidate))
        candidate
      else
        NoSymbol
    }
    ...
    private def matchingInheritedSymbolIn(baseClass: Symbol): Symbol =
      if (canMatchInheritedSymbols)
        matchingSymbol(baseClass, owner.thisType)
      else
        NoSymbol
    ...
    lazy val isOverridingSymbol = (canMatchInheritedSymbols
    ...
    def needsExpandedSetterName =
      (if (isMethod)
         hasStableFlag && !isLazy
       else
         hasNoFlags(LAZY | MUTABLE))
    def setterNameInBase(base: Symbol, expanded: Boolean): TermName =
      if (expanded)
        nme.expandedSetterName(setterName, base)
      else
        setterName
    ...
    final def makeNotPrivate(base: Symbol) {
      if (this.isPrivate) {
        ...
        if (!isStaticModule && !isClassConstructor) {
          expandName(base)
          if (isModule)
            moduleClass.makeNotPrivate(base)
        }
      }
    }
    ...
    def associatedFile_=(f: AbstractFile) {
    ...
    final def toOption: Option[Symbol] =
      if (exists)
        Some(this)
      else
        None
...
// ------ toString -------------------------------------------------------------------
    ...
    final def keyString: String =
      if (isJavaInterface)
        ...
        "class"
      else if (isType && !isParameter)
        "type"
      else if (isVariable)
        "var"
      else if (hasPackageFlag)
        "package"
      else if (isModule)
        "object"
    ...
    private def symbolKind: SymbolKind = {
      var kind =
        if (isTermMacro)
          ("term macro", "macro method", "MACM")
        else if (isInstanceOf[FreeTermSymbol])
          ("free term", "free term", "FTE")
        else if (isInstanceOf[FreeTypeSymbol])
          ("free type", "free type", "FTY")
        else if (isPackageClass)
          ("package class", "package", "PKC")
        else if (hasPackageFlag)
          ("package", "package", "PK")
        else if (isPackageObject)
          ("package object", "package", "PKO")
        else if (isPackageObjectClass)
          ("package object class", "package", "PKOC")
        else if (isAnonymousClass)
          ("anonymous class", "anonymous class", "AC")
        else if (isRefinementClass)
          ("refinement class", "", "RC")
        else if (isModule)
          ("module", "object", "MOD")
        else if (isModuleClass)
          ("module class", "object", "MODC")
        else if (isGetter)
          (
            "getter",
            if (isSourceMethod)
          ...
          (
            ...
            if (isSourceMethod)
              ...
              "value",
            "SET")
        else if (isTerm && isLazy)
          ("lazy value", "lazy value", "LAZ")
        else if (isVariable)
          ("field", "variable", "VAR")
        else if (isTrait)
          ("trait", "trait", "TRT")
          ...
          ("", "", "???")
      if (isSkolem)
        kind = (kind._1, kind._2, kind._3 + "#SKO")
    ...
    def nameString: String = {
      val name_s =
        if (settings.debug.value)
          ...
          unexpandedName.dropLocal.decode
      val kind_s =
    ...
    def fullNameString: String = {
      def recur(sym: Symbol): String = {
        if (sym.isRootSymbol || sym == NoSymbol)
          sym.nameString
        else if (sym.owner.isEffectiveRoot)
          sym.nameString
        else
          recur(sym.effectiveOwner.enclClass) + "." + sym.nameString
      }
    ...
    final def idString = {
      val id_s =
        if (settings.uniqid.value)
          ...
          ""
      val owner_s =
        if (settings.Yshowsymowners.value)
          "@" + owner.id
        else
          ""
      id_s + owner_s
    ...
    final def infoString(tp: Type): String = {
      def parents =
        (if (settings.debug.value)
           parentsString(tp.parents)
         else
           briefParentsString(tp.parents))
    ...
    private def defStringCompose(infoString: String) =
      compose(
        flagString,
        keyString,
  ...
  class TermSymbol protected[Symbols] (
    ...
    ***/
    ...
    override def outerSource: Symbol =
      ...
      if (unexpandedName endsWith (nme.OUTER))
        initialize.referenced
      else
        NoSymbol
  ...
  class MethodSymbol protected[Symbols] (
    ...
    def typeAsMemberOf(pre: Type): Type = {
      if (mtpePeriod == currentPeriod) {
        if ((mtpePre eq pre) && (mtpeInfo eq info))
          return mtpeResult
    ...
    override def exceptions =
      for (ThrownException(tp) <- annotations)
        yield tp.typeSymbol
  }
  ...
  abstract class TypeSymbol protected[Symbols] (
    ...
    private def newPrefix =
      if (this hasFlag EXISTENTIAL | PARAM)
        NoPrefix
      else
        owner.thisType
    ...
    private def updateTypeCache() {
      ...
      tpeCache = newTypeRef(
        if (noTypeParams)
          ...
          unsafeTypeParams map (_.typeConstructor))
  ...
  class ClassSymbol protected[Symbols] (
    ...
    override def toString =
      (if (isAnonOrRefinementClass)
         anonOrRefinementString
       else
         super.toString)
  ...
  def deriveTypeWithWildcards(syms: List[Symbol])(tpe: Type): Type = {
    if (syms.isEmpty)
      ...
      tpe.instantiateTypeParams(syms, syms map (_ => WildcardType))
  }
  ...
  final def closestEnclMethod(from: Symbol): Symbol =
    if (from.isSourceMethod)
      from
    else if (from.isClass)
      NoSymbol
    else
      closestEnclMethod(from.owner)
...
// -------------- Completion --------------------------------------------------------
  ...
  private def relevantSymbols(syms: Seq[Symbol]) =
    syms.flatMap(sym => List(sym, sym.moduleClass, sym.sourceModule))
  def markFlagsCompleted(syms: Symbol*)(mask: Long): Unit =
    relevantSymbols(syms).foreach(_.markFlagsCompleted(mask))
  def markAllCompleted(syms: Symbol*): Unit =
    relevantSymbols(syms).foreach(_.markAllCompleted)
}
