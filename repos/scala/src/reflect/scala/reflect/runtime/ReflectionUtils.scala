...
object ReflectionUtils {
  ...
  def unwrapHandler[T](
    ...
    case ex if pf isDefinedAt unwrapThrowable(ex) =>
      pf(unwrapThrowable(ex))
  }
  ...
  def show(cl: ClassLoader): String = {
    ...
    cl match {
      ...
      case null =>
        "primordial classloader with boot classpath [%s]"
          .format(inferClasspath(cl))
    }
  }
