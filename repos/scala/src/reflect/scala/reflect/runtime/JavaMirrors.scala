...
// ----------- Implementations of mirror operations and classes  -------------------
    ...
    private def checkConstructorOf(sym: Symbol, owner: ClassSymbol) {
      ...
      if (owner == ArrayClass)
        ErrorArrayConstructor(sym, owner)
      ensuringNotFree(sym) {
    ...
    private class BytecodelessMethodMirror[T: ClassTag](
      ...
      def apply(args: Any*): Any = {
        ...
        if (!perfectMatch && !varargMatch) {
          val n_arguments =
            if (isVarArgsList(params))
              s"${params.length - 1} or more"
            else
              s"${params.length}"
          val s_arguments =
            if (params.length == 1 && !isVarArgsList(params))
              "argument"
            else
              "arguments"
        ...
        def invokePrimitiveMethod = {
          val jmeths = classOf[BoxesRunTime]
            .getDeclaredMethods
    ...
    private class JavaModuleMirror(val outer: AnyRef, val symbol: ModuleSymbol)
      ...
      def instance = {
        if (symbol.isTopLevel)
          ...
          innerSingletonInstance(outer, symbol.name.toString)
      }
      override def toString =
...
// -------------------- Java to Scala  -----------------------------------
    ...
    private def assignAssociatedFile(
      ...
      if (module != NoSymbol)
        module.associatedFile = associatedFile
    }
    ...
    private def followStatic(
      ...
      if (mods.isStatic)
        module.moduleClass.orElse(clazz)
      else
        clazz
    ...
    private[JavaMirrors] def makeScalaPackage(fullname: String): ModuleSymbol =
      gilSynchronized {
        ...
        val ownerModule: ModuleSymbol =
          if (split > 0)
            packageNameToScala(fullname take split)
          else
            this.RootPackage
    ...
    private def jconstrAsScala1(jconstr: jConstructor[_]): MethodSymbol = {
      ...
      if (jconstr.javaFlags.isVarargs)
        constr modifyInfo arrayToRepeated
