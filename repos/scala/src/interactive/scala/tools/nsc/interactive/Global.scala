...
/**
  ...
  */
trait CommentPreservingTypers extends Typers { self: Analyzer =>
          ...
          logreplay("exception thrown", scheduler.pollThrowable()) match {
            ...
            case Some(ShutdownReq) =>
              scheduler.synchronized {
                // lock the work queue so no more items are posted while we clean it up
