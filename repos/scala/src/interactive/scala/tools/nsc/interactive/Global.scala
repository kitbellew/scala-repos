...
with ContextTrees with RichCompilationUnits with Picklers {
  ...
  val log =
    if (replayName != "")
      new Replayer(new FileReader(replayName))
    else if (logName != "")
      new Logger(new FileWriter(logName))
    else
      NullLogger
  ...
  def demandNewCompilerRun() = {
    if (outOfDate)
      throw new FreshRunReq // cancel background compile
    else
      outOfDate = true // proceed normally and enable new background compile
  }
            ...
            case Some(ShutdownReq) =>
              scheduler
                .synchronized { // lock the work queue so no more items are posted while we clean it up
                  ...
                  debugLog(
                    "ShutdownReq: cleaning work queue (%d items)"
                      .format(units.size))
                  debugLog(
    ...
    for (s <- allSources; if !ignoredFiles(s.file);
      ...
      try {
        if (!unit.isUpToDate)
          ...
          debugLog("already up to date: " + unit)
...
//          println("tree not found at "+pos)
  ...
  private def findMirrorSymbol(
    ...
    if (newsym == NoSymbol) {
      if (rawsym.exists && !rawsym.isOverloaded)
        rawsym
      else {
    ...
    respond(response) {
      withTempUnits(fragments.unzip._2) { units =>
        for ((sym, src) <- fragments) {
          ...
          if (mirror ne NoSymbol)
            forceDocComment(mirror, units(src))
        }
