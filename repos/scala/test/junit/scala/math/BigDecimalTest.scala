...
class BigDecimalTest {
  ...
  def hashCodesAgreeTest() {
    ...
    val d: Double = 100000
    assert(
      d.## == l.## && l.## == bd.## && bd.## == bi.## && (bd pow 4).hashCode ==
        (bi pow 4).hashCode && BigDecimal("1e150000").hashCode != BigDecimal(
  ...
  def churnRepresentationTest() {
    ...
    for (i <- 1 to 1000) {
      ...
      assert(
        {
          BigDecimal.decimal(d).isDecimalDouble && BigDecimal.binary(d)
            .isBinaryDouble && BigDecimal.exact(d).isExactDouble
        },
    ...
    for (i <- 1 to 1000) {
      ...
      assert(
        {
          BigDecimal.decimal(f).isDecimalFloat && BigDecimal.binary(f)
            .isBinaryFloat && BigDecimal.exact(f).isExactFloat
        },
