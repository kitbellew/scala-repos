...
class CompletionTest {
  ...
  def t4438_arrayCompletion(): Unit = {
    ...
    assert(completer.complete("Array(1, 2, 3) rev").candidates.contains(
      "reverseMap"))
  }
  ...
  def completions(): Unit = {
    ...
    checkExact(
      completer,
      """object O { def x_y_z = 0; val x_z_y = ""; type T = x_""")("x_z_y")
    checkExact(
      completer,
      """def method { def x_y_z = 0; val x_z_y = ""; type T = x_""")("x_z_y")
  ...
  def defString(): Unit = {
    ...
    checkExact(
      completer,
      "(p: {def x_y_z: Int; def x_y_z(a: String): Int }) => p.x_y")("x_y_z")
  ...
  def firstCompletionWithNoPrefixHidesUniversalMethodsAndExtensionMethods()
    ...
    assert(
      Set("asInstanceOf", "==").diff(
        completer.complete("class C(val a: Int, val b: Int) { this.").candidates
          .toSet).isEmpty)
