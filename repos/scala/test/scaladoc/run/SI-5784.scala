...
object Test extends ScaladocModelTest {
  ...
  def testModel(rootPackage: Package) = {
    ...
    assert(
      ...
      Base.members.filter(
        _.inDefinitionTemplates.head == Base).length + " == 5")
    assert(
    ...
    assert(
      ...
      BaseT.members.filter(
        _.inDefinitionTemplates.head == Base).length + " == 0")
    assert(
      BaseT.members.map(_.name).sorted == Foo.members.map(_.name).sorted,
      BaseT.members.map(_.name).sorted + " == " + Foo.members.map(
        _.name).sorted)
    assert(
    ...
    assert(
      ...
      ApiT.members.map(_.name).sorted + " == " + FooApi.members.map(
        _.name).sorted)
    assert(
      ApiT.companion.isDefined,
