...
class Factory(val g: Global, val s: doc.Settings)
  ...
  def parseComment(s: String): Option[Inline] = strip(getComment(s))
...
object Test extends Properties("CommentFactory") {
  ...
  property("parse") = parse("/** One two three */", Text("One two three"))
...
/**
 * hello ^world^ */""", Chain(List(Text("hello "), Superscript(Text("world")))))
 ...
 */""",
  ...
  */
    ...
    body == Body(List(Paragraph(Chain(List(Summary(Chain(List(
      HtmlTag(
        "<strong><code>foo</code> has been deprecated and will be removed in a future version. Please call <code>bar</code> instead.</strong>"),
      Text("\n"),
      Text("")
