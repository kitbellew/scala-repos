...
abstract class RangeTest(kind: String) extends Properties("Range " + kind) {
  ...
  def genBoundaryRange =
    for {
      ...
      if (isStart)
        ...
        Range(boundary - size * boundary.signum, boundary, step * signum)
    }
  ...
  def genRangeClosedByOne =
    for (r <- genRangeOpenByOne)
...
//    println("take "+str(r))
      val t = r take x
      (t.size ==
        (0 max x min r.size) && t.start == r.start && t.step == r.step) :|
        str(r) + " / " + str(t) + ": " + x
    }
