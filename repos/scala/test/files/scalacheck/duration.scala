...
object Test extends Properties("Division of Duration by Long") {
  ...
  val genTwoSmall = for {
    a <- weightedLong
    b <-
      choose(-(Long.MaxValue / max(1, abs(a))), Long.MaxValue / max(1, abs(a)))
  } yield (a, b)

  val genTwoLarge = for {
    a <- weightedLong
    b <-
      arbitrary[Long] suchThat (b => (abs(b) > Long.MaxValue / max(1, abs(a))))
  } yield (a, b)
    ...
    case (a, b) =>
      val shouldFit =
        a != Long.MinValue && // must fail due to illegal duration length
          (b != Long
            .MinValue || a == 0) && // Long factor may only be MinValue if the duration is zero, otherwise the result will be illegal
