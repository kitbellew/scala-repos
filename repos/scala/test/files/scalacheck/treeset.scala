...
object Test extends Properties("TreeSet") {
  ...
  property("worst-case tree height is iterable") = forAll(
    choose(0, 10),
    arbitrary[Boolean]) { (n: Int, even: Boolean) =>
    /*
  ...
  property("drop") = forAll { (subject: TreeSet[Int]) =>
    val n = choose(0, subject.size).sample.get
    (subject.size - n) == subject.drop(n).size && subject.drop(n)
  ...
  property("slice") = forAll(genSliceParms) {
    case (subject, from, until) =>
      ...
      slice.size == until - from && subject.toSeq == subject.take(from)
  ...
  property("from is inclusive") = forAll { (subject: TreeSet[Int]) =>
    subject.nonEmpty ==> {
      ...
      subject.from(from).firstKey == from && subject.from(from)
        .forall(_ >= from)
    }
  }
  ...
  property("ordering must not be null") = throws(classOf[NullPointerException])(
    TreeSet.empty[Int](null))
}
