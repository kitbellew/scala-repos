...
package scala.collection.mutable {
  ...
  object MutableTreeSetViewProperties
      extends Properties("mutable.TreeSetView") {
    type K = String
    ...
    property("iteratorFrom, keysIteratorFrom") = forAll {
      (ks: Set[K], k: K, from: Option[K], until: Option[K]) =>
        ...
        setView.iteratorFrom(k).toSeq ==
          keysInView(ks, newLower, until).toSeq.sorted
    }
    ...
    property("headOption") = forAll {
      (set: mutable.TreeSet[K], from: Option[K], until: Option[K]) =>
        val setView = set.rangeImpl(from, until)
        setView.headOption ==
          Try(keysInView(set.iterator, from, until).next()).toOption
    }
    ...
    property("lastOption") = forAll {
      (set: mutable.TreeSet[K], from: Option[K], until: Option[K]) =>
        val setView = set.rangeImpl(from, until)
        setView.lastOption ==
          Try(keysInView(set.iterator, from, until).max).toOption
    }
