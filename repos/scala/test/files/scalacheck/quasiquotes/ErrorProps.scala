...
object ErrorProps extends QuasiquoteProperties("errors") {
  ...
  property("only literal string arguments") =
    fails("Quasiquotes can only be used with literal strings", """
      val s: String = "foo"
      StringContext(s).q()
    """)
  ...
  property("can't unquote with ... rank here") =
    fails("Can't unquote with ... here", """
  ...
  property("SI-8451 deconstruction: disallow everything except for constructor calls in secondary constructor bodies") =
    fails("'this' expected but unquotee found", """
