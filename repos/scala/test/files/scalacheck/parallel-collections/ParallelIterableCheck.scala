...
abstract class ParallelIterableCheck[T](collName: String)
    extends Properties(collName)
    with Operators[T] {
  ...
  def instances(vals: Seq[Gen[T]]): Gen[Iterable[T]] =
    oneOf(
      ...
      for (sz <- choose(1000, 2000))
        yield ofSize(vals, sz),
      for (sz <- choose(4000, 4001))
        yield ofSize(vals, sz),
      for (sz <- choose(10000, 10001))
        yield ofSize(vals, sz)
    )
  ...
  def areEqual(t1: GenTraversable[T], t2: GenTraversable[T]) =
    if (hasStrictOrder) {
      ...
      (t1, t2) match { // it is slightly delicate what `equal` means if the order is not strict
        case (m1: GenMap[_, _], m2: GenMap[_, _]) =>
          m1 == m2 && m2 == m1
  ...
  property("reductions must be equal for assoc. operators") =
    forAll(collectionPairs) {
      case (t, coll) =>
        if (t.size != 0) {
          val results =
            for ((op, ind) <- reduceOperators.zipWithIndex)
              yield {
  ...
  property("counts must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        val results =
          for ((pred, ind) <- countPredicates.zipWithIndex)
            yield {
              val tc = t.count(pred)
  ...
  property("both must find or not find an element") =
    forAll(collectionPairs) {
      case (t, coll) =>
        val results =
          for ((pred, ind) <- findPredicates.zipWithIndex)
            yield {
              val ft = t.find(pred)
  ...
  property("mappings must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        val results =
  ...
  property("collects must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        val results =
          for ((f, ind) <- partialMapFunctions.zipWithIndex)
            yield {
              val ps = t.collect(f)
  ...
  property("flatMaps must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        (
  ...
  property("filters must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        (
  ...
  property("filterNots must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        (
          for ((p, ind) <- filterNotPredicates.zipWithIndex)
            yield {
              ...
              if (tf != cf || cf != tf)
                printComparison(t, coll, tf, cf, ind)
  ...
  if (!isCheckingViews)
    property("partitions must be equal") =
      forAll(collectionPairs) {
        case (t, coll) =>
          (
            for ((p, ind) <- partitionPredicates.zipWithIndex)
              yield {
  ...
  if (hasStrictOrder)
    property("slices must be equal") =
      forAll(collectionPairsWith2Indices) {
  ...
  if (hasStrictOrder)
    property("takeWhiles must be equal") =
      forAll(collectionPairs) {
        case (t, coll) =>
          (
  ...
  if (hasStrictOrder)
    property("spans must be equal") =
      forAll(collectionPairs) {
        case (t, coll) =>
          (
            for ((pred, ind) <- spanPredicates.zipWithIndex)
              yield {
                val tsp = t.span(pred)
  ...
  if (hasStrictOrder)
    property("dropWhiles must be equal") =
      forAll(collectionPairs) {
        case (t, coll) =>
          (
  ...
  property("++s must be equal") =
    forAll(collectionTriplets) {
  ...
  if (hasStrictOrder)
    property("copies to array must be equal") =
      forAll(collectionPairs) {
        case (t, coll) =>
  ...
  property("groupBy must be equal") =
    forAll(collectionPairs) {
      case (t, coll) =>
        (
          for ((f, ind) <- groupByFunctions.zipWithIndex)
            yield {
