...
abstract class ParallelIterableCheck[T](collName: String)
  ...
  property("mappings must be equal") = forAll(collectionPairs) {
    case (t, coll) =>
      val results =
  ...
  property("collects must be equal") = forAll(collectionPairs) {
    case (t, coll) =>
      val results =
  ...
  property("filterNots must be equal") = forAll(collectionPairs) {
    case (t, coll) =>
  ...
  if (!isCheckingViews)
    property("partitions must be equal") = forAll(collectionPairs) {
      case (t, coll) =>
  ...
  if (hasStrictOrder)
    property("takes must be equal") = forAll(collectionPairsWithLengths) {
      case (t, coll, n) =>
        ("take " + n + " elements") |: t.take(n) == coll.take(n)
    }

  if (hasStrictOrder)
    property("drops must be equal") = forAll(collectionPairsWithLengths) {
      case (t, coll, n) =>
  ...
  if (hasStrictOrder)
    property("splits must be equal") = forAll(collectionPairsWithLengths) {
      case (t, coll, n) =>
  ...
  if (hasStrictOrder)
    property("takeWhiles must be equal") = forAll(collectionPairs) {
      case (t, coll) =>
  ...
  if (hasStrictOrder)
    property("spans must be equal") = forAll(collectionPairs) {
      case (t, coll) =>
  ...
  if (hasStrictOrder)
    property("copies to array must be equal") = forAll(collectionPairs) {
      case (t, coll) =>
  ...
  if (hasStrictOrder)
    property("scans must be equal") = forAll(collectionPairs) {
      case (t, coll) =>
        (for (((first, op), ind) <- foldArguments.zipWithIndex)
          yield {
