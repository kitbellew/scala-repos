...
abstract class ParallelSeqCheck[T](collName: String)
  ...
  def traversable2Seq(t: Traversable[T]): Seq[T] = {
    if (t.isInstanceOf[Iterable[_]])
      t.asInstanceOf[Iterable[T]].iterator.toList
    else
      t.toList
  }
      ...
      case (s, coll) =>
        (
          for ((pred, ind) <- segmentLengthPredicates.zipWithIndex)
            yield {
              ("operator " + ind) |:
                s.prefixLength(pred) == coll.prefixLength(pred)
            }
        ).reduceLeft(_ && _)
    }
  ...
  property("lastIndexWheres must be equal") =
    forAll(collectionPairsWithLengths) {
      case (s, coll, len) =>
        (
          for ((pred, ind) <- lastIndexWherePredicates.zipWithIndex)
            yield {
              val end =
                if (len >= s.size)
                  s.size - 1
  ...
  property("reverses must be equal") =
    forAll(collectionPairs) {
      case (s, coll) =>
        (s.length == 0 &&
          s.getClass == classOf[collection.immutable.Range]) || {
          val sr = s.reverse
          val cr = coll.reverse
          if (sr != cr) {
            println("from: " + s)
  ...
  property("sameElements must be equal") =
    forAll(collectionPairsWithModifiedWithLengths) {
      case (s, coll, collmodif, len) =>
        ...
        ("identity" |:
          s.sameElements(s.map(e => e)) ==
          coll.sameElements(coll.map(e => e))) &&
        ("vice-versa" |: s.sameElements(coll) == coll.sameElements(s)) &&
        ("equal" |: s.sameElements(coll)) &&
        ("modified" |: scm == ccm) &&
        (
          for ((it, ind) <- sameElementsSeqs.zipWithIndex)
            yield {
  ...
  property("startsWiths must be equal") =
    forAll(collectionPairsWithModifiedWithLengths) {
      case (s, coll, collmodif, len) =>
        val pos =
          if (len < 0)
    ...
    forAll(collectionPairsWithModified) {
      case (s, coll, collmodif) =>
        ("ends with self" |: s.endsWith(s) == coll.endsWith(s)) &&
          ("ends with tail" |:
            (s.length == 0 ||
              s.endsWith(s.tail) == coll.endsWith(coll.tail))) &&
      ...
      forAll(collectionTripletsWith2Indices) {
        case (s, coll, pat, from, repl) =>
          ("with seq" |:
            ...
            ("with par" |:
              s.patch(from, pat, repl) ==
              coll.patch(from, fromSeq(pat), repl)) &&
            ("with empty" |:
              s.patch(from, Nil, repl) ==
              coll.patch(from, fromSeq(Nil), repl)) &&
            ("with one" |:
              (s.length == 0 ||
  ...
  property("corresponds must be equal") =
    forAll(collectionPairsWithModified) {
      case (s, coll, modified) =>
        ...
        ("self" |:
          s.corresponds(s)(_ == _) == coll.corresponds(coll)(_ == _)) &&
        ("modified" |:
