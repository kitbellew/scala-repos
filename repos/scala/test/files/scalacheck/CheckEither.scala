...
object Test extends Properties("Either") {
  ...
  val prop_either1 = forAll((n: Int) =>
    Left(n).fold(x => x, b => sys.error("fail")) == n)

  val prop_either2 = forAll((n: Int) =>
    Right(n).fold(a => sys.error("fail"), x => x) == n)
  ...
  object CheckLeftProjection {
    ...
    val prop_flatMapRightIdentity = forAll((e: Either[Int, Int]) =>
      e.left.flatMap(Left(_)) == e)
    ...
    val prop_flatMapComposition = forAll((e: Either[Int, Int]) => {
      ...
      def g(x: Int) =
        if (x % 7 == 0)
          ...
          Left(x)
      e.left.flatMap(f(_)).left.flatMap(g(_)) ==
        e.left.flatMap(f(_).left.flatMap(g(_)))
    })
    ...
    val prop_filter = forAll((e: Either[Int, Int], x: Int) =>
      e.left.filter(_ % 2 == 0) ==
        (if (e.isRight || e.left.get % 2 != 0)
           None
         else
           Some(e)))
    ...
    val prop_flatMapComposition = forAll((e: Either[Int, Int]) => {
      ...
      def g(x: Int) =
        if (x % 7 == 0)
          ...
          Left(x)
      e.right.flatMap(f(_)).right.flatMap(g(_)) ==
        e.right.flatMap(f(_).right.flatMap(g(_)))
    })
