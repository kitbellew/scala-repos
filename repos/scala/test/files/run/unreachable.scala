...
object Test extends App {
  ...
  def unreachableIf: Int = {
    return 42
    if (nextInt % 2 == 0) 0 else 1
  }

  def unreachableIfBranches: Int = {
    if (nextInt % 2 == 0) return 42 else return 42
  ...
  def unreachableLeftBranch: Int = {
    val result = if (nextInt % 2 == 0) return 42 else 42

    return result
  }

  def unreachableRightBranch: Int = {
    val result = if (nextInt % 2 == 0) 42 else return 42
  ...
  def unreachableTryCatchFinally: Int = {
    return 42
    try { return 0 }
    catch { case x: Throwable => return 1 }
    finally { return 2 }
    return 3
  }
  ...
  def unreachableAfterTry: Int = {
    try { return 42 }
    catch { case x: Throwable => return 2 }
    return 3
  }

  def unreachableAfterCatch: Int = {
    try { error("haha") }
    catch { case x: Throwable => return 42 }
    return 3
  }
