...
object M0 {
  ...
  def sqrtIter0(guess: Double, x: Double): Double =
    if (isGoodEnough0(guess, x)) guess else sqrtIter0(improve0(guess, x), x);
  ...
  def sqrt1(x: Double) = {
    def sqrtIter1(guess: Double, x: Double): Double =
      if (isGoodEnough1(guess, x)) guess else sqrtIter1(improve1(guess, x), x);
  ...
  def sqrt2(x: Double) = {
    def sqrtIter2(guess: Double): Double =
      if (isGoodEnough2(guess)) guess else sqrtIter2(improve2(guess));
...
object M1 {
  ...
  def sqrt(x: Double): Double = {
    def sqrtIter(prev: Double, guess: Double): Double =
      if (isGoodEnough(prev, guess)) guess else sqrtIter(guess, improve(guess));
...
object M2 {
  ...
  def sqrt(x: Double): Double = {
    def sqrtIter(guess: Double): Double = {
      ...
      if (isGoodEnough(guess, next)) next else sqrtIter(next)
    }
...
object M3 {
  ...
  def cbrt(x: Double): Double = {
    def cbrtIter(guess: Double): Double = {
      val next = improve(guess);
      if (isGoodEnough(guess, next)) next else cbrtIter(next)
    }
...
object M4 {
  def pascal(c: Int, l: Int): Int =
    if (c <= 0 || c >= l) 1 else pascal(c - 1, l - 1) + pascal(c, l - 1);
