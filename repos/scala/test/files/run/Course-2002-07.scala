      ...
      this match {
        ...
        case Var(name) =>
          if (name == v.name)
            ...
            Number(0)
        case Sum(e1, e2) =>
          Sum(e1 derive v, e2 derive v)
        case Prod(e1, e2) =>
          Sum(Prod(e1, e2 derive v), Prod(e2, e1 derive v))
      }
  }
      ...
      this match {
        ...
        case Var(name) =>
          if (name == v.name)
            ...
            Number(0)
        case Sum(e1, e2) =>
          (e1 derive v) + (e2 derive v)
        case Prod(e1, e2) =>
          e1 * (e2 derive v) + e2 * (e1 derive v)
      }
  }
  case class Number(x: Int) extends Expr {
  ...
  def eval(e: Expr): Int =
    e match {
      case Number(n) =>
        n
      case Var(_) =>
        sys.error("cannot evaluate variable")
      case Sum(e1, e2) =>
        eval(e1) + eval(e2)
      case Prod(e1, e2) =>
        eval(e1) * eval(e2)
  ...
  sealed trait Expr {
    ...
    def *(that: Expr): Expr =
      if (this *<= that)
        (this, that) match {
          ...
          case (Lit(l), Lit(r)) =>
            Lit(l * r)
          case (Var(_), Var(_)) if (this equ that) =>
            Pow(this, 2)
          case (Var(_), Pow(r, n)) if (this equ r) =>
            Pow(this, n + 1)
          case (Pow(ll, lr), Pow(rl, rr)) if (ll equ rl) =>
            Pow(ll, lr + rr)
          case (l, Mul(rl, rr)) if (rl *<= l) =>
            (rl * l) * rr
          case (_, _) =>
            Mul(this, that)
        }
    ...
    def evaluate(vars: List[(String, Int)]): Int =
      this match {
        ...
        case Var(name) =>
          lookup(vars, name)
        case Add(l, r) =>
          l.evaluate(vars) + r.evaluate(vars)
        case Mul(l, r) =>
          l.evaluate(vars) * r.evaluate(vars)
        case Pow(l, r) =>
          power(l.evaluate(vars), r)
      }
    ...
    def equ(that: Expr): Boolean =
      (this, that) match {
        ...
        case (Var(l), Var(r)) =>
          l == r
        case (Add(ll, lr), Add(rl, rr)) =>
          (ll equ rl) && (lr equ rr)
        case (Mul(ll, lr), Mul(rl, rr)) =>
          (ll equ rl) && (lr equ rr)
        case (Pow(ll, lr), Pow(rl, rr)) =>
          (ll equ rl) && (lr == rr)
        case _ =>
          false
      }
