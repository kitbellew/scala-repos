...
object Test {
  def main(args: Array[String]): Unit = {
    "catdog" match {
      case p0.Single(x) =>
        println(s"`${x._1}` has ${x._1.length} chars")
      case x =>
        println("fail: " + x)
    }
    "catdog" match {
      case p0.SingleNoProduct(x) =>
        println(s"`$x` has ${x.length} chars")
      case x =>
        println("fail: " + x)
    }
    "catdog" match {
      case p1.Triple(x, y, z) =>
        List(x, y, z) foreach println
      case x =>
        println("fail: " + x)
    }
    // TODO
    "catdog" match {
      case p2.Triple(x, y, z) =>
        List(x, y, z) foreach println
      case x =>
        println("fail: " + x)
    }
