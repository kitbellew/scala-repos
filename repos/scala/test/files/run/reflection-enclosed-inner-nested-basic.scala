...
class B {
  object BB {
    class B1 {
      override def toString = "B1"; def foo = 1
    }
    private class B2 {
      override def toString = "B2"; def foo = 2
    }
    object B3 {
      override def toString = "B3"; def foo = 3
    }
    private object B4 {
      override def toString = "B4"; def foo = 4
    }
    object B5 extends B1 {
      override def toString = "B5"; override def foo = 5
    }
...
object Test extends App {
  ...
  def testNestedClass(name: String) = {
    ...
    val ctorMirror =
      cm.reflect(outer1.BB).reflectClass(sym).reflectConstructor(ctor)
