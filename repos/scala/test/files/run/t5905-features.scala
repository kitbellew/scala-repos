...
object Test extends DirectTest {
  ...
  override def show() = {
    ...
    exitingTyper {
      ...
      val feats = langf.declarations filter (s => isFeature(s)) map
        (_.name.decoded)
      val xmen = langf.member(TermName("experimental")).typeSignature
        .declarations filter (s => isFeature(s)) map
        (s => s"experimental.${s.name.decoded}")
