...
class FutureTests extends MinimalScalaTest {
    ...
    "have proper const representation for success" in {
      ...
      assert(
        ECNotUsed(ec =>
          f.recover({
            case _ =>
              fail("recover should not have been called")
          })(ec)) eq f)
      assert(
        ECNotUsed(ec =>
          f.recoverWith({
            case _ =>
              fail("flatMap should not have been called")
          })(ec)) eq f)
      assert(
        f.fallbackTo(f) eq f,
    ...
    "have proper const representation for failure" in {
      ...
      assert(
        ECNotUsed(ec =>
          f.collect({
            case _ =>
              fail("collect should not have been called")
          })(ec)) eq f)
      assert(
        ECNotUsed(ec =>
          f.zipWith(f)({ (_, _) =>
  ...
  "The Future companion object" should {
    ...
    "have a never member representing a never completed Future of Nothing" in {
      ...
      test match {
        case Future.`never` =>
        case _ =>
          fail("Future.never did not match Future.`never`")
      }
      ...
      ECNotUsed(ec =>
        test.onComplete({
          case _ =>
            fail("onComplete should not have been called")
        })(ec))
      ...
      ECNotUsed(ec =>
        test.recoverWith({
          case _ =>
            fail("recoverWith should not have been called")
        })(ec) eq test)
      ECNotUsed(ec =>
        test.andThen({
          case _ =>
            fail("andThen should not have been called")
        })(ec) eq test)
      ECNotUsed(ec =>
  ...
  "A future with global ExecutionContext" should {
    ...
    "transform failures to failures" in {
      ...
      intercept[Exception] {
        Await.result(f2, defaultTimeout)
      } mustBe expected2
    }
    ...
    "transform results to failures" in {
      ...
      intercept[Exception] {
        Await.result(f3, defaultTimeout)
      } mustBe expected3
    }
