...
object Test4 {
  ...
  class Foo9 {
    ...
    type myAnn = SourceAnnotation @beanGetter @field @BeanProperty
    @myAnn("http://eppli.com")
    var z = 0

    type myAnn2[T] = SourceAnnotation @beanGetter @field @BeanProperty
    @myAnn2[String]("http://eppli.com")
    var z2 = 0

    type myAnn3[CC[_]] = SourceAnnotation @beanGetter @field @BeanProperty
  ...
  def run {
    ...
    def printSourceAnnotation(a: Annotation) {
      ...
      println(
        "@test.SourceAnnotation(mails=" +
          ann.mails.deep.mkString("{", ",", "}") + ", value=" + ann.value + ")")
    }
    ...
    classOf[Foo9].getDeclaredFields
      .sortWith((x, y) => x.toString < y.toString) foreach
      printSourceAnnotations
...
object Test6 {
  ...
  def run {
    ...
    val d = new D(true)
    d.setProp(false)
    if (!d.isProp()) { println(new D(false).getM()) }
  }
}
