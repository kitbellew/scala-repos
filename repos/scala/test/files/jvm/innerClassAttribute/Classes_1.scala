...
class A4 {
  def f(l: List[String]): List[String] = {
    l map (
      _ + "1"
    ): @noinline // inlining adds a reference to the nested class scala/collection/generic/GenTraversableFactory$GenericCanBuildFrom
  }
}
    ...
    def f = {
      def g =
        List().map(x =>
          (
              (s: String) =>
                x)): @noinline // outer class A, no outer method (g is moved to the companion, doesn't exist in A)
