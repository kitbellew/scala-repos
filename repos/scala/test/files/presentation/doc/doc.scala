...
object Test extends InteractiveTest {
  ...
  override def runDefaultTests() {
            ...
            case Some(_) =>
              ...
              getComment(sym, batch, (sym, batch) :: Nil) match {
                ...
                case Some(comment) =>
                  ...
                  val actual = cnt(example) + cnt(version) + cnt(since) +
                    cnt(todo) + cnt(note) + cnt(see)
                  if (actual != i)
                    println(
    ...
    val (derived, base) = compiler.ask { () =>
      val derived = compiler.rootMirror.RootPackage.info.decl(newTermName("p"))
        .info.decl(newTypeName("Derived"))
