...
class TestSealedExhaustive { // compile only
  ...
  def ma6() =
    List(1, 2) match { // give up
      case List(1, 2) =>
      case x :: xs    =>
    }

  def ma7() =
    List(1, 2) match { //exhaustive
      case 1 :: 2 :: Nil =>
      case _             =>
    }
...
object TestUnreachable extends App {
  ...
  def not_unreachable(xs: Seq[Char]) =
    xs match {
      case Seq(x, y, _*) =>
        x :: y :: Nil
      case Seq(x) =>
  ...
  def not_unreachable2(xs: Seq[Char]) =
    xs match {
      case Seq(x, y) =>
        x :: y :: Nil
      case Seq(x, y, z, _*) =>
