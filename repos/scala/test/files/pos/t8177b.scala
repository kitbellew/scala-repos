...
object IntThing extends Thing {
  type A = Int
}
object ThingHolder {
  type Alias[AIn] = Thing { type A = AIn }
}
...
class View[AIn](val in: ThingHolder.Alias[AIn]) {
  def f(p: in.A): in.A = p
}
class SubView extends View[Int](IntThing) {
  override def f(p: in.A): in.A = p
}
