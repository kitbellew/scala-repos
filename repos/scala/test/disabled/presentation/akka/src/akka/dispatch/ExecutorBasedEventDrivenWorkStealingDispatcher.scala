...
class ExecutorBasedEventDrivenWorkStealingDispatcher(
  ...
  override private[akka] def dispatch(invocation: MessageInvocation) = {
    ...
    if (donationInProgress.value == false && (!mbox.isEmpty || mbox
          .dispatcherLock.locked) && attemptDonationOf(invocation, mbox)) {
  ...
  protected def doFindDonorRecipient(
    ...
    while ((i < prSz) && (recipient eq null)) {
      val actor = potentialRecipients(
        (i + startIndex) % prSz
      ) //Wrap-around, one full lap
