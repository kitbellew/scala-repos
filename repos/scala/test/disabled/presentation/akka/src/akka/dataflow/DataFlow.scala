...
object DataFlow {
  ...
  class DataFlowVariableException(message: String, cause: Throwable = null)
  ...
  sealed class DataFlowVariable[T <: Any](timeoutMs: Long) {
        ...
        case s @ Set(v) =>
          if (dataFlow.value.compareAndSet(None, Some(v.asInstanceOf[T]))) {
            ...
            throw new DataFlowVariableException(
              "Attempt to change data flow variable (from [" + dataFlow.value
                .get + "] to [" + v + "])")
        case Exit => self.stop()
      }
    }
    ...
    def <<(ref: DataFlowVariable[T]) {
      if (this.value.get.isEmpty) in ! Set(ref())
        ...
        throw new DataFlowVariableException(
          "Attempt to change data flow variable (from [" + this.value
            .get + "] to [" + ref() + "])")
    }
