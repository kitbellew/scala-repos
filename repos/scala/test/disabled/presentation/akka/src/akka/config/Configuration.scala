...
object Configuration {
  ...
  def load(
      data: String,
      importer: Importer = DefaultImporter): Configuration = {
  ...
  def fromMap(map: Map[String, Any]) = { new Configuration(map) }

  def fromString(data: String): Configuration = { load(data) }
}
...
class Configuration(val map: Map[String, Any]) {
  ...
  def getBoolean(key: String): Option[Boolean] = {
    getString(key) flatMap { s =>
      ...
      if (!isTrue && !falseValues.contains(s)) None else Some(isTrue)
    }
  }
  ...
  def getSection(name: String): Option[Configuration] = {
    ...
    if (m.isEmpty) None else Some(new Configuration(m))
  }
}
