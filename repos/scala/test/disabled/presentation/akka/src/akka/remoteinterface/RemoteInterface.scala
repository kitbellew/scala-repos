...
trait RemoteModule {
  ...
  private[akka] def findActorFactory(id: String): () => ActorRef =
    actorsFactories.get(id)

  private[akka] def findTypedActorById(id: String): AnyRef = typedActors.get(id)

  private[akka] def findTypedActorFactory(id: String): () => AnyRef =
    typedActorsFactories.get(id)
...
abstract class RemoteSupport
  ...
  private[akka] val actorsFactories =
...
trait RemoteServerModule extends RemoteModule {
  ...
  def start(
      host: String,
      port: Int,
      loader: Option[ClassLoader]): RemoteServerModule
  ...
  /**
    ...
    */
  def registerTypedActor(intfClass: Class[_], typedActor: AnyRef): Unit =
    registerTypedActor(intfClass.getName, typedActor)
  ...
  def registerTypedPerSessionActor(
      intfClass: Class[_],
      factory: => AnyRef): Unit = registerTypedActor(intfClass.getName, factory)
  ...
  /**
    ...
    * Java API
    */
  def registerPerSession(id: String, factory: Creator[ActorRef]): Unit =
    registerPerSession(id, factory.create)
