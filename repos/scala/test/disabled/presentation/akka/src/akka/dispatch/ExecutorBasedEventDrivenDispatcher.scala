...
class ExecutorBasedEventDrivenDispatcher(
  ...
  /**
    ...
    */
  protected def getMailbox(receiver: ActorRef) =
    receiver.mailbox.asInstanceOf[MessageQueue with ExecutableMailbox]
            ...
            nextMessage =
              if (self.suspended.locked) {
                ...
                if ((processedMessages >= dispatcher.throughput) ||
                    (isDeadlineEnabled &&
                    System.nanoTime >=
                      deadlineNs)) // If we're throttled, break out
