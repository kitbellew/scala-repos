...
/**
  ...
  */
trait ActorRef extends ActorRefShared with java.lang.Comparable[ActorRef] {
  ...
  /**
    ...
    * <p/>
    */
  def sendOneWay(message: AnyRef, sender: ActorRef): Unit =
  ...
  def sendRequestReply(message: AnyRef): AnyRef =
    sendRequestReply(message, timeout, null)
  ...
  /**
    ...
    */
  def sendRequestReply(message: AnyRef, sender: ActorRef): AnyRef =
    sendRequestReply(message, timeout, sender)
  ...
  /**
    ...
    */
  def sendRequestReplyFuture[T <: AnyRef](
      message: AnyRef,
      sender: ActorRef): Future[T] =
  ...
  /**
    ...
    */
  @deprecated(
    "Will be removed after 1.1, use Actor.remote.actorOf instead and then link on success",
    "1.1")
  ...
  protected[akka] def restart(
      ...
      withinTimeRange: Option[Int]): Unit

  protected[akka] def restartLinkedActors(
      reason: Throwable,
      maxNrOfRetries: Option[Int],
      withinTimeRange: Option[Int]): Unit
...
class LocalActorRef private[akka] (
  ...
  /**
    ...
    */
  private[akka] final def isClientManaged_? =
    clientManaged && homeAddress.isDefined && isRemotingEnabled
  ...
  @deprecated(
    ...
    "1.1")
  def actorClass: Class[_ <: Actor] =
    actor.getClass.asInstanceOf[Class[_ <: Actor]]
    ...
    if (!isBeingRestarted) {
      if (!isRunning) _dispatcher = md
        ...
        throw new ActorInitializationException(
          "Can not swap dispatcher for " +
            toString + " after it has been started")
    }
  }
  ...
  def spawnRemote(
      ...
      hostname: String,
      port: Int,
      timeout: Long = Actor.TIMEOUT): ActorRef = {
  ...
  protected[akka] def handleTrapExit(dead: ActorRef, reason: Throwable) {
    faultHandler match {
      case AllForOneStrategy(trapExit, maxRetries, within)
          if trapExit.exists(_.isAssignableFrom(reason.getClass)) =>
        restartLinkedActors(reason, maxRetries, within)

      case OneForOneStrategy(trapExit, maxRetries, within)
          if trapExit.exists(_.isAssignableFrom(reason.getClass)) =>
  ...
  private def requestRestartPermission(
      maxNrOfRetries: Option[Int],
      withinTimeRange: Option[Int]): Boolean = {
    val denied =
      if (maxNrOfRetries.isEmpty && withinTimeRange.isEmpty) {
        //Immortal
        false
  ...
  def linkedActors: JMap[Uuid, ActorRef] =
    java.util.Collections.unmodifiableMap(_linkedActors)
...
private[akka] case class RemoteActorRef private[akka] (
  ...
  def spawnRemote(
      ...
      hostname: String,
      port: Int,
      timeout: Long): ActorRef = unsupported
  ...
  protected[akka] def restart(
      reason: Throwable,
      maxNrOfRetries: Option[Int],
      withinTimeRange: Option[Int]): Unit = unsupported
  protected[akka] def restartLinkedActors(
      reason: Throwable,
      maxNrOfRetries: Option[Int],
      withinTimeRange: Option[Int]): Unit =
...
trait ScalaActorRef extends ActorRefShared { ref: ActorRef =>
  ...
  /**
    * Atomically create (from actor class), start and make an actor remote.
    */
  def spawnRemote[T <: Actor: ClassTag](
      hostname: String,
      port: Int,
      timeout: Long): ActorRef = {
    ensureRemotingEnabled
  ...
  /**
    * Atomically create (from actor class), start, link and make an actor remote.
    */
  def spawnLinkRemote[T <: Actor: ClassTag](
      hostname: String,
      port: Int,
      timeout: Long): ActorRef = {
    ensureRemotingEnabled
