...
object ScalaOptionParser {
  ...
  def scalaParser(entryPoint: String, globalBase: File): Parser[String] = {
    ...
    val UniversalOpt = Property | oneOf(
      pathSettingNames.map(PathSetting) ++ phaseSettings.map(
        PhaseSettingParser) ++ booleanSettingNames.map(
  ...
  private def booleanSettingNames = List(
    "-X",
    "-Xcheckinit",
    "-Xdev",
    "-Xdisable-assertions",
  ...
  private def stringSettingNames = List(
  ...
  private def pathSettingNames = List(
    "-bootclasspath",
    "-classpath",
    "-extdirs",
    "-javabootclasspath",
  ...
  private def multiStringSettingNames =
    List("-Xmacro-settings", "-Xplugin", "-Xplugin-disable", "-Xplugin-require")
  private def intSettingNames = List(
    "-Xmax-classfile-name",
    "-Xelide-below",
  ...
  private def scalaChoiceSettingNames = Map(
    "-howtorun" -> List("object", "script", "jar", "guess"))
  private def scalaStringSettingNames = List("-i", "-e")
  private def scalaBooleanSettingNames = List("-nc", "-save")

  private def scalaDocBooleanSettingNames = List(
    "-Yuse-stupid-types",
    "-implicits",
    "-implicits-debug",
    "-implicits-show-all",
  ...
  private def scalaDocIntSettingNames = List(
  ...
  private def scaladocStringSettingNames = List(
    "-doc-title",
    "-doc-version",
    "-doc-footer",
    "-doc-no-compile",
