...
object ScalaOptionParser {
  ...
  def scalaParser(entryPoint: String, globalBase: File): Parser[String] = {
    ...
    def ChoiceSetting(name: String, choices: List[String]): Parser[String] =
      concat(
        token(concat(name ~ ":")) ~
          token(StringBasic.examples(choices: _*)).map(_.mkString))
    ...
    val UniversalOpt = Property |
      oneOf(
        pathSettingNames
          .map(PathSetting) ++ phaseSettings.map(PhaseSettingParser) ++
          booleanSettingNames
            .map(BooleanSetting) ++ stringSettingNames.map(StringSetting) ++
          multiStringSettingNames
            .map(MultiStringSetting) ++ intSettingNames.map(IntSetting) ++
          choiceSettingNames.map {
            case (k, v) =>
              ChoiceSetting(k, v)
          } ++
    ...
    val ScalaDocExtraSettings = oneOf(
      scalaDocBooleanSettingNames
        .map(BooleanSetting) ++ scalaDocIntSettingNames.map(IntSetting) ++
  ...
  private def multiChoiceSettingNames =
    Map[String, List[String]](
      "-Xlint" ->
        List(
          "adapted-args",
