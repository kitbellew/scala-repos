...
class UniqueKeyJoinJob(args: Args) extends Job(args) {
  ...
  val input = TypedTsv[(Timestamp, (Int, Long))](
    "input0"
  ) // left side of the join
  val serv = TypedTsv[(Int, String)]("input1") // service to be looked up
  val versionedSource = VersionedKeyValSource[Int, String](
    "input2"
  ) // source to be looked up

  val uniqueKeyJoiner = UniqueKeyedService
    .fromAndThen[(Int, String), Int, String](
      dr => versionedSource,
...
class UniqueKeyedServiceSpec extends WordSpec {
  ...
  val service = List((1002, "b-1002"), (1001, "a-1001"))
