  ...
  property("readAfterLastBatch should return interval starting from the last batch written") = {
    forAll {
      (
        ...
        result match {
          case Right((
                (Intersection(InclusiveLower(readIntervalLower), ExclusiveUpper(_)), _),
                _)) => {
  ...
  property("the end of merged interval is never extended") = {
    forAll {
      (
        ...
        mergeResult.isRight ==> {
          val Right(
            ((Intersection(InclusiveLower(_), ExclusiveUpper(readIntervalUpper)), _), _)) =
            mergeResult
          ...
          val diskPipeFactory = StateWithError[
            ...
            FlowToPipe[(Int, Int)]] { (timeMode: (Interval[Timestamp], Mode)) =>
            {
              ...
              val flowToPipe: FlowToPipe[(Int, Int)] = Reader {
                (fdM: (FlowDef, Mode)) =>
                  TypedPipe.from[(Timestamp, (Int, Int))](Seq((Timestamp(10), (2, 3))))
              }
