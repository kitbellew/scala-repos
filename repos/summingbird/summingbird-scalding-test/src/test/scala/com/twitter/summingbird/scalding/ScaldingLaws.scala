...
class ScaldingLaws extends WordSpec {
  ...
  "The ScaldingPlatform" should {
    ...
    "match scala for single step jobs" in {
      ...
      val batchCoveredInput = TestUtil
        .pruneToBatchCovered(inWithTime, intr, batcher)
      ...
      val mode: Mode = TestMode(t =>
        (testStore.sourceToBuffer ++ buffer).get(t))
    ...
    "match scala for flatMapKeys jobs" in {
      ...
      val inMemory =
        TestGraphs.singleStepMapKeysInScala(batchCoveredInput)(fnA, fnB)
    ...
    "match scala for multiple summer jobs" in {
      ...
      val testStoreA = TestStore[Int, Int](
        "testA",
        batcher,
        initStoreA,
        inWithTime.size)
      val testStoreB = TestStore[Int, Int](
        "testB",
        batcher,
        initStoreB,
        inWithTime.size)
    ...
    "match scala for leftJoin jobs" in {
      ...
      val batcher = TestUtil.randomBatcher(inWithTime)
      val batchCoveredInput =
        TestUtil.pruneToBatchCovered(inWithTime, intr, batcher).toIterable
    ...
    "match scala for leftJoin  repeated tuple leftJoin jobs" in {
      ...
      val inMemory =
        TestGraphs.repeatedTupleLeftJoinInScala(timeIncIt)(srvWithTime)(
          prejoinMap)(postJoin)
    ...
    "match scala for leftJoin with store (no dependency between the two) jobs" in {
      ...
      // Add a time
      val inWithTime1 = original1
        .zipWithIndex
        .map {
          case (item, time) =>
            (time.toLong, item)
        }
      val inWithTime2 = original2
        .zipWithIndex
        .map {
          case (item, time) =>
            (time.toLong, item)
      ...
      val batchCoveredInput1 = TestUtil
        .pruneToBatchCoveredWithTime(inWithTime1, intr, batcher)
      val batchCoveredInput2 = TestUtil
        .pruneToBatchCoveredWithTime(inWithTime2, intr, batcher)
      ...
      val (inMemoryA, inMemoryB) =
        TestGraphs
          .leftJoinWithStoreInScala(batchCoveredInput1, batchCoveredInput2)(
            fnAWithTime)(fnBWithTime)(postJoinWithTime)
      ...
      val (buffer2, source2) = TestSource(
        inWithTime2,
        Some(DateRange(RichDate(0), RichDate(endTimeOfLastBatch2))))

      val summer =
        TestGraphs.leftJoinWithStoreJob[
          Scalding,
          (Long, Int),
          (Long, Int),
          Int,
          Int,
          Int,
          Int](source1, source2, storeAndService, finalStore)(tup =>
      ...
      val mode: Mode = TestMode(
        (
          storeAndService.sourceToBuffer ++ finalStore
            .sourceToBuffer ++ buffer1 ++ buffer2
        ).get(_))
    ...
    "match scala for leftJoin with store (with dependency between store and join) jobs" in {
      ...
      val batcher = TestUtil.randomBatcher(inWithTime)
      val batchCoveredInput = TestUtil
        .pruneToBatchCoveredWithTime(inWithTime, intr, batcher)

      val inMemoryStore =
        TestGraphs.leftJoinWithDependentStoreInScala(batchCoveredInput)(
          fnAWithTime)(valuesFlatMapWithTime)
      ...
      val ws = new LoopState(intr)
      val mode: Mode = TestMode(
        (storeAndService.sourceToBuffer ++ buffer).get(_))
    ...
    "match scala for leftJoin with store and join fanout (with dependency between store and join) jobs" in {
      ...
      val (inMemoryStoreAfterJoin, inMemoryStoreAfterFlatMap) =
        TestGraphs
          .leftJoinWithDependentStoreJoinFanoutInScala(batchCoveredInput)(
            fnAWithTime)(valuesFlatMapWithTime)(flatMapWithTime)
      ...
      val summer =
        TestGraphs.leftJoinWithDependentStoreJoinFanoutJob[
          Scalding,
          (Long, Int),
          Int,
          Int,
    ...
    "match scala for diamond jobs with write" in {
      ...
      val summer =
        TestGraphs.diamondJob[Scalding, (Long, Int), Int, Int](
          source,
          testSink,
      ...
      val ws = new LoopState(intr)
      val mode: Mode = TestMode(s =>
        (testStore.sourceToBuffer ++ buffer).get(s))
    ...
    "Correctly aggregate multiple sumByKeys" in {
      ...
      val (inMemoryA, inMemoryB) = TestGraphs
        .twoSumByKeyInScala(batchCoveredInput, keyExpand)
    ...
    "compute correct statistics" in {
      ...
      val summer =
        TestGraphs.jobWithStats[Scalding, (Long, Int), Int, Int](
          jobID,
          source,
          testStore)(t => fn(t._2))
      ...
      val mode: Mode = HadoopTest(
        conf,
        t => (testStore.sourceToBuffer ++ buffer).get(t))
      ...
      val origCounter: Long = flowStats
        .getCounterValue("counter.test", "orig_counter")
      val fmCounter: Long = flowStats
        .getCounterValue("counter.test", "fm_counter")
      val fltrCounter: Long = flowStats
        .getCounterValue("counter.test", "fltr_counter")
