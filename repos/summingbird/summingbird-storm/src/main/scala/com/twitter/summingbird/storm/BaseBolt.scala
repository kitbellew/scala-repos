...
case class BaseBolt[I, O](
  ...
  private[this] val rampPeriods = maxExecutePerSec.rampUptimeMS /
    PERIOD_LENGTH_MS
  private[this] val deltaPerPeriod: Long =
    if (rampPeriods > 0) (upperBound - lowerBound) / rampPeriods else 0

  private[this] lazy val startPeriod = System.currentTimeMillis /
    PERIOD_LENGTH_MS
  ...
  override def execute(tuple: Tuple) = {
    ...
    val curResults =
      if (!tuple.getSourceStreamId.equals("__tick")) {
        val tsIn =
          executor.decoder.invert(tuple.getValues)
