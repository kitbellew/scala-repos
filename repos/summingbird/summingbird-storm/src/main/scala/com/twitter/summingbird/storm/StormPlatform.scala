...
abstract class Storm(
  ...
  private def scheduleFlatMapper(
    ...
    val declarer =
      topologyBuilder.setBolt(nodeName, bolt, parallelism).addConfigurations(
        tickConfig)
  ...
  private def scheduleSummerBolt[K, V](
    ...
    val supplier: MergeableStoreFactory[ExecutorKeyType, V] =
      summer.store match {
        ...
        case _ =>
          sys.error(
            "Should never be able to get here, looking for a MergeableStoreFactory from %s".format(
              summer.store))
      }
  ...
  def plan[T](tail: TailProducer[Storm, T]): PlannedTopology = {
    ...
    val jobID = {
      ...
      if (stormJobId == null) {
        JobId(
          java.util.UUID.randomUUID().toString + "-AutoGenerated-ForInbuiltStats-UserStatsWillFail.")
      } else {
        JobId(stormJobId)
      }
    }
