...
abstract class Storm(
  ...
  private def scheduleFlatMapper(
    ...
    } else { dependenciesNames.foreach { declarer.shuffleGrouping(_) } }
  }
  ...
  private def scheduleSpout[K](
    ...
    val parallelism =
      getOrElse(stormDag, node, parOpt.getOrElse(DEFAULT_SOURCE_PARALLELISM))
        .parHint
  ...
  private def scheduleSummerBolt[K, V](
    ...
    val supplier: MergeableStoreFactory[ExecutorKeyType, V] = summer
      .store match {
    ...
    val parallelism = getOrElse(stormDag, node, DEFAULT_SUMMER_PARALLELISM)
      .parHint
    val declarer = topologyBuilder.setBolt(nodeName, sinkBolt, parallelism)
      .addConfigurations(tickConfig)
    val dependenciesNames = stormDag.dependenciesOf(node).collect {
      case x: StormNode => stormDag.getNodeName(x)
    }
  ...
  private def dumpOptions: String = {
    options.map {
      case (k, opts) => "%s -> [%s]".format(k, opts.opts.values.mkString(", "))
    }.mkString("\n || ")
  }
  ...
  def plan[T](tail: TailProducer[Storm, T]): PlannedTopology = {
    ...
    val stormTail = dagOptimizer
      .optimize(tail, dagOptimizer.ValueFlatMapToFlatMap)
      ...
      if (stormJobId == null) {
        JobId(
          java.util.UUID.randomUUID().toString +
            "-AutoGenerated-ForInbuiltStats-UserStatsWillFail.")
      } else { JobId(stormJobId) }
    }
