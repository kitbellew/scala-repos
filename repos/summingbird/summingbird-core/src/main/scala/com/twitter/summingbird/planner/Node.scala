...
sealed trait Node[P <: Platform[P]] {
  ...
  def getNameFallback: String =
    getClass.getName.replaceFirst("com.twitter.summingbird.storm.", "")
...
case class Dag[P <: Platform[P]](
  ...
  def connect(src: Node[P], dest: Node[P]): Dag[P] =
    if (src == dest) {
      ...
      val newSrcDependants =
        if (oldSrcDependants.contains(dest)) oldSrcDependants
        else (dest :: oldSrcDependants)
      val newDependantsOfM = dependantsOfM + (src -> newSrcDependants)

      val oldDestDependencies =
        dependenciesOfM.getOrElse(dest, List[Node[P]]())
      val newDestDependencies =
        if (oldDestDependencies.contains(src)) oldDestDependencies
  ...
  def dependantsOf(n: Node[P]): List[Node[P]] =
    dependantsOfM.get(n).getOrElse(List())
  def dependenciesOf(n: Node[P]): List[Node[P]] =
    dependenciesOfM.get(n).getOrElse(List())

  def dependantsOf(p: Producer[P, _]) = producerDependants.dependantsOf(p)
  def transitiveDependantsOf(p: Producer[P, _]) =
    producerDependants.transitiveDependantsOf(p)
