...
case class OptionalUnzip2[P1 <: Platform[P1], P2 <: Platform[P2]]() {
  private def cast[T](
      p: Any): (Option[Producer[P1, T]], Option[Producer[P2, T]]) =
  ...
  def apply[T](root: Producer[OptionalPlatform2[P1, P2], T])
    ...
    root match {
      case AlsoProducer(ensure, result) =>
        ...
        val alsor =
          for (e <- re; r <- rr)
            yield e.asInstanceOf[TailProducer[P2, Any]].also(r)
        (alsol, alsor)
      ...
      case WrittenProducer(producer, sink) =>
        ...
        val sinkl =
          for (li <- l; leftSinki <- leftSink) yield li.write(leftSinki)
        val sinkr =
          for (ri <- r; rightSinki <- rightSink) yield ri.write(rightSinki)
        (sinkl, sinkr)
