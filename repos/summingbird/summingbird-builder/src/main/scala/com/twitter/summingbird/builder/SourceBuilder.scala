...
case class SourceBuilder[T: Manifest] private (
  ...
  def leftJoin[K, V, JoinedValue](
      ...
      keyMf: Manifest[K],
      valMf: Manifest[V],
      joinedMf: Manifest[JoinedValue]): SourceBuilder[(K, (V, Option[JoinedValue]))] =
    copy(
      node = node
    ...
    val cb = env match {
      case scalding: ScaldingEnv =>
        ...
        val newNode = OptionalUnzip2[Scalding, Storm]()(node)._1
          ...
          .getOrElse(
            sys.error("Scalding mode specified alongside some online-only Source, Service or Sink."))
        CompletedBuilder(
          newNode,
          registrar,
          batcher,
      ...
      case storm: StormEnv =>
        ...
        val newNode = OptionalUnzip2[Scalding, Storm]()(node)._2
          ...
          .getOrElse(
            sys.error("Storm mode specified alongside some offline-only Source, Service or Sink."))
        CompletedBuilder(
          newNode,
          registrar,
          batcher,
