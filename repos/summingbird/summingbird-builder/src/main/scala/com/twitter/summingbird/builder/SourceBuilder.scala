...
import com.twitter.summingbird.storm.{Storm, StormEnv, StormSource, StormSink}
...
object SourceBuilder {
  ...
  def apply[T](eventSource: EventSource[T], timeOf: T => Date)(implicit
    ...
    val scaldingSource = eventSource.offline
...
case class SourceBuilder[T: Manifest] private (
  ...
  def write[U](sink: CompoundSink[U])(
    ...
    val newNode = node.flatMap(conversion).write(
  ...
  def leftJoin[K, V, JoinedValue](
    ...
    copy(node = node.asInstanceOf[Node[(K, V)]].leftJoin((
      service.offline,
  ...
  def set(opt: Any): SourceBuilder[T] =
    copy(opts = adjust(opts, id)(_.set(opt)))
  ...
  def groupAndSumTo[K, V](store: CompoundStore[K, V])(implicit
    ...
    val cb = env match {
      ...
      case storm: StormEnv =>
        val supplier = store.onlineSupplier
