...
class TestJob1(env: Env) extends AbstractJob(env) {
  ...
  try {
    EventSource[Long](Some(null), None).withTime(new java.util.Date(_)).map {
      e =>
        (e % 2, e)
...
class OptionsTest extends WordSpec {
  "Commutative Options should not be lost" in {
    ...
    assert(
      scalding.build.platform.jobName ==
        "com.twitter.summingbird.builder.TestJob1")
    ...
    assert(
      cfg.get("summingbird.options") ==
        Some(scalding.build.platform.options.toString))
