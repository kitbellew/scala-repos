...
object Scalding {
  ...
  def intersect(dr1: DateRange, dr2: DateRange): Option[DateRange] =
    (dr1.as[Interval[Timestamp]] && (dr2.as[Interval[Timestamp]])).as[Option[
      DateRange]]
  ...
  private[scalding] def joinFP[T, U](
      left: FlowToPipe[T],
      right: FlowToPipe[U]): FlowProducer[(TimedPipe[T], TimedPipe[U])] =
    for {
      t <- left
      u <- right
    } yield ((t, u))
        ...
        val (pf, m) = producer match {
          ...
          case ljp @ LeftJoinedProducer(left, StoreService(store))
            ...
            def go[K, U, V](
              ...
              val Summer(storeLog, _, sg) = InternalService.getSummer[K, V](
                dependants,
                bstore)
                .getOrElse(sys.error(
          ...
          case ljp @ LeftJoinedProducer(left, StoreService(store))
            ...
            def go[K, V, U](
              ...
              val Summer(storeLog, _, sg) = InternalService.getSummer[K, U](
                dependants,
                bs)
                .getOrElse(sys.error(
                  "join %s is against store not in the entire job's Dag".format(
...
class Scalding(
  ...
  def run(
    ...
    run(state, mode, pf, (f: Flow[_]) => Unit)
