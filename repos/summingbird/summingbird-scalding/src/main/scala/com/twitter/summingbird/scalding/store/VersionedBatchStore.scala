...
abstract class VersionedBatchStoreBase[K, V](val rootPath: String)
  ...
  /**
    ...
    */
  def batchIDToVersion(b: BatchID): Long =
    batcher.earliestTimeOf(b.next).milliSinceEpoch
  def versionToBatchID(ver: Long): BatchID =
    batcher.batchOf(Timestamp(ver)).prev
...
class VersionedBatchStore[K, V, K2, V2](
  ...
  /**
    ...
    */
  override def select(b: List[BatchID]): List[BatchID] =
    b.takeRight(versionsToKeep)
