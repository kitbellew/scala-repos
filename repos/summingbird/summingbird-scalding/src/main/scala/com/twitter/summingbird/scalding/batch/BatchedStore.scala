...
trait BatchedStore[K, V] extends scalding.Store[K, V] { self =>
  ...
  /**
    ...
    */
  override def partialMerge[K1](
      delta: PipeFactory[(K1, V)],
      sg: Semigroup[V],
  ...
  private def mergeBatched(
    ...
    def prepareDeltas(
        ins: TypedPipe[(Timestamp, (K, V))]): TypedPipe[(K, (BatchID, (Timestamp, V)))] =
    ...
    /**
      ...
      */
    def flatOpt[T](optopt: Option[Option[T]]): Option[T] =
  ...
  /**
    ...
    */
  final def readAfterLastBatch[T](input: PipeFactory[T])
      : PlannerOutput[(BatchID, FlowProducer[TypedPipe[(K, V)]], FlowToPipe[T])] = {
    ...
    for {
      ...
      deltaTimes: Interval[Timestamp] =
        setLower(InclusiveLower(firstDeltaTimestamp), timeSpan)
