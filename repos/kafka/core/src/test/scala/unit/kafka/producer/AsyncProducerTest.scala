...
class AsyncProducerTest {
  ...
  class NegativePartitioner(props: VerifiableProperties = null)
      extends Partitioner {
  ...
  def testPartitionAndCollateEvents() {
    ...
    val partition1Metadata =
      new PartitionMetadata(0, Some(broker1), List(broker1, broker2))
    val partition2Metadata =
      new PartitionMetadata(1, Some(broker2), List(broker1, broker2))
    val topic1Metadata =
      new TopicMetadata("topic1", List(partition1Metadata, partition2Metadata))
    val topic2Metadata =
      new TopicMetadata("topic2", List(partition1Metadata, partition2Metadata))
    ...
    val topic1Broker2Data = ArrayBuffer[KeyedMessage[Int, Message]](
      new KeyedMessage[Int, Message](
        ...
        -101,
        3,
        new Message("msg4".getBytes)))
    val topic2Broker1Data = ArrayBuffer[KeyedMessage[Int, Message]](
      new KeyedMessage[Int, Message]("topic2", 4, new Message("msg5".getBytes)))
    val topic2Broker2Data = ArrayBuffer[KeyedMessage[Int, Message]](
      new KeyedMessage[Int, Message](
        "topic2",
        -99,
        1,
  ...
  def testRandomPartitioner() {
    ...
    producerDataList.append(
      new KeyedMessage[String, Message]("topic2", new Message("msg2".getBytes)))
    producerDataList.append(
      new KeyedMessage[String, Message]("topic1", new Message("msg3".getBytes)))
  ...
  def testFailedSendRetryLogic() {
    ...
    val request1 = TestUtils.produceRequestWithAcks(
      List(topic1),
      List(0, 1),
      messagesToSet(msgs),
      acks = 1,
      correlationId = 11,
      timeout = DefaultAckTimeoutMs,
      clientId = DefaultClientId)
    val request2 = TestUtils.produceRequestWithAcks(
      List(topic1),
      List(0, 1),
      messagesToSet(msgs),
      acks = 1,
    ...
    val request3 = TestUtils.produceRequest(
      topic1,
      0,
      messagesToSet(msgs),
      acks = 1,
    ...
    val handler = new DefaultEventHandler[Int, String](
      ...
      topicPartitionInfos = topicPartitionInfos,
      time = time)
    val data = msgs.map(m => new KeyedMessage[Int, String](topic1, 0, m)) ++ msgs
  ...
  def testJavaProducer() {
    ...
    val javaProducer =
      new kafka.javaapi.producer.Producer[String, String](mockScalaProducer)
