...
class HighwatermarkPersistenceTest {
  ...
  def testHighWatermarkPersistenceSinglePartition() {
    ...
    try {
      ...
      val log0 =
        logManagers(0).createLog(TopicAndPartition(topic, 0), LogConfig())
      val leaderReplicaPartition0 =
        new Replica(configs.head.brokerId, partition0, SystemTime, 0, Some(log0))
  ...
  def testHighWatermarkPersistenceMultiplePartitions() {
    ...
    try {
      ...
      // create leader log
      val topic1Log0 =
        logManagers(0).createLog(TopicAndPartition(topic1, 0), LogConfig())
      ...
      // create leader log
      val topic2Log0 =
        logManagers(0).createLog(TopicAndPartition(topic2, 0), LogConfig())
