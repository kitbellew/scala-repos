...
class SimpleAclAuthorizerTest extends ZooKeeperTestHarness {
  ...
  def testTopicAcl() {
    ...
    assertTrue(
      "User1 should have READ access from host2",
    ...
    assertTrue(
      "User2 should have READ access from host1",
      simpleAclAuthorizer.authorize(user2Session, Read, resource))
    assertTrue(
      "User3 should have WRITE access from host2",
  ...
  def testWildCardAcls(): Unit = {
    ...
    val host1Session = new Session(user1, host1)
    assertTrue(
      "User1 should have Read access from host1",
      simpleAclAuthorizer.authorize(host1Session, Read, resource))
  ...
  def testNoAclFoundOverride() {
    ...
    assertTrue(
      "when acls = null or [],  authorizer should fail open with allow.everyone = true.",
  ...
  def testAclManagementAPIs() {
    ...
    TestUtils.waitUntilTrue(
      () =>
        Map(resource -> Set(acl1, acl2)) == simpleAclAuthorizer.getAcls(user1),
    ...
    TestUtils.waitUntilTrue(
      () =>
        resourceToAcls + (resource -> acls) == simpleAclAuthorizer.getAcls(),
      "changes not propagated in timeout period.")
    ...
    assertTrue(
      !zkUtils.pathExists(simpleAclAuthorizer.toResourcePath(resource)))
