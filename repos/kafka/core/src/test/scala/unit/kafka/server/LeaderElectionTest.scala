...
class LeaderElectionTest extends ZooKeeperTestHarness {
  ...
  def testLeaderElectionAndEpoch {
    ...
    assertTrue(
      "Leader could be broker 0 or broker 1",
    ...
    val leader2 = waitUntilLeaderIsElectedOrChanged(
      zkUtils,
      topic,
      partitionId,
      oldLeaderOpt =
    ...
    if (leader1.get == leader2.get)
      assertEquals(
        "Second epoch value should be " + leaderEpoch1 + 1,
        leaderEpoch1 + 1,
        leaderEpoch2)
    else
      assertEquals(
        "Second epoch value should be %d".format(leaderEpoch1 + 1),
        leaderEpoch1 + 1,
        leaderEpoch2)
    ...
    val leader3 = waitUntilLeaderIsElectedOrChanged(
      zkUtils,
      topic,
      partitionId,
      oldLeaderOpt =
    ...
    if (leader2.get == leader3.get)
      assertEquals(
        "Second epoch value should be " + leaderEpoch2,
        leaderEpoch2,
        leaderEpoch3)
    else
      assertEquals(
        "Second epoch value should be %d".format(leaderEpoch2 + 1),
        leaderEpoch2 + 1,
        leaderEpoch3)
  }
  ...
  def testLeaderElectionWithStaleControllerEpoch() {
    ...
    val controllerConfig = KafkaConfig
      .fromProps(TestUtils.createBrokerConfig(controllerId, zkConnect))
    val brokers = servers
      .map(s => new Broker(s.config.brokerId, "localhost", s.boundPort()))
    ...
    try {
      ...
      val partitionStates = Map(
        new TopicPartition(topic, partitionId) -> new PartitionState(
          2,
          brokerId2,
      ...
      assertTrue(
        "Stale controller epoch not detected by the broker",
  ...
  private def staleControllerEpochCallback(
    ...
    staleControllerEpochDetected = Errors
      .forCode(leaderAndIsrResponse.errorCode) match {
