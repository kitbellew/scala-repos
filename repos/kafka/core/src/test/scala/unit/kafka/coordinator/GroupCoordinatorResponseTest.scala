...
class GroupCoordinatorResponseTest extends JUnitSuite {
  ...
  def setUp() {
    ...
    EasyMock.expect(zkUtils.getPartitionAssignmentForTopics(Seq(
      TopicConstants.GROUP_METADATA_TOPIC_NAME))).andReturn(ret)
  ...
  def testHeartbeatUnknownConsumerExistingGroup() {
    ...
    val syncGroupResult = syncGroupLeader(
      ...
      assignedMemberId,
      Map(assignedMemberId -> Array[Byte]()))
  ...
  def testValidHeartbeat() {
    ...
    val syncGroupResult = syncGroupLeader(
      ...
      generationId,
      assignedConsumerId,
      Map(assignedConsumerId -> Array[Byte]()))
    val syncGroupErrorCode = syncGroupResult._2
    assertEquals(Errors.NONE.code, syncGroupErrorCode)
  ...
  def testJoinGroupFromUnchangedFollowerDoesNotRebalance() {
    ...
    EasyMock.reset(replicaManager)
    val firstSyncResult = syncGroupLeader(
      groupId,
      firstGenerationId,
      firstMemberId,
    ...
    EasyMock.reset(replicaManager)
    val otherJoinFuture = sendJoinGroup(
      groupId,
      JoinGroupRequest.UNKNOWN_MEMBER_ID,
      DefaultSessionTimeout,
      protocolType,
    ...
    EasyMock.reset(replicaManager)
    val followerJoinResult = joinGroup(
      groupId,
  ...
  def testDescribeGroupRebalancing() {
    ...
    assertTrue(
      summary.members.map(_.memberId).contains(joinGroupResult.memberId))
  ...
  private def sendSyncGroupLeader(
      groupId: String,
      generation: Int,
      leaderId: String,
    ...
    EasyMock.expect(replicaManager.appendMessages(
      EasyMock.anyLong(),
      EasyMock.anyShort(),
      EasyMock.anyBoolean(),
      ...
      override def answer =
        capturedArgument.getValue.apply(Map(
          new TopicPartition(
    ...
    EasyMock
      ...
      .andReturn(Some(Message.MagicValue_V1)).anyTimes()
    EasyMock.replay(replicaManager)
  ...
  private def sendSyncGroupFollower(
      groupId: String,
      generation: Int,
  ...
  private def leaveGroup(
    ...
    EasyMock.expect(replicaManager.getPartition(
      TopicConstants.GROUP_METADATA_TOPIC_NAME,
