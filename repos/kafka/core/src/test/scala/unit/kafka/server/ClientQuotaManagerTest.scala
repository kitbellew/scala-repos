...
class ClientQuotaManagerTest {
  ...
  def testQuotaParsing() {
    val clientMetrics =
      new ClientQuotaManager(config, newMetrics, "producer", time)
    ...
    try {
      ...
      throttleTimeMs = clientMetrics
        .recordAndMaybeThrottle("p1", 0, this.callback)
      assertEquals(
        s"throttleTimeMs should be 0. was $throttleTimeMs",
        0,
        throttleTimeMs)
  ...
  def testQuotaViolation() {
    ...
    val clientMetrics =
      new ClientQuotaManager(config, metrics, "producer", time)
    val queueSizeMetric = metrics.metrics()
      .get(metrics.metricName("queue-size", "producer", ""))
    try {
      ...
      time.sleep(500)
      val sleepTime = clientMetrics
        .recordAndMaybeThrottle("unknown", 2300, callback)
  ...
  def testExpireThrottleTimeSensor() {
    ...
    try {
      ...
      val throttleTime = clientMetrics
        .recordAndMaybeThrottle("client1", 10000, callback)
