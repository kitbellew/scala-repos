...
class ByteBufferMessageSetTest extends BaseMessageSetTestCases {
  ...
  def testValidBytes() {
    {
      ...
      assertEquals(
        "Adding invalid bytes shouldn't change byte count",
        messages.validBytes,
  ...
  def testLogAppendTime() {
    ...
    val (validatedMessages, _) = messages.validateMessagesAndAssignOffsets(
    ...
    val (validatedCompressedMessages, _) = compressedMessagesWithRecompresion
  ...
  def testCreateTime() {
    ...
    val (validatedCompressedMessages, _) = compressedMessages
      .validateMessagesAndAssignOffsets(
        offsetCounter = new LongRef(0),
        now = System.currentTimeMillis(),
        sourceCodec = DefaultCompressionCodec,
    ...
    for (messageAndOffset <- validatedMessages) {
      ...
      assertEquals(
        messageAndOffset.message.timestampType,
  ...
  def testRelativeOffsetAssignment() {
    ...
    checkOffsets(messages, 0)
    val offset = 1234567
    val (messageWithOffset, _) = messages.validateMessagesAndAssignOffsets(
      offsetCounter = new LongRef(offset),
      now = System.currentTimeMillis(),
      sourceCodec = NoCompressionCodec,
      targetCodec = NoCompressionCodec,
    ...
    // check compressed messages
    checkOffsets(compressedMessages, 0)
    val (compressedMessagesWithOffset, _) = compressedMessages
      .validateMessagesAndAssignOffsets(
        offsetCounter = new LongRef(offset),
        now = System.currentTimeMillis(),
        sourceCodec = DefaultCompressionCodec,
  ...
  def checkOffsets(messages: ByteBufferMessageSet, baseOffset: Long) {
    ...
    for (entry <- messages) {
      assertEquals(
        "Unexpected offset in message set iterator",
        offset,
        entry.offset)
      offset += 1
    }
