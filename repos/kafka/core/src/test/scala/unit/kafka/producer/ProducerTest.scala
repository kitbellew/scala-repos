...
class ProducerTest extends ZooKeeperTestHarness with Logging {
  ...
  def testUpdateBrokerPartitionInfo() {
    ...
    try producer2.send(new KeyedMessage[String, String](topic, "test", "test1"))
    catch {
    ...
    try producer3.send(new KeyedMessage[String, String](topic, "test", "test1"))
    catch {
      case e: Throwable => fail("Should succeed sending the message", e)
  ...
  def testSendToNewTopic() {
    ...
    // Message 1
    assertTrue(
      ByteBuffer.wrap("test1".getBytes).equals(messageSet(0).message.payload))
    assertTrue(
    ...
    // Message 2
    assertTrue(
      ByteBuffer.wrap("test2".getBytes).equals(messageSet(1).message.payload))
    assertTrue(
