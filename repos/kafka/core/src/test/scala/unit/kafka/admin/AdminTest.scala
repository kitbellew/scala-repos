...
class AdminTest extends ZooKeeperTestHarness with Logging with RackAwareTest {
  ...
  def testPreferredReplicaJsonData() {
    ...
    val partitionsForPreferredReplicaElection = Set(
      TopicAndPartition("test", 1),
      TopicAndPartition("test2", 1))
  ...
  def testShutdownBroker() {
    ...
    try {
      ...
      partitionStateInfo = activeServers
        .head
        .apis
        .metadataCache
        .getPartitionInfo(topic, partition)
        .get
      leaderAfterShutdown = partitionStateInfo
        .leaderIsrAndControllerEpoch
        .leaderAndIsr
        .leader
      assertEquals(0, leaderAfterShutdown)
  ...
  def testTopicConfigChange() {
    ...
    try {
      ...
      val newConfig: Properties = makeConfig(
        2 * maxMessageSize,
        2 * retentionMs)
