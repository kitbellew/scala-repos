...
class AdminTest extends ZooKeeperTestHarness with Logging with RackAwareTest {
  ...
  def testTopicCreationInZK() {
    ...
    val actualReplicaList = leaderForPartitionMap.keys.toArray.map(p =>
      (p -> zkUtils.getReplicasForPartition(topic, p))).toMap
  ...
  def testPartitionReassignmentWithLeaderInNewReplicas() {
    ...
    val topic = "test"
    // create brokers
    val servers = TestUtils.createBrokerConfigs(4, zkConnect, false).map(b =>
    ...
    assertTrue(
      "Partition reassignment attempt failed for [test, 0]",
    ...
    assertEquals(
      ...
      newReplicas,
      assignedReplicas)
    ensureNoUnderReplicatedPartitions(
      zkUtils,
      topic,
      partitionToBeReassigned,
      assignedReplicas,
      servers)
    TestUtils.waitUntilTrue(
      () => getBrokersWithPartitionDir(servers, topic, 0) == newReplicas.toSet,
  ...
  def testPartitionReassignmentWithLeaderNotInNewReplicas() {
    ...
    val reassignPartitionsCommand = new ReassignPartitionsCommand(
      ...
      Map(topicAndPartition -> newReplicas))
    assertTrue(
      "Partition reassignment failed for test, 0",
      reassignPartitionsCommand.reassignPartitions())
    // wait until reassignment is completed
    TestUtils.waitUntilTrue(
      () => {
  ...
  def testReassigningNonExistingPartition() {
    ...
    assertFalse(
      "Partition should not be reassigned",
  ...
  def testPreferredReplicaJsonData() {
    ...
    val partitionsForPreferredReplicaElection =
      Set(TopicAndPartition("test", 1), TopicAndPartition("test2", 1))
  ...
  def testBasicPreferredReplicaElection() {
    ...
    val serverConfigs = TestUtils.createBrokerConfigs(
      3,
      zkConnect,
      false,
      rackInfo = brokerRack).map(KafkaConfig.fromProps)
    // create the topic
    AdminUtils.createOrUpdateTopicPartitionAssignmentPathInZK(
      zkUtils,
      topic,
  ...
  def testShutdownBroker() {
    ...
    val partition = 1
    // create brokers
    val serverConfigs = TestUtils.createBrokerConfigs(3, zkConnect, false).map(
    ...
    try {
      ...
      TestUtils.waitUntilTrue(
        () =>
          activeServers.forall(_.apis.metadataCache.getPartitionInfo(
            topic,
            partition).get.leaderIsrAndControllerEpoch.leaderAndIsr.isr.size != 3),
        "Topic test not created after timeout"
      )
      assertEquals(0, partitionsRemaining.size)
      var partitionStateInfo =
        activeServers.head.apis.metadataCache.getPartitionInfo(
          topic,
          partition).get
      ...
      assertTrue(
        servers.forall(_.apis.metadataCache.getPartitionInfo(
          topic,
          partition).get.leaderIsrAndControllerEpoch.leaderAndIsr.leader == 0))
  ...
  def testTopicConfigChange() {
    val partitions = 3
    val topic = "my-topic"
    val server = TestUtils.createServer(
    ...
    try {
      ...
      AdminUtils.createTopic(
        ...
        partitions,
        1,
        makeConfig(maxMessageSize, retentionMs))
      checkConfig(maxMessageSize, retentionMs)

      // now double the config values for the topic and check that it is applied
      val newConfig: Properties =
        makeConfig(2 * maxMessageSize, 2 * retentionMs)
      AdminUtils.changeTopicConfig(
        server.zkUtils,
        topic,
        makeConfig(2 * maxMessageSize, 2 * retentionMs))
      ...
      val configInZk =
        AdminUtils.fetchEntityConfig(server.zkUtils, ConfigType.Topic, topic)
  ...
  def testBootstrapClientIdConfig() {
    ...
    try {
      assertEquals(
        new Quota(1000, true),
        server.apis.quotaManagers(ApiKeys.PRODUCE.id).quota(clientId))
      assertEquals(
        new Quota(2000, true),
        server.apis.quotaManagers(ApiKeys.FETCH.id).quota(clientId))
    } finally {
      server.shutdown()
      server.config.logDirs.foreach(CoreUtils.rm(_))
    }
  ...
  def testGetBrokerMetadatas() {
    ...
    val rackInfo =
      Map(0 -> "rack1", 1 -> "rack2", 2 -> "rack2", 3 -> "rack1", 5 -> "rack3")
    val brokerMetadatas = toBrokerMetadata(
      rackInfo,
    ...
    val processedMetadatas1 =
      AdminUtils.getBrokerMetadatas(zkUtils, RackAwareMode.Disabled)
    ...
    val processedMetadatas2 =
      AdminUtils.getBrokerMetadatas(zkUtils, RackAwareMode.Safe)
    ...
    AdminUtils.createTopic(
      ...
      numPartitions,
      2,
      rackAwareMode = RackAwareMode.Safe)
