...
class OffsetCommitTest extends ZooKeeperTestHarness {
  ...
  def testUpdateOffsets() {
    ...
    createTopic(
      zkUtils,
      topic,
      partitionReplicaAssignment = expectedReplicaAssignment,
      servers = Seq(server))
    ...
    val commitRequest1 = OffsetCommitRequest(
      ...
      immutable.Map(
        topicAndPartition ->
          OffsetAndMetadata(offset = 100L, metadata = "some metadata")))
    ...
    assertEquals(
      Errors.NONE.code,
      fetchResponse1.requestInfo.get(topicAndPartition).get.error)
    assertEquals(
      "some metadata",
  ...
  def testCommitAndFetchOffsets() {
    ...
    val commitRequest = OffsetCommitRequest(
      ...
      immutable.Map(
        TopicAndPartition(topic1, 0) ->
          OffsetAndMetadata(offset = 42L, metadata = "metadata one"),
        TopicAndPartition(topic2, 0) ->
          OffsetAndMetadata(offset = 43L, metadata = "metadata two"),
        TopicAndPartition(topic3, 0) ->
          OffsetAndMetadata(offset = 44L, metadata = "metadata three"),
    ...
    assertEquals(
      Errors.NONE.code,
      commitResponse.commitStatus.get(TopicAndPartition(topic3, 0)).get)
    assertEquals(
      Errors.NONE.code,
      commitResponse.commitStatus.get(TopicAndPartition(topic2, 1)).get)
    ...
    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic1, 0)).get.error)

    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic2, 0)).get.error)
    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic2, 1)).get.error)

    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 0)).get.error)
    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 1)).get.error)
    assertEquals(
      OffsetMetadataAndError.NoOffset,
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 1)).get)

    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic4, 0)).get.error)
    assertEquals(
      OffsetMetadataAndError.NoOffset,
      fetchResponse.requestInfo.get(TopicAndPartition(topic4, 0)).get)

    assertEquals(
      Errors.NONE.code,
      fetchResponse.requestInfo.get(TopicAndPartition(topic5, 0)).get.error)
    assertEquals(
      OffsetMetadataAndError.NoOffset,
      fetchResponse.requestInfo.get(TopicAndPartition(topic5, 0)).get)

    assertEquals(
      "metadata one",
      fetchResponse.requestInfo.get(TopicAndPartition(topic1, 0)).get.metadata)
    assertEquals(
      "metadata two",
      fetchResponse.requestInfo.get(TopicAndPartition(topic2, 0)).get.metadata)
    assertEquals(
      "metadata three",
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 0)).get.metadata)

    assertEquals(
      OffsetMetadata.NoMetadata,
      fetchResponse.requestInfo.get(TopicAndPartition(topic2, 1)).get.metadata)
    assertEquals(
      OffsetMetadata.NoMetadata,
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 1)).get.metadata)
    assertEquals(
      OffsetMetadata.NoMetadata,
      fetchResponse.requestInfo.get(TopicAndPartition(topic4, 0)).get.metadata)
    assertEquals(
      OffsetMetadata.NoMetadata,
      fetchResponse.requestInfo.get(TopicAndPartition(topic5, 0)).get.metadata)

    assertEquals(
      42L,
      fetchResponse.requestInfo.get(TopicAndPartition(topic1, 0)).get.offset)
    assertEquals(
      43L,
      fetchResponse.requestInfo.get(TopicAndPartition(topic2, 0)).get.offset)
    assertEquals(
      44L,
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 0)).get.offset)
    assertEquals(
      45L,
      fetchResponse.requestInfo.get(TopicAndPartition(topic2, 1)).get.offset)

    assertEquals(
      OffsetMetadata.InvalidOffset,
      fetchResponse.requestInfo.get(TopicAndPartition(topic3, 1)).get.offset)
    assertEquals(
      OffsetMetadata.InvalidOffset,
      fetchResponse.requestInfo.get(TopicAndPartition(topic4, 0)).get.offset)
    assertEquals(
      OffsetMetadata.InvalidOffset,
  ...
  def testOffsetExpiration() {
    ...
    assertEquals(
      Errors.NONE.code,
      simpleConsumer.commitOffsets(commitRequest0).commitStatus
        .get(topicPartition).get)
    assertEquals(
      -1L,
      simpleConsumer.fetchOffsets(fetchRequest).requestInfo.get(topicPartition)
        .get.offset)

    // committed offset should exist with fetch version 0
    assertEquals(
      1L,
      simpleConsumer.fetchOffsets(OffsetFetchRequest(
        group,
        Seq(TopicAndPartition(topic, 0)),
    ...
    assertEquals(
      Errors.NONE.code,
      simpleConsumer.commitOffsets(commitRequest1).commitStatus
        .get(topicPartition).get)
    ...
    assertEquals(
      Errors.NONE.code,
      simpleConsumer.commitOffsets(commitRequest2).commitStatus
        .get(topicPartition).get)
    Thread.sleep(retentionCheckInterval * 2)
    assertEquals(
      -1L,
    ...
    assertEquals(
      Errors.NONE.code,
      simpleConsumer.commitOffsets(commitRequest3).commitStatus
        .get(topicPartition).get)
    Thread.sleep(retentionCheckInterval * 2)
    assertEquals(
      4L,
    ...
    val commitRequest4 = OffsetCommitRequest(
      groupId = "test-group",
      requestInfo = immutable.Map(
        TopicAndPartition(topic, 0) -> OffsetAndMetadata(5L, "metadata", -1L)),
      versionId = 2,
      retentionMs = 0L)
    assertEquals(
      Errors.NONE.code,
      simpleConsumer.commitOffsets(commitRequest4).commitStatus
        .get(topicPartition).get)
    Thread.sleep(retentionCheckInterval * 2)
    assertEquals(
      -1L,
