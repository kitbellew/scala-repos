...
abstract class BaseTopicMetadataTest extends ZooKeeperTestHarness {
  ...
  def testIsrAfterBrokerShutDownAndJoinsBack {
    ...
    // start adHoc brokers
    val adHocServers = adHocConfigs.take(numBrokers - 1)
  ...
  private def checkMetadata(
      ...
      expectedBrokersCount: Int): Unit = {
    var topicMetadata: TopicMetadataResponse =
      new TopicMetadataResponse(Seq(), Seq(), -1)
  ...
  @Test
  def testAliveBrokerListWithNoTopics { checkMetadata(Seq(server1), 1) }

  @Test
  def testAliveBrokersListWithNoTopicsAfterNewBrokerStartup {
    var adHocServers = adHocConfigs.takeRight(adHocConfigs.size - 1)
      .map(p => createServer(p))
