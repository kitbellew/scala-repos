...
class DeleteConsumerGroupTest extends KafkaServerTestHarness {
  ...
  def testTopicWideDeleteInZK() {
    ...
    groupTopicDirsForTopicToDelete.foreach(dir =>
      fillInConsumerGroupInfo(topicToDelete, dir.group, "consumer", 0, 10, false))
    ...
    TestUtils.waitUntilTrue(
      () =>
        !groupTopicDirsForTopicToDelete.exists(groupTopicOffsetAndOwnerDirsExist),
  ...
  def testConsumptionOnRecreatedTopicAfterTopicWideDeleteInZK() {
    ...
    val messageStream =
      consumerConnector.createMessageStreams(Map(topic -> 1))(topic).head
