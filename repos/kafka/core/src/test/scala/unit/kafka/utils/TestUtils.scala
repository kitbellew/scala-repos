...
object TestUtils extends Logging {
  ...
  def randomPartitionLogDir(parentDir: File): File = {
    ...
    val f = Iterator
      ...
      .take(attempts).find(_.mkdir()).getOrElse(sys.error(
  ...
  def createServer(
      config: KafkaConfig,
      time: Time = SystemTime): KafkaServer = {
  ...
  def getBrokerListStrFromServers(
    ...
    servers.map(s => formatAddress(s.config.hostName, s.boundPort(protocol)))
      .mkString(",")
  }
  ...
  def createBrokerConfig(
      ...
      rack: Option[String] = None): Properties = {

    def shouldEnable(protocol: SecurityProtocol) =
      interBrokerSecurityProtocol.fold(false)(_ == protocol)
    ...
    val listeners = protocolAndPorts.map {
      case (protocol, port) => s"${protocol.name}://localhost:$port"
    }.mkString(",")
  ...
  def singleMessageSet(
    ...
    new ByteBufferMessageSet(
      ...
      messages = new Message(payload, key, Message.NoTimestamp, magicValue))
  ...
  def randomString(len: Int): String = {
    ...
    for (i <- 0 until len)
      b.append(
        LettersAndDigits.charAt(seededRandom.nextInt(LettersAndDigits.length)))
    b.toString
  }
  ...
  def hexString(buffer: ByteBuffer): String = {
    ...
    for (i <- 0 until buffer.limit)
      builder.append(
        String.format("%x", Integer.valueOf(buffer.get(buffer.position + i))))
    builder.toString
  }
  ...
  def createNewProducer[K, V](
    ...
    if (!producerProps
      ...
      producerProps
        .putAll(producerSecurityConfigs(securityProtocol, trustStoreFile))
  ...
  def createNewConsumer(
    ...
    if (!consumerProps
          .containsKey(CommonClientConfigs.SECURITY_PROTOCOL_CONFIG))
      consumerProps
        .putAll(consumerSecurityConfigs(securityProtocol, trustStoreFile))
  ...
  def createBrokersInZk(
      brokerMetadatas: Seq[kafka.admin.BrokerMetadata],
  ...
  def deleteBrokersInZk(zkUtils: ZkUtils, ids: Seq[Int]): Seq[Broker] = {
    val brokers = ids
      .map(id => new Broker(id, "localhost", 6667, SecurityProtocol.PLAINTEXT))
  ...
  def produceRequestWithAcks(
    ...
    val data = topics.flatMap(topic =>
      partitions
        .map(partition => (TopicAndPartition(topic, partition), message)))
    new ProducerRequest(
      correlationId,
      clientId,
      acks.toShort,
    ...
    leaderPerPartitionMap.foreach { leaderForPartition =>
      {
        ...
        try {
          val currentLeaderAndIsrOpt = zkUtils
  ...
  /**
    ...
    */
  def waitUntilMetadataIsPropagated(
      servers: Seq[KafkaServer],
      topic: String,
      partition: Int,
        ...
        servers.foldLeft(true) { (result, server) =>
          ...
          partitionStateOpt match {
            ...
            case Some(partitionState) =>
              leader = partitionState.leaderIsrAndControllerEpoch.leaderAndIsr
                .leader
    ...
    TestUtils.waitUntilTrue(
      () =>
        servers.exists { server =>
          server.replicaManager.getPartition(topic, partition)
  ...
  def checkForPhantomInSyncReplicas(
      zkUtils: ZkUtils,
      topic: String,
  ...
  def ensureNoUnderReplicatedPartitions(
      zkUtils: ZkUtils,
      topic: String,
      partitionToBeReassigned: Int,
      assignedReplicas: Seq[Int],
  ...
  def produceMessage(
    ...
    val producer = createNewProducer(
      ...
      retries = 5,
      requestTimeoutMs = 2000)
    producer.send(new ProducerRecord(topic, topic.getBytes, message.getBytes))
      .get
    producer.close()
  }
  ...
  /**
    ...
    */
  def getMessages(
      topicMessageStreams: Map[String, List[KafkaStream[String, String]]],
      ...
      for (messageStream <- messageStreams) {
        ...
        try {
          ...
          while ((shouldGetAllMessages && iterator.hasNext()) ||
            ...
            val message =
              iterator.next
                .message // will throw a timeout exception if the message isn't there
            messages ::= message
  ...
  def verifyTopicDeletion(
      zkUtils: ZkUtils,
      topic: String,
      numPartitions: Int,
      servers: Seq[KafkaServer]) {
    val topicAndPartitions = (0 until numPartitions)
    ...
    assertTrue(
      ...
      servers.forall(server =>
        topicAndPartitions
          .forall(tp => server.getLogManager().getLog(tp).isEmpty))
    )
  ...
  def readString(
      buffer: ByteBuffer,
      encoding: String = Charset.defaultCharset.toString): String = {
  ...
  def assertConcurrent(
    ...
    try {
      ...
      val futures = threadPool
        .invokeAll(runnables, timeoutMs, TimeUnit.MILLISECONDS).asScala
...
@deprecated(
  "This class is deprecated and it will be removed in a future release.",
  "0.10.0.0")
...
@deprecated(
  "This class has been deprecated and it will be removed in a future release.",
  "0.10.0.0")
class FixedValuePartitioner(props: VerifiableProperties = null)
    extends Partitioner {
