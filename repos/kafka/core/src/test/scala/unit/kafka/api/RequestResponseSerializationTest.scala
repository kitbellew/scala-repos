...
object SerializationTestUtils {
  ...
  private val leaderAndIsr0 =
    new LeaderAndIsr(leader = brokers.head.id, isr = brokers.map(_.id))
  private val leaderAndIsr1 =
    new LeaderAndIsr(leader = brokers.head.id, isr = brokers.tail.map(_.id))
  private val leaderAndIsr2 =
    new LeaderAndIsr(leader = brokers.head.id, isr = brokers.map(_.id))
  private val leaderAndIsr3 =
    new LeaderAndIsr(leader = brokers.head.id, isr = brokers.tail.map(_.id))

  private val leaderIsrAndControllerEpoch0 =
    new LeaderIsrAndControllerEpoch(leaderAndIsr0, controllerEpoch = 0)
  private val leaderIsrAndControllerEpoch1 =
    new LeaderIsrAndControllerEpoch(leaderAndIsr1, controllerEpoch = 0)
  private val leaderIsrAndControllerEpoch2 =
    new LeaderIsrAndControllerEpoch(leaderAndIsr2, controllerEpoch = 0)
...
class RequestResponseSerializationTest extends JUnitSuite {
  private val producerRequest =
    SerializationTestUtils.createTestProducerRequest
  ...
  def testProduceResponseVersion() {
    val oldClientResponse = ProducerResponse(
      1,
      Map(
