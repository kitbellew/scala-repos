...
trait RackAwareTest {
    ...
    assignment.foreach {
      case (partitionId, replicaList) =>
        ...
        for (brokerId <- replicaList) {
          ...
          partitionRackMap(partitionId) = rack ::
            partitionRackMap.getOrElse(partitionId, List())
        }
    }
  ...
  def toBrokerMetadata(
    ...
    rackMap.toSeq.map {
      case (brokerId, rack) => BrokerMetadata(brokerId, Some(rack))
    } ++ brokersWithoutRack.map { brokerId => BrokerMetadata(brokerId, None) }
      .sortBy(_.id)

}

case class ReplicaDistributions(
    partitionRacks: Map[Int, Seq[String]],
    brokerLeaderCount: Map[Int, Int],
    brokerReplicasCount: Map[Int, Int])
