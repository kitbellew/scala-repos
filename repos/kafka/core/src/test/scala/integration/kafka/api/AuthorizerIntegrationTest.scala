...
class AuthorizerIntegrationTest extends KafkaServerTestHarness {
  ...
  val RequestKeyToErrorCode = Map[Short, (Nothing) => Short](
    ApiKeys.METADATA.id ->
      ((resp: requests.MetadataResponse) =>
        resp.errors().asScala.find(_._1 == topic)
          .getOrElse(("test", Errors.NONE))._2.code()),
    ApiKeys.PRODUCE.id ->
    ...
    val brokers = Set(new requests.UpdateMetadataRequest.Broker(
      ...
      Map(
        SecurityProtocol.PLAINTEXT ->
          new requests.UpdateMetadataRequest.EndPoint("localhost", 0)).asJava,
      null)).asJava
  ...
  def testCreatePermissionNeededForWritingToNonExistentTopic() {
    ...
    addAndVerifyAcls(
      Set(new Acl(KafkaPrincipal.ANONYMOUS, Allow, Acl.WildCardHost, Create)),
      Resource.ClusterResource)
    sendRecords(numRecords, topicPartition)
  }
  ...
  def testCreatePermissionNeededToReadFromNonExistentTopic() {
    ...
    addAndVerifyAcls(
      ClusterAcl(Resource.ClusterResource),
      Resource.ClusterResource)
    try {
  ...
  def sendRequestAndVerifyResponseErrorCode(
    ...
    if (isAuthorized)
      assertFalse(
        s"${ApiKeys.forId(key)} should be allowed",
        possibleErrorCodes.contains(errorCode))
    else
      assertTrue(
        s"${ApiKeys.forId(key)} should be forbidden",
        possibleErrorCodes.contains(errorCode))
