...
object SimpleConsumerShell extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val replicaIdOpt = parser.accepts(
      ...
      .withRequiredArg.describedAs("replica id")
      .ofType(classOf[java.lang.Integer]).defaultsTo(UseLeaderReplica)
    val offsetOpt = parser.accepts(
      ...
      .withRequiredArg.describedAs("consume offset")
      .ofType(classOf[java.lang.Long]).defaultsTo(OffsetRequest.EarliestTime)
    val clientIdOpt = parser.accepts("clientId", "The ID of this client.")
      .withRequiredArg.describedAs("clientId").ofType(classOf[String])
      .defaultsTo("SimpleConsumerShell")
    val fetchSizeOpt = parser
      .accepts("fetchsize", "The fetch size of each request.").withRequiredArg
      .describedAs("fetchsize").ofType(classOf[java.lang.Integer])
    ...
    val messageFormatterArgOpt = parser.accepts("property").withRequiredArg
    ...
    val maxMessagesOpt = parser
      ...
      .withRequiredArg.describedAs("max-messages")
      .ofType(classOf[java.lang.Integer]).defaultsTo(Integer.MAX_VALUE)
    ...
    val fetchRequestBuilder = new FetchRequestBuilder().clientId(clientId)
      .replicaId(Request.DebuggingConsumerId).maxWait(maxWaitMs)
    ...
    if (!partitionMetadataOpt.isDefined) {
      System.err
        .println("Error: partition %d does not exist for topic %s".format(
          partitionId,
          topic))
      System.exit(1)
    }
    ...
    val formatter = messageFormatterClass.newInstance()
          ...
          try {
            while (numMessagesConsumed < maxMessages) {
              val fetchRequest = fetchRequestBuilder
                .addFetch(topic, partitionId, offset, fetchSize).build()
