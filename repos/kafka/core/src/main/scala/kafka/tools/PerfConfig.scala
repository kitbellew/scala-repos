...
class PerfConfig(args: Array[String]) {
  ...
  val numMessagesOpt = parser.accepts(
    "messages",
  ...
  val messageSizeOpt =
    parser.accepts("message-size", "The size of each message.")
      .withRequiredArg
      .describedAs("size")
      .ofType(classOf[java.lang.Integer])
      .defaultsTo(100)
  ...
  val compressionCodecOpt =
    parser.accepts("compression-codec", "If set, messages are sent compressed")
      .withRequiredArg
      .describedAs(
