...
class PerfConfig(args: Array[String]) {
  ...
  val dateFormatOpt = parser.accepts(
    ...
    .describedAs("date format").ofType(classOf[String])
  ...
  val messageSizeOpt = parser
    .accepts("message-size", "The size of each message.").withRequiredArg
    .describedAs("size").ofType(classOf[java.lang.Integer]).defaultsTo(100)
  val batchSizeOpt = parser
    .accepts("batch-size", "Number of messages to write in a single batch.")
    .withRequiredArg.describedAs("size").ofType(classOf[java.lang.Integer])
    .defaultsTo(200)
