...
object ReplicaVerificationTool extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val fetchSizeOpt = parser
      .accepts("fetch-size", "The fetch size of each request.").withRequiredArg
      .describedAs("bytes").ofType(classOf[java.lang.Integer])
    ...
    val topicWhiteListOpt = parser.accepts(
      ...
      .ofType(classOf[String]).defaultsTo(".*")
    val initialOffsetTimeOpt = parser
      ...
      .ofType(classOf[java.lang.Long]).defaultsTo(-1L)
    val reportIntervalOpt = parser
      .accepts("report-interval-ms", "The reporting interval.").withRequiredArg
      .describedAs("ms").ofType(classOf[java.lang.Long]).defaultsTo(30 * 1000L)
    ...
    val topicsMetadataResponse = ClientUtils.fetchTopicMetadata(
      Set[String](),
      metadataTargetBrokers,
      clientId,
      maxWaitMs)
    val brokerMap = topicsMetadataResponse.brokers.map(b => (b.id, b)).toMap
    val filteredTopicMetadata = topicsMetadataResponse.topicsMetadata
    ...
    val expectedReplicasPerTopicAndPartition: Map[TopicAndPartition, Int] =
      topicPartitionReplicaList.groupBy(replica =>
        new TopicAndPartition(replica.topic, replica.partitionId)).map {
        case (topicAndPartition, replicaSet) =>
          topicAndPartition -> replicaSet.size
      }
    debug(
      "Expected replicas per topic partition: " +
...
private case class TopicPartitionReplica(
    topic: String,
    partitionId: Int,
    replicaId: Int)

private case class ReplicaAndMessageIterator(
    replicaId: Int,
    iterator: Iterator[MessageAndOffset])

private case class MessageInfo(
    replicaId: Int,
    offset: Long,
    nextOffset: Long,
    checksum: Long)

private class ReplicaBuffer(
    expectedReplicasPerTopicAndPartition: Map[TopicAndPartition, Int],
  ...
  def verifyCheckSum() {
    ...
    for ((topicAndPartition, fetchResponsePerReplica) <- messageSetCache) {
      ...
      val messageIteratorMap = fetchResponsePerReplica.map {
        case (replicaId, fetchResponse) =>
          replicaId ->
            fetchResponse.messages.asInstanceOf[ByteBufferMessageSet]
              .shallowIterator
      }
      ...
      while (isMessageInAllReplicas) {
            ...
            if (messageIterator.hasNext) {
              ...
              if (messageAndOffset.offset >=
                ...
                messageInfoFromFirstReplicaOpt match {
                  case None =>
                    messageInfoFromFirstReplicaOpt = Some(MessageInfo(
                      replicaId,
                  ...
                  case Some(messageInfoFromFirstReplica) =>
                    if (messageInfoFromFirstReplica.offset !=
                      ...
                      println(
                        ReplicaVerificationTool.getCurrentTimeString +
                          ": partition " + topicAndPartition + ": replica " +
                          messageInfoFromFirstReplica.replicaId + "'s offset " +
                          messageInfoFromFirstReplica.offset +
                          " doesn't match replica " + replicaId + "'s offset " +
                          messageAndOffset.offset)
                      System.exit(1)
                    }
        ...
        if (isMessageInAllReplicas) {
          ...
          debug(
            expectedReplicasPerTopicAndPartition(topicAndPartition) +
              " replicas match at offset " + nextOffset + " for " +
              topicAndPartition)
        }
      }
    ...
    if (currentTimeMs - lastReportTime > reportInterval) {
      println(
        ReplicaVerificationTool.dateFormat.format(new Date(currentTimeMs)) +
...
private class ReplicaFetcher(
  ...
  val fetchRequestBuilder = new FetchRequestBuilder()
    ...
    .replicaId(Request.DebuggingConsumerId).maxWait(maxWait).minBytes(minBytes)
