...
private[kafka] class ZookeeperConsumerConnector(
  ...
  private def ensureOffsetManagerConnected() {
    if (config.offsetsStorage == "kafka") {
      ...
      debug(
        "Connected to offset manager %s:%d.".format(
          offsetsChannel.host,
          offsetsChannel.port))
    }
  }
  ...
  def shutdown() {
    ...
    if (canShutdown) {
      ...
      rebalanceLock synchronized {
        ...
        info(
          "ZKConsumerConnector shutdown completed in " + (System.nanoTime() - startTime) / 1000000 + " ms")
      }
    }
  }
  ...
  def consume[K, V](
    ...
    loadBalancerListener.kafkaMessageAndMetadataStreams.asInstanceOf[
  ...
  private def sendShutdownToAllQueues() = {
    for (queue <- topicThreadIdAndQueues.values.toSet[BlockingQueue[
           FetchedDataChunk]]) {
  ...
  def commitOffsets(
    ...
    while (!done) {
      ...
      if (!done) {
        debug(
          "Retrying offset commit in %d ms".format(
            config.offsetsChannelBackoffMs))
      ...
      while (!isShuttingDown.get && !offsetFetchResponseOpt.isDefined) {
        offsetFetchResponseOpt = offsetsChannelLock synchronized {
          ...
          try {
            ...
            case e: Exception =>
              warn(
                "Error while fetching offsets from %s:%d. Possible cause: %s".format(
                  offsetsChannel.host,
                  offsetsChannel.port,
                  e.getMessage))
              offsetsChannel.disconnect()
        ...
        if (offsetFetchResponseOpt.isEmpty) {
          debug(
            "Retrying offset fetch in %d ms".format(
              config.offsetsChannelBackoffMs))
          Thread.sleep(config.offsetsChannelBackoffMs)
        }
      }
  ...
  class ZKTopicPartitionChangeListener(
    ...
    def handleDataChange(dataPath: String, data: Object) {
      try {
        info(
          "Topic info for path " + dataPath + " changed to " + data.toString + ", triggering rebalance")
  ...
  class ZKRebalancerListener(
    ...
    def syncedRebalance() {
      ...
      throw new ConsumerRebalanceFailedException(
        consumerIdString + " can't rebalance after " + config.rebalanceMaxRetries + " retries")
    }
          ...
          consumerRebalanceListener.beforeReleasingPartitions(
            if (topicRegistry.size == 0)
              ...
              mapAsJavaMap(topicRegistry.map(topics =>
                topics._1 -> topics._2.keys).toMap).asInstanceOf[java.util.Map[
                String,
    ...
    private def closeFetchers(
      ...
      val queuesTobeCleared = topicThreadIdAndQueues.filter(q =>
        relevantTopicThreadIdsMap.contains(q._1._1)).map(q => q._2)
    ...
    private def updateFetcher(cluster: Cluster) {
      ...
      info(
        "Consumer " + consumerIdString + " selected partitions : " +
          allPartitionInfos.sortWith((s, t) =>
            s.partitionId < t.partitionId).map(_.toString).mkString(","))
  ...
  private def reinitializeConsumer[K, V](
    ...
    val allQueuesAndStreams = topicCount match {
      case wildTopicCount: WildcardTopicCount =>
         ...
         */
        (1 to consumerThreadIdsPerTopic.keySet.size).flatMap(_ =>
          queuesAndStreams).toList
    ...
    threadQueueStreamPairs.foreach(e => {
      ...
      debug(
        "Adding topicThreadId %s and queue %s to topicThreadIdAndQueues data structure".format(
          topicThreadId,
          q.toString))
      newGauge(
        "FetchQueueSize",
