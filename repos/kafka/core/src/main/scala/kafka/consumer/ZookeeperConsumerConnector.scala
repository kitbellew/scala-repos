...
private[kafka] class ZookeeperConsumerConnector(
  ...
  private var topicPartitionChangeListener: ZKTopicPartitionChangeListener =
    null
  ...
  private val zkCommitMeter = newMeter(
    ...
    TimeUnit.SECONDS,
    Map("clientId" -> config.clientId))
  private val rebalanceTimer = new KafkaTimer(newTimer(
  ...
  def setConsumerRebalanceListener(listener: ConsumerRebalanceListener) {
    if (messageStreamCreated.get())
      throw new MessageStreamsExistException(
        this.getClass.getSimpleName +
          " can only set consumer rebalance listener before creating streams",
        null)
  ...
  private def createFetcher() {
    if (enableFetcher)
      fetcher = Some(
        new ConsumerFetcherManager(consumerIdString, config, zkUtils))
  }
    ...
    if (config.offsetsStorage == "kafka") {
      if (offsetsChannel == null || !offsetsChannel.isConnected)
        offsetsChannel = ClientUtils.channelToOffsetManager(
          config.groupId,
          zkUtils,
          config.offsetsChannelSocketTimeoutMs,
          config.offsetsChannelBackoffMs)

      debug("Connected to offset manager %s:%d.".format(
        offsetsChannel.host,
        offsetsChannel.port))
    }
  }
  ...
  def consume[K, V](
    ...
    val topicCount = TopicCount
      .constructTopicCount(consumerIdString, topicCountMap)
  ...
  def getTopicRegistry: Pool[String, Pool[Int, PartitionTopicInfo]] =
    topicRegistry

  private def registerConsumerInZK(
      dirs: ZKGroupDirs,
      consumerIdString: String,
      topicCount: TopicCount) {
    info("begin registering consumer " + consumerIdString + " in ZK")
    val timestamp = SystemTime.milliseconds.toString
    val consumerRegistrationInfo = Json.encode(Map(
      "version" -> 1,
  ...
  def commitOffsetToZooKeeper(topicPartition: TopicAndPartition, offset: Long) {
    if (checkpointedZkOffsets.get(topicPartition) != offset) {
      ...
      zkUtils.updatePersistentPath(
        topicDirs.consumerOffsetDir + "/" + topicPartition.partition,
        offset.toString)
  ...
  def commitOffsets(
    ...
    var retriesRemaining = 1 + (if (isAutoCommit) 0
                                  ...
                                  config
                                    .offsetsCommitMaxRetries) // no retries for commits from auto-commit
    var done = false
    while (!done) {
        ...
        if (offsetsToCommit.size > 0) {
          if (config.offsetsStorage == "zookeeper") {
            ...
            try {
              ...
              val offsetCommitResponse = OffsetCommitResponse
                ...
                offsetCommitResponse.commitStatus
                  .foldLeft(false, false, false, 0) {
                    case (folded, (topicPartition, errorCode)) =>
                      if (errorCode == Errors.NONE.code && config
                            .dualCommitEnabled) {
                      ...
                      (
                        ...
                        folded
                          ._2 || // update retryableIfFailed - (only metadata too large is not retryable)
                          (errorCode != Errors.NONE.code && errorCode != Errors
                            .OFFSET_METADATA_TOO_LARGE.code),
                        folded._3 || // update shouldRefreshCoordinator
                          errorCode == Errors.NOT_COORDINATOR_FOR_GROUP.code ||
                          errorCode == Errors.GROUP_COORDINATOR_NOT_AVAILABLE
                            .code,
                        // update error count
                        folded._4 + (if (errorCode != Errors.NONE.code) 1
                                     else 0))
                  }
              }
      ...
      if (!done) {
        debug("Retrying offset commit in %d ms".format(
          config.offsetsChannelBackoffMs))
  ...
  private def fetchOffsetFromZooKeeper(topicPartition: TopicAndPartition) = {
    ...
    val offsetString = zkUtils.readDataMaybeNull(
      dirs.consumerOffsetDir + "/" + topicPartition.partition)._1
    offsetString match {
      case Some(offsetStr) =>
      ...
      while (!isShuttingDown.get && !offsetFetchResponseOpt.isDefined) {
        offsetFetchResponseOpt = offsetsChannelLock synchronized {
          ...
          try {
            ...
            val (leaderChanged, loadInProgress) = offsetFetchResponse
              .requestInfo.foldLeft(false, false) {
                case (folded, (topicPartition, offsetMetadataAndError)) =>
                  (
                    folded._1 || (offsetMetadataAndError.error == Errors
                      .NOT_COORDINATOR_FOR_GROUP.code),
                    folded._2 || (offsetMetadataAndError.error == Errors
              ...
              if (config.dualCommitEnabled) {
                ...
                val mostRecentOffsets = kafkaOffsets.map {
                  case (topicPartition, kafkaOffset) =>
                    val zkOffset = fetchOffsetFromZooKeeper(topicPartition)._2
                      .offset
        ...
        if (offsetFetchResponseOpt.isEmpty) {
          debug("Retrying offset fetch in %d ms".format(
            config.offsetsChannelBackoffMs))
          Thread.sleep(config.offsetsChannelBackoffMs)
        }
      }
  ...
  class ZKTopicPartitionChangeListener(
    ...
    def handleDataChange(dataPath: String, data: Object) {
      try {
        info(
          "Topic info for path " + dataPath + " changed to " + data
            .toString + ", triggering rebalance")
        ...
        case e: Throwable =>
          error(
            "Error while handling topic partition change for data path " + dataPath,
            e)
      }
    }
    ...
    def handleDataDeleted(dataPath: String) {
      ...
      warn(
        "Topic for path " + dataPath + " gets deleted, which should not happen at this time")
    }
  }
  ...
  class ZKRebalancerListener(
    ...
    private val partitionAssignor = PartitionAssignor
      .createInstance(config.partitionAssignmentStrategy)
    ...
    def syncedRebalance() {
      ...
      throw new ConsumerRebalanceFailedException(
        consumerIdString + " can't rebalance after " + config
          .rebalanceMaxRetries + " retries")
    }
    ...
    private def rebalance(cluster: Cluster): Boolean = {
      ...
      if (brokers.size == 0) {
        ...
        zkUtils.zkClient
          .subscribeChildChanges(BrokerIdsPath, loadBalancerListener)
        true
      } else {
        ...
        val assignmentContext = new AssignmentContext(
          ...
          zkUtils)
        val globalPartitionAssignment = partitionAssignor
          .assign(assignmentContext)
        val partitionAssignment = globalPartitionAssignment
          .get(assignmentContext.consumerId)
        ...
        if (isShuttingDown.get || !offsetFetchResponseOpt.isDefined) false
          ...
          if (reflectPartitionOwnershipDecision(partitionAssignment)) {
            ...
            if (consumerRebalanceListener != null) {
              ...
              val partitionAssginmentGroupByTopic = globalPartitionAssignment
                .values.flatten.groupBy[String] {
              ...
              val partitionAssigmentMapForCallback =
                partitionAssginmentGroupByTopic.map({
                  case (topic, partitionOwnerShips) =>
                    val partitionOwnershipForTopicScalaMap = partitionOwnerShips
                      .map({
                        case (topicAndPartition, consumerThreadId) =>
                          topicAndPartition.partition -> consumerThreadId
                      })
                    topic -> mapAsJavaMap(collection.mutable.Map(
    ...
    private def closeFetchersForQueues(
        cluster: Cluster,
    ...
    private def clearFetcherQueues(
    ...
    private def closeFetchers(
        cluster: Cluster,
        messageStreams: Map[String, List[KafkaStream[_, _]]],
        relevantTopicThreadIdsMap: Map[String, Set[ConsumerThreadId]]) {
      // only clear the fetcher queues for certain topic partitions that *might* no longer be served by this consumer
      // after this rebalancing attempt
      val queuesTobeCleared = topicThreadIdAndQueues
        .filter(q => relevantTopicThreadIdsMap.contains(q._1._1)).map(q => q._2)
    ...
    private def updateFetcher(cluster: Cluster) {
      ...
      info(
        "Consumer " + consumerIdString + " selected partitions : " +
          allPartitionInfos.sortWith((s, t) => s.partitionId < t.partitionId)
    ...
    private def reflectPartitionOwnershipDecision(
      ...
      val partitionOwnershipSuccessful = partitionAssignment.map {
        partitionOwner =>
          ...
          val partitionOwnerPath = zkUtils
            .getConsumerPartitionOwnerPath(group, topic, partition)
          try {
            zkUtils.createEphemeralPathExpectConflict(
              partitionOwnerPath,
              consumerThreadId.toString)
            info(
              consumerThreadId + " successfully owned partition " + partition + " for topic " + topic)
      ...
      val hasPartitionOwnershipFailed = partitionOwnershipSuccessful
        .foldLeft(0)((sum, decision) => sum + (if (decision) 0 else 1))
    ...
    private def addPartitionTopicInfo(
        currentTopicRegistry: Pool[String, Pool[Int, PartitionTopicInfo]],
        partition: Int,
        topic: String,
        offset: Long,
      ...
      val partTopicInfo = new PartitionTopicInfo(
        topic,
        partition,
        queue,
        consumedOffset,
  ...
  private def reinitializeConsumer[K, V](
    ...
    if (loadBalancerListener == null) {
      ...
      loadBalancerListener = new ZKRebalancerListener(
        ...
        consumerIdString,
        topicStreamsMap
          .asInstanceOf[scala.collection.mutable.Map[String, List[
            KafkaStream[_, _]]]])
    }
    ...
    val allQueuesAndStreams = topicCount match {
      case wildTopicCount: WildcardTopicCount =>
        ...
        (1 to consumerThreadIdsPerTopic.keySet.size)
          .flatMap(_ => queuesAndStreams).toList
    ...
    zkUtils.zkClient
      .subscribeChildChanges(dirs.consumerRegistryDir, loadBalancerListener)
  ...
  class WildcardStreamsHandler[K, V](
    ...
    if (messageStreamCreated.getAndSet(true))
      throw new RuntimeException(
        "Each consumer connector can create " +
          "message streams by filter at most once.")

    private val wildcardQueuesAndStreams = (1 to numStreams).map(e => {
      val queue =
        new LinkedBlockingQueue[FetchedDataChunk](config.queuedMaxMessages)
      val stream = new KafkaStream[K, V](
        queue,
    ...
    private var wildcardTopics = zkUtils
      .getChildrenParentMayNotExist(BrokerTopicsPath).filter(topic =>
