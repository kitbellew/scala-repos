...
private[kafka] class ZookeeperConsumerConnector(
  ...
  def createMessageStreamsByFilter[K, V](
    ...
    val wildcardStreamsHandler =
      new WildcardStreamsHandler[K, V](
        topicFilter,
        numStreams,
        keyDecoder,
        valueDecoder)
  ...
  private def sendShutdownToAllQueues() = {
    for (queue <- topicThreadIdAndQueues
           .values
            ...
            partitionTopicInfos.map {
              case (partition, info) =>
                TopicAndPartition(
                  info.topic,
                  info
                    .partitionId) -> OffsetAndMetadata(info.getConsumeOffset())
            }
        }
        .toSeq: _*)
  ...
  def commitOffsets(
    ...
    while (!done) {
      ...
      if (!done) {
        debug(
          "Retrying offset commit in %d ms"
            .format(config.offsetsChannelBackoffMs))
                ...
                .foldLeft(false, false) {
                  case (folded, (topicPartition, offsetMetadataAndError)) =>
                    (
                      ...
                      folded._2 || (
                        offsetMetadataAndError
                          .error == Errors.GROUP_LOAD_IN_PROGRESS.code
                      ))
                }
  ...
  class ZKRebalancerListener(
    ...
    private val watcherExecutorThread =
      new Thread(consumerIdString + "_watcher_executor") {
        override def run() {
          info(
            ...
            if (consumerRebalanceListener != null) {
              ...
              consumerRebalanceListener.beforeStartingFetchers(
                ...
                mapAsJavaMap(
                  collection
                    .mutable
                    .Map(partitionAssigmentMapForCallback.toSeq: _*)))
            }
    ...
    private def closeFetchers(
      ...
      val queuesTobeCleared = topicThreadIdAndQueues
        .filter(q => relevantTopicThreadIdsMap.contains(q._1._1))
        .map(q => q._2)
  ...
  class WildcardStreamsHandler[K, V](
    ...
    private val wildcardQueuesAndStreams =
      (1 to numStreams)
        .map(e => {
          val queue =
