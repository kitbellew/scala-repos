...
class DefaultEventHandler[K, V](
  ...
  val brokerPartitionInfo =
    new BrokerPartitionInfo(config, producerPool, topicPartitionInfos)

  private val topicMetadataRefreshInterval = config
  ...
  def handle(events: Seq[KeyedMessage[K, V]]) {
    ...
    serializedData.foreach { keyed =>
      ...
      producerTopicStats.getProducerTopicStats(keyed.topic).byteRate
        .mark(dataSize)
    ...
    while (remainingRetries > 0 && outstandingProduceRequests.size > 0) {
      ...
      outstandingProduceRequests = dispatchSerializedData(
  ...
  def serialize(
      events: Seq[KeyedMessage[K, V]]): Seq[KeyedMessage[K, Message]] = {
  ...
  def partitionAndCollate(messages: Seq[KeyedMessage[K, Message]])
    ...
    val ret = new HashMap[Int, collection.mutable.Map[TopicAndPartition, Seq[
      KeyedMessage[K, Message]]]]
    try {
      for (message <- messages) {
        ...
        ret.get(leaderBrokerId) match {
          case Some(element) =>
            dataPerBroker = element
              .asInstanceOf[HashMap[TopicAndPartition, Seq[
                KeyedMessage[K, Message]]]]
          case None =>
            dataPerBroker =
  ...
  private def getPartitionListForTopic(
    ...
    debug("Broker partitions registered for topic: %s are %s".format(
      m.topic,
  ...
  private def send(
    ...
    if (brokerId < 0) {
      warn("Failed to send data since partitions %s don't have a leader".format(
      ...
      try {
        ...
        if (response != null) {
          if (response.status.size != producerRequest.data.size)
            throw new KafkaException(
              "Incomplete response (%s) for producer request (%s)".format(
                ...
                producerRequest))
          if (logger.isTraceEnabled) {
            val successfullySentData = response.status
          ...
          val failedPartitionsAndStatus = response.status
            .filter(_._2.error != Errors.NONE.code).toSeq
          ...
          (
            ...
            config.compressionCodec match {
              case NoCompressionCodec =>
                debug("Sending %d messages with no compression to %s".format(
                  messages.size,
                  topicAndPartition))
