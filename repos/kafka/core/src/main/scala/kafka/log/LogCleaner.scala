...
class LogCleaner(
  ...
  private class CleanerThread(threadId: Int)
      extends ShutdownableThread(
        name = "kafka-log-cleaner-thread-" + threadId,
    ...
    def recordStats(
      ...
      if (stats.invalidMessagesRead > 0) {
        warn(
          "\tFound %d invalid messages during compaction."
            .format(stats.invalidMessagesRead))
      }
    }
...
private[log] class Cleaner(
  ...
  private[log] def cleanSegments(
    ...
    val logFile =
      new File(segments.head.log.file.getPath + Log.CleanedFileSuffix)
    logFile.delete()
    val indexFile =
      new File(segments.head.index.file.getPath + Log.CleanedFileSuffix)
    indexFile.delete()
    val messages =
      new FileMessageSet(
        logFile,
        fileAlreadyExists = false,
        initFileSize = log.initFileSize(),
    ...
    val cleaned =
      new LogSegment(
        ...
        index,
        segments.head.baseOffset,
        segments.head.indexIntervalBytes,
        log.config.randomSegmentJitter,
        time)
  ...
  private[log] def cleanInto(
    ...
    while (position < source.log.sizeInBytes) {
      ...
      writeBuffer.clear()
      val messages =
        new ByteBufferMessageSet(source.log.readInto(readBuffer, position))
  ...
  private def shouldRetainMessage(
      source: kafka.log.LogSegment,
  ...
  /**
    ...
    */
  private[log] def groupSegmentsBySize(
      segments: Iterable[LogSegment],
      maxSize: Int,
      maxIndexSize: Int): List[Seq[LogSegment]] = {
  ...
  /**
    ...
    */
  private def buildOffsetMapForSegment(
      topicAndPartition: TopicAndPartition,
      segment: LogSegment,
      map: OffsetMap): Long = {
    var position = 0
    ...
    while (position < segment.log.sizeInBytes) {
      ...
      readBuffer.clear()
      val messages =
        new ByteBufferMessageSet(segment.log.readInto(readBuffer, position))
      throttler.maybeThrottle(messages.sizeInBytes)
