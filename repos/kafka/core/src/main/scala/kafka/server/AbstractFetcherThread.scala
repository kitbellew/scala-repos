...
abstract class AbstractFetcherThread(
  ...
  protected def buildFetchRequest(
      partitionMap: Map[TopicAndPartition, PartitionFetchState]): REQ
          ...
          case (topicAndPartition, partitionData) =>
            ...
            partitionMap.get(topicAndPartition)
              .foreach(currentPartitionFetchState =>
                ...
                if (fetchRequest
                      .offset(topicAndPartition) == currentPartitionFetchState
                      .offset) {
                  Errors.forCode(partitionData.errorCode) match {
                    case Errors.NONE =>
                      try {
                        ...
                        fetcherLagStats.getFetcherLagStats(topic, partitionId)
                          .lag = Math
                    ...
                    case _ =>
                      if (isRunning.get) {
                        error(
                          "Error for partition [%s,%d] to broker %d:%s".format(
                            topic,
                            partitionId,
                            sourceBroker.id,
