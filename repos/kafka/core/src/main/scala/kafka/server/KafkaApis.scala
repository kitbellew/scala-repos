...
class KafkaApis(
  ...
  def handle(request: RequestChannel.Request) {
    ...
    } finally request.apiLocalCompleteTimeMs = SystemTime.milliseconds
  }
    ...
    try {
      ...
      val leaderAndIsrResponse =
        if (authorize(
          ...
          val result = leaderAndIsrRequest.partitionStates.asScala.keys
            .map((_, new JShort(Errors.CLUSTER_AUTHORIZATION_FAILED.code)))
            .toMap
          new LeaderAndIsrResponse(
    ...
    val response =
      if (authorize(request.session, ClusterAction, Resource.ClusterResource)) {
        ...
        new StopReplicaResponse(
          error,
          result.asInstanceOf[Map[TopicPartition, JShort]].asJava)
      } else {
        val result = stopReplicaRequest.partitions.asScala
          .map((_, new JShort(Errors.CLUSTER_AUTHORIZATION_FAILED.code))).toMap
        new StopReplicaResponse(
          Errors.CLUSTER_AUTHORIZATION_FAILED.code,
          result.asJava)
      }
  ...
  def handleControlledShutdownRequest(request: RequestChannel.Request) {
    ...
    val partitionsRemaining = controller
      .shutdownBroker(controlledShutdownRequest.brokerId)
  ...
  def handleProducerRequest(request: RequestChannel.Request) {
    ...
    val (authorizedRequestInfo, unauthorizedRequestInfo) = produceRequest
      .partitionRecords.asScala.partition {
        case (topicPartition, _) =>
          authorize(
            request.session,
            Write,
      ...
      def produceResponseCallback(delayTimeMs: Int) {
        if (produceRequest.acks == 0) {
          ...
          if (errorInResponse) {
            ...
            info(
              s"Closing connection due to error during produce request with correlation id ${request
    ...
    if (authorizedRequestInfo.isEmpty) sendResponseCallback(Map.empty)
      ...
      val authorizedMessagesPerPartition = authorizedRequestInfo.map {
        case (topicPartition, buffer) =>
          (topicPartition, new ByteBufferMessageSet(buffer))
      }
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    val (authorizedRequestInfo, unauthorizedRequestInfo) = fetchRequest
        ...
        if (fetchRequest.versionId <= 1) {
          responsePartitionData.map {
            case (tp, data) =>
              ...
              val convertedData =
                if (replicaManager.getMessageFormatVersion(tp)
                  ...
                  new FetchResponsePartitionData(
                    data.error,
                    data.hw,
                    data.messages.asInstanceOf[FileMessageSet]
      ...
      if (fetchRequest.isFromFollower) { fetchResponseCallback(0) }
        ...
        quotaManagers(ApiKeys.FETCH.id).recordAndMaybeThrottle(
          ...
          FetchResponse.responseSize(
            mergedPartitionData.groupBy(_._1.topic),
            fetchRequest.versionId),
  ...
  def handleOffsetRequest(request: RequestChannel.Request) {
    ...
    val (authorizedRequestInfo, unauthorizedRequestInfo) = offsetRequest
      .offsetData.asScala.partition {
        case (topicPartition, _) =>
          authorize(
            request.session,
            Describe,
  ...
  private def getOrCreateGroupMetadataTopic(
      securityProtocol: SecurityProtocol): MetadataResponse.TopicMetadata = {
    val topicMetadata = metadataCache.getTopicMetadata(
      Set(TopicConstants.GROUP_METADATA_TOPIC_NAME),
      securityProtocol)
  ...
  private def getTopicMetadata(
    ...
    val topicResponses = metadataCache
      .getTopicMetadata(topics, securityProtocol)
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request) {
    ...
    var (authorizedTopics, unauthorizedTopics) =
      if (metadataRequest.topics.isEmpty) {
        ...
        val authorized = metadataCache.getAllTopics().filter(topic =>
    ...
    if (authorizedTopics.nonEmpty) {
      val nonExistingTopics = metadataCache
        .getNonExistingTopics(authorizedTopics)
  ...
  def handleOffsetFetchRequest(request: RequestChannel.Request) {
    ...
    val offsetFetchResponse =
      ...
      if (!authorize(
        ...
        val unauthorizedGroupResponse = new OffsetFetchResponse.PartitionData(
        ...
        val (authorizedTopicPartitions, unauthorizedTopicPartitions) =
          offsetFetchRequest.partitions.asScala.partition { topicPartition =>
            authorize(
              request.session,
              Describe,
              new Resource(Topic, topicPartition.topic))
          }
        val unauthorizedTopicResponse = new OffsetFetchResponse.PartitionData(
          OffsetFetchResponse.INVALID_OFFSET,
          "",
          Errors.TOPIC_AUTHORIZATION_FAILED.code)
        val unauthorizedStatus = unauthorizedTopicPartitions
          .map(topicPartition => (topicPartition, unauthorizedTopicResponse))
          .toMap
        val unknownTopicPartitionResponse =
        ...
        if (header.apiVersion == 0) {
          ...
          val offsets = coordinator.handleFetchOffsets(
            offsetFetchRequest.groupId,
  ...
  def handleGroupCoordinatorRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(
          request.session,
          Describe,
          new Resource(Group, groupCoordinatorRequest.groupId))) {
      ...
      val responseBody =
        if (offsetsTopicMetadata.error != Errors.NONE) {
          ...
          val coordinatorEndpoint = offsetsTopicMetadata.partitionMetadata()
            .asScala.find(_.partition == partition).map(_.leader())
  ...
  def handleJoinGroupRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(
      ...
      val protocols = joinGroupRequest.groupProtocols()
   ...
   */
  private def instantiateQuotaManagers(
      cfg: KafkaConfig): Map[Short, ClientQuotaManager] = {
