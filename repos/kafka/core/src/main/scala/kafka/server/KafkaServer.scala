...
object KafkaServer {
  ...
  private[kafka] def copyKafkaConfigToLog(
...
class KafkaServer(
  ...
  private implicit val kafkaMetricsTime: org.apache.kafka.common.utils.Time =
    ...
    try {
      ...
      if (canStartup) {
        ...
        dynamicConfigManager =
          new DynamicConfigManager(zkUtils, dynamicConfigHandlers)
        dynamicConfigManager.startup()

        /* tell everyone we are alive */
        val listeners = config
          .advertisedListeners
          .map {
  ...
  private def initZk(): ZkUtils = {
    ...
    if (chroot.length > 1) {
      val zkConnForChrootCreation = config
        .zkConnect
  ...
  private def controlledShutdown() {
    ...
    def socketTimeoutException: Throwable =
      new SocketTimeoutException(
        s"Did not receive response within $socketTimeoutMs")
          ...
          if (prevController != null) {
            try {
              ...
              if (shutdownResponse.errorCode == Errors.NONE.code &&
                ...
                info(
                  "Error code from controller: %d"
                    .format(shutdownResponse.errorCode))
              }
            } catch {
          ...
          if (channel != null) {
            ...
            try {
              ...
              if (shutdownResponse.errorCode == Errors.NONE.code &&
                ...
                info(
                  "Remaining partitions to move: %s"
                    .format(shutdownResponse.partitionsRemaining.mkString(",")))
                info(
                  "Error code from controller: %d"
                    .format(shutdownResponse.errorCode))
              }
    ...
    if (startupComplete.get() && config.controlledShutdownEnable) {
      ...
      val shutdownSucceeded =
        ...
        if (config.interBrokerProtocolVersion >= KAFKA_0_9_0)
          networkClientControlledShutdown(
            config.controlledShutdownMaxRetries.intValue)
        else
          blockingChannelControlledShutdown(
            config.controlledShutdownMaxRetries.intValue)
  ...
  def shutdown() {
    try {
      ...
      if (isStartingUp.get)
        throw new IllegalStateException(
          "Kafka server is still starting up, cannot shut down!")
  ...
  private def createLogManager(
      zkClient: ZkClient,
      brokerState: BrokerState): LogManager = {
