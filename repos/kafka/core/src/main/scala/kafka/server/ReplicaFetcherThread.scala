...
class ReplicaFetcherThread(
  ...
  private val fetchRequestVersion: Short =
    if (brokerConfig.interBrokerProtocolVersion >= KAFKA_0_10_0_IV0)
      2
        ...
        trace(
          "Follower %d has replica log end offset %d after appending %d bytes of messages for partition %s"
            .format(
              ...
              messageSet.sizeInBytes,
              topicAndPartition))
      val followerHighWatermark = replica
        .logEndOffset
        .messageOffset
  ...
  protected def fetch(
    ...
    new FetchResponse(clientResponse.responseBody)
      .responseData
      .asScala
      .map {
        case (key, value) =>
...
object ReplicaFetcherThread {
  ...
  private[server] class FetchRequest(val underlying: JFetchRequest)
  ...
  private[server] class PartitionData(
      val underlying: FetchResponse.PartitionData)
