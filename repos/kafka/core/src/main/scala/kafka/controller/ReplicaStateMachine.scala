...
class ReplicaStateMachine(controller: KafkaController) extends Logging {
  ...
  this.logIdent = "[Replica state machine on controller " + controller.config
    .brokerId + "]: "
  ...
  def handleStateChanges(
    ...
    if (replicas.size > 0) {
      ...
      try {
        ...
        case e: Throwable =>
          error(
            "Error while moving some replicas to %s state".format(targetState),
            e)
      }
    }
  }
    ...
    if (!hasStarted.get)
      throw new StateChangeFailedException(
        ("Controller %d epoch %d initiated state change of replica %d for partition %s " +
          "to %s failed because replica state machine has not started").format(
          controllerId,
          controller.epoch,
          replicaId,
          topicAndPartition,
          targetState))
    val currState = replicaState
      .getOrElseUpdate(partitionAndReplica, NonExistentReplica)
    try {
      ...
      targetState match {
        ...
        case OfflineReplica =>
          ...
          val leaderAndIsrIsEmpty: Boolean = controllerContext
            .partitionLeadershipInfo.get(topicAndPartition) match {
  ...
  def areAllReplicasForTopicDeleted(topic: String): Boolean = {
    ...
    val replicaStatesForTopic = replicasForTopic.map(r => (r, replicaState(r)))
      .toMap
  ...
  def partitionsAssignedToBroker(
    ...
    controllerContext.partitionReplicaAssignment.filter(_._2.contains(brokerId))
      .keySet.toSeq
  }
  ...
  class BrokerChangeListener() extends IZkChildListener with Logging {
    this.logIdent = "[BrokerChangeListener on Controller " + controller.config
      .brokerId + "]: "
      ...
      inLock(controllerContext.controllerLock) {
        if (hasStarted.get) {
          ControllerStats.leaderElectionTimer.time {
            try {
              val curBrokers = currentBrokerList.map(_.toInt).toSet
                .flatMap(zkUtils.getBrokerInfo)
              val curBrokerIds = curBrokers.map(_.id)
              val liveOrShuttingDownBrokerIds = controllerContext
                .liveOrShuttingDownBrokerIds
              val newBrokerIds = curBrokerIds -- liveOrShuttingDownBrokerIds
              val deadBrokerIds = liveOrShuttingDownBrokerIds -- curBrokerIds
              val newBrokers = curBrokers
                .filter(broker => newBrokerIds(broker.id))
              ...
              newBrokers
                .foreach(controllerContext.controllerChannelManager.addBroker)
