...
class PartitionStateMachine(controller: KafkaController) extends Logging {
  ...
  private val partitionModificationsListeners
  ...
  def handleStateChanges(
    ...
    info("Invoking state change to %s for partitions %s".format(
      targetState,
      partitions.mkString(",")))
    try {
      brokerRequestBatch.newBatch()
    ...
    if (!hasStarted.get)
      throw new StateChangeFailedException(
        ("Controller %d epoch %d initiated state change for partition %s to %s failed because " +
          ...
          topicAndPartition,
          targetState))
    val currState = partitionState
      .getOrElseUpdate(topicAndPartition, NonExistentPartition)
    try {
      targetState match {
        case NewPartition =>
          ...
          val assignedReplicas = controllerContext
            .partitionReplicaAssignment(topicAndPartition).mkString(",")
      ...
      controllerContext.partitionLeadershipInfo.get(topicPartition) match {
        case Some(currentLeaderIsrAndEpoch) =>
          ...
          controllerContext.liveBrokerIds
            .contains(currentLeaderIsrAndEpoch.leaderAndIsr.leader) match {
  ...
  private def initializeLeaderAndIsrForPartition(
    ...
    val liveAssignedReplicas = replicaAssignment
      ...
      case _ =>
        ...
        try {
          ...
          case e: ZkNodeExistsException =>
            ...
            val leaderIsrAndEpoch = ReplicationUtils
              .getLeaderIsrAndEpochForPartition(
                zkUtils,
                topicAndPartition.topic,
    ...
    try {
      ...
      while (!zookeeperPathUpdateSucceeded) {
        ...
        val (leaderAndIsr, replicas) = leaderSelector
          .selectLeader(topicAndPartition, currentLeaderAndIsr)
      ...
      stateChangeLogger.trace(
        "Controller %d epoch %d elected leader %d for Offline partition %s"
          .format(
            ...
            topicAndPartition))
      val replicas = controllerContext
        .partitionReplicaAssignment(TopicAndPartition(topic, partition))
  ...
  class DeleteTopicsListener() extends IZkChildListener with Logging {
    ...
    def handleChildChange(
      ...
      inLock(controllerContext.controllerLock) {
        ...
        debug("Delete topics listener fired for topics %s to be deleted".format(
          ...
          if (controller.deleteTopicManager.isTopicQueuedUpForDeletion(topic))
            ...
            if (partitionsToBeAdded.size > 0) {
              ...
              controllerContext.partitionReplicaAssignment
                .++=(partitionsToBeAdded)
              controller
                .onNewPartitionCreation(partitionsToBeAdded.keySet.toSet)
            }
          }
        } catch {
          case e: Throwable =>
            error(
              "Error while handling add partitions for data path " + dataPath,
              e)
        }
      }
    }
