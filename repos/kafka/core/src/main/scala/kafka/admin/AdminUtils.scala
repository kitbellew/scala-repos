...
object AdminUtils extends Logging {
  ...
  def assignReplicasToBrokers(
    ...
    if (replicationFactor > brokerMetadatas.size)
      throw new AdminOperationException(
        s"replication factor: $replicationFactor larger than available brokers: ${brokerMetadatas.size}")
    if (brokerMetadatas.forall(_.rack.isEmpty))
      assignReplicasToBrokersRackUnaware(
        nPartitions,
        replicationFactor,
        brokerMetadatas.map(_.id),
        fixedStartIndex,
        startPartitionId)
    else {
  ...
  def addPartitions(
    ...
    val unmatchedRepFactorList = newPartitionReplicaList.values
  ...
  def getManualReplicaAssignment(
    ...
    for (i <- 0 until partitionList.size) {
      ...
      if (ret(partitionId).size != ret(startPartitionId).size)
        throw new AdminOperationException(
          "partition " + i + " has different replication factor: " + brokerList)
  ...
  /**
    ...
    */
  def deleteConsumerGroupInfoForTopicInZK(
      zkUtils: ZkUtils,
      group: String,
      topic: String) = {
  ...
  def createOrUpdateTopicPartitionAssignmentPathInZK(
    ...
    require(
      partitionReplicaAssignment.values.map(_.size).toSet.size == 1,
  ...
  private def writeTopicPartitionAssignment(
    ...
    try {
      ...
      val jsonPartitionData = zkUtils.replicaAssignmentZkData(
        replicaAssignment.map(e => (e._1.toString -> e._2)))
  ...
  private def writeEntityConfig(
    ...
    zkUtils.updatePersistentPath(
      getEntityConfigPath(entityType, entityName),
      Json.encode(map))
  }
  ...
  def fetchTopicMetadataFromZk(
      topic: String,
      zkUtils: ZkUtils): MetadataResponse.TopicMetadata =
  ...
  private def fetchTopicMetadataFromZk(
    ...
    if (zkUtils.pathExists(getTopicPath(topic))) {
      val topicPartitionAssignment = zkUtils
        .getPartitionAssignmentForTopics(List(topic)).get(topic).get
      ...
      val partitionMetadata = sortedPartitions.map { partitionMap =>
        ...
        try {
          leaderInfo = leader match {
            ...
            case None =>
              throw new LeaderNotAvailableException(
                "No leader exists for partition " + partition)
          }
          try {
          ...
          if (replicaInfo.size < replicas.size)
            throw new ReplicaNotAvailableException(
              "Replica information not available for following brokers: " +
                replicas.filterNot(replicaInfo.map(_.id).contains(_))
                  .mkString(","))
          if (isrInfo.size < inSyncReplicas.size)
            throw new ReplicaNotAvailableException(
              "In Sync Replica information not available for following brokers: " +
                inSyncReplicas.filterNot(isrInfo.map(_.id).contains(_))
                  .mkString(","))
    ...
    val brokerMetadata = brokerIds.map { id =>
      ...
      optionalBrokerInfo match {
        case Some(brokerInfo) =>
          Some(brokerInfo) // return broker info from the cache
