...
object SlickBuild extends Build {
  ...
  lazy val aRootProject: Project = Project(
    ...
    base = file("."),
    settings =
      Defaults.coreDefaultSettings ++ sharedSettings ++ extTarget("root") ++
        Seq(
    ...
    settings = Defaults.coreDefaultSettings ++ sdlcSettings ++
      ...
      Seq(
        ...
        previousArtifact :=
          Some(
            "com.typesafe.slick" %
              ("slick_" + scalaBinaryVersion.value) % binaryCompatSlickVersion),
        binaryIssueFilters ++=
          Seq(
  ...
  lazy val slickTestkitProject = Project(
    ...
    settings = Defaults.coreDefaultSettings ++ typeProvidersSettings ++
      sharedSettings ++ extTarget("testkit") ++
      Seq(
        name := "Slick-TestKit",
        description :=
    ...
    .settings(
      unmanagedSourceDirectories in DocTest +=
        (baseDirectory in slickProject).value / "src/sphinx/code",
      unmanagedResourceDirectories in DocTest +=
        (baseDirectory in slickProject).value / "src/sphinx/resources"
    ) dependsOn
    (
      slickProject,
  ...
  lazy val slickCodegenProject = Project(
    ...
    settings = Defaults.coreDefaultSettings ++ sdlcSettings ++ sharedSettings ++
      extTarget("codegen") ++ commonSdlcSettings ++
      Seq(
        name := "Slick-CodeGen",
        description :=
  ...
  lazy val slickHikariCPProject = Project(
    ...
    settings = Defaults.coreDefaultSettings ++ sdlcSettings ++ sharedSettings ++
      extTarget("hikaricp") ++ commonSdlcSettings ++ osgiSettings ++
      Seq(
        name := "Slick-HikariCP",
        description :=
  ...
  lazy val fmppSettings =
    inConfig(Compile)(Seq(sourceGenerators <+= fmpp, fmpp <<= fmppTask)) ++
      Seq(
        libraryDependencies ++=
          Seq(
        ...
        FileFunction
          .cached(s.cacheDirectory / "fmpp", outStyle = FilesInfo.exists) {
            (in: Set[File]) =>
              ...
              val args = "--expert" :: "-q" :: "-S" :: fmppSrc.getPath ::
                "-O" :: output.getPath :: "--replace-extensions=fm, scala" ::
                "-M" :: "execute(**/*.fm), ignore(**/*)" :: Nil
