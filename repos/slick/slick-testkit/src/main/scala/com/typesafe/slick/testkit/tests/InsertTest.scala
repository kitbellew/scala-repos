    ...
    seq(
      ...
      ifCap(jcap.forceInsert)(
        seq(
          ...
          ts.filter(_.id.between(90, 99)).result.headOption.map(
            _ shouldBe Some((90, "X", 1, false, "S1", "S2", 0)))
        ))
    )
  }
    ...
    (for {
      ...
      _ <- ifCap(jcap.returnInsertKey) {
        ...
        for {
          ...
          _ <- ts.sortBy(_.id).result.map(
            _ shouldBe Seq((1, "f"), (2, "b"), (3, "c"), (4, "e")))
        } yield ()
      }
