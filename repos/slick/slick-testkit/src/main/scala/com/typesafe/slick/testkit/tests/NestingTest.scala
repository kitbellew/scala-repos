...
class NestingTest extends AsyncTest[RelationalTestDB] {
  ...
  def testNestedTuples = {
    ...
    val q2b = for {
      (a, b, c) <-
        ts.filter(_.a === 1).map(t => (t.a, t.b, LiteralColumn(4))) unionAll ts
          .filter(_.a === 2)
  ...
  def testNestedOptions = {
    ...
    val q3dt: Query[Rep[Option[(Rep[Int], Rep[Int], ConstColumn[Int])]], _, Seq] =
      q3d
    ...
    lazy val t4 = seq(
      ...
      q1d2.result
        .named("q1d2")
        .map(_ shouldBe r.map(t => Some(t)).map(_.map(x => (x._1, x._2, x._3)))),
      q2d.result
        .named("q2d")
    ...
    lazy val t6 = seq(
      ...
      q1f3.result
        .named("q1f3")
        .map(_ shouldBe r.map(t => Some(t)).map { to => Some(to) }.map(_.flatten)),
      q2f1.result
        .named("q2f1")
