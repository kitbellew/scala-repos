...
class RelationalMapperTest extends AsyncTest[RelationalTestDB] {
  ...
  def testMappedType = {
    ...
    seq(
      ...
      ts.to[Set].result.map(
        _ shouldBe Set((1, False, None), (2, True, Some(True)))),
      ts.filter(_.b === (True: Bool)).to[Set].result.map(
        _ shouldBe Set((2, True, Some(True)))),
      ts.filter(_.b === (False: Bool)).to[Set].result.map(
        _ shouldBe Set((1, False, None)))
    )
  }
    ...
    seq(
      ...
      ts.filter(_.b === (EnumValue1: EnumType)).to[Set].result.map(
        _ shouldBe Set(
          (1, EnumValue1, None),
          (2, EnumValue1, Some(EnumValue2)))),
      ts.filter(_.b === (EnumValue2: EnumType)).to[Set].result.map(
        _ shouldBe Set((3, EnumValue2, Some(EnumValue3))))
    )
  }
  ...
  def testAutoMapped = {
    ...
    seq(
      ...
      ts.to[Set].result.map(
        _ shouldBe Set((MyMappedID(1), 2), (MyMappedID(3), 4))),
      ts.filter(_.id === MyMappedID(1)).to[Set].result.map(
        _ shouldBe Set((MyMappedID(1), 2)))
    )
  }
