...
object StandardTestDBs {
  ...
  lazy val H2MemKeepAlive =
    new H2TestDB("h2mem", true) {
      val url = "jdbc:h2:mem:test1"
    }

  lazy val H2Disk =
    new H2TestDB("h2disk", false) {
    ...
    override def dropUserArtifacts(implicit session: profile.Backend#Session) =
      blockingRunOnSession { implicit ec =>
        for {
          ...
          tables <- localTables
          tableStatements = tables
            .map(t => sqlu"drop table #${profile.quoteIdentifier(t)}")
          _ <- DBIO.sequence(tableStatements)
        } yield ()
      }
  }

  lazy val SQLServerJTDS =
    new SQLServerDB("sqlserver-jtds") {
      override def capabilities =
        super.capabilities - TestDB.capabilities.plainSql
    }
  lazy val SQLServerSQLJDBC =
    new SQLServerDB("sqlserver-sqljdbc") {
      override def capabilities =
        profile.capabilities - JdbcCapabilities.createModel
    }

  lazy val Oracle =
    new ExternalJdbcTestDB("oracle") {
      ...
      override def dropUserArtifacts(implicit
        ...
        session.close()
        val a = DBIO
          .sequence(Seq(drop(0), create(1), create(2)).map(s => sqlu"#$s"))
        await(databaseFor("adminConn").run(a))
      }
    }
}

abstract class H2TestDB(confName: String, keepAlive: Boolean)
    extends InternalJdbcTestDB(confName) {
...
class SQLiteTestDB(dburl: String, confName: String)
    extends InternalJdbcTestDB(confName) {
  import profile.api.actionBasedSQLInterpolation
...
abstract class DerbyDB(confName: String) extends InternalJdbcTestDB(confName) {
  ...
  override def dropUserArtifacts(implicit session: profile.Backend#Session) =
    try {
      blockingRunOnSession { implicit ec =>
        for {
          _ <- sqlu"""create table "__derby_dummy"(x integer primary key)"""
            .asTry
