...
trait TestDB {
  ...
  def isEnabled =
    TestkitConfig.testDBs.map(_.contains(confName)).getOrElse(true)
  ...
  def testClasses: Seq[Class[_ <: GenericTest[_ >: Null <: TestDB]]] =
    TestkitConfig.testClasses
}
        ...
        _ <- DBIO.seq(
          (tables.map(t =>
            sqlu"""drop table if exists #${profile
              .quoteIdentifier(t)} cascade""") ++ sequences
            .map(t =>
