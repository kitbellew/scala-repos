...
class JoinTest extends AsyncTest[RelationalTestDB] {
  ...
  def testJoin = {
    ...
    for {
      ...
      _ <-
        mark("q1", q1.map(p => (p._1, p._2)).result)
      ...
      q5 =
        (
          for {
            c <- categories
          } yield (c, Rep.None[Int])
        ).sortBy(_._1.id)
      _ <-
        mark("q5", q5.result.map(_.map(_._1._1)))
  ...
  def testOptionExtendedJoin = {
    ...
    for {
      ...
      q2 =
        (
          (xs.map(_.b) joinLeft ys.map(_.b) on (_ === _)) joinLeft ys
            .map(_.b) on (_._1 === _)
        ).to[Set]
      ...
      q5 =
        ((xs joinLeft ys on (_.b === _.b)) joinLeft ys on (_._1.b === _.b))
          .to[Set]
      ...
      q7 =
        (
          (ys.map(_.b) joinRight xs.map(_.b) on (_ === _)) joinRight xs
            .map(_.b) on (_._2 === _)
        ).to[Set]
  ...
  def testZip =
    ifCap(rcap.zip) {
      ...
      for {
        ...
        _ <-
          mark("q2", q2.result)
            .map(_ shouldBe List((1, -1), (2, 1), (3, 2), (4, 2)))
        q3 =
          for {
  ...
  def testDiscriminatorCheck = {
    ...
    val q1 =
      for {
        (a, b) <- as joinLeft bs on (_.id.? === _.id)
        if (b.isEmpty)
      } yield (a.id)
