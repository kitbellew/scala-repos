...
class NewQuerySemanticsTest extends AsyncTest[RelationalTestDB] {
    ...
    val q6c =
      (
        for (o <- orders
          ...
          yield o
      ).sortBy(_.orderID).map(o => o.orderID ~ o.userID)
  ...
  def testNewFusion = {
    ...
    for {
      ...
      _ <-
        mark("q19", q19.result).map(
          _.toSet shouldBe
            Set(Some((1, "a", "a")), Some((2, "a", "b")), Some((3, "c", "b"))))
      _ <-
        mark("q19b", q19b.result).map(
          _.toSet shouldBe
            Set(Some((1, "a", "a")), Some((2, "a", "b")), Some((3, "c", "b"))))
    } yield ()
  }
