...
class NewQuerySemanticsTest extends AsyncTest[RelationalTestDB] {
  ...
  def testNewComposition = {
    ...
    def a2 =
      seq(
        ...
        mark("q1", q1.result).named("q1: Plain implicit join").map(_.toSet)
          .map { r1 =>
            r1 shouldBe Set(
              (
        ...
        ifCap(rcap.pagingNested) {
          mark("q1b", q1b.result).named("q1b: Explicit join with condition")
            .map { r1b =>
    ...
    def a3 =
      for {
        ...
        _ <- q3.result.named("Lifting scalar values").map(_.toSet).map { r3 =>
          r3 shouldBe
            Set(("Colombian_Decaf", "Acme, Inc.", "Colombian_Decaf", 0, 3396))
        }
        _ <- q3b.result.named("Lifting scalar values, with extra tuple")
          .map(_.toSet).map { r3b =>
            r3b shouldBe Set(
        ...
        _ <- ifCap(rcap.pagingNested) {
          mark("q4", q4.result).named("q4: Map to tuple, then filter")
    ...
    def a4 =
      seq(
        ...
        mark("q5b", q5b.result).named("q5b: Explicit self-join with condition")
          .map(_.toSet).map { r5b =>
            r5b shouldBe Set(
              (("Colombian", 101, 799, 1, 0), ("Colombian", 101, 799, 1, 0)),
              (
    ...
    def a5 =
      seq(
        ...
        q71.result.named("Transitive push-down without union").map(_.toSet)
          .map { r71 =>
            r71 shouldBe Set(("Colombian", 101, 1), ("French_Roast", 49, 1))
          }
      )
  ...
  def testOldComposition = {
    ...
    val q6a =
      (for (o <- orders
            if o.orderID ===
              (for { o2 <- orders if o.userID === o2.userID } yield o2.orderID)
                .max) yield o.orderID).sorted

    val q6b =
      (for (o <- orders
            if o.orderID ===
              (for { o2 <- orders if o.userID === o2.userID } yield o2.orderID)
                .max) yield o.orderID ~ o.userID).sortBy(_._1)

    val q6c =
      (for (o <- orders
            if o.orderID ===
              (for { o2 <- orders if o.userID === o2.userID } yield o2.orderID)
                .max) yield o).sortBy(_.orderID).map(o => o.orderID ~ o.userID)
  ...
  def testExpansion = {
    ...
    for {
      _ <- as.schema.create
      _ <-
        as.map(a => (a.id, a.a, a.b)) ++=
          Seq((1, "a1", "b1"), (2, "a2", "b2"), (3, "a3", "b3"))
  ...
  def testNewFusion = {
    ...
    val q19 = as.joinLeft(as).on { case (a1, a2) => a1.id === a2.id }
      .joinLeft(as).on { case ((_, a2), a3) => a2.map(_.b) === a3.b }.map(_._2)
    val q19b = as.joinLeft(as).on { case (a1, a2) => a1.id === a2.id }
      .joinLeft(as).on { case ((_, a2), a3) => a2.map(_.b) === a3.b }.subquery
      .map(_._2)
    ...
    for {
      ...
      _ <- mark("q10", q10.result)
        .map(_ shouldBe Seq(1, 2, 3, 4, 5, 6, 7, 8, 9))
      ...
      _ <- mark("q19", q19.result).map(
        _.toSet shouldBe
          Set(Some((1, "a", "a")), Some((2, "a", "b")), Some((3, "c", "b"))))
