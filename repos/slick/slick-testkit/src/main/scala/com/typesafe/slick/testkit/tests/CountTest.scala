...
class CountTest extends AsyncTest[RelationalTestDB] {
  ...
  def testJoinCount = {
    ...
    DBIO.seq(
      ...
      (for { a <- as if a.id === 1L } yield (
        a,
        (for { b <- bs if b.aId === a.id } yield b).length)).result
        .named("directLength").map(_ shouldBe Seq((1L, 2))),
      (for {
        a <- as if a.id === 1L
        l <- Query((for { b <- bs if b.aId === a.id } yield b).length)
      } yield (a, l)).result.named("joinLength").map(_ shouldBe Seq((1L, 2))),
      (for { (a, b) <- as joinLeft bs on (_.id === _.aId) } yield (
        a.id,
        b.map(_.data))).length.result.named("outerJoinLength").map(_ shouldBe 3)
    )
  }
  ...
  def testTableCount = {
    ...
    DBIO.seq(
      ...
      ts.length.result.map(_ shouldBe 1)).withPinnedSession
  }
}
