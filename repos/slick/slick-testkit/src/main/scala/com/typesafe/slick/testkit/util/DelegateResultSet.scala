...
class DelegateResultSet(rs: ResultSet) extends ResultSet {
  ...
  def getObject[T](columnLabel: String, `type`: Class[T]): T =
    ??? // rs.getObject[T](columnLabel, `type`)
  def getObject[T](columnIndex: Int, `type`: Class[T]): T =
    ??? // rs.getObject[T](columnIndex, `type`)
  ...
  def updateObject(
      columnLabel: String,
      x: scala.Any,
      scaleOrLength: Int): Unit = rs.updateObject(columnLabel, x, scaleOrLength)
  def updateCharacterStream(
      columnLabel: String,
      reader: Reader,
  ...
  def updateAsciiStream(
      columnLabel: String,
      x: InputStream,
      length: Int): Unit = rs.updateAsciiStream(columnLabel, x, length)
  ...
  def getCharacterStream(columnLabel: String): Reader =
    rs.getCharacterStream(columnLabel)
  def getCharacterStream(columnIndex: Int): Reader =
    rs.getCharacterStream(columnIndex)
  ...
  def getBigDecimal(columnLabel: String, scale: Int): BigDecimal =
    rs.getBigDecimal(columnLabel, scale)
  ...
  def getUnicodeStream(columnIndex: Int): InputStream =
    rs.getUnicodeStream(columnIndex)
  def getAsciiStream(columnIndex: Int): InputStream =
    rs.getAsciiStream(columnIndex)
  ...
  def getBigDecimal(columnIndex: Int, scale: Int): BigDecimal =
    rs.getBigDecimal(columnIndex, scale)
