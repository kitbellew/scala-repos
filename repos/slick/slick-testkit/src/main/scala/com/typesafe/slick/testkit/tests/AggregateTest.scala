...
class AggregateTest extends AsyncTest[RelationalTestDB] {
  ...
  def testGroupBy = {
    ...
    db.run {
      ...
      db.run(mark("q1", q1.result)).map { r0t: Seq[Int] =>
        r0t shouldBe Vector(2, 3, 3)
      }
    }.flatMap { _ =>
      val q =
        (for { (k, v) <- ts.groupBy(t => t.a) } yield (
          k,
          v.length,
          v.map(_.a).sum,
          v.map(_.b).sum)).sortBy(_._1)
      ...
      .flatMap { _ =>
        ...
        val q4 =
          (for { (x, q) <- ts.groupBy(t => (t.a, t.b)) } yield (x, q.length))
            .sortBy(_._1)
        db.run(mark("q4", q4.result)).map {
          r4t: Seq[((Int, Option[Int]), Int)] =>
            r4t shouldBe Vector(
              ((1, Some(1)), 1),
              ((1, Some(2)), 1),
              ((1, Some(3)), 1),
        ...
        val q5 = ts.filter(_.a === 1).map(t => (t.a, t.b)).sortBy(_._2)
          .groupBy(x => (x._1, x._2)).map { case (a, _) => (a._1, a._2) }
          .to[Set]
        db.run(mark("q5", q5.result))
          .map(_ shouldBe Set((1, Some(1)), (1, Some(2)), (1, Some(3))))
      }.flatMap { _ => db.run(us += 4) }.flatMap { _ =>
        val q6 =
        ...
        val q8a = us.map(_.id.asColumnOf[String] ++ "test").groupBy(x => x)
          .map(_._2.max)
        val q8b =
        ...
        val q8d = us.map(_ => LiteralColumn("te") ++ "st").groupBy(x => x)
          .map(_._2.max)
        db.run(for {
        ...
        val q10 =
          ((for { m <- ts } yield m) groupBy (_.a) map {
            case (id, data) =>
  ...
  def testMultiMapAggregates = {
    ...
    class A(tag: Tag)
        extends Table[(Long, String, Long, Long)](tag, "a_multimap") {
      def id = column[Long]("id", O.PrimaryKey)
  ...
  def testFusedGroupBy = {
    ...
    val as = TableQuery[A]
    val q1 = as.map(t => t.value + LiteralColumn(1).bind).groupBy(identity)
      .map(_._1)
    val q2 = as.map(t => (t.value, t.value + LiteralColumn(1).bind))
      .groupBy(identity).map(_._1._2)
    val q3 = as.map(t => (t.value, t.value + LiteralColumn(1).bind))
      .groupBy(identity).map(_._1._1)
