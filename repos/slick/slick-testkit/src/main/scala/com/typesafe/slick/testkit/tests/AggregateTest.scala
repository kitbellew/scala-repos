...
class AggregateTest extends AsyncTest[RelationalTestDB] {
  ...
  def testAggregates = {
    ...
    val q2_0 = q2(0)
    val q2_1 = q2(1)
    ts.schema.create >> (ts ++= Seq((1, Some(1)), (1, Some(3)), (1, None))) >> q2_0.result
      .map(_ shouldBe (0, 0, 0, None, None, None)) >> q2_1.result.map(
      _ shouldBe (3, 3, 2, Some(3), Some(4), Some(2)))
  }
  ...
  def testIntLength = {
    ...
    val as = TableQuery[A]
    val q1 =
      as.groupBy(_.id).map { case (_, q) => (q.map(_.id).min, q.length) }
    DBIO.seq(
      as.schema.create,
      as += 1,
