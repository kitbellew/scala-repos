...
class AggregateTest extends AsyncTest[RelationalTestDB] {
  ...
  def testGroupBy = {
    ...
    db
      ...
      .flatMap { _ =>
        ...
        db
          .run(mark("q", q.result))
          .map { rt: Seq[(Int, Int, Option[Int], Option[Int])] =>
            rt shouldBe Vector(
              (1, 3, Some(3), Some(6)),
              (2, 3, Some(6), Some(8)),
              (3, 2, Some(6), Some(10)))
      ...
      .flatMap { _ =>
        ...
        db
          .run(mark("q2", q2.result))
          .map { r2t: Seq[(Int, Int, Option[Int], Option[Int])] =>
      ...
      .flatMap { _ =>
        ...
        db
          .run(mark("q3", q3.result))
          .map { r3t: Seq[(Int, Option[Int])] =>
      ...
      .flatMap { _ =>
        ...
        db
          .run(mark("q4", q4.result))
          .map { r4t: Seq[((Int, Option[Int]), Int)] =>
            r4t shouldBe Vector(
              ((1, Some(1)), 1),
              ((1, Some(2)), 1),
              ((1, Some(3)), 1),
  ...
  def testDistinct = {
    ...
    val q3a = as.distinct.map(_.id).filter(_ === 1) unionAll as
      .distinct
      .map(_.id)
      .filter(_ === 2)
