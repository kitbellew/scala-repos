...
class AggregateTest extends AsyncTest[RelationalTestDB] {
  ...
  def testGroupBy = {
    ...
    db.run {
      ...
      val q5 = ts
        ...
        .to[Set]
      db.run(mark("q5", q5.result)).map(
        _ shouldBe Set((1, Some(1)), (1, Some(2)), (1, Some(3))))
      ...
      db.run(mark("q10", q10.result)).map(
        _ shouldBe Set((2, Some(5.0)), (1, Some(3.0)), (3, Some(9.0))))
    }.flatMap { _ =>
  ...
  def testFusedGroupBy = {
    ...
    val as = TableQuery[A]
    val q1 =
      as.map(t => t.value + LiteralColumn(1).bind).groupBy(identity).map(_._1)
    val q2 = as.map(t => (t.value, t.value + LiteralColumn(1).bind)).groupBy(
      identity).map(_._1._2)
    val q3 = as.map(t => (t.value, t.value + LiteralColumn(1).bind)).groupBy(
      identity).map(_._1._1)
  ...
  def testDistinct = {
    ...
    val q3a = as.distinct.map(_.id).filter(_ === 1) unionAll as.distinct.map(
      _.id).filter(_ === 2)
    ...
    DBIO.seq(
      ...
      mark("q5b", q5b.result).map(_.sortBy(identity) should (r =>
        r == Seq(1, 3) || r == Seq(2, 3))),
      mark("q5c", q5c.result).map(_.sortBy(identity) should (r =>
