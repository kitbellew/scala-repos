...
class AggregateTest extends AsyncTest[RelationalTestDB] {
  ...
  def testGroupBy = {
    ...
    db.run {
      ...
      .flatMap { _ =>
        val q = (for { (k, v) <- ts.groupBy(t => t.a) } yield (
          k,
          v.length,
          v.map(_.a).sum,
      ...
      .flatMap { _ =>
        val q4 = (for { (x, q) <- ts.groupBy(t => (t.a, t.b)) } yield (
          x,
          q.length)).sortBy(_._1)
      ...
      .flatMap { _ =>
        val q10 = ((for { m <- ts } yield m) groupBy (_.a) map {
  ...
  def testGroup3 = {
    ...
    for {
      ...
      q1 = Tabs
        ...
        .map { case (grp, t) => (grp._1, grp._2, t.map(_.col4).sum) }
        .to[Set]
      _ <- q1.result.map(
        _ shouldBe Set(
          ("baz", "quux", Some(4)),
      ...
      q2 = Tabs
        .groupBy(t => ((t.col1, t.col2), t.col3))
        .map { case (grp, t) => (grp._1._1, grp._1._2, t.map(_.col4).sum) }
        .to[Set]
      _ <- q2.result.map(
        _ shouldBe Set(
          ("baz", "quux", Some(4)),
      ...
      q3 = Tabs
        .groupBy(_.col1)
        .map { case (grp, t) => (grp, t.map(x => x.col4 + x.col5).sum) }
        .to[Set]
  ...
  def testFusedGroupBy = {
    ...
    val q2 = as
      .map(t => (t.value, t.value + LiteralColumn(1).bind))
      .groupBy(identity)
      .map(_._1._2)
    val q3 = as
      .map(t => (t.value, t.value + LiteralColumn(1).bind))
