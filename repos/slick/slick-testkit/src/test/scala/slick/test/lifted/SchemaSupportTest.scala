...
class SchemaSupportTest {
  ...
  def testSchemaSupport {
    ...
    println(s1)
    assertTrue(
      "select ... from uses schema name",
      s1 contains """from "myschema"."mytable"""")
    ...
    println(s3)
    assertTrue(
      "update uses schema name",
      s3 contains """update "myschema"."mytable"""")

    val s4 = ts.filter(_.id < 5).delete.statements.head
    println(s4)
    assertTrue(
      "delete uses schema name",
      s4 contains """delete from "myschema"."mytable"""")

    val s5 = ts.schema.createStatements
    s5.foreach(println)
    s5.foreach(s =>
      assertTrue(
        "DDL (create) uses schema name",
