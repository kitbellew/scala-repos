      ...
      DBIO
        .seq(
          ...
          (
            ...
            X.map(r => {
              (r.`val`: Rep[Option[Int]]) == null
            })
            X.map(r => {
              (r.column: Rep[Int]) == null
            })
            X.map(r => {
              (r.schemaNameXX: Rep[Double]) == null
            })
            X.map(r => {
              (r.schemaNameX: Rep[String]) == null
            })
            X.map(r => {
              (r.index1: Rep[Option[Int]]) == null
            })
            X.map(r => {
              (r.posts: Rep[Option[Int]]) == null
            })
            X.map(r => {
              (r.pkX: PrimaryKey) == null
            })
            X.map(r => {
