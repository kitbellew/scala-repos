  ...
  @Test def test: Unit =
    runBlocking {
      ...
      DBIO.seq(
        ...
        Posts.length.result.zip(
          Posts.filter(_.title =!= "post 1").map(_.title).to[List].result).map(
          res => assertEquals((3, List("post 2", "post 3")), res)),
        sql"""select * from #${quoteIdentifier(
          "POSTS")} where #${quoteIdentifier("id")} = 2""".as[
          PostsRow].head.map(res =>
          assertEquals(PostsRow(2, "post 2", Some(1)), res)),
        ...
        SelfRef.result, {
          ...
          DBIO.seq(
            ...
            Large += oData2,
            sql"""select * from #${quoteIdentifier(
              "LARGE")} where #${quoteIdentifier("id")} = 0""".as[
              LargeRow].head.map(res => assertEquals(oData, res))
          )
        },
        (X.map(r =>
          (
