    ...
    };
    {
       ...
       */;
      {
        ...
        val (sqlRes, slickRes) = Await
          .result(db.run(sql zip slick), Duration.Inf)
      ...
      };
      {
        ...
        val slick =
          //#slickQueryGroupBy
          people.groupBy(p => p.addressId).map {
      ...
      };
      {
        ...
        val slick =
          ...
          people.groupBy(p => p.addressId).map {
            case (addressId, group) => (addressId, group.map(_.age).avg)
          }.filter { case (addressId, avgAge) => avgAge > 50 }.map(_._1).result
        //#slickQueryHaving
        val (sqlRes, slickRes) = Await
          .result(db.run(sql zip slick), Duration.Inf)
        assert(sqlRes == slickRes)
      ...
      };
      {
        ...
        val slick2 =
          ...
          (for (p <- people;
                a <- addresses if p.addressId === a.id) yield (p.name, a.city))
            .result
        //#slickQueryImplicitJoin
        val ((sqlRes, slickRes), slick2Res) = Await
          .result(db.run(sql zip slick zip slick2), Duration.Inf)
        assert(sqlRes == slickRes)
      ...
      };
      {
        ...
        val slick = {
          ...
          val address_ids = addresses.filter(_.city === "New York City")
            .map(_.id)
      ...
      };
      {
        ...
        val slickUpdate = {
          //#slickQueryUpdate
          people.filter(_.name === "M Odersky").map(p => (p.name, p.age))
        ...
        val (sqlInsertRes, slickInsertRes) = Await
          .result(db.run(sqlInsert zip slickInsert), Duration.Inf)
        assert(sqlInsertRes == slickInsertRes)
        val (sqlUpdateRes, slickUpdateRes) = Await
          .result(db.run(sqlUpdate zip slickUpdate), Duration.Inf)
        assert(sqlUpdateRes == 2)
        assert(slickUpdateRes == 0)
        val (sqlDeleteRes, slickDeleteRes) = Await
          .result(db.run(sqlDelete zip slickDelete), Duration.Inf)
      ...
      };
      {
        ...
        val slick =
          //#slickCase
          people.map(p =>
            Case If (p.addressId === 1) Then "A" If (p.addressId === 2) Then
              "B").result
        //#slickCase
        val (sqlRes, slickRes) = Await
          .result(db.run(sql zip slick), Duration.Inf)
