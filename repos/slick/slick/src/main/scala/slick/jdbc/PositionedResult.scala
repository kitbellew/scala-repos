...
abstract class PositionedResult(val rs: ResultSet) extends Closeable {
  ...
  final def updateBigDecimalOption(v: Option[BigDecimal]) {
    ...
    v match {
      case Some(s) =>
        rs.updateBigDecimal(npos, s.bigDecimal);
      case None =>
        rs.updateNull(npos)
    };
    pos = npos
