...
class LoggingStatement(st: Statement) extends Statement {
  ...
  @inline protected[this] def logged[T](sql: String, what: String = "statement")(
      f: => T) = {
  ...
  def executeUpdate(sql: String): Int =
    logged(sql, "update") { st.executeUpdate(sql) }
...
class LoggingPreparedStatement(st: PreparedStatement)
  ...
  def executeQuery(): java.sql.ResultSet =
    logged(null, "prepared query") { st.executeQuery() }
  ...
  def setTimestamp(
      ...
      cal: java.util.Calendar): Unit = st.setTimestamp(idx, value, cal)
