...
trait JdbcStatementBuilderComponent {
  ...
  def createColumnDDLBuilder(
      column: FieldSymbol,
      table: Table[_]): ColumnDDLBuilder = new ColumnDDLBuilder(column)
  ...
  class JdbcCompiledInsert(source: Node) {
    ...
    def buildReturnColumns(node: Node): (
        ConstArray[String],
      ...
      if (ibr.table.baseIdentity != standardInsert.table.baseIdentity)
        throw new SlickException(
          "Returned key columns must be from same table as inserted columns (" +
            ibr.table.baseIdentity + " != " + standardInsert
            .table
            .baseIdentity + ")")
      val returnOther = ibr.fields.length > 1 || !ibr
        .fields
        .head
        .options
  ...
  def valueToSQLLiteral(v: Any, tpe: Type): String = {
    ...
    if (option)
      v.asInstanceOf[Option[Any]].fold("null")(ti.valueToSQLLiteral)
    else
      ti.valueToSQLLiteral(v)
  }
  ...
  class QueryBuilder(val tree: Node, val state: CompilerState) {
    ...
    protected def buildComprehension(c: Comprehension): Unit = {
      ...
      if (limit0)
        ...
        buildWhereClause(and(c.where.toSeq ++ on))
    ...
    private[this] def and(ns: Seq[Node]): Option[Node] =
      if (ns.isEmpty)
        ...
        Some(ns.reduceLeft((p1, p2) => Library.And.typed[Boolean](p1, p2)))
    ...
    protected def buildFromClause(from: Seq[(TermSymbol, Node)]) =
      building(FromPart) {
        from match {
    ...
    protected def buildOrderByClause(order: ConstArray[(Node, Ordering)]) =
      building(OtherPart) {
        if (!order.isEmpty) {
          b"\norder by "
          b.sep(order, ", ") {
      ...
      n match {
        ...
        case n: Apply =>
          n match {
            ...
            case n =>
              throw new SlickException(
                "Unexpected function call " + n + " -- SQL prefix: " + b
                  .build
                  .sql)
          }
        case c: IfThenElse =>
  ...
  class InsertBuilder(val ins: Insert) {
    ...
    protected lazy val allVars = syms
      .iterator
      .map(_ => "?")
      .mkString("(", ",", ")")
    ...
    protected def buildInsertStart: String =
      allNames.iterator.mkString(s"insert into $tableName (", ",", ") ")

    protected def emptyInsert: String =
      if (allFields.isEmpty)
        s"insert into $tableName default values"
      else
  ...
  class UpsertBuilder(ins: Insert) extends InsertBuilder(ins) {
    protected lazy val (pkSyms, softSyms) = syms
      .toSeq
  ...
  class TableDDLBuilder(val table: Table[_]) {
    self =>
    protected val tableNode = table
      .toNode
      .asInstanceOf[TableExpansion]
      .table
      .asInstanceOf[TableNode]
    protected val columns: Iterable[ColumnDDLBuilder] = table
      .create_*
    ...
    protected def addForeignKeyColumnList(
        columns: IndexedSeq[Node],
        sb: StringBuilder,
        requiredTableName: String) =
      addColumnList(columns, sb, requiredTableName, "foreign key constraint")

    protected def addPrimaryKeyColumnList(
        columns: IndexedSeq[Node],
        sb: StringBuilder,
        requiredTableName: String) =
      addColumnList(columns, sb, requiredTableName, "foreign key constraint")
  ...
  class ColumnDDLBuilder(column: FieldSymbol) {
    ...
    protected def init() {
      for (o <- column.options)
        handleColumnOption(o)
      if (sqlType ne null) {
