...
trait JdbcStatementBuilderComponent { self: JdbcProfile =>
  ...
  class JdbcCompiledInsert(source: Node) {
    ...
    def buildReturnColumns(node: Node): (
      ...
      val returnOther =
        ibr.fields.length > 1 || !ibr.fields.head.options.contains(
  ...
  class QueryBuilder(val tree: Node, val state: CompilerState) { queryBuilder =>
    ...
    protected def buildComprehension(c: Comprehension): Unit = {
      ...
      def containsSymbolInSubquery(s: TermSymbol) =
        c.children.iterator.drop(1).flatMap(_.collect {
          case c: Comprehension => c
        }.toSeq.flatMap(_.findNode(_ == Ref(s)))).nonEmpty
    ...
    def expr(n: Node, skipParens: Boolean = false): Unit =
      n match {
        case p @ Path(path) =>
          val (base, rest) =
            path.foldRight[(Option[TermSymbol], List[TermSymbol])](
              (None, Nil)) {
          ...
          n match {
            ...
            case Library.Substring(n, start, end) =>
              b"\({fn substring($n, ${QueryParameter.constOp[Int]("+")(_ + _)(
                start,
                LiteralNode(1).infer())}, ${QueryParameter.constOp[Int]("-")(
                _ - _)(end, start)})}\)"
  ...
  class UpsertBuilder(ins: Insert) extends InsertBuilder(ins) {
    ...
    override def buildInsert: InsertBuilderResult = {
      ...
      val paramSel = "select " + allNames.map(n =>
        "? as " + n).iterator.mkString(",") + scalarFrom.map(n =>
        " from " + n).getOrElse("")
  ...
  class CheckInsertBuilder(ins: Insert) extends UpsertBuilder(ins) {
    override def buildInsert: InsertBuilderResult =
      new InsertBuilderResult(
        table,
        pkNames.map(n => s"$n=?").mkString(
          s"select 1 from $tableName where ",
          " and ",
          ""),
