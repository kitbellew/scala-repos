...
class JdbcModelBuilder(mTables: Seq[MTable], ignoreInvalidDefaults: Boolean)(
  ...
  def createBuilders(tablesByQName: Map[MQName, TableBuilder]) = new Builders(
    tablesByQName)
  ...
  class TableBuilder(
    ...
    // models
    def buildModel(builders: Builders) = m.Table(
      namer.qualifiedName,
      columns,
      primaryKey,
  ...
  class ColumnBuilder(tableBuilder: TableBuilder, meta: MColumn) {
    ...
    def model = m.Column(
      ...
      options = Set() ++
        ...
        length.map(
          RelationalProfile.ColumnOption.Length.apply(_, varying = varying)) ++
