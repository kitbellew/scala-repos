...
trait JdbcProfile
  ...
  override protected def computeCapabilities =
    super.computeCapabilities ++ JdbcCapabilities.all
  ...
  final def buildTableSchemaDescription(table: Table[_]): DDL =
    createTableDDLBuilder(table).buildDDL
  ...
  trait API extends LowPriorityAPI with super.API with ImplicitColumnTypes {
    ...
    implicit def jdbcActionExtensionMethods[E <: Effect, R, S <: NoStream](
        a: DBIOAction[R, S, E]): JdbcActionExtensionMethods[E, R, S] =
  ...
  def runSynchronousQuery[R](tree: Node, param: Any)(implicit
    ...
    tree match {
      case rsm @ ResultSetMapping(
        ...
        createQueryInvoker[Any](rsm, param, null)
          .foreach({ x => b += x }, 0)(session)
        b.result()
