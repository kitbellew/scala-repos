...
trait StreamingInvokerAction[R, T, -E <: Effect]
  ...
  override final def cancelStream(
      ctx: JdbcBackend#StreamingContext,
      state: StreamState): Unit = state.close()

  override def getDumpInfo =
    super.getDumpInfo.copy(name = "StreamingResultAction")

  final def head: FixedSqlAction[T, NoStream, E] = new HeadAction(statements)

  final def headOption: FixedSqlAction[Option[T], NoStream, E] =
    new HeadOptionAction(statements)
  ...
  private[this] class HeadOptionAction(val statements: Iterable[String])
    ...
    def run(ctx: JdbcBackend#Context): Option[T] =
      createInvoker(statements).firstOption(ctx.session)
    def overrideStatements(_statements: Iterable[String]) =
      new HeadOptionAction(_statements)
  }
