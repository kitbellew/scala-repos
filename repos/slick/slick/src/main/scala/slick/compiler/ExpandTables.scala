...
class ExpandTables extends Phase {
  ...
  def apply(state: CompilerState) = {
      ...
      ClientSideOp.mapServerSide(n) { tree =>
        ...
        if (!tree.nodeType.existsType {
          ...
          val mapping = createResult(
            tables,
            Ref(sym),
            tree3.nodeType.asCollectionType.elementType).infer(Type.Scope(
    ...
    if (createdOption)
      s2 + (Phase.assignUniqueSymbols -> state.get(Phase.assignUniqueSymbols)
        .get.copy(nonPrimitiveOption = true))
    else s2
  }
}
