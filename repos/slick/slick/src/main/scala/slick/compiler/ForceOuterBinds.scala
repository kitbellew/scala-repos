...
class ForceOuterBinds extends Phase {
  ...
  def idBind(n: Node): Bind = {
    ...
    n match {
      case p: Pure =>
        Bind(gen, Pure(ProductNode(ConstArray.empty)), p)
      case _ =>
    ...
    n match {
      ...
      case f: FilteredQuery =>
        f.mapChildren { ch =>
          if ((ch eq f.from) &&
              !(
                ch.isInstanceOf[Join] || ch.isInstanceOf[Distinct] ||
                  ch.isInstanceOf[Pure]
              ))
            nowrap(ch)
          else
            maybewrap(ch)
      ...
      case Path(path) =>
        Path(path) // recreate untyped copy
      case n =>
        n.mapChildren(maybewrap)
    }

  def maybewrap(n: Node): Node =
    n match {
      case _: Join | _: Pure | _: Union | _: FilteredQuery | _: TableNode =>
        wrap(n)
      case _ =>
        nowrap(n)
    }
