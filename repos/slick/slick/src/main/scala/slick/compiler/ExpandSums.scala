...
class ExpandSums extends Phase {
    ...
    def tr(
      ...
      val tree3 =
        tree2 match {
          ...
          case n @ OptionApply(_) :@ OptionType.Primitive(_) =>
            n
    ...
    def extend(side: Node, sym: TermSymbol, on: Node): (Node, Node, Boolean) = {
      ...
      val (disc, createDisc) =
        findDisc(elemType) match {
          ...
          case None =>
            logger
              .debug("No suitable discriminator column found in " + elemType)
            (Disc1, false)
        }
  ...
  def toOptionColumns(tpe: Type): Type =
    tpe match {
      case NominalType(_, str) =>
        toOptionColumns(str)
      case o @ OptionType(ch) if ch.structural.isInstanceOf[AtomicType] =>
        o
      case t: AtomicType =>
        OptionType(t)
      case t =>
        t.mapChildren(toOptionColumns)
    }
  ...
  def expandConditionals(n: Node): Node = {
    ...
    def tr(n: Node): Node =
      n.mapChildren(tr, keepType = true) match {
        ...
        case Select(ProductNode(ch), ElementSymbol(idx)) =>
          ch(idx - 1)
        case Select(StructNode(ch), sym) =>
          ch.find(_._1 == sym).get._2
