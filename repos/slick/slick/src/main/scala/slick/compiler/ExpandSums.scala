...
class ExpandSums extends Phase {
  ...
  def apply(state: CompilerState) =
    if (state.get(Phase.assignUniqueSymbols).map(
          _.nonPrimitiveOption).getOrElse(true)) state.map(expandSums)
    else state
      ...
      val tree3 = tree2 match {
        ...
        case n @ OptionApply(ch) =>
          ...
          ProductNode(
            ConstArray(
              Disc1,
              silentCast(toOptionColumns(ch.nodeType), ch))).infer()
      ...
      n.mapChildren(tr, keepType = true) match {
        ...
        case cast @ Library.SilentCast(ch) :@ Type.Structural(
          ...
          val elems = typeCh.zipWithIndex.map {
            case (t, idx) =>
              tr(
                Library.SilentCast.typed(
                  t,
                  ch.select(ElementSymbol(idx + 1))).infer())
          }
        ...
        case Library.SilentCast(
              LiteralNode(None)) :@ (tpe @ OptionType.Primitive(_)) =>
