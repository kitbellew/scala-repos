...
class RewriteJoins extends Phase {
          ...
          if (m.isEmpty) sel2
            ...
            sel2.replace {
              case p @ FwdPath(r1 :: rest) if r1 == sn && m.contains(rest) =>
                m(rest)
              case r @ Ref(s) if (oj4 ne oj2) && s == sn =>
  ...
  def eliminateIllegalRefs(
    ...
    logger.debug(
      "Trying to eliminate illegal refs [" + illegal.mkString(", ") + "] from:",
      j)
    ...
    def pullOut(
      ...
      if (illegalDefs.isEmpty) (sn, Map.empty)
        ...
        val createDefs = (requiredOkPaths -- existingOkDefs.keySet).map(p =>
          (new AnonSymbol, p)).toMap
  ...
  def rearrangeJoinConditions(j: Join): Join =
    rearrangeJoinConditions(j, Set.empty)
