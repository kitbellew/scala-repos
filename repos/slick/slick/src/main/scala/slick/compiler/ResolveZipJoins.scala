...
class ResolveZipJoins(rownumStyle: Boolean = false) extends Phase {
  ...
  def transformZip(
    ...
    val l2 = transformZipWithIndex(
      ...
      Pure(StructNode(
        ldefs.map {
          case (f, _) => (lmap(f) -> FwdPath(List(l2sym, ElementSymbol(1), f)))
        } :+
    ...
    val r2 = transformZipWithIndex(
      ...
      Pure(StructNode(
        rdefs.map {
          case (f, _) => (rmap(f) -> FwdPath(List(r2sym, ElementSymbol(1), f)))
        } :+
    ...
    Bind(
      ...
      j2,
      sel.replace {
        case FwdPath(Seq(s, ElementSymbol(1), f))
            if s == s1 => FwdPath(List(s, ElementSymbol(1), lmap(f)))
