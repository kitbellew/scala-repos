...
class HoistClientOps extends Phase {
    ...
    state.map(ClientSideOp.mapResultSetMapping(_) { rsm =>
      ...
      from1 match {
        case Bind(s2, from2, Pure(StructNode(defs2), ts2)) =>
          ...
          logger.debug(
            "Hoisting operations from defs: " + hoisted.iterator.filter(t =>
              t._2 ne t._3._1).map(_._1).mkString(", "))
  ...
  def shuffle(n: Node): Node =
    n match {
          ...
          case from2 @ Join(
            ...
            val (
              ...
              if (jt != JoinType.Right) {
                ...
                logger.debug(
                  "Hoisting operations from defs in left side of Join: " + hoisted.iterator.filter(
                    t => t._2 ne t._3._1).map(_._1).mkString(", "))
                val newDefsM = hoisted.iterator.map {
                  case (ts, n, (n2, wrap)) => (n2, new AnonSymbol)
                }.toMap
                logger.debug("New defs: " + newDefsM)
            ...
            val (
              ...
              if (jt != JoinType.Left) {
                ...
                logger.debug(
                  "Hoisting operations from defs in right side of Join: " + hoisted.iterator.filter(
                    t => t._2 ne t._3._1).map(_._1).mkString(", "))
                val newDefsM = hoisted.iterator.map {
                  case (ts, n, (n2, wrap)) => (n2, new AnonSymbol)
                }.toMap
            ...
            if ((bl2 ne bl) || (br2 ne br)) {
              ...
              val sel2 = sel1.replace {
                ...
                case Select(Select(Ref(s), ElementSymbol(2)), f)
                    if s == s1 && (br2 ne br) =>
                  val (wrap, f2) = rrepl(f)
      ...
      case n @ CollectionCast(from1 :@ CollectionType(cons1, _), cons2)
          if !cons1.isUnique || cons2.isUnique =>
        shuffle(from1) match {
