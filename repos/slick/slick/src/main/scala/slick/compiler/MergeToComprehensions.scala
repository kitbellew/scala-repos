...
class MergeToComprehensions extends Phase {
  ...
  def convert(tree: Node): Node = {
    ...
    def mergeGroupBy(
      ...
      n match {
        case Bind(s1, GroupBy(s2, f1, b1, ts1), Pure(str1, ts2)) =>
          ...
          val replacements = defs2.iterator.map { case (f, _) => (ts2, f) -> f }
            .toMap
    ...
    def createSource(n: Node): Option[(Node, Mappings)] =
      n match {
        case t: TableNode =>
          ...
          val mappings = ConstArray
            .from(tableFields.getOrElse(t.identity, Seq.empty).map(f =>
        ...
        case j @ Join(ls, rs, l1, r1, jt, on1) =>
          ...
          if (noLeft && noCondition) { Some((r2, rmap)) }
          else if (noRight && noCondition) { Some((l2, lmap)) }
          else {
            val mappings =
    ...
    def createTopLevel(n: Node): (Node, Mappings) =
      n match {
    ...
    def convert1(n: Node): Node =
      n match {
        ...
        case a: Aggregate =>
          logger.debug(
            "Merging Aggregate into Comprehension:",
            Ellipsis(a, List(0)))
  ...
  def mergeCommon(
      ...
      n: Node,
      buildBase: Boolean,
      allowFilter: Boolean = true): (Comprehension, Replacements) =
    n match {
      case Bind(s1, f1, Pure(StructNode(defs1), ts1))
        ...
        val replacements = defs2.iterator.map { case (f, _) => (ts1, f) -> f }
          .toMap
        logger.debug("Replacements are: " + replacements)
        (c2, replacements)
  ...
  def dealias(n: Node)(f: Node => (Node, Mappings)): (Node, Mappings) = {
    ...
    n match {
      case Bind(_, _: GroupBy, _) =>
        f(n) // we need this Bind for the GroupBy transformation
