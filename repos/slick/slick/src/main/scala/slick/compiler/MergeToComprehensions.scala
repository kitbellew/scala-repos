...
class MergeToComprehensions extends Phase {
  ...
  def convert(tree: Node): Node = {
    ...
    def mergeSortBy(
      ...
      case Distinct(s1, f1, o1) =>
        ...
        val c2 =
          c1a.copy(
            distinct = Some(ProductNode(ConstArray(o2)).flatten
              .infer())) :@ c1a.nodeType
    ...
    def mergeGroupBy(
      ...
      case Bind(s1, GroupBy(s2, f1, b1, ts1), Pure(str1, ts2)) =>
        ...
        logger
          .debug("Merging GroupBy into Comprehension:", Ellipsis(n, List(0, 0)))
        ...
        val replacements =
          defs2.iterator.map { case (f, _) => (ts2, f) -> f }.toMap
    ...
    def createSource(n: Node): Option[(Node, Mappings)] = n match {
      ...
      case j @ Join(ls, rs, l1, r1, jt, on1) =>
        ...
        if (noLeft && noCondition) {
          ...
          val mappings =
            lmap.map { case (key, ss) => (key, ElementSymbol(1) :: ss) } ++ rmap
              .map {
  ...
  def mergeCommon(
    ...
    case Bind(s1, f1, Pure(StructNode(defs1), ts1))
      ...
      val replacements =
        defs2.iterator.map { case (f, _) => (ts1, f) -> f }.toMap
      logger.debug("Replacements are: " + replacements)
      (c2, replacements)
