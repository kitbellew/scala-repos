...
case object UnassignedType extends AtomicType {
  def classTag =
    throw new SlickException("UnassignedType does not have a ClassTag")
}
...
trait TypedType[T] extends Type {
  self =>
  def optionType: OptionTypedType[T] =
    new OptionTypedType[T] {
...
object TypedType {
  @inline
  implicit def typedTypeToOptionTypedType[T](implicit
    ...
    new scala.math.Ordering[T] {
      def compare(x: T, y: T): Int = {
        if ((x.asInstanceOf[AnyRef] eq null) &&
            (y.asInstanceOf[AnyRef] eq null)) 0
...
object ScalaBaseType {
  ...
  private[this] val all: Map[ClassTag[_], ScalaBaseType[_]] = Seq(
    booleanType,
    bigDecimalType,
    byteType,
    charType,
