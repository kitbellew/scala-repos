...
object BeanConfigurator extends Logging {
  def configure(
    ...
    val pds =
      Introspector.getBeanInfo(o.getClass).getPropertyDescriptors.iterator.map(
        pd => (pd.getName, pd)).toMap
  ...
  private def toInt(o: Any): java.lang.Integer =
    ConfigFactory.parseMap(Map("v" -> o.toString).asJava).getInt("v")
  private def toLong(o: Any): java.lang.Long =
    ConfigFactory.parseMap(Map("v" -> o.toString).asJava).getLong("v")
  private def toBoolean(o: Any): java.lang.Boolean =
    ConfigFactory.parseMap(Map("v" -> o.toString).asJava).getBoolean("v")
}
