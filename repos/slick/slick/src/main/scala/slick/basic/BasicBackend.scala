            ...
            def run: Unit =
              try {
                ...
                if (debug) {
                  if (state ne null)
                    streamLogger
                      .debug("Suspending streaming action with continuation (more data available)")
  ...
  protected[this] class BasicStreamingActionContext(
    ...
    def restartStreaming: Unit = {
      ...
      if (s ne null) {
        ...
        if (streamLogger.isDebugEnabled)
          streamLogger.debug(
            "Scheduling stream continuation after transition from demand = 0")
        ...
        if (streamLogger.isDebugEnabled)
          streamLogger
            .debug("Saw transition from demand = 0, but no stream continuation available")
      }
    }
