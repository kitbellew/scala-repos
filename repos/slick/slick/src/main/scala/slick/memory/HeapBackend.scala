...
trait HeapBackend extends RelationalBackend with Logging {
  ...
  def createEmptyDatabase: Database = {
    def err = throw new SlickException(
      "Unsupported operation for empty heap database")
  ...
  class DatabaseFactoryDef {
    ...
    /** Create a new heap database instance that uses the supplied ExecutionContext for
      ...
      */
    def apply(executionContext: ExecutionContext): Database = new DatabaseDef(
      executionContext)
  }
  ...
  class HeapTable(
    ...
    val verifier = {
      val v1 = indexes.foldLeft(Verifier.empty) { case (z, i) =>
        z andThen createIndexVerifier(i)
      }
      val v2 = constraints.foldLeft(v1) { case (z, c) =>
