...
class QueryInterpreter(db: HeapBackend#Database, params: Any) extends Logging {
  override protected[this] lazy val logger = new SlickLogger(
    LoggerFactory.getLogger(classOf[QueryInterpreter]))
    ...
    val res = n match {
      ...
      case Select(in, field) =>
        ...
        field match {
          ...
          case (_: AnonSymbol | _: FieldSymbol) =>
      ...
      case Bind(gen, from, sel) =>
        ...
        val b =
          from.nodeType.asCollectionType.cons.iterableSubstitute.createBuilder[
            Any]
        fromV.foreach { v =>
          scope(gen) = v
      ...
      case c: IfThenElse =>
        ...
        take match {
          ...
          case _ =>
            ...
            if (opt && !c.elseClause.nodeType.asInstanceOf[
                  ScalaType[_]].nullable) Option(res)
            else res
        }
      ...
      case Library.SilentCast(ch) =>
        ...
        (ch.nodeType, n.nodeType) match {
          case (OptionType(tpe), tpe2) if tpe == tpe2 =>
      ...
      case Library.IfNull(cond, default) =>
        ...
        if ((condV.asInstanceOf[AnyRef] eq null) || condV == None) {
          ...
          if (n.nodeType.isInstanceOf[
                OptionType] && !default.nodeType.isInstanceOf[OptionType])
            Some(defaultV)
          else defaultV
        } else if (n.nodeType.isInstanceOf[
                     OptionType] && !cond.nodeType.isInstanceOf[OptionType])
          Some(condV)
        else condV
      case Library.In(what, where) =>
        ...
        if (whatOpt && (whatV.asInstanceOf[AnyRef].eq(null) || whatV == None))
          None
        else {
          val whatBase =
        ...
        if (n.nodeType.isInstanceOf[OptionType]) {
          if (chV.exists {
            ...
            val chPlainV = chV.map {
              case (t: OptionType, v) =>
                (t.elementType, v.asInstanceOf[Option[Any]].get)
              case other => other
            }
  ...
  def evalFunction(sym: TermSymbol, args: Seq[(Type, Any)], retType: Type) =
    sym match {
      case Library.== => args(0)._2 == args(1)._2
      case Library.< =>
        args(0)._1.asInstanceOf[ScalaBaseType[Any]].ordering.lt(
          args(0)._2,
          args(1)._2)
      case Library.<= =>
        args(0)._1.asInstanceOf[ScalaBaseType[Any]].ordering.lteq(
          args(0)._2,
          args(1)._2)
      case Library.> =>
        args(0)._1.asInstanceOf[ScalaBaseType[Any]].ordering.gt(
          args(0)._2,
          args(1)._2)
      case Library.>= =>
        args(0)._1.asInstanceOf[ScalaBaseType[Any]].ordering.gteq(
          args(0)._2,
          args(1)._2)
      case Library.+ =>
        args(0)._1.asInstanceOf[ScalaNumericType[Any]].numeric.plus(
          args(0)._2,
          args(1)._2)
      case Library.- =>
        args(0)._1.asInstanceOf[ScalaNumericType[Any]].numeric.minus(
          args(0)._2,
          args(1)._2)
      case Library.* =>
        args(0)._1.asInstanceOf[ScalaNumericType[Any]].numeric.times(
          args(0)._2,
          args(1)._2)
      case Library.% =>
        args(0)._1.asInstanceOf[ScalaNumericType[Any]].numeric.asInstanceOf[
          Integral[Any]].rem(args(0)._2, args(1)._2)
      case Library.Abs =>
      ...
      case Library.Sign =>
        args(0)._1.asInstanceOf[ScalaNumericType[Any]].numeric.signum(
          args(0)._2)
      ...
      case Library.Substring if args.size == 3 =>
        args(0)._2.asInstanceOf[String].substring(
      ...
      case Library.StartsWith =>
        args(0)._2.asInstanceOf[String].startsWith(
          args(1)._2.asInstanceOf[String])
      case Library.EndsWith =>
        args(0)._2.asInstanceOf[String].endsWith(
          args(1)._2.asInstanceOf[String])
    }

  def unwrapSingleColumn(coll: Coll, tpe: Type): (Iterator[Any], Type) =
    tpe.asCollectionType.elementType match {
      case ProductType(ConstArray(t)) =>
        (coll.iterator.map(_.asInstanceOf[ProductValue](0)), t)
      case StructType(ConstArray((_, t))) =>
        (coll.iterator.map(_.asInstanceOf[StructValue](0)), t)
      case t => (coll.iterator, t)
    }

  def reduceOptionIt[T](
      it: Iterator[Any],
      opt: Boolean,
      map: Any => T,
...
object QueryInterpreter {
  ...
  class StructValue(data: IndexedSeq[Any], symbolToIndex: (TermSymbol => Int))
      extends ProductValue(data) {
    def getBySymbol(sym: TermSymbol): Any = apply(symbolToIndex(sym))
    override def toString =
      "StructValue(" + data.mkString(", ") + "){" + symbolToIndex + "}"
  }
}
