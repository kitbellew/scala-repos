...
class QueryInterpreter(db: HeapBackend#Database, params: Any) extends Logging {
  override protected[this] lazy val logger =
    new SlickLogger(LoggerFactory.getLogger(classOf[QueryInterpreter]))
    ...
    val res =
      n match {
        ...
        case Select(in, field) =>
          ...
          field match {
            case ElementSymbol(idx) =>
              v.asInstanceOf[ProductValue].apply(idx - 1)
        ...
        case Filter(gen, from, where) =>
          ...
          res
        case First(ch) =>
          run(ch).asInstanceOf[Coll].toIterator.next()
        ...
        case Library.SilentCast(ch) =>
          ...
          (ch.nodeType, n.nodeType) match {
            ...
            case (tpe, OptionType(tpe2)) if tpe == tpe2 =>
              Option(chV)
          }
          ...
          val (num, opt) =
            itType match {
              case t: ScalaOptionType[_] =>
                (
                  t.elementType.asInstanceOf[ScalaNumericType[Any]].numeric,
                  true)
              case t =>
                (t.asInstanceOf[ScalaNumericType[Any]].numeric, false)
            }
        ...
        case Library.Min(ch) =>
          ...
          val (ord, opt) =
            itType match {
              ...
              case t =>
                (t.asInstanceOf[ScalaBaseType[Any]].ordering, false)
            }
          reduceOptionIt[Any](
            it,
            opt,
    ...
    sym match {
      ...
      case Library.Cast =>
        ...
        (args(0)._1, retType) match {
          case (a, b) if a == b =>
            v
          case (_, ScalaBaseType.stringType) =>
            v.toString
          case (_, ScalaBaseType.intType) =>
            v.toString.toInt
          case (_, ScalaBaseType.longType) =>
            v.toString.toLong
          case (_, ScalaBaseType.byteType) =>
            v.toString.toByte
          case (_, ScalaBaseType.shortType) =>
            v.toString.toShort
          case (_, ScalaBaseType.doubleType) =>
            v.toString.toDouble
          case (_, ScalaBaseType.floatType) =>
            v.toString.toFloat
          case (_, ScalaBaseType.bigDecimalType) =>
      ...
      case Library.Concat =>
        args.iterator.map(_._2.toString).mkString
      case Library.CountAll =>
        args(0)._2.asInstanceOf[Coll].size
      case Library.Count =>
      ...
      case Library.LCase =>
        args(0)._2.asInstanceOf[String].toLowerCase
      case Library.Length =>
        args(0)._2.asInstanceOf[String].length
      case Library.Like =>
      ...
      case Library.LTrim =>
        ...
        if (start == 0)
          ...
          s.substring(start)
      case Library.Not =>
        !args(0)._2.asInstanceOf[Boolean]
      case Library.Or =>
        args(0)._2.asInstanceOf[Boolean] || args(1)._2.asInstanceOf[Boolean]
      case Library.Pi =>
        scala.math.Pi
      ...
      case Library.Sign =>
        args(0)
          ...
          .signum(args(0)._2)
      case Library.Trim =>
        args(0)._2.asInstanceOf[String].trim
      case Library.UCase =>
        args(0)._2.asInstanceOf[String].toUpperCase
      case Library.User =>
        ""
      ...
      case Library.Reverse =>
        args(0)._2.asInstanceOf[String].reverse
