...
trait MemoryQueryingProfile extends BasicProfile {
  ...
  class MemoryCodeGen
      extends CodeGen
      with ResultConverterCompiler[MemoryResultConverterDomain] {
    ...
    def compileServerSideAndMapping(
        serverSide: Node,
        mapping: Option[Node],
        state: CompilerState) = (serverSide, mapping.map(compileMapping))
    ...
    def transformSimpleGrouping(n: Node) =
      n match {
        case Bind(
              gen,
              g: GroupBy,
    ...
    override def compile(
        n: Node): ResultConverter[MemoryResultConverterDomain, _] =
      n match {
    ...
    class QueryResultConverter(ridx: Int, nullable: Boolean)
      ...
      override def getDumpInfo =
        super.getDumpInfo.copy(mainInfo = s"ridx=$ridx, nullable=$nullable")
      def width = 1
    }
  }
