...
trait MemoryProfile extends RelationalProfile with MemoryQueryingProfile {
  ...
  override protected def computeCapabilities =
    super.computeCapabilities ++ MemoryCapabilities.all

  def createInsertInvoker[T](tree: Node): InsertInvoker[T] =
    new InsertInvokerDef[T](tree)
  def buildSequenceSchemaDescription(seq: Sequence[_]): SchemaDescription = ??
  def buildTableSchemaDescription(table: Table[_]): SchemaDescription = new DDL(
    Vector(table))
    ...
    def headOption: ProfileAction[Option[T], NoStream, Effect.Read] =
      new ProfileAction[Option[T], NoStream, Effect.Read]
        ...
        def getDumpInfo = DumpInfo(
          "MemoryProfile.StreamingQueryAction.firstOption")
      }
  ...
  class InsertActionExtensionMethodsImpl[T](compiled: CompiledInsert)
      extends super.InsertActionExtensionMethodsImpl[T] {
  ...
  override def computeQueryCompiler =
    super.computeQueryCompiler ++ QueryCompiler.interpreterPhases
  ...
  class InsertMappingCompiler(insert: Insert)
    ...
    class InsertResultConverter(tidx: Int)
      ...
      def set(value: Any, pp: MemoryResultConverterDomain#Writer) = pp(tidx) =
        value
      override def getDumpInfo =
        super.getDumpInfo.copy(mainInfo = s"tidx=$tidx")
      def width = 1
    }
  }
