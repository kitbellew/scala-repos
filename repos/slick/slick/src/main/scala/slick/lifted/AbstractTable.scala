...
abstract class AbstractTable[T](
  ...
  def tableConstraints: Iterator[Constraint] =
    for {
      ...
      if m.getParameterTypes.length == 0 && classOf[
        Constraint].isAssignableFrom(m.getReturnType)
  ...
  final def foreignKeys: Iterable[ForeignKey] =
    ...
    }.flatten.toIndexedSeq.sortBy(_.name)

  final def primaryKeys: Iterable[PrimaryKey] =
    tableConstraints.collect { case k: PrimaryKey => k }.toIndexedSeq.sortBy(
      _.name)
