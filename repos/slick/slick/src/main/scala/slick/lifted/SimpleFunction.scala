...
trait SimpleFunction extends Node {
  ...
  override def getDumpInfo =
    super.getDumpInfo.copy(mainInfo = s"$name, $scalar")
}
...
object SimpleBinaryOperator {
  def apply[T: TypedType](fname: String): ((Rep[_], Rep[_]) => Rep[T]) = {
    def build(leftN: Node, rightN: Node): SimpleFeatureNode[T] =
      new SimpleFeatureNode[T] with SimpleBinaryOperator {
        val name = fname
        val left = leftN
        val right = rightN
    ...
    { (leftC: Rep[_], rightC: Rep[_]) =>
      Rep.forNode[T](build(leftC.toNode, rightC.toNode))
    }
  }
}
...
object SimpleLiteral {
  def apply[T](name: String)(implicit tpe: TypedType[T]) =
    Rep.forNode[T](new SimpleLiteral(name)(tpe))
}
...
object SimpleExpression {
  ...
  def unary[T1, R: TypedType](
      ...
      : (Rep[T1] => Rep[R]) = {
    val g =
      apply({ (ch: Seq[Node], qb: JdbcStatementBuilderComponent#QueryBuilder) =>
        f(ch(0), qb)
      });
    { t1: Rep[T1] => g(Seq(t1)) }
  }
