...
class CompiledFunction[F, PT, PU, R <: Rep[_], RU](
    val extract: F,
    val tuple: F => PT => R,
...
class AppliedCompiledFunction[PU, R <: Rep[_], RU](
    val param: PU,
    function: CompiledFunction[_, _, PU, R, RU],
    val profile: BasicProfile)
...
object StreamingExecutable extends StreamingExecutable[Rep[Any], Any, Any] {
  def apply[T <: Rep[_], TU, EU] =
    this.asInstanceOf[StreamingExecutable[T, TU, EU]]
