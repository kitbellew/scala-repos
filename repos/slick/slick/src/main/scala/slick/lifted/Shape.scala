...
object Shape
  ...
  implicit final def primitiveShape[T, Level <: ShapeLevel](implicit
    ...
    new Shape[Level, T, T, ConstColumn[T]] {
      ...
      def buildParams(extract: Any => Unpacked): Packed =
        new ConstColumn[T](new QueryParameter(extract, tm))(tm)
  ...
  val unitShapePrototype: Shape[FlatShapeLevel, Unit, Unit, Unit] =
    new Shape[FlatShapeLevel, Unit, Unit, Unit] {
...
trait AbstractTableShapeImplicits extends RepShapeImplicits {
  @inline implicit final def tableShape[
      ...
      T,
      C <: AbstractTable[_]](implicit ev: C <:< AbstractTable[T]) =
    RepShape[Level, C, T]
}
...
object RepShape extends Shape[FlatShapeLevel, Rep[_], Any, Rep[_]] {
  def apply[Level <: ShapeLevel, MP <: Rep[_], U]: Shape[Level, MP, U, MP] =
    this.asInstanceOf[Shape[Level, MP, U, MP]]

  def pack(value: Mixed): Packed = value
  def packedShape: Shape[FlatShapeLevel, Packed, Unpacked, Packed] = this
  def buildParams(extract: Any => Unpacked): Packed =
    throw new SlickException(
      "Shape does not have the same Mixed and Unpacked type")
...
abstract class ProductNodeShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C]
  ...
  def toNode(value: Mixed): Node = ProductNode(
    ConstArray.from(
      shapes.iterator
...
abstract class MappedProductShape[
  ...
  override def toNode(value: Mixed) = TypeMapping(
    super.toNode(value),
    MappedScalaType.Mapper(toBase, toMapped, None),
    classTag)
  def toBase(v: Any) = new ProductWrapper(
    getIterator(v.asInstanceOf[C]).toIndexedSeq)
  def toMapped(v: Any) = buildValue(
    TupleSupport.buildIndexedSeq(v.asInstanceOf[Product]))
...
abstract class MappedScalaProductShape[
    ...
    M <: C,
    U <: C,
    P <: C](implicit val classTag: ClassTag[U])
...
final class TupleShape[
  ...
  def copy(shapes: Seq[Shape[_ <: ShapeLevel, _, _, _]]) = new TupleShape(
    shapes: _*)
}
...
class CaseClassShape[
  ...
  def buildValue(elems: IndexedSeq[Any]) = mapLifted(
    TupleSupport.buildTuple(elems).asInstanceOf[LiftedTuple])
...
class ProductClassShape[E <: Product, C <: Product](
  ...
  override def toMapped(v: Any) = mapPlain(
...
case class ShapedValue[T, U](
  ...
  def packedValue[R](implicit
      ev: Shape[_ <: FlatShapeLevel, T, _, R]): ShapedValue[R, U] = ShapedValue(
    shape.pack(value).asInstanceOf[R],
    shape.packedShape.asInstanceOf[Shape[FlatShapeLevel, R, U, _]])
  def zip[T2, U2](s2: ShapedValue[T2, U2]) = new ShapedValue[(T, T2), (U, U2)](
    (value, s2.value),
    Shape.tuple2Shape(shape, s2.shape))
  def <>[R: ClassTag](f: (U => R), g: (R => Option[U])) =
    new MappedProjection[R, U](
      shape.toNode(value),
      MappedScalaType.Mapper(
        g.andThen(_.get).asInstanceOf[Any => Any],
...
class MappedProjection[T, P](
  ...
  def encodeRef(path: Node): MappedProjection[T, P] =
    new MappedProjection[T, P](child, mapper, classTag) {
...
object MappedProjection {
  ...
  @inline implicit final def mappedProjectionShape[
      Level >: FlatShapeLevel <: ShapeLevel,
      T,
      P] = RepShape[Level, MappedProjection[T, P], T]
}
