...
sealed abstract class Query[+E, U, C[_]] extends QueryBase[C[U]] { self =>
  ...
  def sortBy[T](f: E => T)(implicit ev: T => Ordered): Query[E, U, C] = {
    ...
    val aliased = shaped.encodeRef(Ref(generator))
    new WrappingQuery[E, U, C](
      SortBy(generator, toNode, ConstArray.from(f(aliased.value).columns)),
      shaped)
  }
  ...
  def take(num: ConstColumn[Long]): Query[E, U, C] =
    new WrappingQuery[E, U, C](Take(toNode, num.toNode), shaped)
  ...
  def drop(num: ConstColumn[Long]): Query[E, U, C] =
    new WrappingQuery[E, U, C](Drop(toNode, num.toNode), shaped)
