...
trait RelationalProfile
  ...
  override protected def computeCapabilities =
    super.computeCapabilities ++ RelationalCapabilities.all
  ...
  class FastPathExtensionMethods[M <: ResultConverterDomain, T, P](
    ...
    def fastPath(fpf: (TypeMappingResultConverter[M, T, _] =>
      SimpleFastPathResultConverter[M, T])): MappedProjection[T, P] =
      mp.genericFastPath {
...
trait RelationalTableComponent { self: RelationalProfile =>
  ...
  abstract class Table[T](
    ...
    def this(_tableTag: Tag, _tableName: String) =
      this(_tableTag, None, _tableName)

    def tableProvider: RelationalProfile = self

    def tableIdentitySymbol: TableIdentitySymbol =
      SimpleTableIdentitySymbol(self, schemaName.getOrElse("_"), tableName)
...
trait RelationalSequenceComponent { self: RelationalProfile =>
  ...
  class Sequence[T] private[Sequence] (
    ...
    def max(v: T) =
      new Sequence[T](name, _minValue, Some(v), _increment, _start, _cycle)
    def inc(v: T) =
      new Sequence[T](name, _minValue, _maxValue, Some(v), _start, _cycle)
    def start(v: T) =
      new Sequence[T](name, _minValue, _maxValue, _increment, Some(v), _cycle)
    def cycle =
      new Sequence[T](name, _minValue, _maxValue, _increment, _start, true)

    final def next = Library.NextValue.column[T](toNode)
    final def curr = Library.CurrentValue.column[T](toNode)

    def toNode =
      SequenceNode(name)(_increment.map(integral.toLong).getOrElse(1))
