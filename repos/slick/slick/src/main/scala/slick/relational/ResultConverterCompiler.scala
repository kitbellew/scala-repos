...
trait ResultConverterCompiler[Domain <: ResultConverterDomain] {
  ...
  def compile(n: Node): ResultConverter[Domain, _] =
    n match {
      ...
      case ProductNode(ch) =>
        if (ch.isEmpty)
          new UnitResultConverter
        else
          new ProductResultConverter(ch.map(n => compile(n)).toSeq: _*)
      case GetOrElse(ch, default) =>
        createGetOrElseResultConverter(
          compile(ch).asInstanceOf[ResultConverter[Domain, Option[Any]]],
          default)
  ...
  def createGetOrElseResultConverter[T](
      ...
      default: () => T): ResultConverter[Domain, T] =
  ...
  def createTypeMappingResultConverter(
      rc: ResultConverter[Domain, Any],
      mapper: MappedScalaType.Mapper): ResultConverter[Domain, Any] =
