...
trait BitraverseSyntax extends BitraverseSyntax1 {
  implicit def bitraverseSyntax[F[_, _]: Bitraverse, A, B](
      fab: F[A, B]): BitraverseOps[F, A, B] = new BitraverseOps[F, A, B](fab)
}
...
final class NestedBitraverseOps[F[_, _], G[_], A, B](fgagb: F[G[A], G[B]])(
    implicit F: Bitraverse[F]) {
  def bisequence(implicit G: Applicative[G]): G[F[A, B]] = F.bisequence(fgagb)
}
