...
final case class Kleisli[F[_], A, B](run: A => F[B]) {
  ...
  def dimap[C, D](f: C => A)(g: B => D)(implicit
      F: Functor[F]): Kleisli[F, C, D] = Kleisli(c => F.map(run(f(c)))(g))
  ...
  def flatMap[C](f: B => Kleisli[F, A, C])(implicit
      F: FlatMap[F]): Kleisli[F, A, C] =
  ...
  def andThen[C](k: Kleisli[F, B, C])(implicit
      F: FlatMap[F]): Kleisli[F, A, C] = this andThen k.run

  def compose[Z](f: Z => F[A])(implicit F: FlatMap[F]): Kleisli[F, Z, B] =
    Kleisli((z: Z) => F.flatMap(f(z))(run))

  def compose[Z](k: Kleisli[F, Z, A])(implicit
      F: FlatMap[F]): Kleisli[F, Z, B] = this compose k.run

  def traverse[G[_]](
      f: G[A])(implicit F: Applicative[F], G: Traverse[G]): F[G[B]] =
    G.traverse(f)(run)
  ...
  def transform[G[_]](f: F ~> G): Kleisli[G, A, B] = Kleisli(a => f(run(a)))
...
private[data] sealed abstract class KleisliInstances extends KleisliInstances0 {
  ...
  implicit def kleisliMonoid[F[_], A, B](implicit
      M: Monoid[F[B]]): Monoid[Kleisli[F, A, B]] =
    new KleisliMonoid[F, A, B] {
      def FB: Monoid[F[B]] = M
    }

  implicit def kleisliMonoidK[F[_]](implicit
      M: Monad[F]): MonoidK[Lambda[A => Kleisli[F, A, A]]] =
  ...
  implicit def kleisliChoice[F[_]](implicit
      ev: Monad[F]): Choice[Kleisli[F, ?, ?]] =
    new Choice[Kleisli[F, ?, ?]] {
      def id[A]: Kleisli[F, A, A] = Kleisli(ev.pure(_))

      def choice[A, B, C](
          f: Kleisli[F, A, C],
          g: Kleisli[F, B, C]): Kleisli[F, Xor[A, B], C] =
        Kleisli(_.fold(f.run, g.run))

      def compose[A, B, C](
          f: Kleisli[F, B, C],
          g: Kleisli[F, A, B]): Kleisli[F, A, C] = f.compose(g)
    }
  ...
  implicit def kleisliContravariant[F[_], C]: Contravariant[Kleisli[F, ?, C]] =
    new Contravariant[Kleisli[F, ?, C]] {
      override def contramap[A, B](fa: Kleisli[F, A, C])(
          f: (B) => A): Kleisli[F, B, C] = fa.local(f)
    }

  implicit def kleisliTransLift[M[_], A]: TransLift[
    ({
      type λ[α[_], β] = Kleisli[α, A, β]
    })#λ,
    M] =
    new TransLift[
      ({
...
private[data] sealed abstract class KleisliInstances0
  ...
  implicit def kleisliSplit[F[_]](implicit
      ev: FlatMap[F]): Split[Kleisli[F, ?, ?]] =
    new KleisliSplit[F] {
      def F: FlatMap[F] = ev
    }

  implicit def kleisliStrong[F[_]](implicit
      ev: Functor[F]): Strong[Kleisli[F, ?, ?]] =
    new KleisliStrong[F] {
      def F: Functor[F] = ev
    }
  ...
  implicit def kleisliFlatMap[F[_]: FlatMap, A]: FlatMap[Kleisli[F, A, ?]] =
    new FlatMap[Kleisli[F, A, ?]] {
      def flatMap[B, C](fa: Kleisli[F, A, B])(
          f: B => Kleisli[F, A, C]): Kleisli[F, A, C] = fa.flatMap(f)
  ...
  implicit def kleisliSemigroup[F[_], A, B](implicit
      M: Semigroup[F[B]]): Semigroup[Kleisli[F, A, B]] =
    new KleisliSemigroup[F, A, B] {
      def FB: Semigroup[F[B]] = M
    }

  implicit def kleisliSemigroupK[F[_]](implicit
      ev: FlatMap[F]): SemigroupK[Lambda[A => Kleisli[F, A, A]]] =
    new KleisliSemigroupK[F] {
      def F: FlatMap[F] = ev
    }
}
...
private[data] sealed abstract class KleisliInstances1
  ...
  implicit def kleisliApplicative[F[_]: Applicative, A]
    ...
    new Applicative[Kleisli[F, A, ?]] {
      def pure[B](x: B): Kleisli[F, A, B] = Kleisli.pure[F, A, B](x)

      def ap[B, C](f: Kleisli[F, A, B => C])(
          fa: Kleisli[F, A, B]): Kleisli[F, A, C] = fa(f)
...
private trait KleisliArrow[F[_]]
  ...
  def lift[A, B](f: A => B): Kleisli[F, A, B] = Kleisli(a => F.pure(f(a)))

  def id[A]: Kleisli[F, A, A] = Kleisli(a => F.pure(a))

  override def second[A, B, C](
      fa: Kleisli[F, A, B]): Kleisli[F, (C, A), (C, B)] =
...
private trait KleisliStrong[F[_]] extends Strong[Kleisli[F, ?, ?]] {
  ...
  override def lmap[A, B, C](fab: Kleisli[F, A, B])(
      f: C => A): Kleisli[F, C, B] = fab.local(f)

  override def rmap[A, B, C](fab: Kleisli[F, A, B])(
      f: B => C): Kleisli[F, A, C] = fab.map(f)

  override def dimap[A, B, C, D](fab: Kleisli[F, A, B])(f: C => A)(
      g: B => D): Kleisli[F, C, D] = fab.dimap(f)(g)
...
private trait KleisliSemigroupK[F[_]]
    extends SemigroupK[Lambda[A => Kleisli[F, A, A]]] {
  implicit def F: FlatMap[F]
