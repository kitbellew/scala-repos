...
final case class Cokleisli[F[_], A, B](run: F[A] => B) { self =>
  ...
  def compose[C](c: Cokleisli[F, C, A])(implicit
      F: CoflatMap[F]): Cokleisli[F, C, B] =
    Cokleisli(fc => run(F.coflatMap(fc)(c.run)))

  def andThen[C](c: Cokleisli[F, B, C])(implicit
      F: CoflatMap[F]): Cokleisli[F, A, C] = c compose this
...
object Cokleisli extends CokleisliInstances {
  def pure[F[_], A, B](x: B): Cokleisli[F, A, B] = Cokleisli(_ => x)
}

private[data] sealed abstract class CokleisliInstances
    extends CokleisliInstances0 {
  implicit def cokleisliArrow[F[_]](implicit
      ev: Comonad[F]): Arrow[Cokleisli[F, ?, ?]] =
  ...
  implicit def cokleisliMonad[F[_], A]: Monad[Cokleisli[F, A, ?]] =
    new Monad[Cokleisli[F, A, ?]] {
      def pure[B](x: B): Cokleisli[F, A, B] = Cokleisli.pure(x)

      def flatMap[B, C](fa: Cokleisli[F, A, B])(
          f: B => Cokleisli[F, A, C]): Cokleisli[F, A, C] = fa.flatMap(f)

      override def map[B, C](fa: Cokleisli[F, A, B])(
          f: B => C): Cokleisli[F, A, C] = fa.map(f)
    }

  implicit def cokleisliMonoidK[F[_]](implicit
      ev: Comonad[F]): MonoidK[Lambda[A => Cokleisli[F, A, A]]] =
...
private[data] sealed abstract class CokleisliInstances0 {
  implicit def cokleisliSplit[F[_]](implicit
      ev: CoflatMap[F]): Split[Cokleisli[F, ?, ?]] =
    new CokleisliSplit[F] {
      def F: CoflatMap[F] = ev
    }

  implicit def cokleisliProfunctor[F[_]](implicit
      ev: Functor[F]): Profunctor[Cokleisli[F, ?, ?]] =
    new CokleisliProfunctor[F] {
      def F: Functor[F] = ev
    }

  implicit def cokleisliSemigroupK[F[_]](implicit
      ev: CoflatMap[F]): SemigroupK[Lambda[A => Cokleisli[F, A, A]]] =
...
private trait CokleisliArrow[F[_]]
  ...
  def id[A]: Cokleisli[F, A, A] = Cokleisli(fa => F.extract(fa))

  def first[A, B, C](fa: Cokleisli[F, A, B]): Cokleisli[F, (A, C), (B, C)] =
    fa.first[C]

  override def second[A, B, C](
      fa: Cokleisli[F, A, B]): Cokleisli[F, (C, A), (C, B)] = fa.second[C]
...
private trait CokleisliSplit[F[_]] extends Split[Cokleisli[F, ?, ?]] {
  ...
  def compose[A, B, C](
      f: Cokleisli[F, B, C],
      g: Cokleisli[F, A, B]): Cokleisli[F, A, C] = f.compose(g)
...
private trait CokleisliProfunctor[F[_]] extends Profunctor[Cokleisli[F, ?, ?]] {
  ...
  def dimap[A, B, C, D](fab: Cokleisli[F, A, B])(f: C => A)(
      g: B => D): Cokleisli[F, C, D] = fab.dimap(f)(g)

  override def lmap[A, B, C](fab: Cokleisli[F, A, B])(
      f: C => A): Cokleisli[F, C, B] = fab.lmap(f)

  override def rmap[A, B, C](fab: Cokleisli[F, A, B])(
      f: B => C): Cokleisli[F, A, C] = fab.map(f)
}

private trait CokleisliSemigroupK[F[_]]
...
private trait CokleisliMonoidK[F[_]]
    extends MonoidK[Lambda[A => Cokleisli[F, A, A]]]
    with CokleisliSemigroupK[F] {
  implicit def F: Comonad[F]
