...
class XorTTests extends CatsSuite {
  ...
  test("withValidated") {
    forAll {
      (xort: XorT[List, String, Int], f: String => Char, g: Int => Double) =>
  ...
  test("transform consistent with value.map") {
    forAll {
      (xort: XorT[List, String, Int], f: String Xor Int => Long Xor Double) =>
  ...
  test("foldRight with Id consistent with Xor foldRight") {
    forAll {
      (
          x: XorT[Id, String, Int],
          l: Eval[Long],
          f: (Int, Eval[Long]) => Eval[Long]) =>
