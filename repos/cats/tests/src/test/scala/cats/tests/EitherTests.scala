...
class EitherTests extends CatsSuite {
  ...
  checkAll(
    "Either[Int, Int]",
    CartesianTests[Either[Int, ?]].cartesian[Int, Int, Int])
  checkAll(
    "Cartesian[Either[Int, ?]]",
    SerializableTests.serializable(Cartesian[Either[Int, ?]]))
...

  {
    ...
    checkAll(
      "Either[ListWrapper[String], ListWrapper[Int]]",
