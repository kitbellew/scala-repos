...
class CoproductTests extends CatsSuite {
...

  {
    ...
    checkAll(
      "Foldable[Coproduct[Option, Option, ?]]",
      SerializableTests.serializable(Foldable[Coproduct[Option, Option, ?]]))
  }

  checkAll(
    "Coproduct[Eval, Eval, ?]",
    ComonadTests[Coproduct[Eval, Eval, ?]].comonad[Int, Int, Int])
  checkAll(
    "Comonad[Coproduct[Eval, Eval, ?]]",
...

  {
    ...
    checkAll(
      "CoflatMap[Coproduct[Eval, Eval, ?]]",
      SerializableTests.serializable(CoflatMap[Coproduct[Eval, Eval, ?]]))
  }

  checkAll(
    "Coproduct[Option, Option, Int]",
    OrderLaws[Coproduct[Option, Option, Int]].eqv)
  checkAll(
    "Eq[Coproduct[Option, Option, Int]]",
  ...
  implicit def showEq[A](implicit
    ...
    new Eq[Show[A]] {
      def eqv(f: Show[A], g: Show[A]): Boolean = {
        val samples = List
          .fill(100)(arbA.arbitrary.sample)
          .collect {
            case Some(a) =>
              a
            case None =>
  ...
  checkAll(
    "Contravariant[Coproduct[Show, Show, ?]]",
